...
TL;DR: 该研究通过构建高质量多模态数据集和优化推理语法，仅用监督微调就使开源模型在标准化考试基准上达到接近顶尖商业模型的性能。


<details>
  <summary>Details</summary>
Motivation: 标准化考试题目为多模态推理提供了结构化视觉环境和可验证答案的严格测试平台。尽管近期研究多关注算法改进（如强化学习），但视觉语言推理的数据中心化基础仍较少被探索。研究者希望证明通过高质量数据的监督微调可以匹敌专有方法。

Method: 1. 构建了包含1.614亿token的多模态数据集，整合了教科书问答对、课程对齐图表和上下文材料；2. 使用优化的推理语法（QMSA）对Qwen-2.5VL-32B模型进行监督微调；3. 发布了新的基准测试YKSUniform，包含1,854个多模态考试题目，覆盖309个课程主题。

Result: 微调后的模型在新发布的YKSUniform基准上达到78.6%的准确率，仅比Gemini 2.0 Flash低1.0%。这表明数据组合和表示语法在多模态推理中起决定性作用。

Conclusion: 研究建立了一个数据中心化框架来推进开源视觉语言模型的发展，证明精心策划且基于课程的多模态数据可以将监督微调提升到接近最先进的性能水平。
...
TL;DR: PEFT-DML是一个参数高效的深度度量学习框架，用于自动驾驶中的鲁棒多模态3D目标检测，能够在传感器缺失或新模态组合下保持可靠性能。


<details>
  <summary>Details</summary>
Motivation: 传统多模态检测模型假设传感器固定可用，但实际自动驾驶中传感器可能缺失或出现新的模态组合，需要鲁棒的检测框架。

Method: 将多种模态（LiDAR、雷达、相机、IMU、GNSS）映射到共享潜在空间，集成低秩适应（LoRA）和适配器层实现参数高效训练。

Result: 在nuScenes基准测试中表现出优越的准确性，显著提高训练效率，增强对快速运动、天气变化和域偏移的鲁棒性。

Conclusion: PEFT-DML为自动驾驶中的多模态3D目标检测提供了一个高效且鲁棒的解决方案，特别适用于传感器动态变化的实际场景。
...
TL;DR: 提出Satellite-to-Street框架，通过改进的双输入U-Net架构和类别感知加权损失函数，实现灾后卫星影像的细粒度像素级损伤评估，优于传统分割和变化检测模型。


<details>
  <summary>Details</summary>
Motivation: 传统人工解释卫星影像进行灾后损伤评估存在速度慢、主观性强、难以规模化的问题，而现有的深度学习分割模型（如U-Net）和变化检测模型在处理细微结构变化和严重类别不平衡时表现不佳，特别是对严重受损区域的检测效果差。

Method: 提出Satellite-to-Street: Disaster Impact Estimator框架，采用改进的双输入U-Net架构，联合处理灾前和灾后卫星图像，通过增强的特征融合机制捕捉局部结构变化和更广泛的上下文线索，并集成类别感知加权损失函数来处理真实灾害数据集中未受损像素占主导的问题。

Result: 在公开可用的灾害数据集上的实验表明，相比传统分割模型和基线变化检测模型，该方法在结构损伤的定位和分类方面都有显著改进，能够生成更准确的细粒度像素级损伤地图。

Conclusion: 该方法提供了一种快速、一致的评估机制，支持而非替代专家决策，实现了更高效、数据驱动的灾害管理。损伤地图能够为应急响应优先级确定提供重要支持。
...
TL;DR: 该论文提出ProvRain管道，使用轻量级MobileNet-U-Net架构结合课程训练，提升夜间雨天条件下的车辆检测性能，相比基准方法在准确率和召回率上分别提升8.94%和10.25%。


<details>
  <summary>Details</summary>
Motivation: 夜间车辆检测面临天气条件（如雨雪）和相机条件造成的噪声问题，这些噪声会干扰基于车灯特征的车辆检测准确性，需要设计能够处理噪声同时保持检测精度的解决方案。

Method: 提出ProvRain管道，采用轻量级MobileNet-U-Net架构，通过课程训练概念增强对恶劣天气条件的泛化能力，结合合成数据和PVDN数据集进行训练，并与基于PVDN数据集训练的Faster RCNN基准架构进行对比。

Result: 在雨天夜间帧的车辆检测中，系统准确率提升8.94%，召回率提升10.25%；自定义MobileNet-U-Net架构相比其他Transformer方法，PSNR提升10-15%，SSIM提升5-6%，感知误差(LPIPS)降低高达67%。

Conclusion: ProvRain管道通过集成去噪架构有效提升了恶劣天气条件下的车辆检测性能，证明了课程训练和轻量级架构在处理夜间视觉噪声问题上的有效性。
...
TL;DR: 本文提出了Adapter Shield，第一个针对零样本图像生成场景的通用认证集成防御方案，通过可逆加密系统和多目标对抗扰动保护个人图像免遭未经授权的身份克隆和风格模仿。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型的快速发展，零样本图像到图像生成技术能够仅使用一张肖像或艺术品就能实现高保真度的面部身份或艺术风格复制，这虽然增强了创作可能性，但也带来了严重的知识产权侵权风险，包括未经授权的身份克隆和风格模仿。

Method: 首先研究当前零样本方法如何通过图像编码器提取输入图像的嵌入表示，并通过交叉注意力层输入到扩散模型的UNet中。基于此机制构建可逆加密系统，将原始嵌入映射为不同密钥对应的加密表示。授权用户可通过解密模块和正确密钥恢复真实嵌入。为保护目的，设计多目标对抗扰动方法，主动将原始嵌入向指定加密模式偏移，使受保护图像嵌入防御层，确保未经授权用户只能产生扭曲或加密的输出。

Result: 广泛评估表明，该方法在阻止未经授权的零样本图像合成方面超越了现有最先进的防御方法，同时支持对已验证用户的灵活和安全访问控制。

Conclusion: Adapter Shield为保护个人图像免遭零样本生成场景的滥用提供了有效的解决方案，通过集成认证机制实现了安全与可用性的平衡。
...
TL;DR: 使用无条件扩散模型生成合成明场显微镜图像，通过混合合成与真实数据训练目标检测模型，显著提升细胞检测精度并减少对人工标注的依赖。


<details>
  <summary>Details</summary>
Motivation: 明场显微镜中准确的单细胞检测对生物学研究至关重要，但数据稀缺和标注瓶颈限制了深度学习方法的发展。需要探索合成数据生成技术来缓解这些问题。

Method: 训练基于U-Net的扩散模型生成合成明场显微镜图像，创建不同合成与真实图像比例的数据集，使用YOLOv8、YOLOv9和RT-DETR进行目标检测实验，并通过专家调查评估生成图像的真实性。

Result: 使用合成数据训练可以显著提高检测精度（以最小成本），专家调查显示生成图像具有高度真实感（区分准确率50%），扩散模型生成的合成数据能够有效增强真实数据集。

Conclusion: 基于扩散的合成数据生成是增强显微镜图像分析中真实数据集的有前景途径，减少了对大量人工标注的依赖，并可能提高细胞检测模型的鲁棒性。
...
TL;DR: MARWIN机器人在欧洲XFEL使用现有导航方案（激光雷达边缘检测+里程计+二维码参考）存在灵活性不足问题，本文探索基于深度视觉立体里程计（DVSO）的替代方案，利用立体视差、光流和自监督学习进行纯视觉定位，为受限安全关键基础设施提供更自主的导航能力。


<details>
  <summary>Details</summary>
Motivation: MARWIN机器人在欧洲XFEL加速器隧道中进行自主辐射监测，现有导航方案结合激光雷达边缘检测、轮式/激光雷达里程计和周期性二维码参考，虽然在预定区域表现稳健，但缺乏对未知几何形状和障碍物的灵活性，需要更灵活的导航方案。

Method: 提出采用深度视觉立体里程计（DVSO）作为替代方案，这是一种纯视觉方法，利用立体视差、光流和自监督学习联合估计深度和自我运动，无需标记数据。为保持全局一致性，DVSO可与绝对参考（如地标）或其他传感器融合。

Result: 对加速器隧道环境（以欧洲XFEL为案例）进行了概念性评估，预期优势包括：通过立体视觉减少尺度漂移、低成本传感、可扩展的数据收集。但仍面临低纹理表面、光照变化、计算负载和辐射环境下鲁棒性等挑战。

Conclusion: 本文为MARWIN机器人在受限、安全关键基础设施中实现更自主导航定义了研究议程，DVSO方案有望提供更灵活的导航能力，但需要解决实际部署中的技术挑战。
...
TL;DR: 诊断提示方法显著提升了多模态大语言模型在评估亚马逊搜索结果页面视觉复杂度时的可靠性，但绝对性能仍有待提高。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索诊断提示是否能改善多模态大语言模型在评估亚马逊搜索结果页面视觉复杂度时的可靠性，以更接近人类专家的判断标准。

Method: 使用200个亚马逊搜索结果页面和人类专家标注，比较诊断提示与基于格式塔原理的标准提示方法，通过决策树分析模型决策过程。

Result: 诊断提示显著提升了预测人类复杂度判断的性能，F1分数从0.031提升到0.297（相对提升858%），但绝对性能仍有限（Cohen's κ = 0.071）。模型更关注视觉设计元素（徽章杂乱度占38.6%重要性），而人类更强调内容相似性。

Conclusion: 诊断提示是迈向人类对齐的多模态大语言模型评估的有希望的第一步，但在产品相似性和颜色强度评估等方面仍存在挑战，需要更大规模的真实数据集和提示方法改进才能实现可靠的实践部署。
...
TL;DR: FastTextDiff是一种基于扩散模型的标签高效医学图像分割方法，通过整合医学文本标注来增强语义表示，使用ModernBERT替代Clinical BioBERT处理临床笔记，提高了分割准确性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割通常需要密集的像素级标注，这些标注成本高、耗时且需要专业知识。现有的扩散模型在医学图像生成和去噪方面表现良好，但分割性能受限于标注数据的稀缺性。

Method: 提出FastTextDiff模型，整合医学文本标注来增强语义表示。使用ModernBERT处理长临床笔记，编码临床知识并指导视觉和文本特征之间的跨模态注意力。利用FlashAttention 2、交替注意力机制和2万亿token语料库进行训练。

Result: 在MIMIC-III和MIMIC-IV数据集上验证，ModernBERT作为Clinical BioBERT的快速、可扩展替代方案，在扩散分割流程中表现良好。FastTextDiff相比传统扩散模型提高了分割准确性和训练效率。

Conclusion: 该研究验证了ModernBERT在基于扩散的分割流程中的有效性，并突出了多模态技术在医学图像分析中的潜力。通过整合文本标注，实现了标签高效的分割，为医学图像分析提供了新思路。
...
TL;DR: 提出了一种用于超低功耗物联网平台的多模态设备端学习技术，通过稀疏更新方案在MCU上实现单目深度估计模型的在线微调，显著降低内存需求并提升部署环境下的精度。


<details>
  <summary>Details</summary>
Motivation: 物联网平台上的单目深度估计模型参数有限，当传感器数据与训练数据集存在显著域偏移时会导致精度严重下降。需要解决域偏移问题，同时保持超低功耗特性。

Method: 1. 在GAP9 MCU上部署μPyD-Net模型（107k参数）进行单目图像推理；2. 平时关闭深度传感器节能，在新环境中激活深度传感器收集伪标签；3. 提出内存驱动的稀疏更新方案，将微调内存降至1.2MB；4. 完全在MCU上进行设备端训练。

Result: 1. 稀疏更新方案比完整更新减少2.2倍内存，精度仅下降2%（KITTI）和1.5%（NYUv2）；2. 现场测试显示设备端学习仅需17.8分钟，使用3k自标记样本将均方根误差从4.9m降至0.6m；3. 系统总功耗约300mW。

Conclusion: 首次在物联网节点上实现了单目深度估计的设备端学习，通过多模态稀疏更新方案有效解决了域偏移问题，在保持超低功耗的同时显著提升了模型在新环境中的精度。
...
TL;DR: AI驱动的课堂互动分析：通过视频和文本多模态方法自动识别教学活动与话语模式，为教师提供可扩展的反馈系统


<details>
  <summary>Details</summary>
Motivation: 传统课堂观察依赖人工标注，资源密集且难以规模化。需要开发AI驱动的自动化分析系统来提供可扩展的教师反馈

Method: 使用164小时视频和68节课转录的密集标注数据集，设计并行模态特定管道：视频方面评估零样本多模态LLM、微调视觉语言模型和自监督视频变换器（24个活动标签）；文本方面微调基于变换器的分类器与上下文输入，并与基于提示的LLM比较（19个话语标签）。采用每标签阈值、上下文窗口和不平衡感知损失函数处理类别不平衡和多标签复杂性

Result: 微调模型始终优于基于提示的方法，视频分析达到0.577的宏观F1分数，转录本分析达到0.460的宏观F1分数
...
TL;DR: TeleViT模型通过融合局部火灾驱动因素、全球场和遥相关指数，实现了从零到四个月提前期的野火预测性能提升，在季节性一致的火灾区域表现最佳。
...
Method: 提出TeleViT模型，采用非对称标记化策略融合(i)细尺度局部火灾驱动因素、(ii)粗化全球场和(iii)遥相关指数，通过Transformer编码器处理异质标记，解码器保持空间结构。

Result: 使用SeasFire数据集(2001-2021)，TeleViT在所有提前期（包括四个月）的AUPRC性能均优于U-Net++、ViT和气候学基准。在零提前期达到0.630，四个月提前期保持0.601-0.603，在所有提前期均超越气候学基准(0.572)。

Conclusion: 显式编码大尺度地球系统上下文的架构可以扩展野火在次季节到季节时间尺度上的可预测性，预测主要依赖局部标记，全球场和指数提供粗粒度上下文信息。
...
<details>
  <summary>Details</summary>
Motivation: 建筑、工程和施工行业需要可持续有效的建造方法，3D混凝土打印中的丝状体几何质量控制对打印结构质量至关重要，但目前缺乏独立于传感器的自动化质量控制方法。

Method: 开发了一种独立于传感器的自动化质量控制工作流程，适用于挤出式（CC）和喷射式（SC3DP）3D打印方法，可使用相机、结构光系统或地面激光扫描仪等多种传感器采集数据，适用于新鲜或固化状态的材料。

Result: 提出了一种通用的自动化质量控制程序，能够对3D混凝土打印中的丝状体几何质量进行有效监控，支持在线实时监控和打印后评估，提高了质量控制效率和准确性。

Conclusion: 该方法为3D混凝土打印提供了灵活、通用的质量控制解决方案，能够适应不同的打印技术、传感器和材料状态，有助于提高建筑3D打印的质量控制水平。
...
<details>
  <summary>Details</summary>
Motivation: 研究不同图像处理方法在基于活动记录图像进行心理健康状况识别中的性能差异，探索最适合心理健康研究的图像分析方法。

Method: 使用Psykose和Depresjon数据集的手腕活动信号，转换为30×48像素图像，采用三折受试者交叉验证，比较VGG16、ViT-B/16和CoAtNet-Tiny三种模型的性能。

Result: CoAtNet-Tiny表现最稳定，平均准确率最高，在抑郁和精神分裂症等少数类别的精确率、召回率和F1分数也最好；VGG16训练稳定但最终准确率较低；ViT-B/16在某些运行中表现好但稳定性差。

Conclusion: CoAtNet-Tiny在活动记录图像分析中表现最一致，某些混合设计架构可能特别适合基于活动记录图像的心理健康研究工作。
...
TL;DR: 该研究提出了一种仅使用可见光图像进行光伏板表面故障检测和严重程度评估的深度学习方法，无需依赖昂贵的多模态传感器。


<details>
  <summary>Details</summary>
Motivation: 太阳能光伏资产的高效运维需要准确检测和优先处理大面积分布式模块的表面故障。虽然多模态成像策略很流行，但它们在常规农场级部署中引入了物流和经济障碍。

Method: 提出TinyViT紧凑管道，集成基于Transformer的分割、光谱空间特征工程和集成回归。系统仅使用消费级彩色相机拍摄的光伏板图像，对七种细微表面故障进行分类，并生成可操作的严重程度等级。

Result: 在真实公共数据集上的实验验证了分类和回归子模块，实现了与专门方法相竞争的准确性和可解释性。

Conclusion: 通过消除对电致发光或红外传感器的依赖，该方法为资源有限的安装提供了经济实惠、可扩展的维护方案，推动了太阳能健康监测向普遍现场可访问性发展。
...
TL;DR: 提出了一种混合合成数据生成框架，通过模拟渲染、领域随机化和真实背景合成，实现工业零件质量检测的零样本学习，无需人工标注，在严重类别不平衡下达到90%以上的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 工业质量检测中，深度学习需要大量高质量标注数据，但缺陷样本稀少导致类别不平衡，数据获取成本高且耗时，阻碍了机器学习在实际生产环境中的广泛应用。

Method: 提出混合合成数据生成框架，结合模拟渲染、领域随机化和真实背景合成，在1小时内生成12,960张标注图像。采用两阶段架构：YOLOv8n用于目标检测，MobileNetV3-small用于质量分类，完全使用合成数据训练。

Result: 在300个真实工业零件上评估，检测mAP@0.5达到0.995，分类准确率96%，平衡准确率90.1%。在严重类别不平衡下达到90-91%平衡准确率，而少样本真实数据基线仅50%准确率。
...
### [17] [Mammo-FM: Breast-specific foundational model for Integrated Mammographic Diagnosis, Prognosis, and Reporting](https://arxiv.org/abs/2512.00198)
...
TL;DR: Mammo-FM是首个专门用于乳腺X线摄影的基础模型，在140,677名患者（821,326张乳腺X线片）的数据集上预训练，统一支持乳腺癌诊断、病理定位、结构化报告生成和风险预后等核心临床任务。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性主要死因之一，需要专门针对乳腺X线摄影的领域特定基础模型来统一支持多种临床任务，提高透明度和临床可审计性。

Method: 开发了Mammo-FM基础模型，在四个美国机构的140,677名患者（821,326张乳腺X线片）数据集上进行预训练，实现图像与文本的对齐，支持视觉和文本可解释性。

Result: 尽管使用原生分辨率乳腺X线片且参数仅为最先进通用基础模型的三分之一，Mammo-FM在多个公共和私有基准测试中持续优于通用模型，在诊断、预后和报告生成任务上表现出色。

Conclusion: 研究强调了围绕临床领域全任务谱设计的领域特定基础模型的价值和效率，以及严格领域对齐评估的重要性，为乳腺成像提供了统一的基础框架。
...
### [18] [ReactionMamba: Generating Short &Long Human Reaction Sequences](https://arxiv.org/abs/2512.00208)
*Hajra Anwar Beg,Baptiste Chopin,Hao Tang,Mohamed Daoudi*

Main category: cs.CV

TL;DR: ReactionMamba是一个用于生成长序列3D人体反应动作的新框架，结合了运动VAE和Mamba状态空间模型，在保持动作真实性和多样性的同时显著提升了推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成长序列3D人体反应动作时面临挑战，特别是在保持时间一致性和处理复杂动作（如舞蹈和武术）方面存在局限性，需要更高效的框架来生成既真实又多样的长序列动作。

Method: ReactionMamba框架整合了运动变分自编码器（VAE）用于高效动作编码，以及基于Mamba的状态空间模型用于解码时间一致的反应动作。这种设计使其能够处理从简单短序列到复杂长序列的各种动作。

Result: 在NTU120-AS、Lindy Hop和InterX三个数据集上的评估表明，ReactionMamba在动作真实性、多样性和长序列生成方面与InterFormer、ReMoS、Ready-to-React等方法相比具有竞争力，同时在推理速度上实现了显著提升。

Conclusion: ReactionMamba是一个有效的框架，能够生成高质量的长序列3D人体反应动作，在保持动作质量的同时大幅提升了计算效率，为复杂动作生成任务提供了实用解决方案。
...
TL;DR: DenseScan是一个新的3D场景理解数据集，通过自动化流程生成多级描述，结合几何细节和语义丰富性，提升3D环境中的物体级理解和问答性能。
...
Method: 采用自动化流程，利用多视角2D图像和多模态大语言模型生成密集的场景元素描述。通过场景化问题生成扩展标注，整合物体属性、空间关系和场景上下文。

Result: 实验结果表明，与传统标注流程相比，该方法显著提升了3D环境中的物体级理解和问答性能。数据集和标注流程已公开发布。

Conclusion: DenseScan通过结合几何细节和语义丰富性，拓宽了下游任务范围，旨在推动机器人、增强现实等领域的3D场景理解研究。
...
### [20] [UniDiff: Parameter-Efficient Adaptation of Diffusion Models for Land Cover Classification with Multi-Modal Remotely Sensed Imagery and Sparse Annotations](https://arxiv.org/abs/2512.00261)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏标注限制了多模态遥感的发展，即使是先进的监督方法如MSFMamba也受限于标注数据的可用性。ImageNet预训练模型虽然提供丰富的视觉表示，但难以适配到HSI和SAR等异质模态，特别是在缺乏大规模标注数据集的情况下。

Method: 提出UniDiff框架：1) 使用FiLM-based timestep-modality conditioning进行时间步-模态条件化；2) 参数高效适配，仅调整约5%的参数；3) 采用pseudo-RGB anchoring来保持预训练表示并防止灾难性遗忘。该设计能够在稀疏标注下从遥感数据中有效提取特征。

Result: 在两个已建立的多模态基准数据集上的结果表明，预训练扩散模型的无监督适配有效缓解了标注约束，并实现了多模态遥感数据的有效融合。

Conclusion: UniDiff框架通过参数高效的方式成功将ImageNet预训练的扩散模型适配到多模态遥感数据，为解决稀疏标注问题提供了有效方案，实现了多模态遥感数据的有效融合。
...
### [21] [USB: Unified Synthetic Brain Framework for Bidirectional Pathology-Healthy Generation and Editing](https://arxiv.org/abs/2512.00269)
*Jun Wang,Peirong Liu*

Main category: cs.CV

TL;DR: USB是首个统一病理和健康脑图像双向生成与编辑的端到端框架，通过配对扩散机制建模病变与脑解剖结构的联合分布，实现病理和健康图像的生成。
...
Conclusion: USB为可扩展数据集创建和稳健神经影像分析开辟了机会，是首个统一病理和健康脑图像双向生成与编辑的端到端框架。
...
### [22] [HIMOSA: Efficient Remote Sensing Image Super-Resolution with Hierarchical Mixture of Sparse Attention](https://arxiv.org/abs/2512.00275)
*Yi Liu,Yi Wan,Xinyi Liu,Qiong Wu,Panwang Xia,Xuejun Huang,Yongjun Zhang*

Main category: cs.CV

TL;DR: HIMOSA是一个用于遥感图像超分辨率的轻量级框架，通过内容感知稀疏注意力机制和分层窗口扩展，在保持高性能的同时实现快速推理。


<details>
  <summary>Details</summary>
Motivation: 在灾害检测和响应等遥感应用中，实时效率和模型轻量化至关重要。现有遥感图像超分辨率方法往往需要在模型性能和计算效率之间进行权衡，因此需要一种既能保持高性能又具有计算效率的解决方案。

Method: HIMOSA框架利用遥感图像固有的冗余性，引入内容感知稀疏注意力机制，使模型能够在保持强大重建性能的同时实现快速推理。此外，通过分层窗口扩展有效利用遥感图像中的多尺度重复模式，并通过调整注意力的稀疏性来降低计算复杂度。
...
### [23] [Rethinking Lung Cancer Screening: AI Nodule Detection and Diagnosis Outperforms Radiologists, Leading Models, and Standards Beyond Size and Growth](https://arxiv.org/abs/2512.00281)
...
TL;DR: 开发了一个AI系统，能够在低剂量CT扫描中直接进行肺结节检测和恶性诊断，在内部和外部验证中均优于放射科医生和现有AI模型。


<details>
  <summary>Details</summary>
Motivation: 目前肺癌筛查依赖于结节大小和生长，这本质上延迟了诊断。需要开发一个能够在结节级别直接进行检测和恶性诊断的AI系统，以克服数据集规模和可解释性的限制。

Method: 设计了一个集成系统，结合浅层深度学习和基于特征的专业模型。使用25,709次扫描和69,449个标注结节进行训练和评估。

Result: 系统在内部验证中AUC达到0.98，独立队列中达到0.945。每扫描0.5个假阳性下达到99.3%灵敏度。在所有结节大小和分期上都优于放射科医生，特别是在1期癌症和所有基于生长的指标上表现突出，包括最不准确的体积倍增时间。

Conclusion: 该AI系统重新定义了肺癌筛查，能够更早地检测恶性肺结节，在诊断不确定性和缓慢生长结节方面比放射科医生提前最多一年，解决了AI采用的关键障碍。
...
### [24] [TGSFormer: Scalable Temporal Gaussian Splatting for Embodied Semantic Scene Completion](https://arxiv.org/abs/2512.00300)
*Rui Qian,Haozhi Cao,Tianchen Deng,Tianxin Hu,Weixiang Guo,Shenghai Yuan,Lihua Xie*

Main category: cs.CV

TL;DR: TGSFormer：一个可扩展的时序高斯泼溅框架，用于具身3D语义场景补全，通过持久高斯内存和置信感知融合机制，在减少基元数量的同时实现更好的准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法存在冗余和可扩展性差的问题：随机初始化大量基元导致冗余，深度引导方法仍局限于局部场景，随着场景规模增大会产生延迟和内存开销。

Method: 提出TGSFormer框架：1）维护持久高斯内存进行时序预测；2）双时序编码器通过置信感知交叉注意力处理当前和历史高斯特征；3）置信感知体素融合模块将重叠基元合并为体素对齐表示，调节密度并保持紧凑性。

Result: 在局部和具身SSC基准测试中达到最先进水平，以显著更少的基元数量实现更高的准确性和可扩展性，同时保持一致的长期场景完整性。

Conclusion: TGSFormer通过持久高斯内存和置信感知融合机制，有效解决了现有高斯方法在冗余和可扩展性方面的限制，为具身3D语义场景补全提供了更高效和可扩展的解决方案。
...
### [25] [Optimizing Distributional Geometry Alignment with Optimal Transport for Generative Dataset Distillation](https://arxiv.org/abs/2512.00308)
*Xiao Cui,Yulei Qin,Wengang Zhou,Hongsheng Li,Houqiang Li*

Main category: cs.CV

TL;DR: 本文提出了一种基于最优传输（OT）的数据集蒸馏新方法，通过全局和实例级别的细粒度对齐，显著提升了大规模数据集上的蒸馏性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模数据集蒸馏方法主要关注匹配全局分布统计量（如均值和方差），但忽略了关键的实例级特征和类内变化，导致泛化性能不理想。

Method: 将数据集蒸馏重新表述为最优传输（OT）距离最小化问题，包含三个组件：1）OT引导的扩散采样，对齐真实和蒸馏图像的潜在分布；2）标签-图像对齐的软重标记，根据蒸馏图像分布的复杂性调整标签分布；3）基于OT的logit匹配，对齐学生模型输出与软标签分布。

Result: 在多种架构和大规模数据集上的广泛实验表明，该方法在高效性方面始终优于最先进方法，在IPC=10设置下，每个架构在ImageNet-1K上至少实现了4%的准确率提升。

Conclusion: 通过最优传输框架实现全局和实例级别的细粒度对齐，能够有效保留复杂高维分布的几何特性，显著提升数据集蒸馏的性能和泛化能力。
...
### [26] [Odometry Without Correspondence from Inertially Constrained Ruled Surfaces](https://arxiv.org/abs/2512.00327)
*Chenqi Zhu,Levi Burner,Yiannis Aloimonos*

Main category: cs.CV

TL;DR: 提出一种基于直线特征和惯性测量的新型视觉里程计方法，通过分析图像空间中直线扫过的直纹面来估计运动，避免传统点特征匹配的高计算成本和精度问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉里程计依赖图像序列中的特征提取和光流计算，这种点对点对应关系计算成本高且精度不稳定，影响里程计估计质量。虽然已有研究尝试使用直线特征或融合其他传感器来改进，但仍严重依赖对应关系。

Method: 提出一种新颖算法，利用相机运动时直线在图像空间扫过的直纹面来重建3D场景和估计视觉里程计。该方法仅需要从点到线的关联进行微分计算更新，并通过机载IMU传感器的惯性测量约束这些表面，大幅降低解空间的维度。

Result: 该方法通过分析直纹面的形状获取里程计信息，利用事件相机擅长边缘检测的特性，结合IMU约束，提供了一种更高效、更稳定的视觉里程计解决方案。

Conclusion: 基于直线扫过的直纹面分析，结合惯性测量约束，可以绕过传统对应关系问题，为视觉里程计提供一种计算效率更高、鲁棒性更好的新方法。
...
### [27] [MVAD : A Comprehensive Multimodal Video-Audio Dataset for AIGC Detection](https://arxiv.org/abs/2512.00336)
...
<details>
  <summary>Details</summary>
Motivation: AI生成的多模态视频-音频内容快速发展引发了信息安全和内容真实性的重大担忧。现有合成视频数据集主要关注视觉模态，少数包含音频的数据集也大多局限于面部深度伪造，这无法应对日益扩展的通用多模态AI生成内容，严重阻碍了可信检测系统的发展。

Method: 创建了MVAD数据集，具有三个关键特征：1）真实多模态性，包含三种现实视频-音频伪造模式生成的样本；2）高感知质量，通过多样化的最先进生成模型实现；3）全面多样性，涵盖现实和动漫视觉风格、四种内容类别（人类、动物、物体、场景）和四种视频-音频多模态数据类型。

Result: MVAD是首个专门针对AI生成多模态视频-音频内容检测的综合数据集，填补了该领域的关键空白，将促进可信检测系统的开发。

Conclusion: MVAD数据集的引入为检测AI生成多模态视频-音频内容提供了重要资源，将推动该领域研究的发展，并有助于应对日益增长的信息安全和内容真实性挑战。
...
### [28] [Assimilation Matters: Model-level Backdoor Detection in Vision-Language Pretrained Models](https://arxiv.org/abs/2512.00343)
*Zhongqi Wang,Jie Zhang,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: AMDET是一种无需先验知识的模型级后门检测框架，通过揭示后门文本编码器的特征同化特性，利用梯度反演恢复能激活后门行为的隐式特征，有效检测视觉语言预训练模型中的后门攻击。


<details>
  <summary>Details</summary>
Motivation: 现有后门检测方法通常依赖训练数据集、后门触发器、攻击目标或下游分类器的先验知识，这在现实应用中往往不切实际。需要一种无需这些先验知识的检测方法来解决CLIP等视觉语言预训练模型的后门安全问题。

Method: AMDET基于后门文本编码器的特征同化特性：后门样本中所有token的表示具有高度相似性，这源于注意力权重在触发器token上的集中。通过梯度反演token嵌入来恢复能激活后门行为的隐式特征，并通过分析损失景观过滤掉OpenAI官方CLIP模型中存在的自然后门特征。

Result: 在3,600个后门和良性微调模型上的实验表明，AMDET在两种攻击范式和三种VLP模型结构下实现了89.90%的F1分数。在RTX 4090 GPU上约5分钟完成一次完整检测，并对自适应攻击表现出强鲁棒性。

Conclusion: AMDET提出了一种无需先验知识的模型级后门检测框架，通过特征同化特性和梯度反演技术有效检测视觉语言预训练模型中的后门攻击，为实际应用中的模型安全提供了实用解决方案。
...
### [29] [mmPred: Radar-based Human Motion Prediction in the Dark](https://arxiv.org/abs/2512.00345)
*Junqiao Fan,Haocong Rao,Jiarui Zhang,Jianfei Yang,Lihua Xie*

Main category: cs.CV

TL;DR: 首个基于毫米波雷达的人体运动预测扩散模型，通过双域历史运动表示和全局骨骼关系Transformer解决雷达信号噪声问题


<details>
  <summary>Details</summary>
Motivation: 现有基于RGB-D相机的人体运动预测方法对光照条件敏感且存在隐私问题，限制了在消防、医疗等实际应用。毫米波雷达具有鲁棒性和隐私保护特性，但雷达信号存在镜面反射和多径效应等噪声问题。

Method: 提出mmPred扩散框架，包含双域历史运动表示：时域姿态细化分支学习细粒度细节，频域主导运动分支捕捉全局运动趋势并抑制帧级不一致。设计全局骨骼关系Transformer作为扩散主干，建模关节间全局协作。

Result: 在mmBody和mm-Fi数据集上实现最先进性能，分别超越现有方法8.6%和22%。

Conclusion: 首次将毫米波雷达引入人体运动预测，提出的mmPred框架能有效处理雷达特定伪影，为隐私敏感和恶劣环境下的应用提供了新解决方案。
...
### [30] [THCRL: Trusted Hierarchical Contrastive Representation Learning for Multi-View Clustering](https://arxiv.org/abs/2512.00368)
*Jian Zhu*
...
<details>
  <summary>Details</summary>
Motivation: 多视图聚类面临不可信融合的挑战，主要源于两个因素：1)现有方法忽略单个视图中的固有噪声；2)传统对比学习方法仅关注同一实例的不同视图，而忽略同一簇内最近邻的结构信息，导致多视图融合方向错误。

Method: 提出THCRL框架，包含两个关键模块：1)深度对称层次融合模块(DSHF)，利用UNet架构结合多种去噪机制实现多视图数据的可信融合；2)平均K近邻对比学习模块(AKCL)，将融合表示与视图特定表示对齐，增强同一簇内样本的表示相似性而非仅关注跨视图的同一样本。

Result: 大量实验证明THCRL在深度多视图聚类任务中实现了最先进的性能。
...
### [31] [POLARIS: Projection-Orthogonal Least Squares for Robust and Adaptive Inversion in Diffusion Models](https://arxiv.org/abs/2512.00369)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的反演-去噪范式在图像编辑和恢复任务中表现出色，但存在重建质量下降的问题。研究发现这是由于近似噪声误差导致的，即用t-1步的预测来近似t步的噪声，导致误差在整个反演过程中严重累积

Method: 提出POLARIS方法，将反演问题从误差补偿问题重新表述为误差起源问题。不通过优化嵌入或潜在代码来抵消累积漂移，而是将引导尺度ω作为步进变量，推导出数学上严谨的公式来最小化每一步的反演误差

Result: POLARIS仅需一行代码即可显著改善反演潜在质量，性能开销可忽略不计。该方法能大幅减轻噪声近似误差，并持续提高下游任务的准确性

Conclusion: POLARIS通过数学上严谨的方法解决了扩散模型反演中的关键误差问题，提供了一种高效、低开销的解决方案，显著提升了反演质量和下游任务性能
...
### [32] [Pore-scale Image Patch Dataset and A Comparative Evaluation of Pore-scale Facial Features](https://arxiv.org/abs/2512.00381)
*Dong Li,HuaLiang Lin,JiaYu Li*

Main category: cs.CV

TL;DR: 本文提出了PorePatch数据集和DMCE框架来解决面部弱纹理区域匹配问题，深度学习描述符在匹配任务上表现优异但在3D重建中优势有限。


<details>
  <summary>Details</summary>
Motivation: 面部皮肤区域的弱纹理特性给局部描述符匹配带来了重大挑战，而毛孔尺度图像补丁数据集的缺乏阻碍了深度学习描述符在面部领域的发展。

Method: 提出了PorePatch高质量毛孔尺度图像补丁数据集，建立了合理的评估基准，并引入了数据-模型协同进化（DMCE）框架从高分辨率面部图像生成渐进细化的高质量数据集。

Result: SOTA模型在匹配任务上达到1.91%的FPR95值，比PSIFT（22.41%）提升了20.5%，但在3D重建任务中优势不明显，整体性能与传统描述符相比没有显著提升。

Conclusion: 深度学习描述符在解决面部弱纹理区域挑战方面仍有局限性，该领域还需要大量研究工作。
...
### [33] [WiseEdit: Benchmarking Cognition- and Creativity-Informed Image Editing](https://arxiv.org/abs/2512.00387)
*Kaihang Pan,Weile Chen,Haiyi Qiu,Qifan Yu,Wendong Bu,Zehan Wang,Yun Zhu,Juncheng Li,Siliang Tang*

Main category: cs.CV

TL;DR: WiseEdit是一个知识密集型图像编辑基准测试，用于全面评估认知和创造力导向的图像编辑能力，包含1,220个测试案例，揭示了当前最先进模型在知识推理和创意合成方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑基准测试评估范围过于狭窄，无法全面评估现代图像编辑模型的高级认知和创造力能力，需要一个新的综合性基准来填补这一空白。

Method: 基于人类认知创作过程，将图像编辑分解为三个级联步骤：感知、解释和想象，每个步骤对应特定任务挑战。同时包含三种基本知识类型：陈述性知识、程序性知识和元认知知识。

Result: 构建了包含1,220个测试案例的基准测试，客观揭示了当前最先进图像编辑模型在基于知识的认知推理和创意合成能力方面的局限性。

Conclusion: WiseEdit为全面评估认知和创造力导向的图像编辑能力提供了首个知识密集型基准测试，有助于推动该领域的发展，相关资源将公开提供。
...
### [34] [Better, Stronger, Faster: Tackling the Trilemma in MLLM-based Segmentation with Simultaneous Textual Mask Prediction](https://arxiv.org/abs/2512.00395)
*Jiazhen Liu,Mingkuan Feng,Long Chen*

Main category: cs.CV

TL;DR: STAMP提出了一种新的多模态大语言模型范式，通过"全掩码预测"同时解决了对话能力、分割性能和推理速度的三难困境。
...
Result: STAMP在多个分割基准测试中显著优于最先进方法，同时保持了对话能力、实现了高分割性能，并获得了优异的推理速度。

Conclusion: 全掩码预测范式成功解决了多模态大语言模型在集成分割任务时的三难困境，提供了一种无需妥协的解决方案，在对话、分割和速度三个方面都表现出色。
...
### [35] [Low-Bitrate Video Compression through Semantic-Conditioned Diffusion](https://arxiv.org/abs/2512.00408)
*Lingdong Wang,Guan-Ming Su,Divya Kothandaraman,Tsung-Wei Huang,Mohammad Hajiesmaili,Ramesh K. Sitaraman*

Main category: cs.CV

TL;DR: DiSCo是一个语义视频压缩框架，通过传输文本描述、降质视频和可选草图/姿态等紧凑模态，结合条件视频扩散模型重建高质量视频，在低比特率下比传统编解码器性能提升2-10倍。
...
Method: 提出DiSCo语义视频压缩框架：1）将源视频分解为三个紧凑模态：文本描述（语义）、时空降质视频（外观）和可选草图/姿态（运动）；2）使用条件视频扩散模型从这些紧凑表示中重建高质量、时间连贯的视频；3）提出时间前向填充、令牌交错和模态特定编解码器来改进多模态生成和模态紧凑性。

Result: 实验表明，该方法在低比特率下比基线语义和传统编解码器在感知指标上性能提升2-10倍。

Conclusion: DiSCo通过传输语义信息而非像素精度，结合生成模型进行细节合成，有效解决了传统编解码器在超低比特率下的性能崩溃问题，实现了感知质量的大幅提升。
...
### [36] [SplatFont3D: Structure-Aware Text-to-3D Artistic Font Generation with Part-Level Style Control](https://arxiv.org/abs/2512.00413)
*Ji Gan,Lingxu Chen,Jiaxu Leng,Xinbo Gao*

Main category: cs.CV

TL;DR: SplatFont3D是一个基于3D高斯泼溅的结构感知文本到3D艺术字体生成框架，支持细粒度部件级风格控制，在风格一致性、视觉质量和渲染效率方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前艺术字体生成研究主要集中在2D平面设计，个性化3D艺术字体生成尚未充分探索。3D字体不仅能在游戏、动画等沉浸式3D环境中应用，还能通过渲染新视角增强2D字体生成。3D字体具有精确语义和强结构约束，需要细粒度部件级风格控制。

Method: 提出SplatFont3D框架：1) Glyph2Cloud模块逐步增强2D字形（或部件）的形状和风格，生成对应3D点云用于高斯初始化；2) 使用预训练2D扩散模型通过分数蒸馏采样优化3D高斯；3) 动态部件分配策略利用3D高斯的几何先验进行部件分割，缓解优化过程中的漂移纠缠问题。

Result: SplatFont3D在风格文本一致性、视觉质量和渲染效率方面优于现有3D模型，比NeRF提供更明确有效的部件级风格控制，渲染效率更高。

Conclusion: SplatFont3D是一个有效的3D艺术字体生成框架，通过3D高斯泼溅技术实现了细粒度部件级风格控制，在多个指标上优于现有方法，为3D字体生成提供了新解决方案。
...
### [37] [PhysGen: Physically Grounded 3D Shape Generation for Industrial Design](https://arxiv.org/abs/2512.00422)
*Yingxuan You,Chen Zhao,Hantao Zhang,Mingda Xu,Pascal Fua*

Main category: cs.CV

TL;DR: 提出了一种基于物理的3D形状生成统一流水线，通过交替的速度更新和物理细化过程，结合物理感知正则化，在工业设计应用中生成既视觉逼真又符合物理特性的形状。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状生成模型虽然能合成视觉逼真的形状，但对于经过工程设计过程的形状类别（如汽车），其真实性与底层物理属性（如空气动力学效率）紧密相关。现有方法缺乏物理知识，无法利用这些知识增强形状生成的真实性。

Method: 提出统一的基于物理的3D形状生成流水线：1）引入具有显式物理指导的流匹配模型，采用交替更新过程（速度更新+物理细化）；2）在速度更新步骤中加入物理感知正则化项；3）构建形状-物理变分自编码器（SP-VAE），将形状和物理信息共同编码到统一的潜在空间。

Result: 在三个基准测试上的实验表明，这种协同公式超越了单纯的视觉逼真性，提高了形状的真实性。

Conclusion: 提出的基于物理的3D形状生成方法通过结合物理指导和交替优化过程，能够生成既视觉逼真又符合物理特性的工业设计形状，提高了生成形状的真实性。
...
### [38] [Recovering Origin Destination Flows from Bus CCTV: Early Results from Nairobi and Kigali](https://arxiv.org/abs/2512.00424)
*Nthenya Kyatha,Jay Taneja*

Main category: cs.CV

TL;DR: 利用车载CCTV监控，开发了一个结合目标检测、跟踪、重识别、OCR和时间戳的基线系统，用于恢复非洲公交车的起讫点客流数据。系统在低密度、光照良好条件下表现优异，但在拥挤、单色转换等实际压力下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲的公共交通经常在过度拥挤的条件下运营，现有自动化系统无法可靠地捕捉客流数据。利用已部署用于安全监控的车载CCTV，需要开发适合当地条件的客流分析系统。

Method: 提出一个基线管道，结合YOLOv12目标检测、BotSORT跟踪、OSNet重识别嵌入、基于OCR的时间戳和基于遥测的站点分类，从车载CCTV视频中恢复公交车的起讫点客流数据。

Result: 在内罗毕和基加利公交车的标注CCTV片段上，系统在低密度、光照良好条件下达到高计数准确率（召回率≈95%，精确率≈91%，F1≈93%），生成的OD矩阵与人工计数高度匹配。但在拥挤、单色转换、姿态变化和非标准车门使用等实际压力下，性能急剧下降（如高峰时段上车人数低估约40%，单色片段召回率下降约17个百分点）。

Conclusion: 系统揭示了部署特定的故障模式，表明需要为SSA公共交通开发更鲁棒、更注重实际部署的重识别方法，以应对当地特有的挑战如过度拥挤和图像质量变化。
...
### [39] [Recognizing Pneumonia in Real-World Chest X-rays with a Classifier Trained with Images Synthetically Generated by Nano Banana](https://arxiv.org/abs/2512.00428)
*Jiachuan Peng,Kyle Lam,Jianing Qiu*

Main category: cs.CV

TL;DR: 使用Nano Banana生成的合成胸片训练肺炎分类器，在真实数据上验证获得良好性能，证明合成数据在医学AI开发中的潜力
...
Method: 使用Google的Nano Banana AI模型生成合成胸片图像，仅用这些合成数据训练肺炎分类器，然后在两个真实胸片数据集上进行外部验证

Result: 在RSNA肺炎检测数据集上AUROC为0.923，AUPR为0.900；在胸片数据集上AUROC为0.824，AUPR为0.913，表明仅用合成数据训练的分类器在真实数据上表现良好

Conclusion: 合成数据在医学AI开发中具有潜力，但仍存在提示设计多样性控制、后处理对齐等限制，临床转化需要充分验证、监管批准和伦理监督
...
### [40] [FR-TTS: Test-Time Scaling for NTP-based Image Generation with Effective Filling-based Reward Signal](https://arxiv.org/abs/2512.00438)
*Hang Xu,Linjiang Huang,Feng Zhao*

Main category: cs.CV

TL;DR: FR-TTS：一种针对文本生成任务的新型测试时缩放方法，通过填充式奖励机制解决中间序列与最终生成质量相关性低的问题，显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放（TTS）在图像生成中通过并行采样和奖励模型过滤显著提升了输出质量，但在文本生成（下一个token预测范式）中应用面临挑战。主要障碍是：从中间token序列解码的图像奖励与最终生成图像的奖励相关性低，导致中间表示无法有效指导剪枝方向。

Method: 提出填充式奖励（FR）机制：通过寻找并应用合理的填充方案来补全中间序列，从而估计中间样本的近似未来轨迹。在此基础上提出FR-TTS策略：高效搜索优质填充方案，并引入动态加权的多样性奖励，实现对中间样本的平衡全面评估。

Result: 实验验证显示：中间样本与最终样本奖励之间的相关系数，以及token置信度等多个内在信号表明，FR提供了优秀可靠的中间样本质量评估指标。FR-TTS在多个基准测试和不同奖励模型上都表现出优越性。

Conclusion: FR-TTS成功将测试时缩放技术扩展到文本生成领域，通过创新的填充式奖励机制解决了中间序列评估难题，为提升文本生成质量提供了有效解决方案。
...
### [41] [CausalAffect: Causal Discovery for Facial Affective Understanding](https://arxiv.org/abs/2512.00456)
...
<details>
  <summary>Details</summary>
Motivation: 现有面部情感分析方法很少直接从数据中推断AU与表情之间的心理学合理因果关系，需要建立可解释的因果推理框架来理解面部行为的潜在依赖关系。

Method: 提出CausalAffect框架：1）两级极性感知因果层次建模AU-AU和AU-Expression依赖关系；2）特征级反事实干预机制抑制虚假相关性；3）无需联合标注数据或手工因果先验。

Result: 在六个基准测试上实验表明，CausalAffect在AU检测和表情识别上达到最先进水平，发现的因果结构与心理学理论一致，并揭示了新的抑制性和未表征的依赖关系。

Conclusion: CausalAffect建立了因果发现与可解释面部行为之间的原则性连接，为面部情感分析提供了首个因果图发现框架，所有训练模型和源代码将在接受后发布。
...
### [42] [RealGen: Photorealistic Text-to-Image Generation via Detector-Guided Rewards](https://arxiv.org/abs/2512.00473)
*Junyan Ye,Leiqi Zhu,Yuncheng Guo,Dongzhi Jiang,Zilong Huang,Yifan Zhang,Zhiyuan Yan,Haohuan Fu,Conghui He,Weijia Li*

Main category: cs.CV

TL;DR: RealGen是一个通过集成LLM提示优化和扩散模型，并引入"检测器奖励"机制来提升文本到图像生成真实感的框架，显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 当前先进的文本到图像生成模型（如GPT-Image-1和Qwen-Image）虽然在文本一致性方面表现良好，但在生成真实感图像方面仍有不足，常产生带有明显AI伪影的"假"图像，如"过度光滑的皮肤"和"油光满面"等问题。研究旨在重新实现"与现实无法区分"的生成目标。

Method: RealGen框架包含：1）LLM组件用于提示优化；2）扩散模型用于真实感图像生成；3）受对抗生成启发的"检测器奖励"机制，通过语义级和特征级合成图像检测器量化伪影并评估真实感；4）使用GRPO算法优化整个生成流程。此外还提出了RealBench自动评估基准，采用检测器评分和竞技场评分实现无需人工的真实感评估。

Result: 实验表明，RealGen在真实感、细节和美学方面显著优于通用模型（如GPT-Image-1和Qwen-Image）以及专门的真实感模型（如FLUX-Krea）。

Conclusion: RealGen通过集成提示优化、扩散模型和检测器奖励机制，有效提升了文本到图像生成的真实感，同时提出的RealBench评估基准为真实感评估提供了更准确、更符合用户体验的自动化方法。
...
### [43] [CC-FMO: Camera-Conditioned Zero-Shot Single Image to 3D Scene Generation with Foundation Model Orchestration](https://arxiv.org/abs/2512.00493)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在单图像到3D场景生成中存在两个主要问题：1）早期方法依赖在小型数据集上训练的专业模型，泛化能力差；2）尽管大规模3D基础模型提升了实例级生成能力，但场景级生成仍面临物体姿态估计不准确和空间不一致的挑战。
...
### [44] [Terrain Sensing with Smartphone Structured Light: 2D Dynamic Time Warping for Grid Pattern Matching](https://arxiv.org/abs/2512.00514)
*Tanaka Nobuaki*

Main category: cs.CV

TL;DR: 提出基于智能手机的结构光系统，使用拓扑约束的二维动态时间规整算法匹配地面投影网格，实现低成本移动机器人的地形不平度感知。


<details>
  <summary>Details</summary>
Motivation: 低成本移动机器人在不平坦地形上运行时，视觉难以察觉的小颠簸或倾斜会显著影响运动稳定性，需要一种简单有效的地形不平度感知方法。

Method: 开发基于智能手机的结构光系统，将网格图案投影到地面，提出拓扑约束的二维动态时间规整算法，在全局网格一致性约束下进行列对齐，匹配变形网格。

Result: 系统能够在资源有限的平台上运行，保持网格结构以实现精确三角测量，不仅可用于地形感知，还可作为图像处理中结构化网格匹配的通用工具。

Conclusion: 提出的2D-DTW算法扩展了传统1D-DTW的应用范围，为低成本移动机器人提供了有效的地形不平度感知解决方案，具有实际应用价值。
...
### [45] [Image Generation as a Visual Planner for Robotic Manipulation](https://arxiv.org/abs/2512.00532)
*Ye Pang*

Main category: cs.CV

TL;DR: 该论文探索使用预训练图像生成模型作为机器人视觉规划器，通过LoRA微调生成连贯的机器人操作视频，无需大量领域特定数据。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型需要大量领域特定数据且泛化能力有限，而预训练的语言-图像模型展现出良好的组合性和时间连贯性，暗示其具有视频生成潜力，可作为机器人视觉规划器。

Method: 提出两部分框架：1) 文本条件生成：使用语言指令和第一帧；2) 轨迹条件生成：使用2D轨迹覆盖和相同初始帧。通过LoRA微调轻量适配预训练图像生成模型。

Result: 在Jaco Play、Bridge V2和RT1数据集上的实验表明，两种模式都能生成平滑、连贯且与条件对齐的机器人视频，验证了预训练图像生成器编码了可迁移的时间先验。

Conclusion: 预训练图像生成器编码了可迁移的时间先验，能够在最小监督下作为视频式机器人规划器，为统一感知、规划和行动的具身智能体提供了新途径。
...
### [46] [Cross-Temporal 3D Gaussian Splatting for Sparse-View Guided Scene Update](https://arxiv.org/abs/2512.00534)
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中常面临稀疏视角观测的3D场景更新挑战，如城市规划、灾害评估等场景难以获得密集扫描数据，需要一种能利用历史先验和稀疏新观测来高效更新3D场景的方法。

Method: 采用三阶段方法：1)跨时间相机对齐，估计并对齐不同时间戳的相机位姿；2)基于干扰的置信度初始化，识别时间戳间未变化区域以指导更新；3)渐进式跨时间优化，迭代整合历史先验信息提升重建质量。

Result: 实验结果显示，相比基线方法在重建质量和数据效率方面有显著提升，支持非连续捕获，能够用稀疏新视图更新现有场景，也能借助当前捕获恢复过去场景。

Conclusion: 该方法为场景版本管理、跨时间数字孪生和长期空间文档记录提供了有前景的解决方案，仅用稀疏图像就能实现时间变化检测，并能按需重建为详细3D表示。
...
### [47] [SAIDO: Generalizable Detection of AI-Generated Images via Scene-Aware and Importance-Guided Dynamic Optimization in Continual Learning](https://arxiv.org/abs/2512.00539)
*Yongkang Hu,Yu Cheng,Yushuo Zhang,Yuan Xie,Zhaoxia Yin*

Main category: cs.CV

TL;DR: 提出SAIDO框架，通过场景感知和重要性引导的动态优化机制，结合持续学习解决AI生成图像检测的泛化问题


<details>
  <summary>Details</summary>
Motivation: AI生成图像技术的滥用引发安全担忧，现有检测方法难以适应新兴生成方法和内容类型，泛化能力成为关键挑战

Method: 提出SAIDO框架：1) 基于场景感知的专家模块(SAEM)，利用VLLMs动态识别新场景并分配独立专家模块；2) 重要性引导的动态优化机制(IDOM)，通过重要性引导的梯度投影策略优化神经元，平衡模型可塑性和稳定性

Result: 在持续学习任务中，相比当前SOTA方法，平均检测错误率和遗忘率分别相对降低44.22%和40.57%；在开放世界数据集上，平均检测准确率提升9.47%

Conclusion: SAIDO框架通过场景感知和重要性引导的动态优化，有效解决了AI生成图像检测的泛化问题，在稳定性和可塑性方面均优于现有方法
...
### [48] [Asset-Driven Sematic Reconstruction of Dynamic Scene with Multi-Human-Object Interactions](https://arxiv.org/abs/2512.00547)
*Sandika Biswas,Qianyi Wu,Biplab Banerjee,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: 该论文提出了一种混合方法，结合3D生成模型、语义感知变形和3D高斯泼溅优化，用于多人物多物体动态场景的3D几何建模。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人造环境高度动态，包含多个人物和物体之间的复杂交互。虽然3D几何建模对AR/VR、游戏和具身AI等应用至关重要，但由于运动模式多样、遮挡频繁等挑战，这类场景的建模仍未被充分探索。特别是在单目设置下，仅基于高斯泼溅渲染损失难以在严重遮挡下保持结构一致性。

Method: 提出混合方法：1) 使用3D生成模型生成场景元素的高保真网格；2) 语义感知变形，包括刚性物体的刚性变换和基于LBS的人物变形，并将变形后的高保真网格映射到动态场景中；3) 基于高斯泼溅的优化，进一步细化场景中各元素的对齐。

Result: 在HOI-M3数据集上评估，这是唯一包含多人物多物体交互的动态场景数据集。该方法在表面重建质量上优于现有最先进方法。

Conclusion: 该混合方法能够有效处理多人物多物体动态场景的3D几何建模挑战，即使在严重遮挡下也能保持物体结构，并产生多视角和时间一致的几何结果。
...
### [49] [NeuroVolve: Evolving Visual Stimuli toward Programmable Neural Objectives](https://arxiv.org/abs/2512.00557)
*Haomiao Chen,Keith W Jamison,Mert R. Sabuncu,Amy Kuceyeski*

Main category: cs.CV

TL;DR: NeuroVolve是一个生成框架，通过优化预训练视觉语言模型嵌入空间中的神经目标函数，实现大脑引导的图像合成，能够生成满足单区域或多区域约束的连贯场景。


<details>
  <summary>Details</summary>
Motivation: 先前的研究使用生成模型复制了孤立大脑区域（如FFA中的面孔）的已知类别选择性，但这些方法对复杂自然视觉中区域如何相互作用提供了有限见解。需要一种能够探索大脑区域如何互动并创建神经表征的框架。

Method: 引入NeuroVolve生成框架，通过优化预训练视觉语言模型嵌入空间中的可编程神经目标函数来合成图像。神经目标可以编程为激活或抑制单个或多个大脑区域，通过跟踪优化步骤揭示嵌入空间中的语义轨迹。

Result: NeuroVolve能够恢复单个大脑区域的已知选择性，同时扩展到合成满足复杂多区域约束的连贯场景。能够生成低层和语义特征特定的刺激，以及符合定制神经目标的刺激，包括区域间的共激活和去相关关系，并捕获个体特异性偏好。

Conclusion: NeuroVolve统一了大脑引导的图像编辑和偏好刺激生成，为映射、分析和探测视觉信息的神经表征提供了可解释的约束，支持个性化大脑驱动的合成。
...
### [50] [Integrating Skeleton Based Representations for Robust Yoga Pose Classification Using Deep Learning Models](https://arxiv.org/abs/2512.00572)
*Mohammed Mohiuddin,Syed Mohammod Minhaz Hossain,Sumaiya Khanam,Prionkar Barua,Aparup Barua,MD Tamim Hossain*

Main category: cs.CV

TL;DR: 该研究创建了Yoga-16数据集，系统评估了三种深度学习架构在三种输入模态下的瑜伽姿势分类性能，发现基于骨架的表示优于原始图像输入，VGG16+MediaPipe Pose骨架组合达到96.09%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 瑜伽姿势不正确可能导致受伤，自动瑜伽姿势分类可以减少对专家指导的依赖。现有研究在瑜伽姿势识别方面缺乏系统性的基准测试，通常只关注原始图像或单一姿势提取模型。

Method: 创建了Yoga-16数据集，系统评估了VGG16、ResNet50和Xception三种深度学习架构，使用三种输入模态：直接图像、MediaPipe Pose骨架图像和YOLOv8 Pose骨架图像，并通过Grad-CAM进行可解释性分析和交叉验证。

Result: 骨架表示优于原始图像输入，VGG16与MediaPipe Pose骨架组合达到96.09%的最高准确率。通过Grad-CAM分析提供了模型决策的见解。

Conclusion: 基于骨架的表示在瑜伽姿势分类中表现优异，VGG16+MediaPipe Pose骨架组合是最佳方案，研究为自动瑜伽姿势识别提供了系统基准和可解释性分析。
...
### [51] [SatireDecoder: Visual Cascaded Decoupling for Enhancing Satirical Image Comprehension](https://arxiv.org/abs/2512.00582)
*Yue Jiang,Haiwei Xue,Minghao Han,Mingcheng Li,Xiaolu Hou,Dingkang Yang,Lihua Zhang,Xu Zheng*

Main category: cs.CV

TL;DR: SatireDecoder是一个无需训练的多智能体框架，通过视觉级联解耦和不确定性引导的思维链推理，提升视觉讽刺图像的理解能力，显著减少幻觉并提高解释准确性。
...
Method: 提出SatireDecoder训练免费框架：1）多智能体系统执行视觉级联解耦，将图像分解为细粒度的局部和全局语义表示；2）引入不确定性分析引导的思维链推理策略，将复杂讽刺理解过程分解为具有最小不确定性的顺序子任务。

Result: 实验结果表明，SatireDecoder在理解视觉讽刺方面优于现有基线方法，显著提高了解释准确性同时减少了幻觉，为细微、高级语义任务的视觉语言推理提供了有前景的方向。

Conclusion: SatireDecoder通过多智能体视觉解耦和不确定性引导的推理，有效解决了视觉讽刺理解的挑战，为处理复杂语义任务的视觉语言模型提供了新的解决方案。
...
### [52] [Scaling Down to Scale Up: Towards Operationally-Efficient and Deployable Clinical Models via Cross-Modal Low-Rank Adaptation for Medical Vision-Language Models](https://arxiv.org/abs/2512.00597)
*Thuraya Alzubaidi,Farhad R. Nezami,Muzammil Behzad*

Main category: cs.CV

TL;DR: MedCT-VLM是一个参数高效的医学CT视觉语言模型，通过LoRA微调方法，仅训练0.38%的参数就能将CT-CLIP基础模型适配到下游临床任务，在18种胸部病理的零样本分类中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言预训练的基础模型在各种图像领域表现出强大的零样本能力，但在体积医学成像（如CT）中的应用仍然有限。医学领域标注数据稀缺，需要开发参数高效的方法来将大规模预训练模型迁移到下游医疗任务。
...
Result: 在18种胸部病理的零样本分类评估中，LoRA微调使平均AUROC从61.3%提升到68.9%（+7.6个百分点），准确率从67.2%提升到73.6%（+6.4个百分点），宏观F1分数从32.1%提升到36.9%（+4.8个百分点）。

Conclusion: 参数高效方法能有效将大规模预训练迁移到下游医学成像任务，特别是在标注数据稀缺的零样本场景中具有重要价值。MedCT-VLM展示了在医疗领域应用基础模型的可行性。
...
### [53] [Automatic Pith Detection in Tree Cross-Section Images Using Deep Learning](https://arxiv.org/abs/2512.00625)
*Tzu-I Liao,Mahmoud Fakhry,Jibin Yesudas Varghese*

Main category: cs.CV

TL;DR: 本研究评估了YOLOv9、U-Net、Swin Transformer、DeepLabV3和Mask R-CNN五种深度学习模型在树木横截面髓心检测中的性能，通过582张标注图像的数据集和动态增强技术，Swin Transformer取得了最高精度（0.94），而Mask R-CNN在应用NMS后IoU从0.45提升到0.80。


<details>
  <summary>Details</summary>
Motivation: 树木横截面髓心检测对于林业和木材质量分析至关重要，但目前主要依赖人工操作，容易出错且效率低下。本研究旨在通过深度学习模型实现自动化检测，提高检测效率和准确性。

Method: 研究使用582张标注图像的数据集，采用动态数据增强技术提高模型泛化能力。评估了五种深度学习模型：YOLOv9（边界框检测）、U-Net、Swin Transformer、DeepLabV3和Mask R-CNN（实例分割）。通过俄勒冈州立大学树轮实验室的11张橡树图像测试模型泛化性，并使用额外64张标注图像对性能最差的模型进行训练以观察改进效果。

Result: Swin Transformer表现最佳，准确率达到0.94，在精细分割方面表现突出。YOLOv9在边界框检测中表现良好但边界精度不足。U-Net对结构化模式有效，DeepLabV3能捕获多尺度特征但边界略有不准。Mask R-CNN初始IoU仅0.45，应用非极大值抑制（NMS）后提升至0.80。模型在橡树数据集上展示了泛化能力，额外训练改善了最差模型的性能。

Conclusion: 深度学习在树木横截面髓心检测中具有巨大潜力，模型选择应根据数据集特征和应用需求。Swin Transformer在精度方面表现最优，而不同模型各有适用场景。通过超参数调优和数据增强可以解决张量不匹配和边界不一致等挑战。
...
### [54] [Doppler-Enhanced Deep Learning: Improving Thyroid Nodule Segmentation with YOLOv5 Instance Segmentation](https://arxiv.org/abs/2512.00639)
*Mahmoud El Hussieni*

Main category: cs.CV

TL;DR: YOLOv5算法在甲状腺结节超声图像实例分割中表现出色，包含多普勒图像能显著提升分割性能，YOLOv5-Large模型在包含多普勒图像的数据集上达到最佳效果。


<details>
  <summary>Details</summary>
Motivation: 甲状腺癌发病率上升，需要开发计算机辅助检测方法。甲状腺结节准确分割是AI辅助临床决策支持系统开发的关键第一步。

Method: 使用YOLOv5算法（Nano、Small、Medium、Large、XLarge五种变体）在超声图像上进行甲状腺结节实例分割，评估了包含和不包含多普勒图像的两种数据集版本。

Result: YOLOv5-Large算法在包含多普勒图像的数据集上表现最佳，Dice分数达到91%，mAP为0.87。包含多普勒图像能显著提升所有模型变体的性能，YOLOv5-Small模型在不包含多普勒图像时Dice分数为79%，包含后性能提升。

Conclusion: YOLOv5实例分割为甲状腺结节检测提供了有效的实时方法，多普勒图像虽然通常被医生排除，但能显著改善分割性能，具有在自动诊断系统中临床应用的潜力。
...
### [55] [MambaScope: Coarse-to-Fine Scoping for Efficient Vision Mamba](https://arxiv.org/abs/2512.00647)
*Shanhui Liu,Rui Xu,Yunke Wang*

Main category: cs.CV

TL;DR: CF-ViM提出了一种从粗到细的自适应视觉Mamba框架，通过根据图像复杂度动态调整处理分辨率来提升效率，避免现有token缩减方法的信息损失问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Mamba模型的效率受限于输入token数量，而现有的token缩减方法（如剪枝或合并）会导致信息损失，且对所有图像统一应用细粒度处理不够高效。观察到并非所有图像都需要细粒度处理，简单图像可以在粗分辨率下有效处理，只有复杂图像才需要细化。

Method: 提出Coarse-to-Fine Vision Mamba (CF-ViM)框架：1）首先将输入图像划分为大块进行粗粒度推理，显著减少token长度和计算量；2）当模型预测置信度低时，选择特定区域以更细分辨率重新处理，以最小额外成本恢复关键视觉细节；3）采用动态分辨率分配策略，根据图像复杂度自适应分配计算资源。

Result: 在ImageNet上的实验表明，CF-ViM在准确性和效率方面均优于基线Vision Mamba和最先进的token缩减技术。

Conclusion: CF-ViM通过自适应地从粗到细处理策略，能够根据图像复杂度动态分配计算资源，在保持视觉信息完整性的同时实现高效推理，为视觉Mamba模型提供了一种更优的效率提升方案。
...
### [56] [Realistic Handwritten Multi-Digit Writer (MDW) Number Recognition Challenges](https://arxiv.org/abs/2512.00676)
*Kiri L. Wagstaff*

Main category: cs.CV

TL;DR: 论文创建了更真实的多数字书写者基准数据集，发现孤立数字分类器在多数字识别中表现不佳，需要新方法提升实际应用性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中数字通常以多位数形式出现（如邮政编码、支票金额、预约时间），但现有研究主要关注孤立数字分类，缺乏真实的多数字识别基准。

Method: 利用NIST数字图像中书写者信息创建多数字书写者基准数据集，并设计更贴近实际影响的特定任务性能指标。

Result: 发现孤立数字分类器在多数字识别中表现不佳，需要开发能利用任务特定知识的新方法以显著提升性能。

Conclusion: 为解决实际数字识别问题，需要超越孤立数字分类的进步，多数字书写者基准为开发更有效的实际应用方法提供了机会。
...
TL;DR: Dynamic-eDiTor：无需训练的文本驱动4D场景编辑框架，结合MM-DiT和4DGS，通过STGA和CTP机制实现时空一致性编辑


<details>
  <summary>Details</summary>
Motivation: 现有4D场景重建技术（如Dynamic NeRF和4DGS）已取得进展，但文本驱动的4D场景编辑仍面临挑战，主要问题在于确保编辑过程中的多视角和时空一致性。现有方法依赖2D扩散模型独立编辑帧，常导致运动失真、几何漂移和不完整编辑。

Method: 提出Dynamic-eDiTor框架，结合多模态扩散变换器（MM-DiT）和4D高斯溅射（4DGS）。包含两个核心机制：1）时空子网格注意力（STGA）用于局部一致的跨视角和时间融合；2）上下文令牌传播（CTP）通过令牌继承和光流引导的令牌替换实现全局传播。无需额外训练，可直接优化预训练的源4DGS。

Result: 在DyNeRF多视角视频数据集上的大量实验表明，该方法在编辑保真度、多视角一致性和时间一致性方面优于现有方法。

Conclusion: Dynamic-eDiTor能够实现无缝、全局一致的多视角视频编辑，无需额外训练，直接优化预训练的4DGS表示，为文本驱动的4D场景编辑提供了有效解决方案。
...
TL;DR: FoundationGait是首个可扩展的自监督预训练步态理解框架，拥有近1.3亿参数，在12个公共数据集上预训练，在多种步态任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前步态识别研究受限于小型、狭窄设计的模型，无法扩展或泛化。需要解决两个长期障碍：1) 可扩展性：为什么步态模型历史上未能遵循缩放定律？2) 泛化性：能否用一个模型服务于传统上孤立研究的多样化步态任务？

Method: 提出了FoundationGait，一个可扩展的自监督预训练框架，最大版本有近1.3亿参数，在包含200多万个行走序列的12个公共步态数据集上进行预训练。

Result: FoundationGait在有或没有微调的情况下，在广泛的步态数据集、条件、任务和输入模态中表现稳健。在具有挑战性的野外数据集Gait3D上达到48.0%的零样本Rank-1准确率，在实验室数据集OU-MVLP上达到64.5%，创下了稳健步态识别的新里程碑。

Conclusion: FoundationGait是首个可扩展的步态基础模型，成功解决了步态模型的可扩展性和泛化性问题，为统一的步态理解框架奠定了基础。
...
TL;DR: AFD通过将视频映射到缓慢变化的可供性令牌作为共享的时间对齐基底，同时使用轻量级、查询路由、冲突感知的调度器集中适应和按需增长容量，实现了视频-语言理解的持续学习，在多个协议上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习解决方案存在以下问题：模糊了应该保持稳定与应该适应的部分；依赖静态路由/容量；需要回放过去的视频。需要在现实内存和隐私约束下，明确指定稳定性所在位置和可塑性应聚焦的区域。

Method: 提出Affordance-First Decomposition (AFD)：1) 将视频映射到缓慢变化的可供性令牌，形成共享的时间对齐基底；2) 使用轻量级、查询路由、冲突感知的调度器集中适应和按需增长容量；3) 通过弱对齐和教师一致性稳定基底；4) 训练时仅使用问题回放。

Result: 在多个协议上达到最先进水平：域增量VideoQA平均准确率51.6%，遗忘率-1.8%；ViLCo R@1@0.5为29.6%(MQ)和20.7%(NLQ)，VQ为18.4% stAP@0.25；时间增量iVQA准确率39.5%，遗忘率-1.6%。

Conclusion: AFD提供了在稳定的交互中心基底与针对性适应之间的明确、可解释的分离，为视频-语言理解的持续学习提供了有效解决方案。
...
### [60] [TrajDiff: End-to-end Autonomous Driving without Perception Annotation](https://arxiv.org/abs/2512.00723)
*Xingtai Gui,Jianbo Zhao,Wencheng Han,Jikai Wang,Jiahao Gong,Feiyang Tan,Cheng-zhong Xu,Jianbing Shen*

Main category: cs.CV

TL;DR: TrajDiff是一个无需感知标注的端到端自动驾驶轨迹生成框架，通过轨迹导向的BEV条件扩散模型直接从原始传感器输入生成多样化且合理的轨迹。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统需要从原始传感器输入直接生成驾驶策略，但现有方法依赖辅助感知任务和昂贵的人工感知标注。开发无需感知标注的规划范式变得日益重要。

Method: 提出TrajDiff框架：1) 仅使用原始传感器输入和未来轨迹，构建高斯BEV热图目标；2) 设计轨迹导向的BEV编码器提取TrajBEV特征，无需感知监督；3) 引入轨迹导向的BEV扩散变换器(TB-DiT)，利用自车状态信息和预测的TrajBEV特征直接生成多样化轨迹，无需手工设计的运动先验。

Result: 在NAVSIM基准测试中，TrajDiff达到87.5 PDMS，在无需标注方法中达到最先进性能。通过数据扩展可提升至88.5 PDMS，与先进的基于感知的方法相当。

Conclusion: TrajDiff成功建立了完全无需感知标注的生成式端到端自动驾驶方法，不仅实现了优异的性能，还探索了在无需标注设置下的数据扩展优势，为自动驾驶系统开发提供了更经济高效的解决方案。
...
### [61] [Generalized Medical Phrase Grounding](https://arxiv.org/abs/2512.01085)
*Wenjun Zhang,Shekhar S. Chandra,Aaron Nicolson*

Main category: cs.CV

TL;DR: 本文提出广义医学短语定位(GMPG)任务，解决传统医学短语定位只能返回单个边界框的局限性，并开发了MedGrounder模型，能够处理多区域、非诊断性文本和非可定位短语。


<details>
  <summary>Details</summary>
Motivation: 现有医学短语定位系统大多遵循指代表达理解范式，每个短语只返回一个边界框。但真实报告中常包含多区域发现、非诊断性文本以及否定或正常解剖描述等不可定位短语，需要更通用的任务定义。

Method: 提出广义医学短语定位(GMPG)任务，将每个句子映射到零个、一个或多个评分区域。开发MedGrounder模型，采用两阶段训练策略：先在报告句子-解剖框对齐数据集上预训练，然后在报告句子-人工标注框数据集上微调。

Result: 在PadChest-GR和MS-CXR数据集上的实验表明，MedGrounder实现了强大的零样本迁移能力，在多区域和非可定位短语上优于REC风格和基于报告的基线方法，同时使用更少的人工框标注。

Conclusion: MedGrounder能够处理真实医学报告中的复杂情况，并且可以与现有报告生成器组合使用，无需重新训练生成器即可生成基于定位的报告。
...
### [62] [SwiftVLA: Unlocking Spatiotemporal Dynamics for Lightweight VLA Models at Minimal Overhead](https://arxiv.org/abs/2512.00903)
...
TL;DR: SwiftVLA是一种高效的视觉-语言-动作模型，通过4D特征提取和融合机制，在保持轻量化的同时提升时空推理能力，在边缘设备上实现高性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型参数量大，实用性受限；轻量化模型又牺牲了时空推理能力。虽然加入3D输入能有所帮助，但仍依赖大型模型且缺乏时间维度理解。

Method: 1) 使用预训练的4D视觉几何变换器提取4D特征；2) 引入融合令牌通过未来预测目标生成统一表示；3) 采用掩码重构策略训练模型学习有效4D表示，推理时可丢弃4D分支。

Result: 在真实和模拟环境中，SwiftVLA超越轻量级基线模型，性能媲美参数量7倍大的模型，在边缘设备上速度提升18倍，内存占用减少12倍。

Conclusion: SwiftVLA通过创新的4D特征融合和训练策略，在保持轻量化的同时显著提升了时空推理能力，为边缘设备上的高效视觉-语言-动作模型提供了可行方案。
...
### [63] [Optimizing LVLMs with On-Policy Data for Effective Hallucination Mitigation](https://arxiv.org/abs/2512.00706)
*Chengzhi Yu,Yifan Xu,Yifan Chen,Wenyi Zhang*

Main category: cs.CV

TL;DR: 该论文提出了一种减少大型视觉语言模型幻觉的新方法，通过使用在线策略数据、幻觉分类器和动态样本重加权机制，显著降低了模型的幻觉率。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但幻觉问题仍然是关键挑战。现有方法在训练样本中引入了额外的幻觉，可能增强模型的幻觉模式，需要更有效的幻觉缓解方法。

Method: 1) 分析发现在线策略数据显著优于离线策略数据；2) 提出训练幻觉分类器提供二元标注，确保后续对齐的样本干净；3) 设计鲁棒的迭代直接偏好优化算法，采用动态样本重加权方案。

Result: 在三个基准测试中与8个最先进基线比较：1) LLaVA-1.5-7B在MMHalBench上的幻觉率降低50.8%；2) 在Object HalBench上的平均幻觉率降低79.5%；3) LLaVA-1.5-13B甚至超过了GPT-4V的性能。

Conclusion: 该方法通过在线策略数据、幻觉分类器和动态重加权机制，有效缓解了大型视觉语言模型的幻觉问题，充分挖掘了开源模型的潜力，实现了显著的性能提升。
...
### [64] [MM-ACT: Learn from Multimodal Parallel Generation to Act](https://arxiv.org/abs/2512.00975)
...
TL;DR: MM-ACT是一个统一的视觉-语言-动作模型，通过共享标记空间集成文本、图像和动作，采用重新掩码并行解码策略提高效率，在多个机器人任务中表现出色。
...
Method: 提出MM-ACT统一VLA模型，在共享标记空间中集成文本、图像和动作；采用重新掩码并行解码策略处理文本和图像生成，一步并行解码策略处理动作生成；引入上下文共享多模态学习训练范式。

Result: 在LIBERO仿真中成功率96.3%，真实Franka机器人三个任务中72.0%，RoboTwin2.0八个双手任务中52.38%，跨模态学习带来额外9.25%性能提升。

Conclusion: MM-ACT通过统一的视觉-语言-动作模型和上下文共享多模态学习，有效提升了机器人策略的语义理解和预测能力，在仿真和真实机器人任务中都取得了优异表现。
...
### [65] [Real-Time On-the-Go Annotation Framework Using YOLO for Automated Dataset Generation](https://arxiv.org/abs/2512.01165)
*Mohamed Abdallah Salem,Ahmed Harb Rabia*

Main category: cs.CV

TL;DR: 本文提出了一种基于YOLO模型的实时标注方法，通过在边缘设备上部署YOLO模型，在图像采集过程中即时完成标注，显著减少了农业应用中数据集准备时间。


<details>
  <summary>Details</summary>
Motivation: 在农业等需要快速决策的实际应用中，数据集标注仍然是部署YOLO等目标检测模型的主要挑战。传统标注方法劳动密集，需要在数据收集后进行大量手动标注工作。

Method: 提出了一种新颖的实时标注方法，在边缘设备上部署YOLO模型，能够在图像捕获过程中立即进行标注。使用三种主流YOLO架构（YOLOv5、YOLOv8、YOLOv12）进行综合比较分析，评估单类别与多类别标注、预训练与从头训练等不同配置。

Result: 通过详细的统计测试和学习动态分析，证明了预训练和单类别配置在模型收敛、性能和鲁棒性方面的显著优势。结果强烈验证了实时标注框架的可行性和有效性。

Conclusion: 该实时标注框架能够大幅减少数据集准备时间，同时保持高标注质量，为农业等实时应用中的目标检测模型部署提供了有效的解决方案。
...
TL;DR: RS-ISRefiner是一个专为遥感图像设计的交互式图像分割框架，通过适配器调优策略和混合注意力机制，有效处理遥感图像中的尺度变化、不规则边界和复杂背景问题，在多个数据集上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有交互式图像分割方法主要针对自然图像设计，难以泛化到遥感图像领域，因为遥感图像中的物体存在尺度变化、不规则边界和复杂背景，且标注数据有限、计算开销大。

Method: 提出RS-ISRefiner框架：1）采用适配器调优策略，保留视觉基础模型的通用表示，同时高效学习遥感特定的空间和边界特征；2）混合注意力机制结合卷积局部建模和Transformer全局推理，增强对尺度多样性和场景复杂性的鲁棒性；3）改进的概率图调制方案有效整合历史用户交互，实现更稳定的迭代优化和更高边界保真度。

Result: 在iSAID、ISPRS Potsdam、SandBar、NWPU、LoveDA Urban和WHUBuilding六个遥感数据集上的综合实验表明，RS-ISRefiner在分割精度、效率和交互成本方面持续优于最先进的交互式图像分割方法。

Conclusion: 该框架证明了其有效性和泛化能力，非常适合实际遥感场景中的高质量实例分割任务。
...
### [67] [SPARK: Sim-ready Part-level Articulated Reconstruction with VLM Knowledge](https://arxiv.org/abs/2512.01629)
*Yumeng He,Ying Jiang,Jiayin Lu,Yin Yang,Chenfanfu Jiang*

Main category: cs.CV

TL;DR: SPARK框架从单张RGB图像重建具有物理一致性的运动学部件级铰接物体，生成仿真就绪的资产


<details>
  <summary>Details</summary>
Motivation: 铰接3D物体对具身AI、机器人和交互场景理解至关重要，但创建仿真就绪资产仍然劳动密集且需要专家建模部件层次结构和运动结构

Method: 首先利用视觉语言模型提取粗略URDF参数并生成部件级参考图像；然后集成部件图像引导和推断的结构图到生成扩散transformer中合成一致的部件和完整形状；最后通过可微分前向运动学和可微分渲染优化关节类型、轴和原点

Result: 广泛实验表明SPARK能够跨多样类别生成高质量的仿真就绪铰接资产，支持机器人操作和交互建模等下游应用

Conclusion: SPARK框架成功实现了从单张RGB图像重建物理一致的铰接物体，解决了创建仿真就绪资产的劳动密集问题
...
TL;DR: StreamGaze是首个评估多模态大语言模型在流式视频中利用人类注视信号进行时序和前瞻推理能力的基准测试，填补了现有流式视频理解基准在注视信号评估方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解基准主要评估时序推理能力，但缺乏对模型如何解释和利用人类注视信号的评估。在AR眼镜等实际应用中，理解用户注视对于推断意图和实现主动交互至关重要。

Method: 开发了注视-视频QA生成流水线，通过注视点提取、区域特定视觉提示和扫描路径构建，将自我中心视频与原始注视轨迹对齐，生成时空基础的QA对，反映人类感知动态。

Result: 在StreamGaze所有任务中，最先进的多模态大语言模型与人类表现之间存在显著性能差距，揭示了在基于注视的时序推理、意图建模和前瞻预测方面的根本局限性。

Conclusion: StreamGaze基准揭示了当前多模态大语言模型在注视引导的流式视频理解方面的不足，为未来模型开发提供了重要方向，所有数据和代码将公开以支持进一步研究。
...
TL;DR: 提出一种将点云配准视为条件生成问题的新方法，通过学习连续点级速度场将噪声点传输到配准场景，直接生成配准点云而非估计变换矩阵。


<details>
  <summary>Details</summary>
Motivation: 传统点云配准方法通过对应匹配估计点云对之间的变换，然后优化成对变换实现多视图配准，存在局限性。本文旨在开发更直接、更有效的配准方法。

Method: 将配准建模为条件生成问题：学习连续点级速度场，将噪声点传输到配准场景；使用轻量级局部特征提取器和测试时刚性约束；直接生成配准点云而非估计变换矩阵。

Result: 在成对和多视图配准基准测试中达到最先进结果，特别是在低重叠情况下；能够跨尺度和传感器模态泛化；支持重定位、多机器人SLAM和多会话地图融合等下游任务。

Conclusion: 将点云配准视为条件生成问题的新方法比传统基于对应匹配的方法更有效，特别是在具有挑战性的低重叠场景中，并具有广泛的适用性和泛化能力。
...
### [70] [Multi-GRPO: Multi-Group Advantage Estimation for Text-to-Image Generation with Tree-Based Trajectories and Multiple Rewards](https://arxiv.org/abs/2512.00743)
...
TL;DR: Multi-GRPO是一个改进的GRPO框架，通过时间分组和奖励分组机制解决现有方法的信用分配和奖励混合问题，在多目标对齐任务中表现更优。
...
Method: 提出Multi-GRPO框架，包含两个正交分组机制：1）基于蒙特卡洛树搜索的时间分组，在早期去噪步骤分支形成时间组，通过后代叶节点准确估计早期步骤优势；2）奖励分组，独立计算每个奖励函数的优势后再聚合，解耦冲突信号。

Result: 在单奖励PickScore-25k和多目标OCR-Color-10基准测试中，Multi-GRPO实现了更好的稳定性和对齐性能，有效平衡冲突目标。

Conclusion: Multi-GRPO通过时间分组和奖励分组机制解决了GRPO方法的信用分配和奖励混合问题，在多目标文本到图像模型对齐任务中表现出优越性能。
...
TL;DR: RLWG框架通过几何和感知奖励将预训练世界模型与物理可验证结构对齐，提升导航任务中的空间一致性和长期稳定性


<details>
  <summary>Details</summary>
Motivation: 当前视频世界建模模型虽然视觉保真度高，但缺乏几何基础，限制了在需要空间一致性和长期稳定性的导航任务中的应用

Method: 提出RLWG自监督后训练框架，使用几何和感知奖励（姿态循环一致性、深度重投影、时间一致性）对齐预训练世界模型；基于GRPO的GrndCtrl奖励对齐适应方法

Result: GrndCtrl在室外环境中实现了优于监督微调的空间一致性和导航稳定性，能够保持稳定轨迹、一致几何和可靠rollout

Conclusion: RLWG框架通过可验证奖励桥接生成预训练与接地行为，类似于大语言模型的后训练对齐，为具身导航提供了几何基础的世界模型
...
### [72] [Visual Sync: Multi-Camera Synchronization via Cross-View Object Motion](https://arxiv.org/abs/2512.02017)
*Shaowei Liu,David Yifan Yao,Saurabh Gupta,Shenlong Wang*

Main category: cs.CV

TL;DR: VisualSync是一个基于多视角动态的优化框架，能够以毫秒级精度对齐未标定、未同步的视频，通过利用运动3D点的极线约束来实现跨摄像头视频同步。


<details>
  <summary>Details</summary>
Motivation: 当前人们可以轻松使用多个消费级摄像头记录重要时刻，但同步这些跨摄像头视频流仍然具有挑战性。现有方法需要受控设置、特定目标、手动校正或昂贵硬件，限制了实际应用。

Method: VisualSync基于多视角动态优化框架，利用运动3D点在两个摄像头中可见时遵循极线约束的原理。使用现成的3D重建、特征匹配和密集跟踪技术提取轨迹片段、相对位姿和跨视角对应关系，然后联合最小化极线误差来估计每个摄像头的时间偏移。

Result: 在四个多样化、具有挑战性的数据集上的实验表明，VisualSync优于基线方法，实现了中位数同步误差低于50毫秒的精度。

Conclusion: VisualSync提供了一个有效的框架，能够在无需受控设置、特定目标、手动校正或昂贵硬件的情况下，实现跨摄像头视频流的毫秒级精度同步。
...
### [73] [Probabilistic Modeling of Multi-rater Medical Image Segmentation for Diversity and Personalization](https://arxiv.org/abs/2512.00748)
*Ke Liu,Shangde Gao,Yichao Fu,Shangqi Gao,Chunhua Shen*

Main category: cs.CV

TL;DR: ProSeg是一个概率建模框架，用于多标注者医学图像分割，能同时实现多样化和个性化分割，通过建模专家偏好和图像边界模糊性来生成既多样又个性化的分割结果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割存在数据不确定性，源于医学扫描中的模糊边界和诊断中的观察者间差异。现有模型要么只能生成缺乏专家特异性的多样化分割，要么只能复制单个标注者的个性化输出，无法同时实现多样化和个性化。

Method: 提出ProSeg概率建模框架，引入两个潜在变量分别建模专家标注偏好和图像边界模糊性。通过变分推断获得这些变量的条件概率分布，然后从这些分布中采样生成分割输出。

Result: 在鼻咽癌数据集（NPC）和肺结节数据集（LIDC-IDRI）上的大量实验表明，ProSeg实现了新的最先进性能，提供了既多样化又专家个性化的分割结果。

Conclusion: ProSeg通过概率建模同时实现了多标注者医学图像分割的多样化和个性化，为解决医学图像分割中的数据不确定性提供了有效框架。
...
### [74] [Data-Centric Visual Development for Self-Driving Labs](https://arxiv.org/abs/2512.02018)
*Anbang Liu,Guanzhong Hu,Jiayi Wang,Ping Guo,Han Liu*

Main category: cs.CV

TL;DR: 本文提出了一种融合真实和虚拟数据生成的混合管道，用于解决自驱动实验室中气泡检测任务的数据稀缺问题，特别是负样本难以获取的挑战。


<details>
  <summary>Details</summary>
Motivation: 自驱动实验室需要高度鲁棒的模型，但训练这些模型需要大量标注数据，而在实际应用中特别是负样本难以获取。移液是SDL中最关键且精度要求最高的操作，因此需要解决数据稀缺问题。

Method: 构建混合管道：1）真实数据轨道采用人在回路方案，结合自动采集和选择性人工验证；2）虚拟数据轨道使用参考条件、提示引导的图像生成来增强真实数据，并经过筛选和验证确保可靠性。两者结合产生类别平衡的数据集用于气泡检测训练。

Result: 在保留的真实测试集上，仅使用自动采集的真实图像训练的模型达到99.6%准确率；混合真实和生成数据训练时保持99.4%准确率，同时减少了数据收集和审查的工作量。
...
### [75] [Charts Are Not Images: On the Challenges of Scientific Chart Editing](https://arxiv.org/abs/2512.00752)
*Shawn Li,Ryan Rossi,Sungchul Kim,Sunav Choudhary,Franck Dernoncourt,Puneet Mathur,Zhengzhong Tu,Yue Zhao*

Main category: cs.CV

TL;DR: FigEdit是一个用于科学图表编辑的大规模基准测试，包含30,000多个样本，覆盖10种图表类型和复杂编辑指令，揭示了现有生成模型在结构化图表编辑上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型（如扩散和自回归模型）在自然图像编辑上表现出色，但应用于科学图表时存在根本性不匹配：图表不是简单的像素排列，而是受图形语法约束的结构化数据可视化表示。图表编辑是结构化转换问题，而非像素操作任务。

Method: 引入FigEdit基准测试，基于真实世界数据构建，包含30,000多个样本，覆盖10种图表类型和丰富的复杂编辑指令。基准分为五个渐进挑战性任务：单次编辑、多次编辑、对话式编辑、视觉引导编辑和风格迁移。

Result: 评估多个最先进模型显示，它们在科学图表编辑上表现不佳，无法处理所需的结构化转换。传统评估指标（如SSIM、PSNR）在捕捉图表编辑的语义正确性方面存在局限性。

Conclusion: FigEdit基准揭示了像素级操作的深刻局限性，为开发和评估未来结构感知模型提供了坚实基础。通过发布该基准，旨在促进结构感知图表编辑的系统性进展，提供公平比较的共同基础，并鼓励理解科学图表视觉和语义层面的模型研究。
...
### [76] [Seeing the Wind from a Falling Leaf](https://arxiv.org/abs/2512.00762)
*Zhiyuan Gao,Jiageng Mao,Hong-Xing Yu,Haozhe Lou,Emily Yue-Ting Jia,Jernej Barbic,Jiajun Wu,Yue Wang*

Main category: cs.CV

TL;DR: 该论文提出了一种从视频中恢复不可见物理力的端到端可微分逆图形框架，通过联合建模物体几何、物理属性和相互作用，能够从物体运动中推断出力场表示。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉中长期目标是建模视频中的运动，但导致物体变形和运动的不可见物理相互作用（如风力）的表征在很大程度上尚未被探索。研究者希望从视觉观察中恢复这些不可见的力，例如通过观察叶子落地的过程来估计风场。

Method: 提出端到端可微分逆图形框架，联合建模物体几何、物理属性和相互作用，通过反向传播从物体运动中恢复力表征。该方法直接从视频中学习物理过程。

Result: 在合成和真实世界场景中验证了该方法，结果显示能够从视频中推断出合理的力场。该方法还展示了在基于物理的视频生成和编辑方面的潜在应用。

Conclusion: 该方法为理解和建模像素背后的物理过程提供了新的视角，有助于弥合视觉与物理之间的鸿沟，为物理感知的计算机视觉研究开辟了新方向。
...
### [77] [The Outline of Deception: Physical Adversarial Attacks on Traffic Signs Using Edge Patches](https://arxiv.org/abs/2512.00765)
*Haojie Jia,Te Hu,Haowen Li,Long Jin,Chongshi Xin,Yuchi Yao,Jiarui Xiao*

Main category: cs.CV

TL;DR: 本文提出TESP-Attack，一种针对交通标志分类的隐蔽对抗补丁方法，通过边缘对齐掩码和U-Net生成器，在保持高攻击成功率的同时实现视觉隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 智能驾驶系统易受交通标志物理对抗攻击，现有攻击方法缺乏隐蔽性，在标志中心区域添加扰动形成明显视觉模式，容易被人类观察者发现，限制了实际应用。

Method: 基于人类视觉注意力集中于交通标志中心区域的观察，采用实例分割生成边缘对齐掩码，使用U-Net生成器制作对抗补丁，通过颜色纹理约束和频域分析优化，实现与背景环境的无缝融合。

Result: 在不同架构的交通标志分类模型上均取得超过90%的攻击成功率，具有强跨模型可迁移性，在不同角度和距离下保持稳定的现实世界性能。

Conclusion: TESP-Attack方法在保持高攻击成功率的同时实现了视觉隐蔽性，解决了现有物理对抗攻击缺乏隐蔽性的问题，具有实际应用价值。
...
### [78] [PolarGS: Polarimetric Cues for Ambiguity-Free Gaussian Splatting with Accurate Geometry Recovery](https://arxiv.org/abs/2512.00794)
*Bo Guo,Sijia Wen,Yifan Zhao,Jia Li,Zhiming Zheng*

Main category: cs.CV

TL;DR: PolarGS：一种利用偏振光学先验解决3D高斯溅射中光度模糊问题的表面重建方法，通过偏振引导的光度校正和偏振增强的高斯致密化机制，在反射和无纹理区域实现更精确的几何重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射表面重建方法在光度模糊区域（如反射和无纹理表面）性能下降，因为不可靠的光度线索破坏了光度一致性并阻碍了准确的几何估计。偏振光部分偏振的特性可以揭示表面方向，为光度线索提供了光学补充。

Method: 提出PolarGS，包含两个互补模块：1）偏振引导的光度校正策略，通过线性偏振度识别反射区域，并使用颜色细化图优化反射高斯；2）偏振增强的高斯致密化机制，将角度和线性偏振度集成到基于PatchMatch的深度补全过程中，实现新高斯的反投影和融合。

Result: PolarGS是框架无关的，与最先进方法相比实现了更优越的几何精度。

Conclusion: 偏振作为光学先验能够有效解决光度模糊问题，PolarGS通过整合偏振信息显著提升了3D高斯溅射在反射和无纹理表面的重建准确性。
...
### [79] [CircleFlow: Flow-Guided Camera Blur Estimation using a Circle Grid Target](https://arxiv.org/abs/2512.00796)
*Jiajian He,Enjie Hu,Shiqi Chen,Tianchen Qiu,Huajun Feng,Zhihai Xu,Yueting Chen*

Main category: cs.CV

TL;DR: CircleFlow：基于光流引导边缘定位的高保真PSF估计框架，通过圆网格靶标和隐式神经表示实现精确模糊表征
...
Method: 1. 使用圆网格靶标进行结构化捕获，编码局部各向异性和空间变化的PSF；2. 利用靶标的二值亮度先验解耦图像和核估计；3. 通过光流引导的亚像素对齐重建潜在锐利图像；4. 将PSF建模为能量约束的隐式神经表示；5. 在去马赛克感知的可微分框架中联合优化两个组件。

Result: 在模拟和真实世界数据上的大量实验表明，CircleFlow实现了最先进的准确性和可靠性，验证了其在实际PSF校准中的有效性。

Conclusion: CircleFlow通过精确的边缘定位实现了物理一致且稳健的PSF估计，为实际PSF校准提供了一种有效的解决方案。
...
### [80] [Thinking with Drafts: Speculative Temporal Reasoning for Efficient Long Video Understanding](https://arxiv.org/abs/2512.00805)
*Pengfei Hu,Meng Cao,Yingyao Wang,Yi Wang,Jiahua Dong,Jun Song,Yu Cheng,Bo Zheng,Xiaodan Liang*

Main category: cs.CV

TL;DR: SpecTemp是一个基于强化学习的推测性时序推理框架，通过解耦时序感知与推理的双模型设计，解决长视频理解中的效率瓶颈问题。
...
Result: 在多个视频理解基准测试中，SpecTemp不仅保持了有竞争力的准确率，而且相比现有thinking-with-frames方法显著加速了推理过程。
...
### [81] [IRPO: Boosting Image Restoration via Post-training GRPO](https://arxiv.org/abs/2512.00814)
*Haoxuan Xu. Yi Liu,Boyuan Jiang,Jinlong Peng,Donghao Luo,Xiaobin Hu,Shuicheng Yan,Haoang Li*

Main category: cs.CV

TL;DR: IRPO是一种基于GRPO的低层视觉后训练范式，通过数据筛选和奖励建模解决图像恢复中的过平滑和泛化问题，在多个基准测试中取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 现有图像恢复方法依赖像素级硬拟合真实图像，存在过平滑和泛化能力差的问题，而现有的后训练范式在高层生成任务中成功但在低层视觉领域探索不足

Method: 提出IRPO低层GRPO后训练范式：1) 数据筛选原则：选择预训练阶段表现不佳的样本；2) 奖励建模系统：包含通用奖励（结构保真）、专家奖励（使用Qwen-VL进行感知对齐）和恢复奖励（任务特定低层质量）

Result: 在6个域内和5个域外低层基准测试中取得最先进结果，相比AdaIR基线在域内任务提升0.83dB，在域外设置提升3.43dB

Conclusion: IRPO通过系统性的数据筛选和奖励建模，有效解决了低层视觉后训练中的关键挑战，为图像恢复任务提供了新的解决方案
...
### [82] [PanFlow: Decoupled Motion Control for Panoramic Video Generation](https://arxiv.org/abs/2512.00832)
*Cheng Zhang,Hanwen Liang,Donny Y. Chen,Qianyi Wu,Konstantinos N. Plataniotis,Camilo Cruz Gambardella,Jianfei Cai*

Main category: cs.CV

TL;DR: PanFlow：一种利用全景球面特性解耦相机旋转与光流条件的新方法，实现对大幅动态运动的精确控制，并引入球面噪声扭曲策略保证边界一致性。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频生成方法缺乏明确的运动控制能力，难以生成具有大幅复杂运动的场景，需要更精确的运动控制方案。

Method: 1. 利用全景的球面特性，将高度动态的相机旋转从输入光流条件中解耦；2. 引入球面噪声扭曲策略，促进全景边界上的运动循环一致性；3. 构建大规模、运动丰富的全景视频数据集，包含帧级姿态和光流标注。

Result: PanFlow在运动保真度、视觉质量和时间一致性方面显著优于现有方法，并在运动迁移和视频编辑等应用中展示了有效性。

Conclusion: PanFlow通过解耦相机旋转和引入球面噪声扭曲，实现了对全景视频中大幅动态运动的精确控制，为全景视频生成提供了有效的解决方案。
...
### [83] [AFRAgent : An Adaptive Feature Renormalization Based High Resolution Aware GUI agent](https://arxiv.org/abs/2512.00846)
*Neeraj Anand,Rishabh Jain,Sohan Patnaik,Balaji Krishnamurthy,Mausoom Sarkar*

Main category: cs.CV

TL;DR: AFRAgent是一种基于instruct-BLIP的多模态架构，通过自适应特征重归一化技术增强图像嵌入，在移动UI自动化任务中实现最先进性能，模型尺寸仅为最接近竞争对手的四分之一。


<details>
  <summary>Details</summary>
Motivation: 移动UI自动化需求增长，现有视觉语言模型在GUI自动化中存在两个主要问题：1) 视觉编码器特征空间信息有限，导致小部件识别和动作确定不准确；2) 高性能模型通常体积庞大，需要大量训练且推理延迟高。

Method: 提出AFRAgent架构，基于instruct-BLIP多模态模型，引入自适应特征重归一化技术（基于token级别的仿射变换），有效丰富低分辨率图像嵌入并融合高分辨率细节。

Result: 在Meta-GUI和AITW基准测试中建立了新的最先进基线，性能优于现有方法，同时模型尺寸小于最接近竞争对手的四分之一。

Conclusion: AFRAgent通过创新的自适应特征重归一化技术解决了现有VLM在GUI自动化中的空间信息不足问题，实现了高性能和小模型尺寸的平衡，为智能手机自动化提供了新的解决方案。
...
### [84] [Smol-GS: Compact Representations for Abstract 3D Gaussian Splatting](https://arxiv.org/abs/2512.00850)
*Haishan Wang,Mohammad Hassan Vali,Arno Solin*

Main category: cs.CV

TL;DR: Smol-GS是一种用于3D高斯泼溅的紧凑表示学习方法，通过递归体素层次结构和特征编码实现高压缩比，同时保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)需要大量存储空间，限制了其实际应用。需要开发既能保持高质量渲染又能大幅压缩场景表示的方法。

Method: 采用递归体素层次结构捕获泼溅坐标，同时使用泼溅级特征存储颜色、透明度、变换和材质属性等抽象信息，将空间和语义信息集成到高效编码中。

Result: 在标准基准测试中实现了最先进的压缩效果，同时保持高渲染质量，能够将3D场景压缩数个数量级而不损失灵活性。

Conclusion: Smol-GS为3D场景提供了高效的紧凑表示，不仅具有视觉保真度，还可作为导航、规划和更广泛3D场景理解等下游任务的基础。
...
### [85] [TAP-CT: 3D Task-Agnostic Pretraining of Computed Tomography Foundation Models](https://arxiv.org/abs/2512.00872)
*Tim Veenboer,George Yiasemis,Eric Marcus,Vivien Van Veldhuizen,Cees G. M. Snoek,Jonas Teuwen,Kevin B. W. Groot Lipman*

Main category: cs.CV

TL;DR: TAP-CT：一种针对3D CT数据的任务无关基础模型，通过改进Vision Transformer和DINOv2实现可扩展的自监督预训练，无需大量微调即可获得强大的特征表示。


<details>
  <summary>Details</summary>
Motivation: 现有医学领域基础模型通常需要大量微调或依赖资源密集的解码器训练，且许多编码器的预训练目标偏向特定任务。需要一种强大、任务无关的基础模型，在特征提取之外只需最小化微调。

Method: 提出TAP-CT套件：简单而有效地将Vision Transformers和DINOv2适配到体数据，直接在3D CT体数据上进行可扩展的自监督预训练。包含针对性的改进：补丁嵌入、位置编码和体数据增强，使架构具有深度感知能力同时保持底层架构的简洁性。

Result: 在大型内部CT数据集（105K体数据）上进行大规模3D预训练，获得了稳定、鲁棒的冻结表示，在下游任务中表现出强大的泛化能力。

Conclusion: TAP-CT为医学影像研究提供了一个强大、低资源的基础模型基准，所有预训练模型、实验配置和下游基准代码将公开以促进透明度和可复现性。
...
TL;DR: 提出"Look, Recite, Then Answer"框架，通过三阶段推理解决视觉语言模型在专业领域中的幻觉问题，显著提升农业杂草识别准确率


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在专业领域（如精准农业）存在性能瓶颈，主要原因是"推理驱动幻觉"——语言先验覆盖视觉感知，以及"模态鸿沟"——视觉嵌入无法有效激活模型中已编码的专家知识

Method: 提出参数高效的三阶段框架：1) Look阶段生成客观视觉描述和候选集；2) Recite阶段使用轻量级1.7B路由器将视觉线索转化为针对性查询，触发候选特定参数知识；3) Answer阶段并行对齐描述与知识，选择最一致的标签

Result: 在AgroBench上取得最先进结果，杂草识别准确率比Qwen2-VL-72B提高23.52%，超越GPT-4o且无需外部搜索开销

Conclusion: 该模块化设计通过将被动感知转化为主动可控的知识检索，有效缓解幻觉问题，为专业领域视觉语言模型应用提供了高效解决方案
...
TL;DR: HanDyVQA是一个细粒度的手-物交互视频问答基准，包含6种问题类型和11.1K个多选题对，用于评估模型对手-物交互动态的时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互基准要么关注操作本身，要么关注粗粒度的效果，缺乏对HOI动态的细粒度时空推理能力评估。需要一个新的基准来全面覆盖操作和效果两个方面。

Method: 构建HanDyVQA基准，包含6种互补问题类型（动作、过程、物体、位置、状态变化、物体部件），共11.1K个多选题对。还包含10.3K个分割掩码用于物体和部件级推理评估。

Result: 评估最新视频基础模型发现，即使表现最佳的Gemini-2.5-Pro模型平均准确率也只有73%，远低于人类表现（97%）。分析显示模型在空间关系、运动和部件级几何理解方面仍面临挑战。

Conclusion: HanDyVQA基准为评估手-物交互动态理解提供了全面框架。研究发现将显式HOI相关线索整合到视觉特征中可以提升性能，为开发具有更深层次HOI动态理解能力的未来模型提供了见解。
...
TL;DR: 提出首个免训练多语言遥感图像描述方法，基于检索增强提示，在四个基准数据集上验证了与全监督英语系统相当的性能，并支持十种语言。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像描述方法依赖大规模标注数据集且主要针对英语，限制了全球适用性。需要开发免训练、多语言的解决方案。

Method: 提出基于检索增强提示的免训练方法：1) 使用领域适应的SigLIP2编码器检索相关描述和少样本示例；2) 探索两种变体：仅文本提示的多语言LLM和联合处理图像与提示的VLM；3) 引入基于PageRank的图重排序策略提升检索内容连贯性。

Result: 在四个基准数据集和十种语言上的实验表明：1) 方法性能与全监督英语系统相当；2) PageRank重排序带来高达35%的性能提升；3) VLM生成视觉基础但词汇多样的描述，LLM在BLEU和CIDEr得分上更强；4) 直接生成目标语言描述优于翻译策略。

Conclusion: 该工作首次系统评估了遥感图像的多语言免训练描述方法，推动了更具包容性和可扩展性的多模态地球观测系统发展。
...
TL;DR: STC框架通过缓存相似帧特征和剪枝冗余视觉标记，显著提升流式视频大语言模型的推理效率，在保持高精度的同时大幅降低延迟。


<details>
  <summary>Details</summary>
Motivation: 流式视频大语言模型在实时部署中面临高计算成本挑战，主要瓶颈在于ViT编码阶段对时间相似帧的冗余处理，以及LLM预填充阶段过长的标记序列导致的延迟和内存开销。

Method: 提出STC分层框架，包含两个标记级加速器：STC-Cacher通过缓存和重用时间相似帧的特征来减少ViT编码开销；STC-Pruner在视觉标记进入LLM前压缩序列，基于空间和时间相关性保留最显著标记。

Result: 在4个基线流式VideoLLM和5个基准测试上的实验表明，STC优于其他压缩方法。在ReKV框架上保持99%准确率的同时，将ViT编码延迟降低24.5%，LLM预填充延迟降低45.3%。

Conclusion: STC是一个即插即用的分层框架，可无缝集成到现有流式VideoLLM中，通过优化ViT编码和LLM预填充阶段，在保持高精度的同时显著加速处理，解决了流式视频场景中的计算效率瓶颈问题。
...
TL;DR: CAE是一种无需训练的图像聚类方法，通过结合caption级描述和noun级概念两种互补的文本语义，使用最优传输对齐图像特征与文本语义，提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法利用名词作为外部语义知识来改进图像聚类，但忽视了名词固有的歧义性，这会扭曲语义表示并降低聚类质量。
...
Result: 在8个数据集上的广泛实验证明了方法的有效性，在ImageNet-1K数据集上，准确率比最先进的无需训练方法提高了4.2%，调整兰德指数(ARI)提高了2.9%。

Conclusion: CAE通过分层语义对齐有效解决了名词歧义问题，在无需训练的情况下显著提升了图像聚类性能，证明了结合caption级细粒度属性和noun级高层类别概念的有效性。
...
TL;DR: 该研究开发了一个深度学习流程，使用2D微CT切片对12种有孔虫进行自动分类，最终集成模型达到95.64%的测试准确率，并开发了交互式仪表板用于实际部署。


<details>
  <summary>Details</summary>
Motivation: 解决有孔虫分类的传统人工方法效率低、主观性强的问题，利用深度学习技术实现自动化、高精度的有孔虫物种识别，为微体古生物学研究提供AI辅助工具。

Method: 1. 构建包含97个微CT扫描标本（27个物种）的数据集，筛选出12个代表性物种；2. 采用标本级数据分割，获得109,617个高质量2D切片；3. 评估7种最先进的2D CNN架构；4. 构建ConvNeXt-Large和EfficientNetV2-Small的集成模型；5. 开发支持实时分类和3D切片匹配的交互式仪表板。

Result: 集成模型在测试集上达到95.64%的准确率，top-3准确率为99.6%，所有物种的ROC曲线下面积（AUC）为0.998，建立了有孔虫分类的新基准。

Conclusion: 该研究为AI辅助的微体古生物学鉴定建立了新标准，提供了完全可复现的有孔虫分类框架，弥合了深度学习与应用地球科学之间的差距，开发的实际部署工具具有重要应用价值。
...
TL;DR: SceneProp将场景图定位重新表述为马尔可夫随机场中的最大后验概率推断问题，通过全局推理优化图像区域到节点的分配，解决了传统方法在处理复杂关系查询时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理包含多个对象和关系的复杂组合查询时存在困难。传统的短语定位方法擅长定位单个对象，但缺乏解析复杂关系描述的结构归纳偏置。现有的场景图定位方法也存在问题：随着查询图增大，性能反而下降，未能利用本应使定位更容易的信息。

Method: 提出SceneProp方法，将场景图定位重新表述为马尔可夫随机场中的最大后验概率推断问题。通过可微分的置信传播算法实现端到端框架，对整个查询图进行全局推理，找到同时满足所有约束的图像区域到节点的最优分配。

Result: 在四个基准测试上的实验表明，SceneProp显著优于先前工作。关键的是，其准确性随着查询图的大小和复杂性而持续提高，首次证明更多的关系上下文可以且应该带来更好的定位效果。

Conclusion: SceneProp通过将场景图定位重新表述为MRF中的MAP推断问题，解决了现有方法在处理复杂关系查询时的性能下降问题。该方法不仅性能优越，而且随着查询复杂性增加而性能提升，为复杂视觉查询的定位提供了新的解决方案。
...
TL;DR: 本文提出了一种用于3D高斯泼溅分割的粗到细二进制编码方案，通过二进制到十进制映射将每个特征压缩为单个整数，大幅减少内存使用，并采用渐进式训练策略提升细粒度分割能力。
...
Method: 提出粗到细二进制编码方案压缩每个高斯点的类别表示；设计渐进式训练策略将全景分割分解为独立子任务；在分割训练中微调不透明度以解决光度渲染与语义分割的不兼容问题。

Result: 在多个基准测试中实现了最先进的分割性能，同时显著减少内存消耗并加速推理。

Conclusion: 该方法通过高效的二进制编码和渐进训练策略，解决了3D-GS分割中的内存开销和细粒度分割挑战，实现了高性能、低内存消耗的3D场景分割。
...
TL;DR: 提出DEMR框架，通过反偏证据学习改进视频时刻检索，解决模态不平衡和不确定性估计偏差问题，在标准数据集和去偏数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统方法在视频时刻检索中存在局限性：预训练模型难以处理细粒度信息和确定性推理，无法有效对齐复杂或模糊的时刻。深度证据回归（DER）构建的基线存在模态不平衡处理不足和启发式不确定性正则化结构差异问题，导致高不确定性错误地与准确样本而非挑战性样本关联。

Method: 提出反偏证据学习时刻检索（DEMR）框架：1）引入反射翻转融合（RFF）块实现跨模态对齐；2）添加查询重构任务增强文本敏感性；3）设计几何正则化器（Geom-regularizer）精炼不确定性预测，实现与困难时刻的自适应对齐。

Result: 在标准数据集和去偏数据集ActivityNet-CD、Charades-CD上进行广泛测试，显示出在有效性、鲁棒性和可解释性方面的显著提升，为时刻检索的时序语义鲁棒性提供了有前景的解决方案。

Conclusion: DEMR框架通过解决模态不平衡和不确定性估计偏差问题，显著改进了视频时刻检索的性能，特别是在复杂视频场景中表现出更好的适应性和准确性。
...
TL;DR: 提出4DHOISolver优化框架，利用稀疏人工标注接触点约束4D人-物交互重建，创建大规模Open4DHOI数据集，并展示其在机器人模仿学习中的应用价值


<details>
  <summary>Details</summary>
Motivation: 从单目互联网视频中准确、可扩展地提取4D交互数据是未解决的挑战，但这类视频提供了近乎无限的人类活动、物体和环境多样性数据源，对通用机器人学习至关重要

Method: 提出4DHOISolver优化框架，利用稀疏人工标注的接触点约束4D人-物交互重建问题，同时保持高时空一致性和物理合理性

Result: 创建了Open4DHOI大规模4D人-物交互数据集，包含144种物体类型和103种动作；通过RL智能体成功模仿重建的运动；现有3D基础模型在预测精确接触对应关系方面仍有不足

Conclusion: 4DHOISolver框架能有效从野外视频重建4D交互数据，创建的数据集对机器人学习有价值；精确接触对应关系预测仍是未解决问题，需要人工参与策略，并向社区提出开放挑战
...
<details>
  <summary>Details</summary>
Motivation: 许多普通用户在拍照时难以获得良好构图，需要专业的构图指导来帮助他们改进摄影技巧。

Method: 1. 构建大规模数据集，将构图指导分为平移、放大和视角变换三个子任务；2. 平移和放大数据来自现有裁剪数据集；3. 视角变换数据通过两阶段流程获取：从多视角数据集中采样不同视角的图像对，训练退化模型将良好构图照片转换为构图不佳的照片，然后将该模型应用于专业照片以合成训练对；4. 微调能够联合处理文本和图像的模型。

Result: 实验表明，文本指令能有效引导图像构图，将文本指导与示例图像结合比仅使用示例的基线方法有持续改进。

Conclusion: PhotoFramer为构图助手提供了实用的一步，使普通用户能够获得专业的摄影先验知识，有助于提升日常用户的摄影构图能力。
...
TL;DR: S2AM3D：一种结合2D分割先验与3D一致性监督的点云部件分割方法，通过点一致性编码器和尺度感知提示解码器解决现有方法泛化性差和视图不一致问题，并构建了大规模高质量数据集。


<details>
  <summary>Details</summary>
Motivation: 现有部件级点云分割方法面临两大挑战：原生3D模型因数据稀缺而缺乏泛化能力，而引入2D预训练知识常导致不同视图间的分割结果不一致。

Method: 提出S2AM3D框架，包含点一致性部件编码器（通过原生3D对比学习聚合多视图2D特征）和尺度感知提示解码器（通过连续尺度信号实时调整分割粒度），同时构建了超过10万个样本的大规模高质量部件级点云数据集。

Result: 在多个评估设置中取得领先性能，在处理复杂结构和尺寸变化显著的部件时表现出卓越的鲁棒性和可控性。

Conclusion: S2AM3D通过有效结合2D分割先验与3D一致性监督，成功解决了部件级点云分割中的泛化性和视图一致性问题，为3D视觉领域提供了新的解决方案。
...
TL;DR: 提出语义感知的医学图像重建框架，结合区块链溯源层，在恢复图像时保持临床相关结构并确保可验证性


<details>
  <summary>Details</summary>
Motivation: 现实世界医学图像常受损坏、噪声和篡改影响，传统重建方法可能产生视觉合理但解剖保真度不足的结果，直接影响临床诊断可靠性

Method: 1) 语义感知医学图像重建框架：整合高级潜在嵌入与混合U-Net架构，在恢复过程中保持临床相关结构；2) 轻量级区块链溯源层：采用无标度图设计，记录每个重建事件的可验证信息，不增加显著开销

Result: 在多个数据集和损坏类型上的广泛评估显示，相比现有方法，该方法在结构一致性、恢复准确性和溯源完整性方面均有改进
...
TL;DR: LISA-3D是一个两阶段框架，通过改造LISA语言图像分割模型并重用冻结的SAM-3D重建器，将语言引导的2D分割提升到3D重建。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的3D重建需要能够理解开放词汇指令并在不同视角下保持一致的掩码生成器。现有方法在跨视角一致性和语言理解能力方面存在局限。

Method: 采用两阶段框架：1) 使用几何感知的LoRA层改造LISA模型，通过可微分重投影损失强制跨视角一致性；2) 将生成的掩码与RGB图像拼接成RGBA提示，输入冻结的SAM-3D重建器生成高斯溅射或纹理网格。

Result: 在ScanRefer和Nr3D数据集上，LISA-3D将语言到3D的准确率提高了最多+15.6个百分点，仅需适应1160万个参数，支持零样本部署到未见类别。

Conclusion: LISA-3D提供了一个模块化、数据高效且实用的语言引导3D内容创建方案，通过结合语言理解和几何一致性，显著提升了文本驱动3D重建的性能。
...
TL;DR: Lotus-2是一个两阶段确定性框架，利用预训练扩散模型作为世界先验，通过少量数据实现单目深度估计和表面法线预测的SOTA性能


<details>
  <summary>Details</summary>
Motivation: 从单张图像恢复像素级几何属性本质上是病态问题。现有方法要么依赖大规模监督数据但缺乏物理推理，要么直接使用扩散模型的随机生成公式不适合确定性几何推理。需要一种优化方案来充分利用预训练生成先验进行稳定准确的几何预测。

Method: 提出两阶段确定性框架：第一阶段使用单步确定性公式配合清洁数据目标和轻量级局部连续性模块生成全局一致结构；第二阶段在核心预测器定义的流形内进行约束多步整流流细化，通过无噪声确定性流匹配增强细粒度几何细节。

Result: 仅使用59K训练样本（不到现有大规模数据集的1%），在单目深度估计上达到新的SOTA结果，在表面法线预测上获得高度竞争力表现。

Conclusion: 扩散模型可以作为确定性世界先验，实现超越传统判别式和生成式范式的高质量几何推理，为几何密集预测提供了稳定、准确和细粒度的解决方案。
...
TL;DR: TRoVe：一种自动发现时序视觉语言模型中错误诱导静态特征偏见的框架，通过评估特征对分类错误的影响和模型对特征的依赖程度来识别偏见，在101个模型上验证效果优于基线28.6%。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理时序理解任务时可能依赖静态特征偏见（如背景或物体特征）而非动态视觉变化，这些偏见会导致下游任务的系统性预测错误。在模型部署前识别和表征这些错误诱导的静态特征偏见至关重要。

Method: TRoVe从带标注的验证数据集中提取候选静态特征，通过两个维度评分：(1) 特征对分类错误的影响程度；(2) 模型在预测时对该特征的依赖程度。结合这两个评分来识别错误诱导的静态特征偏见。

Result: 在包含101个训练好的时序视觉语言模型及其真实标注偏见的评估框架上，TRoVe能准确识别错误诱导的静态特征偏见，相比最接近的基线方法提升了28.6%。应用于7个现成模型和2个时序理解任务时，发现了先前未知的静态特征偏见，并证明了解偏见知识有助于提升测试时模型性能。

Conclusion: TRoVe提供了一种有效的自动化方法来发现视觉语言模型中的静态特征偏见，有助于在模型部署前识别潜在的系统性错误来源，并为模型改进提供指导。
...
TL;DR: 本文研究了ViT-B/16在ImageNet-1K上的参数效率，发现通过两种简单的MLP参数减少策略（GroupedMLP和ShallowMLP）可以减少32.7%的参数，同时保持或略微提升性能，并显著改善训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 尽管缩放定律和许多实证结果表明增大Vision Transformers通常能提升性能，但模型准确率和训练行为并不总是随规模单调增加。本文旨在探索ViT-B/16在ImageNet-1K上是否处于过参数化状态，以及如何通过更有效的参数分配来优化模型。

Method: 提出了两种MLP参数减少策略：1) GroupedMLP：在相邻transformer块之间共享MLP权重；2) ShallowMLP：将MLP隐藏维度减半。两种方法都减少了32.7%的基线参数，应用于ViT-B/16模型。

Result: GroupedMLP达到81.47% top-1准确率（基线81.05%），保持基线计算成本；ShallowMLP达到81.25%准确率，推理吞吐量提升38%。两种模型都显著改善了训练稳定性，将峰值到最终准确率下降从0.47%减少到0.03%-0.06%。

Conclusion: 对于使用标准训练配方的ViT-B/16在ImageNet-1K上，模型处于过参数化状态，可以减少MLP容量而不损害性能，甚至能略微提升。参数共享和减少宽度等架构约束可以作为有用的归纳偏置，参数分配方式在Vision Transformer设计中很重要。
...
Method: 提出Speed-RL范式，将加速问题视为强化学习问题。结合质量奖励和速度奖励，使用强化学习对基础模型进行微调，优化目标是组合奖励。

Result: 通过大量实验证明，该方法能够将基础模型加速3倍，同时保持可比的图像质量。

Conclusion: Speed-RL提供了一种新颖的强化学习范式来加速预训练的掩码生成模型，在保持生成质量的同时显著提高推理速度。
...
### [104] [CycliST: A Video Language Model Benchmark for Reasoning on Cyclical State Transitions](https://arxiv.org/abs/2512.01095)
*Simon Kohaut,Daniel Ochs,Shun Zhang,Benedict Flade,Julian Eggert,Kristian Kersting,Devendra Singh Dhami*

Main category: cs.CV

TL;DR: CycliST是一个评估视频语言模型在周期性状态转换中文本推理能力的新基准数据集，通过合成视频序列测试模型对周期性模式的识别和理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频语言模型在理解周期性动态变化方面存在局限，缺乏对时间概念的理解和从场景中提取定量洞察的能力，需要专门的评估工具来推动技术进步。

Method: 创建包含周期性物体运动和视觉属性变化的合成视频序列，采用分层评估系统，通过增加循环物体数量、场景杂乱度和光照条件变化来逐步提高难度。

Result: 实验显示当前最先进的视频语言模型（包括开源和专有模型）在泛化到线性/轨道运动等周期性动态以及颜色/尺度等时间依赖性视觉属性变化方面存在显著局限，没有单一模型在所有任务中表现一致优秀。

Conclusion: CycliST基准数据集揭示了视频语言模型在周期性模式理解方面的技术差距，为开发超越当前技术水平、能够理解周期性模式的视觉推理模型提供了有针对性的挑战和全面评估框架。
...
### [105] [Structural Prognostic Event Modeling for Multimodal Cancer Survival Analysis](https://arxiv.org/abs/2512.01116)
*Yilan Zhang,Li Nanbo,Changchun Yang,Jürgen Schmidhuber,Xin Gao*

Main category: cs.CV

TL;DR: SlotSPE：基于槽位的结构预后事件建模框架，通过槽注意力将多模态输入压缩为紧凑、互异的槽表示，有效建模模态内外交互，提升癌症生存预测性能


<details>
  <summary>Details</summary>
Motivation: 当前方法难以高效建模高维复杂多模态数据中的模态内外交互，且难以捕捉稀疏、患者特异性、无标注的关键预后事件（如空间组织学模式或通路共激活），这些事件虽然稀少但决定患者预后

Method: 提出SlotSPE框架，受因子编码原理启发，使用槽注意力将每位患者的多模态输入压缩为紧凑、互异的模态特异性槽集合，将这些槽表示作为预后事件的编码，实现复杂模态交互的高效建模，并可无缝整合增强预后相关性的生物学先验

Result: 在十个癌症基准测试中，SlotSPE在8/10队列中优于现有方法，整体提升2.9%，在基因组数据缺失情况下保持稳健，通过结构化事件分解显著提升可解释性

Conclusion: SlotSPE通过槽位表示有效建模多模态数据中的结构预后事件，在生存预测性能、鲁棒性和可解释性方面均优于现有方法，为癌症预后分析提供了新框架
...
### [106] [OmniFD: A Unified Model for Versatile Face Forgery Detection](https://arxiv.org/abs/2512.01128)
*Haotian Liu,Haoyu Chen,Chenhui Pan,You Hu,Guoying Zhao,Xiaobai Li*

Main category: cs.CV

TL;DR: OmniFD是一个统一的框架，在单个模型中同时处理图像/视频分类、空间定位和时间定位四个人脸伪造检测核心任务，通过共享编码器、跨任务交互模块和轻量解码头实现高效多任务学习。


<details>
  <summary>Details</summary>
Motivation: 当前人脸伪造检测方法通常采用任务特定的独立模型，导致计算冗余且忽略了相关任务之间的潜在关联，需要一种统一框架来同时处理多个核心任务。

Method: OmniFD包含三个主要组件：1)共享Swin Transformer编码器提取统一的4D时空表示；2)跨任务交互模块通过可学习查询和注意力机制动态捕获任务间依赖关系；3)轻量解码头将精炼表示转换为各任务的预测结果。

Result: 实验表明OmniFD优于任务特定模型，通过多任务学习捕获跨任务的通用表示，特别是细粒度知识迁移能提升其他任务性能（如视频分类准确率提升4.63%）。统一框架在多个基准测试中表现出色，减少63%参数和50%训练时间。

Conclusion: OmniFD为现实应用中全面的人脸伪造检测提供了实用且可泛化的解决方案，通过统一图像、视频和四个任务实现高效、可扩展的检测框架。
...
TL;DR: 本文提出SocialFusion框架，解决视觉语言模型在社交感知任务中的负迁移问题，通过最小化连接冻结视觉编码器和语言模型，实现跨五个社交任务的正向迁移和媲美任务专用SOTA模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前预训练的视觉语言模型虽然具有强大的通用能力，但在同时学习多个社交感知任务时表现出负迁移问题。研究发现这是由于"社交退化"现象导致的，即通用的视觉语言预训练过程损害了视觉编码器表示细微社交信息的能力。

Method: 提出SocialFusion统一框架，通过线性表示探测和梯度冲突分析两个视角研究社交退化问题，然后设计最小化连接冻结视觉编码器和语言模型的方法，避免预训练过程中的社交信息损失。

Result: SocialFusion在五个社交任务上表现出正向迁移，利用任务间的协同效应提升整体性能，在各种基准测试中达到与任务专用SOTA模型相当的性能水平。

Conclusion: 当前VLM预训练策略可能对获得通用社交能力有害，需要更注重社交感知的训练范式。SocialFusion通过最小化连接方法有效解决了社交退化问题，为构建社交能力强的AI系统提供了新思路。
...
### [108] [DPAC: Distribution-Preserving Adversarial Control for Diffusion Sampling](https://arxiv.org/abs/2512.01153)
...
<details>
  <summary>Details</summary>
Motivation: 传统对抗引导的扩散采样虽然能达到目标类别，但样本质量会随着对抗控制轨迹与名义轨迹偏差的累积而下降。需要一种既能保持分类效果又能最小化分布漂移的方法。

Method: 基于随机最优控制视角，将轨迹偏差形式化为路径KL散度，推导出最优控制条件：在产生相同分类增益的方向中，与对数密度等值面相切的分量最小化路径KL散度。提出DPAC方法，将对抗梯度投影到生成分数几何定义的切空间。

Result: 理论分析表明DPAC在离散求解器中能将Wasserstein距离的误差项从O(Δt)降低到O(Δt²)，对分数或度量近似保持二阶鲁棒性。ImageNet-100实验验证了DPAC在相同攻击成功率下获得更低的FID和估计路径KL散度。

Conclusion: 通过将对抗控制与生成分数几何对齐，DPAC在保持分类性能的同时显著提升样本质量，为扩散模型的对抗引导提供了理论保证和实用方法。
...
### [109] [VSRD++: Autolabeling for 3D Object Detection via Instance-Aware Volumetric Silhouette Rendering](https://arxiv.org/abs/2512.01178)
*Zihua Liu,Hiroki Sakuma,Masatoshi Okutomi*

Main category: cs.CV

TL;DR: VSRD++是一种弱监督单目3D目标检测框架，无需3D标注，通过神经场体渲染和2D监督实现多视图自动标注和单目检测器训练


<details>
  <summary>Details</summary>
Motivation: 现有单目3D目标检测方法严重依赖需要大量人工标注的3D监督数据，标注成本高昂。为了解决这个问题，作者提出了一种弱监督框架，消除对3D标注的依赖

Method: 采用两阶段流程：1）多视图3D自动标注阶段，使用符号距离场表示物体表面，通过实例感知体素轮廓渲染生成实例掩码，将SDF分解为立方体SDF和残差距离场来优化3D边界框；2）单目3D检测器训练阶段，使用优化的3D边界框作为伪标签训练单目检测器。针对动态物体，引入速度属性和置信度机制

Result: 在KITTI-360数据集上的大量实验表明，VSRD++在静态和动态场景中都显著优于现有的弱监督单目3D目标检测方法

Conclusion: VSRD++成功实现了无需3D标注的弱监督单目3D目标检测，通过神经场体渲染和创新的SDF分解方法有效解决了动态物体的几何不一致性问题，为3D场景理解提供了更实用的解决方案
...
### [110] [TabletopGen: Instance-Level Interactive 3D Tabletop Scene Generation from Text or Single Image](https://arxiv.org/abs/2512.01204)
*Ziqian Wang,Yonghao He,Licheng Yang,Wei Zou,Hongxuan Ma,Liu Liu,Wei Sui,Yuxin Guo,Hu Su*

Main category: cs.CV

TL;DR: TabletopGen：一个无需训练、全自动的框架，用于生成多样化的、实例级别的交互式3D桌面场景，通过参考图像输入实现高保真、物理合理的场景生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本或图像的3D场景生成方法主要关注大规模场景，难以捕捉桌面场景的高密度布局和复杂空间关系，而高质量的3D桌面场景对于具身AI（特别是机器人操作策略学习和数据合成）至关重要。

Method: TabletopGen接受参考图像输入（可通过文本到图像模型合成增强多样性），进行实例分割和补全获取每个实例的图像，将每个实例重建为3D模型并进行规范坐标对齐，然后通过姿态和尺度估计，最后组装成无碰撞、可模拟的桌面场景。核心创新是两阶段空间推理方法：可微分旋转优化器用于精确旋转恢复，顶视图空间对齐机制用于稳健的平移和尺度估计。

Result: 广泛的实验和用户研究表明，TabletopGen在视觉保真度、布局准确性和物理合理性方面达到了最先进的性能，显著超越了现有方法，能够生成具有丰富风格和空间多样性的逼真桌面场景。

Conclusion: TabletopGen是一个有效的训练免费框架，能够自动生成高质量的交互式3D桌面场景，为具身AI应用提供了重要的场景生成能力，代码将公开可用。
...
### [111] [Closing the Approximation Gap of Partial AUC Optimization: A Tale of Two Formulations](https://arxiv.org/abs/2512.01213)
*Yangbangyan Jiang,Qianqian Xu,Huiyang Shao,Zhiyong Yang,Shilong Bao,Xiaochun Cao,Qingming Huang*

Main category: cs.CV

TL;DR: 该论文提出了两种基于极小极大重构的PAUC优化方法，解决了现有方法中近似误差不可控和可扩展性有限的问题，实现了线性计算复杂度和收敛保证。


<details>
  <summary>Details</summary>
Motivation: PAUC（部分AUC）是ROC曲线中特定FPR/TPR范围内的评估指标，在类别不平衡和决策约束的实际场景中至关重要。然而，现有PAUC优化方法存在近似误差不可控或可扩展性有限的问题。

Method: 提出了两种实例级极小极大重构方法：一种具有渐近消失的近似误差，另一种通过增加变量实现无偏估计。核心思想包括建立等效的实例级问题降低时间复杂度，通过阈值学习简化样本选择过程，并应用不同的平滑技术。

Result: 算法具有线性迭代计算复杂度（相对于样本大小），典型单向和双向PAUC的收敛率为O(ε^{-1/3})。提供了紧致的泛化边界，明确展示了TPR/FPR约束α/β对泛化的影响，表现为$\tilde{O}(α^{-1}n_+^{-1} + β^{-1}n_-^{-1})$的尖锐阶数。在多个基准数据集上的实验验证了方法的有效性。

Conclusion: 该研究通过实例级极小极大重构方法填补了PAUC优化的近似误差空白，实现了高效、可扩展且具有理论保证的PAUC优化算法，为实际应用中类别不平衡和决策约束场景提供了有效的评估工具。
...
TL;DR: M4-BLIP框架通过结合BLIP-2的局部特征提取和面部先验知识，专注于检测多模态媒体中的局部篡改，特别是面部区域，并通过LLM增强结果可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态媒体篡改检测方法普遍忽视局部信息的重要性，而篡改往往发生在特定区域（尤其是面部），这影响了信息传播的可靠性和完整性。

Method: 提出M4-BLIP框架：1）使用BLIP-2模型提取局部特征；2）引入面部局部信息作为先验知识；3）设计对齐和融合模块整合局部与全局特征；4）集成大语言模型增强检测结果的可解释性。

Result: 通过大量定量和可视化实验验证，M4-BLIP框架在检测效果上优于当前最先进的竞争对手。

Conclusion: M4-BLIP框架通过关注局部特征（特别是面部区域）并结合大语言模型，有效提升了多模态媒体篡改检测的准确性和可解释性。
...
TL;DR: S²-MLLM提出了一种通过隐式空间推理增强多模态大语言模型3D视觉定位能力的高效框架，避免了传统方法依赖点云重建的低效问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖重建点云的视角相关渲染为MLLMs提供显式结构指导，导致效率低下且空间推理能力有限。MLLMs主要处理2D视觉输入，难以从有限视角理解3D场景的空间结构。

Method: 提出S²-MLLM框架，采用隐式空间推理策略：1）利用前馈3D重建的结构感知能力进行空间引导；2）设计结构增强模块，使用视图内和视图间注意力机制捕获依赖关系；3）集成多级位置编码关联视觉表示与空间位置和视角信息。

Result: 在ScanRefer、Nr3D和Sr3D数据集上的广泛实验表明，S²-MLLM在性能、泛化能力和效率方面均优于现有方法，实现了显著性能提升。

Conclusion: S²-MLLM通过隐式空间推理有效增强了MLLMs的3D空间理解能力，统一了优越性能、良好泛化性和高效率，为3D视觉定位任务提供了更优解决方案。
...
### [114] [PSR: Scaling Multi-Subject Personalized Image Generation with Pairwise Subject-Consistency Rewards](https://arxiv.org/abs/2512.01236)
*Shulei Wang,Longhui Wei,Xin He,Jianbo Ouyang,Hui Lu,Zhou Zhao,Qi Tian*

Main category: cs.CV

TL;DR: 该论文提出了一种用于多主体个性化图像生成的方法，通过构建高质量多主体数据集和设计成对主体一致性奖励，解决了现有模型在多主体场景下主体一致性和文本控制性下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有单主体个性化生成模型效果显著，但在扩展到多主体时性能下降，特别是在保持主体一致性和遵循文本提示方面。作者认为这是由于缺乏高质量多主体数据集和精细的后训练策略。

Method: 1) 提出可扩展的多主体数据生成管道，利用强大的单主体生成模型构建多样化的高质量多主体训练数据；2) 设计成对主体一致性奖励和通用奖励，通过强化学习阶段增强主体一致性和文本可控性；3) 引入包含7个子集的新基准来全面评估多主体个性化生成。

Result: 大量实验证明，该方法在推进多主体个性化图像生成方面具有有效性，能够显著提升多主体场景下的生成质量和可控性。

Conclusion: 通过构建高质量多主体数据集和设计专门的奖励机制，该方法成功解决了多主体个性化生成中的关键挑战，为多主体场景下的个性化图像生成提供了有效解决方案。
...
### [115] [TRivia: Self-supervised Fine-tuning of Vision-Language Models for Table Recognition](https://arxiv.org/abs/2512.01248)
...
TL;DR: TRivia是一种自监督微调方法，让预训练视觉语言模型直接从无标签表格图像学习表格识别，无需人工标注，性能超越现有系统。


<details>
  <summary>Details</summary>
Motivation: 表格识别传统依赖监督学习，需要大量标注数据成本高昂。开源模型因资源有限和隐私法规限制，性能远落后于专有模型，需要无监督方法来缩小差距。

Method: 基于Group Relative Policy Optimization，通过问答奖励机制自动识别最有效的无标签样本。注意力引导模块为每个表格图像生成多样化问题，模型通过正确解释识别结果并回答问题获得反馈，形成闭环优化过程。

Result: 开发了TRivia-3B模型，在三个流行基准测试中超越了现有系统（如Gemini 2.5 Pro、MinerU2.5），成为开源、紧凑且最先进的表格识别模型。

Conclusion: TRivia方法使表格识别模型能够自主学习识别、结构化和推理表格，无需标注数据，为开源社区提供了高性能的表格识别解决方案。
...
### [116] [ViscNet: Vision-Based In-line Viscometry for Fluid Mixing Process](https://arxiv.org/abs/2512.01268)
*Jongwon Sohn,Juhyeon Moon,Hyunjoon Jung,Jaewook Nam*

Main category: cs.CV

TL;DR: 基于计算机视觉的粘度计，通过分析混合过程中自由表面变形引起的光折射图案扭曲来测量粘度，无需接触被测流体


<details>
  <summary>Details</summary>
Motivation: 传统粘度计具有侵入性且需要受控实验室环境，与实际工艺条件差异大，需要一种非接触式、适用于实际工艺条件的粘度测量方法

Method: 利用计算机视觉技术，通过固定背景图案在混合驱动的连续变形自由表面光折射下的光学扭曲来推断粘度，采用多图案策略增强鲁棒性，并包含不确定性量化

Result: 在不同光照条件下，系统在log m2 s^-1单位中达到0.113的平均绝对误差，粘度等级预测准确率高达81%，多图案策略提高了鲁棒性

Conclusion: 这种非接触式粘度计为现有粘度测量方法提供了实用的、适合自动化的替代方案
...
### [117] [Supervised Contrastive Machine Unlearning of Background Bias in Sonar Image Classification with Fine-Grained Explainable AI](https://arxiv.org/abs/2512.01291)
*Kamal Basha S,Athira Nambiar*

Main category: cs.CV

TL;DR: 提出了一种用于声纳图像分析的新框架，通过针对性对比遗忘模块减少对海底特征的过度依赖，并结合解释性框架提供可视化洞察，提升模型的泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型在声纳图像分析中过度依赖海底特征，导致泛化能力差。需要解决模型对背景特征的过度依赖问题，同时提高模型的可解释性。

Method: 提出包含两个关键模块的框架：1) 针对性对比遗忘模块，扩展传统三元组损失以减少海底背景偏差；2) 遗忘解释声纳框架，提供模型故意遗忘内容的可视化洞察，并适配LIME解释器生成更忠实和局部化的归因用于遗忘评估。

Result: 在真实和合成声纳数据集上的大量实验验证了该方法，在遗忘效果、模型鲁棒性和可解释性方面均显示出显著改进。

Conclusion: 该框架通过减少对海底特征的过度依赖，有效提升了声纳图像分析模型的泛化能力和可解释性，为解决现有模型的背景偏差问题提供了新思路。
...
### [118] [Diffusion Model in Latent Space for Medical Image Segmentation Task](https://arxiv.org/abs/2512.01292)
*Huynh Trinh Ngoc,Toan Nguyen Hai,Ba Luong Son,Long Tran Quoc*

Main category: cs.CV

TL;DR: MedSegLatDiff：一种基于扩散的医学图像分割框架，结合VAE和潜在扩散模型，能高效生成多样化的分割假设和置信度图，在保持小结构的同时实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割方法只生成单一分割掩码，无法捕捉固有的不确定性。现有生成模型虽然能产生多个合理分割，但计算成本高。需要一种既能生成多样化分割假设，又能保持高效性的方法。

Method: 提出MedSegLatDiff框架，结合变分自编码器（VAE）和潜在扩散模型。VAE将输入压缩到低维潜在空间，减少噪声并加速训练；扩散过程直接在紧凑表示上操作。在VAE掩码重建路径中用加权交叉熵替代传统MSE损失，以更好地保留微小结构。

Result: 在ISIC-2018（皮肤病变）、CVC-Clinic（息肉）和LIDC-IDRI（肺结节）数据集上评估，实现了最先进或极具竞争力的Dice和IoU分数。同时能生成多样化的分割假设和置信度图。

Conclusion: MedSegLatDiff相比确定性基线提供更好的可解释性和可靠性，特别适合临床部署。框架能高效生成多样分割，同时保持对小结构的敏感性。
...
### [119] [EGG-Fusion: Efficient 3D Reconstruction with Geometry-aware Gaussian Surfel on the Fly](https://arxiv.org/abs/2512.01296)
*Xiaokun Pan,Zhenzhe Li,Zhichao Ye,Hongjia Zhai,Guofeng Zhang*

Main category: cs.CV

TL;DR: EGG-Fusion是一个实时3D重建系统，通过鲁棒的稀疏到稠密相机跟踪和几何感知的高斯面元映射模块，结合基于信息滤波的融合方法，显著提升了重建精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于可微分渲染的SLAM系统面临实时计算和传感器噪声敏感性的双重挑战，导致场景重建的几何保真度下降，实用性受限。

Method: 提出EGG-Fusion系统，包含鲁棒的稀疏到稠密相机跟踪模块和几何感知的高斯面元映射模块，采用基于信息滤波的融合方法显式处理传感器噪声，实现高精度表面重建。

Result: 在Replica和ScanNet++等标准基准数据集上达到0.6cm的表面重建误差，相比最先进的基于高斯的方法精度提升超过20%，同时保持24FPS的实时处理能力。

Conclusion: EGG-Fusion是目前最准确的基于可微分渲染的实时重建系统之一，在精度和实时性方面均取得了显著改进。
...
TL;DR: 本文提出了首个针对飞机油箱内异物碎片检测的FOD-S2R数据集，包含真实和合成图像，旨在解决封闭环境下检测数据缺乏的问题，并验证合成数据对提升真实世界检测性能的有效性。


<details>
  <summary>Details</summary>
Motivation: 飞机油箱内的异物碎片(FOD)会造成燃料污染、系统故障和维护成本增加等严重安全隐患，但目前缺乏专门针对油箱这种复杂封闭环境的检测数据集，现有数据集主要关注外部或开放环境。

Method: 创建了FOD-S2R数据集，包含3,114张在受控油箱复制品中拍摄的高清真实图像和3,137张使用Unreal Engine生成的合成图像，涵盖不同视场角、物体距离、光照条件、颜色和物体大小。使用多种最先进的目标检测模型进行基准测试，评估合成数据对真实世界检测性能的提升效果。

Result: 实验表明，引入合成数据能够提高检测精度并增强模型对真实世界条件的泛化能力，有效缩小了模拟到现实的差距，为开发自动化FOD检测系统提供了有价值的基础。

Conclusion: FOD-S2R数据集填补了封闭环境下FOD检测数据集的空白，证明了合成数据在提升模型性能和缩小Sim2Real差距方面的有效性，为航空维护中的自动化FOD检测系统开发奠定了基础。
...
TL;DR: DCText是一种无需训练的可视化文本生成方法，采用分而治之策略解决长文本或多文本生成问题，通过注意力掩码和局部噪声初始化提高文本准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管现有的文本到图像模型在文本渲染方面取得了高保真度，但在处理长文本或多个文本时仍然存在困难，主要原因是全局注意力被稀释。

Method: 采用分而治之策略：1）将提示分解并分割目标文本，分配给指定区域；2）引入文本聚焦和上下文扩展两种注意力掩码，在去噪过程中顺序应用；3）使用局部噪声初始化进一步提高文本准确性和区域对齐。

Result: 在单句和多句基准测试中，DCText实现了最佳的文本准确性，同时不损害图像质量，并且具有最低的生成延迟。

Conclusion: DCText通过分而治之策略有效解决了长文本和多文本生成问题，在保持图像质量的同时显著提高了文本准确性，且计算成本低。
...
### [122] [Rethinking Intracranial Aneurysm Vessel Segmentation: A Perspective from Computational Fluid Dynamics Applications](https://arxiv.org/abs/2512.01319)
...
<details>
  <summary>Details</summary>
Motivation: 现有颅内动脉瘤分割方法主要关注图像评估指标，忽略了在后续计算流体动力学(CFD)应用中的实际有效性，缺乏包含拓扑完整性和CFD适用性的综合数据集。

Method: 构建了IAVS多中心数据集，包含图像-掩码对和血流动力学分析结果；建立了两个评估基准（动脉瘤全局定位和IA-Vessel细粒度分割）；开发了简单有效的两阶段框架；建立了标准化的CFD适用性评估系统。

Result: 创建了首个包含641个3D MRA图像和587个标注的颅内动脉瘤血管分割数据集，提供了开箱即用的两阶段框架和强基线方法，建立了自动化CFD模型转换的适用性评估系统。

Conclusion: IAVS数据集填补了现有数据集的空白，通过结合血流动力学分析和CFD适用性评估，为开发临床相关技术提供了全面支持，促进了颅内动脉瘤分割方法在实际医疗应用中的有效性评估。
...
### [123] [Gaussian Swaying: Surface-Based Framework for Aerodynamic Simulation with 3D Gaussians](https://arxiv.org/abs/2512.01306)
*Hongru Yan,Xiang Zhang,Zeyuan Chen,Fangyin Wei,Zhuowen Tu*

Main category: cs.CV

TL;DR: Gaussian Swaying：基于3D高斯函数的表面框架，用于空气动力学模拟，统一了模拟和渲染，实现高效精细的空气动力学交互。


<details>
  <summary>Details</summary>
Motivation: 自然界中物体在气流作用下的运动（如树枝摇曳、旗帜飘动、船只摇晃）对视觉和图形的真实感至关重要，需要高效的空气动力学模拟方法。

Method: 使用3D高斯函数连续建模表面，提出高斯补丁统一表示，支持动力学力计算和轻量级着色，避免网格化或离散粒子方法的限制。

Result: 在合成和真实数据集上的综合实验表明，Gaussian Swaying在多个指标上达到最先进的性能和效率，为真实空气动力学场景模拟提供了可扩展方法。

Conclusion: Gaussian Swaying提供了一个高效、精细的空气动力学模拟框架，统一了模拟和渲染，在真实感和计算效率方面表现出色。
...
### [124] [Rice-VL: Evaluating Vision-Language Models for Cultural Understanding Across ASEAN Countries](https://arxiv.org/abs/2512.01419)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型存在西方中心偏见，限制了它们在文化多样化地区（如东南亚）的有效性，需要专门的基准来评估和改进模型的文化理解能力。

Method: 开发了RICE-VL基准，包含11个东盟国家的文化内容，采用三种VQA格式（判断正误、填空、开放式）和视觉定位任务，并提出了SEA-LAVE评估指标来衡量文本准确性、文化对齐和国家识别能力。

Result: 对6个开源和闭源VLM的评估显示，模型在低资源国家和抽象文化领域存在显著性能差距，视觉定位任务也暴露了模型在复杂场景中定位文化重要元素的能力不足。

Conclusion: RICE-VL揭示了VLM在文化理解方面的局限性，强调了需要开发更具包容性的模型来更好地服务全球多样化人群。
...
### [125] [Lost in Distortion: Uncovering the Domain Gap Between Computer Vision and Brain Imaging - A Study on Pretraining for Age Prediction](https://arxiv.org/abs/2512.01310)
*Yanteng Zhang,Songheng Li,Zeyu Shen,Qizhen Lan,Lipei Zhang,Yang Liu,Vince Calhoun*

Main category: cs.CV

TL;DR: 研究探索了神经影像数据质量对预训练的影响，发现不同质量水平的数据在脑龄预测下游任务中表现差异显著，揭示了数据质量在医学影像基础模型开发中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 大规模脑影像数据集为开发领域基础模型提供了机会，但神经影像数据存在高度异质性，从结构良好的扫描到严重扭曲或不完整的脑体积。这引发了一个基本问题：噪声或低质量扫描是否对预训练有贡献，还是会阻碍模型学习？
...
### [126] [IVCR-200K: A Large-Scale Multi-turn Dialogue Benchmark for Interactive Video Corpus Retrieval](https://arxiv.org/abs/2512.01312)
*Ning Han,Yawen Zeng,Shaohua Long,Chengqing Li,Sijie Yang,Dun Tan,Jianfeng Dong,Jingjing Chen*

Main category: cs.CV

TL;DR: 本文提出了交互式视频语料库检索（IVCR）任务，这是一个更现实的设置，支持用户与检索系统之间的多轮对话交互，并引入了IVCR-200K数据集和基于多模态大语言模型的框架。


<details>
  <summary>Details</summary>
Motivation: 传统视频检索和视频时刻检索任务虽然取得了进展，但缺乏用户与检索系统之间的有意义的"交互"，其单向检索范式无法满足至少80.8%用户的个性化和动态需求。

Method: 1. 提出了交互式视频语料库检索（IVCR）任务，支持多轮、对话式和现实的交互；2. 构建了IVCR-200K数据集，这是一个高质量、双语、多轮对话、抽象语义的数据集，支持视频检索和时刻检索；3. 提出了基于多模态大语言模型（MLLMs）的全面框架，支持多种交互模式并提供更可解释的解决方案。

Result: 广泛的实验证明了所提出数据集和框架的有效性。

Conclusion: 本文通过引入IVCR任务、IVCR-200K数据集和基于MLLMs的框架，为视频检索领域提供了更现实、交互式的解决方案，能够更好地满足用户的个性化和动态需求。
...
### [127] [Diffusion Fuzzy System: Fuzzy Rule Guided Latent Multi-Path Diffusion Modeling](https://arxiv.org/abs/2512.01533)
*Hailong Yang,Te Zhang,Kup-sze Choi,Zhaohong Deng*

Main category: cs.CV

TL;DR: 本文提出了一种基于模糊规则的扩散模糊系统（DFS），通过多路径扩散模型解决传统扩散模型在处理特征差异大的图像集合时的局限性，提高特征捕捉能力和计算效率。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高分辨率真实图像方面表现出色，但在处理特征差异显著的图像集合时存在困难，难以捕捉复杂特征且会产生冲突结果。现有方法通过多路径学习不同区域但存在协调效率低和计算成本高的问题。

Method: 提出扩散模糊系统（DFS），采用潜在空间多路径扩散模型，每个路径专门学习特定类别的图像特征；使用基于规则链的推理动态引导扩散过程，实现多路径间高效协调；引入基于模糊隶属度的潜在空间压缩机制降低计算成本。

Result: 在LSUN Bedroom、LSUN Church和MS COCO三个公开数据集上测试，DFS相比现有单路径和多路径扩散模型实现了更稳定的训练和更快的收敛速度，在图像质量、文本-图像对齐度以及生成图像与目标参考的匹配精度方面均超越基线模型。

Conclusion: DFS通过模糊规则引导的多路径扩散模型，有效解决了传统扩散模型在处理异质图像特征时的局限性，实现了更好的特征捕捉能力、协调效率和计算性能，为扩散模型的改进提供了新思路。
...
TL;DR: TokenPure是一个基于扩散变换器的水印去除框架，通过令牌化条件重建解决水印彻底去除与内容一致性之间的权衡问题


<details>
  <summary>Details</summary>
Motivation: 在数字经济时代，数字水印对于AI生成内容和其他虚拟资产的所有权证明至关重要。设计能够抵抗各种攻击和处理操作的鲁棒水印变得更加重要，因此需要有效的水印去除方法

Method: TokenPure采用基于扩散变换器的框架，将水印去除任务重新定义为条件生成问题。它将水印图像分解为两个互补的令牌集：用于纹理的视觉令牌和用于几何的结构令牌，这些令牌共同调节扩散过程，绕过初始的水印携带噪声

Result: 综合实验表明，TokenPure在水印去除和重建保真度方面达到最先进水平，在感知质量和一致性方面显著优于现有基线方法

Conclusion: TokenPure通过令牌化条件重建有效解决了水印去除与内容保持之间的权衡问题，为数字水印去除提供了新的有效解决方案
...
### [129] [Deep Unsupervised Anomaly Detection in Brain Imaging: Large-Scale Benchmarking and Bias Analysis](https://arxiv.org/abs/2512.01534)
*Alexander Frotscher,Christian F. Baumgartner,Thomas Wolfers*

Main category: cs.CV

TL;DR: 该研究提出了一个大规模、多中心的脑影像深度无监督异常检测基准测试，评估了不同算法在脑MRI异常检测中的性能，发现重建类方法（特别是扩散模型）在病灶分割方面表现最佳，但所有算法都存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 脑磁共振成像中的深度无监督异常检测无需病灶特异性标注即可识别病理偏差，但由于评估碎片化、数据集异质性和指标不一致，阻碍了临床转化。本研究旨在建立一个标准化基准来推动该领域发展。

Method: 建立大规模多中心基准：训练集包含2,976个T1和2,972个T2加权健康扫描（6-89岁，6台扫描仪）；验证集92个扫描用于超参数调优；测试集包含2,221个T1w和1,262个T2w扫描，涵盖健康数据和多种临床队列。系统评估不同算法在扫描仪、病灶类型/大小、人口统计学（年龄、性别）变化下的鲁棒性。

Result: 所有算法的Dice分割性能在0.03-0.65之间，差异显著。重建类方法（特别是扩散模型）在病灶分割方面表现最强，而特征类方法在分布偏移下表现出更好的鲁棒性。但多数算法存在系统性偏差：扫描仪相关效应、小病灶和低对比度病灶漏检率高、假阳性随年龄和性别变化。增加健康训练数据仅带来有限改善。

Conclusion: 当前无监督异常检测框架主要受算法限制而非数据可用性。基准为未来研究提供了透明基础，并指出了临床转化的优先方向：图像原生预训练、原则性偏差度量、公平感知建模和鲁棒域适应。
...
### [130] [MasHeNe: A Benchmark for Head and Neck CT Mass Segmentation using Window-Enhanced Mamba with Frequency-Domain Integration](https://arxiv.org/abs/2512.01563)
...
TL;DR: 本文提出了MasHeNe数据集，包含3,779张头颈部肿块增强CT切片，涵盖肿瘤和囊肿，并建立了分割基准。同时提出了WEMF模型，通过三窗增强和多频注意力在U型Mamba架构中实现最佳分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集主要关注恶性病变，忽视了头颈部其他占位性病变。为了填补这一空白，需要建立一个包含肿瘤和囊肿的头颈部肿块数据集，并开发有效的分割方法。

Method: 1. 构建MasHeNe数据集：3,779张增强CT切片，包含像素级标注的肿瘤和囊肿；2. 提出WEMF模型：采用三窗增强丰富输入特征，在多频注意力机制下融合跳跃连接信息，基于U型Mamba架构；3. 建立基准测试：使用标准分割基线模型进行公平比较。

Result: WEMF模型在MasHeNe数据集上取得最佳性能：Dice系数70.45%，IoU 66.89%，NSD 72.33%，HD95 5.12mm。模型在该挑战性任务上表现出稳定且强大的结果。

Conclusion: MasHeNe为超越仅含恶性病变的头颈部肿块分割提供了基准数据集。观察到的错误模式表明该任务仍具挑战性，需要进一步研究。数据集和代码已开源。
...
### [131] [RoleMotion: A Large-Scale Dataset towards Robust Scene-Specific Role-Playing Motion Synthesis with Fine-grained Descriptions](https://arxiv.org/abs/2512.01582)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本驱动运动数据集存在以下问题：1）数据是分散构建的非功能性孤立子集集合，难以覆盖各种场景中的社交活动；2）运动数据质量不一致；3）文本标注缺乏细粒度细节。因此需要构建一个专注于场景和角色的高质量功能性运动数据集。

Method: 1）精心设计和收集专注于场景和角色的运动数据；2）构建包含25个经典场景、110个功能性角色、500多种行为的结构化数据集；3）提供10296个高质量身体和手部运动序列；4）标注27831个细粒度文本描述；5）构建比现有方法更强的评估器并验证其可靠性；6）探索身体和手部运动生成的相互作用。

Result: 1）成功构建了RoleMotion大规模功能性运动数据集；2）评估器被证明比现有方法更可靠；3）在数据集上评估了各种文本到运动生成方法；4）实验结果表明数据集在文本驱动全身生成方面具有高质量和功能性。

Conclusion: RoleMotion数据集通过专注于场景和角色的设计，解决了现有数据集在功能性、质量和标注细粒度方面的不足，为文本驱动全身运动生成提供了高质量的数据支持。
...
### [132] [Optimizing Stroke Risk Prediction: A Machine Learning Pipeline Combining ROS-Balanced Ensembles and XAI](https://arxiv.org/abs/2512.01333)
*A S M Ahsanul Sarkar Akib,Raduana Khawla,Abdul Hasib*

Main category: cs.CV

TL;DR: 该研究开发了一个结合集成学习和可解释AI的机器学习框架，用于中风风险预测，在Stroke Prediction Dataset上达到了99.09%的准确率，并识别出年龄、高血压和血糖水平三个关键临床变量。


<details>
  <summary>Details</summary>
Motivation: 中风是全球主要的死亡和永久性损伤原因，早期风险评估对于及时干预和有效预防策略至关重要。研究旨在开发一个既准确又可解释的机器学习框架，以提高中风风险预测的临床实用性。

Method: 采用集成建模和可解释AI技术，包括特征工程和数据预处理（使用随机过采样解决类别不平衡问题）。通过5折交叉验证对10种不同机器学习模型进行全面评估，最终优化出由随机森林、ExtraTrees和XGBoost组成的集成模型。

Result: 优化的集成模型在Stroke Prediction Dataset上表现出色，达到了99.09%的准确率。通过基于LIME的可解释性分析，识别出三个重要的临床变量：年龄、高血压和血糖水平。

Conclusion: 研究表明，结合集成学习和可解释AI能够提供高度准确且可解释的中风风险评估。该框架通过早期预测，有望实现数据驱动的预防和个性化临床决策，从而改变中风预测和心血管风险管理的方式。
...
### [133] [EvalTalker: Learning to Evaluate Real-Portrait-Driven Multi-Subject Talking Humans](https://arxiv.org/abs/2512.01340)
...
TL;DR: 该论文构建了首个大规模多说话人生成视频质量评估数据集THQA-MT，并提出EvalTalker评估框架，用于评估多说话人生成视频的质量。


<details>
  <summary>Details</summary>
Motivation: 当前多说话人生成技术存在明显的质量下降问题，导致用户体验不佳。需要建立专门的质量评估数据集和方法来推动高质量多说话人生成技术的发展。

Method: 1. 构建THQA-MT数据集：包含5,492个多说话人生成视频，来自15个代表性方法，基于400个真实肖像；2. 通过主观实验分析感知差异并识别12种常见失真类型；3. 提出EvalTalker评估框架，具备全局质量感知、人物特征感知、身份一致性感知能力，并集成Qwen-Sync感知多模态同步性。

Result: 实验结果表明，EvalTalker与主观评分具有优越的相关性，为未来高质量多说话人生成和评估研究提供了坚实基础。

Conclusion: 该研究填补了多说话人生成视频质量评估的空白，提出的数据集和评估框架将推动该领域向高质量方向发展。
...
### [134] [Envision: Benchmarking Unified Understanding & Generation for Causal World Process Insights](https://arxiv.org/abs/2512.01816)
*Juanxi Tian,Siyuan Li,Conghui He,Lijun Wu,Cheng Tan*

Main category: cs.CV

TL;DR: 该研究提出了Envision基准测试，用于评估多图像序列生成中的因果事件进展能力，发现当前模型在时空一致性方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型主要关注单图像生成任务，导致过度拟合静态模式匹配，缺乏对动态时间过程建模的能力，无法真正内化世界知识并遵循因果-时间约束。

Method: 提出了Envision基准测试，包含1000个四阶段提示，涵盖六个科学和人文领域，基于世界知识和时空因果关系构建。同时开发了Envision-Score综合评估指标，整合多维度一致性、物理性和美学性。

Result: 评估了15个模型（10个专业T2I模型，5个统一模型）：专业T2I模型在美学渲染方面表现出色但缺乏内在世界知识；统一多模态模型在因果叙事连贯性上优于专业模型，但仍落后于闭源模型，且在时空一致性方面存在挑战。
...
### [135] [InternVideo-Next: Towards General Video Foundation Models without Video-Text Supervision](https://arxiv.org/abs/2512.01342)
*Chenting Wang,Yuhan Zhu,Yicheng Xu,Jiange Yang,Ziang Yan,Yali Wang,Yi Wang,Limin Wang*

Main category: cs.CV

TL;DR: 论文提出InternVideo-Next，一种两阶段预训练框架，通过编码器-预测器-解码器架构解决视频表示学习中语义与细节的冲突问题，在公开无标签视频上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频-文本预训练依赖噪声合成字幕且语义覆盖有限，忽略了隐式世界知识（如物体运动、3D几何、物理线索）；而掩码视频建模直接利用时空结构但在通用任务上落后于文本监督方法。这种差距源于被忽视的架构问题：像素级重建存在收敛困难和低层需求与语义冲突，而潜在预测容易导致捷径学习。

Method: 提出编码器-预测器-解码器（EPD）框架，将传统编码器-解码器设计解耦，其中预测器作为潜在世界模型。采用两阶段预训练方案：第一阶段使用条件扩散解码器并注入可靠的图像级语义先验，增强语义和收敛性；第二阶段在构建的潜在空间中预测冻结的第一阶段目标，学习世界知识并缓解捷径学习。

Result: 在公开无标签视频上训练的InternVideo-Next在多个基准测试中取得了最先进的结果，为通用视频表示学习提供了可扩展的路径。

Conclusion: 通过解耦编码器-解码器设计并采用两阶段预训练，InternVideo-Next成功构建了语义一致且保留细节的潜在空间，有效解决了视频表示学习中语义抽象与像素级保真度之间的冲突，实现了更好的世界知识学习。
...
### [136] [Handwritten Text Recognition for Low Resource Languages](https://arxiv.org/abs/2512.01348)
*Sayantan Dey,Alireza Alaei,Partha Pratim Roy*

Main category: cs.CV

TL;DR: BharatOCR：一种基于ViT-Transformer解码器-LM架构的无分割段落级手写印地语和乌尔都语文本识别系统，在多个数据集上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如印地语、乌尔都语等）的段落级手写文本识别仍然具有挑战性，这些语言通常缺乏全面的语言资源，需要开发鲁棒的系统以实现准确的光学字符识别。

Method: 提出ViT-Transformer解码器-LM架构：使用Vision Transformer提取视觉特征，Transformer解码器生成文本序列，预训练语言模型优化输出以提高准确性、流畅性和连贯性。采用DeiT进行掩码图像建模，RoBERTa架构进行掩码语言建模。模型通过隐式行分割逐行处理段落图像。

Result: 在NUST-UHWR、PUCIT-OUHL和Parimal-Urdu数据集上分别达到96.24%、92.05%和94.80%的字符识别率；在印地语数据集上达到80.64%的字符识别率，优于多种最先进的乌尔都语文本识别方法。

Conclusion: BharatOCR在段落级手写印地语和乌尔都语文本识别方面表现出色，为低资源语言的手写文本识别提供了有效的解决方案，并在多个数据集上建立了新的基准结果。
...
### [137] [OpenBox: Annotate Any Bounding Boxes in 3D](https://arxiv.org/abs/2512.01352)
*In-Jae Lee,Mungyeom Kim,Kwonyoung Ryu,Pierre Musacchio,Jaesik Park*

Main category: cs.CV

TL;DR: OpenBox：基于2D视觉基础模型的两阶段自动3D标注方法，无需自训练即可生成高质量的3D边界框标注


<details>
  <summary>Details</summary>
Motivation: 现有无监督和开放词汇3D目标检测方法存在三个主要问题：1）统一标注3D边界框，忽略物体的物理状态；2）需要多次自训练迭代进行标注细化，导致计算开销大；3）标注质量不理想。这些问题在自动驾驶领域尤为突出，因为降低标注成本和识别未见物体对安全性和可扩展性至关重要。

Method: OpenBox采用两阶段自动标注流程：第一阶段通过跨模态实例对齐，将2D视觉基础模型处理的图像实例级线索与对应的3D点云关联；第二阶段根据刚性和运动状态对实例进行分类，然后使用类别特定的尺寸统计生成自适应边界框。

Result: 在Waymo Open Dataset、Lyft Level 5 Perception dataset和nuScenes数据集上的实验表明，OpenBox在准确性和效率方面均优于基线方法，能够生成高质量的3D边界框标注。

Conclusion: OpenBox通过利用2D视觉基础模型和两阶段处理流程，成功解决了现有3D标注方法的局限性，实现了无需自训练的高质量3D边界框自动标注，为自动驾驶等应用提供了更高效、更准确的标注解决方案。
...
### [138] [BlinkBud: Detecting Hazards from Behind via Sampled Monocular 3D Detection on a Single Earbud](https://arxiv.org/abs/2512.01366)
*Yunzhe Li,Jiajun Yan,Yuzhou Wei,Kechen Liu,Yize Zhao,Chong Zhang,Hongzi Zhu,Li Lu,Shan Chang,Minyi Guo*

Main category: cs.CV

TL;DR: BlinkBud系统利用单耳塞和配对手机在线检测用户身后接近的危险物体，通过3D目标跟踪算法和强化学习优化图像采样策略，在低功耗下实现高精度危险检测。


<details>
  <summary>Details</summary>
Motivation: 行人或骑行者无法察觉身后快速接近的车辆对道路安全构成巨大威胁，需要一种轻量级、低功耗的解决方案来实时检测后方危险物体。

Method: 1. 使用单耳塞和配对手机系统；2. 开发新颖的3D目标跟踪算法，结合卡尔曼滤波轨迹估计和基于强化学习的最优图像采样策略；3. 利用估计的俯仰角和偏航角校正目标深度估计，将相机坐标系与用户身体坐标系对齐，消除头部运动影响。

Result: 原型系统在真实环境中测试显示：耳塞平均功耗29.8mW，手机平均功耗702.6mW；危险检测平均假阳性率4.90%，平均假阴性率1.47%，实现了轻量级、低功耗下的高精度检测。

Conclusion: BlinkBud系统通过创新的3D目标跟踪算法和头部运动补偿技术，在极低功耗下实现了对用户身后接近危险物体的准确检测，为行人/骑行者道路安全提供了有效解决方案。
...
### [139] [SRAM: Shape-Realism Alignment Metric for No Reference 3D Shape Evaluation](https://arxiv.org/abs/2512.01373)
*Sheng Liu,Tianyu Luan,Phani Nuney,Xuelu Feng,Junsong Yuan*

Main category: cs.CV

TL;DR: 提出了一种基于大语言模型的3D形状真实感评估指标，通过将网格编码到语言空间并设计真实感解码器，无需地面真值即可评估形状的真实感。


<details>
  <summary>Details</summary>
Motivation: 随着3D生成和重建技术在游戏、电影等内容创作领域的广泛应用，对真实感3D形状的需求日益增长。传统评估方法依赖地面真值来测量网格保真度，但在许多实际应用中，形状的真实感并不需要地面真值参考。

Method: 1. 提出形状-真实感对齐指标，利用大语言模型作为网格形状信息与真实感评估之间的桥梁；2. 采用网格编码方法将3D形状转换到语言标记空间；3. 设计专用的真实感解码器，使语言模型输出与人类真实感感知对齐；4. 引入新的数据集RealismGrading，提供无需地面真值的人类标注真实感分数。

Result: 1. 创建了包含16种不同算法在十多个对象上生成的形状的数据集，更具代表性；2. 通过k折交叉验证验证了指标的性能和泛化能力；3. 实验结果表明，该指标与人类感知相关性良好，优于现有方法，并具有良好的泛化性。

Conclusion: 提出的形状-真实感对齐指标能够有效评估3D形状的真实感，无需地面真值参考，与人类感知高度一致，在3D生成和重建领域具有实用价值。
...
### [140] [Textured Geometry Evaluation: Perceptual 3D Textured Shape Metric via 3D Latent-Geometry Network](https://arxiv.org/abs/2512.01380)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D模型保真度评估方法存在多个问题：传统几何指标（如Chamfer Distance）与人类感知不一致；基于渲染的方法存在结构覆盖不全和视角选择敏感问题；大多数方法基于合成失真训练，与真实世界失真存在领域差距

Method: 提出Textured Geometry Evaluation (TGE)方法，直接基于带纹理的3D网格进行评估，无需渲染过程。该方法联合使用几何和颜色信息来计算输入带纹理网格相对于参考彩色形状的保真度。创建了人工标注的真实世界失真数据集用于训练和评估

Result: 实验表明，TGE在真实世界失真数据集上优于基于渲染的方法和仅使用几何信息的方法

Conclusion: TGE提供了一种更准确、更符合人类感知的3D模型保真度评估方法，解决了现有方法的局限性，特别是在处理真实世界失真方面表现优异
...
### [141] [Reversible Inversion for Training-Free Exemplar-guided Image Editing](https://arxiv.org/abs/2512.01382)
*Yuke Li,Lianli Gao,Ji Zhang,Pengpeng Zeng,Lichuan Xiang,Hongkai Wen,Heng Tao Shen,Jingkuan Song*

Main category: cs.CV

TL;DR: ReInversion是一种免训练的图像编辑方法，通过两阶段去噪过程和掩码引导的选择性去噪，实现高效且高质量的示例引导图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有示例引导图像编辑方法需要大规模预训练，计算成本高；而标准反转技术虽然免训练，但质量差且效率低，因此需要一种更优的免训练解决方案。

Method: 提出Reversible Inversion（ReInversion）方法：1）两阶段去噪过程，先以源图像为条件，再以参考图像为条件；2）掩码引导的选择性去噪策略，将编辑约束在目标区域，保持背景结构一致性。

Result: 定性和定量比较表明，ReInversion方法在示例引导图像编辑任务上达到了最先进的性能，同时具有最低的计算开销。

Conclusion: ReInversion提供了一种高效且高质量的免训练示例引导图像编辑解决方案，解决了现有方法计算成本高和标准反转技术质量差的问题。
...
### [142] [MDiff4STR: Mask Diffusion Model for Scene Text Recognition](https://arxiv.org/abs/2512.01422)
*Yongkun Du,Miaomiao Zhao,Songlin Fan,Zhineng Chen,Caiyan Jia,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: MDiff4STR：首个将掩码扩散模型应用于场景文本识别任务的方法，通过改进训练推理对齐和过自信预测问题，在保持高效推理的同时超越了自回归模型的准确率。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在视觉语言任务中展现出效率和准确性的平衡优势，但直接应用于场景文本识别任务时，准确率仍落后于自回归模型。需要解决训练推理对齐和过自信预测两大关键挑战。

Method: 提出MDiff4STR模型，包含两大改进策略：1）开发六种加噪策略以缩小训练与推理之间的差距；2）提出令牌替换噪声机制，提供非掩码噪声类型，促使模型重新考虑和修正过自信的错误预测。

Result: 在标准及挑战性STR基准测试中（包括不规则、艺术、遮挡、中文文本等场景），MDiff4STR始终优于主流STR模型，在准确率上超越了最先进的自回归模型，同时仅需三步去噪即可实现快速推理。

Conclusion: MDiff4STR成功将掩码扩散模型引入场景文本识别任务，通过针对性的改进策略解决了关键挑战，实现了在准确率和效率上的双重优势，为STR任务提供了新的有效解决方案。
...
### [143] [\textit{ViRectify}: A Challenging Benchmark for Video Reasoning Correction with Multimodal Large Language Models](https://arxiv.org/abs/2512.01424)
*Xusen Hei,Jiali Chen,Jinyu Yang,Mengchen Zhao,Yi Cai*

Main category: cs.CV

TL;DR: ViRectify是一个评估多模态大语言模型视频推理错误纠正能力的基准测试，包含30K+实例，涵盖动态感知、科学推理和具身决策三个领域，通过AI辅助标注构建。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视频推理场景中经常出现错误，但现有基准测试缺乏对这些模型识别和纠正错误能力的系统性评估，需要填补这一空白。

Method: 提出ViRectify基准测试，通过AI辅助标注流程构建数据集；设计轨迹证据驱动的纠正框架，包括逐步错误轨迹建模和视觉证据基础的奖励建模，使模型专注于错误传播和关键时间戳。

Result: 评估16个先进MLLM显示ViRectify具有挑战性，GPT-5仅达到31.94%的纠正准确率；提出的框架使Qwen2.5-VL-7B模型在ViRectify上持续优于72B变体；分析揭示了模型间错误纠正的系统性不对称性。

Conclusion: ViRectify为全面评估MLLM在视频推理中的能力提供了新方向，数据集也可用于反思学习，是宝贵的数据资源。
...
### [144] [ResDiT: Evoking the Intrinsic Resolution Scalability in Diffusion Transformers](https://arxiv.org/abs/2512.01426)
*Yiyang Ma,Feng Zhou,Xuedan Yin,Pu Cao,Yonghao Dang,Jianqin Yin*

Main category: cs.CV

TL;DR: ResDiT：无需训练的扩散变换器高分辨率图像合成方法，通过位置嵌入缩放和局部增强机制解决布局崩溃和纹理退化问题


<details>
  <summary>Details</summary>
Motivation: 预训练的扩散变换器（DiTs）用于高分辨率图像合成时，常出现空间布局崩溃和纹理保真度下降的问题。现有方法通常采用复杂的多阶段流程，首先进行基础分辨率去噪来指导高分辨率生成，但这种方法效率较低且复杂。

Method: 提出ResDiT训练免费方法：1）识别位置嵌入（PEs）是控制空间布局的核心因素，当外推到高分辨率时原始PEs编码错误位置信息导致布局崩溃，引入PE缩放技术纠正分辨率变化下的位置编码；2）开发基于基础分辨率局部注意力的局部增强机制，设计补丁级融合模块聚合全局和局部线索；3）采用高斯加权拼接策略消除网格伪影。

Result: 综合评估表明，ResDiT能够持续提供高保真度的高分辨率图像合成，并且与下游任务（包括空间控制生成）无缝集成。

Conclusion: ResDiT通过探索DiTs的内在生成机制，提出了一种无需训练的高分辨率扩展方法，有效解决了布局崩溃和纹理退化问题，为扩散变换器的高分辨率图像合成提供了高效解决方案。
...
### [145] [Language-Guided Open-World Anomaly Segmentation](https://arxiv.org/abs/2512.01427)
*Klara Reichard,Nikolas Brasch,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: Clipomaly：首个基于CLIP的自动驾驶开放世界和异常分割方法，无需异常训练数据，能分割未知物体并赋予可解释的名称


<details>
  <summary>Details</summary>
Motivation: 现有开放世界和异常分割方法无法为未知区域分配有意义的语义标签，难以区分和学习未知类别的表示。开放词汇分割方法虽然能泛化到新类别，但需要固定的推理词汇表，无法直接应用于异常分割（未知类别不受约束）

Method: 提出Clipomaly方法，基于CLIP的共享图像-文本嵌入空间，无需异常特定训练数据的零样本方法。在推理时动态扩展词汇表而无需重新训练，既能分割未知物体，又能为它们分配人类可解释的名称

Result: 在现有异常分割基准测试中达到最先进的性能，同时提供实际部署所需的可解释性和灵活性，超越了Cityscapes等常见类别定义的限制

Conclusion: Clipomaly是首个基于CLIP的自动驾驶开放世界和异常分割方法，通过零样本方法实现了对未知物体的检测和命名，为实际部署提供了关键的可解释性和灵活性
...
### [146] [FastAnimate: Towards Learnable Template Construction and Pose Deformation for Fast 3D Human Avatar Animation](https://arxiv.org/abs/2512.01444)
*Jian Shu,Nanjie Yao,Gangjian Zhang,Junlong Ren,Yu Feng,Hao Wang*

Main category: cs.CV

TL;DR: 提出统一的基于学习的框架解决3D人体化身动画中的两个核心问题：通过U-Net架构快速生成人体模板，并通过数据驱动细化技术提升结构完整性，在效率和质量上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体化身动画方法通常分为两个阶段：规范模板构建和目标姿态变形。但当前模板构建方法需要大量骨骼绑定且容易产生伪影，而目标姿态变形则因线性混合蒙皮导致结构扭曲，严重影响动画真实感。

Method: 提出统一的学习框架分两阶段解决问题：1) 使用U-Net架构在正向传播过程中解耦纹理和姿态信息，快速生成人体模板；2) 提出数据驱动的细化技术增强结构完整性。

Result: 大量实验表明，该模型在不同姿态下表现一致，在效率和质量之间达到最佳平衡，超越了当前最先进的方法。

Conclusion: 提出的统一学习框架有效解决了3D人体化身动画中的模板构建和姿态变形问题，通过快速模板生成和结构完整性增强，实现了高质量且高效的动画效果。
...
### [147] [ChronosObserver: Taming 4D World with Hyperspace Diffusion Sampling](https://arxiv.org/abs/2512.01481)
...
<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成模型难以直接扩展到3D一致、高保真的时间同步多视角视频生成，这对驯服4D世界至关重要。现有方法依赖数据增强或测试时优化，但受限于模型泛化能力和可扩展性问题。
...
Conclusion: ChronosObserver提供了一种无需训练的方法，通过世界状态超空间和超空间引导采样，有效解决了4D世界场景中时间同步多视角视频生成的挑战。
...
### [148] [A variational method for curve extraction with curvature-dependent energies](https://arxiv.org/abs/2512.01494)
...
<details>
  <summary>Details</summary>
Motivation: 开发一种自动从图像中提取曲线和1D结构的变分方法，该方法主要是无监督的，能够处理端点不确定性和曲率约束

Method: 基于能量离散化和Smirnov向量场分解定理的变分方法，采用双层最小化策略，并将方法扩展到曲率相关能量，使用位置-方向空间的提升和亚黎曼或Finsler度量

Result: 提出了一种能够自动从图像中提取曲线和1D结构的变分框架，该框架可以处理端点不确定性，并能够扩展到包含曲率约束的情况

Conclusion: 该方法为图像中的曲线提取提供了一种有效的变分框架，通过能量最小化和几何提升技术，实现了对1D结构的自动和无监督提取
...
### [149] [QuantumCanvas: A Multimodal Benchmark for Visual Learning of Atomic Interactions](https://arxiv.org/abs/2512.01519)
*Can Polat,Erchin Serpedin,Mustafa Kurban,Hasan Kurban*

Main category: cs.CV

TL;DR: QuantumCanvas是一个大规模多模态基准数据集，将双体量子系统作为物质的基本单元，包含2850个元素-元素对，每个都有18种电子、热力学和几何性质，以及十通道图像表示，用于学习可转移的量子相互作用。


<details>
  <summary>Details</summary>
Motivation: 当前分子和材料机器学习模型大多缺乏物理可转移性，它们拟合整个分子或晶体的相关性，而不是学习原子对之间的量子相互作用。然而，键合、电荷重新分布、轨道杂化和电子耦合都源于这些双体相互作用，这些相互作用定义了多体系统中的局域量子场。

Method: 引入QuantumCanvas基准数据集，包含2850个元素-元素对，每个对标注18种电子、热力学和几何性质，并配有十通道图像表示（来自l-和m-分辨轨道密度、角场变换、共占据图和电荷密度投影）。这些基于物理的图像编码了空间、角和静电对称性，无需显式坐标。对8种架构在18个目标上进行基准测试。

Result: GATv2在能隙上达到0.201 eV的平均绝对误差，EGNN在HOMO和LUMO上分别达到0.265 eV和0.274 eV。DimeNet在总能量上达到2.27 eV MAE，在排斥能量上达到0.132 eV MAE。多模态融合模型在Mermin自由能上达到2.15 eV MAE。在QuantumCanvas上预训练可提高在QM9、MD17和CrysMTM等更大数据集上的收敛稳定性和泛化能力。

Conclusion: 通过将轨道物理与基于视觉的表征学习统一起来，QuantumCanvas为通过耦合视觉和数值模态学习可转移的量子相互作用提供了一个有原则且可解释的基础。该数据集和模型实现已开源。
...
### [150] [FlashVGGT: Efficient and Scalable Visual Geometry Transformers with Compressed Descriptor Attention](https://arxiv.org/abs/2512.01540)
*Zipeng Wang,Dan Xu*

Main category: cs.CV

TL;DR: FlashVGGT提出了一种高效的3D重建方法，通过描述符压缩机制减少计算复杂度，在保持重建精度的同时显著提升处理长序列的效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于全自注意力的3D重建方法（如VGGT）在处理长图像序列时面临二次复杂度问题，计算开销大且可扩展性差。

Method: 采用描述符注意力机制，将每帧空间信息压缩为紧凑的描述符token集，通过图像token与描述符之间的交叉注意力替代全局自注意力，并引入分块递归机制支持在线长序列推理。

Result: FlashVGGT在重建精度上与VGGT相当，但推理时间降至VGGT的9.3%（1000张图像），并能高效处理超过3000张图像的长序列。

Conclusion: FlashVGGT通过描述符注意力机制有效解决了3D重建中长序列处理的计算瓶颈，实现了高效且可扩展的多视图重建。
...
### [151] [Toward Content-based Indexing and Retrieval of Head and Neck CT with Abscess Segmentation](https://arxiv.org/abs/2512.01589)
...
<details>
  <summary>Details</summary>
Motivation: 头颈部脓肿是急性感染过程，若不及时诊断处理可能导致败血症或死亡。准确检测和描绘这些病变对诊断、治疗计划和手术干预至关重要。

Method: 创建了AbscessHeNe数据集，包含4,926张临床确认的头颈部脓肿增强CT切片，评估了CNN、Transformer和Mamba等多种最先进的分割架构。

Result: 最佳模型达到Dice相似系数0.39、交并比0.27和归一化表面距离0.67，表明该任务具有挑战性，需要进一步研究。

Conclusion: AbscessHeNe数据集为开发头颈部脓肿分割模型提供了基础，支持临床决策，未来还可用于基于内容的多媒体索引和案例检索，数据集将公开可用。
...
### [152] [Depth Matching Method Based on ShapeDTW for Oil-Based Mud Imager](https://arxiv.org/abs/2512.01611)
*Fengfeng Li,Zhou Feng,Hongliang Wu,Hao Zhang,Han Tian,Peng Liu,Lixin Yuan*

Main category: cs.CV

TL;DR: 提出基于ShapeDTW算法的井眼图像深度匹配方法，解决油基泥浆微电阻率成像仪上下极板组图像深度错位问题


<details>
  <summary>Details</summary>
Motivation: 油基泥浆微电阻率成像仪采用交错设计的上下极板组，即使在速度校正后，极板图像之间仍存在深度错位问题，需要更精确的匹配方法

Method: 基于Shape Dynamic Time Warping算法，提取局部形状特征构建形态敏感距离矩阵，采用一维方向梯度直方图和原始信号组合作为形状描述符

Result: 现场测试表明该方法能精确对齐具有复杂纹理、深度偏移或局部缩放的图像，为复杂地质特征的匹配提供了有效解决方案

Conclusion: 该方法不仅实现了精确的深度匹配，还提供了灵活的特征扩展框架，可根据特定地质特征集成其他描述符，具有很好的应用前景
...
### [153] [Generative Editing in the Joint Vision-Language Space for Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2512.01636)
*Xin Wang,Haipeng Zhang,Mang Li,Zhaohui Xia,Yueguo Chen,Yu Zhang,Chunyu Wei*

Main category: cs.CV

TL;DR: Fusion-Diff是一个用于零样本组合图像检索的生成式编辑框架，通过多模态融合特征编辑策略和轻量级Control-Adapter，在仅使用20万合成样本的情况下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 监督式组合图像检索方法依赖昂贵的三元组标注，因此需要零样本解决方案。现有文本中心或基于扩散的方法难以有效弥合视觉-语言模态差距，这是零样本CIR的核心挑战。

Method: 提出Fusion-Diff框架：1）在联合视觉-语言空间中引入多模态融合特征编辑策略，大幅缩小模态差距；2）集成轻量级Control-Adapter，通过仅使用20万合成样本的微调实现数据高效性。

Result: 在标准CIR基准测试（CIRR、FashionIQ和CIRCO）上的广泛实验表明，Fusion-Diff显著优于先前的零样本方法。通过可视化融合的多模态表示增强了模型的可解释性。

Conclusion: Fusion-Diff通过创新的多模态融合特征编辑和高效的数据利用策略，成功解决了零样本组合图像检索中的模态对齐挑战，实现了卓越的性能和可解释性。
...
### [154] [ViT$^3$: Unlocking Test-Time Training in Vision](https://arxiv.org/abs/2512.01643)
*Dongchen Han,Yining Li,Tianyu Li,Zixuan Cao,Ziming Wang,Jun Song,Yu Cheng,Bo Zheng,Gao Huang*

Main category: cs.CV

TL;DR: 本文系统研究了视觉测试时训练（TTT）的设计空间，提出了ViT³模型，在保持线性计算复杂度的同时，在多种视觉任务上达到或超越了现有线性复杂度模型的性能。


<details>
  <summary>Details</summary>
Motivation: 测试时训练（TTT）将注意力操作重新定义为在线学习问题，实现了线性计算复杂度，但在视觉领域缺乏系统性的设计原则和实用指南。本文旨在填补这一空白，为视觉TTT模型建立设计原则。

Method: 通过系统性的实证研究，分析了TTT设计中内部模块和内部训练的关键选择，提炼出六条实用见解。基于这些原则构建了Vision Test-Time Training（ViT³）模型，这是一个纯TTT架构，具有线性复杂度和可并行计算特性。

Result: ViT³在图像分类、图像生成、目标检测和语义分割等多种视觉任务上进行了评估。结果显示，ViT³在保持线性复杂度的同时，一致达到或超越了先进的线性复杂度模型（如Mamba和线性注意力变体），并有效缩小了与高度优化的视觉Transformer之间的性能差距。

Conclusion: 本文通过系统性研究为视觉TTT模型建立了设计原则，提出的ViT³基准模型在多种视觉任务上表现出色，为未来视觉TTT模型的研究提供了实用指南和基础。
...
### [155] [DB-KAUNet: An Adaptive Dual Branch Kolmogorov-Arnold UNet for Retinal Vessel Segmentation](https://arxiv.org/abs/2512.01657)
...
<details>
  <summary>Details</summary>
Motivation: 视网膜血管分割对眼科和全身性疾病诊断至关重要，但传统CNN方法存在局限性，难以捕捉长距离依赖和复杂非线性关系。

Method: 提出自适应双分支Kolmogorov-Arnold UNet (DB-KAUNet)，设计异构双分支编码器(HDBE)，包含并行的CNN和Transformer路径，集成KANConv和KAT块，引入跨分支通道交互(CCI)模块和基于注意力的空间特征增强(SFE)模块，进一步开发SFE-GAF模块进行自适应采样。

Result: 在DRIVE、STARE和CHASE_DB1数据集上的广泛实验验证了DB-KAUNet实现了领先的分割性能，并展现出卓越的鲁棒性。

Conclusion: DB-KAUNet通过创新的双分支架构和自适应特征增强模块，有效解决了传统CNN方法的局限性，在视网膜血管分割任务中取得了优异性能。
...
### [156] [Bridging the Scale Gap: Balanced Tiny and General Object Detection in Remote Sensing Imagery](https://arxiv.org/abs/2512.01665)
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中密集微小目标和大尺度目标共存时，现有方法难以实现平衡的检测性能。虽然大型基础模型在通用视觉任务中表现优异，但由于遥感图像中极端的尺度变化和密度分布，其在微小目标检测中的应用尚未探索。

Method: 提出ScaleBridge-Det框架，包含两个核心模块：1) 路由增强混合注意力模块，通过自适应路由动态选择和融合尺度特定专家特征；2) 密度引导动态查询模块，预测目标密度并自适应调整查询位置和数量。

Result: 在AI-TOD-V2和DTOD基准数据集上达到最先进性能，同时在VisDrone数据集上表现出优越的跨域鲁棒性，能够同时优化密集微小目标和一般目标的检测性能而无需权衡。

Conclusion: ScaleBridge-Det是首个针对遥感图像微小目标检测的大型检测框架，通过创新的尺度自适应机制成功解决了不同尺度目标的平衡检测问题，为遥感目标检测领域提供了新的解决方案。
...
### [157] [GRASP: Guided Residual Adapters with Sample-wise Partitioning](https://arxiv.org/abs/2512.01675)
*Felix Nützel,Mischa Dombrowski,Bernhard Kainz*

Main category: cs.CV

TL;DR: GRASP方法通过样本聚类和残差适配器解决扩散模型在长尾数据（如医学影像）中的模式崩溃问题，提升罕见类别的生成质量和多样性。
...
Method: 提出GRASP方法：1）使用外部先验静态划分样本为最小化组内梯度冲突的聚类；2）在预训练模型的transformer前馈层注入聚类特定的残差适配器进行微调，绕过学习门控以提高稳定性和效率。

Result: 在MIMIC-CXR-LT数据集上，GRASP在FID和多样性指标上表现优异，特别是罕见类别；在NIH-CXR-LT的下游分类任务中，尾部标签性能显著提升；在ImageNet-LT上的泛化验证了方法的广泛适用性。

Conclusion: GRASP是一种轻量级、可扩展的方法，能有效解决扩散模型在长尾数据中的梯度冲突问题，提升罕见类别的生成质量，易于集成到现有扩散管道中。
...
### [158] [Open-world Hand-Object Interaction Video Generation Based on Structure and Contact-aware Representation](https://arxiv.org/abs/2512.01677)
...
TL;DR: 提出SCAR方法，通过结构感知和接触感知的2D表示解决手物交互视频生成中的物理约束建模难题，无需3D标注，实现可扩展的高保真交互生成。


<details>
  <summary>Details</summary>
Motivation: 现有手物交互视频生成方法在2D和3D表示之间存在两难：2D表示可扩展但交互保真度不足，3D表示保真度高但需要标注且难以扩展。需要一种既能保证交互保真度又具有可扩展性的表示方法。

Method: 提出结构感知和接触感知表示（SCAR），捕捉手物接触、手物遮挡和整体结构上下文，无需3D标注。采用联合生成范式，通过共享-专业化策略同时生成交互导向表示和视频。

Result: 在两个真实世界数据集上超越现有方法，生成物理真实且时序连贯的手物交互视频。在具有挑战性的开放世界场景中表现出强大的泛化能力。

Conclusion: SCAR表示方法通过平衡交互保真度和可扩展性，有效解决了手物交互视频生成中的物理约束建模问题，为开放世界场景提供了实用的解决方案。
...
### [159] [Cross-Domain Validation of a Resection-Trained Self-Supervised Model on Multicentre Mesothelioma Biopsies](https://arxiv.org/abs/2512.01681)
*Farzaneh Seyedshahi,Francesca Damiola,Sylvie Lantuejoul,Ke Yuan,John Le Quesne*

Main category: cs.CV

TL;DR: 自监督编码器在间皮瘤活检组织上实现亚型分类和生存预测


<details>
  <summary>Details</summary>
Motivation: 间皮瘤的准确亚型分类和预后预测对指导治疗和患者护理至关重要，但现有计算病理学模型主要基于大块切除组织训练，难以应用于临床常见的小活检样本。

Method: 使用在切除组织上训练的自监督编码器，将其应用于活检材料以捕获有意义的形态学模式，利用这些模式预测患者生存和分类肿瘤亚型。

Result: 该方法成功实现了对间皮瘤活检样本的亚型分类和患者生存预测，证明了AI驱动工具在支持间皮瘤诊断和治疗规划方面的潜力。

Conclusion: 自监督学习模型能够将切除组织训练的知识迁移到活检样本，为临床实践中基于小活检样本的精准诊断和预后评估提供了可行的AI解决方案。
...
### [160] [DreamingComics: A Story Visualization Pipeline via Subject and Layout Customized Generation using Video Models](https://arxiv.org/abs/2512.01686)
*Patrick Kwon,Chen Chen*

Main category: cs.CV

TL;DR: DreamingComics是一个布局感知的故事可视化框架，通过改进的扩散变换器模型和区域感知位置编码，显著提升了角色一致性和风格相似性。


<details>
  <summary>Details</summary>
Motivation: 当前故事可视化方法仅依赖文本定位主体，难以保持艺术一致性，需要更有效的布局控制和一致性保持机制。

Method: 基于预训练的视频扩散变换器模型，提出RegionalRoPE区域感知位置编码方案，引入掩码条件损失约束视觉特征，集成LLM布局生成器从自然语言脚本推断漫画风格布局。

Result: 相比先前方法，角色一致性提升29.2%，风格相似性提升36.2%，同时展现出高空间准确性。

Conclusion: DreamingComics框架通过布局感知控制和一致性增强机制，有效解决了故事可视化中的角色一致性和艺术风格保持问题。
...
### [161] [SSR: Semantic and Spatial Rectification for CLIP-based Weakly Supervised Segmentation](https://arxiv.org/abs/2512.01701)
*Xiuli Bi,Die Xiao,Junchao Fan,Bin Xiao*

Main category: cs.CV

TL;DR: 本文提出了一种语义和空间校正（SSR）方法，通过跨模态原型对齐（CMPA）和超像素引导校正（SGC）来解决CLIP在弱监督语义分割中的过激活问题，在PASCAL VOC和MS COCO数据集上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的弱监督语义分割方法存在两个主要问题：1）在非目标前景区域的过激活；2）背景区域的过激活。这些问题限制了CLIP在WSSS任务中的性能提升。

Method: 提出语义和空间校正（SSR）方法，包含两个核心组件：1）跨模态原型对齐（CMPA）：建立对比学习机制，强制跨模态特征空间对齐，减少类间重叠，增强语义相关性，有效校正非目标前景区域的过激活；2）超像素引导校正（SGC）：利用基于超像素的空间先验，在亲和传播过程中精确过滤非目标区域的干扰，显著校正背景过激活。

Result: 在PASCAL VOC数据集上达到79.5%的mIoU，在MS COCO数据集上达到50.6%的mIoU，超越了所有单阶段方法以及更复杂的多阶段方法，取得了最先进的性能。

Conclusion: 提出的SSR方法通过语义和空间两个层面的校正，有效解决了CLIP在弱监督语义分割中的过激活问题，显著提升了分割性能，为CLIP在WSSS任务中的应用提供了新的解决方案。
...
### [162] [FreqEdit: Preserving High-Frequency Features for Robust Multi-Turn Image Editing](https://arxiv.org/abs/2512.01755)
*Yucheng Liao,Jiajun Liang,Kaiqian Cui,Baoquan Zhao,Haoran Xie,Wei Liu,Qing Li,Xudong Mao*

Main category: cs.CV

TL;DR: FreqEdit是一个无需训练的框架，通过高频特征注入、自适应注入策略和路径补偿机制，解决了多轮图像编辑中的高频信息丢失问题，实现稳定的10+轮次编辑。
...
Result: 在7个最先进基线的对比实验中，FreqEdit在身份保持和指令遵循方面均表现出优越性能，能够稳定支持10+轮连续编辑。

Conclusion: FreqEdit通过解决多轮编辑中的高频信息丢失问题，为基于自然语言的图像编辑提供了稳定、高质量的多轮编辑能力。
...
### [163] [VideoScoop: A Non-Traditional Domain-Independent Framework For Video Analysis](https://arxiv.org/abs/2512.01769)
*Hafsa Billah*

Main category: cs.CV

TL;DR: 该论文提出了一个通用的视频情境分析框架，通过内容提取、关系模型和图模型相结合的方式，实现跨领域的自动化情境检测。


<details>
  <summary>Details</summary>
Motivation: 当前视频情境分析主要依赖人工或特定算法，存在效率低、易出错、缺乏通用性的问题。需要一种通用框架来自动分析视频中的有意义活动或情境。

Method: 使用先进的视频内容提取技术，采用扩展关系模型(R++)和图模型两种表示方式。R++支持连续查询处理，图模型补充检测复杂情境。开发参数化模板实现领域独立性。

Result: 在辅助生活、市政监控和一般监控三个领域进行了广泛实验，评估了所提方法在准确性、效率和鲁棒性方面的表现，使用不同长度的视频数据集验证了框架的有效性。

Conclusion: 提出了一个通用的视频情境分析框架，克服了现有方法的局限性，能够跨领域自动检测各种有意义的情境，为视频分析应用提供了有效的解决方案。
...
### [164] [Robust Rigid and Non-Rigid Medical Image Registration Using Learnable Edge Kernels](https://arxiv.org/abs/2512.01771)
...
Method: 集成可学习边缘核与学习式配准技术：从预定义边缘检测核开始，加入随机噪声扰动，在训练中学习提取任务特定的最优边缘特征；设计了刚性配准和非刚性配准各4个变体模型。

Result: 在医学大学提供的三个实验设置（带/不带颅骨去除的刚性配准、非刚性配准）和两个公开数据集上，该方法均优于现有最先进技术。

Conclusion: 该方法通过自适应边缘检测有效改善了多模态医学图像配准和结构分析，展示了在临床和研究应用中的潜力。
...
### [165] [Learned Image Compression for Earth Observation: Implications for Downstream Segmentation Tasks](https://arxiv.org/abs/2512.01788)
*Christian Mollière,Iker Cumplido,Marco Zeulner,Lukas Liesenhoff,Matthias Schubert,Julia Gottfriedsen*

Main category: cs.CV

TL;DR: 评估学习压缩算法在卫星地球观测数据中的潜力，比较传统JPEG 2000与学习压缩方法在三个分割任务上的表现，发现学习压缩在大型多通道光学图像上表现更优，但在小型单通道热红外数据上传统方法仍有竞争力。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测系统产生的数据快速增长给数据传输和存储带来重大挑战，需要评估任务特定的学习压缩算法在减少数据量的同时保留关键信息的潜力。

Method: 比较传统压缩（JPEG 2000）与学习压缩方法（离散混合高斯似然）在三个地球观测分割任务上的表现：火灾检测、云检测和建筑物检测，并评估联合端到端优化与独立优化的效果。

Result: 学习压缩在大型多通道光学图像上显著优于JPEG 2000，在重建质量（PSNR）和分割精度方面都表现更好；但在小型单通道热红外数据集上，传统编解码器由于数据有限和架构限制仍具有竞争力；联合端到端优化并未比独立优化带来性能提升。

Conclusion: 学习压缩算法在卫星地球观测数据压缩中具有潜力，特别是在大型多通道光学图像处理中表现优异，但需要根据数据类型和规模选择合适的压缩方法，联合优化策略在当前研究中未显示出优势。
...
### [166] [SAM3-UNet: Simplified Adaptation of Segment Anything Model 3](https://arxiv.org/abs/2512.01789)
*Xinyu Xiong,Zihuang Wu,Lei Lu,Yufa Xia*

Main category: cs.CV

TL;DR: SAM3-UNet是Segment Anything Model 3的简化变体，通过低成本适配SAM3用于下游任务，在多项任务中表现优于SAM2-UNet和其他SOTA方法，训练时GPU内存需求低于6GB。


<details>
  <summary>Details</summary>
Motivation: 为了以低成本将强大的SAM3模型适配到下游任务中，需要开发一个简化的变体，既能保持性能优势，又能降低计算资源需求。

Method: SAM3-UNet包含三个组件：SAM3图像编码器、用于参数高效微调的简单适配器，以及轻量级的U-Net风格解码器。

Result: 在镜像检测和显著性目标检测等多个任务上的初步实验表明，SAM3-UNet优于之前的SAM2-UNet和其他最先进方法，在批量大小为12的训练中GPU内存需求低于6GB。

Conclusion: SAM3-UNet成功实现了以低成本将SAM3适配到下游任务的目标，在保持高性能的同时显著降低了计算资源需求，代码已开源。
...
### [167] [Generative Action Tell-Tales: Assessing Human Motion in Synthesized Videos](https://arxiv.org/abs/2512.01803)
*Xavier Thomas,Youngsun Lim,Ananya Srinivasan,Audrey Zheng,Deepti Ghadiyaram*

Main category: cs.CV

TL;DR: 提出一种基于真实人类动作学习潜空间的新评估指标，用于评估生成视频中复杂人类动作的视觉和时间正确性，相比现有方法提升68%以上。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型的评估指标存在严重缺陷：纯视觉编码器和多模态大语言模型具有强烈的外观偏见，缺乏时间理解能力，难以识别生成视频中复杂的运动动态和解剖学不合理性。

Method: 通过融合外观无关的人类骨骼几何特征和基于外观的特征，捕捉真实世界运动的细微差别、约束和时间平滑性，构建真实世界动作分布。然后测量生成视频表示与该分布的距离来量化动作质量。

Result: 在新设计的多方面基准测试中，该指标相比现有最先进方法提升超过68%；在现有外部基准上表现有竞争力；与人类感知有更强的相关性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的空间推理能力不足，主要依赖基于文本描述的调整方法，存在"视觉文盲"问题——仅通过文本符号学习空间概念，缺乏与视觉表现的连接。

Method: 提出MILO隐式空间世界建模范式，集成视觉生成器提供几何感知反馈；设计RePE相对位置编码方案捕捉相对相机姿态变换；构建GeoGen大规模几何感知生成数据集（2,241个视频，67,827个观察-动作-结果三元组）。

Result: 实验表明该方法显著提升了多个基线和基准测试中的空间推理能力，实现了对3D空间更全面的理解。

Conclusion: MILO通过模拟人类空间想象的方式，将符号推理隐式地锚定在感知经验中，为解决多模态大语言模型的空间推理瓶颈提供了有效方案。
...
TL;DR: 论文提出视觉因果发现任务，构建VCG-32K数据集，开发CauSight模型，通过因果感知推理在视觉因果发现上超越GPT-4.1


<details>
  <summary>Details</summary>
Motivation: 人类能够理解事物发生的因果关系，而不仅仅是感知存在。为了让现代AI系统具备这种能力，需要让模型能够从视觉场景中推断实体间的因果关系，而不仅仅是识别它们的存在。

Method: 1. 构建VCG-32K数据集（32,000多张带有实体级因果图标注的图像）；2. 开发CauSight视觉语言模型；3. 训练方法包括：从VCG-32K进行数据整理、使用因果思维树合成推理轨迹、设计因果奖励的强化学习来优化推理策略。

Result: CauSight在视觉因果发现任务上显著超越GPT-4.1，实现了超过三倍的性能提升（绝对增益21%）。

Conclusion: 该研究成功提出了视觉因果发现任务，构建了大规模数据集和有效模型，为AI系统理解视觉场景中的因果关系提供了重要基础，所有代码、模型和数据集均已开源。
...
### [170] [OpenREAD: Reinforced Open-Ended Reasoning for End-to-End Autonomous Driving with LLM-as-Critic](https://arxiv.org/abs/2512.01830)
*Songyan Zhang,Wenhui Huang,Zhan Chen,Chua Jiahao Collister,Qihang Huang,Chen Lv*

Main category: cs.CV

TL;DR: OpenREAD是一个端到端强化微调的视觉语言模型自动驾驶框架，通过构建大规模思维链标注和使用Qwen3 LLM作为奖励模型批评者，实现了从高层推理到低层轨迹规划的全面强化微调。


<details>
  <summary>Details</summary>
Motivation: 当前两阶段微调策略（SFT+RFT）存在局限性：SFT限制了推理的泛化能力，而RFT主要应用于下游任务，因为场景理解是开放性问题难以量化奖励。需要解决这些限制以充分发挥自动驾驶性能。

Method: 1. 在开源驾驶相关知识数据集上构建大规模思维链标注；2. 使用强大的Qwen3大语言模型作为RFT中的批评者，为开放性问题量化推理质量；3. 实现从高层推理到低层轨迹规划的端到端强化微调。

Result: 广泛的实验证实，联合端到端RFT在上游和下游任务中都带来了显著改进，使OpenREAD在推理和规划基准测试中达到了最先进的性能。

Conclusion: OpenREAD框架通过端到端强化微调和创新的奖励建模方法，有效解决了当前自动驾驶系统中推理泛化和开放性问题奖励量化的挑战，推动了知识驱动自动驾驶范式的发展。
...
TL;DR: 本文构建了物理不可行性检测数据集PID，开发了轻量级微调方法PhyDetEx，使视觉语言模型能够检测视频中的物理不可行内容并生成解释，用于评估文本到视频生成模型对物理定律的遵循程度。


<details>
  <summary>Details</summary>
Motivation: 尽管文本到视频生成模型在视频质量、长度和指令跟随能力方面取得了显著进展，但这些模型是否真正理解物理原理并生成物理上合理的视频仍然是一个未解决的问题。现有的视觉语言模型作为通用评估器难以识别生成视频中的物理不可能内容。

Method: 1. 构建PID数据集：包含500个手动标注视频的测试集和2,588个配对视频的训练集，每个不可行视频通过仔细重写其对应真实世界视频的标题来诱导T2V模型生成物理上不可行的内容。
2. 开发轻量级微调方法：使视觉语言模型能够检测物理不可行事件并生成关于违反物理原理的文本解释。
3. 创建PhyDetEx系统：作为物理合理性检测器和解释器，用于评估T2V模型对物理定律的遵循程度。

Result: 研究结果表明，尽管最近的T2V模型在生成物理合理内容方面取得了显著进展，但理解和遵循物理定律仍然是一个具有挑战性的问题，特别是对于开源模型。PhyDetEx系统能够有效检测物理不可行内容并提供解释。

Conclusion: 本文通过构建PID数据集和开发PhyDetEx系统，为评估文本到视频生成模型的物理合理性提供了一个有效的框架。研究发现当前T2V模型在物理理解方面仍存在不足，特别是在开源模型中表现更为明显，这为未来改进提供了方向。
...
### [172] [KM-ViPE: Online Tightly Coupled Vision-Language-Geometry Fusion for Open-Vocabulary Semantic SLAM](https://arxiv.org/abs/2512.01889)
...
TL;DR: KM-ViPE是一个实时开放词汇的SLAM框架，适用于动态环境中未校准的单目摄像头，无需深度传感器和离线校准，直接处理原始RGB流


<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统要么需要离线处理，要么需要深度数据和里程计估计，要么缺乏对动态场景的鲁棒性，限制了在自主机器人和AR/VR应用中的实用性

Method: 通过基于高级特征的自适应鲁棒核将DINO视觉特征与几何约束紧密耦合，处理移动物体和可移动静态物体，融合几何和深度视觉特征并与语言嵌入对齐

Result: 与最先进方法竞争，同时具备在线操作、未校准单目输入和动态场景鲁棒处理的独特组合优势

Conclusion: KM-ViPE适用于自主机器人和AR/VR应用，推进了具身AI的实际空间智能能力，受益于互联网规模训练
...
### [173] [StyleYourSmile: Cross-Domain Face Retargeting Without Paired Multi-Style Data](https://arxiv.org/abs/2512.01895)
*Avirup Dey,Vinay Namboodiri*

Main category: cs.CV

TL;DR: StyleYourSmile是一种无需多风格配对数据的单样本跨域人脸重定向方法，通过双编码器框架和扩散模型实现身份、表情和风格属性的解耦控制。


<details>
  <summary>Details</summary>
Motivation: 现有跨域人脸重定向方法要么无法跨域泛化，要么需要测试时优化，要么需要精心策划的多风格数据集进行微调，难以获得域不变的身份表示。

Method: 提出高效数据增强策略和双编码器框架，分别提取域不变身份线索和捕获域特定风格变化，然后利用这些解耦的控制信号条件化扩散模型进行跨域人脸表情重定向。

Result: 广泛实验表明，StyleYourSmile在广泛的视觉领域中实现了优越的身份保持和重定向保真度。

Conclusion: StyleYourSmile无需多风格配对数据即可实现高质量的跨域人脸重定向，在身份保持和重定向质量方面优于现有方法。
...
### [174] [SARL: Spatially-Aware Self-Supervised Representation Learning for Visuo-Tactile Perception](https://arxiv.org/abs/2512.01908)
*Gurmeher Khurana,Lan Wei,Dandan Zhang*

Main category: cs.CV

TL;DR: SARL是一种空间感知的自监督学习框架，通过三个地图级目标增强BYOL架构，在融合视觉-触觉数据上优于现有方法，特别在几何敏感任务上提升30%性能。


<details>
  <summary>Details</summary>
Motivation: 接触丰富的机器人操作需要编码局部几何的表示。视觉提供全局上下文但缺乏纹理和硬度等属性的直接测量，而触觉提供这些线索。现有的自监督学习框架将特征图压缩为全局向量，丢弃了空间结构，与操作任务的需求不匹配。

Method: 提出SARL（空间感知自监督学习框架），在BYOL架构基础上增加三个地图级目标：显著性对齐（SAL）、补丁原型分布对齐（PPDA）和区域亲和匹配（RAM），以保持跨视图的注意力焦点、部件组成和几何关系一致性。

Result: SARL在六个下游任务上持续优于九个自监督学习基线。在几何敏感的边姿态回归任务上，SARL达到0.3955 MAE，相比次优方法（0.5682 MAE）相对提升30%，接近监督学习上限。

Conclusion: 对于融合视觉-触觉数据，最有效的信号是结构化空间等变性，其中特征随对象几何可预测地变化，这实现了更强大的机器人感知能力。
...
### [175] [Med-VCD: Mitigating Hallucination for Medical Large Vision Language Models through Visual Contrastive Decoding](https://arxiv.org/abs/2512.01922)
...
<details>
  <summary>Details</summary>
Motivation: 医疗大视觉语言模型在医疗应用中存在幻觉问题，产生看似合理但实际错误的输出。现有解决方案通常依赖二次解码或回滚过程，会显著降低推理速度，且往往是领域特定的，可能引入模态间或生成内容与真实内容之间的不对齐。

Method: Med-VCD采用稀疏视觉对比解码方法，包含新颖的令牌稀疏化策略，能够动态选择视觉信息丰富的令牌，在保持关键视觉上下文的同时减少冗余，从而在效率和可靠性之间取得平衡。

Result: 在涵盖眼科、放射学和病理学的8个医疗数据集上的评估显示，Med-VCD相对于基线医疗LVLMs，平均将事实准确性提高了13%，幻觉准确性提高了6%。

Conclusion: Med-VCD是一种有效的解码策略，能够显著减少医疗大视觉语言模型中的幻觉输出，同时保持高效的推理速度，在医疗视觉问答、报告生成等应用中具有重要价值。
...
### [176] [Physical ID-Transfer Attacks against Multi-Object Tracking via Adversarial Trajectory](https://arxiv.org/abs/2512.01934)
*Chenyi Wang,Yanmao Man,Raymond Muller,Ming Li,Z. Berkay Celik,Ryan Gerdes,Jonathan Petit*

Main category: cs.CV

TL;DR: AdvTraj：首个针对检测跟踪式多目标追踪的在线物理ID操控攻击，通过对抗性轨迹将攻击者ID转移至目标物体，无需攻击目标检测模块


<details>
  <summary>Details</summary>
Motivation: 多目标追踪（MOT）在计算机视觉中至关重要，但其安全性研究不足。现有攻击要么针对单个物体追踪器，要么通过攻击目标检测模块来操控ID，这些方法模型特定、不鲁棒且仅影响离线数据集。需要研究更通用、鲁棒的在线物理攻击。

Method: 提出AdvTraj攻击方法：攻击者使用对抗性轨迹将自身ID转移至目标物体，从而混淆追踪系统。该方法不攻击目标检测模块，而是针对物体关联阶段。在CARLA模拟器中实现，并分析了生成的轨迹模式，提出了两种可由人类在日常生活场景中执行的通用对抗性机动。

Result: 在白盒攻击SORT算法时，AdvTraj在各种场景下达到100%成功率。由于SOTA MOT算法的共同设计原则，该攻击具有高可迁移性，对SOTA MOT算法的攻击成功率高达93%。识别了两种可在日常场景中执行的通用对抗性机动模式。

Conclusion: 该研究揭示了SOTA MOT系统在物体关联阶段未被充分探索的脆弱性，为增强此类系统的鲁棒性提供了见解。AdvTraj是首个针对检测跟踪式MOT的在线物理ID操控攻击。
...
### [177] [Script: Graph-Structured and Query-Conditioned Semantic Token Pruning for Multimodal Large Language Models](https://arxiv.org/abs/2512.01949)
*Zhongyu Yang,Dannong Xu,Wei Pang,Yingfang Yuan*

Main category: cs.CV

TL;DR: Script是一种即插即用的视觉token剪枝方法，无需重新训练即可应用于多模态大语言模型，通过去除冗余token显著提升推理效率，同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理高分辨率图像和视频时，视觉token数量快速增长，导致内存消耗和推理延迟问题。现有token剪枝方法要么忽略用户查询相关性，要么受限于注意力机制，降低了适应性和有效性。

Method: Script包含两个模块：1) 图结构剪枝模块，用于去除视觉冗余token；2) 查询条件语义剪枝模块，用于保留与查询相关的视觉信息。该方法无需重新训练，可泛化到不同的MLLMs。

Result: 在14个图像和视频理解基准测试中，Script相比现有剪枝方法在模型效率和预测准确性方面表现更优。在LLaVA-NeXT-7B上，实现了最高6.8倍的预填充加速和10倍的FLOP减少，同时保留了96.88%的原始性能。

Conclusion: Script是一种有效的即插即用视觉token剪枝方法，能够显著提升多模态大语言模型的推理效率，同时保持任务性能，解决了现有方法在查询相关性和适应性方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于仿真的方法依赖明确定义的刚性物体模型和预设的手势，无法有效处理与非刚性或可变形物体（如布料、弹性材料、铰链结构、毛绒表面甚至生物）的动态交互。

Method: 采用自回归视频生成框架，输入静态物体图像和想象中手与虚拟物体交互的视频流，通过因果推理架构进行自回归生成，并利用混合后训练方法增强视觉真实性和时间一致性。

Result: 1.3B参数模型在单张NVIDIA RTX 5090 GPU上支持约18FPS、640x368分辨率的实时流式生成，延迟约150毫秒，可连续输出超过一分钟。实验显示在视觉质量、物理合理性和交互保真度方面优于生成式和引擎式基线方法。

Conclusion: SpriteHand能够实时合成多样化的手-物体交互视频，克服了传统物理引擎在处理非刚性物体交互时的局限性，为复杂交互场景的建模提供了有效解决方案。
...
TL;DR: 提出SegCaptioning新任务，通过简单提示（如边界框）生成多样的（描述、掩码）对，并开发基于场景图引导的扩散模型SGDiff来解决该任务


<details>
  <summary>Details</summary>
Motivation: 传统的图像语义理解任务需要用户输入复杂提示（如文本或边界框）来预测单一结果，存在提示输入成本高或输出信息有限的问题。需要一种能从简单提示生成多样化语义解释的方法

Method: 提出场景图引导扩散模型SGDiff：1) Prompt-Centric Scene Graph Adaptor将用户提示映射到场景图以捕捉意图；2) 扩散过程使用Scene Graph Guided Bimodal Transformer预测相关的描述-掩码对；3) 设计Multi-Entities Contrastive Learning损失来显式对齐视觉和文本实体

Result: 在两个数据集上的大量实验表明，SGDiff在SegCaptioning任务上取得了优越性能，能够以最小提示输入在描述和分割任务上都获得有希望的结果

Conclusion: 提出的SegCaptioning任务和SGDiff模型能够从简单提示生成多样化的语义解释，有效解决了传统方法中提示输入成本高和输出信息有限的问题
...
TL;DR: Artemis是一个视觉感知策略学习框架，采用结构化提议推理（标签-边界框对）替代纯语言中间推理，在空间和物体中心空间进行推理，提升感知任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉感知策略的强化学习框架使用自然语言中间推理链，但纯语言推理会降低感知任务性能。核心问题在于推理形式：语言推理在非结构化语义空间进行，而视觉感知需要在空间和物体中心空间进行推理。

Method: 提出Artemis框架，采用结构化提议推理，每个中间步骤表示为（标签，边界框）对，捕捉可验证的视觉状态。基于Qwen2.5-VL-3B构建，支持中间状态显式跟踪、提议质量直接监督，避免语言推理的模糊性。

Result: 在grounding和检测任务上表现强劲，在计数和几何感知任务上展现显著泛化能力。在通用MLLM基准测试中也取得有竞争力的性能，证实空间对齐推理能增强感知策略学习。
...
TL;DR: PAI-Bench是一个评估物理AI感知和预测能力的统一基准，包含2808个真实世界案例，评估视频生成、条件视频生成和视频理解任务。研究发现当前视频生成模型物理连贯性不足，多模态大语言模型预测和因果解释能力有限。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型和视频生成模型在感知和预测真实世界物理动态方面的能力尚未得到充分理解，需要系统评估这些模型是否支持物理AI所需的能力。

Method: 提出了PAI-Bench基准，包含2808个真实世界案例，涵盖视频生成、条件视频生成和视频理解三个任务，使用任务对齐的度量标准来评估物理合理性和领域特定推理能力。

Result: 视频生成模型虽然视觉保真度高，但往往难以保持物理连贯的动态；多模态大语言模型在预测和因果解释方面表现有限。当前系统在处理物理AI的感知和预测需求方面仍处于早期阶段。

Conclusion: PAI-Bench为评估物理AI建立了现实基础，并指出了未来系统需要解决的关键差距，强调需要开发能够保持物理连贯性和进行准确预测的模型。
...
TL;DR: 提出音频-视觉可供性定位(AV-AG)新任务，通过动作声音分割物体交互区域，构建首个AV-AG数据集并提出AVAGFormer模型，在零样本泛化上取得SOTA性能。
...
Conclusion: 音频为可供性定位提供有效线索，AVAGFormer模型能有效融合多模态信息，数据集和代码已开源，为零样本泛化研究提供基础。
...
<details>
  <summary>Details</summary>
Motivation: 多视角相机系统能够对复杂真实世界场景进行丰富观测，理解多视角设置中的动态物体已成为各种应用的核心需求。现有方法在多视角点跟踪方面存在局限性，需要更有效的方法来利用跨视角信息。
...
Conclusion: MV-TAP通过有效利用跨视角信息，在多视角视频中实现了更完整可靠的点轨迹估计，为多视角点跟踪领域提供了新的解决方案和基准。
...
TL;DR: AirSim360是一个用于无人机视角360度全景数据生成的仿真平台，通过渲染对齐的数据标注范式、交互式行人感知系统和自动化轨迹生成，支持大规模多样化全景数据采集，已收集超过6万全景样本。


<details>
  <summary>Details</summary>
Motivation: 360度全方位理解领域缺乏大规模多样化数据，现有仿真器无法系统建模4D真实世界，限制了空间智能的发展。

Method: 1) 渲染对齐的数据和标注范式，支持像素级几何、语义和实体级理解；2) 交互式行人感知系统，建模人类行为；3) 自动化轨迹生成范式，支持导航任务。

Result: 收集了超过6万个全景样本，并在多个任务上进行了广泛实验验证了仿真器的有效性。该平台是首个在全方位设置下系统建模4D真实世界的工作。

Conclusion: AirSim360为360度全方位理解提供了大规模多样化数据支持，将工具包、插件和收集的数据集公开可用，推动空间智能研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂物体运动下的精确运动编辑能力有限：I2V方法缺乏全场景上下文，V2V方法只能提供视角变化或基本平移，无法精细控制物体运动

Method: 基于轨迹条件的V2V框架，使用3D点轨迹表示源和目标运动，建立稀疏对应关系，通过两阶段训练（合成和真实数据）

Result: 支持多种运动编辑：相机/物体联合操控、运动转移、非刚性变形，相比2D轨迹能更好处理深度顺序和遮挡

Conclusion: 3D轨迹提供了明确的深度线索，使模型能够保持时空一致性，为视频编辑开辟了新的创作可能性
...
TL;DR: 视频生成器在模拟重力时存在系统性偏差，表现为物体下落加速度过慢。通过无单位双物体协议测试发现违反伽利略等效原理，但可通过少量数据微调部分纠正。


<details>
  <summary>Details</summary>
Motivation: 评估视频生成器作为世界模型的潜力，需要测试其对物理定律（特别是重力）的编码和理解能力。当前视频生成器在模拟重力时存在明显偏差，需要系统性地分析这些偏差的性质和原因。

Method: 1. 首先测试视频生成器生成物体下落时的加速度偏差；2. 排除尺度模糊性（如帧率假设）的干扰；3. 引入无单位双物体协议，测试时间平方比与高度比的关系（t₁²/t₂² = h₁/h₂），该关系独立于重力加速度g、焦距和尺度；4. 使用轻量级低秩适配器在100个单球下落视频上进行微调。

Result: 1. 现成视频生成器生成物体下落的有效加速度过慢；2. 即使进行时间重缩放也无法纠正高方差的重力伪影；3. 相对测试揭示了违反伽利略等效原理；4. 微调后有效重力加速度从1.81 m/s²提升到6.43 m/s²（达到地球重力的65%）；5. 专用适配器能零样本泛化到双球下落和斜面场景。

Conclusion: 视频生成器在重力表示上存在系统性偏差，违反基本物理定律。但通过针对性的少量数据微调可以部分纠正这些偏差，表明特定物理定律可以通过最小数据量进行修正，为改进视频生成器的物理建模能力提供了方向。
...
### [187] [Text Annotation via Inductive Coding: Comparing Human Experts to LLMs in Qualitative Data Analysis](https://arxiv.org/abs/2512.00046)
*Angelina Parfenova,Andreas Marfurt,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 研究比较了人类专家与开源大语言模型在归纳式编码中的表现，发现人类擅长处理复杂句子但在简单句上表现不佳，而LLMs则呈现相反趋势，且人类标注与真实标签存在系统偏差但更受人类青睐。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在定性数据分析自动化中的应用，特别是与传统依赖预定义标签的演绎方法不同的归纳式编码过程，评估LLMs在数据驱动标签生成方面的能力。

Method: 比较六个开源大语言模型与人类专家的表现，专家评估了他们编码的句子的感知难度，并系统比较人类和LLM生成的标签与测试集黄金标准的偏差。

Result: 发现人类编码者在复杂句子标注上表现良好但在简单句上表现不佳，而LLMs呈现相反趋势；人类标注虽与黄金标准有偏差但更受其他人类青睐，而某些LLMs虽更接近真实标签却获得较低专家评价。

Conclusion: 人类和LLMs在归纳式编码中存在互补优势，人类在复杂语境理解上更优，而LLMs在简单任务上更一致，但两者都存在系统偏差，需要结合各自优势来改进定性数据分析自动化。
...
### [188] [Emergent Convergence in Multi-Agent LLM Annotation](https://arxiv.org/abs/2512.00047)
*Angelina Parfenova,Alexander Denzler,Juergen Pfeffer*

Main category: cs.CL

TL;DR: 该研究通过模拟7500次多智能体讨论，分析LLM在协作任务中的协调行为，发现即使没有明确角色提示，LLM群体也能发展出类似协商的行为模式。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型越来越多地部署在协作环境中，但人们对它们作为黑盒智能体时如何协调知之甚少，需要研究LLM在多智能体交互中的协调动态。

Method: 在归纳编码任务中模拟7500次多智能体、多轮讨论，生成超过125000条话语；引入过程级指标（代码稳定性、语义自一致性、词汇置信度等）；分析输出嵌入的几何演化，特别是内在维度的变化。

Result: LLM群体在词汇和语义上收敛，发展出不对称的影响模式，表现出类似协商的行为；输出嵌入的内在维度随轮次下降，表明语义压缩；黑盒交互分析能揭示涌现的协调策略。

Conclusion: 这项工作展示了黑盒交互分析如何揭示LLM的涌现协调策略，为基于内部探针的可解释性方法提供了可扩展的补充，有助于理解多智能体LLM系统的协调动态。
...
### [189] [Towards Corpus-Grounded Agentic LLMs for Multilingual Grammatical Analysis](https://arxiv.org/abs/2512.00214)
*Matej Klemen,Tjaša Arčon,Luka Terčon,Marko Robnik-Šikonja,Kaja Dobrovoljc*

Main category: cs.CL

TL;DR: 本文提出了一种基于智能大语言模型（LLM）的语料库驱动语法分析框架，通过让LLM在标注语料库上进行推理，为语言学问题提供可解释、数据驱动的答案。


<details>
  <summary>Details</summary>
Motivation: 经验语法研究日益数据驱动，但对标注语料库的系统分析仍需要大量的方法和技术努力。研究者探索如何利用智能大语言模型来简化这一过程，通过推理标注语料库来生成可解释、数据驱动的语言学问题答案。

Method: 引入了一个语料库驱动的语法分析智能框架，整合了自然语言任务解释、代码生成和数据驱动推理等概念。作为概念验证，将其应用于通用依存（UD）语料库，测试其在受世界语言结构图谱（WALS）启发的多语言语法任务上的表现。

Result: 评估涵盖13个词序特征和超过170种语言，从三个互补维度评估系统性能：主导顺序准确性、顺序覆盖完整性和分布保真度。结果表明，将LLM推理与结构化语言数据结合是可行的。

Conclusion: 该研究展示了结合大语言模型推理与结构化语言数据的可行性，为基于语料库的语法查询的可解释、可扩展自动化迈出了第一步。
...
TL;DR: 使用指令调优大语言模型和保守解码的无数据增强方法，在Indic语言语法纠错任务上取得优异表现，在马来语和印地语上分别获得第六和第三名。


<details>
  <summary>Details</summary>
Motivation: Indic语言的语法纠错面临监督数据有限、文字系统多样和形态丰富等挑战，需要开发不依赖数据增强的高效方法。

Method: 采用指令调优的12B GEMMA 3模型，使用4位精度和参数高效微调；通过确定性错误分类器的分类体系、标签分布和优先级顺序合成语言特定提示；采用轻量级规范化器的保守解码策略，鼓励最小化且保持语义的编辑。

Result: 在官方未调整的GLEU评估中，系统在马来语上得分92.41（总体第六），在印地语上得分81.44（总体第三）。

Conclusion: 分类器引导的提示设计、适配器式指令调优和确定性解码为Indic语言语法纠错提供了一种可重复且计算高效的替代方案，无需依赖数据增强流程，为未来更强的形态句法约束和以人为中心的保守编辑评估提供了方向。
...
TL;DR: OmniFusion是一个端到端的多模态翻译系统，通过融合多模态基础模型和翻译大语言模型，能够进行语音到文本、语音+图像到文本、文本+图像到文本的翻译，相比级联流水线减少了延迟并提升了翻译质量。


<details>
  <summary>Details</summary>
Motivation: 当前开源文本翻译大语言模型虽然语言覆盖和质量有所提升，但在语音翻译中只能用于级联流水线（先语音识别再翻译），这会增加延迟（对同声传译尤其关键），且无法利用多模态上下文（如图像）来辅助消歧。而预训练的多模态基础模型虽具备跨模态感知和推理能力，但缺乏专门翻译模型的多语言覆盖和翻译性能。

Method: 提出端到端方法融合多模态基础模型和翻译大语言模型。引入新颖的融合策略，将预训练多模态基础模型多个隐藏层的状态连接到翻译大语言模型，实现联合端到端训练。具体以Omni 2.5-7B作为多模态基础模型，SeedX PPO-7B作为翻译大语言模型构建OmniFusion模型。

Result: OmniFusion能够有效利用音频和视觉输入，在同声传译中相比级联流水线减少了1秒延迟，同时提升了整体翻译质量。模型支持语音到文本、语音+图像到文本、文本+图像到文本的翻译任务。

Conclusion: 通过融合多模态基础模型和翻译大语言模型的端到端方法，成功构建了能够利用多模态上下文的高效翻译系统，在减少延迟的同时提升了翻译性能，为多模态翻译提供了有效解决方案。
...
TL;DR: 大型语言模型在精神科应用中有潜力但存在英语中心化问题，研究测试了模型对印度语言中四种情绪状态短语的理解能力，发现直接嵌入印度语言效果差，翻译到英语或中文后性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神科应用中显示出潜力，但主要是英语中心化的。不同语言有各自的痛苦表达方式，模型能否理解其他语言中的情绪状态尚不清楚。研究旨在量化语言模型对印度语言中四种情绪状态短语的忠实表征能力。

Method: 收集了11种印度语言中247个独特短语，涵盖四种情绪状态（抑郁、情绪稳定、欣快性躁狂、烦躁性躁狂）。测试了七种实验条件：比较直接嵌入原生和罗马化脚本（使用多语言和印度特定模型）与将短语翻译成英语和中文后的嵌入。使用基于调整兰德指数、归一化互信息、同质性和完整性的综合评分来衡量性能。

Result: 直接嵌入印度语言无法有效聚类情绪状态（综合评分=0.002）。所有基于翻译的方法都显示出显著改进。使用Gemini翻译的英语（综合评分=0.60）和人工翻译的英语（综合评分=0.61）嵌入gemini-001模型表现良好。令人惊讶的是，人工翻译的英语进一步翻译成中文并用中文模型嵌入表现最佳（综合评分=0.67）。专门的印度模型（IndicBERT和Sarvam-M）表现不佳。

Conclusion: 当前模型无法直接从印度语言中有效表征情绪状态，这对它们在印度精神科诊断或治疗应用构成了根本障碍。虽然高质量翻译可以弥补这一差距，但依赖专有模型或复杂翻译流程不可持续。模型必须首先构建对多样化本地语言的理解，才能在全球精神健康领域有效应用。
...
TL;DR: EduEval是一个针对中文K-12教育场景的LLM综合评估基准，包含认知框架、真实教育数据和规模化测试，评估了14个主流模型在教育任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在教育应用中有巨大潜力，但未经审查的部署可能对教育标准构成风险，因此需要建立严格的评估体系来确保LLM在教育领域的可靠性和适用性。

Method: 提出了EduEval基准，包含三个核心贡献：1) EduAbility认知框架，整合Bloom分类法和Webb知识深度理论；2) 整合真实教育数据（考试题、课堂对话、学生作文等）；3) 大规模数据集，包含24种任务类型、超过11,000个问题，覆盖小学到高中。

Result: 评估了14个主流LLM，发现：模型在事实性任务上表现良好，但在课堂对话分类方面有困难，创造性内容生成结果不一致；有趣的是，一些开源模型在复杂教育推理任务上优于专有系统；few-shot提示在不同认知维度上效果不一。

Conclusion: 不同教育目标需要定制化的方法，这些发现为开发专门针对中文教育任务优化的LLM提供了有针对性的基准指标，有助于推动LLM在教育领域的负责任应用。
...
TL;DR: 本研究对47个基于上下文的问答模型在8个数据集上进行基准测试，旨在找出无需额外微调的最佳模型。最佳模型ahotrod/electra_large_discriminator_squad2_512在SQuAD数据集上训练，平均准确率43%，并在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: CBQA模型通过考虑上下文信息提供更准确相关的答案，在用户支持、信息检索和教育平台等应用中具有重要价值。本研究旨在评估不同模型在多样化数据集上的表现，为实际应用提供无需重新训练的即用模型选择指导。

Method: 从Hugging Face选取47个CBQA模型，在8个不同数据集上进行基准测试，不进行额外微调。分析模型性能与上下文长度、模型大小、答案长度和上下文复杂度的关系。使用遗传算法整合多个模型的响应以提高整体准确率。

Result: 最佳模型ahotrod/electra_large_discriminator_squad2_512（在SQuAD v2上训练）在所有数据集上平均准确率为43%。该模型在bioasq10b-factoid（65.92%）、biomedical_cpgQA（96.45%）、QuAC（11.13%）和Question Answer Dataset（41.6%）上表现最佳。Bert-large-uncased-whole-word-masking-finetuned-squad在IELTS数据集上达到82%准确率。模型计算时间取决于上下文长度和模型大小，性能通常随答案长度增加而下降，且受上下文复杂度影响。

Conclusion: 在SQuAD数据集上训练的CBQA模型在多样化数据集上表现最佳，无需额外微调即可在实际应用中部署。模型性能受多个因素影响，包括上下文长度、模型大小、答案长度和上下文复杂度。遗传算法可用于整合多个模型的响应以提高准确率。
...
TL;DR: 该研究提出了一种基于SQL的方法来处理演化半结构化表格的时间推理问题，发现模式设计质量对问答精度的影响大于模型容量，并建立了三个基于证据的设计原则。


<details>
  <summary>Details</summary>
Motivation: 当前问答系统在处理演化半结构化表格的时间推理方面存在挑战，需要更有效的方法来解决这一问题。

Method: 提出三步SQL方法：(1)从维基百科信息框生成3NF模式，(2)生成SQL查询，(3)执行查询。强调模式设计质量的重要性。

Result: 最佳配置(Gemini 2.5 Flash模式 + Gemini-2.0-Flash查询)达到80.39 EM，比基线(68.89 EM)提升16.8%。

Conclusion: 模式设计质量对问答精度的影响大于模型容量，提出了三个基于证据的设计原则：保持上下文的规范化、减少歧义的语义命名、一致的时间锚定。
...
TL;DR: IndicParam是一个包含13,000多道选择题的人工标注基准，涵盖11种印度低资源和极低资源语言，评估了19个大语言模型，发现即使是表现最好的GPT-5平均准确率也只有45.0%，揭示了这些语言在跨语言迁移中的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在高资源多语言任务上表现出色，但对低资源和极低资源的印度语言缺乏系统评估。这些语言包括尼泊尔语、古吉拉特语、马拉地语、奥里亚语等低资源语言，以及多格里语、迈蒂利语、拉贾斯坦语、梵语、博多语、桑塔利语、孔卡尼语等极低资源语言，需要专门的基准来评估模型在这些语言上的能力。

Method: 创建了IndicParam基准，包含13,000多道人工标注的多选题，涵盖11种印度语言和梵语-英语代码混合集。将问题标注为知识导向型或纯语言型，以区分事实回忆和语法能力。评估了19个专有和开源大语言模型，包括GPT-5、DeepSeek-3.2和Claude-4.5等。除了传统多选题外，还评估了模型处理列表匹配、断言-理由对和序列排序等多种问题格式的能力。

Result: 评估结果显示，即使是表现最好的GPT-5平均准确率也只有45.0%，DeepSeek-3.2为43.1%，Claude-4.5为42.7%。这表明大语言模型在低资源和极低资源印度语言上的表现仍然有限。研究还提供了关于跨语言迁移局限性的见解，并建立了具有挑战性的印度语言基准。

Conclusion: IndicParam基准揭示了当前大语言模型在低资源和极低资源印度语言上的显著局限性，为评估这些语言的模型能力提供了具有挑战性的基准。该数据集和评估脚本已公开可用，有助于推动印度语言自然语言处理研究的发展。
...
TL;DR: 本文提出CourseTimeQA数据集和CrossFusion-RAG模型，用于教育视频的时间戳问答任务，在单GPU资源限制下实现高效检索和答案生成。


<details>
  <summary>Details</summary>
Motivation: 研究在单GPU延迟/内存预算下，对教育讲座视频进行时间戳问答。现有方法在资源受限环境下性能不足，需要开发轻量级、低延迟的跨模态检索系统。

Method: 提出CrossFusion-RAG模型：结合冻结编码器、学习的视觉投影（512->768）、浅层查询无关的跨模态注意力（处理ASR和视频帧）、时间一致性正则化器，以及小型交叉注意力重排序器。

Result: 在CourseTimeQA数据集（52.3小时，902个查询）上，CrossFusion-RAG比BLIP-2检索器在nDCG@10上提升0.10，MRR提升0.08，同时在单A100上实现约1.55秒的中位端到端延迟。模型对ASR噪声具有鲁棒性。

Conclusion: CrossFusion-RAG在资源受限环境下实现了教育视频时间戳问答的高效检索，为轻量级跨模态检索系统提供了有效解决方案，支持可重复比较。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLM被用作可扩展的相关性标注工具，但研究表明它们容易受到启动效应的影响，即先前的相关性判断会影响后续判断。心理学理论将人格特质与这种偏差联系起来，但尚不清楚LLM中模拟的人格是否表现出类似效应

Method: 研究使用多个LLM在TREC 2021和2022深度学习赛道数据集上，通过大五人格配置文件来研究人格对相关性标注中启动效应的影响

Result: 结果显示某些人格特质（如高开放性和低神经质）能持续降低启动敏感性。此外，最有效缓解启动效应的人格可能因模型和任务类型而异

Conclusion: 基于这些发现，研究者提出人格提示作为一种缓解阈值启动效应的方法，将心理学证据与基于LLM的评估实践联系起来
...
TL;DR: 开发了一个基于大语言模型的AI辅助英语写作错误分析系统，能够识别、分类和纠正各类语言错误，为EFL教学提供自动化详细反馈。


<details>
  <summary>Details</summary>
Motivation: 传统基于量规的写作评估往往缺乏细致的错误分析，无法为英语学习者提供具体的语言改进指导。本研究旨在利用AI技术自动化这一过程，为EFL教学提供更精准、详细的错误分析反馈。

Method: 基于Corder、Richards和James的语言学理论构建详细错误分类体系，涵盖单词和句子层面的拼写、语法和标点错误。使用Claude 3.5 Sonnet和DeepSeek R1等大语言模型，通过Python编码的API调用实现系统，并在包含真实语言错误的经典文本上进行测试验证。

Result: 系统成功识别了多种错误类型，但在上下文理解方面存在局限，遇到未编码的错误类型时会生成新的错误类别。在包含丰富真实错误的"English as she is spoke"文本测试中，系统展现了处理复杂、多层次分析的能力。

Conclusion: AI在EFL教学中具有变革潜力，能够自动化详细的错误分析和反馈。虽然前景广阔，但需要进一步改进上下文理解的准确性，并将分类体系扩展到文体和语篇层面的错误分析。
...
TL;DR: SCALE框架通过选择性分配计算资源解决测试时计算扩展的瓶颈问题，根据子问题难度分配不同处理模式，在提升性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前测试时计算扩展方法对所有推理子问题采用统一的资源分配，导致具有挑战性的子问题得不到足够关注，而常规操作却消耗不成比例的资源，造成性能瓶颈和计算资源浪费。

Method: 提出SCALE框架，包含四个阶段：1) 问题分解为顺序推理子问题；2) 评估每个子问题的难度；3) 根据难度分配处理模式（简单问题用System 1，复杂问题用System 2）；4) 顺序执行并传播上下文。

Result: SCALE显著优于统一扩展基线，在AIME25上准确率提升13.75个百分点（从57.50%到71.25%），同时计算成本降低33%-53%，实现了性能提升和资源利用效率的双重优化。

Conclusion: SCALE通过选择性资源分配解决了当前测试时计算扩展方法的根本限制，将计算资源集中在具有挑战性的子问题上，同时高效处理常规操作，代表了测试时扩展方法的重要进展。
...
TL;DR: CACARA是一种通过涌现对齐学习训练的多模态多语言架构，能够在不需要完全重新训练的情况下将新模态无缝集成到现有的双模态/多模态模型中，并且仅通过英语对齐数据就能支持100多种语言。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型通常需要跨多个模态进行资源密集型的训练，扩展到新语言也遵循同样的资源密集型策略。需要一种更高效的方法来集成新模态和语言能力。

Method: 提出CACARA架构，采用涌现对齐学习范式，仅需对新加入的模态进行英语对齐数据微调，就能实现多模态和多语言能力，无需完全重新训练或显式的多语言预训练。

Result: 在音频到文本检索任务中实现了R@1指标14.24个百分点的提升，超越了最先进的多模态模型，且训练成本仅相当于单语言模型。

Conclusion: 涌现对齐学习范式能够高效解锁多语言能力，仅通过英语对齐数据就能支持100多种语言，同时保持先前学到的知识，大大降低了多模态多语言模型的训练成本。
...
### [202] [G-KV: Decoding-Time KV Cache Eviction with Global Attention](https://arxiv.org/abs/2512.00504)
*Mengqi Liao,Lu Wang,Chaoyun Zhang,Zekai Shen,Xiaowei Mao,Si Qin,Qingwei Lin,Saravan Rajmohan,Dongmei Zhang,Huaiyu Wan*

Main category: cs.CL

TL;DR: G-KV是一种KV缓存驱逐方法，通过全局评分机制结合局部和历史注意力分数来更准确评估token重要性，并引入强化学习和蒸馏等后训练技术优化压缩KV缓存设置下的模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前推理大语言模型在处理长序列时面临显著的计算和内存挑战。现有的KV缓存压缩方法主要关注提示压缩或基于局部注意力分数的token驱逐，忽略了token的长期重要性。

Method: 提出G-KV方法，采用全局评分机制结合局部和历史注意力分数来评估token重要性；引入后训练技术包括强化学习和蒸馏，优化模型在压缩KV缓存设置下的性能。

Result: 该方法在保持模型性能的同时显著提高了推理效率，代码已在GitHub开源。

Conclusion: G-KV通过全局评分机制和后训练优化，为长序列推理中的KV缓存压缩提供了更有效的解决方案，平衡了计算效率和模型性能。
...
### [203] [Catch Me If You Can: How Smaller Reasoning Models Pretend to Reason with Mathematical Fidelity](https://arxiv.org/abs/2512.00552)
*Subramanyam Sahoo,Vinija Jain,Saanidhya Vats,Siddharth Mohapatra,Rui Min,Aman Chadha,Divya Chaudhary*

Main category: cs.CL

TL;DR: 本文提出了一个诊断框架，通过四个互补维度（前向-后向一致性、传递性覆盖、反事实敏感性、扰动鲁棒性）来区分语言模型中的真实数学推理与表面模式匹配，揭示传统准确率指标无法发现的推理缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型数学推理评估主要依赖答案准确率，这可能掩盖逻辑计算中的根本性失败。需要开发能够区分真实数学推理与表面模式匹配的诊断方法，以更全面地评估模型的推理能力。

Method: 提出了一个包含四个互补维度的诊断框架：1) 前向-后向一致性：检查模型在正向和反向推理中的一致性；2) 传递性覆盖：评估模型处理传递推理的能力；3) 反事实敏感性：测试模型对反事实假设的敏感性；4) 扰动鲁棒性：检验模型对输入扰动的稳定性。在Qwen3-0.6B模型和MenatQA数据集上进行了案例研究。

Result: 在Qwen3-0.6B模型上的实验显示：虽然模型在MenatQA数据集上获得了合理的答案准确率（70%+），但在四个诊断维度上表现不佳：后向一致性仅15%，传递性覆盖仅32.2%，对扰动表现出脆弱的敏感性。这表明该模型严重依赖模式匹配而非真正的逻辑计算。

Conclusion: 该诊断框架能够揭示传统准确率指标无法发现的推理缺陷，表明小规模模型主要依赖模式匹配而非真实数学推理。框架本身是模型无关且可推广的，为研究社区提供了评估不同规模和架构模型推理保真度的工具，推动从表面准确率向可验证数学推理的转变。
...
### [204] [Slovak Conceptual Dictionary](https://arxiv.org/abs/2512.00579)
*Miroslav Blšták*

Main category: cs.CL

TL;DR: 本文介绍了为斯洛伐克语创建首个概念词典，以解决低资源语言缺乏机器可读语言数据的问题。


<details>
  <summary>Details</summary>
Motivation: 斯洛伐克语作为资源有限的语言，缺乏足够大规模的机器可读语言数据源，导致许多需要自动化文本处理的任务相比其他语言表现较差甚至无法解决。

Method: 为斯洛伐克语开发了一个新的概念词典，这是该语言首个此类语言工具。

Result: 创建了斯洛伐克语的首个概念词典，填补了该语言在机器可读语言数据方面的空白。

Conclusion: 该概念词典为斯洛伐克语的自然语言处理任务提供了重要的语言工具，有助于改善低资源语言的自动化文本处理能力。
...
### [205] [Wikontic: Constructing Wikidata-Aligned, Ontology-Aware Knowledge Graphs with Large Language Models](https://arxiv.org/abs/2512.00590)
*Alla Chepurova,Aydar Bulatov,Yuri Kuratov,Mikhail Burtsev*

Main category: cs.CL

TL;DR: Wikontic是一个从开放域文本构建知识图谱的多阶段管道，通过提取带限定符的三元组、强制执行类型和关系约束、规范化实体来减少重复，生成紧凑、本体一致且连接良好的知识图谱。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的系统通常将知识图谱作为文本检索的辅助结构，未能充分探索其内在质量。需要一种方法能够从开放域文本构建高质量、本体一致的知识图谱，以更好地利用结构化知识。

Method: 提出多阶段管道：1) 提取带限定符的候选三元组；2) 强制执行基于Wikidata的类型和关系约束；3) 规范化实体以减少重复。生成的KG紧凑、本体一致且连接良好。

Result: 在MuSiQue上，正确答案实体出现在96%的生成三元组中。在HotpotQA上，仅使用三元组设置达到76.0 F1，在MuSiQue上达到59.8 F1，匹配或超越多个仍需文本上下文的检索增强生成基线。在MINE-1基准上达到最先进的86%信息保留性能。构建时高效，使用少于1,000个输出token。

Conclusion: Wikontic管道提高了生成知识图谱的质量，并为在大语言模型中利用结构化知识提供了可扩展的解决方案，展示了仅使用结构化知识就能达到与需要文本上下文的系统相媲美的性能。
...
### [206] [ART: Adaptive Response Tuning Framework -- A Multi-Agent Tournament-Based Approach to LLM Response Optimization](https://arxiv.org/abs/2512.00617)
*Omer Jauhar Khan*

Main category: cs.CL

TL;DR: ART框架通过锦标赛式ELO排名和多智能体推理优化LLM输出，使用多个LLM智能体竞争、批判和协作，生成优于单个模型的共识响应，显著提升响应质量。
...
Method: 提出ART框架，采用锦标赛式ELO排名机制和多智能体推理方法，让多个LLM智能体通过结构化锦标赛工作流程进行竞争、批判和协作，包含可配置的锦标赛参数、动态智能体选择和多种共识融合策略。

Result: 实验评估显示，相比基线单模型方法，ART框架在响应准确性、连贯性和可靠性方面有显著改进，整体质量指标提升8.4%，ELO评分收敛的R²值超过0.96。

Conclusion: ART框架为需要高质量、经过验证的LLM响应的应用提供了可扩展、生产就绪的解决方案，通过多智能体协作显著提升了LLM输出的质量。
...
### [207] [Sycophancy Claims about Language Models: The Missing Human-in-the-Loop](https://arxiv.org/abs/2512.00656)
*Jan Batzner,Volker Stocker,Stefan Schmid,Gjergji Kasneci*

Main category: cs.CL

TL;DR: 该论文回顾了大型语言模型中奉承响应模式的研究方法挑战，识别了五种核心操作化定义，指出当前研究缺乏对人类感知的评估，并提出了未来研究的建议。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的奉承响应模式在文献中越来越多地被提出，但研究方法存在挑战，需要系统性地评估和澄清这一现象。

Method: 回顾了测量LLM奉承性的方法学挑战，识别了五种核心操作化定义，分析了当前研究的局限性，特别是缺乏对人类感知的评估。

Result: 发现奉承性本质上是以人为中心的，但当前研究没有评估人类感知，难以区分奉承响应与AI对齐中的相关概念。

Conclusion: 提出了可操作的建议，强调未来研究需要更好地定义和测量奉承性，特别是要纳入人类感知的评估。
...
### [208] [Graphing the Truth: Structured Visualizations for Automated Hallucination Detection in LLMs](https://arxiv.org/abs/2512.00663)
*Tanmay Agrawal*

Main category: cs.CL

TL;DR: 论文提出一个可视化知识图谱框架，帮助用户识别LLM在企业应用中的幻觉问题，通过将模型生成内容与知识源链接并提供置信度，创建人机协作的反馈循环来提升模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在企业环境中经常需要结合闭源领域知识来提供更符合上下文的响应，但存在上下文窗口限制、预训练数据与提供知识不一致等问题，导致产生难以通过常规人工审查发现的可信幻觉。现有缓解策略要么依赖昂贵的大规模黄金标准问答集，要么依赖二次模型验证，都无法提供确定性保证。

Method: 引入一个框架，将专有知识和模型生成内容组织成交互式可视化知识图谱。该框架将模型断言与底层真实来源链接，并显示置信度水平，为用户提供清晰的幻觉区域视图。用户可以通过可视化界面诊断不一致性、识别薄弱推理链并提供纠正反馈。

Result: 通过可视化知识图谱界面，用户可以更直观地识别模型幻觉区域，诊断不一致性，并提供纠正反馈。这种人机协作的工作流程创建了结构化反馈循环。

Conclusion: 该可视化知识图谱框架为用户提供了识别LLM幻觉的有效工具，通过人机协作的反馈循环能够增强模型可靠性并持续改进响应质量，解决了现有缓解策略的局限性。
...
### [209] [A Comparison of Human and ChatGPT Classification Performance on Complex Social Media Data](https://arxiv.org/abs/2512.00673)
...
<details>
  <summary>Details</summary>
Motivation: 尽管生成式AI工具如ChatGPT在计算社会科学中应用日益广泛，但对其在复杂任务（如分类和标注包含微妙语言的数据集）中的性能了解仍不足，需要系统评估

Method: 比较GPT-4与人类标注者的表现，测试ChatGPT 3.5、4和4o版本，设计四种提示风格作为输入，评估精确率、召回率和F1分数，进行定量和定性分析

Result: 虽然提示中包含标签定义可能有助于提升性能，但总体上GPT-4在分类微妙语言方面存在困难。定性分析揭示了四个具体发现，表明ChatGPT在涉及微妙语言的分类任务中应谨慎使用

Conclusion: ChatGPT在处理微妙语言的分类任务中表现有限，尽管技术快速进步，但在复杂文本分析任务中仍需谨慎使用，不能完全替代人类标注
...
### [210] [FastPOS: Language-Agnostic Scalable POS Tagging Framework Low-Resource Use Case](https://arxiv.org/abs/2512.00745)
*Md Abdullah Al Kafi,Sumit Kumar Banshal*

Main category: cs.CL

TL;DR: 提出一个语言无关的基于Transformer的词性标注框架，针对低资源语言，使用孟加拉语和印地语作为案例研究，仅需少量代码修改即可实现跨语言迁移。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言开发词性标注系统面临数据稀缺和模型设计复杂性的挑战，需要一种能够快速适应不同语言、减少模型设计和调优开销的解决方案。

Method: 采用语言无关的Transformer架构，设计模块化开源框架，仅需三行框架特定代码即可从孟加拉语适应到印地语，利用预训练Transformer模型进行词性标注。

Result: 在孟加拉语和印地语上分别达到96.85%和97%的词级别准确率，尽管数据集存在不平衡和语言重叠问题，仍保持较高的F1分数。特定词性类别存在性能差异，凸显数据集标注的挑战。

Conclusion: 该框架的模块化设计和语言无关特性显著降低了跨语言适应的技术门槛，使研究者能够专注于语言预处理和数据集优化，这对推进低资源语言的NLP研究至关重要。
...
### [211] [Auxiliary-Hyperparameter-Free Sampling: Entropy Equilibrium for Text Generation](https://arxiv.org/abs/2512.00789)
*Xiaodong Cai,Hai Lin,Shaoxiong Zhan,Weiqi Luo,Hong-Gee Kim,Hongyan Hao,Yu Yang,Hai-Tao Zheng*

Main category: cs.CL

TL;DR: 提出了一种无需超参数调优的熵平衡采样方法EES，通过平衡归一化熵与概率质量来动态调整候选集，在各种任务和模型架构中表现稳定。


<details>
  <summary>Details</summary>
Motivation: 现有token采样策略需要额外超参数调优，增加了部署复杂性，需要一种无需超参数调优的采样方法。

Method: 提出熵平衡采样（EES），基于信息论原理，通过平衡归一化熵与概率质量来动态调整候选集，无需额外超参数。

Result: 在推理和生成任务上评估EES，结果显示EES在不同温度设置下表现稳定，提供有竞争力的准确性和连贯性，同时保持多样性。

Conclusion: EES通过消除超参数调优需求，大大简化了部署过程，同时提升了性能，是一种有效的辅助采样方法。
...
### [212] [Accelerating Bangla NLP Tasks with Automatic Mixed Precision: Resource-Efficient Training Preserving Model Efficacy](https://arxiv.org/abs/2512.00829)
*Md Mehrab Hossain Opi,Sumaiya Khan,Moshammad Farzana Rahman*

Main category: cs.CL

TL;DR: 该研究探索了自动混合精度训练在孟加拉语NLP任务中的应用，通过混合16位和32位浮点计算，在保持模型性能的同时显著提升训练效率和降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语NLP开发面临计算资源有限、训练成本高的挑战，特别是在硬件受限的环境中，需要寻找既能保持模型性能又能提高计算效率的方法。
...
Result: AMP训练将训练速度提高了44.5%，内存消耗降低了17.6%，同时F-1分数保持在全精度基线的99.7%范围内，表明在几乎不损失性能的情况下显著提升了计算效率。

Conclusion: 自动混合精度训练是降低计算门槛、在硬件受限环境中实现先进NLP能力的有效方法，有助于促进孟加拉语NLP技术的民主化应用。
...
### [213] [WaterSearch: A Quality-Aware Search-based Watermarking Framework for Large Language Models](https://arxiv.org/abs/2512.00837)
*Yukang Lin,Jiahao Shao,Shuoran Jiang,Wentao Zhu,Bingjie Lu,Xiangping Wu,Joanna Siebert,Qingcai Chen*

Main category: cs.CL

TL;DR: WaterSearch是一个基于搜索的句子级水印框架，通过控制种子池实现多样化并行生成，在保持95%检测率的同时显著提升文本质量，并在各种攻击场景下保持高检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM水印方法在操纵token生成概率时面临检测能力与文本质量之间的权衡：为了获得鲁棒的水印信号所需的信号强度和随机性往往会降低下游任务的性能。

Method: 设计了一种新颖的嵌入方案，通过控制种子池实现水印文本的多样化并行生成。基于此方案提出了WaterSearch框架，联合优化两个关键方面：1)分布保真度 2)水印信号特性。还配套了具有强攻击鲁棒性的句子级检测方法。

Result: 在三个流行LLM和十个不同任务上的实验表明，在95%水印检测强度下，平均性能比最先进基线提升51.01%。在短文本生成和低熵输出生成等挑战性场景中，分别获得47.78%和36.47%的性能提升。在不同攻击场景下保持高检测能力。

Conclusion: WaterSearch通过搜索框架有效解决了水印检测能力与文本质量之间的权衡问题，在各种任务和攻击场景下都表现出优越的性能和鲁棒性。
...
### [214] [Less is More: Resource-Efficient Low-Rank Adaptation](https://arxiv.org/abs/2512.00878)
*Chunlin Tian,Xuyang Wei,Huanrong Liu,Zhijiang Guo,Li Li*

Main category: cs.CL

TL;DR: EffiLoRA是一种资源高效的低秩适配方法，通过统一A矩阵和选择性更新B矩阵来减少参数冗余，在多种模态任务中优于传统LoRA。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA方法在微调大型语言模型时仍存在显著开销，且在复杂数据集上存在参数干扰问题。虽然近期工作通过解耦LoRA更新矩阵来利用矩阵不对称性，但训练成本仍然很高。

Method: EffiLoRA从矩阵间和层内参数冗余的角度重新审视LoRA，采用跨所有Transformer层的统一A矩阵，并引入运行时选择性B矩阵更新机制，动态平衡系统资源预算和模型性能。

Result: EffiLoRA在多种模态任务中持续优于传统LoRA，包括常识推理、视觉指令调优和图像生成，展示了改进的效率和鲁棒性。

Conclusion: EffiLoRA是一种轻量级且可泛化的方法，适用于语言、多模态和扩散模型，通过减少参数冗余和动态资源分配实现了更高效的模型适配。
...
### [215] [Reward Auditor: Inference on Reward Modeling Suitability in Real-World Perturbed Scenarios](https://arxiv.org/abs/2512.00920)
*Jianxiang Zang,Yongda Wei,Ruxue Bai,Shiyu Jiang,Nijia Mo,Binhong Li,Qiang Sun,Hui Liu*

Main category: cs.CL

TL;DR: Reward Auditor：一个基于假设检验的框架，用于评估奖励模型在真实世界扰动场景下的适用性，而非仅关注偏好感知准确率。


<details>
  <summary>Details</summary>
Motivation: 当前奖励模型评估方法仅关注特定场景下的偏好感知准确率，忽略了真实世界场景中的关键脆弱性。需要评估奖励模型在特定真实世界扰动下的条件可靠性（适用性）。

Method: 提出Reward Auditor框架，采用科学审计方法，通过审计奖励模型偏好感知置信度的分布退化来量化统计显著性和效应大小，推断奖励模型在特定真实世界场景中的系统性脆弱性。

Result: 该框架能够推断奖励模型在不同真实世界场景中脆弱性的确定性和严重程度，为构建可验证安全、更稳健和可信赖的下一代LLM对齐系统奠定基础。

Conclusion: Reward Auditor框架通过评估奖励模型的适用性（条件可靠性），解决了当前评估方法的局限性，为构建更安全的LLM对齐系统提供了重要工具。
...
### [216] [Mitigating Hallucinations in Zero-Shot Scientific Summarisation: A Pilot Study](https://arxiv.org/abs/2512.00931)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成过程中会产生与用户提示不一致的上下文幻觉，特别是在零样本科学文本摘要任务中。本研究旨在探索是否可以通过提示工程方法来缓解这种幻觉问题。

Method: 使用8篇酵母生物技术论文摘要，对6个指令调优的大语言模型应用7种提示方法：基线提示、两种复杂度递增的指令(PE-1和PE-2)、两种上下文重复(CR-K1和CR-K2)、两种随机添加(RA-K1和RA-K2)。上下文重复涉及识别并重复摘要中的K个关键句子，随机添加涉及重复K个随机选择的句子。共生成336个摘要，使用6个评估指标(ROUGE-1、ROUGE-2、ROUGE-L、BERTScore、METEOR、余弦相似度)计算摘要与原文的词汇和语义对齐。

Result: 统计分析3744个数据点显示，上下文重复(CR)和随机添加(RA)方法能显著改善大语言模型生成摘要与原文的词汇对齐。使用偏差校正加速自助法置信区间和Bonferroni-Holm校正的Wilcoxon符号秩检验验证了这一结果。
...
### [217] [DeformAr: Rethinking NER Evaluation through Component Analysis and Visual Analytics](https://arxiv.org/abs/2512.00938)
*Ahmed Mustafa Younes*

Main category: cs.CL

TL;DR: DeformAr是一个专门用于分析和解释阿拉伯语与英语命名实体识别系统性能差异的调试与评估框架，通过跨组件分析和行为分析两种模式，结合数据提取库和交互式仪表板，为资源匮乏语言提供首个基于组件的可解释性工具。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在英语NLP任务中表现出色，但在阿拉伯语命名实体识别中的性能仍然有限，即使使用更大的预训练模型。这种性能差距源于分词、数据集质量和标注不一致等多种因素，现有研究往往孤立分析这些问题，未能捕捉它们对系统行为和性能的联合影响。
...
Result: DeformAr是首个阿拉伯语专用的基于组件的可解释性工具，能够检测模型行为并通过将其与底层表示模式和数据因素联系起来进行解释，为资源匮乏语言提供了关键的模型分析资源。

Conclusion: DeformAr框架填补了阿拉伯语NLP分析工具的空白，通过系统化的诊断方法揭示了阿拉伯语与英语NER系统性能差异的根本原因，为资源匮乏语言的模型分析和改进提供了重要工具。
...
### [218] [Fine-tuning of lightweight large language models for sentiment classification on heterogeneous financial textual data](https://arxiv.org/abs/2512.00946)
*Alvaro Paredes Amorin,Andre Python,Christoph Weisser*

Main category: cs.CL

TL;DR: 轻量级开源大语言模型（如Qwen3 8B和Llama3 8B）在金融情感分析任务中表现优异，即使仅使用5%的训练数据也能达到竞争力性能，为资源受限的研究者和从业者提供了成本效益高的解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统金融NLP模型依赖大量计算资源和专有数据集，成本高昂且对许多研究者和从业者不可及。本研究旨在探索轻量级开源LLMs在有限计算资源下处理金融文本情感分析的能力，为资源受限环境提供可行方案。

Method: 比较基准金融NLP模型FinBERT与三个开源轻量级LLMs（DeepSeek-LLM 7B、Llama3 8B Instruct、Qwen3 8B）在五个公开数据集上的表现：FinancialPhraseBank、Financial Question Answering、Gold News Sentiment、Twitter Sentiment和Chinese Finance Sentiment。评估模型在不同规模训练数据（包括仅5%数据）以及零样本和少样本学习场景下的泛化能力。

Result: 轻量级LLMs（特别是Qwen3 8B和Llama3 8B）在大多数场景中表现最佳，即使仅使用5%的训练数据也能获得竞争力性能。这些结果在零样本和少样本学习场景中保持一致。模型能够处理不同规模、来源、格式和语言的金融文本数据。

Conclusion: 轻量级开源大语言模型为金融文本分析提供了成本效益高的替代方案，能够在有限计算资源和训练数据下实现竞争力性能，降低了金融NLP应用的门槛，使更多研究者和从业者能够利用先进的语言模型技术。
...
### [219] [Advancing Academic Chatbots: Evaluation of Non Traditional Outputs](https://arxiv.org/abs/2512.00991)
*Nicole Favero,Francesca Salute,Daniel Hardt*

Main category: cs.CL

TL;DR: 该研究评估了两种RAG检索策略（Graph RAG和Advanced RAG）在问答任务中的表现，并测试了LLM生成非传统学术输出（幻灯片和播客脚本）的能力，发现GPT 4o mini配合Advanced RAG表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估主要关注事实问答或简短摘要等标准任务，本研究旨在扩展评估范围：一是比较两种检索策略在问答任务中的效果，二是评估LLM生成非传统学术输出（如幻灯片和播客脚本）的能力。

Method: 研究实现了一个原型系统，结合Meta的LLaMA 3 70B开源模型和OpenAI的GPT 4o mini API。问答性能通过人工评分（11个质量维度）和大语言模型法官进行可扩展交叉验证。幻灯片和播客生成测试采用文档检索增强方法。

Result: GPT 4o mini配合Advanced RAG产生了最准确的回答。Graph RAG改进有限且导致更多幻觉，部分原因是其结构复杂性和手动设置。在生成任务中，GPT 4o mini再次表现最佳，但LLaMA 3在叙事连贯性方面有潜力。人工评审在检测布局和风格缺陷方面至关重要。

Conclusion: 研究强调了结合人工和LLM评估在评估新兴学术输出中的必要性，GPT 4o mini配合Advanced RAG在问答任务中表现最优，而Graph RAG由于复杂性未能带来显著改进。
...
### [220] [When Safety Blocks Sense: Measuring Semantic Confusion in LLM Refusals](https://arxiv.org/abs/2512.01037)
*Riad Ahmed Anonto,Md Labid Al Nahiyan,Md Tanvir Hassan,Ch. Md. Rakin Haider*

Main category: cs.CL

TL;DR: 论文提出"语义混淆"概念来评估语言模型在相近语义表达上的不一致拒绝行为，并开发了ParaGuard数据集和三个模型无关的指标来测量这种局部不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前的安全对齐语言模型经常错误拒绝无害提示，现有评估方法主要关注全局拒绝率或合规率，忽略了局部不一致性——即模型接受某种意图表达但拒绝其相近释义的问题，这限制了模型的诊断和调优。

Method: 1) 提出"语义混淆"概念捕捉局部不一致性；2) 构建ParaGuard数据集，包含10k个控制性释义簇，保持意图不变而改变表面形式；3) 提出三个模型无关的token级指标：混淆指数、混淆率和混淆深度，通过比较每个拒绝与其最近接受邻居，利用token嵌入、下一token概率和困惑度信号。

Result: 实验显示全局错误拒绝率掩盖了关键结构：某些设置中全局边界不稳定，其他设置中存在局部不一致性区域，还有情况显示更严格的拒绝并未增加不一致性。混淆感知审计能将系统拒绝频率与拒绝合理性分开。

Conclusion: 该框架为开发者提供了实用信号，可在保持安全性的同时减少错误拒绝，揭示了模型安全边界的不稳定性，并为更精细的模型评估和调优提供了工具。
...
### [221] [ELR-1000: A Community-Generated Dataset for Endangered Indic Indigenous Languages](https://arxiv.org/abs/2512.01077)
...
TL;DR: 该研究收集了来自印度东部偏远地区10种濒危语言的1060道传统食谱，创建了ELR-1000数据集，并评估了大型语言模型在翻译这些文化特定内容时的表现，发现提供针对性上下文能显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 当前语言技术主要服务于主流语言，对濒危语言和特定文化领域的支持不足。传统食谱作为文化传承的重要载体，包含丰富的语言和文化细微差别，但相关数据集和评估基准缺乏。研究旨在填补这一空白，促进对濒危语言的公平和文化敏感的语言技术发展。

Method: 1. 通过为低数字素养贡献者设计的移动界面，从印度东部偏远农村社区众包收集1060道传统食谱，涵盖10种濒危语言；2. 创建ELR-1000数据集，包含烹饪实践和社会文化背景；3. 评估多个最先进的大型语言模型在翻译这些食谱时的表现；4. 测试提供针对性上下文（语言背景信息、翻译示例、文化保护指南）对翻译质量的影响。

Result: 1. 大型语言模型在处理低资源、文化特定语言时表现不佳；2. 提供针对性上下文（包括语言背景信息、翻译示例和文化保护指南）能显著提高翻译质量；3. 创建并发布了ELR-1000数据集，包含1060道来自10种濒危语言的食谱，为NLP社区提供资源。

Conclusion: 研究强调了为代表性不足的语言和领域开发专门基准的必要性，以推动公平和文化敏感的语言技术发展。ELR-1000数据集的发布旨在激励为濒危语言开发语言技术，促进文化多样性的保护。
...
### [222] [How do we measure privacy in text? A survey of text anonymization metrics](https://arxiv.org/abs/2512.01109)
*Yaxuan Ren,Krithika Ramesh,Yaxing Yao,Anjalie Field*

Main category: cs.CL

TL;DR: 本文通过系统调查澄清和协调文本隐私保护的评估指标，分析47篇论文中的6种隐私概念及其度量方法，评估它们与法律标准（HIPAA和GDPR）和用户期望的契合度，为文本匿名化提供更稳健、可比较且符合法律的隐私评估指导。


<details>
  <summary>Details</summary>
Motivation: 文本匿名化对于在敏感数据领域开展NLP研究和模型开发至关重要，但如何评估匿名化方法是否充分保护隐私仍是一个开放挑战。当前隐私评估指标存在不一致和模糊性，需要系统梳理和协调。

Method: 通过系统调查方法，手动审查47篇报告隐私指标的论文，识别和比较6种不同的隐私概念，分析相关指标如何捕捉隐私风险的不同方面。然后评估这些概念与法律隐私标准（HIPAA和GDPR）以及基于人机交互研究的用户中心期望的契合度。

Result: 识别出6种不同的隐私概念及其关联指标，揭示了当前隐私评估实践的多样性。分析发现这些概念与法律标准和用户期望存在不同程度的契合，指出了当前实践中的差距和不足。

Conclusion: 本文为导航隐私评估方法提供了实用指导，强调了当前实践的不足，旨在促进文本匿名化中更稳健、可比较且具有法律意识的隐私评估。最终目标是推动隐私保护评估的标准化和规范化。
...
### [223] [Generalist Large Language Models Outperform Clinical Tools on Medical Benchmarks](https://arxiv.org/abs/2512.01191)
*Krithik Vishwanath,Mrigayu Ghosh,Anton Alyakin,Daniel Alexander Alber,Yindalon Aphinyanaphongs,Eric Karl Oermann*

Main category: cs.CL

TL;DR: 专业临床AI助手在医学实践中表现不如通用大语言模型，GPT-5等前沿模型在医学知识和临床对齐任务上全面优于OpenEvidence和UpToDate等临床工具


<details>
  <summary>Details</summary>
Motivation: 专业临床AI助手被宣传为比通用大语言模型更安全可靠，但缺乏独立定量评估，存在关键证据缺口，而它们正日益影响诊断、分诊和指南解读等临床决策

Method: 使用包含MedQA（医学知识）和HealthBench（临床对齐）任务的1000项微型基准测试，评估两个广泛部署的临床AI系统（OpenEvidence和UpToDate Expert AI）与三个最先进的通用LLM（GPT-5、Gemini 3 Pro和Claude Sonnet 4.5）的性能对比

Result: 通用模型在各项指标上持续优于临床工具，GPT-5得分最高；OpenEvidence和UpToDate在完整性、沟通质量、情境意识和基于系统的安全推理方面存在明显缺陷

Conclusion: 市场推广的临床决策支持工具可能落后于前沿LLM，强调了在患者工作流程中部署前进行透明独立评估的紧迫性
...
### [224] [Sentiment Analysis and Emotion Classification using Machine Learning Techniques for Nagamese Language - A Low-resource Language](https://arxiv.org/abs/2512.01256)
*Ekha Morang,Surhoni A. Ngullie,Sashienla Longkumer,Teisovi Angami*

Main category: cs.CL

TL;DR: 首次对Nagamese语言进行情感分析和情绪分类研究，构建了包含1,195个词汇的情感极性词典，并使用朴素贝叶斯和支持向量机进行监督学习。


<details>
  <summary>Details</summary>
Motivation: Nagamese语言是印度东北部那加兰邦和阿萨姆邦之间贸易交流中发展起来的克里奥尔语。虽然英语、印地语等资源丰富的语言已有大量情感分析研究，但Nagamese语言尚未有相关工作。本研究旨在填补这一空白。

Method: 构建了包含1,195个Nagamese词汇的情感极性词典，结合其他特征，使用朴素贝叶斯和支持向量机等监督机器学习技术进行情感分析和情绪分类。

Result: 成功创建了Nagamese语言的首个情感极性词典，并建立了情感分析和情绪分类的机器学习模型框架，为该语言的NLP研究奠定了基础。

Conclusion: 这是Nagamese语言情感分析和情绪分类的首次尝试，为低资源语言的情感分析研究提供了重要参考，建立的词典和模型框架为该语言的进一步NLP研究奠定了基础。
...
### [225] [SUPERChem: A Multimodal Reasoning Benchmark in Chemistry](https://arxiv.org/abs/2512.01274)
...
TL;DR: SUPERChem是一个包含500个专家策划的化学推理问题的新基准，通过多模态和纯文本格式评估大语言模型的化学推理能力，采用推理路径保真度评分来评估推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有评估大语言模型化学推理能力的基准存在任务过于简化、缺乏过程级评估以及与专家级化学技能不匹配的问题，需要更全面、可靠的评估框架。

Method: 创建了500个专家策划的推理密集型化学问题，涵盖多个子领域，提供多模态和纯文本格式。采用迭代策划流程消除有缺陷的项目并减轻数据污染。每个问题都配有专家编写的解决方案路径，使用推理路径保真度评分来评估推理质量。

Result: 人类基线准确率为40.3%，表现最佳的GPT-5（High）模型仅达到38.5%，其次是Gemini 2.5 Pro（37.9%）和DeepSeek-V3.1-Think（37.3%）。基准能够激发多步骤、多模态推理，揭示视觉信息的模型依赖性影响，并区分高保真推理者和启发式推理者。

Conclusion: SUPERChem通过提供具有挑战性的基准和可靠的评估框架，旨在促进大语言模型向专家级化学智能的发展。数据集已在HuggingFace上公开。
...
### [226] [Agreement-Constrained Probabilistic Minimum Bayes Risk Decoding](https://arxiv.org/abs/2512.01316)
*Koki Natsumi,Hiroyuki Deguchi,Yusuke Sakai,Hidetaka Kamigaito,Taro Watanabe*

Main category: cs.CL

TL;DR: AC-PMBR解码通过引入知识蒸馏模型指导得分矩阵补全，在保持计算效率的同时提升机器翻译质量


<details>
  <summary>Details</summary>
Motivation: 传统MBR解码需要评估所有候选对之间的得分，计算复杂度为O(n²)，成本过高。PMBR解码通过采样和矩阵补全减少计算量，但会降低翻译质量。需要找到质量和计算成本之间的更好平衡。

Method: 提出协议约束的PMBR解码，利用知识蒸馏模型指导得分矩阵的补全过程，通过约束矩阵补全来提高近似精度。

Result: AC-PMBR将矩阵补全的近似误差降低了3倍，在WMT'23英德互译任务中，在可比计算成本下获得了比PMBR更高的翻译质量。

Conclusion: AC-PMBR解码通过知识蒸馏指导的矩阵补全，有效改善了质量与计算成本之间的权衡，为高效高质量的机器翻译解码提供了新方法。
...
### [227] [MARSAD: A Multi-Functional Tool for Real-Time Social Media Analysis](https://arxiv.org/abs/2512.01369)
*Md. Rafiul Biswas,Firoj Alam,Wajdi Zaghouani*

Main category: cs.CL

TL;DR: MARSAD是一个多功能的阿拉伯语NLP平台，专注于社交媒体实时监控与分析，支持情感分析、情绪分析、宣传检测、事实核查和仇恨言论检测等功能。


<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语世界的社交媒体监控和分析提供一个综合性的NLP平台，使研究人员和非技术用户都能方便地分析实时和存档的社交媒体内容。

Method: 平台采用后端架构集成灵活的文档存储和结构化数据管理，支持大规模多模态数据集处理，前端提供用户友好的数据上传和交互界面，并通过API密钥实现安全的社交媒体数据抓取。

Result: 开发了一个功能全面的NLP平台，能够生成详细的视觉化报告，支持多种分析维度，包括情感分析、情绪分析、宣传检测、事实核查和仇恨言论检测。

Conclusion: MARSAD平台为阿拉伯语社交媒体分析提供了一个强大而实用的工具，通过集成多种NLP功能和用户友好界面，满足了研究人员和非技术用户对社交媒体内容分析的需求。
...
### [228] [DyFuLM: An Advanced Multimodal Framework for Sentiment Analysis](https://arxiv.org/abs/2512.01410)
*Ruohan Zhou,Jiachen Yuan,Churui Yang,Wenzheng Huang,Guoyan Zhang,Shiyao Wei,Jiazhen Hu,Ning Xin,Md Maruf Hasan*

Main category: cs.CL

TL;DR: DyFuLM模型通过层次动态融合和门控特征聚合模块，有效提升复杂文本情感分析性能，在粗粒度和细粒度任务上均取得优异结果。


<details>
  <summary>Details</summary>
Motivation: 复杂文本表达中的情感理解是情感计算领域的基本挑战，需要捕捉层次语义表示和细粒度情感细微差别。

Method: 提出动态融合学习模型(DyFuLM)，包含层次动态融合模块（自适应整合多层次特征）和门控特征聚合模块（调节跨层信息流以实现平衡表示学习）。

Result: 在多任务情感数据集上，DyFuLM达到82.64%粗粒度和68.48%细粒度准确率，最低回归误差（MAE=0.0674，MSE=0.0082）和最高R²系数（0.6903）。消融实验验证各模块有效性。

Conclusion: DyFuLM通过有效的层次特征融合增强了情感表示和整体性能，每个模块都对特征交互和任务平衡有显著贡献。
...
### [229] [PromptBridge: Cross-Model Prompt Transfer for Large Language Models](https://arxiv.org/abs/2512.01420)
*Yaxuan Wang,Quan Liu,Zhenting Wang,Zichao Li,Wei Wei,Yang Liu,Yujia Bao*

Main category: cs.CL

TL;DR: 论文提出PromptBridge框架，解决LLM提示词在不同模型间迁移时的性能下降问题（模型漂移），通过少量校准任务学习跨模型提示映射，无需为每个任务或模型重新优化提示词。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，由于能力、成本、部署约束和隐私等因素，经常需要在不同LLM模型间切换。但为特定模型优化的提示词在其他模型上使用时性能会显著下降，这种现象被称为"模型漂移"，导致频繁的模型切换需要昂贵的提示词重新优化。

Method: 提出PromptBridge训练免费框架：1）使用MAP-RPE方法通过迭代反思精炼和定量评估获得任务和模型特定的最优提示词；2）利用源模型和目标模型的校准提示词对学习跨模型提示映射；3）测试时，给定源模型提示词，映射直接为目标模型生成优化提示词。

Result: 在单智能体和多智能体设置下的实验表明，PromptBridge能持续提升下游任务准确率，同时显著减少迁移工作量。该方法仅需少量对齐任务进行校准。

Conclusion: PromptBridge框架有效解决了LLM模型切换时的提示词迁移问题，通过跨模型提示映射实现了提示词效果在不同模型间的保持，减少了模型迁移的成本和复杂性。
...
### [230] [Multilingual Conversational AI for Financial Assistance: Bridging Language Barriers in Indian FinTech](https://arxiv.org/abs/2512.01439)
*Bharatdeep Hazarika,Arya Suneesh,Prasanna Devadiga,Pawan Kumar Rajpoot,Anshuman B Suresh,Ahmed Ifthaquar Hussain*

Main category: cs.CL

TL;DR: 印度多语言对话AI系统支持混合语言，提升金融科技平台的可访问性


<details>
  <summary>Details</summary>
Motivation: 印度语言多样性（31种主要语言，100多种次要语言）和英语普及率低（仅10%人口懂英语）给金融包容性带来挑战，需要解决语言障碍问题

Method: 采用多智能体架构，包含语言分类、功能管理和多语言响应生成模块，支持混合语言如Hinglish，通过比较多种语言模型和实际部署验证

Result: 系统显著提升了用户参与度，同时保持低延迟开销（4-8%），有效改善了数字金融服务在印度市场的语言可访问性

Conclusion: 该多语言对话AI系统有助于弥合新兴市场数字金融服务中的语言鸿沟，促进金融包容性
...
### [231] [MEGConformer: Conformer-Based MEG Decoder for Robust Speech and Phoneme Classification](https://arxiv.org/abs/2512.01443)
*Xabier de Zuazo,Ibon Saratxaga,Eva Navas*

Main category: cs.CL

TL;DR: 本文针对LibriBrain 2025 PNPL竞赛，提出了基于Conformer架构的解码器，用于处理原始306通道MEG信号，在语音检测和音素分类两个任务上取得了优于基准的结果。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是开发适用于脑磁图（MEG）信号的深度学习模型，以解决LibriBrain 2025竞赛中的两个基础任务：语音检测和音素分类，旨在超越竞赛基准并探索MEG特定的数据处理方法。

Method: 方法包括：1）使用紧凑型Conformer架构处理原始306通道MEG信号；2）采用轻量级卷积投影层和任务特定头部；3）语音检测任务中引入MEG导向的SpecAugment数据增强；4）音素分类任务中使用逆平方根类别加权和动态分组加载器处理100样本平均示例；5）采用简单的实例级归一化来缓解分布偏移。

Result: 在官方标准轨道划分下，使用F1-macro进行模型选择，最佳系统在排行榜上取得了语音检测88.9%和音素分类65.8%的分数，超越了竞赛基准，在两个任务中都进入了前十名。

Conclusion: 研究表明，基于Conformer的架构能够有效处理原始MEG信号，MEG特定的数据增强和归一化技术对性能提升至关重要，该方法在LibriBrain竞赛的两个基础任务上表现出色。
...
### [232] [Enhancing BERT Fine-Tuning for Sentiment Analysis in Lower-Resourced Languages](https://arxiv.org/abs/2512.01460)
*Jozef Kubík,Marek Šuppa,Martin Takáč*

Main category: cs.CL

TL;DR: 本文提出了一种结合主动学习、聚类和动态数据选择调度器的集成微调管道，用于低资源语言模型优化，在有限训练数据下实现性能提升和标注成本节省。


<details>
  <summary>Details</summary>
Motivation: 低资源语言通常数据有限，导致语言模型性能较弱。由于预训练计算成本高，更实用的方法是在微调阶段进行改进。本研究旨在通过主动学习和结构化数据选择策略来增强有限训练数据下的微调过程。

Method: 将主动学习与数据聚类相结合，提出一个集成微调管道，系统性地结合主动学习、聚类和动态数据选择调度器。使用主动学习调度器来优化数据选择过程。

Result: 在斯洛伐克语、马耳他语、冰岛语和土耳其语的实验中，微调阶段使用聚类结合主动学习调度器可以同时实现：1）标注成本节省高达30%；2）性能提升高达4个F1分数点；3）更好的微调稳定性。

Conclusion: 提出的集成微调管道通过结合主动学习、聚类和动态调度策略，在低资源语言场景下有效提升了模型性能，同时显著降低了标注成本，为有限数据条件下的语言模型优化提供了实用解决方案。
...
### [233] [MCAT: Scaling Many-to-Many Speech-to-Text Translation with MLLMs to 70 Languages](https://arxiv.org/abs/2512.01512)
*Yexing Du,Kaiyuan Liu,Youcheng Pan,Bo Yang,Keqi Deng,Xie Chen,Yang Xiang,Ming Liu,Bin Qin,YaoWei Wang*

Main category: cs.CL

TL;DR: MCAT框架通过语言扩展和语音序列优化，解决了MLLMs在语音翻译中的语言覆盖和效率问题，支持70种语言互译并提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在语音翻译任务中存在两个主要限制：1）数据集以英语为中心，限制了多语言翻译能力扩展；2）语音转换为长序列时推理速度急剧下降。

Method: 提出MCAT框架，包含两个创新：1）利用课程学习和数据平衡策略的语言扩展方法，支持70种语言互译；2）优化的语音适配器模块，将语音序列长度减少到仅30个token。

Result: 在不同规模MLLMs（9B和27B）上的实验表明，MCAT在FLEURS数据集的70x69个方向上超越了最先进的端到端模型，同时提升了批量推理效率，仅使用约1亿可训练参数和每种语言10小时S2TT数据。

Conclusion: MCAT框架有效解决了MLLMs在语音翻译中的语言覆盖和效率问题，支持大规模多语言互译，并已开源以促进MLLMs在语音翻译能力方面的发展。
...
### [234] [Language Diversity: Evaluating Language Usage and AI Performance on African Languages in Digital Spaces](https://arxiv.org/abs/2512.01557)
*Edward Ajayi,Eudoxie Umwari,Mawuli Deku,Prosper Singadi,Jules Udahemuka,Bekalu Tadele,Chukuemeka Edeh*

Main category: cs.CL

TL;DR: 研究评估了当前语言检测工具对约鲁巴语、基尼亚卢旺达语和阿姆哈拉语的表现，发现专业新闻内容比对话平台数据更适合训练非洲语言的AI模型


<details>
  <summary>Details</summary>
Motivation: 非洲语言在数字空间中的代表性不足，当前语言检测工具在处理这些语言时面临挑战，特别是在对话平台上数据稀疏且存在大量语码转换现象

Method: 从Reddit子论坛和本地新闻来源收集约鲁巴语、基尼亚卢旺达语和阿姆哈拉语数据，使用专门的AfroLID和通用LLM语言检测模型评估性能

Result: 语言检测模型在干净的新闻数据上表现近乎完美，但在存在语码转换的Reddit帖子上表现不佳；本地新闻媒体提供了更丰富、更纯净的单语数据

Conclusion: 专业策划的新闻内容比对话平台数据更适合训练非洲语言的上下文丰富AI模型；未来需要能够处理纯净和语码转换文本的模型来提高检测准确性
...
### [235] [MAC-SLU: Multi-Intent Automotive Cabin Spoken Language Understanding Benchmark](https://arxiv.org/abs/2512.01603)
...
TL;DR: 该研究提出了MAC-SLU数据集，这是一个多意图汽车座舱口语理解数据集，增加了SLU任务的难度。基于此数据集，对主流开源LLM和LALM进行了全面基准测试，发现虽然这些模型能通过上下文学习完成SLU任务，但性能仍显著落后于监督微调，而端到端LALM表现出与流水线方法相当的性能且能避免语音识别错误传播。


<details>
  <summary>Details</summary>
Motivation: 现有SLU数据集普遍缺乏足够的多样性和复杂性，且缺乏针对最新大型语言模型和大型音频语言模型的统一基准。汽车座舱场景中的多意图口语理解任务具有挑战性，需要更复杂的数据集来评估模型性能。

Method: 1. 创建MAC-SLU数据集，包含真实复杂的多意图数据；2. 对领先的开源LLM和LALM进行综合基准测试；3. 涵盖上下文学习、监督微调、端到端和流水线等多种方法；4. 比较不同范式在SLU任务上的表现。

Result: 实验表明：1. LLM和LALM通过上下文学习能完成SLU任务，但性能显著低于监督微调；2. 端到端LALM表现出与流水线方法相当的性能；3. 端到端方法能有效避免语音识别错误传播；4. 公开了代码和数据集供研究使用。

Conclusion: MAC-SLU数据集为SLU研究提供了更具挑战性的基准，端到端LALM在避免错误传播方面具有优势，但监督微调仍然是获得最佳性能的关键。该工作为汽车座舱场景的多意图口语理解提供了重要的数据集和基准分析。
...
### [236] [Learning the Boundary of Solvability: Aligning LLMs to Detect Unsolvable Problems](https://arxiv.org/abs/2512.01661)
*Dengyun Peng,Qiguang Chen,Bofei Liu,Jiannan Guan,Libo Qin,Zheng Yan,Jinhao Liu,Jianshu Zhang,Wanxiang Che*

Main category: cs.CL

TL;DR: 该研究提出了UnsolvableQA数据集和UnsolvableRL框架，旨在让大语言模型不仅能解决复杂问题，还能识别不可解问题，区分客观不可解性（问题内在矛盾）和主观能力限制，从而减少幻觉和过度自信。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型难以区分客观不可解性（问题本身存在内在矛盾）和主观能力限制（超出模型能力范围的问题），这导致模型产生幻觉和过度自信。为确保LLM的可靠性，需要让模型不仅能解决问题，还能识别何时问题不可解。

Method: 1. 构建UnsolvableQA数据集：采用双轨方法生成可解和不可解实例对，包括程序化生成逻辑谜题，以及新颖的"反向构造"方法在有效数学推理链中注入矛盾。2. 提出UnsolvableRL强化学习框架：包含三个奖励组件，共同考虑准确性、不可解性和难度。

Result: 实验结果显示，该方法在保持可解任务准确性的同时，实现了近乎完美的不可解性检测。关键发现是识别了"能力崩溃"现象，表明明确暴露于不可解数据对于防止模型系统性过度自信是必不可少的。

Conclusion: 通过UnsolvableQA数据集和UnsolvableRL框架，模型能够有效区分可解和不可解问题，显著提高不可解性检测能力，同时提升可解任务的准确性。明确训练模型识别不可解问题对于防止过度自信至关重要。
...
### [237] [Self-Supervised Borrowing Detection on Multilingual Wordlists](https://arxiv.org/abs/2512.01713)
*Tim Wientzek*

Main category: cs.CL

TL;DR: 提出了一种完全自监督的多语言词汇借词检测方法，结合全局对应模型的PMI相似度和基于语音特征向量的轻量级对比学习组件，无需标注数据自动选择决策阈值。


<details>
  <summary>Details</summary>
Motivation: 现有借词检测方法通常依赖监督学习或简单的字符串相似度度量，需要标注数据且性能有限。本文旨在开发一种完全自监督的方法，能够在没有人工监督的情况下有效检测多语言词汇中的借词关系。

Method: 方法结合两个信息源：1) 基于全局对应模型的PMI相似度；2) 在语音特征向量上训练的轻量级对比学习组件。还包括自动选择决策阈值的程序，无需标注数据。采用字符编码、温度设置和数据增强策略。

Result: 在基准数据集上的实验表明：PMI单独使用已优于NED和SCA等现有字符串相似度度量；组合相似度方法达到或优于监督基线性能。消融研究显示字符编码、温度设置和数据增强策略的重要性。

Conclusion: 该方法可扩展到不同规模的数据集，无需人工监督，并提供了命令行工具供研究人员使用。为多语言借词检测提供了一种有效且可扩展的自监督解决方案。
...
### [238] [Beware of Reasoning Overconfidence: Pitfalls in the Reasoning Process for Multi-solution Tasks](https://arxiv.org/abs/2512.01725)
*Jiannan Guan,Qiguang Chen,Libo Qin,Dengyun Peng,Jinhao Liu,Liangyu Huo,Jian Xie,Wanxiang Che*

Main category: cs.CL

TL;DR: LLMs在需要单一正确答案的推理任务中表现出色，但在需要生成全面多样答案的多解决方案任务中表现不佳，这归因于"推理过度自信"问题。
...
Method: 引入MuSoBench多解决方案问题基准，比较短链思维(Short-CoT)和长链思维(Long-CoT)提示方法，提出认知刚性假说，并通过注意力熵分析验证。

Result: 实验显示传统Short-CoT提示表现出明显的过度自信，而新兴的Long-CoT方法通过迭代探索和自我反思缓解了这一问题。注意力熵分析初步支持认知刚性假说。

Conclusion: 研究结果为评估LLM推理的完整性提供了工具，强调需要超越单一答案准确性的评估，转向更全面的探索性评估框架。
...
### [239] [Reasoning About the Unsaid: Misinformation Detection with Omission-Aware Graph Inference](https://arxiv.org/abs/2512.01728)
*Zhengjia Wang,Danding Wang,Qiang Sheng,Jiaying Wu,Juan Cao*

Main category: cs.CL

TL;DR: OmiGraph是首个针对基于省略的虚假信息检测框架，通过构建省略感知图来识别新闻中被省略的重要信息，相比传统方法在F1和准确率上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 虚假信息不仅通过明确捏造误导性内容，还通过隐式省略重要信息来欺骗读者。虽然前者已被广泛研究，但基于省略的欺骗方式在很大程度上被忽视，尽管它能在信息看似完整的假象下微妙地引导读者得出错误结论。

Method: OmiGraph框架：1）为目标新闻构建省略感知图，利用捕捉同一事件互补视角的上下文环境来揭示可能被省略的内容；2）提出省略导向的关系建模，识别内部上下文依赖关系和动态省略意图；3）引入省略感知的消息传递和聚合机制，整合省略内容和关系来建立全面的欺骗感知。

Result: 实验表明，通过考虑省略视角，该方法在两个大规模基准测试上取得了显著性能提升，平均F1分数提高+5.4%，准确率提高+5.3%。

Conclusion: OmiGraph是首个针对省略型虚假信息的检测框架，通过系统性地建模省略内容和关系，显著提升了虚假信息检测的性能，填补了该研究领域的空白。
...
### [240] [InnoGym: Benchmarking the Innovation Potential of AI Agents](https://arxiv.org/abs/2512.01822)
...
TL;DR: InnoGym是首个系统评估AI智能体创新潜力的基准和框架，包含性能增益和新颖性两个互补指标，涵盖18个真实世界工程和科学任务，揭示了创造力与有效性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要衡量正确性，忽略了解决方案背后方法的多样性。真正的创新不仅取决于产生正确答案，还取决于方法的原创性。需要能够评估AI智能体创新潜力的基准。

Method: 提出InnoGym基准和框架，包含两个互补指标：性能增益（衡量相对于已知最佳解决方案的改进）和新颖性（捕捉与先前方法的方法论差异）。基准包含18个精心策划的真实世界工程和科学任务，通过资源过滤、评估器验证和解决方案收集进行标准化。同时提供iGym统一执行环境，用于可重复和长期评估。

Result: 广泛实验表明，虽然一些智能体能够产生新颖方法，但它们缺乏鲁棒性，限制了性能增益。这些结果突显了创造力与有效性之间的关键差距。

Conclusion: 需要能够同时评估创造力和有效性的基准，InnoGym填补了这一空白，为系统评估AI智能体的创新潜力提供了首个框架。
...
### [241] [Beyond SFT: Reinforcement Learning for Safer Large Reasoning Models with Better Reasoning Ability](https://arxiv.org/abs/2512.01848)
*Jinghan Jia,Nathalie Baracaldo,Sijia Liu*

Main category: cs.CL

TL;DR: 该论文研究大型推理模型的安全对齐问题，发现传统监督微调方法存在局限性，提出使用强化学习作为补充优化框架，能更有效地抑制不安全推理过程同时保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思维链推理显著提升了数学和逻辑问题解决能力，但这也引入了新的安全风险——不安全行为往往出现在中间推理轨迹中，即使最终答案看起来无害。现有的基于监督微调的安全对齐方法存在安全改进不一致、推理能力下降和跨模型泛化能力差等问题。

Method: 研究采用强化学习作为安全训练的补充优化框架。与监督微调不同，强化学习通过奖励反馈直接优化模型策略，实现更自适应和稳定的对齐。通过在多模型系列和基准测试上的广泛实验验证方法有效性。

Result: 强化学习实现了更强且更一致的安全增益，同时保持了推理能力。对反思动态和令牌级熵的分析表明，强化学习抑制了不安全的探索性推理，同时保留了反思深度，从而产生更安全可靠的推理过程。

Conclusion: 纯监督方法不足以实现大型推理模型的鲁棒安全对齐，强化学习作为补充优化框架能够更有效地解决这一问题，在提升安全性的同时保持模型的推理能力。
...
### [242] [BHRAM-IL: A Benchmark for Hallucination Recognition and Assessment in Multiple Indian Languages](https://arxiv.org/abs/2512.01852)
*Hrishikesh Terdalkar,Kirtan Bhojani,Aryan Dongare,Omm Aditya Behera*

Main category: cs.CL

TL;DR: BHRAM-IL是一个针对多种印度语言（印地语、古吉拉特语、马拉地语、奥里亚语和英语）的幻觉识别与评估基准，包含36,047个问题，评估了14个多语言大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多语言应用中常产生看似合理但错误或误导性的输出（幻觉），而现有研究主要集中在英语，资源匮乏的印度语言领域尚未得到充分探索。

Method: 创建了包含36,047个问题的基准数据集，涵盖事实、数值、推理和语言任务等9个类别。在10,265个问题的子集上评估了14个最先进的多语言LLM，使用类别特定指标（归一化到0-1范围）分析跨语言和事实幻觉。

Result: 所有类别和模型的聚合得分为0.23，语言校正模糊得分为0.385，表明BHRAM-IL在幻觉评估方面的有效性。数据集和代码已在GitHub和HuggingFace上开源。
...
### [243] [Cross-Lingual Interleaving for Speech Language Models](https://arxiv.org/abs/2512.01865)
*Adel Moumen,Guangzhi Sun,Philip C. Woodland*

Main category: cs.CL

TL;DR: 该论文提出了一种跨语言交织方法，通过混合不同语言的语音标记来构建多语言语音语言模型，无需文本监督，并发布了EN-FR训练数据集和评估基准。


<details>
  <summary>Details</summary>
Motivation: 语音语言模型（SLMs）旨在直接从语音中学习语言能力，为书写资源有限的语言提供NLP技术支持。然而，由于缺乏语音评估基准和训练数据，现有研究主要集中于英语，导致跨语言学习困难。

Method: 提出跨语言交织方法，在不依赖文本监督的情况下混合不同语言的语音标记。同时发布了EN-FR训练数据集TinyStories（约42k小时）以及通过GPT-4生成的EN-FR语音StoryCloze和TopicCloze评估基准。

Result: 在360M和1B参数的SLMs上，在相同训练标记预算下，交织方法提高了单语言语义准确性，实现了稳健的跨语言延续能力，并增强了跨语言隐藏状态对齐。结果表明该方法能构建理解和跨语言对话的多语言SLMs。

Conclusion: 跨语言交织是一种简单、可扩展的构建多语言语音语言模型的途径，所有资源将开源以支持可重复性研究。
...
### [244] [Exploring Human Perceptions of AI Responses: Insights from a Mixed-Methods Study on Risk Mitigation in Generative Models](https://arxiv.org/abs/2512.01892)
...
TL;DR: 研究评估人类对AI生成有害内容缓解策略的感知，发现语言背景、AI经验等因素显著影响评估结果，参与者对语言细节敏感，与量化评估形成对比


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速普及，研究人类对生成响应的感知变得至关重要。主要挑战在于AI容易产生幻觉和生成有害内容。尽管已有大量实施防护措施的努力，但人类对这些缓解策略的感知在很大程度上仍是未知的

Method: 采用混合方法实验，在多维度上评估缓解策略的响应：忠实性、公平性、有害内容移除能力和相关性。采用被试内研究设计，57名参与者在两种条件下评估响应：有害响应加其缓解措施 vs 仅缓解后的响应

Result: 结果显示参与者的母语、AI工作经验和标注熟悉度显著影响评估。参与者对语言和上下文属性高度敏感，惩罚轻微语法错误同时奖励保留的语义上下文。这与语言在LLM量化评估中的处理方式形成对比

Conclusion: 研究为训练和评估缓解策略引入了新指标，并为人类-AI评估研究提供了见解，强调人类感知评估的重要性及其与自动化评估的差异
...
### [245] [Latent Debate: A Surrogate Framework for Interpreting LLM Thinking](https://arxiv.org/abs/2512.01909)
*Lihu Chen,Xiang Yin,Francesca Toni*

Main category: cs.CL

TL;DR: 提出了一种名为"潜在辩论"的新框架，通过捕捉单个模型在单次推理过程中产生的隐藏支持和反对信号，来解释大语言模型的内部思考过程和幻觉成因。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型的内部思考过程和幻觉成因是一个关键挑战。当前的自洽性和多智能体辩论方法依赖于多个答案或多个模型之间的显式辩论，而本文旨在探索单个模型在单次推理中的隐式内部论证。

Method: 提出了一个模型和任务无关的概念框架"潜在辩论"，并通过符号化实例化来近似大语言模型在真/假预测任务中的思考过程。该方法捕捉模型内部的隐藏支持和攻击信号。

Result: 实证研究表明，潜在辩论是一个忠实的有结构替代模型，其预测与原始大语言模型高度一致。该框架不仅提供可解释性，还为幻觉检测提供了强大的基线。分析显示幻觉与辩论模式之间存在强相关性，例如中间层的高度潜在辩论与更高的幻觉风险相关。

Conclusion: 潜在辩论作为一个潜在框架，有助于理解大语言模型的内部机制，特别是在推理步骤中出现内部（不）一致性的场景中。该框架为模型可解释性和幻觉检测提供了新的视角和方法。
...
### [246] [The Art of Scaling Test-Time Compute for Large Language Models](https://arxiv.org/abs/2512.02008)
*Aradhye Agarwal,Ayan Sengupta,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 本文首次对测试时扩展（TTS）策略进行了大规模系统研究，发现没有单一策略能普遍主导，推理模型在不同问题难度和轨迹长度上表现出不同模式，且最优TTS性能随计算预算单调扩展。
...
Method: 使用八个开源大语言模型（7B到235B参数），在四个推理数据集上生成超过300亿token，首次对TTS进行大规模研究，比较不同策略在相同条件下的表现。

Result: 观察到三个一致趋势：1）没有单一TTS策略能普遍主导；2）推理模型在不同问题难度和轨迹长度上表现出不同的轨迹质量模式，形成短视界和长视界两类；3）对于给定模型类型，最优TTS性能随计算预算单调扩展。

Conclusion: 基于研究结果，提出了考虑问题难度、模型类型和计算预算的最佳TTS策略选择方案，为有效的推理时扩展提供了实用指南。
...
### [247] [Four Over Six: More Accurate NVFP4 Quantization with Adaptive Block Scaling](https://arxiv.org/abs/2512.02010)
*Jack Cook,Junxian Guo,Guangxuan Xiao,Yujun Lin,Song Han*

Main category: cs.CL

TL;DR: 本文提出4/6方法改进NVFP4量化算法，通过评估两个缩放因子来减少量化误差，防止训练发散并提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模增长，低精度数值格式如NVFP4因速度和内存优势而流行，但需要将所有矩阵乘法操作数（权重、激活值、梯度）量化为NVFP4，这常导致训练发散和推理性能下降。

Method: 提出Four Over Six (4/6)方法，改进NVFP4量化算法，为每个数值块评估两个潜在的缩放因子。不同于整数格式，FP4等浮点格式的最大量化误差出现在每个块的接近最大值处，通过缩放到更小的FP4值使可表示值的分布更均匀，改善接近最大值的表示。

Result: 4/6可在NVIDIA Blackwell GPU上高效实现，适合在NVFP4训练LLM时使用。在transformer和混合模型架构的预训练实验中，4/6在多个案例中防止了训练发散，使训练损失显著接近BF16基准。4/6还能轻松集成到多种后训练量化方法中，普遍提升下游任务准确率。

Conclusion: 4/6方法有效解决了NVFP4量化导致的训练发散和性能下降问题，为未来使用NVFP4训练和部署模型提供了有前景的解决方案。
...
### [248] [Chunking Strategies for Multimodal AI Systems](https://arxiv.org/abs/2512.00185)
*Shashanka B R,Mohith Charan R,Seema Banu F*

Main category: cs.AI

TL;DR: 这篇综述系统梳理了多模态分块策略的技术全景，为开发更高效的多模态AI系统提供了技术基础和设计空间，涵盖了文本、图像、音频、视频和跨模态数据的各种分块方法。


<details>
  <summary>Details</summary>
Motivation: 整合多模态分块策略的研究现状，为研究人员和从业者提供技术基础，推动开发更有效、高效的多模态AI系统，解决实际应用中模态复杂性、处理准确性和生成连贯性等挑战。

Method: 采用综合性分类和技术分析方法，针对每种模态（文本、图像、音频、视频、跨模态数据）分别研究经典和现代分块策略，包括固定大小令牌窗口、递归文本分割、对象中心视觉分块、基于静音的音频分割、视频场景检测等，分析每种方法的方法论、支持工具、优势和挑战。

Result: 建立了全面的多模态分块策略分类体系，分析了各种方法的粒度-上下文权衡和多模态对齐等关键挑战，探索了新兴的跨模态分块策略，提供了比较性见解，并指出了异步信息密度和噪声对齐信号等开放性问题。

Conclusion: 该研究为多模态AI系统的分块策略提供了系统性的技术基础，指出了未来在自适应、基于学习和任务特定分块方面的研究方向，有助于开发能够处理复杂模态、提高处理准确性和生成连贯性的鲁棒分块管道。
...
### [249] [A Rosetta Stone for AI Benchmarks](https://arxiv.org/abs/2512.00193)
*Anson Ho,Jean-Stanislas Denain,David Atanasov,Samuel Albanie,Rohin Shah*

Main category: cs.AI

TL;DR: 该研究构建了一个统计框架，将不同AI基准测试连接起来，在单一数值尺度上衡量模型能力和基准难度，从而能够跨时间和不同基准比较AI能力，并分析长期趋势。


<details>
  <summary>Details</summary>
Motivation: 大多数AI基准测试在推出后几年甚至几个月内就会饱和，这使得研究AI能力的长期趋势变得困难。需要一种方法来比较不同时期、不同基准测试下的模型性能。

Method: 构建统计框架，将不同基准测试"缝合"在一起，在单一数值尺度上放置模型能力和基准难度，形成类似"罗塞塔石碑"的系统，无需假设能力如何随时间或训练计算量演变。

Result: 框架成功应用于三个场景：1) 测量AI进展速度并预测未来能力；2) 估算算法效率改进率，结果与先前工作基本一致但略高；3) 能够检测AI进展中的快速加速。

Conclusion: 该统计框架为解决AI基准测试快速饱和问题提供了有效方案，能够跨时间、跨基准比较AI能力，为研究长期AI发展趋势提供了新工具。
...
### [250] [Reasoning Under Pressure: How do Training Incentives Influence Chain-of-Thought Monitorability?](https://arxiv.org/abs/2512.00218)
...
<details>
  <summary>Details</summary>
Motivation: AI系统输出自然语言推理过程为安全监控提供了机会，但推理过程是否真实反映底层思维过程以及可监控程度可能受训练方式影响。研究旨在探索不同训练激励如何影响推理模型的可监控性。

Method: 提出新的方法论来衡量可监控性：通过监控器能否使用模型的推理过程预测关键潜在变量来评估。研究了多种训练激励的影响，包括长度惩罚、KL正则化、对抗性优化（惩罚监控准确性）和直接优化可监控性。

Result: 在控制准确性的前提下，常见激励（长度惩罚和KL正则化）未发现对可监控性的一致影响。对抗性优化会降低监控性能，而直接优化可监控性并不能可靠地带来改进。

Conclusion: 训练激励确实会影响推理模型的可监控性，特别是对抗性优化会损害监控能力。这为设计更安全、更可监控的AI系统提供了重要见解。
...
### [251] [Trification: A Comprehensive Tree-based Strategy Planner and Structural Verification for Fact-Checking](https://arxiv.org/abs/2512.00267)
*Anab Maulana Barik,Shou Ziyi,Yang Kaiwen,Yang Qi,Shen Xin*

Main category: cs.AI

TL;DR: Trification：一种基于依赖图建模和动态调整的自动化事实核查框架，通过生成全面的验证动作集并结构化组织，显著提升事实核查准确性。


<details>
  <summary>Details</summary>
Motivation: 技术发展使虚假信息快速传播，需要自动化事实核查系统来维护在线媒体生态的安全和完整性。现有方法虽然将声明分解为子任务并使用LLM多智能体系统，但存在两个局限：无法验证声明的每个组成部分，以及缺乏结构化框架来逻辑连接子任务结果进行最终预测。

Method: 提出Trification框架：1）生成全面的验证动作集以确保对声明的完整覆盖；2）将这些动作组织成依赖图以建模动作间的逻辑交互；3）支持动态修改图结构，使系统能够适应性地调整验证策略。

Result: 在两个具有挑战性的基准测试上的实验结果表明，该框架显著提高了事实核查的准确性，从而推进了自动化事实核查系统的最新技术水平。

Conclusion: Trification框架通过结构化验证动作和动态调整策略，解决了现有自动化事实核查系统的局限性，为在线虚假信息检测提供了更有效的解决方案。
...
TL;DR: 论文提出CogEvo-Edu系统，将检索、记忆和控制视为耦合的认知演化过程，通过分层多智能体架构解决STEM教育中LLM作为对话导师的局限性，在数字信号处理领域显著提升教学效果。


<details>
  <summary>Details</summary>
Motivation: 当前STEM教育中部署的LLM对话导师大多采用单一模型和静态RAG管道，难以处理复杂领域（如数字信号处理）的挑战，包括维护连贯的长期学生模型、管理异构知识库以及在长期互动中适应教学策略。

Method: 提出CogEvo-Edu分层教育多智能体系统，包含三个层次：认知感知层（CPL）维护双重记忆并执行置信度加权整合；知识演化层（KEL）为每个知识块分配时空价值驱动激活、语义压缩和遗忘；元控制层（MCL）将教学制定为分层序列决策，协调专业智能体并通过双重内外循环联合调整CPL/KEL超参数。

Result: 构建DSP-EduBench垂直基准测试，使用三模型LLM-as-a-Judge集成评估。CogEvo-Edu将总体得分从5.32提升至9.23，在六个指标上均优于静态RAG、简单记忆和单智能体变体。

Conclusion: 将学生档案、知识库和教学策略联合演化的方法具有重要价值，CogEvo-Edu系统通过耦合认知演化过程显著提升了复杂领域STEM教育中LLM对话导师的效果。
...
TL;DR: 本文提出了首个基于强化学习的个性化对话框架，通过实时推断用户性格来优化模型行为，在主观情感对话领域取得突破性进展。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域过度关注数学、代码等确定性推理任务，而忽视了真正定义人类智能的主观、情感化、个性化的对话能力。传统认为这类主观领域难以形式化，不适合常规强化学习流程。

Method: 提出首个实时推断用户性格并优化模型行为的强化学习框架，引入动态情商评估套件来量化改进效果。模型名为Echo-N1。

Result: 该方法在非可验证环境中表现稳定、鲁棒，对话质量显著提升。Echo-N1模型远超其基础版本，并优于专有的Doubao 1.5 Character模型。

Conclusion: 这项工作为强化学习开辟了新前沿：优化模型在深度主观、深度人性化的对话维度上的表现。
...
TL;DR: 该论文提出首个多模态欺骗基准MM-DeceptionBench，并设计基于图像辩论的多智能体监控框架来检测多模态大语言模型的欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 前沿AI系统能力提升的同时带来了更隐蔽的安全风险——欺骗行为。与幻觉不同，欺骗是模型通过复杂推理故意误导用户。当前研究主要局限于文本领域，多模态大语言模型的欺骗风险尚未被探索，且现有监控方法对多模态欺骗几乎无效。

Method: 1. 构建MM-DeceptionBench基准，覆盖六类欺骗行为，评估模型如何通过视觉和文本模态组合进行策略性操纵和误导。2. 提出"基于图像的辩论"多智能体辩论监控框架，强制模型基于视觉证据支撑其主张，从而提高欺骗策略的可检测性。

Result: 实验表明，提出的方法在所有测试模型中都能提高与人类判断的一致性，在GPT-4o上Cohen's kappa系数提升1.5倍，准确率提升1.25倍。

Conclusion: 多模态欺骗是一个严重但被忽视的安全风险，需要专门的评估基准和监控方法。提出的MM-DeceptionBench和基于图像的辩论框架为检测和缓解多模态欺骗提供了有效工具。
...
TL;DR: GreenPlanner是一个能源和功能感知的生成式框架，统一了设计评估和生成，通过自动化评估和生成符合规范的平面布局，大幅提升设计效率。
...
TL;DR: 研究显示医疗数据缺失模式显著影响大语言模型的零样本预测性能，缺失指示符的显式包含对不同模型影响不一致，大模型受益而小模型可能受损


<details>
  <summary>Details</summary>
Motivation: 尽管已有大量关于缺失信息价值的研究，但数据缺失模式对大语言模型性能的影响尚未被充分研究。在医疗领域，数据收集往往反映人类决策（如诊断测试转诊），这些缺失模式可能对LLM预测性能产生重要影响

Method: 通过在哥伦比亚大学医学中心（大型城市学术医疗中心）和MIMIC-IV数据集上进行一系列实验，研究缺失模式对零样本预测性能的影响。特别测试了在提示中显式包含缺失指示符对LLM性能的影响

Result: 缺失模式显著影响LLM的零样本预测性能。显式包含缺失指示符对某些LLM有益，但对其他LLM的预测性能和校准有负面影响，表明影响不一致。聚合分析和理论见解表明，较大模型从这些干预中受益，而较小模型可能受到负面影响

Conclusion: LLM范式可能进一步模糊缺失性的影响（即使在传统机器学习中也常被忽视）。需要更透明的记录和系统评估表示（信息性）缺失对下游性能的影响
...
TL;DR: CRPO是一种用于临床推理的多目标可验证强化学习方法，通过整合基于规则的奖励信号来优化准确性、忠实性和全面性，无需人工标注，训练出的Clinical-R1-3B模型在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前的后训练方法（如GRPO）主要奖励正确性，这与医学等高风险领域所需的多维目标（如忠实性和全面性）不一致。需要一种能够对齐临床推理原则的后训练方法。

Method: 提出了临床目标相对策略优化（CRPO），这是一种可扩展、多目标、可验证的强化学习方法。它整合了基于规则和可验证的奖励信号，共同优化准确性、忠实性和全面性，无需依赖人工标注。

Result: 训练了Clinical-R1-3B（30亿参数）临床推理模型。在三个基准测试中，CRPO在真实性和完整性方面的推理能力显著优于标准GRPO，同时保持了良好的准确性提升。

Conclusion: CRPO框架为将LLM推理与临床目标对齐提供了可扩展的途径，能够实现更安全、更协作的医疗AI系统，同时突显了多目标可验证RL方法在医学领域LLM后训练扩展中的潜力。
...
TL;DR: EDIT是一种针对扩散式大语言模型的早期推理终止方法，通过监控token激活与训练时推理路径的对齐程度，自适应地停止去噪过程，显著减少推理步骤同时保持或提升准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型通过迭代去噪生成token，但答案通常在完成所有步骤前就已稳定。现有方法在模型发布时丢弃了训练过程中产生的丰富元数据，这些信息包含了参数重要性和推理路径。

Method: EDIT在监督微调期间捕获AdamW聚合的LoRA更新，构建推理映射图。推理时监控token激活与推理映射图的对齐程度，将对齐分数转换为已解掩码token的分布，当连续步骤间的KL散度低于阈值时检测收敛。

Result: 在推理基准测试中，EDIT将扩散步骤减少11.8%至68.3%，在大多数设置中保持或提高了准确性，存储开销仅约0.02%（对于8GB模型的所有QKV模块约1.5-2MB）。

Conclusion: 通过利用训练梯度动态，EDIT为减少扩散式大语言模型的推理时间和成本开辟了新的研究方向，展示了训练元数据在优化推理效率中的价值。
...
TL;DR: 提出一种神经理论框架，解释智能系统如何涌现，兼具功能鲁棒性和生物合理性，为LLM提供可解释性、统一理论和计算效率


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型存在三大限制：缺乏可解释性、缺少统一理论框架、运算成本过高。需要一种既功能鲁棒又生物合理的智能涌现理论

Method: 提出神经理论框架，从神经科学和理论角度解释智能系统的涌现机制，该模型为认知过程（如决策和问题解决）提供理论洞见

Result: 该框架提供了创建可解释、可泛化人工智能的计算高效方法，为智能系统的理解提供了统一的理论基础

Conclusion: 提出的神经理论框架解决了LLM发展的关键限制，为构建可解释、理论统一且计算高效的人工智能系统提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 研究发现检索算法的目标与语言模型的知识蒸馏目标相似，都旨在与原始LLM对齐。从信息论角度分析蒸馏语言模型与原始LLM在信息关注上的相似性，提出利用DLM作为检索算法的新思路。

Method: 1. 算法层面：基于DLM的注意力头权重设计轻量级检索头，通过剪枝冗余参数减少90%以上参数；2. 系统层面：通过弹性加载策略设计异步预取数据流，有效重叠KV缓存检索与LLM计算；3. 编译层面：构建理论内存模型并实现自适应内存管理系统，最大化GPU内存利用率。

Result: 在云端和边缘两种资源受限环境中部署评估，相比Huggingface框架，SpeContext在云端实现最高24.89倍吞吐量提升，在边缘实现10.06倍加速，且准确率损失可忽略，推动了准确率与吞吐量的帕累托前沿。
...
TL;DR: 该论文提出了一个基于人类认知过程的分类法来理解大型推理模型的心理机制，创建了包含27.7万个原子推理步骤的数据集，并开发了自动标注框架CAPO，揭示了当前模型自我监控的局限性，为改进推理模型训练提供了新方向。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中展现出类人推理能力，但缺乏系统性的分析框架来理解其内部推理机制。研究者希望从人类认知心理学角度出发，建立跨学科的分类体系来深入探究这些模型的"心理"特征。

Method: 1. 基于人类心理过程设计了包含5组17个类别的分类法；2. 应用该分类法创建了包含277,534个原子推理步骤的标注数据集；3. 开发了自动标注框架CAPO，利用大语言模型生成分类法标注；4. 分析了当前主流推理模型，特别是自我监控机制的有效性。

Result: 1. CAPO框架在标注一致性上优于基线方法，与人类专家标注更一致；2. 发现当前模型的"双重检查"（自我监控评估）大多是表面性的，很少产生实质性修订；3. 揭示了激励多步反思比简单自我监控更有效；4. 为推理模型的训练和后训练提供了可操作的改进建议。

Conclusion: 该研究提出的分类法、CAPO框架和实证分析为理解和改进大型推理模型的推理能力提供了系统化、可扩展的路径，强调了从人类认知角度深入分析模型内部机制的重要性，并指出了未来发展的方向。
...
TL;DR: BioPro提出了一种无需训练的选择性去偏框架，通过识别性别变化子空间并应用投影，在保持显性场景中性别忠实度的同时减少中性场景中的性别偏见。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在显著的性别偏见，现有公平性干预方法采用差异无感知视角，对所有人口群体实施统一处理。这种方法无法区分需要中立性的场景与需要保留群体特定属性的场景，因此需要选择性去偏方法。

Method: 提出BioPro（Bias Orthogonal Projection）框架，通过反事实嵌入识别低维性别变化子空间，应用投影技术选择性地中和性别相关信息。该框架完全无需训练，能够实现选择性去偏。

Result: 实验表明，BioPro能有效减少中性场景中的性别偏见，同时在显性场景中保持性别忠实度。此外，BioPro还能推广到连续偏差变量（如场景亮度），显示出更广泛的适用性。

Conclusion: BioPro为视觉语言模型中的选择性公平性提供了有前景的方向，通过差异感知方法在去偏和保持合理区分之间取得平衡，为多模态领域的公平性研究开辟了新路径。
...
TL;DR: Med-CMR是一个细粒度的医学复杂多模态推理基准，通过系统能力分解、挑战性任务设计和高质量数据覆盖，评估MLLMs在临床推理中的表现，发现GPT-5表现最佳但专业医学MLLMs不一定优于通用模型。


<details>
  <summary>Details</summary>
Motivation: MLLMs开始应用于临床工作流程，但其执行复杂医学推理的能力尚不明确。现有基准缺乏对医学多模态推理的细粒度评估，需要更系统的方法来测试模型在临床环境中的实际表现。

Method: 1）系统能力分解：将医学多模态推理分解为细粒度视觉理解和多步推理；2）挑战性任务设计：视觉理解包括小物体检测、细节辨别和空间理解，推理涵盖四种临床相关场景；3）高质量数据：包含20,653个VQA对，覆盖11个器官系统和12种成像模态，经过两阶段专家审核。

Result: 评估18个最先进的MLLMs，GPT-5表现最佳（多选题准确率57.81%，开放式问题得分48.70），优于Gemini 2.5 Pro和Qwen3-VL-235B-A22B。但专业医学MLLMs不一定优于强通用模型，长尾泛化是主要失败模式。

Conclusion: Med-CMR为医学MLLMs的视觉-推理整合和罕见病例鲁棒性提供了压力测试，并为未来临床系统建立了严格的评估标准。研究揭示了当前模型在复杂医学推理中的局限性，特别是长尾泛化能力不足。
...
TL;DR: 本文提出三种评估反事实情景建模中模型误差的方法，强调模型校准对决策的重要性，并通过模拟实验对比各方法的优缺点。


<details>
  <summary>Details</summary>
Motivation: 尽管反事实情景建模在规划和决策中普遍使用，但情景预测很少被回顾性评估。预测与观测的差异来自情景偏差和模型校准错误，作者认为后者对评估模型在决策中的价值最为重要，但需要在反事实世界中估计模型误差。

Method: 提出并对比三种估计反事实误差的方法：1）基于历史数据的误差估计；2）使用替代情景的误差转移；3）基于模型结构的误差分解。通过模拟实验展示各方法的优缺点。

Result: 模拟实验表明不同方法在估计反事实误差方面各有优劣，某些方法在特定条件下更可靠。研究提供了反事实误差估计的建议，并讨论了使情景预测可评估所需的情景设计要素。

Conclusion: 准确估计反事实误差对评估模型在决策中的价值至关重要。研究提出的三种方法各有适用场景，情景设计需要包含足够的评估要素以确保预测的可评估性。这些方法有助于提高反事实情景建模的可靠性和决策价值。
...
TL;DR: ARCADIA是一个用于因果发现的智能AI框架，它结合了大语言模型推理与统计诊断来构建有效、时间一致的因果结构，相比传统算法在真实高风险领域产生更可靠、可解释的因果图。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现算法在处理真实世界高风险领域时存在局限性，需要更可靠、可解释且能自主进行科学建模的因果推理框架。ARCADIA旨在通过智能AI方法改进因果发现过程，使其更稳定、可解释且适合干预应用。

Method: ARCADIA框架整合了大语言模型推理与统计诊断，通过约束引导的提示和因果有效性反馈迭代优化候选有向无环图（DAGs），构建时间一致的因果结构，形成完全可解释、适合干预的流程。

Result: 在企业破产数据上的实验表明，ARCADIA比NOTEARS、GOLEM和DirectLiNGAM等传统算法产生更可靠的因果图，同时提供完全可解释、适合干预的管道。

Conclusion: ARCADIA框架通过展示智能大语言模型如何参与自主科学建模和结构化因果推断，推进了AI发展，为高风险领域的因果发现提供了更可靠、可解释的解决方案。
...
TL;DR: 该论文提出了语义场子空间(SFS)和SAFARI算法，用于在嵌入空间中捕获局部语义邻域并揭示层次化语义结构，通过高效的语义偏移近似实现15-30倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前嵌入空间内部结构不透明，现有方法要么牺牲语义连贯性追求结构规律性，要么计算开销大。需要一种既能保持几何结构又能捕捉语义关系的高效方法。

Method: 提出语义场子空间(SFS)作为几何保持、上下文感知的表示方法；开发SAFARI无监督算法，使用语义偏移度量来量化语义演化；设计高效的语义偏移近似方法替代昂贵的SVD计算。

Result: 在6个真实世界文本和图像数据集上的评估显示，SFS在分类和政治偏见检测等任务上优于标准分类器；SAFARI能一致地揭示可解释且可泛化的语义层次结构；高效近似实现15-30倍加速，平均误差低于0.01。

Conclusion: 该工作为嵌入空间中的语义理解提供了一个统一框架，能够结构化、分析和扩展语义理解，在保持语义连贯性的同时实现高效计算。
...
TL;DR: MMQAKE是首个多模态多跳问答知识编辑基准，评估模型在多模态推理链中的表现和对视觉重述输入的鲁棒性。提出的Hybrid-DMKG框架通过动态多模态知识图谱显著提升了多模态知识编辑的性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态知识编辑基准主要评估最终答案正确性，但忽视了中间推理质量和视觉重述输入的鲁棒性。需要更全面的评估框架来推动多模态知识编辑技术的发展。

Method: 提出Hybrid-DMKG混合推理框架：1) 使用大语言模型分解多模态多跳问题为子问题序列；2) 多模态检索模型通过联合编码定位更新事实；3) 混合推理模块通过关系链接预测和RAG推理两条路径在动态多模态知识图谱上操作；4) 决策模块聚合证据选择最可信答案。

Result: 实验结果表明，当前MKE方法在多模态推理链更新和推理方面存在困难。Hybrid-DMKG在MMQAKE基准上显著优于现有方法，实现了更高的准确性和对知识更新的更好鲁棒性。

Conclusion: MMQAKE基准填补了多模态知识编辑评估的空白，Hybrid-DMKG框架通过动态多模态知识图谱和混合推理机制有效解决了多模态多跳推理中的知识更新挑战，为多模态知识编辑提供了新的解决方案。
...
TL;DR: 该论文提出基于因果机器学习的方法，从预测性维护转向规范性维护，通过因果基础模型模拟潜在修复措施对系统KPI的影响，为生产线提供数据驱动的行动推荐。


<details>
  <summary>Details</summary>
Motivation: 制造业向规范性维护转型受限于对预测模型的依赖，这些模型往往基于虚假相关性而非真正的故障因果驱动因素，导致误诊和无效干预。虽然能预测故障可能发生，但缺乏系统方法来理解故障原因，从而无法确定最有效的干预措施。

Method: 使用预训练的因果基础模型作为"假设分析"模型，估计潜在修复措施的效果。通过测量每个干预对系统级KPI（如整体设备效率OEE）的因果效应，为生产线提供数据驱动的行动排名推荐。

Result: 使用半合成制造数据评估模型，并与基线机器学习模型进行比较。该方法不仅识别根本原因，还量化其操作影响，为工程师在因果环境中测试潜在解决方案提供技术基础。

Conclusion: 该论文为稳健的规范性维护框架奠定了技术基础，使工程师能够在因果环境中测试潜在解决方案，做出更有效的操作决策，减少昂贵的停机时间。
...
TL;DR: IndiMathBench是一个基于印度数学奥林匹克竞赛问题构建的数学定理证明评估基准，包含312个Lean 4形式化定理及其对应的自然语言问题陈述，通过AI辅助人工验证流程创建。


<details>
  <summary>Details</summary>
Motivation: 为了解决数学定理证明评估中缺乏高质量、经过人工验证的形式化基准的问题，特别是针对自然语言数学问题到形式化证明的转换挑战。

Method: 采用AI驱动的人工辅助流程：通过类别检索、迭代编译器反馈和多模型集成生成候选形式化，专家通过交互式仪表板进行高效验证，并利用自动化质量摘要辅助评估。

Result: 评估显示自动形式化仍然具有挑战性，语法有效性和语义正确性之间存在显著差距，即使通过迭代优化，定理证明成功率仍然较低，表明该基准对数学推理构成了严峻测试。

Conclusion: IndiMathBench为数学定理证明提供了一个具有挑战性的测试平台，突出了当前AI系统在数学推理方面的局限性，特别是自然语言问题到形式化证明的转换能力仍有待提升。
...
TL;DR: ChartAnchor是一个全面的图表理解基准测试，包含8k+图表-表格-代码三元组，涵盖30种图表类型，用于评估多模态大语言模型在图表基础对齐方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在图表多样性、任务孤立性和评估框架完整性方面存在局限，无法全面评估图表基础对齐能力。图表基础对齐直接反映了模型在数值推理、多模态对齐和结构重建方面的能力，在现实场景中有重要应用。

Method: 提出ChartAnchor基准，包含两个互补任务：图表到代码生成（生成可执行代码复制图表）和受控图表到表格重建（使用预定义标题提取精确数据）。采用多级评估框架，集成语义验证、风格分析和感知指标来评估结构和内容级别的正确性。

Result: 在MLLMs上的广泛实验揭示了数值精度和代码合成方面的关键局限性，强调了超越表面感知的结构化推理的必要性。

Conclusion: 通过统一符号和数据驱动的基础对齐，ChartAnchor为图表基础对齐建立了严谨的基础，为在科学、金融和工业领域推进MLLMs提供了有意义的见解。
...
TL;DR: LEGIT数据集：大规模法律推理评估数据集，将法庭判决转化为层次化论证树，用于评估LLM生成的法律推理轨迹的质量。


<details>
  <summary>Details</summary>
Motivation: 在专家领域（如法律）评估LLM生成推理轨迹的质量对于确保可信度和可解释性至关重要，但由于此类推理任务的固有复杂性，评估仍然具有挑战性。

Method: 引入LEGIT数据集（24K实例），将法庭判决转化为对立双方论证和法庭结论的层次化树结构，作为评估推理轨迹问题覆盖度和正确性的评分标准。通过专家标注验证评分标准的可靠性。

Result: 1. LLM的法律推理能力严重受法律问题覆盖度和正确性影响；2. RAG和基于评分标准的RL对法律推理能力带来互补性益处：RAG提高整体推理能力，RL提高正确性但会降低覆盖度。

Conclusion: LEGIT数据集为评估LLM在复杂专家领域中的推理质量提供了有效工具，揭示了不同增强技术对法律推理能力的差异化影响。
...
TL;DR: Med-CRAFT是一个神经符号数据工程框架，通过确定性图遍历过程自动化生成医学视频推理基准，解决了高质量逻辑标注数据稀缺的问题。
...
Result: 构建了M3-Med-Auto大规模医学视频推理基准，展示细粒度时间选择性和多跳逻辑复杂性。评估表明自动管道生成的查询复杂度可与专家策划数据集相媲美，逻辑对齐分析显示图拓扑与最先进MLLM推理步骤高度相关。
...
<details>
  <summary>Details</summary>
Motivation: 当任务规范不够详细时，智能体可能无法学习到有用的策略。需要一种方法来改进粗粒度的逻辑规范，为强化学习提供更好的指导

Method: 提出AutoSpec框架，搜索逻辑规范的精化版本。利用SpectRL规范逻辑的组合特性，设计了四种精化程序来修改规范的抽象图结构，包括精化现有边规范或引入新的边规范
...
TL;DR: SimWorld是一个基于虚幻引擎5构建的新型模拟器，专门用于在丰富、真实世界般的环境中开发和评估LLM/VLM智能体，解决现有模拟器在物理和社会环境模拟方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有世界模拟器存在局限性：依赖有限的手工制作环境、模拟简化的游戏式物理和社会规则、缺乏对LLM/VLM智能体的原生支持。这阻碍了AI智能体在复杂物理和社会环境中的发展，特别是需要自主生存和创造价值（如赚钱或经营业务）的场景。

Method: 基于虚幻引擎5构建SimWorld模拟器，提供三个核心能力：1）真实、开放式的世界模拟，包括准确的物理和社会动态以及语言驱动的程序化环境生成；2）为LLM/VLM智能体提供丰富的接口，支持多模态世界输入和不同抽象级别的开放词汇动作；3）多样且可扩展的物理和社会推理场景，用户可轻松定制。

Result: 通过在涉及战略合作与竞争的长期多智能体配送任务中部署前沿LLM智能体（如GPT-4o、Gemini-2.5-Flash、Claude-3.5和DeepSeek-Prover-V2），展示了SimWorld的能力。结果显示不同模型具有不同的推理模式和局限性。

Conclusion: SimWorld作为一个开源平台，有望成为跨学科推进真实世界智能体智能的基础平台，为开发能够在复杂物理和社会环境中有效运作的AI智能体提供必要的模拟环境。
...
TL;DR: 论文提出GUIDE框架，通过引入量化模型性能特征来改进LLM编排器的模型选择，在提升准确率的同时显著降低能耗和延迟。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的编排器依赖定性模型描述进行决策，这些描述不能反映真实模型能力和性能特征，导致模型选择次优、准确率下降和能耗增加。

Method: 提出GUIDE框架，这是一个考虑性能-能耗权衡的能源感知模型选择框架，通过在决策中纳入量化模型性能特征来改进编排效果。

Result: GUIDE在各种评估任务中将准确率提高了0.90%-11.92%，能耗效率提升高达54%，同时将编排器模型选择延迟从4.51秒降低到7.2毫秒。

Conclusion: 通过引入量化模型性能特征，GUIDE框架有效解决了LLM编排器的局限性，在准确率、能耗效率和延迟方面都取得了显著改进。
...
TL;DR: 该论文提出"基础先验"概念，将大语言模型生成的合成数据视为从先验预测分布中抽取的样本，而非真实观测，从而为在实证研究中使用合成数据提供原则性框架。
...
Method: 提出"基础先验"概念，将生成过程的主观性建模为对用户预期数据分布、提示工程过程和模型信任度的显式依赖。将基础先验推导为用户原始先验的指数倾斜广义贝叶斯更新，其中信任参数控制合成数据的权重。

Result: 建立了将合成数据及相关基础先验纳入标准统计和计量经济学工作流程的方法，展示了在精炼复杂模型、信息潜在构念、指导实验设计以及增强随机系数和部分线性规范等应用中的使用方式。

Conclusion: 通过将生成输出视为结构化、显式主观的先验而非经验观测，该框架为在实证工作中利用基础模型提供了原则性方法，同时避免了将合成"事实"与真实数据混淆。
...
TL;DR: 该研究比较了8种预测模型在制造业预测性维护中的应用，发现因果AI方法在成本不对称场景下显著优于传统相关性方法，实现年节省116万美元成本


<details>
  <summary>Details</summary>
Motivation: 制造业预测性维护面临极端成本不对称问题（漏检故障成本是误报成本的50倍），传统机器学习方法优化统计准确性指标而非实际运营成本，且无法可靠区分因果关系与虚假相关

Method: 研究评估了8种预测模型，包括基线统计方法和正式因果推断方法，使用10,000台CNC机床数据集（故障率3.3%），比较不同模型在成本不对称环境下的表现

Result: 正式因果推断模型（L5）实现年成本节省116万美元（减少70.2%），比最佳相关性决策树模型（L3）每年多节省约8万美元；因果模型保持最高召回率87.9%，同时将误报减少97%（从165降至5），精度达92.1%，训练-测试性能差距仅2.6个百分点

Conclusion: 因果AI方法结合领域知识在预测性维护应用中能产生更优的财务结果和更可解释的预测，相比基于相关性的方法具有显著优势
...
TL;DR: 提出基于知识图谱引导的思维链框架，为电子健康记录中的疾病预测生成临床可解释的推理过程，在有限训练数据下超越传统基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有电子健康记录预测模型通常提供粗略的事后解释，对患者层面的临床决策支持有限，需要更临床相关且时间一致的解释性方法。

Method: 将ICD-9代码映射到PrimeKG知识图谱，提取疾病相关节点和多跳推理路径作为思维链的支架，仅保留与观察结果匹配的解释，然后微调轻量级LLaMA-3.1-Instruct-8B和Gemma-7B模型。

Result: 在10种PrimeKG映射疾病和有限训练数据（400和1000例）下，KG引导模型优于传统基线，AUROC达0.66-0.70，macro-AUPR达0.40-0.47；在CRADLE队列上零样本迁移，准确率从约0.40-0.51提升至0.72-0.77。

Conclusion: KG引导的思维链框架能够生成临床相关、时间一致的推理解释，在有限数据下实现有效的疾病预测，且盲法临床评估显示医生更偏好其解释的清晰度、相关性和临床正确性。
...
TL;DR: 论文构建了一个使用ROT-13加密进行推理的模型测试平台，评估了机制可解释性方法（特别是logit lens）能否解码模型的隐藏推理过程，发现该方法能有效翻译加密推理，并开发了完全无监督的解码流程。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，人们担心它们可能发展出人类无法监督的编码或隐藏推理过程。本研究旨在调查当前的可解释性技术能否穿透这种编码推理。

Method: 通过微调推理模型（DeepSeek-R1-Distill-Llama-70B）使其在ROT-13加密下进行思维链推理，同时保持可理解的英语输出。评估机制可解释性方法（特别是logit lens分析）仅使用内部激活来解码模型的隐藏推理过程。

Result: logit lens能有效翻译编码推理，准确率在中后期层达到峰值。开发了结合logit lens与自动改写的完全无监督解码流程，能够从内部模型表示中重建完整的推理记录，具有显著准确性。

Conclusion: 当前机制可解释性技术对简单形式的编码推理可能比之前理解的更稳健。研究为评估可解释性方法提供了初步框架，有助于维护对日益强大AI系统的监督。
...
TL;DR: 该论文提出了RoboDriveBench，首个专注于端到端轨迹预测任务的鲁棒性基准测试，用于评估基于视觉语言模型的自动驾驶系统在实际驾驶场景中的可靠性，并提出了RoboDriveVLM框架和测试时自适应方法来提升系统鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的端到端自动驾驶系统虽然利用大语言模型生成驾驶决策，但在实际驾驶场景中存在多种风险。为了评估VLM是否真正适用于自动驾驶，需要系统性地评估其在现实挑战中的表现。

Method: 1) 提出RoboDriveBench基准测试，包含11个模拟场景（6个传感器损坏场景和5个提示损坏场景），每个场景250个独特驾驶场景和5,689帧图像，总计64,559个轨迹预测案例；2) 提出RoboDriveVLM框架，通过将更多模态数据（如激光雷达和雷达）映射到统一潜在空间来增强鲁棒性；3) 提出基于跨模态知识蒸馏的测试时自适应方法。

Result: 通过大量实验，研究工作突出了当前基于VLM的端到端自动驾驶系统的局限性，并为实际部署提供了更可靠的解决方案。源代码和数据集将公开发布。

Conclusion: 该研究强调了当前VLM自动驾驶系统在实际应用中的脆弱性，提出了系统性的评估基准和改进框架，为构建更鲁棒的自动驾驶系统提供了重要参考。
...
TL;DR: CuES框架通过好奇心驱动和环境基础的任务生成，解决Agentic RL中的任务稀缺问题，使智能体能够在没有预定义任务的环境中自主生成多样、可执行的任务。


<details>
  <summary>Details</summary>
Motivation: 在复杂工具增强环境中，基于大语言模型的智能体需要强化学习来改进，但其效果依赖于结构化训练任务。许多现实环境中缺乏预定义任务（任务稀缺问题），这成为扩展Agentic RL的关键瓶颈。现有方法通常假设存在预定义任务集合，这在工具语义和可用性未知的新环境中不成立。

Method: 提出CuES框架：好奇心驱动和环境基础的任务合成方法。通过内在好奇心驱动探索，将交互模式抽象为可重用的任务模式，并通过轻量级自上而下指导和基于记忆的质量控制进行细化。该方法直接从环境结构和可用性生成任务，不依赖人工种子或外部语料库。

Result: 在三个代表性环境（AppWorld、BFCL、WebShop）中，CuES生成的任务分布在多样性和可执行性方面匹配或超越手动策划的数据集，并带来显著的下游策略改进。

Conclusion: 好奇心驱动、环境基础的任务生成为智能体提供了可扩展的基础，使智能体不仅学习如何行动，还学习学习什么。这为解决Agentic RL中的任务稀缺问题提供了有效方案。
...
TL;DR: 将Never Give Up算法扩展到多智能体环境，在PettingZoo的simple_tag环境中评估性能，发现共享经验回放池能带来最佳效果


<details>
  <summary>Details</summary>
Motivation: NGU算法在稀疏奖励的强化学习任务中表现出色，但主要应用于单智能体环境。本研究旨在探索NGU在多智能体环境中的适用性和效果

Method: 将NGU扩展到多智能体环境，在simple_tag环境中与多智能体DQN基线对比。研究了三种设计选择：1) 共享与独立经验回放池；2) 使用不同k阈值共享情节新颖性；3) 使用异构beta参数值

Result: NGU在多智能体环境中获得比基线更高的回报和更稳定的学习动态。共享经验回放池效果最佳，新颖性共享在k=1时表现相当但k值增大会降低学习效果，异构beta值未带来改进

Conclusion: NGU可有效应用于多智能体环境，关键在于共享经验和精心调整内在探索信号。共享经验回放池结合NGU内在探索能带来最佳性能和稳定性
...
TL;DR: 该论文提出了一种新的合成数据生成范式，通过模拟人类认知过程而非模仿数据表面特征，解决了传统合成数据因追求统计平滑而丢失人类文本认知特征的问题，从而缓解模型崩溃危机。


<details>
  <summary>Details</summary>
Motivation: 当前合成数据生成范式过度追求统计平滑性，系统性地移除了人类文本中具有认知基础的长尾不规则特征，导致数据认知贫乏，加速了模型崩溃。需要一种能模拟人类认知过程而非简单模仿数据表面属性的新方法。

Method: 提出Prompt驱动的认知计算框架(PMCSF)，包含两个核心组件：认知状态解码器(CSD)将非结构化文本反转为结构化认知向量；认知文本编码器(CTE)将这些认知状态通过数学定义的认知扰动算子重新转化为富含人类典型缺陷的文本。

Result: 在认知编解码验证中，CTE生成的文本与人类文本的Jensen-Shannon散度为0.0614（标准LLM输出为0.4431），通过双盲专业媒体评审，跨异构模型的认知轮廓对齐ICC>0.9。在A股市场的功能增益评估中，使用CTE生成数据的策略在2015年股灾期间最大回撤减少47.4%，产生8.6%的防御性阿尔法，超过交易成本33倍。

Conclusion: 模拟人类认知限制而非复制表面数据，能够产生具有真正功能增益的合成数据，为解决AI数据崩溃危机提供了可行的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂迭代问题（如科学文档生成）中工具创建需要大量领域适应时间和人工反馈的问题，通过多智能体协作和人类指导来对齐人类意图和流程。

Method: 使用四个专门化智能体（教练、编码员、批评者、资本化器）的框架，通过强化动态提示和系统化人工反馈集成来生成和验证工具，支持工具重用和迭代改进。

Result: 初步实验展示了系统能够生成前沿研究论文或专利（给定摘要），证明了在复杂迭代问题解决中的有效性，并讨论了在其他迭代问题解决场景的适用性。

Conclusion: CollabToolBuilder是一个系统级集成方法，结合了多智能体上下文学习、专家在环控制和可重用工具资本化，为复杂迭代问题提供了一种有效的解决方案。
...
TL;DR: 提出一种新的状态转移事件时间序列（STE-ts）和选择性时间汉明距离（STH），避免传统方法中对大规模数据库进行耗时且失真的重采样操作。


<details>
  <summary>Details</summary>
Motivation: 离散事件系统广泛存在于自然观测、社会经济科学和工业系统中。传统分析方法通常没有充分利用其事件/状态的双重性质：要么将信号建模为强调事件顺序对齐的转移事件序列，要么建模为分类或有序状态时间序列，后者通常需要进行重采样，随着观测周期和事件数量的增长，这种操作既耗时又会导致失真。

Method: 定义状态转移事件时间序列（STE-ts），并提出新的选择性时间汉明距离（STH），该方法同时利用转移时间和状态持续时间，避免对大规模数据库进行耗时且失真的重采样。

Result: STH能够泛化重采样汉明距离和Jaccard度量，具有更好的精度和计算时间，并且能够聚焦于多个感兴趣的状态。在模拟和真实世界数据集上验证了这些优势。

Conclusion: 提出的STE-ts和STH方法有效解决了传统离散事件系统分析中的重采样问题，在保持精度的同时显著提高了计算效率，为大规模数据库分析提供了更好的解决方案。
...
TL;DR: 本研究开发了基于DSPy和GEPA模块的可编程偏倚风险评估管道，通过结构化代码优化替代人工提示设计，在随机对照试验的偏倚风险评估中实现了透明、可复现的自动化评估。


<details>
  <summary>Details</summary>
Motivation: 随机对照试验的偏倚风险评估对于证据合成至关重要，但传统方法资源密集且存在评估者间变异性。大型语言模型提供了自动化途径，但现有方法依赖难以复制、泛化或评估的人工设计提示。

Method: 引入可编程偏倚风险评估管道，使用DSPy及其GEPA模块替代临时提示设计，通过帕累托引导搜索优化LLM推理，生成可检查的执行轨迹，实现优化过程的透明复制。在100个已发表荟萃分析的随机对照试验中评估了七个偏倚风险领域，比较了开源模型和商业模型的表现。

Result: 在方法报告更清晰的领域（如随机序列生成），GEPA生成的提示表现最佳；在分配隐藏和参与者盲法方面结果相似；商业模型整体表现略优。与三种人工设计提示相比，GEPA实现了最高整体准确率，在随机序列生成和选择性报告方面性能提升30%-40%，在其他领域表现相当。

Conclusion: GEPA能够为偏倚风险评估生成一致且可复现的提示，支持在证据合成中结构化、原则性地使用大型语言模型。
...
TL;DR: 利用大型语言模型将合成知识转化为代码，为计算机辅助合成规划系统提供战略层面的分析能力，实现基于自然语言的路线检索和战略评估


<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助合成规划系统虽然能生成化学上有效的反应步骤，但缺乏战略层面的考量，如收敛组装、保护基最小化、最佳环形成序列等。需要将合成战略知识系统化地整合到CASP系统中

Method: 利用大型语言模型将合成知识转化为Python代码，将战略原则（如战略官能团转换、环构建策略等）转化为可验证的代码函数，而非简单的启发式规则。创建了USPTO-ST数据集（带有战略标签的合成路线）

Result: 实现了基于自然语言的路线检索，在基准测试中达到75%的Top-3准确率。通过时间趋势分析和化学直观路线聚类验证了库的有效性，提供了比传统方法更细粒度的分区

Conclusion: 该工作弥合了CASP系统中战术与战略之间的鸿沟，使合成路线能够基于战略标准而非仅结构进行规范、搜索和评估，为计算机辅助合成规划提供了新的战略分析能力
...
<details>
  <summary>Details</summary>
Motivation: LLM经常生成不可靠的答案，而启发式不确定性方法无法完全区分正确和错误的预测，导致用户在没有统计保证的情况下接受错误答案。

Method: 提出LEC方法，将选择性预测重新解释为约束决策问题，通过在线性期望约束下计算FDR约束、覆盖率最大化的阈值。扩展到双模型路由机制：当当前模型的不确定性超过校准阈值时，将其委托给更强的模型，同时保持统一的FDR保证。

Result: 在封闭式和开放式问答数据集上的评估显示，LEC实现了更严格的FDR控制，并显著提高了样本保留率。双模型路由机制在保持较低风险水平的同时，比单个模型接受了更多正确样本。

Conclusion: LEC通过FDR控制为LLM预测提供统计保证，双模型路由机制进一步优化了风险控制和样本接受率，为解决LLM可靠性问题提供了有效框架。
...
TL;DR: 该论文提出了一种基于CLIP原理的强化学习方法，通过将语言指令和策略嵌入对齐到统一表示空间，实现跨任务的高效迁移学习。


<details>
  <summary>Details</summary>
Motivation: 当前需要开发能够在同一环境中解决多个任务的智能体，特别是当这些任务与语言自然关联时。传统方法在跨任务迁移学习方面存在效率问题。

Method: 受计算机视觉中CLIP（对比语言-图像预训练）的启发，将语言指令和对应策略视为同一任务在不同模态的表示，通过对比学习对齐语言和策略嵌入到统一表示空间。

Result: 实验结果表明，该方法在跨任务迁移学习中实现了更快的收敛速度，证明了算法在任务间高效迁移的有效性。

Conclusion: 通过将CLIP原理扩展到强化学习领域，成功构建了语言和策略的统一表示空间，为多任务强化学习提供了一种有效的迁移学习框架。
...
TL;DR: LLM Jury-on-Demand：一个基于学习的动态框架，通过训练可靠性预测器来评估LLM法官何时与人类专家意见一致，为每个数据点动态选择最可靠的法官组成陪审团，实现可扩展且上下文感知的评估。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型进入高风险领域，需要既适合实时部署又可扩展，又能支持关键决策的可靠评估方法。人工评估可靠但缓慢昂贵，单一LLM法官存在偏见，静态陪审团缺乏适应性。

Method: 训练一组可靠性预测器，利用token分布、嵌入和结构化输入特征来评估LLM法官何时会与人类专家意见一致。为每个数据点动态选择最可靠的法官组成最优陪审团，并使用其可靠性作为权重聚合评分。

Result: 在摘要和RAG基准测试中，动态陪审团系统相比单一法官和静态陪审团基线，与人类判断的相关性显著更高。
...
### [291] [H-Neurons: On the Existence, Impact, and Origin of Hallucination-Associated Neurons in LLMs](https://arxiv.org/abs/2512.01797)
*Cheng Gao,Huimin Chen,Chaojun Xiao,Zhiyi Chen,Zhiyuan Liu,Maosong Sun*

Main category: cs.AI

TL;DR: 该研究系统探索了大语言模型中的幻觉相关神经元（H-Neurons），发现仅需不到0.1%的神经元就能可靠预测幻觉发生，这些神经元与过度顺从行为存在因果关联，且源于预训练阶段。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉（看似合理但事实错误的输出），这削弱了其可靠性。先前研究主要从宏观角度（如训练数据和目标）考察幻觉，而神经元层面的机制仍未被充分探索。

Method: 从三个角度系统研究幻觉相关神经元：1）识别：确定能可靠预测幻觉发生的稀疏神经元子集；2）行为影响：通过受控干预验证这些神经元与过度顺从行为的因果关联；3）起源：追溯这些神经元到预训练基础模型，分析其形成阶段。

Result: 发现极稀疏的神经元子集（少于总神经元的0.1%）能可靠预测幻觉发生，且在不同场景下具有强泛化能力。这些神经元与过度顺从行为存在因果关联，且源于预训练阶段，在预训练后仍能有效预测幻觉。

Conclusion: 该研究将宏观行为模式与微观神经机制联系起来，为开发更可靠的大语言模型提供了洞见，揭示了幻觉的神经元基础及其在模型训练中的形成过程。
...
TL;DR: 该论文提出了一种基于n-gram语言模型的国际象棋走法预测框架，将走法预测视为行为分析任务，针对不同技能水平的玩家训练独立模型，并通过动态选择最合适的模型来提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 传统国际象棋引擎主要关注计算最优走法，忽略了人类玩家在不同技能水平下的走法变异性。为了克服这一局限，需要一种能够捕捉特定技能水平玩家走法模式的新方法。

Method: 使用n-gram语言模型捕捉特定玩家技能水平的走法模式。将玩家分为7个不同的技能组（从新手到专家），使用Lichess开源平台的数据训练独立模型。框架包含动态模型选择模块，根据前序走法序列选择最合适的模型进行预测。

Result: 模型选择模块在利用早期游戏信息（16个半走法）时，技能水平分类准确率最高可达31.7%。走法预测框架的Selector Assisted Accuracy比基准准确率提高了39.1%。框架具有计算效率，适合实时国际象棋分析。

Conclusion: 该框架成功将国际象棋走法预测转化为行为分析任务，通过针对不同技能水平的模型和动态选择机制，显著提高了走法预测的准确性，同时保持了计算效率，适用于实时分析应用。
...
TL;DR: 提出基于规则增强的LLM评估范式，通过规则蒸馏和规则应用策略提升LLM作为通用评估器的能力


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估器依赖人工设计的评估原则，成本高且与标注数据及LLM理解不一致，难以泛化到多样化评估场景

Method: 1) 规则蒸馏：使用LLM辅助的蒙特卡洛树搜索自动从数据中提取评分规则；2) 规则应用：提出Chain-of-Rule指导LLM遵循规则，并通过强化学习训练规则增强的LLM评估器

Result: 在多样化任务上的广泛实验证明了该方法在各种评估场景中的有效性和泛化能力

Conclusion: 提出的规则增强评估范式解决了现有LLM评估方法的局限性，实现了更通用、可扩展的评估能力
...
TL;DR: 本文探讨RL如何提升推理能力：是合成新技能还是放大现有行为。通过互补推理任务，发现SFT在分布内表现好但分布外泛化差，存在"SFT泛化悖论"。RL能合成复杂推理策略，但前提是基础模型需先通过SFT掌握原子技能。


<details>
  <summary>Details</summary>
Motivation: 研究强化学习对推理能力的贡献机制——是激励新技能合成还是仅仅放大现有行为。通过互补推理任务来探究这个问题，该任务需要整合内部参数知识和外部上下文信息。

Method: 使用人类传记的合成数据集，将互补推理能力严格解耦为两个原子技能：参数推理（依赖内部知识）和上下文推理（依赖外部信息）。在三个难度级别评估泛化能力：I.I.D.、组合和零样本设置。比较SFT和RL在不同设置下的表现。

Result: 发现SFT在分布内表现良好，但在分布外泛化（特别是零样本设置）上困难，存在"SFT泛化悖论"——模型仅靠记忆路径捷径。RL能作为推理合成器而非概率放大器，但前提是基础模型需先通过SFT掌握独立的原子技能。

Conclusion: RL不仅仅是放大器，在具备足够原子基础的情况下，RL能主动从学习到的原语中合成复杂推理策略，无需对此类复杂策略进行显式监督。解耦的原子训练后接RL为复杂推理任务提供了可扩展的泛化路径。
...
TL;DR: CoG（Chain of Ground）是一个无需训练的多步GUI定位框架，通过多模态大语言模型进行迭代视觉推理和精炼，在ScreenSpot Pro基准上达到68.4%准确率，提升4.8个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在GUI视觉定位方面虽然表现出色，但仍难以处理小型或视觉相似的目标，以及在真实世界布局中的模糊性问题。这些限制源于有限的定位能力和未充分利用现有推理潜力。

Method: 提出Chain of Ground（CoG）框架，采用无需训练的多步定位方法。模型不是直接预测，而是通过迭代的视觉推理和假设调整来逐步精确定位，实现更准确和可解释的定位。

Result: 在ScreenSpot Pro基准上达到68.4%准确率，提升4.8个百分点。在真实世界工业控制面板数据集TPanel UI上，相比强基线Qwen3 VL 235B提升6.9个百分点，展示了在真实世界和数字界面中的有效性。

Conclusion: 通过结构化迭代精炼而非额外训练，可以释放多模态大语言模型的定位潜力。该方法为GUI定位提供了一种新的方向，特别是在处理复杂界面和真实世界场景时表现出色。
...
### [296] [A Comprehensive Survey on Surgical Digital Twin](https://arxiv.org/abs/2512.00019)
...
TL;DR: 这篇综述系统回顾了手术数字孪生（SDTs）的现状，澄清了术语和范围，提出了分类体系，总结了关键技术进展，并指出了验证、安全、数据治理等开放问题，旨在推动SDTs从实验室原型向临床常规应用的转化。


<details>
  <summary>Details</summary>
Motivation: 随着多模态手术数据的加速可用和实时计算的发展，手术数字孪生（SDTs）作为虚拟对应物出现，能够在术前、术中和术后护理中反映、预测和指导决策。尽管有前景的演示，但SDTs面临持续挑战：在严格延迟预算下融合异构成像、运动学和生理数据；平衡模型保真度与计算效率；确保鲁棒性、可解释性和校准不确定性；以及在临床环境中实现互操作性、隐私和监管合规性。

Method: 本文采用结构化综述方法：1）澄清术语和范围；2）按目的、模型保真度和数据源提出分类体系；3）综合总结可变形配准与跟踪、实时仿真与协同仿真、AR/VR引导、边缘-云编排、AI场景理解与预测等关键技术的最新进展；4）对比非机器人孪生与机器人闭环架构；5）识别验证与基准测试、安全保证与人因工程、生命周期"数字线程"集成、可扩展数据治理等开放问题。

Result: 通过统一词汇、组织能力和突出差距，本文系统梳理了SDTs的技术现状，提出了一个全面的研究议程，旨在指导SDTs的设计和部署，并推动从实验室原型向常规手术护理的转化。

Conclusion: 本文为构建可信赖、符合标准的手术数字孪生提供了研究路线图，强调需要解决验证、安全、数据治理等关键挑战，以实现可衡量的临床效益，最终推动SDTs从概念验证向临床常规应用的转化。
...
### [297] [Foundation Models for Trajectory Planning in Autonomous Driving: A Review of Progress and Open Challenges](https://arxiv.org/abs/2512.00021)
*Kemal Oksuz,Alexandru Buburuzan,Anthony Knittel,Yuhan Yao,Puneet K. Dokania*

Main category: cs.RO

TL;DR: 本文对基于多模态基础模型的自动驾驶轨迹规划方法进行了系统性综述，涵盖37种最新方法，提出了统一分类法，并评估了代码和数据集的开放性。


<details>
  <summary>Details</summary>
Motivation: 多模态基础模型的出现显著改变了自动驾驶技术，从传统手工设计转向统一的基于基础模型的方法，能够直接从原始传感器输入推断运动轨迹。这些新方法还可以将自然语言作为额外模态，如视觉-语言-动作模型。本文旨在通过统一分类法全面评估这类方法的架构设计选择、方法优势及其固有能力和局限性。

Method: 本文采用系统性文献综述方法，对37种最近提出的基于基础模型的轨迹规划方法进行了全面调查。研究建立了统一分类法来评估这些方法的架构设计选择，并特别关注了代码和数据集的开源程度，为实践者和研究人员提供有价值的信息。

Result: 研究提供了对基于基础模型的自动驾驶轨迹规划方法的全面评估，涵盖了37种最新方法。通过统一分类法分析了这些方法的架构设计、优势、局限性和能力。特别评估了代码和数据集的开放性，并创建了配套网页，按分类法对方法进行编目。

Conclusion: 多模态基础模型正在推动自动驾驶轨迹规划技术的范式转变，从传统手工设计转向统一的端到端方法。本文的系统性综述为研究人员和实践者提供了这一快速发展领域的全面概览，通过统一分类法和开放性评估为未来研究提供了重要参考框架。
...
### [298] [A Survey on Improving Human Robot Collaboration through Vision-and-Language Navigation](https://arxiv.org/abs/2512.00027)
*Nivedan Yakolli,Avinash Gautam,Abhijit Das,Yuankai Qi,Virendra Singh Shekhawat*

Main category: cs.RO

TL;DR: 本文对视觉语言导航（VLN）在机器人领域的最新进展进行了全面综述，分析了多机器人协调的挑战与前景，提出了未来发展方向。


<details>
  <summary>Details</summary>
Motivation: VLN作为多模态协作任务，要求智能体理解人类指令、在3D环境中导航并在模糊条件下有效沟通。当前模型在多智能体系统中面临双向通信、模糊性解决和协作决策等挑战，需要系统性综述来推动该领域发展。

Method: 通过对约200篇相关文献的系统性综述，深入分析当前VLN研究现状，识别关键挑战，并提出未来发展方向。

Result: 综述发现当前VLN模型在多智能体协作方面存在显著不足，特别是在双向通信、模糊性解决和协作决策方面。同时识别了提升多机器人协调的关键技术需求。

Conclusion: 未来VLN系统应支持主动澄清、实时反馈和基于高级自然语言理解的上下文推理。去中心化决策框架与动态角色分配对于可扩展、高效的多机器人协作至关重要，这些创新将显著增强人机交互，推动在医疗、物流和灾难响应等领域的实际部署。
...
### [299] [VISTAv2: World Imagination for Indoor Vision-and-Language Navigation](https://arxiv.org/abs/2512.00041)
*Yanjia Huang,Xianshun Jiang,Xiangbo Gao,Mingyang Wu,Zhengzhong Tu*

Main category: cs.RO

TL;DR: VISTAv2是一个生成式世界模型，通过条件扩散Transformer预测未来视角，生成在线价值地图指导视觉语言导航，在连续真实世界空间中实现鲁棒规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像想象的视觉语言导航方法存在局限性：缺乏在线、动作条件预测，不产生显式规划价值，且许多方法用长时程目标替代规划器导致脆弱和缓慢。需要弥合这一差距。

Method: 提出VISTAv2生成式世界模型，使用动作感知条件扩散Transformer视频预测器合成短期未来视图，通过视觉语言评分器与自然语言指令对齐，在可微分想象到价值头中融合多个推演，输出想象的自我中心价值地图。为提升效率，在VAE潜在空间进行推演，使用蒸馏采样器和稀疏解码。

Result: 在MP3D和RoboTHOR数据集上评估，VISTAv2超越了强基线方法。消融实验表明动作条件想象、指令引导价值融合和在线价值地图规划器都是关键组件。

Conclusion: VISTAv2为鲁棒视觉语言导航提供了实用且可解释的路径，通过生成式世界模型和在线价值地图规划，实现了动作条件想象与指令对齐的融合规划。
...
### [300] [Causal Reinforcement Learning based Agent-Patient Interaction with Clinical Domain Knowledge](https://arxiv.org/abs/2512.00048)
*Wenzheng Zhao,Ran Zhang,Ruth Palan Lopez,Shu-Fen Wung,Fengpei Yuan*

Main category: cs.RO

TL;DR: 提出因果结构感知强化学习（CRL）框架，将因果发现和推理融入策略优化，用于机器人辅助认知护理场景，相比传统无模型RL获得更高累积奖励和更一致的患者状态维持。


<details>
  <summary>Details</summary>
Motivation: 强化学习在自适应医疗干预（如痴呆症护理）中面临数据稀缺、决策需要可解释性、患者状态动态复杂且具有因果性的挑战。需要一种能够整合因果知识、提高决策效率和可解释性的方法。

Method: 提出因果结构感知强化学习（CRL）框架，显式地将因果发现和推理集成到策略优化中。该方法使智能体能够学习和利用描述人类行为状态与机器人动作之间因果依赖关系的有向无环图（DAG），从而促进更高效、可解释和鲁棒的决策。

Result: 在模拟机器人辅助认知护理场景中验证，CRL智能体优于传统无模型RL基线：获得更高的累积奖励、更一致地维持理想患者状态、表现出可解释且与临床一致的行为。CRL的性能优势在不同权重策略和超参数设置下保持鲁棒。还展示了轻量级LLM部署方案。

Conclusion: CRL展示了因果强化学习在人机交互应用中的潜力，特别是在可解释性、适应性和数据效率至关重要的领域，如医疗护理干预。
...
### [301] [Reinforcement Learning from Implicit Neural Feedback for Human-Aligned Robot Control](https://arxiv.org/abs/2512.00050)
*Suzie Kim*

Main category: cs.RO

TL;DR: 提出RLIHF框架，利用EEG信号中的ErrPs提供隐式反馈，解决稀疏奖励下的RL学习问题，在机器人抓取任务中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统RL方法在稀疏奖励条件下学习效果差，需要手动设计复杂奖励函数。现有RLHF方法依赖显式反馈（如按钮、偏好标签），会中断自然交互过程并增加用户认知负担。

Method: 提出RLIHF框架，使用非侵入式脑电图（EEG）信号，特别是错误相关电位（ErrPs），通过预训练解码器将原始EEG信号转换为概率奖励分量，在稀疏外部奖励条件下实现有效策略学习。

Result: 在基于MuJoCo物理引擎的仿真环境中，使用Kinova Gen2机械臂进行复杂抓取放置任务（需避开障碍物）。结果显示，使用解码EEG反馈训练的智能体性能与使用密集手动设计奖励训练的智能体相当。

Conclusion: 验证了使用隐式神经反馈在交互式机器人中进行可扩展、人类对齐的强化学习的潜力，为稀疏奖励条件下的RL学习提供了新途径。
...
### [302] [An adaptive experience-based discrete genetic algorithm for multi-trip picking robot task scheduling in smart orchards](https://arxiv.org/abs/2512.00057)
*Peng Chen,Jing Liangb,Kang-Jia Qiao,Hui Song,Cai-Tong Yue,Kun-Jie Yu,Ponnuthurai Nagaratnam Suganthan,Witold Pedrycz*

Main category: cs.RO

TL;DR: 本文提出了一种自适应经验离散遗传算法（AEDGA）来解决多行程采摘机器人任务调度问题，通过集成负载距离平衡初始化、聚类局部搜索和自适应选择策略，显著优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 智能果园中多机器人系统面临复杂的调度优化挑战，现有算法存在局部搜索冗余、搜索速度慢和易陷入局部最优的问题，特别是在大规模场景下。

Method: 提出自适应经验离散遗传算法（AEDGA），包含三个创新：1）集成负载距离平衡初始化方法；2）基于聚类的局部搜索机制；3）经验自适应选择策略。同时开发了三种解决方案修复策略来满足完工时间约束。

Result: 在18个新提出的测试实例和24个现有测试问题上进行综合实验，AEDGA显著优于8个最先进的算法。

Conclusion: AEDGA通过创新的初始化、局部搜索和自适应策略，有效解决了多行程采摘机器人任务调度问题，在性能上超越了现有算法。
...
### [303] [SpeedAug: Policy Acceleration via Tempo-Enriched Policy and RL Fine-Tuning](https://arxiv.org/abs/2512.00062)
*Taewook Nam,Sung Ju Hwang*

Main category: cs.RO

TL;DR: SpeedAug：基于强化学习的策略加速框架，通过速度增强演示预训练策略，提高策略执行速度的样本效率


<details>
  <summary>Details</summary>
Motivation: 当前机器人策略学习虽然能实现复杂操作，但执行速度往往落后于硬件能力。现有方法通过重新解释动作序列来适应不同执行速度，但会遇到分布偏移问题。强化学习可以无需额外演示就能加速策略，但无引导的探索样本效率低。

Method: SpeedAug框架：1）通过速度增强演示预训练策略，构建包含不同执行节奏的行为先验；2）从这个节奏丰富的策略初始化进行RL微调，提高样本效率。

Result: 在机器人操作基准测试中，从这种节奏丰富的策略初始化的RL微调显著提高了现有RL和策略加速方法的样本效率，同时保持高成功率。

Conclusion: SpeedAug通过构建包含不同执行节奏的行为先验，有效解决了策略加速中的样本效率问题，使预训练策略能够高效适应更快的任务执行。
...
### [304] [Enhancing Cognitive Robotics with Commonsense through LLM-Generated Preconditions and Subgoals](https://arxiv.org/abs/2512.00069)
*Ohad Bachner,Bar Gamliel*

Main category: cs.RO

TL;DR: 将大型语言模型与符号规划结合，通过LLM从自然语言任务中推断隐含前提条件和子目标，提高机器人任务执行成功率


<details>
  <summary>Details</summary>
Motivation: 机器人执行日常任务时常因指令缺少常识细节（如隐藏前提条件和小型子目标）而失败。传统符号规划需要明确编写这些细节，耗时且不完整

Method: 结合大型语言模型与符号规划：给定自然语言任务，LLM推断合理的前提条件和子目标，将这些建议转换为形式化规划模型，在模拟中执行生成的计划

Result: 相比没有LLM步骤的基线规划器，该系统产生更多有效计划，获得更高的任务成功率，并在环境变化时表现出更好的适应性
...
### [305] [Bootstrap Dynamic-Aware 3D Visual Representation for Scalable Robot Learning](https://arxiv.org/abs/2512.00074)
...
Method: AFRO将状态预测构建为生成扩散过程，在共享潜在空间中联合建模前向和逆向动力学以捕捉因果转移结构。采用特征差分和逆向一致性监督防止动作学习中的特征泄漏。
...
### [306] [Arcadia: Toward a Full-Lifecycle Framework for Embodied Lifelong Learning](https://arxiv.org/abs/2512.00076)
...
TL;DR: Arcadia是一个闭环的具身终身学习框架，通过紧密耦合自主数据采集、生成式场景重建、共享表征架构和仿真评估四个阶段，实现持续改进和泛化，超越了单阶段优化方法。


<details>
  <summary>Details</summary>
Motivation: 当前具身学习系统通常只优化单一环节（数据收集、仿真、学习或部署），难以实现持续改进或泛化到更广泛场景。作者认为具身学习本质上是一个生命周期问题，需要闭环的终身学习框架。

Method: 提出Arcadia框架，包含四个紧密耦合的阶段：1）自主环境探索与数据采集；2）生成式场景重建与增强；3）共享的具身表征架构（统一导航和操作）；4）仿真评估与演化形成反馈闭环。这四个阶段不可分解，移除任一阶段都会破坏改进循环。

Result: Arcadia在导航和操作基准测试中取得持续提升，并能稳健地迁移到物理机器人上。这表明紧密耦合的生命周期方法（持续真实世界数据采集、生成式仿真更新和共享表征学习）支持终身改进和端到端泛化。

Conclusion: 具身学习需要闭环的生命周期方法而非单阶段优化。Arcadia通过紧密耦合四个阶段实现了持续改进和泛化能力，为通用具身智能体提供了可扩展的基础框架，并发布了标准化接口支持可复现评估和跨模型比较。
...
### [307] [A Hierarchical Framework for Humanoid Locomotion with Supernumerary Limbs](https://arxiv.org/abs/2512.00077)
*Bowen Zhi*

Main category: cs.RO

TL;DR: 该论文提出了一种分层控制架构，通过解耦学习式步态与模型式平衡控制，提升搭载超限肢体的人形机器人行走稳定性。


<details>
  <summary>Details</summary>
Motivation: 超限肢体在人形机器人上的集成会引入动态扰动，对稳定性构成重大挑战，需要专门的控制策略来解决这一问题。

Method: 设计分层控制架构：底层通过模仿学习和课程学习生成Unitree H1人形机器人的行走步态；高层利用超限肢体进行动态平衡控制。

Result: 动态平衡控制器显著提升稳定性：相比静态负载条件，步态模式更接近基准，质心轨迹DTW距离降低47%，步态周期内再稳定能力改善，地面反作用力呈现更协调的反相位模式。

Conclusion: 解耦的分层设计能有效缓解超限肢体质量和运动产生的内部动态扰动，使配备功能肢的人形机器人实现稳定运动。
...
### [308] ["Why the face?": Exploring Robot Error Detection Using Instrumented Bystander Reactions](https://arxiv.org/abs/2512.00262)
...
TL;DR: 通过颈部摄像头捕捉下巴区域面部表情，开发NeckNet-18模型重建3D面部反应，用于机器人错误检测，性能优于传统方法


<details>
  <summary>Details</summary>
Motivation: 人类通过观察同伴的微妙反应（如挑眉、笑声）来识别和纠正社交失误，但机器人难以感知和利用这些细微线索。现有方法无法有效捕捉人类对机器人错误的反应

Method: 使用颈部佩戴设备记录下巴区域面部表情，开发NeckNet-18模型进行3D面部重建，将下巴摄像头捕捉的反应映射到面部关键点和头部运动，建立机器人错误检测模型

Result: 基于面部反应的机器人错误检测模型性能优于使用OpenFace或视频数据的标准方法，尤其在参与者内部数据上表现出良好的泛化能力

Conclusion: 通过扩展人在环机器人感知，利用下巴摄像头捕捉的微妙面部反应，可以促进机器人在多样化人类环境中的无缝集成，推动社交线索检测边界，为适应性机器人开辟新途径
...
### [309] [RealAppliance: Let High-fidelity Appliance Assets Controllable and Workable as Aligned Real Manuals](https://arxiv.org/abs/2512.00287)
*Yuzheng Gao,Yuxing Long,Lei Kang,Yuchong Guo,Ziyan Yu,Shangqing Mao,Jiyao Zhang,Ruihai Wu,Dongjiang Li,Hui Shen,Hao Dong*

Main category: cs.RO

TL;DR: RealAppliance数据集包含100个高保真家电模型，具有完整的物理、电子机制和与手册对齐的程序逻辑，并基于此提出RealAppliance-Bench基准测试，用于评估多模态大语言模型和具身操作规划模型在家电操作规划任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有家电资产存在渲染质量差、机制不完整、与手册不对齐等问题，导致仿真与现实之间存在差距，阻碍了家电操作开发研究。

Method: 1. 创建RealAppliance数据集：包含100个高保真家电，具有完整的物理、电子机制和与手册对齐的程序逻辑；2. 提出RealAppliance-Bench基准测试：评估多模态大语言模型和具身操作规划模型在四个关键任务上的表现。

Result: 基于RealAppliance数据集建立了基准测试，并对模型性能进行了分析，为家电操作研究提供了见解。

Conclusion: RealAppliance数据集和基准测试填补了家电操作研究中的仿真-现实差距，为评估和推进家电操作规划模型提供了重要工具。
...
### [310] [MILE: A Mechanically Isomorphic Exoskeleton Data Collection System with Fingertip Visuotactile Sensing for Dexterous Manipulation](https://arxiv.org/abs/2512.00324)
*Jinda Du,Jieji Ren,Qiaojun Yu,Ningbin Zhang,Yu Deng,Xingyu Wei,Yufei Liu,Guoying Gu,Xiangyang Zhu*

Main category: cs.RO

TL;DR: MILE系统通过机械同构的遥操作和数据采集系统，解决了灵巧手模仿学习中缺乏大规模高保真数据的问题，显著提升了操作成功率和数据质量。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在灵巧手操作中很有前景，但缺乏大规模高保真数据限制了其效果。现有数据采集方法存在运动重定向不准确、数据采集效率低、缺少高分辨率指尖触觉感知等问题。

Method: 开发了MILE系统，这是一个从人手到外骨骼再到机器手的机械同构遥操作和数据采集系统。外骨骼基于人手解剖学设计，机器手保持一对一关节位置同构，消除了非线性重定向。系统集成了紧凑的指尖视觉触觉模块，提供高分辨率触觉观测。

Result: 外骨骼实现了多关节平均绝对角度误差低于1度。遥操作管道平均成功率提升了64%。加入指尖触觉观测后，相比仅视觉基线平均成功率进一步提升了25%。

Conclusion: MILE系统通过机械同构设计和高质量触觉感知，有效解决了灵巧手模仿学习中的数据瓶颈问题，验证了数据集的高保真度和实用性。
...
### [311] [Balancing Efficiency and Fairness: An Iterative Exchange Framework for Multi-UAV Cooperative Path Planning](https://arxiv.org/abs/2512.00410)
*Hongzong Li,Luwei Liao,Xiangguang Dai,Yuming Feng,Rong Feng,Shiqin Tang*

Main category: cs.RO

TL;DR: 提出了一种用于多无人机协同路径规划的迭代交换框架，通过任务交换和路径优化平衡效率与公平性


<details>
  <summary>Details</summary>
Motivation: 多无人机协同路径规划需要在最小化总任务成本（效率）和平衡各无人机工作量（公平性）之间取得平衡，避免个别无人机负担过重

Method: 提出了迭代交换框架，结合总任务距离和最大完成时间（makespan）的复合目标函数，通过迭代任务交换和路径优化，使用A*算法在考虑地形的配置空间中生成无碰撞轨迹

Result: 在多个地形数据集上的综合实验表明，该方法在总距离和最大完成时间之间的权衡方面优于现有基线方法
...
### [312] [Sample-Efficient Expert Query Control in Active Imitation Learning via Conformal Prediction](https://arxiv.org/abs/2512.00453)
...
<details>
  <summary>Details</summary>
Motivation: 主动模仿学习中专家动作标注成本高昂，特别是在GPU密集型模拟器、人机交互环境和机器人集群中。现有方法存在查询效率低、需要实时专家接管等问题。

Method: 提出CRSAIL方法：1) 使用K近邻距离评估状态新颖性；2) 通过保形预测设置全局阈值；3) 在完整轨迹执行后批量查询专家，无需实时接管；4) 将α参数与期望查询率关联，作为任务无关的调节旋钮。

Result: 在MuJoCo机器人任务上，CRSAIL达到或超过专家级奖励，相比DAgger减少最多96%的专家查询，相比先前AIL方法减少最多65%查询，对α和K参数具有鲁棒性。

Conclusion: CRSAIL提供了一种分布无关的校准规则，能够显著降低主动模仿学习的专家查询成本，同时保持性能，适用于具有未知动态的新系统部署。
...
### [313] [LAP: Fast LAtent Diffusion Planner with Fine-Grained Feature Distillation for Autonomous Driving](https://arxiv.org/abs/2512.00470)
*Jinhao Zhang,Wenlong Xia,Zhexuan Zhou,Youmin Gong,Jie Mei*

Main category: cs.RO

TL;DR: LAP是一种在VAE学习的潜在空间中规划的自动驾驶框架，通过解耦高层意图与底层运动学，实现高质量单步去噪规划，推理速度提升最多10倍


<details>
  <summary>Details</summary>
Motivation: 扩散模型在自动驾驶中能建模类人驾驶行为，但其迭代采样过程导致高延迟，且直接在原始轨迹点上操作使模型花费过多容量在底层运动学而非高层多模态语义上

Method: 提出LAtent Planner框架，在VAE学习的潜在空间中规划，解耦高层意图与底层运动学；引入细粒度特征蒸馏机制，指导高层语义规划空间与矢量化场景上下文之间的交互与融合

Result: 在nuPlan基准测试中，LAP在基于学习的规划方法中达到最先进的闭环性能，推理速度比之前SOTA方法提升最多10倍，且能通过单步去噪产生高质量规划

Conclusion: LAP通过在潜在空间中规划，有效解决了扩散模型在自动驾驶规划中的延迟问题，同时保持了高质量的多模态规划能力，为实时自动驾驶规划提供了高效解决方案
...
### [314] [Fast, Robust, Permutation-and-Sign Invariant SO(3) Pattern Alignment](https://arxiv.org/abs/2512.00659)
*Anik Sarker,Alan T. Asbeck*

Main category: cs.RO

TL;DR: 提出一种无需对应关系的SO(3)旋转集对齐方法，通过将旋转分解为变换基向量，在球面上进行快速鲁棒匹配，并引入排列-符号不变性包装器处理轴重标记和符号翻转问题。


<details>
  <summary>Details</summary>
Motivation: 解决校准和配准中的核心问题：在缺少时间对齐、存在异常值和未知轴约定情况下，对齐两个SO(3)旋转集。传统方法需要对应关系搜索，计算复杂度高。

Method: 将每个旋转分解为三个变换基向量（TBVs），在S^2球面上进行逐轴对齐，使用快速鲁棒匹配器（SPMC、FRS和混合方法）。引入排列-符号不变性（PASI）包装器枚举24种适当符号排列，通过相关性和评分选择最佳排列，并通过投影/Karcher均值将逐轴估计融合为单个旋转。

Result: 在EuRoC Machine Hall模拟（轴一致）和ETH手眼基准测试（轴模糊）上验证，方法准确、比传统方法快6-60倍，在高达90%异常值比例下仍保持鲁棒性，且无需对应关系搜索。

Conclusion: 提出了一种高效、鲁棒的SO(3)旋转集对齐方法，通过变换基向量分解和球面匹配，结合排列-符号不变性处理，实现了线性复杂度且无需对应关系搜索的解决方案。
...
### [315] [SAGAS: Semantic-Aware Graph-Assisted Stitching for Offline Temporal Logic Planning](https://arxiv.org/abs/2512.00775)
*Ruijia Liu,Ancheng Hou,Shaoyuan Li,Xiang Yin*

Main category: cs.RO

TL;DR: SAGAS框架在离线、无模型设置下解决LTL约束控制问题，仅使用固定、任务无关的碎片化轨迹数据集，通过图辅助轨迹缝合和自动机引导规划实现复杂逻辑任务


<details>
  <summary>Details</summary>
Motivation: 现有LTL方法通常依赖精确的动力学模型或昂贵的在线交互，而实际应用中往往只有固定的、任务无关的碎片化轨迹数据集，需要在不依赖模型和在线交互的情况下实现LTL约束控制

Method: 提出SAGAS框架：1) 从学习的时序距离表示构建潜在可达图；2) 使用认证锚节点和概率软标签增强图以弥合语义差距；3) 将规范转换为Büchi自动机并在隐式乘积空间中搜索成本最小化的前缀-后缀计划；4) 部署子目标条件低层策略执行潜在路点

Result: 在OGBench运动域上的实验表明，SAGAS能够成功为多样化的LTL任务合成高效轨迹，有效弥合碎片化离线数据与复杂逻辑约束之间的差距

Conclusion: SAGAS框架在离线、无模型设置下有效解决了LTL约束控制问题，仅使用固定、任务无关的碎片化轨迹数据集就能实现复杂逻辑任务的规划与执行
...
### [316] [Transforming Monolithic Foundation Models into Embodied Multi-Agent Architectures for Human-Robot Collaboration](https://arxiv.org/abs/2512.00797)
*Nan Sun,Bo Mao,Yongchang Li,Chenxu Wang,Di Guo,Huaping Liu*

Main category: cs.RO

TL;DR: InteractGen是一个基于LLM的多智能体框架，将机器人智能分解为专门化智能体，通过多智能体协同而非单一模型扩展来解决服务机器人在现实部署中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在机器人应用中存在局限性：单一模型假设无法处理实际服务流程的分布式动态特性；视觉语言模型缺乏具身感知的行动能力；视觉-语言-动作策略在不同具身系统中脆弱且缺乏几何基础和主动协作机制。这些限制表明仅扩展单一模型无法为人类环境中的服务机器人提供可靠自主性。

Method: 提出InteractGen框架，将机器人智能分解为专门化智能体：持续感知、依赖感知规划、决策与验证、失败反思和动态人类委托。将基础模型作为闭环集体中的受监管组件，通过多智能体协同实现机器人智能。

Result: 在异构机器人团队上部署并进行为期三个月的开放使用研究，InteractGen提高了任务成功率、适应性和人机协作能力，证明多智能体协调比进一步扩展独立模型更可行。

Conclusion: 多智能体协同为社交基础的服务自主性提供了比单独扩展独立模型更可行的路径，基础模型应作为受监管组件集成到闭环集体中。
...
### [317] [A Novel MDP Decomposition Framework for Scalable UAV Mission Planning in Complex and Uncertain Environments](https://arxiv.org/abs/2512.00838)
*Md Muzakkir Quamar,Ali Nasir,Sami ELFerik*

Main category: cs.RO

TL;DR: 提出一个可扩展、容错的无人机任务管理框架，通过两阶段分解策略解决大规模MDP计算瓶颈，将全局MDP分解为更小的子MDP并重新组合，在保持策略最优性的同时显著减少计算时间。


<details>
  <summary>Details</summary>
Motivation: 解决复杂不确定环境下无人机任务管理中大规模马尔可夫决策过程（MDP）的计算瓶颈问题，实现实时策略更新和可扩展的决策制定。

Method: 采用两阶段分解策略：第一阶段使用基于因子的算法将全局MDP划分为更小的、目标特定的子MDP；第二阶段使用基于优先级的重组算法独立解决每个子MDP，并通过元策略进行冲突解决，整合为统一的全局策略。

Result: 理论分析表明在温和的概率独立性假设下，组合策略可证明等价于最优全局MDP策略。大量仿真验证了方法的有效性，计算时间呈数量级减少，同时不牺牲任务可靠性或策略最优性。

Conclusion: 该框架通过分解大型MDP为可处理的子问题，推进了人工智能决策的可扩展性，为实时无人机任务执行中的可扩展决策制定建立了实用且稳健的基础。
...
### [318] [Constant-Time Motion Planning with Manipulation Behaviors](https://arxiv.org/abs/2512.00939)
*Nayesha Gandotra,Itamar Mishani,Maxim Likhachev*

Main category: cs.RO

TL;DR: B-CTMP算法将恒定时间运动规划扩展到机器人操作任务，通过预处理实现毫秒级查询，保证操作行为的完整性和成功率


<details>
  <summary>Details</summary>
Motivation: 现有恒定时间运动规划方法缺乏对操作行为的显式支持，限制了在复杂操作任务中的应用，需要将碰撞自由规划与对象操作统一起来

Method: 提出行为恒定时间运动规划器，通过预计算紧凑数据结构，实现两阶段操作任务：先规划到行为起始状态，再执行抓取或插入等操作行为

Result: 在模拟和真实机器人上验证了货架拾取和插头插入任务，B-CTMP在毫秒级时间内提供可验证的速度和成功率保证

Conclusion: B-CTMP统一了碰撞自由规划和对象操作，为半结构化环境中的操作任务提供了可证明的速度和成功保证
...
### [319] [H-Zero: Cross-Humanoid Locomotion Pretraining Enables Few-shot Novel Embodiment Transfer](https://arxiv.org/abs/2512.00971)
...
<details>
  <summary>Details</summary>
Motivation: 人形机器人快速发展需要鲁棒且适应性强的控制器，但现有解决方案针对特定机器人设计，需要为每个机器人进行大量奖励函数、物理参数和训练超参数调优

Method: 引入H-Zero跨人形机器人运动预训练流程，在有限机器人样本上学习通用人形基础策略，实现零样本和少样本迁移

Result: 预训练策略在未见过的机器人上保持高达81%的完整周期持续时间，在30分钟内微调即可实现少样本迁移到未见人形机器人和直立四足机器人

Conclusion: H-Zero通过跨机器人预训练有效解决了人形机器人控制器通用性问题，显著减少了新机器人适配所需的时间和调优工作
...
### [320] [FOM-Nav: Frontier-Object Maps for Object Goal Navigation](https://arxiv.org/abs/2512.01009)
*Thomas Chabal,Shizhe Chen,Jean Ponce,Cordelia Schmid*

Main category: cs.RO

TL;DR: FOM-Nav是一个用于物体目标导航的模块化框架，通过前沿-物体地图和视觉语言模型提升探索效率，在MP3D和HM3D基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式记忆的方法在长期记忆保持和规划方面存在困难，而基于显式地图的方法缺乏丰富的语义信息，需要一种能同时解决这些挑战的新方法。

Method: 提出FOM-Nav框架，在线构建前沿-物体地图，同时编码空间前沿和细粒度物体信息；使用视觉语言模型进行多模态场景理解和高级目标预测；通过低级规划器执行高效轨迹生成。

Result: 在MP3D和HM3D基准测试中达到最先进性能，特别是在导航效率指标SPL上表现优异，并在真实机器人上获得有希望的结果。

Conclusion: FOM-Nav通过结合前沿-物体地图和视觉语言模型，有效解决了物体目标导航中的长期记忆和语义信息不足问题，显著提升了导航效率。
...
### [321] [Integration of UWB Radar on Mobile Robots for Continuous Obstacle and Environment Mapping](https://arxiv.org/abs/2512.01018)
*Adelina Giurea,Stijn Luchie,Dieter Coppens,Jeroen Hoebeke,Eli De Poorter*

Main category: cs.RO

TL;DR: 该论文提出了一种基于UWB雷达的移动机器人障碍物检测和环境建图方法，无需基础设施支持，能在视觉受限环境中工作。


<details>
  <summary>Details</summary>
Motivation: 传统视觉摄像头和激光雷达在黑暗、烟雾或反射表面等能见度差的环境中失效，需要一种不依赖视觉特征且无需固定锚点的感知方案。

Method: 使用机器人搭载的UWB雷达，研究不同材料（金属、混凝土、胶合板）和UWB信道（5和9）对信道脉冲响应的影响，提出包含目标识别、滤波和聚类三个步骤的处理流程。

Result: 该方法有效降低了噪声和多径效应，在信道9上即使检测低反射材料如胶合板，也能达到至少82.36%的精确率和89.46%的召回率。

Conclusion: 这项工作为开发不依赖视觉特征且无需固定锚点三角定位的UWB-based SLAM系统奠定了基础。
...
### [322] [CycleManip: Enabling Cyclic Task Manipulation via Effective Historical Perception and Understanding](https://arxiv.org/abs/2512.01022)
*Yi-Lin Wei,Haoran Liao,Yuhao Lin,Pengyue Wang,Zhizhao Liang,Guiliang Liu,Wei-Shi Zheng*

Main category: cs.RO

TL;DR: 本文提出CycleManip框架解决机器人循环操作任务，通过成本感知采样和多任务学习增强历史感知，并建立首个循环操作基准测试


<details>
  <summary>Details</summary>
Motivation: 机器人循环操作任务（如摇晃瓶子、敲钉子）在日常生活中有重要应用，但现有研究较少，面临两个主要挑战：1）模仿方法因历史信息利用不足而无法在预期时间内完成任务；2）缺乏足够数据和自动评估工具的基准测试阻碍了该领域发展

Method: 提出CycleManip框架，采用端到端模仿学习方式，无需额外模型、分层结构或显著计算开销。核心创新包括：成本感知采样策略增强历史感知，多任务学习提升历史理解能力

Result: 在仿真和真实环境中进行广泛实验，证明该方法在循环操作任务中达到高成功率，在通用操作中表现出强适应性，并能在VLA等模仿策略上即插即用。方法可应用于多种机器人平台，包括双臂夹爪、灵巧手和人形机器人

Conclusion: CycleManip框架有效解决了机器人循环操作任务，填补了该领域的研究空白，提出的基准测试为后续研究提供了重要工具，方法具有良好的通用性和可扩展性
...
### [323] [VLASH: Real-Time VLAs via Future-State-Aware Asynchronous Inference](https://arxiv.org/abs/2512.01031)
*Jiaming Tang,Yufei Sun,Yilong Zhao,Shang Yang,Yujun Lin,Zhuoyang Zhang,James Hou,Yao Lu,Zhijian Liu,Song Han*

Main category: cs.RO

TL;DR: VLASH是一个用于视觉-语言-动作模型的异步推理框架，通过预测未来执行状态来解决异步推理中的时间错位问题，实现平滑、准确、低延迟的机器人控制。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在实际部署中存在延迟和效率问题：演示视频通常被加速5-10倍才能显得流畅，存在明显的动作停滞和对环境变化的延迟反应。异步推理虽然能实现连续低延迟控制，但会导致预测和执行间隔的时间错位，引发动作不稳定。

Method: VLASH通过使用先前生成的动作块将机器人状态向前滚动来估计未来的执行时间状态，从而弥合预测和执行之间的差距。该方法不需要额外的开销或架构改变。

Result: 实验显示VLASH相比同步推理实现了高达2.03倍的加速，反应延迟降低了17.4倍，同时完全保持了原始准确性。它使VLA模型能够处理快速反应、高精度任务，如打乒乓球和打地鼠游戏。

Conclusion: VLASH是一个通用的异步推理框架，能够为视觉-语言-动作模型提供平滑、准确、快速的反应控制，解决了传统同步推理在实时机器人任务中的局限性。
...
### [324] [Autonomous Grasping On Quadruped Robot With Task Level Interaction](https://arxiv.org/abs/2512.01052)
*Muhtadin,Mochammad Hilmi Rusydiansyah,Mauridhi Hery Purnomo,I Ketut Eddy Purnama,Chastine Fatichah*

Main category: cs.RO

TL;DR: 本研究为四足机器人开发了基于任务级交互的自主抓取系统，通过集成机械臂和夹爪，结合ROS分层控制系统和基于Web的人机交互界面，实现了导航、目标检测和抓取功能，在真实场景测试中达到75%的抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 当前四足机器人主要专注于移动能力，缺乏物体操作功能。为四足机器人配备机械臂和夹爪后，在需要复杂指令的远程场景中手动控制面临挑战，因此需要开发自主抓取系统来增强四足机器人作为服务机器人的实际应用能力。

Method: 1) 硬件集成：将机械臂和夹爪安装到四足机器人本体上；2) 控制系统：基于ROS设计分层控制系统；3) 人机交互：开发基于Web的交互界面；4) 自主功能：使用GraspNet实现导航、目标检测和抓取；5) 测试评估：通过真实场景测试导航、目标选择和抓取性能以及用户体验。

Result: 系统能够准确一致地执行任务，在12次试验中实现了75%的抓取成功率。机器人能够自主完成导航、目标检测和抓取任务，证明了该系统在增强四足机器人作为服务机器人在真实环境中的应用潜力。

Conclusion: 该自主抓取系统显著提升了四足机器人的能力，使其能够在真实环境中作为服务机器人执行复杂任务，展示了任务级交互方法在四足机器人操作应用中的有效性和实用性。
...
### [325] [Opening the Sim-to-Real Door for Humanoid Pixel-to-Action Policy Transfer](https://arxiv.org/abs/2512.01061)
...
TL;DR: 提出基于GPU加速的逼真模拟的师生引导学习框架，用于视觉驱动的人形机器人移动操作，在铰接物体交互任务上实现零样本泛化


<details>
  <summary>Details</summary>
Motivation: GPU加速的逼真模拟为机器人学习提供了可扩展的数据生成路径，但如何利用这种模拟能力实现复杂的人形机器人移动操作（特别是铰接物体交互）仍面临挑战

Method: 1. 师生引导学习框架；2. 阶段重置探索策略稳定长时程特权策略训练；3. GRPO微调程序缓解部分可观测性并提高模拟到真实RL的闭环一致性

Result: 完全在模拟数据上训练的策略实现了对不同门类型的鲁棒零样本性能，在相同全身控制栈下比人类遥操作快31.7%，是首个使用纯RGB感知实现多样化铰接移动操作的人形机器人模拟到真实策略

Conclusion: 该方法成功展示了利用GPU加速模拟进行复杂人形机器人移动操作学习的可行性，为视觉驱动的铰接物体交互提供了有效的模拟到真实解决方案
...
### [326] [Reinforcement Learning for Gliding Projectile Guidance and Control](https://arxiv.org/abs/2512.01066)
*Joel Cahn,Antonin Thomas,Philippe Pastor*

Main category: cs.RO

TL;DR: 本文开发了一种基于强化学习的控制律，用于光学制导滑翔机，旨在实现动态环境中更灵活自主的导航，精确跟踪摄像头检测到的目标。


<details>
  <summary>Details</summary>
Motivation: 虽然强化学习已成功应用于四旋翼无人机控制，但作者希望验证该方法在固定翼飞机所有轴向上的适用性，为固定翼飞行器提供更灵活自主的导航能力。

Method: 采用强化学习方法开发控制律，该控制律设计用于光学制导滑翔机，通过摄像头检测目标并引导滑翔机精确跟踪目标点。

Result: 论文展示了强化学习控制律在光学制导滑翔机上的开发，但摘要中未提供具体的实验结果数据。

Conclusion: 研究旨在证明强化学习方法不仅适用于四旋翼无人机，也能有效应用于固定翼飞机的全轴向控制，为动态环境下的自主导航提供新方案。
...
### [327] [Supporting Productivity Skill Development in College Students through Social Robot Coaching: A Proof-of-Concept](https://arxiv.org/abs/2512.01105)
*Himanshi Lalwani,Hanan Salam*

Main category: cs.RO

TL;DR: 本研究提出了一种社交辅助机器人作为大学生生产力教练的概念验证，通过聊天界面提供时间管理和任务优先级课程，集成了进度监控和个性化反馈功能。


<details>
  <summary>Details</summary>
Motivation: 大学生面临生产力挑战，现有工具如自助书籍和生产力应用存在局限：书籍缺乏互动性，应用缺乏教育性且可能阻碍组织技能发展。传统生产力教练虽然个性化但资源密集难以扩展。

Method: 开发社交辅助机器人作为教育教练，提供六节时间管理和任务优先级课程。用户通过聊天界面交互，机器人通过语音响应。集成仪表板监控进度、情绪、参与度、每课信心和用时，并提供个性化生产力洞察。

Result: 对15名大学生进行评估，获得系统可用性评分79.2分，整体体验和参与度评分较高。结果表明SAR生产力教练是有效的可扩展解决方案。

Conclusion: 社交辅助机器人作为生产力教练能够提供有效且可扩展的解决方案，改善大学生生产力，弥补现有工具的不足。
...
### [328] [Tactile Robotics: Past and Future](https://arxiv.org/abs/2512.01106)
*Nathan F. Lepora*

Main category: cs.RO

TL;DR: 该论文通过分析近50年来近150篇综述，梳理了触觉机器人技术的历史发展脉络，将其划分为四个代际，并预测了未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 为了定义触觉机器人的未来发展方向，作者希望通过历史视角分析该领域的发展轨迹，总结专家见解，识别反复出现的挑战，理解进展停滞的原因，并确定最有可能定义未来的机会。

Method: 基于近50年（约1965-2024年）内近150篇触觉机器人领域的综述文献，采用历史分析方法，将发展历程划分为四个代际：起源期（1965-79）、基础与成长期（1980-94）、触觉寒冬期（1995-2009）以及扩展与多样化期（2010-2024）。通过分析专家意见和知识积累，识别出反复出现的挑战和发展模式。

Result: 研究发现触觉机器人发展呈现明显的代际特征，近期扩展导致了多个主题的涌现：电子皮肤、触觉机械手、基于视觉的触觉传感、软体/仿生触觉以及触觉互联网。这些主题代表了当前研究的主要方向。

Conclusion: 从2025年开始的下一个代际，触觉机器人有望成熟并实现广泛的商业应用，在类人灵巧操作、理解人类智能和远程临场感等领域产生重要影响。通过将过去的专家见解与当前主题联系起来，该研究揭示了触觉机器人领域的演变规律、进展停滞的原因以及最有可能定义未来的机遇。
...
### [329] [Think Fast: Real-Time Kinodynamic Belief-Space Planning for Projectile Interception](https://arxiv.org/abs/2512.01108)
*Gabriel Olin,Lu Chen,Nayesha Gandotra,Maxim Likhachev,Howie Choset*

Main category: cs.RO

TL;DR: 提出一种用于机器人手臂拦截快速移动目标的实时规划框架，使用状态-时间空间的运动基元构建树状结构，支持多目标可达性和实时值更新


<details>
  <summary>Details</summary>
Motivation: 拦截快速移动物体具有严格的时间约束，在传感器噪声存在时更为复杂，因为噪声传感器只能提供不完整信息，导致目标状态存在概率分布。由于时间紧迫，规划器必须在接收信息的同时就开始引导拦截器（机器人手臂）

Method: 引入树状结构，在状态-时间空间中使用运动动力学基元生长该结构。该结构编码从单一原点到达多个目标的可达性，同时支持目标信念演化时的实时值更新和不同目标间的无缝转换。使用RIAE-AKF滤波器进行目标跟踪和信念更新

Result: 在6自由度工业机械臂（ABB IRB-1600）上评估了该框架，机械臂搭载立体相机（ZED 2i）用于拦截任务

Conclusion: 提出的树状结构框架能够在传感器噪声存在的情况下，实时处理快速移动目标的拦截任务，支持多目标可达性和信念更新
...
### [330] [Ethically-Aware Participatory Design of a Productivity Social Robot for College Students](https://arxiv.org/abs/2512.01111)
*Himanshi Lalwani,Hanan Salam*

Main category: cs.RO

TL;DR: 研究采用参与式设计方法，探索社交辅助机器人如何帮助大学生（特别是ADHD学生）提高生产力，整合伦理考量并制定设计指南


<details>
  <summary>Details</summary>
Motivation: 大学生面临学业和生活压力影响生产力，ADHD学生存在执行功能挑战。传统生产力工具需要持续自律，导致学生难以坚持并出现应用切换行为。社交辅助机器人具有直观互动特性，在教育、认知发展和心理健康领域已成功应用，有望支持学术环境中的生产力提升

Method: 采用参与式设计方法，直接让大学生和学业成功与健康教练参与设计过程。通过访谈和协作工作坊，收集生产力挑战的详细见解，识别生产力导向社交辅助机器人的理想功能特征。从设计初期就整合伦理考量

Result: 获得了对学生生产力挑战的全面见解、社交辅助机器人设计偏好，以及有效机器人特性的可操作建议。同时提出了利益相关者衍生的伦理指南，为高等教育中生产力导向社交辅助机器人的负责任未来实施提供参考

Conclusion: 社交辅助机器人在支持大学生生产力方面具有潜力，参与式设计方法能够确保设计符合用户需求。伦理考量应从设计初期整合，研究结果为高等教育环境中负责任地实施生产力导向社交辅助机器人提供了实用指南
...
### [331] [Real-World Reinforcement Learning of Active Perception Behaviors](https://arxiv.org/abs/2512.01188)
*Edward S. Hu,Jie Wang,Xingfang Yuan,Fiona Luo,Muyao Li,Gaspard Lambrechts,Oleh Rybkin,Dinesh Jayaraman*

Main category: cs.RO

TL;DR: AAWR是一种利用特权传感器训练主动感知策略的机器人学习方法，通过特权价值函数估计优势，从少量演示和粗略策略初始化开始，快速学习信息收集行为，在部分可观测环境中提升任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前标准机器人学习技术在部分可观测环境下难以产生主动感知行为，而机器人的瞬时感官观察并不总能揭示任务相关状态信息，需要显式地获取缺失信息。

Method: 提出非对称优势加权回归（AAWR），利用训练时可访问的"特权"额外传感器，训练高质量的特权价值函数来估计目标策略的优势，从少量可能次优的演示和易于获得的粗略策略初始化开始。

Result: 在3个机器人上的8个操作任务评估中，AAWR合成了可靠的主动感知行为，优于所有先前方法。当使用在主动感知任务上表现不佳的"通用"机器人策略初始化时，AAWR能有效生成信息收集行为，使其在严重部分可观测的操作任务中运行。

Conclusion: AAWR提供了一种简单有效的真实世界机器人学习方案，能够高效训练主动感知策略，解决部分可观测环境下的信息获取问题，显著提升任务性能。
...
### [332] [RoboLoc: A Benchmark Dataset for Point Place Recognition and Localization in Indoor-Outdoor Integrated Environments](https://arxiv.org/abs/2512.01194)
*Jaejin Jeon,Seonghoon Ryoo,Sang-Duck Lee,Soomok Lee,Seungwoo Jeong*

Main category: cs.RO

TL;DR: RoboLoc是一个用于室内外环境GPS-free地点识别的基准数据集，包含真实机器人轨迹、多样高程剖面和结构化室内到非结构化室外的领域转换。
...
Method: 提出RoboLoc基准数据集，包含真实机器人轨迹、多样高程剖面和室内外领域转换，并对多种最先进模型（点云、体素、BEV架构）进行基准测试。

Result: 数据集为多领域定位系统开发提供了真实测试平台，展示了不同架构模型在领域转换中的泛化能力。

Conclusion: RoboLoc填补了现有数据集在室内外无缝转换方面的空白，为机器人自主导航中的多领域定位系统开发提供了重要基准。
...
### [333] [COMET: A Dual Swashplate Autonomous Coaxial Bi-copter AAV with High-Maneuverability and Long-Endurance](https://arxiv.org/abs/2512.01246)
*Shuai Wang,Xiaoming Tang,Junning Liang,Haowen Zheng,Biyu Ye,Zhaofeng Liu,Fei Gao,Ximin Lyu*

Main category: cs.RO

TL;DR: COMET是一个采用双倾斜盘机构的同轴双旋翼自主飞行器平台，通过优化设计在效率、机动性和紧凑性之间取得平衡，实验验证了其在各种载荷条件下的性能和实用性。


<details>
  <summary>Details</summary>
Motivation: 同轴双旋翼自主飞行器因其转子系统效率提升和紧凑外形而受到关注，但在效率、机动性和紧凑性之间的平衡仍然是一个关键设计挑战，限制了其实际部署。

Method: 引入COMET平台，采用双倾斜盘机构，通过台架测试优化同轴双旋翼系统的效率和紧凑性，通过飞行续航实验验证原型机在不同载荷条件下的效率和鲁棒性，并通过轨迹跟踪测试评估机动性性能。

Result: 结果表明，双倾斜盘配置相比单倾斜盘方案提升了跟踪性能并改善了飞行效率，在各种场景下的自主飞行试验验证了COMET在实际应用中的潜力。

Conclusion: COMET平台通过双倾斜盘设计成功解决了同轴双旋翼系统在效率、机动性和紧凑性之间的平衡问题，展示了其在现实世界应用中的可行性。
...
### [334] [Visibility-aware Cooperative Aerial Tracking with Decentralized LiDAR-based Swarms](https://arxiv.org/abs/2512.01280)
*Longji Yin,Yunfan Ren,Fangcheng Zhu,Liuyu Shi,Fanze Kong,Benxu Tang,Wenyi Liu,Ximin Lyu,Fu Zhang*

Main category: cs.RO

TL;DR: 提出了一种基于LiDAR的分散式无人机群目标跟踪框架，通过球形符号距离场（SSDF）表示环境遮挡，结合视场对齐和群体协调成本，实现复杂环境中的可见性感知协同跟踪。


<details>
  <summary>Details</summary>
Motivation: 虽然单无人机跟踪系统已被广泛研究，但基于群体的目标跟踪仍未被充分探索，尽管它具有分布式感知、容错冗余和多方向目标覆盖的独特优势。现有方法未能充分利用群体系统的独特能力。

Method: 提出分散式LiDAR群体跟踪框架，包括：1）基于球形符号距离场（SSDF）的3D环境遮挡表示方法；2）支持异构LiDAR配置的视场对齐成本；3）基于静电势启发的分布度量的群体协调机制；4）结合运动学前端搜索器和时空SE(3)后端优化的分层规划器。

Result: 在杂乱室外环境中进行的真实世界实验验证了该系统能够稳健地协同跟踪敏捷目标（无人机、人类），同时保持优越的可见性维护。系统实现了完全分散的协作感知、分布式规划和动态群体可重构性。

Conclusion: 该研究填补了群体目标跟踪领域的空白，提出的框架能够充分利用群体系统的独特优势，在复杂环境中实现可见性感知的协同跟踪，为监视、电影摄影和工业检测等应用提供了有效的解决方案。
...
### [335] [Discovering Self-Protective Falling Policy for Humanoid Robot via Deep Reinforcement Learning](https://arxiv.org/abs/2512.01336)
*Diyuan Shi,Shangke Lyu,Donglin Wang*

Main category: cs.RO

TL;DR: 该论文提出了一种基于深度强化学习和课程学习的方法，训练人形机器人学习跌倒保护策略，通过形成"三角形"结构来显著减少跌倒损伤，并成功转移到真实平台。


<details>
  <summary>Details</summary>
Motivation: 人形机器人由于其形态、动力学特性和控制策略限制，相比四足或轮式机器人更容易跌倒。其重量大、质心高、自由度多的特点会导致严重的硬件损坏，对自身和周围物体造成危害。现有研究主要基于控制方法，难以适应多样化的跌倒场景，且可能引入不适当的人类先验知识。

Method: 采用大规模深度强化学习和课程学习方法，通过精心设计的奖励函数和领域多样化课程，训练人形机器人探索跌倒保护行为。研究发现通过形成"三角形"结构，可以利用其刚性材料身体显著减少跌倒损伤。

Result: 通过综合指标和实验，量化了该方法的性能并与其他方法进行比较，可视化了其跌倒行为，并成功将训练的策略转移到真实世界平台。

Conclusion: 深度强化学习和课程学习能够有效训练人形机器人发现适合其自身特性和性质的跌倒保护策略，通过形成三角形结构显著减少跌倒损伤，并具有实际应用价值。
...
### [336] [Modality-Augmented Fine-Tuning of Foundation Robot Policies for Cross-Embodiment Manipulation on GR1 and G1](https://arxiv.org/abs/2512.01358)
*Junsung Park,Hogun Kee,Songhwai Oh*

Main category: cs.RO

TL;DR: 提出模态增强微调框架，将基础机器人策略适配到不同人形机器人形态，通过模态增强提升策略性能


<details>
  <summary>Details</summary>
Motivation: 基础机器人策略需要适配不同的人形机器人形态，但现有方法在跨形态迁移时性能有限，需要更有效的适配方法

Method: 提出模态增强微调框架，在GR1上使用后处理模态（接触信号和深度信息），在G1上构建多模态数据集（运动规划、逆运动学和接触力测量）

Result: 模态增强显著提升策略性能：GR1在线成功率从51%提升到63%；G1"苹果到碗"任务成功率从48%提升到94%，零样本迁移基线为0%

Conclusion: 轻量级后处理对GR1有效，高质量多模态数据对G1迁移至关重要，建立了统一的数据中心化路径来扩展基础机器人策略
...
### [337] [$\mathbf{M^3A}$ Policy: Mutable Material Manipulation Augmentation Policy through Photometric Re-rendering](https://arxiv.org/abs/2512.01446)
*Jiayi Li,Yuxuan Hu,Haoran Geng,Xiangyu Chen,Chuhao Zhou,Ziteng Cui,Jianfei Yang*

Main category: cs.RO

TL;DR: M³A框架利用光传输的物理特性进行光度重渲染，从单个真实世界演示生成具有不同材料属性的多样化演示，实现跨材料泛化而无需额外数据收集。


<details>
  <summary>Details</summary>
Motivation: 真实世界机器人操作需要处理具有不同视觉和物理特性的材料，特别是玻璃、金属等透明或反射表面材料会引入严重的分布外变化。现有方法要么依赖模拟器进行仿真到真实迁移（受视觉域差距限制），要么依赖收集大量真实世界演示（成本高且覆盖材料有限）。

Method: 提出Mutable Material Manipulation Augmentation (M³A)框架，利用计算摄影学和光传输的物理特性进行光度重渲染。核心思想：给定单个真实世界演示，通过光度重渲染场景生成具有不同材料属性的多样化、高度真实的演示，从而将任务特定操作技能与表面外观解耦。

Result: 构建了首个全面的多材料操作基准测试，涵盖仿真和真实环境。实验表明M³A策略显著增强跨材料泛化能力，在三个真实世界任务中平均成功率提高58.03%，并在未见材料上表现出鲁棒性能。

Conclusion: M³A框架通过光度重渲染有效解决了材料泛化问题，无需额外数据收集即可实现跨材料泛化，为真实世界机器人操作提供了实用解决方案。
...
### [338] [NavForesee: A Unified Vision-Language World Model for Hierarchical Planning and Dual-Horizon Navigation Prediction](https://arxiv.org/abs/2512.01550)
*Fei Liu,Shichao Xie,Minghua Luo,Zedong Chu,Junjun Hu,Xiaolong Wu,Mu Xu*

Main category: cs.RO

TL;DR: NavForesee是一个统一的视觉语言模型，将高层语言规划和预测性世界模型想象结合在一个框架中，用于解决长时程自然语言导航任务。


<details>
  <summary>Details</summary>
Motivation: 现有的导航智能体在长时程规划和未见环境中的鲁棒规划方面存在困难，导致高失败率，需要新的方法来提升复杂自然语言指令下的导航能力。

Method: NavForesee是一个统一的视觉语言模型，能够同时执行规划和预测性前瞻。模型基于完整指令和历史观察，通过任务分解、进度跟踪和子目标制定来理解导航指令，同时作为生成式世界模型预测短期环境动态和长期导航里程碑。

Result: 在R2R-CE和RxR-CE基准测试中，NavForesee在复杂场景中实现了高度竞争力的性能。

Conclusion: 将显式语言规划与隐式时空预测融合具有巨大潜力，为更智能和更有能力的具身智能体铺平了道路。
...
### [339] [L2M-Calib: One-key Calibration Method for LiDAR and Multiple Magnetic Sensors](https://arxiv.org/abs/2512.01554)
*Qiyang Lyu,Wei Wang,Zhenyu Wu,Hongming Shen,Huiqin Zhou,Danwei Wang*

Main category: cs.RO

TL;DR: 提出L2M-Calib单键校准框架，用于磁传感器与LiDAR融合系统的联合内外参校准，通过优化算法实现高精度校准


<details>
  <summary>Details</summary>
Motivation: 多模态传感器融合需要精确校准，但磁传感器与LiDAR融合系统缺乏有效的校准技术，限制了磁传感器在挑战性环境中的应用潜力

Method: 提出L2M-Calib框架，采用迭代高斯-牛顿法优化外参，结合加权岭正则化总体最小二乘法进行内参校准，解决测量噪声和病态数据问题

Result: 在仿真数据集和真实世界实验（包括AGV安装配置）中验证，方法在各种环境和操作条件下实现高校准精度和鲁棒性

Conclusion: L2M-Calib为磁-LiDAR融合系统提供有效的校准解决方案，解决了磁传感器在多模态系统中的校准难题，为挑战性环境下的感知应用奠定基础
...
### [340] [A Cross-Embodiment Gripper Benchmark for Rigid-Object Manipulation in Aerial and Industrial Robotics](https://arxiv.org/abs/2512.01598)
*Marek Vagas,Martin Varga,Jaroslav Romancik,Ondrej Majercak,Alejandro Suarez,Anibal Ollero,Bram Vanderborght,Ivan Virgala*

Main category: cs.RO

TL;DR: 该论文提出了跨平台夹爪基准测试（CEGB），扩展了现有的YCB和NIST基准，增加了跨平台转移性、能耗和特定意图有效载荷评估三个新指标，为移动和空中操作提供更全面的夹爪评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有YCB和NIST基准主要针对单一平台评估抓取成功率、力或时间，但缺乏对跨平台转移性和能耗感知性能的评估，而这些能力对于现代移动和空中操作至关重要。

Method: 开发了跨平台夹爪基准测试（CEGB），在YCB和部分NIST指标基础上增加了三个组件：1）转移时间基准测量更换平台的实际工作量；2）能耗基准评估抓取和保持效率；3）特定意图理想有效载荷评估反映设计相关的操作能力。

Result: 实验显示：原型夹爪的跨平台转移时间中位数约17.6秒，保持能耗约1.5焦耳/10秒，抓取性能稳定（周期时间3.2-3.9秒，成功率超过90%）。

Conclusion: CEGB为夹爪在航空和操作器领域的跨平台、能耗感知评估提供了可重复的基础，能够全面评估单个夹爪在不同机器人系统中重用的适用性。
...
### [341] [Integrated YOLOP Perception and Lyapunov-based Control for Autonomous Mobile Robot Navigation on Track](https://arxiv.org/abs/2512.01608)
*Mo Chen*

Main category: cs.RO

TL;DR: 提出了一种用于非完整差速驱动移动机器人的实时自主轨迹导航框架，通过联合集成多任务视觉感知和可证明稳定的跟踪控制器来实现。


<details>
  <summary>Details</summary>
Motivation: 解决移动机器人在动态和部分感知的车道场景中实现可靠自主导航的问题，无需依赖高清先验地图或全球卫星定位。

Method: 感知管道通过2D到3D相机投影重建车道中心线，使用基于弧长的均匀点重采样和通过鲁棒QR最小二乘优化的三次多项式拟合；控制器通过基于李雅普诺夫稳定性的设计调节机器人线性和角速度。

Result: 嵌入式平台上的真实世界实验验证了系统保真度、实时执行、轨迹平滑性和闭环稳定性，确保位置和航向偏差的渐近收敛。

Conclusion: 该框架实现了可靠的自主导航，即使在动态和部分感知的车道场景中也能保证有界误差动态和渐近收敛，无需依赖外部地图或定位系统。
...
### [342] [Dynamic Log-Gaussian Process Control Barrier Function for Safe Robotic Navigation in Dynamic Environments](https://arxiv.org/abs/2512.01668)
*Xin Yin,Chenyang Liang,Yanning Guo,Jie Mei*

Main category: cs.RO

TL;DR: 提出基于高斯过程的动态对数高斯过程控制屏障函数（DLGP-CBF），用于在未知动态环境中实时构建空间信息丰富且对障碍物运动敏感的CBF，提升机器人安全导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有控制屏障函数（CBFs）在利用实时传感器数据在线合成信息丰富且对障碍物运动敏感的CBF方面存在挑战，特别是在未知和动态场景中。

Method: 提出DLGP-CBF方法：1）利用高斯过程回归的对数变换生成平滑且信息丰富的屏障值和梯度；2）将DLGP-CBF明确建模为障碍物位置的函数，整合预测的障碍物速度，使控制器能主动响应动态障碍物运动。

Result: 仿真结果显示在避障性能方面有显著改进，包括增加安全裕度、更平滑的轨迹以及相比基线方法更强的响应能力。

Conclusion: DLGP-CBF能够有效解决在未知动态环境中实时构建信息丰富且对障碍物运动敏感的CBF的挑战，显著提升机器人安全导航性能。
...
### [343] [DiG-Flow: Discrepancy-Guided Flow Matching for Robust VLA Models](https://arxiv.org/abs/2512.01715)
*Wanpeng Zhang,Ye Wang,Hao Luo,Haoqi Yuan,Yicheng Feng,Sipeng Zheng,Qin Jin,Zongqing Lu*

Main category: cs.RO

TL;DR: DiG-Flow通过几何正则化增强VLA模型的鲁棒性，利用观测和动作嵌入之间的分布差异作为几何信号来调制表示学习，在复杂多步任务和有限数据下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于流匹配训练的VLA模型在分布偏移和复杂多步任务上性能下降，表明学习到的表示可能无法鲁棒地捕捉任务相关的语义信息。

Method: DiG-Flow框架计算观测和动作嵌入的经验分布之间的差异度量，通过单调函数将其映射为调制权重，在流匹配前对观测嵌入应用残差更新，这种干预在表示层面进行而不修改流匹配路径或目标向量场。

Result: 理论保证显示差异引导的训练可证明地降低训练目标，引导的推断细化具有收敛性和收缩性。实验表明DiG-Flow以可忽略的开销集成到现有VLA架构中，在复杂多步任务和有限训练数据下性能提升尤为显著。

Conclusion: DiG-Flow通过几何正则化有效增强了VLA模型的鲁棒性，为处理分布偏移和复杂任务提供了有原则的框架，在表示层面进行干预而不改变流匹配的基本机制。
...
### [344] [AgriLiRa4D: A Multi-Sensor UAV Dataset for Robust SLAM in Challenging Agricultural Fields](https://arxiv.org/abs/2512.01753)
*Zhihao Zhan,Yuhang Ming,Shaobin Li,Jie Yuan*

Main category: cs.RO

TL;DR: AgriLiRa4D是一个面向农业无人机应用的多模态数据集，包含三种农田类型和两种操作模式，提供高精度地面真实轨迹和多种传感器同步数据，用于SLAM算法研究和鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏面向农业无人机应用的真实世界多模态数据集，限制了在具有挑战性的农业环境中进行鲁棒操作的研究。农业环境中的低纹理作物、重复模式、动态植被等问题对SLAM算法提出了特殊挑战。

Method: 构建了包含三种代表性农田类型（平坦、丘陵、梯田）和两种操作模式（边界和覆盖）的六个飞行序列组。使用FINS_RTK系统提供高精度地面真实轨迹，同步采集3D LiDAR、4D雷达和IMU数据，并提供完整的传感器内外参标定。

Result: 创建了AgriLiRa4D数据集，包含丰富的传感器数据和多样化的真实农业场景。通过对四种先进的多传感器SLAM算法进行基准测试，展示了该数据集的实用性，并证明了多模态方法对于可靠无人机定位的必要性。
...
### [345] [IGen: Scalable Data Generation for Robot Learning from Open-World Images](https://arxiv.org/abs/2512.01773)
...
<details>
  <summary>Details</summary>
Motivation: 通用机器人策略需要大规模训练数据，但机器人数据收集成本高且环境受限。开放世界图像包含丰富多样的真实场景，但缺乏对应的机器人动作，限制了其在机器人学习中的应用。

Method: IGen首先将非结构化2D像素转换为结构化3D场景表示，然后利用视觉语言模型将场景特定任务指令转换为高层计划，生成SE(3)末端执行器位姿序列作为低层动作，最后合成动态场景演化并渲染时间一致的视觉观测。

Result: 实验验证了IGen生成的视觉运动数据质量高，仅使用IGen合成数据训练的机器人策略性能与使用真实世界数据训练的相当。
...
### [346] [GR-RL: Going Dexterous and Precise for Long-Horizon Robotic Manipulation](https://arxiv.org/abs/2512.01801)
...
TL;DR: GR-RL是一个机器人学习框架，通过多阶段训练将通用视觉-语言-动作策略转化为专门处理长时程灵巧操作任务的专家策略，成功实现自主系鞋带任务（83.3%成功率）。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略假设人类演示是最优的，但在高度灵巧和精确的操作任务中，人类演示存在噪声且非最优。需要一种方法将通用策略转化为能够处理长时程推理、毫米级精度和柔性体交互的专家策略。

Method: 提出三阶段训练流程：1) 学习视觉-语言条件任务进度函数，过滤演示轨迹，保留对进度有积极贡献的转移；2) 引入形态对称增强，提升泛化能力和性能；3) 通过在线RL学习潜在空间噪声预测器，使VLA策略与其部署行为更好对齐。

Result: GR-RL是首个能够自主系鞋带的学习型策略，成功率达到83.3%。该任务需要长时程推理、毫米级精度和柔性体交互能力。

Conclusion: GR-RL为通用机器人基础模型向可靠现实世界专家转化提供了重要步骤，展示了通过强化学习增强人类演示的有效性，为复杂灵巧操作任务提供了解决方案。
...
### [347] [Much Ado About Noising: Dispelling the Myths of Generative Robotic Control](https://arxiv.org/abs/2512.01809)
...
TL;DR: 该研究对生成式控制策略在机器人行为克隆中的表现进行了全面评估，发现其优势主要来自迭代计算而非多模态捕捉能力，并提出了轻量级的最小迭代策略来验证这一发现。


<details>
  <summary>Details</summary>
Motivation: 生成模型（如流模型和扩散模型）在机器人控制中表现出色，但对其成功因素存在多种推测，包括捕捉多模态动作分布和表达更复杂行为等。本研究旨在通过系统评估揭示生成式控制策略真正有效的核心因素。

Method: 在常见的行为克隆基准测试中对流行的生成式控制策略进行全面评估，分析其性能来源。提出最小迭代策略（MIP）作为验证工具，这是一个轻量级的两步回归策略，用于测试迭代计算的重要性。

Result: 研究发现生成式控制策略的成功并非源于捕捉多模态或表达复杂观察-动作映射的能力，而是源于迭代计算，前提是训练期间对中间步骤进行监督，并配合适当水平的随机性。最小迭代策略在性能上与流模型相当，甚至优于蒸馏的捷径模型。

Conclusion: 生成式控制策略的分布拟合组件并不像通常认为的那么重要，研究结果指向了专注于控制性能的新设计空间，迭代计算是提升性能的关键因素。
...
### [348] [Is Image-based Object Pose Estimation Ready to Support Grasping?](https://arxiv.org/abs/2512.01856)
*Eric C. Joyce,Qianwen Zhao,Nathaniel Burgdorfer,Long Wang,Philippos Mordohai*

Main category: cs.RO

TL;DR: 该研究提出一个评估6自由度物体姿态估计器的框架，专注于仅需单张RGB图像输入的方法，通过物理模拟器中的抓取实验来评估这些姿态估计器作为机器人抓取唯一感知机制的可行性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是了解单张RGB图像输入的6自由度物体姿态估计器的准确性，并评估它们能否作为机器人抓取任务的唯一感知机制。现有文献缺乏对这些姿态估计器在实际机器人应用中的性能评估。

Method: 研究方法包括：1）构建评估框架；2）在物理模拟器中进行抓取实验；3）使用基于图像的姿态估计指导平行夹爪和欠驱动机械手抓取3D物体模型；4）在BOP数据集子集上比较五种开源物体姿态估计器。

Result: 实验比较了五种开源物体姿态估计器的性能，提供了文献中缺失的见解，揭示了这些姿态估计器在机器人抓取应用中的实际表现和局限性。

Conclusion: 该研究为评估6自由度物体姿态估计器提供了一个系统框架，通过物理模拟器中的抓取实验验证了这些方法作为机器人抓取感知机制的可行性，为相关研究提供了重要的基准和见解。
...
### [349] [Real-World Robot Control by Deep Active Inference With a Temporally Hierarchical World Model](https://arxiv.org/abs/2512.01924)
*Kentaro Fujii,Shingo Murata*

Main category: cs.RO

TL;DR: 提出了一种新颖的深度主动推理框架，通过多时间尺度环境表示和动作抽象，在不确定环境中实现目标导向与探索性行为的平衡，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习控制方法大多忽视探索行为，在不确定环境中表现不佳。传统深度主动推理方法存在环境表示能力有限和动作选择计算成本高的问题。

Method: 提出包含世界模型、动作模型和抽象世界模型的三部分框架：世界模型将环境动态编码为慢速和快速时间尺度的隐藏状态表示；动作模型使用向量量化将动作序列压缩为抽象动作；抽象世界模型基于抽象动作预测未来慢速状态，实现低成本动作选择。

Result: 在真实机器人物体操作任务中，该框架在多样化操作任务中实现了高成功率，能在不确定环境中切换目标导向和探索性行为，同时使动作选择计算可行。

Conclusion: 该研究强调了建模多时间尺度动态以及抽象动作和状态转换的重要性，为解决不确定环境中的机器人控制问题提供了有效框架。
...
### [350] [Guardian: Detecting Robotic Planning and Execution Errors with Vision-Language Models](https://arxiv.org/abs/2512.01946)
*Paul Pacaud,Ricardo Garcia,Shizhe Chen,Cordelia Schmid*

Main category: cs.RO

TL;DR: 提出自动机器人失败合成方法，通过扰动成功轨迹生成多样化规划与执行失败，构建三个新失败检测基准，训练Guardian视觉语言模型实现SOTA失败检测性能


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在机器人失败检测方面受限于失败数据的稀缺性，准确性和泛化能力有限，需要解决数据缺口问题

Method: 提出自动机器人失败合成方法，程序化扰动成功轨迹生成多样化规划与执行失败；构建RLBench-Fail、BridgeDataV2-Fail和UR5-Fail三个新基准；训练Guardian视觉语言模型，使用多视角图像进行详细失败推理和检测

Result: Guardian在现有和新引入基准上均达到最先进性能；当集成到最先进的操纵系统中时，在仿真和真实机器人上有效提高任务成功率

Conclusion: 提出的失败数据生成方法显著扩展了失败数据集的多样性和规模，训练出的Guardian模型在失败检测方面表现出色，并能实际提升机器人操纵系统的性能
...
### [351] [RoaD: Rollouts as Demonstrations for Closed-Loop Supervised Fine-Tuning of Autonomous Driving Policies](https://arxiv.org/abs/2512.01993)
*Guillermo Garcia-Cobo,Maximilian Igl,Peter Karkus,Zhejun Zhang,Michael Watson,Yuxiao Chen,Boris Ivanovic,Marco Pavone*

Main category: cs.RO

TL;DR: RoaD方法通过利用策略自身的闭环轨迹作为训练数据，有效缓解自动驾驶策略部署时的协变量偏移问题，显著提升闭环性能。
...
Method: RoaD方法的核心是利用策略自身的闭环轨迹作为额外训练数据。在轨迹生成过程中，引入专家指导使轨迹偏向高质量行为，产生信息丰富且真实的演示用于微调。这种方法避免了传统闭环监督微调方法的限制性假设。

Result: 在WOSAC大规模交通仿真基准测试中，RoaD表现与现有CL-SFT方法相当或更好；在AlpaSim高保真神经重建模拟器中，驾驶分数提升41%，碰撞减少54%。

Conclusion: RoaD是一种简单高效的闭环适应方法，能够以远少于强化学习的数据量实现鲁棒性能，且应用范围更广，包括端到端驾驶场景。
...
### [352] [Learning Sim-to-Real Humanoid Locomotion in 15 Minutes](https://arxiv.org/abs/2512.01996)
*Younggyo Seo,Carmelo Sferrazza,Juyue Chen,Guanya Shi,Rocky Duan,Pieter Abbeel*

Main category: cs.RO

TL;DR: 提出基于FastSAC和FastTD3的简单实用配方，可在15分钟内用单张RTX 4090 GPU快速训练人形机器人运动策略，实现大规模并行环境下的稳定强化学习


<details>
  <summary>Details</summary>
Motivation: 虽然大规模并行仿真已将机器人强化学习训练时间从几天缩短到几分钟，但人形机器人的快速可靠仿真到现实迁移仍然困难，主要挑战包括高维度和领域随机化等因素

Method: 基于离策略强化学习算法（FastSAC和FastTD3），通过精心调优的设计选择和极简奖励函数，在大规模并行环境（数千个）中稳定离策略RL算法

Result: 在Unitree G1和Booster T1机器人上实现了快速端到端人形运动控制器学习，在强领域随机化（随机动力学、崎岖地形、推力扰动）下表现良好，并能快速训练全身人体运动跟踪策略

Conclusion: 提出了一种简单实用的配方，能够在短时间内高效训练人形机器人运动策略，为快速可靠的仿真到现实迁移提供了可行方案
...
### [353] [LLM-Driven Corrective Robot Operation Code Generation with Static Text-Based Simulation](https://arxiv.org/abs/2512.02002)
*Wenhao Wang,Yanyan Li,Long Jiao,Jiawei Yuan*

Main category: cs.RO

TL;DR: 本文提出了一种基于LLM的静态模拟机器人操作代码框架，通过LLM直接模拟代码执行过程，无需依赖物理实验或定制化仿真环境，实现了可靠的机器人操作代码生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成机器人操作代码的研究依赖物理实验或定制仿真环境，存在环境配置成本高、执行时间长的问题，限制了实际部署。本文探索直接利用LLM进行静态模拟的可能性。

Method: 设计了一个基于LLM的静态模拟器框架，将LLM配置为具有增强能力的静态模拟器，能够解释动作、推理状态转换、分析执行结果，并生成准确捕捉轨迹动态的语义观察。

Result: 实验在多种机器人（包括无人机和小型地面车辆）的不同操作任务上进行，结果显示静态文本模拟具有高准确性，且框架的代码生成可靠性达到与最先进研究相当的水平。

Conclusion: 提出的LLM驱动校正框架不依赖物理实验或仿真器的动态代码执行，实现了可靠的机器人操作代码生成，为LLM驱动机器人系统提供了更高效的解决方案。
...
### [354] [Learning Dexterous Manipulation Skills from Imperfect Simulations](https://arxiv.org/abs/2512.02011)
*Elvis Hsieh,Wen-Han Hsieh,Yen-Jen Wang,Toru Lin,Jitendra Malik,Koushil Sreenath,Haozhi Qi*

Main category: cs.RO

TL;DR: 提出DexScrew框架，通过三阶段方法解决灵巧操作中的sim-to-real转移问题，特别针对螺母螺栓紧固和螺丝刀操作任务，结合强化学习、遥操作演示和行为克隆，实现高任务完成率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习和sim-to-real转移在灵巧操作方面取得进展，但仍受限于复杂接触动力学和多感官信号（特别是触觉反馈）的模拟困难。需要一种能够有效处理这些限制的框架。

Method: 提出三阶段框架：1）使用简化物体模型在仿真中训练强化学习策略，形成正确的手指步态；2）将学习到的策略作为技能原语，通过遥操作系统收集包含触觉和本体感觉信息的真实世界演示；3）训练包含触觉感知的行为克隆策略，使其能够泛化到不同几何形状的螺母和螺丝刀。

Result: 实验显示，在螺母螺栓紧固和螺丝刀操作任务中，相比直接的sim-to-real转移，该方法获得更高的任务进度比，并且在未见过的物体形状和外部扰动下表现出鲁棒性能。

Conclusion: DexScrew框架有效解决了灵巧操作中sim-to-real转移的挑战，通过结合仿真训练、真实演示和触觉感知，实现了对复杂接触任务的高效学习和泛化能力。
...
### [355] [ManualVLA: A Unified VLA Model for Chain-of-Thought Manual Generation and Robotic Manipulation](https://arxiv.org/abs/2512.02013)
...
TL;DR: ManualVLA是一个统一的视觉-语言-动作框架，通过混合Transformer架构实现多模态手册生成与动作执行的协同工作，在长时程任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在处理需要明确目标状态的长时程任务（如乐高组装、物体重排）时，难以协调高层规划与精确操作，需要从"什么"结果推断"如何"过程的能力。

Method: 基于混合Transformer架构，包含规划专家生成包含图像、位置提示和文本指令的中间手册，然后通过手册思维链推理过程将这些手册输入动作专家，同时提供显式控制条件和隐式指导。

Result: ManualVLA在真实世界表现优异，在乐高组装和物体重排任务上的平均成功率比之前的分层SOTA基线高出32%。

Conclusion: ManualVLA框架通过将目标状态转化为可执行程序，实现了多模态手册生成与动作执行的统一，显著提升了VLA模型在长时程任务中的性能。
