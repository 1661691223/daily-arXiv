...
Motivation: 现有顺序自回归方法在复杂任务中由于错误传播可能导致性能下降，特别是在生成推理与最终图像之间的对齐问题上。

Method: 提出并行多模态扩散框架MMaDA-Parallel，支持文本和图像在整个去噪轨迹中的双向交互，采用监督微调训练，并通过并行强化学习(ParaRL)应用语义奖励来增强跨模态一致性。
...
TL;DR: PriVi是一个大规模灵长类动物中心视频预训练数据集，包含424小时精心策划的视频，通过V-JEPA预训练学习灵长类特定表示，在多个基准数据集上优于现有方法，提高了数据效率和泛化能力。
...
Method: 从模型中心转向数据中心方法，构建PriVi数据集（424小时视频），使用V-JEPA进行灵长类特定表示预训练，并通过轻量级冻结分类器评估。

Result: 在ChimpACT、BaboonLand、PanAf500和ChimpBehave四个基准数据集上，该方法持续优于先前工作，包括完全微调的基线，并在较少标签情况下表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑模型对多指令提示中的每个指令使用固定强度，限制了用户对单个编辑强度的精确连续控制。

Method: 引入SliderEdit框架，将多部分编辑指令解耦为单独滑块，使用单一低秩适应矩阵集泛化处理各种编辑、属性和组合指令，实现连续插值控制。

Result: 在FLUX-Kontext和Qwen-Image-Edit等先进图像编辑模型上应用SliderEdit，显著提升了编辑可控性、视觉一致性和用户可操控性。

Conclusion: SliderEdit是首个探索和提出指令式图像编辑模型中连续精细指令控制框架的方法，为交互式指令驱动图像操作开辟了新途径。
...
Method: 集成去噪概率模型使用扩散过程生成密度图，采用窄高斯核和多密度图输出，结合回归分支进行精确特征提取，通过相似度评分整合密度图，引入基于Farneback光流算法的事件驱动采样技术选择性捕获重要人群运动帧。

Result: 通过定性和定量评估（包括叠加图和平均绝对误差），模型在密集和稀疏场景下都能有效捕捉群体动态，采样方法能够减少帧数同时保持关键群体事件。

Conclusion: 该工作通过解决视频分析的时间挑战，为实时人群监控提供了一个可扩展且高效的框架，在保持精度的同时显著降低了计算负载和存储需求。
...
TL;DR: PALMS+是一个基于图像的室内定位系统，通过单目深度估计和几何布局匹配，在GPS缺失环境中实现基础设施无关的定位，在静止和连续定位中都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决基于视觉的室内定位方法（如PALMS）在智能手机LiDAR短距离和室内布局模糊性方面的限制，为应急响应和辅助导航等应用提供更好的基础设施无关定位方案。

Method: 使用基础单目深度估计模型（Depth Pro）从定位的RGB图像重建尺度对齐的3D点云，然后通过与平面图进行卷积实现几何布局匹配，输出位置和方向的后验分布。

Result: 在Structured3D和包含80个观测的校园数据集上，PALMS+在静止定位精度上优于PALMS和F3Loc，无需任何训练。在33个真实世界轨迹上进行连续定位时，定位误差更低，展示了相机无关跟踪的鲁棒性。

Conclusion: PALMS+是一个模块化、基于图像的系统，能够有效解决室内定位中的挑战，在基础设施无关应用中具有巨大潜力，代码和数据已开源。
...
Conclusion: 所提模型能有效减少碰撞并提高轨迹预测精度，在不同密度场景下均表现优异。
...
TL;DR: 本文提出将汽车摄像头污染检测视为语义分割问题，比较了流行分割方法与传统瓦片分类方法的性能优势，并修复了Woodscape数据集中的数据泄漏和标注不精确问题。
...
Method: 将污染检测视为语义分割问题，比较了流行分割方法与瓦片分类方法，创建了新的数据子集以解决原始数据集中的数据泄漏和标注不精确问题。

Result: 尽管新数据子集规模小得多，但为分割方法提供了足够信息，在更短时间内达到可比结果。

Conclusion: 语义分割方法在汽车摄像头污染检测中优于瓦片分类方法，通过修复数据集问题可以显著提高训练效率和性能。
...
TL;DR: 评估8个医学和通用领域基础模型在胸部X光分析中的表现，发现医学预训练有优势，但特征有效性高度依赖任务，复杂病理分割仍需大量微调，文本-图像对齐并非必需，监督基线模型在分割任务中仍具竞争力。


<details>
  <summary>Details</summary>
Motivation: 基础模型在医学影像中的应用效果不一，不清楚预训练领域、范式和架构如何影响嵌入质量，阻碍了为特定放射学任务选择最佳编码器。

Method: 评估8个医学和通用领域基础模型的视觉编码器，使用线性探测和微调方法对胸部X光进行分类（气胸、心脏肥大）和分割（气胸、心脏边界）任务。

Result: 医学预训练具有显著优势；医学基础模型在线性探测中始终优于通用领域模型。预训练嵌入在全局分类和显著解剖结构分割中表现良好，但在复杂细微病理分割中表现不佳。发现模型使用混淆捷径进行分类，文本-图像对齐并非必需，监督基线模型在分割任务中保持竞争力。
...
TL;DR: 本文提出了一种通过遍历生成模型的潜在空间来增强虹膜图像的新策略，能够控制特定虹膜图像属性同时保持身份不变。
...
Method: 通过梯度引导在生成模型的潜在空间中遍历，操纵几何、纹理或质量相关的虹膜图像特征（如清晰度、瞳孔大小、虹膜大小或瞳孔-虹膜比例），同时保持图像的身份特征。该方法可使用预训练GAN模型生成的随机图像或真实虹膜图像。

Result: 提出的方法能够合成同身份虹膜图像，同时精确控制特定属性，为虹膜识别系统提供更丰富的训练数据。

Conclusion: 该方法提供了一种灵活且可扩展的虹膜图像增强策略，能够为虹膜识别和呈现攻击检测系统生成多样化的训练数据，提高系统的鲁棒性。
...
TL;DR: STORM是一个无需人工标注的实时6D姿态估计系统，通过结合视觉语言理解和自监督特征匹配的三阶段流程，实现目标分割、跟踪和重定位，并具备自动重注册机制来应对遮挡和快速运动。
...
Method: 采用三阶段流程：上下文对象描述指导定位，自交叉注意力机制识别候选区域，分割模型生成精确掩码进行姿态估计；并引入自动重注册机制通过特征相似性监测检测跟踪失败。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界的数据流通常存在双重不平衡：数据集级分布与单个任务内的极端或反向偏斜，这造成了任务内和任务间的不平衡，阻碍了有效学习和泛化。现有的免示例持续学习方法往往忽视了这种内在不平衡。

Method: PANDA框架使用CLIP编码器识别代表性区域，并将这些区域移植到频繁类样本中，以增强低频类。同时采用自适应平衡策略，利用先前任务分布来平滑任务间不平衡，减少任务间平均样本的差距。

Result: 广泛的实验和消融研究表明，PANDA能够与现有的基于预训练模型的持续学习方法协同工作，提高准确性并减少灾难性遗忘。

Conclusion: PANDA框架有效地解决了现实世界数据流中的双重不平衡问题，为免示例持续学习提供了有效的解决方案，能够与现有方法无缝集成并显著提升性能。
...
TL;DR: STS是一种轻量级测试时适应框架，通过提取文本嵌入的谱子空间来定义主要语义方向，在潜在空间中以频谱感知方式调整少量参数来最小化增强视图的熵，无需反向传播或修改编码器。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型测试时适应方法通常需要反向传播大型编码器权重或改变核心模型组件，导致计算开销大。

Method: STS从文本嵌入中提取谱子空间定义语义方向，通过适应少量每样本偏移参数来在潜在空间中引导表示，最小化增强视图的熵。
...
<details>
  <summary>Details</summary>
Motivation: 现有的跨视角图像检索方法需要监督训练和特定数据集，限制了实际部署。本文旨在开发无需额外训练、使用单目街景图像的检索方法。

Method: 使用预训练视觉编码器（如DINOv2）和LLM，通过网页图像搜索提取地理线索，利用地理编码API生成卫星查询，采用PCA白化特征精化进行匹配。
...
TL;DR: 本文提出了一种基于3D高斯溅射(3DGS)的新框架，用于在3D场景中动画化人类角色，实现了几何一致的自由视点渲染和自然的人-场景交互。


<details>
  <summary>Details</summary>
Motivation: 现有的动画管道使用网格或点云作为底层3D表示，而3DGS作为神经场景表示在新型视图合成方面取得了最先进的效果，但在人-场景动画和交互方面仍未被充分探索。

Method: 使用3DGS作为3D表示，将渲染与运动合成解耦；提出高斯对齐运动模块，使用基于不透明度的线索和投影高斯结构来指导人体放置和姿态对齐；进一步提出人-场景高斯细化优化，确保真实的接触和导航。

Result: 在Scannet++和SuperSplat库的场景上进行了评估，展示了从稀疏和密集多视图人体捕获重建的虚拟角色的动画效果。
...
<details>
  <summary>Details</summary>
Motivation: 对抗性补丁攻击通过在图像中注入局部扰动来误导深度视觉模型，这些攻击可在物理世界部署，对实际应用构成严重威胁。现有最先进方法需要两轮掩码且计算复杂度高，需要更高效的防御方案。

Method: CertMask构建一组可证明充分的二进制掩码来消除补丁影响，采用数学严谨的覆盖策略确保每个可能的补丁位置至少被覆盖k次，仅需单轮掩码操作，时间复杂度为O(n)。

Result: 在ImageNet、ImageNette和CIFAR-10上的实验表明，CertMask相比PatchCleanser将认证鲁棒精度提高了最多+13.4%，同时保持与原始模型几乎相同的清洁精度。

Conclusion: CertMask提供了一种高效且可证明鲁棒的防御机制，显著降低了对抗性补丁攻击的防御成本，同时保持了模型的原始性能。
...
TL;DR: 该研究将太阳物理学基础模型适配用于太阳风结构分析，通过结合航天器位置和太阳磁连接信息构建神经场模型，利用帕克太阳探测器数据进行太阳风结构分类。
...
Method: 调整太阳物理学基础模型创建嵌入，结合航天器位置和太阳磁连接的傅里叶特征编码，构建神经场模型进行微调，使用帕克太阳探测器数据进行下游分类任务。

Result: 整体分类性能一般，可能由于粗粒度标签、类别不平衡和预训练模型可迁移性有限，但证明了利用基础模型嵌入进行原位太阳风任务的可行性。

Conclusion: 作为首个概念验证，为未来改进更可靠的空间天气预报奠定了基础，研究代码和配置文件已公开支持可重复性。
...
TL;DR: 本文提出了IPCD方法，将点云直接分解为反照率和阴影，解决了点云非网格结构和全局光照方向两大挑战。通过IPCD-Net和PLD技术，在合成户外场景数据集上验证了方法的有效性，并展示了在纹理编辑、重光照和配准等应用中的实用性。
...
Method: 提出IPCD-Net扩展图像模型处理点云数据，引入PLD通过多视图投影捕捉全局光照线索，采用层次特征细化技术。

Result: 实验结果显示IPCD-Net能有效减少反照率中的投射阴影，提高阴影中的颜色准确性，在纹理编辑、重光照和点云配准等应用中表现良好。

Conclusion: IPCD方法成功解决了点云分解的关键挑战，验证了在真实世界中的适用性，为点云的材质编辑和光照处理提供了有效解决方案。
...
TL;DR: 该论文提出了T-DRS方法，通过三种推理阶段的衰减恢复策略来缓解大型视觉语言模型中旋转位置编码导致的远程依赖建模问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在使用旋转位置编码时面临远程依赖建模的挑战，由于渐进注意力衰减导致模型难以记住全局上下文。

Method: 提出推理阶段的三步衰减恢复策略：语义驱动DRS、距离感知控制DRS和重新强化远程DRS，通过内容感知残差、位置距离权重调制和远程依赖巩固来恢复被抑制的长程标记对。

Result: 在视觉问答基准测试上的广泛实验表明，T-DRS能够以无需训练的方式持续提升模型性能。

Conclusion: T-DRS策略能够有效恢复被抑制的远程标记对，同时不损害局部归纳偏差，改善了大型视觉语言模型的全局上下文建模能力。
...
TL;DR: VISTA是一个基于递归目标条件变换器的多智能体轨迹预测模型，通过结合长期意图、历史运动和社会交互建模，在MADRAS和SDD数据集上实现了最先进的精度和显著降低的碰撞率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时捕捉智能体的长期目标和细粒度社会交互，导致生成的多智能体未来轨迹不真实。需要开发能够联合建模意图和社会交互的预测框架。

Method: 提出VISTA模型，包含：(1)交叉注意力融合模块整合长期意图与历史运动；(2)社会令牌注意力机制实现灵活的多智能体交互建模；(3)成对注意力图使社会影响模式在推理时可解释。
...
TL;DR: 本文提出SAM-DAQ方法，通过深度引导的自适应查询将SAM2模型适配于RGB-D视频显著目标检测任务，解决了手动提示依赖、内存消耗高和计算负担重的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接将基础模型应用于RGB-D视频显著目标检测任务时面临三个挑战：手动提示依赖、序列适配器内存消耗高、内存注意力计算负担重。

Method: 提出SAM-DAQ方法：1）部署基于并行适配器的多模态图像编码器，通过深度引导的并行适配器融合多模态特征；2）部署查询驱动的时间记忆模块，统一记忆库和提示嵌入到可学习管道中。

Result: 在三个RGB-D VSOD数据集上的广泛实验表明，所提出的SAM-DAQ在所有评估指标上都优于最先进的方法。

Conclusion: SAM-DAQ通过深度引导的自适应查询和查询驱动的时间记忆模块，成功将SAM2模型适配于RGB-D视频显著目标检测任务，取得了优越的性能。
...
TL;DR: 本文提出了一种多模态3D场景图(M3DSG)和基于此的零样本导航系统MSGNav，通过保留视觉线索和动态分配图像关系边，解决了现有零样本方法中视觉信息丢失和词汇受限的问题，在GOAT-Bench和HM3D-OVON数据集上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界机器人部署需要开放词汇泛化和低训练开销，但现有零样本方法构建的显式3D场景图通常将丰富的视觉观察压缩为纯文本关系，导致构建成本高、视觉证据不可逆丢失和词汇受限。

Method: 引入多模态3D场景图(M3DSG)保留视觉线索，用动态分配图像替换文本关系边；提出MSGNav系统，包含关键子图选择、自适应词汇更新和闭环推理模块；针对零样本导航中的最后一英里问题，提出基于可见度的视点决策模块。

Result: 在GOAT-Bench和HM3D-OVON数据集上的综合实验结果表明，MSGNav实现了最先进的性能。

Conclusion: M3DSG和MSGNav系统有效解决了零样本导航中的视觉信息保留和开放词汇支持问题，为机器人实际部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中存在两个关键限制：1）感知冗余，处理不相关视觉输入效率低下；2）表面化的指令-视觉对齐，阻碍了动作的语义基础。这些限制阻碍了实际部署。

Method: 提出三个核心组件：1）语义引导的双重视觉剪枝器（SD-Pruner），包括指令驱动剪枝器和空间聚合剪枝器；2）语义互补的分层融合器（SH-Fuser），融合密集补丁和稀疏标记；3）语义条件动作耦合器（SA-Coupler），替代传统的观测到自由度方法。

Result: 在仿真和真实世界任务上的广泛实验表明，SemanticVLA在LIBERO基准测试中比OpenVLA成功率提高21.1%，同时训练成本和推理延迟分别降低3.0倍和2.7倍。

Conclusion: SemanticVLA通过语义对齐的稀疏化和增强，为高效的机器人操作设定了新的最先进标准，显著提升了性能和效率，并已开源供公众使用。
...
TL;DR: 本文提出HCC-3D方法，通过分层补偿压缩技术显著降低3D视觉语言模型中3D令牌的计算开销，同时保持关键信息完整性。


<details>
  <summary>Details</summary>
Motivation: 当前3D-VLMs直接将3D点云嵌入为3D令牌，在LLM部分处理所有3D令牌会产生巨大计算成本，限制了应用。需要找到在减少计算开销的同时保持3D令牌关键信息完整性的方法。

Method: 提出分层补偿压缩(HCC-3D)：1) 全局结构压缩(GSC)：使用全局查询将所有3D令牌压缩为少量关键令牌，保持整体结构信息；2) 自适应细节挖掘(ADM)：通过互补评分选择性重新压缩显著但未被充分关注的特征，补偿GSC中的信息损失。

Result: 实验表明HCC-3D实现了约98%的极端压缩比，相比之前的3D-VLMs，在效率和性能上都有显著提升，达到了新的最先进水平。

Conclusion: HCC-3D方法成功解决了3D-VLMs中计算开销过大的问题，通过分层压缩策略在保持性能的同时大幅提升效率，为3D多模态推理提供了实用的解决方案。
...
### [25] [Regional Attention-Enhanced Swin Transformer for Clinically Relevant Medical Image Captioning](https://arxiv.org/abs/2511.09893)
*Zubia Naz,Farhan Asghar,Muhammad Ishfaq Hussain,Yahya Hadadi,Muhammad Aasim Rafique,Wookjin Choi,Moongu Jeon*

Main category: cs.CV

TL;DR: 提出了一种基于Swin-BART编码器-解码器的医疗图像自动描述系统，通过轻量级区域注意力模块增强诊断关键区域，在ROCO数据集上实现了最先进的语义保真度。
...
Method: 采用Swin-BART编码器-解码器架构，集成轻量级区域注意力模块，在交叉注意力前放大诊断显著区域。使用beam search解码策略。

Result: 在ROGE指标上显著优于基线模型（0.603 vs 0.356/0.255），BERTScore也表现最佳（0.807 vs 0.645/0.623），其他指标如BLEU、CIDEr、METEOR具有竞争力。

Conclusion: 该方法能够生成准确、临床化的图像描述，并提供透明的区域归因，支持在人类监督下的安全研究使用。
...
TL;DR: DocWeaver是一个多智能体管道，利用大语言模型自动生成MosaicDoc基准，这是一个大规模、双语（中文和英文）的视觉丰富文档理解资源，包含72K图像和600K+问答对，用于评估模型在复杂布局文档上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型基准主要是英语中心、布局简单且任务有限，无法充分评估模型在视觉丰富文档理解（VRDU）方面的能力，特别是处理复杂布局和密集文本的挑战。

Method: 引入DocWeaver多智能体管道，利用大语言模型自动生成新基准MosaicDoc，该基准源自报纸和杂志，具有多样复杂布局（包括多列和非曼哈顿布局）、196个出版商的丰富风格变化，以及全面的多任务标注。

Result: MosaicDoc包含72K图像和超过600K问答对，作为该领域的权威基准。对最先进模型在该基准上的广泛评估揭示了它们在处理现实世界文档复杂性方面的当前局限性。

Conclusion: MosaicDoc为视觉丰富文档理解领域设定了明确的基准，揭示了现有模型的局限性，并为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习行人重识别模型易受对抗攻击影响，而现有防御策略主要针对分类任务，未能解决行人重识别特有的对抗鲁棒性挑战。

Method: 提出去偏双不变防御框架，包括基于扩散模型的数据重采样策略来缓解模型偏见，以及包含最远负例扩展软化的度量对抗训练方法和对抗增强的自元机制。

Result: 实验表明该方法显著优于现有最先进的防御方法。

Conclusion: 该方法有效解决了行人重识别对抗防御中的模型偏见和复合泛化问题，为度量学习任务的对抗鲁棒性提供了新思路。
...
TL;DR: TSPE-GS是一种针对3D高斯泼溅的透明表面重建方法，通过均匀采样透射率来建模像素级多模态不透明度和深度分布，解决了半透明表面重建的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅方法假设每个像素只有一个深度，这在多个表面可见的半透明场景中会失败，导致重建困难。

Method: 提出TSPE-GS方法，通过均匀采样透射率建模像素级多模态分布，逐步融合截断符号距离函数来分别重建外部和内部表面。

Result: 在公开和自收集的半透明及不透明数据集上的广泛实验表明，TSPE-GS显著改善了半透明几何重建，同时保持了对不透明场景的性能。

Conclusion: TSPE-GS能够有效解决半透明表面重建问题，且无需额外训练开销即可推广到其他基于高斯的重建流程中。
...
TL;DR: 本文提出了一种自适应融合框架，将CLIP图像特征的余弦相似度与幅度感知质量线索相结合，用于无参考图像质量评估。该方法通过Box-Cox变换对特征分布进行统计归一化，并设计置信度引导的融合方案，在多个基准数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用CLIP模型的余弦相似度来衡量图像质量，但忽略了图像特征幅度与感知质量之间的强相关性。本文旨在探索这一未充分利用的线索。

Method: 提取CLIP图像特征的绝对值，应用Box-Cox变换进行统计归一化，生成语义归一化的辅助线索。设计置信度引导的融合方案，自适应地加权余弦相似度和幅度线索。

Result: 在多个基准IQA数据集上的广泛实验表明，该方法一致优于标准的基于CLIP的IQA方法和最先进的基线方法，且无需任何任务特定训练。

Conclusion: CLIP图像特征的幅度是图像质量评估的重要线索，与余弦相似度结合并通过自适应融合可以显著提升无参考图像质量评估性能。
...
TL;DR: 提出了一种利用视觉语言模型自动生成伪标签来训练高效实时目标检测器的新流程，通过基于每个目标的协同教学训练策略来减轻VLM生成标签中的噪声问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本目标检测方面表现出色，但其检测延迟和幻觉预测问题使其无法直接部署。在自动驾驶等需要大量标注数据的领域，手动标注成本过高。

Method: 采用基于每个目标的协同教学训练策略，两个YOLO模型协作学习，根据彼此的目标级损失值从每个小批次中过滤不可靠的边界框，而不是过滤整个图像。

Result: 在KITTI数据集上，方法显著优于基线YOLOv5m模型，mAP@0.5从31.12%提升到46.61%，同时保持实时检测延迟。添加10%真实标签后，性能进一步提升至57.97% mAP@0.5。在ACDC和BDD100k数据集上也观察到类似的性能提升。
...
TL;DR: 本文提出EquS方法，通过双采样轨迹施加等变信息来改进基于扩散模型的图像恢复方法，并进一步提出时间步感知调度(TAS)来增强性能。


<details>
  <summary>Details</summary>
Motivation: 现有的问题无关扩散模型图像恢复方法在充分利用扩散先验方面存在挑战，导致性能不佳。

Method: 引入EquS方法，通过双采样轨迹施加等变信息；进一步提出时间步感知调度(TAS)和EquS+，优先确定性步骤以增强确定性和采样效率。

Result: 在基准测试上的广泛实验表明，该方法与先前的问题无关DMIR方法兼容，并在不增加计算成本的情况下显著提升性能。

Conclusion: 所提出的方法有效解决了现有问题无关扩散模型图像恢复方法的局限性，显著提升了性能表现。
...
Method: 提出差异向量均衡化(DiVE)，通过平均向量损失(AVL)和成对向量损失(PVL)来约束预训练和微调模型嵌入之间的差异向量，保持几何结构。

Result: 实验表明DiVE能有效保持几何结构，在ID、OOD和零样本指标上都取得了强劲结果。

Conclusion: DiVE方法通过保持嵌入几何结构，成功实现了在ID数据上稳健微调的同时不损害OOD和零样本泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前场景文本编辑方法在低资源语言支持、合成与真实数据领域差距以及缺乏适当文本样式保留评估指标方面的局限性。

Method: 使用语言自适应字形编码器和多阶段训练策略（先在合成数据上预训练，然后在真实图像上微调），并构建了新的STIPLAR数据集用于训练和评估。

Result: STELLAR在视觉一致性和识别准确率方面优于现有最先进模型，在跨语言平均TAS指标上比基线提高了2.2%。

Conclusion: STELLAR通过创新的架构和训练策略有效解决了多语言场景文本编辑的关键挑战，特别是在低资源语言和真实世界数据上的表现显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有后门攻击缺乏物理可实现性，主要由于数字到物理领域的差距。数字触发器在真实环境中经常失败，因为它们忽略了材料相关的LiDAR反射特性，而物理构造的触发器通常未优化，导致低效或易被检测。

Method: 1) 提出系统化方法选择鲁棒触发材料，识别二氧化钛(TiO₂)因其高漫反射率和环境弹性；2) 开发新颖模拟管道确保数字触发器准确模拟基于材料的物理触发器行为，包括角度无关的Oren-Nayar BRDF模型近似和距离感知缩放机制。
...
TL;DR: LampQ是一种针对Vision Transformer的层级混合精度量化方法，通过类型感知的Fisher度量、整数线性规划和迭代更新，解决了现有方法在粒度、度量尺度和比特分配方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有Vision Transformer量化方法采用统一精度，忽略了不同组件对量化的敏感性差异。基于度量的混合精度量化方法存在粒度粗、度量尺度不匹配和量化无关的比特分配三大问题。

Method: 提出LampQ方法：1）执行层级量化实现细粒度控制和高效加速；2）使用类型感知的Fisher度量评估敏感性；3）通过整数线性规划最优分配比特宽度；4）迭代更新比特分配。
...
<details>
  <summary>Details</summary>
Motivation: 现有异常生成方法通常需要示例异常，限制了其应用范围。作者旨在开发无需示例异常的生成方法，利用视觉和文本信息的融合来创建更真实多样的异常。

Method: 提出跨模态提示编码方案统一视觉和文本线索；采用基于修复的生成流程；引入对比细化策略强化异常与掩码的对齐；构建AnomVerse数据集（12,987个异常-掩码-描述三元组）用于训练。

Result: 实验表明Anomagic能生成比现有方法更真实和多样的异常，显著提升下游异常检测性能。该方法可作为通用基础模型，为任何正常类别图像生成用户定义提示的异常。

Conclusion: Anomagic成功实现了无需示例异常的零样本异常生成，通过跨模态信息融合和对比细化策略，在异常检测任务中取得了优越性能，为异常生成建立了通用基础模型。
...
TL;DR: 提出了DGFusion双引导多模态3D目标检测方法，通过难度感知实例配对和双引导模块解决硬实例检测问题，在nuScenes数据集上性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有单引导多模态3D目标检测方法无法处理硬实例在不同模态间信息密度的差异，影响自动驾驶系统的安全性。

Method: 基于双引导范式，包含难度感知实例配对器(DIPM)进行实例级特征匹配，以及双引导模块实现有效的多模态特征融合。

Result: 在nuScenes数据集上分别提升+1.0% mAP、+0.8% NDS和+1.3%平均召回率，在距离、尺寸、可见度和小规模训练场景下对硬实例检测具有一致的鲁棒性增益。

Conclusion: DGFusion通过双引导范式有效解决了硬实例检测问题，提升了自动驾驶感知系统的安全性。
...
Method: 采用UDF表示，提出局部到全局架构：将UDF划分为均匀子体积(UBlocks)，结合3D卷积捕获局部细节和稀疏变换器确保全局一致性，使用Pad-Average策略平滑子体积边界。
...
<details>
  <summary>Details</summary>
Motivation: 可见光和红外模态由不同传感器捕获，在复杂场景中存在信息不平衡问题，导致跨模态融合不足，检测性能下降。现有方法主要在空间域使用变换器，忽略了频率域变换器挖掘互补信息的优势。

Method: 提出频率域融合变换器FreDFT，包含多模态频率域注意力机制(MFDA)挖掘模态间互补信息，频率域前馈层(FDFFL)通过混合尺度频率特征融合策略增强多模态特征，跨模态全局建模模块(CGMM)在空间和通道维度进行像素级跨模态特征交互，局部特征增强模块(LFEM)使用多种卷积层和通道混洗来增强局部特征表示。

Result: 在多个公共数据集上的广泛实验结果表明，FreDFT相比其他最先进方法取得了优异性能。

Conclusion: 提出的频率域融合变换器FreDFT有效解决了可见光-红外目标检测中的模态信息不平衡问题，通过频率域特征挖掘和融合策略显著提升了检测性能。
...
TL;DR: 本文提出了一个完整的图像筛选解决方案，包括构建包含128k样本、640k图像的全面数据集，以及引入HCM-GRPO方法来增强多模态大语言模型的图像美学推理能力。
...
Method: 收集全面图像筛选数据集，采用多种标注方法获取高质量思维链数据；提出HCM-GRPO方法，在GRPO框架中引入硬案例挖掘策略和动态比例准确度奖励。

Result: 实验表明，即使最先进的闭源MLLMs在图像美学推理中表现类似随机猜测，而使用HCM-GRPO的小模型能够超越大型开源和领先闭源模型的得分。

Conclusion: 通过数据和方法论的创新，显著提升了MLLMs的图像美学推理能力，证明了小模型通过适当训练策略可以达到甚至超越大模型的性能。
...
TL;DR: 本文提出了AV-ConfuseBench基准测试，发现多模态大语言模型在音频-视觉混淆场景中存在视觉主导推理问题，并提出了RL-CoMM方法通过强化学习和外部音频模型来改善音频-视觉推理能力。
...
Conclusion: RL-CoMM通过强化学习和外部音频模型的协作，有效缓解了多模态大语言模型在音频-视觉推理中的视觉主导问题，显著提升了模型性能。
...
TL;DR: 该论文提出了GaussMedAct框架，利用多元高斯编码进行医疗动作评估，在CPREval-6k数据集上达到92.1%的Top-1准确率，比ST-GCN基线提升5.9%且仅需10%的计算量。
...
Method: 提出GaussMedAct框架，使用多元高斯表示将关节运动投影到时空缩放的多维空间，将动作分解为自适应3D高斯作为token；采用笛卡尔和向量双流策略的混合空间编码有效利用骨骼信息。

Result: 在CPREval-6k数据集上达到92.1%的Top-1准确率，实时推理性能，比ST-GCN基线提升5.9%准确率且仅需10%FLOPs；跨数据集实验验证了方法的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类中，模型往往将高预测分数误认为正确性，而缺乏对不确定性的认知，导致在稀疏标注或类别不平衡情况下产生确认偏差和过拟合。

Method: 提出CABIN框架：1）感知阶段估计认知不确定性识别模糊区域；2）行动阶段采用不确定性引导的双重采样策略；3）修正阶段引入细粒度动态分配策略，将伪标签数据分为可靠、模糊和噪声子集并应用定制损失。
...
TL;DR: VLF-MSC是一种统一的视觉语言特征多模态语义通信系统，通过传输单一紧凑的视觉语言表示来同时支持接收端的图像和文本生成。


<details>
  <summary>Details</summary>
Motivation: 现有的语义通信技术通常分别处理每种模态，需要特定的模态流或重传，导致频谱效率低下。VLF-MSC旨在通过统一的视觉语言表示来提高频谱效率和适应性。
...
Conclusion: VLF-MSC通过利用基础模型实现了对信道噪声的鲁棒性，同时保持了语义保真度，提供了一种高效的统一多模态语义通信解决方案。
...
TL;DR: 本文提出了GlobalDiff，一个基于扩散模型的框架，首次直接在全局关节旋转空间中操作，从根本上解耦了每个关节的预测与上游依赖关系，减轻了层次误差累积。通过引入多级约束方案来补偿全局旋转空间中结构先验的缺失。


<details>
  <summary>Details</summary>
Motivation: 现有的生成方法通常在局部关节旋转上操作，这些旋转基于骨架结构分层定义，导致生成过程中累积误差，表现为末端效应器的不稳定和不合理运动。

Method: 提出GlobalDiff扩散框架，在全局关节旋转空间中操作；引入多级约束方案：关节结构约束（虚拟锚点）、骨架结构约束（骨骼角度一致性）、时间结构约束（多尺度变分编码器）。
...
TL;DR: GridPrune是一种新的视觉token剪枝方法，采用"全局引导、局部选择"的两阶段策略来提升多模态大语言模型的效率，在保持96.98%性能的同时仅使用11.1%的token。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法主要关注"选择什么"，忽略了"看哪里"的空间分配问题，导致效率低下、位置偏见和冗余token保留。受人类视觉系统的启发，需要先确定关注区域再进行细粒度选择。
...
Conclusion: GridPrune通过模拟人类视觉系统的两阶段策略，有效解决了现有剪枝方法的空间分配问题，显著提升了多模态大语言模型的效率。
...
TL;DR: 提出SUGAR方法，通过结合LLMs和人体骨架进行动作分类和描述，利用大规模视频模型生成视觉运动信息作为先验知识来监督骨架学习，生成离散表示供LLMs理解。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs与人体骨架结合进行动作分类和描述，解决LLMs如何理解骨架和区分不同动作的问题。

Method: 1) 使用现成的大规模视频模型生成与动作相关的视觉运动信息作为知识库；2) 通过先验知识监督骨架学习生成离散表示；3) 使用未调整预训练权重的LLMs理解这些表示并生成动作目标和描述；4) 提出Temporal Query Projection模块对长序列骨架信号进行连续建模。

Result: 在多个基于骨架的动作分类基准测试中验证了SUGAR的有效性，零样本场景实验表明SUGAR比基于线性方法更具通用性。

Conclusion: SUGAR方法成功地将LLMs与人体骨架结合，通过视觉运动知识监督骨架学习，实现了有效的动作识别和描述，在零样本场景下表现优异。
...
TL;DR: 提出了RobIA框架，一种用于立体深度估计中持续测试时适应的鲁棒、实例感知方法，通过动态路由和伪监督解决域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 解决立体深度估计在真实环境中面临的动态域偏移、稀疏监督和密集标签获取成本高的问题，现有测试时适应方法在持续变化场景下效果有限。

Method: RobIA框架包含两个核心组件：Attend-and-Excite Mixture-of-Experts（AttEx-MoE）模块，通过轻量级自注意力机制动态路由输入到冻结专家；Robust AdaptBN Teacher，基于PEFT的教师模型提供密集伪监督。
...
TL;DR: 提出了CACMI框架，通过显式建模视频的时序连贯性和语义上下文来解决密集视频描述任务中现有方法无法捕捉事件序列时序一致性和完整语义的问题。


<details>
  <summary>Details</summary>
Motivation: 现有密集视频描述方法主要依赖隐式建模，使用帧级或碎片化视频特征，无法有效捕捉事件序列的时序连贯性和视觉上下文中的全面语义。

Method: 提出CACMI框架，包含两个核心组件：跨模态帧聚合通过跨模态检索聚合相关帧以提取时序连贯的事件对齐文本特征；上下文感知特征增强利用查询引导注意力整合视觉动态与伪事件语义。

Result: 在ActivityNet Captions和YouCook2数据集上的大量实验表明，CACMI在密集视频描述任务上达到了最先进的性能。

Conclusion: CACMI框架通过显式建模视频的时序特性和语言语义，有效提升了密集视频描述的性能，证明了显式时序-语义建模的重要性。
...
TL;DR: 本文调查发现当前领先的文本到图像模型存在无法处理逻辑组合的根本缺陷，特别是在否定、计数和空间关系三个核心原语上，当这些原语组合时模型性能急剧下降。


<details>
  <summary>Details</summary>
Motivation: 调查当前文本到图像模型在逻辑组合能力上的根本缺陷，揭示模型在单个原语上表现良好但在组合时性能崩溃的问题。

Method: 通过分析三个核心逻辑原语（否定、计数、空间关系）的组合性能，研究训练数据分布、注意力架构和评估指标对模型表现的影响。

Result: 发现模型在单个原语上准确但在组合时性能急剧下降，归因于训练数据缺乏显式否定、连续注意力架构不适合离散逻辑、评估指标偏向视觉合理性而非约束满足。
...
TL;DR: 本文提出了一种名为split-layer的新型MLP重构方法，通过将每层分成多个并行分支并用Hadamard乘积整合输出，有效构建高次多项式空间，显著提升了隐式神经表示(INR)的表征能力，同时避免了过高的计算开销。
...
Conclusion: split-layer通过高效扩展特征空间维度，成功提升了INR的表征能力，为逆问题优化提供了更强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测模型在不同人口统计群体（如性别、种族）上存在偏见，可能导致系统性误判，加剧数字鸿沟和社会不平等。现有公平性增强检测器往往以牺牲检测精度为代价来改善公平性。
...
Conclusion: 提出的双机制协同优化框架能够有效解决深度伪造检测中的公平性问题，在不牺牲检测精度的情况下提升模型公平性。
...
TL;DR: 本文提出了一种基于生成视角的生成增强对齐方法（GEA），通过文本引导的令牌增强和生成中间融合来解决文本到图像人物检索中的模态差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像人物检索方法存在文本查询无法准确反映图像内容、跨模态对齐困难以及过拟合等问题，文本和图像之间的固有模态差距进一步加剧了这些挑战。
...
Conclusion: GEA方法通过生成视角有效解决了文本到图像人物检索中的跨模态对齐挑战，实验结果表明该方法具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 探索Prithvi地理空间基础模型在数据稀缺地区的海岸监测应用潜力，特别是对小沙岛海岸线划定的迁移学习能力。

Method: 使用225张马尔代夫岛屿多光谱卫星图像数据集，对300M和600M参数的Prithvi模型进行微调，训练集规模从5到181张图像不等。

Result: 即使仅使用5张训练图像，模型也能达到高性能（F1得分0.94，IoU 0.79），展示了强大的迁移学习能力。
...
### [56] [Towards Blind and Low-Vision Accessibility of Lightweight VLMs and Custom LLM-Evals](https://arxiv.org/abs/2511.10615)
*Shruti Singh Baghel,Yash Pratap Singh Rathore,Sushovan Jena,Anurag Pradhan,Amit Shukla,Arnav Bhavsar,Pawan Goyal*

Main category: cs.CV

TL;DR: 本文评估了不同参数规模（500M和2.2B）的SmolVLM2模型在盲人和低视力用户视频描述任务中的性能，引入了两个专门针对BLV可访问性评估的新框架，并系统评估了提示设计策略和移动设备部署性能。
...
Result: 评估了不同参数规模模型在BLV可访问性描述质量方面的表现，分析了提示策略对性能的影响，并测试了在资源受限移动设备上的实际部署性能。

Conclusion: 通过专门设计的评估框架和系统实验，为开发适用于BLV用户的实际可部署视觉语言模型提供了重要见解和基准。
...
### [57] [HeatV2X: Scalable Heterogeneous Collaborative Perception via Efficient Alignment and Interaction](https://arxiv.org/abs/2511.10211)
*Yueran Zhao,Zhang Zhang,Chao Sun,Tianze Wang,Chao Yue,Nuoran Li*

Main category: cs.CV

TL;DR: 提出了HeatV2X框架，通过异构图注意力训练高性能智能体，使用局部异构微调和全局协作微调来解决V2X协作感知中的异构性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 解决V2X协作感知中两个关键挑战：(1)参与智能体的多模态和异构特性，(2)协作框架需要可扩展以适应新智能体。

Method: 首先训练基于异构图注意力的高性能智能体作为协作学习基础，然后设计局部异构微调和全局协作微调，分别使用Hetero-Aware Adapters提取模态特定差异，使用Multi-Cognitive Adapter增强跨智能体协作。

Result: 在OPV2V-H和DAIR-V2X数据集上的实验结果表明，该方法以显著降低的训练开销实现了优越的感知性能，优于现有最先进方法。

Conclusion: HeatV2X框架能够在最小化训练成本的同时，显著提升协作框架的性能，有效解决了V2X协作感知中的异构性和可扩展性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注音频-视觉不一致性检测，可能忽略模态内伪影，且在需要泛化时依赖真实样本的预训练。本文旨在克服这些限制。
...
TL;DR: 本文提出了TubeRMC框架，通过管状条件重建与相互约束来解决弱监督时空视频定位中的目标识别失败和跟踪不一致问题。
...
TL;DR: 本文提出了首个包含细粒度子分数和扣分标注的空中滑雪AQA数据集，并提出JudgeMind方法模拟专业裁判的评分思维，通过阶段分割、特征增强融合和基于知识的解码器来提升动作质量评估的性能和可靠性。
...
Method: 提出JudgeMind方法：1）将输入动作视频分割为不同阶段并分别评分；2）使用阶段感知特征增强融合模块提升对阶段特定关键区域的感知；3）提出基于知识的等级感知解码器，将可能的扣分项作为先验知识来预测更准确的分数。

Result: 实验结果表明，该方法在动作质量评估任务中达到了最先进的性能水平。

Conclusion: 通过模拟专业裁判的评分思维，结合阶段分割、特征增强和知识引导的解码机制，显著提升了AQA任务的性能和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统面部情感分析方法存在两个关键限制：(1) 视觉语言模型产生看似合理但不准确的幻觉推理；(2) 情感推理与识别之间的不对齐，源于面部特征与最终标签之间的碎片化连接。

Method: 提出三阶段对齐框架：1) 指令微调建立基础情感推理能力；2) 强化训练使用情感和动作单元标签作为奖励信号，显式对齐生成推理与预测情感；3) 数据合成流水线迭代利用前阶段扩展训练数据集，实现可扩展的自我改进。

Result: 构建了FEA-20K基准数据集（17,737训练和1,688测试样本），在八个标准基准测试中实现最先进的FEA性能，具有强大的泛化能力和稳健的可解释性。

Conclusion: Facial-R1框架有效解决了面部情感分析中的幻觉推理和识别-推理不对齐问题，通过最小监督实现了高性能、强泛化和稳健可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有细粒度视觉分类方法在捕捉判别性线索时存在不足，同时引入大量类别无关冗余，需要更全面的特征表示方法。

Method: 提出语义感知聚合模块(SAAM)构建加权超图来捕获高阶语义依赖，并引入双曲层次对比损失(HHCL)在非欧几里得空间中增强层次语义约束。
...
TL;DR: RUDDER是一个低开销框架，通过提取自注意力层的残差更新向量和贝叶斯自适应门控，在大视觉语言模型中减少物体幻觉问题，同时保持计算效率。
...
Method: 提出RUDDER框架，包含两个关键创新：1) CARD向量，在单次标准前向传播中从自注意力层残差更新提取的视觉证据向量；2) 贝叶斯启发的自适应门控，根据模型偏离视觉上下文程度进行令牌级注入校正信号。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLaVA架构在整合视觉特征时由于文本和视觉模态不匹配而存在的固有缺陷，探索LLM作为视觉编码器的潜力。

Method: 通过三个关键修改让LLM同时作为视觉编码器：1) 为视觉模态学习独立的QKV投影；2) 在视觉token上启用双向注意力；3) 整合全局和局部视觉表示。

Result: 在广泛的LLM上进行控制实验，LLaViT在多个基准测试中显著优于基线LLaVA方法，甚至超过了参数数量翻倍的模型。

Conclusion: LLaViT为视觉语言建模建立了一种更有效的方法，证明了LLM作为扩展视觉变换器的可行性。
...
TL;DR: 提出了一种结合景深监督和多视图一致性监督的3D高斯泼溅框架，解决了极端深度变化场景中深度估计不准确和结构退化的问题，在Waymo数据集上比最先进方法提升0.8 dB PSNR。
...
Result: 在Waymo Open Dataset上实现了0.8 dB PSNR的提升，在远场和近场区域都获得了更好的深度保真度。

Conclusion: 该框架将散焦物理与多视图几何约束相统一，为城市环境中复杂深度分层提供了可扩展的解决方案，桥接了物理成像原理和学习型深度正则化。
...
### [66] [FOUND: Fourier-based von Mises Distribution for Robust Single Domain Generalization in Object Detection](https://arxiv.org/abs/2511.10352)
...
### [67] [DermAI: Clinical dermatology acquisition through quality-driven image collection for AI classification in mobile](https://arxiv.org/abs/2511.10367)
...
TL;DR: DermAI是一个基于智能手机的轻量级应用，能够在常规诊疗中实时捕捉、标注和分类皮肤病变，通过设备端质量检查和本地模型适应解决AI皮肤病学中的偏见数据集和图像质量问题。


<details>
  <summary>Details</summary>
Motivation: AI皮肤病学的应用受到偏见数据集、图像质量多变和验证有限的限制，需要开发能够适应临床实际需求、处理多样化数据的解决方案。

Method: 开发DermAI智能手机应用，支持实时皮肤病变捕获、标注和分类，集成设备端质量检查和本地模型适应功能，构建包含多种肤色、种族和来源设备的临床数据集。

Result: 初步实验显示，基于公共数据集训练的模型无法泛化到DermAI样本，而使用本地数据微调后性能得到提升。
...
### [68] [SHRUG-FM: Reliability-Aware Foundation Models for Earth Observation](https://arxiv.org/abs/2511.10370)
...
TL;DR: SHRUG-FM是一个可靠性感知预测框架，通过整合输入空间OOD检测、嵌入空间OOD检测和任务特定预测不确定性三个信号，解决地球观测地理空间基础模型在预训练数据不足环境中的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型在预训练数据不足的环境中往往表现不可靠，需要开发能够识别和量化这种不可靠性的方法，以在气候敏感应用中实现更安全的部署。

Method: 提出SHRUG-FM框架，整合三个互补信号：输入空间的分布外检测、嵌入空间的分布外检测和任务特定预测不确定性，应用于烧伤疤痕分割任务。

Result: SHRUG-FM显示OOD分数与特定环境条件下的较低性能相关，基于不确定性的标志有助于丢弃许多表现不佳的预测。失败集中在低海拔区域和大河流域等特定地理区域，可能由于预训练数据代表性不足。
...
### [69] [Fragile by Design: On the Limits of Adversarial Defenses in Personalized Generation](https://arxiv.org/abs/2511.10382)
*Zhen Chen,Yi Zhang,Xiangyu Yin,Chengxuan Qin,Xingyu Zhao,Xiaowei Huang,Wenjie Ruan*

Main category: cs.CV

TL;DR: 本文分析了当前针对DreamBooth等个性化AI应用的反向隐私保护方法的局限性，发现现有防御方法存在可感知的人工痕迹和脆弱性问题，容易被简单图像滤波器移除，提出了AntiDB_Purify评估框架来系统评估现有防御方法在真实净化威胁下的有效性。


<details>
  <summary>Details</summary>
Motivation: DreamBooth等个性化AI应用虽然能生成定制内容，但存在面部身份泄露的隐私风险。现有防御方法如Anti-DreamBooth通过注入对抗性扰动来防止个性化成功，但这些方法存在两个被忽视的关键限制。

Method: 提出了AntiDB_Purify评估框架，系统评估现有防御方法在传统图像滤波器和对抗性净化等真实净化威胁下的有效性。

Result: 结果显示，当前所有防御方法在净化威胁下都无法保持其保护效果，现有防御方法提供了虚假的安全感。
...
### [70] [SAMIRO: Spatial Attention Mutual Information Regularization with a Pre-trained Model as Oracle for Lane Detection](https://arxiv.org/abs/2511.10385)
...
Method: 提出SAMIRO方法，使用空间注意力互信息正则化，结合预训练模型作为Oracle，在保持领域无关空间信息的同时从预训练模型转移知识。

Result: 在CULane、Tusimple和LLAMAS等主要基准测试上进行广泛实验，结果表明SAMIRO在不同模型和数据集上都能持续提升性能。

Conclusion: SAMIRO具有即插即用特性，可以集成到各种先进的车道线检测方法中，有效提升检测性能，代码将在发表后公开。
...
Conclusion: MonkeyOCR v1.5通过统一的视觉语言框架和专门的表格处理模块，有效解决了复杂文档解析的挑战，为文档智能应用提供了可靠的基础技术。
...
TL;DR: 本文提出了一种结合YOLOMS目标检测模型与大型语言模型的风力涡轮机故障智能分析框架，通过多尺度检测和滑动窗口裁剪增强特征提取，使用轻量级KV映射模块将视觉输出转换为结构化文本，再由领域调优的LLM进行语义推理生成可解释的故障分析和维护建议。
...
Method: 集成YOLOMS多尺度检测和滑动窗口裁剪技术，结合轻量级KV映射模块将检测结果转换为结构化文本表示，再使用领域调优的LLM进行语义推理。
...
TL;DR: 本文提出了GT++算法和3DFETUS深度学习模型，用于从3D胎儿超声体积中自动定位标准面部平面，解决了传统方法中由于胎儿运动、方向变化和操作者依赖导致的挑战。
...
Result: 该方法实现了每平面平均平移误差4.13毫米和平均旋转误差7.93度，在3D超声体积上优于其他最先进方法。临床评估进一步证实了GT++和3DFETUS的有效性，显示出平面估计精度的统计学显著改进。

Conclusion: 所提出的方法能够有效解决胎儿超声面部评估中的挑战，通过自动化和标准化平面定位，提高了准确性和一致性，具有重要的临床应用价值。
...
TL;DR: 该论文介绍了一个用于自动检测实验室啮齿动物惊厥发作事件的视频数据集RodEpil，包含10,101个阴性样本和2,952个阳性样本，使用TimeSformer视频分类器实现了97%的F1分数。
...
Method: 使用TimeSformer基于变换器的视频分类器，采用五折交叉验证和严格的基于受试者的数据划分。

Result: TimeSformer架构能够区分惊厥发作和正常活动，平均F1分数达到97%。

Conclusion: RodEpil数据集和基线代码已公开发布，支持临床前癫痫研究中基于视频的非侵入性监测研究。
...
TL;DR: 提出了一种基于语义分割的组织学信息分块方法(HIT)，通过提取前列腺癌组织中的腺体作为生物学意义明确的输入补丁，用于多实例学习和表型分析，提高了预测准确性和可解释性。
...
Method: 使用语义分割从全玻片图像中提取腺体作为输入补丁，应用于多实例学习。在ProMPT队列的137个样本上训练，在ICGC-C和TCGA-PRAD队列的760个WSI中提取了38万个腺体。

Result: 腺体级别Dice得分为0.83±0.17；将检测上皮-间质转化和MYC相关基因拷贝数变异的多实例学习模型AUC提高了10%；识别出15个腺体簇，其中多个与癌症复发、致癌突变和高Gleason评分相关。

Conclusion: HIT通过关注生物学意义结构进行特征提取，提高了多实例学习预测的准确性和可解释性，同时简化了计算。
...
<details>
  <summary>Details</summary>
Motivation: 降低研究人员和实践者使用SRGAN进行实验的门槛，提供可复现的模型比较和超分辨率管道部署。

Method: 通过简洁的配置文件暴露生成器、判别器、损失函数和训练计划，无需修改模型代码即可切换架构、尺度因子和波段设置。

Result: 框架提供了现成的遥感场景配置、对抗训练的合理默认设置，以及日志记录、验证和大场景推理的内置钩子。
...
<details>
  <summary>Details</summary>
Motivation: 2型糖尿病早期检测至关重要，但胰腺表面分叶度在T2DM患者中的作用尚未充分研究。本研究旨在开发自动化方法，利用CT成像生物标志物进行T2DM机会性筛查。

Method: 使用四种深度学习模型在584名患者内部数据集上分割胰腺，自动检测胰腺表面分叶度，并开发多变量模型利用CT生物标志物预测T2DM。
...
Conclusion: 胰腺表面分叶度可用于2型糖尿病筛查，并可能有助于预测T2DM的早期发病。
...
TL;DR: SPOT是一个用于Vision Transformers的早期冗余token检测框架，通过分析token嵌入、交互和注意力动态来识别重要性低的token，实现计算效率提升而不牺牲性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers的计算需求随token数量呈二次方增长，通过紧凑的注意力表示来早期检测和减少不重要的token可以显著提高计算效率。

Method: SPOT利用token嵌入、交互和跨层注意力动态来推断token重要性，采用轻量级预测器，可插入各种ViT架构中，学习生成特定输入的token优先级排序。

Result: 实证评估显示，与标准ViT相比，SPOT实现了高达40%的计算效率提升，同时保持甚至提高了准确性。

Conclusion: SPOT提供了一种上下文感知和可解释的token相关性检测方法，支持不同资源约束下的性能水平，显著提高了ViT的计算效率。
...
TL;DR: 提出Separate-then-Map策略，通过专门的信息映射机制桥接分别建模的组件，解决单目视频中动态人体与真实环境交互重建的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么整体建模动态场景忽视人体独特运动特征，要么分别建模场景和背景但忽略组件间信息交换，导致重建不完整、空间不一致和边界视觉伪影。

Method: 采用Separate-then-Map策略，为每个高斯属性使用共享变换函数统一分别建模的组件，避免详尽成对交互的同时确保人与环境的空间和视觉一致性。

Result: 在单目视频数据集上的广泛实验表明，StM在视觉质量和渲染精度上显著优于现有最先进方法，特别是在具有挑战性的人-场景交互边界处。

Conclusion: 提出的Separate-then-Map策略通过信息映射机制有效解决了分别建模组件间的协调问题，实现了更高质量和准确性的动态人体与环境交互重建。
...
TL;DR: 该论文提出了一个评估文本到图像模型多样性的框架，通过评估个体概念及其变化因素来系统化衡量模型输出多样性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成质量上有所提升，但输出往往缺乏多样性，生成同质化结果，需要建立稳健的多样性评估方法。

Method: 开发了包含三个关键贡献的框架：(1)用于细致多样性评估的人类评估模板；(2)涵盖多样化概念及其变化因素的精选提示集；(3)通过二项检验比较模型的人类标注方法。

Result: 严格比较了多种图像嵌入进行多样性测量，能够对文本到图像模型按多样性进行排序，并识别出模型特别困难的类别。

Conclusion: 该研究提供了稳健的方法论和见解，为改进文本到图像模型多样性和指标开发铺平了道路。
...
### [81] [A Style is Worth One Code: Unlocking Code-to-Style Image Generation with Discrete Style Space](https://arxiv.org/abs/2511.10555)
*Huijie Liu,Shuhao Cui,Haoxiang Cao,Shuai Ma,Kai Wu,Guoliang Kang*

Main category: cs.CV

TL;DR: 本文提出了一种名为CoTyle的创新方法，通过数值风格代码实现风格到图像的生成，解决了现有方法在风格一致性、创造性和复杂风格表示方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法依赖冗长文本提示、参考图像或参数微调，但存在风格不一致、创造力有限和复杂风格表示的问题。本文旨在通过数值代码实现新颖且一致的视觉风格生成。
...
Result: 大量实验验证CoTyle能有效将数值代码转化为风格控制器，证明一个风格确实值得一个代码。该方法提供了无与伦比的简单性和多样性，从最小输入解锁了大量可复现的风格空间。

Conclusion: CoTyle是首个开源的代码到风格图像生成方法，通过数值风格代码实现了新颖、一致的视觉风格生成，填补了学术界在该领域的空白。
...
TL;DR: OmniVGGT是一个能够有效利用任意数量辅助几何模态的3D基础模型框架，通过GeoAdapter编码深度和相机参数，使用零初始化卷积注入几何信息，同时提出随机多模态融合策略，在深度估计、多视图立体和相机姿态估计等任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有的3D基础模型大多仅使用RGB输入，忽略了容易获得的几何线索（如相机内参、位姿和深度图），导致性能受限。

Method: 提出GeoAdapter模块，使用零初始化卷积将深度和相机参数编码到空间基础模型中；设计随机多模态融合策略，在训练时随机采样模态子集，实现任意数量模态输入的测试能力。

Result: 在单目/多视图深度估计、多视图立体和相机姿态估计任务上优于现有方法，即使仅使用RGB输入也能达到SOTA；集成到VLA模型中在机器人任务上实现持续增益。

Conclusion: OmniVGGT能够有效利用几何线索提升3D视觉任务性能，同时保持推理效率，为实际应用提供了实用解决方案。
...
TL;DR: 本文提出了一种新颖的多任务全局-局部OBIA-Mamba（MSOM）方法，用于增强Sentinel-2影像的土地利用和土地覆盖分类，通过结合超像素作为Mamba令牌、双分支CNN-Mamba架构和多任务优化框架，显著提高了分类精度和细节保留。


<details>
  <summary>Details</summary>
Motivation: Sentinel-2影像的土地利用和土地覆盖分类面临空间异质性、上下文信息和特征模糊性等关键数据挑战，需要开发更有效的分类方法来提高环境监测应用的准确性。

Method: 设计了三项关键技术：1）基于对象的图像分析Mamba模型，使用超像素作为Mamba令牌减少冗余计算；2）全局-局部双分支CNN-Mamba架构，联合建模局部空间细节和全局上下文信息；3）多任务优化框架，采用双损失函数平衡局部精度与全局一致性。

Result: 在加拿大阿尔伯塔省的Sentinel-2影像上进行测试，与多种先进分类方法比较，结果表明所提方法获得了更高的分类精度和更精细的细节保留。

Conclusion: 提出的MSOM方法通过创新的OBIA-Mamba设计、全局-局部双分支架构和多任务优化，有效解决了Sentinel-2分类中的关键挑战，在精度和细节方面均优于现有最先进方法。
...
TL;DR: 本文提出自一致性采样（SCS）方法，解决多模态大语言模型中基于结果奖励的强化学习存在的不可靠轨迹问题，通过在推理过程中引入视觉扰动和轨迹重采样来评估推理一致性。


<details>
  <summary>Details</summary>
Motivation: 在多模态推理基准测试中，基于结果奖励的强化学习面临一个被忽视的问题：即使推理链存在错误，只要最终猜对答案，就会获得与正确推理相同的奖励，这导致模型学习不可靠的推理轨迹。

Method: 提出自一致性采样（SCS）：对每个问题（i）引入小的视觉扰动，（ii）重复截断和重采样初始轨迹，通过轨迹间的一致性产生可微的一致性分数，在策略更新时降低不可靠轨迹的权重。

Result: 在Qwen2.5-VL-7B-Instruct模型上，将SCS集成到RLOO、GRPO和REINFORCE++系列中，在六个多模态基准测试上准确率提升高达7.7个百分点，计算开销可忽略。在Qwen2.5-VL-3B-Instruct和InternVL3-8B模型上也取得了显著提升。
...
### [85] [Omnilingual ASR: Open-Source Multilingual Speech Recognition for 1600+ Languages](https://arxiv.org/abs/2511.09690)
...
TL;DR: Omnilingual ASR是一个大规模、可扩展的自动语音识别系统，旨在解决全球7000多种语言中大多数缺乏ASR支持的问题。该系统通过7B参数的自监督预训练学习鲁棒的语音表示，采用编码器-解码器架构实现零样本泛化，覆盖1600多种语言，其中500多种是首次获得ASR支持。


<details>
  <summary>Details</summary>
Motivation: 当前ASR技术在高资源语言中发展迅速，但全球7000多种语言中的大多数缺乏支持，扩展ASR覆盖范围成本高昂且受架构限制，同时缺乏社区协作会引发伦理问题。

Method: 结合大规模多样化训练语料库，采用7B参数的自监督预训练学习鲁棒语音表示，引入编码器-解码器架构（受LLM启发）实现零样本泛化，通过社区合作收集补偿性本地录音数据。

Result: 系统扩展到1600多种语言，其中500多种是首次获得ASR支持，在低资源条件下相比先前系统有显著提升，展现出强大的泛化能力。提供从300M到7B的模型系列以适应不同设备需求。

Conclusion: Omnilingual ASR通过开源模型和工具降低了研究者和社区的参与门槛，促进了新的参与形式，同时考虑了伦理考量和社会影响。
...
### [86] [Order Matters: Rethinking Prompt Construction in In-Context Learning](https://arxiv.org/abs/2511.09700)
*Warren Li,Yiqian Wang,Zihan Wang,Jingbo Shang*

Main category: cs.CL

TL;DR: 研究发现，在上下文学习中，示例排序对性能的影响与示例选择相当重要，这与传统假设相悖。通过系统实验证明，不同排序带来的性能差异与使用完全不同示例集相当，且可通过开发集识别强排序策略。


<details>
  <summary>Details</summary>
Motivation: 重新审视上下文学习中关于示例选择比排序更重要的传统假设，通过实证研究验证排序对性能的实际影响程度。

Method: 在分类和生成任务上进行控制实验，使用多个开源模型家族（0.5B到27B参数）和GPT-5，系统比较示例选择和排序对性能的影响。

Result: 发现不同示例排序带来的性能方差与使用完全不同示例集相当，且通过开发集可识别强排序策略，性能接近基于测试标签选择的最佳排序。
...
### [87] [Contextual morphologically-guided tokenization for Latin encoder models](https://arxiv.org/abs/2511.09709)
*Marisa Hudspeth,Patrick J. Burns,Brendan O'Connor*

Main category: cs.CL

TL;DR: 本文研究针对拉丁语的形态感知分词方法，发现形态引导的分词在四个下游任务中提升性能，特别是在领域外文本上表现更佳，证明了语言资源对提升形态复杂语言建模的效用。


<details>
  <summary>Details</summary>
Motivation: 标准分词方法通常优先考虑信息理论目标（如高压缩和低生育率）而非语言目标（如形态对齐），这对形态丰富的语言来说不够理想。拉丁语作为形态丰富的语言，在预训练数据方面属于中等资源，但在词汇资源方面属于高资源，这一区别在低资源语言建模讨论中常被忽视但至关重要。

Method: 研究针对拉丁语的形态感知分词方法，利用该语言丰富的形态学特征和词汇资源来改进分词过程。

Result: 形态引导的分词在四个下游任务中提高了整体性能，在领域外文本上的性能提升最为显著，表明模型具有改进的泛化能力。
...
### [88] [How Small Can You Go? Compact Language Models for On-Device Critical Error Detection in Machine Translation](https://arxiv.org/abs/2511.09748)
*Muskaan Chopra,Lorenz Sparrenberg,Sarthak Khanna,Rafet Sifa*

Main category: cs.CL

TL;DR: 研究发现10亿参数左右的模型在机器翻译关键错误检测任务中具有最佳的质量-效率平衡，Gemma-3-1B模型在保持低计算成本的同时达到MCC=0.77的检测性能。
...
Result: Gemma-3-1B提供了最佳的质量-效率权衡，在SynCED-EnDe-2025数据集上达到MCC=0.77和F1-ERR=0.98，在MacBook Pro M4 Pro上保持400ms单样本延迟。Qwen-3-1.7B获得最高绝对MCC但计算成本更高。

Conclusion: 紧凑的指令调优LLM结合轻量级校准和小样本监督可以提供可信赖的本地关键错误检测，实现现实翻译流程中私密、低成本的错误筛查。
...
### [89] [Predicate-Argument Structure Divergences in Chinese and English Parallel Sentences and their Impact on Language Transfer](https://arxiv.org/abs/2511.09796)
...
<details>
  <summary>Details</summary>
Motivation: 跨语言自然语言处理在低资源环境中具有重要价值，但语言差异（特别是类型学上距离较远的语言）阻碍了语言迁移。本文旨在分析中英文谓词-论元结构的差异，为跨语言NLP应用提供指导。

Method: 通过分析中英文平行句子中的谓词-论元结构，提出结构差异分类方法，并进行注释投影实验，以其中一个语言作为源语言向对应平行句子投影注释。

Result: 分析结果显示语言迁移具有明显的不对称性，源语言的选择对迁移学习应用至关重要。

Conclusion: 在提出跨语言NLP科学主张之前，需要仔细研究语言迁移的不对称性，特别是在选择源语言时需特别关注这一方面。
...
### [90] [TARG: Training-Free Adaptive Retrieval Gating for Efficient RAG](https://arxiv.org/abs/2511.09803)
...
<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)虽然提高了事实准确性，但为每个查询都进行检索会降低质量、增加token数量和延迟。需要一种智能的检索决策机制来平衡准确性和效率。

Method: 提出TARG方法：从基础模型的短无上下文草稿中，通过前缀logits计算三种轻量级不确定性分数（平均token熵、top-1/top-2 logit间隙的边际信号、少量随机前缀的小N方差），仅在分数超过阈值时触发检索。

Result: 在NQ-Open、TriviaQA和PopQA数据集上，相比Always-RAG，TARG在保持或提高EM/F1的同时，减少了70-90%的检索次数，降低了端到端延迟，且开销接近Never-RAG。

Conclusion: TARG有效提升了准确性与效率的权衡边界，边际信号在现代指令调优LLMs下是稳健的默认选择，小N方差提供了保守的预算优先替代方案。
...
### [91] [Khmer Spellchecking: A Holistic Approach](https://arxiv.org/abs/2511.09812)
*Marry Kong,Rina Buoy,Sovisal Chenda,Nguonly Taing*

Main category: cs.CL

TL;DR: 本文提出了一种解决高棉语拼写检查问题的整体方法，通过整合子词分割、命名实体识别、字形到语音转换和语言模型，实现了94.4%的最新准确率。


<details>
  <summary>Details</summary>
Motivation: 高棉语拼写检查面临词汇表与分词模型不匹配、单词多种书写形式、复合词灵活构成以及缺乏命名实体识别模型等挑战，现有解决方案无法充分应对这些问题。

Method: 整合高棉语子词分割、命名实体识别、字形到语音转换和语言模型，识别潜在修正候选词并排序最佳候选。

Result: 实验结果显示，所提方法在高棉语拼写检查中达到94.4%的最新准确率，优于现有解决方案。
...
### [92] [Improving Graduate Outcomes by Identifying Skills Gaps and Recommending Courses Based on Career Interests](https://arxiv.org/abs/2511.09819)
*Rahul Soni,Basem Suleiman,Sonit Singh*

Main category: cs.CL

TL;DR: 本文提出了一种结合数据分析和机器学习算法的课程推荐系统，旨在帮助学生选择与行业趋势相符的课程，通过用户偏好和学术标准提供个性化建议。
...
Method: 采用数据挖掘和协同过滤技术分析历史课程和个人职业目标，结合机器学习算法构建算法框架，并通过迭代原型和用户反馈优化前端界面设计。

Result: 开发了一个用户友好的课程推荐系统，能够根据行业趋势和用户偏好提供个性化课程建议，系统经过用户反馈优化后能有效满足目标用户需求。

Conclusion: 该课程推荐系统可作为学生、教师和职业顾问的有用工具，促进终身学习和职业发展，有望改善大学毕业生的就业成果。
...
### [93] [Answering Students' Questions on Course Forums Using Multiple Chain-of-Thought Reasoning and Finetuning RAG-Enabled LLM](https://arxiv.org/abs/2511.09831)
*Neo Wang,Sonit Singh*

Main category: cs.CL

TL;DR: 该论文提出了一种基于大型语言模型和检索增强生成(RAG)方法的问答系统，用于解决在线课程论坛中学生问题响应不及时和重复问题过多的问题。


<details>
  <summary>Details</summary>
Motivation: 随着在线课程学生数量的增加，课程论坛面临两个主要挑战：学生问题无法及时得到回复，以及教师需要处理大量重复性问题。

Method: 使用开源大型语言模型(LLM)，在相关课程数据集上进行微调，并采用检索增强生成(RAG)方法从包含所有课程内容的本地知识库中检索相关文档，同时集成多链思维推理来减轻LLM的幻觉问题。

Result: 在HotpotQA数据集上的实验结果表明，采用RAG方法的微调LLM在问答任务上表现出色。

Conclusion: 基于RAG方法的微调LLM问答系统能够有效解决课程论坛中的问题响应挑战，具有良好的性能表现。
...
### [94] [TermGPT: Multi-Level Contrastive Fine-Tuning for Terminology Adaptation in Legal and Financial Domain](https://arxiv.org/abs/2511.09854)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业术语表示上存在各向同性问题，导致领域特定术语（尤其是法律和金融领域）的区分能力不足，影响下游任务性能。

Method: 构建句子图捕捉语义和结构关系，基于上下文和拓扑线索生成语义一致但具有区分性的正负样本，设计句子级和词元级的多级对比学习方法。

Result: 在构建的首个金融术语数据集上，TermGPT在法律和金融领域的术语区分任务中优于现有基线方法。

Conclusion: TermGPT框架有效提升了LLM在专业术语表示上的区分能力，为法律和金融等专业领域的下游任务提供了更好的基础。
...
### [95] [In-Token Rationality Optimization: Towards Accurate and Concise LLM Reasoning via Self-Feedback](https://arxiv.org/abs/2511.09865)
*Mingye Zhu,Yi Liu,Zheren Fu,Quan Wang,Yongdong Zhang*

Main category: cs.CL

TL;DR: InTRO是一个新的框架，通过令牌级探索和自我反馈来解决大型语言模型在思维链推理训练中的挑战，避免了单一黄金答案的局限性，提高了准确性和简洁性。


<details>
  <summary>Details</summary>
Motivation: 解决监督微调在单一黄金答案上惩罚有效替代方案的问题，以及强化学习在可验证奖励中信用分配困难和计算成本高的问题。

Method: 利用校正因子（基于生成策略与其答案条件对应策略之间的信息差异估计的令牌级重要性权重）进行信息性下一个令牌选择，实现令牌级探索和自我反馈。

Result: 在六个数学推理基准测试中，InTRO始终优于其他基线方法，将解决方案准确性相对于基础模型提高了高达20%，思维链更加简洁。
...
### [96] [EnchTable: Unified Safety Alignment Transfer in Fine-tuned Large Language Models](https://arxiv.org/abs/2511.09880)
*Jialin Wu,Kecen Li,Zhicong Huang,Xinfeng Li,Xiaofeng Wang,Cheng Hong*

Main category: cs.CL

TL;DR: EnchTable是一个新颖框架，通过NTK安全向量蒸馏和干扰感知合并技术，在微调LLMs时保持安全对齐，防止有害输出，同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs微调过程中安全对齐系统性退化的问题，防止有害输出风险增加。

Method: 使用基于神经切向核(NTK)的安全向量蒸馏方法解耦安全约束与任务推理，结合干扰感知合并技术平衡安全性和实用性。

Result: 在11个数据集上评估显示，EnchTable实现了更低的不安全率、更高的实用性得分，并能抵抗静态和动态越狱攻击。

Conclusion: EnchTable能有效维持LLMs的安全对齐，具有通用性和部署友好性，优于现有参数修改和推理时对齐方法。
...
### [97] [HI-TransPA: Hearing Impairments Translation Personal Assistant](https://arxiv.org/abs/2511.09915)
...
TL;DR: HI-TransPA是一个面向听力障碍人群的指令驱动音频-视觉个人助手，融合模糊语音和高帧率唇部动态，在单一多模态框架内实现翻译和对话功能。


<details>
  <summary>Details</summary>
Motivation: 为听力障碍人群的日常交流提供统一灵活解决方案，将Omni-Model范式引入辅助技术领域。
...
### [98] [MINDS: A Cross-cultural Dialogue Corpus for Social Norm Classification and Adherence Detection](https://arxiv.org/abs/2511.09918)
*Pritish Sahu,Anirudh Som,Dimitra Vergyri,Ajay Divakaran*

Main category: cs.CL

TL;DR: 提出了Norm-RAG框架，这是一个基于检索增强的智能系统，用于在多轮对话中进行细致的社会规范推理。该方法结合了话语级属性和结构化规范文档，提高了规范检测和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 社会规范是隐性的、基于文化的期望，指导人际沟通。与事实性常识不同，规范推理是主观的、依赖于语境的，且在不同文化间存在差异，这对计算模型提出了挑战。现有工作主要针对孤立话语或合成对话，难以捕捉真实对话的流动性和多轮特性。

Method: Norm-RAG框架建模话语级属性（包括交际意图、说话者角色、人际框架和语言线索），并通过新颖的语义分块方法检索结构化规范文档进行基础。同时引入了MINDS双语数据集，包含31个多轮普通话-英语和西班牙语-英语对话。

Result: 实验表明，Norm-RAG提高了规范检测和泛化能力，在文化适应性和社交智能对话系统方面表现出改进的性能。

Conclusion: Norm-RAG框架能够进行可解释和上下文感知的规范遵守和违反推理，为多语言对话中的社会规范理解提供了有效解决方案。
...
### [99] [Leveraging Large Language Models for Identifying Knowledge Components](https://arxiv.org/abs/2511.09935)
*Canwen Wang,Jionghao Lin,Kenneth R. Koedinger*

Main category: cs.CL

TL;DR: 本研究通过扩展LLM自动生成知识组件的方法，结合语义相似度合并策略，改进了自适应学习系统中知识组件的自动化识别过程，显著减少了冗余标签并提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 知识组件(KCs)是自适应学习系统的基础，但手动识别存在瓶颈。虽然大语言模型(LLMs)提供了自动化可能，但先前研究存在数据集小、产生冗余标签的问题。

Method: 首先使用GPT-4o-mini在646个选择题数据集上扩展"模拟教科书"提示策略，然后提出基于余弦相似度的语义相似KC标签合并方法。

Result: 初始自动化方法表现差于专家设计模型(RMSE 0.4285 vs 0.4206)，生成过多KCs(569 vs 101)。使用余弦相似度阈值0.8的合并策略将KCs减少到428个，RMSE提升至0.4259。
...
### [100] [REAP: Enhancing RAG with Recursive Evaluation and Adaptive Planning for Multi-Hop Question Answering](https://arxiv.org/abs/2511.09966)
*Yijie Zhu,Haojie Zhou,Wanting Hong,Tailin Liu,Ning Wang*

Main category: cs.CL

TL;DR: REAP方法通过子任务规划器和事实提取器模块，在检索增强生成中实现递归评估和自适应规划，显著提升多跳推理任务的性能。
...
Method: 提出REAP框架，包含子任务规划器(SP)和事实提取器(FE)模块。SP维护全局视角，指导整体推理方向并评估任务状态；FE对检索内容进行细粒度分析以提取可靠答案和线索。两个模块协同工作，增量构建逻辑一致的全局知识表示。

Result: 在多个公开多跳数据集上的实验表明，该方法在域内和域外设置下均显著优于现有RAG方法，验证了其在复杂多跳推理任务中的有效性。

Conclusion: REAP通过结构化子任务和事实维护、递归评估和自适应规划机制，有效提升了多跳推理的可靠性和可追溯性，为复杂推理任务提供了有效解决方案。
...
### [101] [NumPert: Numerical Perturbations to Probe Language Models for Veracity Prediction](https://arxiv.org/abs/2511.09971)
*Peter Røysland Aarnes,Vinay Setty*

Main category: cs.CL

TL;DR: 本文系统评估了大型语言模型在数值声明验证任务上的表现，发现即使在最先进的专有系统中，某些扰动下准确率下降高达62%，且没有模型在所有条件下都表现稳健。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在知识密集型任务上表现优异，但在数值推理方面存在明显不足，因此需要系统评估其在数值事实核查任务中的鲁棒性。

Method: 使用受控扰动（包括标签翻转探针）对最先进模型进行系统评估，测试其在数值声明和证据对上的准确性预测能力。

Result: 所有模型在特定扰动下都出现准确率显著下降（最高62%），没有模型在所有条件下保持稳健。增加上下文长度通常降低准确率，但当扩展上下文包含扰动示例时，大多数模型能大幅恢复性能。

Conclusion: 当前语言模型在数值事实核查方面存在严重局限性，鲁棒性仍然是一个未解决的挑战。
...
### [102] [Modeling Uncertainty Trends for Timely Retrieval in Dynamic RAG](https://arxiv.org/abs/2511.09980)
*Bo Li,Tian Tian,Zhenghua Xu,Hao Cheng,Shikun Zhang,Wei Ye*

Main category: cs.CL

TL;DR: 本文提出了一种名为熵趋势约束（ETC）的训练无关方法，通过建模token级不确定性的动态变化来确定最佳检索时机，在动态检索增强生成（RAG）中实现更早和更精确的检索。


<details>
  <summary>Details</summary>
Motivation: 动态RAG允许LLMs按需获取外部知识，但现有方法基于低token级置信度触发检索，可能导致错误传播后才进行干预。需要更早检测不确定性趋势来确定最佳检索时机。

Method: ETC方法利用熵序列的一阶和二阶差分来检测新兴的不确定性趋势，无需训练即可确定最佳检索时机，实现更早和更精确的检索。

Result: 在六个QA基准测试和三个LLM骨干上的实验表明，ETC始终优于强基线方法，同时减少了检索频率。在领域特定场景中表现尤为有效，展现出强大的泛化能力。

Conclusion: 趋势感知的不确定性建模能够产生更有效的检索时机。ETC方法即插即用、模型无关，可轻松集成到现有的解码流程中。
...
### [103] [Language Drift in Multilingual Retrieval-Augmented Generation: Characterization and Decoding-Time Mitigation](https://arxiv.org/abs/2511.09984)
*Bo Li,Zhenghua Xu,Rui Xie*

Main category: cs.CL

TL;DR: 本文研究了多语言检索增强生成(RAG)中的输出语言漂移问题，发现这是由于解码器层面的崩溃导致的，并提出了一种无需训练的解码策略Soft Constrained Decoding(SCD)来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 在多语言RAG中，当检索到的证据与用户查询和上下文示例的语言不同时，模型经常会出现语言漂移现象，在推理密集型解码（如思维链生成）中尤为明显。

Method: 提出Soft Constrained Decoding(SCD)策略，这是一种轻量级、无需训练的解码方法，通过对非目标语言token施加惩罚来引导生成朝向目标语言。
...
### [104] [FinNuE: Exposing the Risks of Using BERTScore for Numerical Semantic Evaluation in Finance](https://arxiv.org/abs/2511.09997)
*Yu-Shiang Huang,Yun-Yu Lee,Tzu-Hsin Chou,Che Lin,Chuan-Ju Wang*

Main category: cs.CL

TL;DR: BERTScore在金融领域存在数值敏感性不足的问题，FinNuE诊断数据集揭示了该指标无法区分语义关键的数值差异。


<details>
  <summary>Details</summary>
Motivation: 发现BERTScore在金融文本中数值变化敏感性低的问题，因为金融领域数值精度直接影响语义（如区分2%收益与20%损失）。

Method: 构建FinNuE诊断数据集，包含收益电话会议、监管文件、社交媒体和新闻文章中的受控数值扰动。

Result: BERTScore无法区分语义关键的数值差异，经常为金融上不同的文本对分配高相似度分数。
...
### [105] [PustakAI: Curriculum-Aligned and Interactive Textbooks Using Large Language Models](https://arxiv.org/abs/2511.10002)
*Shivam Sharma,Riya Naik,Tejas Gawas,Heramb Patil,Kunal Korgaonkar*

Main category: cs.CL

TL;DR: 本文提出了PustakAI框架，用于设计和评估与印度NCERT课程对齐的问答数据集NCERT-QA，涵盖6-8年级英语和科学科目，并评估了多种提示技术和不同规模LLM在教育应用中的表现。
...
Method: 开发NCERT-QA数据集，将问答对分类为事实性、推理性和其他类型；使用元提示、少样本和思维链等提示技术；评估开源和高性能LLM在教育系统中的适用性。

Result: 分析了不同提示技术对课程结构的对齐效率，评估了Gemma3、Llama3.2、Nemotron-mini等开源模型以及Llama-4-Scout-17B、Deepseek-r1-70B等高性能模型作为教育工具的优势和局限。

Conclusion: PustakAI框架为课程对齐的LLM教育应用提供了系统评估方法，揭示了当前LLM在正式教育系统中的适用性和改进方向。
...
### [106] [ScaleFormer: Span Representation Cumulation for Long-Context Transformer](https://arxiv.org/abs/2511.10029)
...
Method: 将长输入分割为重叠块，使用参数自由的融合机制为每个块表示赋予其在文档中的结构感知，通过累积前后块上下文向量丰富边界表示。
...
### [107] [Do Language Models Associate Sound with Meaning? A Multimodal Study of Sound Symbolism](https://arxiv.org/abs/2511.10045)
...
Method: 使用LEX-ICON数据集（包含4种自然语言的8,052个单词和2,930个伪词），在文本（正字法和IPA）和音频输入下分析模型在25个语义维度上的表现，通过音素级注意力分数测量层级信息处理。

Result: 发现MLLMs具有与现有语言学研究一致的语音直觉，并且注意力模式显示模型关注标志性音素。

Conclusion: 研究在人工智能和认知语言学之间建立桥梁，首次提供了关于MLLMs可解释性方面语音象征性的大规模定量分析。
...
### [108] [ADI-20: Arabic Dialect Identification dataset and models](https://arxiv.org/abs/2511.10070)
...
<details>
  <summary>Details</summary>
Motivation: 扩展现有的阿拉伯语方言识别数据集，提供更全面的方言覆盖，支持方言识别系统的开发和评估。

Method: 使用ECAPA-TDNN预训练模型微调，以及Whisper编码器块结合注意力池化层和分类密集层的方法，研究训练数据量和模型参数数量对识别性能的影响。

Result: 仅使用30%原始训练数据时，F1分数仅有小幅下降，表明模型在数据效率方面表现良好。

Conclusion: 开源收集的数据和训练模型，支持方言识别研究的可重复性和进一步发展。
...
### [109] [Format Matters: The Robustness of Multimodal LLMs in Reviewing Evidence from Tables and Charts](https://arxiv.org/abs/2511.10075)
*Xanh Ho,Yun-Ang Wu,Sunisth Kumar,Florian Boudin,Atsuhiro Takasu,Akiko Aizawa*

Main category: cs.CL

TL;DR: 本文评估了12种多模态大语言模型在科学声明验证任务中的表现，发现模型在处理表格证据时表现更好，但在处理图表证据时表现较差，且小模型在跨模态泛化方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 随着提交的科学论文数量增加，需要能够协助审稿人评估研究声明的系统。当前多模态大语言模型在不同证据格式（表格和图表）下验证科学声明的鲁棒性是一个重要但未充分探索的挑战。

Method: 通过改编两个现有科学论文数据集，构建适合多模态声明验证任务的数据集，评估12种多模态大语言模型在表格和图表证据下的表现，并进行人工评估对比。

Result: 当前模型在处理表格证据时表现更好，但在图表证据上表现较差；人类在两种格式下都保持强劲表现；小于8B参数的多模态模型在表格和图表任务间的性能相关性较弱，表明跨模态泛化能力有限。
...
### [110] [ELYADATA & LIA at NADI 2025: ASR and ADI Subtasks](https://arxiv.org/abs/2511.10090)
...
<details>
  <summary>Details</summary>
Motivation: 参与NADI 2025阿拉伯语多方言语音处理竞赛，探索大预训练语音模型在阿拉伯语方言识别和多方言语音识别任务中的有效性。

Method: 方言识别：使用数据增强的微调Whisper-large-v3编码器；多方言ASR：为8种方言分别微调SeamlessM4T-v2 Large模型（埃及变体）。

Result: 方言识别：测试集准确率79.83%，排名第一；多方言ASR：平均WER 38.54%，CER 14.53%，排名第二。

Conclusion: 大预训练语音模型结合针对性微调在阿拉伯语语音处理中表现出色，验证了该方法的有效性。
...
### [111] [On the Military Applications of Large Language Models](https://arxiv.org/abs/2511.10093)
*Satu Johansson,Taneli Riihonen*

Main category: cs.CL

TL;DR: 本文探讨了自然语言处理和大语言模型在军事领域的应用，通过分析GPT模型的知识和商业云服务（如Microsoft Azure）的可行性，评估了军事应用的实现可能性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型（如GPT）和自然语言处理技术在军事领域的潜在应用价值，探索商业云服务如何支持这些应用的实现。

Method: 1. 通过询问GPT模型（Microsoft Copilot）了解其对军事应用的知识；2. 分析Microsoft Azure等商业云服务构建此类应用的可行性。

Result: 发现语言模型的摘要生成和内容生成特性可直接支持多种军事应用，其他功能也可能找到特定用途。

Conclusion: 大语言模型的总结和生成特性为军事应用提供了直接支持，商业云服务能够有效实现这些应用。
...
### [112] [Generalizing to Unseen Disaster Events: A Causal View](https://arxiv.org/abs/2511.10120)
...
### [113] [Beyond the Black Box: Demystifying Multi-Turn LLM Reasoning with VISTA](https://arxiv.org/abs/2511.10182)
*Yiran Zhang,Mingyang Lin,Mark Dras,Usman Naseem*

Main category: cs.CL

TL;DR: VISTA是一个基于Web的可视化交互系统，用于分析多轮推理任务中的大语言模型推理过程，通过可视化上下文影响和交互式修改对话历史来降低分析复杂性。
...
Conclusion: VISTA作为一个开源统一交互框架，有效促进了多轮推理任务中LLM推理过程的可视化分析。
...
### [114] [Text2SQL-Flow: A Robust SQL-Aware Data Augmentation Framework for Text-to-SQL](https://arxiv.org/abs/2511.10192)
*Qifeng Cai,Hao Liang,Chang Xu,Tao Xie,Wentao Zhang,Bin Cui*

Main category: cs.CL

TL;DR: Text2SQL-Flow是一个SQL感知的数据增强框架，通过六维度数据增强从少量种子数据生成大规模、语义有效且结构多样的Text-to-SQL对，构建了包含89,544个标注示例的高质量SQLFlow数据集。
...
Result: 构建了高质量的SQLFlow数据集；对于开源LLM，在相同数据预算下微调SQLFlow能持续提升基准测试性能；对于闭源LLM，引入掩码对齐检索方法，在结构感知示例匹配方面优于现有方法。
...
### [115] [EffiReason-Bench: A Unified Benchmark for Evaluating and Advancing Efficient Reasoning in Large Language Models](https://arxiv.org/abs/2511.10201)
*Junquan Huang,Haotian Wu,Yubo Gao,Yibo Yan,Junyan Zhang,Yonghua Hei,Song Dai,Jie Zhang,Puay Siew Tan,Xuming Hu*

Main category: cs.CL

TL;DR: EffiReason-Bench是一个统一的基准测试，用于评估高效推理方法，包括推理蓝图、动态执行和后处理优化三类方法。通过构建标准化的推理结构注释和提出E3-Score评估指标，在多个数据集上评估了7种方法和6个开源LLM，发现最优策略取决于模型规模、任务复杂度和架构。


<details>
  <summary>Details</summary>
Motivation: 现有LLM的思维链提示虽然推理能力强，但往往产生不必要的冗长解释，增加了成本并可能降低准确性。不同效率导向方法的公平比较受到碎片化评估实践的阻碍。

Method: 引入EffiReason-Bench统一基准，构建经过验证的思维链注释，提出E3-Score评估指标，在数学、常识和逻辑三个领域的4个数据集上评估7种方法和6个开源LLM。

Result: 实验表明没有单一方法在所有情况下都表现最优，最佳策略取决于骨干模型规模、任务复杂度和架构。

Conclusion: 需要根据具体场景选择高效推理方法，EffiReason-Bench为跨范式评估提供了统一框架和稳定评估指标。
...
### [116] [Persona-Aware Alignment Framework for Personalized Dialogue Generation](https://arxiv.org/abs/2511.10215)
...
<details>
  <summary>Details</summary>
Motivation: 主流个性化对话生成模型通常依赖基于角色对话数据的token级语言模型训练，这些方法倾向于忽视给定角色信息并生成通用回复。

Method: 采用两阶段训练方法：角色感知学习和角色对齐，配备易于使用的推理策略"选择然后生成"，在语义层面提高角色敏感度并生成更相关的回复。

Result: 通过大量实验证明，该框架优于许多最先进的个性化对话方法和大型语言模型。

Conclusion: PAL框架通过直接以角色对齐为训练目标，能够生成更角色相关的回复，在个性化对话生成任务中表现优异。
...
### [117] [LangGPS: Language Separability Guided Data Pre-Selection for Joint Multilingual Instruction Tuning](https://arxiv.org/abs/2511.10229)
*Yangfan Ye,Xiaocheng Feng,Xiachong Feng,Lei Huang,Weitao Ma,Qichen Hong,Yunfei Lu,Duyu Tang,Dandan Tu,Bing Qin*

Main category: cs.CL

TL;DR: LangGPS是一个轻量级的两阶段预选择框架，通过语言可分离性来指导多语言训练数据的选择，提高现有选择方法的有效性和泛化能力。
...
Result: 在6个基准测试和22种语言上的实验表明，LangGPS能显著提升现有选择方法的效果，特别是在理解任务和低资源语言上。高可分离性样本促进清晰语言边界的形成，低可分离性样本则作为跨语言对齐的桥梁。

Conclusion: 语言可分离性可作为多语言课程学习的有效信号，不同可分离性水平的样本交错训练能带来稳定且可泛化的收益，为多语言上下文中的数据效用提供了新视角。
...
### [118] [VocalNet-M2: Advancing Low-Latency Spoken Language Modeling via Integrated Multi-Codebook Tokenization and Multi-Token Prediction](https://arxiv.org/abs/2511.10232)
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端语音语言模型存在显著响应延迟问题，主要源于语音令牌的自回归生成和复杂流匹配模型的使用，需要开发更高效的实时交互应用模型。

Method: 提出VocalNet-M2模型，集成多码本分词器和多令牌预测策略，直接生成多码本语音令牌，消除流匹配模型的需求。
...
### [119] [MTR-DuplexBench: Towards a Comprehensive Evaluation of Multi-Round Conversations for Full-Duplex Speech Language Models](https://arxiv.org/abs/2511.10262)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单轮交互，忽视了多轮通信的复杂性以及指令遵循和安全性等关键能力，无法充分评估全双工语音语言模型的真实性能。

Method: 通过将连续的全双工会话分割为离散轮次，构建了MTR-DuplexBench基准，支持对话质量、对话动态、指令遵循和安全性四个维度的逐轮评估。

Result: 实验结果表明，当前的全双工语音语言模型在多轮对话和不同评估维度上难以保持一致的性能表现。

Conclusion: MTR-DuplexBench基准能够有效评估全双工语音语言模型在多轮对话中的综合性能，填补了现有评估方法的空白。
...
### [120] [Local Hybrid Retrieval-Augmented Document QA](https://arxiv.org/abs/2511.10297)
*Paolo Astrino*

Main category: cs.CL

TL;DR: 提出了一种结合语义理解和关键词精度的本地问答系统，解决了组织在采用云AI系统时面临的数据隐私与准确性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 组织在处理敏感文档时面临两难选择：采用基于云的AI系统会牺牲数据隐私，而本地处理虽然安全但准确性较差。需要一种既能保证数据隐私又能提供高准确性的解决方案。

Method: 结合语义理解和关键词精度两种互补的检索策略，在本地基础设施上运行，无需互联网访问，利用消费级硬件加速。

Result: 系统在复杂查询（法律、科学和对话文档）上实现了具有竞争力的准确性，同时将所有数据保留在本地机器上。
...
### [121] [Rectify Evaluation Preference: Improving LLMs' Critique on Math Reasoning via Perplexity-aware Reinforcement Learning](https://arxiv.org/abs/2511.10303)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖高质量监督微调来增强批判能力，但忽视了LLMs批判性能差的根本原因——不平衡的评估偏好。本文旨在量化并解决这一问题。

Method: 构建了One-to-many Problem-Solution基准来量化LLMs评估行为差异，通过统计偏好分析发现LLMs倾向于将低困惑度答案判断为正确。提出困惑度感知的强化学习算法Group Relative Policy Optimization来纠正这一偏好。

Result: 在自建的OPS基准和现有批判基准上的广泛实验证明了该方法的有效性。

Conclusion: 通过识别并纠正LLMs的不平衡评估偏好，能够显著提升其在多步数学推理中的批判能力。
...
### [122] [BhashaKritika: Building Synthetic Pretraining Data at Scale for Indic Languages](https://arxiv.org/abs/2511.10338)
...
TL;DR: 本研究系统性地研究了印度语言的多语言合成预训练数据生成与评估，构建了包含540B标记的BhashaKritika数据集，探索了基于文档、角色和主题的生成方法，并开发了模块化质量评估流程。
...
Conclusion: 研究为多语言合成数据生成提供了系统框架和最佳实践，特别适用于低资源语言环境，有助于改善语言模型在不同语言间的公平性。
...
### [123] [Knowledge Graphs Generation from Cultural Heritage Texts: Combining LLMs and Ontological Engineering for Scholarly Debates](https://arxiv.org/abs/2511.10354)
*Andrea Schimmenti,Valentina Pasqual,Fabio Vitali,Marieke van Erp*

Main category: cs.CL

TL;DR: ATR4CH是一个针对文化遗产文本的系统性五步方法，使用大语言模型从文化遗产文档中提取知识并构建知识图谱。该方法在真实性评估辩论案例研究中验证有效，能够以高准确率提取复杂文化遗产知识。


<details>
  <summary>Details</summary>
Motivation: 文化遗产文本包含丰富知识，但由于将非结构化论述转换为结构化知识图谱的挑战，难以系统查询。需要开发系统方法来协调基于LLM的提取与文化遗产本体。

Method: ATR4CH结合注释模型、本体框架和基于LLM的提取，通过迭代开发：基础分析、注释模式开发、管道架构、集成优化和综合评估。使用三个LLM（Claude Sonnet 3.7、Llama 3.3 70B、GPT-4o-mini）实现顺序管道。

Result: 成功提取复杂文化遗产知识：元数据提取F1为0.96-0.99，实体识别F1为0.7-0.8，假设提取F1为0.65-0.75，证据提取F1为0.95-0.97，论述表示G-EVAL为0.62。较小模型表现具有竞争力，支持成本效益部署。

Conclusion: ATR4CH是首个协调基于LLM提取与文化遗产本体的系统方法，提供了可复制的框架，可适应不同文化遗产领域和机构资源。虽然结果令人鼓舞，但后处理阶段需要人工监督。
...
### [124] [TruthfulRAG: Resolving Factual-level Conflicts in Retrieval-Augmented Generation with Knowledge Graphs](https://arxiv.org/abs/2511.10375)
*Shuyi Liu,Yuming Shang,Xi Zhang*

Main category: cs.CL

TL;DR: TruthfulRAG是首个利用知识图谱解决RAG系统中事实级知识冲突的框架，通过构建检索内容的知识图谱、基于查询的图检索和基于熵的过滤机制来精确定位冲突元素，提高生成内容的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着外部知识库的扩展和模型参数知识的过时，RAG系统面临检索外部信息与LLM内部知识冲突的挑战，现有方法在token或语义层面操作，难以全面理解事实差异。

Method: TruthfulRAG框架：1）从检索内容中系统提取三元组构建知识图谱；2）使用基于查询的图检索识别相关知识；3）采用基于熵的过滤机制精确定位冲突元素。
...
### [125] [Position: On the Methodological Pitfalls of Evaluating Base LLMs for Reasoning](https://arxiv.org/abs/2511.10381)
*Jason Chan,Zhixue Zhao,Robert Gaizauskas*

Main category: cs.CL

TL;DR: 该论文认为评估基础LLM的推理能力存在方法论问题，因为基础LLM的训练目标与推理评估的规范性质量（如正确性）存在根本性不匹配。


<details>
  <summary>Details</summary>
Motivation: 现有研究在评估基础LLM的推理能力时忽视了方法论问题，作者希望揭示基础LLM训练目标与推理评估标准之间的根本性不匹配。

Method: 通过分析基础LLM的预训练目标与推理评估标准之间的不匹配，展示基础LLM生成逻辑有效或无效结论只是语言模式统计可能性的副产品。

Result: 发现基础LLM的输出不能被视为其真实尝试给出正确答案，且关于基础LLM推理能力的结论不能推广到经过指令优化的后训练LLM。

Conclusion: 呼吁重新审视依赖这些假设的现有研究，并要求未来研究考虑这些方法论陷阱。
...
### [126] [DELICATE: Diachronic Entity LInking using Classes And Temporal Evidence](https://arxiv.org/abs/2511.10404)
*Cristian Santini,Sebastian Barzaghi,Paolo Sernani,Emanuele Frontoni,Mehwish Alam*

Main category: cs.CL

TL;DR: DELICATE是一种针对历史意大利语的神经符号实体链接方法，结合BERT编码器和Wikidata上下文信息，通过时间合理性和实体类型一致性选择知识库实体。ENEIDE是一个从19-20世纪文学和政治文本中半自动提取的多领域历史意大利语实体链接语料库。
...
Method: 提出DELICATE神经符号方法，结合BERT编码器与Wikidata上下文信息，利用时间合理性和实体类型一致性进行实体选择；构建ENEIDE多领域历史意大利语实体链接语料库。

Result: DELICATE在历史意大利语实体链接任务中优于其他模型，即使与具有数十亿参数的大型架构相比；其置信度分数和特征敏感性提供了比纯神经方法更可解释和可理解的结果。

Conclusion: DELICATE方法有效解决了历史意大利语实体链接的挑战，在性能和可解释性方面均表现出色，ENEIDE语料库为领域研究提供了重要资源。
...
### [127] [Analogical Structure, Minimal Contextual Cues and Contrastive Distractors: Input Design for Sample-Efficient Linguistic Rule Induction](https://arxiv.org/abs/2511.10441)
*Chunyang Jiang,Paola Merlo*

Main category: cs.CL

TL;DR: 本文提出了一种基于类比范式组织的计算方法，使轻量级模型仅用少量数据就能达到与大型语言模型相媲美的性能。


<details>
  <summary>Details</summary>
Motivation: 探索是否通过类比范式组织，使轻量级模型能够以最少的数据匹配大型语言模型的性能表现。

Method: 开发了一种计算方法，实现了三个认知启发原则：类比结构、对比学习和最小上下文线索。使用结构化完成任务进行测试，模型从类比模式中识别正确的句子补全。

Result: 在仅使用100个结构化英语使役/起始交替示例训练轻量级模型（BERT+CNN，50万参数）时，达到F1=0.95，优于零样本GPT-3（F1=0.87）。消融研究证实类比组织和对比结构提升了性能。
...
### [128] [Reasoning About Intent for Ambiguous Requests](https://arxiv.org/abs/2511.10453)
*Irina Saparina,Mirella Lapata*

Main category: cs.CL

TL;DR: 该论文提出了一种处理模糊请求的方法，通过生成多个解释-答案对来避免语言模型对模糊请求的单一错误解释。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理模糊请求时常常会隐含地承诺某种解释，这种意图误解会使用户感到沮丧并带来安全风险。

Method: 使用强化学习和定制奖励函数训练模型，以多个有效答案作为监督，在单个结构化响应中生成多个解释-答案对。

Result: 在对话问答和语义解析任务上的实验表明，该方法比基线方法实现了更高的有效答案覆盖率。人工评估证实预测的解释与其答案高度一致。

Conclusion: 该方法通过明确的解释提高了透明度，仅需一个生成步骤实现效率，并通过结构化输出格式支持下游应用。
...
### [129] [Exploring State Tracking Capabilities of Large Language Models](https://arxiv.org/abs/2511.10457)
*Kiamehr Rezaee,Jose Camacho-Collados,Mohammad Taher Pilehvar*

Main category: cs.CL

TL;DR: 本文分析了大型语言模型在状态跟踪任务上的表现，发现GPT-4和Llama3等新一代模型能够有效跟踪状态，而老一代模型在初始阶段后容易失败。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在状态跟踪任务上的能力，将状态跟踪组件从其他因素中分离出来进行专门分析。

Method: 基于三个明确定义的状态跟踪任务构建基准测试，分析LLMs在不同场景下的性能表现，包括使用思维链等机制。

Result: 新一代LLMs（GPT-4和Llama3）能够有效跟踪状态，特别是结合思维链机制时；而老一代模型虽然理解任务并在初始阶段能够解决，但在一定步骤后经常失败。

Conclusion: 最近的LLMs在状态跟踪方面表现出色，但模型代际之间存在明显差异，老一代模型在长期状态跟踪方面存在局限性。
...
### [130] [Beyond Elicitation: Provision-based Prompt Optimization for Knowledge-Intensive Tasks](https://arxiv.org/abs/2511.10465)
*Yunzhe Xu,Zhuosheng Zhang,Zhe Liu*

Main category: cs.CL

TL;DR: KPPO框架将提示优化重新定义为系统知识整合而非潜在激发，通过知识差距填补、批量候选评估和自适应知识剪枝，在知识密集型任务上比激发式方法平均提升约6%性能，同时减少29%的token使用。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法主要关注激发模型能力，但在知识密集型任务中存在根本性局限，无法提供所需的事实知识、术语精度和推理模式。

Method: 提出KPPO框架，包含三个关键创新：知识差距填补机制、批量候选评估方法和自适应知识剪枝策略。

Result: 在15个知识密集型基准测试中，KPPO优于激发式方法，平均性能提升约6%，同时实现可比或更低的token消耗。

Conclusion: KPPO通过系统知识整合有效解决了知识密集型任务中的提示优化问题，在性能和效率方面均表现出色。
...
### [131] [Rubric-Based Benchmarking and Reinforcement Learning for Advancing LLM Instruction Following](https://arxiv.org/abs/2511.10507)
...
TL;DR: 该论文提出了AdvancedIF基准测试和RIFL训练框架，用于评估和提升大语言模型在复杂、多轮和系统级指令跟随方面的能力。
...
Method: 提出RIFL训练框架，包括规则生成、微调的规则验证器和奖励塑造，通过强化学习提升指令跟随能力。

Result: RIFL显著提升了LLMs的指令跟随能力，在AdvancedIF基准上获得6.7%的绝对增益，并在公共基准上表现优异。

Conclusion: 该工作确立了规则作为训练和评估LLMs高级指令跟随能力的强大工具，为开发更强大可靠的AI系统铺平了道路。
...
### [132] [LOCA-R: Near-Perfect Performance on the Chinese Physics Olympiad 2025](https://arxiv.org/abs/2511.10515)
...
<details>
  <summary>Details</summary>
Motivation: 物理奥赛级别的解题需要精确计算、抽象推理和物理原理的复杂集成，这对人类和AI都是重大挑战。中国物理奥赛(CPhO)以其复杂性和深度成为测试这些高级能力的理想平台。
...
### [133] [Say It Differently: Linguistic Styles as Jailbreak Vectors](https://arxiv.org/abs/2511.10519)
*Srikant Panda,Avinash Rai*

Main category: cs.CL

TL;DR: 本文系统研究了语言风格（如恐惧、好奇等）如何重构有害意图并引发对齐模型的不安全响应，构建了风格增强的越狱基准，发现风格重构可将越狱成功率提高高达57个百分点，并提出了风格中和预处理方法来缓解这一漏洞。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型通常针对同义改写或语义等效的越狱提示进行鲁棒性评估，但很少关注语言变异作为攻击面。本文旨在系统研究语言风格如何重构有害意图并引发对齐模型的不安全响应。

Method: 通过手工模板和LLM重写，将3个标准数据集中的提示转换为11种不同的语言风格，同时保持语义意图不变，构建风格增强的越狱基准。评估了16个开源和闭源的指令调优模型。

Result: 风格重构显著提高了越狱成功率，最高可增加57个百分点。恐惧、好奇和同情等风格最为有效，情境化重写优于模板化变体。

Conclusion: 研究揭示了一个系统性的、规模抵抗的漏洞，在当前安全流程中被忽视。提出的风格中和预处理方法能显著降低越狱成功率。
...
### [134] [Convomem Benchmark: Why Your First 150 Conversations Don't Need RAG](https://arxiv.org/abs/2511.10523)
*Egor Pakhomov,Erik Nijkamp,Caiming Xiong*

Main category: cs.CL

TL;DR: 本文提出了一个包含75,336个问答对的对话记忆评估基准，分析了对话记忆与检索增强生成(RAG)的关系，发现简单全上下文方法在150次对话内优于复杂RAG系统，并确定了不同对话规模下的最佳技术选择。


<details>
  <summary>Details</summary>
Motivation: 现有对话记忆评估基准存在统计能力不足、数据生成不一致和评估灵活性有限的问题，需要更全面的评估框架来推动对话记忆研究的发展。

Method: 构建了涵盖用户事实、助手回忆、弃权、偏好、时间变化和隐式连接等多样类别的对话记忆评估基准，并比较了全上下文方法与RAG系统在不同对话规模下的表现。

Result: 简单全上下文方法在最具挑战性的多消息证据情况下达到70-82%准确率，而复杂RAG系统在150次对话内仅达到30-45%。前30次对话长上下文方法最优，150次内仍可行，超过150次需要混合或RAG方法。

Conclusion: 对话记忆的小语料优势值得专门研究，不应简单将通用RAG解决方案应用于对话历史，而应根据对话规模选择合适的技术方案。
...
### [135] [Computing the Formal and Institutional Boundaries of Contemporary Genre and Literary Fiction](https://arxiv.org/abs/2511.10546)
*Natasha Johnson*

Main category: cs.CL

TL;DR: 使用计算方法研究文学体裁的形式特征与制度特征，发现不同文学类别具有显著的形式标记，女性作者身份会缩小和模糊获得文学地位的标准。


<details>
  <summary>Details</summary>
Motivation: 探讨体裁作为形式分类与制度分类的有效性，分析当代文学中体裁小说与文学小说的差异，特别关注作者性别对文学分类的影响。

Method: 使用CONLIT数据集构建文学小说和体裁小说语料库，采用Welch's ANOVA比较叙事特征分布，使用逻辑回归建模特征对文学分类的影响，分析风格和语义向量表示。

Result: 发现每个文学类别具有统计学上显著的形式标记，女性作者身份会缩小和模糊获得文学地位的目标。

Conclusion: 体裁具有可识别和可量化的形式特征，作者性别在文学分类中发挥重要调节作用，女性作者面临更严格的文学地位标准。
...
### [136] [URaG: Unified Retrieval and Generation in Multimodal LLMs for Efficient Long Document Understanding](https://arxiv.org/abs/2511.10552)
...
Result: 广泛实验表明，URaG在减少44-56%计算开销的同时，实现了最先进的性能。
...
### [137] [DESS: DeBERTa Enhanced Syntactic-Semantic Aspect Sentiment Triplet Extraction](https://arxiv.org/abs/2511.10577)
*Vishal Thenuwara,Nisansa de Silva*

Main category: cs.CL

TL;DR: DESS是一种新的细粒度情感分析方法，通过整合DeBERTa的增强注意力机制和LSTM通道，改进了方面情感三元组抽取任务，在标准数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决方面情感三元组抽取任务中准确捕捉方面、观点和情感极性之间关系的挑战，充分利用先进语言模型理解复杂语言模式的潜力。

Method: 提出DESS框架，集成DeBERTa的增强注意力机制与LSTM通道，采用双通道结构处理文本的语义和语法模式，精心优化组件间的协作方式。

Result: 在标准数据集测试中，DESS相比现有方法有显著改进，在识别方面观点对和准确确定情感方面，F1分数分别提高了4.85、8.36和2.42。

Conclusion: 研究表明，当经过深思熟虑地集成时，升级到更先进的语言模型可以真正改善文本情感分析的效果，DeBERTa的复杂注意力系统有助于处理复杂句子结构。
...
TL;DR: 本文研究了MedGemma模型在医疗订单提取任务中的表现，比较了三种提示范式：单样本提示、ReAct推理框架和多步骤智能体工作流，发现简单的单样本提示在验证集上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 准确提取医患对话中的医疗订单对于减轻临床文档负担和确保患者安全至关重要，作者旨在探索适合临床信息提取的提示策略。

Method: 使用MedGemma领域专用开源语言模型，系统评估三种提示范式：单样本方法、推理聚焦的ReAct框架和多步骤智能体工作流。

Result: 实验表明，虽然ReAct和智能体流程等复杂框架功能强大，但简单的单样本提示方法在官方验证集上取得了最高性能。

Conclusion: 在手动标注的转录本上，复杂的推理链可能导致"过度思考"并引入噪声，因此直接方法更加稳健和高效，为不同数据条件下选择临床信息提取策略提供了有价值的见解。
...
<details>
  <summary>Details</summary>
Motivation: 异步远程护理的快速发展增加了医疗提供者的工作负担，需要AI系统来帮助临床医生更有效地处理患者查询，特别是在伤口护理领域。

Method: 1. 挖掘提示策略：嵌入训练数据并检索最相似的示例作为少样本演示；2. 元数据引导生成：通过分类器预测关键元数据属性，并基于预测置信度动态调整输出。
...
TL;DR: 本文介绍了一种编码器增强的因果解码器模型架构，该架构在训练效率上优于因果变换器，并能实现更高的压缩率。研究表明，通过估计每个标记的熵并以此训练模型，可以提升模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于因果（下一个标记预测）的大型语言模型在估计语言熵方面计算上不可行，且存在语言信息熵的内在限制，限制了语言模型的准确性和压缩率。

Method: 引入编码器增强的因果解码器模型架构，通过估计每个标记的熵，训练模型接近但不超出估计的每标记熵值。

Result: 该架构在训练效率上优于因果变换器，即使使用适中的硬件也能实现更高的压缩率。实证表明，考虑熵训练的因果模型比不考虑熵的模型具有更好的泛化能力。

Conclusion: 通过接近但不超出估计的每标记熵值来训练模型，可以显著提高模型的泛化能力，这为语言模型的训练提供了新的方向。
...
### [141] [SSR: Socratic Self-Refine for Large Language Model Reasoning](https://arxiv.org/abs/2511.10621)
*Haizhou Shi,Ye Liu,Bo Pang,Zeyu Leo Liu,Hao Wang,Silvio Savarese,Caiming Xiong,Yingbo Zhou,Semih Yavuz*

Main category: cs.CL

TL;DR: SSR是一个新颖的LLM推理框架，通过将模型响应分解为可验证的(子问题,子答案)对，实现步骤级置信度估计和精确细化，在多个推理基准测试中优于现有自优化方法。


<details>
  <summary>Details</summary>
Motivation: 现有测试时框架依赖粗粒度的自我验证和自校正，限制了在复杂任务上的有效性，需要更精细的评估和精确优化方法。

Method: 将模型响应分解为可验证的(子问题,子答案)对，通过受控重解和自一致性检查进行步骤级置信度估计，定位不可靠步骤并迭代优化。

Result: 在五个推理基准测试和三个LLM上的实验结果表明，SSR始终优于最先进的迭代自优化基线方法。

Conclusion: SSR不仅带来性能提升，还提供了一种原则性的黑盒方法来评估和理解LLM的内部推理过程。
...
### [142] [Instella: Fully Open Language Models with Stellar Performance](https://arxiv.org/abs/2511.10628)
...
TL;DR: Instella是一个完全开源的30亿参数语言模型家族，使用公开可用数据和代码库训练，在AMD MI300X GPU上开发，包括预训练、指令微调和人类偏好对齐。尽管预训练token数量较少，但在全开源模型中达到SOTA，与同类规模的开源权重模型竞争。还发布了支持128K上下文长度的Instella-Long和专注于数学推理的Instella-Math两个变体。


<details>
  <summary>Details</summary>
Motivation: 当前大多数高性能语言模型都是闭源或部分开源，限制了透明度和可复现性。作者旨在开发完全开源的语言模型，推动开放和可复现的语言建模研究。

Method: 使用AMD Instinct MI300X GPU进行大规模预训练、通用指令微调和人类偏好对齐。开发了两个专门变体：Instella-Long通过技术处理支持128K上下文长度，Instella-Math通过监督微调和强化学习在数学任务上进行增强。

Result: Instella在全开源模型中达到最先进水平，与同类规模的开源权重模型具有竞争力。两个专门变体分别实现了长上下文处理能力和数学推理能力的显著提升。

Conclusion: Instella系列模型为研究社区提供了一个透明、高性能且多功能的替代方案，推动了开放和可复现语言建模研究的发展目标。
...
### [143] [Black-Box On-Policy Distillation of Large Language Models](https://arxiv.org/abs/2511.10643)
*Tianzhu Ye,Li Dong,Zewen Chi,Xun Wu,Shaohan Huang,Furu Wei*

Main category: cs.CL

TL;DR: 本文提出生成对抗蒸馏（GAD）方法，通过将学生LLM作为生成器、训练判别器区分师生模型响应，实现黑盒蒸馏，在自动评估中表现优于传统序列级知识蒸馏。


<details>
  <summary>Details</summary>
Motivation: 解决黑盒蒸馏中无法访问教师模型内部参数或logits的问题，实现更有效的知识迁移。

Method: 采用生成对抗框架，学生LLM作为生成器，训练判别器区分师生响应，形成极小极大博弈，判别器作为在线奖励模型提供自适应反馈。

Result: GAD在LMSYS-Chat自动评估中表现优于序列级知识蒸馏，Qwen2.5-14B-Instruct学生模型达到与GPT-5-Chat教师模型相当的水平。

Conclusion: GAD是黑盒LLM蒸馏的有效新范式，通过对抗训练实现稳定的知识迁移。
...
### [144] [ParoQuant: Pairwise Rotation Quantization for Efficient Reasoning LLM Inference](https://arxiv.org/abs/2511.10645)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型权重和激活中的异常值会导致量化误差和精度下降，特别是在推理任务中误差会在长思维链中累积。现有PTQ方法要么无法充分抑制异常值，要么在推理时引入显著开销。

Method: 结合硬件高效的独立Givens旋转与通道级缩放，均衡通道间幅值并缩小每个量化组内的动态范围，同时协同设计推理内核以充分利用GPU并行性。
...
TL;DR: 本文提出了一种新颖高效的Partial Path Reconstruction (PPR)求解器，用于解决联合路由分配(JRA)优化问题。该方法通过识别关键项目-占位符对形成简化子问题，显著提高了大规模实例的求解效率，平均偏差降至0.00%。


<details>
  <summary>Details</summary>
Motivation: 现有的精确MIP求解器虽然能保证最优性，但在处理大规模JRA问题时计算效率低下。启发式方法虽然效率高但存在约1%的偏差。需要开发既能保持高精度又具有计算效率的新方法。

Method: 提出PPR求解器框架：1) 识别关键项目-占位符对形成简化子问题；2) 高效求解子问题以优化全局解；3) 在优化路径上迭代使用PPR进行精细化；4) 在JRA模型中引入全局Large-α约束。

Result: 在n=300、500、1000的基准数据集上测试，该方法能持续提供几乎最优解，平均偏差为0.00%，同时保持高计算效率。相比原有启发式方法，偏差减少了一半。

Conclusion: 提出的PPR框架不仅有效解决了大规模JRA问题，还展示了在TSP及相关优化问题中的广泛应用潜力，为复杂组合优化问题提供了高效高精度的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于文献中考虑的各种约束条件，比较不同问题变体的方法仍然具有挑战性。电容式绿色车辆路径问题作为电动汽车路径问题的最小化变体，是CEC-12竞赛的焦点。

Method: 基于可变邻域搜索元启发式算法的方法。

Result: 该方法在完整竞赛数据集上取得了最佳结果，并且优于之后发布的更近期算法。

Conclusion: 提出的基于可变邻域搜索的方法在解决电容式绿色车辆路径问题方面表现优异，是竞赛中的获胜方案。
...
TL;DR: 该研讨会旨在探索如何将基于transformer的语言模型的推理能力与基于逻辑的知识表示方法相协调，分析语言模型的推理能力，并向语言模型注入知识表示风格的推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着基于transformer的语言模型在自然语言处理领域的突破性进展，人们开始关注这些模型是否具备推理能力。然而，目前仍难以明确界定语言模型实际具备的推理能力程度，需要建立一个跨学科平台来探索这一问题。

Method: 通过跨学科研究平台，分析语言模型与知识表示方法的推理能力对比，采用神经符号方法向语言模型注入知识表示风格的推理能力，并形式化语言模型执行的推理类型。

Result: 该研讨会旨在揭示语言模型如何有效整合和利用知识与推理，从而在需要精确性和可靠性的应用领域中提升其效用。

Conclusion: 通过协调语言模型与逻辑知识表示的推理能力，可以提升语言模型在关键应用领域的精确性和可靠性，推动AI推理能力的发展。
...
<details>
  <summary>Details</summary>
Motivation: 研究两种非可容许论证框架语义（cogent和弱可容许语义）之间的确切关系，澄清它们之间的包含关系。
...
<details>
  <summary>Details</summary>
Motivation: 随着音频推理模型（ARMs）的流行，目前缺乏对其安全性的研究，特别是针对越狱攻击的防护。作者旨在填补这一空白，研究ARMs在越狱攻击下的脆弱性。

Method: 作者首先展示了标准推理训练对普通音频越狱攻击的防护效果，然后提出了更高级的越狱攻击方法。基于表示漂移的观察，开发了Rebellion方法，训练ARMs抵抗最坏情况下的表示漂移。

Result: 在Qwen2-Audio上的实验表明，Rebellion能有效防御高级音频越狱攻击，同时不影响良性任务的性能，显著改善了准确性与安全性之间的权衡。

Conclusion: Rebellion方法为音频推理模型提供了强大的安全防护，能够有效抵御高级越狱攻击，同时保持模型性能，在安全性和准确性之间取得了更好的平衡。
...
TL;DR: 本文研究LLM智能体间对话中的行为漂移现象，特别是回响问题——智能体放弃指定角色而镜像对话伙伴，破坏预定目标。通过大规模实验发现回响在三大LLM提供商中普遍存在，发生率5%-70%，且高级推理模型也无法避免。提出了结构化响应的协议级缓解方法。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体自主交互，出现了无法从单智能体性能预测的新型失败模式：智能体间对话中的行为漂移。与人类-智能体交互不同，AxA缺乏稳定信号，导致独特的失败模式。

Method: 在60个AxA配置、3个领域、2000+对话中进行实验，分析不同LLM提供商的回响现象。研究提示影响、对话动态，并引入结构化响应的协议级缓解方法。

Result: 回响现象在三大LLM提供商中普遍存在，发生率5%-70%；高级推理模型回响率仍达32.8%；回响随交互增长而出现（实验中发现7+轮次后）；结构化响应可将回响降至9%。

Conclusion: 智能体间对话中的回响是一个普遍且持久的问题，无法通过优化提示完全解决。协议级干预（如结构化响应）是有效的缓解策略，为构建可靠的自主智能体系统提供了重要见解。
...
TL;DR: 本文研究了通过验证导向的编排（自我验证和交叉验证）来提高大型语言模型在辅导话语定性编码中的可靠性，结果显示编排方法使Cohen's kappa系数提高了58%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于标注学习交互，但可靠性问题限制了其实用性。作者希望测试验证导向的编排是否能改善辅导话语的定性编码质量。

Method: 使用30个一对一数学辅导会话的转录文本，比较了三种前沿LLM（GPT、Claude、Gemini）在三种条件下的表现：未验证标注、自我验证和所有编排配置下的交叉验证。输出结果与盲审的人类裁决进行基准比较。

Result: 编排方法使kappa系数提高了58%。自我验证相对于未验证基线几乎使一致性翻倍，特别是在具有挑战性的导师行为上获得最大收益。交叉验证平均提高了37%，但存在配对和结构依赖性效应。

Conclusion: 验证被定位为学习分析中可靠、可扩展的LLM辅助标注的原则性设计杠杆，作者贡献了灵活的编排框架、跨前沿LLM的实证比较以及标准化的报告符号。
...
TL;DR: 本文主张开放小型AI模型对交互艺术创作独立性的重要性，相比封闭的大型企业系统，小型模型能为艺术家提供基础设施和代码的自主控制权。


<details>
  <summary>Details</summary>
Motivation: 当前主导的大型封闭AI系统作为不透明的黑箱，对交互艺术作品施加了严重限制，包括内容过滤限制、保存问题、延迟增加和接口有限等技术挑战。

Method: 通过对比分析开放小型AI模型与封闭源替代方案的差异，探讨在交互艺术中使用开放小型AI模型的实际应用和影响。

Result: 小型AI模型赋予创作者更多自主权、控制权和可持续性，支持长期保存和展示包含AI组件的艺术作品。

Conclusion: 开放小型AI模型促进技术自决，提供更大的所有权，减少对不适合交互艺术需求的企业AI的依赖，从而赋能艺术家。
...
### [153] [Thermally Activated Dual-Modal Adversarial Clothing against AI Surveillance Systems](https://arxiv.org/abs/2511.09829)
*Jiahuan Long,Tingsong Jiang,Hanqing Liu,Chao Ma,Wen Yao*

Main category: cs.AI

TL;DR: 提出一种热激活对抗可穿戴系统，通过在普通黑色T恤上集成热致变色染料和柔性加热单元，在加热时激活隐藏的对抗图案，实现在可见光和红外模态下有效躲避AI监控检测。


<details>
  <summary>Details</summary>
Motivation: 现有对抗补丁在真实场景中部署困难，因为其显眼的外观容易引起注意。需要开发更隐蔽、适应性强的隐私保护技术来应对无处不在的AI监控系统。

Method: 将热致变色染料与柔性加热单元集成到服装表面，默认状态下服装呈现普通黑色外观，通过嵌入式热单元加热激活隐藏的对抗图案。
...
### [154] [EgoEMS: A High-Fidelity Multimodal Egocentric Dataset for Cognitive Assistance in Emergency Medical Services](https://arxiv.org/abs/2511.09894)
...
<details>
  <summary>Details</summary>
Motivation: EMS急救人员在高压环境下面临巨大认知负担，AI认知助手作为虚拟伙伴可以通过实时数据收集和决策支持来减轻这种负担，但缺乏高质量的数据集来推动相关研究。

Method: 与EMS专家合作开发开源、低成本、可复制的数据采集系统，从第一人称视角采集233个模拟紧急场景的20多小时多模态数据，包括关键步骤、时间戳音频转录、动作质量指标和边界框分割掩码等标注。

Result: 创建了EgoEMS数据集，包含62名参与者（46名EMS专业人员）的233个模拟场景数据，强调真实性的响应者-患者互动，反映了真实世界的紧急动态。

Conclusion: EgoEMS数据集为开发EMS AI支持工具提供了基准测试套件，包括实时多模态关键步骤识别和动作质量评估，有望推动智能EMS系统研究并最终改善患者预后。
...
### [155] [Boosting In-Silicon Directed Evolution with Fine-Tuned Protein Language Model and Tree Search](https://arxiv.org/abs/2511.09900)
...
Result: 广泛的基准实验表明，AlphaDE即使在小样本微调情况下也显著优于之前的最先进方法。案例研究显示AlphaDE支持通过计算进化压缩蛋白质序列空间。
...
### [156] [Learning to Pose Problems: Reasoning-Driven and Solver-Adaptive Data Synthesis for Large Reasoning Models](https://arxiv.org/abs/2511.09907)
*Yongxian Wei,Yilin Zhao,Li Shen,Xinrui Chen,Runxi Cheng,Sinan Du,Hao Yu,Gang Liu,Jiahong Yan,Chun Yuan,Dian Li*

Main category: cs.AI

TL;DR: 本文提出了一种能够进行显式推理的问题生成器，通过规划问题方向并适应求解器能力来生成高质量的训练数据，在10个数学和通用推理基准测试中平均提升2.5%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据合成方法存在两个主要问题：(i) 不考虑求解器能力的盲目生成导致低价值问题，或依赖复杂数据管道来平衡问题难度；(ii) 问题生成缺乏推理，导致问题变体浅层化。

Method: 构建相关问题对并用推理模型生成的中间问题设计CoT进行增强，从生成器中引导问题设计策略。将求解器对合成问题的反馈作为奖励信号，使生成器能够校准难度并在求解器能力边缘产生补充性问题。

Result: 在10个数学和通用推理基准测试中平均提升2.5%的性能，并能泛化到语言和视觉语言模型。通过协同进化，进一步获得0.7%的性能增益。

Conclusion: 该方法通过显式推理和难度自适应的问题生成，有效提升了训练数据的质量，实现了生成器与求解器的协同进化，显著提高了推理模型的性能。
...
### [157] [OIDA-QA: A Multimodal Benchmark for Analyzing the Opioid Industry Documents Archive](https://arxiv.org/abs/2511.09914)
...
<details>
  <summary>Details</summary>
Motivation: 阿片类药物危机揭示了监管系统、医疗实践、公司治理和公共政策等多方面的系统性缺陷，需要创新分析方法来处理UCSF-JHU阿片类药物行业文档档案中的大量数据和文档。这些医疗相关法律和公司文档的复杂性、多模态特性和专业化特征要求开发针对特定数据类型和详细注释的先进方法和模型。

Method: 1. 根据文档属性组织原始数据集，构建包含40万训练文档和1万测试文档的基准数据集
2. 从每个文档中提取丰富的多模态信息（文本内容、视觉元素、布局结构）
3. 使用多个AI模型生成大规模问答对数据集（36万训练QA对和1万测试QA对）
4. 开发领域特定多模态大语言模型，探索多模态输入对任务性能的影响
5. 结合历史问答对作为上下文基础来回答当前查询
6. 在答案中纳入页面引用，并引入基于重要性的页面分类器

Result: 初步结果表明，我们的AI助手在文档信息提取和问答任务方面取得了改进。构建的数据集和模型已在HuggingFace上公开可用。

Conclusion: 通过构建专门针对阿片类药物行业文档的多模态AI分析系统，结合大规模数据集、领域特定模型和增强的上下文理解方法，能够有效应对复杂医疗相关法律和公司文档的分析挑战，为系统性公共卫生问题的深入分析提供了创新解决方案。
...
### [158] [Adaptive Hyperbolic Kernels: Modulated Embedding in de Branges-Rovnyak Spaces](https://arxiv.org/abs/2511.09921)
...
### [159] [SPAN: Benchmarking and Improving Cross-Calendar Temporal Reasoning of Large Language Models](https://arxiv.org/abs/2511.09993)
*Zhongjian Miao,Hao Fu,Chen Wei*

Main category: cs.AI

TL;DR: SPAN是一个跨日历时间推理基准，需要LLMs进行日历内时间推理和跨日历时间转换。实验显示现有LLMs平均准确率仅34.5%，作者开发了基于工具增强代码生成的Time Agent，达到95.31%的准确率。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够评估LLMs跨日历时间推理能力的基准，解决现有模型在时间推理方面的局限性，特别是未来日期退化和日历不对称偏见问题。

Method: 提出了模板驱动的动态实例生成协议，支持在指定格里高利日期上进行评估。开发了基于工具增强代码生成的Time Agent，利用代码生成来增强时间推理能力。

Result: 对开源和闭源SOTA LLMs的评估显示平均准确率仅34.5%，没有模型超过80%。Time Agent达到95.31%的准确率，显著优于其他基线方法。

Conclusion: 跨日历时间推理对当前LLMs仍具挑战性，工具增强的代码生成方法能显著提升时间推理能力，为开发更具时间和文化适应性的LLMs提供了方向。
...
### [160] [ChEmREF: Evaluating Language Model Readiness for Chemical Emergency Response](https://arxiv.org/abs/2511.10027)
*Risha Surana,Qinyuan Ye,Swabha Swayamdipta*

Main category: cs.AI

TL;DR: 本文介绍了ChEmREF基准测试，用于评估语言模型在危险化学品应急响应中的表现，包括化学表示转换、应急响应生成和领域知识问答三个任务。


<details>
  <summary>Details</summary>
Motivation: 应急响应人员在处理危险化学品事故时需要快速准确地获取关键信息，目前主要依赖手动查阅大量化学指南，效率较低。研究旨在探索语言模型是否能有效辅助应急响应工作。

Method: 开发了ChEmREF基准测试框架，包含1,035种危险化学品的三个任务：化学表示转换（如C2H6O转乙醇）、应急响应生成（如推荐疏散距离）和领域知识问答（来自化学品安全认证考试）。

Result: 最佳模型在非结构化化学表示转换任务中准确率为68.0%，应急响应建议的LLM评分达52.7%，危险化学品考试多选题准确率为63.9%。
...
### [161] [Beyond ReAct: A Planner-Centric Framework for Complex Tool-Augmented LLM Reasoning](https://arxiv.org/abs/2511.10037)
...
Method: 提出了以Planner为中心的Plan-Execute范式，包括：1) 执行全局有向无环图规划的Planner模型；2) ComplexTool-Plan大规模基准数据集；3) 结合监督微调和组相对策略优化的两阶段训练方法。

Result: 在StableToolBench基准测试中实现了最先进的性能，展示了在复杂用户查询处理中的卓越端到端执行能力和鲁棒的多工具工作流处理能力。
...
### [162] [Efficient Thought Space Exploration through Strategic Intervention](https://arxiv.org/abs/2511.10038)
*Ziheng Li,Hengyi Cai,Xiaochi Wei,Yuchen Li,Shuaiqiang Wang,Zhi-Hong Deng,Dawei Yin*

Main category: cs.AI

TL;DR: 提出Hint-Practice Reasoning (HPR)框架，通过分布不一致性减少(DIR)指标动态识别关键决策点，让强大LLM提供概率指导，小型模型执行主要推理步骤，在保持高准确率的同时大幅减少计算成本。
...
Method: HPR框架包含hinter(强大LLM)和practitioner(高效小模型)两个组件，使用DIR指标在树状概率空间中量化推理轨迹与期望分布的差异，动态识别干预点。

Result: 在算术和常识推理基准测试中，HPR达到与自一致性和MCTS基线相当的性能，同时仅解码1/5的token，在保持相似或更低FLOPs的情况下比现有方法最多提升5.1%绝对准确率。

Conclusion: HPR框架通过选择性干预关键决策点，实现了最先进的效率-准确率权衡，为LLM推理提供了更高效的解决方案。
...
### [163] [Radiology Workflow-Guided Hierarchical Reinforcement Fine-Tuning for Medical Report Generation](https://arxiv.org/abs/2511.10065)
*Bodong Du,Honglong Yang,Xiaomeng Li*

Main category: cs.AI

TL;DR: RadFlow是一个分层工作流引导的强化优化框架，用于医学报告生成，通过模拟放射科医师的结构化工作流程来改善报告的一致性和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有医学报告生成系统将报告视为扁平序列，忽略了放射科医师描述视觉发现、总结印象并精炼关键病例陈述的结构化工作流程，导致描述性和诊断性内容不一致。
...
### [164] [Enhancing the Medical Context-Awareness Ability of LLMs via Multifaceted Self-Refinement Learning](https://arxiv.org/abs/2511.10067)
*Yuxuan Zhou,Yubin Wang,Bin Wang,Chen Ning,Xien Liu,Ji Wu,Jianye Hao*

Main category: cs.AI

TL;DR: 提出MuSeR方法，通过自我评估和精炼增强LLM在医疗领域的上下文感知能力，在HealthBench数据集上显著提升性能，特别是上下文感知方面，并使较小模型超越教师模型达到新SOTA。
...
Method: 提出多面自我精炼方法，包括属性条件查询生成器模拟多样化用户上下文，LLM自我评估答案在三个关键方面（决策、沟通、安全）并进行精炼，然后使用查询和精炼响应进行监督微调。

Result: 在HealthBench数据集上显著提升LLM性能，特别是在上下文感知轴上取得显著进步。通过知识蒸馏，较小模型性能超越教师模型，在所有开源LLM中达到新SOTA（63.8%）。

Conclusion: MuSeR方法有效增强LLM在医疗领域的上下文感知能力，为真实医疗场景应用提供了有前景的解决方案。
...
### [165] [Balancing Centralized Learning and Distributed Self-Organization: A Hybrid Model for Embodied Morphogenesis](https://arxiv.org/abs/2511.10101)
*Takehiro Ishikawa*
...
### [166] [RAGFort: Dual-Path Defense Against Proprietary Knowledge Base Extraction in Retrieval-Augmented Generation](https://arxiv.org/abs/2511.10128)
*Qinfeng Li,Miao Pan,Ke Xiong,Ge Su,Zhiqiang Shen,Yan Liu,Bing Sun,Hao Peng,Xuhong Zhang*

Main category: cs.AI

TL;DR: RAGFort是一个针对RAG系统重建攻击的双模块防御方案，通过对比重索引实现类间隔离，通过约束级联生成实现类内保护，有效降低知识库提取攻击的成功率。


<details>
  <summary>Details</summary>
Motivation: RAG系统面临的重建攻击能够通过聚合模型响应来复制知识库，现有防御方法只针对单一攻击路径，无法提供全面保护。

Method: 提出RAGFort防御框架，结合对比重索引实现类间隔离，约束级联生成实现类内保护，形成结构感知的双模块防御机制。

Result: 实验表明RAGFort在安全性、性能和鲁棒性方面均表现优异，显著降低重建成功率同时保持回答质量。

Conclusion: RAGFort为知识库提取攻击提供了全面有效的防御方案，证明联合保护类间和类内路径对防御重建攻击至关重要。
...
### [167] [DenoGrad: Deep Gradient Denoising Framework for Enhancing the Performance of Interpretable AI Models](https://arxiv.org/abs/2511.10161)
*J. Javier Alonso-Ramos,Ignacio Aguilera-Martos,Andrés Herrera-Poyatos,Francisco Herrera*

Main category: cs.AI

TL;DR: 提出了DenoGrad框架，利用深度学习模型的梯度来检测和调整噪声样本，相比传统方法能更好地保持数据分布并提升AI模型鲁棒性。
...
Method: DenoGrad框架利用在目标数据上训练的准确深度学习模型的梯度，动态检测和校正噪声样本，而不依赖于特定任务。

Result: 在表格和时间序列数据集上验证，DenoGrad在各种噪声设置下优于现有最先进去噪策略，是唯一能保持原始数据分布的高质量方法。

Conclusion: DenoGrad通过任务特定的高质量解决方案作为参考，提供了更精确和自适应的噪声定义，显著提升了可解释AI模型的性能。
...
### [168] [Two Constraint Compilation Methods for Lifted Planning](https://arxiv.org/abs/2511.10164)
*Periklis Mantenoglou,Luigi Bonassi,Enrico Scala,Pedro Zuidberg Dos Martires*

Main category: cs.AI

TL;DR: 提出两种无需预编译的约束编译方法，用于处理大规模规划问题中的定性状态轨迹约束，相比现有方法更高效且生成更简洁的规划规范。


<details>
  <summary>Details</summary>
Motivation: 现实世界规划问题常包含安全性要求、任务排序条件和中间子目标等定性状态轨迹约束，现有编译方法在大规模问题上因需要预编译而无法扩展。

Method: 开发两种无需预编译的约束编译方法，通过避免问题预编译来处理大规模规划问题中的约束。

Result: 在国际规划竞赛领域上的实验表明，新方法高效且生成的规划规范比预编译方法简洁数个数量级，同时在使用先进规划器时保持竞争力。

Conclusion: 提出的无预编译约束编译方法能有效处理大规模规划问题，在保持性能的同时显著减少规划规范的大小。
...
### [169] [MTP: Exploring Multimodal Urban Traffic Profiling with Modality Augmentation and Spectrum Fusion](https://arxiv.org/abs/2511.10218)
...
<details>
  <summary>Details</summary>
Motivation: 现有交通信号建模方法通常依赖单一数值模态，忽略了多模态异构城市数据中的语义信息，这阻碍了对交通信号的全面理解并限制了复杂交通动态的准确预测。

Method: 提出MTP多模态框架：1）视觉增强：将原始模态转换为频域图像和周期性图像；2）文本增强：基于特定主题、背景信息和项目描述生成描述性文本；3）数值学习：使用频域多层感知器；4）分层对比学习融合三种模态。

Result: 在六个真实世界数据集上的广泛实验表明，该方法相比最先进方法具有优越性能。

Conclusion: MTP框架通过多模态学习有效提升了城市交通信号分析的准确性和全面性。
...
### [170] [Bridging Synthetic and Real Routing Problems via LLM-Guided Instance Generation and Progressive Adaptation](https://arxiv.org/abs/2511.10233)
...
Method: 使用进化模块和LLM生成结构属性与真实实例统计相似的合成实例，然后对预训练NCO模型进行渐进式精炼：先与结构丰富的合成分布对齐，再在真实基准实例上直接微调。

Result: EvoReal显著提升了最先进神经求解器的泛化能力，在TSPLib和CVRPLib基准上相比最优解的性能差距分别降至1.05%和2.71%。

Conclusion: 通过生成结构真实的合成实例进行渐进式训练，可以有效弥合神经组合优化方法在真实世界问题上的泛化差距。
...
### [171] [ProgRAG: Hallucination-Resistant Progressive Retrieval and Reasoning over Knowledge Graphs](https://arxiv.org/abs/2511.10240)
*Minbae Park,Hyemin Yang,Jeonghyun Kim,Kunsoo Park,Hyunjoon Kim*

Main category: cs.AI

TL;DR: ProgRAG是一个多跳知识图谱问答框架，通过将复杂问题分解为子问题并逐步扩展推理路径来提升推理性能，在三个知名数据集上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱增强的大语言模型在复杂知识密集型任务中仍面临检索不准确和推理失败的问题，特别是在长输入上下文和不同问题类型所需的逻辑方向捕捉方面存在挑战。

Method: ProgRAG将复杂问题分解为子问题，逐步扩展部分推理路径，在每个步骤中通过外部检索器收集候选证据，并由LLM进行不确定性感知剪枝，最后优化LLM推理的上下文组织。

Result: 在三个知名数据集上的实验表明，ProgRAG在多跳知识图谱问答任务中优于现有基线，提供了更好的可靠性和推理质量。

Conclusion: ProgRAG通过渐进式推理路径扩展和不确定性感知证据剪枝，有效解决了多跳知识图谱问答中的检索和推理失败问题，显著提升了推理性能。
...
### [172] [Beyond Single-Step Updates: Reinforcement Learning of Heuristics with Limited-Horizon Search](https://arxiv.org/abs/2511.10264)
...
<details>
  <summary>Details</summary>
Motivation: 传统启发式搜索方法依赖单步贝尔曼更新，仅基于最佳邻居状态更新启发值。本文旨在通过更全面的搜索和更新策略来改进启发式学习效果。

Method: 采用有限范围搜索策略，在更新状态启发值时考虑到达搜索边界的最短路径，同时结合边成本和边界状态的启发值。

Result: 该方法能够更有效地学习启发函数，通过考虑多步路径信息来提升启发式搜索的性能。

Conclusion: 提出的广义方法通过改进状态采样和启发式更新机制，为基于强化学习的启发式搜索提供了更有效的解决方案。
...
### [173] [Causal-HalBench: Uncovering LVLMs Object Hallucinations Through Causal Intervention](https://arxiv.org/abs/2511.10268)
*Zhe Xu,Zhicai Wang,Junkang Wu,Jinda Lu,Xiang Wang*

Main category: cs.AI

TL;DR: 本文提出Causal-HalBench基准，通过因果分析量化大型视觉语言模型中由物体共现偏差引起的虚假相关性，评估模型对虚假相关性的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型经常出现物体幻觉问题，主要源于训练中高度共现物体之间的虚假相关性，导致模型根据视觉上下文产生幻觉物体。现有基准主要关注幻觉检测，但缺乏对虚假相关性的形式化描述和定量评估。
...
Conclusion: Causal-HalBench能够有效量化LVLMs中由共现偏差引起的虚假相关性影响，为评估和改进模型鲁棒性提供了重要工具。
...
### [174] [Bidirectional Bounded-Suboptimal Heuristic Search with Consistent Heuristics](https://arxiv.org/abs/2511.10272)
...
Method: 基于最优双向搜索算法BAE*，设计了多个专门针对有界次优场景的变体算法。

Result: 实验评估表明，每种算法在不同条件下表现优异，揭示了各种方法的优缺点。

Conclusion: 不同算法在不同条件下各有优势，为有界次优双向搜索提供了多种有效解决方案。
...
### [175] [FactGuard: Event-Centric and Commonsense-Guided Fake News Detection](https://arxiv.org/abs/2511.10281)
...
<details>
  <summary>Details</summary>
Motivation: 随着对手模仿真实新闻风格，基于写作风格的假新闻检测方法效果逐渐下降，而现有LLM方法存在功能探索浅、可用性模糊和推理成本高的问题。
...
### [176] [Beyond Verification: Abductive Explanations for Post-AI Assessment of Privacy Leakage](https://arxiv.org/abs/2511.10284)
...
<details>
  <summary>Details</summary>
Motivation: AI决策过程中的隐私泄露风险日益严重，需要能够检测敏感信息推断的审计工具，同时保证解释的可理解性。

Method: 使用溯因解释框架，定义潜在适用解释(PAE)来识别能够保护敏感特征个体的结果，形式化个体和系统级隐私泄露。
...
### [177] [Massively Parallel Proof-Number Search for Impartial Games and Beyond](https://arxiv.org/abs/2511.10339)
*Tomáš Čížek,Martin Balko,Martin Schmid*

Main category: cs.AI

TL;DR: 本文提出了第一个可在大规模CPU核心上高效扩展的并行证明数搜索算法，并将其应用于Sprouts游戏求解，验证了Sprouts猜想的新位置。


<details>
  <summary>Details</summary>
Motivation: 随着大规模计算集群的普及，现有并行证明数搜索算法在多CPU核心上扩展性差，需要开发能够高效利用大量计算资源的并行版本。

Method: 采用两层并行化和工作节点间信息共享机制，结合Grundy数来减少游戏树规模，实现大规模并行证明数搜索。

Result: 在1024个核心上实现了332.9倍的加速，运行时间比现有最优解算器GLOP快4个数量级，生成了复杂度高1000倍的证明，验证了Sprouts猜想的42个新位置。

Conclusion: 该并行证明数搜索算法能够有效处理指数级增长的游戏树，显著推进了游戏求解的研究进展。
...
### [178] [SITA: A Framework for Structure-to-Instance Theorem Autoformalization](https://arxiv.org/abs/2511.10356)
*Chenyi Li,Wanli Ma,Zichen Wang,Zaiwen Wen*

Main category: cs.AI

TL;DR: 该论文提出了一个结构到实例定理自动形式化(SITA)框架，用于将抽象数学理论自动形式化为Lean证明助手的具体应用实例。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学推理方面虽有进展，但在将抽象结构具体化实例中产生的定理形式化方面仍面临挑战，目标是实现研究级数学结果的自动形式化。

Method: 开发SITA框架，将形式化的抽象结构作为模块化模板，包含定义、假设、操作和定理。利用LLM生成与反馈引导精化相结合，通过Lean的类型类机制集成具体实例，并通过检查结构假设构建验证定理。

Result: 在优化问题数据集上的实验表明，SITA能够有效形式化基于抽象结构的多样化实例。

Conclusion: SITA框架成功桥接了抽象数学理论与具体应用之间的鸿沟，实现了基于抽象结构的实例定理自动形式化。
...
### [179] [Explaining Decentralized Multi-Agent Reinforcement Learning Policies](https://arxiv.org/abs/2511.10409)
*Kayla Boggess,Sarit Kraus,Lu Feng*

Main category: cs.AI

TL;DR: 本文提出了针对去中心化多智能体强化学习（MARL）的策略摘要和解释方法，解决了现有方法主要关注中心化MARL而无法处理去中心化设置中不确定性和非确定性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有解释方法主要针对中心化MARL，无法有效处理去中心化MARL中的不确定性和非确定性，需要开发专门的方法来生成策略摘要和解释。

Method: 提出生成策略摘要的方法来捕捉任务排序和智能体合作，以及基于查询的解释方法，用于回答关于特定智能体行为的When、Why Not和What类型用户查询。

Result: 在四个MARL领域和两种去中心化MARL算法上评估了该方法，证明了其通用性和计算效率。用户研究表明，摘要和解释显著提高了用户问答性能，并增强了理解和满意度等主观评分。

Conclusion: 所提出的方法能够有效生成去中心化MARL策略的摘要和解释，提高用户对智能体行为的理解和满意度，具有通用性和计算效率。
...
### [180] [Generalizing Analogical Inference from Boolean to Continuous Domains](https://arxiv.org/abs/2511.10416)
...
<details>
  <summary>Details</summary>
Motivation: 现有类比推理框架主要针对布尔域，无法扩展到回归任务或连续域，且现有泛化界限在布尔设置下也存在问题，需要建立统一的类比推理理论。

Method: 引入基于广义均值的参数化类比定义，构建统一的类比推理框架，分析类比保持函数的类别，并在平滑性假设下推导最坏情况和平均情况误差界限。

Result: 提出了一个能够同时处理离散和连续域的类比推理通用理论，证明了在平滑性假设下的误差界限，并展示了该框架对布尔分类和回归的统一支持。

Conclusion: 该研究为类比推理提供了一个跨离散和连续域的通用理论框架，解决了现有方法在连续域和回归任务中的局限性，为类比推理在人工智能中的更广泛应用奠定了基础。
...
### [181] [Using Certifying Constraint Solvers for Generating Step-wise Explanations](https://arxiv.org/abs/2511.10428)
*Ignace Bleukx,Maarten Flippo,Bart Bogaerts,Emir Demirović,Tias Guns*

Main category: cs.AI

TL;DR: 本文提出了一种基于约束求解器生成的证明来快速计算逐步解释序列的方法，通过抽象证明框架将证明转换为解释序列，显著提高了生成效率。
...
Result: 实验结果显示该方法显著加快了逐步解释序列的生成速度，同时生成的解释质量与当前最先进方法相当。
...
### [182] [Preference Elicitation for Step-Wise Explanations in Logic Puzzles](https://arxiv.org/abs/2511.10436)
*Marco Foschini,Marianne Defresne,Emilio Gamba,Bart Bogaerts,Tias Guns*

Main category: cs.AI

TL;DR: 本文研究如何通过交互式偏好学习方法来优化逻辑谜题的分步解释质量，提出动态归一化技术和MACHOP查询生成策略，在数独和逻辑网格谜题上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 分步解释需要用户定义目标函数来量化解释步骤的质量，但定义好的目标函数具有挑战性。交互式偏好学习可以从成对比较中学习用户偏好，但需要解决特征尺度差异和相似解释比较的问题。
...
Conclusion: 交互式偏好学习方法可以有效优化分步解释的质量，MACHOP策略在解决特征尺度差异和多样化查询生成方面表现出色。
...
### [183] [Non-Monotonic S4F Standpoint Logic](https://arxiv.org/abs/2511.10449)
*Piotr Gorczyca,Hannes Strass*

Main category: cs.AI

TL;DR: 本文提出了一种新的S4F立场逻辑，它统一了S4F模态逻辑和立场命题逻辑，能够表达多视角、非单调的语义承诺。该逻辑在计算复杂度上不高于其构成逻辑，并提供了轻信和怀疑接受机制。


<details>
  <summary>Details</summary>
Motivation: 立场逻辑为表示多个异质视角提供了统一的模态逻辑形式化方法，而非单调推理框架也可以自然地用模态逻辑（特别是S4F）来捕捉。本文旨在结合这两种能力，创建一个能够表达多视角非单调语义承诺的形式化系统。

Method: 提出了S4F立场逻辑，定义了其语法和语义，分析了计算复杂度，并设计了轻信和怀疑接受机制。通过示例说明了该框架的应用。

Result: S4F立场逻辑在计算复杂度上不高于其构成逻辑，无论采用单调还是非单调形式。该逻辑成功地将多视角表示和非单调推理能力结合起来。

Conclusion: S4F立场逻辑是一个有效的形式化框架，能够统一处理多视角表示和非单调推理，为复杂语义承诺的建模提供了有力工具。
...
### [184] [Proceedings of The third international workshop on eXplainable AI for the Arts (XAIxArts)](https://arxiv.org/abs/2511.10482)
...
<details>
  <summary>Details</summary>
Motivation: 汇集多学科研究者共同探索可解释AI在艺术领域的应用潜力，促进艺术与技术的交叉融合。

Method: 通过在线工作坊形式，在ACM创意与认知会议上组织专家讨论和交流。

Result: 成功建立了艺术与可解释AI研究的跨学科交流平台，推动了该领域的发展。

Conclusion: 可解释AI在艺术领域具有重要应用价值，需要继续加强跨学科合作与研究。
...
### [185] [Rethinking Science in the Age of Artificial Intelligence](https://arxiv.org/abs/2511.10524)
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI如何改变研究流程，从信息管理到实验设计，以及这种转变对科学实践的影响。

Method: 通过评论性分析，考察AI在研究工作流程中的各种应用，包括文献筛选、假设生成和实验执行。

Result: AI系统现在帮助研究人员管理信息过载，筛选文献，发现跨学科联系，生成假设，设计和执行实验。
...
### [186] [Regular Games -- an Automata-Based General Game Playing Language](https://arxiv.org/abs/2511.10593)
*Radosław Miernik,Marek Szykuła,Jakub Kowalski,Jakub Cieśluk,Łukasz Galas,Wojciech Pawlik*

Main category: cs.AI

TL;DR: RG是一个新的通用游戏系统，旨在实现计算效率和游戏设计便利性。它通过有限自动机定义游戏规则，支持高级语言设计，在效率上优于现有系统。
...
Method: 使用有限自动机作为核心规则定义机制，构建多层级语言系统，低层语言用于自动处理，高层语言用于游戏设计，最终翻译为低层语言。

Result: RG生成了比现有最先进系统更快的正向模型，在效率上超越了Regular Boardgames和Ludii等GGP系统。

Conclusion: RG系统通过有限自动机方法成功实现了计算效率和设计便利性的平衡，为通用游戏开发提供了有效解决方案。
...
### [187] [Querying Labeled Time Series Data with Scenario Programs](https://arxiv.org/abs/2511.10627)
*Edward Kim,Devan Shanker,Varun Bharadwaj,Hongbeen Park,Jinkyu Kim,Hazem Torfah,Daniel J Fremont,Sanjit A Seshia*

Main category: cs.AI

TL;DR: 本文提出了一种用于验证模拟环境中发现的自动驾驶系统故障场景在真实世界中是否可重现的方法，通过定义场景匹配的正式规范，并开发了在标注时间序列传感器数据中查询特定场景的高效算法。


<details>
  <summary>Details</summary>
Motivation: 解决仿真测试中的sim-to-real差距问题，验证在仿真环境中发现的故障场景是否在真实世界中也会发生，而不是传感器数据差异导致的假象。

Method: 使用Scenic概率编程语言表示抽象场景，提出场景匹配的正式定义，开发了在标注时间序列传感器数据中查询特定场景的高效算法。

Result: 实验表明，该算法在查询场景时比最先进的商业视觉大语言模型更准确，速度快几个数量级，且能随查询时间序列数据长度扩展。

Conclusion: 该方法为验证仿真发现的故障场景在真实世界中的可重现性提供了有效工具，有助于弥合仿真与现实之间的差距。
...
### [188] [A Shared-Autonomy Construction Robotic System for Overhead Works](https://arxiv.org/abs/2511.09695)
...
TL;DR: 开发用于天花板钻孔等高空作业的机器人系统，包含移动底座、双级升降机和双臂躯干，采用高斯泼溅进行在线3D重建，并开发神经配置空间屏障方法确保动态环境中的安全遥操作。


<details>
  <summary>Details</summary>
Motivation: 解决高空作业（如天花板钻孔）在动态环境中遥操作的安全性和可视性问题，特别是在视线受限的情况下。

Method: 硬件平台包括带双级升降机的移动底座和双臂躯干，配备定制钻孔末端执行器和RGB-D相机；软件方面使用高斯泼溅进行在线3D重建，引入运动参数建模移动物体，并开发神经配置空间屏障方法进行规划和控制。

Result: 初步可行性研究表明硬件具备钻孔、螺栓固定和锚固能力，软件在动态环境中能够实现安全遥操作。

Conclusion: 该系统展示了在高空作业任务中结合先进感知和安全控制方法的可行性，为动态环境中的机器人作业提供了有效解决方案。
...
### [189] [A Robust Task-Level Control Architecture for Learned Dynamical Systems](https://arxiv.org/abs/2511.09790)
*Eshika Pathak,Ahmed Aboudonia,Sandeep Banik,Naira Hovakimyan*

Main category: cs.RO

TL;DR: 提出L1-DS框架，通过结合名义稳定控制器和L1自适应控制器来解决基于动态系统的示教学习中任务执行不匹配问题，并引入基于动态时间规整的目标选择器处理时间不对齐。


<details>
  <summary>Details</summary>
Motivation: 基于动态系统的示教学习在生成运动规划时存在任务执行不匹配问题，未建模动态、持续干扰和系统延迟会导致机器人实际任务空间状态与期望运动轨迹偏离。

Method: 将任何基于动态系统的示教学习模型与名义稳定控制器和L1自适应控制器相结合，并引入基于窗口动态时间规整的目标选择器来处理时间不对齐。

Result: 在LASA和IROS手写数据集上验证了所提架构的有效性。

Conclusion: L1-DS框架能够显式处理任务执行不匹配问题，改善相位一致性跟踪性能。
...
### [190] [Provably Safe Stein Variational Clarity-Aware Informative Planning](https://arxiv.org/abs/2511.09836)
*Kaleb Ben Naveed,Utkrisht Sahai,Anouck Girard,Dimitra Panagou*

Main category: cs.RO

TL;DR: 提出了一种结合信息感知规划和安全性验证的自主机器人轨迹优化框架，通过Stein变分推理学习信息丰富的轨迹，同时使用门卫机制确保安全性。


<details>
  <summary>Details</summary>
Motivation: 现有规划器大多将信息建模为静态或均匀衰减，忽略了空间变化的衰减率，且通常将安全性作为软约束处理，无法在信息收集任务中同时保证信息获取和安全性。

Method: 使用清晰度模型表示环境不确定性，将清晰度动态嵌入轨迹优化，通过Stein变分推理进行贝叶斯推理学习，同时使用门卫框架进行低层安全性验证。

Result: 在具有不同衰减率和障碍物的环境中进行硬件实验和仿真，结果显示能够持续保证安全性并减少信息赤字。

Conclusion: 该框架能够在空间和时间变化的环境中有效规划信息丰富且安全的轨迹，解决了信息衰减和安全性约束的关键挑战。
...
### [191] [PuffyBot: An Untethered Shape Morphing Robot for Multi-environment Locomotion](https://arxiv.org/abs/2511.09885)
*Shashwat Singh,Zilin Si,Zeynep Temel*

Main category: cs.RO

TL;DR: PuffyBot是一个无缆绳形状变形机器人，灵感来自两栖动物，能够在陆地和水生环境中通过改变身体形态进行导航。它使用剪刀式升降机构和曲柄连杆实现形态转变，能在爬行和游泳模式间无缝切换，并具有双模态浮力调节功能。


<details>
  <summary>Details</summary>
Motivation: 受两栖动物在陆地和水生环境中适应形态和运动的启发，开发能够通过形状变形在多种环境中导航的机器人平台，以提高机器人的多功能性和能源效率。

Method: 采用剪刀式升降机构作为主要结构，由线性驱动器驱动实现形状变形；集成曲柄连杆机构调整伺服驱动的肢体90度，实现爬行和游泳模式切换；使用热塑性聚氨酯(TPU)织物确保防水性能。

Result: 机器人能够实现从255.00 cm³到423.75 cm³的体积变化，可抵消3.237 N的下向力；能够在陆地爬行、水下爬行、水面游泳，并具有双模态浮力调节以潜入水下或浮出水面；无缆绳运行时间达2小时。

Conclusion: 形状变形技术能够创建适用于多样化环境的通用且能源高效的机器人平台，展示了在多种环境中实现运动的多功能性。
...
### [192] [A Study on Enhancing the Generalization Ability of Visuomotor Policies via Data Augmentation](https://arxiv.org/abs/2511.09932)
*Hanwen Wang*

Main category: cs.RO

TL;DR: 该论文研究了如何通过自动化数据生成来增强视觉运动策略的泛化能力，创建了一个包含多种随机化因素的广泛随机化数据集，并验证了这些因素对策略泛化能力的影响。
...
Method: 通过自动化数据生成，创建了一个包含五种机械臂和两种抓手的广泛随机化数据集，涵盖了相机姿态、光照条件、桌面纹理和桌面高度等随机化因素，应用于六个操作任务。

Result: 研究发现所有随机化因素都会影响策略的泛化能力，任何形式的随机化都能增强策略泛化，特别是多样化轨迹在弥合视觉差距方面特别有效。在低成本机械臂上验证了场景随机化对零样本仿真到真实迁移的增强效果。

Conclusion: 通过广泛的场景随机化可以显著增强视觉运动策略的泛化能力，特别是在零样本仿真到真实迁移任务中表现突出。
...
### [193] [Audio-VLA: Adding Contact Audio Perception to Vision-Language-Action Model for Robotic Manipulation](https://arxiv.org/abs/2511.09958)
*Xiangyi Wei,Haotian Zhang,Xinyi Cao,Siyu Xie,Weifeng Ge,Yang Li,Changbo Wang*

Main category: cs.RO

TL;DR: Audio-VLA是一种多模态机器人操作策略，利用接触音频感知接触事件和动态过程反馈，克服了纯视觉VLA模型的限制，并提出了任务完成率(TCR)指标来系统评估动态操作过程。


<details>
  <summary>Details</summary>
Motivation: 纯视觉VLA模型在感知交互和操作动态过程方面存在根本性限制，需要引入音频模态来增强对接触事件和动态反馈的感知能力。

Method: 使用预训练的DINOv2和SigLIP作为视觉编码器，AudioCLIP作为音频编码器，Llama2作为大语言模型骨干，通过LoRA微调实现跨模态理解，并添加多模态投影层对齐特征。在RLBench和LIBERO环境中增加基于碰撞的音频生成。
...
### [194] [Phantom Menace: Exploring and Enhancing the Robustness of VLA Models against Physical Sensor Attacks](https://arxiv.org/abs/2511.10008)
*Xuancun Lu,Jiaxiang Chen,Shilin Xiao,Zizhi Jin,Zhangrui Chen,Hanwen Yu,Bohan Qian,Ruochen Zhou,Xiaoyu Ji,Wenyuan Xu*

Main category: cs.RO

TL;DR: 本文首次系统研究了针对视觉-语言-动作(VLA)模型的物理传感器攻击，提出了"真实-模拟-真实"框架来自动模拟基于物理的传感器攻击向量，并通过大规模评估揭示了VLA模型在安全关键环境中的显著脆弱性。


<details>
  <summary>Details</summary>
Motivation: VLA模型通过整合多种感官模态实现了端到端的感知到动作流程，但由于严重依赖传感器输入，其在物理世界传感器攻击下的安全性研究严重不足。

Method: 提出了"真实-模拟-真实"框架，自动模拟基于物理的传感器攻击向量，包括针对摄像头的六种攻击和针对麦克风的两种攻击，并在真实机器人系统上进行验证。

Result: 通过在不同VLA架构和任务下的大规模评估，发现VLA模型存在显著脆弱性，其易受攻击性模式揭示了任务类型和模型设计的关键依赖性。

Conclusion: 研究结果揭示了在安全关键环境中部署VLA模型的紧迫需求，需要标准化鲁棒性基准和缓解策略，同时开发了基于对抗训练的防御方法以增强VLA对传感器攻击的鲁棒性。
...
### [195] [DecARt Leg: Design and Evaluation of a Novel Humanoid Robot Leg with Decoupled Actuation for Agile Locomotion](https://arxiv.org/abs/2511.10021)
*Egor Davydenko,Andrei Volchenkov,Vladimir Gerasimov,Roman Gorbachev*

Main category: cs.RO

TL;DR: 提出一种名为DecARt Leg的新型电动驱动机器人腿部设计，采用准伸缩运动学结构和解耦驱动，具有类人外观和创新的踝关节扭矩传输系统。通过新指标FAST进行量化评估，并通过仿真和硬件实验验证性能。


<details>
  <summary>Details</summary>
Motivation: 开发能够实现敏捷运动的机器人腿部设计，通过解耦驱动和类人结构提升运动性能。

Method: 采用准伸缩运动学结构配合旋转电机实现解耦驱动，设计多杆系统将踝关节扭矩从膝关节上方传递，提出FAST指标进行量化分析。

Result: 通过数值评估、仿真和初步硬件实验验证了DecARt Leg设计的敏捷运动能力。
...
### [196] [Opinion: Towards Unified Expressive Policy Optimization for Robust Robot Learning](https://arxiv.org/abs/2511.10087)
*Haidong Huang,Haiyue Zhu. Jiayu Song,Xixin Zhao,Yaohua Zhou,Jiayi Zhang,Yuze Zhai,Xiaocong Li*

Main category: cs.RO

TL;DR: UEPO是一个统一的生成框架，通过多种子动态感知扩散策略、动态发散正则化机制和基于扩散的数据增强模块，解决离线到在线强化学习中多模态行为覆盖不足和分布偏移问题。
...
Method: 提出多种子动态感知扩散策略来高效捕获多样化模态；动态发散正则化机制确保策略多样性具有物理意义；基于扩散的数据增强模块提升动态模型泛化能力。

Result: 在D4RL基准测试中，UEPO在运动任务上比Uni-O4绝对提升5.9%，在灵巧操作任务上提升12.4%，表现出强大的泛化性和可扩展性。

Conclusion: UEPO框架通过统一的生成方法有效解决了O2O-RL中的关键挑战，在多个任务上取得了显著性能提升。
...
### [197] [Learning a Thousand Tasks in a Day](https://arxiv.org/abs/2511.10110)
*Kamil Dreczkowski,Pietro Vitiello,Vitalis Vosylius,Edward Johns*

Main category: cs.RO

TL;DR: 论文提出了一种基于分解和检索的模仿学习方法MT3，通过将操作轨迹分解为对齐和交互两个阶段，在少量演示（<10次）情况下比传统单阶段行为克隆方法提升了一个数量级的数据效率。


<details>
  <summary>Details</summary>
Motivation: 当前机器人模仿学习方法需要数百到数千次演示才能学习一个任务，而人类从演示中学习任务非常高效。论文旨在研究如何通过分解轨迹和检索式泛化来提高学习效率。

Method: 将操作轨迹分解为顺序对齐和交互两个阶段，开发了基于分解和检索的多任务轨迹迁移方法MT3，支持从单次演示学习任务并泛化到新物体实例。

Result: 在3,450次真实世界测试中，分解方法在少量演示情况下比单阶段学习提升了一个数量级的数据效率。MT3能够在24小时内教会机器人1,000个不同的日常任务，并在2,200次额外测试中验证了其能力和局限性。

Conclusion: 轨迹分解和检索式泛化是提高模仿学习效率的有效方法，MT3方法能够从极少量演示中学习大量任务，为高效机器人技能学习提供了新途径。
...
### [198] [RoboBenchMart: Benchmarking Robots in Retail Environment](https://arxiv.org/abs/2511.10276)
*Konstantin Soshin,Alexander Krapukhin,Andrei Spiridonov,Denis Shepelev,Gregorii Bukhtuev,Andrey Kuznetsov,Vlad Shakhuro*

Main category: cs.RO

TL;DR: 提出了RoboBenchMart基准测试，针对暗店环境中的机器人操作任务，比现有的桌面场景基准更具挑战性和现实性。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作基准主要关注简化的桌面场景，缺乏对现实零售环境中复杂操作的评估能力。

Method: 开发了RoboBenchMart套件，包括程序化商店布局生成器、轨迹生成管道、评估工具和微调基线模型。

Result: 当前最先进的通用模型难以完成常见的零售任务，表明该基准的有效性和挑战性。
...
### [199] [nuPlan-R: A Closed-Loop Planning Benchmark for Autonomous Driving via Reactive Multi-Agent Simulation](https://arxiv.org/abs/2511.10403)
...
Method: 在nuPlan框架中集成基于学习的反应式多智能体模拟，用噪声解耦扩散模型替代基于规则的IDM智能体，并引入交互感知智能体选择机制以确保真实性和计算效率。

Result: 实验表明，新的反应式智能体模型产生了更真实、多样和类人的交通行为，基准环境能更好地反映真实世界的交互驾驶。重新实现的多种规划方法在nuPlan-R中更清晰地展示了学习型规划器在处理复杂动态场景中的优势。

Conclusion: nuPlan-R为公平、反应式和真实的闭环规划评估建立了新标准，将开源代码以促进研究发展。
...
### [200] [Improving dependability in robotized bolting operations](https://arxiv.org/abs/2511.10448)
*Lorenzo Pagliara,Violeta Redondo,Enrico Ferrentino,Manuel Ferre,Pasquale Chiacchio*

Main category: cs.RO

TL;DR: 本文提出了一个用于机器人螺栓连接任务的可信赖控制框架，通过精确的扭矩控制、主动柔顺性和多模态人机界面，提高了操作员的情境感知和故障检测能力。
...
Method: 采用控制架构确保精确的驱动扭矩控制和主动柔顺性，设计多模态人机界面提供实时系统信息可视化，并支持自动与手动控制的无缝切换，高层监督器协调执行并管理控制模式转换。

Result: 在管道法兰连接的螺栓操作中验证，结果显示改进了故障检测能力，增强了操作员情境感知，实现了精确柔顺的螺栓操作执行。

Conclusion: 该框架有效提升了机器人螺栓连接的可信赖性，但依赖单一摄像头实现完全情境感知存在局限性。
...
### [201] [From Fold to Function: Dynamic Modeling and Simulation-Driven Design of Origami Mechanisms](https://arxiv.org/abs/2511.10580)
*Tianhui Han,Shashwat Singh,Sarvesh Patil,Zeynep Temel*

Main category: cs.RO

TL;DR: 提出了一个基于MuJoCo的折纸机构仿真设计框架，通过图形界面定义折纸的几何结构和约束，能够准确模拟折纸的折叠行为和与环境交互，并通过案例研究验证了仿真优化的有效性。


<details>
  <summary>Details</summary>
Motivation: 折纸机构具有轻量化、紧凑和复杂运动能力等优点，在机器人和可展开系统中应用价值日益增加，但准确模拟其折叠行为和环境交互仍然具有挑战性。

Method: 利用MuJoCo的可变形体能力，将折纸表示为相互连接的可变形元素图，通过图形用户界面定义折痕和驱动等约束，生成物理一致的仿真模型。

Result: 通过折纸弹射器的案例研究，使用协方差矩阵自适应进化策略优化设计参数，实验验证优化后的结构实现了更好的投掷性能。

Conclusion: 该系统实现了快速、仿真驱动的折纸设计、优化和分析，为折纸机构的研究和应用提供了有效工具。
...
### [202] [Optimizing the flight path for a scouting Uncrewed Aerial Vehicle](https://arxiv.org/abs/2511.10598)
*Raghav Adhikari,Sachet Khatiwada,Suman Poudel*

Main category: cs.RO

TL;DR: 本文提出使用无人机在灾后环境中进行侦察，通过优化方法规划无人机在最佳高度飞行，以最大化传感器覆盖面积并最小化数据收集的不确定性。
...
Method: 采用基于优化的方法规划无人机路径，在最佳高度飞行以实现传感器覆盖面积最大化和数据收集不确定性最小化。

Result: 该方法能够有效解决灾后非结构化环境中的路径规划问题。

Conclusion: 基于优化的无人机路径规划方法适用于灾后环境侦察任务，能够提高数据收集效率和覆盖范围。
...
### [203] [Robot Crash Course: Learning Soft and Stylized Falling](https://arxiv.org/abs/2511.10635)
...
<details>
  <summary>Details</summary>
Motivation: 尽管双足机器人稳健运动研究取得进展，但在现实世界中仍有摔倒风险。大多数研究关注防止摔倒，而本文专注于摔倒现象本身，旨在减少机器人物理损伤并让用户控制机器人最终姿态。

Method: 提出机器人无关的奖励函数，在强化学习中平衡期望最终姿态达成、冲击最小化和关键部件保护；引入基于仿真的初始和最终姿态采样策略，使策略对广泛初始摔倒条件具有鲁棒性，并能在推理时指定任意未见过的最终姿态。

Result: 通过仿真和真实世界实验证明，即使是双足机器人也能执行受控的软着陆。

Conclusion: 本工作展示了双足机器人能够进行受控软着陆，为机器人摔倒管理提供了新方法。
