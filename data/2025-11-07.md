...
TL;DR: 评估地理空间嵌入在Togo农田制图中的应用效果，发现其能简化工作流程并实现高精度分类，支持土地利用变化评估。


<details>
  <summary>Details</summary>
Motivation: 准确的土地覆盖图对理解土地利用变化至关重要，地理空间嵌入提供更高效的地图制作方式，但在实际应用中仍待探索。
...
TL;DR: 提出了一种称为生成提示的训练方法，通过生成模型创建虚拟样本来直接在整个输入空间中强制执行已知的不变性，相比传统数据增强方法在多个数据集和架构上表现更优。
...
Conclusion: 生成提示方法能够有效学习不变性属性，在视觉分类任务中显著优于传统数据增强方法，为模型训练提供了新的思路。
...
TL;DR: ProM3E是一个概率掩码多模态嵌入模型，用于生态学领域的任意到任意多模态表示生成。该模型基于嵌入空间中的掩码模态重建，能够根据少量上下文模态推断缺失模态。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理生态学中多模态数据融合的模型，支持模态反转，并分析不同模态融合在下游任务中的可行性。

Method: 使用掩码模态重建方法，在嵌入空间中学习推断缺失模态。模型具有概率性质，支持模态反转和跨模态检索。

Result: 提出了新颖的跨模态检索方法，结合了模态间和模态内相似性，在所有检索任务中实现了优越性能。线性探测任务验证了模型的表示学习能力。

Conclusion: ProM3E模型在生态学多模态表示学习方面表现出色，支持任意到任意模态生成，并提供了有效的跨模态检索解决方案。
...
TL;DR: EvtSlowTV是一个从YouTube视频构建的大规模事件相机数据集，包含超过130亿个事件，用于自监督的事件相机深度估计，解决了现有数据集规模小、泛化能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机深度估计方法受限于小规模标注数据集，限制了在真实场景中的泛化能力，需要构建更大规模、更自然的数据集来提升模型性能。

Method: 从公开的YouTube慢动作视频中构建EvtSlowTV数据集，包含各种环境条件和运动场景，采用自监督学习框架利用事件流的高动态范围特性，无需基于帧的标注。

Result: EvtSlowTV比现有事件数据集大一个数量级，训练后的模型在复杂场景和运动中的泛化能力得到显著提升，同时保持了事件数据的异步特性。

Conclusion: EvtSlowTV为事件相机深度估计提供了大规模、自然的数据资源，自监督学习方法有效利用了事件流的高动态范围优势，提升了模型在真实场景中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的混合CNN-ViT架构虽然性能优于纯CNN或ViT，但参数量大、计算成本高，不适合tinyML部署。需要寻找在严格模型大小约束下的高效混合架构。

Method: 设计了一个新的混合CNN-ViT搜索空间，包含混合CNN和ViT块用于学习局部和全局信息，以及新颖的可搜索池化层Pooling块用于高效特征图缩减。

Result: 在CIFAR10数据集上的实验结果表明，所提出的搜索空间能够生成在严格模型大小约束下具有优越准确性和推理速度的混合CNN-ViT架构，优于基于ResNet的tinyML模型。

Conclusion: 提出的混合CNN-ViT搜索空间能够有效找到适合tinyML部署的高效混合架构，在保持高性能的同时满足严格的模型大小约束。
...
TL;DR: SCALE-VLP是一个软加权对比视觉语言预训练框架，专门针对医学CT等体数据设计，整合了体空间语义和领域知识语义，在有限监督下实现结构一致和语义基础的表示。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要局限于2D数据和二元监督，忽略了体数据中的连续结构化依赖关系，且通常将体扫描视为独立2D切片处理，损害了空间连贯性和临床语义利用。

Method: 提出SCALE-VLP框架，整合(i)体空间语义以保持解剖结构，(ii)领域感知的知识注入语义（如放射学本体）来指导对齐，采用软加权对比预训练方法。

Result: 在CT报告检索中达到比先前最佳方法高4.3倍的top-1准确率，异常分类提升10个百分点，报告生成的ROUGE-L达到0.44，BERT-F1达到0.89。在零样本跨域评估中表现出持续增益。

Conclusion: SCALE-VLP展示了强大的跨任务可转移性和跨域泛化能力，无需进一步微调即可在检索、报告生成和分类等任务中实现一致性能提升。
...
TL;DR: 本研究提出了一种基于自监督学习的高效标签方法，仅使用1000个标注图像块实现州级1米分辨率土地覆盖分类，通过BYOL预训练策略在大量未标记图像上训练ResNet-101编码器，然后迁移到多种语义分割架构进行微调。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率土地覆盖分类中大规模标注数据收集困难的挑战，探索自监督学习在减少人工标注需求方面的有效性。

Method: 使用BYOL自监督预训练策略在377,921个未标记彩色红外航空图像上训练ResNet-101编码器，然后将学习到的权重迁移到多种语义分割架构（FCN、U-Net、Attention U-Net、DeepLabV3+、UPerNet、PAN），使用少量标注数据（250、500、750个图像块）进行微调。

Result: 在密西西比州超过1230亿像素的区域上实现了87.14%的总体准确率和75.58%的宏观F1分数，准确映射了开阔水域和森林区域，但在耕地、草本植物和贫瘠土地覆盖类型之间的精确划分方面存在挑战。

Conclusion: 自监督学习是减少大规模人工标注数据需求的有效策略，直接解决了大规模高空间分辨率土地覆盖制图的主要限制因素。
...
<details>
  <summary>Details</summary>
Motivation: 在扫描到BIM、变形分析等高精度应用中，评估移动激光扫描点云的不确定性至关重要，但获取地面真值成本高昂且在实际应用中往往不可行。

Method: 开发了集成最优邻域估计与几何特征提取的学习框架，使用XGBoost和随机森林模型进行不确定性预测。
...
<details>
  <summary>Details</summary>
Motivation: 解决建筑机器人感知中缺乏专用图像数据集的问题，因为在活跃建筑工地上收集照片存在物流困难和安全隐私问题。

Method: 创建混合数据集ISC-Perception，包含程序化渲染的CAD图像、游戏引擎照片级真实场景和少量精选真实照片，实现合成部分的完全自动标注。

Result: 生成10,000张图像数据集的人工时间仅为30.5小时，比手动标注节省81.7%；训练出的检测器在IoU 0.50下达到0.756的平均精度，显著优于仅使用合成或照片级数据训练的模型。
...
TL;DR: 提出DentalSplat框架，解决正畸治疗中仅使用前视图和双侧颊视图三张稀疏图像进行3D重建的挑战，通过先验引导的密集立体重建、尺度自适应剪枝和光流几何约束等方法，在稀疏输入下实现高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 在远程正畸治疗中，从多视角观察患者牙合情况对及时临床决策很重要。传统3D高斯泼溅方法需要密集多视角输入和精确相机位姿，而正畸病例通常只有三张稀疏图像，导致重建质量严重下降。

Method: 1. 使用先验引导的密集立体重建模型初始化点云；2. 采用尺度自适应剪枝策略提高训练效率和重建质量；3. 在极稀疏视角下加入光流几何约束和梯度正则化来增强渲染保真度。

Result: 在包含950个临床病例的大规模数据集和195个视频测试集上验证，该方法能有效处理稀疏输入场景，在牙合可视化新视角合成质量上优于现有最优技术。

Conclusion: DentalSplat框架成功解决了正畸图像稀疏性带来的重建挑战，为远程正畸治疗提供了有效的3D可视化解决方案。
...
TL;DR: 提出了IC DefectNCD框架，利用IC SEM图像的固有先验进行缺陷检测和新类别发现，无需支持集，在真实世界数据集上验证了稳健性能。


<details>
  <summary>Details</summary>
Motivation: 集成电路制造过程复杂，缺陷可能导致良率损失和可靠性下降。监督方法需要大量人工标注且难以处理新类别和罕见缺陷，基于聚类的方法因缺乏先验而性能不稳定。

Method: 1. 自归一化信息引导的缺陷检测：通过可学习的正常信息提取器聚合代表性正常特征，利用重建残差粗定位缺陷区域；2. 自适应二值化策略处理缺陷显著性变化；3. 自缺陷信息引导的分类：使用软掩码引导注意力机制将空间缺陷先验注入师生模型。

Result: 在涵盖三个关键制造阶段和15种缺陷类型的真实世界数据集上验证，实验表明在缺陷检测和未见缺陷分类方面均表现出稳健性能。

Conclusion: IC DefectNCD框架有效解决了IC制造中缺陷检测和新类别发现的挑战，通过利用图像固有先验实现了无需支持集的稳健性能。
...
Result: 在ABO数据集上实现62.9-287.2倍加速，物体级物理属性估计精度相当或略优，在材料分割和体素级推理上优于SOTA基线。在IKEA家具店的案例研究中，即使在真实场景中视图较少也能保持高性能。

Conclusion: \sysname通过系统优化显著加速了物理属性推理，为智能眼镜等实时应用提供了可行的解决方案，在真实世界场景中表现出色。
...
TL;DR: 本文开发并部署了首个用于无人机影像中建筑物损坏评估的AI/ML系统，在联邦宣布的飓风灾害中操作使用，解决了灾害响应中数据过载的问题。


<details>
  <summary>Details</summary>
Motivation: 灾害响应中无人机团队每天收集大量影像数据（47GB-369GB），远超专家处理能力，导致响应延迟，需要计算机视觉和机器学习技术来缓解数据过载问题。

Method: 基于最大的灾后无人机航空影像数据集（包含21,716个建筑物损坏标签）训练模型，并对91名灾害从业者进行操作培训，在飓风Debby和Helene响应中部署最佳性能模型。

Result: 在飓风Debby和Helene响应中，系统在约18分钟内评估了总计415座建筑物，建立了基于无人机的损坏评估系统的实践标准。
...
TL;DR: 提出了一种深度引导的肝脏标志点分割框架，结合语义和几何线索，使用SAM2和DA2编码器提取RGB和深度特征，并引入SRFT-GaLore方法进行高效微调。


<details>
  <summary>Details</summary>
Motivation: 在腹腔镜肝脏手术中，2D视频流限制了深度感知，使标志点定位复杂化。现有方法在融合RGB和深度特征以及将大规模视觉模型适应手术领域方面存在挑战。

Method: 使用SAM2编码器提取RGB特征，DA2编码器提取深度感知特征，引入SRFT-GaLore方法替代计算昂贵的SVD，采用交叉注意力融合模块整合RGB和深度线索。

Result: 在L3D数据集上，Dice相似系数提高4.85%，平均对称表面距离降低11.78点。在LLSD数据集上保持竞争力，显著优于基于SAM的基线方法。
...
### [15] [SurgAnt-ViVQA: Learning to Anticipate Surgical Events through GRU-Driven Temporal Cross-Attention](https://arxiv.org/abs/2511.03178)
...
TL;DR: 提出了首个用于手术前瞻性推理的视觉问答数据集PitVQA-Anticipation，并开发了SurgAnt-ViVQA视频语言模型，通过GRU门控时间交叉注意力模块实现手术步骤预测。


<details>
  <summary>Details</summary>
Motivation: 在经鼻蝶垂体手术中，由于视野受限和工作流程快速变化，预测未来手术事件对实时辅助至关重要。现有VQA系统主要关注静态场景，缺乏对未来步骤的预测能力。

Method: 构建包含33.5小时手术视频和734,769个问答对的数据集，提出SurgAnt-ViVQA模型，使用双向GRU编码帧间动态，通过自适应门控在标记级别注入视觉上下文到语言流中。

Result: 在PitVQA-Anticipation和EndoVis数据集上测试，SurgAnt-ViVQA超越了基于图像和视频的基线方法。消融实验显示时间循环和门控融合是性能提升的主要因素。

Conclusion: 通过结合时间感知编码器和细粒度门控交叉注意力，SurgAnt-ViVQA将手术VQA从回顾性描述推进到主动预测，为可靠、前瞻性的手术辅助提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 目前结合功能性和解剖性影像与详细临床报告的跨多种癌症类型的大型数据集仍然稀缺，这阻碍了人工智能模型的开发和验证以及回顾性临床研究。

Method: 收集了490名各种恶性肿瘤患者的全身18F-FDG PET/CT扫描，包括肺癌、肝癌、乳腺癌、前列腺癌和卵巢癌等常见癌症，提供配对的PET和CT图像、去标识化文本报告和结构化临床元数据。

Result: 创建了一个包含多种癌症类型的综合医学影像数据集PETWB-REP，为医学研究提供了宝贵资源。

Conclusion: PETWB-REP数据集填补了结合功能性和解剖性影像与临床报告的跨癌症类型数据集的空白，将促进医学影像、人工智能和多模态学习领域的研究发展。
...
TL;DR: 本文提出了一种新的零样本提示方法QG-CoC，用于解决多模态大语言模型在多图像场景下的细粒度感知和跨图像推理问题，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多图像环境中存在两个关键问题：缺乏对分散图像的细粒度感知能力，以及跨多个视觉输入进行有效推理和信息综合的能力不足。现有研究主要关注单图像或特定受限场景，缺乏对通用复杂多图像推理任务的理解和解决方案。

Method: 提出QG-CoC（问题引导的标题链）方法，这是一种通用的零样本提示方法，能够有效处理任意数量图像的问题。该方法通过问题引导的方式生成图像标题链，实现感知与推理的无缝集成。

Result: 在多个开源和闭源多模态大语言模型上进行评估，实验结果表明QG-CoC在各种任务中表现出竞争力，并在现有提示方法失败的挑战性场景中展现出稳健的改进。

Conclusion: QG-CoC方法有效解决了多模态大语言模型在多图像场景中的感知和推理问题，为处理复杂多图像任务提供了一种有效的零样本解决方案。
...
TL;DR: 提出了一种结合样本混合和扩散合成的数据增强方法MCPMix，通过掩码一致的配对混合和真实锚定学习退火策略，在保持像素级语义的同时增加多样性，在多个医学分割数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统的数据增强方法存在局限性：样本混合会产生软标签模糊问题，扩散合成会引入合成-真实域偏移。需要一种能够融合两者优势、在增加多样性的同时保持语义一致性的方法。

Method: 1. 掩码一致的配对混合(MCPMix)：为每个真实图像生成相同掩码下的合成对应图像，只混合外观而保持原始硬掩码监督；2. 真实锚定学习退火(RLA)：自适应调整混合强度和混合样本的损失权重，逐步将优化重新锚定到真实数据。
...
TL;DR: 本文提出了一个针对真实工业场景的点云异常检测数据集IEC3D-AD，并基于此开发了一种新的3D异常检测范式GMANet，通过几何形态分析生成合成点云样本，并通过空间差异优化来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D数据集如Real3D-AD和MVTec 3D-AD在捕捉真实工业环境中的复杂性和细微缺陷方面存在不足，这限制了工业设备组件（如轴承、环、螺栓）的精确异常检测研究。

Method: 开发了IEC3D-AD数据集，直接从实际生产线收集；提出了GMANet范式，基于几何形态分析生成合成点云样本，并通过空间差异优化来减小正常和异常点级特征之间的差距并增加重叠度。

Result: 在IEC3D-AD和其他数据集上的广泛实验证明了该方法的有效性。

Conclusion: IEC3D-AD数据集具有更高的点云分辨率和缺陷标注粒度，GMANet范式通过创新的几何形态分析和空间优化策略，显著提升了3D异常检测的性能。
...
TL;DR: 提出了一种统一的长视频修复和外推方法，通过扩展文本到视频扩散模型来生成任意长度、空间编辑的高保真视频，解决了长视频生成和可控编辑的挑战。


<details>
  <summary>Details</summary>
Motivation: 长视频生成是基础性挑战，实现视频修复和外推的高可控性尤为困难。需要同时解决这两个问题，实现长视频剪辑的可控修复和外推。

Method: 利用LoRA高效微调预训练视频扩散模型（如阿里Wan 2.1）进行掩码区域视频合成，采用重叠混合时间协同去噪策略和高阶求解器保持长序列一致性。

Result: 在数百帧的修复/外推任务中验证，在质量指标（PSNR/SSIM）和感知真实性（LPIPS）上优于Wan 2.1和VACE基线方法，实现无可见接缝或漂移的任意长度视频生成。

Conclusion: 该方法实现了参数效率与优越性能的平衡，以最小开销实现实用的长范围视频编辑。
...
TL;DR: 本文提出Diffusion-SDPO方法，解决扩散模型直接偏好优化中的关键问题：扩大偏好边界不一定提高生成质量，反而可能增加重构误差。新方法通过自适应缩放失败者梯度来保护获胜者，确保偏好输出的误差不增加。
...
TL;DR: SurgViVQA是一个用于外科视频问答的模型，通过将视觉推理从静态图像扩展到动态手术场景，利用掩码视频-文本编码器融合视频和问题特征，捕获时间线索如运动和工具-组织交互，然后由微调的大型语言模型解码为连贯答案。


<details>
  <summary>Details</summary>
Motivation: 当前方法局限于静态图像特征，可用数据集缺乏时间注释，忽略了准确程序解释所需的关键动态信息。

Method: 提出SurgViVQA模型，使用掩码视频-文本编码器融合视频和问题特征，捕获时间线索，然后由微调的大型语言模型解码答案。同时创建了REAL-Colon-VQA数据集进行评估。

Result: 在REAL-Colon-VQA和EndoVis18-VQA数据集上的实验验证显示，SurgViVQA优于现有的基于图像的VQA基准模型，在关键词准确率上比PitVQA提高了11%（REAL-Colon-VQA）和9%（EndoVis18-VQA）。
...
Method: 采用双分支联合合成架构，包含两个并行扩散变换器构建统一的跨模态潜在空间；核心是不对称跨模态交互机制，实现双向、时间对齐的跨注意力；增强的面部感知调制模块动态优先处理显著区域；推理时使用模态感知无分类器引导策略。
...
TL;DR: AAPL是一种新的提示学习方法，通过引入对抗性token嵌入来解耦图像增强引入的表层视觉变化与类别相关语义表示，从而提升零样本学习的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的提示学习方法如CoOp和CoCoOp主要关注文本修改，而忽略了图像级增强的潜力，特别是在处理未见类别时泛化能力不足的问题。

Method: 提出AAPL方法，通过对抗性token嵌入来分离图像增强带来的视觉变化与语义表示，使学习到的提示能够专注于与目标类别对齐的视觉判别特征。

Result: 在11个基准数据集上的综合实验表明，AAPL在小样本、零样本、跨数据集和领域泛化设置中均优于现有方法。

Conclusion: 图像级增强特别是属性特定变化能够有效支持提示学习，AAPL通过解耦策略显著提升了模型的泛化能力。
...
TL;DR: 提出了一种自动化方法，通过主成分分析（PCA）从胚胎分割掩模中提取主轴线，生成四个候选方向，并使用三种策略选择标准方向，在2166张三维超声图像上实现了98.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 标准化胚胎在三维超声图像中的对齐有助于产前生长监测，促进标准平面检测，改善地标可视化和突出不同扫描之间的差异。

Method: 基于胚胎分割掩模应用主成分分析提取主轴线，生成四个候选方向，使用三种策略选择标准方向：基于皮尔逊相关性的启发式方法、通过归一化互相关的图谱匹配和随机森林分类器。

Result: 在2166张图像中，PCA正确提取胚胎主轴线的比例为99.0%。皮尔逊启发式、图谱匹配和随机森林分别正确选择候选方向的准确率为97.4%、95.8%和98.4%，多数投票策略达到98.5%的准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的形状模板重建方法在处理伴随拓扑变化的变形时会失效，需要扩展该方法以应对实际应用中常见的拓扑变化。

Method: 基于经典SfT解决方案初始化，通过迭代分区模板空间，最小化包含物理合理性和重投影一致性的能量函数。
...
TL;DR: Anny是一个基于人体测量学知识的开源、可微分人体模型，无需3D扫描，通过表型参数控制人体形状，支持从婴儿到老人的广泛年龄范围和体型变化，并基于WHO统计数据校准，提供真实的人口统计学基础。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型依赖昂贵的3D扫描和专有学习形状空间，且人口统计学覆盖范围有限。需要开发一个简单、开源、基于人体测量学知识的人体模型。
...
Conclusion: Anny提供了一个开放、语义可控的人体建模基础，其简单性不影响性能，同时保持可解释性和广泛代表性，为以人为中心的3D建模提供了可访问的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交互动检测方法忽视了面部表情、注视、手势等细微线索，主要依赖个体的整体表示，且直接检测社交群体而未显式建模个体间的底层互动，这限制了捕捉局部社交信号的能力。

Method: 首先检测个体并使用部位感知线索增强其特征，然后通过基于相似性的推理关联个体来推断群体配置，该推理不仅考虑空间关系，还考虑表示互动的细微社交线索。

Result: 在NVI数据集上的实验表明，该方法优于先前方法，达到了新的最先进水平。

Conclusion: 提出的部位感知自下而上群体推理框架能够更准确地捕捉细粒度社交互动，通过显式建模个体间的互动和利用细微社交线索，实现了更精确的群体推断。
...
TL;DR: DANCE框架通过分离运动动态、物体和场景概念来改进视频动作识别的可解释性，在保持竞争力的同时显著提升解释清晰度。


<details>
  <summary>Details</summary>
Motivation: 现有视频动作识别模型的解释方法存在两个问题：基于显著性的方法产生纠缠的解释，无法区分运动和空间上下文；基于语言的方法难以解释默会知识性质的运动。

Method: 提出DANCE框架，使用解耦的概念类型（运动动态、物体、场景）进行预测。运动动态概念定义为人体姿态序列，使用大语言模型自动提取物体和场景概念，并采用前向概念瓶颈设计强制通过这些概念进行预测。

Result: 在KTH、Penn Action、HAA500和UCF-101四个数据集上的实验表明，DANCE显著提高了解释清晰度且保持竞争力性能。用户研究验证了其优越的可解释性。

Conclusion: DANCE框架不仅提高了视频动作识别的可解释性，还证明了对模型调试、编辑和失败分析有益。
...
### [33] [Automatic Machine Translation Detection Using a Surrogate Multilingual Translation Model](https://arxiv.org/abs/2511.02958)
*Cristian García-Romero,Miquel Esplà-Gomis,Felipe Sánchez-Martínez*

Main category: cs.CL

TL;DR: 提出了一种利用多语言机器翻译模型的内部表示来区分人工翻译和机器翻译句子的新方法，在非英语语言对上比现有技术至少提高5个百分点的准确率。


<details>
  <summary>Details</summary>
Motivation: 现代机器翻译系统依赖从互联网收集的大规模平行语料，但其中大量文本是机器生成的翻译，过度依赖这类合成内容会显著降低翻译质量，因此过滤非人工翻译成为构建高质量MT系统的关键预处理步骤。

Method: 利用替代多语言机器翻译模型的内部表示来直接区分人工翻译和机器翻译句子。

Result: 实验结果显示该方法优于当前最先进技术，特别是在非英语语言对上，准确率至少提高5个百分点。

Conclusion: 提出的方法能有效识别机器翻译内容，为构建高质量机器翻译系统提供了重要的预处理解决方案。
...
TL;DR: 本文提出了LEGO-Eval评估框架和LEGO-Bench基准，用于评估细粒度指令与生成的3D场景之间的对齐程度，解决当前方法在3D场景生成评估中的不足。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的3D场景生成方法缺乏真实的空间布局和物体属性，这源于粗粒度指令的不足。需要更细粒度的指令来生成真实世界环境，否则训练出的具身智能体会学习到与现实世界物理和语义显著偏离的先验知识。

Method: 提出了LEGO-Eval评估框架，配备多样化工具来显式地锚定场景组件，实现更准确的对齐评估。同时提出了LEGO-Bench基准，包含详细指令来指定真实世界环境的复杂布局和属性。

Result: 实验表明LEGO-Eval在评估场景-指令对齐方面比VLM-as-a-judge方法高出0.41 F1分数。使用LEGO-Bench基准测试显示当前生成方法存在显著局限性，所有评估方法在生成完全符合细粒度指令的场景时成功率最多只有10%。

Conclusion: LEGO-Eval和LEGO-Bench为解决3D场景生成中的对齐评估问题提供了有效工具，揭示了当前方法在生成真实世界场景方面的严重不足。
...
TL;DR: 本文针对方面级情感分析(ABSA)在教育和医疗等低资源领域的研究不足，提出了三个主要贡献：新的评估方法FTS-OBP、小型生成式语言模型在ABSA任务中的研究，以及首个教育评论ABSA资源集的发布。


<details>
  <summary>Details</summary>
Motivation: 当前ABSA研究主要集中在商业领域，而教育和医疗等高需求领域资源匮乏。传统评估方法过于严格，且现有方法依赖资源密集型知识注入，阻碍了这些领域的发展。

Method: 提出FTS-OBP评估方法，研究小型解码器生成语言模型(SLMs)，探索无数据(上下文学习和权重合并)和轻数据微调方法，并提出多任务微调策略。

Result: 1.5-3.8B参数的小型模型仅用200-1,000个示例就能超越专有大型模型，接近基准结果，且FTS-OBP方法与传统指标保持强相关性。

Conclusion: 该研究为低资源领域的ABSA提供了有效解决方案，证明了小型模型在有限资源下的潜力，并为未来研究提供了首个教育评论ABSA资源集。
...
TL;DR: 该论文提出单边对话问题(1SC)，研究在只能记录对话单边的情况下如何推断和学习对话内容，包括实时重建缺失方对话和生成摘要，通过实验验证了多种方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多场景只能记录对话的单边（如远程医疗、呼叫中心、智能眼镜等），需要开发能够从单边对话中推断和学习的技术。

Method: 使用提示工程和微调模型在MultiWOZ、DailyDialog和Candor数据集上进行评估，采用人工A/B测试和LLM作为评判指标，探索了访问未来一轮对话信息、话语长度信息以及占位符提示等方法。

Result: 研究发现访问未来一轮对话和话语长度信息能改善重建效果，占位符提示有助于减少幻觉，大模型通过提示能生成有前景的重建结果，而小模型需要微调，高质量摘要可以在不重建缺失对话的情况下生成。

Conclusion: 1SC是一个新颖的挑战，研究结果展示了向隐私感知对话AI迈进的进展，证明了从单边对话中有效推断和学习的可行性。
...
TL;DR: PolyNorm是一种基于提示的大语言模型文本归一化方法，旨在减少对手工规则的依赖，通过自动数据管理和评估管道在多语言环境中实现更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统文本归一化系统虽然准确率高，但需要大量工程投入，难以扩展，且在低资源语言中覆盖困难。

Method: 使用基于提示的大语言模型进行文本归一化，并开发了语言无关的自动数据管理和评估管道。

Result: 在八种语言的实验中，相比生产级系统，词错误率持续降低。

Conclusion: PolyNorm方法有效减少了对手工规则的依赖，支持多语言文本归一化研究，并发布了多语言数据集PolyNorm-Benchmark。
...
<details>
  <summary>Details</summary>
Motivation: 精神分裂症患者的语言障碍反映了潜在的认知障碍，可能作为症状严重程度和诊断的客观标记。研究旨在通过计算语言学方法量化这些语言特征。

Method: 使用计算模型计算语言中的惊奇度和语义连贯性，比较精神分裂症患者与健康对照组的差异，并分析这些指标与症状严重程度的变化关系。

Result: 研究发现精神分裂症患者在惊奇度和语义连贯性方面与健康对照组存在显著差异，且这些语言指标随症状严重程度而变化。

Conclusion: 计算语言学指标如惊奇度和语义连贯性能够有效表征精神分裂症的语言障碍，为症状评估和诊断提供了客观的量化工具。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语人群的心理健康检测资源有限，且存在文化污名化问题，而现有研究主要集中在英语领域，阿拉伯语心理健康检测研究严重不足。
...
Conclusion: 该研究为阿拉伯语心理健康检测提供了重要数据集和方法论基础，有望推动该领域的发展。
...
TL;DR: 本文提出了一种基于控制理论的框架，通过控制屏障函数来确保大语言模型生成用户期望的文本，无需对基线模型进行微调。


<details>
  <summary>Details</summary>
Motivation: 为了确保大语言模型生成符合用户期望的文本，同时避免对基线模型进行昂贵的微调过程。

Method: 使用控制屏障函数作为安全过滤器，对基线LLM生成的预测token进行干预，该过滤器是附加型的，可直接应用于对齐评估模型。

Result: 基于开源语言模型实现了整体文本生成系统，能够生成积极的文本内容。

Conclusion: 提出的控制屏障函数框架为LLM对齐提供了一种无需微调的有效方法，可直接利用现有的对齐评估模型进行安全过滤。
...
TL;DR: MME-CC是一个多模态认知能力评估基准，专注于视觉中心认知行为，将11个推理任务分为空间、几何和知识推理三类，对16个主流MLLM进行系统评估。
...
Method: 构建MME-CC基准，组织11个代表性推理任务到三个视觉信息基础类别：空间、几何和知识推理，对16个代表性MLLM进行广泛实验。

Result: 闭源模型整体领先（如Gemini-2.5-Pro得分为42.66 vs GLM-4.5V的30.45），空间和几何推理普遍较弱（≤30%）。发现常见错误模式包括方向错误、跨视图身份持久性脆弱、反事实指令遵循差等。
...
TL;DR: 提出一个基于LLM的利益相关者风险评估框架，通过让LLM扮演法官角色来预测和解释AI系统中的风险，生成可解释的、针对特定利益相关者的政策，并揭示不同利益相关者对相同风险的分歧。
...
Result: 在医疗AI、自动驾驶和欺诈检测三个真实案例中验证，结果显示利益相关者视角显著影响风险感知和冲突模式。
...
TL;DR: 对12种不确定性估计方法在LLM问答任务中的实证研究，分析了ID和OOD数据集下不同方法对aleatoric和epistemic不确定性的表现。
...
Method: 使用12种UE方法和4种生成质量指标（包括LLMScore），在问答任务的ID和OOD数据集上评估LLM生成答案的不确定性。

Result: 信息基方法在ID设置中表现优异，密度基方法和P(True)指标在OOD场景中表现更好，语义一致性方法在不同数据集和指标中表现可靠。

Conclusion: 不同UE方法在不同场景下各有优势，信息基方法适合ID设置，密度基方法适合OOD场景，语义一致性方法具有普遍适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有伦理基准主要基于英语和西方框架，忽视了文化差异，而孟加拉语作为全球第六大语言（2.85亿使用者）的伦理对齐研究不足，需要开发符合当地文化背景的评估工具。

Method: 通过母语者共识标注构建基准，包含5个道德领域（日常活动、习惯、育儿、家庭关系、宗教活动）和50个子主题，使用美德伦理、常识伦理和正义伦理三种伦理视角进行标注。

Result: 对Llama、Gemma、Qwen、DeepSeek等多语言LLM进行零样本评估，准确率在50-91%之间波动，定性分析显示模型在文化基础、常识推理和道德公平性方面存在一致弱点。

Conclusion: BengaliMoralBench为负责任的本土化提供了基础，支持在孟加拉等低资源多语言环境中部署符合文化伦理的AI系统。
...
TL;DR: 提出语言图模型（LGM）来解决LLM在处理模糊或概念不对齐术语时的困难，通过提取元关系并使用反思机制验证，结合概念迭代检索算法动态提供相关信息，无需截断即可处理任意长度文本。
...
Method: 从自然语言中提取继承、别名和组合等元关系，使用反思机制验证这些关系，通过概念迭代检索算法动态向LLM提供相关描述。

Result: 在标准基准测试中，LGM持续优于现有的RAG基线方法。

Conclusion: LGM通过增强概念清晰度和动态提供相关信息，有效提升了LLM处理模糊概念的能力，无需截断即可处理任意长度文本。
...
TL;DR: ISI的SARAL项目在IARPA的MATERIAL计划中开发了跨语言信息检索新方法，专注于检索查询相关的文档集合而非仅排名列表，在第三阶段评估中在波斯语、哈萨克语和格鲁吉亚语等三种语言的六项评估条件中五项超越其他团队。


<details>
  <summary>Details</summary>
Motivation: 推进跨语言信息检索技术，特别是在检索查询相关文档集合而非仅排名文档列表方面的能力。

Method: 开发了新颖的跨语言信息检索方法，专注于检索文档集合而非传统排名列表。

Result: 在MATERIAL第三阶段评估中，SARAL在三种语言（波斯语、哈萨克语、格鲁吉亚语）的六项评估条件中有五项表现优于其他团队。

Conclusion: SARAL的方法在跨语言信息检索方面表现优异，特别是在检索文档集合方面取得了显著成果。
...
<details>
  <summary>Details</summary>
Motivation: 为多语言大语言模型设计有效的tokenizer具有挑战性，因为存在多样化的文字和丰富的形态变化。现有的子词方法（如BPE）在多语言环境中的有效性尚未充分探索。

Method: 结合子词和多词分词方法，采用语言特定的预分词策略，开发了IndicSuperTokenizer。

Result: 在英语、22种印度语言和代码数据上的评估显示，平均生育率得分比LLaMA4提高39.5%，比Sutra提高18%，推理吞吐量比LLaMA4提高44%，同时在英语和印度语言基准测试中保持相当性能。

Conclusion: 通过详细的消融实验验证了设计选择的稳健性，表明IndicSuperTokenizer在多语言环境中具有优越性能。
...
TL;DR: 本研究比较了四种开源LLM（Mistral-7b-instruct、LLaMa2-7b-chat、Falcon-7b-instruct、Orca-mini-v3-7b）和GPT-3.5在计算机科学文献QA任务中的表现，使用RAG技术减少幻觉。GPT-3.5+RAG表现最佳，Mistral-7b-instruct在开源模型中表现最好，Orca-mini-v3-7b响应延迟最短。


<details>
  <summary>Details</summary>
Motivation: 随着RAG技术与LLMs的日益重要，需要比较不同LLMs在不同领域问答任务中的性能表现，特别是在计算机科学文献领域。

Method: 使用RAG技术支持，在计算机科学文献QA任务中评估四种开源LLM和GPT-3.5。评估指标包括二元问题的准确率和精确率，人类专家排名，Gemini AI排名，以及长答案问题的余弦相似度。

Result: GPT-3.5+RAG在二元和长答案问题中表现最佳。在开源LLMs中，Mistral-7b-instruct+RAG表现最好，Orca-mini-v3-7b响应延迟最短，LLaMa2-7b-chat延迟最高。

Conclusion: 研究表明开源LLMs在更好的基础设施支持下可以与GPT-3.5等专有模型相媲美，强调了开源模型的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前持续预训练的进展更多地依赖于扩展正确的结构而非仅仅扩展参数数量，需要解决深度扩展导致的严重遗忘问题。

Method: 采用宽度扩展架构，插入轻量级扩展模块，冻结预训练参数，通过保持性初始化和选择性训练扩展组件来获取新知识。提供SCALE-Preserve、SCALE-Adapt和SCALE-Route三种变体。

Result: 在合成传记基准测试中，SCALE缓解了深度扩展的严重遗忘问题；在韩语语料持续预训练中，SCALE变体在英语评估中遗忘较少，在韩语基准测试中取得有竞争力的提升，提供了最佳稳定性-可塑性权衡。

Conclusion: SCALE架构通过保持性保留和协作适应的原则，在持续预训练中实现了更好的知识保留和新知识获取平衡，相比标准持续学习设置具有更稳定的优化特性。
...
TL;DR: 该论文评估了多模态大语言模型（MLLMs）的"思维模式"在临床任务中的表现，发现相比标准"非思维模式"，思维模式带来的改进有限，特别是在复杂医疗任务中表现欠佳。
...
Method: 评估了Seed1.5-VL和Gemini-2.5-Flash两个领先MLLMs的"思维模式"能力，使用VQA-RAD和ROCOv2数据集在四个视觉医疗任务上进行测试。

Result: 激活思维模式相比标准非思维模式，在大多数任务中改进有限；在开放式VQA和医学图像解释等复杂医疗任务中表现欠佳。

Conclusion: 当前MLLMs在医疗应用中需要领域特定的医学数据和更先进的医学知识整合方法。
...
TL;DR: 这篇综述系统评估了生成式人工智能在生物信息学中的变革性应用，包括基因组学、蛋白质组学、结构生物学和药物发现等领域，通过六个研究问题分析了方法学进展、预测性能和专业化优势。


<details>
  <summary>Details</summary>
Motivation: 系统识别和评估生成式人工智能在生物信息学中日益增长的发展，评估有影响力的GenAI策略在方法学进展、预测性能和专业化方面的表现，并确定先进建模、数据密集型发现和整合生物学分析的有前景方法。

Method: 采用系统综述和荟萃分析方法，提出六个研究问题(RQs)，评估生成式AI在生物信息学多个子领域（序列分析、分子设计、整合数据建模）的应用，比较专业模型架构与通用模型的性能差异。

Result: 生成式AI在生物信息学多个领域表现出优于传统方法的性能，专业模型架构通过针对性预训练和上下文感知策略优于通用模型，在分子分析、数据整合、结构建模、功能预测和合成数据生成方面取得显著改进。

Conclusion: 生成式AI在生物信息学中展现出巨大潜力，但面临可扩展性不足和数据偏差等限制，未来应关注稳健评估和基于生物学的建模，分子数据集、细胞数据集和文本资源广泛支持GenAI模型的训练和泛化。
...
TL;DR: 本文提出了沉默偏见的概念，并开发了Silenced Bias Benchmark (SBB)基准测试，通过激活导向技术减少模型在问答过程中的拒绝响应，从而揭示安全对齐大语言模型中隐藏的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型公平性的方法通常采用标准问答方案，但这种方法将模型的拒绝响应视为积极的公平性测量，忽视了模型中存在的深层偏见问题，造成了公平性的假象。

Method: 提出SBB基准测试，采用激活导向技术来减少模型在问答过程中的拒绝响应，从而揭示模型中隐藏的沉默偏见。该方法支持扩展到新的人口群体和主题，提供了一个超越对齐训练掩蔽效应的公平性评估框架。

Result: 在多个大语言模型上的实验结果表明，模型的直接响应与其底层公平性问题之间存在显著差异，暴露了安全对齐模型中的隐藏偏见问题。

Conclusion: SBB提供了一个有效的公平性评估框架，能够揭示安全对齐大语言模型中隐藏的偏见，促进未来开发更公平的模型和工具。
...
<details>
  <summary>Details</summary>
Motivation: 对于复杂的逻辑数据增强，过度依赖人工标注成本高昂，而直接使用大语言模型生成会产生难以解释且逻辑同质化的示例。

Method: 提出LFC-DA管道：首先将逻辑文本映射为命题表达式，编译紧凑规则库，通过有界状态空间搜索系统性地发现有效公式，然后将其语言化回自然语言问题。

Result: 在ReClor和LogiQA上的实验显示，预训练模型的逻辑推理准确性有显著提升。
...
TL;DR: 研究发现对称BPE分词方法在机器翻译中并非最优，提出使用不对称BPE（源语言和目标语言采用不同合并操作次数）能显著提升性能，特别是在低资源场景下。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译研究通常使用对称BPE分词方法，但这种方法不能保证在不同语言对和数据规模下都能达到最优性能。

Method: 研究不同数据量和语言对下的BPE分词策略，评估不对称BPE（源语言和目标语言使用不同合并操作次数）对机器翻译系统性能的影响。

Result: 不对称BPE在低资源设置（50K、100K和500K句对）中显著优于对称方法，在英语-印地语翻译中平均提升5.32、4.46和0.7 CHRF++分数。在6个额外语言对中，12个系统中有10个观察到统计显著改进。

Conclusion: 源语言使用高NMO（4K-32K）和目标语言使用低NMO（0.5K-2K）的不对称BPE配置在低资源机器翻译中效果最佳。
...
<details>
  <summary>Details</summary>
Motivation: 研究小语言模型在完整RDF图抽取中如何处理数据属性和对象属性，解决稀有属性长尾分布带来的性能瓶颈问题。

Method: 评估了分层抽样、加权损失、数据集扩展和基于模板的合成数据增强等多种策略，通过实验比较不同方法的效果。

Result: 发现最佳策略是构建训练集，确保每个属性的出现次数超过特定阈值，从而在不平衡目标属性上实现均衡性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要压缩冗长的推理输出，但仍依赖显式推理。3TF采用短到长视角，旨在让模型内部化结构化推理，在推理时使用无思想模式输出简洁结果。

Method: 首先训练混合模型支持推理和非推理模式，然后在CoT标注数据上进一步训练以内部化结构化推理，推理时使用无思想模式强制输出简洁结果。
...
<details>
  <summary>Details</summary>
Motivation: QA系统中的输入错误经常导致错误回答，LLM在处理这类任务时存在误解用户意图或过度修正的问题。

Method: 使用外部知识（如搜索结果、相关实体）增强输入来应对误解问题，采用强化学习来对齐模型目标以实现精确修正而非简单改写。
...
TL;DR: CareMedEval是一个专门用于评估大型语言模型在生物医学领域批判性评估和推理能力的数据集，包含534个基于37篇科学文章的问题，来自法国医学生真实考试。


<details>
  <summary>Details</summary>
Motivation: 生物医学领域的科学文献批判性评估是重要技能，但当前大型语言模型在专业领域的批判性推理可靠性有限，需要专门的评估基准。

Method: 构建CareMedEval数据集，基于法国医学生真实考试，包含534个基于37篇科学文章的问题，专门评估基于科学论文的批判性阅读和推理能力。

Result: 基准测试显示，即使是先进的通用和专业生物医学LLM也难以超过0.5的精确匹配率，生成中间推理标记能显著改善结果，但模型在关于研究局限性和统计分析的问题上仍然面临挑战。
...
TL;DR: Kastor框架通过RDF模式提取方法改进小语言模型的关系抽取，从单一SHACL形状验证扩展到评估所有可能的属性组合，并通过迭代学习优化噪声知识库。
...
Method: 将传统验证任务重新表述为评估从SHACL形状派生的所有可能属性组合，为每个训练示例选择最优组合，并采用迭代学习过程来优化噪声知识库。

Result: 显著提高了模型的泛化能力和性能，能够创建强大的模型来发现新的相关事实。
...
TL;DR: BanglaSTEM是一个包含5000个孟加拉语-英语句子对的数据集，专门针对STEM领域的技术术语翻译问题而构建，通过训练T5翻译模型显著提升了技术内容的翻译准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语技术问题解决上表现良好，但在孟加拉语问题上表现不佳。现有孟加拉语-英语翻译系统难以准确翻译专业术语，导致问题含义改变和错误答案。

Method: 构建包含5000个孟加拉语-英语句子对的BanglaSTEM数据集，覆盖计算机科学、数学、物理、化学和生物学等STEM领域。使用语言模型生成12000多个翻译，并通过人工评估选择最高质量的对，然后训练基于T5的翻译模型。

Result: 在代码生成和数学问题解决两个任务上的测试结果显示，技术内容的翻译准确性显著提高，使孟加拉语使用者能更有效地使用英语为中心的语言模型。
...
TL;DR: HaluMem是首个针对记忆系统的操作级幻觉评估基准，包含记忆提取、记忆更新和记忆问答三个任务，通过构建大规模多轮人机交互数据集来全面评估记忆系统在不同操作阶段的幻觉行为。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统在存储和检索过程中经常出现记忆幻觉（包括捏造、错误、冲突和遗漏），但目前的评估主要是端到端的问答测试，难以定位幻觉产生的具体操作阶段。

Method: 提出HaluMem基准，定义三个评估任务：记忆提取、记忆更新和记忆问答。构建用户中心的多轮人机交互数据集HaluMem-Medium和HaluMem-Long，包含约15k记忆点和3.5k多类型问题，对话轮次达1.5k-2.6k，上下文长度超过100万tokens。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常限于固定轮数，容易饱和且无法考虑用户的交互体验，需要评估大语言模型在多主题对话中跟随用户指令的能力。

Method: 采用三层机制跟踪约束、指令和主题，通过动态构建基准测试模拟用户-LLM交互，包含状态变化和回溯，定义了一套捕捉交互过程质量的指标。

Result: GPT-5表现出优越的指令跟随性能，平均维持18.54轮对话，稳健性达70.31%，显著优于Gemini-2.5-Pro 11.41%，其他模型表现落后。

Conclusion: 提出的框架能够有效评估多轮指令跟随能力，GPT-5在当前测试中表现最佳，所有数据和代码将公开。
...
<details>
  <summary>Details</summary>
Motivation: 创建大规模、高质量的多语言数据集，用于比较不同语言模型在不同语言和难度下的逻辑推理能力，填补现有基准测试的空白。

Method: 生成多种语言、主题、大小和线索类型的斑马谜题，包括14种线索类型和8种红鲱鱼类型，通过改变谜题大小、语言和主题来调整难度。

Result: 2x3大小的谜题对GPT-4o mini具有挑战性，4x5大小的谜题对o3-mini具有挑战性；5个红鲱鱼线索使o3-mini在4x5谜题上的准确率下降15±7%；语言和主题对模型性能无显著影响；线索类型与难度无相关性。

Conclusion: 成功创建了MultiZebraLogic数据集，包含9种日耳曼语言的128+1024个谜题，提供了可适应更多语言和主题的谜题生成代码，为评估LLM的逻辑推理能力提供了有效工具。
...
TL;DR: 本文首次实证展示了Transformer语言模型中的可控局部性，通过可调局部性参数实现表示局部化程度的连续控制，可在局部编码和分布式表示之间动态插值。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型仅依赖分布式表示，缺乏可解释性。本文旨在开发一种能动态控制表示局部化程度的架构，为需要透明性和能力的受监管领域提供实用框架。

Method: 使用两层Transformer架构在WikiText语料库上进行实验，系统性地变化局部性参数λ从1.0（完全局部）到0.0（完全分布式），通过明确惩罚阈值和信息论设计原则实现可控局部性。

Result: 局部配置显著降低注意力熵（λ=1.0时为5.36比特，λ=0.0时为7.18比特），同时保持更高的指针保真度。中间局部性值（λ=0.6）在可解释性和性能间达到最优平衡，测试困惑度为4.65，准确率为84.7%。

Conclusion: 局部语言模型为需要透明性和能力的受监管领域提供了实用框架，通过明确惩罚阈值和信息论设计原则实现对可解释性-性能谱系的精确数学控制。
...
TL;DR: 本研究通过微调大语言模型并结合检索增强生成技术，为政策制定者开发了一个能够理解、分析和起草法律法规的智能工具。


<details>
  <summary>Details</summary>
Motivation: 旨在支持政策制定者更好地理解、分析和制定法律法规，解决法律领域专业知识获取和应用的挑战。

Method: 构建了针对法律领域的监督数据集进行模型微调，并集成检索增强生成方法，使模型能够访问和整合最新的外部法律知识。

Result: 该方法显著提升了法律研究和法规制定的效率，为政策制定者提供了有价值的辅助工具。

Conclusion: 结合微调和RAG增强的方法能够有效支持法律领域的专业工作，为不断发展的法律领域提供了实用资源。
...
TL;DR: Step-Audio-EditX是首个开源基于LLM的音频模型，擅长情感、说话风格和副语言学的表达性迭代音频编辑，同时具备强大的零样本文本转语音能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法主要关注表示级解耦，需要嵌入先验或辅助模块。本文旨在通过仅使用大边距合成数据来规避这些需求，实现迭代控制和高表达性。

Method: 核心创新在于仅利用大边距合成数据，无需嵌入先验或辅助模块。这种大边距学习方法支持跨声音的迭代控制和高度表达性。

Result: 评估结果显示，Step-Audio-EditX在情感编辑和其他细粒度控制任务中超越了MiniMax-2.6-hd和Doubao-Seed-TTS-2.0。

Conclusion: 该方法代表了从传统表示级解耦关注的根本性转变，通过大边距学习实现了表达性音频编辑和强大的零样本TTS能力。
...
### [67] [ChiMDQA: Towards Comprehensive Chinese Document QA with Fine-grained Evaluation](https://arxiv.org/abs/2511.03656)
*Jing Gao,Shutiao Luo,Yumeng Liu,Yuanming Li,Hongji Zeng*

Main category: cs.CL

TL;DR: 提出了中文多文档问答数据集ChiMDQA，包含6个领域的6,068个高质量问答对，为中文NLP任务提供数据支持。


<details>
  <summary>Details</summary>
Motivation: 随着NLP技术的发展，对高质量中文文档问答数据集的需求日益增长，但现有数据集难以满足实际业务场景需求。

Method: 通过严格的文档筛选和系统化的问题设计方法，构建了涵盖学术、教育、金融、法律、医疗和新闻等6个领域的多文档问答数据集。

Result: 创建了包含6,068个高质量问答对的ChiMDQA数据集，进一步细分为10个精细类别，确保数据集的多样性和高质量。

Conclusion: ChiMDQA数据集为中文文档理解、知识抽取和智能问答系统等NLP任务提供了重要的数据基础，支持未来研究和实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer模型的兴起，关系抽取领域出现了大量新研究，需要对这些进展进行系统整理，为研究者和从业者提供全面的参考。

Method: 使用自动化框架收集和标注出版物，系统分析2019-2024年间发表的34篇综述、64个数据集和104个模型。

Result: 识别了关系抽取领域的方法论进展、基准资源和语义网技术整合情况，发现了当前趋势、局限性和开放挑战。

Conclusion: 该研究为理解关系抽取的演变和未来方向提供了全面参考，有助于指导该领域的进一步发展。
...
TL;DR: 本文提出了一个视角主义标注方案，用于分析协作对话中的误解现象，通过分别捕捉说话者和听者的理解来追踪理解的形成、分歧和修复过程。


<details>
  <summary>Details</summary>
Motivation: 在不对称的协作对话中，参与者可能认为自己达成共识但实际上指向不同实体，需要研究这种误解如何产生和修复。
...
TL;DR: 本文提出了一种新颖的可解释零样本立场检测框架IRIS，通过隐式推理和显式语言特征来理解文本对目标的立场，无需真实推理依据即可提供内在可解释性。
...
Method: IRIS框架将立场检测视为信息检索排序任务，基于文本中的隐式推理序列和基于语言度量的显式推理来理解立场。隐式推理理解不同立场相关性的相关性，显式推理基于交际特征解码立场的情感和认知维度。

Result: 在VAST、EZ-STANCE、P-Stance和RFD基准数据集上的大量实验证明，即使使用50%、30%甚至10%的训练数据，模型仍具有良好的泛化能力。

Conclusion: IRIS框架通过提出的架构和可解释设计，在零样本立场检测任务中实现了优异的性能和内在可解释性，为理解作者对给定目标的态度提供了可解释的理解。
...
### [71] [Toward an Agricultural Operational Design Domain: A Framework](https://arxiv.org/abs/2511.02937)
...
<details>
  <summary>Details</summary>
Motivation: 农业自动化系统在复杂多变环境中运行，需要整合驾驶和工作过程，每个过程都有不同的操作约束。现有ODD概念无法解决农业应用的特殊挑战，需要结构化、透明且可验证的环境描述方法。

Method: Ag-ODD框架包含三个核心要素：1）Ag-ODD描述概念，使用ASAM Open ODD和CityGML概念结构化定义环境和操作参数；2）基于PEGASUS 6层模型的7层模型，增加了过程层以捕捉动态农业操作；3）迭代验证过程，验证Ag-ODD与其对应逻辑场景的一致性和完整性。

Result: 该框架为创建明确且可验证的Ag-ODD提供了一致方法，演示用例表明Ag-ODD框架可以支持自主农业系统环境描述的标准化和可扩展性。

Conclusion: Ag-ODD框架成功解决了农业自动化系统环境描述的独特需求，为自主农业系统的开发和验证提供了结构化、透明且可验证的方法，有助于推动农业自动化的标准化和规模化应用。
...
### [72] [Comprehensive Assessment of LiDAR Evaluation Metrics: A Comparative Study Using Simulated and Real Data](https://arxiv.org/abs/2511.02994)
*Syed Mostaquim Ali,Taufiq Rahman,Ghazal Farhani,Mohamed H. Zaki,Benoit Anctil,Dominique Charlebois*

Main category: cs.RO

TL;DR: 本文研究了用于比较真实世界和模拟LiDAR扫描的评估指标，发现密度感知Chamfer距离(DCD)在所有测试场景中表现最佳，并验证了虚拟测试环境在自动驾驶系统安全测试中的有效性。


<details>
  <summary>Details</summary>
Motivation: 由于传统物理测试成本高且存在安全风险，需要开发虚拟测试环境(VTE)作为替代方案。通过比较VTE生成的传感器输出与真实世界对应物，可以验证VTE是否准确反映现实。

Method: 采用综合实验方法寻找适合比较真实和模拟LiDAR扫描的评估指标，测试了不同噪声、密度、失真、传感器方向和通道设置下的指标敏感性和准确性。使用真实LiDAR扫描数据生成虚拟测试环境，并在相同姿态下生成模拟LiDAR扫描。

Result: 密度感知Chamfer距离(DCD)在所有情况下表现最佳。模拟和真实LiDAR扫描在语义分割输出上相似(mIoU为21%)，平均DCD为0.63，表明几何特性存在轻微差异，但模型输出存在显著差异。DCD与感知方法的相关性最高。

Conclusion: DCD是评估真实与模拟LiDAR扫描相似度的最佳指标，虚拟测试环境可以作为自动驾驶系统安全测试的有效工具，但需要进一步改进几何特性的准确性。
...
### [73] [WorldPlanner: Monte Carlo Tree Search and MPC with Action-Conditioned Visual World Models](https://arxiv.org/abs/2511.03077)
*R. Khorrambakht,Joaquim Ortiz-Haro,Joseph Amigo,Omar Mostafa,Daniel Dugas,Franziska Meier,Ludovic Righetti*

Main category: cs.RO

TL;DR: 提出了一种基于模型的机器人学习方法，使用少量非结构化游戏数据学习视觉世界模型、扩散动作采样器和奖励模型，结合MCTS规划器优化长序列动作，在真实机器人任务中显著优于行为克隆基线。


<details>
  <summary>Details</summary>
Motivation: 解决行为克隆方法难以迁移到新任务、训练数据收集困难的问题，通过模型化方法实现更灵活的任务适应和规划能力。

Method: 收集非结构化游戏数据学习动作条件视觉世界模型、扩散动作采样器和奖励模型，使用MCTS规划器优化动作序列，通过零阶模型预测控制器执行计划。

Result: 在3个真实机器人任务中验证了方法的有效性，规划方法相比行为克隆基线有显著改进，动作采样器有效缓解了世界模型在规划过程中的幻觉问题。

Conclusion: 基于模型的方法结合规划能够有效解决复杂机器人任务，优于端到端的行为克隆策略，为机器人学习提供了更灵活和可扩展的解决方案。
...
### [74] [Learning Natural and Robust Hexapod Locomotion over Complex Terrains via Motion Priors based on Deep Reinforcement Learning](https://arxiv.org/abs/2511.03167)
*Xin Liu,Jinze Wu,Yinghui Li,Chenkun Qi,Yufei Xue,Feng Gao*

Main category: cs.RO

TL;DR: 本文提出了一种基于运动先验的方法，成功将深度强化学习应用于真实六足机器人，通过生成优化运动先验数据集并训练对抗判别器来引导机器人学习自然步态，最终在复杂地形中实现了稳健行走。


<details>
  <summary>Details</summary>
Motivation: 多足机器人在复杂地形中具有更好的稳定性，但如何在更大的动作探索空间中有效协调多条腿以生成自然且稳健的运动是一个关键问题。

Method: 采用基于运动先验的方法，生成优化运动先验数据集，并训练对抗判别器来引导六足机器人学习自然步态，使用深度强化学习算法进行训练。

Result: 学习到的策略成功转移到真实六足机器人上，在复杂地形中展示了自然步态模式和显著的鲁棒性，无需视觉信息即可实现复杂地形行走。

Conclusion: 这是首次将强化学习控制器成功应用于真实六足机器人实现复杂地形行走，证明了该方法的有效性和实用性。
...
### [75] [Learning-based Cooperative Robotic Paper Wrapping: A Unified Control Policy with Residual Force Control](https://arxiv.org/abs/2511.03181)
*Rewida Ali,Cristian C. Beltran-Hernandez,Weiwei Wan,Kensuke Harada*

Main category: cs.RO

TL;DR: 提出了一种基于学习的框架，将LLM驱动的任务规划器与混合模仿学习和强化学习的底层策略相结合，用于解决人机协作中可变形物体（如礼品包装）的长时程操作问题。


<details>
  <summary>Details</summary>
Motivation: 在仓库和零售店等环境中，人机协作处理可变形物体（如纸张、袋子和织物）时面临挑战，因为可变形材料的动态特性难以预测，且需要自适应力控制。礼品包装任务体现了涉及精确折叠、受控折痕和安全固定的长时程操作问题。

Method: 提出Sub-task Aware Robotic Transformer (START)框架，从人类演示中学习统一策略。关键创新在于通过子任务ID提供显式时间基础，捕获整个包装序列中的长程时间依赖性，使策略学习子目标而非仅仅复制运动序列。

Result: 在真实世界包装任务中实现了97%的成功率。基于Transformer的统一策略减少了对专用模型的需求，允许受控的人类监督，并有效连接了高级意图与可变形物体操作所需的细粒度力控制。

Conclusion: 该学习框架成功解决了人机协作中可变形物体长时程操作的关键挑战，通过统一的Transformer策略实现了高成功率，并为类似复杂操作任务提供了可行的解决方案。
...
### [76] [Collaborative Assembly Policy Learning of a Sightless Robot](https://arxiv.org/abs/2511.03189)
*Zeqing Zhang,Weifeng Lu,Lei Yang,Wei Jing,Bowei Tang,Jia Pan*

Main category: cs.RO

TL;DR: 本文提出了一种结合导纳控制和强化学习的新方法，用于解决盲机器人-人类协作的木板插入任务，相比传统导纳控制提高了成功率并减少了人力消耗。


<details>
  <summary>Details</summary>
Motivation: 传统导纳控制在物理人机协作中难以准确测量人力/力矩来估计人类意图，限制了机器人的协助能力；而纯强化学习方法由于安全约束和稀疏奖励也不适用于木板插入任务。

Method: 提出了一种新颖的强化学习方法，利用人类设计的导纳控制器来促进更主动的机器人行为并减少人力消耗。

Result: 通过仿真和真实世界实验证明，该方法在成功率和任务完成时间上优于导纳控制，并且测量到的力/力矩显著减少。

Conclusion: 所提出的方法有效解决了盲机器人-人类协作任务中的挑战，提高了协作效率并降低了人力需求。
...
### [77] [GUIDES: Guidance Using Instructor-Distilled Embeddings for Pre-trained Robot Policy Enhancement](https://arxiv.org/abs/2511.03400)
*Minquan Gao,Xinyi Li,Qing Yan,Xiaojian Sun,Xiaopan Zhang,Chien-Ming Huang,Jiachen Li*

Main category: cs.RO

TL;DR: GUIDES是一个轻量级框架，通过基础模型的语义指导来增强预训练机器人策略，无需架构重新设计，在保持现有知识的同时提升语义感知能力。
...
Method: 使用微调的视觉语言模型生成上下文指令，通过辅助模块编码为指导嵌入并注入策略的潜在空间，结合LLM反射器进行推理循环以增强推理时鲁棒性。

Result: 在RoboCasa仿真环境中验证，跨多种策略架构均显著提升任务成功率；UR5机器人实际部署显示提高了抓取等关键子任务的运动精度。

Conclusion: GUIDES提供了一种实用且资源高效的途径来升级已验证的机器人策略，而非完全替换。
...
### [78] [Value Elicitation for a Socially Assistive Robot Addressing Social Anxiety: A Participatory Design Approach](https://arxiv.org/abs/2511.03444)
*Vesna Poprcova,Iulia Lefter,Martijn Warnier,Frances Brazier*

Main category: cs.RO

TL;DR: 本研究通过参与式设计工作坊，探讨了社交辅助机器人支持社交焦虑的价值基础，揭示了适应性、接纳性和有效性等核心设计价值观。


<details>
  <summary>Details</summary>
Motivation: 社交焦虑是普遍的心理健康问题，但现有支持不足。社交机器人技术为补充传统心理健康干预提供了新机会，需要理解塑造有意义、可接受和有效干预的核心价值观。

Method: 采用参与式设计工作坊，与心理健康学术研究人员合作，通过创造性、反思性和展望性活动，系统探索场景和设计可能性，引出相关价值观、期望、需求和偏好。

Result: 研究发现丰富的设计相关价值观，包括适应性、接纳性和有效性，这些是支持社交焦虑个体的核心要素。

Conclusion: 研究强调了以研究为主导的价值引出方法的重要性，在社交辅助机器人开发中需要注重以用户为中心和情境感知的设计考量。
...
### [79] [Development of the Bioinspired Tendon-Driven DexHand 021 with Proprioceptive Compliance Control](https://arxiv.org/abs/2511.03481)
*Jianbo Yuan,Haohua Zhu,Jing Dai,Sheng Yi*

Main category: cs.RO

TL;DR: Dex-Hand 021是一种高性能的五指机器人手，采用缆线驱动，具有12个主动和7个被动自由度，在1公斤轻量化设计中实现19个自由度的灵巧性。通过基于本体感知力感的导纳控制方法提升操作能力。


<details>
  <summary>Details</summary>
Motivation: 人类手在日常生活中和工业应用中发挥着重要作用，但复制其多功能能力（包括运动、感知和协调操作）对机器人系统来说仍然是一个巨大挑战。开发灵巧机器人手需要在类人敏捷性与工程约束（如复杂性、尺寸重量比、耐久性和力感性能）之间取得平衡。

Method: 提出了基于本体感知力感的导纳控制方法，采用缆线驱动的五指机器人手设计，具有12个主动和7个被动自由度。

Result: 实验结果显示：单指负载能力超过10N，指尖重复性低于0.001m，力估计误差低于0.2N。与PID控制相比，多物体抓取中的关节扭矩减少了31.19%，显著提高了力感能力同时防止过载。该手在力量和精确抓取方面表现出色，成功执行了33种GRASP分类动作和复杂操作任务。

Conclusion: 这项工作推进了轻量化工业级灵巧手的设计，并增强了本体感知控制，为机器人操作和智能制造做出了贡献。
...
### [80] [ROSBag MCP Server: Analyzing Robot Data with LLMs for Agentic Embodied AI Applications](https://arxiv.org/abs/2511.03497)
*Lei Fu,Sahar Salimpour,Leonardo Militano,Harry Edelman,Jorge Peña Queralta,Giovanni Toffetti*

Main category: cs.RO

TL;DR: 本文提出了一个用于分析ROS和ROS 2数据包的MCP服务器，通过LLM和VLM实现机器人数据的自然语言分析、可视化和处理，并评估了8种不同LLM/VLM模型的工具调用能力。


<details>
  <summary>Details</summary>
Motivation: 当前在Agentic Embodied AI（智能体化具身AI）领域的研究文献稀缺，而MCP正成为智能体应用的关键组件。本文旨在填补ROS数据包分析与自然语言处理之间的空白。

Method: 开发了一个MCP服务器，集成了机器人领域知识的专用工具，支持移动机器人轨迹、激光扫描数据、变换和时间序列数据的分析，同时提供标准ROS 2 CLI工具的接口。

Result: 实验评估了8种最先进的LLM/VLM模型，发现Kimi K2和Claude Sonnet 4在工具调用能力上表现明显优于其他模型。工具描述模式、参数数量、可用工具数量等因素都会影响成功率。

Conclusion: 该工作为Agentic Embodied AI领域提供了实用的工具和基准测试平台，代码已开源发布，有助于推动该交叉领域的发展。
...
### [81] [Indicating Robot Vision Capabilities with Augmented Reality](https://arxiv.org/abs/2511.03550)
*Hong Wang,Ridhima Phatak,James Ocampo,Zhao Han*

Main category: cs.RO

TL;DR: 本研究提出四种增强现实视野指示器，用于校准人类对机器人视野能力的心理模型，通过实验评估发现任务空间分配中心指示器准确率最高，眼窝深度自我中心指示器也能提高准确率。


<details>
  <summary>Details</summary>
Motivation: 人类常错误假设机器人与人类具有相同视野，这种不准确的心理模型可能导致人机协作失败，特别是在机器人无法扫描场景更新世界模型时。

Method: 提出四种增强现实视野指示器，涵盖从自我中心（机器人眼部和头部空间）到分配中心（任务空间）的频谱，并通过41人用户实验评估准确性、信心、任务效率和认知负荷。

Result: 任务空间分配中心指示器准确率最高但解读有延迟，自我中心眼窝深度指示器也能提高准确率，所有指示器下参与者信心高且认知负荷低。

Conclusion: 贡献了六条实践指南，帮助从业者应用增强现实指示器或物理改造来校准人类对机器人视野能力的心理模型。
...
### [82] [OneOcc: Semantic Occupancy Prediction for Legged Robots with a Single Panoramic Camera](https://arxiv.org/abs/2511.03571)
*Hao Shi,Ze Wang,Shangwei Guo,Mengfei Duan,Song Wang,Teng Chen,Kailun Yang,Lin Wang,Kaiwei Wang*

Main category: cs.RO

TL;DR: OneOcc是一个专为腿式/人形机器人设计的全景语义场景补全框架，通过双投影融合、双网格体素化、轻量级解码器和步态位移补偿等技术，在360度全景环境下实现鲁棒的3D语义占据感知，并在两个新发布的数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的语义场景补全系统主要针对轮式平台和前方传感器设计，而腿式/人形机器人由于步态引起的身体抖动和需要360度连续感知，需要专门的全景语义占据感知解决方案。

Method: 结合双投影融合(DP-ER)利用环形全景图和等距柱状展开，保持360度连续性和网格对齐；双网格体素化(BGV)在笛卡尔和圆柱极坐标空间推理，减少离散化偏差；轻量级解码器采用分层AMoE-3D进行动态多尺度融合；可插拔的步态位移补偿(GDC)学习特征级运动校正。

Result: 在QuadOcc数据集上超越强视觉基线和流行的LiDAR方法；在Human360Occ数据集上获得+3.83 mIoU(同城市)和+8.08(跨城市)的性能提升，达到新的最先进水平。

Conclusion: OneOcc模块轻量级，可为腿式/人形机器人提供可部署的全方位感知能力，填补了全景语义场景补全领域的空白。
...
### [83] [Multi-User Personalisation in Human-Robot Interaction: Using Quantitative Bipolar Argumentation Frameworks for Preferences Conflict Resolution](https://arxiv.org/abs/2511.03576)
*Aniol Civit,Antonio Andriella,Carles Sierra,Guillem Alenyà*

Main category: cs.RO

TL;DR: 提出基于定量双极论证框架的多用户个性化框架MUP-QBAF，用于解决多用户偏好冲突，结合用户论证和机器人环境观察，实现动态适应和上下文响应。
...
Method: 基于定量双极论证框架(QBAFs)，将正负偏好表示为论证，根据新信息迭代重新计算论证强度，结合用户输入和机器人环境观察。

Result: 通过辅助机器人在衰弱评估任务中调解护理者和被护理者偏好冲突的现实案例研究验证了框架特性，包括论证基础分数的敏感性分析。

Conclusion: 该工作为多用户HRI领域提供了透明、结构化、上下文敏感的竞争用户偏好解决方法，是数据驱动方法的原理性替代方案，使机器人能够在现实环境中导航冲突。
...
### [84] [Manifold-constrained Hamilton-Jacobi Reachability Learning for Decentralized Multi-Agent Motion Planning](https://arxiv.org/abs/2511.03591)
*Qingyi Chen,Ruiqi Ni,Jun Kim,Ahmed H. Qureshi*

Main category: cs.RO

TL;DR: 提出了一种流形约束的Hamilton-Jacobi可达性学习框架，用于解决多智能体运动规划中的任务诱导约束问题，使机器人能够在动态环境中安全导航并满足任务要求。


<details>
  <summary>Details</summary>
Motivation: 现实世界中机器人需要在动态环境中导航，同时满足任务施加的流形约束（如服务机器人必须保持杯子直立），但现有的去中心化多智能体运动规划方法难以有效整合流形约束。

Method: 通过求解流形约束下的Hamilton-Jacobi可达性问题来捕获任务感知的安全条件，然后将这些条件集成到去中心化轨迹优化规划器中，使机器人能够生成既安全又任务可行的运动规划。

Result: 该方法能够泛化到多种流形约束任务，并有效扩展到高维多智能体操作问题，实验表明其性能优于现有约束运动规划器，且运行速度适合实际应用。
...
### [85] [Multi-robot searching with limited sensing range for static and mobile intruders](https://arxiv.org/abs/2511.03622)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是解决多机器人在几何域内搜索入侵者的问题，特别是针对具有有限感知能力的机器人在正交多边形环境中的搜索任务。

Method: 提出了三种主要算法：基于空间填充曲线的搜索方法、随机搜索方法以及协作随机搜索方法，并分析了搜索机器人数量与搜索时间之间的权衡关系。

Result: 研究结果表明，即使在静态入侵者情况下，搜索问题也是NP难的，但提出的算法能够在考虑几何特性的同时有效完成搜索任务。

Conclusion: 结论是虽然搜索入侵者问题是计算困难的，但通过设计合适的搜索策略和机器人协作机制，可以实现高效的搜索性能，特别是在正交多边形环境中。
...
### [86] [Flying Robotics Art: ROS-based Drone Draws the Record-Breaking Mural](https://arxiv.org/abs/2511.03651)
*Andrei A. Korigodskii,Oleg D. Kalachev,Artem E. Vasiunik,Matvei V. Urvantsev,Georgii E. Bondar*

Main category: cs.RO

TL;DR: 本文介绍了一种创新的自主无人机系统，成功完成了世界上最大的无人机壁画创作。该系统通过结合红外运动捕捉和LiDAR技术实现精确定位，采用独特的控制架构确保轨迹跟踪精度，并设计了专门的喷漆机制来应对螺旋桨气流干扰。


<details>
  <summary>Details</summary>
Motivation: 解决在户外恶劣条件（如风、阳光）下保持艺术精度和操作可靠性的双重挑战，扩展机器人在创意领域的应用功能。

Method: 采用红外运动捕捉相机和LiDAR技术的新型导航系统，结合切向和法向不同调节的控制架构，开发了轨迹规划和路径优化算法，并设计了专门应对螺旋桨气流的定制喷漆机制。

Result: 实验结果表明系统在各种条件下都表现出鲁棒性和精确性，成功完成了大规模壁画创作。

Conclusion: 该系统展示了在自主大规模艺术创作方面的潜力，为机器人在创意领域的应用开辟了新途径。
...
### [87] [Motion Planning Under Temporal Logic Specifications In Semantically Unknown Environments](https://arxiv.org/abs/2511.03652)
*Azizollah Taheri,Derya Aksaray*

Main category: cs.RO

TL;DR: 本文提出了一种在不确定环境中实现时空逻辑任务（用语法安全线性时序逻辑规范表示）的运动规划方法，其中不确定性通过环境语义标签的概率知识建模。


<details>
  <summary>Details</summary>
Motivation: 解决在环境语义标签不确定的情况下（如区域位置只有概率信念而非精确已知），如何有效规划运动以完成复杂时空逻辑任务的问题。

Method: 采用自动机理论方法，构建特殊乘积自动机来捕捉语义标签的不确定性，为自动机的每条边设计奖励函数，并利用值迭代进行在线重规划。

Result: 通过理论分析和仿真实验验证了所提方法的有效性。

Conclusion: 提出的自动机理论方法能够有效处理语义标签不确定性下的时空逻辑任务规划问题。
...
### [88] [Unconscious and Intentional Human Motion Cues for Expressive Robot-Arm Motion Design](https://arxiv.org/abs/2511.03676)
*Taito Tashiro,Tomoko Yonezawa,Hirotake Yamazoe*

Main category: cs.RO

TL;DR: 研究探索如何利用人类运动线索设计富有表现力的机器人手臂运动，通过分析Geister游戏中人类棋子移动的自然和刻意表达，创建了基于速度和停顿时长的特定阶段机器人动作，并评估了物理机器人与视频展示的效果差异。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解人类运动线索如何转化为机器人富有表现力的动作设计，特别是在不完全信息游戏情境下，探索自然游戏行为与刻意表达之间的差异对机器人动作设计的影响。

Method: 使用Geister游戏分析人类棋子移动的两种类型：自然游戏行为（无意识倾向）和指令表达（有意线索），基于这些发现创建了通过改变运动速度和停止时长的阶段特定机器人动作，并在物理机器人和录制视频两种展示模式下评估观察者的印象。

Result: 结果表明，后期运动时机（特别是撤回阶段）在印象形成中起重要作用，物理体现增强了运动线索的可解释性。

Conclusion: 基于人类时机行为的发现为设计富有表现力的机器人运动提供了见解，强调了物理体现和特定阶段运动时机在机器人动作表达中的重要性。
...
### [89] [Source-Free Bistable Fluidic Gripper for Size-Selective and Stiffness-Adaptive Grasping](https://arxiv.org/abs/2511.03691)
*Zhihang Qin,Yueheng Zhang,Wan Su,Linxin Hou,Shenghao Zhou,Zhijun Chen,Yu Jun Tan,Cecilia Laschi*

Main category: cs.RO

TL;DR: 本文提出了一种自包含的软体抓取器，通过内部液体在三个互连的双稳态腔室间重新分配来实现操作，无需外部能源，实现了稳定、尺寸选择性的抓取和被动刚度适应。


<details>
  <summary>Details</summary>
Motivation: 传统流体驱动的软体抓取器依赖外部能源，限制了便携性和长期自主性。本文旨在开发一种无需外部能源的自包含软体抓取器。

Method: 设计包含三个互连的双稳态腔室，当顶部传感腔室接触物体变形时，液体重新分配触发抓取腔室的快速扩张，实现抓取。内部液压反馈机制允许被动适应物体刚度。

Result: 该抓取器实现了无需持续能量输入的稳定、尺寸选择性抓取，并能被动适应不同物体的刚度，具有紧凑、轻量化的特点。

Conclusion: 这种无源、紧凑的设计为软体机器人中的轻量化、刚度自适应流体驱动操作开辟了新可能性，特别适用于水下和野外环境中的目标尺寸特定采样和操作。
...
TL;DR: 本文系统评估了AI控制协议在SHADE-Arena环境中的安全性，发现关键行动延迟协议对红队攻击具有高度鲁棒性，而重采样策略在红队获得协议内部知识时安全性会大幅下降。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统作为智能体被广泛部署，确保其安全运行变得至关重要。需要评估控制协议对当前攻击的有效性及其对自适应对手的鲁棒性。

Method: 在SHADE-Arena多样化智能体环境中系统评估控制协议，包括可信模型延迟、重采样和关键行动延迟等蓝队协议，并迭代开发红队攻击策略。

Result: 重采样和关键行动延迟协议将安全性从50%提升到96%，但当红队获得重采样发生时机或模拟监控器能力时，重采样策略的安全性降至17%，而关键行动延迟协议对最强红队策略仍保持高度鲁棒性。
...
Method: 采用多智能体框架，将分析流程分解为四个专门智能体：意图澄清、数据集发现、分析和报告，每个智能体保持专注的注意力范围并支持阶段验证。

Result: 在5个模型和50个查询上的评估显示：专业化独立于模型强度提供价值；智能体分为通用型和条件型；不同智能体缓解不同的失败模式；架构优势在不同任务复杂度下保持稳定；智能体效果在不同模型间差异显著。

Conclusion: 提出了多智能体LLM系统的五个设计原则，指导何时以及为什么在复杂分析工作流中需要专业化，同时通过自然语言接口实现更广泛的公共数据访问。
...
TL;DR: ScalingEval是一个大规模基准测试研究，系统比较了36个大型语言模型作为评估者的性能，使用共识驱动的评估协议，在多个产品类别中测试了GPT、Gemini、Claude和Llama等模型。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型作为评估者的应用日益重要，需要建立可扩展且可信赖的评估流程，因此需要系统性地比较不同LLM作为评估者的性能。

Method: 采用多智能体框架，通过可扩展的多数投票将模式审计和问题代码聚合成真实标签，无需人工标注即可实现LLM评估者的可重复比较。

Result: 主要发现：Claude 3.5 Sonnet决策置信度最高；Gemini 1.5 Pro在各类别中整体表现最佳；GPT-4o在延迟-准确性-成本权衡方面最优；GPT-OSS 20B在开源模型中领先。结构化领域（电子产品、体育）共识强，生活方式类别（服装、食品）存在持续分歧。

Conclusion: ScalingEval建立了可重复的基准测试和评估协议，为LLM作为评估者提供了关于扩展性、可靠性和模型系列权衡的可操作指导。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在事实性知识方面表现出色，但在理解现实世界概率分布方面能力有限。本文旨在测试LLMs是否能够内化描述现实世界人口分布的统计知识，挑战LLMs作为通用分布近似器的观点。

Method: 开发首个基准测试，直接评估LLMs是否能够获取描述现实世界人口的经验分布，涵盖经济、健康、教育和社会行为等多个领域。

Result: LLMs整体表现不佳，似乎无法自然内化现实世界统计数据。在Pearl因果层次框架下，语言模型缺乏观测分布知识，因此干预和反事实知识也受限。

Conclusion: LLMs在理解现实世界概率分布方面存在根本性局限，无法有效内化真实世界统计特性，这对其作为通用分布学习器的能力提出了挑战。
...
TL;DR: 本文提出了SnapStream，一种可在静态图和连续批处理的生产推理系统中部署的KV缓存压缩方法，能够实现4倍片上内存使用改进，并在DeepSeek-671B等大模型上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 随着支持100k+上下文长度的100B+参数大语言模型的普及，对支持大KV缓存的片上内存需求不断增加。现有KV缓存压缩技术难以在工业部署框架中应用，且对现代指令跟随和推理模型的准确性影响不明确。

Method: 开发SnapStream KV缓存压缩方法，探索其在Llama-3.1-8B-Instruct和DeepSeek-R1上的准确性影响，并在16路张量并行部署的DeepSeek-671B上进行验证。

Result: 在128k上下文长度下达到每秒1832个token的推理速度，实现4倍片上内存使用改进，在LongBench-v2、AIME24和LiveCodeBench基准测试中引入最小的准确性下降。

Conclusion: SnapStream是首个在具有静态图和连续批处理的生产推理系统中部署的稀疏KV注意力技术，能够有效解决大KV缓存的内存需求问题。
...
TL;DR: 提出基于能力的监控方法，作为传统任务型监控的替代方案，用于医疗领域大语言模型的监督。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习监控方法基于任务性能退化假设，但大语言模型并非针对特定任务训练，需要新的监控原则来适应其通用特性。

Method: 基于能力的监控，围绕模型共享能力（如摘要、推理、翻译、安全防护）组织监控，实现跨任务系统性弱点和长尾错误的检测。
...
TL;DR: 本文对miniF2F基准测试中的形式化与非形式化陈述进行了深入分析，发现现有AI系统在数学奥林匹克竞赛问题上的整体准确率仅为36%，远低于自动形式化和定理证明的单独准确率。通过修正错误和差异，创建了miniF2F-v2版本，将准确率提升至70%。


<details>
  <summary>Details</summary>
Motivation: 评估AI系统在数学奥林匹克竞赛中的表现，分析自动形式化与定理证明模型之间的对齐问题，以及基准测试质量对评估进展的重要性。

Method: 对miniF2F基准测试中的形式化和非形式化陈述进行对比分析，识别错误和差异，修正所有问题后创建miniF2F-v2，并在新版本上评估完整的定理证明流程。

Result: 在原始miniF2F上最佳准确率为36%，在修正后的miniF2F-v2上提升至70%；超过一半的问题存在形式化与非形式化陈述之间的差异。
...
<details>
  <summary>Details</summary>
Motivation: 随着优化问题日益复杂多样，传统零阶或一阶方法效率低、梯度信息不准确、优化信息利用不足，难以有效应对非凸、高维、黑箱等挑战。大语言模型在语言理解和代码生成方面的进步为优化算法设计提供了新机遇。

Method: 以烟花算法为基础优化器，结合多模态大语言模型辅助设计。提出关键部分概念，利用大语言模型的多模态特性充分挖掘优化过程中的信息，将烟花算法扩展到复杂高维任务。

Result: 在旅行商问题和电子设计自动化问题上的实验结果表明，基于新框架生成的烟花算法在许多问题实例上达到或超越了当前最优结果。

Conclusion: 多模态大语言模型与烟花算法的结合为复杂优化问题提供了有效的解决方案，证明了这种新框架在优化算法设计中的潜力和价值。
...
TL;DR: 本文提出了一种新颖的大语言模型安全响应框架，通过输入级的安全分类模型和输出级的检索增强生成与解释模型相结合，系统性地保护LLMs的安全部署。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，其安全问题日益突出，严重制约了在关键领域的可信部署。需要构建系统性的安全防护机制来确保LLMs的安全可靠运行。

Method: 在输入级采用基于监督微调的安全分类模型，通过四层级分类（安全、不安全、条件安全、专注注意）进行精确风险识别；在输出级结合检索增强生成和专门微调的解释模型，确保响应基于实时可信知识库。

Result: 实验结果显示，该安全控制模型在公共安全评估基准上的安全得分显著高于基线模型TinyR1-Safety-8B，在专有高风险测试集上组件获得了100%的安全分数。

Conclusion: 该研究为构建高安全性、高可信度的大语言模型应用提供了有效的工程路径，验证了框架在复杂风险场景下的出色保护能力。
...
<details>
  <summary>Details</summary>
Motivation: 形式化可解释人工智能（XAI）相比非形式化方法具有理论严谨性保证，但实际实现的形式化解释器的验证研究较少。

Method: 开发了一种新颖的验证形式化解释器的方法论，并对PyXAI进行了评估。

Result: 实验证明PyXAI在大多数分析的数据集上计算出了错误的解释。

Conclusion: 验证形式化解释器的重要性得到了证实，需要更严格的验证方法来确保解释的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 传统工程设计方法资源密集且效率低下，需要多领域专家协作，存在效率瓶颈。本文旨在通过AI多智能体框架解决这些问题，提升设计过程的效率和质量。

Method: 构建包含三个核心AI智能体的框架：图本体学家（构建领域知识图谱）、系统工程师（制定技术需求）、设计工程师（生成候选设计）。通过迭代反馈循环进行设计优化，最终进行性能指标最大化。

Result: 该框架成功应用于4位数NACA翼型的气动优化，展示了AI智能体如何通过结构化知识表示和协作循环提升设计效率、一致性和质量。

Conclusion: 研究表明，配备结构化知识表示的协作AI智能体能够显著增强工程设计过程的效率、一致性和质量，为复杂工程设计问题提供了新的解决方案。
...
TL;DR: 本文介绍了Summit Concierge，一个为Adobe Summit开发的领域特定AI助手，通过人机协作开发流程解决数据稀疏、质量保证和快速部署等现实约束，展示了敏捷反馈驱动开发在冷启动场景下构建可扩展可靠AI助手的能力。
...
Method: 采用人机协作开发工作流，结合提示工程、检索基础化和轻量级人工验证，解决数据稀疏、质量保证和快速部署等挑战。

Result: 成功开发并部署了Summit Concierge系统，能够处理广泛的活动相关查询，在现实约束下表现出色。
...
TL;DR: 大型语言模型（LLMs）能够仅从大五人格量表的最小定量输入中，准确建模人类心理特质的相关结构，其生成的心理量表反应与人类数据的相关性模式高度一致（R² > 0.89）。


<details>
  <summary>Details</summary>
Motivation: 研究探索LLMs是否以及如何能够从最小定量输入中建模人类心理特质的关联结构，这对于理解LLMs的心理模拟能力和推理过程具有重要意义。

Method: 使用816名人类个体的大五人格量表反应提示各种LLMs，让它们模拟这些个体在九个其他心理量表上的反应，并分析其推理过程。

Result: LLMs在零样本条件下表现出色，其生成的反应与人类数据的相关性模式高度一致，准确度接近直接在数据集上训练的机器学习算法。分析显示LLMs采用两阶段过程：首先将原始大五反应转化为自然语言人格摘要，然后基于这些摘要推理生成目标量表反应。

Conclusion: LLMs能够通过抽象和推理过程从最小数据中精确预测个体的心理特质，这既为心理模拟提供了强大工具，也为理解其涌现的推理能力提供了宝贵见解。
...
TL;DR: 本文提出了一个名为AAA的自动化网页可访问性审计框架，通过人机协作模式实现WCAG-EM标准的规模化执行，包含GRASP采样方法和MaC智能助手两大创新组件。


<details>
  <summary>Details</summary>
Motivation: 当前网页可访问性审计存在资源密集、难以规模化的问题，大多数网站界面不符合可访问性标准，阻碍了数字空间的社会福利、公平与正义发展。

Method: AAA框架包含两个核心创新：GRASP基于图的多模态采样方法，通过视觉、文本和关系线索的嵌入学习确保代表性页面覆盖；MaC基于多模态大语言模型的智能助手，支持跨模态推理和高难度任务的智能辅助。

Result: 实验证明该方法的有效性，提供了小型语言模型经过微调后可作为专业专家的见解，并贡献了四个用于审计流程基准测试的新数据集。

Conclusion: 该框架实现了可扩展的端到端网页可访问性审计，通过AI增强的辅助赋予人类审计员实际影响力。
...
TL;DR: 本文对机器学习模型中解释问题的参数化复杂性进行了理论分析，重点研究具有透明内部机制的模型，包括决策树、决策集、决策列表、布尔电路等，填补了可解释AI领域的理论空白。


<details>
  <summary>Details</summary>
Motivation: 针对当前AI系统普遍被视为黑盒的问题，本研究旨在为具有透明内部机制的机器学习模型提供解释问题的理论基础，推动AI系统的透明度和可问责性。

Method: 采用参数化复杂性理论分析方法，研究两类主要解释问题（溯因解释和对比解释）在决策树、决策集、决策列表、布尔电路等模型中的局部和全局变体。

Result: 为各种机器学习模型的解释问题建立了参数化复杂性理论基础，揭示了不同模型在生成解释时的计算复杂性特征。

Conclusion: 这项工作为可解释AI领域提供了重要的理论支撑，强调了AI系统透明度和可解释性的必要性，为未来研究奠定了基础。
...
TL;DR: Solly是第一个在简化版Liar's Poker中达到精英人类水平的AI智能体，通过自对弈和深度强化学习训练，在单挑和多玩家游戏中都表现出色，胜率超过50%，并超越了大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 虽然AI在德州扑克等游戏中已取得突破，但这些游戏的多玩家动态较弱。Liar's Poker具有更广泛的多玩家参与特性，是测试多玩家动态、不完全信息和不确定性下推理的理想环境。

Method: 使用无模型、演员-评论家深度强化学习算法进行自对弈训练。

Result: Solly在单挑和多玩家Liar's Poker中都达到了精英人类水平，胜率超过50%，在资金收益方面也表现出色。它超越了大型语言模型，开发了新颖的竞标策略，有效随机化游戏，且不易被世界级人类玩家利用。
