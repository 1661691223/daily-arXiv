...
TL;DR: 本文提出两种基于文本引导的方法来提升面部图像性别分类算法的公平性：图像文本匹配指导和图像文本融合，通过利用图像描述中的语义信息来减少人口统计偏见。


<details>
  <summary>Details</summary>
Motivation: 在人工智能公平性追求中，面部图像性别分类算法存在人口统计偏见问题，需要新的方法来提升公平性，特别是在缺乏人口统计标签的情况下。

Method: 提出两种文本引导方法：1) 图像文本匹配指导，训练模型识别图像和文本之间的细粒度对齐以获得增强的多模态表示；2) 图像文本融合，将两种模态结合成综合表示以提升公平性。

Result: 在基准数据集上的广泛实验表明，相比现有方法，这些方法能有效减轻偏见，并提高跨性别和种族群体的分类准确性。

Conclusion: 通过文本引导的语义信息整合提供了一种可解释且直观的训练范式，为开发更公平的面部分析算法提供了有价值的见解，有助于解决面部图像性别分类中的人口统计偏见挑战。
...
<details>
  <summary>Details</summary>
Motivation: 足球理解因其领域特定的复杂性和独特挑战而受到关注。现有方法通常依赖孤立的任务特定专家模型，缺乏统一的解决方案来处理从运动员检测到事件分类等多样化的足球视觉理解任务。

Method: 1) 提出SoccerMaster，首个足球专用视觉基础模型，通过监督多任务预训练统一处理多样化任务；2) 开发自动化数据整理流水线生成可扩展的空间标注，整合现有足球视频数据集构建SoccerFactory预训练数据资源；3) 构建综合预训练数据集。

Result: SoccerMaster在多样化下游任务中始终优于任务特定专家模型，展示了其广泛性和优越性。数据、代码和模型将公开可用。

Conclusion: 该研究提出了首个统一的足球视觉基础模型SoccerMaster，通过综合数据资源和多任务预训练框架，显著提升了足球视觉理解的性能，为领域提供了强大的基础模型解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 结核病是全球主要致死原因之一，胸片是经济有效的诊断工具但需要专家解读，而机器学习模型常依赖伪相关性且泛化能力差。构建高质量标注的医学图像数据集成本高昂且需要专家参与。
...
TL;DR: 该论文提出了SVR框架，通过可控合成OCTA图像和对应文本，解决了医学领域缺乏大规模标注数据的问题，显著提升了视觉语言模型在糖尿病视网膜病变诊断中的性能。


<details>
  <summary>Details</summary>
Motivation: 医学领域如OCTA图像分析缺乏大规模、精确标注的图像-文本数据集，限制了视觉语言模型在详细推理和解释性诊断方面的应用。
...
Result: 在真实OCTA图像上，使用该数据集训练的通用VLM（Qwen3-VL-8b）实现了89.67%的零样本平衡分类准确率，超越了监督基线方法，并通过专家评估显著提升了解释质量和病理定位能力。

Conclusion: SVR框架通过合成数据有效解决了医学领域数据稀缺问题，使视觉语言模型能够在缺乏大规模标注数据的情况下实现高性能的医学诊断和解释性推理。
...
<details>
  <summary>Details</summary>
Motivation: 传统城市动态监测方法存在局限性，需要更先进的技术来更好地理解城市动态。事件相机作为一种新兴传感器技术，具有独特优势，可能为城市动态研究提供新的解决方案。

Method: 通过文献综述方法，分析事件相机的工作原理、应用场景、优势与挑战，以及机器学习在事件相机数据处理中的应用，并提出多传感器融合方案。

Result: 事件相机在低光照条件下表现优异，能保护隐私，适合城市动态监测。与红外、激光雷达或振动传感器融合可增强其能力并克服现有挑战。

Conclusion: 事件相机是研究城市动态的有前景的媒介，多传感器融合方案能进一步提升其应用效果，为城市监测提供更全面、隐私友好的解决方案。
...
Result: 在3D打印机床上实现高精度零样本工件存在检测，无需模型重新训练
...
TL;DR: VGent提出了一种模块化的视觉定位架构，通过解耦高级推理和低级边界框预测，使用冻结的MLLM作为编码器保持推理能力，检测器提供候选框作为查询，实现快速准确的视觉定位。
...
Method: VGent采用模块化编码器-解码器架构：1）冻结的MLLM作为编码器提供推理能力；2）检测器提供高质量候选框作为查询；3）解码器通过交叉注意力在编码器隐藏状态中选择目标框。还引入了QuadThinker增强多目标推理能力、掩码感知标签解决检测-分割歧义、全局目标识别改进目标识别。

Result: 在多目标视觉定位基准测试中，VGent相比先前方法实现了+20.6%的F1分数提升，在视觉参考挑战中进一步提升了gIoU +8.2%和cIoU +5.8%，同时保持恒定快速的推理延迟。

Conclusion: VGent通过模块化设计成功解耦了推理和定位任务，充分利用了目标检测和MLLM的进展，避免了自回归解码的缺陷，实现了快速准确的视觉定位，为视觉定位任务提供了新的解决方案。
...
TL;DR: 该研究提出了一种信息驱动的智能融合策略，将多个病理学基础模型整合为统一表示，用于癌症分级和分期任务，相比单一模型和简单融合方法，智能融合在性能上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在病理学任务中表现出色，但不同模型之间的互补性、嵌入空间冗余性以及特征生物学解释仍不明确。研究旨在探索如何有效整合多个病理学基础模型以提升癌症诊断性能。

Method: 研究使用肾癌、前列腺癌和直肠癌的H&E全切片图像，对比了三种融合策略：多数投票集成、简单特征拼接和基于相关性指导的智能特征剪枝融合。在瓦片级别和切片级别分别评估了多个基础模型。

Result: 智能融合在所有三种癌症分类任务中均优于最佳单一模型和简单融合方法。全局相似性度量显示模型嵌入空间高度对齐，但局部邻域一致性较低，表明不同模型在细粒度信息上具有互补性。

Conclusion: 基于相关性指导的病理学基础模型智能融合能够产生紧凑、任务定制的表示，既提升了预测性能，又增强了计算病理学任务的可解释性。
...
TL;DR: LEGO是一个无需配对数据的后训练域适应框架，通过利用生成式oracle将无监督挑战转化为伪监督学习，显著提升预训练图像修复模型在真实世界分布外退化上的性能。
...
Method: 提出LEGO框架，包含三个阶段：1) 从预训练模型获得初始修复结果；2) 利用冻结的大规模生成式oracle将这些估计精炼为高质量的伪真实标签；3) 使用混合监督策略微调原始模型，结合分布内数据和新的伪配对数据。

Result: 实验表明LEGO能有效弥合领域差距，在多样化的真实世界基准测试中显著提升性能，使模型适应新分布而不牺牲原始鲁棒性或需要架构修改。

Conclusion: LEGO提供了一种实用的后训练域适应方法，无需配对数据即可将无监督挑战转化为伪监督学习，成功适应预训练图像修复模型到未见领域，同时保持原始模型的鲁棒性。
...
TL;DR: ItemizedCLIP：针对医学影像和遥感等非目标中心领域，提出从项目化文本监督中学习完整可解释视觉表征的框架，通过跨注意力模块和定制目标实现项目独立性和表征完整性。


<details>
  <summary>Details</summary>
Motivation: 许多视觉领域（如医学影像和遥感）包含项目化文本标注：单个图像中有多个描述不同独立语义发现的文本项目。这种监督与标准多标题监督不同，后者是冗余或高度重叠的。需要学习完整且可解释的视觉表征。

Method: 使用跨注意力模块生成文本项目条件化的视觉嵌入，并设计定制目标联合强制项目独立性（不同项目对应不同区域）和表征完整性（覆盖所有项目）。

Result: 在四个自然项目化文本监督领域（脑MRI、头CT、胸CT、遥感）和一个合成项目化数据集上，ItemizedCLIP在零样本性能和细粒度可解释性方面显著优于基线方法。

Conclusion: ItemizedCLIP产生的表征具有语义基础、项目可区分性、完整性和视觉可解释性，为处理项目化文本监督的视觉领域提供了有效的学习框架。
...
TL;DR: 该研究对Monkey视觉语言模型进行了复现和批判性分析，验证了图像分块策略在恢复局部细节方面的有效性，同时研究了全局上下文的影响，发现结果偏差与任务类型和分块粒度密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管可复现性是科学进步的基础，但复杂的多模态模型往往缺乏透明的实现细节和可访问的训练基础设施。本研究旨在对Monkey视觉语言模型进行详细的复现和批判性分析，以促进高分辨率多模态建模的进一步发展。

Method: 使用开源检查点复现Monkey VLM，重新实现其训练流程，采用图像分块策略处理高分辨率图像，并进一步研究全局上下文信息对模型性能的影响。

Result: 确认了原始Monkey VLM的关键发现：图像分块策略能有效恢复局部细节。同时发现结果存在偏差，这些偏差的程度严重依赖于任务类型和分块粒度。全局上下文信息的加入为未来高分辨率多模态建模提供了实用见解。

Conclusion: 图像分块是处理高分辨率图像的有效策略，但模型性能受任务类型和分块粒度影响显著。全局上下文信息的整合对提升模型理解能力具有重要价值，为未来高分辨率多模态模型设计提供了重要参考。
...
TL;DR: 提出基于视频编解码器的轻量级3D高斯溅射压缩方法LGSCV，通过两阶段Morton扫描和MiniPLAS优化，在保持高质量的同时大幅降低计算复杂度和编码时间。
...
Method: 1) 提出两阶段Morton扫描生成块状2D图；2) 使用PCA降低球谐函数维度；3) 设计灵活快速的MiniPLAS在特定块大小内排列基元；4) 结合SH PCA和MiniPLAS优化率失真性能。

Result: 在MPEG数据集上，LGSCV相比现有方法获得超过20%的率失真增益，2D图生成时间降至约1秒，编码时间减少50%。

Conclusion: LGSCV通过创新的两阶段扫描和MiniPLAS设计，实现了高效、轻量的3D高斯溅射压缩，显著提升了率失真性能并大幅降低了计算复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD建模耗时费力，现有3D生成方法主要关注单部件生成，不适用于需要多个部件在语义和几何约束下组装的真实应用场景。
...
Conclusion: CADKnitter通过组合式生成和几何引导采样，解决了多部件CAD模型在语义和几何约束下的生成问题，为实际应用提供了有效的解决方案。
...
TL;DR: AutoRefiner：为自回归视频扩散模型设计的噪声优化器，通过路径噪声优化和反射KV缓存提升样本保真度


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型（AR-VDMs）作为双向VDMs的可扩展替代方案具有潜力，但其样本保真度仍有提升空间。推理时对齐方法虽然能优化噪声空间提升保真度，但基于优化或搜索的方法对AR-VDMs计算成本过高。文本到图像领域已有前馈噪声优化器，但直接扩展到AR-VDMs会失败。

Method: 提出AutoRefiner，专门为AR-VDMs设计的噪声优化器，包含两个关键设计：1）路径噪声优化 - 沿随机去噪路径优化噪声；2）反射KV缓存 - 提升优化效率。该方法作为AR-VDMs的高效插件，无需更新模型参数。

Result: 实验证明AutoRefiner能有效提升AR-VDMs的样本保真度，通过单次前向传播优化噪声，相比传统优化方法更高效实用。

Conclusion: AutoRefiner成功解决了文本到图像噪声优化器无法直接应用于AR-VDMs的问题，为自回归视频扩散模型提供了高效、即插即用的样本质量提升方案。
...
TL;DR: 该研究提出了SmokeBench基准测试，用于评估多模态大语言模型在野火烟雾识别和定位方面的能力，发现现有模型在烟雾分类方面表现尚可，但在早期烟雾精确定位方面存在严重不足。


<details>
  <summary>Details</summary>
Motivation: 野火烟雾具有透明、无定形且常与云混淆的视觉特征，使得早期检测特别困难。当前需要评估多模态大语言模型在这项安全关键任务上的实际能力。
...
Conclusion: 当前MLLM模型在安全关键的野火监测方面存在严重局限性，需要开发能够改进早期烟雾定位能力的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个极端：基于像素的随机视频生成方法计算成本高且难以转化为决策信号；基于视觉基础模型特征的确定性回归方法虽然计算高效且可直接转化为有用信号，但无法捕捉不确定性，导致预测准确性下降。需要一种既能保持计算效率又能捕捉不确定性的方法。

Method: 提出生成式预测器，在视觉基础模型特征空间中进行自回归流匹配。关键创新是将VFM特征编码到适合扩散的紧凑潜在空间中，该潜在空间比之前使用的PCA方法能更有效地保留信息。预测结果可解码为多种有用且可解释的输出模态：语义分割、深度、表面法线甚至RGB图像。

Result: 在相同架构和计算条件下，该方法在所有模态上都比回归方法产生更清晰、更准确的预测。潜在空间不仅适用于预测，还可用于图像生成等其他应用。实验表明该方法能有效捕捉不确定性并提高预测质量。

Conclusion: 在视觉基础模型特征空间中进行随机条件生成为未来世界模型提供了一个有前景且可扩展的基础。该方法结合了计算效率和不确定性捕捉能力，能够产生高质量的多模态预测。
...
Method: 提出FutureX框架：1）自动思维开关评估当前场景是否需要额外推理；2）进入思维模式时，潜在世界模型进行思维链引导的推演预测未来场景表示；3）总结器模块进一步优化运动规划；4）简单场景下使用即时模式直接生成规划。

Result: 实验表明FutureX能提升现有方法，产生更合理的运动规划，减少碰撞而不影响效率。在NAVSIM上为TransFuser带来6.2 PDMS的性能提升。

Conclusion: 思维链驱动的未来场景推理能有效增强端到端规划器的复杂运动规划能力，在动态环境中实现更优的决策。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的说话头生成方法存在推理速度慢和非自回归范式的问题，严重限制了其在实际应用中的使用。需要开发能够实时生成且保持时间一致性的流式生成框架。
...
Result: REST在生成速度和整体性能方面均优于现有最先进方法，成功弥合了自回归方法和基于扩散方法之间的差距，为需要实时说话头生成的应用提供了重要价值。

Conclusion: REST是首个基于扩散模型的实时端到端流式音频驱动说话头生成框架，通过创新的紧凑潜在空间表示、缓存机制和训练策略，实现了高质量的实时生成，具有重要的实际应用价值。
...
TL;DR: RoomPilot是一个统一的3D室内场景生成框架，能够将文本描述或CAD平面图等多种模态输入解析为室内领域特定语言（IDSL），从而生成具有交互语义和物理一致性的可控室内场景。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景生成方法要么处理输入模态范围有限，要么依赖随机过程导致可控性差。游戏开发、建筑可视化和具身AI训练等应用需要能够从多种输入模态生成可控、交互式室内场景的通用解决方案。

Method: 提出RoomPilot框架，将文本描述或CAD平面图等多模态输入解析为室内领域特定语言（IDSL），作为共享语义表示。利用带有交互标注的资产数据集，生成具有真实物体行为的场景，确保功能性和交互语义。

Result: 实验验证了RoomPilot在多模态理解、细粒度场景生成可控性、物理一致性和视觉保真度方面的优越性能，相比传统程序化方法能生成功能活跃而非视觉合理但功能惰性的布局。

Conclusion: RoomPilot通过IDSL作为共享语义表示，实现了从单一模态生成高质量、交互式室内场景的统一框架，是向通用可控3D室内场景生成迈出的重要一步。
...
TL;DR: 本文提出了一种新颖的跨模态提示（ComP）方法，用于解决不完全多模态情感识别中的性能差距和模态欠优化问题，通过增强模态特定特征和提升各模态性能来提高整体识别准确率。


<details>
  <summary>Details</summary>
Motivation: 不完全多模态情感识别（IMER）旨在通过探索部分观测的多源数据来理解人类意图和情感。尽管多模态数据预期能提供更丰富信息，但性能差距和模态欠优化问题阻碍了实际中的有效多模态学习，在数据缺失情况下这些问题更加严重。

Method: 提出跨模态提示（ComP）方法，包括：1）带有动态梯度调制器的渐进提示生成模块，用于产生简洁一致的模态语义线索；2）跨模态知识传播，通过传递的提示选择性地放大模态特征中的一致信息；3）协调器设计，动态重新加权模态输出作为平衡策略的补充。
...
TL;DR: PersonaLive是一个基于扩散模型的实时肖像动画框架，通过混合隐式信号、外观蒸馏和自回归微块流式生成，实现了7-22倍的速度提升，支持低延迟实时应用。
...
Result: 实验表明PersonaLive在保持高质量的同时，相比之前的扩散模型实现了7-22倍的加速，达到了最先进的性能水平。

Conclusion: PersonaLive通过创新的训练策略和生成范式，成功解决了扩散模型在实时肖像动画中的延迟问题，为直播等实时应用场景提供了可行的解决方案。
...
TL;DR: 该研究评估了多时相Sentinel-2卫星影像在景观尺度上监测入侵杂草锯齿丛草的效果，发现其分类精度（68%）略优于航空影像模型（67%），证明卫星遥感可作为成本效益更高的替代方案。


<details>
  <summary>Details</summary>
Motivation: 锯齿丛草是澳大利亚维多利亚州的主要入侵杂草，对生态系统和农业造成严重威胁。传统地面调查方法在小尺度有效但难以扩展到景观尺度，航空影像成本高昂，而卫星遥感虽然空间分辨率较低但更具成本效益和可扩展性。研究旨在评估多时相Sentinel-2影像能否成为景观尺度监测的可行替代方案。

Method: 使用多时相Sentinel-2卫星影像，结合光谱波段、纹理特征、植被指数和季节性数据，开发了11个不同特征组合的模型。采用随机森林分类器，将最佳Sentinel-2模型与航空影像模型在相同数据集上进行性能比较。

Result: 最佳Sentinel-2模型（M76*）的总体精度为68%，总体Kappa系数为0.55，略优于最佳航空影像模型的67%总体精度和0.52 Kappa系数。这表明尽管卫星影像空间分辨率较低，但通过利用其更高的光谱分辨率和季节性物候信息，仍能获得可比甚至略优的分类性能。

Conclusion: 多时相特征增强的卫星模型在入侵物种分类方面具有潜力，为景观尺度监测提供了成本效益高且可扩展的替代方案。Sentinel-2影像虽然空间分辨率较低，但通过结合光谱和季节性信息，能够实现与航空影像相当的分类精度，适合大规模入侵物种监测应用。
...
Method: 采用自回归扩散范式实现任意长度视频生成；通过双级缓存机制将一致性问题解耦为镜头间一致性和镜头内连贯性：镜头记忆缓存先前镜头关键帧以保持角色和场景身份，时间记忆缓存当前镜头帧历史以确保平滑连续运动。

Result: 实验结果表明，该方法在一致性和美学质量指标上均超越现有方法，支持多概念注入和视频扩展等下游任务，并构建了高质量多镜头视频数据集用于训练。

Conclusion: FilmWeaver为创建更一致、可控和叙事驱动的视频内容开辟了新可能性，通过解耦设计实现了高灵活性，支持多轮用户交互生成多镜头视频。
...
TL;DR: ARVAE是一种自回归视频自动编码器，通过解耦时空信息实现高效视频压缩与重建，支持任意长度视频处理，在轻量模型和小规模训练数据下取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频自动编码器通常将空间和时间信息纠缠在一起，限制了捕捉时间一致性的能力，导致性能不理想。需要一种能够解耦时空信息、保持时间一致性并支持任意长度视频处理的解决方案。

Method: 提出自回归视频自动编码器(ARVAE)，以前一帧为条件压缩和重建每一帧。引入时空解耦表示：下采样流场用于时间一致性，空间相对补偿用于新出现内容。编码器将当前帧和前一帧压缩为时间运动和空间补充，解码器在给定前一帧的情况下从潜在表示重建原始帧。采用多阶段训练策略逐步优化模型。

Result: ARVAE在极轻量模型和小规模训练数据下实现了卓越的重建质量。在视频生成任务上的评估显示了其在下游应用中的强大潜力。

Conclusion: ARVAE通过自回归方式和时空解耦表示，有效解决了现有视频自动编码器的时空信息纠缠问题，实现了高效压缩和高质量重建，为视频生成等下游任务提供了有力支持。
...
### [25] [MLLM Machine Unlearning via Visual Knowledge Distillation](https://arxiv.org/abs/2512.11325)
*Yuhang Wang,Zhenxing Niu,Haoxuan Ji,Guangyu He,Haichang Gao,Gang Hua*

Main category: cs.CV

TL;DR: 该论文提出了一种针对多模态大语言模型（MLLM）的视觉知识选择性遗忘方法，通过视觉知识蒸馏方案在中间表示层面进行监督，实现了高效且有效的目标视觉知识擦除。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法主要针对LLM，而MLLM的遗忘研究尚处于早期阶段。需要一种能够选择性擦除目标视觉知识同时保留文本知识的专门方法。

Method: 提出视觉知识蒸馏（VKD）方案，利用MLLM内部的中间视觉表示作为监督信号，仅微调MLLM的视觉组件，实现目标视觉知识的选择性擦除。

Result: 实验表明该方法在有效性和效率方面均优于现有最先进的遗忘方法，并且首次评估了MLLM遗忘对重新学习攻击的鲁棒性。

Conclusion: 该方法通过解耦MLLM中的视觉和文本知识，结合中间表示层面的监督，实现了高效、有效的视觉知识选择性遗忘，为MLLM安全应用提供了重要工具。
...
TL;DR: 本文提出基于Transformer架构的交通事故检测模型，通过整合RGB特征与光流特征，在构建的全面平衡数据集上达到88.3%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 交通事故是全球主要死亡原因，传统计算机视觉方法在事故检测中存在时空理解有限和跨域泛化能力差的问题。现有Transformer模型应用受限的主要原因是缺乏全面多样的数据集，且大多数研究忽略了动态场景理解所需的运动线索。

Method: 1) 构建全面平衡的交通事故数据集，涵盖多种交通环境、事故类型和上下文变化；2) 提出基于Transformer架构的事故检测模型，使用卷积层提取帧内局部相关性，利用Transformer捕获提取特征的序列-时间依赖性；3) 评估多种运动线索整合方法，最终采用RGB特征与光流特征拼接策略。

Result: RGB特征与光流特征拼接的方法在测试中达到最高准确率88.3%。研究还比较了与GPT、Gemini、LLaVA-NeXT-Video等视觉语言模型的性能差异。

Conclusion: 提出的基于Transformer的事故检测模型通过整合运动线索，在全面数据集上表现出色，为自动化交通监控系统提供了有效的解决方案，具有更好的时空理解和跨域泛化能力。
...
Method: 设计了基于硬件的数据采集系统和处理流程，实现了跨视角的亚毫秒级时间同步，并结合精确的姿态标注，构建了包含会议、表演和演示等五个典型社交互动场景的数据集。
...
Method: 1）链式思维引导小语言模型生成任务特定隐式查询向量；2）定义包含相关性、覆盖度和冗余度的连续集合级目标函数，通过Gumbel-Softmax实现可微优化；3）采用师生互学习机制，通过KL散度对齐学生选择器和教师推理器的帧重要性分布。

Result: 在Video-MME、LongVideoBench、MLVU和NExT-QA等多个基准测试中，该方法显著优于现有方法。

Conclusion: 提出的端到端可训练任务自适应框架有效解决了传统关键帧选择方法的局限性，通过动态查询生成、集合级优化和师生互学习机制，实现了更优的视频理解性能。
...
### [29] [Flowception: Temporally Expansive Flow Matching for Video Generation](https://arxiv.org/abs/2512.11438)
*Tariq Berrada Ifriqi,John Nguyen,Karteek Alahari,Jakob Verbeek,Ricky T. Q. Chen*

Main category: cs.CV

TL;DR: Flowception是一个非自回归、可变长度的视频生成框架，通过交替离散帧插入和连续帧去噪来学习概率路径，相比自回归方法减少了误差累积，相比全序列流方法降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法存在局限性：自回归方法容易产生误差累积和漂移问题，而全序列流方法计算复杂度高且难以处理长序列。需要一种既能处理长时上下文又计算高效的视频生成框架。

Method: Flowception框架学习一个概率路径，交替进行离散帧插入和连续帧去噪。帧插入机制在采样过程中作为高效的压缩机制来处理长时上下文，同时允许联合学习视频长度和内容。

Result: 实验结果显示，Flowception在FVD和VBench指标上优于自回归和全序列基线方法，训练FLOPs减少了三倍，并且能够无缝集成图像到视频生成和视频插值等不同任务。

Conclusion: Flowception通过创新的概率路径设计，成功解决了现有视频生成方法的局限性，在保持高质量生成的同时显著降低了计算成本，为可变长度视频生成提供了有效的解决方案。
...
TL;DR: SATMapTR是一个在线地图构建模型，通过门控特征精炼模块和几何感知融合模块有效融合卫星图像，提升自动驾驶高精地图的实时构建精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要实时构建高精地图，但车载传感器数据质量低（能力有限、遮挡频繁），导致地图构建不完整、噪声大、精度低。卫星图像作为辅助输入提供稳定广域视角，但卫星图像本身存在阴影、植被和建筑物遮挡等退化问题，现有特征提取和融合方法效果不佳。
...
Conclusion: SATMapTR通过有效的卫星图像融合显著提升了在线地图构建的精度和鲁棒性，特别是在恶劣条件和扩展感知范围内表现优异，为自动驾驶高精地图实时构建提供了有效解决方案。
...
Method: 1）构建迄今为止最大的牙科多模态数据集（超过12万张牙科图像配详细描述）；2）通过高质量领域知识注入训练增强视觉理解能力；3）采用强化学习阶段进一步强化多模态复杂推理能力；4）模型参数为7B规模。

Result: 在口腔内和全景X光片基准测试以及牙科医学视觉问答子集上，DentalGPT在疾病分类和牙科视觉问答任务中表现优异，超越了许多最先进的多模态大语言模型，尽管只有7B参数。
...
TL;DR: KeyframeFace是一个用于文本到3D面部动画生成的大规模多模态数据集，通过关键帧级监督提供表情脚本、视频、ARKit系数等数据，并提出首个利用LLM先验进行可解释面部运动合成的文本到动画框架。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和方法主要关注语音驱动动画或无结构表情序列，缺乏语义基础和时序结构，难以生成富有表现力的人类表演。需要能够理解时序结构化语义和细粒度表情变化的文本到动画系统。

Method: 1) 构建KeyframeFace数据集：包含2100个表情脚本、单目视频、逐帧ARKit系数、上下文背景、复杂情感、手动定义关键帧，以及基于LLM和MLLM的多视角标注。2) 提出基于LLM先验的文本到动画框架：将LLM的语义理解能力与ARKit系数的可解释结构对齐，实现高保真表情动画生成。

Result: KeyframeFace建立了文本到动画研究的新基础，提供关键帧引导和上下文感知的动画生成能力。提出的LLM-based框架实现了可解释的面部运动合成，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态学习在视觉理解方面进展迅速，但在视觉生成中，强大的多模态大语言模型通常被简化为扩散模型的全局文本编码器，其大部分推理和规划能力未被充分利用，导致理解与生成之间存在差距。
...
TL;DR: FreqDINO是一个频率引导的超声图像分割框架，通过多尺度频率提取对齐和边界细化模块，增强了对超声图像边界的感知能力，超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 超声图像分割在临床诊断中很重要，但受到斑点噪声和成像伪影的挑战。DINOv3虽然在医学图像分割中表现出色，但由于在自然图像上预训练，对超声特定的边界退化不够敏感。
...
TL;DR: 提出了一种处理卫星图像时间序列的方法，通过并行处理同一冰川的多帧图像并在特征图间交换时间信息，以稳定冰前缘预测，在CaFFe基准数据集上达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型在自动描绘合成孔径雷达图像中的冰前缘时，经常难以正确分类受季节性条件（如冰混合物或积雪表面）影响的区域，需要更稳定的预测方法。

Method: 提出并行处理同一冰川卫星图像时间序列的多帧图像，并在对应特征图之间交换时间信息以稳定每个预测，将这种方法集成到当前最先进的Tyrion架构中。

Result: 在CaFFe基准数据集上实现了新的最优性能：平均距离误差为184.4米，平均交并比为83.6。

Conclusion: 通过处理时间序列图像并在特征层交换时间信息，可以有效稳定冰前缘的预测，显著提升模型在复杂季节性条件下的性能。
...
TL;DR: UFVideo是首个具备统一多粒度协同理解能力的视频大语言模型，能够在单个模型中处理全局、像素和时间尺度的视频理解任务，并通过统一视觉-语言引导对齐实现灵活的视频分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型局限于特定视频理解任务，缺乏全面、多粒度的视频感知能力。为了填补这一空白，需要开发能够实现综合多粒度协同理解的视频大语言模型。

Method: 设计了统一视觉-语言引导对齐方法，使模型能够灵活处理全局、像素和时间尺度的视频理解。模型动态编码不同任务的视觉和文本输入，生成文本响应、时间定位或基础掩码。

Result: 构建了UFVideo-Bench基准测试，包含三个不同尺度的协作任务，展示了UFVideo相对于GPT-4o的灵活性和优势。在9个公共基准测试中验证了模型的有效性，涵盖了各种常见视频理解任务。

Conclusion: UFVideo作为首个统一多粒度协同理解的视频大语言模型，为未来视频大语言模型的发展提供了有价值的见解，实现了全面的视频感知能力。
...
TL;DR: 该研究首次系统评估了AI生成图像模型指纹检测技术的安全性，发现这些技术在对抗条件下存在严重脆弱性，移除攻击成功率高达80%，伪造攻击效果因目标模型而异，现有方法在鲁棒性和准确性之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 模型指纹检测技术作为追踪AI生成图像来源的有前景方法，但其在对抗条件下的鲁棒性尚未得到充分研究。本研究旨在填补这一空白，系统评估这些技术的安全性，为开发更鲁棒的指纹检测方法提供基础。

Method: 研究首先形式化了威胁模型，包括白盒和黑盒访问条件，以及两种攻击目标：指纹移除（消除识别痕迹）和指纹伪造（导致错误归因）。实现了五种攻击策略，在12个最先进的图像生成器上评估了14种代表性的指纹检测方法，涵盖RGB、频率和学习特征三个领域。

Result: 实验结果显示清洁环境和对抗环境之间存在显著性能差距。移除攻击非常有效，白盒设置下成功率常超过80%，受限黑盒访问下也超过50%。伪造攻击虽然更具挑战性，但其成功率因目标模型而异。研究还发现效用-鲁棒性权衡：归因准确率最高的方法往往最易受攻击。尽管某些技术在特定设置下表现出鲁棒性，但没有一种方法在所有评估的威胁模型中同时实现高鲁棒性和高准确性。

Conclusion: 这些发现强调了开发平衡鲁棒性和准确性的技术的必要性，并指出了实现这一目标的最有前景的方法方向。现有模型指纹检测技术在对抗条件下存在严重脆弱性，需要进一步研究来提高其安全性。
...
TL;DR: 提出全自动单目RGB视频动态场景重建流水线，通过增强动态高斯泼溅的先验知识，结合视频分割、深度优化和轨迹跟踪技术，显著提升重建质量和渲染效果


<details>
  <summary>Details</summary>
Motivation: 现有单目动态场景重建方法在处理细薄结构和复杂运动时存在局限性，需要更精确的先验知识和更鲁棒的跟踪机制来提升重建质量

Method: 1) 视频分割结合极线误差图生成对象级掩码；2) 对象深度损失优化视频深度一致性；3) 基于骨架的采样和掩码引导重识别生成可靠2D轨迹；4) 虚拟视图深度损失去除漂浮物；5) 支架投影损失将运动节点绑定到轨迹，保持精细几何和连贯运动

Result: 系统超越先前单目动态场景重建方法，提供视觉上更优的渲染效果，能够更好地处理细薄结构和复杂运动

Conclusion: 通过增强动态高斯泼溅的先验知识，结合自动化的分割、深度优化和跟踪机制，实现了高质量的单目动态场景重建，为复杂动态场景的建模提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 无人机高效作物检测对精准农业规模化至关重要，但由于目标尺度小和环境变化性，检测仍然具有挑战性。特别是在稻田中检测水稻幼苗面临小目标检测困难和高分辨率航空图像处理难题。
...
Conclusion: 该研究证明了基于迁移学习的Faster R-CNN架构在稻田水稻幼苗检测中的有效性，特别是在处理小目标和高分辨率航空图像方面具有良好性能，为精准农业的规模化应用提供了技术支持。
...
Method: 提出三个核心模块：1）通道信息交互模块（CIIM），在通道维度引入水平-垂直整合机制进行特征重组和交互；2）协作解码架构，通过边界提取（BE）和区域提取（RE）模块生成先验知识，使用混合注意力协同校准解码特征；3）多尺度增强（MSE）模块丰富上下文特征表示。

Result: 在四个COD基准数据集上验证了模型的有效性和最先进性能，并将模型迁移到显著目标检测、息肉分割、透明目标检测、工业和道路缺陷检测等下游任务，展示了良好的适应性。

Conclusion: 提出的ARNet-v2模型通过创新的通道信息交互和协作解码架构，有效解决了伪装目标检测中的关键挑战，在多个任务上表现出优越性能，代码和实验结果已公开。
...
TL;DR: 该论文提出了"N-Body问题"：给定一个第一人称视角视频，如何让N个人并行执行视频中的任务以最大化加速，同时避免物理冲突。作者开发了评估指标和结构化提示策略，在厨房活动视频上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 人类可以直观地并行化复杂活动，但模型能否从观察单个人的行为中学习这种能力？研究旨在解决如何让多个人并行执行单个人视频中的任务，同时避免现实世界约束冲突的问题。

Method: 1. 形式化定义N-Body问题；2. 提出评估指标：性能指标（加速比、任务覆盖率）和可行性指标（空间碰撞、物体冲突、因果约束）；3. 引入结构化提示策略，引导视觉语言模型（VLM）推理3D环境、物体使用和时间依赖关系，生成可行的并行执行方案。

Result: 在EPIC-Kitchens和HD-EPIC的100个视频上测试，当N=2时，相比Gemini 2.5 Pro的基线提示，方法将动作覆盖率提高了45%，同时将碰撞率、物体冲突和因果冲突分别降低了55%、45%和55%。

Conclusion: 该研究成功解决了从单人视频学习并行执行复杂任务的挑战，提出的结构化提示策略显著提高了并行化的可行性和效率，为多智能体协作任务规划提供了新方法。
...
TL;DR: FlowDC是一种用于复杂图像编辑的流匹配方法，通过将复杂编辑分解为多个子编辑效果并行叠加，并分解速度场以保持源图像结构，在复杂编辑任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前基于预训练文本到图像流匹配模型的图像编辑方法在简单编辑（单一编辑目标）上表现良好，但在复杂编辑（多个编辑目标）任务中面临挑战。现有的单轮和多轮编辑方法分别存在长文本跟随和累积不一致性问题，难以在语义对齐和源一致性之间取得平衡。

Method: FlowDC将复杂编辑分解为多个子编辑效果，在编辑过程中并行叠加这些效果。同时观察到与编辑位移正交的速度分量会损害源结构保持，因此分解速度场并衰减正交部分以更好地保持源一致性。还构建了复杂编辑基准Complex-PIE-Bench用于评估。

Result: 在两个基准测试中，FlowDC相比现有方法表现出优越的结果。论文还详细分析了模块设计的消融实验。

Conclusion: FlowDC通过解耦复杂编辑为多个子效果并行叠加，并分解速度场衰减正交分量，有效解决了复杂图像编辑中语义对齐和源一致性的平衡问题，在复杂编辑任务中取得了优异性能。
...
TL;DR: JoyAvatar：一种音频驱动的自回归模型，能够实时推理并生成无限长度视频，通过渐进步长引导、运动条件注入和缓存重置的无界RoPE解决现有方法的计算开销大、错误累积和质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于DiT的音频驱动化身生成方法虽然取得了进展，但受到高计算开销和无法生成长时长视频的限制。自回归方法通过块状自回归扩散方法解决了这个问题，但存在错误累积和质量下降的问题。

Method: 提出JoyAvatar模型，包含三个关键技术：(1) 渐进步长引导(PSB)：为初始帧分配更多去噪步骤以稳定生成并减少错误累积；(2) 运动条件注入(MCI)：通过注入噪声污染的前一帧作为运动条件来增强时间一致性；(3) 通过缓存重置的无界RoPE(URCR)：通过动态位置编码实现无限长度生成。

Result: 13亿参数的因果模型在单个GPU上达到16 FPS，在视觉质量、时间一致性和唇部同步方面取得了有竞争力的结果。

Conclusion: JoyAvatar通过创新的渐进步长引导、运动条件注入和缓存重置的无界RoPE技术，实现了实时推理和无限长度视频生成，解决了现有音频驱动化身生成方法的计算开销大、错误累积和质量下降问题。
...
TL;DR: 开发半自动标注流程提升疲劳驾驶检测精度，在YawDD+数据集上实现99.34%分类准确率和95.69%检测mAP，边缘设备可达59.8 FPS


<details>
  <summary>Details</summary>
Motivation: 驾驶员疲劳是道路事故的主要原因（24%事故涉及疲劳驾驶），打哈欠是早期行为指标。现有机器学习方法面临视频标注数据集引入的粗粒度时间标注系统噪声问题

Method: 开发半自动标注流程，结合人在环验证，应用于YawDD数据集创建YawDD+。使用MNasNet分类器和YOLOv11检测器架构进行训练

Result: 相比视频级监督，帧准确率提升6%，mAP提升5%。达到99.34%分类准确率和95.69%检测mAP。在NVIDIA Jetson Nano边缘AI硬件上实现59.8 FPS
...
TL;DR: DOS是一个自监督学习框架，通过仅在可观察点蒸馏语义相关性软映射来学习3D点云表示，无需额外数据或标注。


<details>
  <summary>Details</summary>
Motivation: 3D点云的自监督学习面临几何不规则、重建易走捷径、语义分布不平衡等挑战，需要更有效的表示学习方法。
...
TL;DR: CADMorph是一个用于几何驱动的参数化CAD编辑的框架，通过规划-生成-验证三阶段，利用预训练模型在数据稀缺情况下实现结构保持、语义有效和形状保真的编辑。


<details>
  <summary>Details</summary>
Motivation: 在CAD设计中，调整几何形状需要同步编辑底层的参数序列，这需要保持原始序列结构、确保语义有效性并维持高形状保真度，而现有方法面临编辑数据三元组稀缺的挑战。

Method: CADMorph采用迭代的规划-生成-验证框架：1) 规划阶段使用P2S模型的交叉注意力图识别需要修改的片段并提供编辑掩码；2) 生成阶段使用MPP模型填充掩码进行语义有效编辑；3) 验证阶段使用P2S模型在形状潜在空间中测量候选序列与目标形状的距离并选择最佳方案。

Result: CADMorph超越了GPT-4o和专门的CAD基线方法，支持迭代编辑和逆向工程增强等下游应用。P2S和MPP模型均无需三元组数据训练，克服了数据稀缺瓶颈。

Conclusion: CADMorph通过协调预训练的领域特定基础模型，有效解决了几何驱动的参数化CAD编辑中的结构保持、语义有效性和形状保真度问题，为CAD设计提供了强大的编辑工具。
...
TL;DR: VGGT是一个3D基础模型，通过单次前向传播推断相机几何和场景结构。研究分析了该模型是否真正理解几何概念，还是主要依赖学习到的外观数据先验。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究VGGT模型的内在机制：它是否像传统多视角方法那样基于几何概念，还是主要依赖学习到的外观数据驱动先验。需要系统分析模型内部表示是否真正涌现出几何理解。

Method: 通过探测中间特征、分析注意力模式和执行干预实验，研究VGGT如何实现其功能。使用空间输入掩码和扰动实验评估模型对遮挡、外观变化和相机配置的鲁棒性，并与经典多阶段流程进行比较。

Result: 研究发现VGGT在其全局注意力层中隐式执行对应点匹配，并编码了极线几何，尽管训练时没有明确的几何约束。模型在内部化几何结构的同时利用了学习到的数据驱动先验。

Conclusion: VGGT模型确实在内部表示中涌现出几何理解，隐式执行几何操作，同时结合了学习到的数据先验。这揭示了现代基础模型如何将几何结构内部化。
...
TL;DR: 该研究提出两种方法将事件相机数据与多模态大语言模型结合，通过重建作为桥梁，并创建首个客观的事件MLLM基准EvQA进行评测。


<details>
  <summary>Details</summary>
Motivation: 将事件相机与多模态大语言模型结合有望在挑战性视觉条件下实现通用场景理解，但需要平衡事件数据独特优势与帧基模型兼容性之间的矛盾。

Method: 提出两种方法：1) 基于帧的重建与标记化(FRT)方法；2) 利用事件稀疏性的自适应重建与标记化(ART)方法。同时创建EvQA基准，包含来自22个公共数据集的1000个事件-Q&A对。

Result: 实验表明，提出的方法在EvQA基准上实现了最先进的性能，凸显了MLLM在事件视觉中的巨大潜力。

Conclusion: 通过重建作为桥梁，成功将事件相机数据与多模态大语言模型集成，为事件视觉理解提供了有效解决方案，并通过客观基准验证了方法的优越性。
...
TL;DR: THREASURE-Net是一个用于树木高度回归和超分辨率的端到端深度学习框架，利用Sentinel-2时间序列和LiDAR高度数据生成高精度年度树高地图，在2.5-10米分辨率上达到2.62-2.88米的平均绝对误差。
...
Method: 提出THREASURE-Net端到端框架，使用Sentinel-2时间序列数据，基于LiDAR HD数据在法国大都会地区多个空间分辨率上训练，生成年度高度地图。模型包含三个变体，分别输出2.5米、5米和10米分辨率的树高预测。该模型不依赖预训练模型或高分辨率光学影像，仅从LiDAR高度信息中学习。

Result: THREASURE-Net在2.5米、5米和10米分辨率上分别达到2.62米、2.72米和2.88米的平均绝对误差，优于基于Sentinel数据的现有方法，与基于高分辨率影像的方法具有竞争力。
...
Result: Infinity-8B在两个基准测试中均实现了最强的整体组合对齐能力，而Infinity-2B在多个类别中也匹配或超越了更大的扩散模型，显示出有利的效率-性能权衡。相比之下，SDXL和PixArt-α在属性敏感和空间任务中表现出持续的弱点。

Conclusion: 该研究首次系统比较了VAR和扩散方法在组合对齐方面的表现，为T2I模型的未来发展建立了统一的基准，表明视觉自回归模型在组合对齐任务上具有显著优势。
...
TL;DR: SSL-MedSAM2：基于SAM2大模型的半监督医学图像分割框架，结合训练无关的少样本学习和迭代全监督学习，在肝脏分割任务中取得优异性能


<details>
  <summary>Details</summary>
Motivation: 医学图像标注耗时且昂贵，限制了深度学习模型在临床中的应用。半监督学习能够减少标注成本，但现有方法仍有改进空间。本文旨在利用预训练大模型Segment Anything Model 2 (SAM2)的优势，构建高效的半监督分割框架。

Method: 提出SSL-MedSAM2框架，包含两个分支：1) TFFS-MedSAM2：基于预训练SAM2的训练无关少样本学习分支，用于生成伪标签；2) FSL-nnUNet：基于nnUNet的迭代全监督学习分支，用于伪标签精炼。两个分支协同工作，通过迭代优化提升分割性能。

Result: 在MICCAI2025挑战赛CARE-LiSeg（肝脏分割）任务中表现优异：GED4测试集平均Dice分数0.9710，Hausdorff距离20.07；T1 MRI测试集平均Dice分数0.9648，Hausdorff距离21.97。代码已开源。

Conclusion: SSL-MedSAM2框架有效结合了预训练大模型的泛化能力和半监督学习的标注效率优势，在肝脏分割任务中取得了state-of-the-art性能，为医学图像分割提供了一种高效且可扩展的解决方案。
...
TL;DR: 3DTeethSAM：基于SAM2的3D牙齿分割方法，通过2D渲染-分割-3D重建流程，引入三个轻量级可学习模块提升分割精度，在3DTeethSeg基准上达到91.90% IoU的新SOTA。
...
Method: 将Segment Anything Model 2 (SAM2)适配到3D牙齿分割：1）从预定义视角渲染3D牙齿模型的2D图像；2）使用SAM2进行2D分割；3）通过2D-3D投影重建3D结果。引入三个轻量级模块：提示嵌入生成器、掩码细化器和掩码分类器，并加入可变形全局注意力插件(DGAP)提升编码器性能。

Result: 在3DTeethSeg基准测试中，该方法在高分辨率3D牙齿网格上实现了91.90%的IoU，创造了该领域的新最先进水平。

Conclusion: 3DTeethSAM成功将SAM2基础模型适配到3D牙齿分割任务，通过创新的2D-3D转换策略和轻量级模块设计，显著提升了分割精度，为数字牙科提供了有效的解决方案。
...
TL;DR: 该研究提出了一个无需微调、直接评估基础模型3D空间理解能力的基准测试，通过多视角图像分割任务来评估模型在视角变化下的性能。
...
Method: 基于Hummingbird框架扩展到3D场景理解，使用MVImgNet数据集，通过给定特定角度的图像（keys）来分割新视角图像（queries），并根据key-query视角差异分为4个难度等级进行评估。

Result: 评估了8个最先进的基础模型，发现基于DINO的编码器在大视角变化下仍保持竞争力，而像VGGT这样的3D感知模型需要专门的多视角调整。

Conclusion: 该基准测试能够有效评估基础模型的固有3D空间理解能力，为机器人、自动驾驶等实际应用提供了重要的评估工具。
...
TL;DR: ContextSeisNet：基于上下文学习的智能地震多次波压制方法，通过空间相关示例对实现用户可控、空间一致的处理效果，相比传统方法和U-Net基线表现更优且数据效率更高。


<details>
  <summary>Details</summary>
Motivation: 传统地震处理方法面临噪声数据、手动参数调优等挑战，现有深度学习方法存在空间不一致性和缺乏用户控制的问题。需要开发一种既能保证空间一致性又能提供用户灵活性的智能处理方法。

Method: 提出ContextSeisNet上下文学习模型，将预测条件建立在空间相关示例对上：同一地震测线的相邻共深度点道集及其对应标签。模型在推理时通过观察相似道集应如何处理来学习任务特定的处理行为，无需重新训练。

Result: 在合成数据上，ContextSeisNet在定量指标上优于U-Net基线，并展现出增强的相邻道集空间一致性。在野外数据上，相比传统Radon多次波压制和U-Net基线，模型实现了更优的横向一致性、改进的近偏移距性能和更完整的多次波去除。值得注意的是，ContextSeisNet仅用10%的训练数据就达到了可比的野外数据性能。

Conclusion: ContextSeisNet为空间一致的地震多次波压制提供了一种实用方法，具有扩展到其他地震处理任务的潜力。该方法通过用户定义示例提供灵活性，同时改善了地震测线的横向一致性，显著提高了数据效率。
...
TL;DR: 本文首次提出"具身图像压缩"科学问题，针对具身AI在多智能体系统中的通信约束，建立标准化基准EmbodiedComp，在超低比特率下评估压缩对具身任务执行的影响。


<details>
  <summary>Details</summary>
Motivation: 随着机器智能的快速发展，压缩目标已从特定任务虚拟模型转向在真实环境中操作的具身智能体。为应对多智能体系统中具身AI的通信限制并确保实时任务执行，需要研究专门针对具身智能体的图像压缩方法。

Method: 建立标准化基准EmbodiedComp，在闭环设置下对超低比特率条件进行系统评估。通过模拟和真实环境中的大量实证研究，分析现有视觉-语言-动作模型在压缩后的任务执行能力。

Result: 研究发现，当压缩比特率低于具身比特率阈值时，现有的视觉-语言-动作模型无法可靠执行即使是简单的操作任务。这凸显了为具身智能体开发领域特定压缩方法的必要性。

Conclusion: EmbodiedComp基准将促进针对具身智能体的领域特定压缩技术的发展，从而加速具身AI在真实世界中的部署和应用。
...
TL;DR: FactorPortrait是一种视频扩散方法，用于可控的肖像动画生成，能够从单张肖像图像、驱动视频和相机轨迹合成逼真的动画，同时实现面部表情、头部运动和相机视角的解耦控制。


<details>
  <summary>Details</summary>
Motivation: 现有肖像动画方法通常在控制精度、真实感和视角一致性方面存在局限，难以同时实现面部表情、头部运动和相机视角的精细解耦控制。需要一种能够从单张图像生成逼真动画，同时支持任意视角合成的综合解决方案。

Method: 1. 使用预训练图像编码器从驱动视频提取面部表情潜在表示作为控制信号；2. 通过提出的表情控制器将这些潜在表示高效注入视频扩散Transformer；3. 使用Plücker射线图和从3D身体网格跟踪渲染的法线图来控制相机和头部姿态；4. 构建大规模合成数据集进行训练，包含多样化的相机视角、头部姿态和面部表情组合。

Result: 实验表明，FactorPortrait在真实感、表现力、控制精度和视角一致性方面优于现有方法，能够生成高质量的肖像动画，同时实现面部表情、头部运动和相机视角的精确解耦控制。

Conclusion: FactorPortrait提出了一种有效的视频扩散方法，通过解耦的面部表情、头部运动和相机视角控制信号，实现了从单张肖像图像生成逼真动画和任意视角合成的能力，在多个评估指标上超越了现有方法。
...
TL;DR: CLV-Net提出了一种跨模态上下文感知学习方法，通过视觉提示（边界框）引导模型生成相关的分割掩码和描述，解决遥感图像中相似对象识别和用户意图对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在只有简单文本提示时难以引导模型关注用户相关区域，且遥感图像中许多对象视觉外观高度相似且具有丰富的对象间关系，这进一步影响了准确识别。

Method: 提出CLV-Net，允许用户提供简单的视觉提示（边界框）来指示感兴趣区域，使用该提示引导模型生成相关的分割掩码和描述。核心设计包括：上下文感知掩码解码器建模和整合对象间关系以增强目标表示；语义和关系对齐模块，包含跨模态语义一致性损失增强细粒度区分，以及关系一致性损失确保文本关系与视觉交互的对齐。

Result: 在两个基准数据集上的综合实验表明，CLV-Net优于现有方法，建立了新的最先进结果。模型有效捕捉用户意图，产生精确的、意图对齐的多模态输出。

Conclusion: CLV-Net通过视觉提示引导和跨模态上下文感知学习，成功解决了遥感图像理解中用户意图对齐和相似对象识别的挑战，为多模态图像理解提供了有效解决方案。
...
TL;DR: Depth Copy Paste：一种多模态深度感知数据增强框架，通过语义匹配、精确分割和深度引导放置，生成物理一致的人脸检测训练样本，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统复制粘贴增强方法由于前景提取不准确、场景几何不一致和背景语义不匹配，往往产生不真实的合成图像。这限制了人脸检测系统在遮挡、光照变化和复杂环境下的鲁棒性提升。

Method: 1. 使用BLIP和CLIP联合评估语义和视觉一致性，自动检索最适合给定前景人物的背景图像；2. 集成SAM3进行精确分割，结合Depth-Anything提取非遮挡可见人物区域；3. 引入深度引导滑动窗口放置机制，在背景深度图上搜索具有最佳深度连续性和尺度对齐的粘贴位置。

Result: Depth Copy Paste能够生成更多样化和真实的训练数据，相比传统复制粘贴和无深度增强方法，在下游人脸检测任务中带来显著的性能提升。
...
Result: 在Total-Text数据集上测试10小时，文本识别率达到约94.62%。该数据集包含高分辨率图像，模拟了各种问题条件，证明了系统在非受控成像条件下的有效性。

Conclusion: 提出的方法在实践中的混合源文本分类任务中表现有效，即使在非受控的成像条件下也能可靠工作，为文档自动化处理提供了实用解决方案。
...
TL;DR: EditMGT：首个基于掩码生成变换器（MGT）的图像编辑框架，通过局部化解码范式实现精确的区域编辑，避免非目标区域被意外修改，相比扩散模型参数更少、速度更快。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像编辑中虽然视觉质量优秀，但其全局去噪机制会将局部编辑目标与全图上下文混淆，导致非目标区域被意外修改。需要寻找替代方法来解决这一挑战。

Method: 1) 利用MGT的交叉注意力图提供定位信号，设计多层注意力整合方案实现细粒度定位；2) 引入区域保持采样，限制在低注意力区域进行标记翻转以抑制虚假编辑；3) 构建CrispEdit-2M高分辨率数据集；4) 通过注意力注入将预训练文本到图像MGT适配为图像编辑模型。

Result: 在四个标准基准测试中，仅用不到10亿参数就实现了相似性性能，同时编辑速度快6倍。在风格变化和风格迁移任务上分别提升3.6%和17.6%，编辑质量相当或更优。

Conclusion: MGT的局部化解码范式为图像编辑提供了新思路，EditMGT框架通过精确的区域定位和编辑控制，有效解决了扩散模型中非目标区域被意外修改的问题，实现了高效高质量的图像编辑。
...
Method: 采用弱到强泛化方法，结合多头Mask-RCNN扩展和高效通道注意力机制，通过伪标签校正和覆盖扩展实现泛化，能够学习分割新仪器/新协议的新图像类别。

Result: 与五种当前广泛使用的方法进行基准测试，显示出显著改进。提供了代码、样本WSI图像和高分辨率分割结果供社区采用和适配。

Conclusion: 该方法实现了全自动、无需人工标注的重叠细胞核分割，在生产环境中提供了自动化的分割质量自诊断指标，为社区提供了可用的开源解决方案。
...
TL;DR: SVG-T2I是一个基于视觉基础模型表示空间的文本到图像生成框架，通过扩展SVG框架，直接在VFM特征域实现高质量图像合成，性能达到GenEval 0.75和DPG-Bench 85.78。


<details>
  <summary>Details</summary>
Motivation: 虽然基于视觉基础模型表示的视觉生成为整合视觉理解、感知和生成提供了统一途径，但完全在VFM表示空间中训练大规模文本到图像扩散模型的研究仍然很少，需要填补这一空白。

Method: 扩展SVG框架，提出SVG-T2I，利用标准文本到图像扩散流程，直接在视觉基础模型特征域进行图像合成，无需传统像素空间操作。

Result: SVG-T2I在GenEval上达到0.75分，在DPG-Bench上达到85.78分，表现出与现有方法竞争的性能，验证了VFM表示在生成任务中的内在能力。

Conclusion: 该工作证明了视觉基础模型表示空间在生成任务中的有效性，并开源了整个项目（包括自编码器、生成模型、训练推理评估流程和预训练权重），以促进表示驱动的视觉生成研究。
...
<details>
  <summary>Details</summary>
Motivation: 在标签稀缺的环境中（如每张图像包含大量细胞的细胞计数任务），生成逼真的合成显微图像对于训练深度学习模型至关重要。传统域适应方法在合成图像缺乏真实样本复杂纹理和视觉模式时，往往难以弥合域差距。

Method: 采用InST（Inversion-Based Style Transfer）框架，结合扩散模型中的潜在空间自适应实例归一化（AdaIN）和随机反转技术，将真实荧光显微图像的风格迁移到合成图像上，同时弱化保留内容结构。

Result: 使用InST合成图像训练的模型在细胞计数任务中表现优异：相比硬编码合成数据训练的模型，MAE降低37%；相比Cell200-s数据集训练的模型，MAE降低52%（从53.70降至25.95）；甚至优于仅使用真实数据训练的模型（25.95 vs. 27.74 MAE）。结合DACS和CutMix等轻量级域适应技术可进一步提升性能。

Conclusion: InST风格迁移能最有效地缩小合成与真实显微数据之间的域差距，为提升细胞计数性能提供可扩展路径，同时最小化人工标注工作量。该方法在生物医学图像分析领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频遮罩数据集规模有限且缺乏真实感，分割数据虽然能增强语义稳定性，但缺乏有效的边界监督导致遮罩缺乏细节

Method: 1. 提出学习型遮罩质量评估器(MQE)评估语义和边界质量；2. 使用MQE进行在线训练反馈和离线数据筛选；3. 构建VMReal大规模真实视频遮罩数据集；4. 引入参考帧训练策略处理长视频外观变化；5. 开发MatAnyone 2模型

Result: 构建了包含28K视频片段和2.4M帧的VMReal数据集，MatAnyone 2在合成和真实基准测试中超越了所有先前方法，各项指标均达到最优

Conclusion: MQE评估器能有效提升视频遮罩质量，VMReal数据集解决了数据稀缺问题，MatAnyone 2模型在视频遮罩任务上实现了最先进的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在保持结构（特别是人体和动物等可变形物体）方面存在挑战，仅靠扩大训练数据无法解决物理上不合理的运动过渡。现有方法依赖于有噪声的运动表示（如光流或骨架），存在局限性。

Method: 1）双向特征融合模块：从SAM2等循环模型中提取全局结构保持运动先验；2）局部Gram流损失：对齐局部特征如何一起运动。将SAM2的结构保持运动先验蒸馏到CogVideoX扩散模型中。

Result: 在VBench上达到95.51%，比REPA提升2.60%；FVD降低到360.57，比REPA和LoRA微调分别改善21.20%和22.46%；人类偏好研究中获得71.4%的偏好率。

Conclusion: 通过从自回归跟踪模型中蒸馏结构保持运动先验到双向扩散模型，显著提升了视频生成的结构保真度和运动真实性，在定量和定性评估中都超越了现有基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模视频生成模型在建模真实世界场景的光照交互方面表现出色，但缺乏一个闭环框架来联合理解场景内在属性、利用这些属性进行视频合成，并支持可编辑的内在表示。

Method: V-RGBX采用交错条件机制，统一三个关键能力：1) 视频逆渲染到内在通道，2) 从内在表示进行真实感视频合成，3) 基于关键帧的视频编辑（以内在通道为条件）。

Result: V-RGBX能够生成时间一致、真实感强的视频，并以物理合理的方式将关键帧编辑传播到整个序列，在物体外观编辑和场景级重光照等应用中超越了先前方法。

Conclusion: V-RGBX是首个端到端的内在感知视频编辑框架，通过交错条件机制实现了直观、物理基础的视频编辑，为视频编辑提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然实现了快速优化和实时渲染，但其简化的alpha混合和密度积分近似限制了渲染复杂重叠半透明物体的能力。现有方法依赖顺序相关的混合和粗糙近似，无法准确模拟光在复杂半透明介质中的衰减。

Method: 基于矩的透射率计算方法：1）从所有贡献的3D高斯中解析推导和计算每像素矩；2）基于统计矩构建紧凑连续的密度分布表示；3）为每条光线重建连续透射率函数；4）在每个高斯内独立采样。该方法无需光线追踪或像素级排序。

Result: 该方法显著提高了重建和渲染质量，能够准确模拟复杂半透明介质中的光衰减，在渲染复杂重叠半透明物体方面表现出色，填补了光栅化与物理准确性之间的差距。

Conclusion: 通过基于矩的透射率计算方法，成功扩展了基于光栅化的3D高斯表示渲染能力，实现了高保真的半透明物体渲染，无需依赖光线追踪或复杂的像素排序，显著提升了渲染质量和物理准确性。
...
### [69] [ASR Under the Stethoscope: Evaluating Biases in Clinical Speech Recognition across Indian Languages](https://arxiv.org/abs/2512.10967)
...
<details>
  <summary>Details</summary>
Motivation: ASR在临床记录中应用日益广泛，但其在印度多语言和人口多样化医疗环境中的可靠性尚不明确，需要进行系统评估以确保公平部署。

Method: 使用真实世界临床访谈数据，评估包括Indic Whisper、Whisper、Sarvam、Google speech to text、Gemma3n、Omnilingual、Vaani和Gemini在内的领先ASR模型，分析转录准确性在语言、说话者和人口亚组间的差异。

Result: 结果显示模型和语言间存在显著性能差异，部分系统在印度英语上表现良好但在代码混合或方言语音上失败；发现与说话者角色（患者vs临床医生）和性别相关的系统性性能差距。
...
### [70] [Benchmarking Automatic Speech Recognition Models for African Languages](https://arxiv.org/abs/2512.10968)
...
TL;DR: 该研究系统性地评估了四种先进ASR模型在13种非洲语言上的表现，通过在不同数据规模（1-400小时）下的微调实验，揭示了不同模型在低资源条件下的数据效率、扩展性和适用场景差异。
...
Method: 在13种非洲语言上对四种最先进的ASR模型进行基准测试，使用1到400小时不同规模的转录数据进行微调。除了报告错误率外，还分析模型在不同条件下的行为差异，并评估外部语言模型解码的效果。
...
Conclusion: 该研究通过揭示预训练覆盖度、模型架构、数据集领域和资源可用性之间的相互作用，为低资源语言ASR系统设计提供了实用见解，有助于为代表性不足的语言选择合适的ASR技术方案。
...
### [71] [MedBioRAG: Semantic Search and Retrieval-Augmented Generation with Large Language Models for Medical and Biological QA](https://arxiv.org/abs/2512.10996)
*Seonok Kim*

Main category: cs.CL

TL;DR: MedBioRAG是一个用于生物医学问答的检索增强生成模型，通过结合语义和词汇搜索、文档检索和监督微调，在多个生物医学QA任务上超越了现有最先进模型和GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）技术显著提升了大型语言模型处理复杂问答任务的能力，但在生物医学领域的应用仍有改进空间。本研究旨在开发一个专门针对生物医学问答的检索增强模型，以提高在该领域的性能表现。

Method: MedBioRAG采用语义和词汇搜索相结合的检索策略，进行文档检索和排序，并结合监督微调。模型在多个基准数据集（NFCorpus、TREC-COVID、MedQA、PubMedQA、BioASQ）上进行评估，涵盖文本检索、封闭式问答和长格式问答任务。

Result: 实验结果显示，MedBioRAG在所有评估任务中都超越了先前的最先进模型和GPT-4o基础模型。具体表现在：文档检索的NDCG和MRR分数提升，封闭式问答准确率提高，长格式问答的ROUGE分数改善。

Conclusion: 研究表明，基于语义搜索的检索和LLM微调在生物医学应用中具有显著效果。MedBioRAG通过高效的文档检索和上下文感知的响应生成，为生物医学问答任务提供了有效的解决方案。
...
### [72] [KBQA-R1: Reinforcing Large Language Models for Knowledge Base Question Answering](https://arxiv.org/abs/2512.10999)
*Xin Sun,Zhongqi Chen,Xing Zheng,Qiang Liu,Shu Wu,Bowen Song,Zilei Wang,Weiqiang Wang,Liang Wang*

Main category: cs.CL

TL;DR: KBQA-R1：通过强化学习将知识库问答从文本模仿转向交互优化，引入参考拒绝采样解决冷启动问题，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前KBQA方法存在两种失败模式：要么生成幻觉查询而不验证模式存在性，要么采用僵化的模板推理模仿合成轨迹而缺乏对环境的真正理解。需要解决这些限制。

Method: 提出KBQA-R1框架，将KBQA视为多轮决策过程，使用强化学习（特别是Group Relative Policy Optimization）通过具体执行反馈优化策略。同时引入Referenced Rejection Sampling数据合成方法，通过严格对齐推理轨迹与真实动作序列解决冷启动问题。

Result: 在WebQSP、GrailQA和GraphQuestions等数据集上的广泛实验表明，KBQA-R1达到了最先进的性能，有效将LLM推理建立在可验证的执行基础上。
...
TL;DR: MultiScript30k是一个新的多语言数据集扩展，将Multi30k的英语版本翻译成阿拉伯语、西班牙语、乌克兰语和简体/繁体中文，以解决原数据集仅支持欧洲语言的问题。


<details>
  <summary>Details</summary>
Motivation: Multi30k数据集在多模态机器翻译研究中被广泛使用，但仅支持捷克语、英语、法语和德语这四种欧洲语言，限制了多语言研究的多样性。现有扩展支持的语言、语系和文字系统仍然有限，需要更全面的全球语言覆盖。

Method: 使用NLLB200-3.3B模型将Multi30k的英语版本翻译成五种语言：阿拉伯语(Ar)、西班牙语(Es)、乌克兰语(Uk)、简体中文(Zh_Hans)和繁体中文(Zh_Hant)，创建包含超过30000个句子的MultiScript30k数据集。

Result: 相似性分析显示，除繁体中文外，所有语言的余弦相似度均大于0.8，对称KL散度小于0.000251，与之前的扩展数据集相当。COMETKiwi评分显示混合结果：阿拉伯语版本与ArEnMulti30k相当，但乌克兰语版本比Multi30k-Uk低6.4%。

Conclusion: MultiScript30k扩展了Multi30k数据集的语言覆盖范围，支持更多文字系统和全球语言，为多模态机器翻译研究提供了更丰富的多语言资源，尽管在某些语言上的翻译质量仍有提升空间。
...
### [74] [Applying NLP to iMessages: Understanding Topic Avoidance, Responsiveness, and Sentiment](https://arxiv.org/abs/2512.11079)
*Alan Gerber,Sam Cooperman*

Main category: cs.CL

TL;DR: 本文介绍了一个iMessage文本消息分析器，用于分析本地存储的iMessage数据，通过主题建模、响应时间分析、不情愿评分和情感分析等方法，探索个人通信数据的研究价值。


<details>
  <summary>Details</summary>
Motivation: 随着社会对短格式电子通信的依赖增加，用户很少考虑消息平台收集的数据用途。虽然大多数公司严格保护数据，但苹果为Mac上的iMessage用户提供了本地存储的消息文件，这为分析个人通信数据创造了机会。

Method: 开发了一个iMessage文本消息分析器，通过分析本地存储的iMessage数据文件，采用主题建模、响应时间分析、不情愿评分和情感分析等方法，探索五个主要研究问题。

Result: 论文展示了如何使用该分析器回答研究问题，包括主题识别、通信模式分析、参与度评估和情感倾向分析，证明了该工具在iMessage数据研究中的潜力。

Conclusion: iMessage分析器能够有效挖掘个人通信数据的价值，为未来iMessage数据研究提供了实用工具和研究框架，展示了个人数据自我分析的可能性。
...
### [75] [Explanation Bias is a Product: Revealing the Hidden Lexical and Position Preferences in Post-Hoc Feature Attribution](https://arxiv.org/abs/2512.11108)
*Jonathan Kamp,Roos Bakker,Dominique Blok*

Main category: cs.CL

TL;DR: 该研究探讨了特征归因方法（如积分梯度）在语言模型解释中的偏见问题，提出了一个模型和方法无关的评估框架，通过三个指标系统评估了词汇和位置偏见。


<details>
  <summary>Details</summary>
Motivation: 不同特征归因方法对同一输入的解释可能存在很大差异，这源于方法本身的偏见。用户可能意识到这个问题而对这些解释不信任，或者不了解的用户可能过度信任。需要超越表面不一致性，结构化地分析这些偏见。

Method: 提出了一个模型和方法无关的评估框架，包含三个评估指标。系统评估了两种Transformer模型的词汇偏见和位置偏见（输入中的"什么"和"哪里"）。首先在人工数据上进行受控的伪随机分类任务，然后在自然数据上进行半受控的因果关系检测任务。

Result: 发现词汇偏见和位置偏见在模型比较中存在结构性不平衡，在一种类型上得分高的模型在另一种类型上得分低。还发现产生异常解释的方法本身更可能存在偏见。

Conclusion: 特征归因方法存在系统性偏见，需要结构化评估框架来理解和比较不同方法的偏见特性，这对提高语言模型解释的可信度和实用性至关重要。
...
### [76] [FIBER: A Multilingual Evaluation Resource for Factual Inference Bias](https://arxiv.org/abs/2512.11110)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注单实体事实和单语言数据，缺乏对多实体设置和多语言环境的系统性评估，因此需要开发一个更全面的基准来评估大语言模型的事实可靠性和偏见问题。

Method: 创建FIBER多语言基准数据集，包含三种语言（英语、意大利语、土耳其语）和三种任务类型（句子补全、问答、对象计数预测）。使用该基准评估提示语言是否诱导实体选择推理偏见，并比较模型在单实体与多实体问题上的表现。

Result: 提示语言会影响模型输出，特别是与语言对应国家相关的实体（31%的主题显示推理偏见分数>0.5）。土耳其语提示比意大利语显示更高偏见（83%的主题）。多实体问题比单实体问题更难处理。英语表现最佳，土耳其语和意大利语得分较低。更大模型（Llama-3.1-8B和Qwen-2.5-7B）表现优于较小模型（3B-4B）。

Conclusion: 大语言模型的事实知识评估需要考虑多语言和多实体设置，提示语言会诱导推理偏见且存在语言依赖性模式，模型在处理多实体问题时面临更大挑战，模型规模和语言类型都会影响性能表现。
...
### [77] [SciLaD: A Large-Scale, Transparent, Reproducible Dataset for Natural Scientific Language Processing](https://arxiv.org/abs/2512.11192)
...
TL;DR: SciLaD是一个全新的大规模科学语言数据集，包含超过1000万篇英文科学出版物和3500万篇多语言出版物，并提供了可扩展的数据生成管道和预训练模型。
...
Method: 使用开源工具构建可扩展的数据生成管道，从公开数据源收集科学出版物，创建两个数据分割：一个经过整理的英文分割（1000万+出版物）和一个多语言的未过滤TEI XML分割（3500万+出版物）。

Result: 成功构建了SciLaD数据集，并在其上预训练了RoBERTa模型，该模型在综合基准测试中表现与类似规模的其他科学语言模型相当，验证了数据集的质量和实用性。

Conclusion: SciLaD数据集展示了开源工具在大规模科学数据整理中的可行性，数据集和评估管道的发布将促进科学自然语言处理领域的可重复性、透明度和进一步研究。
...
### [78] [Multi-Intent Spoken Language Understanding: Methods, Trends, and Challenges](https://arxiv.org/abs/2512.11258)
...
<details>
  <summary>Details</summary>
Motivation: 多意图SLU涉及多个意图检测和槽位填充两个任务，能处理包含多个意图的话语，更贴近实际应用场景。尽管该领域已取得显著进展，但缺乏全面系统的综述研究，因此本文旨在填补这一空白。

Method: 从两个视角进行深入分析：1）解码范式，2）建模方法。在此基础上，对代表性模型的性能进行比较，并分析其优缺点。

Result: 提供了多意图SLU领域的全面综述，系统梳理了现有研究进展，比较了不同模型的性能表现，并指出了各方法的优势与局限性。

Conclusion: 本文为多意图SLU研究提供了有价值的见解和参考，讨论了当前面临的挑战，并展望了未来有前景的研究方向，旨在推动该领域的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着研究出版物快速增长，系统综述中的标题和摘要筛选成为最耗时、资源密集的步骤之一，需要提高效率来减轻人工负担。

Method: 设计了两阶段动态少样本学习方法：第一阶段使用低成本LLM进行初步筛选，第二阶段对低置信度实例使用高性能LLM重新评估，在控制计算成本的同时提升筛选性能。

Result: 在10个系统综述上评估该方法，结果显示其具有强大的泛化能力和成本效益，能够减少人工筛选负担并加速系统综述过程。

Conclusion: 提出的两阶段动态少样本学习方法在系统综述标题和摘要筛选中表现出良好的效率和性能，有潜力在实际应用中减轻人工负担并加速系统综述过程。
...
TL;DR: 该论文提出使用强化学习（RL）来改进大型语言模型的推理能力，通过Group Relative Policy Optimization（GRPO）让模型从任务结果中直接学习推理策略，从而提升工具调用准确性和答案正确性。


<details>
  <summary>Details</summary>
Motivation: 监督微调（SFT）在数据分布变化时泛化能力有限，而高质量推理标注成本高、主观性强且难以扩展。最近推理导向模型（如o1和R1）显示出比非推理模型更好的泛化能力和可靠性，但缺乏高质量推理数据限制了SFT的效果。

Method: 提出一个管道方法：LLMs生成推理步骤来指导工具调用（如函数调用）和最终答案生成。使用Group Relative Policy Optimization（GRPO）强化学习算法，设计围绕工具准确性和答案正确性的奖励函数，让模型迭代优化推理和行动策略。
...
TL;DR: AdaSD是一种无需超参数调整的自适应推测解码方法，通过动态调整生成长度和接受标准，在保持精度的同时显著提升LLM推理速度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数规模增大导致推理速度下降，现有推测解码方法需要额外训练、超参数调优或模型任务预分析，限制了实际部署的便利性。

Method: 提出自适应推测解码(AdaSD)，引入两个自适应阈值：一个基于令牌熵动态决定何时停止候选令牌生成，另一个基于Jensen-Shannon距离决定令牌接受标准，两者在推理过程中实时更新。

Result: 在基准数据集上的实验表明，AdaSD相比标准推测解码实现了高达49%的加速，同时将精度下降控制在2%以内，且无需预分析或微调即可与现成模型兼容。

Conclusion: AdaSD提供了一种实用、高效且自适应的LLM推理解决方案，消除了传统推测解码方法的部署障碍，在保持模型精度的同时显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理长且嘈杂的检索上下文时经常产生幻觉，因为它们依赖于虚假相关性而非真正的因果关系。需要一种方法来改善模型的事实基础和可解释性。

Method: 提出CIP因果提示框架：1) 构建实体、动作和事件之间的因果关系序列；2) 将因果序列注入提示中以引导模型关注因果相关证据；3) 通过因果干预和反事实推理抑制非因果推理路径。

Result: 在包括GPT-4o、Gemini 2.0 Flash和Llama 3.1在内的七个主流语言模型上测试，CIP显著提升推理质量和可靠性：可归因率提高2.6分，因果一致性得分提高0.38，有效信息密度提升四倍，端到端响应延迟降低高达55.1%。

Conclusion: 因果推理可能成为改善大语言模型可解释性、稳定性和效率的有前景范式。CIP框架通过因果引导有效减少了幻觉问题，提升了模型性能。
...
TL;DR: LegalRikai: Open Benchmark是一个由法律专业人士创建的日本企业法律实践基准测试，包含4个复杂任务和100个需要长篇结构化输出的样本，通过人类和自动化评估揭示了模型在文档级编辑方面的弱点。


<details>
  <summary>Details</summary>
Motivation: 创建面向日本企业法律实践的基准测试，以评估LLM在复杂法律任务中的实际表现，弥补传统短文本任务的不足，促进法律领域更实践导向的研究。

Method: 由法律专业人士在律师监督下创建包含4个复杂任务的基准测试，包含100个需要长篇结构化输出的样本。使用GPT-5、Gemini 2.5 Pro和Claude Opus 4.1等领先LLM进行人类和自动化评估，采用多个实践标准进行评估。

Result: 人类评估发现抽象指令会引发不必要的修改，揭示了模型在文档级编辑方面的弱点，这些弱点在传统短文本任务中被忽略。自动化评估在具有明确语言基础的标准上与人类判断一致，但结构一致性评估仍然具有挑战性。自动化评估可作为专家资源有限时的筛选工具。

Conclusion: 提出了数据集评估框架以促进法律领域更实践导向的研究，证明了自动化评估作为筛选工具的实用性，特别是在专家资源有限的情况下。揭示了当前LLM在复杂法律文档编辑任务中的局限性。
...
Result: 在9个多语言复合任务（数学、编程、医疗领域）上的实验表明：qa-FLoRA比静态融合方法在LLaMA-2上提升约5%，在LLaMA-3上提升约6%；比无训练基线在LLaMA-2上提升约7%，在LLaMA-3上提升约10%，同时显著缩小了与监督基线的差距。

Conclusion: qa-FLoRA提供了一种高效、无需训练数据的LoRA融合方案，能够处理多领域复合查询，层级融合权重分析揭示了可解释的融合模式，证明了该方法在多领域适应中的有效性。
...
TL;DR: 本研究开发了自动化方法来检测和分类捷克最高法院判决中的司法推理，挑战了关于中欧和东欧形式主义司法的普遍观点，通过自然语言处理技术实现了对法律论证的可靠分类。


<details>
  <summary>Details</summary>
Motivation: 虽然法院必须为其判决提供理由，但大规模系统分析司法推理仍然困难。本研究旨在反驳关于中欧和东欧形式主义司法的说法，通过开发自动化方法来检测和分类司法推理。

Method: 创建MADON数据集（272个捷克最高法院判决，9,183个段落，8种论证类型和形式主义标签），使用30万捷克法院判决语料库，通过持续预训练调整transformer LLMs用于捷克法律领域，采用非对称损失和类别加权解决数据集不平衡问题，开发三阶段流水线（ModernBERT、Llama 3.1和传统机器学习）。

Result: 最佳模型成功检测论证段落（82.6% macro-F1），分类传统法律论证类型（77.5% macro-F1），并将判决分类为形式主义/非形式主义（83.2% macro-F1）。三阶段流水线在降低计算成本和提高可解释性的同时取得了有希望的结果。

Conclusion: 法律论证挖掘能够实现可靠的司法哲学分类，挑战了关于中欧和东欧形式主义司法的普遍叙事。该方法论易于在不同司法管辖区复制，所有流水线、数据集、指南、模型和源代码均已公开。
...
<details>
  <summary>Details</summary>
Motivation: 探索数据选择策略对机器翻译微调效果的影响，特别是在开放大语言模型背景下，理解不同数据选择方法对最终翻译性能的贡献

Method: 使用日语-英语语料库，在受控训练条件下比较五种数据选择器：TF-IDF、COMET Kiwi、QuRate、FD-Score和随机选择

Result: 语义选择器（如COMET Kiwi、QuRate）在性能上持续优于词汇（TF-IDF）和几何启发式方法；即使所选数据差异小于3%，对模型性能的影响也很显著

Conclusion: 微调对数据质量高度敏感，语义数据选择策略对提升机器翻译性能至关重要，即使微小的数据差异也会产生实质性影响
...
Conclusion: 提出的片段选择方法能够有效识别视频中的关键时刻用于多模态摘要，在性能和计算效率之间取得良好平衡。
...
TL;DR: CLINIC是一个全面的多语言基准测试，用于评估医疗领域语言模型的可信度，涵盖5个关键维度、18个任务、15种语言，揭示了模型在事实准确性、偏见、隐私等方面的不足。
...
Method: 提出了CLINIC基准测试，系统性地评估语言模型在5个可信度维度上的表现：真实性、公平性、安全性、鲁棒性和隐私性。通过18个多样化任务，涵盖15种语言（覆盖各大洲），涉及疾病状况、预防措施、诊断测试、治疗、手术和药物等关键医疗主题。

Result: 广泛评估显示，语言模型在事实正确性方面存在困难，在不同人口统计和语言群体中表现出偏见，容易受到隐私泄露和对抗性攻击的影响。

Conclusion: CLINIC通过揭示这些不足，为增强语言模型在全球多语言医疗环境中的覆盖范围和安全性奠定了基础。
...
TL;DR: 提出了一个基于DictaBERT 2.0的希伯来语医疗语言模型，用于从电子健康记录中提取结构化临床时间线，构建患者旅程。模型在500多万条去标识化医院记录上持续预训练，在两个新数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 需要从希伯来语电子健康记录中提取结构化临床时间线来构建患者旅程，但缺乏专门针对希伯来语医疗文本的预训练语言模型。

Method: 基于DictaBERT 2.0架构，在500多万条去标识化的医院记录上进行持续预训练，引入词汇适应技术提高标记效率，创建了两个新的带时间关系标注的数据集（内科/急诊和肿瘤科）进行评估。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然具有强大的自然语言理解和推理能力，但存在幻觉问题（生成事实错误内容）。虽然已有许多减少幻觉的方法，但这些方法对创造力的影响尚未被探索。这一空白对于需要事实准确性和创造性假设生成的AI辅助科学发现尤为重要。

Method: 研究了三种幻觉减少技术：验证链（CoVe）、层对比解码（DoLa）和检索增强生成（RAG）。在多个模型家族（LLaMA、Qwen、Mistral）和不同规模（1B-70B参数）上，使用两个创造力基准测试（NeoCoder和CS4）进行评估。

Result: 这些方法对发散性创造力有相反的影响：CoVe增强发散性思维，DoLa抑制发散性思维，而RAG影响最小。研究结果为科学应用中平衡事实准确性和创造性探索提供了选择幻觉减少方法的指导。

Conclusion: 在AI辅助科学发现中，需要根据具体需求平衡事实准确性和创造性探索，选择适当的幻觉减少方法。CoVe适合需要发散性思维的任务，DoLa适合需要严格控制事实准确性的任务，而RAG对创造力影响较小。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM审计方法通常关注特定方面（如偏见检测），但缺乏对输入token依赖性的整体理解。在医疗、法律等高风险领域部署黑盒LLM时，需要了解模型输出如何依赖输入token，但现有方法无法处理随机性和梯度不可访问的问题。

Method: 提出基于分布的敏感性分析（DBSA），这是一种轻量级、模型无关的方法，无需对LLM做分布假设。通过分析模型输出对每个输入token的敏感性，为实践者提供即插即用的可视化探索工具。

Result: 通过示例演示DBSA能够帮助用户检查LLM输入，发现现有可解释性方法可能忽略的敏感性。该方法能够处理LLM的随机性，且无需计算提示级梯度。

Conclusion: DBSA为黑盒LLM审计提供了一种实用的敏感性分析工具，特别适用于依赖不可访问API端点的实际应用场景，帮助用户理解模型输出对输入token的依赖关系。
...
TL;DR: 提出基于Merlin-Arthur协议的RAG训练框架，将检索器与生成器视为交互式证明系统，通过对抗性证据训练LLM学会基于证据回答、拒绝或依赖具体证据片段，提高RAG系统的可靠性和可解释性。
...
Method: 引入基于Merlin-Arthur协议的训练框架：Arthur（生成器LLM）处理来源未知的问题，Merlin提供有用证据，Morgana注入对抗性误导上下文。两者使用线性时间XAI方法识别和修改对Arthur影响最大的证据。训练Arthur学会：(1)当上下文支持时回答，(2)当证据不足时拒绝，(3)依赖真正支撑答案的具体上下文片段。同时引入严格评估框架分离解释保真度与基线预测误差。

Result: 在三个RAG数据集和两种不同规模的模型家族上，M/A训练的LLM显示出改进的groundedness、完整性、正确性和拒绝行为，减少幻觉，且无需手动标注不可回答问题。检索器通过自动生成的M/A硬正负样本也提高了召回率和MRR。

Conclusion: 自主交互式证明风格的监督为可靠RAG系统提供了原则性和实用性的路径，使检索文档成为可验证证据而非建议。该方法通过解释信息分数（EIF）量化模型能力与不完美基准的相对互信息保证。
...
TL;DR: 该研究使用BERTopic主题建模方法分析1955-2018年历史报纸档案中关于核能和核安全的公共话语，克服了传统方法的局限性，揭示了主题的时空演变和长期趋势。


<details>
  <summary>Details</summary>
Motivation: 从大量非结构化历史报纸档案中提取连贯且人类可理解的主题面临重大挑战，包括主题演变、OCR噪声和文本量巨大。传统主题建模方法（如LDA）难以捕捉历史文本中话语的复杂性和动态性。

Method: 采用BERTopic神经主题建模方法，利用基于transformer的嵌入来提取和分类主题。研究聚焦1955-2018年发表的报纸文章，专门分析核能和核安全相关话语，分析语料库中的主题分布并追踪其时间演变。

Result: BERTopic能够更准确地探索公共话语模式，包括核能与核武器相关主题的共现及其随时间的重要性变化。该方法展现了可扩展性和上下文敏感性，为从报纸档案中提取历史话语提供了更丰富的见解。

Conclusion: BERTopic作为传统方法的替代方案，在历史研究中具有重要价值。研究结果为历史、核能和社会科学研究做出贡献，同时反思当前局限性并提出未来工作方向。
...
TL;DR: 该论文建立了确定性推测生成算法的首个"紧"下界，通过将令牌生成过程与分支随机游走类比，证明了每个推测迭代成功预测的令牌数存在理论上限。


<details>
  <summary>Details</summary>
Motivation: 推测生成技术通过并行验证多个草稿令牌来加速大型语言模型推理，但可实现的加速上限仍不明确。需要理解推测生成算法的基本限制。

Method: 将令牌生成过程与分支随机游走建立平行关系，分析最优草稿树选择问题。在基本假设下，通过数学证明建立理论界限。

Result: 证明了每个推测迭代成功预测的令牌数期望值存在上界：𝔼[X] ≤ (μ+ μ₂)log(P)/μ² + O(1)，其中P是验证器容量，μ是验证器输出分布的期望熵，μ₂是期望第二对数矩。

Conclusion: 该研究为并行令牌生成的限制提供了新见解，可指导未来推测解码系统的设计。在Llama模型上的实证评估验证了理论预测的紧致性。
...
TL;DR: SUMFORU是一个可操控的评论摘要框架，通过用户角色对齐实现个性化购买决策支持，在一致性、事实基础和偏好对齐方面表现优异。
...
Method: 提出SUMFORU框架，基于Amazon 2023评论数据集构建高质量数据管道，采用两阶段对齐方法：1）通过非对称知识蒸馏进行角色感知的监督微调；2）使用偏好估计器进行基于AI反馈的强化学习，捕捉细粒度的角色相关信号。
...
TL;DR: 论文提出CORL框架，使用强化学习端到端微调MILP方案，将分支定界算法求解的MILP转化为可微随机策略，以最大化实际运营性能而非精确建模。


<details>
  <summary>Details</summary>
Motivation: 传统MILP建模难以准确表示随机现实问题，导致实际性能欠佳。现有机器学习方法依赖监督学习、假设已知最优决策、使用MILP梯度替代，存在局限性。

Method: 提出CORL概念验证框架，将分支定界算法求解的MILP转化为与强化学习兼容的可微随机策略，使用真实世界数据进行端到端微调。
...
TL;DR: FutureWeaver是一个多智能体系统中测试时计算资源分配的框架，通过模块化协作和双级规划优化固定预算下的计算分配，显著提升多智能体协作性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时计算扩展技术（如重复采样、自我验证、自我反思）在单智能体场景中能显著提升性能，但缺乏在多智能体系统中分配计算资源以促进协作的原则性机制，特别是在固定预算约束下如何跨智能体分配计算资源的问题尚未解决。

Method: 1. 引入模块化协作：将可重用的多智能体工作流封装为可调用函数，通过自我反思从历史轨迹中抽象出重复交互模式自动生成模块；2. 双级规划架构：在当前任务状态推理的同时推测未来步骤，优化计算资源分配；3. 固定预算约束下的计算分配优化。

Result: 在复杂的智能体基准测试中，FutureWeaver在不同预算设置下始终优于基线方法，验证了其在多智能体协作推理时优化中的有效性。

Conclusion: FutureWeaver填补了多智能体系统中测试时计算资源分配的理论空白，通过模块化协作和前瞻性规划实现了固定预算约束下的高效计算分配，为多智能体协作优化提供了新范式。
...
TL;DR: TriFlow是一个用于真实世界旅行规划的多智能体框架，通过检索、规划和治理三阶段流水线，将开放式用户请求转化为可执行的行程，在空间、时间和预算约束下满足用户偏好。
...
Method: TriFlow采用渐进式多智能体框架，通过检索、规划和治理三阶段流水线统一结构化推理和语言灵活性。通过规则-LLM协作组装约束一致的行程，并进行有界迭代优化以确保全局可行性和个性化。

Result: 在TravelPlanner和TripTailor基准测试中达到最先进结果，分别获得91.1%和97%的最终通过率，相比当前SOTA实现了超过10倍的运行效率提升。

Conclusion: TriFlow通过渐进式多智能体框架有效解决了真实世界旅行规划中的约束满足和效率问题，展示了结构化推理与语言灵活性结合的优势。
...
TL;DR: 该论文首次为大型视觉语言模型(LVLMs)引入了一个名为CAPTURE的CAPTCHA基准测试，涵盖4种主要类型和25种子类型的验证码，用于全面评估LVLMs在解决验证码任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉验证码的基准测试存在局限性，无法全面覆盖所有验证码类型，且缺乏专门针对LVLMs的基准测试。先前研究根据自身研究目标定制基准，导致评估不够全面。

Method: 创建了CAPTURE CAPTCHA基准测试，涵盖31个供应商的4种主要验证码类型和25种子类型。该基准具有广泛的类别多样性、大规模数据以及专门为LVLMs定制的标签。

Result: 使用该基准测试评估当前LVLMs时，发现它们在解决验证码任务上表现不佳，表明现有模型在验证码识别能力方面仍有很大提升空间。

Conclusion: CAPTURE基准填补了先前研究在数据全面性和标签针对性方面的空白，为LVLMs提供了多维度、全面的验证码性能评估工具，有助于推动相关技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出强大的推理和生成能力，但在多轮任务中的行为往往缺乏可靠性和可验证性。需要一种框架来确保智能体在结构化环境中能够按照明确的行为指导行动。

Method: 框架包含三个组件：1）轻量级任务分析器，选择推理和生成策略；2）推理模块，学习可验证的观察-动作映射；3）生成模块，通过验证或确定性合成来强制执行约束合规的输出。这些组件在与环境交互过程中协同演化。

Result: 随着智能体与环境的交互，这些组件协同演化，产生可信赖的行为。该框架能够确保智能体在强化学习形式化描述的环境中表现出可靠和可验证的行为。

Conclusion: 提出的任务完成框架通过整合任务分析、可验证推理和约束合规生成，使基于LLM的智能体能够在结构化环境中表现出可信赖的行为，解决了当前LLM在多轮任务中可靠性和可验证性不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统设计通常优先考虑通信拓扑结构，而忽略了明确的token成本和延迟预算约束，导致在预算有限时成本效益不佳。实际部署中，成本效益是web规模应用的主要限制因素。

Method: 采用"骨干优先-拓扑后定"的设计策略：1）骨干导向的智能体生成：通过LLM池构建、池选择和角色-骨干匹配构建异构骨干的智能体；2）自适应MAS拓扑生成：通过智能体表示学习、门控机制和延迟感知拓扑合成来指导智能体间通信。

Result: 在14个候选LLM骨干的基准测试中，AgentBalance在匹配的token成本预算下获得高达10%的性能提升，在匹配的延迟预算下获得高达22%的性能提升，并在性能-预算曲线上表现出强大的AUC。同时可作为现有MAS的插件，在相同约束下提升性能。

Conclusion: AgentBalance提供了一个在明确预算约束下构建成本效益多智能体系统的有效框架，通过先优化骨干选择再设计通信拓扑的方法，显著提升了预算约束下的系统性能，并具有良好的泛化能力。
...
TL;DR: 研究发现当前XAI中常用的保真度评估指标（插入和删除）存在严重问题：基线选择会偏向某些归因方法，甚至导致线性模型得出矛盾的"最优方法"结论。作者提出基线应具备信息移除和不产生过度分布外图像两个特性，发现现有基线无法同时满足，并提出了新的模型依赖基线来改善这一权衡。


<details>
  <summary>Details</summary>
Motivation: 当前可解释人工智能中广泛使用的归因方法评估存在根本缺陷。插入和删除等保真度指标依赖于基线函数来修改输入图像，但基线选择会系统性地偏向某些归因方法，导致评估结果不可靠。更严重的是，即使简单的线性模型使用常用基线也会得出相互矛盾的"最优方法"结论，这暴露了当前评估体系的严重问题。

Method: 作者首先分析了基线应具备的两个理想特性：1）能够移除信息；2）不产生过度分布外图像。通过实验验证现有基线无法同时满足这两个标准，存在权衡关系。然后利用特征可视化技术，提出了一种新的模型依赖基线，该基线能够移除信息同时避免产生过度分布外图像。

Result: 研究发现现有基线都存在权衡：要么能有效移除信息但会产生分布外图像，要么能保持分布内但信息移除效果差。提出的新基线在信息移除和避免过度分布外图像之间取得了更好的平衡，相比现有基线改善了这一权衡关系。

Conclusion: 当前XAI评估中的基线选择问题比预期更严重，会系统性地偏向某些归因方法。作者提出的新基线在信息移除和避免过度分布外图像之间取得了更好的权衡，为更可靠的归因方法评估提供了改进方向。这项工作强调了重新审视XAI评估指标的重要性。
...
<details>
  <summary>Details</summary>
Motivation: No-Three-In-Line是组合几何中的著名问题，经典ILP方法能保证最优解但面临指数级扩展问题，机器学习方法为模式近似提供了有前景的替代方案，需要系统比较经典优化与AI方法的表现。

Method: 应用PatternBoost transformer学习和强化学习（PPO）首次解决该问题，并与整数线性规划（ILP）进行对比。ILP提供可证明的最优解，PatternBoost通过transformer学习模式，PPO通过强化学习训练策略。
...
TL;DR: BAID框架系统评估AI文本检测器在7类社会语言因素上的偏见，发现检测器对少数群体文本召回率低，强调部署前需进行偏见评估。


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测器在教育和工作场景中被广泛使用，但之前研究仅发现针对英语学习者的孤立偏见案例，缺乏对社会语言因素的系统性评估。

Method: 提出BAID评估框架，收集超过20万样本覆盖7大类社会语言因素（人口统计、年龄、教育水平、方言、正式程度、政治倾向、主题），并为每个样本生成保留内容但反映特定群体写作风格的合成版本，评估4个开源先进AI文本检测器。

Result: 发现检测器性能存在系统性差异，特别是对少数群体文本的召回率较低，表明检测器存在偏见。

Conclusion: BAID提供了可扩展、透明的AI检测器审计方法，强调在工具公开部署前需要进行偏见感知的评估。
...
<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在临床信息处理中的实际表现，验证其是否存在类似代谢功能障碍的功能衰退现象，为AI在医疗领域的应用提供安全警示。
...
TL;DR: 论文主张从传统静态基准转向动态自适应基准测试框架，以应对AI快速发展和实际部署需求之间的差距，并提出了"AI基准测试木工"的教育理念。


<details>
  <summary>Details</summary>
Motivation: 当前AI基准测试面临多重挑战：模型架构快速演进、规模扩大、数据集变化和部署环境多样化，导致静态基准容易被大型语言模型记忆，造成基准结果与实际性能脱节。传统基准强调顶级硬件上的峰值性能，无法指导多样化的实际应用场景。

Method: 提出建立动态自适应基准测试框架，整合不断演进的模型、更新的数据和异构平台，同时保持透明度、可重复性和可解释性。强调通过"AI基准测试木工"教育计划，培养基准设计和使用的专业知识，包括技术创新和系统化教育。

Result: 识别了关键障碍：高资源需求、专用硬件访问有限、基准设计专业知识缺乏、结果与应用领域关联不确定性。基于MLCommons、教育计划和DOE万亿参数联盟的经验，提出了社区驱动的解决方案。

Conclusion: 基准测试必须动态化、包容化，支持应用相关的比较，使评估能够跟上AI发展步伐，促进负责任、可重复和可访问的AI部署。社区努力可以为AI基准测试木工提供基础。
...
TL;DR: 本文提出了一种基于结构因果模型的能源需求预测方法，利用天气和日历信息的因果关系构建贝叶斯模型，在测试集上取得了3.84%的MAPE，表现出色。
...
Method: 提出了一个结构因果模型来解释变量间的因果关系，通过全面分析验证因果假设。然后构建了一个贝叶斯模型，将学习到的因果洞察作为先验知识。模型在未见数据上进行训练和测试。

Result: 模型在测试集上取得了3.84%的平均绝对百分比误差（MAPE），表现出色。在两年数据的交叉验证中，平均MAPE为3.88%，显示出强大的鲁棒性。因果分析揭示了重要发现：能源需求对温度波动的响应具有季节依赖性敏感性，冬季能源需求方差较低，因为温度变化与日常活动模式之间存在解耦效应。

Conclusion: 提出的基于结构因果模型的贝叶斯方法能够有效捕捉能源需求预测中的复杂因果关系，取得了最先进的性能，并为理解能源需求动态提供了有价值的因果洞察。
...
### [108] [WholeBodyVLA: Towards Unified Latent VLA for Whole-Body Loco-Manipulation Control](https://arxiv.org/abs/2512.11047)
...
TL;DR: WholeBodyVLA：首个实现大空间人形机器人全身运动操作（loco-manipulation）的统一框架，通过VLA系统从低成本无动作视频学习，配合专门的LMO强化学习策略，在AgiBot X2上性能提升21.3%


<details>
  <summary>Details</summary>
Motivation: 现有方法（模块化或端到端）缺乏操作感知的运动能力，限制了机器人的工作空间，无法执行大空间全身运动操作。主要问题包括：（1）人形机器人遥操作数据稀缺导致运动操作知识获取困难；（2）现有RL控制器精度和稳定性有限，难以可靠执行运动指令

Method: 提出WholeBodyVLA统一框架：1）统一潜在学习框架，使视觉-语言-动作（VLA）系统能从低成本无动作的第一人称视频学习；2）高效人类数据收集流程扩展数据集；3）专门的运动操作导向（LMO）RL策略，针对前进、转向、下蹲等核心运动操作动作进行精确稳定控制

Result: 在AgiBot X2人形机器人上验证，性能超越先前基线21.3%。展示了在广泛任务上的强大泛化能力和高扩展性，是首个实现大空间人形机器人全身运动操作的系统

Conclusion: WholeBodyVLA通过结合VLA学习框架和专门的LMO RL策略，成功解决了人形机器人大空间全身运动操作的挑战，实现了精确稳定的运动操作能力，为复杂环境中的机器人应用提供了新解决方案
...
### [109] [Taxonomy and Modular Tool System for Versatile and Effective Non-Prehensile Manipulations](https://arxiv.org/abs/2512.11080)
*Cedric-Pascal Sommer,Robert J. Wood,Justin Werfel*

Main category: cs.RO

TL;DR: 该论文提出了一种基于分类学的模块化工具系统，用于扩展标准两指夹爪在执行非抓取操作（如按压、摩擦、刮擦）时的功能，并在航空航天和家庭场景中进行了演示。


<details>
  <summary>Details</summary>
Motivation: 通用机器人末端执行器（如平行夹爪）虽然简单有效，但主要针对抓取操作优化，缺乏执行非抓取操作（如按压、摩擦、刮擦）的能力。人类在执行这类任务时会根据任务特性使用不同身体部位或工具，因此需要系统化理解非抓取操作器的功能特性。

Method: 首先提出了非驱动末端执行器关键属性的分类学，为系统理解非抓取操作器的功能奠定基础。然后基于该分类学设计了一个模块化工具系统，可由标准两指夹爪使用，扩展其在非抓取操作中的功能。

Result: 开发了基于分类学的模块化工具系统，并在需要多种非抓取和抓取操作的航空航天和家庭场景中进行了演示验证，展示了系统扩展标准夹爪功能的有效性。

Conclusion: 通过建立非抓取操作器的分类学并开发相应的模块化工具系统，可以有效扩展标准机器人末端执行器的功能范围，使其能够执行更广泛的非抓取操作任务，提高机器人在复杂环境中的操作能力。
...
### [110] [Design and Experimental Validation of Closed-Form CBF-Based Safe Control for Stewart Platform Under Multiple Constraints](https://arxiv.org/abs/2512.11125)
*Benedictus C. G. Cinun,Tua A. Tamba,Immanuel R. Santjoko,Xiaofeng Wang,Michael A. Gunarso,Bin Hu*

Main category: cs.RO

TL;DR: 提出一种用于Stewart机器人平台的闭式控制屏障函数（CBF）解决方案，能同时处理多个位置和速度约束，无需在每个控制步骤求解二次规划（QP），显著降低计算时间。


<details>
  <summary>Details</summary>
Motivation: 传统基于QP的CBF方法在实时控制中计算负担重，需要为每个控制步骤求解优化问题。本文旨在为Stewart平台开发一种闭式CBF解决方案，消除QP求解需求，实现高效实时安全控制。

Method: 推导出多约束CBF问题的闭式控制律，建立了确保解非奇异的充要条件，从而保证多约束问题的适定性。该方法避免了每个控制步骤的QP求解。

Result: 在定制Stewart平台原型上进行仿真和硬件实验验证，安全保证性能与基于QP的公式相当，同时计算时间减少了一个数量级以上。

Conclusion: 提出的方法为并联机器人系统提供了一个可靠且计算轻量的实时安全控制框架，显著提高了计算效率，同时保持了与传统QP方法相当的安全性能。
...
### [111] [Learning Category-level Last-meter Navigation from RGB Demonstrations of a Single-instance](https://arxiv.org/abs/2512.11173)
*Tzu-Hsien Lee,Fidan Mahmudova,Karthik Desingh*

Main category: cs.RO

TL;DR: 提出基于目标中心模仿学习的最后一米导航框架，使四足移动机械臂仅使用机载RGB相机就能实现精确的操纵就位定位


<details>
  <summary>Details</summary>
Motivation: 现有RGB导航系统只能达到米级精度，不适合移动操纵的精确定位阶段，导致操纵策略偏离训练分布而频繁失败

Method: 基于目标中心模仿学习框架，结合目标图像、多视角RGB观测和文本提示，通过语言驱动分割模块和空间得分矩阵解码器实现显式目标定位和相对位姿推理

Result: 在未见目标物体上，边缘对齐成功率为73.47%，目标对齐成功率为96.94%，证明无需深度、LiDAR或地图先验即可实现类别级精确导航

Conclusion: 仅使用RGB观测即可实现精确的最后一米导航，为统一移动操纵提供了可扩展的途径
...
### [112] [Seeing to Act, Prompting to Specify: A Bayesian Factorization of Vision Language Action Policy](https://arxiv.org/abs/2512.11218)
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在微调过程中容易出现视觉语言模型骨干的灾难性遗忘问题，现有方法依赖外部推理数据和复杂调优。研究发现VLA数据集存在模态不平衡问题——语言多样性远低于视觉和动作多样性，导致模型偏向视觉捷径和语言遗忘。

Method: 提出BayesVLA，采用贝叶斯分解将策略分解为：1）视觉-动作先验，支持"看到即行动"；2）语言条件似然，实现"提示即指定"。同时引入接触前和接触后阶段以更好地利用预训练基础模型。

Result: 信息论分析正式验证了该方法在缓解捷径学习方面的有效性。大量实验表明，相比现有方法，BayesVLA在未见指令、对象和环境上具有更优越的泛化能力。

Conclusion: BayesVLA通过贝叶斯分解解决了VLA模型中的模态不平衡问题，有效保留了泛化能力并促进了指令跟随，无需依赖外部数据或复杂调优。
...
TL;DR: 提出了一种控制中心框架，将降阶Pontryagin最大原理控制器与物理信息梯度下降阶段结合，用于4自由度刚性机械臂控制，实现计算高效的闭环最优控制。


<details>
  <summary>Details</summary>
Motivation: 开发一个既保持严格控制理论结构，又能嵌入机械臂物理约束和负载行为的计算高效控制框架，解决传统方法在物理约束处理上的不足。

Method: 1) 使用降阶Pontryagin最大原理模型提供关节加速度的闭环最优控制律；2) 梯度下降模块通过最小化基于完整刚体动力学的成本函数确定对应时间范围；3) 结构力学反应分析仅用于初始化可行关节速度；4) 将得到的运动轨迹和时间范围输入符号欧拉-拉格朗日模型生成闭环逆动力学输入。

Result: 该框架在保持严格控制理论结构的同时，以计算高效的方式嵌入了机械臂的物理约束和负载行为，能够生成物理上可接受的关节速度初始化，并产生闭环逆动力学控制输入。

Conclusion: 提出的控制中心框架成功地将降阶最优控制与物理信息优化相结合，为刚性机械臂提供了一种既保持理论严谨性又考虑实际物理约束的高效控制解决方案。
...
### [114] [Towards Logic-Aware Manipulation: A Knowledge Primitive for VLM-Based Assistants in Smart Manufacturing](https://arxiv.org/abs/2512.11275)
*Suchang Chen,Daqiang Guo*

Main category: cs.RO

TL;DR: 论文提出了一种面向机器人操作的物体中心操作逻辑模式τ，将执行关键参数（如力/阻抗、轨迹等）作为知识信号，应用于3D打印机线轴移除任务，支持数据增强和逻辑感知检索增强提示。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLM）在机器人操作中注重语义泛化，但忽略了制造单元中接触密集型动作所需的关键执行参数，需要一种能暴露这些参数作为知识信号的框架。

Method: 提出物体中心操作逻辑模式τ，作为八字段元组序列化，包含物体、接口、轨迹、容差和力/阻抗等信息；在3D打印机线轴移除任务中实例化τ和小型知识库；使用适应VLM/LLM规划基准的计划质量指标分析τ条件VLM规划；支持训练时的分类标记数据增强和测试时的逻辑感知检索增强提示。

Result: 在协作单元中成功实例化了τ模式，展示了该模式如何作为智能制造企业辅助系统的构建模块，支持知识信号在人类操作员、VLM辅助系统和机器人控制器之间的传递。

Conclusion: τ模式为制造环境中的机器人操作提供了关键执行参数的标准化表示，填补了现有VLM管道的空白，支持更可靠的接触密集型操作规划，并为智能制造辅助系统奠定了基础。
...
### [115] [Incremental Validation of Automated Driving Functions using Generic Volumes in Micro- Operational Design Domains](https://arxiv.org/abs/2512.11351)
*Steffen Schäfer,Martin Cichon*

Main category: cs.RO

TL;DR: 论文提出了一种结构化方法，将运行设计域细分为微ODD，并生成基于抽象物体表示的测试用例，用于自动驾驶系统的场景化测试验证。


<details>
  <summary>Details</summary>
Motivation: 当前基于场景的自动驾驶系统测试面临挑战：从分类学到具体测试用例的转换过程缺乏结构化，完整性仅停留在理论层面，难以系统验证感知系统在真实世界各种条件下的正确性。

Method: 提出微ODD概念，将运行设计域细分为可管理的小部分；使用抽象物体表示（如不同尺寸的通用立方体）生成测试用例；在闭环协同仿真虚拟环境中进行测试，包含逼真渲染和模拟传感器。

Result: 通过一维横向引导的机车调车场景演示，展示了如何系统探索障碍物检测的边缘情况，并基于车辆行为（碰撞vs安全停止）评估感知质量，支持安全论证标准化框架的开发。

Conclusion: 该方法为自动驾驶功能验证和授权提供了实用步骤，支持开发标准化的安全论证框架，能够系统评估感知性能并探索边缘情况。
...
### [116] [An Anatomy of Vision-Language-Action Models: From Modules to Milestones and Challenges](https://arxiv.org/abs/2512.11362)
...
TL;DR: 这是一篇关于视觉-语言-动作（VLA）模型的综述论文，为研究人员提供了该领域的结构化指南，重点分析了五大核心挑战：表征、执行、泛化、安全、数据集与评估。


<details>
  <summary>Details</summary>
Motivation: VLA模型正在推动机器人技术革命，但该领域发展迅速，新模型和数据集层出不穷，研究人员难以跟上最新进展。本文旨在为研究人员提供清晰的结构化指南，帮助他们理解VLA领域的发展脉络和核心挑战。

Method: 采用结构化分析方法，按照研究者的自然学习路径：首先介绍VLA模型的基本模块，追溯关键里程碑，然后深入分析五大核心挑战（表征、执行、泛化、安全、数据集与评估）。对每个挑战都回顾现有方法并指出未来机会。

Result: 创建了一个全面的VLA领域综述框架，将发展路线图分为三个层次：建立基本的感知-动作循环、在不同具身和环境间扩展能力、确保可信部署。该框架既可作为新手的入门指南，也可作为经验丰富研究者的战略路线图。

Conclusion: 本文为VLA领域提供了系统性的综述和结构化指南，旨在加速学习过程并激发具身智能领域的新思路。通过持续更新的项目页面，确保内容与时俱进，服务于研究社区的发展需求。
...
### [117] [CarlaNCAP: A Framework for Quantifying the Safety of Vulnerable Road Users in Infrastructure-Assisted Collective Perception Using EuroNCAP Scenarios](https://arxiv.org/abs/2512.11551)
*Jörg Gamerdinger,Sven Teufel,Simon Roller,Oliver Bringmann*

Main category: cs.RO

TL;DR: 提出一个评估基础设施辅助集体感知对弱势道路使用者安全改善的框架，包含11k帧EuroNCAP场景数据集，证明基础设施CP可将事故率从33%降低到100%避免


<details>
  <summary>Details</summary>
Motivation: 道路使用者增加导致事故风险上升，弱势道路使用者在城市环境中尤其危险，常被停车或建筑物遮挡。自动驾驶和集体感知是缓解风险的有前景方案，但需要全面研究和数据集来证明安全改善以鼓励决策者采用

Method: 提出评估基础设施辅助集体感知安全改善的框架，创建包含11k帧EuroNCAP安全关键场景的数据集(CarlaNCAP)，通过深入仿真研究评估基础设施CP对弱势道路使用者的安全影响

Result: 基础设施辅助集体感知在安全关键场景中能显著降低事故率，相比仅配备传感器的车辆33%的事故率，基础设施CP可实现高达100%的事故避免

Conclusion: 基础设施辅助集体感知技术能有效提升弱势道路使用者的安全性，特别是在遮挡严重的城市环境中，为决策者采用该技术提供了有力的数据支持
...
### [118] [Cross-Entropy Optimization of Physically Grounded Task and Motion Plans](https://arxiv.org/abs/2512.11571)
...
<details>
  <summary>Details</summary>
Motivation: 传统TAMP算法为追求计算性能、完备性或最优性而简化问题，导致规划结果可能忽略必要的动力学或复杂接触，无法可靠执行需要物体操作的任务。同时，忽略底层控制器影响的方案可能无法获得真实系统的最优或可行执行方案。

Method: 采用GPU并行物理模拟器计算带运动控制器的规划执行方案，显式考虑动力学和与环境的接触。使用交叉熵优化对控制器参数进行采样，以获得低成本解决方案。该方法使用与真实系统相同的控制器，机器人可直接执行计算出的规划。

Result: 在一系列任务中展示了机器人能够利用环境几何形状移动物体，证明了方法的有效性。

Conclusion: 通过结合GPU并行物理模拟和交叉熵优化，实现了考虑真实系统动力学和接触约束的任务与运动规划，使机器人能够可靠执行需要物体操作的任务。
...
### [119] [UniBYD: A Unified Framework for Learning Robotic Manipulation Across Embodiments Beyond Imitation of Human Demonstrations](https://arxiv.org/abs/2512.11609)
*Tingyu Yuan,Biaoliang Guan,Wen Ye,Ziyan Tian,Yi Yang,Weijie Zhou,Yan Huang,Peng Wang,Chaoyang Zhao,Jinqiao Wang*

Main category: cs.RO

TL;DR: UniBYD是一个统一框架，使用动态强化学习算法发现符合机器人物理特性的操作策略，通过统一形态表示和动态PPO算法，超越单纯模仿人类手部动作，在多种机器人手形态上实现更好的操作性能。


<details>
  <summary>Details</summary>
Motivation: 在具身智能中，机器人手与人类手之间的形态差异给从人类演示中学习带来了重大挑战。现有研究虽然尝试使用强化学习来弥合这一差距，但仍局限于单纯复制人类操作，导致任务性能有限。

Method: 提出UniBYD统一框架，包含：1）统一形态表示（UMR）实现跨不同机器人手形态的一致建模；2）基于UMR设计带有退火奖励调度的动态PPO算法，使强化学习从模仿人类演示过渡到探索适应不同机器人形态的策略；3）设计混合马尔可夫影子引擎，在早期训练阶段精细模仿人类操作。

Result: 实验表明，UniBYD在提出的UniManip基准测试中（首个涵盖多种手形态的机器人操作基准），相比当前最先进方法实现了67.90%的成功率提升。

Conclusion: UniBYD框架通过统一形态表示和动态强化学习算法，能够超越单纯的人类手部模仿，发现适应不同机器人物理特性的操作策略，显著提升了跨形态机器人操作任务的性能。
...
### [120] [Architecting Large Action Models for Human-in-the-Loop Intelligent Robots](https://arxiv.org/abs/2512.11620)
*Kanisorn Sangchai,Methasit Boonpun,Withawin Kraipetchara,Paulo Garcia*

Main category: cs.RO

TL;DR: 通过组合现成的基础模型并加入符号包装器和验证机制，可以构建具有控制性、可解释性和可验证性的大型动作模型，无需大规模端到端训练。


<details>
  <summary>Details</summary>
Motivation: 传统符号AI方法在计算和内存成本上存在可扩展性问题，而大型语言模型等神经方法虽然能力强大但缺乏控制性、可解释性和可靠性。大型动作模型试图扩展LLM以涵盖完整的感知-推理-行动循环，但通常需要更全面的训练且存在同样的可靠性缺陷。

Method: 通过组合现成的基础模型构建大型动作模型，并加入符号包装器和对输出的验证机制，实现可验证的神经符号解决方案。具体采用生成规划域定义语言(PDDL)代码来驱动动作执行，并引入人机交互验证阶段。

Result: 在多模态机器人上的实验表明，大型动作模型智能不需要大规模端到端训练，可以通过集成高效感知模型和逻辑驱动核心实现。生成PDDL代码驱动动作执行能够有效缓解动作幻觉问题。

Conclusion: 通过神经符号方法构建的大型动作模型具有控制性、可解释性和可验证性优势，为智能机器人设计提供了新思路，同时指出了确保现场安全仍需解决的挑战。
...
TL;DR: 提出了首个用于推动式移动机器人导航与操作的统一基准测试Bench-Push，包含多样化仿真环境、新型评估指标和基线实现评估


<details>
  <summary>Details</summary>
Motivation: 移动机器人在杂乱环境中需要超越传统避障，利用推动策略完成任务，但现有评估依赖临时设置，缺乏可重复性和跨比较性

Method: 开发Bench-Push基准测试，包含：1）涵盖推动任务核心挑战的多样化仿真环境；2）效率、交互努力和部分任务完成度的新评估指标；3）使用该基准评估现有基线实现

Result: Bench-Push作为开源Python库发布，提供模块化设计，包含代码、文档和训练模型，可用于推动式机器人导航与操作任务的标准化评估

Conclusion: Bench-Push填补了推动式机器人研究领域的基准测试空白，通过统一框架促进算法比较和进展，为可重复研究提供基础
...
TL;DR: 研究机器人外观对人类期望获得的机器人解释的影响，发现人类化外观与拟人化解释呈正相关


<details>
  <summary>Details</summary>
Motivation: 机器人外观已知会影响用户心智模型和人机交互，但尚未研究其对期望机器人解释的影响。本研究探讨机器人的人类化外观是否及在多大程度上引发拟人化（即心理能力归因），以及拟人化水平如何体现在人们期望获得的解释中。

Method: 设计了一项组间研究，包含三种具有不同人类化外观的家用服务机器人的视觉刺激条件。要求受访者为相同的机器人行为提供他们期望从机器人那里获得的解释。

Result: 在所有条件下，大多数解释都是拟人化的。但拟人化解释与人类化外观之间存在正相关关系。还报告了非拟人化解释和机器人描述中观察到的更细微趋势。

Conclusion: 机器人的人类化外观确实会影响人们期望获得的解释类型，外观越像人类，人们越倾向于期望获得拟人化的解释，这表明外观设计在塑造用户对机器人解释的期望方面起着重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型虽然能实现零样本操作，但推理堆栈过于繁重，无法在普通GPU上实现响应式Web演示或高频机器人控制。

Method: BLURR是一个轻量级推理包装器，结合了指令前缀键值缓存、混合精度执行和单步展开调度，减少每步计算量，保持原始观察接口不变。

Result: 在SimplerEnv评估中，BLURR保持了与原始控制器相当的任务成功率，同时显著降低了有效FLOPs和实际延迟时间。

Conclusion: BLURR为在有限计算预算下部署现代视觉-语言-动作策略提供了一种实用方法，并支持交互式Web演示。
...
TL;DR: 提出ProbeMDE框架，通过主动感知结合RGB图像和稀疏本体感知测量来改进单目深度估计，在手术场景等挑战性环境中提高准确性并减少所需测量次数。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计在手术等挑战性环境中（如纹理缺失表面、镜面反射、遮挡）存在不确定性和不准确性，需要结合稀疏本体感知测量来提高性能。

Method: 提出ProbeMDE框架：1) 使用MDE模型集成，结合RGB图像和稀疏深度测量预测密集深度图；2) 通过集成方差量化预测不确定性；3) 测量不确定性相对于候选测量位置的梯度；4) 使用Stein变分梯度下降(SVGD)避免模式崩溃，选择信息量最大的触摸位置。

Result: 在模拟和物理实验（中央气道阻塞手术模型）中验证，该方法在标准深度估计指标上优于基线方法，在提高准确性的同时最小化所需本体感知测量次数。

Conclusion: ProbeMDE框架通过主动感知结合RGB和稀疏本体感知测量，有效提高了单目深度估计在挑战性手术环境中的准确性和效率。
...
TL;DR: 通过多智能体竞争和获胜的稀疏高层目标，强化学习训练出的智能体能够同时掌握敏捷飞行（如高速运动）和策略（如超车或阻挡），在复杂环境中表现优于传统的基于进度奖励的单智能体训练方法，且具有更好的仿真到现实迁移能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常使用基于进度奖励的单智能体训练，在复杂环境（如存在障碍物）中表现有限，且仿真到现实迁移效果不佳。本研究探索通过多智能体竞争来促进高级飞行技能和策略的自然涌现。

Method: 采用多智能体竞争框架，使用稀疏的高层目标（赢得比赛）作为奖励信号，通过强化学习训练无人机智能体。对比了多智能体竞争方法与基于进度奖励的单智能体训练方法，在仿真和真实世界中进行验证。

Result: 多智能体竞争方法在复杂环境中（如存在障碍物）表现优于基于进度奖励的单智能体方法。多智能体策略具有更好的仿真到现实迁移能力，即使使用相同的仿真环境、随机化策略和硬件。此外，多智能体策略还展现出对未见过对手的泛化能力。

Conclusion: 稀疏任务级奖励足以训练出能够在物理世界中执行高级低级控制的智能体。多智能体竞争不仅促进了敏捷飞行技能和策略的自然涌现，还显著提高了仿真到现实的迁移能力，为物理世界中的强化学习应用提供了新思路。
...
TL;DR: AnchorDream：一种基于预训练视频扩散模型的机器人数据合成方法，通过将扩散过程锚定在机器人运动渲染上，从少量人类遥操作演示扩展生成大规模、多样化、高质量的数据集，显著提升下游策略学习性能。


<details>
  <summary>Details</summary>
Motivation: 机器人演示数据的大规模收集是模仿学习的主要瓶颈，真实世界数据获取成本高，仿真器提供的多样性和保真度有限且存在明显的仿真到真实差距。现有生成方法要么只改变视觉外观而不创建新行为，要么存在本体不一致导致运动不自然的问题。

Method: 提出AnchorDream，一种本体感知的世界模型，重新利用预训练的视频扩散模型进行机器人数据合成。该方法将扩散过程锚定在机器人运动渲染上，防止幻觉生成，同时合成与机器人运动学一致的对象和环境。仅需少量人类遥操作演示即可扩展为大规模数据集，无需显式环境建模。

Result: 实验表明，生成的数据在下游策略学习中带来持续改进：在仿真基准测试中相对增益达36.4%，在真实世界研究中性能提升近一倍。这些结果表明，将生成世界模型锚定在机器人运动上为扩展模仿学习提供了实用路径。

Conclusion: AnchorDream通过将扩散过程锚定在机器人运动渲染上，解决了现有生成方法的本体不一致问题，能够从少量演示扩展生成高质量、多样化的机器人数据集，显著提升模仿学习性能，为机器人数据合成提供了有效解决方案。
