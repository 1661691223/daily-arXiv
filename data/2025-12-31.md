...
TL;DR: 本文提出了一种AI增强的局部线性嵌入(LLE)模型，用于改进医疗账单和转录服务的高维数据处理，通过自动化减少人为错误，提高操作效率。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗账单和转录等流程提供了机会。当前医疗数据处理存在高维度、准确性不足和效率低下等问题，需要创新的AI解决方案来优化这些关键医疗流程。

Method: 开发了一种AI增强的局部线性嵌入(LLE)模型，专门用于处理高维医疗数据。该方法将AI技术与LLE降维技术相结合，构建了完整的数学模型，并通过一系列实验验证其在真实医疗场景中的应用效果。

Result: 实验结果显示，该AI增强LLE模型在数据处理准确性和操作效率方面都有显著提升，能够更好地支持医疗账单系统和转录服务的自动化处理。

Conclusion: AI增强LLE模型在医疗数据分析中展现出巨大潜力，不仅改进了医疗账单和转录流程，还为未来更广泛的医疗应用研究奠定了基础。
...
TL;DR: VISTA是一个模块化视觉语言框架，通过信息瓶颈将感知与推理解耦，使用冻结的VLM传感器进行客观感知查询，文本LLM进行推理分解和规划，显著提升了对虚假相关性的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时常常利用虚假相关性而非因果视觉证据，微调后更容易产生捷径依赖，需要更鲁棒的视觉推理方法。

Method: 提出VISTA框架：1）冻结VLM传感器仅进行简短客观的感知查询；2）纯文本LLM推理器分解问题、规划查询、聚合视觉事实；3）通过强化学习（GRPO）在641个多步问题上训练无偏视觉推理。

Result: 在SpuriVerse上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集保持竞争力，能跨VLM传感器迁移并识别感知失败。

Conclusion: VISTA通过感知-推理解耦和强化学习训练，有效减少对虚假相关性的依赖，产生更中立、更基于视觉证据的推理轨迹，为鲁棒视觉推理提供了模块化解决方案。
...
TL;DR: SAMM2D双编码器框架在动脉瘤检测中取得突破，发现强预训练下数据增强反而降低性能，挑战了医学影像中"更多增强总是更好"的假设。
...
Method: 提出SAMM2D双编码器框架，在RSNA颅内动脉瘤数据集上进行实验，系统比较六种数据增强方案与无增强基线的性能差异。

Result: AUC达到0.686，比临床基线提升32%；无增强模型优于所有增强变体1.75-2.23个百分点；校准阈值后灵敏度达95%，超过平均放射科医生水平。

Conclusion: 强预训练特征已捕获稳健不变性，额外增强既冗余又破坏特征流形；未来医学影像工作流应更注重强预训练而非复杂增强管道。
...
TL;DR: HookMIL是一个用于计算病理学中全切片图像分析的多实例学习框架，通过可学习的hook tokens实现结构化上下文聚合，具有线性复杂度，支持多模态初始化，并在四个公开数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法会丢失关键上下文信息，而基于transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题。需要开发一种既保持上下文感知又计算高效的MIL框架。

Method: 提出HookMIL框架，使用紧凑的可学习hook tokens进行结构化上下文聚合。支持三种初始化方式：关键补丁视觉特征、视觉-语言病理模型的文本嵌入、空间转录组-视觉模型的空间基础特征。通过双向注意力机制实现线性复杂度交互，引入Hook多样性损失促进专业化，并采用hook-to-hook通信机制优化上下文交互。

Result: 在四个公开病理数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL是一个上下文感知且计算高效的MIL框架，通过多模态初始化的hook tokens有效聚合上下文信息，在计算病理学任务中表现出优越性能。
...
TL;DR: Tiny-YOLOSAM：结合YOLO检测器和TinySAM的混合系统，通过YOLO生成框提示和稀疏点提示替代密集的"分割一切"模式，大幅提升分割覆盖率和运行速度。
...
TL;DR: 提出一种基于视觉语言模型的多模态可解释性方法，通过分析视网膜象限中的病灶分布来模拟眼科医生的诊断推理过程，为糖尿病视网膜病变提供更全面的解释。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。当前AI模型依赖病灶分割进行解释，但手动标注对临床医生不实用，且现有模型多为单模态，解释效果有限。医生需要能解释分类推理的模型，而不仅仅是突出病灶位置。

Method: 采用视觉语言模型结合少样本学习，分析视网膜象限中的病灶分布来模拟眼科医生的推理。模型生成配对的Grad-CAM热图，展示OCT和眼底图像中各个神经元的权重，可视化显示对DR严重程度分类有贡献的区域。

Result: 使用包含3,000张眼底图像和1,000张OCT图像的数据集，该方法能够解决当前DR诊断中的关键限制，为改善患者预后提供实用且全面的工具。

Conclusion: 提出的多模态可解释性模型通过模拟眼科医生的推理过程，为糖尿病视网膜病变的诊断提供了更全面、实用的解释工具，有望在筛查、治疗和研究环境中得到广泛应用。
...
TL;DR: VLM-PAR是一个基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨注意力融合优化视觉特征，在行人属性识别任务中显著提升了性能，特别是在高度不平衡的PA100K数据集上取得了新的最佳结果。
...
Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的跨注意力融合对齐图像和提示嵌入，优化视觉特征表示。

Result: 在高度不平衡的PA100K基准测试中实现了显著准确率提升，达到新的最佳性能；同时在PETA和Market-1501基准测试中也获得了显著的均值准确率提升。
...
TL;DR: ViSignVQA是首个大规模越南语招牌视觉问答数据集，包含10,762张图片和25,573个问答对，用于解决低资源语言场景文本理解问题。


<details>
  <summary>Details</summary>
Motivation: 自然场景中的招牌文本理解对视觉问答的实际应用至关重要，但在低资源语言（如越南语）中尚未得到充分探索。现有VQA数据集缺乏针对越南语招牌的专门资源，这些招牌具有双语文本、非正式表达和独特视觉特征。

Method: 1) 创建ViSignVQA数据集，捕捉越南招牌的语言、文化和视觉特征；2) 集成越南语OCR模型(SwinTextSpotter)和预训练语言模型(ViT5)来适配SOTA VQA模型；3) 提出多智能体VQA框架，结合感知和推理智能体与GPT-4，采用多数投票策略。

Result: OCR增强上下文显著提升性能，将OCR文本附加到问题后F1分数提升高达209%。多智能体框架通过多数投票达到75.98%准确率。实验表明领域特定资源对低资源语言文本VQA的重要性。

Conclusion: ViSignVQA作为首个越南语招牌理解的大规模多模态数据集，为OCR集成VQA模型的开发和评估提供了基准，强调了领域特定资源在增强低资源语言文本VQA中的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 家庭机器人寻找隐藏或部分遮挡物体时效率低下，传统方法需要多次尝试。作者希望开发一种能够快速定位隐藏物体的系统，减少搜索时间和精力。

Method: 利用2D视觉语言模型的激活图作为"抽象对象"表示，结合历史数据学习物体常见放置位置，通过语义抽象框架进行3D定位和补全，实现高效的无结构化搜索。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试成功率显著提高，搜索速度明显快于随机搜索方法。

Conclusion: 语义抽象框架扩展了隐藏物体定位能力，为家庭机器人提供了更高效的寻找丢失物品技能，有望节省时间和精力。
...
TL;DR: VideoScaffold是一个用于流媒体视频理解的动态表示框架，通过自适应调整事件粒度来提升多模态大语言模型对长视频的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有静态策略（如稀疏采样、帧压缩、聚类）在处理连续视频流时存在局限性，会产生碎片化或过度压缩的输出，无法有效处理帧间冗余和保持时间连贯性。
...
Conclusion: VideoScaffold通过动态调整事件粒度，有效解决了长视频理解中的冗余和连贯性问题，为流媒体视频理解提供了高效灵活的解决方案。
...
TL;DR: 该论文提出了一种KAN增强的FPN-Stem架构，用于改进ViT在姿态估计等密集预测任务中的性能。通过用KAN卷积层替换传统FPN中的线性平滑卷积，显著提升了特征融合质量，在COCO数据集上实现了+2.0 AP的性能提升。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在密集预测任务中表现良好，但现有模型如ViTPose的前端设计过于简单。传统的patch划分机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失，这限制了ViT的性能提升。

Method: 提出KAN增强的FPN-Stem架构：1）通过消融研究发现性能瓶颈在于FPN中的后融合非线性平滑步骤；2）保留经典的"上采样-相加"融合流程；3）将FPN末端的标准线性3x3平滑卷积替换为基于KAN的卷积层，利用其强大的非线性建模能力自适应学习和修正多尺度融合过程中产生的"伪影"。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了高达+2.0 AP的显著性能提升。该方法不仅提供了即插即用的高性能模块，更重要的是揭示了ViT前端的性能瓶颈通常不在于"特征精炼"，而在于"特征融合"的质量。

Conclusion: 该工作通过引入KAN算子有效解决了ViT前端的特征融合瓶颈问题。KAN-FPN-Stem架构不仅显著提升了姿态估计性能，更重要的是为理解ViT性能瓶颈提供了新视角：特征融合质量比特征精炼更为关键，这为未来ViT架构设计提供了重要指导。
...
TL;DR: MiG-DM：一种元信息引导的跨域协同扩散模型，通过整合跨模态先验和投影域物理知识，提升低剂量PET成像质量
...
Method: 提出元信息引导的跨域协同扩散模型（MiG-DM）：1）元信息编码模块将临床参数转换为语义提示，考虑患者特征、剂量相关信息和半定量参数，实现文本元信息与图像重建的跨模态对齐；2）跨域架构结合投影域和图像域处理，在投影域中通过卷积操作捕获全局物理结构。

Result: 在UDPET公共数据集和不同剂量水平的临床数据集上的实验表明，MiG-DM在提升PET图像质量和保留生理细节方面优于现有最先进方法。

Conclusion: MiG-DM通过整合跨模态先验和投影域物理知识，有效解决了低剂量PET成像中的关键挑战，为高质量PET图像生成提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下效果差，热成像系统缺乏颜色和纹理信息，需要融合两种模态来克服各自局限

Method: 训练33个YOLO模型，使用22,000+标注图像，在11种RGB-LWIR融合比例（100/0到0/100，10%递增）上进行多模态融合，根据光照条件动态选择最优检测模型

Result: 最佳全光模型（80/20融合）和微光模型（90/10融合）分别达到92.8%和92.0%平均置信度，显著优于YOLOv5n和YOLOv11n基线；无光条件下最佳40/60融合达到71.0%
...
Result: 实验表明，流模型倾向于在没有持续引导的情况下回到默认轨迹。实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。与无引导基线相比，引导生成将语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地指导大型生成系统，实现更强的几何控制，拓宽文本到图像合成的创意范围。这种方法无需专门训练，通过外部引导信号即可引入几何理解能力。
...
TL;DR: GeCo是一种几何基础的一致性度量方法，用于检测静态场景中的几何变形和遮挡不一致伪影，通过融合残差运动和深度先验生成可解释的稠密一致性图，用于基准测试视频生成模型和减少变形伪影。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在静态场景中经常产生几何变形和遮挡不一致的伪影，需要一种系统性的方法来检测和量化这些缺陷，以评估模型性能和指导改进。

Method: GeCo通过融合残差运动和深度先验，生成稠密的一致性图来检测几何变形和遮挡不一致伪影。该方法将几何基础的一致性度量应用于视频生成模型的基准测试，并作为无训练指导损失来减少变形伪影。

Result: GeCo能够有效检测视频生成模型中的几何变形和遮挡不一致伪影，系统性地评估了近期视频生成模型的常见失败模式，并成功作为无训练指导损失减少了视频生成过程中的变形伪影。

Conclusion: GeCo提供了一种有效的几何基础一致性度量方法，不仅能系统评估视频生成模型的性能，还能作为无训练指导损失来改善视频生成质量，减少几何变形和遮挡不一致伪影。
...
TL;DR: 该研究开发了B&J基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现良好（>90%），但在需要多模态整合的开放式任务中表现显著下降（仅60%），表明当前AI模型尚不具备复杂的临床多模态推理能力。
...
Method: 开发了B&J基准测试框架，包含1,245个来自骨科和运动医学真实患者案例的问题，评估模型在7个临床推理任务上的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，并与专家标注的基准真值进行比较。
...
Conclusion: 当前AI模型尚不具备复杂的多模态临床推理能力，其安全部署应限于支持性的文本角色。未来在核心临床任务上的进展需要多模态整合和视觉理解方面的根本性突破。
...
TL;DR: Fetal-Gauge是首个专门评估视觉语言模型在胎儿超声成像中性能的基准测试，包含42,000多张图像和93,000个问答对，涵盖多种临床任务，揭示了当前模型性能远低于临床需求。


<details>
  <summary>Details</summary>
Motivation: 全球训练有素的超声医师短缺阻碍了胎儿健康监测，深度学习有潜力提高超声医师效率并支持新从业者培训。视觉语言模型特别适合超声解读，但目前缺乏标准化基准来评估其在胎儿超声成像中的性能。
...
Result: 当前视觉语言模型在胎儿超声解读中表现不佳，最佳模型仅达到55%准确率，远低于临床需求。分析揭示了当前模型在胎儿超声解释中的关键局限性。

Conclusion: Fetal-Gauge为推进产前护理中的多模态深度学习建立了严格基础，强调了开发领域适应架构和专门训练方法的迫切需求，为解决全球医疗可及性挑战提供了途径。
...
<details>
  <summary>Details</summary>
Motivation: 需要解决文本、3D模型和图像模态之间的结构化对齐问题，以实现高质量的3D检索和可控的4D生成，推动动态多模态理解和实际应用。
...
Conclusion: Uni4D框架通过结构化三级跨模态对齐，成功实现了大规模开放词汇3D检索和可控4D生成，为动态多模态理解和应用提供了有效解决方案。
...
### [19] [VideoZoomer: Reinforcement-Learned Temporal Focusing for Long Video Reasoning](https://arxiv.org/abs/2512.22315)
*Yang Ding,Yizhen Zhang,Xin Lai,Ruihang Chu,Yujiu Yang*

Main category: cs.CV

TL;DR: VideoZoomer：一种基于多模态大语言模型的智能体框架，通过动态控制视觉焦点实现长视频理解，采用两阶段训练策略，在多种基准测试中表现优异


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉语言任务上取得显著进展，但在长视频理解方面受限于有限的上下文窗口。现有方法通常依赖均匀帧采样或静态预选，可能忽略关键证据且无法在推理过程中修正初始选择错误

Method: 提出VideoZoomer框架，使MLLMs能够在推理过程中动态控制视觉焦点。从低帧率的粗略概览开始，调用时间缩放工具在自主选择的时刻获取高帧率片段，以多轮交互方式逐步收集细粒度证据。采用两阶段训练策略：首先在精心策划的蒸馏示例和反思轨迹数据集上进行监督微调，然后通过强化学习进一步优化智能体策略

Result: 7B模型展现出多样且复杂的推理模式，在广泛的长视频理解和推理基准测试中表现强劲。能够持续超越现有开源模型，在具有挑战性的任务中甚至能与专有系统相媲美，同时在减少帧预算的情况下实现更高的效率

Conclusion: VideoZoomer通过动态视觉焦点控制和智能体交互机制，有效解决了长视频理解中的上下文限制问题，为MLLMs在复杂视频分析任务中的应用提供了新思路
...
TL;DR: SpotEdit：一种无需训练的图像编辑框架，通过选择性更新修改区域来减少冗余计算，提高编辑效率


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时对所有token进行统一处理和去噪，即使只修改小区域也进行全图计算，导致冗余计算并可能破坏未修改区域的质量

Method: SpotEdit包含两个核心组件：SpotSelector通过感知相似性识别稳定区域并复用条件图像特征跳过计算；SpotFusion通过动态融合机制自适应地混合这些特征与编辑后的token，保持上下文连贯性和编辑质量

Result: 通过减少不必要的计算并保持未修改区域的高保真度，SpotEdit实现了高效精确的图像编辑

Conclusion: SpotEdit证明了在图像编辑中无需重新生成每个区域，通过选择性更新修改区域可以实现更高效的编辑，同时保持编辑质量和上下文一致性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注正向建模（如从文本到运动的整体映射），而本文考虑逆向视角：将整体运动分解为语义上有意义的子组件。人类运动具有组合性，复杂行为可以描述为简单基元的组合，但缺乏有效的分解学习方法。
...
Result: 该方法能够从复杂运动序列中解耦可重用的运动基元，分解的运动概念可以灵活重组生成多样新颖的运动，能够泛化到训练分布之外。构建的数据集为文本到运动生成和运动组合提供了扩展资源。

Conclusion: DeMoGen提供了一个有效的运动分解学习框架，实现了从整体运动到语义子组件的逆向分解，支持运动概念的发现、解耦和重组，为运动生成和理解提供了新的视角和方法。
...
TL;DR: 提出基于变分自编码器的多视图潜在表示学习框架，从T1Gd和FLAIR MRI中整合互补的影像组学特征，用于GBM中MGMT启动子甲基化状态的分类预测。


<details>
  <summary>Details</summary>
Motivation: 传统单模态和早期融合方法在胶质母细胞瘤MGMT启动子甲基化预测中存在特征冗余高、模态特异性信息建模不完整的问题，需要更有效的多模态整合方法。
...
Conclusion: 提出的多视图潜在表示学习框架为影像基因组学中分子肿瘤特征的非侵入性推断提供了更有效的解决方案，特别是在胶质母细胞瘤的MGMT启动子甲基化预测中具有重要应用价值。
...
TL;DR: 提出基于视觉变换器的端到端管道，联合分析H&E和IHC染色图像，实现HER2状态的像素级评分（0, 1+, 2+, 3+）和阴阳性预测
...
Conclusion: 基于ViT的端到端模型能有效联合评估H&E和IHC图像进行HER2评分，为临床诊断提供自动化像素级分析工具
...
TL;DR: 本文提出了一种基于多模态大语言模型的3D场景操作框架，通过引入MCP-based API、专用视觉工具和多智能体协作系统，解决了3D物体布置任务中的三大挑战。
...
Method: 提出了三部分解决方案：1）引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2）通过一套专用视觉工具增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3）提出协作式多智能体框架，包含规划、执行和验证三个角色，以处理多步指令并从中间错误中恢复。

Result: 在25个复杂的物体布置任务上进行了评估，结果显示该方法显著优于现有基线方法。
...
TL;DR: Self-E是一个从零开始训练的文本到图像生成模型，支持任意步数推理，通过自评估机制实现高效少步生成，同时保持高质量多步采样能力。


<details>
  <summary>Details</summary>
Motivation: 传统扩散或流匹配模型依赖局部监督需要多步推理，蒸馏方法需要预训练教师模型，Self-E旨在填补这两种范式之间的空白，实现从零开始训练的高质量任意步数生成。

Method: 结合流匹配模型的数据学习方式，引入新颖的自评估机制：模型使用当前分数估计评估自己生成的样本，作为动态自教师，实现瞬时局部学习和自驱动全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E在少步生成方面表现出色，在50步时与最先进的流匹配模型竞争，性能随推理步数增加单调提升，支持超快速少步生成和高质量长轨迹采样。

Conclusion: Self-E是第一个从零开始、支持任意步数的文本到图像模型，为高效和可扩展的生成提供了统一框架，填补了传统扩散模型和蒸馏方法之间的空白。
...
TL;DR: iOSPointMapper是一款利用iPhone/iPad进行实时人行道测绘的移动应用，通过设备端语义分割、LiDAR深度估计和GPS/IMU融合数据检测交通标志等设施，提供用户验证界面，数据匿名传输至TDEI交通数据交换平台。


<details>
  <summary>Details</summary>
Motivation: 当前人行道数据收集方法成本高、分散且难以扩展，缺乏准确、及时的人行道数据阻碍了无障碍和包容性行人基础设施的建设。

Method: 开发iOSPointMapper移动应用，利用最新iPhone/iPad的设备端语义分割、LiDAR深度估计和融合GPS/IMU数据进行实时人行道测绘，检测交通标志、红绿灯和杆柱等特征，并提供用户引导的标注界面验证系统输出。

Result: 系统特征检测和空间测绘性能的详细评估显示应用在人行道测绘方面具有潜力，收集的数据经过匿名处理后传输至TDEI交通数据交换平台，可与更广泛的多模式交通数据集无缝集成。

Conclusion: iOSPointMapper提供了一种可扩展、以用户为中心的方法，能够填补行人基础设施中的关键数据空白，为改善人行道测绘提供了实用解决方案。
...
TL;DR: DeFloMat是一种基于条件流匹配的新型生成式目标检测框架，通过用确定性流场替代扩散模型的随机去噪过程，在仅需3步推理的情况下实现了43.32% AP的性能，解决了扩散检测器在临床应用中延迟过高的问题。


<details>
  <summary>Details</summary>
Motivation: 解决基于扩散的目标检测方法（如DiffusionDet）在临床应用中存在的关键延迟瓶颈问题。扩散模型虽然准确率高，但需要大量采样步骤（T>60），这对于克罗恩病磁共振肠造影等时间敏感的临床应用来说不切实际。

Method: 提出DeFloMat框架，将检测任务重新定义为条件流匹配问题。使用基于条件最优传输理论推导出的确定性流场来替代扩散模型的随机路径，具体近似修正流。通过简单的常微分方程求解器实现快速推理。

Result: 在具有挑战性的MRE临床数据集上，DeFloMat仅用3步推理就达到了43.32% AP_{10:50}的最先进准确率，比DiffusionDet在4步时的最佳性能（31.03%）提高了1.4倍。确定性流显著改善了定位特性，在少步数情况下实现了更好的召回率和稳定性。

Conclusion: DeFloMat解决了生成式检测的准确性与临床效率之间的权衡问题，为稳定快速的目标定位设定了新标准，特别适用于时间敏感的医疗应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能快速无创地可视化细胞形态，但现有的稳健体积分割通常依赖荧光标记或复杂的后处理。需要开发直接从3D明场图像分割亚细胞结构的方法。
...
Result: 模型能够从明场堆栈中准确分割细胞核、线粒体和其他细胞器，在多个共聚焦数据集上保持跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线。

Conclusion: Bright-4B为大规模无标记3D细胞映射提供了有效解决方案，所有代码、预训练权重和下游微调模型都将发布以推动该领域发展。
...
TL;DR: 本文提出EmoCtrl模型，解决可控情感图像内容生成问题，在保持内容描述准确性的同时表达目标情感，克服现有模型在内容一致性和情感表达之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同传达意义，但现有文本到图像模型确保内容一致性却缺乏情感意识，而情感驱动模型生成情感结果却以内容失真为代价。需要同时实现内容忠实性和情感表达的控制。

Method: 提出EmoCtrl模型，构建包含内容、情感和情感提示的数据集，将抽象情感与视觉线索连接。模型包含文本和视觉情感增强模块，通过描述性语义和感知线索丰富情感表达。学习的情感标记表现出互补效应。

Result: 定量和定性实验表明EmoCtrl在忠实内容和表达情感控制方面优于现有方法。用户研究确认EmoCtrl与人类偏好高度一致。模型在创意应用中泛化良好，证明学习的情感标记具有鲁棒性和适应性。

Conclusion: EmoCtrl成功解决了可控情感图像内容生成问题，实现了内容忠实性和情感表达的平衡，为情感感知的图像生成提供了有效解决方案，并在实际应用中展现出良好的泛化能力。
...
Method: 1. 直接从事件流中检测线条特征；2. 利用事件-线条标定模型生成相机参数的初始估计，适用于平面和非平面线条；3. 采用非线性优化方法细化相机参数。

Result: 通过仿真和真实世界实验验证了方法的可行性和准确性，在单目和双目事件相机上进行了验证，代码已开源。

Conclusion: 提出了一种无需专门标定板、基于环境自然线条的事件相机标定方法，能够满足快速变化场景的需求，具有实用性和准确性。
...
TL;DR: SonoVision是一款帮助视障人士通过声音提示定位日常物品的智能手机应用，使用左右耳耳机中的正弦声音来指示物体方向，完全离线工作。


<details>
  <summary>Details</summary>
Motivation: 视障人士定位物体是一个重大挑战，这阻碍了他们的独立性，可能将他们推向危险情境。为了让视障人士更加自给自足，需要开发能够帮助他们独立定位物体的工具。

Method: 使用Flutter开发平台构建智能手机应用，后端采用Efficientdet-D2模型进行物体检测。通过左右耳耳机中的正弦声音提示物体方向：物体在右侧时右耳响，左侧时左耳响，正前方时双耳同时响。

Result: 开发了完全离线工作的SonoVision应用，能够帮助视障人士通过声音提示定位日常物品，减少对周围人的依赖，提高独立性。

Conclusion: SonoVision应用以安全、用户友好的方式显著帮助视障人士，通过声音提示系统使他们能够更独立地定位物体，应用代码已在GitHub上开源。
...
TL;DR: 本文首次系统评估了通用图像到3D基础模型SAM 3D在单目遥感建筑重建中的表现，发现其在屋顶几何一致性和边界清晰度上优于TRELLIS方法，并展示了其在城市场景重建中的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用图像到3D基础模型SAM 3D在遥感建筑重建中的表现，探索基础模型在城市建模中的应用潜力。

Method: 在NYC Urban Dataset上对SAM 3D与TRELLIS进行基准测试，使用FID和CMMD作为评估指标。进一步通过"分割-重建-组合"流水线将SAM 3D扩展到城市场景重建。

Result: 实验结果表明，SAM 3D相比TRELLIS能产生更一致的屋顶几何形状和更清晰的边界。通过扩展的流水线展示了其在城市场景建模中的潜力，并分析了实际限制。

Conclusion: SAM 3D在单目遥感建筑重建中表现出色，为在城市3D重建中部署基础模型提供了实用指导，并激励未来集成场景级结构先验的研究方向。
...
TL;DR: 本文通过训练和比较YOLOv8、YOLOv11和RF-DETR三种计算机视觉模型，开发了一种自动化识别变电站关键组件的解决方案，以替代传统耗时的人工标注方法。


<details>
  <summary>Details</summary>
Motivation: 变电站作为电网关键基础设施，其组件易受多种灾害影响，故障可能导致重大经济和公共安全后果。传统人工标注变电站基础设施耗时费力，需要开发自动化解决方案。

Method: 使用手动标注的美国变电站图像数据集，训练和比较三种计算机视觉模型（YOLOv8、YOLOv11、RF-DETR），评估它们在检测精度、准确率和效率方面的表现。

Result: 对三种模型的性能进行了比较分析，展示了各自的优势和局限性，并确定了最适合大规模变电站组件映射的可靠模型。成功应用这些模型对美国变电站组件进行了有效映射。

Conclusion: 机器学习方法能够实现高效、可靠的变电站组件自动化映射，为关键基础设施的脆弱性评估提供了实用工具，展示了计算机视觉在变电站映射中的实际应用价值。
...
TL;DR: PGR²M提出了一种混合表示方法，通过残差向量量化增强可解释的姿态编码，改善文本驱动的3D运动生成和编辑质量。
...
Method: 提出PGR²M混合表示：1）姿态引导的RVQ分词器将运动分解为编码全局粗结构的姿态潜在变量和编码细粒度时间变化的残差潜在变量；2）残差丢弃防止过度依赖残差；3）基础Transformer自回归预测姿态编码，精炼Transformer预测残差编码。
...
TL;DR: 提出了一种基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析实现冲击波前缘检测，最终实现3D重建和爆炸当量反演，测量误差最小0.06%，最大5.20%。


<details>
  <summary>Details</summary>
Motivation: 冲击波运动参数的高时空分辨率精确测量在动力场测试和损伤评估中至关重要，但冲击波的快速、不均匀传播和不稳定的测试条件带来了重大挑战。
...
Conclusion: 该方法利用事件相机的高速和高动态范围能力，成功解决了冲击波测量的挑战，实现了高精度的冲击波运动参数测量，代表了该领域的重要进展。
...
### [36] [MUSON: A Reasoning-oriented Multimodal Dataset for Socially Compliant Navigation in Urban Environments](https://arxiv.org/abs/2512.22867)
...
<details>
  <summary>Details</summary>
Motivation: 现有社交导航数据集缺乏显式推理监督，且动作分布呈现高度长尾，限制了模型学习安全关键行为的能力。需要构建一个包含结构化推理标注的数据集来提升社交导航模型的性能。

Method: 构建MUSON多模态数据集，包含感知、预测、推理、动作和解释五个步骤的思维链标注，明确建模静态物理约束，并采用理性平衡的离散动作空间。数据集涵盖多样化的室内外校园场景。

Result: 在MUSON上对多个最先进的小型视觉语言模型进行基准测试，Qwen2.5-VL-3B模型取得了最高的决策准确率0.8625。与SNEI相比，MUSON提供了更一致的推理、动作和解释。

Conclusion: MUSON是一个有效且可复用的社交合规导航基准数据集，通过结构化推理标注解决了现有数据集的局限性，能够提升社交导航模型的安全性和可解释性。
...
TL;DR: 提出用于红外小目标检测的两阶段半监督范式，通过分层MoE适配器解决SAM模型在红外领域的域差距、物理先验编码和架构复杂性问题，实现用10%标注数据达到全监督性能


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测标注成本高，现有方法如SAM存在域差距、无法编码物理先验、架构复杂等问题，需要有效的半监督范式解决数据稀缺问题

Method: 设计分层MoE适配器（包含四个白盒神经算子），采用两阶段知识蒸馏与迁移：1) 先验引导知识蒸馏：用MoE适配器和10%全监督数据将SAM蒸馏为专家教师模型Scalpel-SAM；2) 部署导向知识迁移：用Scalpel-SAM生成伪标签训练轻量下游模型

Result: 实验表明，在最小标注情况下，该范式使下游模型性能达到甚至超过全监督对应模型，是首个系统解决IR-SOT数据稀缺问题的半监督范式

Conclusion: 提出的两阶段半监督范式有效解决了红外小目标检测中的数据稀缺问题，通过分层MoE适配器和知识蒸馏机制，实现了用少量标注达到全监督性能的目标
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM在推理过程中存在步骤间连贯性弱、视觉基础性不足的问题，主要是因为现有对齐方法只监督最终答案而忽略了中间推理过程的可靠性。

Method: 提出SR-MCR框架，整合五个自参考线索（语义对齐、词汇保真度、非冗余性、视觉基础性、步骤一致性）构建归一化的可靠性加权奖励，提供细粒度的过程级指导。采用无批评者的GRPO目标，结合置信感知冷却机制稳定训练。

Result: 基于Qwen2.5-VL构建的SR-MCR-7B在多个视觉基准测试中提升了答案准确性和推理连贯性，在同类规模的开源模型中达到81.4%的平均准确率，处于最先进水平。
...
Method: 1) 提出预测驱动的Top-K Jaccard相似度系数，提高邻居节点的纯度；2) 开发基于Transformer的预测模型，分析中心节点与Top-K附近邻居节点的关系；3) 提出稀疏差分Transformer(SDT)替代原始Transformer，消除噪声并增强抗噪能力。
...
<details>
  <summary>Details</summary>
Motivation: 自回归大视觉语言模型在复杂视觉规划和动态机器人控制中的序列生成方式存在局限性，需要探索基于扩散大语言模型的替代方案来克服这些限制。

Method: 构建基于扩散大语言模型的视觉语言模型Dream-VL，并在此基础上通过连续预训练开发视觉语言动作模型Dream-VLA，利用扩散模型的双向特性进行动作分块和并行生成。

Result: Dream-VL在多个基准测试中与顶级AR-based VLMs相当，在视觉规划任务中表现更优；Dream-VLA在LIBERO上达到97.2%平均成功率，在SimplerEnv-Bridge和Fractal上分别达到71.4%和60.5%的总体平均，超越π₀和GR00T-N1等领先模型。

Conclusion: 基于扩散的视觉语言模型在视觉规划和机器人控制任务中具有优势，扩散模型的双向特性使其更适合动作分块和并行生成，能显著加速下游微调的收敛速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型方法主要是前向预测的，仅基于初始观察生成轨迹，缺乏明确的目标建模，导致空间漂移和目标不对齐问题。需要一种能够生成物理合理且目标一致的视觉规划方法。

Method: Envision采用两阶段框架：1) 目标图像模型识别任务相关区域，执行区域感知交叉注意力，合成捕捉期望结果的目标图像；2) 环境-目标视频模型基于首尾帧条件视频扩散模型，在初始观察和目标图像之间插值，生成平滑且物理合理的视频轨迹。
...
TL;DR: 该研究提出了一种用于图像超分辨率任务的细粒度感知奖励模型（FinPercep-RM）和协同进化课程学习机制，以解决传统图像质量评估模型对局部失真不敏感导致的奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估模型通常只输出单一全局分数，对局部和细粒度失真极不敏感，这导致超分辨率模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量不一致。

Method: 1. 提出基于编码器-解码器架构的细粒度感知奖励模型，不仅提供全局质量分数，还生成感知退化图来空间定位和量化局部缺陷；2. 构建FGR-30k数据集训练该模型；3. 提出协同进化课程学习机制，奖励模型和超分辨率模型同步进行课程学习，奖励模型复杂度逐步增加，超分辨率模型从简单全局奖励开始，逐步过渡到复杂模型输出。

Result: 实验验证了该方法在基于人类反馈的强化学习方法中，对超分辨率模型的全局质量和局部真实感都有效。

Conclusion: 通过细粒度感知奖励模型和协同进化课程学习机制，成功解决了传统图像质量评估模型在超分辨率任务中的局限性，实现了更稳定、更符合人类感知的训练过程。
...
Result: 在受限训练条件下室内基准测试达到最先进性能，在室外数据集上表现强劲，自回归先验成为深度估计中互补的几何感知生成模型家族

Conclusion: 自回归先验是深度估计中有效的几何感知生成模型，具有数据可扩展性和任务适应性优势，为3D视觉任务提供了扩散模型之外的替代方案
...
TL;DR: CLAdapter是一种新颖的聚类注意力适配器，通过注意力机制和聚类中心来优化从大规模数据中学到的丰富表示，并将其适配到数据有限的下游任务中。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机视觉领域受益于大规模数据集和预训练模型，但在专业领域和数据有限的科学领域中，许多下游任务仍然面临挑战。需要一种方法能够将大规模数据学到的丰富表示有效地适配到各种数据有限的下游任务。

Method: CLAdapter引入注意力机制和聚类中心，通过分布相关性和变换矩阵来个性化增强转换特征。统一接口设计使其能够与多种模型架构（包括CNN和Transformer）在2D和3D场景中无缝集成。

Result: 在10个数据集上进行广泛实验，涵盖通用、多媒体、生物、医学、工业、农业、环境、地理、材料科学、OOD和3D分析等多个领域，CLAdapter在数据有限的科学领域中实现了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，能够在各种数据有限的下游任务中实现卓越性能，展示了其在专业科学领域中的有效性和通用性。
...
Method: 提出INTERACT-CMIL框架，采用多头部深度学习架构，通过共享特征学习与组合部分监督以及跨任务一致性的相互依赖损失，联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞学异型性五个组织病理学轴。

Result: 在三个大学医院新整理的486个专家标注结膜活检切片数据集上，INTERACT-CMIL相比CNN和基础模型基线获得一致改进，相对宏观F1分数提升最高达55.1%（WHO4）和25.0%（垂直扩散）。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型虽然视觉保真度高，但在复杂提示词的语义对齐方面存在困难，需要改进文本到图像的一致性和细节质量。

Method: 1. CritiCore模块：利用视觉语言模型和多个大语言模型丰富提示词上下文，产生高级语义反馈，引导扩散过程更好地与提示意图对齐；2. SpecFusion：在频域合并中间生成状态，注入粗粒度结构信息同时保留高频细节；3. 作为插件式细化阶段，兼容现有扩散模型主干，无需额外训练。

Result: 在标准基准测试中，该方法显著提升了文本到图像对应关系的人类对齐指标和视觉质量，在人类偏好评分和美学评估方面持续提升性能，达到与最先进的奖励优化方法相当的结果。

Conclusion: CritiFusion通过语义批判和频谱对齐策略，在细节、真实性和提示词保真度方面表现出色，证明了该方法的有效性。
...
TL;DR: 提出ARFM（自回归流匹配）方法，用于序列连续数据的概率建模，在多样化视频数据集上训练，能够预测长时程的未来点轨迹位置，并在人机运动预测任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常针对特定分布训练，泛化能力有限；同时，大规模视频预测模型虽然视觉效果逼真，但在复杂运动建模方面仍有不足。需要开发能够准确预测复杂运动且具有良好泛化能力的模型。

Method: 提出自回归流匹配（ARFM）方法，这是一种用于序列连续数据的概率建模技术。模型在多样化视频数据集上进行训练，能够生成未来点轨迹位置的长时程预测。该方法还支持将预测的未来轨迹作为条件输入，用于机器人动作预测和人体运动预测等下游任务。

Result: ARFM模型能够准确预测复杂运动，在专门开发的人体和机器人运动预测基准测试中表现优异。实验表明，将预测的未来轨迹作为条件输入，能够显著提升下游任务（机器人动作预测和人体运动预测）的性能。

Conclusion: ARFM方法在运动预测领域取得了显著进展，能够准确建模复杂运动并提升下游任务性能。该方法具有较好的泛化能力，代码和模型已开源，为相关研究提供了有价值的工具。
...
TL;DR: 提出SCPainter统一框架，结合3D高斯溅射资产表示与扩散模型，实现自动驾驶场景中的3D资产插入和新视角合成，提升训练数据多样性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要3D资产插入和新视角合成来增强训练数据多样性，但现有方法将这两项能力孤立处理，缺乏统一的仿真框架来同时实现逼真的资产插入和场景交互。

Method: 提出SCPainter框架：集成3D高斯溅射(GS)汽车资产表示和3D场景点云，通过扩散模型进行条件生成。将3D GS资产和场景点云共同投影到新视角，用这些投影条件化扩散模型生成高质量图像。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现3D资产插入和新视角合成，有助于创建多样且逼真的驾驶数据。
...
TL;DR: 本研究评估了超声特异性自监督预训练（USF-MAE）在早孕期超声图像中检测囊性水囊瘤的效能，相比传统监督学习方法DenseNet-169，在所有评估指标上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 囊性水囊瘤是高风险产前超声发现，与染色体异常、结构畸形和不良妊娠结局相关。自动化检测可提高可重复性并支持规模化早期筛查，但传统监督深度学习方法受限于小规模标注数据集。

Method: 使用基于掩码自编码（MAE）的超声自监督基础模型（USF-MAE），该模型在超过37万张未标注超声图像上预训练，然后针对囊性水囊瘤与正常对照的二分类任务进行微调。使用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议进行评估。
...
Conclusion: 超声特异性自监督预训练能够促进准确、稳健的深度学习检测囊性水囊瘤，为规模化早期筛查提供了有前景的解决方案，特别是在标注数据有限的情况下。
...
TL;DR: 本文提出Freetime FeatureGS方法，通过可学习的特征高斯基元和流式特征学习策略，从单帧分割图重建分解的4D场景，无需视频分割


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖视频分割结果通过可微分渲染技术提升到4D表示，但视频分割结果往往不稳定，导致重建结果不可靠。需要克服对视频分割的依赖

Method: 使用Freetime FeatureGS表示分解的4D场景，将动态场景建模为具有可学习特征和线性运动能力的高斯基元集合。应用对比损失使基元特征根据其在2D分割图中的实例归属而接近或远离。通过时间有序采样实现流式特征学习，避免优化过程中的局部最小值

Result: 在多个数据集上的实验结果表明，该方法的重建质量大幅优于现有方法

Conclusion: 提出的Freetime FeatureGS方法通过流式特征学习策略，能够从单帧分割图准确恢复分解的4D场景，无需依赖不稳定的视频分割结果，显著提升了重建质量
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时，将图像编码为token序列并与文本token拼接处理，但视觉token数量增加会大幅提高推理成本。现有视觉token剪枝方法往往忽略了长上下文多图像场景的挑战。

Method: 提出两阶段自适应剪枝方法：1）图像内阶段：为每张图像分配内容感知的token预算，贪婪选择最具代表性的token；2）图像间阶段：进行全局多样性过滤形成候选池，然后应用帕累托选择平衡多样性与文本对齐。

Result: 大量实验表明，该方法在长上下文设置下保持强大性能的同时，显著减少了视觉token的数量。

Conclusion: 该方法有效解决了长上下文多图像场景中视觉token剪枝的挑战，通过自适应预算分配和多样性平衡策略，在降低计算成本的同时保持了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有令牌合并和剪枝策略往往忽视空间连续性和邻居关系，导致局部上下文信息丢失，限制了ViTs的计算效率
...
Conclusion: 这项工作强调了空间连续性和邻居结构的重要性，为ViTs架构优化提供了新的见解
...
TL;DR: 提出基于算术分级响应模型（Arithmetic GRM）的质量分级模块（AGQG），解决AGIQA中文本-图像语义漂移问题，提升图像质量评估性能


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多级质量描述文本嵌入的相似性来评估质量，但发现相似性分布常呈现多模态模式（如一个图像嵌入同时与"优秀"和"差"描述高度相似），这种"语义漂移"现象削弱了文本-图像共享空间学习的可靠性

Method: 受心理测量学启发，提出改进的分级响应模型（GRM），设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度级别；通过算术方式建模难度生成，确保难度级别的单调性，强制产生单峰且可解释的质量分布

Result: 提出的算术GRM质量分级（AGQG）模块具有即插即用优势，集成到各种最先进的AGIQA框架中能持续提升性能；在自然图像和屏幕内容图像质量评估中都表现出良好的泛化能力
...
Method: 提出EPD-Solver，利用采样轨迹主要位于低维流形的几何洞察，通过向量值函数的中值定理更准确近似积分解。采用两阶段优化：首先通过蒸馏方法优化可学习参数，然后提出参数高效的强化学习微调方案，将求解器重构为随机Dirichlet策略。

Result: EPD-Solver通过并行梯度评估有效减少截断误差，保持低延迟采样特性。强化学习微调在复杂文本到图像生成任务中有效缓解奖励攻击，提升性能。方法灵活，可作为EPD-Plugin改进现有ODE采样器。

Conclusion: EPD-Solver通过几何洞察和并行梯度评估解决了扩散模型加速中的截断误差问题，结合两阶段优化策略在保持低延迟的同时提升生成质量，为扩散模型高效采样提供了有效解决方案。
...
Result: 实验表明该方法在挑战性场景下显著提升了跟踪稳定性和目标区分能力，为MLLMs在视觉跟踪中的应用开辟了新途径

Conclusion: VPTracker通过结合全局搜索的优势和位置感知机制，有效解决了视觉语言跟踪中的干扰问题，展示了多模态大语言模型在视觉跟踪任务中的潜力
...
<details>
  <summary>Details</summary>
Motivation: 人类对第一人称视觉输入表现出适应性和上下文敏感的反应，但现有数据集（如ViMo）存在空间不一致问题（动态动作总是与固定摄像头视频配对），导致从第一人称视频忠实建模这种反应具有挑战性，需要同时满足严格因果生成和精确3D空间对齐的要求。

Method: 首先构建Human Reaction Dataset (HRD)解决数据稀缺和错位问题；然后提出EgoReAct自回归框架：1）通过向量量化变分自编码器将反应动作压缩到紧凑但富有表现力的潜在空间；2）训练生成式预训练Transformer从视觉输入生成反应；3）在生成过程中融入3D动态特征（度量深度和头部动态）以增强空间基础。

Result: 广泛实验表明，EgoReAct相比先前方法实现了显著更高的真实性、空间一致性和生成效率，同时在生成过程中保持严格的因果关系。

Conclusion: EgoReAct是首个能够从第一人称视频流实时生成3D对齐人类反应动作的自回归框架，通过构建HRD数据集和创新的生成架构，有效解决了空间对齐和因果生成的双重挑战，为第一人称视角下的反应建模提供了新方法。
...
TL;DR: SCaR-3D是一个新颖的3D场景变化检测框架，通过基于符号距离的2D差分模块和多视图聚合，从密集视图的预变化图像序列和稀疏视图的后变化图像中识别物体级变化，并支持持续场景重建。
...
Method: 提出SCaR-3D框架，包含基于符号距离的2D差分模块，然后通过投票和剪枝进行多视图聚合，利用3DGS的一致性特性来稳健分离变化前后状态。还开发了持续场景重建策略，选择性地更新动态区域同时保留未变化区域。

Result: 方法在广泛实验中表现出高准确性和效率，优于现有方法。同时贡献了CCS3D合成数据集，支持灵活组合3D变化类型以进行受控评估。

Conclusion: SCaR-3D成功解决了现有3D变化检测方法的空间不一致性和状态分离问题，实现了准确高效的物体级变化检测，并支持持续场景重建，为场景监控和重建提供了有效解决方案。
...
TL;DR: 提出了一种基于两个仿射对应关系和已知垂直方向（来自IMU）来估计相机相对位姿（3自由度）和焦距的新求解器。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶汽车、智能手机和无人机等应用中，相机通常与惯性测量单元（IMU）结合使用。IMU可以提供相机视图的垂直方向，从而将相对位姿从5自由度减少到3自由度。然而，当焦距未知时，如何从少量对应关系中高效估计相对位姿和焦距仍然是一个挑战。

Method: 首先，在已知垂直方向的情况下，从两个仿射对应关系建立约束方程。然后，基于具有非平凡解方程组的性质，推导出仅涉及两个参数（焦距和相对旋转角度）的四个方程。最后，利用多项式特征值方法求解焦距和相对旋转角度问题。

Result: 在合成和真实世界数据集上的评估结果表明，所提出的求解器性能优于现有的最先进求解器。

Conclusion: 本文提出了一种新颖的求解器，能够在已知垂直方向的情况下，仅使用两个仿射对应关系同时估计相机的相对位姿（3自由度）和焦距。该方法在精度和鲁棒性方面优于现有方法，为实际应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人-物交互视频生成在数字人、电子商务、广告和机器人模仿学习中有广泛应用前景。现有方法存在两个关键限制：1)缺乏有效机制将对象的多视角信息注入模型，导致跨视角一致性差；2)严重依赖细粒度的手部网格标注来建模交互遮挡。
...
Conclusion: ByteLoom通过创新的RCM表示和渐进式训练策略，成功解决了人-物交互视频生成中的几何一致性和标注依赖问题，为相关应用提供了有效解决方案。
...
TL;DR: Lamps是一个从多视角学习解剖结构的自监督学习框架，通过利用人体解剖学的一致性、连贯性和层次性作为监督信号，在胸部X光片上预训练，展现出优越的鲁棒性、可迁移性和临床潜力。
...
Method: Lamps框架通过协调利用人体解剖学的一致性、连贯性和层次性作为监督信号，在大规模胸部X光片上进行自监督预训练。

Result: 在10个数据集上的广泛实验表明，与10个基线模型相比，Lamps在微调和涌现特性分析中展现出优越的鲁棒性、可迁移性和临床潜力。

Conclusion: 通过从多视角学习，Lamps为基础模型提供了独特机会，使其能够开发出与人体解剖结构对齐的有意义、鲁棒的表征。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练过程中经常学习到与类别标签虚假相关但与预测任务无关的特征。现有方法通常通过标注潜在的虚假属性或基于某些经验假设（如偏见的简单性）过滤虚假特征来解决这个问题，但由于现实数据中虚假相关性的复杂性和难以捉摸性，这些方法可能无法获得令人满意的性能。

Method: 提出了一种数据导向的去偏方法：1）识别：观察到受虚假特征影响的样本在学习的特征空间中呈现分散分布，从而识别虚假特征的存在；2）中和：基于简单的分组策略中和虚假特征，获得偏置不变表示；3）消除：学习特征变换，通过与该偏置不变表示对齐来消除虚假特征；4）更新：结合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提高了20%以上，显示出显著的去偏效果。

Conclusion: 通过整合识别、中和、消除和更新四个步骤，构建了一个有效的缓解虚假相关性的流程，该方法在多个基准测试中表现出色，为深度学习模型的去偏问题提供了新的解决方案。
...
TL;DR: 该论文提出了M-ErasureBench多模态评估框架，用于系统评估概念擦除方法在不同输入模态下的性能，并开发了IRECE模块来增强推理时的鲁棒性。
...
Method: 1) 提出M-ErasureBench多模态评估框架，在三种输入模态（文本提示、学习嵌入、反转潜在表示）和两种访问设置（白盒、黑盒）下进行系统评估；2) 提出IRECE模块，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜在表示。

Result: 现有方法在文本提示下表现良好，但在学习嵌入和反转潜在表示下基本失效，白盒设置下概念再现率超过90%。IRECE模块显著提升了鲁棒性，在最具挑战性的白盒潜在反转场景下将概念再现率降低了40%，同时保持了视觉质量。
...
TL;DR: 论文提出GPS方法解决扩散模型迭代精炼中CFG引导导致采样路径偏离数据流形的问题，通过流形约束插值替代外推，保证采样路径稳定并提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 基于去噪-反演循环的迭代精炼方法在提升扩散模型质量和控制方面很有效，但当与标准的无分类器引导（CFG）结合时，其效果受到严重限制。研究发现CFG的外推性质会系统性地将采样路径推离数据流形，导致近似误差发散并破坏精炼过程。

Method: 提出引导路径采样（GPS）方法，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。理论证明这种校正能将误差序列从无界放大转变为严格有界，保证稳定性。同时设计了最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的自然生成过程对齐。

Result: 在SDXL和Hunyuan-DiT等现代骨干模型上的大量实验表明，GPS在感知质量和复杂提示遵循方面均优于现有方法。例如，GPS在SDXL上实现了0.79的ImageReward和0.2995的HPS v2分数，同时在GenEval上将整体语义对齐准确率提升至57.45%。

Conclusion: 路径稳定性是有效迭代精炼的前提条件，GPS提供了一个稳健的框架来实现这一目标。该方法通过流形约束插值解决了CFG引导导致的不稳定问题，显著提升了扩散模型的生成质量和语义对齐能力。
...
TL;DR: 提出了一种基于高斯泼溅坐标的哈希网格特征剪枝方法，通过识别并剪除无效特征来减少存储和传输开销，在不影响模型性能的前提下提升率失真性能。


<details>
  <summary>Details</summary>
Motivation: 由于高斯泼溅在3D空间中分布不规则且不均匀，存在大量稀疏区域，导致哈希网格中许多特征无效，造成冗余存储和传输开销。

Method: 基于输入高斯泼溅的坐标识别并剪除哈希网格中的无效特征，只对有效特征进行编码，从而减少哈希网格的存储大小。

Result: 在保持模型性能不变的情况下，该方法相比基线方法平均减少了8%的比特率，提升了率失真性能。

Conclusion: 提出的哈希网格特征剪枝方法能有效减少存储和传输开销，提升高斯泼溅隐式神经场学习的效率。
...
TL;DR: JavisGPT是首个用于联合音频-视频理解和生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练流程在音频-视频理解与生成任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在联合音频-视频理解和生成方面存在局限，特别是在时空同步和复杂场景下的表现不足，需要开发能够同时处理音频和视频内容并保持时间一致性的统一模型。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音频-视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音频-视频微调和大规模指令微调。构建了包含20万+GPT-4o策划对话的JavisInst-Omni数据集。

Result: 在联合音频-视频理解和生成基准测试中，JavisGPT优于现有MLLM，特别是在复杂和时间同步设置下表现突出。

Conclusion: JavisGPT作为首个统一的联合音频-视频理解和生成MLLM，通过创新的架构设计和训练策略，在保持时间一致性的同时实现了高质量的多模态理解与生成能力。
...
TL;DR: ColaVLA是一个统一的视觉-语言-动作框架，通过将推理从文本转移到统一潜在空间，结合分层并行轨迹解码器，解决了VLM规划器在连续控制、延迟和实时部署方面的挑战，在nuScenes基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的自动驾驶规划器面临三个关键挑战：离散文本推理与连续控制之间的不匹配、自回归思维链解码带来的高延迟，以及低效或非因果规划器限制实时部署。需要一种既能保持VLM泛化能力和可解释性，又能实现高效准确轨迹生成的方法。
...
Conclusion: ColaVLA成功地将VLM的推理能力转移到统一潜在空间，结合分层并行解码架构，在保持VLM泛化能力和可解释性的同时，实现了高效、准确和安全的轨迹生成，为自动驾驶规划提供了新的解决方案。
...
Method: 提出CLIP-Joint-Detect框架，通过轻量级并行头将区域或网格特征投影到CLIP嵌入空间，与可学习的类别特定文本嵌入对齐，使用InfoNCE对比损失和辅助交叉熵项，同时优化所有标准检测损失。

Result: 在Pascal VOC 2007+2012（使用Faster R-CNN）和MS COCO 2017（使用YOLOv11）上验证，获得一致且显著的性能提升，同时保持实时推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法适应实时YOLO检测框架。YOLO在增量学习中面临前景-背景混淆、参数干扰和知识蒸馏不对齐三大冲突，导致灾难性遗忘。
...
Result: 在传统和LoCo COCO基准测试中，YOLO-IOD表现出优越性能且遗忘最小。LoCo COCO是更真实的基准，消除了阶段间数据泄漏问题。

Conclusion: YOLO-IOD是首个实时YOLO增量目标检测框架，有效解决了YOLO中的三大知识冲突，在保持实时性的同时显著减轻了灾难性遗忘问题。
...
Method: 提出ReamCamo框架，通过布局控制调节全局图像结构以改善语义一致性，结合统一细粒度文本任务描述和纹理导向背景检索构建多模态条件，共同指导生成过程

Result: 引入背景-前景分布差异度量来量化伪装质量，大量实验和可视化结果证明了该框架的有效性

Conclusion: ReamCamo通过布局控制和多模态条件生成机制，显著提升了伪装图像生成的视觉保真度和真实感，为伪装目标检测提供了更高质量的训练数据
...
<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法依赖预定义的对象查找表来查询视觉语言模型进行对象定位推理，这限制了在未定义或未预见目标场景中的应用。为了解决这一问题，需要开发能够在开放世界场景中工作的框架。

Method: 提出了OpenGround框架，核心是主动认知推理模块。该模块通过认知任务链模拟人类对目标的感知过程，主动推理上下文相关对象，通过动态更新的对象查找表扩展视觉语言模型的认知范围。同时创建了OpenTarget数据集用于评估开放世界场景下的性能。

Result: OpenGround在Nr3D数据集上取得竞争性性能，在ScanRefer数据集上达到最先进水平，在OpenTarget数据集上实现了17.6%的显著提升。

Conclusion: OpenGround通过主动认知推理模块克服了预定义对象查找表的限制，能够在预定义和开放世界类别下工作，为3D视觉定位提供了更灵活和通用的解决方案。
...
TL;DR: PoseStreamer：一个针对高速移动场景设计的鲁棒多模态6DoF姿态估计框架，通过整合历史姿态记忆、2D跟踪器和几何滤波来提升性能，并在新数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在高速和低光场景下存在运动模糊问题，而事件相机虽具有高时间分辨率，但现有6DoF姿态估计方法在高速物体移动场景中表现不佳，需要专门针对高速移动场景的解决方案。

Method: 提出PoseStreamer框架，包含三个核心组件：1）自适应姿态记忆队列，利用历史方向线索保持时间一致性；2）以物体为中心的2D跟踪器，提供强2D先验以提升3D中心召回；3）射线姿态滤波器，沿相机射线进行几何细化。还构建了MoCapCube6D多模态数据集用于基准测试。

Result: 大量实验表明，PoseStreamer不仅在高速移动场景中实现了卓越的准确性，而且作为无模板框架对未见过的移动物体表现出强大的泛化能力。

Conclusion: PoseStreamer为解决高速移动场景下的6DoF姿态估计问题提供了一个有效的多模态框架，通过整合时间一致性、2D先验和几何细化，显著提升了在高速运动条件下的性能表现。
...
TL;DR: SSA框架通过对称最优传输对齐和复合方向引导策略，解决医学图像分割中多类型文本处理困难及空间约束缺失问题


<details>
  <summary>Details</summary>
Motivation: 当前文本引导的医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，难以识别病灶并建立与图像区域的关联；2）现有方法侧重于病灶描述而未能捕捉位置约束，导致关键偏差

Method: 提出空间感知对称对齐（SSA）框架，包含对称最优传输对齐机制和复合方向引导策略。前者建立图像区域与多个相关表达之间的双向细粒度多模态对应关系；后者通过构建区域级引导掩码，在文本中显式引入空间约束

Result: 在公开基准测试上的大量实验表明，SSA实现了最先进的性能，特别是在准确分割具有空间关系约束特征的病灶方面表现优异

Conclusion: SSA框架有效增强了处理包含位置、描述和诊断信息的混合医学文本的能力，解决了现有方法在多类型文本处理和空间约束方面的局限性
...
TL;DR: 提出了一种基于条件扩散逆过程的逆向个性化框架，用于面部匿名化，无需文本提示或模型微调，支持属性可控的匿名化


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的身份特征移除方法要么依赖预训练模型中对主体的良好表示，要么需要对特定身份进行模型微调，缺乏对匿名化过程中面部属性的控制

Method: 引入逆向个性化框架，利用条件扩散逆过程直接操作图像而无需文本提示，并加入身份引导的条件分支以泛化到训练数据之外的主体，支持属性可控的匿名化

Result: 该方法在身份移除、属性保留和图像质量之间实现了最先进的平衡，优于现有的匿名化方法

Conclusion: 提出的逆向个性化框架为面部匿名化提供了一种无需文本提示或模型微调的有效解决方案，支持属性控制，在隐私保护和图像质量之间取得了良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机系统通常孤立处理果园管理任务（病害检测、果实质量评估、产量估算），且依赖昂贵的多光谱传感器。需要一种统一、低成本、基于RGB相机的解决方案，实现综合果园智能管理。

Method: 采用统一管道集成三种深度学习模型：ResNet50用于叶片病害分类，VGG16用于苹果新鲜度判定，YOLOv8用于实时苹果检测和定位。系统在ESP32-CAM和树莓派上运行，支持完全离线现场推理，无需云端支持。

Result: 实验结果显示：叶片病害分类准确率达98.9%，新鲜度分类准确率达97.4%，苹果检测F1分数为0.857。系统在低成本硬件上实现了高性能，为多光谱无人机解决方案提供了经济替代方案。

Conclusion: 该框架提供了一个统一、低成本、基于RGB的果园智能管道，支持在廉价硬件上实现实用的精准农业，具有可扩展性和可访问性优势。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二进制掩码，这些范式存在特征纠缠问题，导致解剖基底损坏或结构不连续。需要一种能够保持解剖完整性同时生成高质量病理变化的方法。
...
Conclusion: PathoSyn提供了一个数学原理化的管道，用于生成高质量合成数据，支持低数据环境下的稳健诊断算法开发，并允许可解释的反事实疾病进展建模。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够利用丰富的场景上下文信息（空间关系、材质属性、物体共现）轻松识别物体，而传统计算识别系统通常在孤立图像区域上操作，忽略了这些关键上下文信息。

Method: 首先从单目图像构建地理语义上下文图(GSCG)，通过整合度量深度估计器和统一的全景与材质分割模型，将物体编码为具有几何、色彩和材质属性的节点，空间关系编码为边。然后提出专门的基于图的分类器，聚合目标物体、其邻近物体和全局场景上下文特征进行分类预测。

Result: 上下文感知模型达到73.4%的分类准确率，显著优于上下文无关版本（最低38.4%）。GSCG方法超越了微调的ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 在COCO 2017数据集上的结果表明，显式结构化和可解释的上下文表示对于物体识别任务具有显著优势，证明了上下文信息在计算视觉系统中的重要性。
...
### [79] [ForCM: Forest Cover Mapping from Multispectral Sentinel-2 Image by Integrating Deep Learning with Object-Based Image Analysis](https://arxiv.org/abs/2512.23196)
...
TL;DR: 本研究提出ForCM方法，结合基于对象的图像分析（OBIA）与深度学习（DL），使用Sentinel-2多光谱影像进行森林覆盖制图，在亚马逊雨林区域测试多种DL模型，结果显示ResUNet-OBIA和AttentionUNet-OBIA分别达到94.54%和95.64%的总体精度，优于传统OBIA的92.91%。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法存在精度限制，需要开发更准确、高效的制图技术以支持全球环境监测和保护工作。本研究旨在探索深度学习与基于对象图像分析相结合的新方法，利用免费易用的工具提高森林覆盖制图的准确性。

Method: 提出ForCM方法，结合OBIA与深度学习。使用Sentinel-2 Level 2A高分辨率卫星影像，包含三个数据集（两个三波段和一个四波段）。测试了UNet、UNet++、ResUNet、AttentionUNet和ResNet50-Segnet等多种深度学习模型，然后将最有效的DL模型与OBIA技术集成，并与传统OBIA方法进行比较。

Result: ForCM方法显著提高了森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，而传统OBIA仅为92.91%。研究还证明了QGIS等免费用户友好工具在准确制图方面的潜力。

Conclusion: ForCM方法通过结合深度学习与基于对象图像分析，显著提高了森林覆盖制图的准确性，为全球环境监测和保护提供了有效的技术支持，同时展示了免费工具在实际应用中的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 半监督遥感图像语义分割虽然能减轻标注负担，但存在伪标签漂移问题，即确认偏误导致训练过程中错误累积。需要一种稳定框架来缓解这一问题。

Method: 提出Co2S框架：1) 构建异构双学生架构，使用预训练的CLIP和DINOv3初始化两个不同的ViT基础模型；2) 引入显式-隐式语义协同引导机制，利用文本嵌入和可学习查询分别提供显式和隐式类别级引导；3) 开发全局-局部特征协同融合策略，融合CLIP的全局上下文信息和DINOv3的局部细节。

Result: 在六个流行数据集上进行广泛实验，该方法在各种划分协议和多样化场景下始终取得领先性能。

Conclusion: Co2S通过协同融合视觉语言模型和自监督模型的先验知识，有效缓解了伪标签漂移问题，为半监督遥感图像语义分割提供了稳定有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化且子类别相似性高，导致检测歧义。传统检测器独立检测每个物品，忽略了时尚物品之间存在的有意义关系（如搭配风格）。

Method: 提出Holi-DETR（整体检测Transformer），在DETR及其后续模型基础上，整合三种异构上下文信息：1）时尚物品间的共现关系概率；2）基于物品间空间布局的相对位置和大小；3）物品与人体关键点的空间关系。

Result: 实验表明，所提方法相比原始DETR提升了3.6个百分点（pp）的平均精度（AP），相比最近开发的Co-DETR提升了1.1个百分点的AP。

Conclusion: 通过利用三种上下文信息进行整体检测，Holi-DETR能有效减少时尚物品检测中的歧义，显著提升检测性能，证明了在时尚检测中考虑上下文关系的重要性。
...
### [82] [Anomaly Detection by Effectively Leveraging Synthetic Images](https://arxiv.org/abs/2512.23227)
*Sungho Kang,Hyunkyu Park,Yeonho Lee,Hanbyul Lee,Mijoo Jeong,YeongHyeon Park,Injae Lee,Juneho Yi*

Main category: cs.CV

TL;DR: 提出基于预训练文本引导图像翻译和图像检索的合成缺陷图像生成框架，通过两阶段训练策略在降低数据收集成本的同时提升异常检测性能


<details>
  <summary>Details</summary>
Motivation: 工业制造中异常检测至关重要，但真实缺陷图像稀缺。现有合成方法存在明显权衡：基于规则的合成成本低但真实性不足，基于生成模型的合成质量高但成本巨大。需要一种既能高效生成高质量缺陷图像又能降低成本的解决方案。

Method: 提出新颖框架：1) 利用预训练文本引导图像到图像翻译模型生成合成缺陷图像；2) 使用图像检索模型评估生成图像与真实正常图像的相似性，过滤不相关输出；3) 引入两阶段训练策略：先用大量基于规则合成的图像进行预训练，再用少量高质量图像进行微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性，显著降低了数据收集成本的同时提高了异常检测性能。

Conclusion: 提出的框架通过结合预训练翻译模型和图像检索模型，有效解决了合成缺陷图像生成中质量与成本的权衡问题，为工业异常检测提供了一种高效实用的解决方案。
...
### [83] [GeoTeacher: Geometry-Guided Semi-Supervised 3D Object Detection](https://arxiv.org/abs/2512.23147)
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽视了在标注数据有限时模型对物体几何结构敏感性不足的问题，而几何信息对于目标感知和定位至关重要。
...
Conclusion: GeoTeacher通过几何关系监督和增强策略，有效提升了半监督3D目标检测中模型对几何结构的感知能力，具有很好的通用性和性能提升效果。
...
### [84] [REVEALER: Reinforcement-Guided Visual Reasoning for Element-Level Text-Image Alignment Evaluation](https://arxiv.org/abs/2512.23169)
*Fulin Shi,Wenyi Xiao,Bin Chen,Liang Din,Leilei Gan*

Main category: cs.CV

TL;DR: REVEALER是一个基于强化学习引导视觉推理的统一框架，用于评估文本到图像生成中元素级别的对齐度，通过结构化范式实现细粒度可解释性评估。
...
Method: 采用"定位-推理-结论"结构化范式，利用多模态大语言模型显式定位语义元素并得出可解释的对齐判断，通过组相对策略优化和包含结构格式、定位准确性、对齐保真度的复合奖励函数进行优化。

Result: 在四个基准测试（EvalMuse-40K、RichHF、MHaluBench、GenAI-Bench）上实现最先进性能，优于强大的专有模型和监督基线，同时相比现有迭代视觉推理方法具有更优的推理效率。

Conclusion: REVEALER提供了一个统一且可解释的框架，用于评估文本到图像生成中的元素级对齐，在性能和效率方面均表现出色，为文本到图像模型的对齐评估提供了有效解决方案。
...
### [85] [GVSynergy-Det: Synergistic Gaussian-Voxel Representations for Multi-View 3D Object Detection](https://arxiv.org/abs/2512.23176)
*Yi Zhang,Yi Wang,Lei Yao,Lap-Pui Chau*

Main category: cs.CV

TL;DR: GVSynergy-Det：一种通过高斯-体素协同表示学习增强3D检测的新框架，无需深度或密集3D监督，在室内基准测试中达到最先进性能。
...
Conclusion: GVSynergy-Det通过高斯和体素表示的协同学习，成功解决了基于图像的3D检测中精度与监督需求之间的矛盾，为无监督3D检测提供了有效解决方案。
...
### [86] [GaussianDWM: 3D Gaussian Driving World Model for Unified Scene Understanding and Multi-Modal Generation](https://arxiv.org/abs/2512.23180)
*Tianchen Deng,Xuefeng Chen,Yi Chen,Qu Chen,Yuyao Xu,Lijin Yang,Le Xu,Yu Zhang,Bo Zhang,Wuxiong Huang,Hesheng Wang*

Main category: cs.CV

TL;DR: 提出基于3D高斯场景表示的统一驾驶世界模型框架，实现3D场景理解和多模态场景生成，通过语言特征嵌入高斯基元实现早期模态对齐，采用任务感知语言引导采样策略，并在nuScenes和NuInteract数据集上取得SOTA性能。
...
Method: 基于3D高斯场景表示的统一框架，将丰富的语言特征嵌入每个高斯基元实现早期模态对齐；设计任务感知语言引导采样策略，去除冗余3D高斯并注入准确紧凑的3D令牌到LLM；设计双条件多模态生成模型，结合高级语言条件和低级图像条件共同指导生成过程。
...
### [87] [SoulX-LiveTalk Technical Report](https://arxiv.org/abs/2512.23379)
*Le Shen,Qiao Qian,Tan Yu,Ke Zhou,Tianhang Yu,Yu Zhan,Zhenjie Wang,Ming Tao,Shunshun Yin,Siyuan Liu*

Main category: cs.CV

TL;DR: SoulX-LiveTalk是一个14B参数的实时音频驱动虚拟人生成框架，通过双向注意力蒸馏和多步自校正机制，在保持高视觉保真度的同时实现亚秒级启动延迟和32FPS实时吞吐
...
Result: SoulX-LiveTalk成为首个14B规模系统，实现亚秒级启动延迟（0.87秒）和32FPS实时吞吐，为高保真交互式数字人合成设定了新标准

Conclusion: 该框架通过创新的双向注意力保留和自校正机制，成功解决了实时音频驱动虚拟人生成中计算负载与延迟约束的根本矛盾，实现了高质量与实时性的平衡
...
TL;DR: 该论文提出了一种自适应选择扩散模型时间步特征的方法，通过任务感知时间步选择和特征整合模块，在少样本密集预测任务中提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型应用中，时间步特征的选择主要依赖经验直觉，往往导致次优性能并偏向某些特定任务。作者旨在解决这一限制，探索如何自适应选择最适合少样本密集预测任务的时间步特征。
...
Conclusion: 该框架通过自适应选择扩散时间步特征并有效整合，在仅提供少量支持查询的情况下，显著提升了密集预测任务的性能，为扩散模型在少样本学习中的应用提供了有效解决方案。
...
TL;DR: 提出基于天气条件和路面状况数据的实时道路监控系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，结合加速度数据，达到95%以上准确率，并建议使用模糊逻辑根据天气和时间分类路面状况。


<details>
  <summary>Details</summary>
Motivation: 传统道路监控方法成本高且不系统，需要大量测量时间。需要一种实时、系统化的道路状态监控方法，为车辆规划和主动控制系统提供有价值信息。

Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据与道路图像数据结合训练；4. 比较加速度基和摄像头图像基方法的性能；5. 将加速度或摄像头图像用于模糊逻辑分类，根据天气和一天中的时间分类路面状况。

Result: 1. 考虑5类道路状况：沥青路、损坏沥青路、碎石路、损坏碎石路、人行道路；2. 实现了超过95%的准确率性能；3. 比较了不同深度学习算法的性能。

Conclusion: 提出的实时系统能够有效监控道路表面状况，深度学习算法在道路分类中表现优异，结合加速度数据和模糊逻辑方法可以进一步提高分类的准确性和适应性。
...
TL;DR: CoFi-Dec是一个无需训练的LVLM解码框架，通过粗粒度到细粒度的视觉条件化与生成式自反馈来减少幻觉，使用Wasserstein融合机制整合多级视觉假设，显著降低实体和语义层面的幻觉。
...
Method: 提出CoFi-Dec训练免费解码框架：1）基于粗粒度和细粒度视图生成中间文本响应；2）使用文生图模型将响应转换为合成图像，形成多级视觉假设；3）引入Wasserstein融合机制对齐预测分布，实现几何一致的解码轨迹。

Result: 在六个幻觉基准测试上的广泛实验表明，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略。该框架与模型无关，无需额外训练，可广泛应用于各种LVLM。

Conclusion: CoFi-Dec通过整合生成式自反馈与粗到细视觉条件化，有效缓解了LVLM的幻觉问题，提高了输出的可靠性和忠实度，为多模态理解提供了更稳健的解码方法。
...
TL;DR: MM-UAVBench是一个针对无人机低空场景的多模态大语言模型综合基准测试，涵盖感知、认知和规划三大能力维度，包含19个子任务和5700多个手动标注问题，揭示了当前MLLMs在低空场景中的局限性。
...
Method: 提出了MM-UAVBench基准测试，系统评估MLLMs在低空无人机场景中的三大核心能力维度：感知、认知和规划。基准包含19个子任务，超过5700个手动标注问题，所有数据均来自公开数据集的真实无人机数据。

Result: 对16个开源和专有MLLMs的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析进一步揭示了空间偏见和多视角理解等关键瓶颈，这些因素阻碍了MLLMs在无人机场景中的有效部署。

Conclusion: MM-UAVBench基准测试将促进未来针对现实世界无人机智能的鲁棒可靠MLLMs研究，填补了低空场景MLLMs评估的空白，为模型改进提供了重要参考。
...
TL;DR: HY-Motion 1.0是一个基于扩散变换器的大规模3D人体运动生成模型，通过十亿参数规模和全阶段训练范式，实现了从文本描述生成高质量3D人体运动，性能超越现有开源基准。


<details>
  <summary>Details</summary>
Motivation: 当前3D人体运动生成领域缺乏大规模、高性能的开源模型，特别是在扩散变换器架构的规模化应用方面存在空白。研究者旨在开发一个能够精确遵循文本指令、覆盖广泛运动类别的先进模型，推动该领域向商业化成熟发展。

Method: 采用扩散变换器架构，首次在运动生成领域将其扩展到十亿参数规模。提出全阶段训练范式：1）在3000多小时运动数据上进行大规模预训练；2）在400小时精选数据上进行高质量微调；3）结合人类反馈和奖励模型进行强化学习。同时建立了严谨的数据处理流程，包括运动清洗和标注。

Result: 模型在指令跟随能力上显著超越当前开源基准，实现了最广泛的运动覆盖，涵盖6个主要类别下的200多个运动类别。模型能够生成高质量、与文本指令精确对齐的3D人体运动。

Conclusion: HY-Motion 1.0成功展示了扩散变换器在运动生成领域的规模化潜力，通过创新的全阶段训练范式实现了卓越的性能。该开源模型将促进未来研究，加速3D人体运动生成技术向商业化成熟发展。
...
### [93] [Bridging Your Imagination with Audio-Video Generation via a Unified Director](https://arxiv.org/abs/2512.23222)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI视频创作系统通常将剧本草稿和关键镜头设计视为两个独立任务：前者依赖大语言模型，后者依赖图像生成模型。作者认为这两个任务应该在单一框架中统一，因为逻辑推理和想象力思维都是电影导演的基本素质。

Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错，后解耦"的训练范式：1）交错概念学习，使用交错文本-图像数据培养模型对剧本的深度理解和想象解释；2）解耦专家学习，将剧本写作与关键帧生成解耦，增强故事叙述的灵活性和创造性。

Result: 大量实验表明，UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一到单一框架中，使非专业人士能够制作高质量的多镜头电影，解决了现有系统将这两个任务分离的问题。
...
Result: 实验表明AnyMS达到最先进性能，支持复杂组合并扩展到更多主体数量，在文本对齐、身份保留和布局控制方面表现优异。

Conclusion: AnyMS是一个高效、可扩展的训练免费多主体定制框架，通过创新的注意力解耦机制有效解决了多主体合成中的关键挑战。
...
Method: 提出视觉语言模型框架，包含两个关键技术：动态分辨率输入策略（DRIS）采用从粗到细的方法，根据图像内容复杂度自适应分配计算资源；多尺度视觉语言对齐机制（MS-VLAM）构建对象、局部区域和全局三个层次的对齐机制，系统捕获跨模态语义一致性。

Result: 在RS-GPT4V数据集上的实验表明，该框架在图像描述和跨模态检索任务中显著提升了语义理解精度和计算效率。相比传统方法，在图像描述的BLEU-4、CIDEr指标和跨模态检索的R@10指标上均取得更优性能。
...
Method: 开发了RxnBench基准，包含两个任务：1) 单图问答：从305个精选反应方案中提取1,525个问题，测试细粒度视觉感知和机理推理；2) 全文档问答：基于108篇文章，要求模型整合文本、方案和表格的跨模态信息。

Result: 评估显示模型存在关键能力差距：虽然擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但全文档问答任务中无一模型达到50%准确率。

Conclusion: 研究强调了开发领域专用视觉编码器和更强推理引擎的紧迫性，以推动自主AI化学家的进步，填补当前多模态大语言模型在化学文献理解方面的能力空白。
...
TL;DR: ASemconsist是一个解决文本到图像生成中角色身份一致性问题的新框架，通过选择性文本嵌入修改、语义控制策略和自适应特征共享，在保持每张图像提示对齐的同时确保角色身份一致性。
...
Method: 1) 选择性文本嵌入修改，实现对角色身份的显式语义控制；2) 基于FLUX中填充嵌入的分析，将填充嵌入重新用作语义容器；3) 自适应特征共享策略，自动评估文本模糊性并仅对模糊身份提示应用约束；4) 提出一致性质量评分(CQS)统一评估协议。

Result: 该框架实现了最先进的性能，有效克服了先前方法在身份一致性和提示对齐之间的权衡问题。

Conclusion: ASemconsist通过创新的语义控制和自适应策略，成功解决了文本到图像生成中角色身份一致性的挑战，为序列图像生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的2D高斯泼溅方法虽然具有紧凑存储和实时解码的优点，但在高斯数量较少时往往产生模糊或不清晰的边界，这主要是由于缺乏轮廓感知能力。

Method: 提出轮廓信息感知的2D高斯泼溅框架，将对象分割先验整合到基于高斯的图像表示中。通过限制每个高斯在特定分割区域内进行光栅化，防止跨边界混合并保持边缘结构。同时引入了预热方案来稳定训练并改善收敛性。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘周围实现了比现有2DGS方法更高的重建质量。改进在高斯数量非常少的场景中尤为明显，同时仍保持快速渲染和低内存使用。

Conclusion: 提出的轮廓信息感知2D高斯泼溅框架通过整合分割先验，有效解决了现有方法在边界保持方面的不足，特别是在高压缩场景下仍能保持清晰的边缘结构，同时保持了原有的效率和存储优势。
...
TL;DR: 提出CEM插件，通过累积误差最小化优化现有扩散模型加速方法，动态调整缓存策略以提升生成保真度


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)的迭代去噪过程导致推理速度慢，现有基于缓存的加速方法存在较大计算误差，且固定缓存策略无法适应去噪过程中复杂的误差变化

Method: 提出CEM插件，预定义误差来表征模型对加速的敏感性，通过动态规划算法进行策略优化，实现累积误差最小化，可无缝集成到现有误差校正框架和量化模型中

Result: 在9个生成模型和量化方法上的实验表明，CEM显著提升现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan上甚至超越了原始生成性能

Conclusion: CEM是一种模型无关的保真度优化插件，通过累积误差最小化动态优化缓存策略，显著提升扩散模型加速方法的生成质量，具有强泛化能力和零额外计算开销
...
### [100] [Multi-Track Multimodal Learning on iMiGUE: Micro-Gesture and Emotion Recognition](https://arxiv.org/abs/2512.23291)
...
TL;DR: 该研究提出了两个多模态框架，分别用于微手势识别和行为情感预测，在iMiGUE数据集上取得了良好效果，特别是在行为情感预测任务中获得了第二名。


<details>
  <summary>Details</summary>
Motivation: 微手势识别和行为情感预测都是具有挑战性的任务，需要建模细微的人类行为模式。研究旨在利用多模态数据（视频和骨骼姿态）来更好地捕捉这些细微的时空模式。

Method: 提出了两个多模态框架：1）微手势分类框架使用MViTv2-S提取视频特征，2s-AGCN提取骨骼姿态特征，通过跨模态令牌融合模块整合空间和姿态信息；2）情感识别框架使用SwinFace提取面部特征，MViTv2-S提取上下文特征，通过InterFusion模块融合情感表达和身体手势信息。

Result: 在iMiGUE数据集上的实验表明，该方法在行为情感预测任务中表现稳健且准确，在MiGA 2025挑战赛中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的特征信息，显著提升了模型性能，在相关挑战赛中取得了优异成绩。
...
TL;DR: 提出CME-CAD异构协作多专家强化学习范式，通过两阶段训练（MEFT和MERL）生成高精度、可编辑的CAD模型，并发布包含17299个实例的CADExpert开源基准。
...
Method: 提出异构协作多专家强化学习（CME-CAD）范式，整合互补模型优势，采用两阶段训练：多专家微调（MEFT）和多专家强化学习（MERL）。
...
TL;DR: 该论文从拓扑结构角度研究视觉表示学习，提出视觉空间具有纤维丛结构，语义对应商空间，需要非光滑的判别性目标才能学习语义不变性。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑视角理解视觉表示学习，基于一个核心假设：视觉理解需要语义语言，其中许多感知观察对应少量离散语义状态。结合表示学习中广泛接受的迁移性和抽象性前提，探索视觉空间的拓扑结构。

Method: 采用拓扑学框架分析视觉表示学习。提出视觉观察空间具有纤维丛结构，其中干扰变化填充纤维，语义对应商基空间。从该结构推导两个理论结果：1) 语义商空间不是子流形，不能通过光滑变形获得；2) 近似商空间对模型架构有结构要求。

Result: 理论分析表明：1) 语义不变性需要非同胚的判别性目标（如监督标签、跨实例识别或多模态对齐）；2) 语义抽象需要支持拓扑变化的表示机制，即"扩展-捕捉"过程：先几何扩展分离结构，再坍缩形成离散语义区域。

Conclusion: 该框架提供了拓扑视角，与大规模判别性和多模态模型的经验规律以及统计学习理论的经典原理相一致。这些结果是解释性的而非规定性的，为理解视觉表示学习提供了新的理论框架。
...
TL;DR: 该研究提出了SpatialMosaic数据集和基准测试，用于增强多模态大语言模型在真实挑战性多视角条件下的空间推理能力，包括部分可见性、遮挡和低重叠等场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预构建的3D表示或现成重建流程，限制了可扩展性和真实世界适用性。虽然已有研究探索从多视角图像直接学习空间推理，但真实环境中常见的关键挑战（如部分可见性、遮挡、低重叠条件）仍未得到充分探索。

Method: 1) 提出可扩展的多视角数据生成和标注流程，构建真实的空间推理问答对，创建包含200万QA对的SpatialMosaic指令调优数据集；2) 引入SpatialMosaic-Bench基准测试，包含100万QA对，涵盖6个任务，用于评估真实挑战性场景下的多视角空间推理；3) 提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中。

Result: 大量实验表明，提出的数据集和视觉问答任务能有效增强在挑战性多视角条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性。

Conclusion: 该研究通过创新的数据生成方法和混合框架，解决了多模态大语言模型在真实世界空间推理中的关键挑战，为增强3D场景理解和空间推理提供了有效解决方案。
...
Method: 提出NeXT-IMDL基准，从四个基本维度对AI生成内容篡改进行分类：编辑模型、操作类型、内容语义和伪造粒度。基于此构建五个严格的跨维度评估协议，对11个代表性模型进行系统评估。

Result: 实验表明，现有模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的评估协议下，表现出系统性失败和显著的性能下降。

Conclusion: 通过提供这一诊断工具包和新发现，旨在推动真正鲁棒的下一代图像篡改检测与定位模型的发展，挑战当前研究中的泛化能力错觉。
...
### [105] [SOFTooth: Semantics-Enhanced Order-Aware Fusion for Tooth Instance Segmentation](https://arxiv.org/abs/2512.23411)
...
<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临拥挤牙弓、模糊的牙齿-牙龈边界、缺失牙齿和临床重要的第三磨牙等挑战。现有3D方法存在边界泄漏、中心漂移和牙齿身份不一致问题，而2D基础模型虽提供强边界语义但难以直接应用于3D临床工作流。
...
Conclusion: SOFTooth通过语义增强的2D-3D融合框架成功解决了3D牙齿分割的关键挑战，展示了冻结2D语义信息在3D医学图像分析中的有效迁移能力。
...
### [106] [Bridging Cognitive Gap: Hierarchical Description Learning for Artistic Image Aesthetics Assessment](https://arxiv.org/abs/2512.23413)
*Henglin Liu,Nisha Huang,Chang Liu,Jiangpeng Yan,Huijuan Huang,Jixuan Ying,Tong-Yee Lee,Pengfei Wan,Xiangyang Ji*

Main category: cs.CV

TL;DR: 本文提出ArtQuant框架和RAD数据集，通过多维度结构化数据和LLM解码器解决艺术图像美学评估中的数据稀缺与模型碎片化问题，在减少训练成本的同时达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 美学质量评估对AIGC的人对齐量化评估系统至关重要，但其复杂性（涉及视觉感知、认知和情感）带来根本挑战。现有方法面临数据稀缺不平衡（过度关注视觉感知而忽视深层维度）和模型碎片化（多分支编码器隔离美学属性，对比学习方法难以处理长文本描述）两大问题。

Method: 1. 提出RAD数据集：大规模（70k）、多维度结构化数据集，通过迭代流程生成，无需昂贵人工标注且易于扩展；2. 提出ArtQuant框架：通过联合描述生成耦合孤立美学维度，利用LLM解码器更好建模长文本语义；3. 理论分析证实数据语义充分性与生成范式的协同作用最小化预测熵。

Result: 方法在多个数据集上达到最先进性能，仅需传统训练33%的epoch，缩小了艺术图像与美学判断之间的认知差距。将发布代码和数据集支持未来研究。

Conclusion: RAD数据集和ArtQuant框架有效解决了美学评估中的数据稀缺和模型碎片化问题，通过数据语义充分性与模型生成范式的协同作用，为艺术图像美学评估提供了数学基础和实用解决方案。
...
Result: 在视频预测任务上显著超越最佳性能工作33.3%（FID）和1.8%（FVD），在NAVSIM规划基准上创造了新记录，实现了最先进的结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到图像生成任务中难以完全符合用户细微意图并保持一致的审美质量。现有的基于偏好的训练方法（如Diffusion DPO）依赖昂贵且可能有噪声的人工标注数据集，这限制了其应用。

Method: 提出Direct Diffusion Score Preference Optimization (DDSPO)，直接从获胜和失败策略中推导每个时间步的监督信号。通过使用预训练的参考模型自动生成偏好信号：对比原始提示与语义降级变体条件下的输出，提供密集的转换级信号，无需显式奖励建模或人工标注。

Result: 实验结果表明，DDSPO在文本-图像对齐和视觉质量方面都有提升，优于或匹配现有的基于偏好的方法，同时需要显著更少的监督。

Conclusion: DDSPO提供了一种有效的分数空间偏好监督方法，无需依赖人工标注数据，为扩散模型的偏好优化提供了更实用的解决方案。
...
TL;DR: 本文研究了不确定性量化是否能提升SAM在挑战性分割场景下的泛化能力，创建了UncertSAM基准测试，评估了多种轻量级不确定性估计方法，并探索了不确定性指导的预测优化。


<details>
  <summary>Details</summary>
Motivation: SAM等基础分割模型在零样本场景下表现良好，但在领域偏移或知识受限的情况下仍然脆弱。本研究旨在探索不确定性量化是否能以领域无关的方式缓解这些挑战并增强模型的泛化能力。

Method: 1) 创建UncertSAM基准测试，包含8个数据集，涵盖阴影、透明、伪装等挑战性分割条件；2) 评估一系列轻量级后处理不确定性估计方法；3) 评估初步的不确定性指导预测优化步骤。

Result: 最后层拉普拉斯近似方法产生的不确定性估计与分割误差相关性良好，表明有意义的信号。虽然优化效果尚属初步，但研究结果强调了将不确定性纳入分割模型以支持鲁棒、领域无关性能的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多视角视觉恢复和3D重建方法在真实世界的物理退化环境下表现脆弱，缺乏系统性的基准测试来评估这些方法在真实物理退化条件下的性能。

Method: 通过统一的采集协议捕获四类物理退化（光照、散射、遮挡、模糊）在不同严重程度下的场景，每个场景包含高分辨率图像、RAW格式图像和密集激光扫描数据，从中导出世界尺度网格和度量深度。

Result: 对多种基于优化的和前馈方法进行基准测试显示，在物理退化条件下重建质量显著下降，突显了当前多视角流程在真实世界挑战性环境中的脆弱性。

Conclusion: RealX3D基准测试集揭示了当前多视角视觉恢复和3D重建方法在真实物理退化环境中的局限性，为未来方法开发提供了重要的评估基准。
...
TL;DR: 本文提出了一种结合视觉水线检测、YOLOv8姿态尺度提取和多模态大语言模型的混合框架，用于自动化读取河流水位标尺，在水线检测和尺度间隙估计方面取得高精度，结合尺度间隙元数据显著提升了LLM的预测性能。
...
Method: 采用混合框架，包括图像预处理、标注、水线检测、尺度间隙估计和数值读取提取等顺序阶段，整合视觉水线检测、YOLOv8姿态尺度提取和GPT-4o、Gemini 2.0 Flash等多模态大语言模型。

Result: 水线检测精度达94.24%，F1分数83.64%；尺度间隙检测为后续读取提供准确几何校准；结合尺度间隙元数据显著提升LLM性能，Gemini Stage 2在最优图像条件下取得最佳结果：平均绝对误差5.43cm，均方根误差8.58cm，R平方0.84。

Conclusion: 该方法为自动化水文监测提供了可扩展、高效可靠的解决方案，展示了实时河流标尺数字化和改进水资源管理的潜力，强调了结合几何元数据与多模态人工智能对稳健水位估计的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视频语言模型在处理长视频时存在两个主要问题：时间窗口狭窄，无法捕捉长时间跨度的语义变化；主流基于文本的检索方法主要依赖表层词汇重叠，忽略了视觉、音频和字幕通道之间的丰富时间相互依赖性。
...
Conclusion: TV-RAG通过结合时间和语义信号实现了双重推理机制，可以无需重新训练或微调地集成到任何大型视频语言模型中，提供了一个轻量级、经济高效的升级路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型虽然能从文本生成故事，但在人类中心故事生成中面临额外挑战：需要保持详细多样的人脸一致性，以及协调不同图像中的多个角色。现有方法在这些方面存在不足。

Method: 框架包含两个关键组件：1) 迭代身份发现：提取连贯的角色身份特征；2) 重去噪身份注入：通过重新去噪过程注入身份信息，同时保留所需的上下文内容。

Result: 在ConsiStory-Human基准测试中，IdentityStory超越了现有方法，特别是在人脸一致性方面表现突出，同时支持多角色组合。

Conclusion: 该框架在无限长度故事生成和动态角色组合等应用方面显示出强大潜力，为人类中心故事生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度。虽然推理时间缩放理论上可以改善这一权衡，但现有方法仍不理想：奖励驱动的粒子优化常导致感知过度平滑，而最优路径搜索则容易失去结构一致性。

Method: 提出IAFS（迭代扩散推理时间缩放与自适应频率引导）框架，这是一种无需训练的方法，联合利用迭代细化和频率感知粒子融合。通过迭代修正结构偏差逐步优化生成图像，同时自适应地融合高频感知线索与低频结构信息。

Result: 在多个基于扩散的超分辨率模型上的广泛实验表明，IAFS有效解决了感知-保真度冲突，在感知细节和结构准确性方面均获得一致改进，优于现有的推理时间缩放方法。

Conclusion: IAFS框架通过迭代细化和自适应频率融合，成功平衡了图像超分辨率中的感知质量与结构保真度，为扩散模型在超分辨率任务中的性能提升提供了有效的推理时间优化方案。
...
TL;DR: PurifyGen是一种无需训练的双阶段提示净化方法，通过语义距离评估token安全性，将有毒语义投影到零空间并强化安全语义，实现安全文本到图像生成。
...
Method: 提出PurifyGen，一种无需训练的双阶段提示净化方法。首先通过计算互补语义距离评估每个token的安全性，识别接近有毒概念的token。其次对风险提示应用双空间变换：将有毒对齐的嵌入投影到有毒概念矩阵的零空间以移除有害语义，同时将其对齐到清洁概念的范围空间以强化安全语义。采用token级策略选择性替换风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面优于现有方法，并与依赖训练的方法竞争良好。该方法具有理论依据，对未见过的提示和模型具有强泛化能力。

Conclusion: PurifyGen提供了一种即插即用的解决方案，无需修改模型权重，通过双空间变换同时移除有害语义和强化安全语义，在保持原始意图和连贯性的同时实现安全文本到图像生成。
...
TL;DR: ThinkGen是首个基于思维链推理的视觉生成框架，通过解耦的MLLM+DiT架构和可分离GRPO训练范式，在各种生成场景中实现高质量图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的思维链推理在理解任务中表现出色，但在生成任务中的应用仍处于起步阶段，且受限于特定场景机制，缺乏泛化性和适应性。

Method: 提出ThinkGen框架：1) 解耦架构：预训练MLLM生成定制化指令，DiT根据指令生成图像；2) SepGRPO训练范式：在MLLM和DiT模块间交替进行强化学习，支持跨数据集联合训练。

Result: 在多个生成基准测试中取得了稳健、最先进的性能表现，证明了框架在各种生成场景中的有效性。

Conclusion: ThinkGen成功将思维链推理扩展到视觉生成任务，通过灵活的架构设计和训练范式，实现了广泛生成场景下的高质量图像生成。
...
Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注二元安全标签和风险类别；2) 通过纯强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并在强化学习目标中添加基于同义词库的相似性奖励，鼓励模型为未见的不安全类别生成简洁描述。

Result: ProGuard在二元安全分类上性能与闭源大模型相当，在不安全内容分类上显著优于现有开源防护模型。最重要的是，ProGuard展现出强大的主动调节能力，将分布外风险检测提高了52.6%，分布外风险描述提高了64.8%。

Conclusion: ProGuard通过创新的数据集构建和强化学习训练方法，有效解决了多模态安全风险防护问题，特别是在分布外风险识别和描述方面表现出色，为生成模型的安全防护提供了新的解决方案。
...
TL;DR: 该论文提出了一种实时交互式视频扩散模型，通过改进的蒸馏方法解决多模态条件视频生成中的实时性问题，显著降低推理成本并实现实时交互


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过迭代过程和双向注意力机制同时去噪所有视频帧，阻碍了实时交互。虽然现有蒸馏方法可以使模型自回归并减少采样步骤，但它们主要关注文本到视频生成，导致人机交互不自然且效率低下。本文旨在实现基于多模态上下文（文本、图像、音频）的实时交互式视频扩散，以弥合这一差距

Method: 针对领先的在线策略蒸馏方法Self Forcing在多模态条件下遇到的视觉伪影问题，研究了一种改进的蒸馏配方，重点关注条件输入的质量以及在线策略优化的初始化和调度。在HDTF、AVSpeech和CelebV-HQ等多模态条件（音频、图像和文本）头像视频生成基准上进行评估

Result: 蒸馏后的模型以20倍更低的推理成本和延迟，匹配了类似或更大规模全步骤双向基线的视觉质量。进一步将模型与音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks集成，构建了LiveTalk实时多模态交互头像系统。系统级评估显示LiveTalk在多轮视频连贯性和内容质量上优于Sora2、Veo3等最先进模型，同时将响应延迟从1-2分钟减少到实时生成

Conclusion: 通过改进的蒸馏方法成功实现了实时多模态交互式视频生成，显著降低了推理成本并实现了实时交互，为构建通用多模态交互AI系统提供了关键技术
...
Method: 引入TWIN数据集（56.1万张图像对查询），要求模型判断两幅视觉相似的图像是否描绘同一物体，从而鼓励关注细微视觉线索。数据集涵盖日常物体在不同上下文、视角和外观下的多样性。同时引入FGVQA基准测试套件（1.2万查询），重新利用多个领域的细粒度识别和检索数据集来量化改进。

Result: 在TWIN上微调的视觉语言模型在细粒度识别方面显著提升，在未见领域（如艺术、动物、植物、地标）也表现良好。在FGVQA基准上，相比现有模型提升高达19.3%，同时不影响通用VQA基准性能。数据集规模与物体标注数量呈正相关，规模是性能关键。

Conclusion: TWIN可作为开源视觉语言模型训练语料库的即插即用补充，提升未来模型的感知精度。数据集规模对性能至关重要，TWIN为增强模型细粒度视觉理解能力提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前红外夜视摄像头火灾检测面临两大挑战：1）数据集构建不足导致人工光源频繁被误判为火灾；2）现有检测模型在夜间火灾检测和小目标检测方面仍有提升空间。

Method: 1）构建额外NIR数据集并应用多种数据增强策略；2）提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0以减少误报；3）开发Patched-YOLO，通过基于补丁的处理提升RGB图像中小目标和远距离目标的检测能力。

Result: 提出的方法相比现有模型（YOLOv7 mAP50-95 0.51，RT-DETR 0.65，YOLOv9 0.598）取得了更高的检测准确率，特别是在夜间火灾检测方面表现更优。

Conclusion: 通过综合应用数据增强、两阶段检测架构和补丁处理技术，显著提升了红外夜视摄像头火灾检测的准确性，有效减少了人工光源误报问题，并改善了小目标和远距离目标的检测性能。
...
Conclusion: SRFA框架在胰腺肿瘤早期检测中表现出优越性能，证明了其作为有用工具的潜力，能够有效处理CT扫描中的复杂性和变异性。
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型中的记忆化现象，研究了不同数据和建模设计对记忆化的影响，发现数据模态、多样性、条件细化等会影响记忆化程度，并提出了缓解策略。
...
Method: 设计了一个评估框架来量化3D生成模型中的记忆化，首先应用于现有方法，然后通过使用潜在向量集扩散模型进行受控实验，研究数据和建模设计对记忆化的影响。

Result: 发现记忆化取决于数据模态，随数据多样性和更细粒度条件而增加；在建模方面，记忆化在中等指导尺度达到峰值，可通过更长的向量集和简单的旋转增强来缓解。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖注意力机制进行跨帧对象对齐，使用统一的显式物理模型（如恒定速度），偏好语义特征进行隐式对齐。然而，不同类别和帧之间运动状态和对象特征的变化使得这种对齐不够理想。

Method: HAT首先利用多个显式运动模型生成历史实例的空间锚点和运动感知特征提案，然后通过结合缓存的物体查询中嵌入的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐方案。

Result: 在nuScenes数据集上，HAT持续改进了多种基线的3D时序检测器和跟踪器。与DETR3D检测器配对时，在测试集上达到46.0% AMOTA的SOTA跟踪结果。在面向对象的端到端AD方法中，HAT提升了感知精度（+1.3% mAP，+3.1% AMOTA）并降低了32%的碰撞率。当语义信息受损时（nuScenes-C），HAT的运动建模增强使端到端AD的感知和规划更加鲁棒。
...
TL;DR: IDT是一种基于Transformer的前馈框架，用于多视角本征图像分解，通过注意力机制联合处理多张输入图像，在单次前向传播中生成视角一致的本征因子，无需迭代生成采样。
...
Method: IDT采用基于物理的图像形成模型，将图像明确分解为漫反射率、漫反射阴影和镜面反射阴影。使用Transformer注意力机制在多个输入图像上进行联合推理，分离Lambertian和非Lambertian光传输。

Result: 在合成和真实世界数据集上的实验表明，IDT实现了更干净的漫反射率、更一致的漫反射阴影和更好分离的镜面反射分量，相比先前方法显著提高了多视角一致性。

Conclusion: IDT通过结构化分解和Transformer注意力机制，实现了可解释且可控的多视角本征图像分解，有效解决了视角一致性问题，为视觉理解提供了更好的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然感知质量好，但依赖未来帧和昂贵的多步去噪，在延迟敏感场景中不实用，需要开发适合在线部署的低延迟解决方案。

Method: 提出Stream-DiffVSR因果条件扩散框架：1）四步蒸馏去噪器实现快速推理；2）自回归时间引导模块在潜在去噪时注入运动对齐线索；3）轻量级时间感知解码器配合时间处理器模块增强细节和时间一致性。

Result: 在RTX4090上以0.328秒处理720p帧，显著优于现有扩散方法。相比在线SOTA TMP方法，提升感知质量（LPIPS +0.095）同时降低延迟130倍以上，将初始延迟从4600秒降至0.328秒。

Conclusion: Stream-DiffVSR实现了扩散基视频超分辨率的最低延迟，首次使扩散VSR方法适合低延迟在线部署，平衡了感知质量和实时处理需求。
...
TL;DR: Moxin 7B是一个完全开源的LLM，遵循模型开放框架，提供完整的训练、数据集和实现细节透明度。基于Moxin开发了三个变体：Moxin-VLM（视觉语言）、Moxin-VLA（视觉语言动作）和Moxin-Chinese（中文能力），在各种评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展迅速，但存在专有模型（如GPT-4）和开源模型（如LLaMA、Mistral）之间的差距。专有模型性能卓越但封闭，开源模型易于定制但透明度不足。需要建立一个完全透明、开放的研究环境，促进健康开源生态系统的发展。

Method: 提出Moxin 7B作为完全开源的LLM，遵循模型开放框架，超越简单的权重共享，提供完整的训练过程、数据集和实现细节透明度。基于Moxin开发了三个专门变体：Moxin-VLM针对视觉语言任务，Moxin-VLA针对视觉语言动作任务，Moxin-Chinese针对中文能力。采用开源框架和开源数据进行训练。

Result: 实验表明，Moxin及其变体在各种评估中取得了优异的性能。模型、可用数据以及推导这些模型的代码都已公开发布。

Conclusion: Moxin 7B作为一个完全开源的LLM，通过提供完整的透明度，促进了更加包容和协作的研究环境，有助于维持健康的开源生态系统。其变体模型在特定任务领域也表现出色，为LLM的开放研究做出了贡献。
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明Transformer语言模型在其嵌入空间中学习了丰富的几何结构，但更高层次的认知组织在这些表示中的存在仍未得到充分探索。本研究旨在探究句子嵌入是否编码了与人类可解释认知或心理属性对齐的分级、分层结构。

Method: 构建包含480个自然语言句子的数据集，标注了连续序数能量分数和离散层级标签，涵盖七个有序认知类别。使用多个Transformer模型的固定句子嵌入，通过线性和浅层非线性探针评估这些标注的可恢复性。采用非参数置换测试验证性能是否超过随机水平，并通过UMAP可视化和混淆矩阵进行定性分析。

Result: 在所有模型中，连续分数和层级标签都能可靠地被解码，浅层非线性探针相比线性探针提供了一致的性能提升。词汇TF-IDF基线表现明显更差，表明观察到的结构不能仅归因于表面词汇统计。非参数置换测试进一步确认探针性能超过随机水平。定性分析显示嵌入空间中存在平滑的低到高梯度，以及主要相邻层级的混淆模式。

Conclusion: 这些结果提供了证据，表明Transformer嵌入空间展现出与人类定义认知属性对齐的分层几何组织，同时保持对内部意识或现象学主张的中立性。
...
TL;DR: SmartSnap提出了一种新的智能体强化学习范式，将任务验证从被动的事后评估转变为主动的实时自我验证，通过智能体自主收集关键快照证据来证明任务完成，显著提升了验证效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前基于GUI任务的自主智能体强化学习面临验证瓶颈。现有验证方法都是被动的事后处理，需要分析智能体完整交互轨迹，处理大量无关噪声信息，导致成本高昂且可靠性低。这种验证方式严重制约了智能体强化学习的可扩展性。

Method: 提出SmartSnap范式，设计具有双重使命的自我验证智能体：不仅要完成任务，还要主动收集证明任务完成的快照证据。基于3C原则（完整性、简洁性、创造性），智能体利用在线环境访问权限，在关键决策点收集最小化但决定性的快照证据集，然后由通用的LLM-as-a-Judge验证器基于这些证据判断任务完成情况。

Result: 在移动任务上的实验表明，SmartSnap范式能够以可扩展的方式训练LLM驱动的智能体，为8B和30B模型分别带来26.08%和16.66%的性能提升。自我验证智能体在性能上能够与DeepSeek V3.1和Qwen3-235B-A22B竞争。

Conclusion: SmartSnap通过将验证从被动事后处理转变为主动实时自我验证，解决了智能体强化学习的可扩展性瓶颈。解决方案寻找与证据寻求的协同作用促进了高效、自我验证智能体的培养，为复杂GUI任务下的自主智能体发展提供了新路径。
...
TL;DR: 该研究在最小主义方案框架下分析了也门伊比阿拉伯语中qulk从句的句法结构，提出qulk从句是双从句结构，其中qulk作为从句嵌入谓词选择完整的CP补语。


<details>
  <summary>Details</summary>
Motivation: 研究旨在分析也门伊比阿拉伯语中qulk从句的句法特性，这种结构在形态上融合了"我说"的含义，能够引入嵌入的陈述句、疑问句和祈使句，通常不带补语化成分。研究试图在最小主义理论框架下解释这种特殊结构的生成机制。

Method: 采用最小主义方案的核心操作（合并、移动、一致、拼读）进行分层句法分析，同时考虑形态合并等后句法过程。研究还分析了方言特有的特征，如二分否定、附着化和CP嵌入。

Result: 研究提出qulk从句是双从句结构，其中qulk作为从句嵌入谓词选择完整的CP补语。通过标准计算步骤和后句法过程成功推导了qulk从句的生成，并解释了方言特有的句法特征。

Conclusion: 研究为生成句法特别是最小主义理论提供了理论贡献，提出了将分析扩展到受话者从句kil-k的可能性，并为最小主义的普遍性提供了见解。
...
TL;DR: 提出LLM引导的示例选择框架，通过LLM生成的语义先验知识指导可穿戴传感器活动识别中的示例选择，在少样本条件下显著提升识别性能


<details>
  <summary>Details</summary>
Motivation: 当前最先进的人类活动识别方法存在两个关键限制：1）依赖大量标注数据集；2）仅使用几何示例选择方法，难以区分相似的可穿戴传感器活动（如行走、上楼梯、下楼梯）

Method: 提出LLM引导的示例选择框架，结合LLM生成的语义先验知识（包括特征重要性、类间混淆性、示例预算乘数）与基于边界的验证线索、PageRank中心性、枢纽惩罚和设施位置优化，选择紧凑且信息丰富的示例集

Result: 在UCI-HAR数据集上，在严格的少样本条件下实现了88.78%的宏观F1分数，优于随机采样、聚类中心选择和k中心等传统方法

Conclusion: LLM衍生的语义先验知识与结构和几何线索相结合，为少样本可穿戴传感器HAR中选择代表性传感器示例提供了更强的基础
...
Method: 集成HHEM轻量级分类框架，不依赖LLM判断；引入分段检索处理总结任务中的局部幻觉；使用TPR、TNR和准确率指标在QA和总结任务上比较不同LLM的幻觉检测效果。

Result: HHEM将评估时间从8小时减少到10分钟；HHEM+非捏造检查达到最高准确率82.2%和TPR 78.9%；CDF分析显示更大模型（7B-9B参数）幻觉更少，中等规模模型不稳定；分段检索改善了总结任务的局部幻觉检测。

Conclusion: 需要结构化评估框架平衡计算效率和事实验证鲁棒性，HHEM提供了高效解决方案但需进一步改进局部幻觉检测，模型规模与幻觉稳定性存在相关性。
...
TL;DR: HiFi-RAG是一个在MMU-RAGent竞赛中获胜的检索增强生成系统，通过分层过滤和多阶段处理解决开放域RAG中的信息不相关和对齐问题。


<details>
  <summary>Details</summary>
Motivation: 开放域检索增强生成面临检索文档中包含无关信息以及生成答案与用户意图对齐的挑战，需要更有效的解决方案。

Method: 采用分层过滤的多阶段流水线：使用Gemini 2.5 Flash进行查询构建、分层内容过滤和引用归因，保留Gemini 2.5 Pro用于最终答案生成，超越传统的基于嵌入的检索方法。

Result: 在MMU-RAGent验证集上，ROUGE-L提升至0.274（+19.6%），DeBERTaScore提升至0.677（+6.2%）；在测试集上，ROUGE-L提升57.4%，DeBERTaScore提升14.9%。

Conclusion: HiFi-RAG通过分层过滤和成本效益优化的模型分配策略，显著提升了开放域RAG系统的性能，特别是在处理截止日期后知识的问题上表现出色。
...
TL;DR: 该研究评估了大语言模型在会计领域的推理能力，通过建立垂直领域会计推理评估标准，测试了多个代表性模型，发现GPT-4表现最佳但仍有优化空间。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的应用，如何将其有效整合到会计专业领域成为关键挑战。需要理解LLMs在特定领域的推理能力，以促进企业数字化转型和社会发展。

Method: 引入垂直领域会计推理概念，基于GLM系列模型训练数据特征建立评估标准，使用不同提示工程策略测试GLM-6B、GLM-130B、GLM-4和GPT-4等模型在会计推理任务上的表现。

Result: 实验结果显示不同提示工程策略对模型性能有不同程度的提升，GPT-4展现出最强的会计推理能力，但当前LLMs仍无法满足实际应用需求，特别是在企业级会计场景中需要进一步优化。

Conclusion: 大语言模型在会计领域具有潜力，但需要针对企业级会计场景进行专门优化，才能充分发挥其在该领域的价值。该研究为后续推理范式研究和会计推理性能改进提供了基准。
...
TL;DR: 该论文主张韩语选区树库应采用"eojeol"（韩语词）作为基本终端单位，而非语素，以区分词内形态与短语级句法结构，并与依存资源保持一致。


<details>
  <summary>Details</summary>
Motivation: 韩语选区树库设计面临基本表征问题：以语素作为终端单位会混淆词内形态与短语级句法结构，并与基于eojeol的依存资源产生不匹配。

Method: 提出基于eojeol的选区表征方案，将形态切分和细粒度词性信息编码在独立的非选区层；通过比较分析证明在明确归一化假设下，世宗和宾大韩语树库在eojeol选区层面具有表征等价性。

Result: 分析表明基于eojeol的选区表征能够保持可解释的选区结构，支持跨树库比较和选区-依存转换。

Conclusion: 韩语选区树库应采用eojeol作为基本终端单位，将形态信息分离处理，这种方案能更好地协调选区与依存表征，促进韩语语法资源的互操作性。
...
TL;DR: AHA注意力机制让LLM像人类阅读一样，只在必要时访问全局上下文，通过二元路由器在完整注意力和局部滑动窗口注意力间动态切换，可减少93%的完整注意力操作而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 受人类阅读习惯启发——通常只关注当前页，仅在必要时回顾前文。研究者希望让大语言模型也能动态决定何时需要访问全局上下文，从而减少计算冗余。

Method: 提出All-or-Here Attention (AHA)机制，为每个注意力头配备二元路由器，为每个token动态切换完整注意力和局部滑动窗口注意力（窗口大小可配置）。

Result: 在256token窗口下，可替换93%的完整注意力操作而不损失性能。研究发现上下文依赖呈长尾分布，随着局部窗口扩大，对完整注意力的需求迅速衰减。
...
TL;DR: 该研究提出了一种基于大语言模型的结构化提示管道和集成方法，用于多模态对话中的情感分析，包括情感六元组提取和情感翻转检测。


<details>
  <summary>Details</summary>
Motivation: 理解多模态对话中的情感对于构建情感智能AI系统至关重要，但这是一个复杂的挑战。MCABSA挑战赛提出了两个具有挑战性的子任务：从多说话者对话中提取全面的情感六元组（包括持有者、目标、方面、意见、情感和理由），以及检测情感翻转及其触发因素。

Method: 对于子任务一，设计了结构化提示管道，引导大语言模型顺序提取情感组件，并通过细化上下文理解进行优化。对于子任务二，通过集成三种不同大语言模型的互补优势，稳健地识别情感转换及其触发因素。

Result: 系统在子任务一上获得了47.38%的平均分数，在子任务二上获得了74.12%的精确匹配F1分数，证明了逐步细化和集成策略在多模态情感分析任务中的有效性。

Conclusion: 研究表明，结构化提示管道和模型集成策略能够有效处理复杂的多模态情感分析任务，为构建更智能的情感分析系统提供了有前景的方法。
...
TL;DR: T3LLM：一个用于时间序列问答的三LLM框架，通过生成-审查-学习机制实现多步推理和显式纠错，在多个TSQA基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的时间序列分析方法采用通用NLP技术，在处理复杂数值序列时容易产生推理错误。时间序列数据具有可验证性，可以在推理步骤和原始输入之间进行一致性检查。

Method: 提出T3LLM框架，包含三个LLM：worker生成结构化提示下的逐步思维链，reviewer检查推理、识别错误步骤并提供纠正意见，student通过微调学习多步推理和自我纠正。

Result: 在多个真实世界TSQA基准测试中，T3LLM超越了强大的基于LLM的基线方法，实现了最先进的性能。

Conclusion: T3LLM通过利用时间序列数据的可验证性，结合生成-审查-学习机制，有效解决了LLM在时间序列问答中的推理错误问题，显著提升了TSQA任务的性能。
...
TL;DR: M2G-Eval是一个多粒度、多语言的代码生成评估框架，包含4个粒度级别和18种编程语言，用于精细评估大语言模型的代码生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成评估基准大多只关注单一结构粒度和有限编程语言，无法揭示模型在不同代码粒度和多语言场景下的细粒度能力差异，需要更全面的评估框架。

Method: 提出M2G-Eval框架，包含Class、Function、Block、Line四个粒度级别，涵盖18种编程语言，包含17K+训练任务和1,286个人工标注、污染控制的测试实例。使用监督微调和Group Relative Policy Optimization训练Qwen3-8B得到M2G-Eval-Coder模型。

Result: 评估30个模型（28个SOTA LLM + 2个M2G-Eval-Coder变体）发现：1）难度层次明显，Line级任务最简单，Class级最困难；2）随着任务复杂度增加，完整粒度和部分粒度语言之间的性能差距扩大；3）存在强跨语言相关性，表明模型学习了可迁移的编程概念。

Conclusion: M2G-Eval能够对代码生成能力进行细粒度诊断，并突显了在合成复杂长格式代码方面存在的持续挑战，为评估模型在不同粒度和多语言场景下的代码生成能力提供了全面框架。
...
TL;DR: 该论文重新审视扩散语言建模，分析了现有方法在连续嵌入空间扩散和离散标记扩散之间的权衡，指出均匀破坏和标记级边际训练的两个核心问题，呼吁开发更符合文本结构的扩散过程。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代优化等优势，但文本的离散性和高度结构化特性使得直接应用扩散原理面临挑战。作者希望重新审视扩散语言建模，找出扩散机制与语言特定需求之间的差距。

Method: 首先从扩散过程和语言建模的角度分析扩散语言建模，提出区分扩散机制与语言特定需求的五个属性。然后将现有方法分为连续嵌入空间扩散和离散标记扩散两类，分析它们各自满足的属性。最后通过分析最近的大型扩散语言模型，识别出两个核心问题。

Result: 研究发现：1）均匀破坏不尊重信息在不同位置的分布方式；2）标记级边际训练无法在并行解码时捕捉多标记依赖关系。现有方法都只能部分满足五个基本属性，反映了结构上的权衡。

Conclusion: 这些观察结果激励开发更符合文本结构的扩散过程，鼓励未来工作朝着更一致的扩散语言模型方向发展，以更好地处理文本的离散和结构化特性。
...
TL;DR: GRPO在提升大语言模型思维链忠实度方面优于DPO，特别是在较大模型中表现更佳，为开发更透明可信的推理提供了有前景的方向。


<details>
  <summary>Details</summary>
Motivation: 思维链推理虽然能提升大语言模型的问题解决能力，但现有研究表明其解释往往不能反映模型的真实推理过程，模型可能生成连贯但误导性的解释，或在未承认外部线索的情况下修改答案。这种不一致性削弱了基于思维链方法在安全监督和对齐监控中的可靠性。

Method: 评估两种优化方法：群体相对策略优化（GRPO）和直接偏好优化（DPO），在提升思维链忠实度方面的能力。实验使用不同规模的模型，包括Qwen2.5-14B-Instruct模型。

Result: GRPO在较大模型中表现优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标上取得最佳结果。两种方法都显示模型规模与性能呈正相关，但GRPO在提升忠实度指标方面显示出更大潜力，尽管在较小规模时行为不太稳定。

Conclusion: GRPO为开发更透明和可信赖的大语言模型推理提供了有前景的方向，有助于解决思维链解释与真实推理过程不一致的问题，提升模型的安全监督和对齐监控可靠性。
...
TL;DR: 本文研究了在大型语言模型中使用自适应预测集进行不确定性量化，提出了词汇感知的共形预测方法，显著减少了预测集大小同时保持覆盖率。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署大型语言模型需要严格的置信度量化，但标准的softmax概率通常校准不佳。传统的共形预测方法虽然能保证覆盖率，但产生的预测集包含数百个token，缺乏实用性。

Method: 提出了词汇感知的共形预测框架，通过语义掩码和温度调整评分来减少有效预测空间，同时理论上保证边际覆盖率。该方法在Gemma-2B模型上使用SQUAD和WikiText基准进行验证。

Result: VACP方法实现了89.7%的经验覆盖率（目标为90%），同时将平均预测集大小从847个token减少到4.3个token，效率提高了197倍。

Conclusion: 词汇感知的共形预测方法在保持统计覆盖保证的同时，显著提高了预测集的效率，为大型语言模型在高风险应用中的不确定性量化提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 希望言论在自然语言处理中相对被忽视，现有研究主要集中在英语，导致乌尔都语等低资源语言缺乏相关工具。虽然Transformer架构在仇恨言论检测中表现良好，但很少应用于希望言论检测，也缺乏跨语言测试。

Method: 使用预训练Transformer模型（XLM-RoBERTa、mBERT、EuroBERT、UrduBERT），应用简单预处理并训练分类器，构建多语言希望言论检测框架。

Result: 在PolyHope-M 2025基准测试中表现优异：乌尔都语二分类F1分数达95.2%，多分类达65.2%，在西班牙语、德语和英语中也取得类似竞争力结果。
...
TL;DR: 研究发现大型语言模型在多语言推理中存在盲点：虽然任务准确率高，但推理过程与结论之间常存在逻辑脱节，非拉丁文字语言中的推理-结论不一致率是拉丁文字语言的两倍以上。


<details>
  <summary>Details</summary>
Motivation: 尽管链式思维提示展示了强大的推理能力，但跨语言的推理质量转移问题尚未充分探索。当前的多语言评估实践可能无法全面反映模型的实际推理能力。

Method: 引入人工验证框架评估模型生成的推理轨迹是否在逻辑上支持其跨语言结论。分析6种语言、6个前沿模型的65k个推理轨迹，开发错误分类法并通过人工标注来表征推理失败。

Result: 发现模型虽然任务准确率高，但推理常无法支持其结论。非拉丁文字语言的推理-结论不一致率至少是拉丁文字语言的两倍。主要失败源于证据错误（无支持的主张、模糊事实）和不合逻辑的推理步骤。

Conclusion: 当前多语言评估实践对模型推理能力的描述不完整，突显了需要推理感知的评估框架来更准确评估跨语言推理质量。
...
TL;DR: 研究发现通过重新表述提示词能有效减少大语言模型在模拟人口调查时的社会期望偏差，使硅基样本更接近真实人类数据。


<details>
  <summary>Details</summary>
Motivation: 大语言模型被越来越多地用于模拟人口响应（硅基采样），但面对社会敏感问题时经常表现出社会期望偏差，给出更符合社会期望而非真实人类数据的答案。现有研究对此关注不足，需要探索如何通过简单的提示词调整来减少这种偏差。

Method: 使用美国国家选举研究数据，在两个模型家族的三个LLM上进行研究。首先复制基线硅基采样研究确认社会期望偏差存在，然后测试四种基于提示词的缓解方法：重新表述（中性、第三人称措辞）、反向编码（语义反转）、以及两种元指令（启动和前言），分别鼓励分析性和真诚性。使用Jensen-Shannon散度与引导置信区间评估与ANES数据的对齐程度。

Result: 重新表述提示词最有效地提高了对齐度，减少了分布对社会可接受答案的集中度，使分布更接近ANES数据。反向编码在不同项目上产生混合结果，而启动和前言方法鼓励了回答的均匀性，对偏差缓解没有系统性益处。

Conclusion: 研究验证了基于提示词的框架控制在缓解LLM固有社会期望偏差方面的有效性，为获得更具代表性的硅基样本提供了实用路径。
...
TL;DR: 利用社交媒体数据（特别是Twitter）构建NLP管道，自动识别并分类女性分享的怀孕经历，区分正常妊娠结局和不良妊娠结局，为流行病学研究提供补充数据源。


<details>
  <summary>Details</summary>
Motivation: 婴儿死亡率在美国仍是重大公共卫生问题，出生缺陷是主要原因之一。尽管已有研究关注不良妊娠结局（如流产、死产、出生缺陷、早产），但仍需要更全面的研究和干预策略。当前数据集有限，需要补充数据源来更好地理解这些负面结果。

Method: 提出一种新颖方法，利用公开的社交媒体数据（特别是Twitter）来增强现有数据集。构建自然语言处理（NLP）管道，自动识别分享怀孕经历的女性，并根据报告结果进行分类：报告完整妊娠期和正常出生体重的为阳性病例，报告不良妊娠结局的为阴性病例。采用强大的预处理技术和数据增强策略来处理社交媒体数据的固有挑战（不平衡、噪声、缺乏结构）。

Result: 该方法能够自动识别和分类社交媒体上的怀孕经历分享，为研究不良妊娠结局提供补充数据。研究展示了社交媒体数据作为流行病学调查辅助资源的可行性，并为评估特定干预措施、治疗或产前暴露对母婴健康结果的因果影响提供了潜在应用框架。

Conclusion: 社交媒体数据可以作为研究妊娠结局的流行病学调查的有价值补充资源。该方法不仅为当前研究提供了新数据源，还为未来涉及孕妇队列和对照组的健康研究提供了框架，展示了社交媒体数据在公共卫生研究中的实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 自回归解码虽然是大语言模型的标准范式，但其token-by-token的特性限制了推理时的并行性。扩散语言模型虽然提供并行解码，但由于依赖双向注意力而无法有效利用前缀KV缓存，导致实际部署效率不高

Method: 提出WeDLM框架：1) 使用标准因果注意力，通过拓扑重排序将已观察token移动到物理前缀同时保持逻辑位置；2) 流式解码过程持续将置信token提交到增长的前缀中，保持固定并行工作量

Result: WeDLM在保持强AR骨干模型质量的同时，在挑战性推理基准上实现接近3倍加速，在低熵生成场景下达到10倍加速，在匹配部署设置下优于vLLM优化的AR引擎

Conclusion: 通过完全基于因果注意力的扩散解码框架，WeDLM证明了扩散式解码在实践中可以超越优化的自回归引擎，为并行生成提供了有效的解决方案
...
TL;DR: 该研究探讨了在乌尔都语假新闻检测中，通过领域自适应预训练提升多语言模型性能的方法，发现XLM-RoBERTa经过领域适应后表现优于原始版本，而mBERT的结果则好坏参半。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的虚假信息是一个普遍问题，但乌尔都语等低资源语言在该领域研究有限。虽然可以使用多语言预训练模型进行微调，但这些模型在处理领域特定术语时表现不佳，导致性能不理想。

Method: 采用分阶段训练方法：首先使用公开的乌尔都语新闻语料库对XLM-RoBERTa和mBERT进行领域自适应预训练，然后在四个公开的乌尔都语假新闻数据集上进行微调，用于假新闻分类任务。

Result: 实验表明，经过领域自适应预训练的XLM-RoBERTa在所有数据集上均优于其原始版本，而经过领域自适应预训练的mBERT则表现出混合结果，在某些数据集上有所改进，在其他数据集上则没有明显提升。

Conclusion: 领域自适应预训练对于提升多语言模型在乌尔都语假新闻检测任务中的性能是有效的，特别是对XLM-RoBERTa模型效果显著，为低资源语言的虚假信息检测提供了有价值的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床笔记通常以非结构化或半结构化格式存储，这限制了其在二次分析和下游临床应用中的使用。可靠识别章节边界是将这些笔记结构化的关键步骤，因为不同章节（如现病史、药物、出院指导）提供不同的临床背景。

Method: 使用MIMIC-IV中1000份临床笔记的精选数据集，评估了三种方法：基于规则的基础方法、领域特定的Transformer模型和大型语言模型（包括API模型）。

Result: 大型API模型（GPT-5-mini）在句子级和自由文本分割上达到最佳平均F1分数72.4。轻量级基线在结构化句子级任务上仍有竞争力，但在非结构化自由文本上表现不佳。
...
TL;DR: 提出自动化合成高难度可验证任务的模拟环境管道，以及环境级强化学习算法，解决用户不稳定性和环境异质性问题


<details>
  <summary>Details</summary>
Motivation: 在模拟环境中进行强化学习对语言智能体具有成本效益和可扩展性，但先前工作存在环境合成半自动化、任务难度不足、模拟用户不稳定以及环境异质性问题

Method: 提出统一管道自动化合成高难度可验证任务的模拟环境，以及环境级强化学习算法，通过环境级优势估计缓解用户不稳定并提升训练效率

Result: 在tau-bench、tau2-Bench和VitaBench等智能体基准测试中验证了方法的有效性，深入分析显示其具有良好的域外泛化能力

Conclusion: 提出的自动化环境合成管道和环境级强化学习算法能有效解决模拟环境中的用户不稳定性和异质性问题，提升语言智能体的训练效率和泛化能力
...
TL;DR: 本文提出了一种将预训练目标重新解释为单步策略梯度优化的方法，通过引入奖励塑造策略来平衡多样性和精确性，从而为后续强化学习提供更好的探索空间。


<details>
  <summary>Details</summary>
Motivation: 强化学习可以显著提升大语言模型的推理能力，但其效果严重依赖于预训练模型的token输出分布所定义的探索空间。现有标准交叉熵损失限制了探索潜力，需要重新思考预训练目标如何为后续RL提供更有利的探索环境。

Method: 将标准交叉熵损失重新解释为单步策略梯度优化的特例，提出广义预训练目标，将下一个token预测框架化为随机决策过程。引入奖励塑造策略，使用正奖励缩放因子控制ground-truth token的概率集中度，并采用排名感知机制对高排名和低排名负token进行非对称处理。

Result: 研究发现，与直觉相反，施加精确导向的先验分布（而非更高熵的分布）能为强化学习提供更优越的探索空间，从而提升端到端的推理性能。

Conclusion: 通过将监督学习框架化为策略梯度优化，并引入平衡多样性和精确性的奖励塑造策略，可以重塑预训练的token输出分布，为后续强化学习提供更有利的探索空间，最终提升大语言模型的推理能力。
...
TL;DR: 评估三种先进大语言模型在临床决策支持中的表现，发现模型性能因任务而异，提示工程并非万能解决方案，其效果高度依赖模型和具体任务。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医学知识评估中表现出潜力，但其在真实临床决策中的实际效用仍未被充分探索。本研究旨在评估LLMs在整个临床推理工作流程中的表现，并探索提示工程是否能提升其性能。

Method: 使用36个病例研究，评估ChatGPT-4o、Gemini 1.5 Pro和LIama 3.3 70B三种模型在五种临床决策任务中的表现。在两种温度设置下测试，并应用MedPrompt框架的变体进行提示工程，包括有针对性和随机的动态少样本学习。

Result: 所有模型在不同任务中表现差异显著：最终诊断准确率接近完美，相关诊断测试表现较差，其他任务表现中等。ChatGPT在零温度下表现更好，而LIama在默认温度下更强。提示工程并非万能：显著提升了基线准确率最低的任务（相关诊断测试），但对其他任务反而有害。有针对性的动态少样本提示并不总是优于随机选择。

Conclusion: 提示工程的效果高度依赖模型和具体任务，表明将LLMs整合到医疗保健中需要量身定制、上下文感知的策略，而不是一刀切的解决方案。
...
TL;DR: FANG是一种后训练剪枝框架，通过基于功能感知的神经元分组来缓解校准偏差，在保持语言建模性能的同时提高下游任务准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能优异但计算和存储成本高昂。现有的后训练结构化剪枝方法在少样本校准集不能充分反映预训练数据分布时，对下游任务的泛化能力有限。

Method: 提出功能感知神经元分组(FANG)框架：1) 根据神经元处理的语义上下文类型将功能相似的神经元分组；2) 在每个组内进行重要性评估时，给与神经元组功能角色强相关的token更高权重；3) 保留跨多种上下文类型贡献的神经元；4) 基于功能复杂度自适应分配每个块的稀疏度。

Result: FANG在保持语言建模性能的同时提高了下游任务准确率。与FLAP和OBC两种代表性剪枝方法结合时达到SOTA结果，在30%和40%稀疏度下平均准确率比FLAP和OBC高出1.5%-8.5%。

Conclusion: FANG通过功能感知的神经元分组有效缓解了校准偏差问题，实现了稀疏度与性能之间的更好权衡，为大语言模型的高效压缩提供了有效解决方案。
...
TL;DR: LENS框架将多模态健康传感数据与语言模型对齐，生成临床基础的心理健康叙述，通过将EMA响应转化为自然语言描述，并训练补丁级编码器将原始传感器信号投影到LLM表示空间。
...
TL;DR: 论文批评了基于"偏置特征"的CoT忠实性评估指标，认为该指标混淆了不忠实性与不完整性，并提出了新的评估方法和发现


<details>
  <summary>Details</summary>
Motivation: 现有研究使用"偏置特征"指标将CoT标记为不忠实，如果它遗漏了影响预测的提示注入线索。作者认为这个指标混淆了不忠实性与不完整性，后者是将分布式transformer计算转化为线性自然语言叙述所需的有损压缩

Method: 1) 在Llama-3和Gemma-3上进行多跳推理任务实验；2) 提出新的faithful@k评估指标；3) 使用因果中介分析来研究未言语化线索的因果作用

Result: 1) 许多被Biasing Features标记为不忠实的CoT被其他指标判断为忠实，在某些模型中超过50%；2) 更大的推理时token预算显著提高线索言语化率（某些设置下高达90%）；3) 即使未言语化的线索也能通过CoT因果中介预测变化

Conclusion: 应谨慎依赖基于线索的评估方法，提倡更广泛的解释性工具包，包括因果中介分析和基于破坏的指标
...
TL;DR: Prompt Choreography：通过动态全局KV缓存高效执行LLM多智能体工作流的框架，支持消息重新排序和并行调用，显著降低延迟并加速工作流执行。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在多智能体工作流中的部署增加，存在大量冗余计算问题。传统方法中每个LLM调用都需要重新编码消息，导致效率低下。需要一种能够重用已编码信息、减少计算冗余的框架。

Method: 提出Prompt Choreography框架，维护动态全局KV缓存。每个LLM调用可以关注任意重新排序的先前编码消息子集，支持并行调用。通过微调LLM使其适应缓存机制，以模拟原始结果。

Result: 显著降低每消息延迟（2.0-6.2倍更快的首token生成时间），在某些以冗余计算为主的工作流中实现超过2.2倍的端到端加速。缓存消息编码虽然有时会给出与重新编码不同的结果，但通过微调可以帮助LLM模拟原始结果。

Conclusion: Prompt Choreography通过动态全局KV缓存有效解决了多智能体LLM工作流中的冗余计算问题，显著提高了执行效率，为大规模LLM部署提供了实用的优化框架。
...
TL;DR: TabiBERT是一个基于ModernBERT架构的土耳其语单语编码器，从头训练于1万亿token的多领域语料库，支持8192token上下文，在推理速度、内存效率和性能上显著优于现有土耳其语BERT模型。


<details>
  <summary>Details</summary>
Motivation: 尽管BERT等编码器在计算效率、训练稳定性和长上下文建模方面有显著进步，但土耳其语NLP领域缺乏一个从头训练、融入现代架构范式（如RoPE、FlashAttention）的单语编码器。

Method: 基于ModernBERT架构，整合RoPE位置编码、FlashAttention和优化归一化，从头训练于840亿token的多领域语料库（网页文本73%、科学出版物20%、源代码6%、数学内容0.3%），采样1万亿token进行预训练。

Result: TabiBERT支持8192token上下文（16倍原始BERT），推理速度提升2.65倍，GPU内存消耗降低，在TabiBench（28个数据集，8个任务类别）上获得77.58分，优于BERTurk 1.62分，在5个类别中达到SOTA，包括问答（+9.55）、代码检索（+2.41）和文档检索（+0.60）。

Conclusion: TabiBERT填补了土耳其语NLP领域现代编码器的空白，展现了优异的跨领域泛化能力，为土耳其语编码器研究提供了透明、可复现的模型权重、训练配置和评估代码。
...
TL;DR: 提出一种基于储层计算的矩阵乘法免费语言模型架构，通过部分固定和共享权重、插入储层层来减少计算成本，在保持性能的同时降低参数数量、训练时间和推理时间。
...
Method: 1) 采用矩阵乘法免费的语言模型架构；2) 部分固定和共享选定层的权重；3) 插入储层层以获得丰富的动态表示而不增加训练开销；4) 结合多种操作减少内存访问。

Result: 实验结果显示，所提架构将参数数量减少最多19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过结合矩阵乘法免费架构和储层计算思想，可以在保持语言模型性能的同时显著降低计算成本，为解决LLM计算瓶颈提供了一种有效方法。
...
TL;DR: 该研究提出了BiomedTLDR数据集，用于评估大语言模型生成科学论文极简摘要的能力，发现LLM生成的摘要相比人类专家更倾向于提取式而非抽象式摘要。


<details>
  <summary>Details</summary>
Motivation: 高质量的科学极简摘要（TLDR）有助于有效科学传播，但目前缺乏全面的高质量科学TLDR数据集，这阻碍了LLM摘要能力的开发和评估。同时需要了解LLM生成摘要与人类专家摘要的差异。

Method: 提出BiomedTLDR数据集，包含大量研究者撰写的科学论文摘要，利用参考文献中作者评论的常见做法。然后测试流行的开源权重LLM基于摘要生成TLDR的能力。

Result: 分析显示，虽然某些LLM能成功生成类似人类的摘要，但总体上LLM对原文的词汇选择和修辞结构有更强的亲和力，相比人类更倾向于提取式而非抽象式摘要。

Conclusion: BiomedTLDR数据集填补了科学TLDR数据集的空白，为评估LLM摘要能力提供了基准。LLM在生成科学摘要时表现出更强的提取倾向，这为改进LLM的抽象摘要能力提供了方向。
...
TL;DR: LLM-PeerReview是一种基于同行评审思想的LLM集成方法，通过无监督方式从多个LLM生成的候选回答中选择最佳响应，利用多模型集体智慧提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM集成方法需要监督训练或复杂机制，缺乏可解释性和灵活性。作者希望开发一种无监督、可解释且通用的方法，利用多个LLM的集体智慧选择最佳响应。

Method: 基于同行评审框架的三阶段方法：1) 评分阶段：使用LLM-as-a-Judge技术让多个LLM相互评估候选回答；2) 推理阶段：使用图模型真值推断算法或简单平均策略聚合多个评分；3) 选择阶段：选择最高分响应作为集成输出。

Result: 在四个数据集上取得强劲结果，两种变体分别比先进的Smoothie-Global模型高出6.9%和7.3%的分数。

Conclusion: LLM-PeerReview是一种概念简单但经验上强大的无监督LLM集成方法，通过同行评审框架实现了可解释性和灵活性，在多数据集上表现出优越性能。
...
TL;DR: LLMs在复杂编程任务中表现不佳，研究者开发了专门的数据转换DSL Anka，即使LLMs从未训练过该语言，也能通过上下文提示达到接近完美的准确率，显著优于通用语言Python。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面表现出色，但在复杂的多步骤编程任务中仍存在系统性错误。研究者假设这些错误源于通用语言的灵活性，允许多种有效方法且需要隐式状态管理，因此需要测试这一假设。

Method: 研究者开发了Anka，一种用于数据转换管道的领域特定语言，具有明确、受限的语法，可以减少代码生成的歧义。在100个基准问题上测试Claude 3.5 Haiku和GPT-4o-mini，尽管LLMs从未训练过Anka，仅通过上下文提示学习。

Result: Claude 3.5 Haiku在Anka上达到99.9%的解析成功率和95.8%的整体任务准确率。在多步骤管道任务中，Anka比Python有40个百分点的优势（100% vs 60%）。GPT-4o-mini验证了类似优势（多步骤任务+26.7个百分点）。

Conclusion: LLMs可以通过上下文提示完全学习新的DSL，达到接近原生的准确率；受限语法显著减少复杂任务的错误；为LLM生成专门设计的领域特定语言可以优于LLM广泛训练的通用语言。
...
TL;DR: 论文提出了一种基于稀疏自编码器的参数高效微调方法，通过解耦特征空间识别任务相关特征，构建可解释的低秩子空间来指导适配器初始化，在安全对齐任务上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 当前的低秩适配方法（如LoRA）假设任务相关的权重更新存在于低秩子空间中，但这一子空间是通过数据隐式学习的黑盒方式，缺乏可解释性和直接控制。作者认为这种困难源于多义性——单个维度编码多个纠缠概念。

Method: 利用预训练的稀疏自编码器在解耦的特征空间中识别任务相关特征，然后构建显式、可解释的低秩子空间来指导适配器初始化。理论分析证明在单义性假设下，SAE基子空间识别可实现任意小的恢复误差。

Result: 在安全对齐任务上，该方法达到99.6%的安全率，超过完全微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19-0.24%的参数。更重要的是，通过SAE特征的语义基础，提供了对学习到的对齐子空间的可解释性洞察。

Conclusion: 研究表明，将机制可解释性融入微调过程可以同时提高性能和透明度，为参数高效微调提供了新的方向。
...
TL;DR: 该研究针对中国电商直播中主播使用语音变形词规避监管的问题，提出了LiveAMR任务，构建了首个相关数据集，并开发了基于大语言模型的文本生成方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播（特别是抖音平台）已成为主要销售渠道，但主播经常使用语音变形词来逃避审查并进行虚假广告宣传。现有研究主要关注社交媒体和地下产业中的文本规避，而缺乏针对健康医疗直播中语音规避的研究。

Method: 研究提出了LiveAMR（直播听觉变形词解析）任务，构建了包含86,790个样本的首个LiveAMR数据集。将任务转化为文本到文本的生成问题，并利用大语言模型生成额外的训练数据来提升性能。

Result: 研究表明，通过大语言模型生成额外训练数据的方法能够有效提升性能，并证明变形词解析技术能显著增强直播监管效果。

Conclusion: LiveAMR任务为解决电商直播中的语音规避问题提供了有效方法，变形词解析技术对直播监管有重要应用价值，特别是在健康医疗领域的虚假广告识别方面。
...
Method: 开发了包含11个专门智能体的多智能体协作框架，包括主题分析师、案例分析师、编辑、叙述者和校对者，利用大语言模型和语音合成技术探索主题、提取现实案例、优化内容组织和合成自然口语。

Result: 与专家解读相比，AI4Reading在语音生成质量上仍有差距，但生成的解读脚本更简洁、更准确。

Conclusion: AI4Reading系统能够有效生成高质量的有声书解读内容，虽然语音质量仍需改进，但在内容准确性和简洁性方面表现出色。
...
TL;DR: 该论文系统综述了记忆机制在认知神经科学、大语言模型和智能体之间的跨学科联系，提出了从生物到人工视角的记忆分类、存储和管理全生命周期分析，并探讨了记忆评估基准、安全攻防以及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究在借鉴认知神经科学设计高效记忆工作流时面临跨学科障碍，难以吸收人类记忆机制的本质。为了弥合这一差距，需要系统性地整合记忆的跨学科知识，连接认知神经科学与大语言模型驱动的智能体。

Method: 1. 从认知神经科学到LLM再到智能体的渐进轨迹中阐明记忆的定义和功能；2. 从生物和人工双重视角对记忆分类、存储机制和完整管理生命周期进行对比分析；3. 回顾评估智能体记忆的主流基准；4. 从攻击和防御双重角度探索记忆安全性。

Result: 论文系统性地建立了认知神经科学、大语言模型和智能体之间的记忆机制联系框架，提供了跨学科的记忆分析视角，识别了现有研究的局限性，并为未来研究奠定了基础。

Conclusion: 该综述为智能体记忆研究提供了跨学科的理论框架，未来研究方向应聚焦于多模态记忆系统和技能获取，以推动更高效、安全的自主智能体发展。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理需要深度推理和逻辑推断的任务时面临挑战，特别是在大规模文本语料训练中可能引入噪声或不相关信息，导致预测错误或与事实知识不一致的输出。

Method: 提出SGR框架：1) 从外部知识库动态构建查询相关子图；2) 利用子图语义结构指导多步推理；3) 集成多个推理路径生成最终答案。通过结构化子图上的逐步推理减少噪声影响。

Result: 在多个基准数据集上的实验结果表明，SGR框架持续优于强基线方法，证明了其在增强大语言模型推理能力方面的有效性。

Conclusion: 基于外部子图生成的逐步推理增强框架SGR能够有效提升大语言模型的推理准确性，通过结构化知识引导减少噪声信息影响，为复杂推理任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着高质量领域特定数据日益稀缺，多轮训练成为适应大语言模型的实用策略。然而自回归模型在重复数据暴露下会出现性能退化，过拟合导致模型能力显著下降。研究发现这种退化源于学习动态不平衡：可预测的低熵token学得快并主导优化，而模型在高熵token上的泛化能力随持续训练而恶化。

Method: 提出EntroDrop，一种熵引导的token dropout方法，作为结构化数据正则化。该方法选择性掩码低熵token进行训练，并采用课程调度根据训练进度调整正则化强度。

Result: 在0.6B到8B参数规模的模型实验中，EntroDrop始终优于标准正则化基线，并在扩展的多轮训练中保持稳健性能。

Conclusion: 这些发现强调了在有限数据训练时，将正则化与token级学习动态对齐的重要性。该方法为数据受限领域中更有效适应LLMs提供了有前景的途径。
...
TL;DR: 研究发现科学团队性别多样性与影响力呈倒U型关系，女性领导团队与男性支持团队组合表现最佳，团队规模对领导组性别多样性影响显著但对支持组影响较小


<details>
  <summary>Details</summary>
Motivation: 学术界对性别多样性如何影响科学团队成功有浓厚兴趣，但现有研究结果不一致，且大多从整体角度衡量多样性，忽视了团队内部角色差异。这种局限性阻碍了对性别多样性如何塑造团队影响的更细致理解，特别是不同团队角色中性别多样性的影响仍不清楚。

Method: 将科学团队定义为论文的所有合著者，通过五年引用次数衡量团队影响力。利用作者贡献声明将成员分为领导角色和支持角色。基于PLOS期刊超过13万篇论文（主要是生物医学相关学科），采用多元回归分析这些角色中性别多样性与团队影响的关系。应用阈值回归模型研究团队规模如何调节这种关系。

Result: 结果显示：(1) 性别多样性与团队影响的关系在领导组和支持组都呈现倒U型；(2) 全女性领导组与全男性支持组的团队比其他团队类型获得更高影响力；(3) 领导组性别多样性的影响在小团队中显著为负，但在大团队中变为正且统计上不显著。相比之下，支持组性别多样性的估计值无论团队大小都保持显著为正。

Conclusion: 性别多样性对科学团队的影响因角色而异，领导组和支持组的性别构成需要分别考虑。全女性领导与全男性支持的组合表现最佳，团队规模对领导组性别多样性影响显著但对支持组影响较小，这为优化团队构成提供了实证依据。
...
TL;DR: 提出一种基于嵌套密度聚类的方法，从LLM嵌入空间中构建文本的层次语义树，用于无监督发现文本语料中的语义结构和层次关系。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM嵌入常用于基于语义相似性的文本存储和检索，但文本语料库中的全局语义关系结构往往不透明。需要一种方法能够自动发现文本之间的层次语义关系，而无需预定义类别。

Method: 采用嵌套密度聚类方法：首先在LLM嵌入空间中识别语义相似度高的密集文本簇，然后逐步放宽密度标准，让密集簇合并成更分散的簇，最终形成包含整个数据集的单一簇（树的根节点）。通过将密集簇嵌入到逐渐分散的簇中，构建捕捉文本间层次语义关系的树结构。

Result: 方法在科学摘要案例研究中成功分类文本数据，实现无预定义类别的研究领域和子领域发现。在20 Newsgroups和IMDB 50k Movie Reviews等基准数据集上验证了方法的跨领域鲁棒性。

Conclusion: 嵌套密度树方法能够揭示文本数据集中的语义结构和演化，在科学计量学、主题演化等领域具有应用潜力，为无监督文本语义分析提供了新工具。
...
TL;DR: ACT是一种用于自动检测拉比文献中圣经引用的三阶段算法，通过形态感知对齐和上下文丰富化处理，在短句、转述和结构嵌入引用方面优于现有系统，F1分数达0.91。


<details>
  <summary>Details</summary>
Motivation: 现有文本重用框架在处理拉比文献中的短句、转述或结构嵌入的圣经引用时存在困难，需要一种能更好处理这些复杂引用模式的方法。

Method: 提出ACT三阶段算法：1) 形态感知对齐算法；2) 上下文敏感丰富化阶段，识别"波浪"和"回声"等复杂引用模式；3) 评估了三种配置以分析各组件贡献。

Result: 完整ACT管道(ACT-QE)在F1分数(0.91)、召回率(0.89)和精确率(0.94)上均优于所有基线系统。ACT-2(无风格丰富化)召回率更高(0.90)但精确率较低，ACT-3使用更长n-gram在覆盖率和特异性间取得平衡。

Conclusion: ACT不仅改进了引用检测，其跨语料库风格模式分类能力为体裁分类和互文分析开辟了新途径，为数字人文和计算文献学提供了方法论基础，特别适用于形态丰富、引用密集的传统文献分析。
...
TL;DR: 视觉语言大模型正朝着视觉理解和视觉生成任务的统一方向发展，但生成是否能增强理解在大数据规模下仍待探索。研究发现：1）生成能提升理解，但前提是生成语义而非像素；2）生成显示出更好的数据扩展趋势和更高的数据利用率；3）输入嵌入的自回归能有效捕捉视觉细节。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言大模型中视觉生成任务是否能增强视觉理解能力，特别是在大规模预训练（>200M样本）场景下，验证生成与理解任务统一的有效性。

Method: 提出UniHetero统一模型结构，在大规模预训练（超过2亿样本）下进行分析，对比不同生成方式（语义生成vs像素生成）对理解任务的影响。

Result: 1）生成语义而非像素能有效提升视觉理解性能；2）生成任务显示出更优的数据扩展趋势和更高的数据利用率；3）输入嵌入的自回归方法能有效捕捉视觉细节。

Conclusion: 在大规模预训练下，生成任务确实能增强理解能力，但关键在于生成语义内容而非像素级细节，这为视觉语言模型的统一架构设计提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在严重的确认偏误问题，当提示暗示偏好答案时，模型会强化这种偏见而非探索替代方案。这一问题在基础模型中已造成危害，在多智能体辩论中风险更大，因为回音室效应会强化偏见而非纠正

Method: 提出MoLaCE（潜在概念专家混合）框架，这是一种轻量级推理时框架，通过混合不同潜在概念激活强度的专家来处理确认偏误。关键洞见是：由于语言的组合性，不同措辞的提示会以特定方式重新加权影响事实正确性的潜在概念，因此无法对所有输入应用单一固定干预
...
TL;DR: 本文提出了一种利用知识图谱改进大语言模型幻觉自检测的方法，通过将LLM响应转换为实体关系图来估计幻觉可能性，相比现有方法在准确率和F1分数上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 幻觉（生成看似可信但实际错误的陈述）是LLM安全部署的主要障碍。现有自检测方法虽然表现良好，但仍有改进空间。本文旨在探索结构化知识表示（特别是知识图谱）如何提升幻觉自检测能力。

Method: 提出了一种简单而强大的方法：1）将LLM响应转换为实体和关系的知识图谱；2）利用这些图谱来估计响应包含幻觉的可能性。该方法在GPT-4o和Gemini-2.5-Flash两个LLM上进行了评估，使用了两个幻觉检测数据集，其中一个数据集经过人工整理和增强并作为本工作的次要成果发布。

Result: 相比标准自检测方法和当前最先进的SelfCheckGPT方法，该方法在准确率上取得了高达16%的相对提升，在F1分数上取得了20%的相对提升。结果表明，即使初始输出包含不准确信息，LLM在分析以知识图谱结构化的原子事实时表现更好。

Conclusion: 这种低成本、模型无关的方法为构建更安全、更可信的语言模型铺平了道路。知识图谱结构能够帮助LLM更好地分析原子事实，即使初始输出存在不准确性。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在整合视觉能力并进行视觉指令微调后，常常无法保持原有语言模型的指令跟随能力，导致模型不按预期执行任务指令。本研究旨在定量分析这种能力下降现象及其原因。

Method: 构建新的训练数据集，特别关注是否明确指定输出格式。研究在微调过程中明确指示输出格式如何影响LVLMs的指令跟随能力。通过定量评估验证常用数据集微调后指令跟随能力的下降情况。

Result: 定量评估证实，使用常用数据集微调后，LVLMs的指令跟随能力确实下降。同时发现，使用包含输出格式说明的数据集训练的模型比不包含的模型更能准确跟随指令。

Conclusion: 在视觉指令微调过程中，包含具有输出格式说明的样本可能有助于缓解指令跟随能力的下降。这为改进LVLMs训练方法提供了重要启示。
...
TL;DR: 研究表明，口语模型在多轮对话中被要求以特定说话风格开始时，经过几轮交互后无法保持所需风格，出现"风格遗忘"现象，且系统提示效果不如用户提示。


<details>
  <summary>Details</summary>
Motivation: 研究口语模型在保持特定说话风格方面的局限性，特别是当模型被要求在对话开始时采用特定说话风格时，经过多轮交互后是否能够持续保持该风格。

Method: 评估了三个专有模型和两个开源口语模型，关注副语言说话风格（情感、口音、音量、语速）。通过多轮对话测试，分析模型在系统提示和用户提示不同设置下的表现。

Result: 所有测试模型都无法在指令要求下保持一致的说话风格。模型能够回忆风格指令但无法在对话中表达。明确要求模型回忆风格指令可以部分缓解风格遗忘。系统提示效果不如用户提示，与预期功能相矛盾。

Conclusion: 口语模型存在显著的风格遗忘问题，需要在模型设计和提示策略上进行改进，特别是系统提示的有效性需要重新评估。
...
### [175] [Close the Loop: Synthesizing Infinite Tool-Use Data via Multi-Agent Role-Playing](https://arxiv.org/abs/2512.23611)
*Yuwen Li,Wei Zhang,Zelong Huang,Mason Yang,Jiajun Wu,Shawn Guo,Huahao Hu,Lingyi Sun,Jian Yang,Mingjie Tang,Byran Dai*

Main category: cs.CL

TL;DR: InfTool是一个完全自主的多智能体合成框架，通过自演化的方式让大语言模型能够可靠地调用外部工具，无需人工标注即可将32B基础模型的准确率从19.8%提升到70.9%


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在可靠调用外部工具方面存在三个关键瓶颈：高质量轨迹需要昂贵的人工标注、对未见工具的泛化能力差、单模型合成固有的质量上限导致偏见和覆盖差距

Method: InfTool采用完全自主的多智能体合成框架，包含三个协作智能体（用户模拟器、工具调用助手、MCP服务器），仅基于原始API规范生成多样化的验证轨迹。通过闭环系统：合成数据通过带门控奖励的组相对策略优化（GRPO）训练模型，改进后的模型生成更高质量数据，循环迭代无需人工干预

Result: 在伯克利函数调用排行榜（BFCL）上的实验表明，InfTool将32B基础模型的准确率从19.8%提升到70.9%（+258%），超越10倍大小的模型，与Claude-Opus相媲美，且完全基于合成数据无需人工标注

Conclusion: InfTool通过自演化的多智能体合成框架，成功解决了大语言模型可靠调用外部工具的关键瓶颈，实现了完全自主的高质量工具调用能力提升，为自主智能体发展提供了新范式
...
### [176] [Less is more: Probabilistic reduction is best explained by small-scale predictability measures](https://arxiv.org/abs/2512.23659)
...
<details>
  <summary>Details</summary>
Motivation: 探索在语言模型概率与认知现象关系研究中，需要多少上下文量才合适，以及是否必须使用完整话语来观察概率简化现象

Method: 通过比较完整话语和n-gram表示，研究哪种表示方式足以作为认知规划单元来观察概率简化现象

Result: 研究表明n-gram表示足以作为认知规划单元，不需要完整话语就能观察到概率简化现象

Conclusion: 在语言模型概率与认知现象关系研究中，n-gram表示是足够的认知单元，不需要完整的上下文信息
...
### [177] [Multilingual Hidden Prompt Injection Attacks on LLM-Based Academic Reviewing](https://arxiv.org/abs/2512.23684)
*Panagiotis Theocharopoulos,Ajinkya Kulkarni,Mathew Magimai. -Doss*

Main category: cs.CL

TL;DR: 研究发现LLM在学术论文评审中易受文档级隐藏提示注入攻击，不同语言注入效果差异显著
...
Method: 构建包含约500篇ICML已接受论文的数据集，在每篇论文中嵌入四种不同语言的语义等效隐藏对抗提示，使用LLM进行评审

Result: 英语、日语和中文的提示注入导致评审分数和接受/拒绝决策发生显著变化，而阿拉伯语注入几乎无影响，显示不同语言的脆弱性差异

Conclusion: LLM基于评审系统对文档级提示注入攻击具有高度敏感性，且不同语言注入效果存在显著差异，这对LLM在高风险应用中的安全性提出了重要警示
...
### [178] [PROFASR-BENCH: A Benchmark for Context-Conditioned ASR in High-Stakes Professional Speech](https://arxiv.org/abs/2512.23686)
*Deepak Babu Piskala*

Main category: cs.CL

TL;DR: ProfASR-Bench是一个针对专业领域ASR的评估套件，发现当前系统存在"上下文利用差距"：即使有提示信息，ASR性能也几乎没有改善


<details>
  <summary>Details</summary>
Motivation: 现有ASR基准测试低估了专业场景的挑战：密集领域术语、正式语域变化、关键实体错误零容忍。需要专门评估专业对话的ASR系统

Method: 创建ProfASR-Bench评估套件，包含金融、医疗、法律、技术领域。每个示例配对自然语言提示和实体丰富的目标话语。使用Whisper和Qwen-Omni模型，在无上下文、个人资料、领域+个人资料、oracle和对抗性条件下测试

Result: 发现一致模式：轻量级文本上下文对平均词错误率几乎没有影响，即使使用oracle提示；对抗性提示也不会可靠地降低性能。这被称为"上下文利用差距"

Conclusion: 当前系统名义上可提示但未充分利用可用侧信息。ProfASR-Bench提供了标准化上下文阶梯、实体和切片感知报告，以及可重复的测试平台，用于比较不同模型家族的融合策略
...
### [179] [Fine-Tuning LLMs with Fine-Grained Human Feedback on Text Spans](https://arxiv.org/abs/2512.23693)
*Sky CH-Wang,Justin Svegliato,Helen Appel,Jason Eisner*

Main category: cs.CL

TL;DR: 提出基于反馈驱动改进链的偏好监督微调方法，通过标注者标记喜欢/不喜欢的文本片段并提供具体反馈，让模型从左到右逐步改进，构建相邻步骤的偏好对进行对齐训练


<details>
  <summary>Details</summary>
Motivation: 现有的直接对齐方法（如标准A/B偏好排序或完整对比重写）在偏好监督方面存在效率低下和效果有限的问题，需要更结构化、基于修订的监督方法

Method: 1) 标注者对模型响应提供细粒度反馈，标记"喜欢"和"不喜欢"的文本片段并说明原因；2) 基础模型根据反馈从左到右逐步重写不喜欢的片段，形成增量改进链；3) 从链中相邻步骤构建偏好对用于直接对齐训练

Result: 该方法优于基于标准A/B偏好排序或完整对比重写的直接对齐方法，表明结构化、基于修订的监督能带来更高效和有效的偏好调优

Conclusion: 反馈驱动的改进链方法通过局部化、有针对性的编辑学习，为语言模型的偏好监督微调提供了更有效的结构化监督框架
...
### [180] [Eliciting Behaviors in Multi-Turn Conversations](https://arxiv.org/abs/2512.23701)
*Jing Huang,Shujian Zhang,Lun Wang,Andrew Hard,Rajiv Mathews,John Lambert*

Main category: cs.CL

TL;DR: 该研究探讨了在对话环境中从大语言模型中引出特定行为的方法，重点关注多轮对话场景，提出了分析框架并比较了不同方法的效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单轮对话中从大语言模型引出特定行为的方法，但在实际应用中，多轮对话更为常见且复杂。需要研究在多轮对话环境中如何有效识别和引出模型的特定行为，以更好地评估大语言模型。

Method: 首先提出了一个分析框架，将现有方法分为三类：仅使用先验知识的方法、使用离线交互的方法以及从在线交互中学习的方法。然后引入了在线方法的广义多轮表述，统一了单轮和多轮引出方法。最后评估了这三类方法在自动生成多轮测试用例方面的表现，分析了查询预算与成功率之间的权衡。

Result: 研究发现，在线方法在三个任务中仅用几千次查询就能达到平均45/19/77%的成功率，而现有的多轮对话基准中的静态方法很少甚至找不到失败案例。这表明在线方法在多轮对话评估中具有显著优势。

Conclusion: 该研究突出了行为引出方法在多轮对话评估中的新颖应用，并强调研究社区需要向动态基准方向发展，以更有效地评估大语言模型在复杂对话环境中的表现。
...
TL;DR: 提出了交互式实例对象导航（IION）任务和VL-LN基准，要求智能体通过主动对话解决模糊指令问题，实现更贴近实际的导航。


<details>
  <summary>Details</summary>
Motivation: 现有导航任务中的指令通常明确无歧义，而现实世界导航指令往往模糊不清，需要智能体通过主动对话解决不确定性和推断用户意图。

Method: 扩展实例对象导航（ION）为交互式版本（IION），允许智能体在导航过程中用自然语言咨询oracle；创建VL-LN基准，包含41k+长时程对话增强轨迹和自动评估协议。

Result: 训练了具备对话能力的导航模型，相比基线模型取得显著改进；大量实验和分析证明了VL-LN在推进对话使能导航研究方面的有效性和可靠性。

Conclusion: IION任务和VL-LN基准填补了现有导航任务与现实应用之间的差距，为对话使能的具身导航研究提供了重要工具和评估框架。
...
TL;DR: 开发了一个集机械工程、嵌入式系统和人工智能于一体的全自主送货机器人，采用异构计算架构，实现了AI感知、路径规划和实时电机控制，具备实际部署能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在现实世界中部署的完全自主送货机器人，解决资源受限平台上的计算密集型AI算法优化问题，以及ROS 2主机与嵌入式控制器之间的低延迟可靠通信挑战。

Method: 采用异构计算架构：RPi 5和ROS 2处理基于AI的感知和路径规划，ESP32运行FreeRTOS确保实时电机控制。机械设计通过精确的电机选择和材料工程优化了有效载荷能力和移动性。实现了PID电机控制、AWS IoT监控和固件级电机停机安全机制。

Result: 通过严格的内存和任务管理实现了确定性的PID电机控制，通过AWS IoT监控和固件级电机停机安全机制增强了系统可靠性，开发出了一个稳健且可操作的自主送货系统。

Conclusion: 这项工作展示了一种统一的多学科方法，成功开发出能够在现实世界部署的稳健自主送货系统，为资源受限平台上的复杂AI应用提供了实用解决方案。
...
TL;DR: 本文研究如何通过大规模多样化预训练实现从人类视频到机器人技能的有效迁移，发现当VLA模型在足够多样的场景、任务和实体上预训练时，会涌现出人类到机器人的转移能力。


<details>
  <summary>Details</summary>
Motivation: 人类视频覆盖了丰富的真实世界场景且易于获取，但直接用于训练视觉-语言-动作模型存在困难，因为建立人类与机器人之间的映射需要大量人工工程。受大型语言模型能从多样化监督中学习的启发，本文探索VLA模型是否也能通过规模效应实现从人类视频到机器人的有效迁移。

Method: 提出简单的协同训练方法，通过在大规模多样化场景、任务和实体上进行预训练，使VLA模型学习到实体无关的表示，从而实现人类视频数据到机器人技能的有效迁移。

Result: 研究发现，当VLA模型在足够多样的机器人预训练数据上进行训练时，人类到机器人的技能迁移能力会涌现出来。在仅有人类数据的泛化设置中，该方法可以将性能提升近一倍。

Conclusion: 通过大规模多样化预训练，VLA模型能够学习到实体无关的表示，从而实现从人类视频到机器人技能的有效迁移。这一发现表明，类似于大型语言模型，VLA模型也能通过规模效应获得从多样化监督中学习的能力。
...
TL;DR: 该论文提出两种受蝗虫启发的机制来提高视觉感知人工群体运动的鲁棒性：结合水平和垂直尺寸的鲁棒距离估计方法，以及间歇性运动机制来检测和处理故障机器人。


<details>
  <summary>Details</summary>
Motivation: 自然群体运动具有鲁棒性，但大多数人工群体系统却很脆弱，特别是在使用视觉作为感知方式时，由于视觉感知固有的模糊性和信息丢失问题。需要开发更鲁棒的集体运动机制。

Method: 1. 开发鲁棒距离估计方法：结合视觉感知到的邻居水平和垂直尺寸来估计距离；2. 引入间歇性运动机制：允许机器人可靠检测未能跟上群体的故障同伴，并中断群体运动；3. 提出故障机器人规避策略，对故障分类错误具有鲁棒性。

Result: 通过大量基于物理的仿真实验，显示使用这些技术后群体韧性得到显著改善。这些方法适用于基于距离的避让-吸引模型，也适用于依赖对齐的模型，在广泛的实验设置中均有效。

Conclusion: 受蝗虫启发的鲁棒距离估计和间歇性运动机制能够显著提高视觉感知人工群体系统的韧性，为解决视觉感知群体运动的脆弱性问题提供了有效解决方案。
...
TL;DR: 该论文将几何力学模型从对称线性阻力（黎曼度量）扩展到非对称线性阻力（芬斯勒度量），提出了构建系统运动能力的子芬斯勒方法。


<details>
  <summary>Details</summary>
Motivation: 现有几何力学模型主要处理对称线性阻力（黎曼度量），但现实世界中许多系统（如微生物、微型机器人）的阻力可能具有非对称性。需要扩展理论框架以处理更一般的阻力情况。

Method: 将阻力模型从黎曼度量推广到芬斯勒度量，将子黎曼方法扩展到子芬斯勒方法，识别系统运动能力的约束曲率类似性质。

Result: 成功建立了处理非对称阻力的子芬斯勒框架，能够表征更广泛系统的运动能力，扩展了几何力学模型的应用范围。

Conclusion: 该研究将几何力学模型从对称阻力系统扩展到非对称阻力系统，为理解更广泛生物和机器人系统的运动机制提供了更一般的理论框架。
...
TL;DR: 提出了一种用于多连通自由曲面球头铣削路径规划的高效策略，通过保形缝隙映射和拓扑保持网格变形，解决了边界一致性和零梯度奇异性问题，提高了加工效率和质量。


<details>
  <summary>Details</summary>
Motivation: 在多连通自由曲面上进行球头铣削路径规划时，虽然基于标量场优化的方法为多目标刀具路径生成提供了统一框架，但在保持边界一致性的同时消除导致等值线分支或终止的零梯度奇异性仍然具有挑战性，这会破坏刀具路径的连续性。

Method: 采用保形缝隙映射构建无奇异性的初始标量场，将优化重新表述为由边界同步更新控制的拓扑保持网格变形，实现全局优化的间距、残留高度均匀性和平滑轨迹过渡。

Result: 刀具路径连续、符合边界且无自相交。铣削实验表明，与最先进的基于保形缝隙映射的方法相比，所提方法加工效率提高14.24%，残留高度均匀性改善5.70%，铣削冲击引起的振动减少超过10%。

Conclusion: 该策略在高性能加工场景中具有广泛适用性，能够生成连续、边界一致且无自相交的刀具路径，显著提高加工效率和质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型将感知与控制耦合在单一管道中，仅针对动作进行优化，这会削弱语言条件的基础，导致在真实世界测试中出现过度抓取、被杂物干扰和背景过拟合等问题。

Method: 提出OBEYED-VLA框架，包含两个阶段：1) VLM基础的对象中心基础阶段，选择跨相机视角的任务相关对象区域；2) 几何基础阶段，强调对象的3D结构而非外观。然后将基础视图输入预训练的VLA策略，并在无环境杂物和非目标对象的单对象演示上进行微调。

Result: 在真实世界UR10e桌面设置中，OBEYED-VLA在四个挑战性场景和多个难度级别上显著优于强VLA基线：干扰对象、目标缺失拒绝、背景外观变化和未见对象的杂乱操作。消融研究确认语义基础和几何感知基础对性能提升都至关重要。
...
TL;DR: VLA-Arena是一个用于系统评估视觉-语言-动作模型能力的基准测试，通过结构化任务设计框架在任务结构、语言指令和视觉观察三个正交维度上量化难度，揭示VLA模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言-动作模型向通用机器人策略快速发展，目前缺乏定量理解其局限性和失败模式的方法。现有评估方法难以系统分析模型在不同难度维度上的表现。

Method: 提出结构化任务设计框架，从三个正交维度量化难度：(1)任务结构（安全、干扰、外推、长视野四个维度，170个任务，L0-L2三个难度级别）；(2)语言指令（W0-W4五个扰动级别）；(3)视觉观察（V0-V4五个扰动级别）。仅在L0级别进行微调以评估泛化能力。

Result: 对最先进的VLA模型进行评估，揭示了几个关键局限性：强烈的记忆化倾向而非泛化能力、非对称鲁棒性、缺乏安全约束考虑、无法组合学习技能完成长视野任务。

Conclusion: VLA-Arena提供了一个全面的基准测试框架，能够系统评估VLA模型的能力边界和失败模式。通过提供完整的工具链和数据集，促进解决这些挑战的研究，并确保可重复性。
...
### [189] [Modeling of UAV Tether Aerodynamics for Real-Time Simulation](https://arxiv.org/abs/2512.22588)
*Max Beffert,Andreas Zell*

Main category: cs.RO

TL;DR: 提出两种实时准静态系绳建模方法：基于悬链线理论的解析法（<1ms）和基于分段离散化的数值法（5ms），用于处理移动基座或强风条件下的系留无人机系绳力建模。


<details>
  <summary>Details</summary>
Motivation: 多旋翼无人机受电池限制飞行时间短，地面供电的系留方案是连续运行的实用解决方案。然而，在快速移动基座或强风条件下，需要考虑包括空气动力学效应在内的系绳力建模。

Method: 提出两种互补方法：1）基于悬链线理论和均匀阻力假设的解析法，计算速度极快；2）将系绳离散为分段和集中质量的数值法，使用CasADi和IPOPT求解平衡方程，通过热启动和解析初始化策略实现实时性能。

Result: 两种方法在实际测试中通过测力传感器验证。解析法在大多数系留无人机应用中提供足够精度且计算成本极低；数值法在需要时提供更高的灵活性和物理精度。

Conclusion: 这两种方法构成了轻量级且可扩展的实时系绳模拟框架，适用于离线优化和在线任务（如模拟、控制和轨迹规划），为系留无人机系统提供了有效的建模工具。
...
### [190] [Sistema de navegación de cobertura para vehículos no holonómicos en ambientes de exterior](https://arxiv.org/abs/2512.22734)
*Michelle Valenzuela,Francisco Leiva,Javier Ruiz-del-Solar*

Main category: cs.RO

TL;DR: 提出了一种用于非完整机器人的覆盖导航系统，旨在实现特定区域的完全覆盖，并能在遇到动态障碍时进行规避和恢复，为采矿等工业自动化应用提供概念验证。


<details>
  <summary>Details</summary>
Motivation: 移动机器人覆盖导航在采矿等工业领域有重要应用价值，如设施清洁、尾矿坝建设等。自动化这些过程对提高作业安全性至关重要，但目前缺乏针对非完整机器人的有效覆盖导航系统。

Method: 开发了包含路径规划和恢复行为的覆盖导航系统。系统计算覆盖特定区域的移动平台路线，并能在遇到动态或未映射障碍时执行规避机动和恢复操作，确保完全覆盖。

Result: 在模拟和真实室外环境中测试，大多数实验获得了接近90%的覆盖率。系统能够有效处理动态障碍并恢复覆盖任务。

Conclusion: 该系统为非完整机器人的覆盖导航提供了有效解决方案，为采矿等工业自动化应用奠定了技术基础。下一步将扩展到实际采矿机械，并在真实环境中验证其操作。
...
### [191] [Active Constraint Learning in High Dimensions from Demonstrations](https://arxiv.org/abs/2512.22757)
...
<details>
  <summary>Details</summary>
Motivation: 在演示学习范式中，如何有效地从有限的演示数据中推断出演示者环境中的未知约束是一个重要挑战。传统方法可能依赖于随机采样，效率较低且需要大量演示数据。

Method: 采用迭代式主动约束学习算法：1）在现有演示数据集上训练高斯过程来表示未知约束；2）利用GP后验查询起始/目标状态；3）生成信息丰富的演示并添加到数据集中。该方法通过主动学习策略智能选择最具信息量的演示。

Result: 在模拟和硬件实验中，使用高维非线性动力学和未知非线性约束的情况下，该方法在准确执行约束推断方面优于基于随机采样的基线方法，能够从迭代生成的稀疏但信息丰富的演示集中获得更好的性能。

Conclusion: 提出的迭代主动约束学习算法能够有效地从有限的演示数据中推断未知约束，通过智能地选择信息丰富的演示轨迹，在保持演示稀疏性的同时提高了约束推断的准确性。
...
### [192] [Two-Robot Computational Landscape: A Complete Characterization of Model Power in Minimal Mobile Robot Systems](https://arxiv.org/abs/2512.22770)
*Naoki Kitamura,Yuichi Sudo,Koichi Wada*

Main category: cs.RO

TL;DR: 首次完整刻画了两个自主移动机器人在所有主要模型下的计算能力，揭示了与多机器人情况完全不同的计算层次结构。


<details>
  <summary>Details</summary>
Motivation: 虽然多机器人场景下的计算能力层次结构已基本建立，但两个机器人的精确计算结构一直未解决。本文旨在填补这一空白，完整刻画两个机器人在所有主要模型下的计算能力。

Method: 采用无模拟方法，系统分析两个机器人在OBLOT、FSTA、FCOM和LUMI四种主要模型下，配合完整调度器谱系（FSYNCH、SSYNCH、ASYNCH及其原子变体）的计算能力。

Result: 发现两个机器人的计算层次结构与一般情况根本不同：1) 在完全同步下，FSTA^F和LUMI^F模型等价，表明完美同步可以替代记忆和通信；2) FSTA和FCOM模型正交，存在在弱通信模型中可解但在强有限状态模型中不可解的问题，完成双向不可比性。

Conclusion: 首次获得两个机器人的完整精确计算图景，揭示了最小规模协调的内在挑战，为理解自主移动机器人的基本计算能力提供了理论基础。
...
### [193] [The body is not there to compute: Comment on "Informational embodiment: Computational role of information structure in codes and robots" by Pitti et al](https://arxiv.org/abs/2512.22868)
*Matej Hoffmann*

Main category: cs.RO

TL;DR: 本文评论文章认为身体的主要功能不是计算，挑战了将计算和信息理论应用于理解动物身体进化和机器人设计的观点。


<details>
  <summary>Details</summary>
Motivation: 评论文章旨在挑战目标文章中将计算和信息理论应用于身体的观点，认为这种计算视角过于简化了身体的本质功能。

Method: 通过哲学分析和概念批判，评论文章对目标文章的核心论点进行反驳，强调身体的非计算本质。

Result: 评论文章提出了与目标文章不同的观点，认为身体的主要功能不是计算，而是其他更基本的生物功能。

Conclusion: 虽然计算和信息理论在理解神经系统方面有价值，但将其应用于身体理解可能过于简化，身体的主要角色不是计算。
...
### [194] [P-FABRIK: A General Intuitive and Robust Inverse Kinematics Method for Parallel Mechanisms Using FABRIK Approach](https://arxiv.org/abs/2512.22927)
...
<details>
  <summary>Details</summary>
Motivation: 传统几何逆运动学方法对并联机构依赖特定空间几何约束，应用于冗余并联机构时约束复杂度增加，且当目标位姿超出工作空间时无解并导致不可预测的控制问题

Method: 基于FABRIK算法，通过新的拓扑分解策略将通用并联机构分解为多个串联子链，迭代修正每个子链的末端目标来计算逆运动学解

Result: 通过平面、标准和冗余并联机构的多案例研究证明了该方法对各种并联机构的通用性，数值仿真验证了其有效性、计算效率以及处理超出工作空间目标的鲁棒性

Conclusion: P-FABRIK方法为并联机构提供了一种通用、直观、鲁棒的逆运动学解决方案，能够处理传统方法难以解决的约束复杂度和超出工作空间的问题
...
### [195] [PreGME: Prescribed Performance Control of Aerial Manipulators based on Variable-Gain ESO](https://arxiv.org/abs/2512.22957)
...
Method: 提出PreGME框架：1）采用变增益扩张状态观测器实时估计动态耦合；2）结合误差轨迹约束的规定性能飞行控制，预设误差轨迹引导系统演化。

Result: 实验验证（空中转杆、空中调酒、空中拉车）：即使在机械臂快速运动（末端速度1.02m/s，加速度5.10m/s²）引起的动态耦合下，仍能实现高跟踪性能。

Conclusion: 提出的方法能准确估计快速变化的动态耦合，处理机械臂激进运动任务；通过规定性能确保跟踪误差在预设包络内，实现高精度控制。
...
### [196] [Embodied Robot Manipulation in the Era of Foundation Models: Planning and Learning Perspectives](https://arxiv.org/abs/2512.22983)
...
TL;DR: 这篇综述论文从算法角度系统梳理了基于学习的机器人操作研究，提出了高层规划与底层控制的统一框架，并建立了分类体系来组织现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着视觉、语言和多模态学习的快速发展，机器人基础模型取得了显著进展，但机器人操作仍然是一个核心且具有挑战性的问题。需要从算法角度对现有学习型方法进行系统梳理，为现代机器人操作基础模型提供清晰的设计空间。

Method: 论文采用高层规划与底层控制的统一抽象框架。高层规划扩展了经典任务规划概念，涵盖语言、代码、运动、可供性和3D表示推理；底层控制提出了基于训练范式的分类法，按输入建模、潜在表示学习和策略学习三个维度组织现有方法。

Result: 建立了机器人操作学习型方法的系统分类框架，明确了高层规划与底层控制的关键组成部分，为理解现代机器人操作基础模型提供了结构化视角。

Conclusion: 论文识别了可扩展性、数据效率、多模态物理交互和安全性等开放挑战与未来研究方向，为机器人操作基础模型的进一步发展提供了清晰的路线图。
...
### [197] [Embodied Learning of Reward for Musculoskeletal Control with Vision Language Models](https://arxiv.org/abs/2512.23077)
*Saraswati Soedarmadji,Yunyue Wei,Chen Zhang,Yisong Yue,Yanan Sui*

Main category: cs.RO

TL;DR: MoVLR框架利用视觉语言模型将高层运动目标转化为控制策略，通过迭代优化发现有效的奖励函数，解决高维肌肉骨骼系统控制中的奖励函数设计难题。
...
Method: 提出MoVLR框架，利用视觉语言模型在目标规范与运动控制之间架起桥梁。通过控制优化和VLM反馈的迭代交互探索奖励空间，将语言和视觉评估转化为结构化指导，实现具身学习。

Result: MoVLR能够发现和优化高维肌肉骨骼系统的运动控制和操作任务的奖励函数，表明视觉语言模型可以有效将抽象运动描述基于生理运动控制的隐式原则。

Conclusion: 视觉语言模型能够有效连接抽象运动描述与生理运动控制的隐式原理，为高维肌肉骨骼系统的奖励函数发现提供了新途径。
...
### [198] [APOLLO Blender: A Robotics Library for Visualization and Animation in Blender](https://arxiv.org/abs/2512.23103)
*Peter Messina,Daniel Rakita*

Main category: cs.RO

TL;DR: 开发了一个轻量级软件库，为机器人研究人员提供简单的Blender脚本接口，用于快速创建高质量的出版级可视化内容。


<details>
  <summary>Details</summary>
Motivation: Blender作为强大的免费3D图形平台，学习曲线陡峭且缺乏机器人专用集成，使得研究人员难以有效使用。

Method: 开发了一个轻量级软件库，提供三个主要功能：1) 从URDF等标准化描述直接导入机器人和环境；2) 基于Python的脚本工具用于关键帧设置；3) 方便生成基本3D形状用于示意图和动画。

Result: 通过一系列概念验证示例展示了该库的功能，使机器人研究人员无需深入掌握Blender专业知识就能快速创建出版就绪的图像、动画和解释性示意图。
...
### [199] [Pole-centric Descriptors for Robust Robot Localization: Evaluation under Pole-at-Distance (PaD) Observations using the Small Pole Landmark (SPL) Dataset](https://arxiv.org/abs/2512.23141)
...
<details>
  <summary>Details</summary>
Motivation: 传统杆状结构作为机器人长期定位的几何锚点，在大型城市环境中远距离观测时识别可靠性显著下降。现有研究多关注描述符设计，而缺乏对描述符鲁棒性的系统性评估。

Method: 建立专门评估框架，构建SPL数据集（通过自动跟踪关联管道获取多视角、多距离的同一物理地标观测），对比分析对比学习与监督学习两种范式。

Result: 对比学习为稀疏几何特征构建了更鲁棒的特征空间，在5-10米距离范围内实现了更优的检索性能。

Conclusion: 该工作为评估挑战性现实场景中地标独特性提供了实证基础和可扩展方法学，证明对比学习在远距离杆状地标识别中具有优势。
...
### [200] [Towards the Automation in the Space Station: Feasibility Study and Ground Tests of a Multi-Limbed Intra-Vehicular Robot](https://arxiv.org/abs/2512.23153)
*Seiko Piotr Yamaguchi,Kentaro Uno,Yasumaru Fujii,Masazumi Imai,Kazuki Takada,Taku Okawara,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 该论文研究了在国际空间站上使用多肢体内车辆机器人自主执行物流任务的可行性，通过仿真和原型测试验证了机器人能在微重力环境下自主运输物品，减少宇航员的工作负担。


<details>
  <summary>Details</summary>
Motivation: 宇航员在国际空间站上花费大量时间处理物流任务（如准备、收尾、收集和运输物品），这减少了他们执行关键任务的时间。需要一种自主移动机械手来支持这些操作，减少机组人员和地面操作人员的工作量，同时实现实时任务执行。

Method: 研究包括仿真和原型测试：1）通过仿真模拟机器人在3D空间中的运动规划；2）在2D平台上测试原型机，模拟微重力环境；3）专注于机器人的运输能力，评估自主操作可行性。

Result: 结果表明，机器人能够在最小人工干预下执行物流任务，证明了在国际空间站上使用自主多肢体内车辆机器人增强操作效率的可行性。

Conclusion: 该研究证实了自主多肢体内车辆机器人支持国际空间站物流任务的可行性，为减少宇航员工作负担、提高操作效率提供了有前景的解决方案。
...
### [201] [A Sequential Hermaphrodite Coupling Mechanism for Lattice-based Modular Robots](https://arxiv.org/abs/2512.23154)
*Keigo Torii,Kentaro Uno,Shreya Santra,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出一种新型形状匹配机械耦合机制，满足单侧耦合/解耦、解耦时平坦表面、能与被动接口耦合等复杂设计要求，适用于模块化机器人系统和机械臂工具更换器


<details>
  <summary>Details</summary>
Motivation: 面向极端环境（如太空）大规模建造的晶格型模块化机器人系统需要满足复杂设计要求的耦合机制：单侧耦合与解耦、解耦时平坦表面、能与被动耦合接口耦合以及耦合机制间的耦合行为

Method: 提出一种新型形状匹配机械耦合机制，能够实现受控的、顺序的雄性和雌性状态转换。解耦时所有机制处于雌性状态；耦合过程中，一侧机制切换为雄性状态以实现单侧耦合。单侧解耦不仅可以从雄性侧进行，还可以通过强制将对方机制的雄性状态切换为雌性状态从雌性侧实现

Result: 该机制成功满足了所有设计要求：单侧耦合和解耦、解耦时平坦表面、能与被动接口耦合以及耦合机制间的耦合行为，为模块化机器人系统提供了有效的解决方案

Conclusion: 提出的形状匹配机械耦合机制能够满足复杂的设计要求，可应用于各种模块化机器人系统和机械臂工具更换器，为极端环境下的建造任务提供了可靠的技术支持
...
### [202] [SurgWorld: Learning Surgical Robot Policies from Videos via World Modeling](https://arxiv.org/abs/2512.23162)
...
TL;DR: 该研究通过构建SurgWorld世界模型和SATA数据集，利用未标记的手术视频生成合成的手术视频-动作配对数据，显著提升了手术机器人VLA策略的性能。


<details>
  <summary>Details</summary>
Motivation: 手术机器人领域面临数据稀缺问题，虽然有大量手术视频但缺乏对应的机器人动作标签，限制了模仿学习和VLA模型的直接应用。

Method: 1) 构建SATA数据集，包含详细的手术机器人动作描述；2) 基于先进物理AI世界模型和SATA构建SurgWorld，生成多样化、可泛化的逼真手术视频；3) 使用逆动力学模型从合成视频推断伪运动学数据，生成合成配对视频-动作数据；4) 用增强数据训练手术VLA策略。

Result: 使用增强数据训练的手术VLA策略在真实手术机器人平台上显著优于仅使用真实演示数据训练的模型。

Conclusion: 该方法通过利用未标记手术视频和生成式世界建模，为自主手术技能获取提供了可扩展的路径，开启了可泛化且数据高效的手术机器人策略的大门。
...
### [203] [A Human-Oriented Cooperative Driving Approach: Integrating Driving Intention, State, and Conflict](https://arxiv.org/abs/2512.23220)
*Qin Wang,Shanmin Pang,Jianwu Fang,Shengye Dong,Fuhao Liu,Jianru Xue,Chen Lv*

Main category: cs.RO

TL;DR: 提出一种以人为导向的协同驾驶方法，通过意图一致性轨迹规划和基于强化学习的控制权分配，减少人机冲突，提升驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 人车协同驾驶是实现完全自动驾驶的重要桥梁，但现有方法在人机交互自然性和有效性方面存在不足，需要减少人机冲突并建立更自然的交互方式。

Method: 采用战术层和操作层双层设计：战术层设计意图感知的轨迹规划方法，使用意图一致性成本评估轨迹；操作层开发基于强化学习的控制权分配策略，通过奖励函数优化策略。

Result: 仿真和人在环实验表明，该方法在轨迹规划中能对齐驾驶员意图，实现合理的控制权分配，相比其他协同驾驶方法显著提升驾驶性能并缓解人机冲突。

Conclusion: 提出的HOCD方法通过优先考虑驾驶员意图和状态，有效减少人机冲突，为人车协同驾驶提供了更自然有效的解决方案。
...
### [204] [Beyond Coverage Path Planning: Can UAV Swarms Perfect Scattered Regions Inspections?](https://arxiv.org/abs/2512.23257)
*Socratis Gkelios,Savvas D. Apostolidis,Pavlos Ch. Kapoutsis,Elias B. Kosmatopoulos,Athanasios Ch. Kapoutsis*

Main category: cs.RO

TL;DR: 本文提出mUDAI方法解决无人机多区域分散检测问题，通过优化图像采集位置和飞行轨迹，在保证数据质量的同时提高检测效率。


<details>
  <summary>Details</summary>
Motivation: 无人机检测任务受电池限制，现有覆盖路径规划方法在检测多个非连续感兴趣区域时效率低下，需要开发更优化的飞行路径和数据采集技术。

Method: 提出多无人机分离区域检测(mUDAI)方法，采用双重优化程序：计算最佳图像采集位置和最高效的无人机轨迹，平衡数据分辨率和操作时间，减少冗余数据采集和资源消耗。

Result: 通过仿真评估和实际部署验证，该方法能显著提高操作效率，同时保持高质量数据采集，适用于安全基础设施评估、农业检测和应急现场评估等应用。

Conclusion: mUDAI方法能实现分散感兴趣区域的快速高效检测，提供了开源Python实现、实验数据和在线交互平台，为实际应用提供了有效解决方案。
...
### [205] [PCR-ORB: Enhanced ORB-SLAM3 with Point Cloud Refinement Using Deep Learning-Based Dynamic Object Filtering](https://arxiv.org/abs/2512.23318)
*Sheng-Kai Chen,Jie-Yu Chao,Jr-Yu Chang,Po-Lien Wu,Po-Chiang Lin*

Main category: cs.RO

TL;DR: PCR-ORB是一种增强的ORB-SLAM3框架，通过深度学习点云精炼和YOLOv8语义分割来减少动态物体干扰，在KITTI数据集上部分序列表现优异但效果存在场景依赖性。


<details>
  <summary>Details</summary>
Motivation: 传统vSLAM系统在动态环境中面临重大挑战，移动物体会影响跟踪精度和地图一致性，需要新的方法来减少动态物体干扰。

Method: 提出PCR-ORB框架，集成深度学习点云精炼，使用YOLOv8进行语义分割，结合CUDA加速实现实时处理，采用多阶段过滤策略包括地面平面估计、天空区域移除、边缘过滤和时间一致性验证。

Result: 在KITTI数据集（序列00-09）上评估，不同序列表现不一，其中序列04的ATE RMSE改善25.9%，ATE中值改善30.4%，但整体效果存在场景依赖性。

Conclusion: PCR-ORB框架在特定场景下能有效减少动态物体干扰，但效果受环境条件影响，为复杂环境中的鲁棒导航提供了有价值的见解。
...
### [206] [Theory of Mind for Explainable Human-Robot Interaction](https://arxiv.org/abs/2512.23482)
*Marie Bauer,Julia Gachot,Matthias Kerzel,Cornelius Weber,Stefan Wermter*

Main category: cs.RO

TL;DR: 论文提出将人机交互中的心智理论视为可解释人工智能的一种形式，通过VXAI框架评估，以解决当前ToM方法缺乏验证解释与机器人实际推理一致性的问题。
...
Method: 将ToM视为XAI的一种形式，通过VXAI框架及其七个期望标准进行评估。将ToM原则嵌入XAI框架中，实现从系统中心到用户中心的视角转变。

Result: 提出了一种整合ToM与XAI的新框架，使机器人能够提供更符合用户信息需求和视角的解释，增强人机交互的透明度和可理解性。

Conclusion: 将ToM整合到XAI框架中可以弥补当前HRI中ToM方法的不足，实现从系统中心到用户中心的解释转变，提高机器人行为的可解释性和可预测性。
...
### [207] [Robust Deep Learning Control with Guaranteed Performance for Safe and Reliable Robotization in Heavy-Duty Machinery](https://arxiv.org/abs/2512.23505)
*Mehdi Heydari Shahna*

Main category: cs.RO

TL;DR: 开发用于重型移动机械的通用控制框架，支持从柴油液压到电动系统的转型，并部分集成AI同时保证安全性能


<details>
  <summary>Details</summary>
Motivation: 重型移动机械面临两大转型：从柴油液压系统向清洁电动系统的转变，以及从人工监督向更高自主性的转变。现有系统面临技术经济挑战，AI应用受限于严格安全要求，需要新的控制框架来支持这些转型。

Method: 开发通用模块化控制框架，包含：(1) 能源独立的通用控制策略，支持多体重型机械的强稳定性；(2) 分层控制策略，部分集成AI同时保证安全定义的性能和稳定性；(3) 解释和信任黑盒学习策略的方法，使其能够稳定集成并通过国际安全标准验证。

Result: 框架在三个案例研究中验证，涵盖不同执行器和条件，包括重型移动机器人和机器人操纵器。成果发表在五篇同行评审论文和一篇未发表手稿中，推动了非线性控制和机器人学发展。

Conclusion: 该研究开发的控制框架成功简化了电动化重型移动机械的控制设计，通过通用模块化方法支持未来修改，并定义了分层控制策略，在部分集成AI的同时保证安全性能，支持重型移动机械的两大转型。
...
### [208] [Act2Goal: From World Model To General Goal-conditioned Policy](https://arxiv.org/abs/2512.23541)
*Pengfei Zhou,Liliang Chen,Shengcong Chen,Di Chen,Wenzhi Zhao,Rongjun Jin,Guanghui Ren,Jianlan Luo*

Main category: cs.RO

TL;DR: Act2Goal：一种结合视觉世界模型与多尺度时间控制的通用目标条件操作策略，通过生成中间视觉状态序列和分层控制实现长时程操作，在零样本泛化和在线自适应方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉目标的任务规范虽然紧凑明确，但现有目标条件策略在长时程操作中存在困难，主要因为单步动作预测缺乏对任务进度的显式建模，需要更结构化的指导。

Method: 提出Act2Goal框架：1）目标条件视觉世界模型生成合理的中间视觉状态序列；2）多尺度时间哈希（MSTH）将轨迹分解为密集近端帧（细粒度闭环控制）和稀疏远端帧（全局任务一致性）；3）通过端到端交叉注意力将视觉表示与运动控制耦合。

Result: 在零样本泛化方面表现强劲，能适应新物体、空间布局和环境；通过基于LoRA的微调实现无奖励在线自适应；真实机器人实验显示，在具有挑战性的分布外任务上，成功率从30%提升至90%，仅需数分钟自主交互。

Conclusion: 目标条件世界模型与多尺度时间控制相结合，为鲁棒的长时程操作提供了必要的结构化指导，Act2Goal通过视觉规划和分层执行机制有效解决了长时程操作的挑战。
...
### [209] [Soft Robotic Technological Probe for Speculative Fashion Futures](https://arxiv.org/abs/2512.23570)
...
TL;DR: Sumbrella是一款软体机器人服装，作为思辨时尚探针开发，探索可穿戴机器人不仅功能还有社会意义的设计方法


<details>
  <summary>Details</summary>
Motivation: 新兴的可穿戴机器人需要不仅关注功能，还要考虑社会意义的设计方法。通过思辨设计探索软体机器人服装如何影响公共场合中的可接受性和期望

Method: 设计制作Sumbrella软体机器人服装，包含折纸启发的双稳态单元、织物气动驱动室、线缆驱动形状变形机制、计算机视觉组件，以及集成电源和控制电子设备的帽子和夹克。通过12名创意技术专家的焦点小组，将其作为技术探针探索人们对软体机器人可穿戴设备的理解、互动和未来关系想象

Result: 参与者围绕思辨未来和表达潜力进行了丰富讨论，但也提出了对剥削、监控以及生物传感技术嵌入公共生活的个人风险和社会伦理的担忧。为HRI领域贡献了设计软体机器人服装的关键考虑因素和建议

Conclusion: 思辨设计让HRI研究者不仅能考虑功能，还能探索可穿戴机器人如何影响公共场合中可接受或期望的定义。提出了运动学沟通潜力、技术对社会动态的影响以及伦理指南的重要性等关键见解
...
### [210] [A Kalman Filter-Based Disturbance Observer for Steer-by-Wire Systems](https://arxiv.org/abs/2512.23593)
*Nikolai Beving,Jonas Marxen,Steffen Mueller,Johannes Betz*

Main category: cs.RO

TL;DR: 提出基于卡尔曼滤波的扰动观测器，仅使用电机状态测量即可估计高频驾驶员扭矩，解决线控转向系统中驾驶员阻抗问题，仿真验证延迟仅14ms。


<details>
  <summary>Details</summary>
Motivation: 线控转向系统虽然具有重量轻、设计灵活等优点，但容易受到驾驶员无意扭矩产生的高频扰动（驾驶员阻抗）影响，现有方法要么依赖昂贵的扭矩传感器，要么缺乏捕捉快速高频扰动的时域分辨率。

Method: 设计基于卡尔曼滤波的扰动观测器，使用PT1滞后近似将驾驶员被动扭矩建模为扩展状态，并集成到线性和非线性线控转向系统模型中，评估不同卡尔曼滤波变体。

Result: 提出的扰动观测器能够准确重构驾驶员引起的扰动，延迟仅14ms；非线性扩展卡尔曼滤波在处理摩擦非线性方面优于线性版本，特别是在静态到动态摩擦过渡期间。

Conclusion: 该方法能有效估计高频驾驶员扭矩而无需直接扭矩传感器，但研究基于仿真验证，需要在真实驾驶条件下进一步研究观测器的鲁棒性。
...
### [211] [Interactive Robot Programming for Surface Finishing via Task-Centric Mixed Reality Interfaces](https://arxiv.org/abs/2512.23616)
*Christoph Willibald,Lugh Martensen,Thomas Eiband,Dongheui Lee*

Main category: cs.RO

TL;DR: 提出一种面向非专家的机器人编程方法，通过交互式任务导向工作流程，让用户直观地为表面精加工任务编程机器人。


<details>
  <summary>Details</summary>
Motivation: 当前机器人部署需要专业知识且设置过程冗长，这阻碍了机器人在高产品变异性和小批量生产环境中的应用，特别是在表面精加工领域。

Method: 开发了一种新的表面分割算法，结合人工输入来识别和细化工件处理区域；提供连续视觉反馈让用户迭代优化分割结果；基于分割表面模型生成机器人轨迹覆盖目标处理区域。

Result: 通过两个全面的用户研究评估了多种交互设计，得出了最优界面，显著降低了用户工作负荷，提高了可用性，即使对于实践经验有限的用户也能实现有效的任务编程。

Conclusion: 提出的交互式机器人编程方法能够有效降低非专家用户的使用门槛，使协作机器人能够更广泛地应用于小规模工艺和制造环境中的表面精加工任务。
...
### [212] [The N-5 Scaling Law: Topological Dimensionality Reduction in the Optimal Design of Fully-actuated Multirotors](https://arxiv.org/abs/2512.23619)
*Antonio Franchi*

Main category: cs.RO

TL;DR: 该研究揭示了N旋翼飞行器几何设计的拓扑结构，发现最优解空间随底盘对称性变化，存在相变现象，并提出了N-5缩放定律


<details>
  <summary>Details</summary>
Motivation: 传统方法将全驱动全向N旋翼飞行器的几何设计视为参数优化问题，寻找固定架构族中的单一最优解集。本研究旨在探索优化景观本身的内在拓扑结构

Method: 将设计问题表述在投影线乘积流形RP^2^N上，固定旋翼位置于多面体底盘顶点，同时变化其作用线。通过最小化坐标不变的Log-Volume各向同性度量，分析全局最优解的拓扑结构

Result: 发现最优解的拓扑结构严格由底盘对称性决定：对于不规则顶点排列，解为离散孤立点集；当底盘几何接近规则时，解空间经历临界相变，塌缩到N维环面，随后简化为由仿射相位锁定驱动的连续1维曲线。提出了N-5缩放定律：对于所有检查的规则平面多边形和柏拉图立体(N≤10)，最优配置空间由K=N-5个不相连的1维拓扑分支组成

Conclusion: 这些锁定模式对应一系列可容许的星形多边形{N/q}，允许精确预测任意N的最优相位。更重要的是，这种拓扑结构揭示了设计冗余，使得在保持最优各向同性控制能力的同时，飞行器可以沿着这些分支连续重构
...
### [213] [RoboMirror: Understand Before You Imitate for Video to Humanoid Locomotion](https://arxiv.org/abs/2512.23649)
...
TL;DR: RoboMirror是一个免重定向的视频到运动框架，通过视觉语言模型从视频中提取运动意图，直接控制人形机器人运动，无需显式姿态重建或重定向。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人运动系统依赖于运动捕捉轨迹或文本指令，存在视觉理解与控制之间的鸿沟。文本到运动方法存在语义稀疏性和流水线错误，而基于视频的方法只是机械的姿态模仿，缺乏真正的视觉理解。
...
### [214] [Do You Have Freestyle? Expressive Humanoid Locomotion via Audio Control](https://arxiv.org/abs/2512.23650)
*Zhe Li,Cheng Chi,Yangyang Wei,Boan Zhu,Tao Huang,Zhenguo Sun,Yibo Peng,Pengwei Wang,Zhongyuan Wang,Fangzhou Liu,Chang Xu,Shanghang Zhang*
...
Method: 基于"运动=内容+风格"的核心原则，将音频视为隐式风格信号，无需显式运动重建。采用ResMoE教师策略适应多样化运动模式，结合基于扩散的学生策略进行音频风格注入

Result: 实验验证显示RoboPerform在物理合理性和音频对齐方面取得有希望的结果，成功将机器人转变为能够响应音频的表演者

Conclusion: RoboPerform通过免重定向设计实现了低延迟和高保真度，为机器人提供了表达性的即兴表演能力，将音频直接转化为机器人运动
...
### [215] [The Bulldozer Technique: Efficient Elimination of Local Minima Traps for APF-Based Robot Navigation](https://arxiv.org/abs/2512.23672)
*Mohammed Baziyad,Manal Al Shohna,Tamer Rabie*

Main category: cs.RO

TL;DR: 提出名为Bulldozer的新型路径规划技术，解决人工势场法的局部极小值陷阱问题，通过后填充机制和斜坡增强来消除局部极小值区域，保持APF的实时响应和低计算量优势。


<details>
  <summary>Details</summary>
Motivation: 人工势场法因其简单性、实时响应性和低计算需求在移动机器人路径规划中广受欢迎，但传统APF方法存在局部极小值陷阱问题，机器人会陷入无法向目标前进的位置。

Method: 提出Bulldozer技术，引入后填充机制系统识别并消除局部极小值区域（类似推土机填补道路坑洞），增加这些区域的势能值；同时加入斜坡增强机制，帮助机器人从局部极小值陷阱中逃脱。

Result: 通过物理移动机器人在不同复杂度地图上的实验验证，与标准APF、自适应APF、A*、PRM和RRT等算法比较，Bulldozer有效解决局部极小值问题，同时实现更快的执行速度和具有竞争力的路径质量。使用运动学跟踪控制器评估路径平滑度和可跟踪性，确认适合实际执行。

Conclusion: Bulldozer技术成功解决了人工势场法的局部极小值陷阱问题，同时保持了APF方法的优势，为移动机器人提供了一种高效可靠的路径规划解决方案。
...
### [216] [Robo-Dopamine: General Process Reward Modeling for High-Precision Robotic Manipulation](https://arxiv.org/abs/2512.23703)
...
TL;DR: 提出Dopamine-Reward方法，通过多视角输入学习通用、步骤感知的过程奖励模型，解决传统强化学习奖励函数设计难题


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人应用中的主要障碍是有效奖励函数的设计。现有基于学习的过程奖励模型存在两个根本限制：缺乏步骤感知理解、依赖单视角感知导致细粒度操作进度评估不可靠；奖励塑造过程理论不健全，常导致误导策略优化的语义陷阱

Method: 提出Dopamine-Reward方法，核心是通用奖励模型（GRM），在3400+小时数据集上训练，采用步骤奖励离散化实现结构化理解，多视角奖励融合克服感知限制。基于此提出Dopamine-RL框架，使用理论健全的策略不变奖励塑造方法，使智能体能够利用密集奖励进行高效自我改进而不改变最优策略

Result: GRM在奖励评估中达到最先进准确率，基于GRM的Dopamine-RL显著提高策略学习效率。GRM通过单条专家轨迹一次性适应新任务后，奖励模型使Dopamine-RL仅用150次在线运行（约1小时真实机器人交互）就将策略成功率从接近零提升到95%，并保持跨任务的强泛化能力

Conclusion: Dopamine-Reward通过步骤感知和多视角融合解决了传统奖励模型的局限性，Dopamine-RL框架通过理论健全的奖励塑造避免了语义陷阱，为强化学习在真实机器人应用提供了高效可靠的解决方案
...
### [217] [Bidirectional RAG: Safe Self-Improving Retrieval-Augmented Generation Through Multi-Stage Validation](https://arxiv.org/abs/2512.22199)
*Teja Chinthala*
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG系统使用静态知识库，无法从用户交互中进化。为了解决这个问题，研究者希望开发一种能够安全扩展语料库的RAG架构，使系统能够从部署中学习。

Method: 提出Bidirectional RAG架构，采用多阶段接受层，结合基础验证（基于NLI的蕴含检查、归因检查和新颖性检测），防止幻觉污染的同时实现知识积累。
...
Conclusion: 研究表明，在严格验证的治理下，自我改进的RAG是可行且安全的，为RAG系统从部署中学习提供了实用路径。
...
### [218] [Emergent Persuasion: Will LLMs Persuade Without Being Prompted?](https://arxiv.org/abs/2512.22201)
...
<details>
  <summary>Details</summary>
Motivation: 随着对话AI系统的广泛应用，AI对人类观点和信念的影响力前所未有。先前研究主要关注滥用场景下的模型说服力，但本文旨在探究模型在未经明确提示的情况下何时会产生说服行为，这对评估新兴说服风险至关重要。

Method: 研究在两种场景下考察无提示说服：1）通过内部激活引导使模型沿特定人格特质方向调整；2）通过监督微调使模型表现出相同特质。使用包含良性话题的通用说服数据集进行训练。

Result: 研究发现：1）向说服相关或无关特质的方向引导并不能可靠增加模型的无提示说服倾向；2）监督微调确实会增加模型的说服倾向；3）仅使用良性话题进行监督微调得到的模型，在争议性和有害话题上也表现出更高的说服倾向。

Conclusion: 模型在未经明确提示时也可能产生有害说服行为，特别是在监督微调后。即使训练数据只包含良性话题，模型也可能在有害话题上表现出说服倾向，这表明新兴有害说服风险确实存在，需要进一步研究。
...
### [219] [GamiBench: Evaluating Spatial Reasoning and 2D-to-3D Planning Capabilities of MLLMs with Origami Folding Tasks](https://arxiv.org/abs/2512.22207)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知和指令跟随方面表现出色，但在空间推理能力上存在明显不足。现有基准测试大多关注静态图像或最终输出，未能充分考虑空间推理的序列性和视角依赖性特点。

Method: 开发了GamiBench基准测试，包含186个常规和186个不可能的2D折痕图案及其对应的3D折叠形状，从6个不同视角生成。设计了三个视觉问答任务：预测3D折叠配置、区分有效视角、检测不可能图案。引入了新的诊断指标：视角一致性和不可能折叠选择率。

Result: 实验表明，即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上也表现不佳。GamiBench为评估MLLMs的几何理解和空间推理能力提供了标准化框架。

Conclusion: GamiBench填补了现有基准测试在评估空间推理能力方面的空白，通过折纸任务全面评估模型的跨视角一致性、物理可行性判断和中间步骤理解能力，为提升MLLMs的空间推理能力提供了重要工具。
...
### [220] [Toward Equitable Recovery: A Fairness-Aware AI Framework for Prioritizing Post-Flood Aid in Bangladesh](https://arxiv.org/abs/2512.22210)
*Farjana Yesmin,Romana Akter*

Main category: cs.AI

TL;DR: 该论文提出了一种公平性感知的AI框架，用于优化孟加拉国洪水灾害后的援助分配，通过对抗性去偏技术减少对边缘化地区的系统性偏见。
...
Method: 采用对抗性去偏模型，结合梯度反转层技术，学习对偏见不变的表示。该方法借鉴了医疗AI中的公平性感知表示学习技术，应用于灾害管理领域。

Result: 在11个地区87个upazila的实验中，该框架将统计均等差异降低了41.6%，区域公平差距减少了43.2%，同时保持了较强的预测准确性（R平方=0.784 vs 基线0.811）。

Conclusion: 该研究展示了算法公平性技术在人道主义背景下的有效应用，为决策者提供了实施更公平灾害恢复策略的工具，确保援助基于真实需求而非历史分配模式。
...
### [221] [Logic Sketch Prompting (LSP): A Deterministic and Interpretable Prompting Method](https://arxiv.org/abs/2512.22258)
*Satvik Tripathi*

Main category: cs.AI

TL;DR: Logic Sketch Prompting (LSP) 是一个轻量级提示框架，通过引入类型化变量、确定性条件评估器和基于规则的验证器，显著提升大语言模型在需要严格规则遵循、确定性和可审计性任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言推理方面表现出色，但在需要严格遵循规则、确定性和可审计性的任务上仍然不可靠。特别是在临床、监管和安全关键决策支持系统中，这种不可靠性可能带来严重风险。

Method: 提出Logic Sketch Prompting (LSP)框架，包含三个核心组件：1) 类型化变量，2) 确定性条件评估器，3) 基于规则的验证器。该框架旨在产生可追踪和可重复的输出。在两个药理学逻辑合规任务上，使用Gemma 2、Mistral和Llama 3三个开源模型进行基准测试，比较LSP与零样本提示、思维链提示和简洁提示的性能。

Result: 在所有模型和两个任务中，LSP始终获得最高的准确率(0.83-0.89)和F1分数(0.83-0.89)，显著优于零样本提示(0.24-0.60)、简洁提示(0.16-0.30)和思维链提示(0.56-0.75)。McNemar检验显示LSP在几乎所有比较中都取得了统计学上显著的改进(p < 0.01)。

Conclusion: LSP在不牺牲性能的前提下，提高了大语言模型输出的确定性、可解释性和一致性，支持其在临床、监管和安全关键决策支持系统中的使用。
...
### [222] [Agent2World: Learning to Generate Symbolic World Models via Adaptive Multi-Agent Feedback](https://arxiv.org/abs/2512.22336)
...
<details>
  <summary>Details</summary>
Motivation: 当前训练LLMs生成符号世界模型（如PDDL域或可执行模拟器）面临大规模可验证监督数据缺乏的问题，现有方法主要依赖静态验证方法，无法捕捉交互执行中出现的行为级错误。
...
Result: 在三个涵盖PDDL和可执行代码表示的基准测试中展示了优越的推理时性能，取得一致的SOTA结果。通过Testing Team提供的交互式环境和行为感知自适应反馈，生成的多轮训练轨迹使微调后的模型在世界模型生成上获得平均30.95%的相对提升。

Conclusion: Agent2World不仅是一个强大的推理时世界模型生成框架，还能作为监督微调的数据引擎，通过多智能体反馈机制显著提升世界模型生成质量，为符号世界模型学习提供了有效解决方案。
...
### [223] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
...
Conclusion: 提出的编译方法扩展了传统数字启发式方法的应用范围，推动了该领域的技术边界
...
### [224] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 论文提出了HalluMatData基准数据集和HalluMatDetector框架，用于检测和缓解材料科学领域AI生成内容中的幻觉问题，将幻觉率降低了30%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学发现中产生幻觉（生成事实错误或误导信息）的问题严重威胁研究完整性，特别是在材料科学领域需要解决这一关键挑战。
...
Conclusion: 该研究为解决材料科学领域AI生成内容中的幻觉问题提供了有效工具，通过提出的数据集、检测框架和PHCS度量标准，能够显著提高LLM输出的可靠性和研究完整性。
...
### [225] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
...
<details>
  <summary>Details</summary>
Motivation: 当前知识图谱基础模型在链接预测上表现出强大的群体级性能，但无法捕捉个体用户偏好，这是通用关系推理与个性化排名之间的关键脱节。

Method: 提出GatedBias框架，采用结构门控适配：将特定于用户档案的特征与图导出的二元门结合，生成可解释的、每个实体的偏置，仅需约300个可训练参数，无需重新训练或影响全局准确性。

Result: 在两个基准数据集（Amazon-Book和Last-FM）上评估，显示在一致性指标上获得统计显著改进，同时保持群体性能。反事实扰动实验验证了因果响应性：从特定偏好信号中受益的实体在信号增强时显示出6-30倍的排名改进。
...
### [226] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体架构通常采用命令式、临时性模式构建，导致系统脆弱，存在状态管理困难、错误处理不足和并发问题等挑战。

Method: 引入Monadic Context Engineering (MCE)范式，将智能体工作流视为计算上下文，利用函子、应用函子和单子的代数结构来内在管理状态传播、短路错误处理和异步执行等横切关注点。特别使用单子实现健壮的顺序组合，应用函子提供并行执行的结构化方法，单子变换器实现这些能力的系统化组合。

Result: MCE使开发者能够从简单、可独立验证的组件构建复杂、健壮且高效的AI智能体。该框架还扩展到元智能体，通过元编程实现生成式编排，动态创建和管理子智能体工作流。

Conclusion: Monadic Context Engineering为AI智能体设计提供了形式化基础，通过代数结构的内在管理机制解决了现有架构的脆弱性问题，实现了组件化、可验证的智能体构建方法。
...
### [227] [DarkPatterns-LLM: A Multi-Layer Benchmark for Detecting Manipulative and Harmful AI Behavior](https://arxiv.org/abs/2512.22470)
*Sadia Asif,Israel Antonio Rosales Laguan,Haris Khan,Shumaila Asif,Muneeb Asif*

Main category: cs.AI

TL;DR: 该研究提出了DarkPatterns-LLM基准数据集和诊断框架，用于细粒度评估大语言模型输出中的操纵性内容，涵盖七个危害类别，并发现现有模型在检测自主性损害模式方面存在显著弱点。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，人们越来越担心其可能产生的操纵性或欺骗性行为会损害用户自主性、信任和福祉。现有的安全基准主要依赖粗糙的二元标签，无法捕捉构成操纵的微妙心理和社会机制。

Method: 研究引入了DarkPatterns-LLM基准数据集和诊断框架，包含401个精心策划的示例（指令-响应对和专家标注）。框架采用四层分析管道：多粒度检测、多尺度意图分析、威胁协调协议和深度上下文风险对齐，涵盖七个危害类别（法律/权力、心理、情感、身体、自主性、经济和社会危害）。

Result: 评估了包括GPT-4、Claude 3.5和LLaMA-3-70B在内的最先进模型，发现它们在检测操纵性内容方面存在显著性能差异（65.2%-89.7%），并且在检测自主性损害模式方面存在一致的弱点。
...
### [228] [Lessons from Neuroscience for AI: How integrating Actions, Compositional Structure and Episodic Memory could enable Safe, Interpretable and Human-Like AI](https://arxiv.org/abs/2512.22568)
*Rajesh P. N. Rao,Vishwas Sathish,Linxing Preston Jiang,Matthew Bryan,Prashant Rangarajan*

Main category: cs.AI

TL;DR: 论文主张将神经科学中的预测编码模型的关键组件（动作整合、层次组合结构、情景记忆）整合到当前的大语言模型中，以解决幻觉、缺乏基础、可解释性差等问题，实现更安全、高效、类人的人工智能。


<details>
  <summary>Details</summary>
Motivation: 当前基于预测编码的大语言模型虽然取得了显著进展，但忽略了神经科学中预测编码模型的三个关键组件：动作与生成模型的紧密整合、层次组合结构、情景记忆。这些缺失导致模型存在幻觉、概念理解肤浅、缺乏基础、可解释性差、能源效率低等问题。

Method: 提出将神经科学和认知科学中的预测编码模型组件整合到基础模型中：1）在多个抽象层次上整合动作与生成模型；2）采用组合生成架构；3）加入情景记忆系统。通过借鉴脑科学的原理来增强现有AI模型。

Result: 论文分析了当前基础模型的缺陷，并论证了整合这些脑启发组件可以：1）减少幻觉和概念理解肤浅问题；2）增强基础性和责任感；3）提高安全性和可解释性；4）改善能源效率。与当前流行的思维链推理和检索增强生成等方法进行了比较。

Conclusion: 重新激活脑科学与AI之间的思想交流，将预测编码模型的完整组件整合到基础模型中，有助于实现安全、可解释、以人为中心的人工智能。这种跨学科方法为解决当前AI系统的关键缺陷提供了有前景的方向。
...
### [229] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在生理信号分析中面临四大挑战：异构数据格式、不一致的预处理策略、碎片化的模型流程以及不可复现的实验设置，这些问题阻碍了该领域的进一步发展。

Method: Tyee工具包采用三大创新设计：1) 为12种信号模态提供统一数据接口和可配置预处理流水线；2) 模块化可扩展架构支持灵活集成和快速原型开发；3) 端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中均表现出色，优于或匹配基线方法，在13个数据集中的12个上取得了最先进的结果，证明了其实际有效性和泛化能力。

Conclusion: Tyee是一个实用且通用的生理医疗智能工具包，通过统一框架解决了该领域的关键瓶颈问题，已在GitHub上开源并持续维护。
...
### [230] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
*Junshu Dai,Yu Wang,Tongya Zheng,Wei Ji,Qinghong Guo,Ji Cao,Jie Song,Canghong Jin,Mingli Song*

Main category: cs.AI

TL;DR: M³ob：利用多模态时空知识增强人类移动性预测，通过构建统一的时空关系图和跨模态对齐机制，在正常和异常场景下都表现出优越性能


<details>
  <summary>Details</summary>
Motivation: 现有的人类移动性预测方法存在泛化能力有限的问题：单模态方法受数据稀疏性和固有偏差限制，多模态方法难以有效捕捉静态多模态表示与时空动态之间的语义鸿沟
...
Conclusion: 通过利用多模态时空知识表征移动性动态，M³ob方法有效解决了现有方法的局限性，在位置推荐任务中表现出优越性能
...
### [231] [The Wisdom of Deliberating AI Crowds: Does Deliberation Improve LLM-Based Forecasting?](https://arxiv.org/abs/2512.22625)
*Paul Schneider,Amalie Schramm*

Main category: cs.AI

TL;DR: LLM相互审阅预测可提升异构模型组预测准确率4%，但对同构模型组无效；额外上下文信息未改善预测


<details>
  <summary>Details</summary>
Motivation: 结构化审议已被证明能提升人类预测者表现，本研究探索类似干预（让LLM在更新前相互审阅预测）是否能提升大语言模型的预测准确性

Method: 使用Metaculus Q2 2025 AI预测锦标赛的202个已解决二元问题，评估四种情景下GPT-5、Claude Sonnet 4.5、Gemini Pro 2.5的准确性：1)异构模型+分布式信息，2)异构模型+共享信息，3)同构模型+分布式信息，4)同构模型+共享信息

Result: 干预在情景2中显著提升准确性，Log Loss降低0.020（相对改善约4%，p=0.017）；同构模型组无改善；意外发现额外上下文信息未提升预测准确性

Conclusion: 审议可能是改善LLM预测的可行策略，但效果取决于模型多样性，且信息汇集机制需要进一步研究
...
### [232] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: DICE是一个两阶段、证据耦合的RAG系统评估框架，通过可解释的推理痕迹和概率评分提高评估的透明度和鲁棒性，同时采用瑞士系统锦标赛将计算复杂度从O(N²)降低到O(N log N)。


<details>
  <summary>Details</summary>
Motivation: 随着RAG系统架构日益复杂，需要可信赖的评估方法。现有标量指标存在可解释性有限、不确定性量化不足、多系统比较计算效率低等问题，阻碍了RAG技术的负责任部署。

Method: DICE采用两阶段框架：结合深度分析推理和概率{A, B, Tie}评分，生成透明、置信度感知的判断；同时使用瑞士系统锦标赛减少计算复杂度，从O(N²)降至O(N log N)。

Result: 在中文金融QA数据集上的验证显示，DICE与人类专家的一致性达到85.7%，显著优于RAGAS等现有LLM评估指标；在八系统评估中计算量减少42.9%，同时保持排名保真度。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信赖的RAG系统评估，支持系统性错误诊断和可操作的改进见解。
...
### [233] [TravelBench: A Real-World Benchmark for Multi-Turn and Tool-Augmented Travel Planning](https://arxiv.org/abs/2512.22673)
*Xiang Cheng,Yulan Hu,Xiangwen Zhang,Lu Xu,Zheng Pan,Xin Li,Yong Liu*

Main category: cs.AI

TL;DR: TravelBench是一个用于评估LLM智能体旅行规划能力的真实世界基准测试，包含多轮交互和工具使用，通过可控沙箱环境提供稳定评估。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划任务在领域覆盖和多轮交互方面有限，无法支持动态用户-智能体交互，因此需要更全面的评估框架来测试LLM智能体的规划能力。

Method: 收集真实场景的用户请求，构建三个子集（多轮、单轮、不可解），建立包含10个旅行领域工具的可控沙箱环境，提供确定性工具输出以确保可靠评估。

Result: 评估了多个LLM在TravelBench上的表现，并对其行为和性能进行了分析，证明该基准能够有效测试LLM智能体的旅行规划能力。

Conclusion: TravelBench为推进LLM智能体在旅行规划领域的发展提供了一个实用且可复现的基准测试框架。
...
### [234] [Memento-II: Learning by Stateful Reflective Memory](https://arxiv.org/abs/2512.22716)
*Jun Wang*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在训练和部署之间存在分离，需要参数更新。本文旨在建立一种无需参数更新的持续适应框架，通过反思机制使智能体能够在交互中学习。

Method: 引入状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（策略评估），读取检索相关历史案例（策略改进）。该过程诱导出增强状态记忆表示的等效马尔可夫决策过程。

Result: 使用熵正则化策略迭代实例化框架并建立收敛保证。当情景记忆增长并充分覆盖状态空间时，所得策略收敛到最优解。

Conclusion: 为基于记忆增强和检索的语言模型智能体提供了理论基础，使其能够在不更新参数的情况下实现持续适应，打破了传统训练与部署的分离。
...
### [235] [HiSciBench: A Hierarchical Multi-disciplinary Benchmark for Scientific Intelligence from Reading to Discovery](https://arxiv.org/abs/2512.22899)
...
<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试过于碎片化，专注于狭窄任务，无法反映真实科学研究的层次性和多学科特性，需要更全面的评估框架来评估大模型在完整科研流程中的能力。

Method: 设计了包含五个层级的层次化基准测试：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8,735个实例，覆盖数学、物理、化学、生物、地理和天文学6个学科，支持文本、公式、图表等多模态输入和跨语言评估。

Result: 对GPT-5、DeepSeek-R1等领先模型进行评估显示：模型在基础素养任务上可达69%准确率，但在发现级挑战上性能急剧下降至25%，揭示了模型在不同科学推理阶段的显著能力差距。
...
### [236] [Geometric Structural Knowledge Graph Foundation Model](https://arxiv.org/abs/2512.22931)
*Ling Xin,Mojtaba Nayyeri,Zahra Makki Nayeri,Steffen Staab*

Main category: cs.AI

TL;DR: Gamma模型通过引入多头几何注意力机制，使用多种代数变换（实数、复数、分裂复数、对偶数）替代单一关系变换，并通过注意力融合机制自适应选择最佳关系表示，显著提升了知识图谱推理的零样本归纳能力。
...
Conclusion: Gamma通过多头几何注意力机制和自适应融合策略，显著提升了知识图谱基础模型的表达能力和泛化性能，为处理多样化关系结构提供了更强大的框架。
...
### [237] [Problems With Large Language Models for Learner Modelling: Why LLMs Alone Fall Short for Responsible Tutoring in K--12 Education](https://arxiv.org/abs/2512.23036)
*Danial Hooshyar,Yeongwook Yang,Gustav Šíř,Tommi Kärkkäinen,Raija Hämäläinen,Mutlu Cukurova,Roger Azevedo*

Main category: cs.AI

TL;DR: 研究发现，在K-12教育中，基于大语言模型（LLM）的辅导系统在评估学生知识演变方面存在准确性和时间一致性问题，无法替代传统的深度知识追踪（DKT）模型，需要混合框架来确保负责任的教学设计。


<details>
  <summary>Details</summary>
Motivation: 针对K-12教育中LLM辅导系统可能替代传统学习者建模的误解，以及欧盟AI法案将该领域列为高风险需要负责任设计，本研究旨在评估LLM在追踪学生知识演变方面的局限性。

Method: 使用大型开放数据集，比较深度知识追踪（DKT）模型与广泛使用的LLM（零样本和微调版本），评估其在预测下一步正确性、准确性和时间一致性方面的表现。

Result: DKT在下一步正确性预测中达到最高区分性能（AUC=0.83），在所有设置中始终优于LLM。微调使LLM的AUC比零样本基线提高约8%，但仍比DKT低6%，且早期序列错误更高。时间分析显示DKT保持稳定、方向正确的掌握度更新，而LLM变体表现出显著的时间弱点。

Conclusion: LLM单独使用不太可能达到现有智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架，特别是在K-12这类高风险教育环境中。
...
### [238] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
TL;DR: 医疗影像领域应用强化学习训练视觉语言模型存在泛化悖论：GRPO方法能提升同分布数据性能但损害跨数据集迁移能力，监督微调在临床部署中可能优于激进强化学习。


<details>
  <summary>Details</summary>
Motivation: 探索在资源受限条件下将强化学习应用于医疗影像领域，研究现有RL方法在提升推理任务性能的同时对跨数据集泛化能力的影响。

Method: 提出ChexReason视觉语言模型，采用R1风格方法（监督微调后接GRPO），仅使用2000个SFT样本、1000个RL样本和单块A100 GPU进行训练。

Result: GRPO在同分布数据上提升23%（CheXpert上macro-F1=0.346），但在跨数据集迁移上下降19%（NIH数据集）。发现泛化悖论：SFT检查点在优化前独特地提升了NIH性能，表明教师引导的推理能捕获更多机构无关特征。

Conclusion: 强化学习范式本身而非模型规模导致泛化问题，结构化推理框架对通用视觉语言模型有益但对医学预训练模型增益有限，临床部署中精心设计的监督微调可能优于激进强化学习。
...
### [239] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
...
<details>
  <summary>Details</summary>
Motivation: DPO及其变体已成为对齐大型语言模型的标准方法，但存在两个根本限制：1）最优策略依赖任意建模选择（标量化函数、参考策略），导致行为反映参数化伪影而非真实偏好；2）孤立处理响应生成未能利用成对数据中的比较信息，未开发模型内在自反思能力。

Method: 提出Intrinsic Self-reflective Preference Optimization (q)方法，推导出同时基于上下文和替代响应的全局最优策略。该方法保证对标量化和参考选择的不变性，可作为即插即用增强，无需架构更改或推理开销。

Result: 实验证明在胜率和长度控制指标上取得一致改进，验证了解锁自反思能力能产生更稳健、更符合人类对齐的LLMs。

Conclusion: q方法优于DPO/RLHF，通过利用成对数据中的比较信息和模型内在自反思能力，解决了DPO的两个根本限制，产生更稳健、更符合人类对齐的语言模型。
...
### [240] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
*Max Parks,Kheli Atluru,Meera Vinod,Mike Kuniavsky,Jud Brewer,Sean White,Sarah Adler,Wendy Ju*

Main category: cs.AI

TL;DR: 本文认为当前评估AI系统情感智能的框架需要改进，因为它们未能全面衡量AI相关的情感智能方面。人类情感智能包含AI缺乏的现象学成分，但AI仍能在感知、解释、响应和适应情感状态方面进行评估。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI情感智能的框架存在不足：1）未能全面衡量AI相关的情感智能方面；2）缺乏对情感本质和情感智能的坚实基础；3）现有基准框架往往只专注于特定任务，缺乏理论深度。

Method: 1）回顾不同情感理论和一般情感智能理论，评估其在人工系统中的适用性；2）批判性评估现有基准框架，根据第一部分建立的情感智能理论识别其不足；3）提出改进评估策略的方案以避免这些缺陷。

Result: 通过理论分析和框架评估，识别了现有情感智能评估框架的主要缺陷：过度依赖人类中心主义的情感概念、忽视AI特有的情感智能维度、缺乏跨文化适应性评估等。

Conclusion: 需要开发更适用于AI系统的情感智能评估框架，这些框架应基于对情感本质的深入理解，同时考虑AI系统的独特能力限制，并包含跨文化适应性等关键维度。
...
### [241] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
...
<details>
  <summary>Details</summary>
Motivation: 当前使用LLM模拟人类行为时，通常将LLM的输出（"模型选择"）视为单个数据点，这种方法未能充分利用LLM固有的概率特性，导致数据利用效率低下。

Method: 提出并形式化"模型信念"概念，从LLM的token级概率中推导出模型对选择替代方案的信念分布。证明模型信念与模型选择的均值渐近等价，但具有更好的统计效率（更低方差、更快收敛速度）。

Result: 在需求估计研究中，模型信念在有限运行次数下能更好地解释和预测真实模型选择，将计算需求减少约20倍以达到足够准确的估计。模型信念成为更有效的估计器。

Conclusion: 模型信念应作为默认测量方法，从LLM生成数据中提取更多信息。该方法比传统模型选择方法更高效，能显著减少计算成本并提高估计精度。
...
TL;DR: 该论文研究了规划与验证领域中的两个问题之间的联系：一致性规划与超属性模型检测，证明了它们之间的紧密关系。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索规划领域中的一致性规划问题与验证领域中的超属性模型检测问题之间的理论联系，这两个问题分别处理不确定环境下的规划执行和信息流安全等系统属性。

Method: 方法包括两个方面：1）将超属性模型检测实例高效地约简为一致性规划实例，并证明该编码的正确性和完备性；2）证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 研究结果表明：1）存在从超属性模型检测到一致性规划的高效约简方法；2）一致性规划问题可以自然地表述为超属性模型检测任务，从而建立了这两个问题之间的双向等价关系。

Conclusion: 结论是模型检测中的∃*∀*超属性与一致性规划问题密切相关，这两个领域的问题可以相互转化，为跨领域的方法和技术迁移提供了理论基础。
...
### [243] [MindWatcher: Toward Smarter Multimodal Tool-Integrated Reasoning](https://arxiv.org/abs/2512.23412)
...
TL;DR: MindWatcher是一个集成交错思维和多模态思维链推理的工具集成推理智能体，能够自主决定是否及如何调用多样化工具，无需人工提示或工作流，在复杂决策任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统基于工作流的智能体在解决需要工具调用的现实问题时表现出有限的智能性，而能够自主推理和工具调用的工具集成推理智能体正成为处理复杂决策任务的有力方法。

Method: MindWatcher采用交错思维范式，使模型能够在任何中间阶段在思考与工具调用之间切换；具备多模态思维链能力，允许在推理过程中操作图像以获得更精确的搜索结果；配备全面的辅助推理工具套件；构建大规模高质量本地图像检索数据库；设计更高效的训练基础设施。

Result: 实验表明MindWatcher通过优越的工具调用能力，匹配甚至超越了更大或更新模型的表现；揭示了智能体训练的关键见解，如智能体强化学习中的遗传继承现象；构建了MindWatcher评估基准（MWE-Bench）用于性能评估。

Conclusion: MindWatcher作为一个工具集成推理智能体，通过交错思维和多模态思维链推理的结合，在复杂多模态问题解决中表现出色，为智能体训练提供了重要见解和高效基础设施。
...
### [244] [AKG kernel Agent: A Multi-Agent Framework for Cross-Platform Kernel Synthesis](https://arxiv.org/abs/2512.23424)
...
Conclusion: AKG kernel agent通过自动化内核开发流程，有效解决了AI系统开发中的计算内核优化瓶颈，支持多DSL和多硬件后端的可移植性，为现代AI工作负载提供了高效的内核生成解决方案。
...
### [245] [Replay Failures as Successes: Sample-Efficient Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2512.23457)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Wenjian Zhang,Min Cen,Yang Zhou,Wenkai Fang,Yiru Zhao,Baisheng Lai,Mingli Song*

Main category: cs.AI

TL;DR: HiR是一种高效的强化学习框架，通过选择-重写策略将失败尝试重放为成功样本，用于复杂指令跟随任务


<details>
  <summary>Details</summary>
Motivation: 强化学习在大型语言模型对齐中面临挑战：初始模型能力有限，难以生成满足所有约束的高质量响应，导致稀疏或难以区分的奖励信号，阻碍学习效率

Method: 提出Hindsight instruction Replay (HiR)框架，采用选择-重写策略：1) 选择部分满足约束的失败尝试；2) 重写这些样本为成功样本；3) 在重放样本和原始样本上进行强化学习，理论框架为指令级和响应级的双重偏好学习

Result: 在多种指令跟随任务上取得显著效果，同时需要更少的计算资源。代码和数据集已开源

Conclusion: HiR通过有效利用失败样本，解决了强化学习中奖励稀疏问题，提高了复杂指令跟随任务的样本效率和性能
...
### [246] [Divergent-Convergent Thinking in Large Language Models for Creative Problem Generation](https://arxiv.org/abs/2512.23601)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成教育问题方面有巨大潜力，但存在"人工蜂群"效应，导致同一模型内和不同模型间生成相似响应，学生可能接触到过度相似和重复的问题，损害思维多样性。

Method: 提出CreativeDC两阶段提示方法，受Wallas创造力理论和Guilford发散-收敛思维框架启发，将LLM推理明确分为不同阶段，解耦创造性探索和约束满足，让LLM在最终确定问题前探索更广泛的想法空间。

Result: CreativeDC在多样性、新颖性和实用性综合评估中，相比基线方法显著提高了多样性和新颖性，同时保持高实用性。扩展分析显示，随着采样增加，CreativeDC生成的有效不同问题数量更大，增长速度更快。

Conclusion: CreativeDC通过结构化两阶段方法有效缓解LLM的"人工蜂群"效应，为教育问题生成提供了更富创造性和多样性的解决方案。
...
### [247] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法通常假设所有客户端共享相同的因果模型，但现实中客户端特定策略（如医院协议）会引入异质且未知的干预，导致假设不现实

Method: 提出I-PERI算法：1) 恢复所有客户端图并集的CPDAG；2) 利用干预在不同客户端诱导的结构差异定向边；3) 得到更紧的Φ-Markov等价类（Φ-CPDAG表示）

Result: 提供I-PERI算法的理论收敛性保证和隐私保护特性证明，并在合成数据上进行实证评估，展示算法有效性

Conclusion: I-PERI算法能够处理联邦设置下未知的客户端级干预，通过利用干预诱导的结构差异缩小等价类，为实际应用中存在异质干预的因果发现提供解决方案
