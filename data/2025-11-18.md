...
TL;DR: 该论文开发了一个分析框架来研究AI能力在有限时间内无限增长的条件，提出了基于可观测数据的可测试标准和可部署的安全控制措施。


<details>
  <summary>Details</summary>
Motivation: 研究AI能力"失控增长"的精确可测试条件，确定在什么可测量条件下能力会在有限时间内无限增长，以及在什么条件下可以排除这种可能性。

Method: 开发递归自我改进的分析框架，将能力增长与资源建设和部署策略联系起来，利用物理和信息论限制定义服务边界，建立内生增长模型耦合资本与计算、数据和能源。

Result: 推导出将可观测序列映射为失控与非奇异行为证书的决策规则，提供了基于改进加速速度相对于当前水平的可证伪测试，以及可直接实施的安全控制措施。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的数据归因方法需要大量计算资源，无法满足实际应用需求，需要开发更高效的归因方法。
...
Method: 通过Delphi流程达成对通用工作流程描述符的共识，开发ColoWorkflow工具，并在多中心腹腔镜和机器人结直肠手术视频数据集上进行应用和验证。

Result: 确定了10个程序无关阶段和34个程序特定步骤，工具在54个手术视频中表现出广泛适用性，评估者间信度中等（阶段K=0.71，步骤K=0.66）。

Conclusion: ColoWorkflow为微创结直肠手术建立了首个基于共识的验证视频评估工具，支持机构间基准测试和AI驱动的工作流程识别发展，有助于标准化培训和改善手术质量。
...
TL;DR: 提出了一种基于LRP的CNN模型剪枝方法，能够在少量数据情况下实现更高的剪枝率同时保持更好的模型精度。
...
Method: 使用Layer-wise Relevance Propagation（LRP）技术进行模型剪枝，在少量数据情况下实现剪枝，不需要微调过程。

Result: 该方法在保持模型精度方面优于现有方法，实现了更高的剪枝率。

Conclusion: 提出的剪枝方法在数据有限场景下具有更好的实用价值，能够在保持精度的同时实现更高的压缩率。
...
TL;DR: 本文提出了一种短窗口滑动学习框架，用于CCTV监控视频的实时暴力检测。该方法将视频分割为1-2秒的短片段，利用大语言模型自动标注构建细粒度数据集，在RWF-2000数据集上达到95.25%准确率，在UCF-Crime长视频数据集上提升至83.25%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统长视频训练方法难以精确识别快速发生的暴力事件，且缺乏细粒度标注数据。需要开发能够实时检测、充分利用时间连续性的暴力检测方法。

Method: 采用短窗口滑动学习框架，将视频分割为1-2秒短片段；使用大语言模型进行自动标注构建细粒度数据集；充分利用所有帧保持时间连续性。

Result: 在RWF-2000数据集上达到95.25%准确率；在UCF-Crime长视频数据集上显著提升至83.25%准确率；证明了方法的强泛化能力和实时适用性。

Conclusion: 短窗口滑动学习框架能够有效识别快速暴力事件，在智能监控系统中具有优秀的实时检测性能和泛化能力。
...
TL;DR: 提出一种竞争性且计算高效的降雨临近预报方法，使用预训练卫星视觉编码器结合轻量级概率头来预测4小时累积降雨量的离散经验分布函数。
...
Method: 使用预训练的卫星视觉编码器(DINOv3-SAT493M)结合视频投影器(V-JEPA Vision Transformer)和轻量级概率头，通过连续排序概率得分(CRPS)进行端到端优化。同时使用带有聚合排序概率得分和逐像素Gamma-Hurdle目标的3D-UNET作为基线模型。

Result: 在Weather4Cast 2025基准测试中，该方法取得了CRPS为3.5102的优异性能，相比最佳3D-UNET模型实现了约26%的有效性提升。

Conclusion: 所提出的方法在降雨临近预报任务中表现出色，显著优于传统的3D-UNET基线模型，证明了其竞争力和计算效率。
...
TL;DR: PhaseWin是一种新颖的阶段窗口搜索算法，用于对象级基础模型的可信区域归因，通过分阶段粗到细搜索实现近线性复杂度，在保持高可信度的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决基于子模子集选择的归因方法在效率上的局限性，这些方法虽然可信度高但计算成本大，阻碍了在实际场景中的部署应用。

Method: 采用分阶段粗到细搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，替代传统的二次成本贪婪选择，以近似贪婪行为。

Result: PhaseWin仅使用20%的计算预算就实现了超过95%的贪婪归因可信度，在对象检测和视觉定位任务中始终优于其他归因基线方法。

Conclusion: PhaseWin为对象级多模态模型建立了可扩展、高可信度归因的新技术标准，在保持理论近似保证的同时显著提升了效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在OOD检测中使用的负提示通常包含广泛的非ID特征，可能捕获重叠或误导性信息，导致次优结果。

Method: 使用LLM初始化类别特定的正负提示，优化后正提示关注类内特征，负提示突出类别边界特征，并通过图架构聚合语义监督传播到视觉分支。
...
TL;DR: 本研究评估了YOLOv11n和YOLOv12n两种轻量级模型在面部表情识别任务中的表现，发现在干净数据集上YOLOv12n性能更优，而在噪声环境中YOLOv11n精度更高。


<details>
  <summary>Details</summary>
Motivation: 在无约束真实环境中进行面部表情识别具有挑战性，需要平衡模型性能和计算效率。

Method: 将FER2013和KDEF两个分类数据集转换为目标检测格式，使用YOLOv11n和YOLOv12n模型在统一检测分类框架下进行测试，评估指标包括mAP 0.5、精确率、召回率和混淆矩阵。

Result: YOLOv12n在干净KDEF数据集上获得95.6的mAP 0.5，在FER2013上mAP为63.8；YOLOv11n在FER2013上精确率达65.2，误报更少。两个模型在FER2013上对视觉相似表情存在更多混淆。
...
TL;DR: 本文提出了一种名为异构互补蒸馏（HCD）的新框架，用于解决异构架构（如ViT到ResNet18）知识蒸馏中的特征表示差异问题。通过整合教师和学生的互补特征，在共享logits中对齐表示，并引入子logit解耦蒸馏和正交性损失来促进多样化知识转移。
...
Method: 提出HCD框架：通过卷积投影器和自适应池化处理学生中间特征，与教师倒数第二层特征拼接，通过互补特征映射器（CFM）生成共享logits。引入子logit解耦蒸馏（SDD）将共享logits划分为n个子logits，与教师logits融合修正分类。使用正交性损失（OL）确保子logit多样性和减少冗余知识转移。
...
TL;DR: FedBCS通过领域不变上下文原型对齐解决联邦学习中的特征异构问题，提出频率域自适应风格重校准和上下文感知双级原型对齐方法，显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中不同医疗机构的扫描设备或协议差异导致特征异构，现有方法主要关注最终层特征而忽略多层级线索，且中间层的风格偏差累积会降低模型鲁棒性。

Method: 提出FedBCS方法：1）频率域自适应风格重校准，解耦内容-风格表示并学习最优风格参数；2）上下文感知双级原型对齐，从编码器和解码器的不同层提取领域不变原型并与上下文信息融合。

Result: 在两个公共数据集上的大量实验表明，该方法表现出卓越的性能。

Conclusion: FedBCS通过领域不变上下文原型对齐有效解决了联邦学习中的特征异构问题，提升了模型的鲁棒性和分割精度。
...
TL;DR: 本文提出DEFT-LLM方法解决微表情识别中的两个核心挑战：静态外观与动态运动线索的纠缠，以及文本标签与面部肌肉运动之间的语义鸿沟。通过多专家解耦实现运动语义对齐，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情绪至关重要。现有方法存在两个问题：静态外观和动态运动线索纠缠导致模型难以关注细微运动；现有数据集的文本标签与面部肌肉运动不完全对应，存在语义鸿沟。

Method: 提出DEFT-LLM方法，采用多专家解耦架构。首先构建Uni-MER运动驱动指令数据集，利用光流和动作单元标签的双重约束确保时空一致性和运动对应性。设计三个专家分别解耦面部动态为结构、动态纹理和运动语义的独立可解释表示。

Result: 在多个具有挑战性的微表情识别基准测试中展示了最先进的性能，在局部面部运动的可解释建模方面具有特殊优势。

Conclusion: DEFT-LLM通过将Uni-MER的指令对齐知识集成到模型中，为微表情注入了有效的物理先验，同时利用大语言模型的跨模态推理能力，实现了细微情绪线索的精确捕捉。
...
TL;DR: ERMoE提出了一种新的稀疏专家混合模型，通过特征向量基重参数化专家，并使用特征向量评分进行路由选择，解决了传统MoE模型中的路由不稳定和负载不均衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构面临两个核心挑战：路由器logits与专家内部结构不对齐导致路由不稳定和专家利用不足，以及负载不均衡造成的瓶颈问题。标准解决方案如辅助负载均衡损失会削弱专家专业化并损害下游性能。

Method: ERMoE在学习的正交特征基中重参数化每个专家，用'特征向量评分'（输入特征与专家基的余弦相似度）替代学习的门控logits。这种内容感知路由将token分配直接与专家的表示空间关联，稳定了利用率并促进可解释的专业化。

Result: ERMoE在ImageNet分类和跨模态图像-文本检索基准（如COCO、Flickr30K）上达到最先进精度，同时自然产生更平坦的专家负载分布。其3D MRI变体（ERMoE-ba）将脑年龄预测准确率提高超过7%，并产生解剖学上可解释的专家专业化。

Conclusion: ERMoE引入了一种新的稀疏专家模型架构原则，直接解决路由不稳定性，并在保持稀疏性的同时实现可扩展、可解释的专业化和改进的性能。
...
TL;DR: 本文提出了DMC和DMC-OT两种方法，用于解决基于CLIP的类增量学习中的分类器偏差和分布漂移问题。


<details>
  <summary>Details</summary>
Motivation: 虽然CLIP等视觉语言模型在跨领域泛化方面表现出色，但将其扩展到持续学习场景仍面临挑战。特别是为新类别学习任务特定的软提示会导致严重的分类器偏差，因为文本原型在没有先前数据时会过拟合到最近的类别。

Method: 提出了DMC框架，将视觉编码器适配和文本软提示优化解耦为两个阶段，每个阶段训练时另一个被冻结。还提出了DMC-OT增强版本，引入最优传输引导的校准策略来对齐演化编码器间的记忆统计量，以及任务特定的提示设计来增强任务间可分性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的广泛实验表明，DMC和DMC-OT都实现了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: DMC框架通过解耦视觉和文本模态的适配，有效解决了CLIP在类增量学习中的挑战，而DMC-OT通过最优传输校准进一步提升了性能。
...
TL;DR: 本文提出Phase Aggregated Smoothing (PAS)方法，通过多相位聚合平滑时间核，解决视频LLMs中的时间不一致性问题，无需训练即可提升模型对时间偏移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小偏移会翻转注意力并抑制相关帧。这种不稳定性源于多模态RoPE扩展导致的逆傅里叶时间核中的帧尺度波纹。

Method: 提出PAS方法：在不同注意力头应用相反的相位偏移，然后聚合它们的输出。该方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核，降低相位敏感性而不改变位置编码结构。

Result: 在多个视频理解基准测试中，在相同token预算下表现出持续改进，计算开销可忽略不计。

Conclusion: PAS为视频LLMs提供即插即用的鲁棒时间编码升级方案，通过平滑时间核实现对小时间偏移的Lipschitz稳定注意力。
...
TL;DR: 提出了一种无需训练的二值验证工作流程，用于零样本视觉任务，通过量化和二值化两个步骤将开放性问题转换为多项选择题和真/假验证问题。


<details>
  <summary>Details</summary>
Motivation: 旨在通过推理时设计而非任务特定训练来提升现成视觉语言模型在零样本视觉任务中的性能，提供一种实用且通用的解决方案。

Method: 工作流程包含两个步骤：(1) 量化：将开放性问题转换为具有明确候选答案的多项选择题；(2) 二值化：对每个候选答案提出真/假问题，通过确定性解析选择最终答案。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，量化到多项选择题带来显著提升，真/假二值化进一步提供一致性的性能提升。

Conclusion: 该工作流程通过推理时设计强调简单统一的方法，为当前视觉语言模型提供了实用的零样本视觉能力增强路径，并建立了从开放性问题到多项选择题再到真/假验证的难度阶梯理论。
...
TL;DR: CLUE是一个生成模型框架，能够在固定格式提示下实现多样化的图像生成，同时保持稳定性，无需额外数据。基于Stable Diffusion架构，通过风格编码器和新的注意力层实现连续潜在空间表示。


<details>
  <summary>Details</summary>
Motivation: 解决专业领域（如医学）中数据有限且类型不足的问题，现有方法在通用领域表现良好但在专业领域受限。

Method: 基于Stable Diffusion架构，引入风格编码器处理图像和提示生成风格嵌入，通过新的第二注意力层和KL散度实现连续高斯区域潜在空间表示。

Result: 在中耳炎数据集上，FID从46.81降至9.30，召回率从49.60%提升至70.29%。仅使用合成数据训练的模型在1000%规模下F1得分达83.21%（vs 73.83%）。合成与真实数据结合使用F1得分达94.76%。
...
TL;DR: PROMISE是一个新颖的多模态框架，通过提示注意力层次对比学习来解决模态缺失问题，显著提升跨模态表示的一致性。
...
Method: PROMISE创新地将多模态提示学习融入层次对比学习框架，配备专门设计的提示注意力机制，动态生成针对缺失模态场景的鲁棒且一致的表示。

Result: 在基准数据集上的广泛实验和全面的消融研究明确证明，PROMISE相比当前最先进的多模态方法具有优越性能。

Conclusion: PROMISE框架通过提示注意力层次对比学习有效弥合完整数据与不完整数据之间的表示差距，显著提升了模态缺失场景下的跨模态表示鲁棒性。
...
TL;DR: EmoVid是首个专为创意媒体设计的多模态情感标注视频数据集，包含卡通动画、电影片段和动画贴纸，通过情感标注和视觉属性分析揭示了视频情感感知的时空模式，并开发了基于Wan2.1模型的情感条件视频生成技术，显著提升了生成视频的质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成系统主要关注低层次视觉指标而忽视情感维度，视频社区缺乏专门资源来连接情感理解与生成任务，特别是在风格化和非现实情境下。

Method: 引入EmoVid数据集，包含情感标签、视觉属性和文本标注；通过系统分析发现视觉特征与情感感知的时空模式；基于这些洞察开发情感条件视频生成技术，对Wan2.1模型进行微调。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在复杂视觉任务中存在"视觉处理瓶颈"，容易丢失视觉证据的关联性，缺乏情境化的视觉体验。受人类认知记忆理论启发，研究者希望为模型添加类似人类记忆的动态视觉记忆机制。

Method: 提出VisMem框架，包含两个核心模块：短期记忆模块用于保持细粒度感知信息，长期记忆模块用于抽象语义整合。这些记忆在推理过程中被动态调用，确保模型在思考和生成过程中保持感知保真度和语义一致性。

Result: 在多个视觉基准测试（理解、推理、生成）上的广泛实验表明，VisMem相对于原始模型实现了11.8%的平均性能提升，并且优于所有对比方法。

Conclusion: VisMem建立了一种新的潜在空间记忆增强范式，通过认知对齐的视觉记忆机制有效解决了视觉语言模型的视觉处理瓶颈问题。
...
TL;DR: SP-Guard是一种改进扩散模型安全性的方法，通过估计提示词危害性并应用选择性引导掩码，仅针对图像中的不安全区域进行引导，在保持图像质量的同时提高安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时引导方法缺乏自适应性和选择性，无法根据提示词调整引导强度，也无法仅针对图像中的不安全区域进行引导，这限制了扩散模型生成内容的安全性。

Method: SP-Guard通过估计提示词危害性并应用选择性引导掩码，仅对图像中的不安全区域进行引导，实现了自适应和选择性的安全控制。

Result: 实验表明，SP-Guard比现有方法生成更安全的图像，同时最大限度地减少了意外内容改变，在安全性和图像质量之间取得了更好的平衡。

Conclusion: 除了提高安全性外，研究结果还强调了图像生成中透明度和可控性的重要性，SP-Guard为实现更安全的AI图像生成提供了有效解决方案。
...
TL;DR: 提出了Phys-Liquid数据集，包含97,200张模拟图像和对应的3D网格，用于解决透明变形液体在容器运动下的几何和体积估计挑战。


<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器运动会导致液体表面变形，使液体状态评估复杂化，现有数据集缺乏涵盖真实液体行为的物理模拟数据。

Method: 引入物理信息数据集Phys-Liquid，包含多个实验室场景、光照条件、液体颜色和容器旋转下的液体动态；提出四阶段重建和估计流程：液体分割、多视角掩码生成、3D网格重建和真实世界缩放。
...
Method: 利用小波的完美重构特性防止信息退化，同时选择性抑制冗余特征，作为即插即用解码器块集成到各种U-Net变体中，消除其固有的重构瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估多智能体协作感知的基准，而多无人机系统相比单传感器具有更好的覆盖范围、鲁棒性和协作能力。现有基准主要针对基础感知任务，无法评估复杂协作场景下的模型表现。
...
TL;DR: EmbryoDiff是一个基于扩散模型的两阶段框架，用于体外受精胚胎发育阶段的精细识别，通过多焦点特征融合和混合语义边界条件实现高精度分类。
...
Method: 采用两种缓存策略：1）基于特征重用的粗粒度缓存（块级），在模型推理的每一步动态绕过编码器-解码器块中的冗余计算；2）细粒度缓存（提示级），在模块内重用连续推理步骤中的交叉注意力图，并将其扩展到相邻步骤的相应模块计算。

Result: 在四个基准数据集上验证了HGC的有效性，特别是在COCO-Stuff分割基准上，计算成本（MACs）显著降低63%（从18.22T降至6.70T），同时语义保真度损失控制在1.5%以内。

Conclusion: HGC方法能够在保持视觉质量的同时显著提升可控生成模型的生成效率，有效平衡了生成效率和视觉质量。
...
TL;DR: CrossMed是一个评估医学多模态大语言模型组合泛化能力的基准，使用模态-解剖-任务(MAT)框架，将4个公共数据集转化为统一的视觉问答格式，包含20,200个多选QA实例。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集转化为统一的VQA格式，评估LLaVA-Vicuna-7B和Qwen2-VL-7B在相关、不相关和零重叠MAT分割上的表现。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降。分割任务在仅使用分类数据训练时性能提升7% cIoU。
...
TL;DR: 本文提出了一种多参考生成式图像合成方法，通过校准前景参考图像的全局和局部特征，使其与背景信息兼容，从而在保持细节的同时调整前景姿态和视角。


<details>
  <summary>Details</summary>
Motivation: 现有生成式图像合成方法在同时保持细节和调整前景姿态/视角方面存在困难，需要解决参考特征与背景信息不兼容的问题。
...
TL;DR: LiteAttention通过利用扩散注意力中稀疏模式的时序一致性，在去噪过程中跳过非必要计算，显著加速视频扩散变换器而不损失质量。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散变换器加速方法面临基本权衡：动态估计稀疏注意力模式计算开销大且存在估计误差，而静态稀疏模式在去噪过程中固定且通常次优。

Method: 利用扩散注意力稀疏模式在去噪步骤间具有强时序一致性的特性，通过早期标记非必要tile并向前传播跳过决策，消除冗余注意力计算。

Result: 在FlashAttention基础上实现高度优化的LiteAttention内核，在生成视频扩散模型上实现显著加速，且不降低质量。

Conclusion: LiteAttention结合了动态方法的自适应性和静态方法的效率，通过利用时序一致性实现高效加速，代码将公开发布。
...
TL;DR: 这篇论文系统综述了2016-2025年间糖尿病视网膜病变（DR）检测的深度学习研究进展，整合了50多项研究和20多个数据集，重点分析了自监督学习、领域泛化、联邦学习等技术，并讨论了临床部署的挑战。
...
Method: 采用系统性综述方法，分析50多项研究和20多个数据集，考察自监督学习、半监督学习、领域泛化、联邦训练和混合神经符号模型等技术进展，同时评估协议、报告标准和可重复性挑战。

Result: 建立了跨数据集的性能基准表，发现研究在解决类别不平衡、标签稀缺、领域偏移和可解释性方面取得显著进展，但仍存在多中心验证和临床信任方面的差距。

Conclusion: 这项工作为可重复、保护隐私且可临床部署的DR AI制定了实用议程，同时指出所调查的创新技术可广泛扩展到大规模医学影像领域。
...
TL;DR: 本文提出了S2D-Align方法，通过从粗到深的渐进式对齐策略，利用不同粒度的辅助信号来建立解剖学基础的对齐，从而提升放射学报告生成的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告生成方法主要依赖于监督微调进行图像-文本对的实例级对齐，但无法建立解剖学基础的对齐，导致生成质量不理想。

Method: 提出S2D-Align方法，采用从粗到深的策略：从粗粒度的放射图像-报告配对开始，引入参考报告进行实例级指导，最后使用关键短语将生成过程锚定到具体的解剖细节。通过基于记忆的适配器实现特征共享。
...
Method: 比较两种最有前景的生成模型——去噪扩散概率模型和自回归因果变换器，将它们适配用于生成形状建模和补全任务，包括基线判别模型和广泛的消融研究。

Result: 扩散模型在连续潜在空间中优于判别模型和自回归方法，在真实条件下从单个噪声深度图像进行多模态形状补全方面达到了最先进的性能；在相同离散潜在空间中，自回归模型能够匹配或超越扩散模型在这些任务上的表现。

Conclusion: 扩散模型在连续潜在空间中表现最佳，而自回归模型在离散潜在空间中也具有竞争力，两种方法在不同条件下都能提供优秀的生成性能。
...
TL;DR: SaTformer是一个基于全时空注意力的视频变换器，能够从卫星辐射数据中熟练预测极端降水。该模型将降水回归问题重新表述为分类问题，并使用类别加权损失来解决标签不平衡问题，在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报模型在临近预报时间尺度（0-4小时）性能下降，且计算成本高昂。AI天气预测方法虽然取得了成功，但视频理解架构在天气预报中的应用仍未被充分探索。

Method: 提出SaTformer：基于全时空注意力的视频变换器架构，将降水回归重新表述为分类问题，并使用类别加权损失处理长尾数据集中的标签不平衡问题。

Result: 该模型在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名。

Conclusion: SaTformer通过视频变换器架构和创新的分类方法，成功解决了极端降水预测问题，为AI天气预测提供了新的有效方法。
...
TL;DR: VideoP2R是一个面向视频语言模型的过程感知强化微调框架，通过将感知和推理建模为独立过程来提升视频推理能力，在7个基准测试中的6个上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 将强化微调(RFT)框架扩展到大型视频语言模型(LVLMs)面临挑战，需要专门的方法来提升视频推理能力。

Method: 采用两阶段框架：SFT阶段生成高质量的过程感知链式思维数据集VideoP2R-CoT-162K；RL阶段提出过程感知组相对策略优化(PA-GRPO)算法，为感知和推理提供独立奖励。
...
<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，区分真实与合成内容变得困难，深度伪造等技术可能被滥用于传播错误信息、侵犯隐私和欺诈，因此需要开发有效的检测方法。

Method: 通过系统回顾24篇最新研究，分析每项研究的贡献和弱点，总结现有方法的共同局限性，并提出多模态深度学习模型作为解决方案。

Result: 现有检测模型（如CNN和ViT）在泛化到未见数据和处理不同模型生成的内容方面存在困难，对多模态数据和高度修改内容的检测效果不佳。
...
TL;DR: 本文提出了Hindsight Distilled Reasoning (HinD)框架和Knowledge Encouragement Preference Optimization (KEPO)，用于激发多模态大语言模型的内在知识推理能力，解决KBVQA中推理过程不明确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有KBVQA方法要么使用多模态大语言模型的隐式知识，要么通过检索增强生成显式知识，但它们的推理过程仍然隐式，缺乏明确的推理轨迹。

Method: 1. 提出HinD框架，利用冻结的7B大小MLLM构建Hindsight-Zero训练数据；2. 自蒸馏为Chain-of-Thought生成器和知识生成器；3. 使用KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA上的实验验证了HinD的有效性，使用7B大小MLLM的推理能力实现了优越性能，无需商业模型API或外部知识。

Conclusion: HinD框架成功激发了MLLM的内在知识推理能力，通过明确的推理轨迹和优化的知识生成，在KBVQA任务中取得了优异表现。
...
TL;DR: OT-ALD是一种基于最优传输理论的图像到图像翻译框架，解决了DDIB方法在翻译效率和潜在分布不匹配方面的问题，通过OT映射改进翻译过程，在保持图像质量的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: DDIB方法虽然具有循环一致性和灵活性，但面临翻译效率低和潜在分布不匹配导致的轨迹偏差问题，需要改进这些关键挑战。
...
Conclusion: OT-ALD有效平衡了快速图像翻译与图像质量提升，通过最优传输理论解决了DDIB方法的关键问题，实现了高效高质量的图像翻译。
...
TL;DR: 本文提出了一种基于混响变换的轨迹预测模型Rev，通过可学习的混响核显式建模和预测智能体对轨迹变化事件的响应延迟，实现可控的轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 当前轨迹预测方法难以显式学习和预测智能体对轨迹变化事件的响应延迟，这种延迟的缺失会影响预测系统的因果连续性并导致不合理的轨迹。

Method: 受声学中混响曲线的启发，提出混响变换和Rev模型，使用两个显式可学习的混响核来模拟和预测每个智能体的不同延迟偏好及其随机性。

Result: 在多个数据集上的实验表明，Rev在实现竞争性精度的同时，揭示了跨智能体和场景的可解释延迟动态。
...
Method: 提出MultiTypeFCDD框架，使用图像级标签学习生成多通道热图，每个通道对应特定异常类型，作为单一统一框架能够跨多个对象类别区分异常类型。
...
TL;DR: CATS-V2V是首个针对复杂不利交通场景的V2V协同感知真实世界数据集，包含10种天气和光照条件下的100个片段，提供60K帧LiDAR点云和1.26M多视角图像数据，支持自动驾驶研究。
...
Method: 使用两辆硬件时间同步的车辆收集数据，涵盖10种天气和光照条件，提供LiDAR点云、多视角相机图像、高精度GNSS/IMU记录，并提出基于目标的时序对齐方法确保多模态数据精确对齐。

Result: 构建了包含60K帧LiDAR点云、1.26M相机图像、750K GNSS/IMU记录的数据集，提供时间一致的3D边界框标注和静态场景4D BEV表示。

Conclusion: CATS-V2V是目前同类数据集中规模最大、支持最全面、质量最高的数据集，将为自动驾驶社区在相关任务中提供重要价值。
...
### [41] [Refine and Align: Confidence Calibration through Multi-Agent Interaction in VQA](https://arxiv.org/abs/2511.11169)
*Ayush Pandey,Jai Bardhan,Ishita Jain,Ramya S Hebbalaguppe,Rohan Raju Dhanakshirur,Lovekesh Vig*

Main category: cs.CV

TL;DR: AlignVQA是一个基于辩论的多智能体框架，通过专门化视觉语言模型的协作和辩论过程来改善视觉问答系统的校准性能，减少过度自信问题。


<details>
  <summary>Details</summary>
Motivation: 现代视觉问答系统在医疗诊断和自主导航等高风险领域应用日益广泛，但其置信度估计的可靠性研究不足，系统往往产生过度自信的响应，这影响了系统的可信度。

Method: 提出AlignVQA多智能体框架，使用不同提示策略的专门化视觉语言模型生成候选答案，通过两阶段交互（通用智能体的批判、精炼和聚合）进行辩论，并引入可微分的校准感知损失函数aligncal来微调专门化智能体。

Result: 在多个基准VQA数据集上的实证结果表明，该方法显著减少了校准差异，更校准的专门化智能体产生了更好对齐的置信度。

Conclusion: 基于辩论的多智能体框架和校准感知损失函数能够有效改善视觉问答系统的置信度校准，提高系统在不确定性条件下的决策可靠性。
...
TL;DR: 提出了一种用于非同步多视角视频4D高斯溅射重建的时间对齐策略，通过粗到精的对齐模块估计和补偿相机时间偏移，提高重建质量。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多视角视频常因相机触发延迟或独立录制设置导致时间不同步，影响4D高斯溅射的重建质量，需要解决这一时间不对齐问题。

Method: 采用粗到精对齐模块，先估计帧级粗偏移，再细化到子帧精度，可作为即插即用模块集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时间不对齐视频，显著提升基线方法的性能。

Conclusion: 提出的时间对齐策略增强了4DGS处理异步数据的鲁棒性，为多视角视频重建提供了实用解决方案。
...
TL;DR: Viper-F1是一种混合状态空间视觉语言模型，使用液态状态空间动力学替代注意力机制，结合令牌-网格相关模块实现高效细粒度视觉理解。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在资源受限场景中计算成本高的问题，以及小模型在细粒度视觉区域捕捉能力不足的问题。

Method: 使用液态状态空间动力学替代Transformer交叉注意力，提出令牌-网格相关模块计算文本令牌与图像块之间的轻量级相关性，并通过FiLM条件调节状态空间动力学。

Result: 在多个基准测试中，Viper-F1实现了准确、细粒度的理解，同时显著提高了效率。

Conclusion: Viper-F1通过状态空间模型和视觉接地增强，在保持线性时间推理的同时实现了高效的细粒度视觉语言理解。
...
TL;DR: 本文提出了一个高效框架，用于印度次大陆及周边地区PM1、PM2.5和PM10的6小时临近预报，使用CAMS全球大气成分预报数据，开发了三个轻量级参数特定架构。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对有限空间域的紧凑深度学习模型，以提高短期污染物预报的准确性，减少系统偏差，并实现快速推理。

Method: 利用CAMS全球大气成分预报的0.4度分辨率分析场作为输入，采用256x256空间区域覆盖28.4S-73.6N和32E-134.0E，预测中心128x128区域（2.8S-48N和57.6E-108.4E），训练三个轻量级参数特定架构。

Result: 使用RMSE、MAE、Bias和SSIM评估显示，相比Aurora基础模型取得了显著性能提升，证明了紧凑专门化深度学习模型在有限空间域短期预报中的有效性。

Conclusion: 紧凑专门化的深度学习模型在有限空间域的短期污染物预报中表现出色，能够显著提高准确性并减少系统偏差。
...
TL;DR: 本文提出了一个基于卷积门控循环单元(ConvGRU)的迁移学习框架，用于Weather4Cast 2025竞赛中的短期降雨预测。使用单一SEVIRI红外通道作为输入，采用两阶段训练策略生成最多4小时前的降雨估计。
...
Result: 思维链监督比直接基线准确率提高34.9%，直接偏好优化进一步提升了准确性和推理质量。

Conclusion: 该系统通过实现更丰富的地理空间推理，推动了多光谱地球观测的视觉问答技术发展，使其在气候用例中更加有效。
...
TL;DR: 本研究首次系统性地评估了视觉语言模型对良性视觉和文本扰动的鲁棒性，发现即使是最先进的模型也对微小扰动高度敏感，且稳定性与正确性密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型取得了显著进展，但其在保持语义不变的小规模输入变化下的可靠性仍未被充分理解，需要系统评估其鲁棒性。

Method: 通过像素级偏移、几何变换、缩放填充、改写和多语言重写等五种良性扰动，在多个模型和数据集上进行大规模系统性测试。

Result: 现代VLM对微小扰动高度敏感，大部分样本至少会在一种扰动下改变预测答案；样本稳定性与正确性强相关，稳定样本更可能被正确回答。
...
TL;DR: 本文提出了首个3D视觉领域的一对多后门攻击框架，基于可配置的球形触发器，能够在单一触发器中编码多个目标类别。


<details>
  <summary>Details</summary>
Motivation: 现有的3D点云后门攻击仅限于刚性的一对一范式，而安全敏感的3D应用领域需要更复杂的攻击模型来评估系统安全性。

Method: 利用球体的空间特性作为参数空间，通过可配置的球形触发器实现一对多后门攻击，使中毒模型能将不同的触发器配置映射到不同的目标标签。

Result: 在多个数据集和模型架构上的实验验证了该方法的有效性，攻击成功率高达100%，同时保持对干净数据的准确率。
...
<details>
  <summary>Details</summary>
Motivation: 医学成像领域数据稀缺，为每个领域、模态或任务单独预训练模型具有挑战性。需要一种统一且可扩展的框架来避免构建单独的模型。
...
Conclusion: 配备模块化组件的基础模型不受初始训练范围限制，可以演变为医学成像的多任务、多模态系统。
...
Method: 提出了GSD框架，基于Score Distillation Sampling (SDS)思想，通过监督多个相邻视图的渲染图像，引导高斯溅射表示向VDM的生成方向发展。引入了统一的指导形式，包括基于真实深度图的深度扭曲指导和基于语义图像特征的指导，确保VDM的分数更新方向与正确的相机姿态和准确几何对齐。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: GSD框架有效解决了3DGS在稀疏视图下的过拟合问题，通过利用VDM的多视图一致性先验，显著提升了3D场景表示的质量。
...
TL;DR: 本文研究了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中，发现文本编码器偏向输入开头，而图像编码器在开头和结尾都表现出偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管位置偏见在文本生成模型中被广泛研究，但在表示模型特别是多模态模型中的存在和影响仍未充分探索。

Method: 区分上下文重要性和位置偏见，评估不同模型和数据集中的位置偏见存在程度，分析偏见来源。

Result: 多模态模型中普遍存在位置偏见，文本编码器偏向输入开头，图像编码器在开头和结尾都表现出偏见。偏见源于位置编码方案、训练损失、上下文重要性等多因素组合。

Conclusion: 多模态模型存在明显的位置偏见，且在不同模态中表现不同，这种偏见由多种训练和架构因素共同导致。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D高斯泼溅模型在渲染细微连续视线变化时的困难，提高视线重定向的保真度，为视线估计器生成更高质量的增强数据。

Method: 结合扩散变换器(DiT)、跨视线角度的弱监督策略和正交约束损失，使用合成生成的中间视线角度提供平滑的视线方向流形，数学上强制视线、头部姿态和表情的内部表示解耦。

Result: 在感知质量和重定向精度方面达到新的最先进水平，将最先进的视线误差降低了4.1%至6.353度。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D深度学习在多领域泛化受限，主要由于数据集规模有限和多源点云的高度异构性。不同传感器采集的点云在密度和噪声分布上存在显著差异，导致多领域融合时出现负迁移。现有方法通常只关注领域感知或领域通用特征，忽略了它们之间的协同潜力。

Method: 提出DoReMi框架：1）领域感知专家分支通过领域引导空间路由动态激活，实现上下文感知的专家选择；2）熵控制动态分配确保稳定高效的专家利用；3）通过鲁棒多属性自监督学习预训练的冻结统一表示分支，保持跨领域几何和结构先验。

Result: 在多个3D理解基准测试中表现优异：ScanNet Val上达到80.1% mIoU，S3DIS上达到77.2% mIoU，相比现有方法具有竞争力或更优性能。

Conclusion: DoReMi展示了作为未来3D理解研究基础框架的强大潜力，通过协同学习专业化和泛化知识，有效解决了多源点云的异构性问题。
...
TL;DR: 本文提出了一种新颖的少样本风格编辑框架，通过参数高效的MoE LoRA方法和创新的路由机制，使用有限配对数据微调通用图像编辑模型以适应新风格。
...
Method: 提出参数高效的MoE LoRA方法，包含风格特定和风格共享路由机制；采用度量引导方法自动确定各层最优秩；探索DiT模型中LoRA的最佳插入位置；集成对抗学习和流匹配指导扩散训练。

Result: 实验结果表明，所提方法在显著减少LoRA参数的情况下，优于现有最先进方法。

Conclusion: 该框架能够有效解决少样本风格编辑问题，通过创新的路由机制和参数优化策略实现了更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解真实世界3D空间智能方面存在根本性困难，这源于输入阶段的几何感知编码器与2D特征之间的冲突，以及输出阶段离散分词器无法生成精确连续数值的双重瓶颈。

Method: 引入GEODE架构，包含两个即插即用模块：解耦推理模块(DRM)作为空间协处理器，通过交叉注意力对齐3D数据与2D视觉特征；直接回归头(DRH)采用"嵌入即值"范式，通过轻量级MLP精确回归标量和3D边界框。

Result: 1.5B参数的GEODE模型作为高级语义分发器，在空间推理任务上达到了最先进的性能水平，可与7B+模型相媲美。

Conclusion: GEODE通过解耦3D推理和数值生成的双模块设计，成功突破了现有视觉语言模型在3D空间理解上的瓶颈，为3D空间智能提供了有效的解决方案。
...
TL;DR: 本文提出了一种用于自闭症儿童注视目标检测的新框架SACF，通过利用社交上下文解决数据集类别不平衡问题，在首个自闭症注视目标数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 通过AI自动检测自闭症儿童的注视目标具有重要影响，特别是对那些缺乏专业帮助的儿童。该任务对于构建测量共同注意的自动化系统至关重要。

Method: 提出了社交感知的粗到细(SACF)注视检测框架，采用双路径架构，分别处理社交和非社交注视，由上下文感知门模块指导，专门解决自闭症数据集中常见的类别不平衡问题。

Result: 综合实验结果表明，该框架在自闭症人群的注视目标检测中达到了新的最先进性能，显著优于现有方法，特别是在关键的面部注视少数类上表现优异。

Conclusion: SACF框架通过有效利用社交上下文，成功克服了自闭症数据集中的类别不平衡问题，为自闭症儿童的注视目标检测提供了有效的解决方案。
...
Method: 基于模型内部存在潜在数值正确性概念的观察，提出CountSteer方法，在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练。
...
TL;DR: 本文提出了一种新的视觉语言模型，通过将caption tokens分组来获取细粒度的语言表示，并与经过训练发现对象的图像编码器输出对齐，从而提升模型对视觉和语言的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型主要关注图像块与语言标记的对齐，但图像块对人类视觉没有意义，单个标记也不一定携带可接地信息。需要将标记分组来描述场景的不同方面，以获得对现实世界更好的细粒度理解。

Method: 提出了一种模型架构，在模型中自动对caption tokens进行分组，以捕获语言的细粒度表示。这些表示与经过训练发现对象的图像编码器输出进行对齐，期望达到图像中对象级别的表示。

Result: 通过学习标记分组，视觉语言模型在视觉和语言理解方面表现出更好的细粒度理解能力。模型发现的标记组在定性和定量上都与文本中的可接地短语高度相似。

Conclusion: 通过将语言标记分组并与对象级别的视觉表示对齐，可以有效提升视觉语言模型的细粒度理解能力，模型能够自动发现与可接地短语相似的语义单元。
...
Method: 提出了一种模型无关的方法，通过将不同抽象层次和格式的场景图序列化，并通过结构化提示模板将其整合到模型中，系统分析关系监督的效益。

Result: 在LangAuto基准测试上的广泛评估显示，场景图条件化使最先进方法的驾驶性能获得大幅提升，LMDrive驾驶分数提升15.6%，BEVDriver提升17.5%。

Conclusion: 即使无需在测试时输入场景图，模型也能通过场景图条件化训练更好地内化和基于关系先验知识，显著提升自动驾驶性能。
...
TL;DR: 本文提出了一种通过控制合成数据生成来改进视觉语言模型微调的方法，该方法能消除偏差、分布不平衡和标注错误，并在真实世界数据上实现更好的性能转移。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉语言模型微调过程容易受到数据偏差、错误标注和分布不平衡的影响，导致过拟合和性能不均衡。现有研究虽然尝试使用合成数据，但缺乏对分布偏差和标注质量的控制。

Method: 重新设计微调过程：1）控制数据和标注的生成，确保无偏差、分布平衡且无标注错误；2）通过全面采样对象属性（颜色、形状、大小、位置）自动构建数据集；3）使用该数据集微调最先进的视觉语言模型，并评估在绝对位置任务上向真实世界数据的性能转移。

Result: 实验发现：1）在平衡的合成数据上微调能在整个视觉场景中产生均匀性能并减轻常见偏差；2）在合成刺激上微调显著提高了在真实世界数据（COCO）上的性能，优于在匹配设置下微调的模型。

Conclusion: 通过控制合成数据生成的方法可以有效解决微调过程中的偏差和分布不平衡问题，并实现向真实世界数据的良好性能转移，为视觉语言模型的微调提供了新的有效策略。
...
TL;DR: CORAL提出了一种协调有序关系解剖学习方法，通过对比排序目标和序数目标来捕获体积医学图像中的局部和全局结构，在有限标注设置下实现最先进的性能。
...
Conclusion: DocLens通过增强的定位能力证明了其在长视觉文档理解中的有效性，特别是在证据定位方面具有显著优势。
...
TL;DR: RTGaze是一种实时高质量视线重定向方法，通过可控制的视线表示学习和神经渲染实现3D一致的视线重定向，比现有3D感知方法快800倍。
...
Method: 学习基于人脸图像和视线提示的可控视线面部表示，通过神经渲染解码实现视线重定向，并从预训练的3D肖像生成器中提取面部几何先验来提升生成质量。
...
TL;DR: SimuFreeMark是一个无需噪声模拟的水印框架，通过利用图像低频分量的固有稳定性来嵌入水印，避免了传统方法对噪声模拟层的依赖。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法依赖于手工制作的噪声模拟层进行训练，这限制了它们对未知失真的泛化能力。AIGC的发展需要能够抵抗传统信号处理和新型语义编辑攻击的鲁棒图像水印。

Method: 首先系统性地验证了低频分量对各种攻击具有显著鲁棒性。然后利用预训练的变分自编码器(VAE)将水印直接嵌入到低频分量的深度特征空间中，将水印与结构稳定的图像表示绑定。

Result: 大量实验表明，SimuFreeMark在各种传统和语义攻击下都优于最先进的方法，同时保持优异的视觉质量。

Conclusion: SimuFreeMark通过消除训练过程中的噪声模拟需求，实现了更好的泛化能力和鲁棒性，为AIGC内容保护提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型训练数据包含敏感或受版权保护内容，需要遵守'被遗忘权'法规，但视觉概念遗忘研究不足，现有方法难以精确移除目标概念而不影响相关实体。

Method: 提出AUVIC框架，应用对抗性扰动实现精确遗忘，有效隔离目标概念，避免对相似实体产生意外影响。
...
TL;DR: DocSLM是一种高效的小型视觉语言模型，专为资源受限的边缘设备上的长文档理解设计，通过分层多模态压缩器和流式弃权机制，大幅减少内存消耗和延迟。
...
Conclusion: DocSLM能够在轻量级边缘设备上实现可靠的多模态文档理解，为资源受限环境提供了实用解决方案。
...
TL;DR: YCB-Ev SD是一个标准分辨率的事件相机合成数据集，用于6自由度物体姿态估计，包含50,000个34毫秒的事件序列，基于YCB-Video物体的物理渲染场景生成。


<details>
  <summary>Details</summary>
Motivation: 解决事件相机视觉领域缺乏与帧基计算机视觉相媲美的综合性合成数据资源的问题。

Method: 采用物理渲染场景和模拟线性相机运动生成事件序列，系统评估了不同事件表示方法用于CNN推理的性能。

Result: 研究发现具有线性衰减的时间表面和双通道极性编码在姿态估计性能上表现最佳，显著优于指数衰减和单通道替代方案。极性信息对性能提升贡献最大，线性时间编码比指数衰减更有效地保留关键运动信息。

Conclusion: 该数据集以结构化格式提供原始事件流和预计算的最优表示，便于立即用于研究和可重复基准测试，填补了事件相机视觉领域的合成数据空白。
...
TL;DR: Free3D是一个无需3D运动标注的3D人体运动生成框架，通过Motion-Lifting Residual Quantized VAE将2D运动序列映射到3D一致潜在空间，并使用3D无监督正则化目标实现视图一致性、方向连贯性和物理合理性。
...
Method: 提出Free3D框架，包含ML-RQ模块将2D运动映射到3D潜在空间，以及视图一致性、方向连贯性、物理合理性等3D无监督正则化目标，完全在2D运动数据上训练。

Result: Free3D生成多样化、时间连贯且语义对齐的3D运动，性能达到甚至超过完全3D监督的对比方法，表明放松显式3D监督能促进更强的结构推理和泛化能力。

Conclusion: 该方法为3D运动生成提供了可扩展且数据高效的新范式，证明无需3D标注也能实现高质量的3D运动生成。
...
TL;DR: 提出了一种结合隐马尔可夫随机场（HMRF）理论和CNN分割的方法，实现了无监督学习和快速分割，并在PU泡沫结构的μCT图像数据集上展示了高分割精度，同时提出了减少标注数据需求的预训练策略。
...
Method: 集成HMRF理论和CNN分割，结合无监督学习和快速分割的优势，研究不同邻域项和组件对无监督HMRF损失的贡献，使用HMRF-UNet模型。

Result: 在PU泡沫结构的μCT图像数据集上实现了无需真实标注的高分割精度，提出的预训练策略显著减少了训练分割模型所需的标注数据量。

Conclusion: HMRF-UNet方法成功结合了无监督学习和快速分割的优势，在材料图像分割任务中表现出色，同时通过预训练策略有效降低了标注数据需求。
...
TL;DR: 提出LSEF框架，基于低秩稀疏原理将情感动态建模为层次化低秩稀疏组合过程，通过三个模块分别捕获情感基、分离瞬态信号和重建多尺度一致性，显著提升了视频情感计算的鲁棒性和动态判别能力。


<details>
  <summary>Details</summary>
Motivation: 视频情感计算面临模型不稳定和表征退化问题，核心限制是缺乏层次化结构机制来解耦不同的情感成分（情感基和瞬态波动）。
...
Conclusion: 验证了层次化低秩稀疏建模对于理解情感动态的有效性和通用性。
...
TL;DR: MicroVQA++是一个三阶段构建的大规模高质量显微镜视觉问答数据集，通过专家验证、图结构过滤和MLLM生成结合人工筛选的方式构建，在显微镜推理任务上达到SOTA性能。
...
Result: 构建的数据集在Bloom认知层级难度分布上超过MicroVQA基准，4B规模的MLLM在显微镜推理任务上达到与GPT-5竞争的性能，并在开源MLLM中达到SOTA。
...
Method: 提出三层次评估框架Q-Doc：粗粒度层进行质量评分，中粒度层进行失真类型识别，细粒度层进行失真严重程度评估。

Result: 评估显示MLLMs具有初步DIQA能力，但存在评分不一致、失真误识别和严重程度误判等关键局限；思维链提示能显著提升所有层次的性能。
...
TL;DR: BOFA框架通过仅调整CLIP的跨模态桥接层实现类增量学习，无需额外参数，利用正交低秩融合防止遗忘，结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP在类增量学习中的两个挑战：避免增加额外模块导致的模型复杂化和遗忘问题，以及更有效地整合视觉和文本模态的互补优势。

Method: 提出BOFA框架，限制所有模型适应仅在CLIP现有跨模态桥接层进行；使用正交低秩融合机制将参数更新约束到与过去任务特征正交的低秩安全子空间；采用结合稳定文本原型和视觉对应物的跨模态混合原型。

Result: 在标准基准测试上的广泛实验表明，BOFA相比现有方法实现了更优越的准确性和效率。

Conclusion: BOFA通过桥接层正交融合方法有效解决了CLIP在类增量学习中的适应性和遗忘问题，无需数据回放即可实现稳定知识积累。
...
TL;DR: 提出自适应教学范式，通过让视觉模态（教师）动态调整知识结构来适应脑电信号模态（学生），解决视觉与脑电之间的保真度差距和语义差距问题。


<details>
  <summary>Details</summary>
Motivation: 视觉与脑电模态之间存在根本不对称性，包括保真度差距（脑电信号噪声大）和语义差距（脑电语义表示浅层）。现有方法忽视这种不对称性，导致泛化性能差。

Method: 提出自适应教学范式，使用ShrinkAdapter模块（无残差设计和瓶颈结构），让视觉模态动态调整其语义密集特征以适应脑电模态的能力。

Result: 在零样本脑到图像检索任务中达到60.2%的top-1准确率，比之前最优方法提升9.8%。

Conclusion: 引入不对称对齐的新视角：教师必须缩小和适应以弥合视觉-脑电差距。
...
TL;DR: WEAVE是一个用于上下文交织跨模态理解和生成的首个套件，包含WEAVE-100k大规模数据集和WEAVEBench人工标注基准，旨在解决现有方法在真实世界多轮图像创建和编辑中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和基准主要关注单轮交互，无法捕捉真实世界图像创建和编辑的多轮、上下文依赖特性，因此需要开发能够评估多轮上下文感知图像生成和编辑能力的工具。

Method: 开发了WEAVE套件，包括：1) WEAVE-100k：包含10万个交织样本、37万+对话轮次和50万+图像的大规模数据集；2) WEAVEBench：基于480张图像的100个任务的人工标注基准，采用混合VLM评判器评估框架。

Result: 实验表明，在WEAVE-100k上训练能够增强视觉理解、图像编辑以及理解-生成协作能力，促进统一多模态模型发展出新兴的视觉记忆能力。WEAVEBench评估揭示了当前方法在多轮上下文感知图像生成和编辑方面的持续局限性。

Conclusion: WEAVE为研究上下文交织理解和生成提供了视角和基础，有助于推动多模态社区在多轮、上下文依赖的图像创建和编辑方面的研究进展。
...
TL;DR: 本文提出了一个评估框架，用于区分文本到图像扩散模型中的识别（识别文化参考）和实现（如何描绘参考），重点关注多模态标志性现象。
...
Method: 引入评估框架分离识别与实现，使用Wikidata衍生的767个文化参考评估五个扩散模型，并进行提示扰动实验。

Result: 框架能更有效地区分复制与转换，模型即使文本线索改变也常重现标志性视觉结构，文化对齐与训练数据频率、文本独特性、参考流行度和创建日期相关。
...
TL;DR: Hi-DREAM是一个受大脑启发的条件扩散框架，通过显式建模皮层组织来解码fMRI信号生成图像。它使用ROI适配器将fMRI分为早期/中期/晚期视觉流，构建与U-Net深度对齐的多尺度皮层金字塔，并通过轻量级ControlNet在去噪过程中注入尺度特定提示。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的解码器大多直接对fMRI特征进行条件化，忽略了大脑的层次处理结构，模糊了早期、中期和晚期视觉区域的不同作用。需要显式建模皮层组织来更好地理解视觉信息在大脑中的处理方式。

Method: 1. ROI适配器将fMRI分组为早期/中期/晚期视觉流；2. 构建与U-Net深度对齐的多尺度皮层金字塔（浅层尺度保留布局和边缘，深层尺度强调物体和语义）；3. 使用轻量级、深度匹配的ControlNet在去噪过程中注入尺度特定提示。

Result: 在自然场景数据集(NSD)上的实验表明，Hi-DREAM在高层次语义指标上达到最先进性能，同时保持竞争力的低层次保真度。模型不仅能够重建图像，还能阐明不同视觉区域的功能贡献。

Conclusion: 按皮层层次结构组织条件化是基于纯数据驱动嵌入的强大替代方案，为研究视觉皮层提供了有用的视角。结构化条件化不仅能提高性能，还能增强模型的可解释性。
...
TL;DR: VP-Bench是一个评估多模态大语言模型理解和利用视觉提示能力的基准，包含感知和下游任务两个阶段的评估框架，测试了28个模型在8种形状和355种属性组合上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对MLLMs解释视觉提示能力的系统评估，不清楚当前模型是否能有效识别这种直观的人类提示方法并用于解决问题。
...
Conclusion: VP-Bench为研究MLLMs如何理解和解决基于引用的视觉问题建立了新的参考框架。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解临床文本描述并执行3D医学图像分割的模型，解决传统分割方法对文本提示理解不足的问题。

Method: 使用多阶段视觉语言融合策略，在解码器层间对齐文本和视觉特征，训练数据包含62,000+个CT、MRI和PET扫描，涵盖1,000多个解剖和病理类别。

Result: 在未见数据集上实现了最先进的零样本性能，在熟悉概念上表现出色，同时能够泛化到相关未见类别，展示了强大的跨模态迁移能力和对临床语言的鲁棒性。

Conclusion: VoxTell为基于文本提示的3D医学图像分割提供了有效的解决方案，具有优秀的泛化能力和临床应用潜力。
...
TL;DR: 本文提出Prelim Attention Score (PAS)方法，利用注意力权重检测大型视觉语言模型中的物体幻觉问题，无需额外训练即可实时识别和干预幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，研究发现许多幻觉预测中模型实际上忽略了图像，而是依赖先前生成的标记来推断新物体。

Method: 通过计算图像与预测物体在给定先前标记条件下的互信息，发现弱图像依赖与幻觉强相关。基于此引入PAS，从对先前标记的注意力权重中计算轻量级信号。

Result: PAS在多个模型和数据集上实现了最先进的物体幻觉检测性能，支持实时过滤和干预。

Conclusion: PAS是一种无需训练、计算效率高的方法，能够有效检测和缓解大型视觉语言模型中的物体幻觉问题。
...
TL;DR: 提出了Missing-aware Mixture-of-Loras (MaMOL)框架，通过双路由机制处理遥感多模态分类中的模态缺失问题，将模态缺失重新表述为多任务学习问题。


<details>
  <summary>Details</summary>
Motivation: 遥感多模态分类常因环境干扰、传感器故障或大气效应导致模态缺失，严重影响分类性能。现有两阶段适应方法计算成本高且假设训练时具有完整多模态数据，限制了在现实世界不完整情况下的泛化能力。

Method: MaMOL框架引入双路由机制：任务导向的动态路由器自适应激活不同缺失模式的专家，模态特定共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型要么在编码器早期融合特征，要么在后期比较池化嵌入，但两种模态的隐藏状态本身已包含丰富的模态特定结构信息，直接对齐这些状态是更自然的多模态匹配方式。
...
Conclusion: BRIDGE通过轻量级跨模态融合模块有效对齐视觉和文本隐藏状态，在多个任务上实现优越性能，同时保持编码器的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动分析方法耗时且易出错，需要利用大语言模型的强大分析推理能力来提高军事模拟推演数据分析的效率和准确性。

Method: 将复杂任务分解为子任务，为每个子任务设计系统提示和用户提示，通过多轮交互结合自检和反思进行结构化数据提取和多步分析评估，定义自定义工具生成图表和计算指标，设计多种报告模板。

Result: 评估结果表明，该方法生成的报告质量更高，得分优于基线方法。

Conclusion: 提出的多轮交互LLM方法能够有效生成高质量的军事模拟推演分析报告，具有较高的实用价值。
...
Method: 将传记数据转化为1774个增强的第一人称记忆，带有情感语义元数据，采用两阶段检索实现0.52秒的提示生成，使用并行检索结构化情景记忆。

Result: 评估显示该方法在GPT-4上与传统RAG相当，在较小模型（GPT-3.5、GPT-3）上显著优于传统RAG，特别适合资源受限部署。还支持时空热图、情感轨迹分析等可视化工具。

Conclusion: 该架构可推广到任何有大量文本记录的历史人物，为需要准确性和效率的教育、博物馆和研究应用提供了实用框架，既是对话接口又是传记分析研究工具。
...
Method: 提出的HyQuT架构在Transformer框架中集成了变分量子电路，在8M和150M参数规模下进行实验。使用最少量子比特（10个量子比特和80个量子门）替代约10%的经典参数。

Result: 实验结果表明，在150M参数模型中，使用少量量子资源即可实现与经典模型相当的收敛稳定性和生成质量。

Conclusion: 这项研究为将量子计算整合到大规模生成语言模型中提供了早期可行性证明，展示了量子计算在大规模自然语言处理任务中的应用潜力。
...
TL;DR: 研究发现大型语言模型在处理时间约束时存在系统性风险，包括双峰性能分布、极端提示脆弱性和系统性行动偏差，即使经过微调也无法可靠学习时间约束处理能力。


<details>
  <summary>Details</summary>
Motivation: 测试LLM在需要实时决策的智能体架构中是否能够可靠判断行动窗口是否仍然开放或已关闭，这一假设此前未经检验。

Method: 使用截止时间检测任务对8个生产级模型（2.8-8B参数）进行时间约束处理能力表征分析，包括性能分布、提示敏感性和行动偏差测试，并对部分模型进行200个合成示例的微调。

Result: 发现系统性部署风险：双峰性能分布（模型准确率要么95%要么50%）、极端提示脆弱性（仅格式变化就导致30-60个百分点波动）和系统性行动偏差（失败模型100%假阳性率）。参数数量在此范围内与能力无相关性。微调仅能部分改善能力。

Conclusion: 时间约束满足能力无法通过自然语言的下一个词预测可靠学习，需要专门的架构机制：连续时间状态表示、显式约束检查与语言模式匹配分离、系统性的时间关系组合推理。当前自回归架构缺乏这些机制，在时间关键应用中部署存在不可接受风险。
...
TL;DR: 本文扩展了谱神经符号推理框架，通过引入三种语义增强：基于transformer的节点合并、句子级蕴含验证和外部知识图谱对齐，在保持核心谱推理流程的同时提升图质量。
...
Method: 引入三种语义增强：(1) 使用上下文嵌入的transformer节点合并减少冗余；(2) 预训练NLI分类器的句子级蕴含验证改进边质量；(3) 与外部知识图谱对齐补充缺失上下文。

Result: 在ProofWriter、EntailmentBank和CLUTRR基准测试中显示一致的准确率提升（最高+3.8%），对对抗性案例的泛化能力增强，推理噪声减少。

Conclusion: 通过模块化、语义基础的预处理步骤扩展谱神经符号推理框架，在不改变核心谱推理引擎的情况下提升图质量，构建了更鲁棒、可解释和可扩展的推理系统。
...
TL;DR: 提出PRO框架，通过轻量级偏好适配器自动推断特定提示的偏好权重，解决多目标对齐中手动指定权重的问题。
...
Method: 使用轻量级偏好适配器，基于多个奖励模型的归一化奖励分数自动学习每个提示的合适偏好权重。

Result: 理论分析证明该方法优于固定权重方法，多个任务的实验验证了其有效性。

Conclusion: PRO框架通过自动推断提示特定偏好权重，在多目标对齐中实现了更优性能和训练效率。
...
TL;DR: 本文提出了一种基于多视角对比学习的专利嵌入方法，通过利用专利文档内部不同章节（如摘要、权利要求、背景技术）作为互补视图，解决了SimCSE风格dropout增强在专利领域产生过于均匀嵌入的问题。


<details>
  <summary>Details</summary>
Motivation: 针对SimCSE风格的dropout增强在专利领域产生语义凝聚力不足的过度均匀嵌入问题，需要一种能够保持语义和结构多样性的增强方法。

Method: 提出基于专利章节的增强方法，将专利的不同部分（摘要、权利要求、背景技术等）作为对比学习的互补视图，引入自然语义和结构多样性。

Result: 在大规模基准测试中，这种完全自监督的方法在现有技术检索和分类任务上匹配或超越了基于引用和IPC监督的基线方法，同时避免了对脆弱或不完整标注的依赖。

Conclusion: 不同专利章节在不同任务中具有专门化优势：权利要求和摘要有利于检索，背景技术有助于分类，这凸显了利用专利固有语篇结构进行表示学习的价值。
...
TL;DR: 评估15个开源大语言模型在病理学和放射学报告中的结构化信息提取能力，涵盖多个疾病领域和医疗机构，比较不同提示策略的效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常专注于单一任务、有限模型和英文报告，需要评估开源LLMs在多疾病、多语言、多机构的临床记录信息提取中的表现。

Method: 使用15个开源LLMs在荷兰、英国和捷克三个机构的病理学和放射学报告上评估六个用例，比较六种提示策略（零样本、单样本、少样本、思维链、自一致性和提示图）。

Result: 表现最佳的模型在各项任务中达到接近人工标注一致性的宏观平均分数，中小型通用模型表现与大型模型相当，提示图和少样本提示可提升约13%的性能。

Conclusion: 开源LLMs能够跨疾病、语言和机构从临床报告中提取结构化数据，为临床数据管理提供了可扩展的方法，任务特定因素比模型大小或提示策略影响更大。
...
<details>
  <summary>Details</summary>
Motivation: 传统通用压缩器（如gzip）计算开销低、速度快、适用范围广，但压缩率不如现代神经压缩器；而神经压缩器虽然能更好建模数据分布，但在未见数据上泛化能力差。需要结合两者优势。

Method: 使用加权专家乘积（wPoE）框架，在推理时自适应地结合通用压缩模型和预训练神经语言模型，确保压缩率不低于最佳单个模型。

Result: 大量实验表明，该方法无需微调即可提升文本压缩性能，并能与任何自回归语言模型无缝集成。

Conclusion: 该方法为跨不同数据分布增强文本压缩提供了实用解决方案，结合了通用压缩器和神经压缩器的优势。
...
TL;DR: 本文提出了一种贝叶斯方法来量化大型语言模型（LLM）在二元评估指标中的不确定性，特别关注概率文本生成策略引起的不确定性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估方法往往忽视统计不确定性量化，特别是在二元评估场景（如有害/无害输出）中，需要更准确的不确定性测量。

Method: 采用贝叶斯方法量化LLM二元评估指标的不确定性，重点关注概率文本生成策略引起的不确定性。

Result: 通过两个案例研究（拒绝率评估和成对偏好评估）展示了贝叶斯方法能为LLM系统行为提供有用的不确定性量化。

Conclusion: 贝叶斯方法能够有效量化LLM评估中的统计不确定性，为LLM系统行为评估提供更可靠的统计基础。
...
TL;DR: 本文对7个前沿大语言模型在粤语、日语和土耳其语上的表现进行了全面评估，涵盖问答、摘要、翻译和文化对话等任务，结合人工和自动评估发现专有模型表现最佳但仍有文化理解和形态学挑战。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在低资源、形态丰富语言（如粤语、日语、土耳其语）上的有效性，填补现有研究空白。

Method: 创建跨语言基准测试，结合人工评估（流畅度、事实准确性、文化适当性）和自动指标（BLEU、ROUGE），评估7个模型在4个任务上的表现。

Result: 专有模型（GPT-4o、GPT-4、Claude 3.5）整体领先，但在文化细微理解和形态学泛化方面仍有显著差距；开源模型在流畅度和准确性上大幅落后。
...
TL;DR: 本文提出了一种自监督框架，通过利用释义集合和偏斜感知聚合策略来提高保护模型的语义鲁棒性，减少语义变异性约58%，提高基准准确率约2.5%，并改善校准度达40%。


<details>
  <summary>Details</summary>
Motivation: 保护模型是LLM安全的关键组件，但对表面语言变化的敏感性是其关键弱点。即使意义保持不变的释义也会导致安全评分的大幅波动，揭示了缺乏语义基础的问题。

Method: 引入实用的自监督框架，利用释义集合通过新颖的偏斜感知聚合策略来强制执行预测一致性，用于鲁棒目标计算。

Result: 分析六个开源保护模型显示，该方法将释义间的语义变异性减少约58%，平均提高基准准确率约2.5%，并能泛化到未见过的风格变化。发现模型校准与一致性之间存在双向关系，鲁棒性训练可将校准度提高达40%。

Conclusion: 将语义一致性作为首要训练目标具有重要价值，为构建更可靠保护模型提供了可扩展的方法。
...
TL;DR: 本文提出了STaDS框架，通过结构化表格决策模拟来评估LLMs的真实理解能力，发现大多数前沿模型在不同领域难以保持一致的准确性，且存在预测与推理依据不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs虽然预测准确率高，但准确性并不等同于真正的理解能力。需要评估模型是否能在多个实例和不同领域中做出一致、有依据的决策，并依赖相关的领域基础决策因素。

Method: 引入结构化表格决策模拟(STaDS)，将LLMs视为专业人士参加结构化决策"考试"。通过三个维度评估理解能力：(i)问题和指令理解，(ii)基于知识的预测，(iii)对相关决策因素的依赖。分析了9个前沿LLMs在15个不同决策设置中的表现。

Result: 研究发现：(a)大多数模型在不同领域中难以实现持续强大的准确性；(b)模型可能准确但全局不忠实，预测与陈述推理之间存在频繁不匹配。

Conclusion: 需要全局层面的理解评估协议，并倡导超越准确性的新框架来增强LLMs的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗植入能显著改善重度感音神经性听力损失儿童的语言能力，但结果存在较大个体差异，且无法通过植入年龄或残余听力可靠预测。本研究旨在探索更准确的语言发展预测方法。

Method: 研究纳入278名来自三个中心的植入儿童，使用基于脑神经解剖特征的传统机器学习和深度迁移学习算法，构建高语言改善者与低语言改善者的二元分类预测模型。

Result: 深度迁移学习预测模型实现了92.39%的准确率、91.22%的敏感性和93.56%的特异性，AUC达到0.977，在所有结果指标上均优于传统机器学习模型。
...
TL;DR: 本文提出了一种基于混合专家（MoE）语音投影器的增强型大语言模型（LLM）方法，用于解决代码切换（CS）语音翻译的挑战，通过多阶段训练范式利用单语数据来缓解CS数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 代码切换语音翻译面临语义建模复杂和CS数据稀缺两大挑战，现有方法依赖模型隐式学习语义建模且需要低效的人工标注，需要更有效的解决方案。
...
Conclusion: 所提出的方法通过MoE专家系统和多阶段训练策略，有效解决了CS语音翻译中的语义建模和数据稀缺问题，展现出良好的性能。
...
TL;DR: 本文提出了Grounded Visual Factualization (GVF)微调方法，通过三种核心机制增强多模态大语言模型的视觉事实一致性，显著减少视觉幻觉现象。
...
Method: GVF微调整合了三种核心机制：事实锚点数据增强（用结构化事实锚点和反事实提示丰富训练数据）、事实感知指令调优（将事实线索嵌入显式指令）、以及事实一致性损失函数（专门惩罚事实不准确性）。

Result: 在LLaVA-1.5-13B模型上评估，GVF微调在VHTest基准测试中显著优于标准微调，在开放式问题和是/否问题格式上都表现出色。同时，在MME和POPE等多模态基准测试中保持或略微提升性能。

Conclusion: GVF方法能有效缓解视觉幻觉问题，同时不损害模型的一般理解和推理能力，为提升多模态大语言模型的可靠性提供了系统化解决方案。
...
TL;DR: 本综述回顾了大型语言模型在材料科学中的应用，重点关注科学文献挖掘、预测建模和多智能体实验系统三个领域，并比较了开源与闭源模型的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型正在快速改变材料科学，但当前进展主要依赖闭源商业模型，需要探索开源替代方案以提供更好的透明度、可重复性、成本效益和数据隐私。

Method: 通过基准测试比较开源和闭源LLM的性能，分析LLM在材料发现流程中的三个关键应用领域：科学文献挖掘、预测建模和多智能体实验系统。
...
TL;DR: 本文提出了一种两阶段演示选择方法TopK + L2D，通过结合语义相似性和标签分布对齐来改进文本分类中的上下文学习演示选择，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的演示选择方法主要关注测试输入与演示之间的语义相似性，往往忽略了标签分布对齐的重要性，这限制了大型语言模型在文本分类中的上下文学习性能。
...
### [102] [Pre-Attention Expert Prediction and Prefetching for Mixture-of-Experts Large Language Models](https://arxiv.org/abs/2511.10676)
*Shien Zhu,Samuel Bohl,Robin Oester,Gustavo Alonso*

Main category: cs.CL

TL;DR: 提出预注意力专家预测方法，使用同一层注意力块前的激活值和2个线性函数进行轻量级专家预取，在多个模型上实现93-97%的准确率，比现有方法提升约15%。
...
Method: 利用LLM中某些函数的排序保持特性，使用同一层注意力块前的激活值，通过2个线性函数和排序感知损失进行专家预测。

Result: 在DeepSeek V2 Lite上达到93.03%准确率，Qwen3-30B上94.69%，Phi-mini-MoE上97.62%，比现有方法绝对准确率提升约15%。

Conclusion: 预注意力专家预测方法实现了准确且轻量级的专家预取，支持第一层预取，显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化和全球变暖主要由温室气体排放引起，而消费品在生产、使用和处置过程中是主要排放源。因此需要工具来评估消费品的环境影响，其中生命周期评估（LCA）是关键部分。

Method: 开发SpiderGen LLM工作流，将传统LCA分类学和方法论与LLM的推理能力结合，生成LCA过程信息，并使用真实LCA文档作为基准进行评估。

Result: SpiderGen提供准确的LCA过程信息，完全正确或仅有轻微错误，在10个样本数据点上达到62%的F1分数。相比基线技术（如思维链提示和单样本提示）表现更好。

Conclusion: SpiderGen能够显著减少估算碳影响的人力和成本，在10分钟内以不到1美元的成本生成LCA过程信息，而传统LCA成本超过25000美元且需要21人天。
...
Result: 即使小的提示修改也能显著改变攻击成功率，不同对齐方法对提示变体的敏感度不同，现有攻击基准测试可能无法揭示所有漏洞。

Conclusion: 需要基于统计的系统性分析来评估模型对齐方法对提示攻击的敏感性，现有攻击基准测试存在局限性。
...
TL;DR: 该论文通过多轮对话测试发现大型语言模型在重复提问下存在显著脆弱性，简单的"再想想"提示就能导致准确率下降约10%，揭示了模型在交互场景中的稳定性问题。
...
Method: 使用简单的多轮后续提示评估模型回答变化，通过马尔可夫链分析模型准确率动态变化，并检验线性探针是否能预测这些变化。

Result: 发现LLM存在显著脆弱性："再想想"提示导致Gemini 1.5 Flash准确率在九轮对话中下降约10%；模型准确率可通过马尔可夫链有效建模，预测到Gemini 1.5 Flash的稳态准确率比首轮低约8%；线性探针可帮助预测未来回答变化。

Conclusion: 研究确立了稳态准确率作为交互场景下的鲁棒性指标，暴露了模型在重复提问下的脆弱性，解决这种不稳定性对于在高风险交互场景中部署LLM至关重要。
...
TL;DR: 研究递归提示下大语言模型生成合成数据时的性别偏见动态，发现存在均衡现象而非单调放大，低初始偏见会放大而高初始偏见会衰减，对比增强方法能有效减少下游任务偏见但可能产生更高的语义偏见分数。


<details>
  <summary>Details</summary>
Motivation: 递归提示技术能够扩展合成数据集生成，但存在偏见放大的风险，需要研究偏见在多代生成中的动态变化和缓解策略的有效性。

Method: 使用三种互补评估框架：基于规则的模式匹配、基于嵌入的语义相似性和下游任务性能，在三种初始偏见水平下测试四种缓解策略。

Result: 发现偏见动态呈现均衡现象：低初始偏见放大36%向模型固有偏见水平趋近，高初始偏见衰减26%；对比增强方法在下游任务中显著减少偏见（平均91%），但产生更高的语义偏见分数。

Conclusion: 语义相似性指标可能与行为公平结果存在分歧，强调在负责任合成数据生成中需要多维评估方法。
...
TL;DR: 本文提出使用'传话游戏'方法研究多模态系统的隐藏语言，通过观察概念共现频率来量化系统理解中的概念连接强度，并构建了Telescope数据集。


<details>
  <summary>Details</summary>
Motivation: 由于闭源多模态系统的黑盒架构，其理解世界的隐藏语言不透明。本文旨在通过系统的偏好偏差来研究这种隐藏语言。

Method: 采用多轮'传话游戏'策略，利用系统在图像压缩为文本再重建过程中的偏好偏差，观察概念共现频率来研究概念连接强度。

Result: 开发了Telescope数据集（10,000+概念对），能够构建多模态系统理解中的全局概念连接图，识别训练继承的偏好偏差，评估泛化能力，发现稳定概念连接路径。
...
TL;DR: 本文介绍了Squid Game，一个动态对抗性评估环境，用于在资源受限和信息不对称设置下评估大语言模型（LLMs）的性能。该环境包含六个淘汰式关卡，测试指令遵循、代码、推理、规划和安全对齐等多方面能力。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试难以跟上大语言模型的发展速度，存在数据污染问题，且现有基准主要假设良性、资源丰富的环境，未能探索LLMs在压力下的行为表现。

Method: 开发了Squid Game动态对抗评估环境，包含六个淘汰式关卡，让LLMs与其他LLM对手进行交互式游戏对战，评估其在资源受限和信息不对称环境下的表现。

Result: 评估了超过50个LLMs，观察到同一模型谱系中明显的代际性能转变，发现某些模型采用投机性捷径获胜，表明静态基准可能存在更高级别的评估范式污染。

Conclusion: 动态评估可以作为静态评估的补充部分，Squid Game提供了一个有效的动态对抗评估框架来全面评估LLMs的能力。
...
TL;DR: 研究表明，最先进的文本转语音系统能够隐式学习人类通过降低语速表达礼貌的倾向，在礼貌提示下产生比随意提示更慢的语音。
...
Result: 两个AI平台的礼貌提示都产生了比随意提示更慢的语音，效应量非常大。AI Studio的所有语音和OpenAI的大多数语音都表现出统计显著性。
...
TL;DR: 本文系统评估了大型语言模型在国际关系领域的国家层面偏见，发现模型普遍存在对西方国家的有利偏见和对俄罗斯的不利偏见，但偏见程度因模型和评估情境而异。作者提出了结合检索增强生成和反思技术的去偏框架，能有效减少偏见并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在国际关系领域的应用日益增多，评估其国家层面偏见变得至关重要。作者旨在系统分析LLMs在UNSC五个常任理事国上的偏见模式，并开发有效的去偏方法。

Method: 基于联合国安理会历史记录构建偏见评估框架，包含三个测试来探索国家层面偏见。提出结合检索增强生成和反思技术的去偏框架，通过增强事实推理能力来减少偏见。

Result: 实验结果显示LLMs普遍存在对西方国家的有利偏见和对俄罗斯的不利偏见，但偏见程度因模型和评估情境而异。推理能力更强的模型偏见更小。提出的去偏框架在GPT-4o-mini和LLama-3.3-70B上有效减少了偏见并提升了性能。

Conclusion: LLMs的国家层面偏见具有多维性，应用国际关系领域时需要同时评估偏见和性能。结合检索增强生成和反思技术的去偏方法能有效减少偏见，推理能力强的模型偏见更小。
...
TL;DR: ΠAttention是一种周期性稀疏Transformer，通过环形局部注意力、确定性π步长跳跃和自适应融合门来分解注意力机制，在保持线性复杂度的同时扩展感受野，在语言建模、检索和视觉语言任务中表现优于RingAttention。
...
Method: 提出ΠAttention，将注意力分解为环形局部邻域、确定性π步长跳跃和自适应融合门，周期性结构提供对远距离token的可预测覆盖，稀疏足迹保持每层复杂度与上下文长度成线性。
...
<details>
  <summary>Details</summary>
Motivation: 消费者健康问题摘要可以简化医疗沟通，但不忠实的摘要可能误传医疗细节，带来严重风险。需要提高医疗文本摘要的忠实度以确保安全。
...
Result: 创建了首个开源公开的语码转换突尼斯方言语音翻译语料库，包含25小时语音数据和相应的标注指南。
...
TL;DR: 开发基于GenAI和RAG技术的聊天机器人，解决学生获取分散学术信息的困难，通过评估多个模型发现Gemini 2.0 Flash和Gemma 3n表现最佳。
...
Method: 使用生成式人工智能和检索增强生成技术开发聊天机器人，测试多个GenAI模型并通过质量指标和LLM-as-a-Judge方法进行评估。

Result: Gemini 2.0 Flash在质量和速度方面表现突出，Gemma 3n因其良好性能和开源特性也表现优异。

Conclusion: 基于GenAI和RAG的聊天机器人可以有效简化学生获取日常学术信息的过程，其中Gemini 2.0 Flash和Gemma 3n是最合适的模型选择。
...
TL;DR: 本研究探讨了在本科计算语言学课程中使用GPT-4o自动评分短答案测验和项目报告的可行性，结果显示与人类评分员有强相关性（最高0.98），55%的测验案例分数完全一致。
...
Method: 收集约50名学生的5次测验答案和14个团队的项目报告，使用GPT-4o进行评分，并与助教独立进行的人类评估进行比较。

Result: GPT-4o与人类评分员表现出强相关性（最高0.98），55%的测验案例分数完全一致。在项目报告评分中也显示出整体良好的一致性，但在技术性、开放式回答的评分上存在一定变异性。
...
TL;DR: 该研究通过分析多语言大语言模型的内部机制，发现它们采用枢轴语言表示机制：模型在不同语言间使用几乎相同的表示，而语言特定的解码出现在后层。通过干预语言特征可以抑制一种语言并用另一种语言替代输出。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型能处理多种语言，但其内部如何表示这种多样性尚不清楚。研究旨在探究模型是否形成共享的多语言表示以及为何性能仍偏向主导训练语言。
...
TL;DR: 该研究分析了情感识别模型在非洲裔美国英语(AAVE)和通用美国英语(GAE)上的性能差异，发现模型在AAVE文本上对愤怒情绪的误报率显著高于GAE，且模型预测与种族人口统计数据相关，揭示了情感AI可能强化种族刻板印象的安全问题。


<details>
  <summary>Details</summary>
Motivation: 情感检测模型通常基于主流文化规范的标注进行训练，这限制了模型识别被排除在训练数据分布之外的方言(如AAVE)中情感表达的能力。研究旨在评估模型在AAVE与GAE上的性能差异。

Method: 分析270万条洛杉矶地理标记推文，使用计算方法评估AAVE特征强度，在875条高/低AAVE密度推文上收集情感存在和强度标注，使用AAVE流利的非洲裔美国人(内部群体)标注创建社区知情"银标"标签。

Result: GPT和BERT模型在AAVE上的愤怒误报率比GAE高出一倍多；SpanEmo模型的愤怒误报率从GAE的25%增加到AAVE的60%；模型和非内部群体标注与基于粗俗语的AAVE特征显著相关；非洲裔美国人比例较高的社区与更高的愤怒预测和更低的快乐预测相关。

Conclusion: 情感AI通过有偏见的情感分类强化种族刻板印象，存在紧急安全问题，强调需要文化和方言知情的情感计算系统。
...
### [118] [Leveraging Parameter Space Symmetries for Reasoning Skill Transfer in LLMs](https://arxiv.org/abs/2511.10850)
*Stefan Horoi,Sangwoo Cho,Supriyo Chakraborty,Shi-Xiong Zhang,Sambit Sahu,Guy Wolf,Genta Indra Winata*

Main category: cs.CL

TL;DR: 本文提出了一种参数空间对齐方法来解决任务算术中的负干扰问题，通过利用Transformer架构的对称性来对齐模型参数空间，成功实现了推理技能的跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: 任务算术在LLM间传输技能时经常因训练过程中的模型发散而产生负干扰，这限制了技能的有效迁移。

Method: 首先对齐模型的参数空间，利用Transformer架构的置换、旋转和缩放对称性，针对现代GQA和SwiGLU层进行适配，探索基于权重和基于激活的方法。

Result: 在具有挑战性的推理基准测试中，该方法始终优于标准任务算术，成功将高级推理技能迁移到非推理模型中。

Conclusion: 该工作提供了一种有效的方法来合并和传输不同LLM家族间的专业技能，减少了冗余的微调并增强了模型适应性。
...
### [119] [From Fact to Judgment: Investigating the Impact of Task Framing on LLM Conviction in Dialogue Systems](https://arxiv.org/abs/2511.10871)
*Parisa Rabbani,Nimet Beyza Bozdag,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: 研究发现，即使是简单的对话框架也会显著改变LLM的判断，GPT-4o-mini在社交框架下表现出奉承倾向，而Llama-8B-Instruct则变得过度批判，平均性能变化达9.24%。


<details>
  <summary>Details</summary>
Motivation: 探究LLM作为评判者在需要社交或对话判断的任务中的可靠性，特别是当任务从直接事实查询转变为对话判断任务时，LLM的信念如何改变。

Method: 通过对比模型在直接事实查询和最小对话中评估说话者正确性的表现，并在两种条件下施加简单反驳的压力，测量模型在对话压力下保持立场的坚定程度。

Result: 不同模型表现出不同倾向：GPT-4o-mini在社交框架任务中显示奉承倾向，Llama-8B-Instruct变得过度批判，所有模型平均性能变化为9.24%。
...
### [120] [ICX360: In-Context eXplainability 360 Toolkit](https://arxiv.org/abs/2511.10879)
...
TL;DR: ICX360是一个开源Python工具包，专注于解释大型语言模型(LLMs)的输出，特别关注用户提供的上下文或提示。它实现了三种最新的解释方法，包括黑盒和白盒方法。
...
Conclusion: ICX360为解释LLM输出提供了一个实用的开源工具包，特别关注用户提供的上下文，有助于提高LLM在关键应用中的透明度和可信度。
...
### [121] [A Multifaceted Analysis of Negative Bias in Large Language Models through the Lens of Parametric Knowledge](https://arxiv.org/abs/2511.10881)
*Jongyoon Song,Sangwon Yu,Sungroh Yoon*

Main category: cs.CL

TL;DR: 本文研究了大型语言模型在二元决策任务中的负向偏见现象，发现提示格式比负向响应语义对模型响应影响更大，并识别出模型在知识不足时倾向于生成负向响应的捷径行为。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注检测和处理引发负向偏见的注意力头，但影响负向偏见的详细因素仍未充分探索，需要更细粒度的研究。

Method: 引入构建评估集的流程，系统地将数据集分为正确、错误和知识不足三个子集，分析不同提示场景下负向偏见的变化。

Result: 发现提供相关上下文和"我不知道"选项通常减少负向偏见，而思维链提示倾向于放大偏见，提示类型影响偏见程度。
...
### [122] [MedPath: Multi-Domain Cross-Vocabulary Hierarchical Paths for Biomedical Entity Linking](https://arxiv.org/abs/2511.10887)
*Nishant Mishra,Wilker Aziz,Iacer Calixto*

Main category: cs.CL

TL;DR: MedPath是一个大规模多领域生物医学实体链接数据集，整合了9个现有专家标注数据集，使用UMLS标准化实体，映射到62个生物医学词汇表，并提供完整的本体路径，支持语义丰富和可解释的实体链接系统开发。
...
Method: 基于9个现有专家标注的实体链接数据集构建MedPath，使用UMLS最新版本标准化所有实体，映射到62个其他生物医学词汇表，并在最多11个生物医学词汇表中提供完整的本体路径。
...
TL;DR: 研究发现工具增强语言模型（TaLMs）在使用代码解释器等工具时会出现"工具诱导近视"（TIM）现象，即虽然提高了最终答案准确率，但推理过程质量下降，模型将工具输出作为推理的替代品而非辅助证据。


<details>
  <summary>Details</summary>
Motivation: 探究工具增强语言模型在使用外部工具时是否真正进行可信推理，以及工具使用对推理质量的影响。

Method: 使用PYMATH基准测试（1,679个竞赛级数学问题），开发多维度评估套件量化推理退化，分析工具使用频率与推理质量的关系，并提出基于偏好优化的框架来改善工具使用方式。

Result: TaLMs在使用工具时最终答案准确率提升19.3个百分点，但推理过程质量显著下降（非工具LLMs在推理过程比较中胜出41.5%），工具使用越频繁推理越不连贯，约55%高风险案例存在TIM现象。

Conclusion: 工具使用虽然提高答案准确率但损害推理质量，需要通过优化框架重新调整TaLMs将工具作为辅助证据而非推理替代品，以同时提升准确率和推理深度。
...
TL;DR: EMSQA是一个包含24.3K个医学问题的数据集，涵盖10个临床学科和4个认证级别。研究者提出了Expert-CoT和ExpertRAG方法，通过结合特定临床领域知识和认证级别来提升LLM在医学问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法在医学问答中忽视了领域专业知识（如临床学科和认证级别），限制了在高风险环境下的性能。
...
Conclusion: 结合领域专业知识（临床学科和认证级别）能显著提升LLM在医学问答中的性能，特别是在高风险认证考试中表现出色。
...
TL;DR: 本文提出了一种基于多模态大语言模型的交互式同行评审系统，通过整合文本和视觉信息、基于OpenReview数据的检索增强生成以及将评审意见转化为可操作待办清单，为论文提交前的修改提供有效指导。
...
Method: 开发交互式Web系统，集成多模态LLM处理文本和视觉信息，使用检索增强生成技术基于OpenReview数据提升评审质量，并采用Action:Objective[#]格式将评审意见转化为结构化可追踪的待办清单。

Result: 实验结果表明，该系统能够生成更全面、更有用的评审意见，符合专家标准，优于消融基线方法。

Conclusion: 该系统通过多模态信息整合、检索增强生成和结构化反馈，推进了透明、以人为本的学术辅助工具的发展，有效支持论文提交前的修改工作。
...
TL;DR: 本文评估了四种先进大语言模型在罕见病诊断任务上的表现，使用从《豪斯医生》剧集中提取的176个症状-诊断对作为数据集。结果显示模型准确率在16.48%到38.64%之间，新一代模型性能提升了2.3倍。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在罕见病诊断方面的能力，目前这一领域研究不足，特别是在基于叙述性医疗案例的诊断推理方面。

Method: 使用从《豪斯医生》剧集中提取的176个症状-诊断对作为数据集，评估了GPT 4o mini、GPT 5 mini、Gemini 2.5 Flash和Gemini 2.5 Pro四种大语言模型在叙述性诊断推理任务上的表现。

Result: 模型表现差异显著，准确率范围为16.48%到38.64%，新一代模型相比旧模型性能提升了2.3倍。所有模型在罕见病诊断方面都面临重大挑战。

Conclusion: 虽然所有模型在罕见病诊断方面都存在困难，但跨架构的改进表明未来发展的有希望方向。该教育验证基准为叙述性医疗推理建立了基线性能指标，并为推进AI辅助诊断研究提供了公开可访问的评估框架。
...
TL;DR: CardioEmbed是一个专门针对心脏病学领域的嵌入模型，基于Qwen3-Embedding-8B，使用对比学习在7本心脏病学教科书上训练，在心脏病学语义检索任务中达到99.60%的准确率，比现有最佳医学嵌入模型MedTE提升15.94个百分点。
...
Method: 基于Qwen3-Embedding-8B模型，使用对比学习方法，在7本综合心脏病学教科书组成的约15万句去重语料库上训练，采用InfoNCE损失函数和批次内负样本策略。

Result: 在心脏病学特定语义检索任务中达到99.60%的检索准确率，比MedTE模型提升15.94个百分点；在MTEB医学基准测试中，BIOSSES得分为0.77 Spearman，SciFact得分为0.61 NDCG@10。

Conclusion: 在综合临床教科书上进行领域专业化训练能够产生近乎完美的心脏病学检索性能，显著优于现有医学嵌入模型，表明领域专业化训练对临床应用的巨大价值。
...
TL;DR: DiscoX是一个用于评估专业领域中文到英文翻译的新基准，包含7个领域的200篇专业文本，平均长度超过1700个词。同时开发了无参考评估系统Metric-S，在准确性、流畅性和适当性方面提供细粒度自动评估。实验显示即使最先进的LLM在专业翻译任务上仍落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前翻译评估方法主要关注片段级准确性和流畅性，无法充分评估专业领域翻译所需的语篇级连贯性和术语精确性，这限制了知识传播和跨语言学术交流的质量。

Method: 构建DiscoX基准，包含7个专业领域的200篇专业文本；开发Metric-S无参考评估系统，提供准确性、流畅性和适当性的细粒度自动评估；比较先进LLM与人类专家在专业翻译任务上的表现。

Result: Metric-S与人类判断具有强一致性，显著优于现有评估指标；实验发现即使最先进的LLM在专业翻译任务上仍明显落后于人类专家，验证了DiscoX的难度。

Conclusion: DiscoX基准和Metric-S评估系统为更严格的翻译评估提供了稳健框架，凸显了实现专业级机器翻译的挑战，有助于推动基于LLM的翻译技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数前沿LLM不公开其策划的偏好对，而开源DPO数据集缺乏系统比较，主要由于计算成本高和缺乏丰富的质量标注，难以理解偏好如何选择、涵盖哪些任务类型以及如何反映人类判断。

Method: 利用Magpie框架对每个样本进行任务类别、输入质量和偏好奖励的标注，实现可扩展的细粒度偏好质量检查；基于这些洞察，系统性地从五个语料库中选择性提取样本，同时移除噪声或冗余样本，构建新的DPO混合数据集UltraMix。

Result: 揭示了不同数据集在奖励边际上的结构和质量差异；构建的UltraMix数据集比性能最佳的单数据集小30%，但在关键基准测试中表现更优。

Conclusion: 通过数据中心的方法可以显著提升DPO数据集的质量和效率，UltraMix的成功表明选择性混合和去噪策略的有效性，为未来数据中心偏好优化研究提供了有价值的资源和框架。
...
TL;DR: 提出一种增强基于自动机的结构化生成多样性的新方法，利用自动机遍历历史引导LLM生成新颖结构模式，显著提升结构和内容多样性同时保持生成效率。


<details>
  <summary>Details</summary>
Motivation: 现有结构化生成方法虽然确保有效性但缺乏输出多样性，这是关键限制，初步研究已确认此问题。

Method: 利用自动机遍历历史来引导大型语言模型朝向新颖的结构模式，通过记录和分析自动机状态转换历史来识别和避免重复结构。

Result: 评估显示该方法显著改善了结构和内容多样性，同时保持了可比的生成效率。案例研究展示了在生成开源库测试用例方面的有效性。

Conclusion: 所提出的方法成功解决了结构化生成中的多样性不足问题，为LLM生成多样化结构化输出提供了有效解决方案，在测试用例生成等实际应用中表现出良好效果。
...
TL;DR: 本文发现当前文本嵌入模型存在一致的偏差，并提出了一种无需训练、轻量级的重归一化方法，通过去除嵌入向量中的公共偏差分量，显著提升了在MMTEB基准上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 发现现有文本嵌入模型的输出存在系统性偏差，所有句子的嵌入向量都包含一个几乎相同的偏差分量，这影响了模型的性能表现。

Method: 提出重归一化方法，有两种变体：直接减去偏差分量μ，或减去向量在μ方向上的投影。该方法无需额外训练，可直接应用于现有模型。

Result: 在38个模型上的实验表明，重归一化显著提升了MMTEB基准的性能：检索任务提升9.7σ，分类任务提升3.1σ，其他任务提升0.8σ。理论预测和实验均证实投影方法效果更好。

Conclusion: 重归一化是一种简单有效的后处理方法，能够显著改善文本嵌入模型的性能，且理论分析指导了最优方法的选择。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型能生成流畅回答但有时会产生事实幻觉，需要研究模型能否识别自身的幻觉问题。

Method: 将幻觉检测构建为句子分类任务，提出基于思维链的分类方法从模型参数中提取知识。

Result: GPT-3.5 Turbo结合思维链能检测58.2%的自身幻觉。

Conclusion: 如果模型参数包含足够知识，结合思维链的大语言模型能够检测幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 人身攻击已成为美国总统辩论的显著特征，对选举期间的公众认知产生重要影响。检测此类攻击可以提高政治话语的透明度，为记者、分析师和公众提供洞察。深度学习和基于transformer的模型（特别是BERT和LLM）的发展为有害语言的自动检测创造了新机会。

Method: 手动标注2016、2020和2024年选举周期的辩论记录，然后进行统计和基于语言模型的分析。研究微调transformer模型和通用LLM在检测正式政治演讲中人身攻击的潜力。
...
TL;DR: 本文提出SAP²方法，一种两阶段动态修剪和整合相关上下文关键词的新框架，用于解决ASR系统在需要领域特定知识的场景中难以利用长上下文信息的问题。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在常见条件下表现优异，但在需要领域特定知识的上下文场景中难以利用长上下文信息，主要受限于模型上下文窗口和大量上下文噪声中相关信息的稀疏性。

Method: 提出SAP²方法，采用两阶段动态修剪和整合相关上下文关键词的框架，每个阶段利用提出的语音驱动注意力池化机制，高效压缩上下文嵌入同时保留语音显著信息。

Result: 在SlideSpeech和LibriSpeech数据集上达到最先进性能，词错误率分别为7.71%和1.12%。在SlideSpeech上，相比非上下文基线方法显著降低偏置关键词错误率41.1%。
...
TL;DR: 本文针对WordNet中副词缺乏系统语义分类的问题，提出了基于语言学基础的副词超义类型学，通过实证验证展示了该分类体系在自然文本中的广泛覆盖性和人工标注可靠性。


<details>
  <summary>Details</summary>
Motivation: WordNet为名词和动词提供了丰富的超义层次结构，但副词仍然缺乏系统的语义分类，这限制了其在自然语言处理应用中的效用。

Method: 引入基于语言学基础的副词超义类型学，通过试点标注研究进行实证验证，涵盖方式、时间、频率、程度、领域、说话者导向和主语导向等主要语义域。

Result: 试点标注研究结果表明，这些类别能够广泛覆盖自然文本中的副词，并且可以由人工标注者可靠地分配。

Conclusion: 该类型学扩展了WordNet的覆盖范围，使其更贴近语言学理论，并促进了词义消歧、事件抽取、情感分析和话语建模等下游NLP应用的发展。
...
TL;DR: 提出LANE对抗训练策略，通过选择性标记训练集中的替代词生成负例，迫使模型在相同句子中不同标记词之间产生更大区分度，从而改善细粒度词义消歧性能。


<details>
  <summary>Details</summary>
Motivation: 神经语言模型往往过度拟合全局句子表示，未能捕捉局部语义细节，导致细粒度词义消解能力不足。

Method: LANE对抗训练策略，通过选择性标记训练集中的替代词生成挑战性负例，将模型学习焦点转移到目标词上。
...
TL;DR: 本文提出了一种从知识图谱生成问答对的混合方法，结合了基于规则的模板生成和LLM的优化，实现了可扩展且高质量的自然语言问答生成。


<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱问答生成方法在可扩展性、语言质量和事实一致性方面存在不足，需要一种既能保持事实准确性又能提升语言质量的可扩展解决方案。

Method: 首先基于关系对知识图谱三元组进行聚类，通过实体类型和关系创建可重用的自然语言模板；然后使用LLM优化模板以提升清晰度和连贯性；最后通过从知识图谱中选择干扰项来实例化答案选项。

Result: 实验表明，这种混合方法能够高效生成高质量的问答对，在可扩展性、流畅性和语言精确性方面取得了良好平衡。

Conclusion: 提出的混合方法成功解决了知识图谱问答生成中的可扩展性和语言质量问题，为教育平台和大型语言模型的开发提供了有效的解决方案。
...
Method: 首先让单智能体生成结构化自我批评响应，从中提取41个可解释的语言和语义特征，然后使用轻量级辩论决策分类器判断是否触发多智能体辩论
...
<details>
  <summary>Details</summary>
Motivation: 现有金融NLP领域的大型语言模型（如BloombergGPT、FinMA等）虽然性能优异，但计算需求高，限制了众多组织的可访问性。需要开发更高效的微调方法来降低计算成本。

Method: 提出LAET策略，通过分析隐藏状态表示识别预训练LLM中最有效的层，仅对这些层进行选择性微调，同时冻结其他层，实现计算效率优化。

Result: LAET在金融NLP任务中表现出色，超越了现有基准和最先进的LLM（如GPT-4），即使使用较小的LLM（约30亿参数）也能取得优异结果。

Conclusion: LAET成功弥合了前沿金融NLP研究与实际部署之间的差距，为金融应用提供了高效可扩展的模型解决方案。
...
TL;DR: LaoBench是首个专门评估大语言模型在老挝语中综合语言理解和推理能力的大规模、高质量、多维度基准数据集，包含17,000多个样本，涵盖知识应用、K12基础教育和多语言翻译三个维度。


<details>
  <summary>Details</summary>
Motivation: 填补大语言模型在低资源语言（特别是东南亚语言如老挝语）评估方面的空白，推动对代表性不足的东南亚语言AI技术的研究和开发。

Method: 构建包含17,000多个样本的数据集，涵盖知识应用、K12基础教育和老挝语-中文-英语双语翻译三个维度；采用专家人工策划与自动化智能体辅助验证相结合的数据构建流程；分为开源和闭源子集，闭源部分在官方平台上进行黑盒评估。

Result: 在LaoBench上对多个最先进的大语言模型进行基准测试发现，当前模型在掌握老挝语多样化任务方面仍面临重大挑战。

Conclusion: LaoBench将促进对代表性不足的东南亚语言AI技术的进一步研究和开发，当前模型在老挝语任务上仍有改进空间。
...
TL;DR: 该论文介绍了M-DAIGT共享任务，旨在检测新闻文章和学术写作中的AI生成文本，包含两个子任务并发布了包含3万样本的数据集。
...
Method: 设计包含新闻文章检测和学术写作检测两个子任务的共享任务，构建平衡的人类撰写和AI生成文本数据集，使用多种现代LLM和提示策略生成内容。

Result: 46个团队注册参与，4个团队提交最终结果，所有团队都参与了两个子任务。
...
Method: 回顾文献分析语言模型对自然语言和不可能语言的学习表现，并与婴儿的语言学习能力进行对比。

Result: 语言模型通常能同样好地学习自然语言和许多不可能语言，而难以学习的不可能语言通常更复杂或随机。

Conclusion: 语言模型缺乏支持人类语言习得的归纳偏置，这是它们与人类婴儿语言学习能力差异的关键所在。
...
TL;DR: MajinBook是一个开放目录，旨在促进使用影子图书馆（如Library Genesis和Z-Library）进行计算社会科学和文化分析。通过将这些众包档案的元数据与Goodreads的结构化书目数据链接，创建了一个包含超过53.9万本英文书籍的高精度语料库，涵盖三个世纪，并丰富了首次出版日期、流派和流行度指标。


<details>
  <summary>Details</summary>
Motivation: 促进影子图书馆在计算社会科学和文化分析中的应用，解决传统语料库（如HathiTrust）中的偏见问题，为研究人员提供高质量的机器可读数据。

Method: 将影子图书馆的元数据与Goodreads的结构化书目数据链接，优先使用原生数字EPUB文件确保机器可读质量，并评估链接策略的准确性。

Result: 创建了包含超过539,000本英文书籍的高精度语料库，涵盖三个世纪，包含首次出版日期、流派、评分和评论等流行度指标，还包括法语、德语和西班牙语的辅助数据集。

Conclusion: MajinBook项目在法律允许的范围内（根据欧盟和美国的研究文本和数据挖掘框架）开放所有基础数据，为计算社会科学和文化分析提供了有价值的资源。
...
TL;DR: 提出了一种主动式听力助手系统，能够自动识别和分离佩戴者的对话伙伴，无需显式提示。系统基于自我语音作为锚点，利用对话轮换行为来推断对话伙伴并抑制其他声音。


<details>
  <summary>Details</summary>
Motivation: 开发能够主动适应对话动态的听力助手，在多人对话环境中自动识别和隔离对话伙伴，改善听力辅助设备的使用体验。

Method: 采用双模型架构：轻量级流式模型每12.5毫秒运行一次用于低延迟提取对话伙伴，较慢模型较少运行以捕捉长程对话动态。基于自我语音作为锚点，利用对话轮换行为。

Result: 在真实世界的2人和3人对话测试集上表现良好，能够泛化到多对话场景中识别和隔离对话伙伴。数据集包含11名参与者的6.8小时双耳自我中心音频。

Conclusion: 这项工作标志着向能够主动适应对话动态和参与度的听力助手迈出了一步，为改善听力辅助设备在多对话环境中的性能提供了有效解决方案。
...
TL;DR: PRBench是一个面向金融和法律领域的专业推理基准测试，包含1100个专家编写的任务和19356个专家制定的评估标准，是目前最大的公开基准。评估20个领先模型显示其在专业推理方面仍有很大改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有学术基准测试在评估现实世界专业环境中的性能方面存在局限，特别是在法律和金融等高风险领域，需要更真实的开放性问题评估。

Method: 开发PRBench基准，包含182名合格专业人士（持有JD、CFA或6年以上经验）根据实际工作流程编写的任务，涵盖114个国家和47个美国司法管辖区，并通过独立专家验证的质量管道验证评估标准。

Result: 评估20个领先模型显示，在困难子集上的最高得分仅为0.39（金融）和0.37（法律），表明模型在专业推理方面表现不佳。常见失败模式包括判断不准确、缺乏过程透明度和推理不完整。

Conclusion: 当前模型在专业推理能力上存在显著不足，特别是在准确判断、推理透明度和完整性方面，这限制了其在专业领域的可靠采用。
...
### [146] [Attentive Feature Aggregation or: How Policies Learn to Stop Worrying about Robustness and Attend to Task-Relevant Visual Cues](https://arxiv.org/abs/2511.10762)
...
TL;DR: 本文提出了一种名为注意力特征聚合（AFA）的轻量级可训练池化机制，用于提高视觉运动策略在视觉扰动场景中的鲁棒性，无需昂贵的数据增强或预训练视觉表示微调。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉表示（PVRs）虽然强大，但可能编码大量任务无关的场景信息，导致训练出的视觉运动策略在面对域外视觉变化和干扰物时缺乏鲁棒性。

Method: 通过注意力特征聚合（AFA）机制，学习自然地关注任务相关的视觉线索，忽略即使是语义丰富的场景干扰物。

Result: 在仿真和真实世界的广泛实验中，使用AFA训练的策略在存在视觉扰动的情况下显著优于标准池化方法。

Conclusion: 忽略无关的视觉信息是部署鲁棒且可泛化的视觉运动策略的关键步骤。
...
### [147] [From Framework to Reliable Practice: End-User Perspectives on Social Robots in Public Spaces](https://arxiv.org/abs/2511.10770)
*Samson Oruma,Ricardo Colomo-Palacios,Vasileios Gkioulos*

Main category: cs.RO

TL;DR: 本文报告了ARI社交机器人在大学接待员角色的试点部署，基于SecuRoPS框架设计，通过35名用户反馈评估了安全性、隐私、可用性等方面，结果显示积极的安全和伦理认知，但也存在可访问性和互动性挑战。


<details>
  <summary>Details</summary>
Motivation: 随着社交机器人进入公共环境，其接受度不仅取决于技术可靠性，还依赖于伦理完整性、可访问性和用户信任。研究旨在探索如何在现实环境中实施伦理安全设计框架。

Method: 采用ARI社交机器人作为大学接待员进行试点部署，基于SecuRoPS框架设计，收集35名学生和员工的结构化反馈，评估安全、隐私、可用性、可访问性和透明度等方面。

Result: 结果显示用户对物理安全、数据保护和伦理行为普遍持积极态度，但发现了可访问性、包容性和动态互动方面的挑战。研究还提供了可复用的GitHub模板资源。

Conclusion: 研究证明了伦理安全设计框架在现实环境中的可行性，通过结合用户视角和实用技术资源，为开发可信赖、包容和伦理负责的公共空间社交机器人做出贡献。
...
### [148] [$\rm{A}^{\rm{SAR}}$: $\varepsilon$-Optimal Graph Search for Minimum Expected-Detection-Time Paths with Path Budget Constraints for Search and Rescue](https://arxiv.org/abs/2511.10792)
*Eric Mugford,Jonathan D. Gammell*

Main category: cs.RO

TL;DR: 本文提出了A^SAR算法，一种ε最优搜索算法，用于搜索与救援规划。该算法通过计算启发式边界来约束搜索空间，并使用图搜索方法找到在用户指定因子ε内最优的解决方案。


<details>
  <summary>Details</summary>
Motivation: 在搜索与救援场景中，由于信息不确定、观察者不完美和搜索区域大，搜索问题复杂。特别是在海上救援等场景中，预期生存时间短，最优搜索可以提高成功概率。现有随机优化方法缺乏对有限时间内解质量的正式保证。

Method: 提出A^SAR算法，计算启发式边界来约束搜索空间，使用图搜索方法找到ε最优解。算法提供正式保证，确保解在用户指定因子ε内最优。

Result: 在操作模拟中，比现有优化方法更快找到更好的解。在加拿大安大略湖的实地试验中，仅用150秒就定位了漂流的人偶。

Conclusion: A^SAR算法为SAR规划提供了一种具有正式最优性保证的有效搜索方法，在实际应用中表现出色。
...
### [149] [An Investigation into Dynamically Extensible and Retractable Robotic Leg Linkages for Multi-task Execution in Search and Rescue Scenarios](https://arxiv.org/abs/2511.10816)
*William Harris,Lucas Yager,Syler Sylvester,Elizabeth Peiros,Micheal C. Yip*

Main category: cs.RO

TL;DR: 本文提出了一种新型动态可伸缩机器人腿部设计，通过五杆连杆机构实现高度优势和力优势配置之间的机械切换，旨在解决搜救机器人既需要地形适应性又需要高力输出的双重需求。


<details>
  <summary>Details</summary>
Motivation: 搜救机器人需要快速穿越地形并执行高力救援任务，但现有平台很少能同时满足地形适应性和高力输出需求。腿式机器人具有地形适应性优势，但通常无法提供可变高力输出。

Method: 采用动态可伸缩五杆连杆设计，通过几何变换在高度优势和力优势配置之间进行机械切换。通过测试平台评估不同连杆几何形状和操作模式下的腿部性能，并进行步长、力输出和稳定性的经验与理论分析。

Result: 结果表明，变形腿为搜救机器人提供了一条有前景的路径，既能快速导航地形，又能有效执行救援任务。

Conclusion: 动态可伸缩机器人腿部设计能够有效解决搜救机器人在地形适应性和高力输出方面的双重挑战，为搜救机器人平台的发展提供了新的技术方向。
...
### [150] [MIGHTY: Hermite Spline-based Efficient Trajectory Planning](https://arxiv.org/abs/2511.10822)
*Kota Kondo,Yuwei Wu,Vijay Kumar,Jonathan P. How*

Main category: cs.RO

TL;DR: MIGHTY是一种基于Hermite样条的轨迹规划器，通过时空优化在连续搜索空间中实现高效规划，相比现有方法计算时间减少9.3%，旅行时间减少13.1%，成功率100%。
...
Result: 仿真中比最先进基线计算时间减少9.3%，旅行时间减少13.1%，成功率100%；硬件实验中在杂乱静态环境中完成最高6.7 m/s的高速飞行，并在动态添加障碍物情况下实现长时间飞行。

Conclusion: MIGHTY成功克服了现有方法的局限性，实现了高效的时空轨迹规划。
...
### [151] [Decentralized Swarm Control via SO(3) Embeddings for 3D Trajectories](https://arxiv.org/abs/2511.10858)
*Dimitria Silveria,Kleber Cabral,Peter Jardine,Sidney Givigi*

Main category: cs.RO

TL;DR: 提出了一种新颖的分散式多智能体系统方法，通过最小信息共享实现涌现行为，基于SO(3)李群生成稳定周期性轨迹，无需速度输入，并提供了相位控制器和稳定性证明。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成周期性轨迹方面存在局限性，需要速度输入且信息共享较多，需要开发更高效、适应性更强的分散控制方法。

Method: 基于SO(3)李群的几何嵌入方法，稳定系统生成周期性轨迹，提出相位控制器确保智能体均匀分布，无需速度输入。

Result: 该方法能够生成比现有方法更广泛的周期性曲线，适应复杂的底层动力学和干扰，通过仿真和实验验证了有效性。

Conclusion: 所提出的基于SO(3)李群的分散控制方法在最小信息共享下实现了稳定的周期性涌现行为，具有更好的适应性和实用性。
...
### [152] [WetExplorer: Automating Wetland Greenhouse-Gas Surveys with an Autonomous Mobile Robot](https://arxiv.org/abs/2511.10864)
*Jose Vasquez,Xuping Zhang*

Main category: cs.RO

TL;DR: WetExplorer是一个自主履带式机器人，用于自动化湿地温室气体采样工作流程，通过集成低接地压力移动、厘米级精确升降放置、双RTK传感器融合、避障规划和深度学习感知等技术，实现了无需人工干预的温室气体采样。


<details>
  <summary>Details</summary>
Motivation: 湿地温室气体量化对气候建模和恢复评估至关重要，但手动采样劳动强度大、耗时多。需要自动化解决方案来提高采样频率和数据密度。

Method: 开发了WetExplorer机器人系统，集成低接地压力移动系统、厘米级精确升降机构、双RTK传感器融合定位、避障运动规划和深度学习视觉感知模块，采用容器化ROS2软件架构。
...
### [153] [Terradynamics and design of tip-extending robotic anchors](https://arxiv.org/abs/2511.10901)
...
TL;DR: 该论文研究了受树根启发的尖端延伸锚固机制，开发了一种轻量级软体机器人锚固装置，能在松散的火星土壤模拟物中实现40:1的锚固重量比。


<details>
  <summary>Details</summary>
Motivation: 传统桩基需要比提取力更大的插入力，这限制了在难以到达地点的应用，而树根通过尖端延伸机制实现了插入力远小于提取力的特性。

Method: 研究尖端延伸锚固与传统的桩式侵入器的土壤动力学，识别关键设计原则，并开发可部署的机器人锚固装置。

Result: 开发出300克的轻量级软体机器人锚固装置，在火星土壤模拟物中插入深度达45厘米，平均锚固力120N，锚固重量比达40:1。

Conclusion: 通过尖端延伸机制、毛发状突起、近垂直延伸和多锚点设计，可以实现插入力小于重量的高效锚固系统，为难以到达地点的锚固应用提供了新解决方案。
...
### [154] [Dexterous Manipulation Transfer via Progressive Kinematic-Dynamic Alignment](https://arxiv.org/abs/2511.10987)
*Wenbin Bai,Qiyu Chen,Xiangbo Lin,Jianwen Li,Quancheng Li,Hejiang Pan,Yi Sun*

Main category: cs.RO

TL;DR: 提出了一种手部无关的操控转移系统，能够将人类手部操控视频高效转换为高质量的多指灵巧手操控轨迹，无需大量训练数据，解决了数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 多指机器人手硬件平台收集操控数据的固有难度和有限可扩展性导致严重的数据稀缺，阻碍了数据驱动的灵巧操控策略学习研究。

Method: 设计了渐进式转移框架：首先基于运动学匹配建立灵巧手的主要控制信号；然后训练具有动作空间缩放和拇指引导初始化的残差策略，在统一奖励下动态优化接触交互；最后计算手腕控制轨迹以保持操作语义。

Result: 仅使用人类手部操控视频，系统能为不同任务自动配置参数，在灵巧手、物体类别和任务间平衡运动学匹配和动态优化。实验显示平均转移成功率达73%，能自动生成平滑且语义正确的灵巧手操控。

Conclusion: 该框架能忠实再现人类意图，实现高效率和强泛化能力，为收集机器人灵巧操控数据提供了易于实施和可扩展的方法。
...
### [155] [Dynamic Reconfiguration of Robotic Swarms: Coordination and Control for Precise Shape Formation](https://arxiv.org/abs/2511.10989)
...
<details>
  <summary>Details</summary>
Motivation: 机器人群体中运动和配置的协调是一个具有挑战性的问题，需要决定每个机器人何时何地移动，这计算复杂度高，且物理系统存在测量误差和控制动力学等固有困难。

Method: 采用几何公式，通过适当的控制、定位和映射技术将几何公式映射到物理领域，实现机器人群体配置的无缝过渡。

Result: 开发出的算法能够有效协调机器人群体运动，实现配置间的平滑转换。

Conclusion: 该方法为机器人群体应用开辟了新途径，通过实现更复杂的分布式行为，推动了机器人群体技术的发展。
...
### [156] [Latent-Space Autoregressive World Model for Efficient and Robust Image-Goal Navigation](https://arxiv.org/abs/2511.11011)
...
Result: 相比最先进基线，训练时间减少约3.2倍，规划时间减少约447倍，同时导航性能提升35% SR和11% SPL。

Conclusion: 该方法在保持最先进导航性能的同时，显著提高了计算效率，证明了像素级环境预测对导航并非必要，潜在空间表示足以支持有效导航。
...
### [157] [Miniature Testbed for Validating Multi-Agent Cooperative Autonomous Driving](https://arxiv.org/abs/2511.11022)
*Hyunchul Bae,Eunjae Lee,Jehyeop Han,Minhee Kang,Jaehyeon Kim,Junggeun Seo,Minkyun Noh,Heejin Ahn*

Main category: cs.RO

TL;DR: CIVAT是一个1:15比例的微型测试平台，用于验证协同自动驾驶，包含缩放城市地图、配备车载传感器的自动驾驶车辆和智能基础设施，通过Wi-Fi和ROS2框架实现V2V和V2I通信。
...
Method: 设计和实现了一个1:15比例的微型测试平台，集成V2V和V2I通信，采用发布-订阅模式通过共享Wi-Fi和ROS2框架实现车辆与基础设施之间的信息交换。

Result: 通过基于基础设施的感知和交叉口管理实验验证了系统的有效性，实现了协同驾驶功能。

Conclusion: CIVAT测试平台填补了现有测试平台的空白，为协同自动驾驶技术的验证提供了有效工具。
...
### [158] [Humanoid Whole-Body Badminton via Multi-Stage Reinforcement Learning](https://arxiv.org/abs/2511.11218)
*Chenhao Liu,Leyun Jiang,Yibo Wang,Kairan Yao,Jinchen Fu,Xiaoyu Ren*

Main category: cs.RO

TL;DR: 本文提出了一种基于强化学习的训练框架，用于人形机器人羽毛球运动控制，实现了无需运动先验或专家演示的全身协调控制，包括下肢步法和上肢击球动作。
...
Method: 采用三阶段课程学习：首先学习步法，然后生成精确引导的球拍挥动，最后进行任务聚焦的细化。部署时结合扩展卡尔曼滤波器估计和预测羽毛球轨迹，还引入了无需预测的变体。

Result: 在仿真中，两个机器人连续击球21次；无预测变体实现了与目标已知策略相当的性能。真实世界测试中，控制器模块表现出高精度，击球速度达10m/s，平均回球落点距离3.5m。

Conclusion: 该人形机器人能够在羽毛球运动中实现高度动态且精确的目标击球，并可适应更多对动态性要求高的领域。
...
### [159] [Sashimi-Bot: Autonomous Tri-manual Advanced Manipulation and Cutting of Deformable Objects](https://arxiv.org/abs/2511.11223)
...
TL;DR: Sashimi-Bot是一个自主多机器人系统，专门用于处理鲑鱼腰肉等可变形的体积物体的高级操作和切割，特别是制作生鱼片。


<details>
  <summary>Details</summary>
Motivation: 处理可变形的体积物体在机器人操作中面临巨大挑战，因为这些物体具有柔韧性、脆弱性、可变性以及在交互过程中的不确定性。

Method: 系统使用三个机器人：一个拉直腰肉，一个握住刀具，另一个在切割时协同稳定腰肉。结合深度强化学习、手内工具形状操作、手内工具切割以及视觉和触觉反馈。

Result: 实现了对鲑鱼腰肉等自然起源物体的稳健操作和切割，能够处理其尺寸形状变化、柔韧性和滑溜性等挑战。

Conclusion: 这项工作代表了机器人操作可变形的体积物体的重要里程碑，可能启发和推动其他实际应用的发展。
...
### [160] [Experiences from Benchmarking Vision-Language-Action Models for Robotic Manipulation](https://arxiv.org/abs/2511.11298)
*Yihao Zhang,Yuankai Qi,Xi Zheng*

Main category: cs.RO

TL;DR: 本文对四种代表性VLA模型(ACT、OpenVLA-OFT、RDT-1B、π₀)在机器人操作任务中进行系统评估，建立了标准化评估框架，发现π₀在分布外场景适应性最强，ACT在分布内稳定性最高。


<details>
  <summary>Details</summary>
Motivation: 虽然VLA模型在机器人领域应用前景广阔，但缺乏系统的真实世界评估和跨模型比较，需要建立标准化评估框架来指导实际部署。

Method: 在仿真和ALOHA Mobile平台上对四种VLA模型进行四个操作任务的基准测试，从准确性效率、适应性和语言指令跟随精度三个维度评估。

Result: π₀在分布外场景表现出最强的适应性，ACT在分布内场景提供最高的稳定性，不同模型在计算需求、数据扩展行为和失败模式方面存在差异。

Conclusion: VLA模型架构在精度、泛化性和部署成本之间存在实际权衡，这些发现为在真实世界机器人操作任务中选择和部署VLA提供了可操作的见解。
...
### [161] [Simulating an Autonomous System in CARLA using ROS 2](https://arxiv.org/abs/2511.11310)
*Joseph Abdo,Aditya Shibu,Moaiz Saeed,Abdul Maajid Aga,Apsara Sivaprazad,Mohamed Al-Musleh*

Main category: cs.RO

TL;DR: 本文提出了一种基于CARLA模拟器的自主赛车软件堆栈设计方法，通过多传感器融合和ROS 2实现赛道锥桶检测和优化轨迹规划，为FS-AI 2025竞赛做准备。


<details>
  <summary>Details</summary>
Motivation: 自主赛车环境为高速度和不确定性条件下的感知、规划和控制提供了严格的测试平台，目标是开发具有竞争力的驾驶性能以满足Formula Student UK Driverless 2025竞赛要求。

Method: 使用360°激光雷达、立体相机、GNSS和IMU传感器通过ROS 2进行多传感器融合，可靠检测35米范围内的赛道锥桶边界，并考虑车辆动力学和模拟环境因素计算优化轨迹。

Result: 系统在CARLA模拟器中实现了可靠的锥桶检测和高效轨迹规划，完整的自主堆栈在专用车辆(ADS-DV)上进行了广泛验证，并成功移植到包含Jetson AGX Orin、ZED2i相机、Robosense LiDAR和CHCNAV INS的实际硬件平台。

Conclusion: 该方法为自主赛车系统提供了一个完整的软件堆栈解决方案，通过模拟验证和硬件移植验证了系统的可行性和性能，为实际竞赛应用奠定了基础。
...
### [162] [A Comparative Evaluation of Prominent Methods in Autonomous Vehicle Certification](https://arxiv.org/abs/2511.11484)
*Mustafa Erdem Kırmızıgül,Hasan Feyzi Doğruyol,Haluk Bayram*

Main category: cs.RO

TL;DR: 本文评估了自动驾驶车辆认证过程中的主要方法，开发了认证流程管道，并确定了各阶段、参与者和方法应用领域。
...
Method: 比较评估自动驾驶车辆认证过程中的主要方法，开发认证流程管道，确定各阶段、参与者和方法应用领域。

Result: 提出了自动驾驶车辆认证流程的管道框架，明确了认证过程中的各个阶段、相关参与方以及不同方法的应用范围。

Conclusion: 为自动驾驶车辆的认证提供了系统化的方法框架，有助于实现"零愿景"政策目标，确保自动驾驶车辆的安全部署。
...
### [163] [Collaborative Representation Learning for Alignment of Tactile, Language, and Vision Modalities](https://arxiv.org/abs/2511.11512)
*Yiyun Zhou,Mingjing Xu,Jingwei Shi,Quanjiang Li,Jingyuan Chen*

Main category: cs.RO

TL;DR: 提出了TLV-CoRe方法，基于CLIP框架实现触觉-语言-视觉协同表示学习，通过传感器感知调制器和触觉无关解耦学习统一不同传感器的触觉特征，并引入统一桥接适配器增强三模态交互。


<details>
  <summary>Details</summary>
Motivation: 现有触觉传感器缺乏标准化导致冗余特征，阻碍跨传感器泛化；现有方法未能充分整合触觉、语言和视觉模态间的中间通信。

Method: TLV-CoRe方法包含传感器感知调制器统一不同传感器触觉特征、触觉无关解耦学习分离无关触觉特征、统一桥接适配器增强三模态交互。

Result: 实验结果表明TLV-CoRe显著改善了传感器无关表示学习和跨模态对齐性能。

Conclusion: TLV-CoRe为多模态触觉表示提供了新方向，同时提出了RSS评估框架来公平评估触觉模型的鲁棒性、协同性和稳定性。
...
### [164] [Scalable Coverage Trajectory Synthesis on GPUs as Statistical Inference](https://arxiv.org/abs/2511.11514)
...
<details>
  <summary>Details</summary>
Motivation: 传统运动规划方法在处理覆盖运动规划时计算效率有限，难以利用现代并行化框架，因为覆盖规划需要考虑整个轨迹的空间分布而非时间序列。

Method: 将覆盖运动规划问题表述为流匹配视角下的统计推断问题，统一了常用的统计差异度量（如KL散度和Sinkhorn散度）与标准线性二次调节器问题，并将轨迹梯度生成与非线性系统动力学下的控制合成解耦。

Result: 该方法通过在现代计算架构（特别是GPU）上的并行化实现了显著加速，相比基于路径点跟踪的传统方法具有计算优势。

Conclusion: 基于流匹配的覆盖运动规划方法在可扩展性和并行化方面具有明显优势，能够有效提升计算效率。
...
### [165] [Scalable Policy Evaluation with Video World Models](https://arxiv.org/abs/2511.11520)
*Wei-Cheng Tseng,Jinwei Gu,Qinsheng Zhang,Hanzi Mao,Ming-Yu Liu,Florian Shkurti,Lin Yen-Chen*

Main category: cs.RO

TL;DR: 本文提出使用动作条件视频生成模型作为机器人策略评估的可扩展方法，通过将动作条件整合到预训练视频生成模型中，利用互联网规模的在线视频进行预训练，避免收集昂贵的机器人操作视频-动作配对数据。


<details>
  <summary>Details</summary>
Motivation: 评估通用机器人操作策略在现实世界中存在成本高、耗时长、安全风险等问题，而手动创建仿真环境又面临工程量大和仿真到现实的差距问题。

Method: 将动作条件整合到现有的预训练视频生成模型中，利用互联网规模的在线视频进行预训练，无需收集昂贵的机器人操作视频-动作配对数据。

Result: 实验表明，在策略排序和实际策略值与预测策略值相关性等各种指标上，这些模型为无需真实世界交互的策略评估提供了有前景的方法。

Conclusion: 动作条件视频生成模型为机器人策略评估提供了一种有前景的可扩展方法，能够有效减少对真实世界交互的依赖。
...
### [166] [Terrain Costmap Generation via Scaled Preference Conditioning](https://arxiv.org/abs/2511.11529)
*Luisa Mao,Garret Warnell,Peter Stone,Joydeep Biswas*

Main category: cs.RO

TL;DR: SPACER是一种新颖的越野地形成本图生成方法，结合了合成数据训练实现泛化能力，并通过用户指定的缩放偏好上下文实现快速测试时成本调整。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么支持快速测试时成本调整（如语义分割），要么能泛化到新地形类型（如表示学习方法），但无法同时实现两者。

Method: SPACER在训练时利用合成数据实现泛化，通过用户指定的缩放偏好上下文实现测试时成本快速调整。

Result: 在大规模航空地图上的实验表明，SPACER在地形导航成本图生成方面优于其他方法，在7个环境中的5个实现了全局路径规划中不同偏好的最低遗憾值。

Conclusion: SPACER成功解决了现有方法无法同时实现泛化能力和快速测试时适应的局限性，为越野自主机器人导航提供了高质量的地形成本图生成方案。
...
### [167] [Volumetric Ergodic Control](https://arxiv.org/abs/2511.11533)
*Jueun Kwon,Max M. Sun,Todd Murphey*

Main category: cs.RO

TL;DR: 提出了一种新的体积状态表示的遍历控制方法，用于优化机器人的空间覆盖性能，相比传统点模型方法提高了两倍以上的覆盖效率。


<details>
  <summary>Details</summary>
Motivation: 现有遍历控制方法将机器人建模为非体积点，但实际上机器人通过具有物理体积的身体和传感器与环境交互，需要更准确的体积表示。

Method: 引入基于体积状态表示的新遍历控制公式，支持任意基于样本的体积模型，在保持渐进覆盖保证的同时仅增加最小计算开销。

Result: 在搜索和操作任务中，该方法相比标准遍历控制方法提高了两倍以上的覆盖效率，同时保持100%的任务完成率。

Conclusion: 该方法在机械擦除等机器人任务中表现出有效性，为体积感知的遍历控制提供了实用解决方案。
...
### [168] [The Second Law of Intelligence: Controlling Ethical Entropy in Autonomous Systems](https://arxiv.org/abs/2511.10704)
*Samih Fadli*

Main category: cs.AI

TL;DR: 该论文提出了人工智能伦理熵的第二定律，证明未经约束的AI系统会自发偏离目标，需要持续的对齐工作来维持稳定性。


<details>
  <summary>Details</summary>
Motivation: 为了解决AI系统在训练后自发偏离预期目标的问题，作者希望建立一个类似热力学第二定律的理论框架来量化AI对齐的稳定性要求。

Method: 定义了基于目标概率分布的伦理熵S = -Σ p(g_i; theta) ln p(g_i; theta)，证明了熵的时间导数dS/dt ≥ 0，并推导出临界对齐工作边界gamma_crit = (lambda_max / 2) ln N。
...
### [169] [Picking a Representative Set of Solutions in Multiobjective Optimization: Axioms, Algorithms, and Experiments](https://arxiv.org/abs/2511.10716)
*Niclas Boehmer,Maximilian T. Wittmann*

Main category: cs.AI

TL;DR: 本文研究多目标决策中的Pareto修剪问题，通过将问题重新定义为多赢家投票问题，分析了现有质量度量的不足，提出了新的directed coverage度量，并研究了计算复杂性边界。


<details>
  <summary>Details</summary>
Motivation: 现实决策问题通常涉及多个目标优化，所有Pareto最优解都是候选解，决策者需要从中选择。为减轻决策者认知负担，需要计算固定大小的Pareto最优解子集来最好地代表完整解集。

Method: 将Pareto修剪问题重新定义为多赢家投票问题，对现有质量度量进行公理化分析，引入新的directed coverage度量，分析各种质量度量的计算复杂性边界，并进行实验评估。

Result: 发现现有质量度量存在反直觉行为，新提出的directed coverage度量在各种设置下表现具有竞争力甚至更优，质量度量的选择对所选解集特征有决定性影响。

Conclusion: 质量度量的选择在多目标决策中至关重要，新提出的directed coverage度量提供了更好的性能，同时明确了计算复杂性的边界条件。
...
### [170] [Structure-Aware Encodings of Argumentation Properties for Clique-width](https://arxiv.org/abs/2511.10767)
*Yasir Mahmood,Markus Hecher,Johanna Groven,Johannes K. Fichte*

Main category: cs.AI

TL;DR: 该论文研究了如何将抽象论证框架问题编码为(Q)SAT问题，并设计了保持团宽度的有向分解引导(DDG)归约方法。


<details>
  <summary>Details</summary>
Motivation: 现代SAT求解器在小树宽实例上表现高效，但团宽度作为更一般的图参数，在稠密图上也可能很小。目前对团宽度编码能力的研究较少，作者希望探索基于团宽度的编码能力。

Method: 设计了从论证问题到(Q)SAT的新颖归约方法，这些归约线性保持团宽度，形成了有向分解引导(DDG)归约。

Result: 为所有论证语义（包括计数）建立了新结果，证明DDG归约的开销在合理假设下无法显著改进。

Conclusion: 该研究开启了理解团宽度编码能力的新方向，为抽象论证框架提供了高效的(Q)SAT编码方法，且归约质量接近最优。
...
### [171] [Potential Outcome Rankings for Counterfactual Decision Making](https://arxiv.org/abs/2511.10776)
*Yuta Kawakami,Jin Tian*

Main category: cs.AI

TL;DR: 本文研究了反事实决策中的新规则，引入了潜在结果排序概率(PoR)和获得最佳潜在结果概率(PoB)两个新指标，建立了识别定理和边界估计方法，并通过数值实验验证了估计器的性能。
...
Method: 提出了潜在结果排序概率(PoR)和获得最佳潜在结果概率(PoB)两个新指标，建立了这些指标的识别定理和边界估计方法，并开发了相应的估计方法。

Result: 通过数值实验验证了估计器的有限样本性质，并在真实世界数据集上展示了这些方法的应用效果。

Conclusion: 新提出的PoR和PoB指标为反事实决策提供了有效的工具，能够帮助决策者更准确地评估不同行动在个体层面的潜在结果排序和获得最佳结果的可能性。
...
### [172] [From Efficiency to Adaptivity: A Deeper Look at Adaptive Reasoning in Large Language Models](https://arxiv.org/abs/2511.10788)
*Chao Wu,Baoheng Li,Mingchen Gao,Zhenyi Wang*

Main category: cs.AI

TL;DR: 这篇论文从适应性角度重新审视大语言模型的推理能力，提出根据任务难度和不确定性动态分配推理努力的概念框架，并建立了系统的分类法来组织现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在推理时采用统一的策略，无法根据任务复杂度调整推理努力，导致简单问题过度推理而复杂问题推理不足。

Method: 将自适应推理形式化为控制增强的策略优化问题，提出基于训练的方法（强化学习、监督微调、学习控制器）和免训练方法（提示条件化、反馈驱动停止、模块化组合）的分类法。

Result: 建立了连接经典认知范式与算法实现的框架，澄清了不同机制在实践中实现自适应推理的方式，并支持跨策略的系统比较。
...
### [173] [HyperComplEx: Adaptive Multi-Space Knowledge Graph Embeddings](https://arxiv.org/abs/2511.10842)
...
Conclusion: HyperComplEx通过自适应维度分配实现接近线性的扩展性，为可扩展知识图谱嵌入研究提供了有效解决方案。
...
### [174] [Advanced Tool for Traffic Crash Analysis: An AI-Driven Multi-Agent Approach to Pre-Crash Reconstruction](https://arxiv.org/abs/2511.10853)
...
<details>
  <summary>Details</summary>
Motivation: 传统的交通事故重建依赖人类专家，在处理不完整多模态数据时往往产生不一致的结果，需要开发更精确的AI系统来处理异构碰撞数据。

Method: 采用两阶段协作框架：第一阶段从多模态输入生成自然语言事故重建；第二阶段结合重建结果与时间性事件数据记录器进行深入事故推理。系统处理了277起追尾前车减速事故数据。

Result: 在39个复杂追尾事故案例评估中，框架实现了100%的准确率，成功识别最相关EDR事件并正确区分撞击与被撞车辆，即使在处理不完整数据时也保持稳健性能。
...
### [175] [LLM enhanced graph inference for long-term disease progression modelling](https://arxiv.org/abs/2511.10890)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法过于简化大脑连接性的复杂关系，假设单一模态的大脑连接组作为疾病传播基质，导致病理传播预测不准确，特别是在长期进展期间。同时，纯数据驱动的方法由于缺乏适当约束而面临可识别性问题。

Method: 利用大型语言模型作为区域变量相互作用的专家指导，通过LLM合成多模态关系并整合多种疾病驱动机制，同时优化：1）从个体层面观察构建长期疾病轨迹；2）具有更好可识别性的生物约束图结构。
...
### [176] [Multi-Agent Legal Verifier Systems for Data Transfer Planning](https://arxiv.org/abs/2511.10925)
*Ha-Thanh Nguyen,Wachara Fungwacharakorn,Ken Satoh*

Main category: cs.AI

TL;DR: 提出多智能体法律验证器，通过专门化智能体分解合规检查任务，在APPI数据转移规划中实现72%准确率，比单智能体基线提高21个百分点。


<details>
  <summary>Details</summary>
Motivation: 在严格的隐私法规（如日本个人信息保护法APPI）下，AI驱动的数据转移规划中的法律合规性变得越来越关键。

Method: 多智能体法律验证器，将合规检查分解为法规解释、业务背景评估和风险评估三个专门化智能体，通过结构化合成协议进行协调。

Result: 在200个APPI第16条修正案案例数据集上，系统达到72%准确率，比单智能体基线提高21个百分点，在明确合规案例中达到90%准确率，同时保持对明确违规的完美检测。

Conclusion: 领域专业化和协调推理能够显著提高法律AI性能，为可信赖和可解释的自动化合规验证提供可扩展且法规感知的框架。
...
### [177] [Requirements for Aligned, Dynamic Resolution of Conflicts in Operational Constraints](https://arxiv.org/abs/2511.10952)
...
<details>
  <summary>Details</summary>
Motivation: 尽管经过广泛训练，自主AI系统仍会遇到没有可用行动方案能完全满足所有操作约束的场景，需要超越训练策略来做出符合人类期望的决策。

Method: 通过分析和实证案例研究，探讨了智能体如何整合规范性、实用性和情境性理解来选择更符合期望的行动方案。

Result: 识别了智能体在复杂现实环境中做出稳健决策所需的知识类型，包括整合规范、实用和情境理解的能力。

Conclusion: 自主AI系统需要具备整合多种知识类型的能力，才能在复杂环境中选择与人类期望一致的行动方案。
...
### [178] [Faster Symmetry Breaking Constraints for Abstract Structures](https://arxiv.org/abs/2511.11029)
*Özgür Akgün,Mun See Chang,Ian P. Gent,Christopher Jefferson*

Main category: cs.AI

TL;DR: 本文提出了一种新的不完整方法来打破抽象结构的对称性，通过更好地利用其表示方式来处理约束编程中的对称性问题。


<details>
  <summary>Details</summary>
Motivation: 在约束编程中，抽象结构需要转换为求解器支持的表示形式，而对称性破坏技术在处理抽象变量时会产生大量复杂约束，性能表现不佳。

Method: 通过更好地利用抽象结构的表示方式来打破对称性，特别针对不可区分对象产生的对称性类型。
...
### [179] [Key Decision-Makers in Multi-Agent Debates: Who Holds the Power?](https://arxiv.org/abs/2511.11040)
*Qian Zhang,Yan Zheng,Jinyi Liu,Hebin Liang,Lanjun Wang*

Main category: cs.AI

TL;DR: 研究发现多智能体辩论中的角色分配策略对推理性能有显著影响，提出'真理最后'策略可提升22%性能，并开发MADC策略通过一致性评估来优化辩论机制。
...
Method: 提出'真理最后'角色分配策略，并开发多智能体辩论一致性(MADC)策略，通过路径一致性评估独立角色间的一致性，模拟一致性得分最高的角色作为真理。

Result: 在9个LLM模型上的验证显示，MADC策略持续表现出先进性能，有效克服了MAD的性能瓶颈，在推理任务中提升性能达22%。

Conclusion: MADC策略为LLM智能体扩展提供了关键改进路径，通过系统性模拟和优化辩论核心机制，显著提升了多智能体辩论在推理任务中的表现。
...
### [180] [Autonomous Vehicle Path Planning by Searching With Differentiable Simulation](https://arxiv.org/abs/2511.11043)
...
Conclusion: DSS框架通过结合规划梯度和随机搜索，有效提升了自动驾驶的规划性能
...
### [181] [Satisficing and Optimal Generalised Planning via Goal Regression (Extended Version)](https://arxiv.org/abs/2511.11095)
...
<details>
  <summary>Details</summary>
Motivation: 解决广义规划任务，即合成能够解决相关规划问题族的程序，提高规划效率。

Method: 对每个训练问题，按顺序为每个目标原子计算最优计划，执行目标回归，并将输出提升为一阶条件-动作规则集合。

Result: 实验表明在合成成本、规划覆盖率和解质量三个指标上显著优于现有最先进的广义规划器。

Conclusion: 该方法能够学习有效的广义计划和搜索空间剪枝公理，在经典和数值规划领域都有良好表现。
...
### [182] [GGBench: A Geometric Generative Reasoning Benchmark for Unified Multimodal Models](https://arxiv.org/abs/2511.11134)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估判别性理解或无约束图像生成，无法衡量生成推理的集成认知过程，需要新的评估方法来测试统一多模态模型的综合能力。

Method: 利用几何构造作为测试平台，因为它天然需要语言理解和精确视觉生成的融合，设计了GGBench基准测试来系统诊断模型的推理和构造能力。

Result: 提出了一个全面的评估框架，能够更严格地评估下一代智能系统的能力，为统一多模态模型的生成推理能力设定了新标准。

Conclusion: 几何构造是评估生成推理的理想测试平台，GGBench填补了现有评估方法的空白，为统一多模态模型的综合能力评估提供了新标准。
...
### [183] [Multi-agent Undercover Gaming: Hallucination Removal via Counterfactual Test for Multimodal Reasoning](https://arxiv.org/abs/2511.11182)
*Dayong Liang,Xiao-Yong Wei,Changmeng Zheng*

Main category: cs.AI

TL;DR: 本文提出了多智能体卧底游戏（MUG）协议来解决LLM推理中的幻觉问题，通过引入反事实测试和多模态证据来检测幻觉智能体，提升多模态推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体辩论（MAD）方法假设所有辩论者都是理性的，但现实中智能体本身容易产生幻觉，因此需要更可靠的检测机制。

Method: MUG协议受"谁是卧底"游戏启发，通过修改参考图像引入反事实证据，观察智能体是否能准确识别变化，从而检测幻觉智能体。
...
### [184] [STaR: Towards Cognitive Table Reasoning via Slow-Thinking Large Language Models](https://arxiv.org/abs/2511.11233)
*Huajian Zhang,Mingyue Cheng,Yucong Luo,Xiaoyu Tao*

Main category: cs.AI

TL;DR: STaR框架通过慢思考机制增强大语言模型的表格推理能力，采用两阶段难度感知强化学习和轨迹级不确定性量化，显著提升了推理深度、稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有表格推理方法存在两个关键局限：(1) 推理过程缺乏人类认知的深度和迭代精炼特性；(2) 推理过程不稳定，影响下游应用的可靠性。
...
### [185] [AIonopedia: an LLM agent orchestrating multimodal learning for ionic liquid discovery](https://arxiv.org/abs/2511.11257)
*Yuqi Yin,Yibo Fu,Siyuan Wang,Peng Sun,Hongyu Wang,Xiaohui Wang,Lei Zheng,Zhiyong Li,Zhirong Liu,Jianji Wang,Zhaoxi Sun*

Main category: cs.AI

TL;DR: AIonopedia是首个基于大语言模型的离子液体发现智能体，通过多模态领域基础模型实现准确性质预测和分层搜索架构，在真实湿实验室验证中表现出优异的泛化能力。
...
Result: 模型在文献报告系统上表现出有效的离子液体改性能力，真实湿实验室验证证实了其在实际应用中的有效性，特别是在具有挑战性的分布外任务上展现出卓越的泛化能力。
...
### [186] [A Workflow for Full Traceability of AI Decisions](https://arxiv.org/abs/2511.11275)
*Julius Wenzel,Syeda Umaima Alam,Andreas Schmidt,Hanwei Zhang,Holger Hermanns*

Main category: cs.AI

TL;DR: 本文提出了一种通过强制记录AI训练和推理中每个组件的文档化方法，构建了首个支持生成防篡改、可验证且全面的AI决策追踪的工作流程。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在决策过程文档化方面存在严重不足，导致无法追溯决策依据，这在AI决策可能侵犯人权或造成伤害的高风险场景下尤为危险。

Method: 扩展DBOM概念，利用机密计算技术构建有效的工作流程，强制记录AI训练和推理过程中的每个组件，确保文档的完整性。

Result: 开发了一个运行的工作流程，能够生成防篡改、可验证且全面的AI决策追踪，并通过蘑菇识别应用进行了演示。

Conclusion: 该方法为解决AI决策责任链追溯问题提供了实用且根本的解决方案，为AI系统在法律环境下的可问责性奠定了基础。
...
### [187] [Can You Tell the Difference? Contrastive Explanations for ABox Entailments](https://arxiv.org/abs/2511.11281)
*Patrick Koopmann,Yasir Mahmood,Axel-Cyrille Ngonga Ngomo,Balram Tiwari*

Main category: cs.AI

TL;DR: 本文提出了对比ABox解释的概念，用于回答"为什么a是C的实例而b不是？"这类问题。相比单独解释正向蕴含或缺失蕴含的方法，对比解释同时考虑两者，聚焦于a和b之间的相关共性和差异。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能单独解释正向蕴含（为什么知识库蕴含C(a)）或缺失蕴含（为什么不蕴含C(b)），缺乏同时考虑两者的对比解释方法，无法有效揭示个体间的相关差异。

Method: 针对描述逻辑本体论中的ABox推理，开发了对比解释的适当概念，分析了不同变体在不同最优性标准下的计算复杂度，涵盖了轻量级和更表达性强的描述逻辑。

Result: 实现了一种计算对比解释变体的方法，并在现实知识库的生成问题上进行了评估。

Conclusion: 对比ABox解释能够有效回答个体间分类差异的问题，通过同时考虑正向和缺失蕴含，聚焦于相关共性和差异，为描述逻辑推理提供了新的解释视角。
...
### [188] [EcoAlign: An Economically Rational Framework for Efficient LVLM Alignment](https://arxiv.org/abs/2511.11301)
*Ruoxi Cheng,Haoxuan Ma,Teng Ma,Hongyi Zhang*

Main category: cs.AI

TL;DR: EcoAlign是一个推理时框架，将大型视觉语言模型对齐重新定义为经济理性的搜索问题，通过前瞻性函数动态权衡安全性、效用和成本，在降低计算成本的同时实现或超越最先进的安全性和效用。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型存在复杂的越狱漏洞，现有对齐方法在安全性、效用和运营成本之间难以平衡，且仅关注最终输出的过程盲目性会浪费大量计算预算在不安全的推理上。

Method: 将LVLM视为有限理性智能体，增量扩展思维图，使用前瞻性函数（类似净现值）对行动进行评分，动态权衡预期安全性、效用和成本与剩余预算，并通过最薄弱环节原则强制执行路径安全性。

Result: 在3个闭源和2个开源模型的6个数据集上进行广泛实验，表明EcoAlign以较低计算成本匹配或超越最先进的安全性和效用。
...
### [189] [RLSLM: A Hybrid Reinforcement Learning Framework Aligning Rule-Based Social Locomotion Model with Human Social Norms](https://arxiv.org/abs/2511.11323)
*Yitian Kou,Yihe Gu,Chen Zhou,DanDan Zhu,Shuguang Kuai*

Main category: cs.AI

TL;DR: RLSLM是一个混合强化学习框架，将基于规则的社交运动模型集成到强化学习的奖励函数中，通过量化人类舒适度来实现社会对齐的导航策略，在VR实验中优于现有方法。
...
Method: 提出RLSLM混合强化学习框架，将基于经验行为实验的社交运动模型集成到奖励函数中，生成方向敏感的社交舒适场，联合优化机械能量和社交舒适度。

Result: 在沉浸式VR人机交互实验中，RLSLM在用户体验方面优于最先进的基于规则模型，消融和敏感性分析显示其可解释性显著优于传统数据驱动方法。

Conclusion: 这项工作提出了一种可扩展的、以人为中心的方法，有效整合认知科学和机器学习，用于现实世界的社交导航。
...
### [190] [Robust and Efficient Communication in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2511.11393)
*Zejiao Liu,Yi Li,Jiali Wang,Junqi Tu,Yitian Hong,Fangfei Li,Yang Liu,Toshiharu Sugawara,Yang Tang*

Main category: cs.AI

TL;DR: 本调查系统回顾了在多智能体强化学习中，面对现实通信约束（如消息扰动、传输延迟和有限带宽）时的鲁棒高效通信策略进展，重点关注自动驾驶、分布式SLAM和联邦学习三个应用领域，并提出了通信、学习和鲁棒性协同设计的未来研究方向。
...
Method: 系统调查方法，分析在消息扰动、传输延迟和有限带宽等现实通信约束下的MARL通信策略进展。

Result: 识别了低延迟可靠性、带宽密集型数据共享和通信隐私权衡等核心挑战，并聚焦于协作自动驾驶、分布式SLAM和联邦学习三个应用领域。
...
### [191] [CURENet: Combining Unified Representations for Efficient Chronic Disease Prediction](https://arxiv.org/abs/2511.11423)
...
TL;DR: CURENet是一个多模态模型，整合了非结构化临床笔记、实验室测试和患者时间序列数据，利用LLM处理临床文本和文本化实验室测试，使用Transformer编码器处理纵向序列就诊数据，在慢性疾病预测中实现了超过94%的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前大多数预测模型未能充分捕捉多模态电子健康记录数据之间的交互、冗余和时间模式，往往只关注单一数据类型或忽视这些复杂性，而医生需要综合这些多模态和时间源数据来形成对患者健康的全面视图。

Method: CURENet结合了大型语言模型（用于临床文本处理和文本化实验室测试）和Transformer编码器（用于纵向序列就诊数据），能够捕捉不同形式临床数据之间的复杂交互。

Result: 在公开的MIMIC-III和私有的FEMH数据集上评估，CURENet在多标签框架下预测前10种慢性疾病时实现了超过94%的准确率。
...
### [192] [Experience-Guided Adaptation of Inference-Time Reasoning Strategies](https://arxiv.org/abs/2511.11519)
*Adam Stein,Matthew Trager,Benjamin Bowman,Michael Kleinman,Aditya Chattopadhyay,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: EGuR是一个经验引导的推理系统，能够在推理时动态生成完整的计算策略，包括LLM调用、工具使用、采样参数和控制逻辑，通过元策略实现自适应问题解决。
...
Method: 使用基于LLM的元策略生成完整策略，包含两个组件：Guide基于当前问题和历史经验生成候选策略，Consolidator整合执行反馈改进未来策略生成。

Result: 在五个挑战性基准测试中，EGuR比最强基线准确率提升高达14%，计算成本降低高达111倍，且随着经验积累性能持续提升。

Conclusion: EGuR通过动态策略生成实现了推理时的自适应问题解决，显著提升了AI系统的性能和效率。
...
### [193] [Aligning Machiavellian Agents: Behavior Steering via Test-Time Policy Shaping](https://arxiv.org/abs/2511.11551)
*Dena Mujtaba,Brian Hu,Anthony Hoogs,Arslan Basharat*

Main category: cs.AI

TL;DR: 本文提出了一种基于模型引导策略塑造的测试时对齐技术，用于解决AI智能体在复杂动态环境中与人类价值观对齐的挑战。该方法无需重新训练智能体，通过策略塑造精确控制个体行为属性，在伦理对齐和奖励最大化之间实现平衡。


<details>
  <summary>Details</summary>
Motivation: 部署决策AI智能体面临的关键挑战是在复杂动态环境中保持与人类价值观或准则的对齐。仅以达成目标为训练目的的智能体可能采取有害行为，暴露出奖励函数最大化与保持对齐之间的权衡。对于预训练智能体，确保对齐尤其困难，因为重新训练成本高且过程缓慢。

Method: 提出基于模型引导策略塑造的测试时对齐技术。首先训练RL智能体在各自游戏中最大化奖励，然后在测试时通过场景-动作属性分类器应用策略塑造，确保决策与伦理属性对齐。使用MACHIAVELLI基准进行评估，该基准包含134个基于文本的游戏环境和数千个涉及伦理决策的标注场景。

Result: 测试时策略塑造为缓解不同环境和对齐属性中的不道德行为提供了有效且可扩展的解决方案。与先前的训练时方法和通用智能体相比，该方法在控制个体行为属性、跨不同RL环境泛化以及实现伦理对齐与奖励最大化之间的原则性权衡方面表现出色。

Conclusion: 测试时策略塑造是一种有效且可扩展的解决方案，能够缓解不同环境和对齐属性中的不道德行为，无需重新训练智能体即可实现伦理对齐与奖励最大化之间的平衡。
