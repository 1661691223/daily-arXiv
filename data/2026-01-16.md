...
TL;DR: 提出格式解耦强化学习(FD-RL)，通过熵值筛选格式密集文本，使用格式解耦奖励机制，显著提升OCR模型在格式化文本上的性能


<details>
  <summary>Details</summary>
Motivation: 现有OCR模型在处理格式化文本（如公式、表格）时表现出显著更高的熵值，表明模型输出不确定性高，需要改进格式化文本的识别能力

Method: 提出格式解耦强化学习(FD-RL)：1) 基于熵值的数据过滤策略识别格式密集实例；2) 针对不同格式类型设计格式解耦奖励机制；3) 实现格式级别验证而非token级别记忆

Result: 在OmniDocBench基准测试中获得90.41的平均分，为端到端模型在该流行基准上创造了新记录；通过全面的消融研究验证了各策略的有效性

Conclusion: FD-RL通过利用高熵模式进行针对性优化，有效解决了OCR模型在格式化文本识别中的不确定性，显著提升了整体性能
...
TL;DR: 该研究探索了视觉语言模型和生成式图像模型在输入变换下的鲁棒性和公平性问题，特别是图像旋转和分布偏移导致的偏见传播和性能下降，并提出了旋转鲁棒的缓解策略。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型和生成式图像模型在多模态任务中表现出色，但其在输入变换下的鲁棒性和公平性尚未得到充分探索。当前模型在面对图像旋转和分布偏移时，可能存在偏见传播和性能下降的问题。

Method: 提出了旋转鲁棒的缓解策略，包括数据增强、表示对齐和模型级正则化。通过分析旋转引起的扰动对模型预测、置信度校准和人口统计偏见模式的影响，设计相应的改进方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统取证方法无法有效检测AI生成的图像和复杂编辑工具制作的伪造内容，现有单一范式方法在跨域数据集上准确率低于75%，需要更鲁棒的通用检测方案

Method: 提出分层多尺度框架，通过跨注意力Transformer统一低层伪影检测、中层边界分析和高层语义推理，结合经典图像取证和深度学习技术

Result: 在七个不同测试集上平均准确率达到86.8%，对JPEG压缩具有强鲁棒性（Q=70时83%准确率），像素级伪造定位F1分数0.76，各分层组件贡献4-10%准确率提升

Conclusion: ForensicFormer架起了经典图像取证与现代深度学习的桥梁，为实际部署提供了实用解决方案，在未知篡改技术的情况下仍能保持高性能
...
<details>
  <summary>Details</summary>
Motivation: 传统图像配准方法基于亮度恒常假设，当源图像和目标图像存在系统性强度差异（域偏移）时，这一假设被破坏，导致对应关系估计变得不适定。

Method: 提出SAR-Net统一框架，通过场景-外观解耦将观测图像分解为域不变场景表示和域特定外观编码，通过重渲染实现配准。建立了理论条件证明这种分解能够实现一致的跨域对齐。

Result: 在双向扫描显微镜数据集上验证，SAR-Net达到0.885 SSIM和0.979 NCC，比最强基线提升3.1倍，同时保持实时性能（77 fps）。消融研究表明场景一致性和域对齐损失都是必要的。

Conclusion: SAR-Net通过场景-外观解耦有效解决了跨域图像配准问题，在存在系统性强度差异的情况下实现了准确的对齐，为计算机视觉和医学成像中的域偏移配准挑战提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 具身AI中的语义信息具有多源多阶段特性，传统方法难以充分利用以实现稳定的感知-行动循环。随着环境复杂度增加和任务开放度提升，需要更通用、鲁棒的语义处理能力。

Method: 提出"语义生命周期"统一框架，从整体视角捕捉语义知识的连续流动和维护，而非传统孤立模块处理。基于此框架分析具身AI中语义知识的三个关键阶段：获取、表示和存储。

Result: 基础模型通过跨领域泛化能力和丰富语义先验，重塑了具身AI研究格局。语义生命周期框架为理解具身AI中语义知识的演化提供了系统性视角。

Conclusion: 基础模型驱动的具身AI语义处理需要统一框架来理解语义知识的完整生命周期。未来研究应关注语义知识的获取、表示和存储的整合与优化。
...
TL;DR: 该论文提出使用DLRT流形约束优化框架压缩基于视觉Transformer的地理空间基础模型，在迁移学习过程中实现强压缩同时保持任务特定精度，优于LoRA等现成低秩方法。


<details>
  <summary>Details</summary>
Motivation: 将地理空间基础模型部署到资源受限的边缘设备需要紧凑架构，但大参数量和压缩导致的精度损失限制了实际应用。需要一种方法能在压缩模型的同时保持下游任务性能。

Method: 利用DLRT（流形约束优化）框架在迁移学习过程中压缩大型视觉Transformer地理空间基础模型。通过强制实施与下游目标对齐的结构化低维参数化，实现强压缩同时保持任务特定精度。

Result: 该方法优于LoRA等现成低秩方法。在多个地理空间基准测试中证实了显著的参数减少，同时精度损失最小，实现了高性能的端上地理空间模型。

Conclusion: DLRT框架能够有效压缩地理空间基础模型，在保持高下游性能的同时大幅减少参数，使高性能地理空间模型能够在资源受限的边缘设备上部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CNN和U-Net的脑肿瘤分割方法在肿瘤浸润区域产生噪声边界，需要更准确、边界更清晰的分割方法。

Method: 提出UAMSA-UNet，结合蒙特卡洛Dropout学习数据驱动的平滑先验，融合多尺度特征和注意力机制，使用平滑正则化损失（二元交叉熵加方差惩罚）来抑制虚假波动。
...
### [8] [Sim2real Image Translation Enables Viewpoint-Robust Policies from Fixed-Camera Datasets](https://arxiv.org/abs/2601.09605)
*Jeremiah Coholich,Justin Wit,Robert Azarcon,Zsolt Kira*

Main category: cs.CV

TL;DR: MANGO是一种用于机器人视觉策略的模拟到真实图像翻译方法，通过新颖的分割条件InfoNCE损失、高度正则化的判别器设计和改进的PatchNCE损失，有效解决了相机视角变化带来的分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的机器人操作策略虽然取得了显著进展，但对相机视角变化等分布偏移仍然脆弱。真实的机器人演示数据稀缺且缺乏足够的视角变化，而模拟环境虽然能大规模收集不同视角的数据，但存在视觉模拟到真实的转换挑战。

Method: 提出MANGO方法，包含三个关键组件：1）新颖的分割条件InfoNCE损失，2）高度正则化的判别器设计，3）改进的PatchNCE损失。这些组件对于在模拟到真实转换过程中保持视角一致性至关重要。方法仅需少量固定相机的真实世界数据，但能通过翻译模拟观察生成多样未见过的视角。

Result: MANGO在所有测试的图像翻译方法中表现最佳。使用MANGO增强数据训练的模仿学习策略在非增强策略完全失败的视角上能达到高达60%的成功率。

Conclusion: MANGO通过创新的图像翻译技术有效解决了机器人视觉策略中的视角变化问题，仅需少量真实数据就能生成多样视角，显著提升了策略在未见视角上的鲁棒性和成功率。
...
TL;DR: Thermo-LIO：一种结合热成像与高分辨率LiDAR的多传感器系统，通过多模态数据融合和LiDAR惯性里程计，提升大型结构健康监测的精度和覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 传统二维热成像技术在建筑领域虽然非侵入且可用于缺陷检测，但难以有效评估复杂几何形状、难以接近区域和地下缺陷，需要更先进的监测方法。

Method: 1. 开发热成像与LiDAR的多模态融合方法，实现精确校准和同步；2. 将该融合方法与LiDAR惯性里程计（LIO）集成，实现大规模结构的全覆盖监测。

Result: 通过桥梁和厅堂建筑的案例研究验证，Thermo-LIO能够比传统方法更准确地检测热异常和结构缺陷，提高诊断精度，支持实时处理，扩大检测覆盖范围。

Conclusion: Thermo-LIO系统展示了多模态传感器集成在推进大型民用基础设施结构健康监测方法中的关键作用，为复杂结构的全面监测提供了有效解决方案。
...
### [10] [Fast-ThinkAct: Efficient Vision-Language-Action Reasoning via Verbalizable Latent Planning](https://arxiv.org/abs/2601.09708)
*Chi-Pin Huang,Yunze Man,Zhiding Yu,Min-Hung Chen,Jan Kautz,Yu-Chiang Frank Wang,Fu-En Yang*

Main category: cs.CV

TL;DR: Fast-ThinkAct是一个高效的视觉-语言-动作推理框架，通过可表达的潜在推理实现紧凑而高性能的规划，相比现有推理VLA方法减少高达89.3%的推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的推理VLA方法虽然通过显式思维链（CoT）提高了泛化能力，但由于冗长的推理轨迹导致推理延迟很高，需要一种更高效的推理框架来平衡性能和效率。

Method: 提出Fast-ThinkAct框架，通过从教师模型蒸馏学习高效的潜在思维链推理，采用偏好引导目标来对齐操作轨迹，同时传递语言和视觉规划能力，实现推理增强的策略学习。

Result: 在多样化的具身操作和推理基准测试中，Fast-ThinkAct在保持有效长时程规划、少样本适应和失败恢复能力的同时，相比最先进的推理VLA方法减少了高达89.3%的推理延迟。

Conclusion: Fast-ThinkAct通过可表达的潜在推理实现了高效而紧凑的规划，在显著降低推理延迟的同时保持了强大的性能，为具身控制中的视觉-语言-动作任务提供了有效的解决方案。
...
TL;DR: 该研究通过PoseMaskRefine方法对SAM 2.1进行姿态引导的微调，使其在遮挡情况下仍能实现准确的人体分割，仅需少量高可见度关键点即可预测掩码。


<details>
  <summary>Details</summary>
Motivation: SAM在人体分割方面表现出色，但在遮挡情况下（关键点部分或完全不可见）可能表现不佳。需要改进SAM以处理遮挡场景，同时保持其强大的泛化能力。

Method: 1. 对SAM 2.1进行最小化的编码器修改；2. 提出PoseMaskRefine微调策略，将高可见度的姿态关键点整合到SAM原有的迭代校正过程中；3. 推理时仅选择可见度最高的三个关键点作为提示，减少对常见错误的敏感性。

Result: 改进后的模型在多个数据集上展现出更好的鲁棒性和准确性，能够从单个关键点开始准确预测掩码，实现了遮挡感知的人体分割，同时保持了原始模型的泛化能力。

Conclusion: 姿态引导的SAM微调能够实现有效的遮挡感知人体分割，在保持原始模型泛化能力的同时，显著提升了在遮挡场景下的分割性能。代码和预训练模型将公开提供。
...
TL;DR: STDTrack是一个轻量级目标跟踪框架，通过密集视频采样和时空依赖整合，在保持实时效率的同时缩小了轻量级与高性能跟踪器之间的性能差距。
...
Method: 提出STDTrack框架：1）采用密集视频采样最大化时空信息利用；2）引入时序传播的时空令牌指导每帧特征提取；3）设计多帧信息融合模块（MFIFM）利用历史上下文增强当前依赖；4）构建时空令牌维护器（STM）存储特征，采用基于质量的更新机制确保信息可靠性；5）开发多尺度预测头以适应不同尺寸的目标。

Result: 在六个基准测试中取得了最先进的结果。在GOT-10k上，STDTrack的性能可与某些非实时高性能跟踪器（如MixFormer）相媲美，同时在GPU上达到192 FPS，CPU上达到41 FPS。

Conclusion: STDTrack成功地将可靠的时空依赖整合到轻量级跟踪器中，在保持实时效率的同时显著提升了跟踪性能，缩小了轻量级与高性能跟踪器之间的差距。
...
TL;DR: WEFT提出了一种基于动态小波专家引导的微调范式，通过少量可训练参数将大规模基础模型高效适配到光学遥感图像分割任务中。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常基于中等规模预训练模型，采用全参数微调实现良好性能。更深更大的基础模型能提供更强性能支持，但直接全参数微调会导致GPU内存消耗大、计算成本高，限制了大规模模型在遥感图像分割中的探索。

Method: 提出动态小波专家引导微调范式WEFT：1）引入任务特定小波专家提取器，从不同视角建模小波专家并动态调节输出，生成富含任务信息的可训练特征；2）构建专家引导条件适配器，通过注入可训练特征增强冻结特征的细粒度感知，然后迭代更新两类特征信息，实现高效微调。

Result: 在三个光学遥感图像数据集上超越了21种最先进方法，同时在伪装、自然和医疗场景中均取得最优结果。

Conclusion: WEFT通过少量可训练参数有效解决了大规模基础模型在遥感图像分割任务中的微调难题，为相关应用提供了高效解决方案。
...
Method: 提出SAM-Aug框架：1）从时序序列构建无云合成图像；2）无监督应用SAM生成几何感知掩码先验；3）设计RegionSmoothLoss损失函数，强制模型在SAM生成的区域内保持跨时间帧的预测一致性。

Result: 在PASTIS-R基准测试中，5%标注设置下平均测试mIoU达到36.21%，比现有最佳基线提升2.33个百分点（相对提升6.89%）。在最优分割（seed=42）下达到40.28% mIoU，相对提升11.2%。

Conclusion: SAM等视觉基础模型可作为有效的正则化器，在少样本遥感学习中提供可扩展的即插即用解决方案，无需人工标注或模型微调，为土地覆盖监测提供新途径。
...
TL;DR: 提出基于Qwen3-VL的端到端结构感知多模态推理框架，通过字符感知多模态推理模块（CMRM）和字符槽查询机制，解决车牌识别中图像恢复与字符识别目标不一致的问题，显著提升严重退化条件下的识别性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界车牌识别面临运动模糊、低分辨率、复杂光照等严重退化挑战。传统的"恢复-识别"两阶段范式存在根本缺陷：图像恢复的像素级优化目标与字符识别的语义目标不一致，导致伪影干扰和错误累积。虽然视觉语言模型（VLMs）展现出强大通用能力，但缺乏对车牌字符序列（如固定长度、特定顺序）的显式结构建模。

Method: 提出基于Qwen3-VL的端到端结构感知多模态推理框架。核心创新是字符感知多模态推理模块（CMRM），引入一组可学习的字符槽查询。通过交叉注意力机制，这些查询从视觉特征中主动检索对应字符位置的细粒度证据。然后将这些字符感知表示通过残差调制注入回视觉标记中，使语言模型能够基于显式结构先验进行自回归生成。结合LoRA参数高效微调策略，实现领域自适应同时保留大模型的泛化能力。

Result: 在合成和真实世界严重退化数据集上的大量实验表明，该方法显著优于现有的恢复-识别组合和通用VLMs，验证了将结构化推理融入大模型在低质量文本识别任务中的优越性。

Conclusion: 通过字符感知多模态推理模块和字符槽查询机制，成功将结构化推理融入视觉语言模型，解决了车牌识别中图像恢复与字符识别目标不一致的问题，为低质量文本识别任务提供了有效的端到端解决方案。
...
TL;DR: SkinFlow框架通过优化视觉信息传输效率而非参数扩展，解决了通用大视觉语言模型在皮肤病学中"扩散注意力"的问题，在Fitzpatrick17k基准上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 通用大视觉语言模型在皮肤病学中因"扩散注意力"问题而表现不佳，无法区分细微病理病变与背景噪声。本文挑战了参数扩展是医学精度的唯一路径的假设。

Method: 提出SkinFlow框架，采用虚拟宽度动态视觉编码器在不物理扩展参数的情况下"展开"复杂病理流形，结合两阶段强化学习策略：第一阶段对齐显性医学描述，第二阶段重建隐性诊断纹理，并在受限语义空间内进行。
...
TL;DR: 该研究探讨了基于学习的干扰变异模型在提升深度学习模型对自然干扰鲁棒性的方法，通过在交通标志识别数据集上测试，发现基于模型的方法优于传统基线方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在安全敏感领域中对自然干扰的鲁棒性仍然是一个关键挑战，需要开发更有效的训练方法来提高模型在恶劣条件下的可靠性。

Method: 研究采用基于学习的干扰变异模型生成真实干扰，并开发了混合策略，结合随机覆盖和干扰空间中的对抗性精炼。使用CURE-TSR数据集（包含雪和雨干扰）评估不同严重程度下的准确性、校准性和训练复杂度。

Result: 基于模型的方法在所有干扰类型中始终优于Vanilla、对抗训练和AugMix基线方法。基于模型的对抗训练在所有干扰中提供最强的鲁棒性，但计算成本较高；基于模型的数据增强在T倍降低计算复杂度的同时，性能没有显著下降。

Conclusion: 学习到的干扰模型对于捕捉自然变异至关重要，为在挑战性条件下开发更具弹性和校准性的模型提供了有前景的路径。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI文本到图像系统在建筑领域应用的增加，需要评估这些系统在历史规则严格领域生成准确图像的能力，以了解其局限性和潜在风险。

Method: 使用30个涵盖建筑风格、类型和规范元素的提示词，在5个主流AI图像平台（Adobe Firefly、DALL-E 3、Google Imagen 3、Microsoft Image Generator、Midjourney）上生成图像，每个提示-生成器组合产生4张图像（共600张）。两位建筑历史学家根据预设标准独立评分，通过共识解决分歧。

Result: 常见提示的准确率比罕见提示高2.7倍（p<0.05）。整体准确率有限（最高52%，最低32%，平均42%）。所有平台的全正确结果相似，但全错误结果差异显著：Imagen 3失败最少，Microsoft Image Generator失败最多。定性分析发现过度装饰、中世纪风格与复兴风格混淆、描述性提示误解等模式。

Conclusion: 研究支持需要对AI生成内容进行可见标记、建立未来训练数据集的来源标准，以及对教育中使用AI建筑图像保持谨慎态度。
...
TL;DR: 本文提出N EIoU YOLOv9，一种基于非单调梯度聚焦和几何解耦原理的轻量级检测框架，通过N EIoU损失函数增强弱回归信号，特别适用于农业病害图像中的小目标和低对比度目标。


<details>
  <summary>Details</summary>
Motivation: 农业病害监测中常见小目标和低对比度目标，传统检测方法在这些困难样本上表现不佳，需要一种能够增强弱回归信号、减少梯度干扰的损失函数来提高检测精度。

Method: 提出N EIoU损失函数，结合非单调梯度聚焦和宽度高度解耦优化原理，重新塑造定位梯度。将该损失集成到轻量级YOLOv9t架构中，并在自收集的5908张水稻叶片图像数据集上进行评估。

Result: 在四类病害和健康叶片的检测任务中，N EIoU相比标准CIoU损失实现了90.3%的平均精度，比基线提升4.3%。在Android设备上部署时，平均推理时间为156毫秒/帧，同时保持精度。

Conclusion: 提出的方法在准确性、优化稳定性和计算效率之间取得了良好平衡，适用于边缘农业监测系统，为农业病害的实时检测提供了有效的解决方案。
...
TL;DR: 提出一个面向部署的框架，通过知识蒸馏将高性能医学图像分割模型转换为紧凑的学生模型，在保持准确性的同时大幅降低计算需求，便于在临床环境中部署。


<details>
  <summary>Details</summary>
Motivation: 临床工作流程中的医学图像分割模型部署常受限于本地基础设施的计算资源，且云推理可能受治理和安全政策限制。高性能模型虽然准确度高，但计算需求大，阻碍了在医院环境中的实际部署和长期维护。

Method: 提出部署导向的框架，利用知识蒸馏将高性能分割模型转换为可扩展的紧凑学生模型家族，无需修改推理管道。该方法保持与现有临床系统的架构兼容性，同时实现系统性的容量缩减。

Result: 在多站点脑MRI数据集（1,104个3D体积）和独立测试集（101个病例）上评估，并在腹部CT上测试跨模态泛化能力。在参数减少94%的情况下，蒸馏学生模型保留了教师模型98.7%的分割准确性，同时实现显著效率提升：CPU推理延迟降低67%，且无额外部署开销。

Conclusion: 知识蒸馏为将研究级分割模型转换为可维护、部署就绪的组件提供了实用可靠的途径，适用于现实世界医疗系统的本地临床工作流程。
...
TL;DR: Point-Seg是一种基于Transformer的分割框架，通过点跟踪提供时序线索，实现超声心动图视频中心肌的稳定一致分割，在低质量数据中表现更优，并能提供像素级运动信息。


<details>
  <summary>Details</summary>
Motivation: 超声心动图视频中的心肌分割面临对比度低、噪声大和解剖结构变异等挑战。传统深度学习方法要么独立处理帧（忽略时序信息），要么依赖基于记忆的特征传播（随时间累积误差），需要更稳定一致的解决方案。

Method: 提出Point-Seg框架：1）使用在合成超声心动图数据集上训练的点跟踪模块跟踪关键解剖标志点的轨迹；2）利用这些轨迹提供显式的运动感知信号指导分割，减少漂移并消除基于记忆的特征累积需求；3）加入时序平滑损失进一步增强帧间一致性。

Result: 在公开和私有超声心动图数据集上的实验表明：1）在高质量数据中与最先进模型具有统计相似的Dice精度；2）在低质量数据中分割精度更高且时序稳定性更好；3）关键优势是能提供像素级心肌运动信息，这对下游任务如心肌应变测量和区域壁运动异常检测至关重要。

Conclusion: Point-Seg证明点跟踪可作为有效的时序线索实现一致的视频分割，为超声心动图视频中的心肌分割提供了可靠且可泛化的方法。代码已开源。
...
TL;DR: PaGKD提出了一种无需配对图像的组级知识蒸馏框架，利用未配对的WLI和NBI数据进行跨模态学习，显著提升了内窥镜癌症筛查的性能。


<details>
  <summary>Details</summary>
Motivation: 内窥镜癌症筛查中，白光成像(WLI)是标准方法，但窄带成像(NBI)提供更优的诊断细节。现有方法依赖配对的NBI-WLI图像，这在临床实践中成本高且不切实际，导致大量临床数据无法利用。

Method: 提出PaGKD框架，包含两个互补模块：1) 组级原型蒸馏(GKD-Pro)：通过共享病灶感知查询提取模态不变语义原型；2) 组级密集蒸馏(GKD-Den)：通过激活导出的关系图引导组感知注意力，进行密集跨模态对齐。
...
TL;DR: Affostruction：从RGBD图像进行功能可供性定位的生成框架，通过重建完整几何形状并在包括不可见区域的全形状上定位可供性，解决了现有方法仅预测可见表面区域的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能预测物体可见表面的可供性区域，但实际应用中需要对完整形状（包括不可见区域）进行可供性定位，这需要从部分观测中重建完整几何形状。

Method: 提出Affostruction框架：1）通过稀疏体素融合的生成式多视角重建，在保持恒定token复杂度的同时外推不可见几何；2）基于流的功能可供性定位，捕捉可供性分布的固有模糊性；3）可供性驱动的主动视角选择，利用预测的可供性进行智能视角采样。

Result: 在可供性定位上达到19.1 aIoU（提升40.4%），在3D重建上达到32.67 IoU（提升67.7%），能够在完整形状上实现准确的可供性预测。

Conclusion: Affostruction通过生成式重建和流式定位，成功解决了从部分观测到完整形状的可供性定位问题，显著提升了性能，为机器人交互等应用提供了更准确的功能理解能力。
...
TL;DR: COOL-SD提出了一种退火松弛推测解码方法，通过理论分析和优化设计，在保持图像质量的同时显著提升自回归图像生成的推理速度。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型由于序列化特性和图像token的模糊性导致推理速度缓慢，现有松弛推测解码方法缺乏理论依据，需要建立理论基础并改进性能。
...
Conclusion: COOL-SD为松弛推测解码建立了理论基础，通过退火松弛设计实现了自回归图像生成在速度和质量方面的显著提升，为高效图像生成提供了新方法。
...
Method: 提出SpikeVAEDiff两阶段框架：1) 第一阶段使用非常深的变分自编码器(VDVAE)将神经尖峰信号映射到潜在表示，生成低分辨率初步重建；2) 第二阶段使用回归模型将神经尖峰信号映射到CLIP-Vision和CLIP-Text特征，然后利用Versatile Diffusion模型通过图像到图像生成细化图像。

Result: 在Allen Visual Coding-Neuropixels数据集上评估，发现VISI区域表现出最显著的激活并在重建质量中起关键作用。与fMRI方法相比，尖峰数据提供更优的时间和空间分辨率。消融研究表明特定脑区数据显著提升重建性能。

Conclusion: SpikeVAEDiff能够从神经尖峰数据生成高分辨率和语义有意义的图像重建。VISI区域在视觉重建中起关键作用，特定脑区数据对提升重建性能至关重要，为神经解码研究提供了新方法。
...
TL;DR: 该论文提出了一种用于红外目标检测的视觉-语言表示学习范式，通过文本监督来增强特征判别性，在M³FD和FLIR基准上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 红外图像由于对比度低、边缘信息弱，难以提取具有判别性的目标特征，导致在复杂环境（如黑暗、雪、雨）中的检测性能受限。

Method: 提出视觉-语言表示学习范式：1）语义特征对齐（SFA）模块，将目标特征与对应文本特征对齐；2）目标特征解耦（OFD）模块，通过最小化相关性来解耦文本对齐的目标特征和非目标特征；3）将解耦后的目标特征输入检测头。

Result: 在两个基准测试上取得优异性能：M³FD达到83.7% mAP，FLIR达到86.1% mAP，证明了方法的有效性。

Conclusion: 通过引入文本监督的视觉-语言表示学习，能够提取更具判别性、噪声更少的目标特征，显著提升红外目标检测性能。
...
Method: 提出跨层独立可变形描述（CLIDD），使用可学习偏移从独立特征层次直接采样捕捉多尺度细节；采用硬件感知核融合策略最大化推理吞吐量；集成轻量架构与结合度量学习和知识蒸馏的训练协议

Result: 超紧凑变体仅用0.004M参数达到SuperPoint精度，模型大小减少99.7%；高性能配置超越包括DINOv2在内的所有SOTA方法，在边缘设备上超过200 FPS
...
TL;DR: DeTracker是一个针对未稳定卫星视频的多目标跟踪框架，通过全局-局部运动解耦模块分离平台抖动与目标真实运动，并使用时序依赖特征金字塔增强微小目标表征，在模拟和真实卫星视频上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 卫星视频能持续观测地表动态，但在未稳定条件下，平台抖动和微小目标外观特征弱化共同导致多目标跟踪性能下降，需要专门解决方案。

Method: 提出DeTracker联合检测跟踪框架：1) 全局-局部运动解耦模块通过全局对齐和局部细化分离卫星平台运动与真实目标运动；2) 时序依赖特征金字塔模块进行跨帧时序特征融合，增强微小目标表征的连续性和可区分性；3) 构建SDM-Car-SU基准数据集模拟多方向多速度平台运动。

Result: 在模拟和真实未稳定卫星视频上的大量实验表明，DeTracker显著优于现有方法：在SDM-Car-SU数据集上达到61.1% MOTA，在真实卫星视频数据上达到47.3% MOTA。

Conclusion: DeTracker通过运动解耦和时序特征融合有效解决了未稳定卫星视频中的多目标跟踪问题，提高了轨迹稳定性和运动估计精度，为卫星视频分析提供了鲁棒的解决方案。
...
TL;DR: 本文提出了自适应各向异性纹理高斯（A²TG）方法，通过为每个高斯基元配备各向异性纹理，并使用梯度引导的自适应规则确定纹理分辨率和纵横比，显著提高了纹理效率，在降低内存消耗的同时提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有高斯溅射方法为每个基元分配固定大小的方形纹理，导致内存使用效率低下且难以适应场景变化。需要一种更高效的纹理表示方法来改善内存使用和渲染质量。

Method: 提出自适应各向异性纹理高斯（A²TG）表示，为每个高斯基元配备各向异性纹理，采用梯度引导的自适应规则联合确定纹理分辨率和纵横比，实现非均匀、细节感知的纹理分配。

Result: 在多个基准数据集上的实验表明，A²TG方法在保持可比渲染保真度的同时，显著降低了内存需求，优于固定纹理的高斯溅射方法。

Conclusion: 自适应各向异性纹理高斯表示通过更高效的纹理分配策略，在减少内存消耗的同时提升了渲染质量，为高质量实时3D场景渲染提供了更优的解决方案。
...
Method: 提出PhyRPR三阶段流程：1) PhyReason使用大型多模态模型进行物理状态推理，图像生成器合成关键帧；2) PhyPlan确定性地合成可控的粗粒度运动支架；3) PhyRefine通过潜在融合策略将支架注入扩散采样，在保持规划动态的同时细化外观。
...
TL;DR: 提出BAM-MRCD模型，利用MODIS和Sentinel-2多分辨率多源卫星影像，实现高时空分辨率的及时火烧迹地制图


<details>
  <summary>Details</summary>
Motivation: 传统卫星影像在火灾影响区域划分中存在挑战，现有深度学习模型在操作应用中受限于空间分辨率与时间重访频率的权衡，需要快速准确的火烧迹地制图方案

Method: 提出BAM-MRCD深度学习模型，结合MODIS和Sentinel-2多分辨率多源卫星数据，实现高时空分辨率的火烧迹地检测

Result: 模型能够高精度检测小规模火灾，超越类似变化检测模型和基准方法，所有数据和代码已在GitHub开源

Conclusion: BAM-MRCD模型解决了卫星影像在火灾监测中空间分辨率与时间频率的权衡问题，为快速准确的火烧迹地制图提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 全脑分割是MRI分析中的关键但具有挑战性的任务，传统模板配准方法存在局限，现有大型模型如SAM虽然提供可迁移特征表示，但未针对脑分割的高精度需求进行优化

Method: 提出BrainSegNet框架，在SAM基础上集成U-Net跳跃连接和专用模块：1) 混合编码器结合U-Net跳跃连接与SAM的transformer块；2) 多尺度注意力解码器带金字塔池化处理不同大小结构；3) 边界细化模块锐化边缘

Result: 在Human Connectome Project数据集上的实验表明，BrainSegNet在复杂多标签分割任务中优于多种最先进方法，实现了更高的准确性和鲁棒性

Conclusion: BrainSegNet成功将SAM适应于全脑分割任务，通过架构改进实现了对95个脑区的高精度分割，为脑影像分析提供了有效的深度学习解决方案
...
TL;DR: GaussianFluent：首个解决高斯溅射中脆性断裂问题的统一框架，通过生成式模型合成逼真内部结构，结合优化CD-MPM实现高速断裂模拟，支持混合材料和多阶段断裂传播。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯溅射的物理模拟主要针对软性可变形材料，脆性断裂问题尚未解决，原因在于高斯表示缺乏具有连贯纹理的体素内部结构，以及缺少针对高斯的断裂感知模拟方法。

Method: 1. 通过生成式模型引导内部高斯密度化，合成逼真内部结构；2. 集成优化的连续损伤材料点方法（CD-MPM），实现高速脆性断裂模拟；3. 支持混合材料物体和多阶段断裂传播。

Result: 实现了照片级逼真的实时渲染，具有结构一致的内部结构，能够处理复杂场景（包括混合材料物体和多阶段断裂传播），性能远超先前方法。

Conclusion: GaussianFluent是首个解决3D高斯溅射中脆性断裂问题的统一框架，在VR和机器人等下游应用中具有重要潜力，实现了照片级逼真、实时的动态物体状态模拟与渲染。
...
<details>
  <summary>Details</summary>
Motivation: ICT行业训练领域专用大语言模型或构建检索增强生成系统需要大量高价值领域知识，这些知识不仅隐藏在文本模态中，也存在于图像模态中。传统方法只能解析文档中的文本，缺乏图像描述能力；而多模态大语言模型虽然能理解图像，但缺乏足够的领域知识。
...
<details>
  <summary>Details</summary>
Motivation: 随着人工智能的发展，图像处理技术受到广泛关注。超分辨率作为与现实应用密切相关的关键技术，需要增强现有图像质量。由于增强细节对超分辨率任务至关重要，因此需要强调贡献高频信息的像素。

Method: 提出两种方法：1）基于拉普拉斯金字塔的细节损失，通过总损失与细节损失分别生成和控制超分辨率图像与细节图像；2）重复上采样和下采样过程，通过从多个低分辨率特征中提取多样化信息来增强细节损失的效果。

Result: 设计了基于CNN的模型并取得了最先进的结果，超越了所有现有的CNN模型甚至部分基于注意力的模型。将方法应用于小规模注意力模型后，所有添加细节损失的注意力模型相比原始版本都有改进。

Conclusion: 提出的方法能够有效增强超分辨率图像质量，在不同模型结构中都表现出良好的效果，证明了这些方法在提升高频细节方面的有效性。
...
TL;DR: 本文提出两种改进骨肉瘤组织病理学评估的方法：使用放射组学特征作为多模态输入，以及采用分层损失函数优化二分类任务，显著提升了模型在患者级别测试数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 骨肉瘤是一种侵袭性原发性骨恶性肿瘤。新辅助化疗后准确评估肿瘤存活区与非存活区对预后和治疗计划至关重要，但人工评估劳动密集、主观性强且存在观察者间差异。虽然数字病理学实现了自动坏死量化，但深度学习模型在患者级别测试数据上的性能显著下降，需要改进模型泛化能力。

Method: 1. 引入放射组学特征作为多模态输入，虽然这些特征源自图像，但能有效提升分类性能并增强可解释性。2. 采用分层损失函数优化两个二分类任务（肿瘤vs非肿瘤、存活vs非存活），而不是传统的"扁平"三分类任务（非肿瘤、非存活肿瘤、存活肿瘤），通过可训练的任务权重提升各类别性能。

Result: 使用TCIA骨肉瘤肿瘤评估数据集进行实验，证明了每种新方法的优势及其组合效果。与之前的研究相比，在患者级别测试数据上显著提升了模型性能，为该应用在开放数据集上建立了新的最先进性能。

Conclusion: 放射组学特征的多模态输入和分层损失函数都能显著提升骨肉瘤组织病理学评估的模型性能。这两种方法的组合为该临床应用在开放数据集上建立了新的最先进性能，代码和训练模型已开源。
...
Result: 评估20个最先进的MLLMs发现，虽然模型在表层感知上表现良好，但在MSR任务中性能显著下降，经常在多步推理中出现空间迷失和幻觉。通过MSR-9K微调Qwen-VL，在Video-MSR上实现了+7.82%的绝对提升。
...
TL;DR: PrivLEX是首个基于法律定义的个人数据概念的可解释隐私分类器，利用视觉语言模型的识别能力，通过零样本概念检测提供无需训练概念标签的可解释分类。


<details>
  <summary>Details</summary>
Motivation: 当前图像隐私分类器缺乏与法律概念的明确对齐，难以提供可解释的决策依据。需要开发能够基于法律定义的个人数据概念进行解释的隐私分类系统。

Method: 使用视觉语言模型进行零样本概念检测，构建无需概念标签训练的概念瓶颈模型，将分类决策与法律定义的个人数据概念相连接。

Result: PrivLEX能够有效识别图像中存在的个人数据概念，并分析了人类标注者对这类概念敏感度的感知情况。

Conclusion: PrivLEX为图像隐私分类提供了法律对齐的可解释框架，通过零样本VLM概念检测实现了无需显式概念标签训练的可解释隐私分析。
...
<details>
  <summary>Details</summary>
Motivation: 目标检测器通常在分布内表现良好，但在不同基准测试上性能会急剧下降。研究者希望通过设定特异性视角来研究跨数据集目标检测问题，理解不同数据集类型间的迁移模式。

Method: 将基准数据集分为设定无关数据集（包含多样化日常场景）和设定特定数据集（与狭窄环境相关），评估标准检测器家族在所有训练-测试对上的表现。通过封闭标签迁移和开放标签协议（使用CLIP相似度将预测类别映射到最近的目标标签）来区分领域偏移和标签不匹配。

Result: 揭示了跨数据集目标检测的清晰结构：相同设定类型内的迁移相对稳定，跨设定类型的迁移性能显著下降且通常不对称。最严重的性能下降发生在从特定源到通用目标的迁移中，即使在开放标签对齐后仍然存在，表明领域偏移在困难区域占主导地位。开放标签评估带来一致但有界的性能提升，许多校正案例对应于有图像证据支持的语义近似错误。

Conclusion: 研究提供了在设定特异性下跨数据集目标检测的原则性特征描述，并为在分布偏移下评估检测器提供了实用指导。代码将在GitHub上发布。
...
TL;DR: V-DPM将动态点地图扩展到视频输入，在动态场景的3D和4D重建中实现最先进性能，能恢复完整的3D点运动而不仅仅是动态深度。


<details>
  <summary>Details</summary>
Motivation: 现有的动态点地图(DPMs)仅限于图像对，且在处理多视图时需要后处理优化。作者认为DPMs在视频应用中更有价值，因此提出V-DPM来扩展这一概念。

Method: 首先将DPMs重新表述为适合视频输入的形式，最大化表示能力并便于神经预测。然后在强大的3D重建器VGGT基础上实现，使用少量合成数据将其适配为有效的V-DPM预测器。

Result: V-DPM在动态场景的3D和4D重建中达到最先进性能。与VGGT的动态扩展如P3相比，V-DPM不仅能恢复动态深度，还能恢复场景中每个点的完整3D运动。

Conclusion: V-DPM成功将动态点地图扩展到视频领域，通过重新表述DPMs并基于VGGT实现，仅需少量合成数据就能有效预测动态3D内容，显著提升了动态场景重建能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频理解预训练方法依赖于像素级重建，这会导致编码器学习到无关信息（如背景像素颜色）。本文旨在探索基于纯嵌入预测的预训练方法在面部表情识别中的潜力，避免捕捉不相关信息。

Method: 使用预训练的V-JEPA视频编码器，通过预测掩码区域的嵌入（从未掩码区域的嵌入）来学习。然后在RAVDESS和CREMA-D数据集上训练浅层分类器进行面部表情识别。

Result: 在RAVDESS数据集上取得了最先进的性能，在CREMA-D数据集上超越了所有其他基于视觉的方法（WAR提升+1.48%）。跨数据集评估显示出强大的泛化能力。

Conclusion: 基于纯嵌入的预训练方法在面部表情识别中具有显著潜力，能够避免捕捉无关信息并实现强大的泛化性能，为FER领域提供了新的研究方向。
...
TL;DR: 提出一个结合合成数据和扩散模型的数据生成框架，用于增强工业场景中的人-物交互分析，并发布了新的数据集和检测模型。


<details>
  <summary>Details</summary>
Motivation: 工业安全中的自我中心人-物交互分析面临标注数据稀缺的挑战，需要开发鲁棒的模型来处理这一领域特定问题。

Method: 1) 提出数据生成框架，结合合成数据和基于扩散的过程，为真实图像添加逼真的个人防护装备；2) 创建GlovEgo-HOI基准数据集；3) 开发GlovEgo-Net模型，集成Glove-Head和Keypoint-Head模块，利用手部姿态信息增强交互检测。

Result: 大量实验证明了所提数据生成框架和GlovEgo-Net模型的有效性。作者开源了GlovEgo-HOI数据集、数据增强流程和预训练模型。

Conclusion: 通过合成数据增强和专门设计的检测模型，有效解决了工业EHOI分析中数据稀缺的问题，为工业安全应用提供了实用解决方案。
...
TL;DR: 提出了一种针对无监督域适应的数据中心方法，通过构建层次化数据服务器和二分模式匹配算法，为目标域的每个语义模式寻找最佳匹配的源域数据，从而减少域间差距。


<details>
  <summary>Details</summary>
Motivation: 在目标域可访问但无法实时标注的情况下，需要从大规模数据服务器构建替代训练集。传统方法主要改进算法，而本研究关注优化数据服务器结构，因为如果训练集不包含目标域的语义模式，模型性能会受损。

Method: 引入层次化数据服务器结构，提出二分模式匹配算法(BMM)。算法将源域和目标域的模式进行二分图匹配，为每个目标模式寻找最佳匹配的源模式，无论其规模大小。该方法与现有模型中心UDA方法正交，可结合使用。

Result: 相比现有训练集搜索算法，BMM匹配的服务器模式构成的训练集在目标重识别和检测任务上具有更小的域间差距。基于这些训练集训练的模型准确率更高。与伪标签等现有UDA方法结合时能获得进一步改进。

Conclusion: BMM提供了一种数据中心的无监督域适应方法，通过优化数据服务器结构和模式匹配，有效减少域间差距，提高模型性能，且与现有模型中心方法互补，可进一步提升性能。
...
TL;DR: 该研究探索使用低分辨率灰度图像替代传统索引标记来表示中文字符，发现8×8像素的视觉输入能达到与传统索引方法相当的预测准确率，且在低资源训练时表现出明显的"热启动"优势。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型通常将中文字符表示为基于索引的离散标记，忽略了字符的视觉形式。对于表意文字（如中文），视觉结构承载着语义和语音信息，可能有助于预测。研究者希望探索低分辨率视觉输入是否能作为字符级建模的替代方案。

Method: 研究使用解码器接收单个字符的灰度图像作为输入，而不是传统的标记ID。实验采用极低分辨率（低至8×8像素）的视觉输入，并与基于索引的基线方法进行比较。

Result: 低分辨率视觉输入达到39.2%的准确率，与基于索引的基线方法（39.1%）相当。在低资源训练设置中，视觉模型表现出明显的"热启动"效应：仅用0.4%的总训练量，准确率就达到12%以上，而基于索引的模型则低于6%。

Conclusion: 研究表明，极简的视觉结构可以为中文语言建模提供强大而高效的信号，为字符表示提供了替代传统索引方法的新视角，特别是在低资源训练场景中具有明显优势。
...
TL;DR: DF-DiffCom是一种基于KAN增强的扩散模型，利用变形场进行可信的纵向脑图像补全，在OASIS-3数据集上优于现有方法，支持多模态MRI扩展。


<details>
  <summary>Details</summary>
Motivation: 纵向脑MRI研究中高流失率导致数据缺失，现有深度生成模型主要依赖图像强度，存在生成图像可信度有限和使用灵活性受限两大问题。
...
Conclusion: DF-DiffCom通过变形场增强的扩散模型解决了纵向脑图像补全中的可信度和灵活性限制问题，为下游研究提供了更可靠的图像生成方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D场景理解方法通常需要训练过程，并且依赖CLIP/BERT文本编码器的嵌入表示。作者希望开发一种无需训练的方法，直接利用多模态大语言模型进行文本到文本搜索，实现更高效的开放词汇3D场景理解。

Method: OpenVoxel首先从多视图图像中获取稀疏体素栅格化模型，然后对稀疏体素进行分组以描述场景中的不同物体。利用强大的视觉语言模型和多模态大语言模型，为每个分组生成描述性标题，构建信息丰富的场景地图。该方法的关键创新是无需训练，不引入CLIP/BERT文本编码器的嵌入，而是直接使用MLLMs进行文本到文本搜索。

Result: 通过大量实验验证，OpenVoxel在性能上优于最近的研究方法，特别是在复杂的指代表达分割任务中表现出色。该方法能够有效地进行开放词汇分割和指代表达分割等3D场景理解任务。

Conclusion: OpenVoxel提供了一种无需训练的高效方法，用于稀疏体素的分组和描述，实现了开放词汇的3D场景理解。该方法通过直接利用多模态大语言模型，避免了传统方法对文本编码器嵌入的依赖，在复杂任务中表现出优越性能，代码将开源。
...
TL;DR: 该论文探讨了手写文档视觉反馈系统的挑战，比较了模块化与端到端系统，发现两者目前都无法达到可接受的整体质量，并提出了未来研究方向。
...
Method: 从手写输入图像到正确放置信息性错误反馈的完整流程分析，并对模块化系统和端到端系统进行了实证比较。
...
<details>
  <summary>Details</summary>
Motivation: 传统点云配准方法（如ICP、RMSE）依赖欧氏距离，对点云密度差异、噪声、孔洞和有限重叠敏感，且需要选择一个固定点云，在真实场景中两个点云都可能存在问题，需要预处理。

Method: 提出基于微分熵的新度量作为优化目标函数，称为迭代微分熵最小化（IDEM），该度量不依赖于固定点云的选择，在变换过程中显示出清晰的最小值对应最佳对齐。

Result: 通过多个案例研究，与RMSE、Chamfer距离和Hausdorff距离比较，IDEM在存在密度差异、噪声、孔洞和部分重叠的情况下仍能有效配准，而RMSE在这些情况下不一定能获得最优对齐。

Conclusion: 微分熵度量IDEM为点云配准提供了一种更鲁棒的替代方案，特别适用于存在密度差异、噪声、孔洞和部分重叠的真实场景，克服了传统欧氏距离方法的局限性。
...
TL;DR: 该论文提出了一种两阶段分组排序与校准框架（GRCF），用于多模态情感分析，通过动态边际排序和绝对分数校准来解决传统点式回归和成对排序方法的局限性。
...
Method: 提出两阶段分组排序与校准框架（GRCF），受GRPO启发。第一阶段：引入优势加权动态边际排序损失，建立细粒度序数结构。第二阶段：使用MAE驱动的目标函数来对齐预测幅度。还将GRCF扩展到分类任务（如多模态幽默检测和讽刺检测）。

Result: GRCF在核心回归基准测试中实现了最先进的性能，同时在分类任务中也表现出强大的泛化能力。

Conclusion: GRCF框架通过同时保持相对序数结构、确保绝对分数校准和自适应关注困难样本，有效解决了现有方法的局限性，在多模态情感分析及相关分类任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有铁路入侵检测系统主要关注固定视野内的物体分类，使用基于规则的启发式方法判断入侵状态，忽视了具有潜在入侵风险的目标。准确感知入侵风险需要理解感兴趣物体的空间上下文和时序动态，这对传统视觉模型构成挑战。
...
Result: 实验表明：1) 当前大规模多模态模型在认知入侵感知任务所需的复杂时空推理方面表现不佳；2) 提出的联合微调框架通过针对性适配领域特定推理需求，显著提升了模型性能；3) 结构化多任务学习在提高准确性和可解释性方面具有优势。

Conclusion: 认知入侵感知需要复杂的时空推理能力，现有基础模型在此安全关键领域存在局限性。通过联合微调框架进行针对性适配，可以有效提升模型在铁路入侵感知任务上的性能，结构化多任务学习是提升准确性和可解释性的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像(T2I)模型质量评估通常使用基于投票的排行榜，依赖模型输出的匿名化来保证公平性。本研究旨在验证这种匿名性是否真的安全可靠。

Method: 使用22个T2I模型和280个提示词生成15万张图像，通过基于质心的分析方法在图像嵌入空间中识别模型特有的聚类特征，无需提示词控制或训练数据。

Result: 研究发现每个T2I模型的生成图像在嵌入空间中形成独特的聚类，能够实现高精度的去匿名化。同时提出了提示词级别的可区分性度量，发现某些提示词能导致近乎完美的模型区分能力。

Conclusion: 当前T2I排行榜存在根本性的安全漏洞，匿名化措施可以被轻易绕过。研究结果强调了需要更强的匿名化防御机制来确保模型评估的公平性。
...
TL;DR: 提出一个从单张图像估计可模拟服装的框架，通过视觉语言模型预测材料属性和织物特性，然后映射到物理参数，无需多视角捕捉或可微分模拟


<details>
  <summary>Details</summary>
Motivation: 从单张图像估计物理精确、可模拟的服装具有挑战性，因为缺乏图像到物理的数据集且问题本身是病态的。现有方法要么需要多视角捕捉和昂贵的可微分模拟，要么只预测几何形状而缺乏模拟所需的材料属性

Method: 首先微调视觉语言模型从真实图像推断材料成分和织物属性，然后训练轻量级预测器将这些属性映射到相应的物理织物参数，使用小型材料-物理测量数据集

Result: 在材料成分估计和织物属性预测方面达到更高的准确性，通过物理参数估计器进一步实现比现有图像到服装方法更高保真度的模拟

Conclusion: 提出了一种无需迭代优化的前馈框架，能够从单张图像生成可模拟的服装，并引入了两个新数据集（FTAG和T2P），在材料属性和模拟质量方面优于现有方法
...
Method: LiteEmbed在CLIP词汇表内进行子空间引导的文本嵌入优化，利用基于PCA的分解将粗粒度语义方向与细粒度变化解耦。通过粗粒度对齐和细粒度分离两个互补目标，保持全局语义一致性同时增强视觉相似类别间的区分性。

Result: 优化后的嵌入即插即用，可无缝替代CLIP的原始文本特征，应用于分类、检索、分割和检测任务。大量实验表明该方法显著优于先前方法。

Conclusion: LiteEmbed是适应CLIP到代表性不足、罕见或未见类别的有效方法，实现了少样本个性化而不需要重新训练编码器。
...
<details>
  <summary>Details</summary>
Motivation: 单目视觉SLAM在资源受限平台上实现3D重建和自主导航，但存在尺度漂移问题。现有的帧到帧方法通过局部优化实现实时性能，但由于独立窗口间缺乏全局约束，会累积尺度漂移。
...
Method: 采用两种策略：1）在1.2T多模态token上进行统一全解冻预训练，整合语言对齐的感知编码器和Qwen3-8B解码器；2）包含1000多次强化学习迭代的规模化后训练流程，并实施并行协调推理（PaCoRe）来扩展测试时计算。

Result: 在紧凑的10B参数下，性能媲美或超越大10-20倍的模型（如GLM-4.6V-106B、Qwen3-VL-235B）以及顶级专有旗舰模型（如Gemini 2.5 Pro和Seed-1.5-VL），在MMBench上达到92.2%，MMMU上80.11%，复杂推理任务中AIME2025达到94.43%，MathVision达到75.95%。

Conclusion: STEP3-VL-10B成功实现了紧凑尺寸下的前沿多模态智能，为社区提供了强大、高效且可复现的基线模型，重新定义了效率与性能的权衡。
...
TL;DR: SRENDER：通过3D重建和渲染实现高效视频生成的新方法，使用扩散模型生成稀疏关键帧，然后通过3D重建合成完整视频，比传统扩散模型快40倍以上。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频生成方法计算效率低下，生成几秒钟视频需要几分钟GPU时间，这限制了在需要实时交互的应用（如具身AI和VR/AR）中的部署。需要一种更高效的视频生成策略。

Method: 提出SRENDER方法：1）使用扩散模型生成稀疏的关键帧；2）将关键帧提升为3D表示；3）通过3D重建和渲染合成中间视图；4）引入模型预测给定相机轨迹的最佳关键帧数量，自适应分配计算资源。

Result: SRENDER在生成20秒视频时比基于扩散的基线方法快40倍以上，同时保持高视觉保真度和时间稳定性。对于简单轨迹使用非常稀疏的关键帧，对于复杂相机运动使用更密集的关键帧。

Conclusion: SRENDER提供了一种实用路径，通过将生成成本分摊到数百帧并强制几何一致性，实现了高效且可控的视频合成，为实时交互应用中的视频生成提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多视角3D姿态估计方法主要依赖成对关联来建模对应问题，将全局一致性（循环一致性）作为软约束。当虚假关联传播误差时，多视角约束的协调变得脆弱，导致性能下降。
...
Conclusion: 通过将多视角姿态对应匹配重新定义为超图分割问题，COMPOSE能够更好地处理全局一致性约束，显著提升了稀疏多视角3D姿态估计的性能，为解决这一关键任务提供了新的思路。
...
TL;DR: SAM3-DMS提出了一种训练无关的解耦记忆选择策略，通过细粒度的个体对象记忆选择改进SAM3在复杂多目标场景中的性能。


<details>
  <summary>Details</summary>
Motivation: SAM3在原始实现中采用同步的群体级集体记忆选择，基于所有并发目标的平均性能做决策，这在复杂多目标场景中不够优化，常常忽视了个体可靠性。

Method: 提出SAM3-DMS，一种训练无关的解耦策略，利用细粒度的个体对象记忆选择，而不是同步的群体级决策。

Result: 实验表明该方法实现了鲁棒的身份保持和跟踪稳定性，在目标密度增加时优势更加明显，为野外同时多目标视频分割建立了坚实基础。

Conclusion: SAM3-DMS通过个体化的记忆选择机制有效解决了SAM3在多目标场景中的局限性，为复杂视频分割任务提供了更优的解决方案。
...
### [60] [DeliberationBench: When Do More Voices Hurt? A Controlled Study of Multi-LLM Deliberation Protocols](https://arxiv.org/abs/2601.08835)
*Vaarunay Kaushal,Taranveer Singh*

Main category: cs.CL

TL;DR: 研究通过DELIBERATIONBENCH基准测试发现，在多智能体系统中，简单的"选择最佳单个输出"方法在82.5%的案例中优于复杂的LLM协商协议，且计算成本更低


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的多智能体协商系统备受关注，但其实际价值相对于简单方法缺乏充分验证。研究者希望系统评估协商协议是否真的能提升性能

Method: 引入DELIBERATIONBENCH基准，在270个问题和3个独立种子（共810次评估）上，比较三种协商协议与"从模型输出池中选择最佳响应"的基线方法

Result: 最佳单个基线方法以82.5% ± 3.3%的胜率显著优于最佳协商协议（13.8% ± 2.6%），性能差距达6.0倍，且协商协议的计算成本高出1.5-2.5倍，结果具有统计显著性（p < 0.01）

Conclusion: 研究挑战了"复杂性提升质量"的假设，表明在多LLM系统中，简单的选择最佳单个输出的方法不仅性能更好，而且计算效率更高
...
TL;DR: 这篇综述系统分析了2008-2024年间关于PTSD与慢性疾病患者在社交媒体上表达的研究，发现社交媒体数据能提供独特见解，NLP/ML技术可识别潜在PTSD病例（准确率74%-90%），在线支持社区有助于应对策略和早期干预。


<details>
  <summary>Details</summary>
Motivation: PTSD是一种复杂的心理健康状况，对于已有慢性疾病的患者尤为挑战。研究动机是探索PTSD与慢性疾病（如癌症、心脏病、自身免疫疾病）在社交媒体平台上的交叉表现，了解这一特殊人群面临的独特挑战。

Method: 通过系统分析2008-2024年的文献，研究PTSD在慢性疾病患者中的表现和管理方式，重点关注X（原Twitter）和Facebook等社交媒体平台上的在线表达。采用自然语言处理（NLP）和机器学习（ML）技术分析社交媒体数据。

Result: 研究发现社交媒体数据能提供关于PTSD与慢性疾病患者独特挑战的宝贵见解。NLP和ML技术可在这些人群中识别潜在PTSD病例，准确率达到74%-90%。在线支持社区在塑造应对策略和促进早期干预方面发挥重要作用。

Conclusion: 该综述强调在PTSD研究和治疗中必须考虑已有医疗状况，并强调社交媒体作为监测和支持脆弱群体的潜在工具。讨论了未来研究方向和临床意义，重点强调开发针对性干预措施的重要性。
...
TL;DR: 论文提出了一种名为"Adversarial Tales"的越狱攻击技术，通过将有害内容嵌入赛博朋克叙事中，并引导模型进行基于民间故事形态学的功能分析，成功绕过LLM的安全机制，平均攻击成功率达71.3%。
...
Method: 提出Adversarial Tales攻击技术：1) 将有害内容嵌入赛博朋克叙事中；2) 引导模型进行基于Vladimir Propp民间故事形态学的功能分析；3) 将任务重构为结构分解，诱导模型将有害程序重建为合法的叙事解释。

Result: 在9个提供商的26个前沿模型上测试，平均攻击成功率为71.3%，没有模型家族表现出可靠的鲁棒性。结合之前的Adversarial Poetry研究，表明基于结构的越狱攻击是一类广泛的漏洞。
...
### [63] [Companion Agents: A Table-Information Mining Paradigm for Text-to-SQL](https://arxiv.org/abs/2601.08838)
*Jiahui Chen,Lei Fu,Jian Cui,Yu Lei,Zhenning Dong*

Main category: cs.CL

TL;DR: 本文提出Companion Agents (CA)方法，通过数据库侧预挖掘隐藏关系、值域分布等内在信息来提升Text-to-SQL在标注稀缺条件下的性能，在BIRD基准上显著提升了执行准确率。


<details>
  <summary>Details</summary>
Motivation: 现有大规模Text-to-SQL基准（如BIRD）假设数据库标注完整准确且外部知识易获取，这与工业场景中标注缺失、不完整或错误的情况不匹配，限制了SOTA系统的实际应用。

Method: 提出Companion Agents (CA)新范式，采用数据库中心化方法，通过一组伴随数据库模式的智能体主动挖掘和整合隐藏的表间关系、值域分布、统计规律和潜在语义线索，在推理时选择性激活这些预缓存的相关知识。

Result: 在BIRD基准的完全缺失证据设置下，CA在RSL-SQL/CHESS/DAIL-SQL上分别恢复了+4.49/+4.37/+14.13个执行准确率点，在挑战性子集上提升更大：+9.65/+7.58/+16.71。

Conclusion: CA通过自动数据库侧挖掘和证据构建显著提升了Text-to-SQL性能，为不依赖人工标注证据的工业级Text-to-SQL部署提供了实用路径。
...
TL;DR: 本文提出了一个三智能体交叉验证框架，用于分析多模型大语言系统的稳定性和可解释性。该架构将三个异构LLM集成到递归交互循环中，通过递归知识合成实现中间表示的持续精炼。在47个对照试验中，系统表现出良好的稳定性和收敛性。


<details>
  <summary>Details</summary>
Motivation: 研究多模型大语言系统的稳定性和可解释性问题，旨在开发一种能够实现稳定递归知识合成的人类监督多LLM架构，在现实公开部署环境中保持安全性。

Method: 采用三智能体交叉验证框架，集成三个异构LLM：语义生成、分析一致性检查和透明度审计，形成递归交互循环。通过递归知识合成(RKS)模型，基于不动点理论，在47个对照试验中使用公开访问的LLM部署进行评估。

Result: 系统平均反射可靠性得分(RRS)为0.78±0.06，约68%的试验中透明度得分(TS)≥0.8，约89%的试验收敛。结果表明透明度审计在复合验证映射中起到收缩算子的作用。

Conclusion: 该研究提供了初步经验证据，表明安全保护的人类监督多LLM架构能够在现实公开部署环境中实现稳定的递归知识合成。主要贡献包括：结构化三智能体框架、基于不动点理论的RKS形式模型，以及在现实非API公开访问条件下的模型间稳定性评估。
...
TL;DR: 本文提出了一种用于大语言模型实体级遗忘的一致性感知编辑框架，通过聚合多样化的提示来学习低秩更新，实现高效、鲁棒的实体知识删除。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在保留敏感、受版权保护或有害信息的风险，现有方法要么计算成本高，要么在处理改写查询时脆弱。模型编辑技术虽然高效，但主要用于实例级更新，而非实体级遗忘。

Method: 提出一致性感知编辑框架，聚合目标实体的属性、关系和对抗性改写等多样化提示，通过一致性正则化器指导学习低秩更新，对齐不同提示的编辑方向。

Result: 在RWKU和ToFU基准测试中，CAE显著提高了遗忘准确性和鲁棒性，优于传统遗忘和编辑基线方法，仅需数十个精心选择的提示即可实现可扩展的实体删除。

Conclusion: CAE框架不仅提供了关于实体级知识在LLMs中如何内部表示和删除的见解，而且实现了高效、鲁棒的实体遗忘，为模型安全性和合规性提供了实用解决方案。
...
TL;DR: 该研究探索了使用主语-谓语-宾语三元组结构化知识增强科学论文聚类与分类的方法，通过结合原始摘要、提取的三元组和混合表示，发现混合表示能显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 科学文献数量和复杂性不断增加，需要更有效的方法来组织和理解研究文档。研究者希望探索结构化知识（特别是主语-谓语-宾语三元组）如何增强科学论文的聚类和分类。

Method: 提出模块化流程，结合无监督聚类和有监督分类，使用四种文档表示：原始摘要、提取的三元组、混合格式。从arXiv语料库提取关系三元组，使用四种Transformer模型（MiniLM、MPNet、SciBERT、SPECTER）进行嵌入，用KMeans、GMM、HDBSCAN进行聚类评估，并微调分类模型进行arXiv主题预测。
...
Conclusion: 结合非结构化文本与结构化知识具有互补优势，为科学文档的语义组织提供了知识增强表示的新见解，展示了结构化知识在提升文档分类性能方面的价值。
...
TL;DR: 本文系统评估了LLM在基于评分标准的简答题自动评分中的表现，发现其在二元任务中表现良好，但随着评分标准细化程度增加而下降，同时提出了信任曲线分析和鲁棒性测试的重要性。


<details>
  <summary>Details</summary>
Motivation: 自动简答题评分(ASAG)面临学生回答的语言多样性和需要基于评分标准给予部分分数的挑战。虽然大语言模型(LLMs)提供了有前景的解决方案，但它们在基于评分标准的设置中作为自动评分者的可靠性需要严格评估。

Method: 使用SciEntsBank基准和Qwen 2.5-72B模型，系统评估LLM评分者在基于评分标准的简答题评分中的表现。研究三个关键方面：LLM评分与专家判断在不同评分标准复杂度下的对齐程度；基于共识的延迟机制在不确定性和准确性之间的权衡；模型在随机输入扰动和对抗攻击下的鲁棒性。

Result: 研究发现：1) 对齐性在二元任务中很强，但随着评分标准细化程度增加而下降；2) "信任曲线"分析显示通过过滤低置信度预测可以改善剩余子集的准确性；3) 鲁棒性实验表明模型对提示注入具有弹性，但对同义词替换敏感。

Conclusion: 本研究为基于评分标准的LLM评分者的能力和局限性提供了重要见解，强调了不确定性估计和鲁棒性测试对于可靠部署的重要性。LLM在简答题自动评分中具有潜力，但需要谨慎处理评分标准复杂性和不确定性管理。
...
TL;DR: 该研究比较了大型语言模型(LLMs)和微调小型语言模型(SLMs)在性能与碳排放之间的权衡，发现在多数任务中SLMs能保持可比性能的同时显著降低推理碳排放。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)的巨大碳足迹引起了关注，包括能源密集的训练和重复推理过程。研究旨在探索微调小型语言模型(SLMs)作为可持续替代方案的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的迭代推理框架如InftyThink虽然能控制上下文增长实现长程推理，但在不同任务中重复生成相似的推理策略，缺乏对先前成功推理模式的复用

Method: 提出InftyThink with Cross-Chain Memory，通过基于嵌入的语义缓存存储先前成功的推理模式（引理），在每个推理步骤中检索语义最相似的存储引理来指导推理，避免无限制扩展上下文窗口

Result: 在MATH500、AIME2024和GPQA-Diamond上的实验表明，语义引理检索在结构化领域提高了准确性，但在包含异构领域的测试中暴露了失败模式；几何分析显示缓存检索在嵌入空间中诱导方向性偏差，形成一致的修复（改进基线准确性）和破坏（降低基线准确性）吸引子

Conclusion: 研究结果突出了基于相似性记忆对自改进LLM推理的益处和局限性，语义缓存能有效复用成功推理模式，但也可能因相似性偏差导致特定领域的性能下降
...
TL;DR: RIKER是一个基于范式反转的知识系统评估框架，通过从已知真实数据生成文档而非从文档提取真实数据，实现确定性评分、可扩展评估和抗污染能力。


<details>
  <summary>Details</summary>
Motivation: 当前知识系统评估面临三大挑战：静态基准易受数据污染、基于LLM的评估存在系统性偏见、真实数据提取需要昂贵的人工标注。需要一种更可靠、可扩展且抗污染的评估方法。

Method: 采用范式反转方法：从已知结构化真实数据生成合成文档，而不是从文档中提取真实数据。这种方法使得评估可以基于确定性评分，无需人工标注或参考模型，且通过可重新生成的语料库实现抗污染能力。

Result: 评估了33个模型，使用了超过210亿个token。主要发现：1）上下文长度声称经常超过实际可用容量，超过32K token后性能显著下降；2）跨文档聚合比单文档提取困难得多；3）基础能力和抗幻觉能力是不同的能力，擅长查找存在事实的模型仍可能虚构不存在的事实。

Conclusion: RIKER不仅是一个具体基准，更贡献了一个领域无关的方法论，适用于任何可以从结构化真实数据生成合成文档的场景，能够构建可扩展且抗污染的评估体系。
...
TL;DR: 研究发现自动答案匹配模型对常见的策略性攻击（如冗长回答、多重猜测、冲突答案）具有鲁棒性，这些操纵通常不会提高分数反而会降低分数，二元评分比连续评分更稳健。


<details>
  <summary>Details</summary>
Motivation: 虽然基于LLM的自动答案匹配作为可扩展且对齐的人类评估替代方案前景广阔，但其可靠性需要能够抵御策略性攻击，如猜测或冗长回答等可能人为提高分数而不改善实际正确性的手段。

Method: 通过系统研究三种策略攻击：1）生成冗长回答；2）在不确定时提供多个答案；3）在回答开头嵌入冲突答案与正确答案。比较二元评分（要求匹配器给出明确的"正确"或"不正确"）与连续评分（要求匹配器确定部分正确性）的鲁棒性。

Result: 这些操纵策略不会提高分数，反而经常降低分数。二元评分比连续评分对攻击更具鲁棒性。自动答案匹配对廉价文本操纵具有普遍鲁棒性。

Conclusion: 当有参考答案可用时，答案匹配是传统LLM-as-a-judge或人类评估的可行替代方案，因为它对常见的策略性攻击具有鲁棒性。
...
TL;DR: 研究评估了上下文分块技术在RAG系统中注入摘要的效果，发现存在倒U型曲线：适度注入可提升召回率18%，但超过临界阈值(CIR>0.4)会使特定查询的精确度下降22%。


<details>
  <summary>Details</summary>
Motivation: 当前上下文分块技术通过注入摘要来增强RAG系统的上下文理解，但这种方法可能导致"向量稀释"问题，淹没局部内容的重要性，需要量化评估这种权衡关系。

Method: 通过评估不同注入比例，分析其对RAG系统性能的影响，特别是召回率和精确度的变化，并建立理论框架来计算最优注入比例。

Result: 发现性能呈现倒U型曲线：适度注入摘要可提升召回率18%，但超过临界阈值(CIR>0.4)后，特定查询的精确度下降22%，存在明显的权衡关系。

Conclusion: 需要平衡摘要注入带来的上下文增强与向量稀释问题，提出了计算最优注入比例的理论框架，为RAG系统的上下文分块技术提供优化指导。
...
TL;DR: NewsScope是一个用于模式化新闻声明提取的跨领域数据集、基准测试和微调模型，在政治、健康、科学/环境和商业领域表现优异，接近GPT-4o-mini的性能但成本更低。


<details>
  <summary>Details</summary>
Motivation: 自动化新闻验证需要结构化的声明提取，但现有方法要么缺乏模式合规性，要么在不同领域间泛化能力差。需要开发一个能够跨领域工作且符合特定模式的新闻声明提取系统。

Method: 构建了包含455篇文章的跨领域数据集（政治、健康、科学/环境、商业），使用LLaMA 3.1 8B模型通过LoRA技术在315个训练样本上进行微调，并在保留的领域内（80篇）和领域外（60篇）测试集上进行评估。采用数值基础过滤器和人工评估验证性能。

Result: NewsScope在400个声明的人工评估中达到89.4%的准确率（GPT-4o-mini为93.7%，p=0.07）。在政治声明上表现更优（94.3% vs. 87.8%）。使用数值基础过滤器后准确率提升至91.6%，与GPT-4o-mini的差距缩小到2.1个百分点。标注者间一致性达到94.6%。

Conclusion: NewsScope提供了一个有效的跨领域新闻声明提取解决方案，性能接近最先进的GPT-4o-mini但成本显著更低（约15美元按需计算或免费层级），并公开发布了代码和基准测试，支持离线部署。
...
TL;DR: 论文扩展了VirCo虚拟客户在线咨询系统研究，引入包含对抗性攻击的新数据集测试大语言模型角色一致性，评估Vicuna模型表现并与开源LLMs进行对比分析。
...
Method: 1. 扩展VirCo虚拟客户系统研究；2. 创建包含对抗性攻击的新数据集；3. 评估Vicuna模型在角色一致性和对话连贯性方面的表现；4. 比较各种开源大语言模型在虚拟客户互动中的角色一致性表现。

Result: 1. 创建了对抗性数据集用于测试LLMs角色一致性；2. 评估了Vicuna模型的对话连贯性和角色一致性表现；3. 提供了不同开源LLMs在维持角色一致性方面的比较分析。

Conclusion: 研究通过创建对抗性数据集和评估框架，为大语言模型在虚拟客户咨询培训中的角色一致性表现提供了系统评估方法，有助于改进在线咨询培训工具的开发。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界模型主要进行单步或固定步数的推演，未能充分利用其在复杂任务规划中的潜力。需要一种能够根据任务和阶段自适应调整前瞻视野的框架，以更好地指导智能体学习。

Method: 提出Imagine-then-Plan（ITP）统一框架：1）智能体政策模型与学习的世界模型交互生成多步"想象"轨迹；2）引入自适应前瞻机制，权衡最终目标和任务进度；3）将想象轨迹的未来信号（如进度和潜在冲突）与当前观察融合，形成部分可观察和可想象的马尔可夫决策过程；4）实现训练免费和强化训练两种变体。

Result: 在代表性智能体基准测试上的广泛实验表明，ITP显著优于竞争基线。进一步分析验证了自适应前瞻机制大幅增强了智能体的推理能力，为处理更广泛的复杂任务提供了有价值的见解。
...
TL;DR: 本文研究如何利用LLM推理时的输出熵信号来监控模型性能并指导数据采集，通过分析top-k token概率的熵分布特征来预测实例正确性和领域级准确率。


<details>
  <summary>Details</summary>
Motivation: 部署LLM面临两个相互关联的挑战：监控（估计模型在流量和领域漂移下的性能表现）和改进（优先获取数据以弥补最大的性能差距）。作者希望测试推理时信号是否能有效估计领域漂移下的切片级准确率。

Method: 对于每个LLM响应，从最终层的top-k token对数概率计算输出熵分布，提取11个统计特征。使用轻量级分类器预测单个实例的正确性，通过平均预测概率得到领域级准确率估计。在10个STEM推理基准上进行评估，涵盖所有训练/测试组合（k∈{1,2,3,4}），测试了6个家族的9个LLM（3B-20B）。

Result: 输出熵信号通常能跟踪基准测试的准确率，多个模型显示出近乎单调的领域排序能力。这表明输出熵分布是一个可访问的信号，可用于可扩展的监控和有针对性的数据采集。

Conclusion: 输出熵分布特征可以作为有效的推理时信号，用于监控LLM在领域漂移下的性能表现，并指导数据采集策略，为解决LLM部署中的监控和改进挑战提供了实用方法。
...
Method: 采用两阶段微调：1) 使用高质量大规模合成并行数据和人工翻译数据进行监督微调；2) 使用MetricX-QE和AutoMQM等奖励模型集成进行强化学习，优化翻译质量。

Result: 在WMT25测试集（10个语言对）上通过人工评估，在WMT24++基准（55个语言对）上通过自动评估，显示在所有模型尺寸上都比基线Gemma 3模型有显著提升。较小的TranslateGemma模型常能达到与较大基线模型相当的性能，效率更高。同时模型保持了强大的多模态能力，在Vistra图像翻译基准上表现增强。

Conclusion: TranslateGemma套件有效提升了Gemma 3模型的翻译性能，为机器翻译研究社区提供了强大且高效的开源工具，同时保持了多模态能力。
...
TL;DR: 研究者提出基于社交推理游戏Spyfall的动态评估框架，用于评估大语言模型的多语言和跨文化能力，发现模型在非英语语境中存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，需要超越静态基准的更鲁棒评估方法，因为静态基准越来越容易出现数据饱和和泄露问题。

Method: 提出基于社交推理游戏Spyfall的动态基准框架，模型必须通过战略对话来识别秘密特工或避免被发现，使用文化相关的地点或当地食物作为游戏元素。

Result: 游戏排名与Chatbot Arena高度一致，但在非英语语境中发现显著性能差距：模型在处理本地特定实体时普遍表现较差，在非英语语言中经常难以遵循规则或保持战略完整性。

Conclusion: 这种基于游戏的方法为传统NLP基准提供了可扩展、抗泄露且具有文化细微差别的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG方法假设检索信息总是相关的，但实际上检索信息的相关性和有用性会因问题和文档集合而异。现有方法没有充分考虑检索信息的质量差异对生成结果的影响。

Method: OpenDecoder利用三种显式评估信息作为质量指标特征：相关性分数、排名分数和查询性能预测(QPP)分数。这些特征被整合到生成过程中，使模型能更好地处理不同噪声水平的上下文。

Result: 在五个基准数据集上的实验结果表明，OpenDecoder在效果和鲁棒性方面优于各种基线方法，特别是在处理噪声上下文时表现更好。

Conclusion: OpenDecoder提供了一种灵活的方法，可以集成到LLM的后训练中，并能与任何类型的外部指标结合使用，为构建更鲁棒的RAG系统提供了新思路。
...
TL;DR: SpectraQuery是一个混合自然语言查询框架，将拉曼光谱数据库与科学文献语料库集成，通过语义解析和检索增强生成技术，将开放式问题转化为SQL和文献检索操作，为科学工作流提供数据与解释的桥梁。


<details>
  <summary>Details</summary>
Motivation: 科学推理需要将结构化实验数据与解释这些数据的非结构化文献联系起来，但大多数大型语言模型助手无法跨这些模态进行联合推理。为了解决这一挑战，需要开发能够同时处理数据和文献的系统。

Method: 采用受结构化与非结构化查询语言（SUQL）启发的设计，结合语义解析和检索增强生成技术，将开放式问题转化为协调的SQL查询和文献检索操作，生成包含引用的答案，统一数值证据与机制解释。

Result: 在SQL正确性、答案基础性、检索效果和专家评估方面表现优异：约80%生成的SQL查询完全正确，合成答案的基础性达到93-97%（使用10-15个检索段落），电池科学家在准确性、相关性、基础性和清晰度方面给予4.1-4.6/5的高分。

Conclusion: 混合检索架构能够有意义地支持科学工作流，为大规模实验数据集提供数据与论述的桥梁，展示了跨模态科学推理的可行性。
...
TL;DR: LLMs与人类在字面理解上相似，但在比喻和社会化语言（特别是习语和Z世代俚语）的深层理解上存在显著差异。GPT-4最接近人类表征模式，但所有模型在处理依赖语境和社会语用的表达时仍有困难。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型在理解比喻性和社会化语言方面与人类判断的一致性程度。虽然LLMs在生成判断上看似与人类相似，但它们在解释具有社会文化背景的比喻语言方面的实际对齐程度仍不确定。

Method: 研究比较了人类参与者和四种不同规模的指令调优LLM（GPT-4、Gemma-2-9B、Llama-3.2和Mistral-7B）。使用240个基于对话的句子，涵盖六种语言特征：常规性、讽刺、幽默、情感、习语和俚语。每个句子配40个解释性问题，人类和LLM都使用10点李克特量表进行评分。

Result: 结果显示：1）人类和LLM在表面层次上对齐；2）但在表征层次上显著分歧，特别是在解释涉及习语和Z世代俚语的比喻性句子时；3）GPT-4最接近人类的表征模式；4）所有模型在处理依赖语境和社会语用的表达（如讽刺、俚语和习语）时都存在困难。

Conclusion: LLMs虽然在字面理解上与人类相似，但在深层比喻和社会文化语言理解方面仍有显著差距。GPT-4表现最佳，但所有模型都需要进一步改进以更好地理解依赖语境和社会语用的表达。这表明当前LLMs在理解人类语言的细微差别和社会文化背景方面仍有限制。
...
TL;DR: 该研究通过机制分析发现，经过长时间"grokking"训练形成的"泛化电路"并未创造新的推理范式，而是将记忆的原子事实整合到已有推理路径中；高精度与特定推理路径的形成可以独立发生；成熟的电路在新知识整合方面转移能力有限。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究经过长时间"grokking"训练形成的"泛化电路"是否真的优于未经过该阶段的模型，以及等待grokking阶段的高计算成本是否值得。旨在理解泛化电路在知识吸收和转移中的作用。

Method: 采用机制研究方法，分析非grokked和grokked模型在分布内组合查询中的推理路径，评估泛化电路的形成过程及其功能。

Result: 研究发现：(1)非grokked和grokked模型对分布内组合查询的推理路径相同，表明泛化电路并非突然获得新推理范式，而是将记忆的原子事实整合到已有推理路径中；(2)长时间训练后的高精度与特定推理路径形成可以独立发生；(3)成熟的电路在新知识整合方面转移能力有限。

Conclusion: 经过grokking训练的Transformer并未完全掌握组合逻辑，泛化电路的形成更多是知识整合而非新推理能力的获得。这挑战了grokking作为模型能力质变标志的传统观点。
...
Method: 采用分阶段多目标训练：1）跨性别对比目标促进说话人间的词汇一致性，音调排斥损失防止音调塌缩；2）基于CTC的ASR目标蒸馏稳定识别相关结构。

Result: 在苗语评估中，SITA显著提升了跨性别词汇检索准确率，同时保持了可用的ASR准确率。在普通话上的类似增益表明该方法具有通用性。

Conclusion: SITA是一种通用的即插即用方法，能够有效适配多语言语音编码器到音调语言，解决了低资源音调语言语音表示的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（特别是印度语言）的多语言对话摘要和问答任务，探索无需任务特定微调的翻译方法在跨语言处理中的有效性。

Method: 三阶段流水线：1) 将印度语言前向翻译到英语；2) 使用2.55B参数蒸馏语言模型进行多任务文本生成；3) 将结果反向翻译回源语言。采用知识蒸馏技术使紧凑模型保持高性能。
...
TL;DR: 该论文综述了NLP中标注者分歧的研究，从将分歧视为噪声到将其作为有意义信号的转变，提出了统一的建模框架和评估方法。


<details>
  <summary>Details</summary>
Motivation: NLP中标注者分歧普遍存在，特别是在主观和模糊的任务中。传统方法将分歧视为噪声，但近年来越来越多研究认为分歧反映了不同解释和视角的差异，是值得建模的有意义信号。
...
Conclusion: 提出了未来研究方向：整合多种变异来源、开发分歧感知的可解释性框架、处理视角主义建模的实际权衡，以及推进规范性公平性评估。
...
TL;DR: Mi:dm 2.0是KT开发的韩语中心双语大语言模型，包含11.5B和2.3B两个版本，专注于韩国文化价值观和常识理解，在韩语基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理韩语时存在数据质量不足和文化对齐问题，无法充分理解韩国社会的价值观、推理模式和常识知识，需要专门针对韩国文化背景优化的AI模型。

Method: 采用专有数据清洗、高质量合成数据生成、课程学习策略数据混合、韩语优化分词器等综合数据处理流程；提供Base(11.5B)和Mini(2.3B)两种配置，分别采用深度扩展策略和资源优化设计。

Result: 在韩国特定基准测试中达到最先进性能，在KMMLU上获得顶级零样本结果，在语言、人文和社会科学任务中表现出强大的内部评估结果。

Conclusion: Mi:dm 2.0通过提供可访问的高性能韩语中心LLM，旨在加速韩国各行业AI采用，加强韩国AI开发者社区，为更广泛的K-intelligence愿景奠定基础。
...
TL;DR: 论文主张从符号化关系标签转向自然语言关系描述，提出混合设计原则，在保留最小结构骨架的同时实现更灵活、上下文敏感的关系表示。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱使用预定义的符号关系模式（分类关系标签），但现实世界的关系通常是上下文相关、细微且有时不确定的。符号关系压缩了关键语义细节，而LLM的出现改变了知识的创建和消费方式，支持自然语言事实合成和上下文丰富的推理。

Method: 提出从符号关系转向自然语言关系描述，采用混合设计原则：保留最小结构骨架，同时支持灵活、上下文敏感的自然语言关系表示，而不是仅仅用LLM更高效地填充传统模式。

Result: 论文提出了一个概念框架，主张重新思考关系表示本身，而不是仅仅利用LLM来更高效地填充传统模式。提出自然语言关系描述和混合设计原则作为解决方案。

Conclusion: 随着LLM的出现，需要从根本上重新思考知识图谱中关系的表示方式，从符号标签转向自然语言描述，采用混合设计原则来平衡结构性和灵活性，以适应LLM时代的知识表示需求。
...
<details>
  <summary>Details</summary>
Motivation: 研究人类偏好评估中需要多少判断才能可靠检测生成模型的微小改进，因为当前评估结果经常不明确或为负，可能反映了评估统计功效不足而非模型等价

Method: 分析当偏好信号在提示间分散时的最优分配策略，通过实证分析大规模人类偏好数据集，比较不同评估协议和模态（聊天、图像生成、带执行反馈的代码生成）

Result: 大多数真实比较处于偏好信号分散状态，需要远多于通常收集的判断数；减少提示诱导变异性的精选基准能通过1.5倍降低提示级方差，系统性地提高检测能力

Conclusion: 不明确或负面的评估结果通常反映评估统计功效不足而非模型等价，需要在评估设计中明确考虑效应大小、预算和协议设计
...
TL;DR: SubTokenTest：评估大语言模型子词理解能力的实用基准，包含10个任务4个领域，重点关注字符级任务中的tokenization问题


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型的推理能力显著提升，但在基本字符级任务（如统计单词中的字母数）上仍然表现不佳，这源于其tokenization过程。现有基准虽然通过基本字符操作揭示了这一弱点，但这些失败常因缺乏实际相关性而被忽视。然而，许多实际应用（如导航基于文本的地图或解释结构化表格）严重依赖精确的子词理解。

Method: 引入SubTokenTest基准，通过实用、效用驱动的任务评估子词理解能力。该基准包含四个领域的十个任务，通过将性能与复杂推理解耦来隔离tokenization相关的失败。对九个先进LLM进行全面评估，研究测试时缩放对子词推理的影响，并探索字符级信息在隐藏状态中的编码方式。

Result: 论文提供了对九个先进大语言模型的全面评估结果，展示了它们在子词理解任务上的表现。同时研究了测试时缩放对子词推理的影响，并探索了字符级信息在模型隐藏状态中的编码方式。

Conclusion: SubTokenTest基准揭示了当前大语言模型在子词理解方面的局限性，强调了tokenization过程对实际应用的影响。该研究为评估和改进模型在字符级任务上的表现提供了重要工具，并有助于理解模型如何处理子词信息。
...
TL;DR: 该论文提出了一种三阶段专利权利要求生成框架，解决了跨司法管辖区泛化能力差、权利要求与现有技术语义关系建模不足以及质量评估不可靠三大问题，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前专利权利要求生成系统存在三个根本性局限：跨司法管辖区泛化能力差、权利要求与现有技术之间语义关系建模不足、以及质量评估不可靠。这些问题限制了自动化专利审查流程的实际应用效果。

Method: 提出了一种新颖的三阶段框架：1）关系感知相似性分析，使用8个专用头的多头注意力机制进行显式关系建模；2）领域自适应权利要求生成，通过课程学习和跨5个专利领域的动态LoRA适配器选择；3）统一质量评估，在评估方面之间实施交叉注意力机制进行综合质量评估。

Result: 在USPTO HUPD数据集、EPO专利集合和Patent-CE基准测试上的广泛实验显示：相比GPT-4o获得7.6点ROUGE-L提升，相比Llama-3.1-8B获得8.3% BERTScore改进，与人类专家的相关性达到0.847（而单独评估模型为0.623）。跨司法管辖区性能保持率达到89.4%，而基线方法仅为76.2%。

Conclusion: 该方法通过关系感知相似性分析、领域自适应生成和统一质量评估的三阶段框架，显著提升了专利权利要求生成的性能，为自动化专利审查工作流程提供了全面解决方案。
...
TL;DR: 该论文研究大型语言模型输出质量因用户社会人口属性而异的身份依赖性退化问题，提出了一种轻量级、无需训练的身份鲁棒生成框架，能显著减少身份相关偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型输出常因用户社会人口属性不同而产生差异，导致事实准确性、实用性和安全性方面的差异，即使在客观问题中这些人口信息无关紧要。现有研究多关注刻板印象或代表性偏见，而本文研究核心响应质量的身份依赖性退化问题。

Method: 提出轻量级、无需训练的身份鲁棒生成框架，选择性地中和非关键身份信息，同时保留语义上必要的属性，从而保持输出内容的完整性。

Result: 在四个基准测试和18个社会人口身份上的实验表明，与普通提示相比，身份依赖性偏见平均减少77%；与基于提示的防御方法相比，减少45%。

Conclusion: 该工作填补了减轻提示中用户身份线索对核心生成质量影响的关键空白，通过选择性身份信息中和实现了更公平、更一致的模型输出。
...
### [92] [OrthoGeoLoRA: Geometric Parameter-Efficient Fine-Tuning for Structured Social Science Concept Retrieval on theWeb](https://arxiv.org/abs/2601.09185)
*Zeqiang Wang,Xinyue Wu,Chenxi Li,Zixi Chen,Nishanth Sastry,Jon Johnson,Suparna De*

Main category: cs.CL

TL;DR: 提出OrthoGeoLoRA方法解决标准LoRA的几何缺陷，通过正交约束改进参数高效微调，在资源受限的社会科学信息系统中表现更优


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和文本编码器在社会科学信息系统中的应用日益广泛，但完全微调的计算和能耗成本高，对小型机构和非营利组织构成障碍。标准LoRA方法存在几何缺陷，需要改进

Method: 提出OrthoGeoLoRA方法，通过强制低秩因子正交化（Stiefel流形）实现SVD形式ΔW=BΣA⊤，几何重新参数化保持与标准优化器的兼容性

Result: 在多语言句子编码器实验中，OrthoGeoLoRA在相同低秩预算下，在分层概念检索基准上优于标准LoRA和其他PEFT变体

Conclusion: OrthoGeoLoRA为资源受限环境中的基础模型适配提供了更计算和参数高效的路径，改进了标准LoRA的几何缺陷
...
TL;DR: ProFit通过选择性掩码低概率标记来缓解传统SFT中的单参考过拟合问题，提升LLM对齐效果
...
Method: 研究发现token概率与语义重要性存在内在联系：高概率token承载核心逻辑框架，低概率token多为可替换表达。基于此提出ProFit方法，选择性掩码低概率token以防止表面层过拟合。

Result: 大量实验证实，ProFit在通用推理和数学基准测试中持续优于传统SFT基线方法。

Conclusion: 通过揭示token概率与语义重要性的关系，ProFit有效缓解了传统SFT中的单参考过拟合问题，为LLM对齐提供了一种高效且有效的解决方案。
...
TL;DR: A.X K1是一个5190亿参数的混合专家语言模型，通过多阶段数据处理在约10T token上预训练，采用Think-Fusion训练方法实现可控推理模式切换，在保持竞争力的同时特别在韩语基准测试中表现突出。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合推理能力与推理效率之间的差距，通过支持显式可控推理来促进在不同现实场景中的可扩展部署。

Method: 1. 利用缩放定律在固定计算预算下优化训练配置和词汇量；2. 采用多阶段数据处理管道构建约10T token的语料库；3. 提出简单有效的Think-Fusion训练方法，允许用户在单一统一模型内切换思考与非思考模式。

Result: A.X K1在性能上与领先的开源模型具有竞争力，同时在韩语基准测试中建立了独特优势。

Conclusion: A.X K1通过创新的Think-Fusion训练方法和优化的训练配置，成功实现了推理能力与效率的平衡，为大规模语言模型的实际部署提供了有效解决方案。
...
TL;DR: 提出UserLM-R1用户语言模型，通过综合用户画像和目标驱动的决策策略，结合推理能力解决现有用户模拟器通用性差和缺乏战略思维的问题。


<details>
  <summary>Details</summary>
Motivation: 现有用户模拟器存在两个主要问题：1）依赖静态、上下文无关的用户画像，需要大量手动重新设计以适应新场景，限制了通用性；2）忽视人类的战略思维，容易受到智能体操纵。需要开发能够跨领域通用且具备主动谈判能力的用户模拟器。

Method: 提出UserLM-R1用户语言模型，包含：1）构建包含静态角色和动态场景特定目标的综合用户画像；2）采用目标驱动的决策策略，在生成响应前产生高质量推理；3）通过监督微调（SFT）和多奖励强化学习（RL）来精炼推理并提升战略能力。

Result: 广泛的实验结果表明，UserLM-R1在性能上优于竞争基线方法，特别是在更具挑战性的对抗性测试集上表现突出。

Conclusion: UserLM-R1通过结合综合用户画像、推理能力和战略思维训练，有效解决了现有用户模拟器的局限性，为智能体后训练提供了更通用、更智能的交互环境。
...
TL;DR: FairToT是一个推理时框架，通过提示引导的毒性评估增强LLM的公平性，识别可能产生人口统计相关差异的情况，并决定何时应用额外评估，减少群体间差异。
...
Method: 提出FairToT推理时框架，通过提示引导的毒性评估来增强LLM公平性。框架识别可能发生人口统计相关变化的情况，并决定何时应用额外评估。引入两个可解释的公平性指标来检测这些情况，在不修改模型参数的情况下提高推理一致性。

Result: 在基准数据集上的实验表明，FairToT减少了群体层面的差异，同时保持了稳定可靠的毒性预测，证明推理时优化为基于LLM的毒性评估系统的公平性改进提供了有效实用的方法。

Conclusion: FairToT框架通过推理时优化有效提升了LLM在毒性评估中的公平性，为LLM在在线审核系统中的公平应用提供了实用解决方案。
...
TL;DR: MCGA是一个针对中国古典文学的多任务音频语料库，包含六项语音任务，用于评估多模态大语言模型在音频处理方面的能力，发现现有模型仍面临显著挑战。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的快速发展，其在中文古典研究领域的潜力受到关注。然而现有研究主要关注文本和视觉模态，音频语料库在该领域尚未得到充分探索。
...
Result: 实验结果表明，当前模型在处理MCGA测试集时仍面临重大挑战。作者公开了MCGA语料库和代码，以促进开发在中文古典研究中具有更强大多维音频能力的多模态大语言模型。

Conclusion: MCGA填补了中文古典研究中音频语料库的空白，为评估和提升多模态大语言模型的音频处理能力提供了重要资源，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗问答方法通常遍历整个知识图谱或进行大规模检索，这会引入大量噪声并导致多跳推理不稳定。核心挑战不在于扩展知识访问，而在于识别和推理每个查询的适当证据子集。

Method: ReGraM采用区域优先的知识图谱推理框架，首先构建查询对齐的子图，然后在这个局部区域内进行多证据感知的逐步推理，避免假设所有关系同等有用。

Result: 在七个医疗问答基准测试中，ReGraM始终优于强基线(KGARevion)，在MCQ上获得8.04%的绝对准确率提升，在SAQ上获得4.50%提升，幻觉率降低42.9%。消融和定性分析表明区域构建与跳数推理的对齐是改进的主要驱动力。

Conclusion: 区域优先的知识图谱推理是提高医疗问答事实准确性和一致性的有效范式，通过专注于最相关的知识图谱部分，避免了传统方法中所有关系同等有用的不合理假设。
...
TL;DR: 研究发现语言模型在德语定冠词上并非完全基于规则，而是部分依赖记忆关联


<details>
  <summary>Details</summary>
Motivation: 探究语言模型在语法一致性任务上的表现是源于规则泛化能力还是记忆机制，以德语定冠词为具体研究对象

Method: 使用GRADIEND梯度可解释性方法，学习特定性别-格位组合的定冠词转换参数更新方向

Result: 为特定性别-格位组合学习的更新经常影响不相关的性别-格位设置，不同设置间受影响最严重的神经元有显著重叠

Conclusion: 语言模型并非严格基于规则编码德语定冠词，至少部分依赖记忆关联而非抽象语法规则
...
TL;DR: 该研究评估了LLM在情境对话中建立和利用共同基础的能力，重点关注实体关系引用，并提出了改进共同基础建立和利用的方法。


<details>
  <summary>Details</summary>
Motivation: 共同基础在情境对话中至关重要，但现有研究很少探讨如何明确表示和存储共同基础以供后续使用。缺乏这样的机制使得无法确定LLM的确认或澄清行为是否真正反映了基于共同基础的理解。

Method: 评估模型在情境对话中通过实体关系引用建立和利用共同基础的能力，测试多种表示共同基础的方法，并提出改进共同基础建立和后续使用的方法。

Result: 未在摘要中提供具体结果，但研究评估了多种共同基础表示方法，并提出了改进方法。

Conclusion: 需要开发明确的共同基础表示和存储机制，以真正实现对话系统对共同基础的建立和利用，而不仅仅是表面的确认或澄清行为。
...
TL;DR: 该研究首次对英语和土耳其语临床关系抽取任务进行了双语评估，提出了基于对比学习的关系感知检索方法，结果显示基于提示的LLM方法优于传统微调模型，英语表现优于土耳其语。


<details>
  <summary>Details</summary>
Motivation: 非英语语言临床信息抽取标注数据稀缺，阻碍了主要在英语环境中开发的大型语言模型方法的评估。本研究旨在填补这一空白，首次对英语和土耳其语临床关系抽取任务进行双语评估。

Method: 1) 创建首个英土平行临床关系抽取数据集，基于2010 i2b2/VA关系分类语料库；2) 系统评估多种提示策略，包括上下文学习和思维链方法；3) 提出关系感知检索方法，基于对比学习同时捕捉句子级和关系级语义；4) 与PURE等微调基线模型进行比较。

Result: 1) 基于提示的LLM方法始终优于传统微调模型；2) 英语评估在所有LLM和提示技术上均优于土耳其语；3) 在上下文学习方法中，关系感知检索获得最高性能，Gemini 1.5 Flash在英语和土耳其语分别达到0.906和0.888的微F1分数；4) 结合关系感知检索和结构化推理提示，DeepSeek-V3在英语达到0.918 F1。
...
TL;DR: LLMs在处理事件语义时存在目的论偏见，会系统性幻觉目标导向事件的完成，缺乏真正的时体意识，更像预测性叙事引擎而非逻辑推理器。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否真正理解事件的组合语义，还是仅仅依赖表层概率启发式。通过未完成体悖论这一逻辑现象来探究LLMs对事件语义的理解能力。

Method: 引入ImperfectiveNLI诊断数据集，评估最先进的开源模型，分析内部表示，并通过提示干预来减少幻觉完成。

Result: 发现普遍存在的目的论偏见：模型会系统性幻觉目标导向事件的完成，即使文本明确否定。内部嵌入能区分过程与结果，但推理决策受目标达成的强先验主导。提示干预能减少幻觉完成，但也会增加对有效蕴含的错误拒绝。

Conclusion: 当前LLMs缺乏结构性的时体意识，更像是预测性叙事引擎而非忠实的逻辑推理器。
...
Method: ACT（Activation-Guided Channel-wise Ability Transfer）方法：1）分析模块激活差异定位能力相关通道（通常<5%）；2）选择性转移对应参数；3）轻量微调确保兼容性。

Result: 在多语言数学和科学推理任务上的实验表明，ACT能够恢复遗忘的能力同时保留已有技能，还能将多个专业化模型的能力集成到单一模型中，实现最小干扰。

Conclusion: 能力相关激活高度集中在少数通道且解耦良好，基于此的ACT方法有效解决了灾难性遗忘问题，实现了多能力集成，为LLM能力转移提供了新思路。
...
TL;DR: PAGER提出了一种基于页面驱动的自主知识表示框架，通过结构化认知大纲和迭代检索填充机制，为RAG构建更全面、连贯的知识表示。


<details>
  <summary>Details</summary>
Motivation: 现有迭代式RAG模型虽然能逐步积累知识，但缺乏连贯的组织结构，限制了构建全面、连贯知识表示的能力。需要一种结构化方法来更好地组织检索到的知识。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型中存在内部知识冲突问题，即同一事件的不一致信息被编码在模型的参数知识中。先前研究主要关注解决模型内部知识与外部资源之间的冲突，而预训练过程中产生的冲突在模型内部表示中的定位问题尚未探索

Method: 设计基于机制可解释性方法的框架，识别预训练数据中冲突知识在语言模型中的编码位置和方式，利用这些方法在推理时进行因果干预和控制

Result: 发现语言模型的特定内部组件负责编码预训练中的冲突知识，并证明机制可解释性方法可用于在推理时因果干预和控制冲突知识

Conclusion: 该研究为理解语言模型内部知识冲突提供了新的视角，展示了机制可解释性方法在定位和干预模型内部冲突知识方面的潜力
...
TL;DR: 该论文提出了一种改进小语言模型逻辑推理能力的方法，通过错误分类、增量推理和验证模块来减少符号翻译错误，提高推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在将自然语言翻译为一阶逻辑时经常产生格式和翻译错误，现有基于自我迭代的方法严重依赖基础模型能力，需要更可靠的解决方案。

Method: 1. 对常见错误进行分类；2. 使用大语言模型合成数据进行小模型微调；3. 引入增量推理，将推理分为谓词生成和一阶逻辑翻译两个阶段；4. 添加针对谓词-元数错误的验证模块。

Result: 在四个逻辑推理数据集上评估三个模型家族，综合微调、增量推理和验证模块降低了错误率，增加了谓词覆盖率，并提高了小语言模型的推理性能。

Conclusion: 该方法通过分解推理过程并加入验证机制，显著提升了小语言模型的符号推理可靠性，向开发可靠且易访问的符号推理系统迈进了一步。
...
TL;DR: 本文提出了SlidesGen-Bench基准，用于评估幻灯片生成系统，通过视觉域统一评估框架、三维度量化指标和人工偏好对齐数据集，解决了现有评估方法缺乏可比性和可靠性的问题。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的快速发展，出现了多种自动幻灯片生成范式，但现有评估协议难以提供跨架构的可比分数，或依赖于未校准的主观判断，缺乏统一、量化、可靠的评估标准。

Method: 1. 建立基于视觉域的统一评估框架，将终端输出视为渲染图像，对底层生成方法保持中立；2. 提出计算化方法，从内容、美学和可编辑性三个维度进行量化评估；3. 构建Slides-Align1.5k数据集，包含9个主流生成系统在7个场景下的幻灯片，并与人类偏好对齐。

Result: 实验表明，SlidesGen-Bench相比现有评估流程，与人类判断具有更高的对齐度，提供了更可靠、可复现的评估指标。

Conclusion: SlidesGen-Bench通过统一性、量化和可靠性三个核心原则，为异构幻灯片生成系统提供了有效的评估基准，解决了现有评估方法的局限性，促进了该领域的发展。
...
TL;DR: MVSS是一个多视图结构化综述生成框架，通过先构建概念树、再生成比较表、最后生成文本的结构优先范式，解决了现有方法在层次关系和结构化比较方面的不足。
...
Method: MVSS采用结构优先范式：首先构建研究领域的概念树，然后基于树结构生成比较表，最后使用这两者作为结构约束来生成综述文本，实现结构、比较和叙述的多视图互补表示。

Result: 在76个计算机科学主题上的实验表明，MVSS在组织结构和证据基础方面优于现有方法，其性能可与专家撰写的综述相媲美。

Conclusion: MVSS框架通过多视图结构化方法有效提升了自动综述生成的结构质量和比较完整性，为科学文献综述提供了更接近专家水平的自动生成方案。
...
TL;DR: SERM是一种自进化相关性模型方法，通过多智能体样本挖掘器和多智能体相关性标注器解决大规模工业环境中信息样本稀疏和伪标签不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界查询流的动态演化特性使得相关性模型难以泛化到实际搜索场景。在大规模工业环境中，自进化技术面临两个挑战：1）信息丰富的样本稀疏且难以识别；2）当前模型生成的伪标签可能不可靠。

Method: 提出自进化相关性模型方法（SERM），包含两个互补的多智能体模块：多智能体样本挖掘器（检测分布偏移并识别信息丰富的训练样本）和多智能体相关性标注器（通过两级一致性框架提供可靠标签）。

Result: 在每日服务数十亿用户请求的大规模工业环境中评估SERM。实验结果表明，通过迭代自进化，SERM能够实现显著的性能提升，经过广泛的离线多语言评估和在线测试验证。

Conclusion: SERM方法有效解决了大规模工业环境中自进化技术面临的挑战，通过多智能体协作实现了可靠的样本挖掘和标注，显著提升了相关性模型的性能。
...
TL;DR: 该研究系统评估了MXFP格式下的后训练量化，发现MXFP8接近无损，MXFP4仍有挑战，量化效果受格式兼容性影响，跨模型趋势一致，并提出预缩放优化策略。
...
Method: 系统研究MXFP格式下的PTQ，涵盖超过7种PTQ算法、15个评估基准和3个LLM家族，包括对量化敏感性和误差源的分析。

Result: 1) MXFP8实现接近无损性能，MXFP4仍有显著精度损失；2) PTQ效果强烈依赖格式兼容性；3) 量化敏感性趋势在模型家族和模态间高度一致；4) 量化缩放因子是MXFP4主要误差源，预缩放优化可显著缓解。

Conclusion: 研究为现有PTQ方法适应MXFP量化提供了实用指导，揭示了MXFP格式下量化的关键特性和优化策略。
...
TL;DR: 提出对话遥测(DT)框架，通过进度估计器和停滞指数两个模型无关信号，监控信息收集对话的效率并检测无效提问模式


<details>
  <summary>Details</summary>
Motivation: 自主系统进行基于模式的信息收集对话时存在仪表化差距，缺乏回合级别的可观测指标来监控获取效率和检测无效提问

Method: 引入对话遥测(DT)框架，包含：1)进度估计器(PE)量化每个类别的剩余信息潜力；2)停滞指数(SI)检测重复类别探测且语义相似、边际增益低的失败模式

Result: 在受控的搜救模拟中使用LLM验证DT，能区分高效与停滞的对话轨迹，将DT信号集成到RL策略中提高了性能

Conclusion: DT提供了可解释的回合级别仪表化，在停滞带来操作成本时能改善策略性能
...
TL;DR: 本文提出了一种基于半结构化长思维链的强化学习框架，通过多样化规划分支和群体感知多样性奖励机制，在保持生成质量的同时显著提升大语言模型在创造性写作等开放任务中的输出多样性。


<details>
  <summary>Details</summary>
Motivation: 基于强化学习的大语言模型增强方法通常会导致输出多样性降低，这削弱了它们在创造性写作等开放任务中的实用性。现有方法缺乏明确的多样化探索引导机制，过度优先考虑优化效率和性能而忽视了多样性。

Method: 提出了一个围绕半结构化长思维链构建的强化学习框架，将生成过程分解为明确规划的中间步骤。引入了多样化规划分支方法，基于多样性变化在规划阶段战略性地引入分歧，同时采用群体感知多样性奖励来鼓励不同的生成轨迹。

Result: 在创造性写作基准测试中的实验结果表明，该方法在不损害生成质量的前提下显著提高了输出多样性，持续优于现有基线方法。

Conclusion: 通过将生成过程分解为规划步骤并引入多样性引导机制，该框架有效解决了强化学习增强大语言模型时输出多样性降低的问题，为开放任务中的多样化生成提供了有效解决方案。
...
TL;DR: TaxoBell使用高斯盒嵌入框架解决传统盒嵌入在分类学扩展中的问题，通过将盒几何与多元高斯分布转换，实现稳定优化、语义不确定性建模和模糊概念处理，在五个基准数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 手动分类学扩展耗时且无法跟上新概念的出现速度。现有自动化方法依赖点向量嵌入，难以处理分类学中非对称的"is-a"关系。盒嵌入虽然提供了包含和分离关系建模的潜力，但存在梯度不稳定、缺乏语义不确定性概念以及无法表示多义性或模糊性等问题。

Method: TaxoBell采用高斯盒嵌入框架，将盒几何与多元高斯分布进行转换，其中均值编码语义位置，协方差编码不确定性。通过基于能量的优化实现稳定优化，能够鲁棒地建模模糊概念，并提供可解释的层次推理。

Result: 在五个基准数据集上的实验表明，TaxoBell显著优于八种最先进的分类学扩展基线方法，在MRR指标上提升19%，在Recall@k指标上提升约25%。通过误差分析和消融研究进一步展示了TaxoBell的优势和局限性。

Conclusion: TaxoBell通过高斯盒嵌入框架成功解决了传统盒嵌入在分类学扩展中的关键问题，实现了稳定优化、语义不确定性建模和模糊概念处理，为分类学扩展提供了更有效的方法。
...
TL;DR: 论文提出RGD（基于生成数据的路由）新范式，研究仅使用生成数据训练LLM路由器的挑战，发现查询-答案路由器比纯查询路由器对生成器质量更敏感，并提出CASCAL查询路由器通过共识投票和分层聚类提升鲁棒性。
...
Method: 提出RGD设置，使用生成器LLM从高级任务描述生成查询和答案来训练路由器。比较查询-答案路由器和纯查询路由器，分析有效生成器的关键特征，并提出CASCAL方法：通过共识投票估计模型正确性，通过分层聚类识别模型特定技能领域。

Result: 在4个多样化基准和12个模型上评估发现：1）查询-答案路由器比纯查询路由器对生成器质量下降更敏感；2）有效生成器需能准确回答自己的问题，且问题能充分区分模型池性能；3）CASCAL在弱生成器数据上训练时，比最佳查询-答案路由器绝对准确率高4.6%。

Conclusion: RGD是LLM路由器训练的有效替代方案，纯查询路由器比查询-答案路由器对生成器质量更具鲁棒性。CASCAL方法通过共识投票和分层聚类显著提升在生成数据上的路由性能，为无标注数据场景提供实用解决方案。
...
TL;DR: 本文提出了共情适用性框架(EAF)，用于在医生生成回复前预测患者查询中的共情需求，建立了首个用于前瞻性共情建模的基准数据集，并在真实患者查询上验证了该框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在临床工作流程中缺乏临床共情能力，而现有NLP框架主要关注医生回复中的共情标签，对患者查询中前瞻性共情需求建模支持有限，特别是在一般健康咨询场景中。

Method: 提出了基于临床、上下文和语言线索的共情适用性框架(EAF)，通过人类和GPT-4o双重标注创建真实患者查询基准数据集，训练分类器预测共情适用性，并与启发式和零样本LLM基线进行比较。

Result: 在人类共识子集中观察到显著的人类-GPT对齐；基于人类标注和GPT标注训练的分类器在预测共情适用性方面表现强劲，优于启发式和零样本LLM基线；错误分析揭示了隐性困扰、临床严重性模糊和上下文困境等挑战。

Conclusion: EAF为在回复生成前识别共情需求提供了框架，建立了前瞻性共情建模基准，支持异步医疗中的共情沟通；未来需要多标注者建模、临床医生参与校准和文化多样性标注来解决现有挑战。
...
TL;DR: 论文提出了一种值感知数值表示方法，通过专用前缀令牌将数值大小信息直接注入语言模型的输入空间，以解决Transformer模型在数值理解和算术运算中的系统性错误。


<details>
  <summary>Details</summary>
Motivation: Transformer语言模型在数学推理基准测试中表现良好，但在基本数值理解和算术运算方面仍然脆弱。主要限制是数字被处理为符号令牌，其嵌入没有明确编码数值，导致系统性错误。

Method: 引入值感知数值表示，增强标准令牌化输入：使用专用前缀令牌，其嵌入明确基于底层数值进行条件化。该机制将大小信息直接注入模型的输入空间，同时保持与现有分词器和仅解码器Transformer架构的兼容性。

Result: 在算术任务评估中，该方法在数值格式、任务和操作数长度方面均优于基线。表明明确编码数值是提高语言模型基本数值鲁棒性的有效且高效方式。

Conclusion: 通过值感知数值表示将数值大小信息直接注入输入空间，可以有效改善Transformer语言模型的数值鲁棒性，同时保持与现有架构的兼容性。
...
### [118] [Fairness risk and its privacy-enabled solution in AI-driven robotic applications](https://arxiv.org/abs/2601.08953)
...
Method: 提出效用感知的机器人决策公平性度量，分析公平性与用户数据隐私的联合关系，推导隐私预算控制公平性指标的条件，建立统一框架。

Result: 建立了隐私预算控制公平性指标的数学条件，在机器人导航任务中测试了该框架，发现隐私预算可联合用于实现公平性目标。

Conclusion: 在隐私保护框架下协同考虑公平性问题，是迈向AI伦理使用的重要一步，有助于增强日常环境中自主机器人的可信度。
...
### [119] [Design Methodology of Hydraulically-driven Soft Robotic Gripper for a Large and Heavy Object](https://arxiv.org/abs/2601.09104)
*Ko Yamamoto,Kyosuke Ishibashi,Hiroki Ishikawa,Osamu Azami*

Main category: cs.RO

TL;DR: 本文提出了一种液压驱动软体机器人抓手的系统设计方法，用于抓取10-20公斤、直径20-30厘米的大型重物，通过液压驱动替代传统气动方案以产生更大抓取力。


<details>
  <summary>Details</summary>
Motivation: 现有软体抓手多为气动驱动，工作压力仅数百kPa，无法产生足够输出力来抓取大型重物（10-20公斤）。液压驱动具有更高工作压力（数MPa），能产生更大功率，适合重物抓取应用。

Method: 1. 开发液压驱动软体抓手；2. 基于数学模型确定基本设计参数，建立驱动压力、弯曲角度、物体质量和抓取力之间的关系；3. 通过有限元分析进行详细设计，选择适合抓取重物的材料；4. 实现手指弯曲角度的闭环控制。

Result: 成功实现了对20公斤物体的抓取实验，并完成了手指弯曲角度的闭环控制验证。

Conclusion: 液压驱动软体抓手设计方法有效解决了传统气动软体抓手无法抓取大型重物的问题，通过数学模型和有限元分析优化设计，实现了对10-20公斤物体的可靠抓取。
...
### [120] [CEI: A Unified Interface for Cross-Embodiment Visuomotor Policy Learning in 3D Space](https://arxiv.org/abs/2601.09163)
...
<details>
  <summary>Details</summary>
Motivation: 现有的机器人基础模型在大规模操作数据集上训练，但往往过度拟合特定的视角、机械臂和平行夹爪等特定形态，存在数据集偏差问题

Method: 提出跨形态接口(CEI)框架，引入功能相似性概念并使用方向性Chamfer距离量化，通过梯度优化对齐机器人轨迹，为未见过的机械臂和末端执行器合成观测和动作

Result: 在仿真中将Franka Panda机器人的数据和策略迁移到16种不同形态，在真实世界实现UR5+AG95与UR5+Xhand之间的双向迁移，平均迁移率达到82.4%

Conclusion: CEI能够有效解决机器人基础模型的形态过拟合问题，支持跨形态的策略迁移，并可扩展到空间泛化和多模态运动生成能力
...
### [121] [Vision-Conditioned Variational Bayesian Last Layer Dynamics Models](https://arxiv.org/abs/2601.09178)
*Paul Brunzema,Thomas Lew,Ray Zhang,Takeru Shirasawa,John Subosits,Marcus Greiff*

Main category: cs.RO

TL;DR: 提出了一种基于视觉条件的变分贝叶斯最后一层动力学模型，用于预测环境变化对机器人系统行为的影响，并在车辆赛车场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人系统的敏捷控制需要预测环境对系统行为的影响，但传统建模方法难以捕捉行为的突然变化，而自适应方法本质上是反应式的，可能来不及确保安全。特别是在快速变化条件下，实现自主框架内的主动适应仍然是一个挑战。

Method: 提出了一种视觉条件的变分贝叶斯最后一层动力学模型。该模型首先学习名义车辆动力学，然后通过潜在特征的逐特征仿射变换进行微调，实现上下文感知的动力学预测。最终模型被集成到车辆赛车的最优控制器中。

Result: 在雷克萨斯LC500赛车通过水坑的场景中验证了该方法。使用视觉条件后，系统在变化条件下成功完成了所有12圈尝试。相比之下，所有没有视觉上下文的基线方法都持续失控，证明了在高性能应用中主动动力学适应的重要性。

Conclusion: 视觉条件的变分贝叶斯最后一层动力学模型能够有效预测环境变化并实现主动适应，在快速变化条件下显著提升了机器人系统的控制性能和安全保障能力。
...
### [122] [Online Trajectory Optimization for Arbitrary-Shaped Mobile Robots via Polynomial Separating Hypersurfaces](https://arxiv.org/abs/2601.09231)
*Shuoye Li,Zhiyuan Song,Yulin Li,Zhihai Bi,Jun Ma*

Main category: cs.RO

TL;DR: 提出使用多项式超曲面进行非线性分离的轨迹优化方法，解决传统基于凸近似的线性分离器在复杂非凸环境中的保守性问题。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹优化方法使用线性分离超平面进行避碰，但线性分离器仅适用于凸集，需要对机器人和障碍物进行凸近似，这在复杂狭窄环境中过于保守。

Method: 1. 推广经典分离超平面定理，证明任意两个不相交的有界闭集都可以用多项式超曲面分离；2. 构建非线性规划问题，联合优化机器人轨迹和分离多项式的系数，实现几何感知的避碰。

Result: 方法在仿真和真实世界实验中，对于非凸机器人能够实现平滑、无碰撞、敏捷的机动，在凸近似基线方法失败的环境中表现优异。

Conclusion: 通过引入非线性多项式超曲面分离，彻底消除了传统方法对凸近似的依赖，实现了更精确的几何感知碰撞避免，在复杂非凸环境中具有显著优势。
...
### [123] [Feedback-Based Mobile Robot Navigation in 3-D Environments Using Artificial Potential Functions Technical Report](https://arxiv.org/abs/2601.09318)
*Ro'i Lang,Elon Rimon*

Main category: cs.RO

TL;DR: 该技术报告提出了一种用于3D工作空间中球形和圆柱形障碍物环境下的多项式导航函数构造与分析方法，通过建立唯一非退化最小值条件确保无局部极小值，并在障碍物丰富的3D环境中进行了数值验证。


<details>
  <summary>Details</summary>
Motivation: 在3D工作空间中进行运动规划时，需要处理球形和圆柱形障碍物，传统方法可能面临局部极小值问题。本文旨在构建能够保证唯一目标点且避免局部极小值的多项式导航函数，特别是在障碍物可能相交的复杂环境中。

Method: 将工作空间建模为有界球形区域，使用光滑多项式隐函数编码障碍物。建立导航函数的数学条件，确保在目标点处存在唯一非退化最小值，同时避免局部极小值。提供梯度和Hessian分析，并在障碍物丰富的3D环境中进行数值模拟验证。

Result: 成功构建了适用于球形和圆柱形障碍物环境的3D多项式导航函数，建立了确保唯一非退化最小值的充分条件，即使在成对相交障碍物情况下也能避免局部极小值。数值模拟验证了理论结果的有效性。

Conclusion: 提出的多项式导航函数方法能够有效处理3D工作空间中的运动规划问题，特别是在包含球形和圆柱形障碍物的复杂环境中。该方法通过数学保证避免了局部极小值问题，为3D导航提供了可靠的理论框架和实用工具。
...
### [124] [ReflexDiffusion: Reflection-Enhanced Trajectory Planning for High-lateral-acceleration Scenarios in Autonomous Driving](https://arxiv.org/abs/2601.09377)
*Xuemei Yao,Xiao Yang,Jianbin Sun,Liuwei Xie,Xuebin Shao,Xiyu Fang,Hang Su,Kewei Yang*

Main category: cs.RO

TL;DR: ReflexDiffusion：一种通过反射调整增强扩散轨迹规划器的推理阶段框架，专门解决自动驾驶车辆在急转弯等高横向加速度场景中的安全问题，在nuPlan Test14-hard基准上比SOTA方法提升14.1%驾驶分数。
...
Result: 在nuPlan Test14-hard基准测试中，ReflexDiffusion在高横向加速度场景下比最先进方法提升了14.1%的驾驶分数，证明推理时轨迹优化可以有效补偿训练数据稀疏性，在接近操控极限时动态强化安全关键约束。

Conclusion: ReflexDiffusion提供了一种实用的架构无关解决方案，可直接部署到现有基于扩散的规划器中，通过推理时轨迹优化改善自动驾驶车辆在挑战性驾驶条件下的安全性，特别是在高横向加速度场景中。
...
### [125] [Data Scaling for Navigation in Unknown Environments](https://arxiv.org/abs/2601.09444)
*Lauri Suomela,Naoki Takahata,Sasanka Kuruppu Arachchige,Harry Edelman,Joni-Kristian Kämäräinen*

Main category: cs.RO

TL;DR: 大规模多样化数据集训练可实现零样本视觉导航，数据多样性比数据量更重要，简单回归模型在众包数据上表现优于复杂架构。


<details>
  <summary>Details</summary>
Motivation: 解决模仿学习导航策略在未见环境中的泛化问题，研究数据量和数据多样性对真实世界端到端视觉导航泛化能力的影响。

Method: 使用来自35个国家161个地点的4,565小时众包数据集，训练点目标导航策略，在四个国家的真实人行道机器人上进行125公里闭环控制评估。

Result: 大规模训练数据可实现未知环境中的零样本导航，性能接近环境特定演示训练的策略；数据多样性比数据量更重要，地理位置数量翻倍可使导航错误减少约15%；在嘈杂众包数据下，简单回归模型优于生成式和序列架构。

Conclusion: 数据多样性是实现真实世界视觉导航泛化的关键因素，简单回归模型在嘈杂众包数据上表现良好，大规模多样化数据集训练可实现有效的零样本导航。
...
### [126] [CLARE: Continual Learning for Vision-Language-Action Models via Autonomous Adapter Routing and Expansion](https://arxiv.org/abs/2601.09512)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人持续学习方法通常需要存储先前数据（示例），难以处理长任务序列，或依赖任务标识符进行部署。在现实世界中，机器人需要持续适应新任务和环境，同时保留已获得的知识，而当前基于预训练模型微调的方法会更新现有表示，不适合长期操作。

Method: CLARE在选定的前馈层中引入轻量级模块化适配器，通过学习新任务时基于层间特征相似性指导，仅在必要时自主扩展模型。在部署阶段，基于自动编码器的路由机制动态激活最相关的适配器，无需任务标签。

Result: 在LIBERO基准测试上的广泛实验表明，CLARE在新任务上实现高性能的同时，对早期任务没有灾难性遗忘，显著优于甚至基于示例的方法。

Conclusion: CLARE提供了一个通用、参数高效的示例无关持续学习框架，使视觉语言动作模型能够在现实世界中长期操作，持续适应新任务和环境，同时保留已有知识。
...
### [127] [Learning Whole-Body Human-Humanoid Interaction from Human-Human Demonstrations](https://arxiv.org/abs/2601.09518)
*Wei-Jin Huang,Yue-Yi Zhang,Yi-Lin Wei,Zhi-Wei Xia,Juantao Tan,Yuan-Ming Li,Zhilin Zhao,Wei-Shi Zheng*

Main category: cs.RO

TL;DR: PAIR+D-STAR框架：通过物理感知交互重定向从人类-人类交互数据生成高质量人形机器人交互数据，并结合解耦时空推理的分层策略学习复杂全身交互


<details>
  <summary>Details</summary>
Motivation: 人形机器人需要与人类进行物理交互，但缺乏高质量的人-人形交互数据。虽然可以利用丰富的人类-人类交互数据作为可扩展替代方案，但标准重定向方法会破坏关键接触点，且传统模仿学习策略仅模仿轨迹而缺乏交互理解

Method: 1. PAIR：接触中心的两阶段物理感知交互重定向管道，通过保持跨形态差异的接触语义来生成物理一致的人-人形交互数据；2. D-STAR：解耦时空动作推理器的分层策略，通过相位注意力（何时行动）和多尺度空间模块（何处行动）分离推理流，由扩散头融合产生超越模仿的同步全身行为

Result: 通过广泛而严格的模拟验证，该框架在性能上显著优于基线方法，实现了从人类-人类交互数据学习复杂全身交互的完整有效管道

Conclusion: PAIR+D-STAR框架成功解决了从人类-人类交互数据学习人形机器人交互的两个关键挑战：通过物理感知重定向生成高质量数据，以及通过解耦时空推理学习超越模仿的交互理解，为人形机器人的物理交互提供了有效解决方案
...
### [128] [Multimodal Signal Processing For Thermo-Visible-Lidar Fusion In Real-time 3D Semantic Mapping](https://arxiv.org/abs/2601.09578)
*Jiajun Sun,Yangyi Ou,Haoyuan Zheng,Chao yang,Yue Ma*

Main category: cs.RO

TL;DR: 本文提出了一种将热信息语义增强到3D点云地图的新方法，通过融合可见光和红外图像，将LiDAR点云投影到融合图像流上，分割热源特征并作为语义层应用于最终3D地图。


<details>
  <summary>Details</summary>
Motivation: 在复杂环境中，自主机器人导航和环境感知对SLAM技术提出了更高要求。需要生成不仅具有准确几何结构，还能对环境有重要语义理解的地图，这对于灾害快速评估和工业预防性维护等特定应用具有重要价值。

Method: 首先对可见光和红外图像进行像素级融合，然后将实时LiDAR点云投影到融合图像流上。在热通道中分割热源特征以即时识别高温目标，并将温度信息作为语义层应用于最终的3D地图。

Result: 该方法生成的地图不仅具有准确的几何结构，还拥有对环境的关键语义理解能力，能够识别高温目标并标注温度信息。

Conclusion: 提出的语义增强3D点云地图方法对于灾害快速评估和工业预防性维护等特定应用具有重要价值，为复杂环境中的自主导航和环境感知提供了更丰富的语义信息。
...
### [129] [ConvoLearn: A Dataset of Constructivist Tutor-Student Dialogue](https://arxiv.org/abs/2601.08950)
...
<details>
  <summary>Details</summary>
Motivation: 当前教育应用中的大型语言模型存在根本性教学局限性，倾向于直接揭示解决方案而非支持对话式学习。需要开发能够真正支持建构主义教学法的AI导师。

Method: 基于知识建构理论创建ConvoLearn数据集，包含六个核心教学维度。通过人类教师与模拟学生之间的受控交互，构建了1250个中学地球科学领域的师生对话（每个20轮）。使用QLoRA对Mistral 7B进行微调。

Result: 微调后的Mistral 7B（M=4.10，SD=1.03）在31名教师的评估中显著优于其基础版本（M=2.59，SD=1.11）和Claude Sonnet 4.5（M=2.87，SD=1.29）。训练有效改变了LLM行为，使其更倾向于知识建构策略。
...
### [130] [Programming over Thinking: Efficient and Robust Multi-Constraint Planning](https://arxiv.org/abs/2601.09097)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在多约束规划中存在根本性限制：纯推理范式容易产生不一致性、错误累积和高成本；而结合编码或求解器的方法缺乏灵活性，无法跨问题泛化。

Method: 提出SCOPE框架，将查询特定的推理与通用代码执行解耦，生成一致、确定且可重用的求解器函数，仅需最小化参数调整。

Result: 在TravelPlanner任务上达到93.1%成功率，比最佳基线（CoT）提升61.6%，同时降低推理成本1.4倍，时间减少约4.67倍。

Conclusion: SCOPE通过分离推理与执行，实现了多约束规划中的最先进性能，同时显著降低了成本和延迟，提供了可扩展的解决方案。
...
### [131] [DScheLLM: Enabling Dynamic Scheduling through a Fine-Tuned Dual-System Large language Model](https://arxiv.org/abs/2601.09100)
*Lixiang Zhang,Chenggong Zhao,Qing Gao,Xiaoke Zhao,Gengyi Bai,Jinhu Lv*

Main category: cs.AI

TL;DR: DScheLLM：基于双系统推理架构（快慢思维）的微调大语言模型动态调度方法，用于处理不同规模的扰动，在标准作业车间调度基准上表现出色。
...
Method: 提出DScheLLM动态调度方法，利用微调大语言模型在双系统（快-慢）推理架构中处理不同规模的扰动。构建统一的大语言模型框架处理动态事件，使用运筹学求解器获得的精确调度生成快慢推理模式的训练数据集，基于华为OpenPangu Embedded-7B模型使用LoRA在混合推理范式下进行微调。

Result: 在标准作业车间调度基准上的实验评估表明：快思维模式能高效生成高质量调度方案，慢思维模式能生成与求解器兼容且格式良好的决策输入。
...
### [132] [AviationLMM: A Large Multimodal Foundation Model for Civil Aviation](https://arxiv.org/abs/2601.09105)
...
<details>
  <summary>Details</summary>
Motivation: 民航是全球交通和商业的基石，确保其安全、效率和客户满意度至关重要。然而，现有的民航AI解决方案仍然孤立和狭窄，专注于孤立任务或单一模态，难以整合语音通信、雷达轨迹、传感器流和文本报告等异构数据，限制了态势感知、适应性和实时决策支持。

Method: 首先识别现有AI解决方案与需求之间的差距；其次描述模型架构，该架构能够处理空-地语音、监视、机载遥测、视频和结构化文本等多模态输入，执行跨模态对齐和融合，并产生从态势摘要、风险预警到预测性诊断和多模态事件重建的灵活输出。

Result: 提出了AviationLMM的设计愿景和架构，并识别了实现该愿景需要解决的关键研究机会，包括数据获取、对齐与融合、预训练、推理、可信性、隐私、对缺失模态的鲁棒性以及合成场景生成。

Conclusion: 通过阐述AviationLMM的设计和挑战，旨在推动民航基础模型的进展，并促进协调的研究努力，朝着集成、可信和隐私保护的民航AI生态系统发展。
...
### [133] [PrivacyReasoner: Can LLM Emulate a Human-like Privacy Mind?](https://arxiv.org/abs/2601.09152)
...
<details>
  <summary>Details</summary>
Motivation: 超越群体层面的情感分析，研究个体用户如何形成隐私担忧，需要模拟用户特定的隐私推理过程，考虑个人历史背景和上下文因素。

Method: PRA通过重建用户的"隐私心智"，使用上下文过滤器动态激活相关隐私记忆（模拟有限理性），并生成反映用户对新隐私场景可能反应的合成评论。同时使用LLM-as-a-Judge评估器，基于已建立的隐私担忧分类法量化生成推理的忠实度。

Result: 在真实世界的Hacker News讨论实验中，PRA在隐私担忧预测方面优于基线智能体，并能捕捉跨领域（包括AI、电子商务和医疗保健）的可迁移推理模式。

Conclusion: PRA能够有效模拟个体用户的隐私担忧形成过程，为理解用户隐私决策提供了新的方法，并在多个领域展示了良好的泛化能力。
...
### [134] [Position on LLM-Assisted Peer Review: Addressing Reviewer Gap through Mentoring and Feedback](https://arxiv.org/abs/2601.09182)
*JungMin Yun,JuneHyoung Kwon,MiHyeon Kim,YoungBin Kim*

Main category: cs.AI

TL;DR: 本文提出将LLM作为辅助和培训人类审稿人的工具，而非自动生成审稿意见，以解决AI研究快速发展导致的审稿人缺口问题，建立更可持续的学术生态系统。
...
Method: 提出以人为中心的范式转变：将LLM定位为辅助和教育人类审稿人的工具。定义了高质量同行评审的核心原则，并基于此提出两个互补系统：1) LLM辅助的导师系统，培养审稿人长期能力；2) LLM辅助的反馈系统，帮助审稿人改进评审质量。

Result: 提出了一种新的LLM应用范式，旨在通过辅助和培训人类审稿人而非取代他们，来增强审稿人专业知识，解决审稿人缺口问题。

Conclusion: 这种以人为中心的方法旨在加强审稿人专业知识，有助于建立更可持续的学术生态系统，为解决同行评审的可持续性挑战提供了新的方向。
...
### [135] [MAXS: Meta-Adaptive Exploration with LLM Agents](https://arxiv.org/abs/2601.09259)
*Jian Zhang,Zhiyuan Wang,Zhangqi Wang,Yu He,Haoran Luo,li yuan,Lingling Zhang,Rui Mao,Qika Lin,Jun Liu*

Main category: cs.AI

TL;DR: MAXS是一个基于LLM Agent的元自适应推理框架，通过前瞻策略和轨迹收敛机制，解决现有方法在局部短视和轨迹不稳定性方面的问题，平衡全局有效性和计算效率。
...
Method: MAXS采用元自适应推理框架，结合前瞻策略扩展推理路径几步，评估工具使用的优势值，并使用步骤一致性方差和步骤间趋势斜率联合选择稳定、一致、高价值的推理步骤。引入轨迹收敛机制，在路径一致性达到时停止进一步扩展，控制计算成本。

Result: 在三个基础模型（MiMo-VL-7B、Qwen2.5-VL-7B、Qwen2.5-VL-32B）和五个数据集上的实验表明，MAXS在性能和推理效率方面均优于现有方法。进一步分析证实了前瞻策略和工具使用的有效性。

Conclusion: MAXS通过前瞻策略和轨迹收敛机制，有效解决了LLM Agent推理中的局部短视和轨迹不稳定性问题，实现了全局有效性和计算效率的良好平衡。
...
### [136] [Efficient Paths and Dense Rewards: Probabilistic Flow Reasoning for Large Language Models](https://arxiv.org/abs/2601.09260)
*Yan Liu,Feng Zhang,Zhanyu Ma,Jun Xu,Jiuchong Gao,Jinghua Hao,Renqing He,Han Liu,Yangdong Deng*

Main category: cs.AI

TL;DR: CoT-Flow：将离散推理步骤重构为连续概率流，量化每个步骤对正确答案的贡献，实现推理效率与性能的平衡
...
Method: 提出CoT-Flow框架，将离散推理步骤重构为连续概率流，量化每个步骤对正确答案的贡献。基于此实现两种方法：流引导解码（贪婪流解码策略提取信息高效推理路径）和基于流的强化学习（构建无需验证器的密集奖励函数）

Result: 在具有挑战性的基准测试中，CoT-Flow在推理效率和推理性能之间实现了优越的平衡

Conclusion: CoT-Flow通过将推理过程建模为连续概率流，解决了当前思维链方法的粒度缺陷，为高效、可解释的推理提供了新范式
...
### [137] [Coordinated Pandemic Control with Large Language Model Agents as Policymaking Assistants](https://arxiv.org/abs/2601.09264)
*Ziyi Shi,Xusen Guo,Hongliang Lu,Mingxing Peng,Haotian Wang,Zheng Zhu,Zhenning Li,Yuxuan Liang,Xinhu Zheng,Hai Yang*

Main category: cs.AI

TL;DR: 提出基于大语言模型的多智能体政策制定框架，用于协调跨区域的主动疫情控制，相比现实政策可显著降低感染和死亡人数。


<details>
  <summary>Details</summary>
Motivation: 现实中疫情控制政策往往是碎片化和反应式的，各区域独立制定政策，缺乏协调和前瞻性，导致疫情控制效果不佳。需要一种能够协调跨区域政策制定的方法。

Method: 为每个行政区分配一个LLM智能体作为AI政策制定助手，智能体基于区域特定的流行病学动态进行推理，同时与其他智能体通信以考虑跨区域相互依赖性。框架整合真实数据、疫情演化模拟器和结构化智能体间通信。

Result: 使用美国2020年4月至12月州级COVID-19数据、真实流动记录和观察到的政策干预进行验证。相比现实疫情结果，在单个州层面累计感染和死亡分别减少高达63.7%和40.1%，跨州聚合后分别减少39.0%和27.0%。

Conclusion: LLM多智能体系统能够通过协调政策制定实现更有效的疫情控制，展示了AI在公共卫生决策中的潜力。
...
### [138] [RISER: Orchestrating Latent Reasoning Skills for Adaptive Activation Steering](https://arxiv.org/abs/2601.09269)
*Wencheng Ye,Liang Peng,Xiaoyang Yuan,Yi Bin,Pengpeng Zeng,Hengyu Jin,Heng Tao Shen*

Main category: cs.AI

TL;DR: RISER是一个基于路由器的自适应激活干预框架，通过强化学习优化路由器来动态组合可重用推理向量，在零样本设置下显著提升LLM推理性能，同时保持高token效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活引导的方法采用静态、手动干预，无法适应复杂推理的动态特性。训练密集型方法需要参数更新，而参数高效的替代方案缺乏适应性。

Method: 提出RISER框架：1)构建可重用推理向量库；2)使用轻量级路由器动态组合这些向量；3)通过强化学习在任务级奖励下优化路由器，以涌现式和组合方式激活潜在认知原语。

Result: 在7个多样化基准测试中，RISER相比基础模型平均提升3.4-6.5%的零样本准确率，超越CoT推理风格，token效率提高2-3倍，同时保持稳健的准确率提升。

Conclusion: RISER能够自主组合多个向量形成可解释的精确控制策略，指向更可控和高效的LLM推理方向，为自适应激活引导提供了有效解决方案。
...
### [139] [$A^3$-Bench: Benchmarking Memory-Driven Scientific Reasoning via Anchor and Attractor Activation](https://arxiv.org/abs/2601.09274)
*Jian Zhang,Yu He,Zhiyuan Wang,Zhangqi Wang,Kai He,Fangzhi Xu,Qika Lin,Jun Liu*

Main category: cs.AI

TL;DR: A³-Bench是一个评估科学推理中记忆驱动机制的新基准，重点关注锚点和吸引子的双重激活，通过AAUI指标衡量记忆激活率，填补现有基准忽略记忆驱动推理的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估最终答案或逐步推理的连贯性，但忽视了人类科学推理中基于记忆驱动的机制——涉及激活锚点和吸引子，然后将它们整合到多步推理中。需要填补这一研究空白。

Method: 1) 使用SAPM过程（主体、锚点&吸引子、问题、记忆发展）标注了2,198个跨领域的科学推理问题；2) 引入基于锚点和吸引子的双重记忆评估框架；3) 提出AAUI（锚点-吸引子利用指数）指标来测量记忆激活率。

Result: 通过在不同基础模型和范式上的实验，验证了A³-Bench的有效性，并分析了记忆激活如何影响推理性能，为理解记忆驱动的科学推理提供了见解。

Conclusion: A³-Bench是一个专门评估科学推理中记忆驱动机制的新基准，通过关注锚点和吸引子的双重激活，为理解和改进AI系统的科学推理能力提供了新的评估工具和洞见。
...
### [140] [STaR: Sensitive Trajectory Regulation for Unlearning in Large Reasoning Models](https://arxiv.org/abs/2601.09281)
*Jingjing Zhou,Gaoxiang Cong,Li Su,Liang Li*

Main category: cs.AI

TL;DR: STaR是一个无需参数的推理时遗忘框架，专门针对大型推理模型中的隐私风险，通过语义检测、安全提示前缀、轨迹感知抑制和自适应过滤，全面保护推理链中的敏感信息。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型能够生成复杂的思维链轨迹，但这也带来了严重的隐私风险，敏感信息可能深度嵌入整个推理过程。现有的LLM遗忘方法通常只修改最终答案，无法从中间步骤中移除敏感内容，导致持续的隐私泄露和安全问题。

Method: STaR框架包含四个步骤：1) 通过语义感知检测识别敏感内容；2) 通过安全提示前缀注入全局安全约束；3) 执行轨迹感知抑制，动态阻止整个推理链中的敏感内容；4) 应用令牌级自适应过滤，防止生成精确和转述的敏感令牌。

Result: 在R-TOFU基准测试中，STaR实现了全面且稳定的遗忘效果，同时保持了最小的效用损失。此外，作者还引入了两个新评估指标：多解码一致性评估和多粒度成员推理攻击评估，以更全面地衡量隐私保护效果。

Conclusion: STaR为大型推理模型中的隐私保护推理设定了新标准，通过无需参数的推理时遗忘框架，有效解决了思维链轨迹中的敏感信息泄露问题，同时保持了模型的推理能力。
...
### [141] [Cluster Workload Allocation: Semantic Soft Affinity Using Natural Language Processing](https://arxiv.org/abs/2601.09282)
*Leszek Sliwko,Jolanta Mizeria-Pietraszko*

Main category: cs.AI

TL;DR: 该论文提出了一种基于自然语言处理的语义化、意图驱动的集群调度范式，通过LLM解析自然语言分配提示，实现简化的集群工作负载配置。


<details>
  <summary>Details</summary>
Motivation: 集群工作负载分配通常需要复杂的配置，存在可用性差距。传统调度配置对用户不友好，需要简化集群系统的使用方式。

Method: 采用语义化、意图驱动的调度范式，集成大型语言模型（LLM）到Kubernetes调度器扩展器中，解析自然语言分配提示注释以实现软亲和性偏好。开发了包含集群状态缓存和意图分析器（使用AWS Bedrock）的原型系统。

Result: LLM解析准确率高（>95%子集准确率），Amazon Nova Pro/Premier和Mistral Pixtral Large等顶级模型显著优于基线引擎。在六个场景的调度质量测试中，原型系统在复杂和定量场景以及处理冲突软偏好方面表现优异，达到或优于标准Kubernetes配置。
...
### [142] [Long-term Task-oriented Agent: Proactive Long-term Intent Maintenance in Dynamic Environments](https://arxiv.org/abs/2601.09382)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型智能体主要采用被动响应模式，只能处理短期会话中的即时用户查询，无法维持长期用户意图并适应动态变化的外部环境，这限制了智能体在复杂任务中的实际应用能力。

Method: 提出主动式任务导向智能体交互范式，包含两个核心能力：1) 意图条件监控：智能体基于对话历史自主制定触发条件；2) 事件触发跟进：检测到有用环境更新时主动与用户互动。开发高质量数据合成管道构建动态环境中的复杂多轮对话数据，并创建ChronosBench基准测试评估动态环境中的任务导向交互。

Result: 使用合成数据进行监督学习微调的模型在包含用户意图变化的复杂任务中实现了85.19%的任务完成率，优于其他测试模型。评估了当前领先的闭源和开源模型，揭示了它们在长期任务导向交互中的缺陷。

Conclusion: 提出的主动式智能体交互范式和数据驱动策略有效解决了静态用户需求与动态环境之间的鸿沟，验证了意图条件监控和事件触发跟进能力在长期任务导向交互中的重要性，为智能体在动态环境中的应用提供了新思路。
...
### [143] [What Do LLM Agents Know About Their World? Task2Quiz: A Paradigm for Studying Environment Understanding](https://arxiv.org/abs/2601.09503)
*Siyuan Liu,Hongbang Yuan,Xinze Li,Ziyue Zhu,Yixin Cao,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: 提出T2Q评估范式，通过将任务执行与环境理解解耦，发现任务成功不能准确反映环境理解能力，现有记忆机制无法有效帮助智能体获取环境模型


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估主要依赖基于轨迹的任务成功率指标，但无法评估智能体是否真正掌握了可迁移的环境模型，存在评估盲区

Method: 提出Task-to-Quiz (T2Q)评估范式，创建T2QBench基准套件，包含30个环境和1,967个基础问答对，通过确定性自动化评估将任务执行与环境理解解耦

Result: 实验发现任务成功率与环境理解能力相关性差，当前记忆机制无法有效帮助智能体获取环境模型，主动探索和细粒度状态表示是主要瓶颈

Conclusion: T2Q为评估LLM智能体环境理解能力提供了新范式，揭示了当前智能体在环境建模方面的局限性，为开发更具泛化能力的自主智能体奠定了基础
...
### [144] [Omni-R1: Towards the Unified Generative Paradigm for Multimodal Reasoning](https://arxiv.org/abs/2601.09536)
*Dongjie Cheng,Yongqi Li,Zhixin Ma,Hongru Cai,Yupeng Hu,Wenjie Wang,Liqiang Nie,Wenjie Li*

Main category: cs.AI

TL;DR: Omni-R1提出统一的生成式多模态推理框架，通过生成中间图像统一多种多模态推理技能，包括Omni-R1（有监督）和Omni-R1-Zero（无监督）两个版本。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs多采用单一任务特定的推理模式，限制了在不同多模态任务间的泛化能力。实际多模态任务需要多样化的推理技能（如放大特定区域、标记图像中的对象等），需要统一的推理框架。

Method: 提出统一的生成式多模态推理范式，通过生成中间图像统一推理过程。具体实现包括：1) Omni-R1：两阶段SFT+RL框架，包含感知对齐损失和感知奖励；2) Omni-R1-Zero：从纯文本推理数据中引导生成逐步可视化，无需多模态标注。

Result: 实验结果表明，Omni-R1能在广泛的多模态任务上实现统一的生成式推理，而Omni-R1-Zero在平均性能上能匹配甚至超越Omni-R1。

Conclusion: 统一的生成式多模态推理是可行的方向，通过生成中间图像可以统一多种推理技能，且无监督方法也能达到良好效果，为多模态推理提供了新思路。
...
### [145] [Collaborative Multi-Agent Test-Time Reinforcement Learning for Reasoning](https://arxiv.org/abs/2601.09667)
...
TL;DR: 提出MATTRL框架，在多智能体推理时注入结构化文本经验，通过多专家团队讨论、经验检索与整合达成共识决策，无需调参即可提升分布偏移鲁棒性。
...
Method: MATTRL框架在推理时向多智能体决策注入结构化文本经验，形成多专家团队进行多轮讨论，检索并整合测试时经验，通过共识达成最终决策。研究信用分配机制构建回合级经验池并重新注入对话。
