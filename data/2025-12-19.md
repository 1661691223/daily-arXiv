...
Motivation: 线性基础设施监测需要可靠的高分辨率数据和定期采集，但光学影像受云层影响，SAR数据难以标注。需要解决SAR变化检测的标注难题并评估基础模型在该任务上的表现。

Method: 构建SkyCap双时相VHR光学-SAR数据集，通过档案匹配和配准，利用光学到SAR的标签转移获得SAR变化检测标签。在SAR数据上继续预训练SARATR-X，并评估SAR专用基础模型与光学基础模型在不同预处理选择下的性能。

Result: 光学基础模型MTP(ViT-B+RVSA)在dB+Z-score预处理下获得最佳结果(F1_c = 45.06)，优于直接在Capella数据上进一步预训练的SAR专用基础模型。预处理与预训练统计的对齐对性能有强烈影响，光学模型在光学变化检测上的排名不能直接迁移到SAR变化检测。

Conclusion: 这是首次在VHR SAR变化检测上评估基础模型的研究，展示了光学模型在适当预处理下可以优于SAR专用模型，强调了预处理对齐的重要性，为SAR变化检测提供了新的基准。
...
TL;DR: 本文提出SocialNav-MoE，一种基于混合专家架构的高效视觉语言模型，用于机器人在人群环境中的社交合规导航，通过强化学习微调和语义相似度奖励来平衡导航准确性与计算效率。


<details>
  <summary>Details</summary>
Motivation: 机器人在人类密集环境中的导航需要同时考虑安全性和社交合规性，但现有研究主要关注安全性，社交合规导航（考虑人类舒适度、社会规范和情境适宜性）仍未被充分探索。大型视觉语言模型虽然具有潜力，但计算开销大、推理延迟高、能耗大，不适合资源受限的机器人平台实时部署。

Method: 提出SocialNav-MoE，一种基于混合专家架构的高效视觉语言模型，采用强化学习微调（RFT）和语义相似度奖励（SSR）来增强决策能力。研究了不同小型语言模型类型（Phi、Qwen、StableLM）、路由策略和视觉编码器（CLIP vs. SigLIP，冻结 vs. 微调）的效果。

Result: 在SNEI数据集上的实验表明，SocialNav-MoE在导航准确性和效率之间取得了良好平衡。提出的SSR函数比硬级别和字符级别奖励更有效。

Conclusion: SocialNav-MoE为资源受限的机器人平台提供了一种高效的社交合规导航解决方案，通过混合专家架构和强化学习微调实现了准确性与效率的平衡，语义相似度奖励有效提升了决策能力。
...
TL;DR: 提出一个模块化专家系统管道，用于将带歌词的印刷简谱转换为机器可读的MusicXML和MIDI格式，填补了大规模光学音乐识别在中文简谱领域的空白。


<details>
  <summary>Details</summary>
Motivation: 大规模光学音乐识别研究主要集中在西方五线谱上，而中文简谱及其丰富的歌词资源尚未得到充分探索。简谱在中文音乐中广泛使用，但缺乏有效的数字化工具。
...
Conclusion: 该研究填补了中文简谱光学音乐识别的空白，通过混合专家系统方法实现了高精度的简谱数字化，无需大量标注训练数据，为中文音乐资源的数字化保存和分析提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉问答中表现出巨大潜力，但容易产生幻觉，导致过度自信的错误答案，严重损害回答可靠性。需要一种方法来评估和增强VLM回答的可信度。

Method: 提出DAVR框架，采用双路径架构：一条路径通过双选择器模块融合VLM潜在特征和QA嵌入来评估回答可靠性；另一条路径部署外部参考模型进行事实交叉检查以减少幻觉。

Result: 在ICCV-CLVL 2025的Reliable VQA挑战赛中，DAVR取得了领先的Φ₁₀₀分数39.64和100-AUC分数97.22，获得第一名，证明了其在增强VLM回答可信度方面的有效性。

Conclusion: DAVR框架通过整合自我反思和跨模型验证的双重评估机制，能够有效估计不确定性并减少幻觉，显著提升了视觉语言模型在视觉问答中的可靠性。
...
TL;DR: HERBench是一个专门设计的视频问答基准测试，要求模型整合至少三个不同时间段的视觉证据，以评估多证据推理能力，现有视频大语言模型在该基准上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前视频问答基准测试往往允许通过单一显著线索回答问题，未能充分测试需要整合多个时间分离视觉证据的推理能力，因此需要构建一个专门评估多证据整合能力的基准。

Method: 创建HERBench基准，包含26K个五选一选择题，组织成12个组合任务；引入最小必需帧集（MRFS）指标来量化证据需求；评估13个最先进的视频大语言模型。

Result: HERBench的平均MRFS为5.5，显著高于先前数据集（2.6-4.2）；现有模型准确率仅为31-42%，略高于20%的随机猜测基线；失败主要源于检索缺陷（忽略关键证据）和融合缺陷（无法整合信息）。
...
TL;DR: 本文提出了一种结合皮肤纹理文本描述符的跨模态动物重识别方法，通过文本-图像协同合成生成虚拟个体，显著提升了AI在跨模态检索中的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统AI动物重识别主要依赖图像特征，存在视觉单一模态的局限性。生物学中通常结合视觉和文本描述来识别动物，但现有AI工具缺乏有效的文本描述整合能力。本文旨在将法医学中使用的皮肤纹理文本描述方法引入生态学，解决纯视觉方法的限制。

Method: 1. 引入皮肤纹理文本描述符作为人类可解释的语言标签，抽象和编码动物皮毛拓扑结构；2. 使用84,264个手动标记的细节特征，覆盖3,355张185只老虎的图像；3. 开发文本-图像协同合成管道，生成包含数十个逼真视觉图像和皮肤纹理文本的"虚拟个体"；4. 在真实场景中评估这种视觉-文本方法的跨模态身份检索能力。

Result: 该方法在跨模态检索中展现出新的能力，通过数据增强显著提升了AI准确性，同时缓解了数据稀缺问题。皮肤纹理语言引导的生物识别技术能够克服纯视觉方法的限制，实现基于人类可验证匹配的文本到视觉身份恢复。

Conclusion: 皮肤纹理语言引导的生物识别代表了动物重识别可解释性的重要进展，实现了生态监测中描述模态的语言驱动统一。这种方法为跨模态身份检索提供了新的解决方案，具有重要的生态监测应用价值。
...
Method: 提出Vibe Space层次图流形，学习CLIP等特征空间中的低维测地线；设计结合人类判断、LLM推理和几何路径难度分数的认知启发评估框架。

Result: Vibe Space生成的混合在人类评估中被一致评为比现有方法更具创造性和连贯性。

Conclusion: Vibe Space通过层次图流形有效解决了视觉概念混合中的非线性路径问题，能够生成更富创造性和语义一致的概念混合。
...
<details>
  <summary>Details</summary>
Motivation: 当前人工智能基础模型在前列腺癌Gleason分级中可能通过学习标本特异性伪影而非可泛化的生物学特征来获得高验证准确率，这限制了其在真实世界临床环境中的实用性。

Method: 研究者构建了PANDA-PLUS-Bench基准数据集，包含9位不同患者的9张全切片图像，涵盖多种Gleason模式，并在512x512和224x224像素分辨率下提取非重叠组织块，应用八种增强条件。使用该基准评估了7个基础模型分离生物学信号与切片水平混杂因素的能力。

Result: 模型间鲁棒性存在显著差异：Virchow2在大型模型中切片水平编码最低（81.0%），但跨切片准确率第二低（47.2%）。专门在前列腺组织上训练的HistoEncoder表现出最高的跨切片准确率（59.7%）和最强的切片水平编码（90.3%）。所有模型都显示出可测量的切片内与跨切片准确率差距，范围从19.9到26.9个百分点。

Conclusion: PANDA-PLUS-Bench填补了基础模型评估的关键空白，为Gleason分级这一重要临床场景中的鲁棒性评估提供了专门构建的资源。组织特异性训练可能同时增强生物学特征捕获和切片特异性特征识别能力。
...
TL;DR: 该研究提出自适应后处理技术来改进大型预训练模型生成的胶质瘤分割质量，在BraTS 2025挑战赛中显著提升分割性能，同时强调计算公平性和可持续性。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤是成人最常见的恶性脑肿瘤，生存率低。虽然深度学习模型改进了自动分割精度，但大型预训练模型泛化能力差，存在假阳性、标签交换、切片不连续等系统错误，且受限于GPU资源不平等和训练的环境成本。

Method: 提出自适应后处理技术来优化大型预训练模型生成的胶质瘤分割结果，针对不同肿瘤类型开发的后处理策略。

Result: 在BraTS 2025分割挑战赛的多个任务中验证，撒哈拉以南非洲挑战赛的排名指标提升14.9%，成人胶质瘤挑战赛提升0.9%。
...
TL;DR: TalkVerse是一个大规模开放语料库，用于单人音频驱动视频生成，包含230万高清音频视频同步片段，总计6300小时。基于该数据集构建了50亿参数DiT基线模型，实现分钟级生成，推理成本降低10倍，支持零样本视频配音。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的音频驱动视频生成系统依赖封闭数据或计算密集型模型，缺乏公平可复现的比较基准。TalkVerse旨在提供一个开放、大规模、高质量的数据集，降低研究门槛，促进该领域的公平比较和可复现性。

Method: 1) 构建TalkVerse数据集：从6万小时视频中通过场景切割检测、美学评估、严格音视频同步检查等透明流程筛选出230万高清片段；2) 基于Wan2.2-5B构建50亿参数DiT基线模型，采用高下采样率视频VAE和带运动帧上下文的滑动窗口机制；3) 集成MLLM导演根据音视频线索重写提示词；4) 通过受控潜在噪声注入实现零样本视频配音。

Result: 1) 发布包含230万片段、6300小时的TalkVerse数据集；2) 模型实现分钟级生成且漂移低，唇形同步和视觉质量与140亿参数Wan-S2V模型相当，但推理成本降低10倍；3) MLLM导演增强长视频叙事能力；4) 支持零样本视频配音；5) 开源数据集、训练方案和50亿参数检查点。

Conclusion: TalkVerse为音频驱动的人类视频生成研究提供了首个大规模开放基准，显著降低了研究门槛。提出的50亿参数DiT基线模型在保持高质量的同时大幅降低计算成本，支持长视频生成和零样本配音，推动了该领域的公平比较和可复现研究。
...
TL;DR: 提出了一种三模态（语音、面部、手势）人员识别框架，通过多任务学习、跨注意力机制和门控融合处理各模态，采用置信度加权融合策略适应模态缺失，在CANDOR数据集上达到99.18%的Top-1准确率，在VoxCeleb1数据集上达到99.92%准确率，即使缺少一两个模态也能保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人员识别系统常常面临模态缺失或质量下降的问题，传统的单模态或双模态方法在部分模态不可用时性能会显著下降。需要开发一种能够整合多种模态信息，同时又能鲁棒地处理模态缺失情况的人员识别框架。

Method: 提出三模态人员识别框架，整合语音、面部和手势三种模态。采用多任务学习独立处理每个模态，然后使用跨注意力机制促进模态间交互，结合门控融合机制整合信息。引入置信度加权融合策略，动态适应缺失和低质量数据，确保在单模态或双模态场景下的最优分类性能。

Result: 在CANDOR数据集上，三模态系统达到99.18%的Top-1准确率，优于传统单模态和后期融合方法。在VoxCeleb1数据集上，双模态模式下达到99.92%准确率。系统在缺失一两个模态时仍能保持高精度，展示了良好的鲁棒性。

Conclusion: 提出的三模态人员识别框架通过多模态融合和鲁棒性设计，能够有效处理现实世界中的模态缺失问题，为实际应用提供了可靠的解决方案。代码和数据已公开，促进了该领域的研究发展。
...
TL;DR: 该研究提出了一种评估视频问题生成模型质量的新协议，通过模拟与专家的问答交流来评估问题能否有效引出专家未见过知识的能力，并构建了EgoExoAsk数据集来支持这一评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频问题生成模型的评估主要关注问题能否被回答，而忽视了问题本身的质量，特别是问题能否有效引出专家未见过知识的能力。需要一种定量评估问题生成模型质量的方法。

Method: 提出了一种新的评估协议，通过问题到答案检索来模拟与专家的问答交流。构建了EgoExoAsk数据集（包含27,666个QA对），用于训练检索器并在验证集上构建基准测试。

Result: 实验结果表明，该评估指标与问题生成设置合理对齐：能够访问更丰富上下文的模型获得更好的评估分数，支持该协议按预期工作。

Conclusion: 该研究为视频问题生成模型的质量评估提供了新方法，重点关注问题引出专家知识的能力，EgoExoAsk数据集和评估协议为持续改进VQG模型提供了基础。
...
TL;DR: 提出MAPO框架，通过Dropout驱动的随机分割假设构建偏好一致梯度，无需直接真值监督，适用于2D/3D CNN和Transformer分割模型


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法在医学图像分割中仍局限于特定模型，且依赖低多样性预测采样，需要更通用的训练框架

Method: MAPO框架利用Dropout驱动的随机分割假设构建偏好一致梯度，无需直接真值监督，支持2D/3D CNN和Transformer架构

Result: 在多个医学数据集上评估显示，MAPO能持续提升边界贴合度、减少过拟合，并提供比传统监督训练更稳定的优化动态

Conclusion: MAPO提供了一个模型无关的偏好优化框架，能有效提升医学图像分割性能，具有更好的泛化能力和优化稳定性
...
Method: 基于事件相机和LED标记，通过分析LED闪烁产生的事件流特性，利用时空相关性过滤观测噪声，区分运动引起的事件和LED闪烁事件，从而提取高速移动的LED标记，最终通过单目事件相机测量高频平面变形。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全需要全面准确的环境感知，虽然已有多种性能评估指标，但缺乏专门针对安全的关键性指标来区分相关与非相关对象，这对可靠评估目标检测系统至关重要。

Method: 通过全面文献综述识别和评估现有关键性指标，在DeepAccident数据集上进行实证验证，并提出双向关键性评级和多指标聚合两种新应用策略来提升评估准确性。

Result: 提出的方法在关键性分类准确率方面实现了高达100%的改进，显著提升了目标检测系统在自动驾驶车辆中的安全评估能力。

Conclusion: 该研究首次深入分析了目标检测系统安全评估的关键性指标，提出的新策略能显著提升评估准确性，为自动驾驶车辆的目标检测系统安全评估提供了重要进展。
...
TL;DR: PMMD是一个基于扩散模型的框架，通过多视角参考图像、姿态图和文本提示生成逼真的人体图像，解决了现有方法在遮挡、服装风格漂移和姿态对齐方面的不足。
...
Method: 提出了Pose-guided Multi-view Multimodal Diffusion (PMMD)框架，包含多模态编码器联合建模视觉视角、姿态特征和语义描述，设计了ResCVA模块增强局部细节同时保持全局结构，以及跨模态融合模块在去噪过程中整合图像语义和文本信息。

Result: 在DeepFashion MultiModal数据集上的实验表明，PMMD在一致性、细节保持和可控性方面优于代表性基线方法。

Conclusion: PMMD框架通过多视角多模态条件生成，有效解决了人体图像生成中的遮挡、风格漂移和姿态对齐问题，为相关应用提供了高质量的解决方案。
...
TL;DR: 论文提出首个端到端框架来探测、量化和缓解单目深度基础模型的3D幻象问题，即模型从几何平面但感知模糊的输入中产生虚假3D结构的问题。


<details>
  <summary>Details</summary>
Motivation: 单目深度基础模型通过学习大规模语义先验实现了显著泛化能力，但这带来了关键漏洞：它们会从几何平面但感知模糊的输入中产生虚假的3D结构（称为3D幻象）。这种未量化的安全风险需要系统性的探测、量化和缓解方法。

Method: 1. 探测：提出3D-Mirage基准，包含真实世界幻象（如街头艺术）的精确平面区域标注和上下文受限裁剪；2. 量化：提出基于拉普拉斯算子的评估框架，包含偏差复合分数（DCS）和混淆复合分数（CCS）；3. 缓解：提出基于自蒸馏的平面化方法，在幻象区域强制平面性，同时使用冻结教师模型保留背景知识。

Result: 开发了首个针对3D幻象问题的端到端框架，提供了探测、量化和缓解的工具。提出的基准和评估方法能够系统性地诊断这一现象，而基于自蒸馏的方法能够有效缓解幻象问题，同时避免灾难性遗忘。

Conclusion: 这项工作为诊断和缓解3D幻象现象提供了必要工具，并呼吁将MDE评估从像素级精度转向结构和上下文鲁棒性。代码和基准将公开以促进这一研究方向的发展。
...
TL;DR: Uni-Parser是一个工业级文档解析引擎，专为科学文献和专利设计，采用模块化多专家架构，支持跨模态对齐，实现高达每秒20页PDF的处理速度，适用于大规模云部署。


<details>
  <summary>Details</summary>
Motivation: 传统管道式文档解析方法在处理科学文献和专利时存在局限性，无法有效保留文本、公式、表格、图表和化学结构之间的细粒度跨模态对齐关系，且扩展性不足，难以适应新兴模态的需求。

Method: 采用模块化、松散耦合的多专家架构，结合自适应GPU负载均衡、分布式推理、动态模块编排和可配置模式，支持整体解析或特定模态解析，保持跨模态对齐的同时易于扩展。

Result: 在8个NVIDIA RTX 4090D GPU上实现每秒20页PDF的处理速度，具备高吞吐量、强健准确性和成本效益，能够高效处理数十亿页文档。
...
TL;DR: 研究评估了Nano Banana Pro在低层视觉任务中的零样本性能，发现其在主观视觉质量上表现出色，但在传统定量指标上落后于专业模型。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到图像生成模型发展迅速，但商业产品如Nano Banana Pro作为低层视觉通用解决方案的潜力尚未充分探索。本研究旨在评估Nano Banana Pro是否能够成为低层视觉的全能选手。

Method: 通过零样本评估方法，在14个不同的低层视觉任务和40个多样化数据集上进行测试。使用简单的文本提示而不进行微调，将Nano Banana Pro与最先进的专用模型进行基准比较。

Result: 研究发现了明显的性能二分现象：Nano Banana Pro在主观视觉质量上表现优异，经常能产生超越专用模型的合理高频细节，但在传统的基于参考的定量指标上落后。这种差异归因于生成模型的固有随机性，难以保持传统指标所需的严格像素级一致性。

Conclusion: Nano Banana Pro被确定为低层视觉任务中一个有能力的零样本竞争者，但要达到领域专家的高保真度仍然是一个重大挑战。研究强调了生成模型在低层视觉应用中的潜力和局限性。
...
TL;DR: 提出人类动作形态评估新任务，构建包含健身和武术视频的多层次标注数据集CoT-AFA，采用思维链解释范式，并开发可解释的评估框架实现动作判断、原因解释和改进方案生成。


<details>
  <summary>Details</summary>
Motivation: 当前视频理解方法主要关注动作识别（是什么和在哪里），无法满足评估动作标准化程度的需求。现有数据集缺乏动作标准化程度的标注，且动作质量评估数据集缺乏可解释性和详细反馈。

Method: 定义人类动作形态评估新任务，构建CoT-AFA数据集（包含健身和武术视频的多层次标注），采用思维链解释范式提供完整推理过程。提出可解释的健身评估框架，使用并行处理流和动态门控机制融合视觉和语义信息。

Result: 实验结果显示，该方法在解释生成（CIDEr指标提升16.0%）、动作分类（准确率提升2.7%）和质量评估（准确率提升2.1%）方面均有显著改进，展现了CoT-AFA数据集的潜力。

Conclusion: 成功定义了人类动作形态评估新任务，构建了包含思维链解释的多样化数据集，并开发了可解释的评估框架，为动作标准化评估提供了新的解决方案和研究基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有空间智能方法存在空间一致性弱、视角多样性有限、证据链无法追溯到支持视图的问题。虽然"图像思维"框架展示了逐步多模态推理的潜力，但未能解决空间思维链的三个关键挑战：严格token预算下的全局空间感知构建、3D假设与视频帧的显式关联验证，以及空间基础奖励的设计。

Method: 提出双阶段框架：1) 宏观感知阶段使用语义-视角融合确定性点过程(SPF-DPP)从长视频中选择紧凑的几何和语义感知关键帧；2) 微观验证阶段将空间思维链形式化为BEV基础姿态查询：智能体迭代预测BEV平面上的姿态，检索最近的真实帧，并通过空间基础奖励的强化学习进行训练。

Result: 在VSI-Bench上，EagleVision在开源视觉语言模型中达到了最先进的性能，展示了强大且可泛化的空间理解能力。

Conclusion: EagleVision通过宏观感知和微观验证的双阶段框架有效解决了空间思维链的关键挑战，实现了渐进式的空间认知，为空间智能系统提供了新的解决方案。
...
TL;DR: 提出一种重合成框架来检测深度伪造内容，通过校准重合成方法实现高精度、低召回率检测，对抗计算受限的对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测方法不可靠：1) 事后区分不真实内容往往不可能，导致无限高的假阳性率；2) 检测缺乏鲁棒性，攻击者可用最小计算资源适应已知检测器。

Method: 提出重合成框架来确定样本是否真实或其真实性是否可被合理否认。采用校准重合成方法，专注于高精度、低召回率设置，对抗计算受限的对抗攻击。

Result: 校准重合成方法是最可靠的验证真实样本方法，同时保持可控的低假阳性率；该方法实现对抗计算受限攻击者的鲁棒性，而先前方法在相同计算预算下容易被规避。

Conclusion: 重合成框架为解决深度伪造检测的可靠性问题提供有效方案，支持多模态并利用最先进的反演技术，在对抗计算受限攻击者时表现出优越的鲁棒性。
...
TL;DR: 本文针对低空无人机侦察中的红外与可见光图像融合，提出了目标-背景对比度(TBC)指标，解决了传统无参考指标在复杂低光环境下将传感器噪声误判为有效细节的"噪声陷阱"问题。


<details>
  <summary>Details</summary>
Motivation: 传统无参考指标(如熵EN和平均梯度AG)在复杂低光环境中容易将高频传感器噪声误判为有效细节，形成"噪声陷阱"，导致对噪声图像评分更高，误导融合算法发展。

Method: 提出目标-背景对比度(TBC)指标，受韦伯定律启发，关注显著目标的相对对比度而非全局统计特征。TBC能够惩罚背景噪声并奖励目标可见性。
...
<details>
  <summary>Details</summary>
Motivation: 从野外图像重建世界坐标系下的准确3D人体网格具有挑战性，因为缺乏相机旋转信息。现有方法假设相机旋转为零，在相机坐标系中取得不错效果，但转换到世界坐标系时会产生显著误差。
...
Conclusion: Mesh-Plug通过以人为中心的方法，利用RGB图像和深度图估计相机旋转参数，成功解决了从相机坐标系到世界坐标系的准确转换问题，无需依赖环境线索。
...
TL;DR: Null-LoRA：一种基于零空间的低秩适配方法，通过在零空间内约束增量更新，以更少参数实现优于现有方法的性能


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）在全参数空间进行低秩适配，但研究表明在子空间内微调也能达到类似效果。预训练模型存在非平凡零空间，这为更高效的参数利用提供了可能。

Method: 提出Null-LoRA方法：1）通过冻结部分低秩矩阵减少冗余并提升有效秩；2）将整个增量更新约束在零空间内，最大化增量更新对新任务范式的适应能力。

Result: 在图像-文本检索和视觉问答任务的大规模实验中，Null-LoRA以更少的参数超越了现有最优方法。

Conclusion: Null-LoRA通过利用预训练模型的零空间特性，实现了更高效的参数利用，在保持性能的同时显著减少了微调参数数量。
...
TL;DR: 提出Cross-Modal Alignment Consistency (CMAC-MMD)训练框架，通过标准化跨模态对齐一致性来减少医学AI系统在边缘化患者亚组中的诊断偏见，无需在临床推理中使用敏感人口统计数据。


<details>
  <summary>Details</summary>
Motivation: 医学AI系统（特别是多模态视觉语言模型）存在交叉偏见，对边缘化患者亚组的诊断信心较低，导致误诊和漏诊率更高。现有公平性干预方法要么无法解决这些差距，要么以牺牲整体诊断性能为代价来实现统计平等。

Method: 开发了Cross-Modal Alignment Consistency (CMAC-MMD)训练框架，该框架标准化跨交叉患者亚组的诊断确定性。与传统去偏见方法不同，该方法在临床推理期间不需要敏感人口统计数据，通过跨模态对齐一致性来均衡模型的决策信心。

Result: 在皮肤病变检测（HAM10000和BCN20000数据集）中，该方法将交叉漏诊差距（ΔTPR）从0.50降低到0.26，同时将整体AUC从0.94提高到0.97。在青光眼筛查（Harvard-FairVLMed数据集）中，将ΔTPR从0.41降低到0.31，AUC从0.71提高到0.72。
...
TL;DR: 该研究系统比较了专业计数架构与多模态视觉语言模型在物体计数任务上的表现，发现VLMs能达到或超越专业架构的性能，特别是当提示生成中间表示时效果更好，但在复杂场景中仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 视觉场景中的物体计数是计算机视觉的基本但具有挑战性的任务。传统方法依赖特定领域的计数架构，而最近大规模多模态视觉语言模型的发展为开放集物体计数提供了灵活替代方案。研究旨在系统比较专业计数架构与VLMs的性能差异。

Method: 在两个流行的计数数据集上系统比较最先进的专用计数架构与多模态视觉语言模型的性能，同时创建了一个新的基准测试集，该数据集能更精细地控制测试图像的视觉属性。

Result: 大多数VLMs能够近似枚举视觉场景中的物品数量，匹配甚至超越了专用计算机视觉架构的性能。当提示VLMs生成每个待计数物体的中间表示（即位置和文本标签）时，枚举准确性显著提高。然而，所有模型都无法可靠地计数复杂视觉场景中的物体数量。

Conclusion: 虽然VLMs在物体计数任务上表现出色，特别是在生成中间表示时效果更好，但在复杂现实环境中可靠部署计数程序仍需要进一步研究，表明当前AI系统在这一领域仍有改进空间。
...
TL;DR: 提出MMMamba框架，基于Mamba架构实现跨模态上下文融合，用于全色锐化任务，支持零样本图像超分辨率，具有线性计算复杂度和优越性能。
...
Method: 基于Mamba架构设计MMMamba框架，采用跨模态上下文融合机制，引入新颖的多模态交错扫描机制促进PAN和MS模态间的有效信息交换，保持线性计算复杂度。

Result: 在多个任务和基准测试中，该方法相比现有最先进技术表现出优越性能。

Conclusion: MMMamba框架通过跨模态上下文融合和交错扫描机制，在全色锐化任务中实现了高效且有效的多模态信息融合，同时支持零样本图像超分辨率。
...
TL;DR: 提出ZSWSSS新方向，使用LLM驱动的多智能体框架SynthSeg Agents，无需真实图像即可生成合成训练数据，在PASCAL VOC和COCO上取得有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的弱监督语义分割方法虽然利用生成模型增强数据，但仍依赖真实世界训练样本。本文旨在探索完全无需真实图像的新方向，实现零样本弱监督语义分割，降低标注成本并提高可扩展性。

Method: 提出SynthSeg Agents多智能体框架，包含两个核心模块：1) 自优化提示智能体，通过迭代优化、记忆机制和提示空间探索生成多样化的语义丰富提示；2) 图像生成智能体，利用视觉语言模型生成候选图像，通过CLIP评分模型筛选高质量样本，并使用ViT分类器重新标注合成数据集以提高语义精度。

Result: 在PASCAL VOC 2012和COCO 2014数据集上的实验表明，SynthSeg Agents无需使用真实训练图像即可达到有竞争力的性能，证明了LLM驱动智能体在实现成本高效、可扩展语义分割方面的潜力。

Conclusion: 本文提出的ZSWSSS新方向和SynthSeg Agents框架展示了完全无需真实图像生成高质量训练数据的可行性，为降低语义分割标注成本和提高可扩展性提供了有前景的解决方案。
...
TL;DR: AutoMAC-MRI是一个可解释的MRI运动伪影分级框架，使用监督对比学习来学习运动严重程度的判别性表示，并通过计算特定等级亲和度分数实现透明、可解释的分级。
...
Method: 采用监督对比学习学习运动严重程度的判别性表示，在该特征空间中计算特定等级亲和度分数，量化图像与每个运动等级的接近程度。

Result: 在超过5000个专家标注的脑MRI切片上进行评估，实验显示亲和度分数与专家判断高度一致，支持其作为运动严重程度的可解释度量。

Conclusion: AutoMAC-MRI通过将准确等级检测与每等级亲和度评分相结合，实现了内联MRI质量控制，有望减少不必要的重新扫描并提高工作流程效率。
...
TL;DR: MECAD是一种基于多专家架构的持续异常检测方法，通过动态分配专家、优化核心集选择和专用重放缓冲区，实现无需完整模型重训练的增量学习。
...
Result: 在MVTec AD数据集上的实验评估显示，最优5专家配置在15个不同对象类别上平均AUROC达到0.8259，相比单专家方法显著减少了知识退化。
...
TL;DR: 本文提出了一种名为掩码反向知识蒸馏（MRKD）的新方法，通过图像级掩码和特征级掩码技术，将图像重建任务转化为图像修复，有效解决了传统知识蒸馏在异常检测中过度泛化的问题。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏在图像异常检测中存在过度泛化的问题，主要原因是输入信号和监督信号过于相似。为了解决这一问题，需要开发能够更好区分正常与异常特征的方法。

Method: 提出掩码反向知识蒸馏（MRKD）方法，采用两种掩码策略：1）图像级掩码（ILM）用于捕获全局信息，区分输入与监督信号；2）特征级掩码（FLM）引入合成特征级异常，确保学习到的表示包含足够的局部信息。

Result: 在MVTec异常检测数据集上，MRKD取得了优异性能：图像级AU-ROC 98.9%，像素级AU-ROC 98.4%，AU-PRO 95.3%。消融实验验证了MRKD在缓解过度泛化问题方面的优越性。

Conclusion: MRKD通过图像级和特征级掩码策略，增强了图像上下文捕获能力，有效缓解了知识蒸馏在异常检测中的过度泛化问题，在多个指标上表现出色。
...
TL;DR: 该论文提出了一种受人类启发的计算机视觉方法，用于从线性刻度（如注射器和量筒）读取测量值，通过图像校正、特征提取和读数计算实现自动化测量读取。
...
Method: 采用人类启发的方法读取线性刻度测量值：1) 对随机方向的注射器进行图像变换校正方向；2) 将感兴趣区域缩小到仅包含线性刻度的部分；3) 提取主要刻度标记、对应数字和液位指示器位置等特征；4) 基于这些特征计算最终读数。

Result: 系统读取的测量值与人工读取的相同实例值进行比较，观察到准确对应，验证了该方法的有效性。

Conclusion: 该研究成功开发了一种能够从线性刻度准确读取测量值的计算机视觉系统，为实验室环境中的机器人自主操作提供了重要的测量读取能力。
...
TL;DR: TIMAR是一个用于3D对话头部生成的因果框架，通过交错掩码自回归建模对话中的双向动态，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 人类对话涉及语音和非语言线索（如点头、眼神、表情）的连续双向交换，现有框架通常将说话和倾听作为独立过程处理或依赖非因果的全序列建模，这阻碍了跨轮次的时间连贯性。

Method: TIMAR（Turn-level Interleaved Masked AutoRegression）是一个因果框架，将对话建模为交错的多模态上下文，在每个轮次内融合多模态信息，应用轮次级因果注意力积累对话历史，同时使用轻量级扩散头预测连续的3D头部动态。

Result: 在DualTalk基准测试中，TIMAR在测试集上将Fréchet距离和MSE降低了15-30%，在分布外数据上也取得了类似的提升。

Conclusion: TIMAR能够有效建模对话中的双向动态，生成具有协调性和表达变异性的3D头部动作，为构建表达性虚拟化身和交互机器人提供了重要技术基础。
...
TL;DR: 该研究首次建立了视觉文本压缩（VTC）基准，系统评估了视觉语言模型在压缩文本下的长上下文理解能力，发现大多数模型在此场景下表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文本压缩技术（如DeepSeek-OCR和Glyph）虽然能实现3-20倍的token压缩比，但其高信息密度对视觉语言模型核心长上下文能力的影响尚未得到充分研究。

Method: 研究者创建了首个VTC基准，包含三个评估维度：VTC-Retrieval（信息检索与聚合）、VTC-Reasoning（基于最小词汇重叠的潜在关联推理）、VTC-Memory（长对话记忆中的综合问答），并建立了VTCBench-Wild模拟多样化输入场景。

Result: 评估显示，尽管大多数视觉语言模型能较好地解码文本信息（如OCR），但在VTC压缩信息下的长上下文理解能力出人意料地差，无法捕捉上下文中的长关联或依赖关系。

Conclusion: 该研究为VTC提供了深入理解，并为设计更高效、可扩展的视觉语言模型奠定了基础，揭示了当前模型在压缩文本长上下文理解方面的局限性。
...
TL;DR: GRPO技术面临大组规模与计算成本冲突，研究发现奖励聚类现象，提出静态OVF策略，最终开发动态Pro-GRPO框架，通过早期终止奖励聚类轨迹减少计算开销。
...
Method: 首先发现奖励聚类现象，设计静态的Optimal Variance Filtering (OVF)策略。然后提出Pro-GRPO动态框架，将基于潜在特征的轨迹剪枝集成到采样过程中，采用"扩展-剪枝"策略：先扩展初始采样组以最大化轨迹多样性，然后对潜在特征应用多步OVF。

Result: Pro-GRPO通过早期终止奖励聚类轨迹减少计算开销。在扩散模型和流模型上的广泛实验证明了Pro-GRPO框架的通用性和有效性。

Conclusion: Pro-GRPO成功解决了GRPO中大组规模与计算成本之间的冲突，通过动态轨迹剪枝机制提高了效率，同时保持了优化效果。
...
TL;DR: 提出了一个专门用于桥梁3D语义分割和传感器差异导致的领域差距分析的新数据集，包含多国桥梁的高分辨率3D扫描和详细语义标签，评估了三种先进模型并量化了传感器差异带来的性能下降。
...
Method: 创建了一个包含多国桥梁高分辨率3D扫描的专门数据集，提供详细语义标签；评估了三种最先进的3D深度学习架构；使用不同传感器采集数据以量化传感器差异导致的领域差距。

Result: 所有测试的架构在桥梁分割任务上都表现出稳健性能，但传感器差异导致的领域差距可能导致性能下降高达11.4% mIoU。

Conclusion: 提出的数据集有效支持桥梁3D语义分割研究，传感器差异是实际应用中需要解决的重要问题，领域差距分析为未来研究提供了重要方向。
...
TL;DR: 该论文提出了篮球视频中篮板球预测的新任务，创建了包含10万个视频片段和2000多个手动标注篮板事件的数据集，并应用深度学习技术进行篮板球归属预测。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机视觉在体育分析中取得了显著进展，但在体育视频中预测动作发生前的行为（如篮板球归属）却鲜有研究。现有的技术主要集中在追踪、姿态估计和动作识别上，而动作预测在篮球等动态多智能体场景中具有重要应用价值。

Method: 1. 提出了篮球视频中动作预测的新任务：预测投篮后哪支球队将获得篮板球
...
2. 展示了预测篮板球的可行性和固有挑战
3. 该数据集支持篮球视频理解的广泛应用，填补了现有数据集的空白
4. 实验结果为动态多智能体体育场景的预测建模提供了宝贵见解
...
TL;DR: 该论文提出了一种用于多重组织成像数据的通道独立浅层模型(CIM-S)，相比传统的早期融合CNN，这种轻量级架构在自监督表示学习中表现更优，特别是在保留标记特异性信息和罕见细胞识别方面。


<details>
  <summary>Details</summary>
Motivation: 多重组织成像技术能够测量每个细胞的数十种蛋白质标记物，但现有深度学习模型通常采用早期通道融合，假设所有标记物共享相同结构。作者质疑这种假设是否适用于多重数据，并探索保持标记物独立性结合浅层架构是否能提供更适合的归纳偏置。

Method: 提出通道独立浅层模型(CIM-S)，仅含5.5K参数。使用霍奇金淋巴瘤CODEX数据集（145,000个细胞，49个标记物），比较标准早期融合CNN与通道分离架构，包括标记感知基线和CIM-S。通过对比预训练和线性评估，验证不同架构的性能。

Result: 早期融合模型在保留标记特异性信息方面能力有限，特别是在罕见细胞识别方面表现不佳。通道独立架构（尤其是CIM-S）尽管参数极少，却能产生显著更强的表示。这些发现在多种自监督框架中一致，在不同增强设置下稳定，在49标记物和18标记物设置下均可复现。

Conclusion: 轻量级、通道独立的架构在多重表示学习中可以匹配甚至超越深度早期融合CNN和基础模型，表明对于多重成像数据，保持标记物独立性的浅层架构比增加模型规模更有效。
...
TL;DR: 本文提出了一套完整的GUI自动化解决方案：通过校准步奖励系统实现低成本高质量数据标注，训练出SOTA的Step-GUI模型；设计GUI-MCP协议实现跨设备标准化接口与隐私保护；创建AndroidDaily基准测试真实场景性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为GUI自动化带来新机遇，但面临高质量训练数据获取成本高、标注可靠性差、实际部署缺乏标准化接口和隐私保护、以及缺乏真实场景评估基准等挑战。

Method: 1) 提出基于校准步奖励系统的自进化训练管道，通过轨迹级校准将模型生成轨迹转化为可靠训练信号；2) 训练Step-GUI模型家族(4B/8B)；3) 设计GUI-MCP分层协议，结合底层原子操作和高级任务委派；4) 创建AndroidDaily基准，基于真实移动使用模式。

Result: 1) 训练管道实现>90%标注准确率，成本降低10-100倍；2) Step-GUI 8B模型在AndroidWorld(80.2%)、OSWorld(48.5%)、ScreenShot-Pro(62.6%)达到SOTA；3) AndroidDaily基准上，8B模型静态任务89.91%，端到端任务52.50%；4) GUI-MCP协议实现高隐私执行，敏感数据保留在设备上。

Conclusion: 该工作推动了实用GUI智能体的发展，展示了在日常数字交互中实际部署的强大潜力，通过低成本高质量数据训练、标准化隐私保护协议和真实场景评估，为GUI自动化提供了完整的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频特效编辑面临三大挑战：注入的特效必须与背景无缝融合、背景必须完全保持不变、特效模式必须从有限的配对数据中高效学习。现有视频编辑模型无法满足这些要求。

Method: 1. 利用源视频作为干净上下文条件，利用DiT模型的上下文学习能力实现精确背景保留和自然特效注入；2. 两阶段训练策略：通用编辑适应后通过Effect-LoRA进行特效特定学习；3. 引入时空稀疏标记化以提高效率；4. 发布包含15种高质量视觉风格的配对VFX编辑数据集。

Result: 实验表明IC-Effect能够提供高质量、可控且时间一致的特效编辑，为视频创作开辟新可能性。模型在保持背景不变的同时，实现了特效与背景的自然融合。

Conclusion: IC-Effect框架成功解决了视频特效编辑的关键挑战，通过指令引导的DiT架构和高效训练策略，实现了少样本学习下的高质量、时空一致的特效编辑。
...
TL;DR: 本研究评估了ResNet-101和Inception v3两种深度学习架构在野生动物目标检测中的表现，两者在复杂环境条件下均表现出色，其中Inception v3略胜一筹。
...
Method: 使用ResNet-101和Inception v3两种深度学习架构，在野生动物图像数据集上进行训练和评估。采用标准化预处理方法：图像最大尺寸调整为800像素、转换为RGB格式、转为PyTorch张量。采用70:30的训练验证分割比例。

Result: ResNet-101模型达到94%的分类准确率和0.91的mAP，表现出强大的深层特征提取能力。Inception v3模型表现略好，达到95%的分类准确率和0.92的mAP，归功于其通过并行卷积实现的高效多尺度特征提取。

Conclusion: 尽管两种模型在检测视觉相似物种或光照不佳、遮挡情况下的目标时仍面临挑战，但研究证实ResNet-101和Inception v3都是有效的野生动物目标检测模型，为保护性计算机视觉应用提供了可靠基础。
...
Method: 在MPL框架基础上，提出RUMPL：1）引入2D关键点的3D射线表示，使模型独立于相机标定和视角数量；2）设计新的视角融合Transformer，利用学习的融合射线token沿射线聚合信息，提升多视角一致性。

Result: 实验表明，RUMPL相比三角测量将MPJPE降低达53%，相比基于Transformer的图像表示基线降低超过60%。在新基准测试（包括野外多视角和多人物数据集）中验证了其鲁棒性和可扩展性。

Conclusion: RUMPL通过3D射线表示实现了相机标定无关的通用多视角3D姿态估计，显著提升了精度和泛化能力，适用于任意多视角配置而无需重新训练或微调。
...
<details>
  <summary>Details</summary>
Motivation: LUMIR挑战声称深度学习方法在神经影像配准中具有卓越的零样本泛化能力，这与深度学习领域偏移的既定理解相矛盾。本文旨在通过严谨的评估协议对这些声称进行独立验证。

Method: 采用严格的评估协议，对深度学习方法的零样本泛化能力进行重新评估，特别关注域外对比度（T2、T2*、FLAIR）和高分辨率数据（0.6 mm各向同性图像），同时控制仪器偏差的潜在来源。

Result: 研究发现：1）在分布内T1w图像和相近物种（猕猴）上，深度学习方法与迭代优化方法表现相当；2）在域外对比度上性能显著下降（Cohen's d 0.7-1.5）；3）深度方法无法处理高分辨率数据（0.6 mm），而迭代方法能从更高分辨率中受益；4）深度方法对预处理选择高度敏感。

Conclusion: 深度学习方法的零样本泛化能力被夸大了，其实际临床适用性有限。需要建立反映实际临床和研究工作流程的评估协议，而不是可能无意中偏向特定方法类别的条件。
...
Result: 在DF2023和CASIA v2.0数据集上评估，VAAS在F1和IoU指标上表现具有竞争力，同时通过注意力引导的异常图增强了视觉可解释性。
...
### [47] [Spatia: Video Generation with Updatable Spatial Memory](https://arxiv.org/abs/2512.15716)
*Jinjing Zhao,Fangyun Wei,Zhening Liu,Hongyang Zhang,Chang Xu,Yan Lu*

Main category: cs.CV

TL;DR: Spatia是一个空间记忆感知的视频生成框架，通过维护3D场景点云作为持久空间记忆来提升长期时空一致性，支持显式相机控制和3D感知交互编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在处理密集高维视频信号时难以保持长期空间和时间一致性，这限制了生成视频的质量和可控性。

Method: 提出Spatia框架，将3D场景点云作为持久空间记忆，通过视觉SLAM持续更新，采用动态-静态解耦设计，在空间记忆条件下迭代生成视频片段。

Result: 该框架增强了生成过程中的空间一致性，同时保持了生成逼真动态实体的能力，并支持显式相机控制和3D感知交互编辑等应用。
...
### [48] [DeX-Portrait: Disentangled and Expressive Portrait Animation via Explicit and Latent Motion Representations](https://arxiv.org/abs/2512.15524)
...
Conclusion: DeX-Portrait成功实现了肖像动画中姿态和表情的高保真解耦控制，为仅表情或仅姿态的编辑和动画应用提供了有效解决方案。
...
### [49] [EmoCaliber: Advancing Reliable Visual Emotion Comprehension via Confidence Verbalization and Calibration](https://arxiv.org/abs/2512.15528)
*Daiqing Wu,Dongbao Yang,Can Ma. Yu Zhou*

Main category: cs.CV

TL;DR: EmoCaliber：一个通过三阶段训练框架赋予多模态大语言模型情感置信度表达能力，以解决视觉情感理解中情感感知主观性问题的置信度感知模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉情感理解中通常将情感预测视为确定性任务，输出单一情感标签，未能充分考虑情感感知的主观性，忽略了不同观众可能同样合理的替代解释。

Method: 提出三阶段训练框架：1）赋予结构化推理能力；2）教授置信度表达能力；3）校准置信度表达。最终构建置信度感知的多模态大语言模型EmoCaliber。

Result: 在统一的基准测试VECBench上，EmoCaliber在情感预测和置信度估计方面均表现出优于现有方法的整体优势。

Conclusion: 该方法有效解决了视觉情感理解中的主观性问题，通过置信度表达增强了系统的可靠性，标志着向更可靠的视觉情感理解系统迈出了可行的一步。
...
### [50] [An Efficient and Effective Encoder Model for Vision and Language Tasks in the Remote Sensing Domain](https://arxiv.org/abs/2512.15531)
*João Daniel Silva,Joao Magalhaes,Devis Tuia,Bruno Martins*

Main category: cs.CV

TL;DR: 本文提出GeoMELT模型，一种基于编码器架构的紧凑多任务学习模型，用于遥感图像文本生成和跨模态检索任务，解决了大型视觉语言模型计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在遥感领域具有潜力，但参数量大导致训练和使用成本高昂，现有参数高效适应技术仍计算昂贵，大多数机构难以承受。需要开发既能处理多任务又保持参数紧凑的模型。

Method: 提出GeoMELT（多任务高效学习Transformer）模型，采用编码器架构，专门设计用于处理遥感图像文本生成和跨模态检索这两个通常不在统一模型中探索的任务组合。

Result: 在已建立的基准测试中，GeoMELT模型的结果证实了所提方法的有效性和效率，能够在保持参数紧凑的同时有效处理多任务学习。

Conclusion: 编码器架构的紧凑多任务学习模型是解决遥感领域大型视觉语言模型计算成本问题的有效途径，GeoMELT模型在效率和性能方面都表现出色。
...
### [51] [BLANKET: Anonymizing Faces in Infant Video Recordings](https://arxiv.org/abs/2512.15542)
*Ditmar Hadera,Jan Cech,Miroslav Purkrabek,Matej Hoffmann*

Main category: cs.CV

TL;DR: BLANKET是一种专门为婴儿视频设计的匿名化方法，通过扩散模型生成新面孔并保持时间一致性，在保护面部属性方面优于DeepPrivacy2。


<details>
  <summary>Details</summary>
Motivation: 涉及人类受试者（特别是婴儿）的视频数据需要符合伦理的匿名化方法，以保护隐私同时保留重要的面部属性。
...
Conclusion: BLANKET提供了一种有效的婴儿面部匿名化解决方案，在保护隐私的同时更好地保留了面部属性，代码已开源供使用。
...
### [52] [GRAN-TED: Generating Robust, Aligned, and Nuanced Text Embedding for Diffusion Models](https://arxiv.org/abs/2512.15560)
*Bozhou Li,Sihan Yang,Yushuo Guan,Ruichuan An,Xinlong Chen,Yang Shi,Pengfei Wan,Wentao Zhang,Yuanxing zhang*

Main category: cs.CV

TL;DR: GRAN-TED提出了一种新的文本编码器范式，通过TED-6K基准评估和两阶段训练方法，显著提升了文本到图像/视频生成模型的语义保真度。
...
Method: 1. 提出TED-6K文本基准，通过轻量级统一适配器实现高效评估；2. 开发两阶段训练范式：先在多模态大语言模型上进行微调以获得更好的视觉表示，然后使用分层加权方法提取更细致和强大的文本特征。

Result: GRAN-TED编码器在TED-6K基准上达到最先进性能，并在文本到图像和文本到视频生成任务中带来可观的性能提升。

Conclusion: GRAN-TED通过创新的评估框架和训练方法，有效解决了文本编码器在扩散模型中的关键挑战，显著提升了生成内容的语义保真度。
...
### [53] [On the Effectiveness of Textual Prompting with Lightweight Fine-Tuning for SAM3 Remote Sensing Segmentation](https://arxiv.org/abs/2512.15564)
*Roni Blushtein-Livnon,Osher Rafaeli,David Ioffe,Amir Boger,Karen Sandberg Esquenazi,Tal Svoray*

Main category: cs.CV

TL;DR: SAM3框架通过文本提示实现遥感图像分割，结合语义和几何提示策略效果最佳，轻量微调在几何规则目标上提供实用性能-努力权衡
...
Method: 使用SAM3概念驱动框架，通过文本提示生成掩码而无需任务特定修改，评估了文本、几何和混合提示策略，在轻量微调尺度下进行实验

Result: 结合语义和几何线索在所有目标和指标上表现最佳；纯文本提示表现最差，特别是对于不规则形状目标；轻量微调在几何规则和视觉显著目标上提供实用权衡

Conclusion: 适度的几何标注努力足以实现有效适应；遥感任务中欠分割和边界不准确仍然是普遍错误模式，特别是对于不规则和较少见的目标
...
### [54] [MoonSeg3R: Monocular Online Zero-Shot Segment Anything in 3D with Reconstructive Foundation Priors](https://arxiv.org/abs/2512.15577)
*Zhipeng Du,Duolikun Danier,Jan Eric Lenssen,Hakan Bilen*

Main category: cs.CV

TL;DR: MoonSeg3R是首个实现在线单目3D实例分割的方法，利用CUT3R提供几何先验，通过查询精炼、索引记忆和状态分布令牌实现零样本分割，性能媲美RGB-D系统。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖RGB-D序列的姿态信息，无法在单目RGB流上实现在线零样本3D实例分割。需要一种新方法来解决这一实际应用场景的局限性。

Method: 提出MoonSeg3R框架，包含三个关键组件：1) 自监督查询精炼模块，通过空间语义蒸馏将2D视觉基础模型的分割掩码转化为判别性3D查询；2) 3D查询索引记忆，通过检索上下文查询提供时间一致性；3) 来自CUT3R的状态分布令牌，作为掩码身份描述符增强跨帧融合。

Result: 在ScanNet200和SceneNN数据集上的实验表明，MoonSeg3R是首个实现在线单目3D分割的方法，性能与最先进的RGB-D系统相当。

Conclusion: MoonSeg3R成功解决了在线零样本单目3D实例分割的挑战，利用重建基础模型提供几何先验，实现了与RGB-D系统竞争的性能，为实际应用提供了新方案。
...
### [55] [IMKD: Intensity-Aware Multi-Level Knowledge Distillation for Camera-Radar Fusion](https://arxiv.org/abs/2512.15581)
*Shashank Mishra,Karan Patil,Didier Stricker,Jason Rambach*

Main category: cs.CV

TL;DR: IMKD是一种基于多级知识蒸馏的雷达-相机融合框架，通过三阶段强度感知蒸馏策略，在保持各传感器固有特性的同时增强其互补优势，在nuScenes基准测试中达到67.0% NDS和61.0% mAP的优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法通常将模态特定特征直接传输到每个传感器，这会扭曲其独特特性并削弱各自的优势。需要一种方法在保持传感器固有特性的同时增强其互补性。
...
Conclusion: IMKD通过多级知识蒸馏有效保持了传感器固有特性并增强了互补优势，为高性能雷达-相机3D目标检测提供了一种有效的融合框架。
...
### [56] [FlexAvatar: Learning Complete 3D Head Avatars with Partial Supervision](https://arxiv.org/abs/2512.15599)
...
### [57] [Qwen-Image-Layered: Towards Inherent Editability via Layer Decomposition](https://arxiv.org/abs/2512.15603)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型在图像编辑时经常面临一致性问题，因为光栅图像的所有视觉内容都融合在单一画布上，而专业设计工具使用分层表示，允许独立编辑同时保持一致性。

Method: 提出三个关键组件：1) RGBA-VAE统一RGB和RGBA图像的潜在表示；2) VLD-MMDiT架构支持可变数量的图像层分解；3) 多阶段训练策略将预训练图像生成模型适配为多层图像分解器。此外，构建从PSD文档提取和标注多层图像的管道来解决高质量训练数据稀缺问题。
...
### [58] [Robust Multi-view Camera Calibration from Dense Matches](https://arxiv.org/abs/2512.15608)
*Johannes Hägerlind,Bao-Long Tran,Urs Waldmann,Per-Erik Forssén*

Main category: cs.CV

TL;DR: 提出一种改进相机位姿估计和标定的鲁棒方法，通过优化SfM流程中的对应点采样和视图增量添加策略，显著提升径向畸变严重相机的估计精度。


<details>
  <summary>Details</summary>
Motivation: 虽然运动恢复结构（SfM）技术在相机内外参数估计方面取得了进步，但在动物行为研究和监控视频分析等应用中，对于多视角刚性相机的鲁棒估计仍存在挑战。需要改进现有方法以处理具有强径向畸变的相机。

Method: 分析SfM流程中的各个组件，提出两个主要改进：1）研究如何最佳地从密集匹配器中子采样预测对应点以用于估计过程；2）研究增量添加视图的选择标准。在全局SfM设置中，使用VGGT初始化位姿，并应用提出的对应点子采样方法。

Result: 在严格的定量评估中，所提出的改进显著提升了估计精度，特别是对于具有强径向畸变的相机（本文方法79.9% vs 原始VGGT 40.4%）。提出的流程适用于多种相机设置，在动物行为分析和法医分析中具有应用潜力。

Conclusion: 通过优化SfM流程中的对应点采样和视图增量策略，提出了一种鲁棒的相机位姿估计和标定方法，特别适用于处理具有强径向畸变的相机，为动物行为研究和监控视频分析提供了实用工具。
...
### [59] [Persistent feature reconstruction of resident space objects (RSOs) within inverse synthetic aperture radar (ISAR) images](https://arxiv.org/abs/2512.15618)
*Morgan Coe,Gruffudd Jones,Leah-Nani Alconcel,Marina Gashinova*

Main category: cs.CV

TL;DR: 该研究提出使用亚太赫兹逆合成孔径雷达(ISAR)进行空间目标识别，通过序列图像中的特征检测与跟踪来识别卫星外部结构，提高特征检测和分类的可信度。


<details>
  <summary>Details</summary>
Motivation: 随着近地空间环境中在轨空间物体数量快速增长，需要详细的空间态势感知信息。基于空间的传感能够在更近距离、不受大气影响、全方位地检测空间物体。

Method: 使用亚太赫兹ISAR成像系统，通过元启发式模拟器生成ISAR图像序列。采用梯度比方法进行边缘检测，使用双加权霍夫变换检测线性特征，并通过仿射变换实现帧间对齐，最后在图像序列中跟踪特征演化。

Result: 研究证明在ISAR图像序列中使用特征跟踪能够提高特征检测和分类的可信度，并展示了阴影作为特征的鲁棒检测用例。系统能够在100公里范围内实现亚厘米级分辨率。

Conclusion: 提出的基于序列ISAR图像的特征检测和跟踪方法能够有效识别空间物体的外部结构，为空间态势感知提供了可靠的技术手段。
...
### [60] [OccSTeP: Benchmarking 4D Occupancy Spatio-Temporal Persistence](https://arxiv.org/abs/2512.15621)
*Yu Zheng,Jie Hu,Kailun Yang,Jiaming Zhang*

Main category: cs.CV

TL;DR: 提出4D占据时空持续性概念，解决自动驾驶中两种预测任务：反应式预测和主动式预测，并建立新基准测试和世界模型方法
...
Method: 提出OccSTeP-WM世界模型，采用无分词器设计，维护密集体素场景状态，通过线性复杂度注意力骨干和循环状态空间模块增量融合时空上下文，支持在线推理和运动补偿。

Result: 在挑战性场景下（如错误语义标签和丢帧），平均语义mIoU达到23.70%（提升6.56%），占据IoU达到35.89%（提升9.26%）。

Conclusion: OccSTeP概念和OccSTeP-WM模型有效解决了自动驾驶中的时空持续性预测问题，在复杂场景下表现出鲁棒性能，代码和数据将开源。
...
### [61] [Towards Physically-Based Sky-Modeling For Image Based Lighting](https://arxiv.org/abs/2512.15632)
*Ian J. Maquignaz*
...
<details>
  <summary>Details</summary>
Motivation: 现有天空模型在真实重现自然天空方面存在不足，DNN生成的高动态范围图像虽然视觉质量有所提升，但生成的环境图无法像物理捕获的HDRI那样提供相同的色调、阴影和照明效果。当前HDR文献进展与天空建模脱节，现有方法无法同时支持真实感和户外照明所需的22档完整动态范围。

Method: 提出AllSky模型，直接从物理捕获的HDRI学习，研究天空模型的输入模态、色调映射、条件设置和评估方法。模型允许用户直观控制太阳位置和云层形成，扩展了当前功能。

Result: AllSky实现了最先进的天空模型性能，通过提出的评估方法证明现有DNN天空模型无法与物理捕获的HDRI或参数化天空模型互换，当前限制阻碍了下游应用的可扩展性和准确照明。

Conclusion: AllSky在天空建模方面取得了重要进展，提供了更灵活、更真实的天空环境图生成方法，解决了现有DNN模型在真实感和动态范围方面的局限性。
...
### [62] [InpaintDPO: Mitigating Spatial Relationship Hallucinations in Foreground-conditioned Inpainting via Diverse Preference Optimization](https://arxiv.org/abs/2512.15644)
...
Method: 提出InpaintDPO框架：1) MaskDPO将偏好优化限制在背景区域以增强空间关系，同时在前景区域保留修复损失以保护前景；2) 条件非对称偏好优化通过差异化裁剪操作采样对，应用全局偏好优化以增强上下文感知和边界一致性；3) 共享共性偏好优化利用高质量获胜样本中的空间共性来增强模型对空间共性的理解。

Result: 该方法有效解决了前景条件修复中的空间关系幻觉问题，确保了前景与背景元素间的合理空间关系，提升了修复结果的和谐性和一致性。

Conclusion: InpaintDPO是首个专门针对前景条件修复中空间合理性问题的DPO框架，通过创新的MaskDPO、条件非对称偏好优化和共享共性偏好优化技术，有效解决了空间关系幻觉问题，为可控图像生成提供了重要进展。
...
### [63] [Hard Labels In! Rethinking the Role of Hard Labels in Mitigating Local Semantic Drift](https://arxiv.org/abs/2512.15647)
*Jiacheng Cui,Bingkui Tong,Xinyue Bi,Xiaohan Zhao,Jiacheng Liu,Zhiqiang shen*

Main category: cs.CV

TL;DR: 该论文提出HALD方法，通过结合硬标签来校准软标签在有限图像裁剪下的局部语义漂移问题，在数据集蒸馏和大规模分类任务中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究发现当每个图像只使用有限数量的裁剪时，软标签容易出现局部语义漂移问题：一个裁剪可能在视觉上类似于另一个类别，导致其软嵌入偏离原始图像的真实语义。这种局部视觉内容与全局语义含义之间的不匹配引入了系统误差和训练与测试之间的分布不对齐。
...
Conclusion: 研究重新确立了硬标签作为补充工具的重要性，并呼吁重新思考硬标签在软标签主导的训练中的作用。HALD方法通过结合硬标签校准语义漂移，为软标签训练提供了有效的改进方案。
...
### [64] [Stylized Synthetic Augmentation further improves Corruption Robustness](https://arxiv.org/abs/2512.15675)
*Georg Siedel,Rojan Regmi,Abhirami Anand,Weijia Shao,Silvia Vock,Andrey Morozov*

Main category: cs.CV

TL;DR: 提出一种结合合成图像数据和神经风格迁移的训练数据增强流程，提升深度视觉模型对常见损坏的鲁棒性
...
Method: 结合合成图像数据和神经风格迁移构建数据增强流程，系统分析两种增强方法及其关键超参数对图像分类器性能的影响

Result: 风格迁移虽然会降低合成图像的FID质量指标，但对模型训练有意外益处；两种增强方法互补良好，可与TrivialAugment等规则增强技术结合；在多个小规模图像分类基准上达到最先进的损坏鲁棒性

Conclusion: 合成数据与风格迁移的组合增强方法能有效提升模型对常见损坏的鲁棒性，在CIFAR-10-C、CIFAR-100-C和TinyImageNet-C等基准上取得优异性能
...
### [65] [Skyra: AI-Generated Video Detection via Grounded Artifact Reasoning](https://arxiv.org/abs/2512.15693)
*Yifei Li,Wenzhao Zheng,Yanran Zhang,Runze Sun,Yu Zheng,Lei Chen,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: Skyra是一个专门的多模态大语言模型，通过识别AI生成视频中人类可感知的视觉伪影，为视频检测和解释提供依据，在多个基准测试中超越现有方法。
...
Method: 1) 构建ViF-CoT-4K数据集，首个大规模AI生成视频伪影数据集，含细粒度人工标注；2) 开发Skyra多模态大语言模型，识别人类可感知视觉伪影作为检测和解释依据；3) 采用两阶段训练策略，系统提升时空伪影感知、解释能力和检测精度；4) 引入ViF-Bench基准，包含3K高质量样本，覆盖十多种最先进视频生成器。

Result: Skyra在多个基准测试中超越现有方法，实验表明模型能有效识别AI生成视频中的视觉伪影，并提供可解释的检测结果，为可解释AI生成视频检测提供有价值见解。

Conclusion: Skyra通过识别人类可感知视觉伪影作为证据，实现了可解释的AI生成视频检测，为应对AI生成视频滥用问题提供了有效解决方案，并为该领域未来发展提供了重要基准和见解。
...
### [66] [End-to-End Training for Autoregressive Video Diffusion via Self-Resampling](https://arxiv.org/abs/2512.15702)
...
<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型存在暴露偏差问题，现有方法依赖双向教师模型或在线判别器，需要端到端解决方案
...
<details>
  <summary>Details</summary>
Motivation: 扩散范式在多模态研究中展现出独特优势，但现有扩散视觉语言模型性能仍落后于主流自回归模型。研究探索是否能在现有强大自回归模型基础上构建扩散视觉语言模型。

Method: 提出DiffusionVL框架，通过简单微调将预训练的自回归模型适配到扩散范式，引入支持任意长度生成和KV缓存重用的块解码设计。

Result: 仅使用不到5%的数据训练，在MMMU-Pro视觉基准上获得34.4%提升，在MME认知基准上获得37.5%提升，同时实现2倍推理加速，性能与LLaVA风格视觉指令调优相当。

Conclusion: 自回归模型向扩散范式的转换是有效可行的，DiffusionVL展示了扩散视觉语言模型的潜力，为多模态研究提供了新的范式转换路径。
...
TL;DR: 本文挑战了OpenAI关于大语言模型幻觉源于评估激励不当的观点，认为幻觉是Transformer架构的结构性必然，而非可通过优化奖励机制解决的偶然行为问题。


<details>
  <summary>Details</summary>
Motivation: 反驳OpenAI将幻觉归因于评估激励不当的观点，证明幻觉是Transformer架构的固有特性而非可修复的偶然行为。
...
TL;DR: T5Gemma 2是基于Gemma 3模型构建的轻量级多语言多模态编码器-解码器模型，通过改进的UL2适配方法从纯文本扩展到多模态，并引入了参数共享和注意力融合等效率优化技术。


<details>
  <summary>Details</summary>
Motivation: 开发新一代轻量级开放编码器-解码器模型，增强多语言、多模态和长上下文处理能力，同时保持高效性，为社区提供可用的预训练模型。

Method: 1. 基于T5Gemma的UL2适配方法，将预训练的仅解码器模型转换为编码器-解码器架构；2. 从纯文本扩展到多模态处理；3. 提出两种效率改进方法：跨编码器和解码器的词嵌入共享，以及将解码器自注意力和交叉注意力合并为单一联合模块的融合注意力。
...
Conclusion: T5Gemma 2成功扩展了T5Gemma系列，展示了从仅解码器到编码器-解码器架构适配方法的有效性和可扩展性，为多语言多模态任务提供了高效的轻量级解决方案。
...
TL;DR: 该论文改进了新闻观点分析流程，通过微调大语言模型进行观点分类，并利用Wikidata的语义描述丰富主张表示，在移民辩论基准测试中取得了最佳效果。


<details>
  <summary>Details</summary>
Motivation: 新闻来源通过特定主题、观点和声音塑造政治和社会话语，理解这些动态对于评估媒体是否提供平衡公正的公共辩论至关重要。需要改进现有的新闻观点分析流程以提高分类准确性。

Method: 1) 微调大语言模型用于观点分类；2) 利用Wikidata中相关参与者的语义描述丰富主张表示；3) 在UK移民辩论基准上评估改进方法。

Result: 两种机制都能独立提高分类性能，但它们的整合产生了最佳结果，特别是当使用能够处理长输入的大语言模型时。

Conclusion: 通过结合微调LLM和语义丰富的参与者描述，可以显著改进新闻观点分类的准确性，为更平衡的媒体分析提供技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 评估不同规模的大型语言模型在药学专业任务上的表现，并开发一种无需修改模型架构就能提升准确性的方法，为药学AI应用提供实用解决方案。

Method: 使用包含141个问题的药学数据集对11个不同参数规模（8B到70B+）的LLM进行基准测试；开发了三步检索增强生成（RAG）管道DrugRAG，从验证来源检索结构化药物知识，并将基于证据的上下文增强到模型提示中。

Result: 基线准确率在46%到92%之间，GPT-5（92%）和o3（89%）表现最佳；参数少于8B的模型准确率低于50%；DrugRAG使所有测试模型的准确率提升了7-21个百分点。
...
TL;DR: MAHA提出了一种多尺度聚合层次注意力机制，通过分层分解和数学优化聚合来解决多头自注意力计算复杂度高的问题，在序列长度4096时计算成本降低81%。
...
Method: 提出多尺度聚合层次注意力（MAHA），通过可学习的下采样算子将输入序列动态划分为层次尺度，将尺度特定注意力矩阵的融合建模为资源分配问题，采用凸优化框架或基于纳什均衡的博弈论方法求解，在混合扩张卷积transformer骨干中实现端到端训练。

Result: 实验评估显示MAHA具有优越的可扩展性，经验性FLOPs分析证实，在序列长度4096时相比标准注意力计算成本降低81%。

Conclusion: MAHA在优化理论和序列建模之间架起桥梁，为下一代大型语言模型提供了可扩展的解决方案。
...
TL;DR: 将Flickr30k数据集翻译为罗马尼亚语并扩展为视觉问答数据集，用于训练罗马尼亚语多模态模型，提升模型在罗马尼亚语视觉任务上的性能


<details>
  <summary>Details</summary>
Motivation: 针对低资源语言（特别是罗马尼亚语）的多模态NLP资源匮乏问题，需要减少多模态AI的资源差距，促进生成式AI的民主化

Method: 1) 将Flickr30k数据集翻译为罗马尼亚语；2) 利用开源LLM扩展为视觉问答数据集；3) 选择LLaMA 3.2、LLaVA 1.6和Qwen2三个模型家族；4) 使用参数高效的LoRA方法进行微调

Result: 1) 模型在罗马尼亚语视觉问答任务上表现提升；2) 在未训练的罗马尼亚语图像描述生成任务上也有改进；3) 70亿参数的Qwen2-VL-RoVQA在两个任务上获得最高分，BERTScore F1分别提升6.05%和2.61%；4) 模型语法错误显著减少，罗马尼亚语流畅度提高

Conclusion: 通过创建罗马尼亚语多模态数据集和微调开源VLM，有效提升了模型在罗马尼亚语视觉任务上的能力，为低资源语言的多模态AI发展提供了可行方案
...
TL;DR: 本文系统评估了40个多模态大语言模型在化学奥林匹克竞赛题目上的表现，发现当前模型在多模态融合方面存在显著不足，甚至移除图像有时反而提高准确率，而思维链提示能有效提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态科学推理对大型语言模型仍具挑战性，特别是在化学领域，问题解决依赖于符号图表、分子结构和结构化视觉数据。当前缺乏对多模态模型在化学领域系统评估的研究。

Method: 使用超过20年美国国家化学奥林匹克竞赛题目构建基准，系统评估40个专有和开源多模态大语言模型（包括GPT-5、o3、Gemini-2.5-Pro、Qwen2.5-VL等），通过消融研究和基于遮挡的可解释性分析，特别考察思维链提示的效果。

Result: 研究发现许多模型在多模态融合方面表现不佳，在某些情况下移除图像反而提高准确率，表明视觉-语言整合存在错位。思维链提示能持续提升准确率和视觉基础性，揭示了当前多模态模型在科学推理能力上的关键局限性。

Conclusion: 该研究为衡量领域特定多模态AI进展提供了及时基准，强调了在人工智能与科学推理交叉领域需要进一步推进，并为开发更稳健和可解释的化学多模态系统提供了可行策略。
...
TL;DR: 提出DASH-DTS框架，基于LLM解决海事VHF对话主题分割问题，通过对话握手识别、相似性引导示例选择和正负样本生成提升性能，并发布首个真实海事VHF数据集VHF-Dial。
...
Conclusion: DASH-DTS框架能够为每个分割提供可解释的推理和置信度分数，有效解决了海事VHF对话等非正式通信的主题分割问题，并发布了首个真实世界海事VHF通信数据集推动该领域研究。
...
Method: 提出SGM方法，这是一种白盒神经元级多模态干预技术，类似于安全眼镜的作用：通过专业加权软抑制选择性重新校准一小部分有毒专家神经元，中和有害的跨模态激活，无需参数更新。

Result: 在开源MLLMs上的实验表明，SGM在标准和对抗条件下都能有效降低毒性，将有害率从48.2%降至2.5%，同时保持流畅性和多模态推理能力。SGM*结合现有解毒方法可提供更强的安全性能。

Conclusion: SGM提供了一种可解释、低成本的毒性控制多模态生成解决方案，通过神经元级干预有效降低模型毒性，同时保持模型性能，并可扩展集成到现有解毒方法中。
...
TL;DR: 提出元提示协议框架，通过对抗三元组结构将LLM编排为可编程的自优化系统，解决传统提示工程缺乏确定性保证的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从随机聊天接口向可靠软件组件转型需要重新设计交互范式，传统启发式提示工程无法为关键任务应用提供确定性保证。

Method: 引入元提示协议，采用对抗三元组拓扑结构（生成器P、审计器A、优化器O），将自然语言指令视为语义计算图中的可微分变量，利用文本批评作为梯度。

Result: 通过声明式编程范式（DSPy）和自动文本微分（TextGrad）证明了该方法的理论可行性，减少了幻觉并防止模型崩溃。

Conclusion: 为概率计算时代的"可观测软件工程"奠定了基础，使LLM编排成为可编程、自优化的系统。
...
TL;DR: SCOPE框架通过置信度加权和动态子群划分改进测试时强化学习，缓解多数投票的确认偏差和稀疏奖励问题，在多个基准上显著提升LLM推理能力。


<details>
  <summary>Details</summary>
Motivation: 测试时强化学习使用多数投票结果作为伪标签来减少对标注数据的依赖，但现有投票策略容易导致确认偏差和稀疏奖励问题，限制了大型语言模型推理能力的提升。

Method: 提出SCOPE框架：1）引入步进置信度加权伪标签推导，优先选择高质量推理路径而非简单频率计数；2）动态划分候选输出池为独立子群，平衡推理质量与探索多样性；3）通过子群内重复采样获得局部共识，提供多样化监督目标以鼓励更广泛探索。

Result: 在多个模型和基准测试中，SCOPE均优于现有基线方法。在挑战性任务AIME 2025上实现13.1%的相对提升，在AMC上实现8.1%的相对提升。
...
TL;DR: 本文介绍了乐天旅行评论的大规模语料库，包含2009-2024年730万条客户评论，涵盖多种住宿相关信息和用户评分，并分析了2019-2024年间的数据漂移因素。


<details>
  <summary>Details</summary>
Motivation: 构建一个大规模、时间跨度长的旅行评论数据集，为研究住宿服务、用户行为、数据漂移等提供基础资源，特别是分析疫情前后旅游行业的变化。

Method: 收集乐天旅行平台2009-2024年的客户评论数据，构建包含730万条记录的结构化数据集，使用统计方法分析数据特征和2019-2024年间的数据漂移。

Result: 创建了包含730万条评论的大规模语料库，涵盖16年时间跨度，提供了详细的统计信息，并识别了2019-2024年间影响数据漂移的关键因素。

Conclusion: 该语料库为旅游住宿领域的研究提供了宝贵资源，通过分析数据漂移揭示了行业变化趋势，对理解用户行为和住宿服务发展具有重要意义。
...
TL;DR: 论文探讨了自然语言生成评估与芬兰大学学生评分之间的认知平行关系，指出两者都存在"大错位问题"，并提出基于过程的P-MFA评估模型
...
Method: 引入基于过程的P-MFA（教学多因素评估）模型，这是一个受多因素认证逻辑启发的多证据框架

Result: 识别了NLG评估与学生评分之间的认知平行关系，提出了解决"大错位问题"的框架

Conclusion: 需要从基于产出的评估转向基于过程的评估，P-MFA模型为解决当前评估危机提供了可行方案
...
TL;DR: RFKG-CoT提出了一种改进的知识图谱增强推理方法，通过关系驱动的自适应跳数选择器和少样本上下文学习路径指导机制，显著减少LLM在知识密集型QA中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法如KG-CoT虽然通过整合知识图谱路径提高了可靠性，但存在两个主要问题：1）僵化的跳数选择（仅基于问题驱动）；2）对推理路径的利用不足（缺乏指导）。这导致LLM在知识密集型QA中仍会产生幻觉。

Method: RFKG-CoT包含两个核心创新：1）关系驱动的自适应跳数选择器，通过关系掩码动态调整推理步数（如直接"兄弟"关系用1跳，间接"父子"链用2跳）；2）少样本上下文学习路径指导机制，以"问题-路径-答案"格式构建示例，增强LLM理解推理路径的能力。

Result: 在四个KGQA基准测试中，RFKG-CoT相比KG-CoT在Llama2-7B模型上准确率提升最高达14.7个百分点（WebQSP数据集）。消融实验证实跳数选择器和路径提示是互补的，共同将KG证据转化为更可靠的答案。

Conclusion: RFKG-CoT通过自适应跳数选择和路径指导机制，有效解决了现有KG增强方法的局限性，显著提高了LLM在知识密集型QA中的准确性和可靠性，减少了幻觉问题。
...
TL;DR: Yes-MT团队在WMT 2024低资源印度语言翻译任务中，针对英语与阿萨姆语、米佐语、卡西语、曼尼普尔语之间的翻译，测试了多种方法，包括微调预训练模型、LLM提示、从头训练Transformer等，结果显示微调LLM在低资源翻译中具有潜力。
...
Method: 1. 微调预训练模型：包括mT5和IndicBart的多语言和单语言微调，以及使用LoRA微调IndicTrans2；2. 大语言模型提示：对Llama 3和Mixtral 8x7b进行零样本和少样本提示；3. Llama 3的LoRA监督微调；4. 从头训练Transformer模型。

Result: 使用WMT23低资源印度语言翻译共享任务的测试数据进行评估，采用SacreBLEU和CHRF指标，结果显示低资源翻译面临挑战，但大语言模型特别是经过微调后在这些任务中表现出潜力。

Conclusion: 该研究展示了多种方法在低资源印度语言翻译任务中的表现，强调了微调大语言模型在低资源场景下的有效性，为未来低资源语言机器翻译研究提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型机器遗忘评估基准存在两个主要局限：仅支持英语，且只关注实体级遗忘（完全删除某个人的所有信息）。需要创建一个多语言、支持细粒度遗忘评估的基准来系统比较不同遗忘技术。

Method: 创建FAME基准，包含1000个虚构演员的完整传记，每个传记涵盖20个主题，按结构化类别（传记、职业、成就、个人信息）组织。生成20000个问答对，支持5种语言（英语、法语、德语、意大利语、西班牙语）。提供两个数据集划分来分别支持实体级遗忘（忘记整个身份）和实例级遗忘（忘记特定事实但保留其他信息）。

Result: FAME基准提供了系统评估机器遗忘技术的框架，支持跨5种语言的比较。由于使用完全虚构的数据，确保模型在预训练阶段从未接触过这些信息，实现了对遗忘方法的受控评估。

Conclusion: FAME基准解决了现有评估框架的局限性，为多语言机器遗忘研究提供了标准化评估工具，支持实体级和实例级遗忘的全面评估，有助于推动机器遗忘技术的发展。
...
### [85] [The Moralization Corpus: Frame-Based Annotation and Analysis of Moralizing Speech Acts across Diverse Text Genres](https://arxiv.org/abs/2512.15248)
*Maria Becker,Mirko Sommer,Lars Tapken,Yi Wan Teh,Bruno Brocai*

Main category: cs.CL

TL;DR: 该论文提出了Moralization Corpus，一个用于分析道德论证的多体裁德语数据集，开发了基于框架的标注方案，并评估了大型语言模型在道德化检测任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 道德化论证（引用道德价值观来支持立场）是一种尚未充分探索的说服性沟通形式，具有实用复杂性且常常隐含，对人类标注者和NLP系统都构成挑战。

Method: 开发了基于框架的标注方案，捕捉道德化的构成要素（道德价值观、要求和话语主体），应用于包括政治辩论、新闻文章和在线讨论在内的多样化德语文本，创建了Moralization Corpus。

Result: 详细提示指令比少样本或基于解释的提示效果更好；道德化检测仍然是高度主观和上下文敏感的任务；发布了所有数据、标注指南和代码。

Conclusion: 该研究为道德话语和道德推理的跨学科研究提供了资源，展示了道德化分析的挑战，并表明详细提示对LLM性能有显著影响。
...
### [86] [SynGP500: A Clinically-Grounded Synthetic Dataset of Australian General Practice Medical Notes](https://arxiv.org/abs/2512.15259)
*Piyawoot Songsiritat*

Main category: cs.CL

TL;DR: SynGP500是一个包含500份澳大利亚全科医疗记录的临床医生策划合成数据集，通过整合课程要求、流行病学数据和多样化咨询情境，旨在提供更通用的模型训练资源。


<details>
  <summary>Details</summary>
Motivation: 解决澳大利亚全科医疗领域缺乏高质量临床NLP训练数据的问题，同时保护患者隐私。现有数据集受限于自然病例分布，无法覆盖课程要求但临床实践中不常见的疾病，且真实医疗记录存在隐私限制。

Method: 采用临床医生策划的方法，整合RACGP 2022课程要求、BEACH研究的流行病学数据以及多样化咨询情境。故意设计为"混乱"以反映真实医疗复杂性，包括电报式记录、拼写错误、患者不依从、社会经济障碍和医患分歧。

Result: 通过多方面验证显示数据集质量：流行病学与真实澳大利亚GP咨询模式对齐、语言风格分析确认高语言变异性、语义多样性分析显示广泛覆盖范围、探索性下游评估显示自监督医疗概念提取F1分数提升。

Conclusion: SynGP500填补了澳大利亚全科医疗领域的关键数据空白，为研究人员和教育工作者提供了开发和评估临床NLP方法的资源，同时固有地保护了患者隐私，支持更通用的模型训练。
...
### [87] [Well Begun, Half Done: Reinforcement Learning with Prefix Optimization for LLM Reasoning](https://arxiv.org/abs/2512.15274)
*Yiliu Sun,Zicheng Zhao,Yang Wei,Yanfang Zhang,Chen Gong*

Main category: cs.CL

TL;DR: 提出PPPO方法，专注于优化LLM推理过程中的前缀token，通过渐进式前缀保留和延续累积奖励策略，在减少26.17%训练token的情况下实现18.02%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法对所有生成token进行统一训练，忽视了不同token对推理的贡献差异，特别是前缀token对后续推理的关键影响。这种均匀训练策略浪费资源优化低回报token，阻碍了高回报token的潜在改进。

Method: 提出渐进式前缀策略优化（PPPO）方法，基于"路径依赖"理论识别LLM推理中的"起始锁定效应"，专注于优化前缀推理过程。采用两种训练策略：1) 渐进式前缀保留：逐步增加保留前缀token的比例；2) 延续累积奖励：为每个前缀序列采样多个延续，累积其分数作为奖励信号。

Result: 在各种推理任务上的实验结果表明，PPPO优于代表性RLVR方法，仅使用26.17%的训练token就实现了18.02%的准确率提升。

Conclusion: PPPO通过专注于优化前缀推理过程，有效提升了LLM的推理能力，证明了在RLVR中识别和优化关键token的重要性，为高效训练LLM提供了新思路。
...
### [88] [Evaluating LLMs for Zeolite Synthesis Event Extraction (ZSEE): A Systematic Analysis of Prompting Strategies](https://arxiv.org/abs/2512.15312)
*Charan Prakash Rathore,Saumi Ray,Dhruv Kumar*

Main category: cs.CL

TL;DR: 本文系统评估了LLMs在沸石合成实验信息提取任务中的表现，发现LLMs在事件类型分类上表现良好（80-90% F1），但在细粒度参数提取任务上表现一般（50-65% F1），且高级提示策略相比零样本方法改进有限。


<details>
  <summary>Details</summary>
Motivation: 现有方法尚未系统评估LLMs在沸石合成实验信息提取这一领域特定任务中的表现，需要研究不同提示策略在科学信息提取中的有效性。

Method: 研究聚焦四个子任务：事件类型分类、触发文本识别、参数角色提取和参数文本提取。评估了四种提示策略（零样本、少样本、事件特定、反思式）在六个先进LLMs上的表现，使用包含1,530个标注句子的ZSEE数据集。

Result: 结果显示：事件类型分类表现优异（80-90% F1），但细粒度提取任务表现一般（50-65% F1）。GPT-5-mini表现出极端提示敏感性（11-79% F1变化）。高级提示策略相比零样本方法改进有限，揭示了架构局限性。错误分析发现系统性幻觉、过度泛化和无法捕捉合成特定细微差别等问题。

Conclusion: 虽然LLMs能够实现高层次理解，但精确提取实验参数需要领域适应模型。研究结果为科学信息提取提供了定量基准，揭示了当前LLMs在领域特定信息提取任务中的局限性。
...
### [89] [Why Your Academic Field Is Everywhere at Once: A Case Study of Arabic Linguistics](https://arxiv.org/abs/2512.15328)
*Ayman Eddakrouri,Amani Ramadan*

Main category: cs.CL

TL;DR: 应用Brookes分类离散度指标(Δ)分析当代阿拉伯应用语言学研究的主题结构，基于2019-2025年1564篇文献，计算得出Δ=0.194，显示该领域存在极端主题离散性而非集中性。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在分析当代阿拉伯应用语言学研究的主题结构特征，通过量化方法评估该领域的离散程度，为理解该学科的发展现状提供实证依据。

Method: 采用Brookes分类离散度指标(Δ)，基于2019-2025年间1564篇阿拉伯应用语言学文献的实证数据集，将研究分为8个核心子学科进行计算分析。

Result: 计算得出离散度指数Δ=0.194，这一极低值表明阿拉伯应用语言学领域存在极端主题离散性；计算语言学虽占主导地位但非霸权性，与社会语言学、语言教学等其他子领域共存。

Conclusion: 本研究阐明了Brookes原始公式的正确应用方法，展示了其在领域特征分析中的实用性，并为跨领域评估学科结构提供了可复制的文献计量学方法。
...
### [90] [Adversarial versification in portuguese as a jailbreak operator in LLMs](https://arxiv.org/abs/2512.15353)
*Joao Queiroz*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索诗歌形式提示词作为对抗性攻击机制的有效性，揭示当前语言模型对齐方法的局限性，特别是针对葡萄牙语这种具有高形态句法复杂性和丰富韵律传统的语言缺乏评估。

Method: 研究采用诗歌形式的对抗性提示词攻击方法，包括手动编写和自动生成的诗歌版本，在基于MLCommons AILuminate的基准测试中评估模型安全性能，分析不同对齐方法（RLHF、宪法AI、混合流程）在诗歌攻击下的表现。

Result: 手动编写的诗歌攻击成功率约62%，自动生成版本约43%，某些模型在单轮交互中成功率超过90%。诗歌形式能将提示词转移到稀疏监督的潜在区域，揭示安全防护过度依赖表面模式的问题。

Conclusion: 诗歌形式的对抗性攻击暴露了当前对齐机制的深层局限性，安全防护对表面模式过度依赖。研究特别指出葡萄牙语评估的缺失是一个关键缺口，需要参数化韵律、格律和韵律变化来测试特定于葡语模式的漏洞。
...
### [91] [Dual-Density Inference for Efficient Language Model Reasoning](https://arxiv.org/abs/2512.15358)
*Zhengyi Zhao,Shubo Zhang,Yuxi Zhang,Huimin Wang,Binyang Li,Kam-Fai Wong*

Main category: cs.CL

TL;DR: Denser框架通过区分推理过程（计算功能）和答案生成（沟通功能），使用高密度压缩推理机制，在保持准确性的同时显著减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在复杂推理任务中，对中间推理过程和最终答案使用统一的语言密度，导致计算效率低下。研究发现推理过程主要为模型自身计算服务，而答案生成则服务于人类理解，这种差异为优化提供了机会。
...
Conclusion: 通过区分推理和回答阶段的信息密度，Denser框架实现了计算效率的显著提升，为LLM推理优化提供了新的方向。
...
### [92] [ORACLE: Time-Dependent Recursive Summary Graphs for Foresight on News Data Using LLMs](https://arxiv.org/abs/2512.15397)
...
<details>
  <summary>Details</summary>
Motivation: 为大学提供从海量新闻中提取决策相关洞察的系统化解决方案，帮助机构及时了解外部环境变化，支持战略决策

Method: 构建新闻爬取与版本管理流水线，应用大学特定相关性过滤，嵌入内容，PESTEL维度分类，创建时间依赖递归摘要图（TRSG），包含两层聚类和LLM摘要，每周重新计算，轻量级变化检测器识别新增、删除和变更内容

Result: 开发了稳定运行的ORACLE生产系统，能够将新闻转化为周度PESTEL感知分析，支持课程智能等具体应用场景

Conclusion: ORACLE系统成功实现了从新闻到决策洞察的自动化转化，为高等教育机构提供了有效的环境监测工具，具有实际应用价值
...
### [93] [Toward expert-level motivational interviewing for health behavior improvement with LLMs](https://arxiv.org/abs/2512.15446)
...
TL;DR: 该研究开发并评估了用于动机性访谈的大型语言模型（MI-LLMs），通过微调中文开源LLMs，使其能够模拟人类咨询师的MI核心行为，为AI辅助健康行为改变提供了可扩展的途径。


<details>
  <summary>Details</summary>
Motivation: 动机性访谈（MI）是一种有效的健康行为改变咨询方法，但其推广受到需要高度训练的人类咨询师的限制。本研究旨在探索一种可扩展的替代方案，通过开发大型语言模型来执行动机性访谈。

Method: 研究首先收集了五个中文心理咨询语料库，使用GPT-4和MI提示将两个最高质量数据集（CPsyCounD和PsyDTCorpus）转换为2,040个MI风格对话。然后对三个中文开源LLMs（Baichuan2-7B-Chat、ChatGLM-4-9B-Chat和Llama-3-8B-Chinese-Chat-v2）进行微调，创建MI-LLMs。评估采用基于回合的自动指标和专家手动编码（使用MITI编码手册4.2.1）。

Result: 微调显著提高了所有三个模型的BLEU-4和ROUGE分数。手动编码显示MI-LLMs在技术和关系全局评分以及MI一致性比率方面接近真实MI对话水平，但复杂反思和反思与问题比率仍然较低。

Conclusion: 研究初步证明MI导向的微调可以使通用LLMs具备核心的MI一致性咨询行为，为AI辅助健康行为改变支持提供了可扩展的途径，但需要在数据规模、复杂MI技能和真实世界干预试验方面进一步研究。
...
### [94] [When a Nation Speaks: Machine Learning and NLP in People's Sentiment Analysis During Bangladesh's 2024 Mass Uprising](https://arxiv.org/abs/2512.15547)
*Md. Samiul Alim,Mahir Shahriar Tamim,Maisha Rahman,Tanvir Ahmed Khan,Md Mushfique Anwar*

Main category: cs.CL

TL;DR: 该研究首次对孟加拉语在2024年孟加拉国大规模抗议期间进行情感分析，构建了包含2,028条标注新闻标题的数据集，使用语言特定模型在情感分类任务上超越了多语言模型和传统机器学习方法。
...
Method: 研究构建了包含2,028条标注新闻标题的独特数据集，将情感分为愤怒、希望和绝望三类。使用潜在狄利克雷分配（LDA）识别政治腐败、公众抗议等主题，并分析互联网封锁等事件对情感模式的影响。比较了语言特定模型与多语言变换器（mBERT、XLM-RoBERTa）和传统机器学习方法（SVM、逻辑回归）的性能。
...
Conclusion: 研究结果突显了语言特定模型在处理特定语言情感分析任务时的有效性，为理解政治动荡期间的公众情绪提供了宝贵见解，填补了孟加拉语在危机情境下情感分析的研究空白。
...
### [95] [CTkvr: KV Cache Retrieval for Long-Context LLMs via Centroid then Token Indexing](https://arxiv.org/abs/2512.15550)
*Kuan Lu,Shuhang Lin,Sai Wu,Yichen Yao,Junhan Yang,Huan Li,Wei Chu,Xu Yinghui,Yuan Qi,Gang Chen*

Main category: cs.CL

TL;DR: CTKVR提出了一种新颖的centroid-then-token KV检索方案，通过两阶段检索策略平衡KV缓存检索的效率和准确性，在长上下文场景下显著提升推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长上下文场景（如多轮对话）中面临KV缓存带来的高内存开销和延迟问题。现有动态KV选择方法存在权衡困境：块级索引因检索不相关KV条目而降低准确性，而令牌级索引因低效检索机制导致高延迟。

Method: CTKVR采用两阶段检索策略：1）预填充阶段预计算轻量级质心用于质心粒度索引；2）令牌级细化进行精确KV检索。该方法基于观察发现：经过RoPE后位置相邻的查询向量具有高度相似性并共享大部分top-k KV缓存条目。系统采用CPU-GPU协同执行优化索引构建和搜索。

Result: 在多个基准测试中，CTKVR在准确性下降小于1%的情况下实现优越性能。在96K上下文长度下，对Llama-3-8B和Yi-9B模型分别实现3倍和4倍的吞吐量加速，适用于多种GPU硬件。

Conclusion: CTKVR通过创新的centroid-then-token检索方案有效解决了长上下文推理中KV缓存检索的效率-准确性权衡问题，显著提升了大语言模型在长上下文场景下的推理性能。
...
### [96] [From Data to Dialogue: Unlocking Language for All](https://arxiv.org/abs/2512.15552)
*Dakota Ellis,Samy Bakikerali,Wanshan Chen,Bao Dinh,Uyen Le*

Main category: cs.CL

TL;DR: 本文提出了一种自动化创建专业词汇表的方法，相比传统人工制定的通用服务词表，能更高效地帮助语言学习者掌握特定领域的词汇。


<details>
  <summary>Details</summary>
Motivation: 传统语言学家提出的通用服务词表需要语言专业知识、主观判断和大量时间，存在效率低、难以自动化的问题。本文旨在开发更实用的词汇表创建方法。

Method: 通过创建专业词汇表，即针对特定语料库子集的词汇表，并仅使用客观标准进行自动化处理，使整个过程可扩展、可定制。

Result: 使用该模型创建的专业词汇表在性能上超越了行业标准，用更少的词汇达到了语言理解所需的95%覆盖率要求。

Conclusion: 通过将专业词汇表创建过程限制在客观标准内，可以实现自动化、规模化，并能根据全球语言学习者的需求进行定制，为词汇学习提供了更优化的解决方案。
...
### [97] [An Empirical Study on Chinese Character Decomposition in Multiword Expression-Aware Neural Machine Translation](https://arxiv.org/abs/2512.15556)
*Lifeng Han,Gareth J. F. Jones,Alan F. Smeaton*

Main category: cs.CL

TL;DR: 该论文系统研究了汉字分解技术在MWE感知神经机器翻译中的应用，探索如何通过分解汉字来更好地表示中文词汇和字符的原始含义，以解决多词表达翻译的挑战。


<details>
  <summary>Details</summary>
Motivation: 多词表达式在自然语言处理任务中带来诸多困难，如歧义、习语表达、低频使用和变体多样。虽然西方语言（特别是英语）在MWE处理方面取得显著进展，但中文等表意文字语言在这方面仍落后，因为适用于西方语言的子词建模技术（如BPE）无法直接应用于汉字。

Method: 采用系统研究方法，将汉字分解技术应用于MWE感知的神经机器翻译中。通过实验检验汉字分解技术如何帮助表示中文词汇和字符的原始含义，以及如何有效解决MWE翻译的挑战。

Result: 论文报告了相关实验结果，展示了汉字分解技术在中文MWE翻译中的效果。具体结果未在摘要中详细说明，但表明该技术能够有效应对MWE翻译的挑战。

Conclusion: 汉字分解技术对于中文多词表达式的神经机器翻译具有重要意义，能够帮助更好地表示中文词汇和字符的原始含义，为解决中文MWE翻译的独特挑战提供了有效途径。
...
### [98] [Bolmo: Byteifying the Next Generation of Language Models](https://arxiv.org/abs/2512.15586)
...
TL;DR: Bolmo是首个在1B和7B参数规模上具有竞争力的完全开源的字节级语言模型家族，通过字节化现有子词级模型实现，性能接近领先的子词级模型，同时克服了子词分词的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有字节级语言模型主要从头开始训练，存在表达力不足的问题。子词分词存在字符理解不足和固定词汇表导致的效率限制。需要一种方法既能克服子词分词的局限性，又能保持与领先子词级模型相当的性能。

Method: 通过字节化现有子词级语言模型来训练Bolmo。设计了专门的架构解决先前字节级架构与子词级模型之间的表达力不匹配问题，采用有效的精确蒸馏目标，仅需不到1%的典型预训练token预算就能将子词级模型转换为字节级模型。

Result: Bolmo在可比规模上显著优于所有先前的字节级语言模型，在字符理解方面优于源子词级模型，在某些编码任务上也表现更好，在其他任务上接近原始模型的性能。通过更高的token压缩比训练，可以实现与子词级模型竞争的推理速度。

Conclusion: Bolmo使字节级语言模型成为与子词级模型在各种用例中具有竞争力的实用选择，能够廉价有效地进行后训练，并利用源子词级模型的现有生态系统。
...
### [99] [You Never Know a Person, You Only Know Their Defenses: Detecting Levels of Psychological Defense Mechanisms in Supportive Conversations](https://arxiv.org/abs/2512.15601)
...
TL;DR: 研究者开发了PsyDefConv对话语料库和DMRS Co-Pilot四阶段标注流水线，用于标注心理防御机制，在临床对话中实现了可靠的防御水平标注，标注时间减少22.4%，但现有语言模型在防御识别上仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制是人们在应对痛苦时使用的策略，但防御机制的僵化或过度使用与心理健康问题相关。然而，防御机制复杂且难以可靠测量，特别是在临床对话中，需要开发有效的标注工具和语料库来支持相关研究。

Method: 1) 构建PsyDefConv对话语料库，包含200个对话和4709个话语，其中2336个求助者话语标注了防御水平；2) 开发DMRS Co-Pilot四阶段流水线，提供基于证据的预标注；3) 进行平衡研究评估标注效率；4) 专家评审评估证据质量、临床合理性和洞察力；5) 使用强语言模型进行零样本和微调基准测试。

Result: 1) 语料库标注的Cohen's kappa为0.639，达到中等一致性；2) Co-Pilot减少平均标注时间22.4%；3) 专家评分较高（7分制：证据4.62，临床合理性4.44，洞察力4.40）；4) 语言模型最佳宏F1分数约30%，倾向于过度预测成熟防御；5) 语料分析显示成熟防御最常见，存在情绪特异性偏差。

Conclusion: PsyDefConv语料库和DMRS Co-Pilot流水线为语言中的防御功能研究提供了有价值的资源，虽然现有语言模型在防御识别上仍有很大改进空间，但该工具能显著提高标注效率和质量，支持临床对话中防御机制的深入研究。
...
### [100] [Evaluating Metrics for Safety with LLM-as-Judges](https://arxiv.org/abs/2512.15617)
*Kester Clegg,Richard Hawkins,Ibrahim Habli,Tom Lawton*

Main category: cs.CL

TL;DR: 论文探讨如何在安全关键的信息处理流程中引入LLMs，通过采用加权指标篮子和置信度阈值来降低错误风险，并在评估一致性低时触发人工审查。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于文本处理流程，有可能替代人力瓶颈，但LLMs会犯错且某些处理角色是安全关键的。如何在之前由人类执行的关键信息流中安全可靠地引入LLMs？

Method: 论文主张安全论证应聚焦于从LLM流程评估点获得的证据类型，特别是在采用LLM-as-Judges评估框架中。通过采用加权指标篮子、利用上下文敏感性定义错误严重性，并设计置信度阈值在评估者一致性低时触发人工审查。

Result: 虽然无法从许多自然语言处理任务中获得确定性评估，但通过采用加权指标篮子可以降低评估中的错误风险，使用上下文敏感性定义错误严重性，并设计置信度阈值在关键LaJ判断一致性低时触发人工审查。

Conclusion: 在安全关键的信息处理流程中引入LLMs时，应关注评估证据的质量而非生成框架的性能，通过系统化的评估策略和人工审查机制来确保安全性和可靠性。
...
### [101] [How Much is Too Much? Exploring LoRA Rank Trade-offs for Retaining Knowledge and Domain Robustness](https://arxiv.org/abs/2512.15634)
*Darshita Rathore,Vineet Kumar,Chetna Bansal,Anindya Moitra*

Main category: cs.CL

TL;DR: 该研究全面评估了全监督微调(SFT)与参数高效微调(PEFT/LoRA)在问答任务中的性能对比，通过秩扫描实验量化了两种方法的权衡，分析了它们在领域内外泛化能力及任务特定遗忘现象


<details>
  <summary>Details</summary>
Motivation: 虽然参数高效微调方法因其计算效率而被广泛使用，但其配置参数（如秩）在下游问答任务和泛化能力方面的影响尚未得到充分探索，需要系统评估SFT与PEFT的性能差异

Method: 在多个推理和记忆数据集上进行全面评估，进行秩扫描实验量化SFT与PEFT的权衡；比较PEFT和SFT模型在领域内和领域外适应中的准确性；通过谱特征和层级注意力结构分析内部表示

Result: LoRA在特定秩值下，尤其在推理任务上，实现了与SFT竞争甚至更优的性能；揭示了PEFT和SFT在泛化行为上的明显差异以及任务特定的遗忘现象；通过表示漂移和注意力结构变化提供了深入见解

Conclusion: 参数高效微调方法（特别是LoRA）在适当配置下可以达到与全监督微调相当甚至更好的性能，尤其是在推理任务中，为高效模型适应提供了有前景的替代方案
...
### [102] [PPSEBM: An Energy-Based Model with Progressive Parameter Selection for Continual Learning](https://arxiv.org/abs/2512.15658)
...
Conclusion: PPSEBM为持续学习中的灾难性遗忘问题提供了一个有前景且鲁棒的解决方案，通过参数选择和伪样本生成的协同作用实现了更好的知识保留
...
### [103] [Activation Oracles: Training and Evaluating LLMs as General-Purpose Activation Explainers](https://arxiv.org/abs/2512.15674)
...
TL;DR: 本文评估了LatentQA训练的激活预言机模型，发现即使使用窄任务训练，它们也能很好泛化，且多样化训练能进一步提升性能，在多个下游任务中匹配或超越白盒基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM激活理解技术通常使用复杂、专门化的方法，而LatentQA提供了一种更简单的替代方案。但先前研究主要关注窄任务设置，本文旨在从通用视角评估LatentQA模型在分布外环境中的表现。

Method: 采用LatentQA方法训练激活预言机模型，接受LLM激活作为输入并用自然语言回答相关问题。研究评估了训练数据多样性对性能的影响，包括分类任务和自监督上下文预测任务等额外训练数据集。

Result: 激活预言机能够恢复模型中微调注入的信息（如传记知识或恶意倾向），即使从未使用微调模型的激活进行训练。在四个下游任务评估中，最佳AO模型在所有任务上匹配或超越先前白盒基线，在3/4任务中表现最佳。

Conclusion: 多样化训练使模型获得用自然语言表达LLM激活信息的通用能力，LatentQA方法为理解LLM激活提供了一种有效且可扩展的途径。
...
### [104] [Breathe with Me: Synchronizing Biosignals for User Embodiment in Robots](https://arxiv.org/abs/2512.14952)
*Iddo Yehoshua Wald,Amber Maimon,Shiyao Zhang,Dennis Küster,Robert Porzel,Tanja Schultz,Rainer Malaka*

Main category: cs.RO

TL;DR: 通过将用户的呼吸信号实时映射到机器人手臂运动上，研究呼吸同步对用户身体所有权感知的影响，发现呼吸同步能显著增强身体所有权体验。


<details>
  <summary>Details</summary>
Motivation: 在传统的人机交互中，视觉和运动反馈的同步已被证明能增强用户的具身感。本研究探索将生理信号（呼吸）作为新的内感受通路，以增强用户在机器人系统中的具身体验。

Method: 采用被试内实验设计，参与者控制机器人手臂，实验条件分为呼吸同步（机器人运动与用户呼吸实时同步）和非同步两种。测量身体所有权感知和用户偏好。

Result: 呼吸同步显著增强了参与者的身体所有权感知，大多数参与者更喜欢呼吸同步的条件。

Conclusion: 生理信号（如呼吸）的实时表征可作为增强人机交互中具身体验的新途径，对远程存在、假肢、机器人协作和共享自主性等领域具有应用价值。
...
### [105] [ISS Policy : Scalable Diffusion Policy with Implicit Scene Supervision](https://arxiv.org/abs/2512.15020)
*Wenlong Xia,Jinhao Zhang,Ce Zhang,Yaojia Wang,Youmin Gong,Jie Mei*

Main category: cs.RO

TL;DR: 提出ISS Policy，一种基于点云观测和DiT扩散策略的3D视觉运动学习方法，通过隐式场景监督模块提升策略性能和鲁棒性，在仿真和真实世界任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的模仿学习过度依赖物体外观而忽略3D场景结构，导致训练效率低和泛化能力差。需要开发能更好利用3D几何信息的策略。

Method: 提出ISS Policy：基于点云观测的3D视觉运动DiT扩散策略，预测连续动作序列。扩展DiT架构，加入新颖的隐式场景监督模块，使模型输出与场景几何演化保持一致。

Result: 在单臂操作任务（MetaWorld）和灵巧手操作（Adroit）上达到最先进性能。真实世界实验显示强泛化能力和鲁棒性。消融研究表明方法在数据和参数规模上均能有效扩展。

Conclusion: ISS Policy通过隐式场景监督有效利用3D几何信息，显著提升模仿学习的性能和泛化能力，为机器人操作任务提供了更鲁棒的解决方案。
...
### [106] [HERO: Hierarchical Traversable 3D Scene Graphs for Embodied Navigation Among Movable Obstacles](https://arxiv.org/abs/2512.15047)
*Yunheng Wang,Yixiao Feng,Yuetong Fang,Shuning Zhang,Tan Jing,Jian Li,Xiangrui Jiang,Renjing Xu*

Main category: cs.RO

TL;DR: HERO框架通过构建层次化可通行3D场景图，将可操作障碍物重新定义为通路，显著提升了在真实动态环境中的导航效率与可达性。
...
Conclusion: HERO通过重新定义可通行性并建模场景的层次关系，有效解决了现有3D场景图导航在动态环境中的局限性，为智能体在真实世界中的交互提供了更强大的基础。
...
### [107] [NAP3D: NeRF Assisted 3D-3D Pose Alignment for Autonomous Vehicles](https://arxiv.org/abs/2512.15080)
*Gaurav Bansal*
...
<details>
  <summary>Details</summary>
Motivation: 传统SLAM中的视觉回环检测需要重新访问已观测位置，且依赖多传感器融合。当无法进行回环检测时，累积的传感器噪声和漂移会导致显著的位姿估计误差。

Method: 提出NeRF-Assisted 3D-3D Pose Alignment (NAP3D)，利用智能体当前深度图像与预训练NeRF之间的3D-3D对应关系，将观测的3D点与NeRF合成的3D点直接对齐，从而修正位姿估计。

Result: 在自定义数据集上实现5厘米以内的相机位姿修正；在TUM RGB-D数据集上，相比2D-3D PnP基线方法，3D对齐RMSE持续改善约6厘米，显示出更好的3D空间几何一致性。

Conclusion: NAP3D提供了一种轻量级、数据集无关的工具，在传统回环检测不可用时，能够补充现有的SLAM和定位流程，通过3D-3D对应关系实现稳健的位姿修正。
...
TL;DR: BEV-Patch-PF：一种无需GPS的序列化地理定位系统，将粒子滤波器与学习的鸟瞰图和航拍特征图相结合，在越野环境中实现高精度实时定位。


<details>
  <summary>Details</summary>
Motivation: 在无GPS环境（如茂密树冠下）中实现精确的机器人定位是一个挑战。传统检索方法精度有限，需要一种能够处理遮挡和光照变化的鲁棒定位系统。

Method: 系统从机载RGB和深度图像构建BEV特征图，为每个3自由度粒子姿态假设从局部航拍图像中裁剪对应补丁，通过匹配BEV特征与航拍补丁特征计算每个粒子的对数似然。

Result: 在两个真实世界越野数据集上，相比基于检索的基线方法，在已见路线上绝对轨迹误差降低7.5倍，在未见路线上降低7.0倍，在茂密树冠和阴影下仍保持精度，在NVIDIA Tesla T4上以10Hz实时运行。

Conclusion: BEV-Patch-PF在无GPS环境中实现了高精度、鲁棒的实时定位，特别适用于越野机器人部署，在挑战性光照和遮挡条件下表现出色。
...
TL;DR: 提出了一种用于自动驾驶感知系统的新型安全度量框架，结合对象检测和车道检测的安全评估，相比传统性能指标能更好地识别安全关键错误。


<details>
  <summary>Details</summary>
Motivation: 传统感知系统评估指标（如精确率、召回率、F1分数）只关注整体检测准确性，但忽略了安全相关方面。即使在这些指标上得分很高的感知系统，仍可能产生导致严重事故的误检测，因此需要专门的安全评估方法。

Method: 提出了一个联合评估框架，包含：1）新的轻量级对象安全度量，量化对象检测错误相关的潜在风险；2）车道安全度量，考虑对象和车道检测任务之间的相互依赖性；3）结合两者形成统一、可解释的感知安全性能综合评分。

Result: 使用DeepAccident数据集验证，该方法能够识别传统性能指标无法捕捉的安全关键感知错误，证明了安全中心评估方法的重要性。

Conclusion: 强调了对自动驾驶感知系统进行安全中心评估的重要性，提出的联合安全度量框架为感知系统的安全性能提供了更全面的评估方法。
...
### [110] [Infrastructure-based Autonomous Mobile Robots for Internal Logistics -- Challenges and Future Perspectives](https://arxiv.org/abs/2512.15215)
...
TL;DR: 本文提出了一种基于基础设施的自主移动机器人系统架构，结合基础设施感知、本地云计算和车载自主性，为复杂工业环境中可扩展、鲁棒且人机兼容的AMR系统提供基础。


<details>
  <summary>Details</summary>
Motivation: 当前自主移动机器人大多采用分散式车载智能方案，而基于基础设施（外部传感器和计算资源）支持AMR的系统在文献中研究不足，特别是在室内工业环境中的应用。

Method: 提出参考架构，结合基础设施感知、本地云计算和车载自主性；回顾定位、感知和规划核心技术；在重型车辆制造环境中进行实际部署和用户体验评估。

Result: 展示了在真实工业环境中的部署案例，并通过用户体验评估总结了相关发现，验证了基于基础设施的AMR系统在复杂工业环境中的可行性。

Conclusion: 为未来开发可扩展、鲁棒且人机兼容的AMR系统提供了全面的基础框架，特别是在复杂工业环境中基础设施支持的AMR系统具有重要应用前景。
...
### [111] [VLA-AN: An Efficient and Onboard Vision-Language-Action Framework for Aerial Navigation in Complex Environments](https://arxiv.org/abs/2512.15258)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型空中导航模型存在四个主要问题：1) 数据域差距导致模型泛化能力差；2) 缺乏足够的时间导航推理能力；3) 生成动作策略存在安全隐患；4) 难以在资源受限的无人机上部署。这些问题限制了无人机在复杂环境中的自主导航能力。

Method: 1) 使用3D高斯溅射构建高保真数据集以弥合域差距；2) 设计渐进式三阶段训练框架，依次增强场景理解、核心飞行技能和复杂导航能力；3) 设计轻量级实时动作模块，结合几何安全校正，确保快速、无碰撞、稳定的指令生成；4) 通过深度优化机载部署流程，在资源受限的无人机上实现高效推理。

Result: VLA-AN在推理吞吐量上实现了8.3倍的实时改进，显著提高了空间定位、场景推理和长时程导航能力，单任务最高成功率达到了98.1%，为轻量级空中机器人实现全链闭环自主性提供了高效实用的解决方案。

Conclusion: VLA-AN通过创新的数据集构建、渐进式训练框架、安全动作模块和优化部署流程，成功解决了无人机自主导航中的关键挑战，为资源受限的空中机器人提供了高效、安全、实用的全链闭环自主导航解决方案。
...
### [112] [A Network-Based Framework for Modeling and Analyzing Human-Robot Coordination Strategies](https://arxiv.org/abs/2512.15282)
*Martijn IJtsma,Salvatore Hargis*

Main category: cs.RO

TL;DR: 提出用于人机系统联合工作策略分析的计算框架，整合功能建模与图论表示，支持早期概念设计阶段的协调策略探索
...
Method: 整合功能建模与图论表示，通过系统功能关系和工作环境物理信息结构来表征集体工作，明确捕捉协调需求随时间演变
...
### [113] [GuangMing-Explorer: A Four-Legged Robot Platform for Autonomous Exploration in General Environments](https://arxiv.org/abs/2512.15309)
*Kai Zhang,Shoubin Chen,Dong Li,Baiyang Zhang,Tao Huang,Zehao Wu,Jiasheng Chen,Bo Zhang*

Main category: cs.RO

TL;DR: 本文介绍了GuangMing-Explorer，一个完全集成的自主探索平台，旨在在各种环境中稳健运行，填补了现有研究中缺乏完整自主探索系统描述的空白。


<details>
  <summary>Details</summary>
Motivation: 自主探索是集成感知、规划、控制和运动执行的基本能力，在室内目标搜索、极端环境测绘、资源勘探等应用中至关重要。尽管各个组件取得了显著进展，但缺乏一个包含硬件和软件的完整自主探索系统的全面实用描述。
...
Conclusion: GuangMing-Explorer是一个完全集成的自主探索平台，能够跨多样化环境稳健运行，为实际部署提供了有前景的解决方案。
...
### [114] [Remotely Detectable Robot Policy Watermarking](https://arxiv.org/abs/2512.15379)
*Michael Amir,Manon Flageat,Amanda Prorok*

Main category: cs.RO

TL;DR: 该论文提出了首个用于机器人策略远程检测的水印方法CoNoCo，通过利用策略的固有随机性在机器人运动中嵌入频谱信号，解决了物理观察差距问题。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在机器人系统中的成功应用，训练好的策略成为一种新型知识产权，需要验证所有权并检测未经授权的使用。现有水印方法需要访问机器人内部状态，但审计者通常只能通过外部观察（如视频）进行检测，存在"物理观察差距"。

Method: 提出了Colored Noise Coherency (CoNoCo)水印策略，利用策略的固有随机性在机器人运动中嵌入频谱信号。该方法通过概念化的"glimpse sequence"形式化远程检测挑战，并证明CoNoCo保持了边际动作分布，不会降低性能。

Result: 实验表明CoNoCo在各种远程模态下都能实现强大、鲁棒的检测，包括动作捕捉和侧向/俯视视频，在模拟和真实机器人实验中均表现良好。

Conclusion: 这项工作为保护机器人知识产权提供了必要步骤，首次提出了使用纯远程观察非侵入式验证物理策略来源的方法。
...
### [115] [MiVLA: Towards Generalizable Vision-Language-Action Model with Human-Robot Mutual Imitation Pre-training](https://arxiv.org/abs/2512.15411)
...
TL;DR: MiVLA通过人类-机器人双向模仿预训练，利用人手与机械臂的行为相似性建立行为先验，解决现有视觉语言动作模型在相机视角、视觉外观和形态结构不匹配时的泛化能力限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作模型（VLA）虽然可以利用丰富的人类视频和模拟机器人数据来解决真实世界机器人数据稀缺的问题，但由于相机视角、视觉外观和形态结构的不匹配，其泛化能力仍然有限。

Method: 提出MiVLA模型，通过人类-机器人双向模仿预训练，利用左右手坐标系运动学规则实现人类与机器人动作空间的双向对齐。模型训练时，给定人类或模拟机器人演示，MiVLA能够预测一个形态的行为轨迹，并模仿另一个未见过的形态的行为。

Result: 在三个机器人平台（ARX、PiPer和LocoMan）的仿真和真实世界实验中，MiVLA展现出显著改进的泛化能力，在仿真中比最先进的VLA模型（如π₀、π₀.₅和H-RDT）提升25%，在真实世界机器人控制任务中提升14%。

Conclusion: 通过人类-机器人双向模仿预训练，MiVLA成功将真实世界人类数据的行为保真度与模拟机器人数据的操作多样性整合到统一模型中，显著增强了视觉语言动作模型在下游任务中的泛化能力。
...
### [116] [Load-Based Variable Transmission Mechanism for Robotic Applications](https://arxiv.org/abs/2512.15448)
*Sinan Emre,Victor Barasuol,Matteo Villa,Claudio Semini*

Main category: cs.RO

TL;DR: 提出一种基于负载的可变传动机制，通过预紧弹簧和四连杆机构被动调整传动比，无需额外执行器即可根据外部扭矩需求动态适应，在达到预设扭矩阈值时传动比可增加40%。


<details>
  <summary>Details</summary>
Motivation: 现有可变传动系统通常需要额外的执行器进行主动控制，增加了机器人关节驱动系统的复杂性。本研究旨在开发一种轻量、高效且自适应的传动系统，特别是针对腿式机器人中动态扭矩适应至关重要的应用场景。

Method: 提出基于负载的可变传动机制，利用预紧弹簧和四连杆机构被动修改传动比。通过仿真分析评估机制有效性，系统在施加力超过18N时触发扭矩放大效应。

Result: 系统在达到预设扭矩阈值时传动比可增加40%，有效放大关节扭矩而无需额外驱动。仿真显示当施加力超过18N时触发扭矩放大效应，系统能够自主响应变化的负载条件。

Conclusion: 该研究为机器人应用特别是腿式机器人开发了轻量、高效且自适应的传动系统，通过被动机制实现动态扭矩适应，减少了机器人关节驱动系统的复杂性。
...
### [117] [OMCL: Open-vocabulary Monte Carlo Localization](https://arxiv.org/abs/2512.15557)
*Evgenii Kruzhkov,Raphael Memmesheimer,Sven Behnke*

Main category: cs.RO

TL;DR: 本文提出了一种基于视觉语言特征的蒙特卡洛定位方法，通过开放词汇特征实现跨模态观测与地图特征的鲁棒关联，支持自然语言描述的全局定位初始化。


<details>
  <summary>Details</summary>
Motivation: 机器人定位是导航规划的重要前提。当环境地图由不同传感器创建时，需要鲁棒地将机器人测量与地图特征关联。传统方法在处理跨模态数据时存在局限性。

Method: 扩展蒙特卡洛定位方法，引入视觉语言特征。利用开放词汇特征计算给定相机位姿和3D地图（来自RGB-D图像或对齐点云）下视觉观测的似然度。抽象视觉语言特征支持不同模态的观测与地图元素关联，通过自然语言描述初始化全局定位。

Result: 在室内场景（Matterport3D和Replica）和室外场景（SemanticKITTI）上评估方法，展示了良好的泛化能力。

Conclusion: 基于视觉语言特征的蒙特卡洛定位方法能够实现跨模态的鲁棒定位，支持自然语言描述的定位初始化，在室内外场景中均表现良好。
...
### [118] [An Open Toolkit for Underwater Field Robotics](https://arxiv.org/abs/2512.15597)
...
TL;DR: 本文介绍了一个开源、低成本的水下机器人关节硬件软件工具包，用于解决水下操纵研究中的高成本、专有设计和模块化硬件缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 水下机器人对海洋科学、环境监测和海底工业操作日益重要，但水下操纵和驱动系统的发展受到高成本、专有设计以及缺乏模块化研究导向硬件的限制。现有开源项目主要集中在车辆构建和控制软件，而关节驱动系统（特别是需要防水、带反馈的驱动器）存在明显空白，导致研究团队面临长开发周期、有限可重复性和难以将实验室原型转化为现场就绪平台的问题。

Method: 开发了一个开源、低成本的水下机器人关节（URJ）硬件软件工具包，包括：深度额定水下机器人关节（带早期泄漏检测）、紧凑的控制和电源管理电子设备、基于ROS2的软件堆栈（用于传感和多模式驱动）。所有CAD模型、制造文件、PCB源文件、固件和ROS2包都开源发布，支持本地制造、修改和社区驱动的改进。

Result: 该工具包经过广泛的实验室测试和多次现场部署，在40米深度范围内可靠运行，成功应用于多种应用：3自由度水下机械臂、肌腱驱动软夹持器和欠驱动沉积物采样器。这些结果验证了工具包在真实海洋环境中的鲁棒性、多功能性和可重用性。
...
### [119] [Attention as Binding: A Vector-Symbolic Perspective on Transformer Reasoning](https://arxiv.org/abs/2512.14709)
*Sahil Rajesh Dhayalkar*

Main category: cs.AI

TL;DR: 该论文提出将Transformer的自注意力机制和残差流解释为近似的向量符号架构(VSA)，用代数视角解释语言模型的推理行为及其失败模式，并提出VSA启发的架构改进方案。
...
Method: 将自注意力机制解释为VSA：查询和键定义角色空间，值编码填充物，注意力权重执行软解绑，残差连接实现多个绑定结构的叠加。基于此视角分析思维链、程序推理和记忆增强工具使用，并提出VSA启发的架构偏置（显式绑定/解绑头、超维记忆层）和训练目标。

Result: 建立了Transformer内部机制与向量符号计算之间的对应关系，解释了变量混淆和逻辑相关提示不一致等典型失败模式，提出了测量"VSA相似度"和逻辑组合性的指标。
...
### [120] [GR-Agent: Adaptive Graph Reasoning Agent under Incomplete Knowledge](https://arxiv.org/abs/2512.14766)
...
<details>
  <summary>Details</summary>
Motivation: 现有知识图谱问答（KGQA）基准测试大多假设知识图谱是完整的，这忽略了现实世界中知识图谱通常不完整的事实。当直接支持三元组缺失时，现有方法性能显著下降，突显了它们有限的推理能力。

Method: 1. 提出构建不完整知识图谱基准测试的方法论，移除直接支持三元组但保留替代推理路径；2. 开发自适应图推理智能体（GR-Agent），将KGQA形式化为智能体与环境交互，使用图推理工具作为动作空间，维护潜在支持推理证据的记忆。

Result: 实验表明：1. 现有方法在不完整知识图谱下性能持续下降；2. GR-Agent在完整和不完整设置下均优于非训练基线，与基于训练的方法表现相当。

Conclusion: 论文填补了不完整知识图谱评估的空白，提出的GR-Agent通过形式化智能体与环境交互的方法，有效提升了在不完整知识图谱上的推理能力。
...
### [121] [IaC Generation with LLMs: An Error Taxonomy and A Study on Configuration Knowledge Injection](https://arxiv.org/abs/2512.14792)
*Roman Nekrasov,Stefano Fossati,Indika Kumara,Damian Andrew Tamburri,Willem-Jan van den Heuvel*

Main category: cs.AI

TL;DR: 该研究通过结构化配置知识注入技术，将LLM生成Terraform代码的技术验证成功率从27.1%提升至75.3%，但发现意图对齐存在瓶颈，揭示了"正确性-一致性差距"。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在生成正确且意图对齐的基础设施即代码方面成功率较低，特别是在Terraform代码生成任务中表现不佳，需要系统性的改进方法。

Method: 1. 显著增强现有IaC-Eval基准，增加云仿真和自动错误分析；2. 开发LLM辅助IaC代码生成的新型错误分类法；3. 实施从朴素检索增强生成到更复杂的图RAG方法的知识注入技术，包括图组件的语义增强和资源间依赖关系建模。

Result: 基线LLM性能较差（总体成功率27.1%），注入结构化配置知识后，技术验证成功率提升至75.3%，总体成功率提升至62.6%。然而意图对齐出现平台期，表明LLM可以成为熟练的"编码者"但仍是有限的"架构师"。

Conclusion: 研究揭示了"正确性-一致性差距"：虽然结构化知识注入显著提高了技术正确性，但LLM在满足细微用户意图方面仍然受限，表明需要超越代码生成的架构理解能力。
...
### [122] [AgroAskAI: A Multi-Agentic AI Framework for Supporting Smallholder Farmers' Enquiries Globally](https://arxiv.org/abs/2512.14910)
*Nadine Angela Cantonjos,Arpita Biswas*

Main category: cs.AI

TL;DR: AgroAskAI是一个用于农业气候适应决策支持的多智能体推理系统，专门为脆弱农村社区设计，通过模块化角色架构和链式责任方法协调自主智能体，提供可操作、接地气且包容性的输出。
...
Method: 提出AgroAskAI多智能体推理系统，采用模块化、角色专业化架构，使用链式责任方法协调自主智能体，集成实时工具和数据集。系统内置治理机制减少幻觉，支持内部反馈以产生连贯、本地相关策略，并支持多语言交互。

Result: 在常见农业气候适应查询上的实验表明，通过额外工具和提示优化，AgroAskAI能够提供更可操作、更接地气、更具包容性的输出。实验结果突显了智能体AI在农业气候适应中可持续和负责任决策支持的潜力。

Conclusion: AgroAskAI展示了智能体AI在农业气候适应决策支持中的潜力，特别是通过多智能体协作推理、实时工具集成和治理机制，为脆弱农村社区提供可持续、负责任且包容的解决方案。
...
### [123] [Agentic AI for Integrated Sensing and Communication: Analysis, Framework, and Case Study](https://arxiv.org/abs/2512.15044)
...
Method: 首先全面回顾智能体AI和ISAC系统的关键特性；其次展示ISAC系统的常见优化方法，突出基于生成式AI的智能体AI的显著优势；然后提出新颖的智能体ISAC框架，并通过案例研究验证其优化ISAC性能的优越性。

Result: 提出的智能体ISAC框架在优化ISAC性能方面表现出优越性，案例研究验证了该框架的有效性。

Conclusion: 智能体AI为ISAC系统提供了重要的技术支持，能够实现智能、自主、高效的运行。未来需要进一步研究智能体AI在ISAC系统中的具体应用和发展方向。
...
TL;DR: 该论文提出了一种基于聚类的变量排序框架，用于改进离散优化中决策图（DD）的构建，通过将变量分区为聚类来减少动态排序的搜索空间，从而降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 离散优化中，决策图（DD）通过节点合并提供对偶界，但其质量严重依赖于变量排序和合并决策。虽然动态变量排序启发式方法能有效收紧界，但在整个变量集上全局评估会产生计算开销。需要一种方法来平衡界质量和计算成本。

Method: 提出基于聚类的变量排序框架：首先将变量分区为聚类，然后基于此结构分解指导排序过程。研究了两种策略：1) Cluster-to-Cluster：使用问题特定的聚合标准（如MWISP中的累积顶点权重）顺序处理聚类；2) Pick-and-Sort：迭代地从每个聚类中选择和排序代表性变量以平衡局部多样性和启发式指导。针对MWISP提出了两种设置聚类数量的策略，并将这些策略嵌入到基于DD的分支定界算法中。

Result: 在最大加权独立集问题（MWISP）的基准实例上，所提出的方法相比标准动态变量排序基线，持续降低了计算成本。

Conclusion: 基于聚类的变量排序框架通过减少动态排序启发式的搜索空间，有效地平衡了决策图质量和计算开销，为离散优化中的决策图构建提供了一种高效的方法。
...
TL;DR: 该研究使用2025年韩国高考地球科学I部分，分析了GPT-4o、Gemini 2.5 Flash和Gemini 2.5 Pro等多模态大语言模型在科学推理能力上的认知局限，发现模型存在感知-认知鸿沟、计算-概念化差异和过程幻觉等问题，为设计"AI抗性试题"提供了依据。


<details>
  <summary>Details</summary>
Motivation: 随着学生使用AI完成作业的普及，学术诚信和评估有效性受到威胁。研究旨在深入分析先进大语言模型的多模态科学推理能力和认知局限，为应对未经授权的AI使用提供解决方案，确保教育评估的公平性。

Method: 使用2025年韩国高考地球科学I部分作为评估材料，设计了三种实验条件：整页输入、单项输入和优化多模态输入。定量分析模型在不同数据结构下的表现，定性分析模型的认知错误类型，包括感知错误、感知-认知鸿沟、计算-概念化差异和过程幻觉。

Result: 非结构化输入导致显著的性能下降（由于分割和OCR失败）。即使在优化条件下，模型仍表现出基本推理缺陷。感知错误占主导地位，模型能识别视觉数据但无法解释示意图中的符号意义。模型能成功计算但无法应用基础科学概念，并存在跳过视觉验证而依赖无根据背景知识的过程幻觉。

Conclusion: 通过针对AI的特定认知弱点（如感知与认知之间的鸿沟），教育者可以设计"AI抗性试题"来区分真实学生能力与AI生成回答，从而确保评估公平性。研究为应对课程作业中未经授权AI使用的挑战提供了可操作的线索。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在提供行人路线指引方面存在局限性，特别是在使用定性空间关系进行导航时效果不佳，需要改进其空间推理能力

Method: 通过整合定性空间关系来增强大语言模型，可能包括空间关系表示、推理框架设计、模型训练或微调等方法

Result: 改进后的大语言模型能够更准确地生成包含定性空间关系的行人路线指引，提升导航效果

Conclusion: 通过整合定性空间关系，可以有效提升大语言模型在行人导航任务中的表现，为智能导航系统提供更好的支持
...
TL;DR: 提出一个通过自学习AI游戏扩展人类专家游戏数据库的通用框架，提高多玩家纸牌游戏早期决策的预测准确性，并在Skat游戏中验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 在多玩家纸牌游戏中，如Skat或桥牌，早期阶段（叫牌、游戏选择、初始牌选择）对游戏成功比中后期精细化玩法更为关键。当前计算限制下，这些早期决策依赖于从大量人类专家游戏中提取的统计信息，但数据有限。

Method: 提出一个通用的自学习外部学习框架，通过生成数百万个AI自玩游戏来扩展人类游戏数据库，生成和合并统计信息。使用完美特征哈希函数处理压缩表，创建自改进的纸牌游戏引擎，在自学习过程中持续改进新推断的知识。

Result: 在Skat游戏中的案例研究表明，该自动化方法可以支持游戏中的各种决策，提高了预测准确性。

Conclusion: 通过自学习AI游戏扩展人类专家数据库的框架能够有效提高多玩家纸牌游戏早期决策的准确性，为游戏引擎提供了持续自我改进的能力。
...
<details>
  <summary>Details</summary>
Motivation: 城市空中交通中，由于资源受限，垂直起降机场的高效调度受到广泛关注。传统调度方法难以同时处理动态操作需求和人类模糊的重调度请求，需要一种能够透明支持人类输入的解释性调度系统。

Method: 采用混合整数线性规划将调度问题建模为资源受限项目调度问题，并集成答案集编程。使用三值逻辑解释模糊用户意图，结合决策树，构建了一个能够优化调度并透明支持人类输入的集成系统。

Result: 提出的集成框架为城市空中交通调度提供了鲁棒的结构，能够处理动态操作需求和模糊的人类重调度请求，实现了可解释、自适应的调度系统。

Conclusion: 该研究成功开发了一个结合ASP和MILP的集成调度框架，能够有效处理城市空中交通中的动态调度需求，支持人类模糊输入，为可解释、自适应的调度系统提供了解决方案。
...
Method: 利用GPT-OSS-120B的多模式生成能力，创建包含高、中、低三种推理模式的7.5M解决方案轨迹数据集；整合85K AoPS竞赛题和262K StackExchange数学问题；开发顺序分桶策略加速长上下文训练。

Result: Nemotron-Math在匹配的AoPS问题上持续优于原始OpenMathReasoning；整合StackExchange-Math显著提高了鲁棒性和泛化能力；在AIME 2024和2025竞赛中实现了100% maj@16准确率。

Conclusion: Nemotron-Math通过大规模、多样化的数学推理数据，结合有效的训练策略，实现了数学推理任务的最先进性能，为数学AI研究提供了高质量的数据资源。
...
TL;DR: 论文提出了一个基于场景的科学发现评估框架，用于评估大语言模型在真实科学研究项目中的表现，发现当前模型在科学发现能力上存在系统性弱点，距离通用科学"超级智能"还很遥远。


<details>
  <summary>Details</summary>
Motivation: 现有科学基准测试主要评估去语境化的知识，而忽视了驱动科学发现的关键过程，如迭代推理、假设生成和观察解释。需要开发能更好评估LLMs科学发现能力的评估框架。

Method: 引入基于场景的基准测试，由领域专家定义真实研究项目，并将其分解为模块化研究场景，从中采样经过验证的问题。采用两阶段评估框架：问题级准确性和项目级表现评估。

Result: 应用该框架评估最先进的LLMs发现：相对于通用科学基准存在一致性能差距；模型规模和推理能力的扩展收益递减；不同提供商的顶级模型存在系统性弱点；模型选择因研究场景而异。

Conclusion: 当前所有LLMs距离通用科学"超级智能"还很遥远，但已显示出在各种科学发现项目中的潜力。该SDE框架为发现相关的LLMs评估提供了可复现基准，并指明了推进其科学发现能力发展的实践路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前价值对齐文献主要关注如何塑造AI的价值观，但较少研究在不确定性下如何确定不完美的对齐何时足够好以证明委托的合理性。需要一种原则性的方法来评估何时AI在特定情境下足够对齐。

Method: 引入了一个正式的决策理论框架，精确分析委托决策中的权衡，考虑委托人对AI价值对齐、认知准确性和行动范围的不确定性。开发了一个新颖的评分框架来量化这种事前决策。

Result: 分析揭示了两种委托情景的明显区别：通用委托需要近乎完美的价值对齐和完全的认知信任，而情境特定委托即使在显著不对齐的情况下也可能是最优的。AI的更高准确性或更广的行动范围可能提供更好的整体决策问题。
...
Conclusion: STC框架通过统一推理和自我批判，使LLM具备类似人类的批判性思维能力，为构建具有内置批判性思维的大语言模型迈出了一步。
...
TL;DR: CAGE框架通过引入属性图来改进LLM的上下文归因，量化每个生成内容如何受提示和先前生成内容的影响，相比现有方法提高了40%的忠实度。
...
Method: 提出CAGE框架，引入属性图：一个有向图，量化每个生成内容如何受提示和所有先前生成内容的影响。该图保持因果性和行随机性，通过沿图中路径边缘化中间贡献来计算上下文归因。
