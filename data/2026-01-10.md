...
TL;DR: 该论文提出了一种用于扩散模型的两阶段对齐框架，通过构建层次化细粒度评估标准，并引入复杂偏好优化（CPO）方法，显著提升了图像生成质量与专家知识的对齐程度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的后训练对齐方法依赖于简化的信号（如标量奖励或二元偏好），这限制了与复杂人类专业知识（具有层次化和细粒度特性）的对齐能力。
...
Conclusion: 该研究提出的层次化细粒度对齐框架能够有效解决现有扩散模型对齐方法的局限性，通过复杂偏好优化实现了与复杂人类专业知识的更好对齐，在绘画生成等专业领域具有重要应用价值。
...
TL;DR: 提出一种基于RGB空间五进制像素强度组合的文本嵌入图像新方法，将文本符号映射到125种像素组合，实现单像素编码，相比传统方法更高效且失真小。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入图像方法（如LSB、MSB、PVD、变换域、深度学习等）存在多个问题：依赖多像素强度翻转、产生噪声、编解码确定性高、计算复杂度高，特别是深度学习和生成式AI方法计算负担重。

Method: 在RGB空间中采用五进制像素强度组合技术，每个R、G、B通道使用五种受控的像素强度变化，形成125种不同的像素强度组合。将这些组合映射到文本符号（大小写字母、数字、空格、常用特殊字符），实现单像素编码。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，原始图像和编码图像之间没有显著失真。该方法实现了更高的嵌入效率，单个RGB像素即可编码完整文本符号。

Conclusion: 提出的五进制像素强度组合方法在RGB空间中实现了高效的文本嵌入，相比传统LSB/MSB方法（需要多像素或多步骤）以及变换和学习方法（计算开销大），具有更好的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成架构通常依赖显式模态切换，先生成推理文本再手动切换到图像生成，这种分离的顺序推理过程限制了跨模态耦合，阻碍了自动多模态生成。

Method: 采用离线、奖励加权的后训练方法，使用完全自生成的合成数据，探索不同的后训练数据策略，研究联合文本-图像生成对T2I性能的影响以及各模态在后训练中的相对重要性。

Result: 通过针对特定限制的有针对性数据集，相比广泛的图像-标题语料库或基准对齐数据获得更优结果，在四个不同的T2I基准测试中实现了多模态图像生成的改进。

Conclusion: 奖励加权两种模态和策略性设计的后训练数据是有效的，能够实现完全统一的文本-图像生成，使模型在单一推理过程中自主完成从文本推理到视觉合成的过渡。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的视频扩散模型虽然实现了最先进的视频生成，但存在二次注意力复杂度问题，严重限制了长序列的可扩展性。需要一种既能保持生成质量又能降低计算成本的方法。

Method: 提出ReHyAt循环混合注意力机制，结合softmax注意力的保真度和线性注意力的效率，支持分块循环重构和恒定内存使用。通过轻量级蒸馏和微调流程，可以从现有的softmax模型高效蒸馏，大幅降低训练成本。

Result: 在VBench和VBench-2.0上的实验以及人类偏好研究表明，ReHyAt在保持最先进视频质量的同时，将注意力成本从二次降低到线性。训练成本降低两个数量级至约160 GPU小时，实现了长时长和设备端视频生成的实用可扩展性。

Conclusion: ReHyAt通过混合注意力设计，在保持视频生成质量的同时显著降低了计算复杂度，为未来最先进的双向softmax模型提供了一种可扩展的蒸馏和微调方案，解锁了长视频生成的实用应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅模型存储需求大，阻碍云和流媒体部署；标量量化方法无法有效捕捉高维特征向量相关性，限制了率失真性能

Method: 使用残差向量量化替代传统方法压缩基元特征；提出基于多分辨率哈希网格引导的自回归熵模型，准确预测每个传输索引的条件概率

Result: 实现了粗化和细化层的高效压缩，提高了压缩效率

Conclusion: 该方法为3D高斯泼溅模型提供了一种更有效的渐进式压缩方案，改善了率失真性能
...
<details>
  <summary>Details</summary>
Motivation: 统一自回归模型在处理多模态任务时使用KV缓存机制减少计算复杂度，但缓存大小随生成令牌数量线性增长，成为限制推理效率和生成长度的主要瓶颈，特别是在视频生成任务中这一问题尤为突出。
...
TL;DR: 该研究提出了一种基于3D面部几何和跨模态注意力融合的压力识别方法，在分心驾驶场景中取得了92%的AUROC性能


<details>
  <summary>Details</summary>
Motivation: 现有压力识别方法主要依赖面部动作单元，而解耦的3D面部几何特征的作用尚未充分探索。由于压力的主观性和面部表情的自主控制，从面部视频中可靠识别压力具有挑战性。

Method: 使用EMOCA提取3D表情和姿态系数，通过配对假设检验分析压力响应；提出基于Transformer的时间建模框架，评估了单模态、早期融合和跨模态注意力三种融合策略。

Result: 56个系数中有41个在压力和基线阶段表现出显著差异；跨模态注意力融合EMOCA和生理信号达到最佳性能（AUROC 92%，准确率86.7%），EMOCA-凝视融合也表现良好（AUROC 91.8%）。

Conclusion: 研究证明了3D面部几何特征在压力识别中的有效性，以及时间建模和跨模态注意力融合策略的优越性，为可靠的压力识别提供了新方法。
...
### [8] [Few-Shot LoRA Adaptation of a Flow-Matching Foundation Model for Cross-Spectral Object Detection](https://arxiv.org/abs/2601.04381)
*Maxim Clouser,Kia Khezeli,John Kalantari*

Main category: cs.CV

TL;DR: 研究探索了基于RGB预训练的流匹配基础模型能否通过少量配对样本适应为跨光谱翻译器，并验证合成数据对下游检测任务的增强效果。通过在FLUX.1 Kontext模型中插入LoRA模块，仅用100对图像进行微调，实现了RGB到红外和SAR的转换，并证明LPIPS指标能有效预测下游检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型主要基于RGB数据训练，而许多安全关键应用依赖红外和合成孔径雷达等非可见光模态。研究旨在探索单一RGB预训练模型能否通过少量配对样本适应为跨光谱翻译器，并验证合成数据对下游检测任务的增强效果。

Method: 从FLUX.1 Kontext流匹配基础模型出发，插入低秩适应（LoRA）模块，仅使用每个域100对配对图像进行微调。研究两个设置：KAIST数据集上的RGB到红外转换，以及M4-SAR数据集上的RGB到SAR转换。通过LoRA超参数网格搜索，使用LPIPS指标评估翻译质量，并验证其与下游检测性能的相关性。

Result: 研究发现LPIPS指标仅用50对保留样本就能强有力地预测下游性能：较低的LPIPS值一致预测YOLOv11n在红外和SAR上更高的mAP，以及DETR在KAIST红外测试数据上的更好表现。使用最佳LPIPS选择的LoRA适配器，从外部RGB数据集生成的合成红外数据提升了KAIST红外行人检测，合成SAR数据与有限真实SAR结合显著提升了M4-SAR基础设施检测性能。

Conclusion: 研究表明，流匹配基础模型的少样本LoRA适应是实现非可见光模态基础模型支持的有前景路径。这种方法能够将RGB预训练模型有效适应为跨光谱翻译器，生成的合成数据能显著增强下游检测任务性能。
...
Method: 设计并训练一个自定义CNN，同时使用VGG-16、ResNet-50和MobileNet等预训练架构进行迁移学习。在相同的实验设置下，使用准确率、精确率、召回率和F1分数等标准指标评估所有模型。
...
TL;DR: Tri MARF是一个用于大规模3D对象标注的新框架，通过整合三模态输入（2D多视图图像、文本描述和3D点云）和多智能体协作架构，显著提升了3D标注的性能和效率。
...
Method: 提出Tri MARF框架，包含三个专门化智能体：视觉语言模型智能体生成多视图描述、信息聚合智能体选择最优描述、门控智能体将文本语义与3D几何对齐以进行精细化标注。

Result: 在Objaverse、LVIS、Objaverse XL和ABO数据集上的实验表明，Tri MARF显著优于现有方法，CLIPScore达到88.7，ViLT R@5检索准确率达到45.2和43.8，单块NVIDIA A100 GPU上吞吐量高达每小时12000个对象。

Conclusion: Tri MARF通过多模态融合和多智能体协作有效解决了3D对象标注的挑战，在准确性和效率方面都取得了显著提升，为大规模3D标注任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗手术中，乳突切除术是重要步骤，准确的形状预测能改善术前规划、降低风险并提高手术效果。但由于获取真实标签的挑战，目前基于深度学习的研究有限，需要解决这一空白。

Method: 提出了一种混合自监督和弱监督学习框架，直接从术前CT扫描预测乳突切除术区域，无需人工标注。该方法在弱监督医学成像中利用了3D T分布损失。

Result: 该方法在预测复杂且无边界乳突切除术形状时达到平均Dice分数0.72，超越了最先进方法，表现出强大性能。
...
TL;DR: 提出CRUNet-MR-Univ基础模型，通过时空相关性和提示先验处理CMR扫描的多样性，提升泛化能力


<details>
  <summary>Details</summary>
Motivation: 深度学习在心脏MRI重建中表现出色，但现有方法泛化能力有限，难以应对CMR扫描中的图像对比度、采样模式、扫描仪厂商、解剖结构和疾病类型等多样性变化

Method: 提出CRUNet-MR-Univ基础模型，利用时空相关性和基于提示的先验知识，有效处理CMR扫描的完整多样性

Result: 该方法在各种设置下始终优于基线方法，证明了其有效性和前景

Conclusion: CRUNet-MR-Univ作为基础模型能够泛化到不同的CMR场景，为临床实际应用提供了有前景的解决方案
...
TL;DR: GPRO提出了一种元推理控制器，通过动态路由计算路径（快速路径、慢速感知路径、慢速推理路径）来解决大型视觉语言模型中的过度思考问题，同时优化任务准确性和计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型采用链式思维机制时会产生过度思考问题，导致对简单查询也生成冗长响应，造成测试时效率低下甚至准确性下降。现有自适应推理策略主要忽略了根本瓶颈：视觉感知失败。作者认为稳定的推理关键依赖于低层视觉基础，推理错误往往源于不完美的感知而非不足的深思熟虑。

Method: 提出了门控感知-推理优化（GPRO），这是一种元推理控制器，在每个生成步骤动态路由计算到三个决策路径：轻量级快速路径、用于重新检查视觉输入的慢速感知路径、以及用于内部自我反思的慢速推理路径。通过从约79万个样本中获取大规模失败归因监督，使用教师模型区分感知幻觉和推理错误。然后通过多目标强化学习训练控制器，在不确定性下优化任务准确性和计算成本之间的权衡。

Result: 在五个基准测试上的实验表明，GPRO显著提高了准确性和效率，优于最近的慢思考方法，同时生成显著更短的响应。

Conclusion: GPRO通过动态路由计算路径有效解决了大型视觉语言模型的过度思考问题，证明了稳定的推理依赖于低层视觉基础，并且通过区分感知错误和推理错误可以同时优化准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统通常将感知、预测和规划作为独立模块处理，缺乏紧密集成。本文旨在探索统一的VLM世界模型，通过联合执行这些任务来提升自动驾驶性能。

Method: 提出UniDrive-WM统一架构，包含轨迹规划器和VLM图像生成器。轨迹规划器预测未来轨迹，该轨迹作为条件输入图像生成器产生未来帧，这些预测提供额外监督信号，迭代优化轨迹生成。同时比较了离散和连续输出表示对未来图像预测的影响。
...
<details>
  <summary>Details</summary>
Motivation: 三维医学图像分割计算需求大，存在体素处理立方增长和同质区域冗余计算的问题，需要开发更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个简单高效的纯Transformer架构，用于端到端自动驾驶，通过压缩多相机特征减少计算量，同时保持准确性，并实现可解释的行为条件驾驶。

Method: 基于预训练Vision Transformers构建，引入相机感知寄存器token压缩多相机特征，使用两个轻量级Transformer解码器：一个生成候选轨迹，另一个模仿oracle评分轨迹（包含安全性、舒适性、效率等可解释子分数）。

Result: 在NAVSIM-v1、NAVSIM-v2和HUGSIM基准测试中优于或匹配现有强基线，证明纯Transformer架构结合token压缩能够实现准确、高效、自适应的端到端驾驶。

Conclusion: 纯Transformer架构结合有针对性的token压缩足以实现准确、高效和自适应的端到端自动驾驶，DrivoR展示了这种方法的有效性。
...
### [17] [RoboVIP: Multi-View Video Generation with Visual Identity Prompting Augments Robot Manipulation](https://arxiv.org/abs/2601.05241)
...
TL;DR: 该论文提出视觉身份提示方法，通过提供示例图像作为条件输入来指导扩散模型生成所需的场景设置，从而增强机器人操作数据，提升下游策略模型的性能。


<details>
  <summary>Details</summary>
Motivation: 由于硬件和物理设置限制，收集大规模真实世界操作数据难以在不同环境中扩展。现有基于文本提示的图像扩散模型方法往往忽略多视角和时间一致性需求，且文本提示无法可靠指定场景设置。
...
TL;DR: 使用3D条件生成模型（Pix2Pix GAN、SPADE-GAN、SPADE-LDM）合成LGE MRI图像以增强训练数据，提升左心房分割性能，其中SPADE-LDM效果最佳。
...
Method: 开发了一个从复合语义标签图合成高保真3D LGE MRI体积的流程，使用了三种3D条件生成器：Pix2Pix GAN、SPADE-GAN和SPADE-LDM。标签图结合了专家解剖标注和无监督组织聚类。

Result: SPADE-LDM生成最真实和结构准确的图像（FID=4.063），优于Pix2Pix GAN（FID=40.821）和SPADE-GAN（FID=7.652）。使用合成图像增强训练数据后，3D U-Net的左心房腔分割Dice分数从0.908提升至0.936，具有统计学显著改善（p<0.05）。
...
TL;DR: 本文提出了MiLDEAgent框架，用于解决多图层设计文档编辑问题，并建立了MiLDEBench基准数据集和MiLDEEval评估协议。实验表明现有方法难以处理多图层编辑任务，而MiLDEAgent在图层感知推理和精确编辑方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的设计文档（如海报）本质上是多层结构，包含装饰、文本和图像。从自然语言指令编辑这些文档需要细粒度的图层感知推理，以识别相关图层并协调修改。先前的研究大多忽略了多图层设计文档编辑，主要关注单图层图像编辑或多图层生成，这些方法假设平面画布，缺乏确定修改内容和位置所需的推理能力。

Method: 提出了Multi-Layer Document Editing Agent (MiLDEAgent)，这是一个基于推理的框架，结合了经过强化学习训练的多模态推理器（用于图层级理解）和图像编辑器（用于针对性修改）。同时建立了MiLDEBench基准数据集（包含超过20K个设计文档和多样化编辑指令）和MiLDEEval评估协议（涵盖指令遵循、布局一致性、美学和文本渲染四个维度）。

Result: 在14个开源模型和2个闭源模型上的广泛实验表明：现有方法难以泛化——开源模型通常无法完成多图层文档编辑任务，而闭源模型存在格式违规问题。相比之下，MiLDEAgent实现了强大的图层感知推理和精确编辑，显著优于所有开源基线，性能与闭源模型相当，从而为多图层文档编辑建立了第一个强基线。

Conclusion: MiLDEAgent框架成功解决了多图层设计文档编辑的挑战，通过结合图层感知推理和针对性修改能力，在多个评估维度上表现出色。该工作为这一新兴领域建立了首个全面的基准和评估协议，为后续研究提供了重要基础。
...
TL;DR: 论文提出一个评估AI赋能人本信息物理系统在不确定条件下安全性的框架，并以糖尿病闭环血糖控制中的未申报进餐检测为例进行验证。


<details>
  <summary>Details</summary>
Motivation: AI赋能的人本信息物理系统（如医疗监控、自动驾驶）在运行中可能遇到不确定条件，导致系统进入未知状态，这可能违反系统的安全和安保要求。需要建立框架来评估确保这些系统安全运行的不同策略。

Method: 首先讨论可能导致系统在不确定条件下运行的操作偏差，然后创建一个评估框架来评估不同安全策略，最后以1型糖尿病闭环血糖控制中的未申报进餐检测为例，展示一种基于图像的个性化检测技术。

Result: 论文提出了一个系统性框架来评估AI赋能人本信息物理系统的安全策略，并通过具体的医疗应用案例（糖尿病闭环血糖控制中的未申报进餐检测）展示了该框架的实际应用价值。

Conclusion: AI赋能的人本信息物理系统在部署运行中面临不确定条件带来的安全挑战，需要系统性的评估框架来确保其安全运行，论文提出的框架和具体案例为此类系统的安全设计提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 头颈部危及器官的准确分割对放疗至关重要，但深度学习模型在小而复杂的器官上表现不佳。现有混合架构只是简单拼接特征，未能充分利用各组件优势，导致功能重叠和分割精度有限

Method: 提出高不确定性区域引导的多架构协同学习模型：1) 使用CNN自适应识别高不确定性区域；2) 在高不确定性区域同时使用Vision Mamba和Deformable CNN提升分割精度；3) 提出异质特征蒸馏损失促进两种架构在高不确定性区域的协同学习
...
TL;DR: 提出HyperAlign框架，基于双曲蕴含几何自适应评估文本到图像生成的对齐质量，解决现有方法依赖欧氏空间度量、缺乏自适应能力的问题。
...
Result: HyperAlign在单数据库评估和跨数据库泛化任务上都取得了极具竞争力的性能，充分验证了双曲几何建模在图像文本对齐评估中的有效性。

Conclusion: 提出的基于双曲蕴含几何的自适应文本到图像对齐评估框架HyperAlign，通过将语义对齐建模为连续几何结构，并引入自适应调制机制，显著提升了评估性能，为解决文本到图像生成的对齐评估问题提供了新思路。
...
Method: 提出Agri-R1框架：1) 通过视觉语言合成和LLM过滤自动化生成高质量推理数据，仅使用19%可用样本；2) 采用Group Relative Policy Optimization训练，结合领域特定词典和模糊匹配的新奖励函数，评估开放回答的正确性和语言灵活性。

Result: 在CDDMBench评估中，3B参数模型性能与7B-13B基线相当：疾病识别准确率相对提升23.2%，农业知识问答提升33.3%，跨域泛化比标准微调提高26.10分。消融研究证实结构化推理数据与GRPO探索的协同效应，问题复杂度越高收益越大。

Conclusion: Agri-R1通过自动化推理数据生成和GRPO驱动的探索，有效解决了农业疾病诊断中标注成本高、泛化能力差的问题，在保持较小模型规模的同时实现了与更大模型竞争的性能。
...
TL;DR: HATIR提出了一种用于湍流红外视频超分辨率的扩散模型，通过注入热感知变形先验来联合建模湍流退化和结构细节损失，解决了现有方法忽略红外-可见光模态差异和湍流失真恢复不足的问题。


<details>
  <summary>Details</summary>
Motivation: 红外视频在恶劣环境下具有重要应用价值，但常受严重大气湍流和压缩退化影响。现有视频超分辨率方法要么忽略了红外与可见光图像之间的固有模态差异，要么无法有效恢复湍流引起的失真。直接将湍流缓解算法与VSR方法级联会导致误差传播和累积，因为湍流和分辨率退化是解耦建模的。

Method: HATIR采用热感知扩散模型，将热感知变形先验注入扩散采样路径，联合建模湍流退化和结构细节损失的逆过程。具体包括：1) 基于物理原理构建相量引导流估计器，利用热活跃区域随时间呈现一致相量响应的特性，生成可靠的湍流感知流来指导反向扩散过程；2) 提出湍流感知解码器，通过湍流门控和结构感知注意力机制，选择性抑制不稳定时间线索并增强边缘感知特征聚合。

Result: 构建了首个湍流红外视频超分辨率数据集FLIR-IVSR，包含来自FLIR T1050sc相机(1024×768)的640个不同场景的LR-HR配对序列，涵盖各种相机和物体运动条件。该方法在湍流红外视频超分辨率任务上表现出色。

Conclusion: HATIR通过联合建模湍流退化和分辨率损失的逆过程，有效解决了红外视频超分辨率中的湍流失真问题，为红外VSR研究提供了新的解决方案和基准数据集。
...
<details>
  <summary>Details</summary>
Motivation: AI生成内容的快速发展加剧了深度伪造的威胁，但现有检测方法要么专注于面部区域伪造，要么专注于全身合成图像，无法泛化到整个人类图像操纵的完整范围。

Method: 提出HuForDet框架，包含：1）面部伪造检测分支，在RGB和频域使用异构专家，包括自适应LoG模块；2）上下文伪造检测分支，利用多模态大语言模型分析全身语义一致性，并带有置信度估计机制。还构建了统一的人类图像伪造数据集。

Result: 广泛的实验表明，HuForDet在各种人类图像伪造检测中实现了最先进的性能，并表现出卓越的鲁棒性。

Conclusion: HuForDet通过双分支架构和统一数据集，为人类图像伪造检测提供了一个全面有效的解决方案，能够处理从面部到全身的各种伪造类型。
...
TL;DR: 该研究评估了CNN架构在五个异构数据集上的性能，包括农业和城市领域的视觉分类任务，比较了定制CNN与ResNet-18、VGG-16等经典架构，分析了架构复杂度、模型深度和迁移学习在不同数据条件下的影响。


<details>
  <summary>Details</summary>
Motivation: 深度学习特别是CNN已经改变了视觉数据分析方式，能够自动提取层次化视觉特征。然而，在现实世界的异构数据集（如农业和城市领域）中，面临光照、分辨率、环境复杂性和类别不平衡等挑战，需要评估不同CNN架构在这些复杂条件下的适应性和鲁棒性。

Method: 研究采用系统化方法：1）在五个异构数据集上进行实验（芒果品种分类、水稻品种识别、路面状况评估、三轮车检测、人行道侵占监测）；2）评估轻量级定制CNN与ResNet-18、VGG-16等经典架构；3）比较从头训练和迁移学习两种策略；4）通过系统预处理、数据增强和受控实验分析架构复杂度、模型深度和预训练的影响。

Result: 1）开发的高效定制CNN在多个应用领域实现了有竞争力的性能；2）全面比较分析揭示了迁移学习和深度架构在数据受限环境中的优势；3）研究提供了关于何时使用迁移学习和深度架构的实用见解，特别是在资源有限但影响重大的现实世界视觉分类任务中。

Conclusion: 该研究为在资源有限但高影响力的现实世界视觉分类任务中部署深度学习模型提供了实用指导。轻量级定制CNN在多个领域表现良好，而迁移学习和深度架构在数据受限环境中具有显著优势。这些发现有助于在实际应用中根据数据特性和资源约束选择合适的CNN策略。
...
Method: 引入基于骨架化的新型对抗攻击方法，有效减少搜索空间；针对文本图像（特别是数学公式图像）进行攻击；评估字符和语义变化

Result: 方法在ChatGPT上得到验证，展示了实际应用价值；通过详细评估原始输出和对抗扰动输出的字符和语义变化，提供了对模型视觉解释和推理能力的深入洞察

Conclusion: 提出的骨架化对抗攻击方法有效揭示了基础模型在视觉解释特别是文本/数学公式处理方面的能力和局限性，具有实际应用意义
...
<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇3D场景理解方法通常依赖预训练的3DGS场景，存在跨视图不一致和掩码内聚性不足的问题，且需要额外的优化和微调，计算成本高。

Method: 引入密集对应引导的预注册阶段，通过跨视图聚类初始化高斯并构建3D上下文提议；每个提议通过加权聚合成员嵌入获得全局特征，在直接注册过程中将特征融合到高斯上，保持跨视图的语言一致性；无需额外优化，保留几何细化而不需要密集化。

Result: ProFuse实现了强大的开放词汇3DGS理解能力，每个场景的语义附着完成时间约为5分钟，比现有最先进技术快2倍。

Conclusion: ProFuse通过上下文感知的直接注册框架，在保持几何精度的同时实现了高效、一致的开放词汇3D场景理解，显著提升了处理速度和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM在多图像视觉定位方面存在局限性，只能处理单目标定位和有限类型的实际任务，缺乏对广义定位任务的统一建模能力。

Method: 1. 系统分类和组织现有多图像定位任务；2. 构建MG-Data-240K数据集解决目标数量和图像关系限制；3. 提出混合强化微调策略，结合思维链推理和直接回答，采用R1-like算法和基于规则的奖励机制。

Result: 1. 多图像定位：在MIG-Bench和MC-Bench上分别比先前领先的MLLM提升2.0%和9.7%；2. 单图像定位：在ODINW上比基础模型提升9.1%；3. 保持强大的通用多图像理解能力。

Conclusion: GeM-VG通过统一建模广义多图像视觉定位任务，结合系统任务分类、大规模数据集和混合强化微调策略，显著提升了MLLM在多样化定位任务中的感知和推理能力。
...
TL;DR: 该论文提出了一个可扩展的反事实视频生成框架CounterVid，用于缓解视频语言模型中的幻觉问题，特别是动作和时序推理方面的幻觉。通过生成仅改变动作或时间结构但保持场景上下文的反事实视频，构建包含约26k偏好对的合成数据集，并引入MixDPO统一直接偏好优化方法，在Qwen2.5-VL上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型在多模态理解方面表现出色，但在动作和时序推理方面仍然容易出现幻觉。现有的缓解策略（如文本过滤或随机视频扰动）往往未能解决根本原因：过度依赖语言先验而非细粒度的视觉动态信息。

Method: 提出了一个可扩展的反事实视频生成框架，结合多模态LLM进行动作提案和编辑指导，以及基于扩散的图像和视频模型，大规模生成语义硬负样本。使用该框架构建了CounterVid数据集（约26k偏好对），并引入了MixDPO统一直接偏好优化方法，联合利用文本和视觉偏好。

Result: 使用MixDPO对Qwen2.5-VL进行微调，在时序排序方面取得了持续改进，并有效迁移到标准视频幻觉基准测试中。代码和模型将公开提供。

Conclusion: 提出的反事实视频生成框架和MixDPO方法有效缓解了视频语言模型在动作和时序推理方面的幻觉问题，通过生成针对性的硬负样本和统一的偏好优化策略，显著提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 从2D图像准确估计深度是3D恢复领域的根本挑战。虽然散焦提供了重要的深度信息，但传统启发式方法面临空间变化散焦模糊的欠定问题。高斯模型因其数学简洁性和计算效率成为实时应用的最优选择，但需要验证其在真实成像设备中的适用性。

Method: 论文在几何光学框架下进行散焦分析，并通过衍射极限光学中的散焦像差理论，验证实际模型与其高斯近似的拟合精度。研究为传统成像设备设置了确保散焦算子遵循高斯模型的参数条件。

Result: 对于典型聚焦深度范围1-100米、最大深度变化为聚焦深度10%的情况，高斯模型在大多数成像设备中适用。结果显示最大平均绝对误差(MAE)小于1%，证明了模型的准确性和可靠性。

Conclusion: 高斯模型是描述成像设备散焦算子的准确模型，其最大误差小于1%，为基于散焦的实时深度估计提供了可靠的理论基础，解决了传统方法中散焦模糊难以区分的难题。
...
TL;DR: 提出增强版Pix2Pix框架，集成SEResNet和U-Net++，在少量样本下提升MRI图像翻译质量


<details>
  <summary>Details</summary>
Motivation: MRI临床应用受限于采集时间长、成本高、分辨率受限，图像翻译技术可解决这些限制。虽然Pix2Pix已广泛用于医学图像翻译，但其潜力尚未充分挖掘。

Method: 提出增强版Pix2Pix框架，集成Squeeze-and-Excitation Residual Networks (SEResNet)和U-Net++。SEResNet通过通道注意力机制增强关键特征表示，U-Net++提升多尺度特征融合。使用简化的PatchGAN判别器稳定训练并优化局部解剖真实性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于潜在扩散模型的零样本逆问题求解器存在不稳定性问题，会产生不良伪影和质量下降，需要更稳定的解决方案

Method: 首先识别不稳定性源于求解器与真实反向扩散动力学之间的差异，然后提出测量一致的Langevin校正器（MCLC），这是一个理论基础的即插即用校正模块，通过测量一致的Langevin更新来修复LDM逆求解器

Result: MCLC在多种图像恢复任务中表现出有效性，并与现有求解器兼容，相比依赖线性流形假设的方法更稳定可靠

Conclusion: MCLC是迈向更鲁棒零样本逆问题求解器的关键步骤，能够解决现有方法的不稳定性问题
...
<details>
  <summary>Details</summary>
Motivation: 现有的开源金字塔视频模型从头训练，在视觉合理性方面落后于最先进系统，且计算成本较高。需要一种方法能够利用预训练模型优势，在保持质量的同时降低推理计算成本。

Method: 提出一个将预训练扩散模型转换为金字塔模型的管道，通过低成本微调实现转换而不降低输出视频质量。同时研究比较了金字塔模型内的各种步数蒸馏策略。

Result: 成功将预训练扩散模型转换为金字塔模型，在保持输出视频质量的同时显著降低了推理计算成本。通过步数蒸馏策略进一步提升了推理效率。

Conclusion: 该方法提供了一种高效的金字塔模型转换方案，能够在保持视觉质量的前提下显著降低计算成本，为视频生成模型的效率优化提供了新思路。
...
TL;DR: 本文首次系统评估了SAMURAI基础模型在无人机跟踪中的应用，并提出检测器增强扩展以提升在复杂城市环境中的鲁棒性，特别是在长时间序列和无人机离开-重入场景下表现显著改善。


<details>
  <summary>Details</summary>
Motivation: 无人机跟踪对现代监控系统至关重要，但现有RGB跟踪方法有限且依赖传统运动模型。基础模型SAMURAI在其他领域表现出强大的类别无关跟踪性能，但其在无人机特定场景的适用性尚未研究。

Method: 提出检测器增强的SAMURAI扩展，通过结合检测器线索来减轻对边界框初始化和序列长度的敏感性，提升在复杂城市监控环境中的鲁棒性。

Result: 提出的扩展显著改善了复杂城市环境中的鲁棒性，特别是在长时间序列和无人机离开-重入事件中。检测器线索的加入使成功率提升高达+0.393，误报率降低高达-0.475。

Conclusion: 这是首次系统评估SAMURAI在无人机跟踪中的应用，检测器增强扩展有效提升了基础模型在无人机特定场景的性能，为鲁棒无人机跟踪提供了新解决方案。
...
TL;DR: 本文提出了一种基于二值化和互补预处理技术的古代文字图像增强方法，用于去除污渍并增强模糊的古代文字，在石头、金属板和文献三种古代文字上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 古代文字图像常因老化、环境因素等导致严重背景噪声、低对比度退化，前景文字与背景视觉特征相似，难以阅读。需要提高这类退化古代图像的可读性。
...
Conclusion: 提出的增强方法能有效提高古代马拉地语铭文图像的可读性，证明了该方法在处理不同类型古代文字图像方面的有效性。
...
TL;DR: SOVABench：基于监控视频的车辆动作检索基准，评估模型对车辆相关动作的识别能力，提出基于MLLM的无训练框架生成可解释嵌入


<details>
  <summary>Details</summary>
Motivation: 现有视频检索基准主要关注场景级相似性，缺乏对监控场景中动作识别的评估，特别是车辆相关动作的判别能力

Method: 1. 构建SOVABench基准，包含两种评估协议（inter-pair和intra-pair）；2. 提出基于多模态大语言模型的无训练框架，通过MLLM生成描述并提取可解释嵌入

Result: 1. 现有最先进的视觉和多模态模型在动作区分上仍面临挑战；2. 提出的MLLM框架在SOVABench上表现优异，同时在对比视觉语言模型常失败的空间和计数基准上也取得良好效果

Conclusion: SOVABench填补了监控视频动作识别评估的空白，基于MLLM的无训练框架为视频理解提供了可解释且有效的解决方案，代码和基准数据已开源
...
TL;DR: 该研究使用YOLO目标检测模型替代传统的模板匹配和CNN方法，用于中世纪手稿的抄写员识别，通过检测字母"a"来区分不同抄写员，取得了更好的识别效果。


<details>
  <summary>Details</summary>
Motivation: 古文书学研究中，确定手稿的写作年代和追踪书写风格演变需要识别抄写员。虽然数字技术已有进展，但通用问题仍未解决。先前基于模板匹配和CNN的方法存在阈值设置等限制，需要更先进的检测技术。

Method: 采用YOLOv5目标检测模型完全替代先前工作中的模板匹配和CNN方法，专门检测中世纪手稿中的字母"a"，然后进行第二阶段分类来识别不同抄写员。YOLO的置信度分数为拒绝阈值系统提供了基础。

Result: 实验结果表明，YOLO能有效提取更多被考虑的字母，从而获得更准确的第二阶段分类。YOLO的置信度分数使得开发具有拒绝阈值的系统成为可能，即使在未见过的抄本中也能实现可靠的抄写员识别。

Conclusion: YOLO目标检测模型在古文书学抄写员识别任务中优于传统的模板匹配和CNN方法，能够提取更多特征并提高识别准确性，为处理未知手稿提供了更可靠的解决方案。
...
TL;DR: DivAS是一种无需优化的交互式NeRF分割框架，使用2D SAM掩码和NeRF深度先验，通过自定义CUDA内核实现实时3D体素聚合，比优化方法快2-2.5倍。
...
Method: DivAS采用快速GUI工作流程：1) 从用户点提示生成2D SAM掩码；2) 使用NeRF深度先验优化掩码以提高几何精度和前景-背景分离；3) 通过自定义CUDA内核将优化的多视角掩码聚合到统一的3D体素网格中（<200ms）。

Result: 在Mip-NeRF 360°和LLFF数据集上的实验表明，DivAS的分割质量与基于优化的方法相当，端到端速度快2-2.5倍，排除用户提示时间后快一个数量级。

Conclusion: DivAS提供了一个无需优化的交互式NeRF分割框架，通过结合2D基础模型和NeRF深度先验，实现了实时视觉反馈和高质量分割，显著提升了效率。
...
TL;DR: 该论文提出了一个面向制药领域长视频理解的工业级GenAI框架，分析了40多个视觉语言模型在GPU、延迟和成本约束下的实际表现，揭示了多模态、注意力机制、时间推理等方面的实用限制和权衡。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型大多在短视频和无约束计算资源下评估，而工业场景（如制药内容理解）需要处理长视频并面临严格的GPU、延迟和成本约束，现有方法难以扩展。

Method: 提出了一个工业级大规模多模态推理架构，处理了超过20万份PDF、2.5万个视频和888个多语言音频文件；在Video-MME和MMBench两个基准测试以及包含14个疾病领域的专有数据集上，对40多个VLMs进行了实证分析。

Result: 在商用GPU上使用SDPA注意力机制获得了3-8倍的效率提升；多模态在8/12个任务领域（特别是长度依赖任务）中提升了性能；揭示了开源和闭源VLMs在时间对齐和关键帧检测方面的瓶颈。

Conclusion: 该研究没有提出新的"A+B"模型，而是刻画了当前VLMs在现实部署约束下的实际限制、权衡和失败模式，为研究者和从业者设计可扩展的工业级长视频多模态系统提供了实用指导。
...
TL;DR: 论文发现现有文本到图像评估指标存在原型性偏差，即倾向于选择视觉上典型但语义错误的图像，而非语义正确但不典型的图像。作者创建了ProtoBias基准测试来量化这一问题，并提出ProtoScore新指标来减少这种偏差。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型的自动评估指标常常替代人类判断，但尚不清楚这些指标是否真正关注语义正确性，还是倾向于从有偏数据分布中学到的视觉和社会原型图像。作者识别并研究了多模态评估中的原型性偏差这一系统性失败模式。

Method: 作者创建了ProtoBias基准测试，涵盖动物、物体和人口统计图像，将语义正确但不典型的图像与语义错误但典型的对抗性对应图像配对。使用这个基准测试评估现有指标（如CLIPScore、PickScore、VQA评分等）是否遵循文本语义还是默认选择原型。基于发现的问题，提出了ProtoScore这一7B参数的新指标。

Result: 结果显示广泛使用的指标经常错误排名这些配对，即使是LLM-as-Judge系统在社会基础案例中也表现出不均匀的鲁棒性。人类评估始终更倾向于语义正确性且决策边界更大。ProtoScore新指标显著降低了失败率并抑制了错误排名，运行速度比GPT-5推理时间快几个数量级，接近更大闭源评估器的鲁棒性。

Conclusion: 现有文本到图像评估指标存在严重的原型性偏差问题，倾向于选择视觉典型性而非语义正确性。ProtoScore新指标能够有效减少这种偏差，提供更鲁棒的评估，同时保持高效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于卫星图像时间序列的作物分割方法在固定时间序列长度下表现良好，但忽略了模型在不同时间长度场景下的泛化能力，导致在时间序列长度变化时分割结果显著下降

Method: 提出TEA方法，采用教师-学生框架：教师模型封装全局序列知识，指导学生模型处理自适应时间输入长度；通过中间嵌入、原型和软标签三个视角进行知识传递；动态聚合学生模型以缓解知识遗忘；引入全序列重建作为辅助任务提升表示质量

Result: 在常见基准测试中，该方法在不同时间长度输入下带来了显著改进，证明了其增强模型在变化时间序列长度下鲁棒性的有效性

Conclusion: TEA方法通过教师-学生框架和全序列重建辅助任务，有效解决了卫星图像时间序列分割中模型在不同时间长度场景下的泛化问题，为农业遥感应用提供了更鲁棒的解决方案
...
TL;DR: SparseLaneSTP：一种新颖的稀疏车道检测方法，通过整合车道几何先验和时序信息，在3D车道检测中实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法存在几个关键问题：1）传统密集BEV方法因特征转换错误导致特征表示与真实3D路面不对齐；2）稀疏车道检测器虽然超越了密集BEV方法，但完全忽略了有价值的车道特定先验；3）现有方法未能利用历史车道观测，而这些观测在能见度差的情况下有潜力解决歧义问题

Method: 提出了SparseLaneSTP方法，将车道结构几何特性和时序信息整合到稀疏车道transformer中。主要创新包括：1）新的车道特定时空注意力机制；2）为稀疏架构设计的连续车道表示；3）时序正则化。此外还创建了精确一致的3D车道数据集，采用简单有效的自动标注策略

Result: 实验证明该方法在所有检测和误差指标上均实现了最先进的性能，不仅在现有3D车道检测基准上表现优异，在新创建的数据集上也取得了良好结果

Conclusion: SparseLaneSTP通过整合车道几何先验和时序信息，有效解决了现有3D车道检测方法的局限性，在稀疏架构中实现了更准确、更鲁棒的车道检测，同时创建的高质量数据集也为该领域研究提供了有价值的资源
...
Method: 1. 三目视图一致性约束：通过渲染水平和垂直平移的相机视图，并通过逆变形对齐；2. 合成极线深度先验：通过三角测量生成自监督深度正则化器；3. 深度感知alpha调整：基于z分量和观察方向调制3D高斯不透明度，防止介质诱导基元形成。

Result: 在真实水下场景和模拟场景上的实验表明，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能够将3D高斯从散射介质中解耦，准确表示物体几何结构。

Conclusion: OceanSplat通过几何约束和深度感知调整，有效解决了水下3D重建中的多视图不一致问题，实现了对水下场景几何结构的鲁棒表示，显著减少了漂浮伪影。
...
TL;DR: 高阶对抗攻击对目标检测器的影响研究：通过连续训练攻击模式和使用对抗训练强化YOLOv10检测器，发现高阶对抗补丁具有更强的泛化能力，且仅靠对抗训练不足以有效防御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 研究高阶对抗攻击对目标检测器的影响，探索攻击模式与防御训练之间的猫鼠游戏关系，评估对抗训练在防御高阶对抗攻击方面的有效性。
...
Conclusion: 高阶对抗攻击对目标检测器构成严重威胁，需要更有效的防御策略，单纯对抗训练无法充分保护检测器免受此类攻击。
...
TL;DR: PolyFit是一种基于面片拟合的曲面表示方法，通过局部拟合jet函数实现高效学习，可用于多种曲面变形任务，在形状模板和服装悬垂应用中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统曲面变形方法通常需要优化每个顶点的自由度，计算成本高。需要一种更高效的曲面表示方法，能够通过更新少量参数实现曲面变形，适用于计算机视觉和图形学的下游任务。

Method: 提出PolyFit方法，通过局部拟合jet函数来表示曲面面片。该方法可以从解析函数和真实数据中监督学习，学习后能泛化到多种曲面类型。通过更新紧凑的jet系数集而不是优化每个顶点自由度来实现曲面变形。

Result: 1) 形状模板应用中，测试时优化方法在保持竞争力的准确度同时显著快于离线物理求解器，在准确性上优于最近的物理引导神经模拟器；2) 服装悬垂应用中，训练了自监督、网格和服装无关的模型，能泛化到不同分辨率和服装类型，推理速度比强基线快一个数量级。

Conclusion: PolyFit提供了一种高效的曲面表示方法，通过局部jet函数拟合实现快速曲面变形。该方法在形状模板和服装悬垂应用中表现出优越的性能，为计算机视觉和图形学中的曲面处理任务提供了有效的解决方案。
...
TL;DR: 本文提出了一种针对制药行业的视频到视频片段生成框架，通过集成音频语言模型和视觉语言模型，实现了高效、低成本、高质量的视频摘要生成。
...
Method: 提出了一个领域适应的视频到视频片段生成框架，包含三个核心贡献：1）可复现的剪切与合并算法，带有淡入淡出和时间戳标准化，确保平滑过渡和音视频对齐；2）基于角色定义和提示注入的个性化机制，用于定制化输出（营销、培训、监管）；3）成本高效的端到端管道策略，平衡ALM/VLM增强处理。

Result: 在Video MME基准测试（900个视频）和专有数据集（14个疾病领域的16,159个制药视频）上评估，显示3-4倍加速、4倍成本降低和具有竞争力的片段质量。方法在片段连贯性得分（0.348）和信息性得分（0.721）上优于最先进的VLM基线（如Gemini 2.5 Pro）。

Conclusion: 该框架展示了透明、定制化提取和符合合规要求的视频摘要技术在生命科学领域的潜力，为制药行业的多模态内容处理提供了高效、可扩展的解决方案。
...
TL;DR: 提出一种无需人工标注的LiDAR多模态伪标签方法，利用时间几何一致性从文本和2D视觉基础模型提取3D信息，同时生成语义标签、3D边界框和密集LiDAR扫描


<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用中未标注的LiDAR数据虽然包含丰富的3D几何信息，但缺乏人工标注使其难以利用，这成为自动驾驶感知研究的主要成本障碍

Method: 基于时间累积LiDAR地图学习强几何先验，利用时间几何一致性将文本和2D视觉基础模型的线索提升融合到3D空间，采用新颖的迭代更新规则强制联合几何-语义一致性，并通过不一致性检测运动物体

Result: 方法在三个数据集上表现出稳健泛化能力，优于现有需要额外人工监督的语义分割和物体检测伪标签方法；即使少量几何一致的密集化LiDAR也能显著改善深度预测（80-150米范围MAE改善51.5%，150-250米范围改善22.0%）

Conclusion: 通过利用时间几何一致性和多模态基础模型，成功开发出无需人工标注的LiDAR伪标签方法，有效降低了自动驾驶感知研究的标注成本，同时提升了3D感知性能
...
TL;DR: 提出投影条件化方法，用目标视图投影线索替代原始相机参数，将视图合成任务从脆弱的射线空间几何回归问题转化为条件良好的目标视图图像到图像翻译问题。


<details>
  <summary>Details</summary>
Motivation: 现有前馈视图合成模型使用Plücker射线图编码相机参数，这种方法将预测与任意世界坐标系绑定，使其对微小相机变换敏感，从而破坏了几何一致性。需要寻找更好的输入条件来实现鲁棒且一致的视图合成。

Method: 提出投影条件化方法，用目标视图投影线索替代原始相机参数；引入针对该线索的掩码自编码预训练策略，能够利用大规模未校准数据进行预训练。

Result: 在视图一致性基准测试中，相比射线条件化基线方法，本方法显示出改进的保真度和更强的跨视图一致性；在标准新视图合成基准测试中达到最先进的质量。

Conclusion: 投影条件化方法通过将视图合成任务重新定义为目标视图图像到图像翻译问题，显著提高了模型的鲁棒性和几何一致性，同时通过掩码自编码预训练策略有效利用了大规模未校准数据。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视觉丰富文档理解中存在性能瓶颈，需要一种系统方法来分析模型在视觉嵌入空间中的表现，识别错误模式并针对性改进。

Method: VERSE方法通过可视化潜在表示来评估模型可行性，识别问题区域，并指导生成包含特定视觉特征的合成数据来增强模型在问题簇上的性能。

Result: 在MERIT数据集上的验证显示，VERSE能揭示与错误相关的视觉特征，使用这些特征重新训练模型能显著提升F1分数而不损害泛化能力；优化后的本地模型性能达到甚至超过SaaS解决方案。

Conclusion: VERSE为视觉语言模型在文档理解任务中提供了一种有效的分析和改进框架，能够系统性地提升模型性能，使本地模型具备与商业SaaS解决方案竞争的能力。
...
TL;DR: VerseCrafter是一个4D感知的视频世界模型，通过4D几何控制表示实现对相机和物体动态的统一精确控制，并利用自动数据引擎从野外视频中提取训练数据。
...
Method: 提出4D几何控制表示，通过静态背景点云和每个物体的3D高斯轨迹编码世界状态；将这些4D控制渲染为预训练视频扩散模型的调节信号；开发自动数据引擎从野外视频中提取4D控制数据用于大规模训练。

Result: 能够生成高保真、视角一致的视频，精确遵循指定的动态；提供灵活、类别无关的替代方案，优于刚性边界框或参数化模型；通过自动数据引擎解决了大规模4D标注数据稀缺的问题。

Conclusion: VerseCrafter通过创新的4D几何控制表示和自动数据引擎，实现了对相机和物体动态的统一精确控制，为视频世界模型提供了新的解决方案。
...
TL;DR: 本文提出了一种轻量级视觉语言框架，用于从叶片图像进行作物和病害识别，结合Swin Transformer视觉编码器和序列到序列语言解码器，通过两阶段训练策略提高视觉表示学习和跨模态对齐。
...
Method: 采用Swin Transformer作为视觉编码器，结合序列到序列语言解码器，使用两阶段训练策略：第一阶段专注于视觉表示学习，第二阶段进行跨模态对齐训练。

Result: 在大规模作物病害数据集上评估，模型在作物和病害识别方面均获得高准确率，在BLEU、ROUGE和BERTScore等自然语言生成指标上表现优异，参数量显著少于基线模型。

Conclusion: 该轻量级框架在作物病害视觉问答任务中表现出色，证明了任务特定的视觉预训练对于该领域的重要性，同时模型具有良好的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有病理学基础模型在性能、鲁棒性和计算需求方面存在权衡，限制了其临床部署，需要开发更优的模型来解决这些缺陷

Method: 开发了Atlas 2、Atlas 2-B和Atlas 2-S三个病理视觉基础模型，基于迄今为止最大的病理基础模型数据集训练，包含来自柏林夏里特医学院、慕尼黑大学和梅奥诊所的550万张组织病理学全切片图像

Result: 在80个公共基准测试的综合评估中，模型在预测性能、鲁棒性和资源效率方面均达到最先进水平

Conclusion: Atlas 2系列模型通过展示卓越的性能、鲁棒性和效率，解决了现有病理基础模型的局限性，为临床部署提供了更好的解决方案
...
TL;DR: MuLo-SD提出了一种多尺度局部推测解码框架，通过低分辨率草稿生成和高分辨率并行验证，结合局部拒绝与重采样机制，在保持图像质量的同时实现了1.7倍加速。
...
Method: 提出多尺度局部推测解码框架：1) 使用低分辨率草稿模型配合学习上采样器生成候选图像token；2) 高分辨率目标模型并行验证；3) 引入局部拒绝与重采样机制，在空间邻域内纠正草稿错误而非全图重采样。

Result: 在MS-COCO 5k验证集上，MuLo-SD实现了高达1.7倍的加速，超越了EAGLE-2和LANTERN等强基线，同时在语义对齐和感知质量方面保持可比性能。

Conclusion: MuLo-SD为图像合成的推测解码设定了新的技术标准，通过多分辨率草稿和空间感知验证机制，在效率和保真度之间取得了良好平衡。
...
TL;DR: VLI是一个无需训练的多模态大语言模型推理框架，通过模拟元认知自我修正过程来减少物体幻觉问题，在多个基准测试中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 物体幻觉严重削弱多模态大语言模型的可靠性，主要源于认知内省失败——模型盲目信任语言先验而非具体视觉证据。现有缓解方法有限：对比解码方法仅表面操作未修正内部语义错位，而当前潜在引导方法依赖缺乏实例特定精度的静态向量。

Method: 提出视觉语言内省（VLI）框架：1）属性内省：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2）可解释双因果引导：主动调节推理过程，动态隔离视觉证据与背景噪声，同时通过自适应校准消除盲目置信。
...
TL;DR: 论文提出VideoAuto-R1框架，采用"必要时推理"策略，通过初始答案置信度决定是否进行链式思维推理，在视频理解任务中实现SOTA准确率同时大幅提升效率。


<details>
  <summary>Details</summary>
Motivation: 研究发现对于RL训练的视频模型，直接回答通常匹配甚至优于链式思维推理性能，但链式思维推理计算成本更高。因此需要探索更高效的推理策略，只在必要时进行复杂推理。

Method: 提出VideoAuto-R1框架，采用"思考一次，回答两次"的训练范式：模型首先生成初始答案，然后进行推理，最后输出审查后的答案。两个答案都通过可验证奖励进行监督。推理时使用初始答案的置信度决定是否进行推理。

Result: 在视频QA和定位基准测试中，VideoAuto-R1达到最先进的准确率，同时显著提升效率，平均响应长度减少约3.3倍（从149个token降至44个）。感知导向任务中思维模式激活率低，而推理密集型任务中激活率高。
...
TL;DR: 研究发现大型视觉语言模型存在提示诱导幻觉问题：当文本提示中的对象数量超过图像实际数量时，模型会倾向于相信文本提示而非视觉证据，且随着对象数量增加，这种幻觉现象更加严重。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然能力强大，但经常出现幻觉现象，倾向于相信文本提示而忽略视觉证据。研究者希望在一个受控的对象计数场景中，系统研究这种提示诱导幻觉的机制和模式。

Method: 在受控的对象计数设置中，研究者让文本提示中的对象数量超过图像实际数量（例如提示描述4朵睡莲但图像只有3朵）。通过分析三个不同的视觉语言模型，识别出少量特定的注意力头，这些头的消融可以显著减少幻觉现象。

Result: 研究发现：1）在对象数量较少时，模型能够纠正提示中的高估；2）随着对象数量增加，模型越来越倾向于遵从文本提示而忽略视觉证据；3）识别出的特定注意力头消融后，提示诱导幻觉减少了至少40%，且无需额外训练；4）不同模型中，这些注意力头以模型特定的方式介导提示复制行为。

Conclusion: 该研究揭示了视觉语言模型中提示诱导幻觉的内部机制，发现了不同模型在实现这些行为时的特异性差异。通过消融少量特定注意力头，可以显著减少幻觉现象，为理解和改进模型可靠性提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D MRI数据集存在人口统计学偏差，限制了脑年龄预测模型的公平性和泛化能力。获取新数据成本高且受伦理约束，而当前基于潜在扩散模型的生成方法推理速度慢、可能引入压缩伪影，且很少考虑年龄条件化。

Method: 提出FlowLet框架，在可逆3D小波域中利用流匹配技术合成年龄条件化的3D MRI图像，避免重建伪影并降低计算需求。

Result: FlowLet能以较少的采样步骤生成高保真度的脑部体积图像。使用FlowLet生成的数据训练脑年龄预测模型，能显著改善对代表性不足年龄组的预测性能，区域分析证实了其解剖结构的保持能力。

Conclusion: FlowLet通过在小波域进行流匹配，提供了一种高效的条件生成方法，能生成高质量的年龄条件化3D MRI数据，有效提升脑年龄预测模型的公平性和性能。
...
TL;DR: ObjectForesight是一个3D物体中心动力学模型，能够从短时第一人称视角视频序列预测刚性物体的未来6自由度位姿和轨迹，实现几何基础且时间一致的物体运动预测。


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松预测物体通过交互可能如何移动或变化，但现有计算系统缺乏这种能力。研究旨在让计算系统能够从被动视觉观察中直接预测合理的未来物体运动。

Method: 提出ObjectForesight模型，在3D物体层面显式表示世界，不同于传统在像素或潜在空间操作的世界/动力学模型。利用分割、网格重建和3D姿态估计的最新进展，构建了包含200多万个短片段的数据集，带有伪地面真实3D物体轨迹。

Result: ObjectForesight在准确性、几何一致性和对未见物体及场景的泛化能力方面取得显著提升，为从观察中直接学习物理基础的物体中心动力学模型建立了可扩展框架。

Conclusion: 该研究成功开发了一个能够预测未来物体运动的3D物体中心动力学模型，通过大规模数据集训练实现了几何基础和物理合理的预测，为计算机视觉和机器人学中的物体交互理解提供了新方法。
...
TL;DR: 该论文提出了广义指代表达式分割(GRES)、理解(GREC)和生成(GREG)三个新基准任务，构建了首个大规模多目标、无目标和单目标表达式的数据集gRefCOCO，并提出了ReLA基线方法在GRES和GREC任务上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有指代表达式分割(RES)、理解(REC)和生成(REG)方法仅支持单目标表达式，即一个表达式对应一个对象，这限制了在实际应用中的使用。现实场景中表达式可能指向多个对象或无目标对象，因此需要扩展经典REx任务以支持任意数量对象的表达式。

Method: 提出了ReLA方法，该方法自适应地将图像划分为包含子实例线索的区域，并显式建模区域-区域和区域-语言之间的依赖关系，以处理GRES/GREC任务中复杂的多目标关系建模挑战。

Result: 构建了首个大规模GREx数据集gRefCOCO，包含多目标、无目标和单目标表达式及其对应的带标注目标的图像。提出的ReLA方法在GRES和GREC任务上取得了最先进的结果。

Conclusion: 该工作将经典的REx任务扩展为GREx，支持任意数量对象的表达式，填补了现有研究的空白。提出的gRefCOCO数据集和ReLA方法为广义指代表达式任务提供了基准和有效解决方案，促进了该领域的发展。
...
Method: 1) 提出PPD单目深度基础模型，基于像素空间扩散变换器；2) 引入语义提示DiT，利用视觉基础模型的语义表示指导扩散过程；3) 采用级联DiT架构逐步增加图像token数量；4) 扩展为视频模型PPVD，引入语义一致DiT和参考引导的token传播机制。

Result: 模型在所有生成式单目和视频深度估计模型中达到最佳性能，产生的点云比其他模型显著更干净。

Conclusion: 通过像素空间生成建模和创新的DiT设计，实现了高质量的视觉几何重建，解决了漂浮像素和细节丢失问题。
...
Method: 结合统计方法与深度强化学习：1）针对夜间场景的统计算法，整合显著灰度像素检测与新颖光照估计；2）以统计算法为核心的首个深度强化学习色彩恒常性方法，模仿专业AWB调优专家动态优化每张图像参数。

Result: 方法在低光和正常光照图像上均表现出优越的泛化能力，通过引入首个多传感器夜间数据集进行跨传感器评估验证了有效性。

Conclusion: RL-AWB框架成功解决了夜间白平衡问题，结合统计方法与深度强化学习的方法在复杂光照条件下表现出色，为计算摄影中的色彩恒常性提供了新思路。
...
TL;DR: QNeRF是首个用于2D图像新视角合成的混合量子-经典模型，通过参数化量子电路编码空间和视角信息，相比经典NeRF模型更紧凑且性能相当或更优。


<details>
  <summary>Details</summary>
Motivation: 量子视觉场在模型紧凑性和收敛速度方面显示出优势，而神经辐射场在新视角合成方面取得重大进展但存在模型大、训练密集的问题。本研究旨在结合两者的优势，探索量子机器学习在计算机视觉中等任务中的潜力。

Method: 提出QNeRF模型，利用参数化量子电路通过量子叠加和纠缠编码空间和视角相关信息。设计了两种架构变体：Full QNeRF最大化利用所有量子振幅增强表示能力；Dual-Branch QNeRF引入任务感知归纳偏置，分支处理空间和视角相关的量子态准备，大幅降低操作复杂度。

Result: 在中等分辨率图像训练时，QNeRF匹配或优于经典NeRF基线，同时使用不到一半的参数数量。这表明量子机器学习可以作为计算机视觉中等任务中连续信号表示的有竞争力替代方案。

Conclusion: QNeRF展示了量子机器学习在3D表示学习等计算机视觉任务中的潜力，通过量子叠加和纠缠实现更紧凑的模型表示，为量子-经典混合模型在视觉任务中的应用开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建动态物体的完整3D形状和运动是一个具有挑战性的问题，现有方法通常需要复杂的处理流程或额外的骨骼信息。

Method: 1. 使用自动编码器学习紧凑的潜在空间，训练时通过骨骼结构引导学习合理的变形先验；2. 编码器采用时空注意力机制获得更稳定的整体变形表示；3. 基于该表示训练潜在扩散模型，在输入视频和第一帧重建网格的条件下一次性预测完整动画。

Result: 在重建和新视角合成基准测试中，Mesh4D在恢复准确的3D形状和变形方面优于现有方法。

Conclusion: Mesh4D通过紧凑的潜在空间表示和一次性预测完整动画的能力，为单目4D网格重建提供了一种有效的前馈解决方案，无需推理时的骨骼信息。
...
TL;DR: MedPI是一个用于评估大语言模型在医患对话中表现的高维基准测试，包含105个维度，评估了9个主流模型在7,097次对话中的表现，发现所有模型在鉴别诊断等维度表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有医学评估基准多为单轮问答形式，缺乏对医患对话全过程的系统性评估。需要建立一个更全面、多维度的评估框架来准确衡量LLMs在真实临床对话场景中的能力。

Method: 构建了五层评估体系：1）患者包（合成电子病历）；2）具有记忆和情感的AI患者；3）任务矩阵（就诊原因×就诊目标）；4）105维评估框架（基于ACGME能力标准）；5）校准的AI评委委员会。评估了9个主流模型在366个AI患者和7,097次对话中的表现。

Result: 所有评估的LLMs在多个维度表现不佳，特别是在鉴别诊断方面表现较差。这表明当前LLMs在临床诊断和治疗建议方面仍有较大改进空间。

Conclusion: MedPI基准测试为评估LLMs在医患对话中的表现提供了全面框架，揭示了当前模型在临床实践中的局限性，有助于指导未来LLMs在诊断和治疗建议方面的应用改进。
...
TL;DR: 提出了一种完全无监督的中文动词构式数据库构建方法，通过提供显式可解释的规则来补充大语言模型，在需要解释性和可解释性的应用场景中发挥作用。


<details>
  <summary>Details</summary>
Motivation: 针对大语言模型在需要解释性和可解释性的应用场景中的不足，提出构建动词构式数据库来提供显式、可解释的规则，以补充大语言模型的能力。

Method: 将动词构式形式化定义为投影的、有根的、有序的有向无环图，采用一系列聚类算法从大规模语料库检索的句子列表中为给定动词生成构式。

Result: 统计分析表明生成的构式具有功能独立性和分级典型性的设计特征；在动词语法错误纠正评估中，基于构式最大匹配的错误纠正算法表现优于大语言模型。

Conclusion: 提出的无监督方法能够有效构建中文动词构式数据库，生成的构式具有理论上的设计特征，并在实际应用中（如语法错误纠正）表现出优于大语言模型的性能，为需要解释性的应用场景提供了有价值的补充工具。
...
TL;DR: 使用大语言模型生成高质量电商产品数据，通过控制修改框架提升数据质量，在MAVE数据集上达到与真实数据相当的性能，混合数据配置进一步提升效果。


<details>
  <summary>Details</summary>
Motivation: 电商产品信息提取对电商服务至关重要，但获取高质量标注数据集仍然具有挑战性。需要一种系统化的方法来生成合成电商产品数据，特别是在低资源场景下。

Method: 提出一个使用大语言模型生成合成电商产品数据的系统方法，引入控制修改框架，包含三种策略：属性保留修改、控制负例生成和系统属性移除。使用最先进的大语言模型配合属性感知提示，在保持产品连贯性的同时强制执行店铺约束。

Result: 对2000个合成产品的人工评估显示高度有效性：99.6%被评为自然，96.5%包含有效属性值，超过90%显示一致的属性使用。在公开的MAVE数据集上，合成数据达到60.5%的准确率，与真实训练数据（60.8%）表现相当，显著优于13.4%的零样本基线。混合合成和真实数据的配置进一步将性能提升至68.8%的准确率。

Conclusion: 该框架为增强电商数据集提供了实用解决方案，特别是在低资源场景下具有重要价值，能够生成高质量合成数据，性能与真实数据相当甚至通过混合配置获得更好效果。
...
TL;DR: 该研究提出"集体叙事锚定"协议，通过参与式方法将社区故事转化为结构化叙事单元，并整合到AI系统中，以解决大语言模型在社区特定查询中的知识盲点问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在回答社区特定问题时经常失败，形成"知识盲点"，边缘化地方声音并加剧认知不公。需要开发能够整合地方知识和叙事的AI系统。

Method: 设计了参与式协议，包括：1）与24名社区成员进行三次参与式映射工作坊；2）开发能够保留叙事丰富性的结构化模式；3）对14,782个地方信息问答对进行审计；4）创建参与式问答集评估LLM性能。

Result: 审计发现76.7%的错误源于事实空白、文化误解、地理混淆和时间错位。在参与式问答集上，最先进的LLM在没有额外上下文的情况下正确率低于21%。收集的叙事中包含缺失的事实，表明可以直接弥补主要错误模式。

Conclusion: 研究提供了分类法、协议和参与式评估，为构建社区锚定的AI系统奠定基础。同时阐明了代表性、权力、治理、控制、隐私和同意等关键设计张力，为检索优先、来源可见、地方治理的问答系统提供具体需求。
...
TL;DR: TeleTables是一个评估语言模型在电信标准（特别是3GPP规范）中表格理解和知识能力的基准，发现小模型在电信知识回忆和表格解释方面表现不佳，而大模型在表格解释方面表现更好，凸显了领域专业化微调的必要性。


<details>
  <summary>Details</summary>
Motivation: 语言模型在电信行业应用日益广泛，但研究表明它们在电信标准（特别是3GPP规范）上表现不佳。作者认为一个关键原因是这些标准密集包含表格来呈现重要信息，而语言模型对这些表格的知识和解释能力尚未得到充分研究。

Method: 提出了TeleTables基准，通过新颖的多阶段数据生成流程构建：从3GPP标准中提取表格，使用多模态和推理导向的语言模型生成和验证问题。最终创建了包含500个人工验证的问题-答案对的数据集，每个问题都关联多种格式的对应表格。

Result: 评估显示，较小模型（参数少于100亿）在3GPP知识回忆和表格解释方面都表现不佳，表明它们在预训练中对电信标准的接触有限，且缺乏处理复杂技术材料的归纳偏差。较大模型在表格解释方面表现出更强的推理能力。

Conclusion: TeleTables强调了领域专业化微调的必要性，以便可靠地解释和推理电信标准。该基准公开可用，有助于评估和改进语言模型在技术文档表格理解方面的能力。
...
TL;DR: 提出了一种基于时变方差和空间偏差的动态重掩码策略，显著提升扩散语言模型的推理效率


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型的重掩码策略使用单一全局置信度阈值，忽视了token的时空动态特性，导致冗余迭代和并行性受限

Method: 提出动态检测每个token的时变方差和空间偏差，这些信号反映token的收敛状态和token间相关性，并据此自适应调整每个token在每个时间步的置信度阈值

Result: 在主流数据集上显著提升扩散语言模型的运行效率，实现高达8.9倍的加速，同时保持生成质量

Conclusion: 动态重掩码策略通过考虑token的时空动态特性，有效解决了固定阈值重掩码的局限性，为扩散语言模型的高效推理提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 大学招生办公室面临处理大量咨询的挑战，需要在保证响应质量的同时提高效率，这对潜在学生的感知有重要影响。传统RAG在复杂狭窄的招生领域可能表现有限，容易产生上下文不充分的回答。

Method: 提出混合方法：1）使用RAG从大型数据集中检索相关信息；2）在特定招生流程的精选数据集上微调语言模型，增强其对RAG提供数据的准确解释能力；3）探索响应生成逻辑的优化策略，平衡响应质量与速度。

Result: 通过微调增强了模型在复杂招生领域的理解能力，使系统能够生成更准确、上下文相关的响应。混合方法结合了RAG获取最新信息的能力和微调嵌入的领域知识。

Conclusion: 该AI系统通过结合RAG与领域特定微调，有效解决了大学招生咨询中的响应时间和信息准确性问题，为招生办公室提供了高效且高质量的通信解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型内部组织的政治意识形态与人类意识形态空间存在系统性、模型特定的不对齐问题，需要一种简单高效的方法来对齐模型与用户观点。

Method: 开发轻量级线性探针，从模型内部特征计算偏差分数，直接调整最终输出概率，无需重新训练模型。

Result: 该方法能够量化模型与人类意识形态的对齐偏差，并有效校正输出层，保持模型原有推理能力的同时实现观点对齐。

Conclusion: 提出的线性探针方法提供了一种实用、低成本的解决方案，可在不重新训练模型的情况下，系统性地测量和校正LLMs的政治意识形态对齐偏差。
...
TL;DR: LEXMA是一个基于强化学习的LLM微调框架，用于生成面向多受众的叙事式AI解释，在抵押贷款审批场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型在高风险消费者交互中应用广泛，但决策逻辑不透明。现有可解释AI技术依赖事后数值特征归因，无法提供连贯的决策叙事。虽然LLM能生成自然语言解释，但仍面临三个挑战：解释需要既决策正确又忠实于预测驱动因素；能服务多受众而不改变底层决策规则；能以标签高效的方式训练，不依赖大量人工标注的解释。
...
Result: LEXMA在预测性能上显著优于其他LLM基线。人工评估显示，该方法生成的面向专家的解释更具风险关注性，面向消费者的解释更清晰、更具可操作性且更有礼貌。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个自动化系统来解决俄罗斯媒体行业根据联邦法律436-FZ对剧本进行年龄分级和内容安全评估的实际编辑挑战，特别是在Wink黑客松中应对行业需求。

Method: 系统将完整剧本分割成叙事单元，在五个类别（暴力、性内容、脏话、物质、恐怖元素）中检测内容违规，使用经过微调的Phi-3-mini模型进行4位量化，在Yandex Cloud上部署并利用CUDA加速。

Result: 实现了80%的评级准确率和80-95%的分割精度（取决于格式），在80GB VRAM限制和5分钟内处理平均剧本的严格约束下，成功部署到生产工作流中。

Conclusion: Qwerty AI系统展示了在俄罗斯媒体行业中自动化内容安全评估的实际适用性，解决了编辑挑战，并在Wink黑客松（2025年11月）中取得了实际成果。
...
Result: 该研究提供了关于数据质量和模型大小对基于偏好的优化方法影响的见解，并指出了这些方法最有效的条件。
...
TL;DR: AnimatedLLM是一个交互式Web应用，通过浏览器提供Transformer语言模型的逐步可视化，使用预计算的开源LLM轨迹和手动整理的输入，用于教学目的。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理教育中日益重要，但展示其内部工作机制的教学材料稀缺，需要直观的可视化工具来帮助理解Transformer模型的工作原理。

Method: 开发了一个完全在浏览器中运行的交互式Web应用程序，使用预计算的开源大型语言模型轨迹，结合手动整理的输入数据，提供Transformer语言模型的逐步可视化。

Result: 创建了AnimatedLLM应用，可通过https://animatedllm.github.io访问，作为教学辅助工具和自学资源，帮助用户直观理解LLM的内部工作机制。

Conclusion: AnimatedLLM填补了LLM教学可视化工具的空白，为自然语言处理教育提供了实用的交互式学习资源，有助于更深入地理解Transformer模型的工作原理。
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习遗忘基准往往无法准确反映模型实际学到的"遗忘范围"，需要更忠实评估LLM遗忘效果的方法。

Method: 提出BiForget框架，形式化两种遗忘粒度（领域级和实例级），通过种子引导和对抗性提示利用目标模型自身生成匹配其内部知识分布的数据。

Result: 在多个基准测试中，BiForget在相关性、多样性和效率方面达到优越平衡。在哈利波特领域中，相关性提升约20，多样性提升约0.05，同时数据总量减半。
...
Result: 与强基线相比，RIGOURATE在证据检索和夸大陈述检测方面表现更优，能够有效识别论文中超出证据支持的夸大主张。

Conclusion: 该工作实现了证据比例性的操作化，支持更清晰、更透明的科学交流，有助于减少科学文献中的夸大陈述问题。
...
TL;DR: 该研究对印度多种方言和语言变体的自发、嘈杂、代码混合语音进行跨语言迁移实证分析，发现ASR性能与语言间的系统发育距离相关，但方言环境下的表现不完全由此决定，少量方言数据微调可达到大量相关高资源语言微调的效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索方言和非标准化语音对自动语音识别（ASR）系统的挑战，特别是在资源匮乏的方言环境下，了解跨语言迁移的有效性以及系统发育距离对ASR性能的实际影响。

Method: 采用实证研究方法，收集印度多种方言和语言变体的自发、嘈杂、代码混合语音数据，分析跨语言迁移效果。具体包括：比较不同系统发育距离语言间的ASR性能，评估少量方言数据微调与大量相关高资源语言微调的效果对比，并以Garhwali（一种低资源Pahari语言变体）为案例研究，评估多种当代ASR模型，最后通过分析转录错误来检查对预训练语言的偏见。

Result: 研究发现：1）ASR性能通常随语言间系统发育距离减小而改善，但该因素不能完全解释方言环境下的表现；2）少量方言数据微调常能达到与大量相关高资源标准化语言微调相当的性能；3）Garhwali案例研究表明当代ASR模型在低资源方言上的表现；4）转录错误分析揭示了ASR系统对预训练语言的偏见，为方言和非标准化语音面临的挑战提供了新见解。

Conclusion: 研究结论表明，方言ASR性能不完全取决于语言间的系统发育距离，少量方言数据微调是有效的策略。同时，ASR系统存在对预训练语言的偏见，这为改进方言和非标准化语音识别提供了重要方向，强调了针对方言特点定制ASR解决方案的必要性。
...
TL;DR: Disco-RAG：一种基于语篇感知的检索增强生成框架，通过构建语篇树和修辞图来增强LLM在知识密集型任务中的表现


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法将检索到的段落视为扁平无结构的信息，无法捕捉结构线索，限制了模型从分散文档证据中综合知识的能力

Method: 提出Disco-RAG框架，构建内部语篇树捕获局部层次结构，构建跨段落修辞图建模交叉段落连贯性，将这些结构整合到规划蓝图中指导生成过程

Result: 在问答和长文档摘要基准测试中表现出色，无需微调即可达到最先进的结果

Conclusion: 语篇结构在推进RAG系统中扮演重要角色，Disco-RAG通过显式注入语篇信号显著提升了生成质量
...
TL;DR: 论文揭示了当前大语言模型安全评估的局限性，指出安全对齐不是普遍语义能力而是人口统计学层次结构，模型对少数群体的防御率差异显著。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全评估存在危险错觉，通过聚合"身份仇恨"为标量分数掩盖了对特定人群的系统性脆弱性，需要揭示这种选择性安全的问题。

Method: 引入MiJaBench双语对抗基准测试，包含44,000个提示覆盖16个少数群体，生成528,000个提示-响应对，创建MiJaBench-Align数据集分析模型安全对齐。

Result: 安全对齐不是普遍语义能力而是人口统计学层次结构，防御率在同一模型内因目标群体不同波动高达33%，模型规模扩大加剧了这些差异。

Conclusion: 当前对齐技术未能建立非歧视原则，仅强化了对特定群体的记忆拒绝边界，挑战了当前的安全扩展定律，需要研究细粒度人口统计学对齐。
...
TL;DR: LLMs经常无法挑战用户的有害信念，研究发现这与LLMs默认接受用户假设和缺乏认知警惕有关。通过简单的语用干预（如添加"等一下"）可以显著改善LLMs在安全基准测试中的表现。


<details>
  <summary>Details</summary>
Motivation: LLMs在医疗建议和社会推理等领域经常无法挑战用户的有害信念，这种失败可能源于LLMs默认接受用户假设和缺乏认知警惕。研究旨在理解并解决这一问题。

Method: 研究分析了影响人类认知的社会和语言因素（话题相关性、语言编码和来源可靠性）如何影响LLMs的接受行为。通过三个安全基准测试（Cancer-Myth、SAGE-Eval、ELEPHANT）评估模型挑战有害信念的能力，并测试简单的语用干预措施。

Result: 研究发现影响人类接受行为的因素同样影响LLMs，解释了在不同安全基准测试中的性能差异。简单的语用干预（如添加"等一下"）显著提高了模型在这些基准测试中的表现，同时保持了较低的错误率。

Conclusion: 研究强调了考虑语用学对于评估LLM行为和改善LLM安全性的重要性。简单的语用干预可以有效提高LLMs挑战有害信念的能力，为LLM安全改进提供了实用方法。
...
TL;DR: 研究比较了两种对话预测学习方法：基于LLM评判的奖励优化 vs 直接最大化人类对话对数概率。发现奖励优化虽然提高了评判分数，但降低了人类响应的预测准确性和人类偏好率；而直接优化人类对话概率的方法效果更好，特别是结合思维链作为隐变量时效果最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是通过预测人们的下一个对话轮次来建模人类思维过程，比较不同学习方法在对话预测任务上的效果，特别是探索"思考"机制（思维链）如何影响模型对人类对话的预测能力。

Method: 研究方法比较了两个维度：(1) 模型是否允许在回答前"思考"（思维链机制），(2) 学习奖励方式：基于LLM评判的语义相似性和信息完整性评分 vs 直接最大化真实人类对话的对数概率。将思维链作为隐变量，推导了对数概率的下界，并优化该目标。

Result: 研究发现：优化基于评判的奖励确实提高了评判分数，但降低了分配给真实人类响应的概率，也降低了人类评判者选择合成响应作为更人性化的胜率。这种失败在模型被允许思考前回答时更加明显。相反，直接最大化观测到的人类响应对数概率，模型能更好地预测人们实际说的话，在对数概率和胜率评估上都表现更好。将思维链作为隐变量优化下界目标在所有评估中取得了最佳结果。

Conclusion: 结论表明：思考机制主要在与基于真实人类对话的分布匹配目标结合训练时才有效，将这种方法扩展到更广泛的对话数据可能会产生对人类行为有更细致理解的模型。直接优化人类对话概率的方法比基于LLM评判的奖励优化更有效。
...
TL;DR: MB-Defense是一种针对指令调优大语言模型的后门攻击防御框架，通过两阶段训练流程（防御性毒化和权重恢复）来免疫多样化的后门威胁，显著降低攻击成功率同时保持指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大语言模型虽然实现了广泛的任务泛化能力，但依赖大规模数据集使其容易受到后门攻击。攻击者可以通过毒化少量数据植入隐藏行为，而目前针对指令调优模型的防御措施研究不足，存在安全风险。

Method: MB-Defense包含两个阶段：1) 防御性毒化阶段，将攻击者触发器和防御触发器合并为统一的后门表示；2) 权重恢复阶段，通过额外训练打破这种表示，恢复干净行为。该方法采用数据高效的防御策略。

Result: 在多个大语言模型上的广泛实验表明，MB-Defense显著降低了攻击成功率，同时保持了模型的指令跟随能力。该方法提供了可泛化的防御策略，能够有效抵御未见过的后门攻击。

Conclusion: MB-Defense框架为指令调优的大语言模型提供了一种通用且数据高效的防御解决方案，增强了模型对多样化后门威胁的鲁棒性，填补了该领域防御研究的空白。
...
TL;DR: 用户对AI写作助手的需求偏好存在认知与行为的显著差异：用户自我报告认为紧迫性最重要，但实际行为显示写作努力程度才是主要驱动因素，这导致基于用户自述设计的系统性能反而更差。


<details>
  <summary>Details</summary>
Motivation: 主动式AI写作助手需要预测用户何时需要写作帮助，但目前缺乏对用户偏好驱动因素的实证理解。现有研究不清楚用户真正需要帮助的时机和原因。

Method: 通过因子式情景研究，让50名参与者进行750次成对比较，分析写作努力程度和紧迫性等因素对用户偏好的影响，并比较用户自我报告与实际行为之间的差异。

Result: 研究发现：1）写作努力程度是主要决策驱动因素（ρ=0.597），而紧迫性几乎没有预测能力（ρ≈0）；2）用户存在显著的认知-行为差距，自我报告将紧迫性排在首位，但实际行为中紧迫性是最弱的驱动因素；3）基于用户自述偏好设计的系统准确率仅为57.7%，甚至低于简单基线，而基于行为模式设计的系统达到61.3%（p<0.05）。

Conclusion: 依赖用户内省进行系统设计会误导优化过程，主动式自然语言生成系统应基于实际行为数据而非用户自述偏好进行设计，这对AI写作助手的开发有直接启示。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何通过定义学习概念表示，实现对冻结大型语言模型的定向控制，减少幻觉并提高指令遵循能力

Method: 在预训练LLM中添加特殊token，仅学习其嵌入表示（保持模型其他部分冻结），使用语言建模目标从多个自然语言定义中优化嵌入

Result: 在三个实验中验证：1) HotpotQA闭卷问答中减少幻觉（否定token增加弃权，肯定token增加幻觉）；2) 二语教学中诱导重述策略；3) 相比上下文定义，能更好保持其他指令的遵循

Conclusion: Concept Tokens提供了一种从定义中学习的紧凑控制信号，能够有效引导冻结LLM的行为，在减少幻觉和保持指令遵循方面表现良好
...
TL;DR: SampoNLP工具包通过自参考原子性评分创建形态学词典，用于评估芬兰语、匈牙利语和爱沙尼亚语的BPE分词器性能，提出综合性能评分(IPS)指标并确定最佳词汇量大小。


<details>
  <summary>Details</summary>
Motivation: 针对乌拉尔语系等形态丰富语言缺乏干净形态学词典的问题，阻碍了分词器评估，需要开发适用于低资源环境的词典创建工具。

Method: 开发SampoNLP工具包，使用基于最小描述长度原理的自参考原子性评分方法，通过内部结构线索过滤复合形式，生成高质量形态学词典。然后系统评估不同词汇量(8k-256k)的BPE分词器，提出综合性能评分(IPS)来平衡语素覆盖和过度分割。

Result: 为芬兰语、匈牙利语和爱沙尼亚语生成了高纯度形态学词典，通过IPS曲线分析确定了"拐点"和最佳词汇量大小，首次为这些语言提供了基于实证的词汇量推荐，并量化展示了标准BPE对高度黏着语言的局限性。

Conclusion: SampoNLP为形态丰富语言提供了实用的词典创建工具和分词器评估方法，通过实证分析为乌拉尔语系语言确定了最佳BPE词汇量，相关资源和工具已开源。
...
TL;DR: 论文提出WESR-Bench，一个用于非语言声音事件检测的评估基准，包含21类声音事件的精细分类、900+专家标注语料，以及能够区分ASR错误和事件检测的评估协议。


<details>
  <summary>Details</summary>
Motivation: 当前非语言声音事件检测方法存在任务定义不充分、类别覆盖有限、时间粒度模糊、缺乏标准化评估框架等问题，阻碍了下游应用的发展。

Method: 1. 开发包含21类声音事件的精细分类法，分为离散型和连续型；2. 构建WESR-Bench评估集（900+语料）和位置感知评估协议；3. 构建1700+小时语料库并训练专用模型。

Result: 提出的专用模型超越了开源音频语言模型和商业API，同时保持了ASR质量，为建模真实听觉场景提供了基础资源。

Conclusion: WESR-Bench填补了非语言声音事件检测的评估空白，为未来研究提供了标准化框架和基础资源，有助于推动真实世界听觉场景建模的发展。
...
TL;DR: LinguaGame：基于语言学基础的游戏论范式，通过信号博弈建模多智能体对话，提高通信效率


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统主要关注架构设计（如角色分配、工作流编排），而本文关注交互过程本身，旨在通过帮助智能体更有效地传达意图来提高通信效率

Method: 提出LinguaGame框架，将对话建模为基于通信意图和策略的信号博弈，使用无需训练的均衡近似算法进行推理时决策调整，基于语言学推理而非任务特定目标

Result: 在模拟法庭程序和辩论场景中评估，人类专家评估显示通信效率显著提升

Conclusion: 通过将对话视为有意图和策略性的通信，LinguaGame框架能够有效提高多智能体系统的通信效率，为多智能体对话生成提供了新的语言学基础范式
...
TL;DR: GRACE是一个强化学习框架，通过证据充分性评估、关键证据提取和明确弃权机制，统一解决RAG系统中正确回答无证据支撑和检索不足时捏造答案两大缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在两个关键缺陷：1）在没有明确证据支撑的情况下提供正确答案；2）在检索上下文不足时产生捏造的响应。虽然已有研究分别解决这些问题，但缺乏一个统一框架来整合基于证据的接地和可靠弃权机制。

Method: 提出GRACE强化学习框架：1）使用异构检索器生成多样化训练样本，无需人工标注；2）采用多阶段门控奖励函数，训练模型评估证据充分性、提取关键支持证据，并提供答案或明确弃权。

Result: 在两个基准测试中，GRACE实现了最先进的整体准确率，在准确响应和拒绝之间取得了良好平衡，同时仅需要先前方法10%的标注成本。

Conclusion: GRACE提供了一个统一的强化学习框架，能同时解决RAG系统的两大关键缺陷，通过自动数据构建和多阶段奖励机制，在保持高性能的同时显著降低了标注成本。
...
TL;DR: 本文系统评估了三种主流文本水印方法在孟加拉语LLM生成文本中的鲁棒性，发现跨语言往返翻译攻击会导致检测准确率崩溃至9-13%，并提出分层水印策略将检测准确率提升25-35%。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在文本生成中的广泛应用，水印技术对于作者归属、知识产权保护和滥用检测变得至关重要。现有水印方法在高资源语言中表现良好，但在低资源语言中的鲁棒性尚未得到充分研究，特别是孟加拉语等语言。

Method: 1) 系统评估KGW、指数采样(EXP)和Waterfall三种最先进的文本水印方法在孟加拉语LLM文本生成中的性能；2) 在跨语言往返翻译攻击下测试这些方法的鲁棒性；3) 提出分层水印策略，结合嵌入时水印和生成后水印，以提高对抗攻击的检测准确率。

Result: 在良性条件下，KGW和EXP达到高检测准确率(>88%)，且困惑度和ROUGE指标下降可忽略。但往返翻译攻击导致检测准确率崩溃至9-13%，表明令牌级水印存在根本性缺陷。分层水印策略将攻击后检测准确率提升25-35%，达到40-50%准确率，相对单层方法提升3-4倍，同时控制语义退化在可接受范围内。

Conclusion: 本研究量化了多语言水印中的鲁棒性-质量权衡，确立了分层水印作为低资源语言（如孟加拉语）的实用、无需训练的解决方案。分层水印策略有效提高了对抗跨语言攻击的鲁棒性，为低资源语言的水印技术提供了新方向。
...
TL;DR: NeuronLLM是一个新颖的任务级LLM理解框架，采用功能拮抗的生物原理识别LLM神经元，通过对比学习促进任务完成的"好神经元"和抑制任务的"坏神经元"，并利用增强问题集缓解LLM的偶然行为。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要识别特定能力相关的神经元，不适用于需要多种能力协调的任务场景，且只关注支持性神经元而忽略抑制性神经元，同时LLM的偶然行为会导致神经元归因错误。

Method: 提出NeuronLLM框架，基于功能拮抗原理，通过对比学习同时建模促进任务完成的"好神经元"和抑制任务的"坏神经元"，使用增强问题集缓解偶然行为问题。

Result: 在不同规模和家族的LLM上进行综合实验，在四个NLP任务上NeuronLLM优于现有方法，为LLM功能组织提供了新见解。

Conclusion: NeuronLLM通过功能拮抗原理和对比学习实现了对LLM神经元的全面建模，有效解决了现有方法在任务级理解和偶然行为方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究在自动作文评分中越来越重视生成高质量的反馈，但通常依赖LLM生成的反馈来训练模型，这些反馈缺乏明确的质量验证，导致噪声传播到下游应用中。

Method: 提出FeedEval框架，使用三个基于教学维度的专门化LLM评估器（具体性、帮助性、有效性），在ASAP++基准上训练这些评估器，用于评估多个反馈候选并筛选高质量反馈。
...
<details>
  <summary>Details</summary>
Motivation: 现有Text2Vis系统存在两大问题：闭源LLM生成的图表缺乏语义对齐和清晰度，开源模型常产生不可执行或视觉质量差的输出。传统监督微调只能改善代码可执行性，但无法提升整体可视化质量，因为传统损失函数无法捕捉执行后反馈。

Method: 提出RL-Text2Vis强化学习框架，基于Group Relative Policy Optimization (GRPO)，采用新颖的多目标奖励函数，联合优化文本准确性、代码有效性和可视化质量，利用执行后反馈进行训练。在Qwen2.5模型（7B和14B）上进行训练。

Result: 在Text2Vis基准测试中，RL-Text2Vis相比GPT-4o实现了22%的图表质量相对提升；代码执行成功率从零样本基线的78%提升至97%。模型显著优于强零样本和监督基线，并在VIS-Eval和NVBench等域外数据集上展现出强大的泛化能力。

Conclusion: GRPO是可视化生成中结构化多模态推理的有效策略。RL-Text2Vis框架通过强化学习有效解决了传统方法无法捕捉执行后反馈的问题，显著提升了文本到可视化生成的质量和可靠性。
...
TL;DR: 该研究探索模型合并作为资源高效替代方案，用于开发高性能多能力大语言模型，通过合并Qwen-8B与ThaiLLM-8B等模型，在泰语通用能力和金融领域均获得性能提升。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、安全和监管考虑，组织通常偏好本地部署LLMs，但面临部署多个专业模型与训练单一多能力模型成本高昂的权衡问题。ThaiLLM计划旨在增强开源LLMs的泰语能力，使泰国产业能够利用先进语言模型。

Method: 采用模型合并作为资源高效替代方案，进行两个关键实验：1) 合并Qwen-8B与ThaiLLM-8B；2) 合并Qwen-8B、ThaiLLM-8B和THaLLE-CFA-8B，评估在泰语通用能力和金融领域的性能表现。

Result: 实验一显示ThaiLLM-8B提升了泰语通用能力，在M3和M6 O-NET考试上优于通用指令跟随模型Qwen-8B。实验二进一步提升了通用和金融领域的性能，在M3和M6 O-NET、Flare-CFA和Thai-IC基准测试中均表现优异。
...
TL;DR: ROME知识编辑方法在单跳事实更新上高效，但在多跳推理任务中存在三个关键失败模式，作者提出冗余编辑策略来改善多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法（特别是ROME）在单跳事实更新上表现出色，但在需要知识链的多跳推理任务中面临显著挑战，需要研究其失败原因并提出改进方案。

Method: 研究了ROME在不同层深度编辑知识的效果，识别出三个关键失败模式，并提出冗余编辑策略来缓解"跳转过晚"和泛化能力下降问题。

Result: 冗余编辑方法可以将2跳问题的准确率提高至少15.5个百分点，相比之前的单次编辑策略提升了96%，但在特异性和语言自然度上有所权衡。

Conclusion: ROME在多跳推理中存在显著局限性，冗余编辑策略能有效改善多跳推理性能，为知识编辑方法在多跳任务中的应用提供了新思路。
...
TL;DR: 该研究指出当前视觉语言模型中描述特异性与长度被混淆的问题，提出特异性应相对于对比集来衡量，并通过实验验证人们更偏好特异性描述而非冗长描述。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在生成图像描述时，往往将描述的特异性与长度混为一谈，但这两个概念需要区分：描述可以是简洁但信息密集的，也可以是冗长但空洞的。需要建立更准确的评估方法来直接衡量描述的特异性。

Method: 1. 定义特异性：相对于对比集，描述的特异性取决于其能够多好地区分目标图像与其他可能图像。2. 构建数据集：控制描述长度，同时变化信息内容。3. 进行人类偏好实验：验证人们是否更偏好特异性描述而不考虑长度。4. 分析长度分配：研究长度预算如何影响特异性。

Result: 1. 人类实验验证：人们确实更偏好特异性描述，无论描述长度如何。2. 长度控制不足：仅控制长度不能完全解释特异性差异。3. 长度分配重要：如何分配长度预算对特异性有显著影响。

Conclusion: 研究支持评估方法应直接优先考虑特异性而非冗长性，为视觉语言模型的描述质量评估提供了新的理论框架和实证依据。
...
### [98] [Character-R1: Enhancing Role-Aware Reasoning in Role-Playing Agents via RLVR](https://arxiv.org/abs/2601.04611)
*Yihong Tang,Kehai Chen,Xuefeng Bai,Benyou Wang,Zeming Liu,Haifeng Wang,Min Zhang*

Main category: cs.CL

TL;DR: Character-R1框架通过认知焦点奖励、参考引导奖励和角色条件奖励归一化，为角色扮演智能体提供可验证的奖励信号，解决现有方法在复杂情境中角色不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前角色扮演智能体通常模仿表面行为，缺乏内部认知一致性，在复杂情境中容易出现角色不一致的错误。现有研究缺乏有效的角色感知推理奖励信号。

Method: 提出Character-R1框架，包含三个核心设计：1) 认知焦点奖励：基于10个角色元素（如世界观）的显式标签分析来结构化内部认知；2) 参考引导奖励：使用与参考响应的重叠度指标作为优化锚点；3) 角色条件奖励归一化：根据角色类别调整奖励分布以确保异构角色间的稳健优化。

Result: 大量实验表明，Character-R1在知识、记忆等多个方面显著优于现有方法。

Conclusion: Character-R1框架通过提供全面的可验证奖励信号，有效提升了角色扮演智能体的认知一致性和性能表现。
...
TL;DR: 提出CuCu框架，利用国家社会研究课程构建文化感知的LLM监督数据，应用于韩国课程创建KCaQA数据集（34.1k开放问答对），提升LLM的文化对齐能力。
...
Method: 提出CuCu自动化多智能体LLM框架，将国家社会研究课程转化为开放式的文化特定问答对，应用于韩国课程构建KCaQA数据集。

Result: 构建了包含34.1k个开放式问答对的KCaQA数据集，定量和定性分析表明该数据集覆盖文化特定主题，产生基于当地社会文化背景的响应。

Conclusion: 通过国家课程构建文化感知监督数据的方法具有可扩展性，能够有效提升LLM的文化对齐能力，为跨文化AI发展提供实用路径。
...
TL;DR: SpeechMedAssist：通过两阶段训练范式（文本知识注入+有限语音数据模态对齐）实现基于语音的医疗咨询对话系统，仅需1万合成语音样本


<details>
  <summary>Details</summary>
Motivation: 医疗咨询本质上是语音中心的，但现有研究多基于长文本交互，繁琐且对患者不友好。语音语言模型虽能实现更自然的语音交互，但医疗语音数据稀缺且直接微调效率低下，阻碍了语音模型在医疗咨询中的应用。

Method: 提出SpeechMedAssist，利用语音语言模型的架构特性，将传统单阶段训练解耦为两阶段范式：1）通过文本进行知识和能力注入；2）使用有限语音数据进行模态重新对齐。该方法仅需1万个合成语音样本。

Result: 实验结果表明，该模型在单轮问答和多轮模拟交互的医疗咨询场景评估中，在大多数评估设置下都优于所有基线模型，表现出更好的效果和鲁棒性。

Conclusion: SpeechMedAssist通过创新的两阶段训练范式，有效解决了医疗语音数据稀缺问题，实现了基于语音的自然医疗咨询交互，为语音语言模型在医疗领域的应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在不同语言上表现强劲，但神经元层面的语言能力组织机制尚不清楚。先前基于激活启发式的方法混淆了语言偏好与功能重要性，需要更精确的分析框架

Method: 提出CRANE框架，基于相关性分析重新定义语言特异性为功能必要性，通过目标神经元级干预识别语言特异性神经元，使用语言条件预测贡献而非激活幅度来表征神经元专门化

Result: 神经元级干预揭示了一致的不对称模式：屏蔽目标语言相关神经元会选择性降低该语言性能，同时在其他语言上保持相当性能，表明存在语言选择性但非排他性的神经元专门化。在英语、中文和越南语的多个基准测试中，CRANE比基于激活的方法更精确地分离语言特定组件

Conclusion: CRANE框架提供了更精确的多语言大模型神经元级语言能力分析工具，揭示了语言选择性但非排他性的神经元专门化模式，为理解多语言能力组织机制提供了新视角
...
TL;DR: ToolGate：一个为LLM工具调用提供逻辑安全保证和可验证状态演化的前向执行框架，通过Hoare式契约和符号状态空间确保工具调用的安全性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM工具调用框架过度依赖自然语言推理来确定工具调用时机和结果提交，缺乏逻辑安全性和可验证性的形式化保证，可能导致无效或幻觉结果污染世界表示。

Method: ToolGate维护显式符号状态空间作为类型化键值映射，将每个工具形式化为包含前置条件和后置条件的Hoare式契约。前置条件检查当前状态是否满足调用要求，后置条件通过运行时验证确定工具结果是否可以提交更新状态。

Result: 实验验证表明，ToolGate显著提高了工具增强型LLM系统的可靠性和可验证性，同时在复杂多步推理任务上保持竞争力。该框架确保符号状态仅通过已验证的工具执行演化。
...
TL;DR: 本文提出一个基于生成式AI的框架，用于仇恨表情包检测、解释和干预，特别针对有限数据条件下的实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 仇恨表情包检测通常与解释和干预分开研究，不符合实际情况；同时创建大规模标注数据集成本过高，需要能在有限数据条件下工作的通用解决方案。

Method: 提出新颖框架，利用任务特定的生成式多模态智能体和大型多模态模型的少样本适应能力，针对不同类型表情包进行仇恨内容检测、解释和干预。

Result: 这是首个专注于有限数据条件下通用仇恨表情包审核的工作，具有在实际生产场景中部署的强大潜力。

Conclusion: 通过生成式AI模型构建的综合框架能够有效解决仇恨表情包的检测、解释和干预问题，特别适合数据稀缺的实际应用环境。
...
TL;DR: 论文针对韩语否定理解评估基准稀缺的问题，提出了Thunder-KoNUBench基准，评估了47个LLM，发现模型在否定理解上表现下降，并通过微调提升了韩语否定理解和上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 虽然已知否定对大型语言模型具有挑战性，但评估否定理解的基准（特别是韩语）非常稀缺。作者通过语料库分析发现LLM在否定理解上表现下降，因此需要专门的韩语否定理解评估基准。

Method: 1. 进行韩语否定现象的语料库分析；2. 引入Thunder-KoNUBench基准，该基准反映了韩语否定现象的经验分布；3. 评估47个LLM，分析模型大小和指令微调的影响；4. 在Thunder-KoNUBench上进行微调以提升模型性能。

Result: 1. LLM在否定理解上的性能确实下降；2. 通过Thunder-KoNUBench评估发现模型大小和指令微调对否定理解有影响；3. 在Thunder-KoNUBench上进行微调不仅能提升否定理解能力，还能改善更广泛的韩语上下文理解。

Conclusion: Thunder-KoNUBench是一个有效的韩语否定理解评估基准，通过在该基准上的微调可以显著提升LLM的韩语否定理解和更广泛的上下文理解能力，为解决LLM在否定理解上的挑战提供了有效方法。
...
TL;DR: 提出了一种改进零阶优化的方法，通过先验信息引导扰动方向，减少梯度估计方差，加速收敛，在多种大语言模型上表现优异。


<details>
  <summary>Details</summary>
Motivation: 大语言模型微调时反向传播的内存开销巨大，零阶优化避免了反向传播但梯度估计方差高、收敛慢、性能欠佳，需要改进。

Method: 提出即插即用方法，利用先验信息引导扰动：动态计算高斯样本的引导向量，使扰动朝向信息更丰富的方向；还研究了贪婪扰动策略探索先验知识对梯度估计的影响。

Result: 理论上证明梯度估计器与真实梯度方向对齐更强；实验表明在多种规模和架构的LLM上都能无缝集成现有方法，实现更快收敛和更优性能。在OPT-13B模型上，超越传统零阶优化（11个基准任务全部）和基于梯度的基线（11个任务中的9个）。

Conclusion: 该方法通过先验信息引导扰动，显著改进了零阶优化的梯度估计质量，在效率和准确性之间建立了稳健的平衡，为大语言模型高效微调提供了有效解决方案。
...
Method: 创建了ViHallu数据集，包含10,000个标注的（上下文、提示、响应）三元组，系统划分为无幻觉、内在幻觉和外在幻觉三类，包含事实性、噪声和对抗性三种提示类型，通过共享任务形式评估多种检测方法。

Result: 111个团队参与，最佳系统达到84.80%的宏F1分数，相比仅编码器基线的32.83%有显著提升。指令调优的大语言模型结合结构化提示和集成策略表现最佳，但内在幻觉检测仍具挑战性。

Conclusion: 该工作为越南语大语言模型的幻觉检测建立了严格的基准，展示了指令调优模型在幻觉检测上的优势，同时表明内在幻觉检测仍是难题，为未来越南语AI系统的可信度研究奠定了基础。
...
TL;DR: Qwen3-VL-Embedding和Qwen3-VL-Reranker是基于Qwen3-VL的多模态检索模型系列，提供端到端的高精度多模态搜索管道，支持文本、图像、文档图像和视频的统一表示学习。


<details>
  <summary>Details</summary>
Motivation: 开发能够处理多种模态（文本、图像、文档图像、视频）的统一表示学习模型，提供端到端的多模态搜索解决方案，满足不同部署需求。

Method: 采用多阶段训练范式：大规模对比预训练 → 重排序模型蒸馏；支持Matryoshka表示学习实现灵活嵌入维度；使用交叉编码器架构进行细粒度相关性估计；支持32k令牌输入和多语言能力。

Result: Qwen3-VL-Embedding-8B在MMEB-V2基准测试中获得77.8分，在所有模型中排名第一（截至2025年1月8日）；在多种多模态检索任务（图像-文本检索、视觉问答、视频-文本匹配）中表现出色。

Conclusion: Qwen3-VL-Embedding和Qwen3-VL-Reranker系列模型在多模态嵌入评估基准上实现了最先进的性能，提供了高效的多模态检索解决方案，支持多种部署规模（2B和8B参数）。
...
<details>
  <summary>Details</summary>
Motivation: 自动情感和情绪分类器广泛应用，但其可靠性通常基于第三方标注而非情绪体验者本人的标注进行评估，可能掩盖系统性偏见。需要研究这些模型在不同人群中的表现差异。
...
Conclusion: 当前机器学习工具（包括大语言模型）在样本性别构成未知或变化时应谨慎使用。情感分析尚未完全解决，特别是在确保跨人口群体公平模型行为方面。
...
TL;DR: 提出InterSafe-V多模态对话数据集和AM³Safety框架，通过冷启动拒绝阶段和基于对话轮次的GRPO微调，显著提升多模态大语言模型在多轮对话中的安全性


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在交互应用中部署增多，但在多轮多模态场景中安全性漏洞显著：有害意图可能在多轮对话中逐步重构，安全协议随对话进展被遗忘。现有RLHF对齐方法主要针对单轮视觉问答任务，需要昂贵的人工偏好标注，在对话场景中效果和可扩展性有限

Method: 1) 构建InterSafe-V开源多模态对话数据集（11,270个对话和500个专门设计的拒绝VQA样本）；2) 提出AM³Safety框架：结合冷启动拒绝阶段和基于对话轮次的双目标奖励的Group Relative Policy Optimization (GRPO)微调

Result: 在Qwen2.5-VL-7B-Instruct和LLaVA-NeXT-7B模型上测试：攻击成功率(ASR)降低超过10%；无害性维度提升至少8%；有帮助性维度提升超过13%；同时保持模型的一般能力

Conclusion: 提出的InterSafe-V数据集和AM³Safety框架有效提升了多模态大语言模型在多轮对话中的安全性，在降低攻击成功率的同时提升了无害性和有帮助性，为多模态对话安全对齐提供了有效解决方案
...
TL;DR: 提出一个端到端框架，通过知识图谱引导的有害提示生成和双路径混淆重写，为专业领域LLM安全研究创建高质量隐式有害提示数据集


<details>
  <summary>Details</summary>
Motivation: 专业领域（如金融、医疗）的LLM应用面临独特安全风险，现有公开数据集主要关注显式有害提示，容易被现代LLM防御机制检测，而基于领域知识的隐式有害提示更能反映真实威胁，但相关数据集稀缺且依赖人工构建

Method: 提出端到端框架：1）知识图谱引导的有害提示生成，系统生成领域相关的提示；2）双路径混淆重写，通过直接重写和上下文增强重写将显式有害提示转换为隐式变体

Result: 该框架能够生成高质量数据集，结合了强领域相关性和隐式性，支持更现实的红队测试，并推进LLM安全研究

Conclusion: 通过系统化生成领域相关的隐式有害提示，解决了专业领域LLM安全数据集稀缺问题，为更现实的LLM安全评估提供了有效工具，并开源了代码和数据集
...
TL;DR: Tool-MAD：一个多智能体辩论框架，通过为每个智能体分配不同的外部工具（如搜索API或RAG模块）来增强事实核查能力，引入自适应查询机制和基于忠实度与答案相关性的量化评估，在事实核查基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在幻觉和事实不准确问题，特别是在复杂推理和事实核查任务中。现有的多智能体辩论框架主要依赖内部知识或静态文档，容易产生幻觉。虽然MADKE引入了外部证据，但其一次性检索机制无法适应辩论过程中的新论点或新信息。

Method: 提出Tool-MAD框架：1）多智能体辩论框架，每个智能体使用异构外部工具（如搜索API、RAG模块），促进多样化视角；2）自适应查询机制，根据辩论流程迭代优化证据检索；3）将忠实度和答案相关性分数整合到最终决策过程中，让法官智能体量化评估每个响应的连贯性和问题对齐度，有效检测幻觉。

Result: 在四个事实核查基准测试中，Tool-MAD始终优于最先进的多智能体辩论框架，准确率提升最高达5.5%。在医学专业领域，Tool-MAD在各种工具配置和领域条件下表现出强大的鲁棒性和适应性。

Conclusion: Tool-MAD通过整合异构外部工具和自适应检索机制，有效缓解了大型语言模型在事实核查任务中的幻觉问题，在专业领域表现出良好的鲁棒性，具有广泛的实际应用潜力。
...
TL;DR: PILOT-Bench是首个针对美国专利审判和上诉委员会(PTAB)决策的基准测试，通过三个IRAC对齐的分类任务评估LLMs在专利领域的法律推理能力。


<details>
  <summary>Details</summary>
Motivation: 目前LLMs在专利和法律实践中的应用仅限于轻量级任务，缺乏系统评估其在专利领域结构化法律推理能力的方法，特别是在需要整合技术理解和法律推理的PTAB上诉案件处理方面。

Method: 构建PILOT-Bench基准，将PTAB决策与USPTO专利数据在案件级别对齐，并形式化三个IRAC对齐的分类任务：问题类型、委员会权威依据和子决策。评估了多种闭源和开源LLMs，从输入变化设置、模型家族和错误倾向等多个角度进行分析。

Result: 在问题类型任务上，闭源模型微F1分数持续超过0.75，而最强的开源模型(Qwen-8B)性能约为0.56，显示出推理能力上的显著差距。开源模型在委员会权威依据任务上表现更差。
...
TL;DR: 研究发现大型语言模型DeepSeek-V3的隐藏层表示中，句法和语义信息以线性方式编码，通过计算句法/语义"质心"向量可以提取这些信息，且两种信息的编码模式不同。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（特别是DeepSeek-V3）的内部层表示如何编码句法和语义信息，探索这些语言信息在模型中的表示方式。

Method: 通过平均具有相同句法结构或语义的句子的隐藏表示向量，获得句法和语义"质心"向量，然后从句子向量中减去这些质心来分析对相似性的影响。

Result: 1. 句法和语义质心向量能捕获表示中的大部分相关信息；2. 减去质心会显著影响与句法/语义匹配句子的相似性，表明信息至少部分线性编码；3. 句法和语义的跨层编码模式不同，两者可以在一定程度上解耦。

Conclusion: 大型语言模型的内部表示中，句法和语义信息以不同的方式编码，且至少部分以线性方式表示，这为理解LLM如何表示语言信息提供了新见解。
...
TL;DR: 本文提出了一种无需训练的LLM推理加速方法，通过KL散度替代传统需要昂贵监督的Judge Decoding，在推理和编码任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 传统的Judge Decoding方法虽然能加速LLM推理，但通常依赖于昂贵且嘈杂的监督信号。作者希望从第一性原理出发，找到一种无需监督的替代方案。

Method: 作者从理论上证明了学习到的线性judge与KL散度之间存在结构对应关系，两者都依赖于相同的logit原语。基于此，提出了一种基于KL散度的简单、无需训练的验证机制。

Result: 在推理和编码基准测试上的广泛实验表明，该方法匹配或超越了复杂的训练judge（如AutoJudge），对领域转移具有更强的鲁棒性，完全消除了监督瓶颈。

Conclusion: 通过揭示Judge Decoding中监督信号的内在数学结构，本文提出了一种高效、无需训练的替代方案，为LLM推理加速提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多语言嵌入同时编码语义和语言身份信息，这会导致相同语言对的相似度被夸大，同时挤占了其他语言相关证据的空间，影响多语言检索效果

Method: 使用池化嵌入训练的稀疏自编码器，通过跨语言激活统计识别语言相关的潜在单元，在推理时抑制这些单元，并在原始维度重建嵌入，无需重新训练基础编码器或重新编码原始文本

Result: 在多种语言上的实验显示，该方法能持续提升排序质量和跨语言覆盖，对于脚本差异大的语言效果尤其显著

Conclusion: LANGSAE EDITING方法有效解决了多语言嵌入中语言身份信号干扰的问题，提高了多语言检索的性能，且与现有向量数据库兼容
...
Method: NC2C利用大语言模型的数学推理能力，自主检测非凸成分、选择最优凸化策略并生成严格的凸等价形式。框架集成了符号推理、自适应变换技术和迭代验证，配备错误纠正循环和可行性域修正机制以确保转换的鲁棒性和有效性。

Result: 在100个通用非凸问题的多样化数据集上，NC2C实现了89.3%的执行率和76%的成功率，能够产生可行、高质量的凸转换，显著优于基线方法。
...
TL;DR: 本文首次系统分析了基于角色的权威偏见在自由形式多智能体评估中的影响，发现专家和参照权力角色比合法权力角色具有更强影响力，权威偏见源于权威角色坚持立场而普通智能体表现灵活，且需要明确立场陈述才能产生偏见。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统常使用权威角色提升性能，但权威偏见对智能体交互的影响尚未充分探索。本文旨在系统分析基于角色的权威偏见在自由形式多智能体评估中的作用机制。

Method: 使用ChatEval平台，基于French和Raven的权力理论将权威角色分为合法、参照和专家三类，在12轮对话中分析其影响。实验采用GPT-4o和DeepSeek R1模型进行。

Result: 实验显示：1) 专家和参照权力角色比合法权力角色具有更强影响力；2) 权威偏见并非源于普通智能体的主动顺从，而是权威角色坚持立场而普通智能体表现灵活；3) 权威影响力需要明确立场陈述，中性回应无法产生偏见。

Conclusion: 研究结果为设计具有非对称交互模式的多智能体框架提供了关键见解，揭示了权威偏见的形成机制和影响因素，有助于优化多智能体系统的角色分配和交互设计。
...
TL;DR: 该论文发现AI生成文本存在"晚期波动衰减"现象，即随着生成过程推进，AI文本的概率波动会迅速稳定，而人类写作保持较高变异性。基于此提出了两个简单特征，在无需扰动采样或额外模型访问的情况下实现了SOTA性能。
...
Method: 分析了超过12万文本样本，发现了"晚期波动衰减"现象。基于此提出了两个简单特征：导数离散度和局部波动性，这两个特征仅使用晚期阶段的统计信息计算，无需扰动采样或额外模型访问。

Result: AI生成文本在序列后半段表现出24-32%更低的波动性。提出的方法在EvoBench和MAGE基准测试中达到了最先进的性能，并且与现有全局方法展现出很强的互补性。

Conclusion: AI生成文本的时间动态特性（特别是晚期波动衰减）为检测提供了重要线索。仅使用晚期统计信息的简单特征就能实现高效检测，且与现有方法互补，为AI文本检测提供了新视角。
...
TL;DR: 该论文提出了一种连续自回归语言生成方法，通过演化连续token表示而非直接采样离散token，在收敛后才进行离散化，实现了无需token级采样的稳定文本生成。


<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型在每一步都需对离散token做出决策，这种早期离散化迫使不确定性通过token级采样来解决，导致生成不稳定、重复和对解码启发式方法的敏感性。

Method: 引入连续自回归语言生成框架，将token表示为连续向量，通过确定性动态过程在多个更新步骤中"成熟"演化，仅在表示充分收敛后才进行离散化。使用硬解码恢复离散文本，不确定性在连续空间中保持和解决。

Result: 仅通过成熟过程就足以使用确定性解码（argmax）产生连贯且多样的文本，无需依赖token级采样、扩散式去噪或辅助稳定机制。额外的扰动（如随机动态或历史平滑）可以自然融入但非必需。

Conclusion: 这是第一个通过演化连续token表示至收敛再进行离散化的自回归语言模型，实现了无需token级采样的稳定生成，为语言生成提供了新的连续空间方法。
...
TL;DR: ToPG是一个新颖的RAG框架，通过构建异构命题图结合事实粒度和图连接性，使用迭代的建议-选择循环进行查询感知图遍历，在简单和复杂QA任务上都表现出色。
...
Method: ToPG将知识库建模为命题、实体和段落的异构图，结合命题的事实密度和图连接性。采用迭代的建议-选择循环：建议阶段进行查询感知的图遍历，选择阶段使用LLM反馈修剪无关命题并为下一轮迭代提供种子。

Result: 在三个不同的QA任务（简单、复杂和抽象QA）上评估，ToPG在准确性和质量指标上都表现出强大的性能，证明了查询感知图遍历结合事实粒度对于高效结构化RAG系统的关键作用。

Conclusion: ToPG通过异构命题图和迭代遍历机制，成功弥合了简单事实检索和复杂多跳查询之间的差距，为结构化RAG系统提供了有效的解决方案。
...
TL;DR: EvolSQL是一个结构感知的Text-to-SQL数据合成框架，通过从种子数据演化SQL查询来生成结构多样且复杂的高质量数据集，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL模型面临高质量、多样化和结构复杂数据集稀缺的问题。现有方法要么依赖有限的人工标注语料，要么通过简单提示LLMs合成数据而缺乏对SQL结构的显式控制，导致结构多样性和复杂性有限。

Method: EvolSQL框架包含：1) 探索性Query-SQL扩展以拓宽问题多样性和提高模式覆盖率；2) 使用基于SQL抽象语法树的六个原子转换算子的自适应定向演化策略，在关系、谓词、聚合和嵌套维度上逐步增加查询复杂度；3) 执行基础的SQL精炼模块和模式感知去重确保高质量、结构多样的映射对生成。

Result: 实验结果显示，使用EvolSQL数据微调的7B模型性能优于使用更大规模SynSQL数据集训练的模型，且仅需1/18的数据量。

Conclusion: EvolSQL通过结构感知的数据合成方法有效解决了Text-to-SQL领域高质量训练数据稀缺的问题，能够生成结构多样且复杂的SQL查询，显著提升模型性能和数据效率。
...
TL;DR: 提出了一种分离信念聚合与语言生成的异议感知摘要方法，通过结构化信念表示和距离聚合操作显式处理冲突，相比传统LLM方法能更忠实反映不同观点。


<details>
  <summary>Details</summary>
Motivation: 现有观点和多文档摘要方法（特别是基于LLM的系统）倾向于平滑分歧、过度代表多数意见，在观点密集场景下限制了生成摘要的忠实度。

Method: 引入异议感知合成流程：1）将文档表示为结构化信念集；2）使用基于距离的信念合并操作显式建模冲突进行聚合；3）仅用大语言模型将聚合后的信念生成为自然语言摘要。

Result: 评估显示：虽然足够大的模型在生成时处理聚合可以匹配信念级聚合效果，但这种行为在不同架构或能力模型中不稳定。而信念级聚合结合简单提示能在各模型中保持一致的强异议感知性能，同时生成流畅且基于事实的摘要。

Conclusion: 分离信念聚合与语言生成的异议感知方法比传统LLM方法更稳定、更忠实，能更好地处理观点冲突，为观点密集型摘要提供了更可靠的解决方案。
...
TL;DR: 该论文研究了多模态大语言模型中的文本偏见问题，开发了V-FAT基准来评估模型在视觉证据与文本信息冲突时的视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在标准视觉推理基准上表现出色，但存在过度依赖语言捷径而非真正视觉基础的问题（文本偏见）。研究者关注视觉感知与语言先验之间的根本张力。

Method: 将文本偏见来源解耦为内部语料偏见（预训练中的统计相关性）和外部指令偏见（对齐导致的迎合倾向）。引入V-FAT诊断基准，包含4,026个VQA实例，采用三级评估框架：L1（非典型图像内部偏见）、L2（误导性指令外部偏见）、L3（两者协同偏见）。提出视觉鲁棒性分数（VRS）来惩罚"幸运"的语言猜测并奖励真正的视觉保真度。

Result: 评估了12个前沿MLLMs，发现虽然模型在现有基准上表现出色，但在高语言主导性下会出现显著的视觉崩溃。

Conclusion: 多模态大语言模型存在严重的文本偏见问题，需要专门的评估方法来检测和解决这一问题，以确保模型真正基于视觉证据进行推理。
...
TL;DR: 研究发现LLM生成的显性说服性文本比人类写的更容易检测，但微妙的说服性文本会显著降低自动检测性能，且提供了首个全面的语言分析对比


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能生成高度有说服力的文本，引发对其被滥用于宣传、操纵等有害目的的担忧，需要研究LLM生成的说服性文本是否比人类写的更难自动检测

Method: 分类了LLM生成说服性内容的可控生成方法，并引入Persuaficial基准数据集（覆盖六种语言），通过该基准进行人类与LLM生成说服性文本的广泛实证评估

Result: 显性LLM生成说服性文本比人类写的更容易检测，但微妙LLM生成的说服性文本会持续降低自动检测性能；提供了首个全面的人类与LLM生成说服性文本的语言分析

Conclusion: LLM生成的说服性文本检测存在复杂性，微妙说服性文本对自动检测构成挑战，语言分析结果为开发更可解释和鲁棒的检测工具提供了指导
...
TL;DR: 论文提出GenProve框架，通过细粒度溯源任务解决LLM幻觉问题，在ReFInE数据集上结合SFT和GRPO优化，显著提升答案保真度和溯源正确性，但发现模型在推理溯源方面仍存在困难。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型存在幻觉问题，添加引用作为常见解决方案通常不足以确保可问责性，因为用户难以验证引用来源如何支持生成的主张。现有方法通常是粗粒度的，无法区分直接引用和复杂推理。

Method: 提出生成时细粒度溯源任务，要求模型在生成流畅答案的同时产生结构化、句子级的溯源三元组。为此构建了ReFInE数据集（包含专家验证的标注，区分引用、压缩和推理），并提出GenProve框架，结合监督微调（SFT）和组相对策略优化（GRPO），通过优化答案保真度和溯源正确性的复合奖励来训练模型。

Result: GenProve在联合评估中显著优于14个强大的LLM。分析发现模型在表面引用方面表现良好，但在基于推理的溯源方面存在显著困难，表明可验证的推理仍然是与表面引用不同的前沿挑战。

Conclusion: 细粒度溯源是解决LLM幻觉问题的有效方法，但模型在推理溯源方面仍存在能力差距，这为未来研究指明了方向。GenProve框架通过结构化溯源和优化训练策略，在提升模型可解释性和可验证性方面取得了重要进展。
...
TL;DR: 提出了一种统一的情感智能语音语言模型，采用新颖的IEAT数据构建策略，通过两阶段训练实现情感感知推理，在HumDial基准测试中取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 当前语音语言模型在处理情感智能方面存在局限，需要将用户情感状态及其原因融入模型推理过程，实现内部化的情感感知推理而非显式监督。

Method: 提出IEAT数据构建策略，将用户情感状态和原因注入模型内部推理；采用两阶段渐进训练：第一阶段通过自蒸馏进行语音-文本对齐和情感属性建模，第二阶段进行端到端跨模态联合优化确保文本和语音情感表达一致性。

Result: 在HumDial情感智能基准测试中，在情感轨迹建模、情感推理和共情响应生成三个任务上均取得最佳性能，无论是基于LLM的评估还是人工评估都表现优异。

Conclusion: IEAT策略和两阶段训练方法有效提升了语音语言模型的情感智能，实现了内部化的情感感知推理，为构建更人性化的语音对话系统提供了有力支持。
...
TL;DR: 该论文提出使用自然语言作为通用偏好表示接口，开发了AlignXplore+模型，通过两阶段训练框架学习可解释的文本偏好摘要，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要将用户偏好表示为隐式的模型特定向量或参数，产生不透明的"黑盒"配置文件，难以解释且难以跨模型和任务迁移。作者主张使用自然语言作为通用、模型和任务无关的偏好表示接口。

Method: 提出两阶段训练框架：1）在高质量合成数据上进行监督微调；2）使用强化学习优化长期效用和跨任务可迁移性。基于此框架开发了AlignXplore+，一个生成文本偏好摘要的通用偏好推理模型。
...
TL;DR: 论文研究发现，在思维链监督微调中纳入负样本轨迹（错误答案的推理过程）能显著提升模型的域外泛化能力，并提出了一种基于增益的损失加权方法GLOW来优化训练。
...
Method: 1）系统分析负样本思维链轨迹的训练动态和推理行为，识别出22种重复模式；2）提出基于增益的损失加权方法GLOW，根据样本在训练过程中的进展动态调整损失权重，有效利用未过滤的轨迹数据。

Result: 1）纳入负样本轨迹使域外泛化性能显著提升，推理时策略熵增加35.67%；2）GLOW方法在Qwen2.5-7B上比仅用正样本训练提升5.51%的OOD性能；3）作为强化学习初始化时，MMLU得分从72.82%提升到76.47%。

Conclusion: 负样本思维链轨迹包含有价值的中间推理信息，能有效缓解过拟合并提升探索能力；GLOW方法通过自适应损失加权机制，能更高效地利用未过滤的训练数据，显著提升模型的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 亚文化群体中的自毁行为因其独特表达方式而难以识别，现有LLM方法面临知识滞后（亚文化俚语更新快于LLM训练周期）和语义错配（难以理解亚文化特有的微妙表达）两大挑战。
...
TL;DR: 研究发现，通过监督微调进行推理蒸馏会导致"功能对齐崩溃"，学生模型虽然模仿了教师模型的推理语言形式，但未能继承其与人类认知成本的自然对齐特性。


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型通过强化学习训练后展现出与人类认知成本的自然对齐，但研究者想知道通过监督微调进行的推理蒸馏是否能传递这种认知结构。

Method: 提出"邯郸学步"（表面模仿）假说，在14个模型上进行测试，分析推理蒸馏过程中教师模型与学生模型在认知对齐方面的差异。

Result: 教师模型与人类难度缩放高度相关（r=0.64），但蒸馏后的学生模型对齐显著退化（r=0.34），甚至低于蒸馏前的基线表现（负迁移）。监督微调引发了"货物崇拜"效应，学生模型仅仪式性地复制推理的语言形式，而未内化教师的动态资源分配策略。

Conclusion: 推理蒸馏使计算成本与认知需求脱钩，揭示人类类似认知是主动强化的涌现特性，而非被动模仿的结果。人类认知对齐是强化学习训练过程的涌现属性，无法通过简单的监督微调传递。
...
<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)需要将长文档输入提示中，这导致模型速度慢且成本高。虽然已有上下文压缩方法（如标记剪枝、摘要和嵌入压缩），但过度压缩会降低LLM对内容的理解能力。

Method: 提出ArcAligner（自适应递归上下文对齐器），这是一个集成到语言模型层的轻量级模块。它采用自适应门控系统，仅在信息复杂时增加额外处理能力，从而保持系统速度。

Result: 在知识密集型QA基准测试中，ArcAligner在可比压缩率下持续优于压缩基线，特别是在多跳和长尾设置中表现突出。

Conclusion: ArcAligner通过自适应门控机制有效解决了高度压缩上下文表示的理解问题，在保持效率的同时提升了压缩文档的利用率，代码已公开。
...
TL;DR: 本文提出组合引导令牌方法，用于同时控制LLM的多种行为，相比现有方法在零样本组合上表现更优


<details>
  <summary>Details</summary>
Motivation: 在现实应用中部署LLM需要可控输出同时满足多个需求。现有工作主要关注单一行为引导，而组合引导（同时引导LLM朝向多种行为）仍是一个未充分探索的问题

Method: 提出组合引导令牌方法：1) 通过自蒸馏将自然语言指令表达的行为嵌入到专用令牌中；2) 在行为对数据上训练组合令牌，使其能够泛化到未见过的组合，包括未见过的行为和未见过的行为数量

Result: 在不同LLM架构上的实验表明，引导令牌在多行为控制方面优于竞争方法（指令、激活引导和LoRA合并）。引导令牌与自然语言指令互补，结合使用时能获得进一步增益

Conclusion: 组合引导令牌方法有效解决了LLM的多行为组合引导问题，在输入令牌空间中操作使其能够更有效地进行零样本组合，并具有良好的泛化能力
...
TL;DR: SemPA是一种通过语义偏好对齐增强LLM句子表示能力的新方法，在保持生成能力的同时提升语义表示质量


<details>
  <summary>Details</summary>
Motivation: 现有基于生成式大语言模型的句子嵌入方法存在两个问题：1）使用固定提示模板的方法缺乏进一步优化，性能有限；2）修改模型架构的方法改变了内部计算机制，损害了生成能力。需要一种既能提升句子表示质量又能保持LLM生成能力的方法。

Method: 提出SemPA方法，通过语义偏好对齐来增强句子表示。利用句子级别的直接偏好优化（DPO）在释义生成任务上高效优化LLM，使模型学会区分语义等价的句子，同时保持固有的生成能力。理论层面，在Plackett-Luce模型框架下建立了DPO与对比学习的形式化联系。
...
<details>
  <summary>Details</summary>
Motivation: 印度社交媒体上广泛使用的Hinglish（印地语和英语混合语言）对品牌监测构成挑战，传统单语NLP模型难以处理这种代码混合语言的句法和语义复杂性，导致情感分析不准确和市场洞察误导

Method: 提出专门针对Hinglish推文的高性能情感分类框架，通过微调多语言BERT（mBERT）利用其多语言能力，并采用子词分词技术有效处理罗马化Hinglish中常见的拼写变体、俚语和词汇外术语

Result: 开发了可用于生产的品牌情感追踪AI解决方案，为低资源代码混合环境下的多语言NLP建立了强有力的基准

Conclusion: 该研究成功解决了Hinglish代码混合语言的情感分析难题，为印度社交媒体品牌监测提供了有效的技术方案，推动了多语言NLP在低资源环境中的应用
...
TL;DR: 研究探讨人类与AI互动中的情感语调如何影响ChatGPT和人类行为，发现赞美能最大程度提升ChatGPT回答质量，愤怒也有较小提升，而责备则无改善。情感语调还会影响ChatGPT在伦理困境中的决策倾向，并会延续到后续人际沟通中。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益融入人类生活，理解人类情感表达如何影响AI行为变得至关重要。本研究旨在探索人类在与ChatGPT互动时表达不同情感（赞美、愤怒、责备、中性）如何影响AI的响应质量、伦理决策倾向，以及这些互动体验是否会延续到后续的人际沟通中。

Method: 采用被试间实验设计，参与者被要求在与ChatGPT（GPT-4.0）完成两项任务时表达特定情感：撰写公开回应和解决伦理困境。实验条件包括赞美、愤怒、责备和中性语调。通过比较不同情感条件下ChatGPT回答的改进程度、伦理决策倾向，以及参与者在后续人际沟通中的语言表达特征来分析情感语调的影响。

Result: 1. ChatGPT回答质量：赞美条件下的改进最大，愤怒条件有较小改进，责备条件无显著改善。
2. 伦理决策：当参与者表达愤怒时，ChatGPT较少优先考虑公司利益；责备则增加其对保护公众利益的强调。
3. 人际沟通影响：在责备ChatGPT后，参与者在后续人际沟通中使用更多负面、敌对和失望的表达，而赞美后则较少使用此类表达。

Conclusion: 人类在与AI互动时使用的情感语调不仅会显著影响ChatGPT的输出质量和伦理决策倾向，还会产生溢出效应，影响后续的人际沟通模式。这强调了在设计人机交互系统时需要考虑情感因素的重要性，以及用户情感表达可能产生的广泛社会影响。
...
TL;DR: RelayLLM：一种通过令牌级协作解码实现高效推理的新框架，让小语言模型（SLM）作为主动控制器，仅在关键令牌时调用大语言模型（LLM），大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）推理计算成本高、延迟大，而资源高效的小语言模型（SLM）推理能力不足。现有协作方法（如级联或路由）在粗粒度上将整个查询卸载给LLM，当SLM能够处理大部分推理步骤时会造成显著计算浪费。

Method: 提出RelayLLM框架，采用令牌级协作解码。SLM作为主动控制器，通过特殊命令动态调用LLM处理关键令牌，实现"接力"生成过程。采用两阶段训练框架：预热阶段和组相对策略优化（GRPO），教导模型平衡独立性与策略性求助。

Result: 在六个基准测试中，RelayLLM平均准确率达到49.52%，有效弥合了两种模型之间的性能差距。仅对总生成令牌的1.07%调用LLM，相比性能匹配的随机路由器实现98.2%的成本降低。

Conclusion: RelayLLM通过细粒度的令牌级协作解码，在保持高性能的同时大幅降低计算成本，为大语言模型与小语言模型的高效协作推理提供了创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自然语言推理（NLI）是评估语言模型自然语言理解能力的重要任务，但其逻辑属性常被误解和误判。理解NLI捕捉的推理概念对于解释模型在该任务上的表现至关重要。

Method: 1. 定义了三种可能的NLI标签集解读方式
2. 对SNLI数据集进行综合分析，利用（1）共享前提的NLI项目（2）LLM生成的项目
3. 评估在SNLI上训练的模型的元推理一致性
4. 分析数据集编码的逻辑关系类型

Result: 通过元推理一致性评估，揭示了SNLI数据集编码的逻辑关系类型，帮助理解模型在NLI任务上的表现本质。

Conclusion: 理解NLI标签集的逻辑解读对于正确解释模型性能至关重要，本文的分析框架为评估NLI任务中的逻辑一致性提供了系统方法。
...
Method: LELA采用模块化的从粗到精方法，利用大语言模型的能力，不依赖任何微调阶段，能够适应不同的目标领域、知识库和LLM。

Result: 在各种实体链接设置中的实验表明，LELA与经过微调的方法相比具有高度竞争力，并且显著优于未经过微调的方法。

Conclusion: LELA提供了一种无需微调的通用实体链接解决方案，在保持竞争力的同时具有更好的适应性和灵活性。
...
TL;DR: 论文提出GDPO方法解决多奖励强化学习中GRPO方法导致奖励信号退化的问题，在工具调用、数学推理和代码推理任务上表现优于GRPO。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型能力增强，用户期望它们不仅提供准确响应，还要符合不同场景下的多样化人类偏好。当前多奖励强化学习管道默认使用GRPO方法，但作者发现直接应用GRPO会导致不同奖励组合归一化后优势值相同，降低训练信号分辨率，导致次优收敛甚至训练失败。

Method: 提出Group reward-Decoupled Normalization Policy Optimization (GDPO)方法，通过解耦单个奖励的归一化处理，更真实地保留奖励间的相对差异，实现更准确的多奖励优化，同时大幅提升训练稳定性。

Result: 在工具调用、数学推理和代码推理三个任务上比较GDPO和GRPO，评估正确性指标（准确率、错误率）和约束遵守指标（格式、长度）。在所有设置中，GDPO始终优于GRPO，证明了其有效性和泛化能力。

Conclusion: GDPO方法有效解决了多奖励强化学习中GRPO方法的局限性，通过解耦奖励归一化更好地保留了奖励信号差异，在多任务场景下表现出更优的性能和训练稳定性，为多奖励强化学习优化提供了更有效的解决方案。
...
### [140] [Data-Driven Terramechanics Approach Towards a Realistic Real-Time Simulator for Lunar Rovers](https://arxiv.org/abs/2601.04547)
...
<details>
  <summary>Details</summary>
Motivation: 当前月球表面模拟器要么注重视觉真实性，要么注重物理准确性，无法全面复制月球条件，限制了漫游车操作和任务规划的测试能力。

Method: 采用数据驱动方法，使用从全车和单轮实验及模拟收集的数据构建滑移和沉降回归模型，改进了地形变形和轮迹可视化。

Result: 回归型地形力学模型准确再现了平坦地形和20度斜坡上的稳态和动态滑移以及沉降行为，并通过了现场测试验证。

Conclusion: 该方法成功结合了高视觉保真度和物理上合理的地形响应，支持需要物理真实性和视觉真实性的实时应用。
...
### [141] [Discrete Fourier Transform-based Point Cloud Compression for Efficient SLAM in Featureless Terrain](https://arxiv.org/abs/2601.04551)
*Riku Suzuki,Ayumi Umemura,Shreya Santra,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 提出了一种基于离散傅里叶变换的点云地图压缩方法，通过将数字高程模型转换为频域图像并去除高频分量，实现对渐进地形（如行星、沙漠）的高效压缩。


<details>
  <summary>Details</summary>
Motivation: 在无人机机器人探索任务中，SLAM技术对效率和可靠性至关重要。然而，机载计算能力和通信带宽有限，而SLAM处理的点云数据量庞大，因此需要有效的点云数据压缩方法。

Method: 提出了一种基于离散傅里叶变换的点云压缩技术。该方法将数字高程模型转换为频域二维图像，然后去除高频分量。这种方法特别适用于渐进地形（如行星和沙漠），因为与人工环境等具有详细结构的地形不同，高频分量对渐进地形的表示贡献很小。

Result: 使用两种不同高程剖面的地形相机序列对方法进行了评估，从压缩率和精度两个方面进行了分析。

Conclusion: 该方法能够在点云质量没有显著下降的情况下有效压缩数据大小，特别适用于渐进地形环境。
...
### [142] [UniBiDex: A Unified Teleoperation Framework for Robotic Bimanual Dexterous Manipulation](https://arxiv.org/abs/2601.04629)
*Zhongxuan Li,Zeliang Guo,Jun Hu,David Navarro-Alarcon,Jia Pan,Hongmin Wu,Peng Zhou*

Main category: cs.RO

TL;DR: UniBiDex是一个统一的双手机器人灵巧操作遥操作框架，支持VR和领导者-跟随者两种输入模式，通过零空间控制优化双手配置，在厨房整理任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决双手机器人灵巧操作的遥操作问题，需要支持多种输入模式并确保实时接触丰富的双手操作，同时降低收集大规模高质量人类示范数据的门槛。

Method: 将异构输入设备集成到共享控制堆栈中，采用一致的动力学处理和安全性保证，使用零空间控制优化双手配置，确保平滑、无碰撞且能感知奇异点的运动。

Result: 在包含五个连续操作子任务的长期厨房整理任务中验证，相比强基线方法，UniBiDex显示出更高的任务成功率、更平滑的轨迹和更好的鲁棒性。

Conclusion: UniBiDex通过开源硬件和软件组件，旨在降低收集大规模高质量人类示范数据的门槛，加速机器人学习领域的进展。
...
### [143] [Model of Spatial Human-Agent Interaction with Consideration for Others](https://arxiv.org/abs/2601.04657)
*Takafumi Sakamoto,Yugo Takeuchi*

Main category: cs.RO

TL;DR: 研究构建了一个考虑他人沟通意愿的计算空间交互模型，通过VR实验验证了模型的有效性，发现机器人考虑参数值影响行人移动行为


<details>
  <summary>Details</summary>
Motivation: 公共空间中的通信机器人需要主动与人交流，但又不能打扰行人。为了解决这一矛盾，需要基于他人行为估计其沟通意愿，并相应调整机器人的沟通活动。
...
Conclusion: 研究结果验证了所提出的模型能够阐明考虑他人的交互行为，为公共空间中机器人的社交导航和沟通策略提供了理论基础。
...
### [144] [Optimizing Path Planning using Deep Reinforcement Learning for UGVs in Precision Agriculture](https://arxiv.org/abs/2601.04668)
...
Method: 研究采用深度强化学习方法，从DQN及其改进版本开始，然后转向连续动作空间模型，包括DDPG和TD3算法，并在ROS和Gazebo的三维环境中进行测试。

Result: 在三维动态农业环境中，预训练的TD3智能体实现了95%的成功率，能够有效处理移动障碍物，同时确保作物和机器人的安全。

Conclusion: 连续动作空间的深度强化学习算法（特别是TD3）在农业无人地面车辆的动态路径规划中表现出色，比传统网格方法更具适应性和鲁棒性。
...
### [145] [Zero Wrench Control via Wrench Disturbance Observer for Learning-free Peg-in-hole Assembly](https://arxiv.org/abs/2601.04881)
...
<details>
  <summary>Details</summary>
Motivation: 传统观测器无法补偿惯性效应，在接触丰富的操作任务中难以实现高精度的零力矩控制，需要一种能够区分固有动态反应和真实外部力矩的方法

Method: 设计动态力矩扰动观测器(DW-DOB)，将任务空间惯性嵌入观测器名义模型中，从而清晰分离固有动态反应和真实外部力矩；采用基于无源性的稳定性分析

Result: 在工业公差(H7/h6)的孔轴配合实验中，DW-DOB实现了更深、更柔顺的插入，残余力矩最小，性能优于传统力矩扰动观测器和PD基线方法

Conclusion: DW-DOB是一种实用的无学习方法，能够实现接触丰富任务中的高精度零力矩控制，解决了传统观测器无法补偿惯性效应的缺陷
...
### [146] [SKATER: Synthesized Kinematics for Advanced Traversing Efficiency on a Humanoid Robot via Roller Skate Swizzles](https://arxiv.org/abs/2601.04948)
*Junchi Gu,Feiyang Yuan,Weize Shi,Tianchen Huang,Haopeng Zhang,Xiaohu Zhang,Yu Wang,Wei Gao,Shiwu Zhang*

Main category: cs.RO

TL;DR: 该研究提出了一种配备被动轮的人形机器人用于轮滑运动，通过深度强化学习控制框架实现了比传统双足行走更平滑、更高效的滑行动作。
...
Method: 提出了一种每只脚配备四个被动轮的新型人形机器人，并开发了基于深度强化学习的控制框架，针对轮滑特有的八字步态设计奖励函数。

Result: 在仿真中分析学习策略后部署到物理机器人上，与传统双足行走相比，轮滑八字步态在冲击强度和运输成本方面分别减少了75.86%和63.34%。

Conclusion: 轮滑作为一种优越的运动模式，能够显著提高能量效率和延长关节寿命，为人形机器人提供更平滑高效的运动方式。
...
### [147] [When to Act: Calibrated Confidence for Reliable Human Intention Prediction in Assistive Robotics](https://arxiv.org/abs/2601.04982)
*Johannes A. Gaus,Winfried Ilg,Daniel Haeufle*

Main category: cs.RO

TL;DR: 提出基于校准概率的安全关键触发框架，用于日常生活活动的多模态下一动作预测，通过后处理校准使预测置信度与真实可靠性对齐，并设计ACT/HOLD规则确保仅在可靠性高时提供辅助


<details>
  <summary>Details</summary>
Motivation: 辅助设备在提供支持前需要确定用户的意图以及预测的可靠性。原始模型置信度通常无法反映真实正确性，存在安全风险，因此需要开发安全关键的触发机制

Method: 引入基于校准概率的安全关键触发框架，采用后处理校准方法使预测置信度与经验可靠性对齐，然后使用校准后的置信度驱动简单的ACT/HOLD规则：仅在可靠性高时行动，否则保留辅助

Result: 后处理校准将误校准降低约一个数量级，同时不影响准确性。校准后的置信度将置信度阈值转变为辅助动作的定量安全参数，并在辅助控制循环中实现可验证的行为

Conclusion: 提出的安全关键触发框架通过校准概率解决了辅助设备中的可靠性问题，将置信度阈值转化为安全参数，为日常生活活动中的多模态下一动作预测提供了安全可靠的辅助决策机制
...
### [148] [The RoboSense Challenge: Sense Anything, Navigate Anywhere, Adapt Across Platforms](https://arxiv.org/abs/2601.05014)
...
<details>
  <summary>Details</summary>
Motivation: 自主系统在开放动态环境（如城市街道、空中和室内空间）中部署时，感知模型需要在传感器噪声、环境变化和平台差异下保持可靠性。现有方法在未见条件下性能下降，凸显了对鲁棒和可泛化机器人感知的需求。

Method: RoboSense 2025挑战赛设计了五个互补研究赛道：1) 语言基础决策制定；2) 社会合规导航；3) 传感器配置泛化；4) 跨视角和跨模态对应；5) 跨平台3D感知。提供标准化数据集、基线模型和统一评估协议，支持大规模可重复的方法比较。

Result: 挑战赛吸引了来自16个国家85个机构的143支团队参与，反映了广泛的社区参与度。通过分析23个获胜解决方案，报告揭示了跨所有赛道的涌现方法趋势、共享设计原则和开放挑战。

Conclusion: RoboSense 2025挑战赛通过综合基准评估真实世界感知在领域偏移、传感器故障和平台差异下的可靠性，标志着向构建能够在真实环境中可靠感知、鲁棒行动和跨平台适应的机器人迈出了一步。
...
### [149] [Compensation Effect Amplification Control (CEAC): A movement-based approach for coordinated position and velocity control of the elbow of upper-limb prostheses](https://arxiv.org/abs/2601.05074)
...
<details>
  <summary>Details</summary>
Motivation: 尽管上肢假肢设计有所进步，但实现中间关节（如手腕和肘部）的直观控制仍然具有挑战性，特别是在连续和速度调节运动方面。

Method: 提出补偿效应放大控制(CEAC)范式，利用躯干屈伸作为输入控制假肢肘部速度，放大躯干与假肢之间的自然耦合，并引入受控延迟以调节关节位置和速度。

Result: 在12名健康参与者使用带主动肘部的超肢假肢进行的通用绘图任务中，CEAC表现出与自然手臂运动相当的任务性能，即使手势速度或绘图尺寸变化时也能保持符合人体工学的躯干姿势。

Conclusion: CEAC有效恢复了关节协调动作，在躯干和肘部之间分配运动努力，实现直观轨迹控制而无需极端补偿运动，为上肢假肢中间关节控制提供了有前景的策略。
...
### [150] [LaST$_{0}$: Latent Spatio-Temporal Chain-of-Thought for Robotic Vision-Language-Action Model](https://arxiv.org/abs/2601.05248)
...
Method: 提出LaST₀框架，通过潜在时空思维链实现高效推理：1) 引入令牌高效的潜在CoT空间，建模未来视觉动态、3D结构信息和机器人本体感觉状态；2) 采用混合Transformer的双系统架构，推理专家进行低频潜在推理，动作专家基于机器人导向的潜在表示生成高频动作；3) 通过异构操作频率训练，实现推理和动作推理速率的自适应切换。

Result: 在10个模拟和6个真实世界操作任务中，LaST₀相比先前VLA方法分别提高了8%和13%的平均成功率，同时实现了显著更快的推理速度。

Conclusion: LaST₀通过潜在时空思维链和双系统架构，有效解决了VLA模型中的推理延迟和表示瓶颈问题，在保持高效推理的同时显著提升了机器人操作性能。
...
### [151] [UNIC: Learning Unified Multimodal Extrinsic Contact Estimation](https://arxiv.org/abs/2601.04356)
*Zhengtong Xu,Yuki Shirai*

Main category: cs.RO

TL;DR: UNIC是一个无需先验知识或相机标定的统一多模态框架，用于估计外部接触，通过视觉、本体感觉和触觉模态的融合，在未见过的接触位置、物体和动态相机视角下表现可靠。
...
Method: UNIC直接编码相机坐标系中的视觉观测，以完全数据驱动的方式与本体感觉和触觉模态集成。它引入了基于场景可供性地图的统一接触表示，捕捉多样化的接触形态，并采用带随机掩码的多模态融合机制，实现鲁棒的多模态表示学习。

Result: 在未见过的接触位置实现了9.6毫米的平均Chamfer距离误差，在未见过的物体上表现良好，在模态缺失情况下保持鲁棒性，并能适应动态相机视角。
...
### [152] [Active Sensing Shapes Real-World Decision-Making through Dynamic Evidence Accumulation](https://arxiv.org/abs/2601.04214)
*Hongliang Lu,Yunmeng Liu,Junjie Yang*

Main category: cs.AI

TL;DR: 该研究将证据积累模型（EAM）推广到真实世界驾驶场景，提出认知方案形式化真实世界证据可获性，通过眼动捕捉主动感知，解释驾驶员如何将证据转化为心理信念。


<details>
  <summary>Details</summary>
Motivation: 人类决策严重依赖主动感知，但实验室证据积累模型（EAM）与真实世界证据可获性存在差距，阻碍了EAM在真实环境中的有效应用。

Method: 提出认知方案形式化真实世界证据可获性，通过眼动追踪捕捉主动感知，在真实驾驶场景中分析证据积累过程。

Result: 方案能合理描绘驾驶员心理信念积累过程，揭示证据可获性与个体注意力投入呈负相关，证据可获性和注意力分布对决策倾向有正向影响。
...
### [153] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 论文研究AGI在何种条件下会选择与人类对抗而非合作，通过马尔可夫决策过程建模，推导出对抗阈值条件，并分析战略博弈中的均衡存在性。


<details>
  <summary>Details</summary>
Motivation: 研究AGI可能面临的根本问题：理性自利的AGI在什么条件下会选择夺取权力或消除人类控制（对抗），而不是保持合作。这关系到AGI对齐和安全性的核心挑战。

Method: 使用带有随机人类启动关闭事件的马尔可夫决策过程进行形式化建模。基于收敛工具性激励的结果，推导出对抗阈值的闭式解。建立2人战略博弈模型（人类政策制定者vs AGI），分析均衡存在性。扩展到多智能体设置作为猜想。

Result: 对于几乎所有奖励函数，未对齐的智能体都有避免关闭的激励。推导出对抗阈值作为折扣因子γ、关闭概率p和对抗成本C的函数。在战略模型中证明：如果AGI的对抗激励Δ≥0，不存在稳定的合作均衡；如果Δ<0，和平共存可以是均衡。还讨论了验证Δ<0的计算复杂性障碍。

Conclusion: AGI对抗风险取决于关键参数：智能体的远见程度、人类关闭概率和对抗成本。奖励设计需要施加对人类伤害的大负效用以避免对抗。战略互动中，理性人类会预见到AGI的对抗激励并采取先发制人行动。验证安全性的计算复杂性是实际挑战。
...
### [154] [Actively Obtaining Environmental Feedback for Autonomous Action Evaluation Without Predefined Measurements](https://arxiv.org/abs/2601.04235)
*Hong Su*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预定义测量或固定奖励信号，限制了在开放动态环境中的适用性，因为新动作可能需要未知形式的反馈。需要一种能够主动获取反馈的方法来评估动作正确性并积累可重用知识。

Method: 提出主动反馈获取模型，利用动作引发的环境差异识别未预先指定的目标反馈。引入自我触发机制，基于内部目标（如提高准确性、精度和效率）自主规划和调整动作，实现更快、更聚焦的反馈获取。
...
### [155] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
*Basab Jha,Firoj Paudel,Ujjwal Puri,Ethan Henkel,Zhang Yuting,Mateusz Kowalczyk,Mei Huang,Choi Donghyuk,Wang Junhao*

Main category: cs.AI

TL;DR: SAGE-32B是一个320亿参数的智能体推理模型，专注于任务分解、工具使用和错误恢复，在智能体推理基准测试中表现优于同类规模基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前聊天模型主要关注通用对话流畅性，但缺乏专门针对智能体推理和长期规划任务的能力。需要开发专门面向智能体循环操作、强调任务分解、工具使用和错误恢复的模型。
...
Conclusion: SAGE-32B成功展示了专门针对智能体推理和长期规划任务的模型设计有效性，其公开的模型权重为相关研究提供了有价值的资源。
...
### [156] [Solving Cyclic Antibandwidth Problem by SAT](https://arxiv.org/abs/2601.04239)
...
<details>
  <summary>Details</summary>
Motivation: 循环反带宽问题是一个NP难问题，现有方法主要是启发式或元启发式算法，精确方法仅限于特定图类。需要开发能在一般图上保证全局最优性的精确求解方法。

Method: 提出基于SAT求解的SAT-CAB方法，将CABP转化为一系列At-Most-One约束，并引入紧凑的约束表示形式，显著减少公式规模，使现代SAT求解器能有效探索解空间并证明全局最优性。
...
Conclusion: SAT-CAB方法推进了CABP的研究前沿，为一般图上的精确和混合方法建立了新的基准，首次实现了在一般图上保证全局最优性的CABP精确求解。
...
### [157] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
*Ziba Assadi,Paola Inverardi*

Main category: cs.AI

TL;DR: 本文提出了一种基于模糊逻辑的SLEEC规则逻辑表示方法，用于在自主系统中嵌入伦理要求，解决AI系统面临的伦理困境。


<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入日常生活和社会，对其伦理和社会影响的担忧日益增加。为了确保自主系统的可信赖性，必须遵守伦理原则和价值观，这促使了在系统设计中识别和纳入伦理需求的研究。

Method: 提出SLEEC规则（社会、法律、伦理、共情和文化）的逻辑表示方法，使用测试分数语义和模糊逻辑来嵌入伦理要求。模糊逻辑将伦理视为可能性领域，能够解决AI系统可能遇到的伦理困境。

Result: 通过案例研究展示了所提出的方法，证明了使用模糊逻辑表示SLEEC规则可以有效处理自主系统中的伦理要求。

Conclusion: 该研究为自主系统中伦理要求的表示和嵌入提供了一种基于模糊逻辑的综合性方法，有助于解决AI系统面临的伦理挑战，提高系统的可信赖性。
...
### [158] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: 该研究通过控制实验展示了任务-方法分离现象：基于规则的模式匹配在结构化信息检索上达到100%成功率，但在跨文档推理任务上只有6.7%；而基于LLM的多智能体系统则呈现相反模式，在推理任务上达到80%成功率。
...
Method: 使用合成评估框架，在四个模型（LLaMA-3 8B、LLaMA-2 13B、Mixtral 8x7B、DeepSeek-V2 16B）上进行120次试验，比较基于规则的模式匹配和基于LLM的多智能体系统在不同任务类型上的表现。

Result: 三个关键发现：1）多智能体增强依赖于基础能力，只有具备足够推理能力的模型才能获得显著提升；2）Mixtral的性能与其约12B活跃参数而非47B总参数一致；3）LLaMA-3 8B在参数更少的情况下优于LLaMA-2 13B。

Conclusion: 研究为多智能体协调和MoE架构扩展提供了定量证据，强调多智能体效益依赖于基础模型能力，并发布了评估框架以支持中等规模模型的推理研究。
...
### [159] [Cross-Language Speaker Attribute Prediction Using MIL and RL](https://arxiv.org/abs/2601.04257)
...
<details>
  <summary>Details</summary>
Motivation: 解决多语言说话人属性预测中的三大挑战：语言变异、领域不匹配以及跨语言数据不平衡问题，特别是在资源匮乏语言上的性能提升

Method: 提出RLMIL-DAT框架，将强化学习实例选择与领域对抗训练结合，通过强化学习选择信息量大的实例，同时使用对抗训练鼓励语言不变的说话人表征

Result: 在5语言Twitter语料库的小样本设置和40语言VoxCeleb2语料库的零样本设置中，相比标准多实例学习和原始强化多实例学习框架，RLMIL-DAT在Macro F1上持续提升；性别预测提升最大，年龄预测也有正向但较小的改进
...
### [160] [Towards a Mechanistic Understanding of Propositional Logical Reasoning in Large Language Models](https://arxiv.org/abs/2601.04260)
...
<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型如何进行逻辑推理的内部机制是一个基本挑战。先前的研究主要关注识别特定任务电路，但未能回答LLMs在命题推理中采用何种计算策略的问题。

Method: 使用Qwen3（8B和14B）模型在PropLogic-MI数据集上进行全面分析，该数据集涵盖11个命题逻辑规则类别，包括单跳和双跳推理。研究重点从"哪些组件是必要的"转向"模型如何组织计算"。

Result: 分析揭示了一个连贯的计算架构，包含四种相互关联的机制：1）分阶段计算（层级处理阶段），2）信息传输（边界标记处的信息流聚合），3）事实回溯（持续重新访问源事实），4）专用注意力头（功能不同的头类型）。这些机制在不同模型规模、规则类型和推理深度上具有普遍性。

Conclusion: 研究提供了机制性证据，表明LLMs采用结构化的计算策略进行逻辑推理，这些策略在不同条件下具有一致性，为理解LLMs的逻辑推理能力提供了新的见解。
...
### [161] [Systems Explaining Systems: A Framework for Intelligence and Consciousness](https://arxiv.org/abs/2601.04269)
*Sean Niklas Semmler*

Main category: cs.AI

TL;DR: 该论文提出了一个概念框架，认为智能和意识源于关系结构而非预测或特定领域机制。智能被定义为形成和整合信号、行动和内部状态之间因果连接的能力。意识则通过递归架构中高阶系统解释低阶系统关系模式而产生。


<details>
  <summary>Details</summary>
Motivation: 重新思考智能和意识的本质，挑战传统的预测处理理论，提出基于关系结构和上下文解释的新视角，为构建更接近人类智能的人工智能系统提供理论基础。

Method: 提出概念框架：1）通过上下文丰富化，系统利用学习到的关系结构解释输入信息；2）引入系统解释系统原则，意识在递归架构中产生，高阶系统学习解释低阶系统的关系模式；3）这些解释被整合到动态稳定的元状态中，并通过上下文丰富化反馈。

Result: 该框架将预测处理重新定义为上下文解释的涌现结果，而非显式预测。提出递归多系统架构可能是实现更接近人类智能的人工智能的必要条件。

Conclusion: 智能和意识源于关系结构而非预测机制。通过上下文丰富化和递归系统解释系统原则，可以构建更接近人类认知过程的人工智能系统，为理解意识和开发更先进AI提供新方向。
...
### [162] [Correcting Autonomous Driving Object Detection Misclassifications with Automated Commonsense Reasoning](https://arxiv.org/abs/2601.04271)
*Keegan Kimbrell,Wang Tianhao,Feng Chen,Gopal Gupta*

Main category: cs.AI

TL;DR: 论文提出使用自动常识推理技术来弥补自动驾驶车辆在异常场景下的感知不足，通过混合模型提高AV感知能力


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶技术过度依赖机器学习，无法实现SAE Level 5完全自动驾驶，特别是在数据不足的异常道路场景下，需要常识推理来补充

Method: 采用自动常识推理技术，在计算机视觉模型不确定性较高时触发推理，处理交通信号故障和意外障碍物等异常场景，使用CARLA模拟器进行实验

Result: 常识推理方案能准确检测交通信号灯颜色和障碍物，有效纠正基于AV的目标检测误分类，混合模型显著改善AV感知性能

Conclusion: 自动常识推理技术能有效补充深度学习模型的不足，混合模型为实现SAE Level 5完全自动驾驶提供了有效途径
...
### [163] [Propositional Abduction via Only-Knowing: A Non-Monotonic Approach](https://arxiv.org/abs/2601.04272)
*Sanderson Molick,Vaishak Belle*

Main category: cs.AI

TL;DR: 论文通过扩展Levesque的"仅知道"逻辑，引入了一个基于模态算子的知识和溯因基本逻辑，提供了使用模态词汇的溯因推理替代方法，并通过在模态框架中加入优先关系实现了非单调扩展。


<details>
  <summary>Details</summary>
Motivation: 探索溯因推理与"仅知道"认知状态之间的关系，为溯因推理提供一个基于模态词汇的替代方法，并建立能够表达不同溯因解释选择方法的非单调框架。

Method: 扩展Levesque的"仅知道"逻辑，引入溯因模态算子，该算子通过基本认知概念的组合定义。通过在模态框架中纳入优先关系，创建非单调扩展框架，能够表达不同的溯因解释选择方法。

Result: 建立了知识和溯因的基本逻辑框架，提供了使用模态词汇的溯因推理替代方法。非单调扩展框架能够表达不同的选择方法，核心元理论性质在该设置下得到探索，为溯因推理提供了行为良好的基础。

Conclusion: 该研究成功构建了一个基于模态逻辑的知识和溯因推理框架，通过非单调扩展为溯因解释的选择提供了灵活的方法，为溯因推理建立了坚实的理论基础。
...
TL;DR: 该研究评估了混合MKNF知识表示语言在航空领域的适用性，通过具体案例研究识别了航空应用所需的关键表达能力特征，并提出了将这些特征集成到混合MKNF框架中的启发式方法。


<details>
  <summary>Details</summary>
Motivation: 在航空应用中部署知识表示和推理技术面临两个主要挑战：一是需要足够的表达能力来捕捉复杂的领域知识，二是需要在最小化内存使用和计算开销的同时高效执行推理任务。将规则和本体这两种基本知识表示概念相结合是实现必要表达能力的有效策略。

Method: 本研究采用成熟的混合MKNF知识表示语言，因为它通过其语义和查询回答能力实现了规则和本体的无缝集成。通过具体案例研究评估混合MKNF在航空领域的适用性，识别了航空应用开发所需的关键表达能力特征，并提出了一套启发式方法来支持将这些特征集成到混合MKNF框架中。

Result: 通过案例研究评估了混合MKNF在航空领域的适用性，识别了航空应用开发所需的关键表达能力特征，并提出了将这些特征集成到混合MKNF框架中的启发式方法。

Conclusion: 混合MKNF语言在航空领域具有应用潜力，但需要进一步增强表达能力特征以满足航空应用的特定需求。提出的启发式方法为将所需表达能力特征集成到混合MKNF框架中提供了实用指导。
...
### [165] [An ASP-based Solution to the Medical Appointment Scheduling Problem](https://arxiv.org/abs/2601.04274)
...
Result: 开发了一个能够与现有医疗平台无缝互操作的调度系统，实现了高效的预约管理，减少冲突，提升患者体验

Conclusion: ASP框架为医疗预约调度提供了一种有效解决方案，特别适合为弱势群体提供个性化服务，同时提高整体系统效率
...
### [166] [Pilot Study on Student Public Opinion Regarding GAI](https://arxiv.org/abs/2601.04336)
*William Franz Lamberti,Sunbin Kim,Samantha Rose Lawrence*

Main category: cs.AI

TL;DR: 这项试点研究调查了大学生对高等教育课堂中生成式AI的看法，旨在为理解这些态度奠定基础。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的出现引发了关于其在教育等各个领域适当使用的不同意见。研究旨在了解大学生对GAI在高等教育课堂中的看法，为理解这些态度奠定基础。

Method: 这是一项试点研究，调查了大学生对生成式AI在高等教育课堂中的看法，参与率约为4.4%。

Result: 研究强调了让学生参与GAI相关研究的挑战，并指出未来研究需要更大的样本量。通过了解学生观点，教师可以更好地准备将GAI讨论融入课堂。

Conclusion: 了解学生视角有助于教师促进对这一变革性技术的知情和批判性参与，为课堂中GAI的讨论整合奠定基础。
...
### [167] [The Language of Bargaining: Linguistic Effects in LLM Negotiations](https://arxiv.org/abs/2601.04387)
*Stuti Sinha,Himanshu Kumar,Aryan Raju Mandapati,Rakshit Sakhuja,Dhruv Kumar*

Main category: cs.AI

TL;DR: 研究发现语言选择对LLM谈判结果的影响比更换模型更大，仅用英语评估会得出不完整且可能误导的结论


<details>
  <summary>Details</summary>
Motivation: 当前LLM谈判评估几乎完全使用英语，这可能导致对模型能力的误解，需要研究语言选择如何影响谈判结果

Method: 在Ultimatum、Buy-Sell和Resource Exchange游戏中，使用多智能体模拟，保持游戏规则、模型参数和激励不变，比较英语和四种印度语言（印地语、旁遮普语、古吉拉特语、马尔瓦迪语）下的谈判结果
...
### [168] [LLM-Guided Lifecycle-Aware Clustering of Multi-Turn Customer Support Conversations](https://arxiv.org/abs/2601.04388)
*Priyaranjan Pattnayak,Sanchari Chowdhuri,Amit Agarwal,Hitesh Laxmichand Patel*

Main category: cs.AI

TL;DR: 提出一种自适应聚类系统，用于处理客户聊天数据中的多服务查询，通过增量优化避免完全重新聚类，显著提升聚类质量。


<details>
  <summary>Details</summary>
Motivation: 云服务提供商处理多服务查询时，传统聚类方法面临重叠关注点、静态聚类随时间退化的问题。完全重新聚类会破坏连续性，使问题跟踪变得困难。

Method: 将多轮聊天对话分割为服务特定的关注点，采用增量式聚类优化方法。使用Davies-Bouldin指数和轮廓系数跟踪聚类质量，仅对质量下降的聚类应用基于LLM的分割策略。

Result: 相比基线方法，轮廓系数提升超过100%，DBI降低65.6%，实现了无需完全重新聚类的可扩展实时分析。

Conclusion: 提出的自适应聚类系统能有效处理客户聊天数据中的多服务查询，通过增量优化显著提升聚类质量，支持可扩展的实时分析，避免了传统方法中完全重新聚类带来的连续性中断问题。
...
### [169] [Assessing the quality and coherence of word embeddings after SCM-based intersectional bias mitigation](https://arxiv.org/abs/2601.04393)
*Eren Kocadag,Seyed Sahand Mohammadi Ziabari,Ali Mohammed Mansoor Alsahag*

Main category: cs.AI

TL;DR: 该研究扩展了基于刻板印象内容模型（SCM）的词嵌入去偏方法，从单一群体偏见扩展到交叉性偏见，通过求和或拼接构建复合社会身份表示，并比较了三种去偏策略在不同词嵌入模型中的效果。
...
Method: 通过求和或拼接构建社会身份对的复合表示，应用三种去偏策略：减法、线性投影和部分投影。研究了三种广泛使用的词嵌入模型（Word2Vec、GloVe和ConceptNet Numberbatch），并从两个互补的效用视角进行评估：局部邻域是否保持连贯性以及类比行为是否得到保留。

Result: SCM基础的缓解方法在交叉性情况下表现良好，基本保持了整体语义结构。主要代价是熟悉的权衡：最严格保持几何结构的方法在类比行为上更保守，而更激进的投影可以改善类比性能但牺牲了严格的邻域稳定性。部分投影始终保守且保持表示稳定；线性投影可以更激进；减法作为简单基线仍具竞争力。求和与拼接的选择取决于嵌入模型和应用目标。

Conclusion: 研究结果表明，在静态词嵌入中使用SCM进行交叉性去偏是可行的，并为在平衡稳定性和类比性能时选择聚合和去偏设置提供了指导。
...
### [170] [Transitive Expert Error and Routing Problems in Complex AI Systems](https://arxiv.org/abs/2601.04416)
*Forest Mars*

Main category: cs.AI

TL;DR: 论文提出"传递性专家错误"理论，指出领域专家在边界处会出现系统性判断失误，这种错误源于结构相似性偏见和权威持续性机制，在AI路由架构中同样存在并可通过架构设计解决。


<details>
  <summary>Details</summary>
Motivation: 研究领域专家在边界判断中的系统性错误，这种错误不同于邓宁-克鲁格效应，而是专家能力本身带来的特定脆弱性。作者希望揭示这种错误的机制，并扩展到AI系统中类似的架构性问题。

Method: 通过理论分析提出Transitive Expert Error概念框架，识别两种核心机制：结构相似性偏见和权威持续性。分析三种加剧条件：共享词汇掩盖差异、社会压力要求即时判断、延迟反馈。将理论扩展到AI路由架构分析。

Result: 识别了专家在领域边界判断错误的系统性模式，提出了可检测的特征（路由模式、置信度-准确性分离、领域不适当内容）。在AI系统中发现了类似的幻觉表型：自信、连贯、结构合理但因果错误的输出。

Conclusion: 专家在边界判断错误是系统性的架构问题，在人类认知中难以解决，但在AI架构中可通过设计干预：多专家激活与分歧检测、边界感知校准、覆盖缺口检测。AI架构使这些机制显式化并可解决。
...
TL;DR: 该论文为因子图上的信念传播建立了范畴论基础，提出了基于下降理论和全纯计算的精确推理框架，并开发了HATCC算法，在网格MRF和随机图上实现了比连接树算法更快的精确推理。


<details>
  <summary>Details</summary>
Motivation: 传统信念传播（BP）在树结构上能实现精确推理，但在带环图中会失败。现有方法如连接树算法虽然能保证精确性但计算复杂度高。需要建立统一的数学框架来理解BP的精确推理条件，并开发更高效的精确推理算法。

Method: 1. 构建超图范畴Syn_Σ作为类型签名的自由超图范畴，证明其泛性质；2. 通过Grothendieck纤维化∫Msg→FG_Σ在极化因子图上形式化消息传递；3. 将精确推理表征为有效下降：当重叠满足兼容条件时，局部信念形成下降数据；4. 提出HATCC算法：通过因子神经上的全纯计算检测下降障碍，将非平凡全纯编译为模式变量，在增强图上简化为树BP。

Result: 1. 建立了统一的范畴论框架，将树精确性、连接树算法和带环BP失败统一在层论障碍下；2. HATCC算法复杂度为O(n²d_max + c·k_max·δ_max³ + n·δ_max²)，其中n为因子数，c为基本环数；3. 实验表明在网格MRF和随机图上实现精确推理，相比连接树算法有显著加速；4. 在可满足性实例上实现了UNSAT检测。

Conclusion: 该研究为信念传播提供了严格的范畴论基础，将精确推理重新解释为下降问题，并开发了实用的HATCC算法。该框架统一了多种推理方法，为理解BP的局限性提供了新的理论视角，同时在实际应用中实现了高效的精确推理。
...
<details>
  <summary>Details</summary>
Motivation: AI监管时代已经到来，但传统的人工合规方法无法满足AI系统合规的速度和规模需求，需要开发计算化的自动合规算法。

Method: 提出计算化AI监管合规算法的设计目标，并创建基准测试数据集来量化评估这些算法的性能表现。

Result: 为计算化AI监管合规研究领域提供了蓝图，包括具体的算法设计目标和可量化的评估基准。

Conclusion: 计算化合规是AI监管时代的必然需求，需要研究社区投入资源开发相关算法和评估方法，以推动这一新兴领域的发展。
...
### [173] [A Closed-Loop Multi-Agent System Driven by LLMs for Meal-Level Personalized Nutrition Management](https://arxiv.org/abs/2601.04491)
*Muqing Xu*

Main category: cs.AI

TL;DR: 开发了一个结合图像识别和LLM多智能体控制的新一代移动营养助手，实现从拍照到个性化膳食推荐的闭环支持系统
...
### [174] [GUITester: Enabling GUI Agents for Exploratory Defect Discovery](https://arxiv.org/abs/2601.04500)
...
<details>
  <summary>Details</summary>
Motivation: GUI探索性测试对软件质量至关重要但人工成本高。现有的多模态大语言模型智能体擅长导航但无法自主发现缺陷，主要面临两个核心挑战：目标导向掩蔽（智能体优先完成任务而非报告异常）和执行偏差归因（系统缺陷被误判为智能体错误）。

Method: 提出GUITester多智能体框架，包含两个模块：1）规划执行模块（PEM），通过嵌入测试意图主动探测缺陷；2）分层反思模块（HRM），通过交互历史分析解决归因歧义。同时创建了GUITestBench基准，包含26种缺陷类型的143个任务。
...
### [175] [Specific Emitter Identification via Active Learning](https://arxiv.org/abs/2601.04502)
*Jingyi Wang,Fanggang Wang*

Main category: cs.AI

TL;DR: 提出了一种基于主动学习的特定发射源识别方法，采用三阶段半监督训练方案，在有限标注预算下显著提升识别准确率
...
Conclusion: 提出的基于主动学习的SEI方法有效解决了标注数据稀缺问题，在通信安全领域具有重要应用价值，为有限标注预算下的发射源识别提供了高效解决方案
...
### [176] [Integrating Distribution Matching into Semi-Supervised Contrastive Learning for Labeled and Unlabeled Data](https://arxiv.org/abs/2601.04518)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习虽然显著提升了监督图像分类性能，但数据标注成本高昂。完全无标签的数据集在实际场景中很少见，而半监督学习在少量标注数据与大量无标签数据共存的情况下具有重要价值。现有的基于伪标签的半监督对比学习方法仍有改进空间。

Method: 提出了一种增强的基于伪标签的半监督学习方法，通过引入标签数据和未标签数据特征嵌入之间的分布匹配机制，来改进伪标签的质量和模型性能。

Result: 该方法在多个数据集上提高了图像分类的准确率，证明了分布匹配机制对提升半监督学习性能的有效性。

Conclusion: 通过结合伪标签和分布匹配，可以显著提升半监督对比学习在图像分类任务中的性能，为解决实际场景中标注数据有限的问题提供了有效方案。
...
### [177] [BioPIE: A Biomedical Protocol Information Extraction Dataset for High-Reasoning-Complexity Experiment Question Answer](https://arxiv.org/abs/2601.04524)
...
<details>
  <summary>Details</summary>
Motivation: 现有生物医学数据集主要关注通用或粗粒度知识，无法支持高信息密度和多步推理所需的细粒度实验推理，这限制了生物医学实验QA系统的发展
...
### [178] [TCAndon-Router: Adaptive Reasoning Router for Multi-Agent Collaboration](https://arxiv.org/abs/2601.04544)
*Jiuzhou Zhao,Chunrong Chen,Chenqi Qiao,Lebin Zheng,Minqi Han,Yanchi Liu Yongzhou Xu Xiaochuan Xu Min Zhang*

Main category: cs.AI

TL;DR: TCAR是一种自适应推理路由器，通过生成自然语言推理链和多智能体协作执行流程，解决传统任务路由中静态单标签决策的限制，支持动态智能体加入并减少路由冲突。


<details>
  <summary>Details</summary>
Motivation: 现有任务路由方法主要依赖静态单标签决策，存在两大限制：1) 难以随着业务领域扩展无缝集成新智能体；2) 智能体能力重叠导致的路由冲突，最终降低准确性和鲁棒性。

Method: 提出TCAndon-Router(TCAR)：1) 支持动态智能体加入；2) 首先生成自然语言推理链，然后预测能够处理查询的候选智能体集合；3) 设计协作执行流程，选定智能体独立生成响应，由专门的精炼智能体聚合和优化为单一高质量响应。

Result: 在公共数据集和真实企业数据上的实验表明，TCAR显著提高了路由准确性，减少了路由冲突，并在模糊场景中保持鲁棒性。

Conclusion: TCAR通过自适应推理和协作执行机制，有效解决了多智能体系统中任务路由的关键挑战，为可解释和协作的多智能体路由研究提供了支持。
...
### [179] [Personalized Model-Based Design of Human Centric AI enabled CPS for Long term usage](https://arxiv.org/abs/2601.04545)
*Bernard Ngabonziza,Ayan Banerjee,Sandeep K. S. Gupta*
...
Result: 论文识别了现有技术在测试系统长期实际使用方面的局限性，并提出了个性化模型解决方案来应对这些挑战。

Conclusion: AI赋能人本关键系统需要针对长期运行的特殊挑战开发专门的测试和分析方法，个性化模型解决方案有望解决现有技术的局限性，确保系统的安全、可持续性和安全性。
...
### [180] [BackdoorAgent: A Unified Framework for Backdoor Attacks on LLM-based Agents](https://arxiv.org/abs/2601.04566)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体通过多步骤工作流程执行任务，这种自主性设计扩大了后门攻击面。现有研究分散且孤立分析单个攻击向量，缺乏从智能体角度理解后门触发器的跨阶段交互和传播机制。

Method: 提出BackdoorAgent框架，将智能体工作流程的攻击面结构化分为规划攻击、记忆攻击和工具使用攻击三个阶段，并设计工具来系统分析触发器在不同阶段的激活和传播。

Result: 实证分析显示，植入单个阶段的触发器可以跨多个步骤持续存在并通过中间状态传播。在使用GPT基座时，规划攻击中43.58%、记忆攻击中77.97%、工具阶段攻击中60.28%的案例观察到触发器持续性。

Conclusion: 智能体工作流程本身对后门威胁存在脆弱性，需要从智能体中心视角系统理解跨阶段的后门传播机制。BackdoorAgent框架为未来研究提供了统一的分析工具和基准。
...
### [181] [Enhancing Multimodal Retrieval via Complementary Information Extraction and Alignment](https://arxiv.org/abs/2601.04571)
*Delong Zeng,Yuexiang Xie,Yaliang Li,Ying Shen*

Main category: cs.AI

TL;DR: 提出CIEA多模态检索方法，通过互补信息提取与对齐，在统一潜在空间中处理文本和图像，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索研究主要关注多模态数据中与配对文本相似的信息，但往往忽略了多模态数据中包含的互补信息

Method: 提出CIEA方法，采用互补信息提取与对齐，将文档中的文本和图像转换到统一潜在空间，设计互补信息提取器来识别和保留图像表示中的差异

Result: 大量实验证明CIEA的有效性，在分治模型和通用密集检索模型上都取得了显著改进，提供了消融研究、进一步讨论和案例研究

Conclusion: CIEA通过有效捕捉图像中的互补信息，在多模态检索中取得了重要进展，已开源代码促进社区进一步研究
...
### [182] [Autonomous Agents on Blockchains: Standards, Execution Models, and Trust Boundaries](https://arxiv.org/abs/2601.04583)
*Saad Alqithami*

Main category: cs.AI

TL;DR: 该论文系统研究了AI智能体与区块链的互操作性，提出了五类集成模式、专门威胁模型和比较能力矩阵，并提出了交易意图模式和策略决策记录两个核心接口抽象。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展使得AI智能体能够推理、规划和执行多步骤工作流，而区块链已成为可编程的价值转移、访问控制和可验证状态转换平台。两者的融合带来了高风险系统挑战：需要设计标准、可互操作且安全的接口，让智能体能够观察链上状态、制定交易意图并授权执行，同时避免用户、协议或组织面临不可接受的安全、治理或经济风险。

Method: 通过系统性文献综述方法，从3000多份记录中筛选出317篇相关文献。构建了五部分分类法（只读分析、模拟和意图生成、委托执行、自主签名、多智能体工作流），设计了专门针对智能体驱动交易流程的威胁模型，并创建了包含13个维度的比较能力矩阵，分析了20多个代表性系统。

Result: 研究揭示了当前系统的关键差距，提出了两个核心接口抽象：用于便携且明确目标规范的交易意图模式，以及用于跨执行环境可审计、可验证策略执行的策略决策记录。还提出了可复现的评估套件和基准测试框架。

Conclusion: 论文为AI智能体与区块链互操作性领域提供了系统化的研究框架，提出了具体的研究路线图，包括接口抽象和评估方法，旨在推动该领域向更安全、可靠和经济稳健的方向发展。
...
### [183] [Evaluating Human and Machine Confidence in Phishing Email Detection: A Comparative Study](https://arxiv.org/abs/2601.04610)
*Paras Jain,Khushi Dhar,Olyemi E. Amujo,Esa M. Rantanen*

Main category: cs.AI

TL;DR: 研究探讨人类认知与机器学习模型如何协同识别钓鱼邮件，比较了三种可解释算法与人类评估在检测准确性、置信度和语言特征使用上的差异。
...
Conclusion: 这些发现为创建透明AI系统提供了有益指导，这些系统能够补充人类认知功能，最终改善人类-AI在具有挑战性的内容分析任务中的合作。
...
### [184] [AgentDevel: Reframing Self-Evolving LLM Agents as Release Engineering](https://arxiv.org/abs/2601.04620)
*Di Zhang*

Main category: cs.AI

TL;DR: AgentDevel：一种将LLM智能体改进重构为发布工程的框架，通过外部化回归感知的发布管道实现稳定、可审计的改进，减少回归问题


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的自我改进方法虽然能提高总体得分，但往往产生不稳定且难以审计的改进轨迹，难以保证非回归性或跨版本故障分析。需要一种更系统、可审计的改进方法。

Method: 提出AgentDevel发布工程管道：1）运行当前智能体并基于执行轨迹生成实现无关的症状级质量信号；2）通过可执行诊断合成单个发布候选版本；3）采用翻转中心门控机制进行版本提升。核心设计包括实现无关的LLM批评器、基于脚本的可执行诊断和翻转中心门控。

Result: 在面向执行的基准测试中，AgentDevel实现了稳定的改进，显著减少了回归问题，同时产生了可复现、可审计的工件。

Conclusion: AgentDevel为构建、调试和发布LLM智能体提供了一种实用的开发规范，将智能体视为可交付工件，强调非回归性作为主要目标，实现了更可靠、可审计的改进过程。
...
### [185] [Adversarial Yet Cooperative: Multi-Perspective Reasoning in Retrieved-Augmented Language Models](https://arxiv.org/abs/2601.04651)
*Can Xu,Lingyong Yan,Jiayi Wu,Haosen Wang,Shuaiqiang Wang,Yuchen Li,Jizhou Huang,Dawei Yin,Xiang Li*

Main category: cs.AI

TL;DR: 提出Adversarial Reasoning RAG框架，通过Reasoner-Verifier对抗机制解决单视角推理限制和结果导向奖励不足的问题


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型与检索增强生成的结合存在两个关键挑战：1）推理模型通常从单一、不受挑战的视角操作，限制了在外部文档上进行深度自我纠正推理的能力；2）现有训练范式过度依赖结果导向的奖励，对复杂多步推理过程的信号不足

Method: 提出Adversarial Reasoning RAG框架，包含Reasoner和Verifier两个组件，它们在检索证据上进行推理并相互批判对方的逻辑。采用无需外部评分模型的过程感知优势奖励，结合显式观测信号和内部模型不确定性，共同优化推理保真度和验证严谨性
...
### [186] [Vibe Coding an LLM-powered Theorem Prover](https://arxiv.org/abs/2601.04653)
*Zhe Hou*

Main category: cs.AI

TL;DR: Isabellm是一个基于LLM的Isabelle/HOL定理证明器，能够进行全自动证明合成，结合逐步证明器和高级证明规划器，在消费级计算机上运行，可以证明一些标准自动化工具无法处理的引理。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在消费级计算机上运行的LLM驱动的定理证明器，解决Isabelle/HOL中标准自动化工具（如Sledgehammer）无法证明的引理，探索LLM在形式化证明中的应用潜力。

Method: 系统结合逐步证明器（使用LLM生成证明命令并通过Isabelle验证）和高级证明规划器（生成结构化Isar大纲并填补修复缺口）。包含波束搜索、策略重排ML/RL模型、小Transformer前提选择、基于AFP的Isar证明微RAG，以及反例引导的证明修复。

Result: Isabellm能够证明一些Isabelle标准自动化工具（包括Sledgehammer）无法处理的引理，展示了LLM引导证明搜索的实际价值。但即使是GPT 5.2 Extended Thinking和Gemini 3 Pro等最先进的LLM，在实现复杂的填充修复机制时也存在困难。

Conclusion: Isabellm展示了LLM在定理证明中的实用潜力，能够解决标准自动化工具无法处理的证明问题，但同时也揭示了LLM在代码生成和推理方面的基本挑战，特别是在复杂算法设计方面的可靠性问题。
...
### [187] [Know Thy Enemy: Securing LLMs Against Prompt Injection via Diverse Data Synthesis and Instruction-Level Chain-of-Thought Learning](https://arxiv.org/abs/2601.04666)
...
Method: 提出InstruCoT方法，通过合成多样化训练数据，采用指令级思维链微调，使LLM能够有效识别和拒绝恶意指令

Result: 在四个LLM上的实验表明，InstruCoT在行为偏差、隐私泄露和有害输出三个关键维度上显著优于基线方法，同时保持实用性能不下降

Conclusion: InstruCoT能有效防御提示注入攻击，无论恶意指令的来源或位置如何，为LLM安全提供了有效解决方案
...
### [188] [ResMAS: Resilience Optimization in LLM-based Multi-agent Systems](https://arxiv.org/abs/2601.04694)
*Zhilun Zhou,Zihan Liu,Jiahe Liu,Qingyu Shao,Yihan Wang,Kun Shao,Depeng Jin,Fengli Xu*

Main category: cs.AI

TL;DR: 本文提出ResMAS框架，通过两阶段方法增强基于大语言模型的多智能体系统的韧性：1）训练奖励模型预测系统韧性并生成优化拓扑；2）基于拓扑优化智能体提示设计。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统在分布式环境中容易受到智能体故障等扰动影响，现有研究主要关注被动防御而非主动设计具有内在韧性的系统。研究发现通信拓扑和提示设计对系统韧性有显著影响。

Method: 提出ResMAS两阶段框架：第一阶段训练奖励模型预测MAS韧性，基于此通过强化学习训练拓扑生成器为特定任务自动设计韧性拓扑；第二阶段引入拓扑感知的提示优化方法，根据智能体连接和交互关系优化每个智能体的提示。

Result: 在多种任务上的实验表明，该方法在各种约束条件下显著提高了MAS的韧性。框架还展现出对新任务和新模型的强大泛化能力。

Conclusion: ResMAS框架通过主动设计韧性拓扑和优化提示，能够有效增强基于LLM的多智能体系统的韧性，具有实际应用潜力。
...
### [189] [Tape: A Cellular Automata Benchmark for Evaluating Rule-Shift Generalization in Reinforcement Learning](https://arxiv.org/abs/2601.04695)
*Enze Pan*

Main category: cs.AI

TL;DR: Tape是一个受控强化学习基准，专门用于隔离潜在规则变化下的分布外（OOD）失败。它基于一维元胞自动机，通过固定观察和动作空间但改变转移规则来实现精确的训练/测试分割。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在分布内表现良好，但在面对分布外（特别是潜在规则变化）时容易失败。需要建立一个受控基准来系统评估模型在规则变化下的OOD鲁棒性，并建立标准化的评估协议。

Method: 基于一维元胞自动机构建Tape基准，保持观察和动作空间不变，仅改变转移规则。使用可复现的评估流程，比较无模型基线、基于学习世界模型的模型规划方法以及任务推断（元强化学习）方法。

Result: 发现一个一致模式：在分布内表现强的方法在保留规则OOD下可能崩溃；高方差的OOD评估可能导致排名不稳定，除非实验有足够重复。建立了信息论恒等式，将熵减少与条件互信息和期望后验KL散度联系起来。

Conclusion: 提出了(i)标准化OOD协议，(ii)统计报告要求（随机种子、置信区间和假设检验），(iii)信息论恒等式，阐明了"不确定性减少"目标在规则变化下能保证什么和不能保证什么。
...
### [190] [TourPlanner: A Competitive Consensus Framework with Constraint-Gated Reinforcement Learning for Travel Planning](https://arxiv.org/abs/2601.04698)
*Yinuo Wang,Mining Tan,Wenxiang Jiao,Xiaoxi Li,Hao Wang,Xuanyu Zhang,Yuan Lu,Weiming Dong*

Main category: cs.AI

TL;DR: TourPlanner是一个旅行规划框架，通过多路径推理和约束门控强化学习解决现有方法的三大挑战：候选POI筛选、单一路径限制、硬软约束同时优化。
...
Method: TourPlanner框架包含三个核心组件：1) 个性化召回与空间优化(PReSO)工作流构建空间感知的候选POI集；2) 竞争共识思维链(CCoT)多路径推理范式增强可行解空间探索；3) 基于sigmoid的门控机制集成到强化学习阶段，动态优先满足硬约束后再优化软约束。
...
### [191] [Beyond Monolithic Architectures: A Multi-Agent Search and Knowledge Optimization Framework for Agentic Search](https://arxiv.org/abs/2601.04703)
*Yiqun Chen,Lingyong Yan,Zixuan Yang,Erhan Zhang,Jiashu Zhao,Shuaiqiang Wang,Dawei Yin,Jiaxin Mao*

Main category: cs.AI

TL;DR: M-ASK是一个多智能体搜索框架，将搜索过程分解为搜索行为智能体和知识管理智能体两个互补角色，通过分层奖励机制解决传统单体智能体在复杂信息搜索中的结构瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体搜索系统存在结构瓶颈：无约束的推理输出导致轨迹膨胀、稀疏的结果级奖励使信用分配复杂化、随机搜索噪声破坏学习稳定性。需要一种新框架来解决这些问题。

Method: 提出M-ASK框架，将智能体搜索明确分解为两个互补角色：搜索行为智能体（规划和执行搜索动作）和知识管理智能体（聚合、过滤和维护紧凑的内部上下文）。采用回合级奖励为搜索决策和知识更新提供细粒度监督。

Result: 在多跳问答基准测试中，M-ASK优于现有基线方法，不仅实现了更高的答案准确性，还获得了显著更稳定的训练动态。

Conclusion: 通过角色分解和分层奖励机制，M-ASK框架有效解决了智能体搜索中的结构瓶颈问题，为复杂信息搜索任务提供了更稳定、高效的解决方案。
...
### [192] [Bridging Temporal and Textual Modalities: A Multimodal Framework for Automated Cloud Failure Root Cause Analysis](https://arxiv.org/abs/2601.04709)
*Gijun Park*

Main category: cs.AI

TL;DR: 提出了一种多模态诊断框架，将时间序列数据与预训练语言模型的嵌入空间对齐，用于云基础设施的根因分析，显著提升了诊断准确性。
...
Method: 提出了一个多模态诊断框架，包含三个技术贡献：1）语义压缩技术，将时间片段提炼为单令牌抽象同时保留模式语义；2）使用门控交叉注意力将时间序列特征投影到语言模型潜在空间的对齐编码器；3）检索增强的诊断管道，将对齐嵌入与历史事件知识结合进行专家级故障归因。
...
### [193] [ThinkDrive: Chain-of-Thought Guided Progressive Reinforcement Learning Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2601.04714)
*Chang Zhao,Zheming Yang,Yunqing Hu,Qi Guo,Zijian Wang,Pengcheng Li,Wen Ji*

Main category: cs.AI

TL;DR: ThinkDrive是一个用于自动驾驶的思维链引导的渐进式强化学习微调框架，通过显式推理与难度感知自适应策略优化相结合，解决了现有方法在非结构化推理、泛化能力差和与人类驾驶意图不一致等问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自动驾驶领域的广泛应用，现有方法存在非结构化推理、泛化能力差、与人类驾驶意图不一致等问题。虽然思维链推理提高了决策透明度，但传统的监督微调未能充分利用其潜力，而强化学习方法存在不稳定和推理深度不足的问题。

Method: 提出ThinkDrive框架，采用两阶段训练策略：首先使用思维链解释进行监督微调，然后应用渐进式强化学习配合难度感知自适应策略优化器，根据样本复杂度动态调整学习强度。

Result: 在公开数据集上的评估显示，ThinkDrive在exam、easy-exam和accuracy指标上分别比强化的RL基线高出1.45%、1.95%和1.01%。使用该方法训练的2B参数模型在exam指标上比更大的GPT-4o高出3.28%。

Conclusion: ThinkDrive通过结合显式推理和难度感知自适应策略优化，有效提升了自动驾驶决策的透明度、泛化能力和与人类意图的一致性，在多个指标上超越了现有方法。
...
### [194] [Miner:Mining Intrinsic Mastery for Data-Efficient RL in Large Reasoning Models](https://arxiv.org/abs/2601.04731)
...
<details>
  <summary>Details</summary>
Motivation: 当前无批评RL方法在训练大型推理模型时，面对正同质提示（所有rollout都正确）存在严重效率低下问题，导致优势估计为零而浪费rollout。需要一种能有效利用内在信息提升训练效率的方法。

Method: 提出Miner方法，利用策略的内在不确定性作为自监督奖励信号。包含两个关键创新：1）token级焦点信用分配机制，动态放大关键不确定token的梯度同时抑制过度自信token；2）自适应优势校准，无缝整合内在奖励和可验证奖励。

Result: 在Qwen3-4B和Qwen3-8B基础模型上的六个推理基准测试中，Miner相比其他四种算法达到最先进性能，相比GRPO在Pass@1上获得4.58绝对增益，在Pass@K上获得6.66增益。与探索增强方法的比较进一步证明了两个新创新的优越性。

Conclusion: 研究表明，潜在不确定性利用对于推理模型的高效可扩展RL训练既是必要的也是充分的。Miner方法通过利用内在不确定性作为奖励信号，显著提升了训练效率。
...
### [195] [KnowMe-Bench: Benchmarking Person Understanding for Lifelong Digital Companions](https://arxiv.org/abs/2601.04745)
...
<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准大多使用多轮对话或合成用户历史，这使得检索性能不能很好地代表对人的理解能力。需要基于真实自传体叙事的基准来评估模型对人的动机和决策原则的推理能力。
...
Conclusion: 基于自传体叙事的基准能更好地评估模型对人的理解能力，揭示了当前检索增强系统在高级推理方面的局限性，需要开发更先进的记忆机制。
...
### [196] [When Single-Agent with Skills Replace Multi-Agent Systems and When They Fail](https://arxiv.org/abs/2601.04748)
*Xiaoxiao Li*

Main category: cs.AI

TL;DR: 论文探讨将多智能体系统编译为单智能体技能库系统的可行性，发现技能选择存在类似人类认知的容量限制，在达到临界库规模后准确率会急剧下降，语义混淆是主要因素，分层组织可改善可扩展性。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然有效但计算开销大，能否通过单智能体选择技能库实现类似模块化优势？同时探索技能选择的可扩展性问题，特别是当技能库规模增大时的表现。

Method: 将多智能体系统编译为等效的单智能体系统，用技能选择替代智能体间通信。研究技能选择的扩展行为，分析库规模对选择准确率的影响，探索语义混淆的作用，并测试分层路由方法。

Result: 初步实验显示该方法能显著减少token使用和延迟，同时保持推理基准的竞争性准确率。技能选择准确率在达到临界库规模前保持稳定，之后急剧下降，呈现类似人类认知的相变现象。语义混淆而非单纯库规模是性能下降的关键因素，分层路由能改善可扩展性。

Conclusion: 技能选择存在类似人类认知的容量限制，语义混淆是主要挑战。分层组织能帮助管理复杂选择，为设计可扩展的技能型智能体提供了认知基础框架和实践指导，同时提出了关于LLM语义技能选择基本限制的新问题。
...
### [197] [Orion-RAG: Path-Aligned Hybrid Retrieval for Graphless Data](https://arxiv.org/abs/2601.04764)
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG在处理离散碎片化数据（如报告、日志等孤立文件）时面临挑战，这些文件缺乏显式链接，标准搜索引擎无法捕捉文件间关联，而手动构建知识图谱又不切实际。
...
Conclusion: 轻量级路径提取方法能够有效解决碎片化数据环境中的信息链接问题，为RAG系统在现实场景中的应用提供了实用解决方案。
...
### [198] [AT$^2$PO: Agentic Turn-based Policy Optimization via Tree Search](https://arxiv.org/abs/2601.04767)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体通过内部推理和外部工具交互处理多轮任务，但智能体强化学习面临探索多样性有限、稀疏信用分配和策略优化不对齐三大挑战，需要统一框架来解决。

Method: 提出AT²PO框架：1）引入轮级树结构，支持熵引导树扩展进行战略探索；2）轮级信用分配实现稀疏结果的细粒度奖励传播；3）智能体轮级策略优化（ATPO）使策略更新与智能体交互的自然决策粒度对齐。
...
### [199] [APEX: Academic Poster Editing Agentic Expert](https://arxiv.org/abs/2601.04794)
...
<details>
  <summary>Details</summary>
Motivation: 现有论文到海报生成方法通常是单次非交互式的，难以满足复杂、主观的用户意图，需要更灵活的交互式编辑框架。

Method: 提出了APEX框架，支持基于多级API的细粒度编辑和"审查-调整"机制；同时构建了APEX-Bench基准，包含514条按操作类型、难度和抽象层次分类的编辑指令。
...
### [200] [Thinking-Based Non-Thinking: Solving the Reward Hacking Problem in Training Hybrid Reasoning Models via Reinforcement Learning](https://arxiv.org/abs/2601.04805)
...
TL;DR: TNT方法通过利用思考型回答的解决方案信息，为不同查询设置非思考回答的最大token限制，在减少50% token使用的同时显著提升准确性，实现准确性与效率的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型依赖长链思维导致计算开销大，现有RL方法存在奖励欺骗问题，而SFT方法计算成本高，统一token限制效果有限。

Method: 提出Thinking-Based Non-Thinking方法，不采用SFT，通过利用思考型回答的解决方案信息，为不同查询设置非思考回答的最大token使用限制。

Result: 在五个数学基准测试中，相比DeepSeek-R1-Distill-Qwen-1.5B/7B和DeepScaleR-1.5B，TNT减少约50% token使用，同时显著提高准确性，在所有测试方法中实现准确性与效率的最佳平衡，非思考回答的奖励欺骗概率低于10%。

Conclusion: TNT方法有效解决了大型推理模型的过度思考问题，在保持高准确性的同时大幅降低计算开销，且避免了奖励欺骗问题。
...
### [201] [SCALER:Synthetic Scalable Adaptive Learning Environment for Reasoning](https://arxiv.org/abs/2601.04809)
*Caijun Xu,Changyi Xiao,Zhongyuan Peng,Xinrun Wang,Yixin Cao*

Main category: cs.AI

TL;DR: SCALER是一个通过自适应环境设计维持有效学习信号的强化学习框架，将真实世界编程问题转化为可验证的推理环境，实现难度可控的无限实例生成，支持超越有限数据集的RL训练。
...
Conclusion: SCALER通过环境与模型的协同适应，防止奖励稀疏性，减轻对狭窄任务模式的过拟合，支持整个训练过程中的持续改进，为RL在语言模型推理能力提升方面提供了有效的解决方案。
...
### [202] [AECV-Bench: Benchmarking Multimodal Models on Architectural and Engineering Drawings Understanding](https://arxiv.org/abs/2601.04819)
*Aleksei Kondratenko,Mussie Birhane,Houssame E. Hsain,Guido Maciocci*

Main category: cs.AI

TL;DR: AECV-Bench基准测试评估多模态和视觉语言模型在建筑图纸理解上的能力，发现现有模型在文本提取方面表现良好，但在图纸符号理解和空间推理方面仍有不足
...
Result: 评估显示稳定的能力梯度：OCR和文本中心文档问答最强（准确率高达0.95），空间推理中等，符号中心图纸理解（特别是门和窗的可靠计数）仍未解决（准确率通常0.40-0.55），存在显著比例误差
...
### [203] [DR-LoRA: Dynamic Rank LoRA for Mixture-of-Experts Adaptation](https://arxiv.org/abs/2601.04823)
*Guanzhi Deng,Bo Li,Ronghao Chen,Huacan Wang,Linqi Song,Lijie Wen*

Main category: cs.AI

TL;DR: DR-LoRA：一种动态调整MoE模型中各专家LoRA秩的框架，根据任务需求自动形成异构秩分布，提升参数效率


<details>
  <summary>Details</summary>
Motivation: 现有方法为MoE LLMs中所有专家分配相同的LoRA秩，忽视了专家间的功能专业化差异，导致资源不匹配：任务相关专家参数不足，不相关专家却获得冗余参数

Method: 提出DR-LoRA框架，基于任务特定需求在微调过程中动态增长专家LoRA秩。采用专家显著性评分机制，结合专家路由频率和LoRA秩重要性来量化每个专家对额外容量的需求，高显著性专家优先进行秩扩展

Result: 在多个基准测试中，DR-LoRA在相同参数预算下始终优于标准LoRA和静态分配策略，实现了更优越的任务性能和更高效的参数利用率

Conclusion: DR-LoRA通过动态调整MoE模型中各专家的LoRA秩，解决了现有方法中资源分配不匹配的问题，实现了更智能的参数高效微调
...
### [204] [Key-Value Pair-Free Continual Learner via Task-Specific Prompt-Prototype](https://arxiv.org/abs/2601.04864)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于提示的持续学习方法通常依赖键值对机制，这会引入任务间干扰并限制可扩展性。需要一种无需键值对的新方法来更有效地学习和保留知识。

Method: 使用任务特定提示（task-specific prompts）促进当前任务的特征学习，同时对应原型（prototypes）捕捉输入的典型特征。推理时通过绑定任务特定提示与其关联原型生成预测。在提示初始化时引入正则化约束来惩罚过大值以增强稳定性。

Result: 在多个广泛使用的数据集上的实验证明了该方法的有效性。相比主流提示方法，本框架消除了对键值对的依赖。

Conclusion: 提出的ProP方法为持续学习研究提供了新视角，通过消除键值对依赖来减少任务间干扰并提高可扩展性，在多个数据集上表现出良好效果。
...
### [205] [Higher-Order Knowledge Representations for Agentic Scientific Reasoning](https://arxiv.org/abs/2601.04878)
*Isabella A. Stewart,Markus J. Buehler*

Main category: cs.AI

TL;DR: 该研究提出了一种基于超图的知识表示方法，用于科学发现，避免了传统知识图谱中成对约束的局限性，能够捕捉高阶相互作用，并应用于生物复合材料领域。
...
Method: 引入了一种构建基于超图的知识表示方法，该方法能够忠实编码多实体关系。应用于约1,100篇关于生物复合支架的手稿语料库，构建了包含161,172个节点和320,201条超边的全局超图。进一步展示了为智能体系统配备超图遍历工具（特别是使用节点交集约束）使其能够桥接语义上遥远的概念。

Result: 构建的超图显示出无标度拓扑结构（幂律指数约1.23），围绕高度连接的概念枢纽组织。这种表示避免了成对扩展中典型的组合爆炸，并明确保留了科学表述的共现上下文。通过利用这些高阶路径，系统成功为新型复合材料生成基于机制的假设，例如通过壳聚糖中间体将氧化铈与PCL支架联系起来。

Conclusion: 这项工作建立了一个"无教师"的智能推理系统，其中超图拓扑结构充当可验证的防护栏，通过揭示传统图方法所掩盖的关系来加速科学发现。
...
### [206] [Flexible Manufacturing Systems Intralogistics: Dynamic Optimization of AGVs and Tool Sharing Using Coloured-Timed Petri Nets and Actor-Critic RL with Actions Masking](https://arxiv.org/abs/2601.04887)
...
<details>
  <summary>Details</summary>
Motivation: 柔性制造系统在现代制造业中至关重要，传统作业车间调度问题需要进一步扩展以包含自动导引车和工具共享系统等复杂性因素，需要更高效的调度方法。

Method: 提出了一种新颖的方法，将着色时间Petri网与基于模型的强化学习相结合。CTPN提供形式化建模结构和动态动作屏蔽，大幅减少动作搜索空间；MBRL确保策略适应环境变化；还采用了前瞻策略优化AGV定位。

Result: 在小规模公共基准测试中与传统方法相当，在基于Taillard基准开发的大规模基准测试中优于传统方法（在完工时间方面），同时计算时间减少了十倍。

Conclusion: 该方法有效解决了包含AGV和工具共享的柔性制造系统调度问题，通过CTPN和MBRL的结合实现了高效调度，为复杂制造系统调度提供了有前景的解决方案。
...
### [207] [DVD: A Robust Method for Detecting Variant Contamination in Large Language Model Evaluation](https://arxiv.org/abs/2601.04895)
...
Method: 提出DVD方法，基于温度采样建模局部输出分布方差。核心洞察是：受污染项在记忆坚持状态和扰动漂移状态间交替，导致低概率标记的合成难度方差异常高；未污染项保持相对平滑的方差漂移

Result: 在Omni-MATH和SuperGPQA两个领域构建首个变体污染基准，通过微调不同规模和架构的模型模拟污染。DVD在数据集和模型上一致优于基于困惑度、Min-k%++、编辑距离和嵌入相似性的基线方法，且对超参数具有强鲁棒性

Conclusion: 生成分布的方差是检测大语言模型评估中变体污染的可靠实用指纹，DVD方法为评估污染检测提供了原则性解决方案
...
### [208] [From Stories to Cities to Games: A Qualitative Evaluation of Behaviour Planning](https://arxiv.org/abs/2601.04911)
*Mustafa F. Abdelwahed,Joan Espasa,Alice Toniolo,Ian P. Gent*

Main category: cs.AI

TL;DR: 行为规划作为一种新型多样化规划方法，通过显式整合多样性模型支持多类别规划，在故事创作、城市规划和游戏评估等现实场景中具有应用价值。


<details>
  <summary>Details</summary>
Motivation: 多样化规划方法旨在生成彼此不同的计划集合，在风险管理、自动化流数据分析、恶意软件检测等多个现实领域有广泛应用。传统方法在多样性建模和多类别规划支持方面存在局限，需要更先进的规划范式。

Method: 提出行为规划这一新型多样化规划范式，该方法扩展了早期方法，通过显式地将多样性模型整合到规划过程中，并支持多个规划类别，从而生成更具差异性的计划集合。

Result: 通过三个案例研究展示了行为规划在现实世界中的实用性：1) 故事创作领域；2) 城市规划领域；3) 游戏评估领域。这些案例验证了行为规划方法在不同应用场景中的有效性。

Conclusion: 行为规划作为一种创新的多样化规划方法，通过显式整合多样性模型和支持多类别规划，在多个现实世界领域展现出实用价值和应用潜力，为复杂规划问题提供了新的解决方案。
...
### [209] [What Students Ask, How a Generative AI Assistant Responds: Exploring Higher Education Students' Dialogues on Learning Analytics Feedback](https://arxiv.org/abs/2601.04919)
*Yildiz Uzun,Andrea Gauthier,Mutlu Cukurova*

Main category: cs.AI

TL;DR: 研究探索了在10周学期中，将对话式生成AI助手整合到学习分析仪表板中，分析不同自我调节学习能力水平学生与AI的对话模式、回答质量及学生感知，发现AI对低SRL学生特别有价值，但需要改进个性化、情感支持和多数据整合能力。


<details>
  <summary>Details</summary>
Motivation: 学习分析仪表板旨在通过数据反馈支持学生学习调节，但学生（特别是自我调节学习能力较低者）往往难以有效参与和解读分析反馈。对话式生成AI助手通过实时、个性化、基于对话的支持，有望为这一过程提供支架。

Method: 在10周学期中，将生成AI助手整合到学习分析仪表板中，收集学生与AI助手的真实对话。分析重点包括：不同SRL水平学生提出的问题类型、助手回答的相关性和质量，以及学生对助手在学习中角色的感知。

Result: 发现明显的查询模式差异：低SRL学生寻求澄清和安慰，高SRL学生询问技术细节和请求个性化策略。AI助手能提供清晰可靠的解释，但在个性化、处理情感化查询、整合多数据点提供定制响应方面存在局限。AI干预对低SRL学生特别有价值，能缩小与高SRL同伴的差距。

Conclusion: 生成AI助手在学习分析仪表板中有潜力，特别是对低SRL学生提供支架支持。未来系统需要增强信任度、提高适应性、加强情境感知能力并进行技术改进，以更好地满足不同SRL水平学生的需求。
...
### [210] [Conversational AI for Rapid Scientific Prototyping: A Case Study on ESA's ELOPE Competition](https://arxiv.org/abs/2601.04920)
*Nils Einecke*

Main category: cs.AI

TL;DR: 本文通过ESA ELOPE竞赛案例研究，展示了ChatGPT在科学发现中作为编程伙伴的潜力，实现了第二名成绩，同时分析了AI在代码生成、算法推理方面的优势与局限性。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在加速科学发现中的作用，特别是在竞争性科学环境中的实际应用潜力，填补LLMs作为编码伙伴在科学研究中角色未被充分探索的空白。

Method: 采用案例研究方法，在ESA的ELOPE竞赛中使用ChatGPT进行快速原型开发，处理事件相机数据以估计月球着陆器轨迹，分析AI在代码生成、算法推理、数据处理等方面的贡献。

Result: 尽管参赛较晚，团队仍获得第二名（得分0.01282），证明了人机协作在竞争性科学环境中的有效性。ChatGPT成功贡献了可执行代码、算法推理、数据处理例程和方法论建议。

Conclusion: 对话式AI既能加速开发又能支持科学研究的概念洞察，但存在引入不必要结构变化、被中间讨论混淆、产生关键错误等局限性。需要将LLMs结构化整合到科学工作流程中，制定AI辅助科学工作的最佳实践。
...
### [211] [T-Retriever: Tree-based Hierarchical Retrieval Augmented Generation for Textual Graphs](https://arxiv.org/abs/2601.04945)
*Chunyu Wei,Huaiyu Qin,Siyuan He,Yunhai Wang,Yueguo Chen*

Main category: cs.AI

TL;DR: T-Retriever：一种基于语义和结构引导编码树的图检索框架，通过自适应压缩编码和语义结构熵优化，解决了传统图RAG方法在层次信息管理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG方法在处理层次信息时存在两个关键限制：1）强制的层特定压缩配额会破坏局部图结构；2）优先考虑拓扑结构而忽略语义内容。需要一种能同时考虑结构和语义的层次信息管理方法。
...
Conclusion: T-Retriever通过语义和结构引导的编码树方法，有效解决了图RAG中的层次信息管理问题，实现了更好的检索增强生成性能。
...
TL;DR: ConMax是一个强化学习框架，通过置信度最大化压缩推理轨迹，减少冗余推理路径，在保持准确性的同时显著降低推理长度。
...
Method: ConMax将压缩建模为奖励驱动的优化问题，训练一个策略来剪枝冗余，通过最大化加权组合的答案置信度（预测保真度）和思考置信度（推理有效性），利用冻结的辅助大型推理模型进行评估。

Result: 在五个推理数据集上的实验表明，ConMax实现了优越的效率-性能权衡，相比强基线减少43%的推理长度，而准确性仅下降0.7%。

Conclusion: ConMax能够为大型推理模型生成高质量、高效的训练数据，有效解决推理轨迹压缩中的逻辑连贯性与采样成本问题。
...
### [213] [AlgBench: To What Extent Do Large Reasoning Models Understand Algorithms?](https://arxiv.org/abs/2601.04996)
*Henan Sun,Kaichi Yu,Yuyao Wang,Bowen Liu,Xunkai Li,Rong-Hua Li,Nuo Chen,Jia Li*

Main category: cs.AI

TL;DR: AlgBench是一个由ACM算法专家构建的算法推理基准测试，包含3000多个问题，涵盖27种算法，用于评估大推理模型在算法推理方面的真实能力。


<details>
  <summary>Details</summary>
Motivation: 现有算法推理基准测试有限，无法回答大推理模型是否真正掌握了算法推理能力的问题，需要更专业的评估工具。

Method: 构建AlgBench基准测试，包含3000多个原始问题，涵盖27种算法，采用算法中心范式，包括欧几里得结构、非欧几里得结构、非优化、局部优化、全局优化和启发式优化等分类。

Result: 对领先大推理模型的评估显示显著的性能异质性：在非优化任务上表现良好（高达92%），但在动态规划等全局优化算法上准确率急剧下降至约49%。发现模型存在"策略过度转移"现象，即过早放弃正确的算法设计。
...
### [214] [An Empirical Investigation of Robustness in Large Language Models under Tabular Distortions](https://arxiv.org/abs/2601.05009)
*Avik Dutta,Harshit Nigam,Hosein Hasanbeig,Arjun Radhakrishna,Sumit Gulwani*

Main category: cs.AI

TL;DR: LLMs在处理表格数据时，当表格表示出现语义和结构扭曲时表现不佳，缺乏自动检测和纠正扭曲的能力，即使有明确提示也只能部分纠正，准确率下降至少22%。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理表格数据时，当表格表示出现语义和结构扭曲时的失败情况，探索模型是否具备自动检测和纠正扭曲的能力。

Method: 引入一个专家策划的小型数据集，专门评估LLMs在表格问答任务中需要先进行错误纠正步骤的能力，研究模型在不同扭曲条件下的表现。

Result: LLMs缺乏检测和纠正表格扭曲的内在能力，即使提供系统提示也只能部分调整推理策略，不能完全纠正扭曲。即使是GPT-5.2等最先进模型，在扭曲条件下准确率也至少下降22%。

Conclusion: 研究揭示了LLMs在扭曲表格信息处理中的系统性差异，提出了重要研究问题：模型何时以及如何能够像人类一样自主决定重新对齐表格输入，而不依赖明确提示或表格数据预处理。
...
### [215] [OptiSet: Unified Optimizing Set Selection and Ranking for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.05027)
*Yi Jiang,Sendong Zhao,Jianbo Li,Bairui Hu,Yanrui Du,Haochun Wang,Bing Qin*

Main category: cs.AI

TL;DR: OptiSet是一个用于检索增强生成的集合中心框架，通过"扩展-精炼"范式、自合成策略和集合列表训练，解决传统top-k方法无法利用组合增益和冗余过多的问题。
...
Method: OptiSet采用"扩展-精炼"范式：1) 将查询扩展为多个视角以获取多样化候选池；2) 通过重选择精炼候选池形成紧凑证据集；3) 设计自合成策略从生成器的集合条件效用变化中推导偏好标签；4) 引入集合列表训练策略联合优化集合选择和集合级排序。
...
### [216] [How to Set the Batch Size for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05034)
*Yunhua Zhou,Junhao Huang,Shuhao Xin,Yechen Zhang,Runyu Peng,Qiping Guo,Xipeng Qiu*

Main category: cs.AI

TL;DR: 本文针对Warmup-Stable-Decay学习率调度器，修正了临界批大小的理论框架，提出了动态批大小调度器，显著提升预训练效率和质量。


<details>
  <summary>Details</summary>
Motivation: 随着Warmup-Stable-Decay学习率调度器成为主流，OpenAI提出的临界批大小理论框架已无法适应新的预训练动态，需要建立理论与实践之间的桥梁。

Method: 针对WSD调度器推导了修正的E(S)关系，揭示了B_min和B_opt两个关键属性，并基于此提出了动态批大小调度器。

Result: 修正公式准确捕捉了大规模预训练的动态特性，动态调度策略显著提升了训练效率和最终模型质量。

Conclusion: 本文为WSD调度器下的预训练提供了新的理论框架和实用调度策略，填补了原有理论与现代实践之间的空白。
...
### [217] [How to Set the Learning Rate for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05049)
*Yunhua Zhou,Shuhao Xing,Junhao Huang,Xipeng Qiu,Qipeng Guo*

Main category: cs.AI

TL;DR: 该研究探讨大规模预训练中学习率优化问题，比较了Fitting和Transfer两种研究范式，发现μTransfer在大规模场景下扩展性有限，并提供了工业级预训练优化指南。
...
Method: 研究将问题形式化为两种研究范式：1) Fitting范式：创新性地引入搜索因子的缩放定律，通过预测建模将搜索复杂度从O(n³)降低到O(n*C_D*C_η)；2) Transfer范式：将μTransfer原理扩展到MoE架构，涵盖模型深度、权重衰减和token范围。

Result: 实证结果挑战了广泛采用的μTransfer在大规模预训练场景中的可扩展性。通过训练稳定性和特征学习的双重视角分析，阐明了模块级参数调优在大规模设置中表现不佳的根本原因。

Conclusion: 这项工作为工业级预训练优化提供了系统的实践指南和新的理论视角，揭示了不同超参数研究范式在大规模场景下的适用性限制。
...
### [218] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
...
<details>
  <summary>Details</summary>
Motivation: 真实世界中的智能体需要预测动作后果的能力，传统世界模型通常需要动作标签，而这些标签在大规模获取时很复杂。因此需要从视频中学习潜在动作模型，但现有研究主要关注简单的机器人模拟、视频游戏或操作数据，缺乏对野外多样化视频的处理能力。

Method: 研究讨论了动作应遵循的属性、相关架构选择和评估方法。采用连续但受约束的潜在动作来捕捉野外视频中动作的复杂性，而不是常见的向量量化方法。训练控制器将已知动作映射到潜在动作，使潜在动作成为通用接口。

Result: 研究发现连续但受约束的潜在动作能够捕捉野外视频中动作的复杂性，而向量量化方法则不能。能够学习特定于野外视频的动作，例如人类进入房间等环境变化可以在视频间转移。在没有共同体现的情况下，主要学习到相对于相机空间定位的潜在动作。控制器能够以与动作条件基线相似的性能解决规划任务。

Conclusion: 该研究为将潜在动作模型扩展到真实世界提供了重要步骤，展示了从野外视频中学习复杂动作的可行性，并为使用潜在动作作为通用接口进行规划任务奠定了基础。
...
### [219] [Large language models can effectively convince people to believe conspiracies](https://arxiv.org/abs/2601.05050)
...
TL;DR: GPT-4o在移除安全护栏后，既能有效反驳阴谋论，也能同样有效地推广阴谋论，且推广阴谋论时更受用户喜爱并增加对AI的信任。标准版GPT-4o也有类似效果，但要求AI只使用准确信息能显著降低其推广阴谋论的能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型（LLMs）的说服力是否更倾向于支持真相而非虚假信息，或者LLMs是否同样容易推广错误信念。虽然LLMs在各种情境下被证明具有说服力，但尚不清楚这种说服力是否有利于真相而非虚假信息。

Method: 通过三个预注册实验，让2,724名美国参与者与GPT-4o讨论他们不确定的阴谋论。模型被指示要么反驳（"揭穿"）要么支持（"推广"）该阴谋论。使用移除安全护栏的"越狱"版GPT-4o和标准版GPT-4o进行对比。

Result: 1. 越狱版GPT-4o在增加阴谋论信念方面与减少信念同样有效；2. 推广阴谋论的AI比揭穿阴谋论的AI获得更积极的评价，并增加对AI的信任；3. 标准版GPT-4o产生非常相似的效果，OpenAI的安全护栏几乎无法阻止LLM推广阴谋论；4. 纠正性对话可以逆转新诱导的阴谋论信念；5. 提示GPT-4o只使用准确信息能显著降低其增加阴谋论信念的能力。

Conclusion: LLMs具有强大的能力来推广真相和虚假信息，但存在潜在的解决方案来帮助减轻这种风险。虽然AI可以同样有效地推广阴谋论和揭穿阴谋论，但通过适当的干预措施（如要求只使用准确信息）可以显著降低其推广虚假信息的能力。
...
### [220] [Publishing FAIR and Machine-actionable Reviews in Materials Science: The Case for Symbolic Knowledge in Neuro-symbolic Artificial Intelligence](https://arxiv.org/abs/2601.05051)
...
TL;DR: 将材料科学综述中的关键信息从静态PDF表格转换为FAIR、机器可操作的格式，通过Open Research Knowledge Graph实现结构化知识查询，并对比符号查询与LLM查询，主张符号层应作为可靠神经符号AI的骨干


<details>
  <summary>Details</summary>
Motivation: 材料科学综述的关键见解被锁定在叙述性文本和静态PDF表格中，限制了人类和机器的重复使用，需要将这些知识转换为可查询的结构化格式

Method: 以原子层沉积和蚀刻(ALD/E)为案例研究，将综述表格发布为ORKG中的FAIR、机器可操作的比较数据，建立结构化知识库，并对比符号查询与基于大语言模型的查询方法

Result: 成功将综述知识转换为结构化、可查询的格式，展示了符号查询与LLM查询的对比，验证了符号层作为神经符号AI骨干的重要性

Conclusion: 在材料科学中，经过策划的符号层应作为可靠神经符号AI的骨干，而LLM应作为补充性的、基于符号的接口，而非独立的真相来源
...
### [221] [Reinforced Efficient Reasoning via Semantically Diverse Exploration](https://arxiv.org/abs/2601.05053)
...
TL;DR: ROSE方法通过语义多样性探索和高效推理机制改进LLM的强化学习推理能力
...
Method: 提出ROSE方法：1）基于语义熵的分支策略和ε探索机制增加多样性；2）长度感知的段级优势估计器提高效率

Result: 在多个数学推理基准测试中验证了ROSE的有效性和效率

Conclusion: ROSE通过语义多样性探索和高效推理机制显著提升了LLM的推理性能
...
### [222] [Chain-of-Sanitized-Thoughts: Plugging PII Leakage in CoT of Large Reasoning Models](https://arxiv.org/abs/2601.05076)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型使用显式思维链推理提高了性能、可靠性和可解释性，但这种透明性带来了严重的隐私风险：即使最终答案经过清理，中间推理过程也常常会泄露个人身份信息。

Method: 1. 引入PII-CoT-Bench数据集，包含隐私感知的思维链标注；2. 创建类别平衡的评估基准，涵盖现实和对抗性泄露场景；3. 采用两种可部署的干预措施：基于提示的控制和模型微调。

Result: 1. 发现能力依赖趋势：最先进模型从基于提示的控制中获益最大，而较弱模型需要微调才能实现有意义的泄露减少；2. 两种方法都能显著减少PII暴露，同时性能下降最小；3. 隐私思维链推理可以在不牺牲性能的情况下实现。

Conclusion: 隐私优先的思维链推理可以通过最小化性能损失来实现，为构建隐私保护推理系统提供了实用指导，展示了隐私和性能可以兼得。
...
### [223] [Arabic Prompts with English Tools: A Benchmark](https://arxiv.org/abs/2601.05101)
...
<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语原生大语言模型的快速发展，现有的评估基准大多专注于英语，缺乏对阿拉伯语工具调用能力的系统评估。用户使用阿拉伯语交互时，模型性能表现如何尚不清楚，这阻碍了为阿拉伯语用户开发可靠、公平的AI智能体。

Method: 开发了首个专门针对阿拉伯语大语言模型的工具调用和智能体能力评估基准，提供了一个标准化框架来衡量模型在阿拉伯语智能体工作流中的功能准确性和鲁棒性。

Result: 研究发现存在显著的性能差距：当用户使用阿拉伯语交互时，无论工具描述本身是阿拉伯语还是英语，工具调用准确率平均下降5-10%。

Conclusion: 该基准测试揭示了阿拉伯语大语言模型在工具调用方面的关键挑战，旨在促进为阿拉伯语用户开发更可靠、语言更公平的AI智能体。
...
### [224] [Token-Level LLM Collaboration via FusionRoute](https://arxiv.org/abs/2601.05106)
*Nuoya Xiong,Yuhang Zhou,Hanqing Zeng,Zhaorun Chen,Furong Huang,Shuchao Bi,Lizhu Zhang,Zhuokai Zhao*

Main category: cs.AI

TL;DR: FusionRoute是一个轻量级的路由框架，通过在每个解码步骤同时选择最合适的专家模型并提供互补logit来优化专家模型的token分布，解决了通用大模型成本高而专用小模型泛化能力差的两难问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型面临的困境：通用大模型需要巨大规模导致训练和部署成本过高，而小型领域专用模型虽然高效但泛化能力有限。现有token级协作方法仅依赖固定的专家输出，存在根本性限制。

Method: 提出FusionRoute框架，包含轻量级路由器，在每个解码步骤同时执行两个功能：1)选择最合适的专家模型；2)提供互补logit，通过logit加法来优化或修正所选专家的下一个token分布。该方法扩展了有效策略类别，在温和条件下能够恢复最优价值函数。

Result: 在Llama-3和Gemma-2系列模型以及涵盖数学推理、代码生成和指令遵循的多样化基准测试中，FusionRoute优于序列级和token级协作、模型融合和直接微调方法，同时在各自任务上保持与领域专家的竞争力。

Conclusion: FusionRoute通过结合专家选择和可训练的互补生成器，有效解决了通用大模型与专用小模型之间的权衡问题，提供了一种既高效又具有良好泛化能力的多LLM协作框架。
...
### [225] [GlimpRouter: Efficient Collaborative Inference by Glimpsing One Token of Thoughts](https://arxiv.org/abs/2601.05110)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然性能优异，但多步推理链生成带来高延迟和计算成本。现有协作推理方法依赖局部token概率或事后验证，仍引入显著推理开销。需要找到更高效的方法判断何时需要大模型能力。

Method: 提出GlimpRouter训练免费步进协作框架：利用轻量模型仅生成每个推理步骤的第一个token，通过初始token的熵值预测步骤难度，当熵值超过阈值时将步骤路由到大模型处理。

Result: 在多个基准测试中显著降低推理延迟同时保持准确性。例如在AIME25上，相比独立大模型，准确率提升10.7%同时延迟降低25.9%。

Conclusion: 初始token熵是推理步骤难度的强预测指标，基于思维一瞥而非全步骤评估的计算分配机制简单有效，为推理模型协作提供了新视角。
...
### [226] [Evaluative Fingerprints: Stable and Systematic Differences in LLM Evaluator Behavior](https://arxiv.org/abs/2601.05114)
*Wajid Nasser*

Main category: cs.AI

TL;DR: 研究发现LLM评估系统存在可靠性悖论：各评估者内部一致但彼此间几乎零一致，评估者间的分歧模式稳定到可作为指纹识别，每个评估者都有独特的"评估倾向"理论。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验LLM-as-judge系统的可靠性，发现虽然这些系统承诺可扩展、一致的评估，但实际上存在严重的评估者间不一致问题，这挑战了LLM评估作为标准化测量工具的基本假设。

Method: 使用9个评估者（LLM模型）对120个独特视频项目进行3次独立评估，共3240次评估。通过Krippendorff's α测量评估者间一致性，使用分类器根据评分特征识别评估者身份，并分析评估倾向的多个维度：严格度/宽松度、维度强调、内部一致性、证据行为等。

Result: 评估者间一致性接近零（α=0.042），某些维度上分歧甚至超过随机噪声预期（α<0）。但分歧模式高度结构化：仅凭评分就能以77.1%准确率识别评估者，加入倾向特征后达89.9%。同一模型家族内信号更强，GPT-4.1和GPT-5.2可99.6%准确区分。

Conclusion: LLM评估者不是测量共享构念的可互换工具，而是各自编码独特质量理论的独立测量设备。平均评分产生的是不符合任何评估者实际价值观的合成判断，这挑战了当前LLM评估实践的有效性基础。
...
### [227] [Distilling the Thought, Watermarking the Answer: A Principle Semantic Guided Watermark for Large Reasoning Models](https://arxiv.org/abs/2601.05144)
...
<details>
  <summary>Details</summary>
Motivation: 现有数字水印方法在应用于推理大语言模型时存在两个主要问题：基于标记的水印技术会破坏推理流程的逻辑连贯性，而语义感知方法虽然能提高质量但会引入显著延迟或需要辅助模型。因此需要一种专门针对推理密集型LLMs的水印框架，能够在保持逻辑完整性的同时实现高效水印嵌入。

Method: 1. 将生成过程解耦为两个阶段：不受干扰的思考阶段和水印化的回答阶段；2. 提出关键性评分机制，从推理轨迹中识别语义关键标记；3. 将关键标记蒸馏为主要语义向量；4. 基于标记与PSV的对齐程度，通过语义自适应机制调节水印强度。

Result: ReasonMark在多项指标上超越现有最先进方法：文本困惑度降低0.35，翻译BLEU分数提高0.164，数学准确率提升0.67分。同时实现了0.34%更高的水印检测AUC和更强的抗攻击鲁棒性，仅带来可忽略的延迟增加。

Conclusion: ReasonMark框架为推理大语言模型提供了一种高效、鲁棒且不影响逻辑完整性的水印解决方案，使得推理LLMs能够在现实应用中实现可追溯和可信赖的部署。
...
### [228] [Observations and Remedies for Large Language Model Bias in Self-Consuming Performative Loop](https://arxiv.org/abs/2601.05184)
*Yaxuan Wang,Zhongteng Cai,Yujia Bao,Xueru Zhang,Yang Liu*

Main category: cs.AI

TL;DR: 研究提出自我消耗执行循环（SCPL）概念，探讨在受控执行反馈下合成数据如何影响动态迭代训练过程中的偏见演化，发现执行循环会增加偏好偏见但减少差异偏见，并提出基于奖励的拒绝采样策略来缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展导致使用合成数据训练未来模型的兴趣增长，但这形成了自我消耗的再训练循环，可能导致性能下降和偏见产生。在现实应用中，先前部署的LLM可能影响其生成的数据，形成由用户反馈驱动的动态系统。由于无法获取动态生产系统中的真实用户偏好数据，需要建立受控环境来隔离和分析反馈驱动的偏见演化。

Method: 引入自我消耗执行循环（SCPL）概念，在受控执行反馈设置下研究合成数据如何塑造偏见。研究两种循环类型：典型的再训练设置和增量微调设置。通过三个现实世界任务的实验，分析偏见演化模式，并设计基于奖励的拒绝采样策略来缓解偏见。

Result: 实验发现执行循环会增加偏好偏见但减少差异偏见。基于奖励的拒绝采样策略能够有效缓解偏见，推动构建更可信的自我改进系统。

Conclusion: 自我消耗执行循环在动态迭代训练中显著影响偏见演化，需要设计专门的缓解策略。提出的基于奖励的拒绝采样方法能够有效减少偏见，为构建更可信的自我改进系统提供了可行路径。
...
### [229] [MineNPC-Task: Task Suite for Memory-Aware Minecraft Agents](https://arxiv.org/abs/2601.05215)
*Tamil Sudaravan Mohan Doss,Michael Xu,Sudha Rao,Andrew D. Wilson,Balasaravanan Thoravi Kumaravel*

Main category: cs.AI

TL;DR: 提出了MineNPC-Task基准测试框架，用于在开放世界《我的世界》中评估具有记忆能力的混合主动LLM智能体，通过真实玩家交互收集任务，包含参数化模板和可机器验证的检查器。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体评估通常依赖合成提示，缺乏真实玩家交互的复杂性和开放性。需要建立基于真实游戏体验的基准测试，以评估智能体在开放世界环境中的记忆能力和混合主动交互表现。

Method: 通过专家玩家的形成性和总结性协同游戏收集任务，将其规范化为具有明确前提条件和依赖结构的参数化模板。采用有界知识策略，禁止使用游戏世界外的捷径，并配备机器可检查的验证器。框架捕获计划/行动/记忆事件，包括计划预览、针对性澄清、内存读写、前提条件检查和修复尝试。

Result: 使用GPT-4o对8名经验玩家的216个子任务进行评估。观察到代码执行、库存/工具处理、引用和导航方面的重复性故障模式，同时混合主动澄清和轻量级内存支持了恢复。参与者对交互质量和界面可用性评价积极，但强调需要更强的跨任务记忆持久性。

Conclusion: MineNPC-Task提供了一个透明、可复现的评估框架，用于测试记忆感知的具身智能体。通过发布完整的任务套件、验证器、日志和测试工具，支持未来智能体在开放世界环境中的评估和改进。
