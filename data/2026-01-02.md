...
TL;DR: 利用Depth Anything V2的高保真合成先验，通过DV-LORA高效适配到医疗领域，在手术内镜单目深度估计中实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 传统自监督方法在手术内镜环境中（特别是反光和液体表面）表现脆弱，容易在薄手术工具和透明表面上出现边界塌陷问题

Method: 1. 利用Depth Anything V2架构的高保真合成先验；2. 采用动态向量低秩适配(DV-LORA)高效适配到医疗领域；3. 在SCARED数据集上引入物理分层评估协议

Result: 在SCARED数据集上达到98.1%的准确率(<1.25)，平方相对误差比现有基线降低超过17%，在不利手术光照条件下表现出优越鲁棒性

Conclusion: 通过合成先验和高效适配策略，显著提升了手术内镜单目深度估计的准确性和鲁棒性，特别是在高反光区域
...
<details>
  <summary>Details</summary>
Motivation: 城市作战训练需要情境意识和肌肉记忆，但传统评估方法依赖昂贵传感器或主观观察，限制了可扩展性和准确性。需要一种客观、可扩展的自动评估方法来评估认知、心理运动和团队合作技能。

Method: 开发视频评估管道，利用计算机视觉模型从训练视频中提取2D骨骼、视线向量和运动轨迹。基于这些数据开发特定任务指标，测量心理运动流畅性、情境意识和团队协调。将这些指标整合到扩展的认知任务分析层次结构中，使用加权组合生成团队合作和认知的整体表现分数。

Result: 通过真实世界ECR演练的案例研究展示了该方法，提供了可操作的领域特定指标，能够捕捉个人和团队表现。这些见解可以支持行动后审查，通过交互式仪表板在Gamemaster和GIFT框架中提供直观易懂的反馈。

Conclusion: 该方法为合成训练环境中的可扩展评估提供了有前景的解决方案，但存在跟踪困难、地面真实验证和更广泛适用性等限制。未来工作包括扩展到3D视频数据分析，利用视频分析实现STE中的可扩展评估。
...
TL;DR: 提出基于深度学习的儿科MRI转合成CT框架，实现颅骨分割和骨缝可视化，解决儿童颅骨发育评估中CT辐射风险与MRI骨结构显示不足的矛盾。


<details>
  <summary>Details</summary>
Motivation: 儿科颅骨发育和骨缝骨化评估对诊断生长相关颅脑疾病至关重要。CT虽有辐射风险，MRI虽无辐射但无法清晰显示颅骨和骨缝结构，需要一种无辐射且能提供骨结构信息的替代方案。

Method: 提出深度学习驱动的流程：将T1加权MRI转换为合成CT，预测详细的颅骨分割，生成骨缝概率热图，并从热图中直接分割骨缝。使用领域特定的变分自编码器生成感知上难以区分的合成CT。

Result: 合成CT与真实CT的结构相似度达99%，Frechet inception距离为1.01。颅骨分割在七个颅骨上平均Dice系数达85%，骨缝分割Dice系数达80%。通过TOST检验证实合成CT与真实CT在颅骨和骨缝分割上的等效性。

Conclusion: 这是首个能从MRI生成合成CT并进行骨缝分割的儿科颅骨CT合成框架，填补了无创颅骨评估的关键空白，为儿科颅骨发育监测提供了安全有效的替代方案。
...
Conclusion: MGML框架能有效处理不完整多模态MRI数据，不改变原始模型架构，可方便集成到训练流程中实现端到端优化
...
TL;DR: 提出基于AI的微血管吻合术自动动作分割与性能评估框架，用于神经外科手术培训的客观评估


<details>
  <summary>Details</summary>
Motivation: 传统微血管吻合术评估依赖专家主观评分，存在评分者间差异、不一致性和时间成本高的问题，需要自动化、可扩展的客观评估方案

Method: 基于边缘计算的三模块框架：1) YOLO和DeepSORT的目标尖端跟踪定位；2) 自相似矩阵的动作边界检测和无监督聚类；3) 监督分类模块评估手术手势熟练度

Result: 在58个专家评分的微血管吻合视频数据集上验证，帧级动作分割准确率达92.4%，技能分类准确率达85.5%

Conclusion: 该方法能为微外科教育提供客观、实时反馈，实现标准化、数据驱动的培训协议，推进高风险手术环境中的能力评估
...
TL;DR: T2VAttack：针对文本到视频扩散模型的对抗攻击研究，从语义和时间两个维度评估模型脆弱性，通过替换或插入关键词实现有效攻击。


<details>
  <summary>Details</summary>
Motivation: 尽管文本到视频（T2V）扩散模型在生成高质量、时间连贯的视频方面取得了显著进展，但其对抗攻击的脆弱性尚未得到充分研究。考虑到视频数据的动态特性，需要从语义和时间两个角度全面评估这些模型的鲁棒性。

Method: 提出两种对抗攻击方法：1) T2VAttack-S：识别提示中语义或时间关键词，通过贪婪搜索用同义词替换；2) T2VAttack-I：迭代插入经过优化的关键词，对提示进行最小扰动。同时定义了语义目标（评估视频-文本对齐）和时间目标（评估时间动态）。

Result: 在多个最先进的T2V模型（ModelScope、CogVideoX、Open-Sora、HunyuanVideo）上进行全面评估。实验表明，即使对提示进行微小修改（如替换或插入单个词），也会导致语义保真度和时间动态的显著退化，揭示了当前T2V扩散模型的关键脆弱性。

Conclusion: 当前文本到视频扩散模型在面对精心设计的对抗攻击时存在显著脆弱性，即使是微小的提示修改也能严重影响生成视频的质量。这突显了提高T2V模型鲁棒性的重要性，并为未来防御机制的设计提供了基础。
...
TL;DR: 提出了一种仅使用图像和可选相机位姿的自适应驾驶场景视图外推方法，通过4D高斯模型与视频扩散模型的迭代优化，无需依赖昂贵的传感器或标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖激光雷达点云、3D边界框和车道标注等先验信息，需要昂贵传感器或人工标注，限制了实际部署的适用性。本文旨在仅使用图像和可选相机位姿实现高质量视图外推。

Method: 首先从图像和相机位姿估计全局静态点云和逐帧动态点云，融合为统一表示；采用可变形4D高斯框架重建场景；用初始训练的4D高斯模型生成降质伪图像训练视频扩散模型；通过扩散模型迭代优化逐步偏移的高斯渲染结果，并将增强结果反馈给4DGS作为训练数据，直至外推到目标视点。

Result: 与基线方法相比，该方法在新外推视点上生成更高质量的图像。

Conclusion: 该方法仅需图像和可选相机位姿即可实现高质量视图外推，无需昂贵传感器或人工标注，提高了实际部署的适用性，通过4D高斯模型与视频扩散模型的迭代优化机制取得了显著效果。
...
TL;DR: 基于SATLAS基础模型的深度学习方法，利用卫星时间序列的时序冗余性，通过学习预测无变化情况下的地表状态来检测异常，相比传统方法灵敏度提高约3倍。
...
Method: 使用基于SATLAS基础模型的修复模型，利用全球分布的训练数据（涵盖不同气候带和土地覆盖类型），从先前采集的Sentinel-2时间序列中重建最后一帧图像。

Result: 在2023年土耳其-叙利亚地震序列引发的地表破裂检测中，该方法在Tepehan地区检测到裂谷特征，比时间中值或Reed-Xiaoli异常检测器具有更高的灵敏度和特异性。

Conclusion: 该方法检测阈值比基线方法低约3倍，为利用免费多光谱卫星数据实现自动化、全球尺度的地表变化监测提供了途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有U-Net类CNN模型存在局部性限制，难以建模长距离上下文依赖，尤其在多器官和低对比度区域；而Transformer方法虽然能解决此问题，但计算资源需求高，不利于临床部署。需要一种既能增强全局表征又保持计算效率的解决方案。

Method: 提出轻量级即插即用的分组坐标注意力(GCA)模块：1)将通道上下文建模解耦为多个组，显式考虑通道间的语义异质性；2)集成方向感知坐标编码，沿水平和垂直轴捕获结构化空间依赖。将GCA模块集成到ResUNet架构中形成GCA-ResUNet。

Result: 在Synapse和ACDC两个基准数据集上分别达到86.11%和92.64%的Dice分数，优于包括Swin-UNet和TransUNet在内的多种代表性CNN和Transformer方法。特别在复杂边界的小解剖结构分割上表现一致提升。

Conclusion: GCA-ResUNet在分割精度和计算效率之间取得了良好平衡，为临床部署提供了实用且可扩展的解决方案。分组坐标注意力模块有效增强了全局表征能力，同时保持了CNN主干的效率优势。
...
<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观歧义时经常失败，这主要是由于过度依赖不稳定的外观特征（如阴影、眩光、局部纹理）。需要引入更稳定的拓扑信息来弥补这一缺陷。

Method: 提出GASeg框架，核心包括：1) 可微分盒计数模块从几何和外观两个并行流中量化多尺度拓扑统计；2) 拓扑增强策略通过形态学操作模拟真实世界歧义；3) GALoss多目标损失强制几何与外观特征的跨模态对齐。

Result: 在四个基准测试（包括COCO-Stuff、Cityscapes和PASCAL）上实现了最先进的性能，验证了通过拓扑信息桥接几何和外观的方法有效性。

Conclusion: 通过引入拓扑信息，GASeg成功解决了自监督语义分割中的外观歧义问题，实现了几何与外观特征的稳定对齐，为语义分割提供了更鲁棒的解决方案。
...
TL;DR: 提出了一种新的管道，利用3D高斯泼溅技术从RPO图像序列中重建未知航天器的3D结构，并通过整合太阳位置先验知识来改善光照条件下的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 在空间交会与接近操作中，从图像序列重建未知航天器的3D结构对导航和操作至关重要。传统3DGS需要静态场景假设，而空间图像存在动态光照变化，这影响了重建质量和后续姿态估计的准确性。

Method: 使用3D高斯泼溅表示目标几何和外观，将服务航天器估计的太阳位置先验知识整合到训练管道中，通过光度优化改善渲染质量，使模型能够适应空间快速变化的光照条件。

Result: 实验研究表明，提出的解决方案有效，3DGS模型能够学习适应空间快速变化的光照条件，准确反映全局阴影和自遮挡现象，提高了渲染的光度准确性。

Conclusion: 通过整合太阳位置先验知识，提出的方法成功解决了空间动态光照对3DGS重建的挑战，为RPO过程中的航天器3D重建和姿态估计提供了更准确的解决方案。
...
TL;DR: 本文提出CVC框架，通过双视角速度转换机制和基于经验贝叶斯推断的后验一致更新，解决流式扩散编辑中速度误差累积导致的语义不一致和结构保真度损失问题。


<details>
  <summary>Details</summary>
Motivation: 现有流式扩散编辑方法在源图像和目标图像分布间直接转换时，潜在轨迹存在累积速度误差，导致语义不一致和结构保真度损失，需要更稳定的编辑框架。

Method: 提出条件速度校正(CVC)框架，将流式编辑重构为基于已知源先验的分布转换问题。引入双视角速度转换机制，将潜在演化分解为结构保持分支和语义引导分支，并通过经验贝叶斯推断和Tweedie校正进行后验一致更新。

Result: CVC方法产生稳定且可解释的潜在动态，实现忠实重建和平滑的局部语义转换。实验表明CVC在多种任务中始终实现更优的保真度、更好的语义对齐和更可靠的编辑行为。

Conclusion: CVC框架通过理论基础的误差补偿机制，解决了流式扩散编辑中的速度误差累积问题，为图像编辑提供了更稳定、保真度更高的解决方案。
...
TL;DR: 提出FitControler，一个可学习的插件，用于虚拟试穿中的服装合身度控制，通过合身感知布局生成器和多尺度合身注入器实现定制化合身度调整


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法主要关注服装细节的忠实渲染，但忽略了塑造整体风格的关键因素——服装合身度。合身度定义了服装如何与穿着者的身体对齐，是时尚设计的基本要素

Method: 提出FitControler插件，包含合身感知布局生成器（重新绘制身体-服装布局）和多尺度合身注入器（传递布局线索实现布局驱动的虚拟试穿）。构建了Fit4Men数据集，包含13,000个不同合身度的身体-服装对

Result: FitControler可以与各种虚拟试穿模型配合工作，实现准确的合身度控制。引入了两种合身一致性指标来评估生成结果的合身度

Conclusion: 该工作首次将合身度控制引入虚拟试穿领域，通过可学习的插件架构实现了定制化的合身度调整，为虚拟试穿提供了更全面的风格协调能力
...
TL;DR: MF-RSVLM是一个多特征融合的遥感视觉语言模型，通过提取和融合多尺度视觉特征，结合全局上下文与局部细节，并采用循环视觉特征注入机制，在遥感分类、图像描述和视觉问答任务上取得了先进性能。
...
Method: 提出MF-RSVLM模型，学习多尺度视觉表示，结合全局上下文与局部细节；采用循环视觉特征注入方案，确保语言模型在生成过程中保持对视觉证据的依赖，减少视觉遗忘。

Result: 在多个遥感基准测试上的广泛实验表明，MF-RSVLM在遥感分类、图像描述和视觉问答任务上达到了最先进的或极具竞争力的性能。

Conclusion: MF-RSVLM通过有效的多特征融合和循环视觉特征注入机制，成功解决了遥感视觉语言模型中的细粒度特征提取和视觉遗忘问题，在遥感理解任务上表现出色。
...
TL;DR: RSAgent是一种基于多模态大语言模型的智能体，通过多轮工具调用实现文本引导的目标分割，相比传统单次预测方法具有更好的验证、重新聚焦和细化能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导分割方法通常采用单次预测方式，直接将像素提示传递给外部分割器，当初始定位错误时缺乏验证、重新聚焦和细化的能力，限制了分割性能。

Method: 提出RSAgent智能体，通过多轮工具调用交替进行推理和分割操作；构建多轮推理分割轨迹的数据管道；采用两阶段训练框架：冷启动监督微调后接基于细粒度任务特定奖励的智能体强化学习。

Result: 在ReasonSeg测试集上达到66.5%的零样本gIoU，比Seg-Zero-7B提升9%；在RefCOCOg上达到81.5%的cIoU，在领域内和领域外基准测试中都达到最先进性能。

Conclusion: RSAgent通过多轮工具调用的智能体方法，显著提升了文本引导分割的性能，证明了交替推理和行动策略在视觉定位任务中的有效性。
...
TL;DR: 提出基于强化学习的可训练各向异性扩散框架，通过深度Q学习选择扩散动作序列，构建自适应图像结构的随机扩散过程，在去噪性能上超越传统扩散方法，与深度CNN方法竞争。


<details>
  <summary>Details</summary>
Motivation: 传统各向异性扩散方法使用显式扩散算子，难以适应复杂图像结构，性能有限；而学习型方法表现更好，因此需要开发结合两者优势的新框架。

Method: 将去噪过程建模为一系列朴素扩散动作序列，通过深度Q学习优化动作顺序选择，构建可训练的各向异性扩散框架，形成自适应图像结构的随机扩散过程。

Result: 提出的去噪器在三种常见噪声去除任务中表现出色，超越了现有基于扩散的方法，并与代表性的深度CNN方法竞争。

Conclusion: 基于强化学习的可训练各向异性扩散框架能够有效提升图像去噪性能，通过自适应扩散过程克服传统方法的局限性，为图像恢复任务提供了新思路。
...
TL;DR: 提出NIRNL框架解决跨模态检索中的噪声标签问题，通过跨模态边界保持和邻居感知实例细化，在不同噪声子集上采用定制化优化策略，实现高性能、高可靠性和高数据利用率。
...
Method: 提出NIRNL框架：1) 跨模态边界保持(CMP)调整正负样本对的相对距离，增强样本对区分度；2) 邻居感知实例细化(NIR)通过跨模态邻居共识识别纯子集、困难子集和噪声子集；3) 为这种细粒度划分构建不同的定制优化策略，最大化利用所有可用数据同时减轻错误传播。

Result: 在三个基准数据集上的大量实验表明，NIRNL实现了最先进的性能，表现出显著的鲁棒性，特别是在高噪声率下。

Conclusion: NIRNL框架通过跨模态边界保持和邻居感知实例细化，有效解决了跨模态检索中的噪声标签问题，实现了高性能、高可靠性和高数据利用率的统一。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集使用层次标签结构进行粗到细的对象区分，但将这种语义层次嵌入表示学习空间以提升检测性能仍具挑战。现有方法在不同层次应用监督对比学习，但忽视了两个关键问题：1) 标签层次中数据分布不平衡导致高频类别主导学习过程；2) 类别间语义关系学习干扰与类别无关的定位任务。
...
<details>
  <summary>Details</summary>
Motivation: Diffusion Transformer在视频和图像生成任务中因注意力机制导致极高的计算成本，限制了实际应用。随着硬件发展，除了GPU外，ASIC等设备也被广泛采用。现有稀疏注意力方法存在两个关键限制：稀疏模式预测的开销和缺乏硬件通用性。
...
Result: 实验结果表明，RainFusion2.0能够实现80%的稀疏度，在保持视频质量的同时获得1.5-1.8倍的端到端加速。该方法在各种生成模型中都表现出有效性，并验证了在不同硬件平台上的泛化能力。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，提供了一种高效、通用且低开销的稀疏注意力机制，能够显著加速Diffusion Transformer在视频和图像生成任务中的推理过程，具有广泛的硬件兼容性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在事件级感知的准确时序定位方面存在困难。研究发现时序定位和文本响应这两个主要因素存在逻辑层次关系：准确的时序证据定位是可靠文本响应的基础。然而现有方法通常以耦合方式处理这两个任务，缺乏清晰的逻辑结构，导致目标次优。

Method: 提出D²VLM框架，解耦两个任务的学习同时强调其内在依赖，采用"先定位后基于证据回答"范式，引入证据令牌进行证据定位，强调超越时间戳表示的事件级视觉语义捕获。进一步提出因子化偏好优化算法，将概率时序定位建模明确纳入优化目标，实现时序定位和文本响应的偏好学习。还构建合成数据集以支持因子化偏好学习。

Result: 在各种任务上的实验证明了该方法的明显优势。

Conclusion: 通过解耦学习和因子化偏好优化，D²VLM框架显著提升了视频语言模型的时序定位能力和事件级感知性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法将问题视为直接翻译，面临语义-运动学阻抗不匹配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图落地到运动学密集的高频运动数据。

Method: 提出Latent Motion Reasoning (LMR)框架，采用两阶段"思考-行动"决策过程。核心是新颖的双粒度分词器，将动作解耦为两个不同的流形：用于规划全局拓扑的压缩语义丰富的推理潜在空间，以及用于保持物理保真度的高频执行潜在空间。

Result: 在T2M-GPT（离散）和MotionStreamer（连续）两个代表性基线上实现LMR，实验表明LMR在语义对齐和物理合理性方面都带来了非平凡的改进，验证了运动规划的最佳基底不是自然语言，而是学习的动作对齐概念空间。

Conclusion: 通过引入潜在系统2推理架构，将文本到动作生成重新表述为两阶段决策过程，有效弥合了语言和物理之间的不可言说性鸿沟，为动作生成提供了更优的解决方案。
...
TL;DR: 提出一种无需训练的生成式对抗攻击框架，通过扩散模型生成自然、场景一致的对抗物体，显著影响单目深度估计系统，在自动驾驶安全评估中具有实际意义。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计是自动驾驶系统的核心感知模块，但容易受到对抗攻击。现有物理攻击主要依赖纹理贴片，存在放置限制大、真实性有限的问题，在复杂驾驶环境中效果不佳。

Method: 提出训练免费的生成式对抗攻击框架：1）显著区域选择模块识别对MDE影响最大的区域；2）雅可比向量积引导机制将对抗梯度引导至预训练扩散模型支持的更新方向；3）基于扩散的条件生成过程生成自然、场景一致的对抗物体。
...
TL;DR: GeoBench是一个几何推理分层基准，包含四个推理级别，通过六个正式验证的任务评估视觉语言模型在几何问题解决中的能力，发现推理模型性能随任务复杂度增加而显著下降。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在几何推理评估中存在三个主要问题：1) 基于教科书的基准测试存在测试数据污染风险；2) 过于关注最终答案而忽视推理过程；3) 诊断粒度不足。需要更全面的评估框架来系统分析几何问题解决能力。

Method: 提出GeoBench分层基准，包含四个推理级别：视觉感知、目标导向规划、严格定理应用、自反思回溯。通过TrustGeoGen生成六个正式验证的任务，系统评估从属性提取到逻辑错误纠正的能力范围。

Result: 实验显示：1) 推理模型（如OpenAI-o3）优于通用多模态大语言模型；2) 性能随任务复杂度增加而显著下降；3) 子目标分解和无关前提过滤对最终解题准确性有重要影响；4) 思维链提示在某些任务中意外降低性能。

Conclusion: GeoBench为几何问题解决提供了一个全面的评估基准，并为开发几何问题解决系统提供了可操作的指导方针，揭示了当前模型在复杂几何推理中的局限性。
...
TL;DR: 论文提出D²-Align框架解决文本到图像扩散模型对齐中的偏好模式崩溃问题，通过方向性解耦奖励信号来保持生成多样性


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈强化学习的方法虽然能在自动化奖励指标上获得高分，但会导致偏好模式崩溃，即模型收敛到狭窄的高分输出模式，严重降低生成多样性

Method: 提出方向性解耦对齐框架，首先在奖励模型的嵌入空间中学习方向性修正，然后在优化过程中应用修正后的奖励信号，防止模型崩溃到特定模式

Result: D²-Align在质量和多样性指标上均表现出色，实现了与人类偏好的更好对齐，同时保持了生成多样性

Conclusion: 通过方向性修正奖励信号可以有效缓解偏好模式崩溃问题，在保持生成多样性的同时实现更好的对齐效果
...
TL;DR: 提出了首个大规模工业多模态缺陷数据集IMDD-1M，包含100万对齐的图像-文本对，并基于此训练了面向工业场景的扩散式视觉语言基础模型，该模型通过轻量微调即可高效适应特定领域任务。


<details>
  <summary>Details</summary>
Motivation: 工业制造和质量检测领域缺乏大规模、高质量的多模态数据集，限制了多模态学习在工业场景中的应用。现有方法通常需要大量任务特定数据，缺乏通用且可高效适应的基础模型。

Method: 1) 构建IMDD-1M数据集：包含100万对齐图像-文本对，覆盖60+材料类别和400+缺陷类型，每对数据包含专家验证的标注和细粒度文本描述；2) 从零开始训练扩散式视觉语言基础模型，专门针对工业场景设计；3) 通过轻量微调将基础模型适应到特定领域任务。

Result: IMDD-1M数据集支持分类、分割、检索、描述生成和生成建模等多种应用。训练的基础模型仅需不到5%的任务特定数据即可达到专用专家模型的性能水平，展示了数据高效的基础模型适应能力。

Conclusion: IMDD-1M数据集和相应的基础模型为工业检测和生成任务提供了可扩展、领域自适应和知识基础的解决方案，推动了面向制造业智能的多模态学习发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）的推理过程仍然以文本为中心，在复杂的长期、视觉中心任务中表现不佳。需要一种新的推理范式来提升视觉任务的逻辑一致性和空间精度。

Method: 提出DiffThinker框架，采用基于扩散的生成式多模态推理范式，将多模态推理重新定义为原生的图像到图像生成任务。该方法具有效率、可控性、原生并行性和协作性四个核心特性。

Result: 在四个领域（顺序规划、组合优化、约束满足、空间配置）的广泛实验中，DiffThinker显著优于GPT-5（+314.2%）、Gemini-3-Flash（+111.6%）和微调的Qwen3-VL-32B基线（+39.0%）。

Conclusion: 生成式多模态推理是视觉中心推理的有前景方法，DiffThinker展示了在复杂视觉任务中超越现有文本中心方法的优势，为多模态推理开辟了新方向。
...
Result: 在UniMoCap（20小时人形动作基准）上验证，实现了低于500ms的延迟，零样本跟踪不完美参考动作的成功率提升19%，在多样化真实场景中表现出鲁棒泛化能力。

Conclusion: UniAct框架是实现响应式通用人形助手的关键一步，通过统一感知与控制实现无缝交互，为人形机器人执行多模态指令提供了有效解决方案。
...
TL;DR: DGC是一种用于高光谱图像分割的内存高效框架，通过局部补丁学习全局聚类结构，无需预训练，在消费级硬件上30分钟内完成训练，但存在优化不稳定性问题。
...
Method: 提出深度全局聚类（DGC）框架，通过重叠区域的小补丁操作来强制执行一致性，从局部补丁观察中学习全局聚类结构，无需预训练，保持恒定内存使用。

Result: 在叶片病害数据集上，DGC实现了背景-组织分离（平均IoU 0.925），并通过可导航的语义粒度展示了无监督病害检测能力。但框架存在优化不稳定性，源于多目标损失平衡问题。

Conclusion: 该工作作为智力支架具有价值，设计理念有优点，但稳定实现需要动态损失平衡的原则性方法。代码和数据已开源。
...
TL;DR: 本文提出了一个多模态预训练的综合框架，旨在解决自动驾驶系统中不同传感器（如摄像头和LiDAR）数据融合的挑战，以实现真正的空间智能。
...
Method: 提出了一个多模态预训练的综合框架，分析了基础传感器特性与学习策略之间的相互作用，评估了平台特定数据集的作用。核心贡献是制定了预训练范式的统一分类法：从单模态基线到学习整体表示的复杂统一框架，用于3D物体检测和语义占用预测等高级任务。还研究了文本输入和占用表示的集成，以促进开放世界感知和规划。

Result: 该框架为多模态预训练提供了系统化的方法，识别了推动进展的核心技术集。通过统一分类法，能够更好地理解和比较不同的预训练范式。

Conclusion: 本文识别了计算效率和模型可扩展性等关键瓶颈，并提出了实现通用多模态基础模型的路线图，这些模型能够为实际部署实现鲁棒的空间智能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能捕捉完整的数据分布，但由于训练数据不足难以覆盖低概率区域，导致生成对应区域的高质量图像时表现不佳。现有的指导策略如CFG存在样本过度简化或失真的问题，而基于"坏版本"的指导方法则需要精心设计的退化策略、额外训练和采样步骤。
...
Conclusion: 内部指导（IG）是一种简单而有效的策略，通过引入中间层监督和外推机制，显著提升了扩散模型的生成质量，在多个基准测试中取得了突破性的性能表现，特别是在与CFG结合时达到了当前最优水平。
...
TL;DR: CorGi是一种无需训练的DiT推理加速框架，通过选择性重用transformer块输出来减少去噪步骤间的冗余计算，实现最高2倍加速


<details>
  <summary>Details</summary>
Motivation: DiT模型在视觉生成中表现出色，但其迭代去噪过程结合大容量导致高推理成本。现有研究表明DiT的去噪过程在步骤间存在大量冗余计算

Method: 提出CorGi框架，基于贡献度指导的块级间隔缓存机制，缓存低贡献块并在后续步骤中重用；针对文本到图像任务提出CorGi+，利用跨注意力图识别关键token并应用部分注意力更新

Result: 在先进的DiT模型上评估显示，CorGi和CorGi+平均实现最高2.0倍加速，同时保持高质量生成
...
TL;DR: CropTrack是一个用于农业环境多目标跟踪的新框架，通过结合外观和运动信息来解决重复模式、相似外观和频繁遮挡带来的挑战，在身份保持方面优于传统运动跟踪方法。


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似物体外观、突然光照变化和频繁遮挡等挑战。现有跟踪器主要依赖运动信息，但在频繁强遮挡时难以保持物体身份，而相似的外观特征使得外观关联在农业场景中难以有效整合。

Method: CropTrack提出一个结合外观和运动信息的MOT框架，包含：1）重排序增强的外观关联；2）一对多关联与基于外观的冲突解决策略；3）指数移动平均原型特征库来改进外观关联。

Result: 在公开的农业MOT数据集上评估，CropTrack展现出持续的身份保持能力，优于传统的基于运动的跟踪方法。与最先进方法相比，CropTrack在识别F1分数和关联准确率方面取得显著提升，同时减少了身份切换次数。

Conclusion: CropTrack通过有效整合外观和运动信息，解决了农业环境中多目标跟踪的挑战，特别是在频繁遮挡下保持物体身份方面表现出色，为农业MOT提供了更可靠的解决方案。
...
TL;DR: 本文提出ARM模块，一种轻量级可学习模块，通过自适应融合CLIP的层次特征来提升开放词汇语义分割性能，实现"一次训练，随处使用"的范式。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法要么依赖昂贵的外部基础模型（如SAM、DINO），要么使用静态启发式方法处理CLIP内部特征，这些方法要么计算成本高，要么效果欠佳。CLIP的图像级表示缺乏像素级细节，限制了开放词汇语义分割的性能。

Method: 提出注意力精炼模块（ARM），包含语义引导的交叉注意力块（使用深层特征K、V来选择和精炼细节丰富的浅层特征Q），以及自注意力块。该模块采用"一次训练，随处使用"的范式，在通用数据集（如COCO-Stuff）上训练一次后，可作为通用即插即用的后处理器用于各种训练免费框架。

Result: 大量实验表明，ARM在多个基准测试中持续提升基线性能，且推理开销可忽略不计，为训练免费的开放词汇语义分割建立了高效有效的范式。

Conclusion: ARM模块有效解锁和精炼了CLIP的内部潜力，通过自适应融合层次特征解决了现有方法的局限性，为开放词汇语义分割提供了一种轻量级、高效的解决方案。
...
Method: 开发DarkEQA基准测试，在原始RAW空间模拟物理真实的照明下降和传感器噪声，然后通过ISP渲染管道处理，评估模型在控制退化条件下的问答能力。
...
TL;DR: Mirage是一个用于驾驶场景资产编辑的一步视频扩散模型，通过结合2D和3D编码器特征保持时间一致性和视觉保真度，并采用两阶段数据对齐策略解决分布不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的自动驾驶系统需要多样化和可扩展的训练数据来实现鲁棒性能。现有的视频对象编辑方法在保持高视觉保真度和时间一致性方面存在困难，需要一种更好的数据增强方法。

Method: 1. 基于文本到视频扩散先验确保时间一致性；2. 将预训练2D编码器的时序无关潜在特征注入3D解码器以恢复细节；3. 引入两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）解决分布不匹配问题。

Result: Mirage在多种编辑场景中实现了高真实感和时间一致性，并能泛化到其他视频到视频转换任务，为未来研究提供了可靠的基准。

Conclusion: Mirage是一个有效的一步视频扩散模型，能够实现驾驶场景中逼真且时间一致的资产编辑，解决了现有方法在视觉保真度和时间一致性方面的挑战。
...
TL;DR: SpaceTimePilot是一个视频扩散模型，能够解耦空间和时间控制，实现可控的生成式渲染。给定单目视频，它可以独立改变摄像机视角和运动序列，在生成过程中重新渲染场景，实现跨空间和时间的连续任意探索。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以实现对空间（摄像机视角）和时间（运动序列）的独立精确控制。缺乏成对的具有连续时间变化的动态场景视频数据集，限制了模型学习时空解耦控制的能力。

Method: 1. 引入有效的动画时间嵌入机制，在扩散过程中显式控制输出视频相对于源视频的运动序列；2. 提出简单有效的时间扭曲训练方案，重新利用现有的多视角数据集模拟时间差异；3. 改进摄像机条件机制，允许从第一帧开始改变摄像机；4. 创建CamxTime数据集，首个合成时空全覆盖渲染数据集，提供场景内完全自由的时空视频轨迹。

Result: 在真实世界和合成数据上的评估表明，SpaceTimePilot实现了清晰的时空解耦，与先前工作相比取得了显著改进。联合训练时间扭曲方案和CamxTime数据集产生了更精确的时间控制。

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、时间扭曲训练方案和专门的时空数据集，成功实现了视频生成中空间和时间的解耦控制，为可控生成式渲染提供了新的解决方案。
...
TL;DR: MambaSeg：一种新颖的双分支语义分割框架，使用并行Mamba编码器处理RGB图像和事件流，通过双维交互模块在时空维度进行细粒度融合，在降低计算成本的同时实现最先进的性能。
...
Method: 提出MambaSeg框架：1）使用并行Mamba编码器分别处理RGB图像和事件流；2）引入双维交互模块（DDIM），包含跨空间交互模块（CSIM）和跨时序交互模块（CTIM），在时空维度进行细粒度融合；3）通过跨模态对齐减少模糊性，利用各模态互补特性。

Result: 在DDD17和DSEC数据集上的广泛实验表明，MambaSeg实现了最先进的语义分割性能，同时显著降低了计算成本，展示了其在高效、可扩展和鲁棒的多模态感知方面的潜力。

Conclusion: MambaSeg通过并行Mamba编码器和双维交互模块，有效融合RGB和事件数据，在保持高效计算的同时提升了语义分割性能，为多模态感知系统提供了有前景的解决方案。
...
TL;DR: PGMP框架通过物理基础流形投影解决牙科CBCT金属伪影问题，结合解剖自适应物理模拟生成训练数据，使用DMP-Former进行确定性单次前向恢复，并通过语义结构对齐确保临床合理性。
...
Method: 提出物理基础流形投影(PGMP)框架：1) 解剖自适应物理模拟(AAPS)通过蒙特卡洛频谱建模和患者特异性数字孪生合成高质量训练对；2) DMP-Former采用直接x预测范式，将恢复重构为确定性流形投影，单次前向通过即可恢复干净解剖；3) 语义结构对齐(SSA)模块利用医学基础模型(MedDINOv3)的先验知识锚定解决方案。

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于现有最先进方法，在效率和诊断可靠性方面设定了新的基准。

Conclusion: PGMP框架有效解决了牙科CBCT金属伪影减少的挑战，通过物理基础模拟、确定性流形投影和医学先验知识的结合，实现了高效且临床可靠的伪影去除。
...
Method: 提出DualityForge框架：1) 使用可控扩散视频编辑将真实视频转换为反事实场景；2) 嵌入结构化上下文信息自动生成高质量QA对和原始-编辑视频对；3) 构建DualityVidQA数据集；4) 提出DNA-Train两阶段训练方法（SFT-RL），在RL阶段应用成对ℓ₁优势归一化实现稳定高效策略优化。

Result: 在DualityVidQA-Test上，方法显著减少模型在反事实视频上的幻觉，相比Qwen2.5-VL-7B基线相对提升24.0%。在幻觉和通用基准测试中都取得显著增益，表现出强大的泛化能力。

Conclusion: DualityForge框架通过合成高质量反事实数据和DNA-Train训练方法，有效解决了MLLM在视频理解中的幻觉问题，提高了模型对反事实场景的理解能力，具有很好的泛化性能。作者将开源数据集和代码。
...
TL;DR: 提出一种空间提升的全景拼接框架，将2D图像提升为3D点云表示，通过全局融合和圆柱投影生成几何一致的360°全景图，有效减少视差和遮挡带来的失真问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于2D单应性变换和网格扭曲的图像拼接方法在处理真实3D场景（多深度层、遮挡）时会产生重影、结构弯曲和拉伸失真，特别是在多视角累积和360°闭环拼接场景中问题更加严重。

Method: 1. 将输入图像提升为统一坐标系下的密集3D点表示；2. 通过置信度指标进行全局跨视图融合；3. 在3D空间建立统一投影中心；4. 使用等距圆柱投影将融合数据映射到全景流形；5. 在画布域进行空洞填充以恢复连续纹理和语义一致性。

Result: 实验评估表明，该方法在显著视差和复杂遮挡场景中大幅减少了几何失真和重影伪影，生成了更自然、更一致的全景结果。

Conclusion: 该框架将拼接从2D扭曲范式重新概念化为3D一致性范式，能够灵活集成各种3D提升和补全模块，为复杂3D场景的全景拼接提供了更有效的解决方案。
...
TL;DR: EndoRare是一个单样本、无需重新训练的生框架，能够从单张参考图像合成多样化的高保真罕见胃肠道病变图像，用于AI模型训练和临床教育。
...
Method: 通过语言引导的概念解缠，将病变的病理特征与非诊断属性分离，将前者编码为可学习的原型嵌入，同时变化后者以确保多样性。

Result: 在四种罕见病理上验证，合成图像被专家判断为临床可信；用于数据增强显著提升下游AI分类器性能；新手内镜医生使用EndoRare生成案例后，召回率提高0.400，精确率提高0.267。
...
Method: 开发Virtual-Eyes 16位CT质量控制流程：强制执行512x512平面分辨率，剔除短序列或非诊断序列，通过亨氏单位滤波和双侧肺覆盖评分提取连续肺块，保持原生16位网格。使用765名NLST患者数据，计算RAD-DINO和Merlin的切片级嵌入，训练无泄漏的患者级MLP头部，同时评估Sybil和2D ResNet-18基线模型

Result: Virtual-Eyes显著提升RAD-DINO性能：切片级AUC从0.576提升至0.610，患者级AUC从0.646提升至0.683（平均池化），从0.619提升至0.735（最大池化），校准度改善（Brier分数从0.188降至0.112）。但Sybil和ResNet-18在Virtual-Eyes下性能下降，Merlin转移性有限

Conclusion: 解剖学针对性的质量控制可以稳定并改进通用基础模型工作流程，但可能破坏已适应原始临床环境的专用模型，表明预处理策略需要根据模型类型进行定制
...
Method: CERES采用双模态因果干预：1）后门调整原则解决语言表示偏见，对抗数据集统计中的偏见；2）前门调整概念解决视觉混淆，通过因果原则指导将语义视觉特征与几何深度信息智能整合，创建对自我中心失真更鲁棒的表征。

Result: 在Ego-RVOS基准测试中，CERES实现了最先进的性能，证明了因果推理在构建更可靠的自我中心视频理解模型方面的潜力。

Conclusion: CERES框架通过因果推理有效解决了自我中心视频中的语言偏见和视觉混淆问题，为更广泛的自我中心视频理解任务提供了可靠的方法。
...
TL;DR: SenseNova-MARS是一个多模态智能体推理与搜索框架，通过强化学习赋予视觉语言模型交错视觉推理和工具使用能力，在搜索导向的视觉理解任务中达到最先进性能。
...
Method: 提出SenseNova-MARS多模态智能体推理与搜索框架，通过强化学习赋予视觉语言模型交错视觉推理和工具使用能力。框架动态整合图像搜索、文本搜索和图像裁剪工具来处理细粒度和知识密集的视觉理解挑战。在强化学习阶段，提出了批量归一化组序列策略优化算法来提升训练稳定性并增强模型调用工具和有效推理的能力。
...
Conclusion: SenseNova-MARS代表了向智能体视觉语言模型迈出的有希望的一步，通过提供有效且稳健的工具使用能力。为了促进该领域的进一步研究，将发布所有代码、模型和数据集。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的自动驾驶方法在复杂场景理解和决策中存在瓶颈，主要问题是缺乏准确的度量空间推理和几何推断能力，导致驾驶策略不可靠。视觉语言模型虽然具有常识推理潜力，但仅依赖2D图像线索难以保证安全性和可靠性。

Method: 提出LVLDrive框架，将LiDAR点云作为额外输入模态。采用渐进融合Q-Former逐步注入LiDAR特征，确保预训练VLM知识库的稳定性和保留。同时开发空间感知问答数据集，专门训练模型的3D感知和推理能力。

Result: 在多个驾驶基准测试中，LVLDrive在场景理解、度量空间感知和可靠驾驶决策方面均优于仅使用视觉的对比方法，证明了3D度量数据对于构建可信VLM自动驾驶系统的必要性。

Conclusion: 研究强调了显式3D度量数据对于构建可信赖的基于VLM的自动驾驶系统的重要性，LVLDrive通过融合LiDAR数据有效提升了模型的3D空间理解能力，为更安全可靠的自动驾驶提供了新思路。
...
TL;DR: DermaVQA-DAS扩展了皮肤病图像分析数据集，新增患者查询和临床背景，支持封闭式问答和病变分割任务，引入专家开发的皮肤病评估框架DAS，并评估了多种多模态模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有皮肤病图像分析数据集主要关注皮肤镜图像，缺乏患者查询和临床背景，限制了其在以患者为中心的护理中的应用。需要填补这一空白，支持更全面的皮肤病视觉语言建模研究。

Method: 引入Dermatology Assessment Schema (DAS)专家框架，包含36个高级和27个细粒度评估问题，支持中英文多选项。扩展DermaVQA数据集为DermaVQA-DAS，提供专家标注的封闭式问答和分割数据集，并评估多种多模态模型和提示策略。

Result: 分割任务中，提示设计影响性能：默认提示在Mean-of-Max和Mean-of-Mean评估方案中表现最佳，而增强提示（结合患者查询标题和内容）在多数投票评估中获得最高性能（Jaccard指数0.395，Dice分数0.566）。封闭式问答中，o3模型总体准确率最高（0.798），GPT-4.1紧随其后（0.796），Gemini-1.5-Pro在Gemini家族中表现竞争性（0.783）。

Conclusion: DermaVQA-DAS填补了现有皮肤病图像分析数据集的空白，提供了患者查询和临床背景支持，通过DAS框架实现了标准化评估，为以患者为中心的皮肤病视觉语言建模研究提供了重要资源，公开数据集和评估协议以加速未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 实时视频分析在大量摄像头上运行成本高昂，现有动态模型大小选择方法无法很好地适应多样化工作负载，特别是在移动视频和低精度目标场景下，存在采样成本过高和精度预测不准确的问题

Method: RedunCut采用测量驱动的规划器来估计采样的成本效益权衡，并使用轻量级、数据驱动的性能模型来改进精度预测，解决了现有方法的两个主要缺陷

Result: 在道路车辆、无人机和监控视频以及多种模型家族和任务中，RedunCut在固定精度下减少14-62%的计算成本，并且在有限历史数据和漂移情况下保持鲁棒性

Conclusion: RedunCut通过改进的动态模型大小选择方法，有效解决了实时视频分析中的计算成本问题，为多样化工作负载提供了更高效、更鲁棒的解决方案
...
TL;DR: DyStream：基于流匹配的自回归模型，实现实时双人对话头部视频生成，延迟低于100ms，在HDTF数据集上达到SOTA唇同步质量
...
Method: 采用流友好的自回归框架，结合流匹配头部进行概率建模；提出由前瞻模块增强的因果编码器，纳入短期未来上下文（如60ms）以提升质量同时保持低延迟

Result: 每帧生成时间34ms，系统总延迟低于100ms；在HDTF数据集上达到离线8.13和在线7.61的LipSync Confidence分数，显著优于蒸馏和生成编码器等替代因果策略

Conclusion: DyStream通过简单有效的设计实现了实时、高质量的对话头部视频生成，解决了现有方法的高延迟问题，为实时交互应用提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 传统手术技能评估依赖专家监督，存在主观性、评估者间差异大、耗时耗力等问题，尤其在低收入国家难以规模化实施。需要开发客观、可扩展的自动化评估方法。

Method: 提出AI驱动框架：1) 基于TimeSformer的视频Transformer架构，改进为分层时间注意力和加权空间注意力机制，实现准确动作识别；2) 使用YOLO目标检测和跟踪方法提取精细运动特征，分析器械运动学；3) 从五个方面评估微血管吻合术技能。

Result: 在58个专家标注视频数据集上验证：动作分割帧级准确率达87.7%，后处理后提升至93.62%；在复制专家评估方面，所有技能方面的平均分类准确率达76%。

Conclusion: 该系统能够提供客观、一致且可解释的反馈，有望实现更标准化、数据驱动的手术教育和评估，特别有助于资源有限地区的培训可扩展性。
...
TL;DR: 本文通过离散小波变换(DWT)分析视觉Transformer(ViT)编码器中的组合性，发现单层DWT分解的基元能在潜在空间中近似组合，揭示了ViT如何结构化视觉信息。


<details>
  <summary>Details</summary>
Motivation: 虽然对Transformer模型的理解主要来自语言任务分析，但本文旨在研究视觉Transformer(ViT)编码器通过组合性视角学习的表示，探索ViT如何结构化视觉信息。

Method: 引入一个框架来测量ViT编码器中的组合性，使用离散小波变换(DWT)获取视觉输入相关的基元，通过检查组合表示重构原始图像表示的能力来实证测试表示空间中的组合性程度。

Result: 研究发现，单层DWT分解产生的基元能够在潜在空间中近似组合，这为理解ViT如何结构化信息提供了新的视角。

Conclusion: 视觉Transformer编码器通过离散小波变换基元表现出组合性特征，这为理解ViT如何处理和结构化视觉信息提供了新的见解。
...
Method: 使用两个互补模块：1) 光谱间窗口图嵌入(iSWGE)，将光谱通道表示为图节点并学习边缘特征；2) 窗口空间图注意力与卷积块注意力(WSGA-C)，结合稀疏图注意力和卷积注意力减少空间冗余并强调精细结构
...
<details>
  <summary>Details</summary>
Motivation: 虽然AI在医学影像分类和检测任务上表现出色，但现有系统通常只输出结构化预测结果，放射科医生仍需将这些结果转化为完整的叙述性报告。大型语言模型的出现为弥合这一差距提供了新机会，能够从结构化发现生成诊断性叙述。

Method: 研究引入了一个集成YOLOv5和YOLOv8进行胸部X光图像异常检测的管道，然后将检测结果（边界框预测和类别标签）输入大型语言模型（LLM）生成自然语言的放射学报告。比较了YOLOv5和YOLOv8在检测准确性、推理延迟方面的性能，并通过余弦相似度评估生成文本的质量。

Result: 结果显示AI生成报告与人类报告之间具有强烈的语义相似性。人工评估表明GPT-4在清晰度方面表现出色（4.88/5），但在自然写作流畅性方面得分较低（2.81/5），表明当前系统虽然达到临床准确性，但在风格上仍可与放射科医生撰写的文本区分开来。

Conclusion: 该研究证明了结合计算机视觉和大型语言模型生成放射学报告的可行性，系统能够产生临床准确的报告，但在写作风格的自然流畅性方面仍有改进空间，需要进一步优化以实现更接近人类放射科医生的叙述质量。
...
TL;DR: 提出PhyGDPO框架，通过物理增强视频数据管道和物理感知分组直接偏好优化，解决文本到视频生成中物理一致性不足的问题


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在视觉质量上取得进展，但难以生成符合物理规律的视频。基于图形或提示扩展的方法难以泛化到复杂模拟环境之外，且缺乏包含丰富物理交互的训练数据

Method: 1) 提出物理增强视频数据管道PhyAugPipe，利用视觉语言模型进行链式思维推理，构建大规模训练数据集PhyVidGen-135K；2) 设计物理感知分组直接偏好优化框架PhyGDPO，基于分组Plackett-Luce概率模型捕捉整体偏好；3) 引入物理引导奖励方案PGR，嵌入基于VLM的物理奖励以引导优化方向；4) 提出LoRA-Switch Reference方案，消除内存密集型参考复制以提高训练效率

Result: 在PhyGenBench和VideoPhy2基准测试中，该方法显著优于现有最先进的开源方法

Conclusion: 通过物理增强数据管道和物理感知优化框架，有效提升了文本到视频生成中的物理一致性，为生成符合物理规律的视频提供了系统解决方案
...
TL;DR: 提出一种新颖的二阶优化算法，通过OCP方法和Hessian矩阵对角元素近似，解决深度学习中的大规模优化问题，在视觉定位任务上表现出色。
...
Conclusion: 提出的二阶优化算法在深度学习的大规模优化问题上具有显著优势，为视觉定位等任务提供了更高效、稳定的优化解决方案。
...
TL;DR: EVOL-SAM3：一个零样本推理分割框架，通过进化搜索过程在推理时迭代优化提示假设，显著超越静态方法和完全监督方法。
...
Method: 提出EVOL-SAM3框架，将推理分割重新定义为推理时的进化搜索过程。维护提示假设种群，通过"生成-评估-进化"循环迭代优化。引入视觉竞技场进行无参考成对竞赛评估提示适应度，语义变异算子注入多样性并纠正语义错误，异构竞技场模块整合几何先验与语义推理确保最终选择的鲁棒性。

Result: 在具有挑战性的ReasonSeg基准测试中，EVOL-SAM3不仅显著超越静态基线方法，而且在零样本设置下显著超过了完全监督的最先进方法。

Conclusion: EVOL-SAM3通过进化搜索范式成功解决了当前推理分割方法的局限性，实现了零样本下的优异性能，为复杂语言查询的像素级定位提供了新的解决方案。
...
TL;DR: 该论文提出DARFT框架，通过决策模糊性引导的强化微调来解决变化检测视觉问答中的决策模糊问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 在变化检测视觉问答中，许多失败案例并非源于明显错误预测，而是由于决策模糊性——模型对正确答案和强干扰项赋予相似的置信度。这种决策模糊样本限制了模型的判别能力和鲁棒性。

Method: 提出DARFT框架：1) 使用监督微调训练的参考策略挖掘决策模糊样本；2) 在挖掘的子集上应用组相对策略优化，利用多样本解码和组内相对优势来抑制强干扰项并锐化决策边界。

Result: 实验表明DARFT在监督微调基线上取得了一致的性能提升，特别是在少样本设置下效果显著，有效提高了模型的判别能力和鲁棒性。

Conclusion: 决策模糊性是变化检测视觉问答中的重要挑战，DARFT框架通过针对性地优化决策模糊样本，无需额外监督就能有效提升模型性能，为相关领域提供了有价值的解决方案。
...
TL;DR: SliceLens：一个基于LLM/VLM的假设驱动框架，通过视觉推理发现多实例视觉任务中的细粒度错误切片，并提出了首个细粒度切片发现基准FeSD。


<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类任务，难以应用于检测、分割、姿态估计等多实例任务。真实场景中的错误切片通常涉及复杂的视觉关系，现有方法缺乏细粒度推理能力，且现有基准多针对特定算法或偏向图像分类，无法反映真实模型失败情况。

Method: 提出SliceLens框架，利用大语言模型（LLMs）和视觉语言模型（VLMs）生成和验证多样化的失败假设，通过基于视觉的推理来可靠识别细粒度且可解释的错误切片。

Result: 在现有基准和新提出的FeSD基准上，SliceLens实现了最先进的性能，在FeSD上将Precision@10提高了0.42（0.73 vs 0.31）。识别出的可解释切片有助于模型改进，模型修复实验验证了其有效性。

Conclusion: SliceLens通过LLM/VLM驱动的假设生成和验证，能够有效发现多实例视觉任务中的细粒度错误切片，为模型评估和修复提供了可靠工具。FeSD基准为细粒度切片发现研究提供了标准化评估平台。
...
TL;DR: 该研究创建了首个针对飓风灾后环境的3D语义分割数据集，评估了现有SOTA模型在灾后场景中的表现，发现现有方法存在显著局限性，强调了开发专门灾后3D数据集和技术的必要性。


<details>
  <summary>Details</summary>
Motivation: 自然灾害频发对人类生命和经济造成严重威胁，而现有的3D语义分割模型缺乏专门针对灾后环境的数据集，这限制了灾后评估和响应的能力。

Method: 使用无人机采集飓风伊恩（2022）灾区的航拍影像，通过运动恢复结构（SfM）和多视图立体（MVS）技术重建3D点云，创建专门的3D数据集。在该数据集上评估了Fast Point Transformer（FPT）、Point Transformer v3（PTv3）和OA-CNNs等最先进的3D语义分割模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有作物疾病诊断方法依赖昂贵的监督微调，且在领域转移下表现不佳，需要准确且可解释的解决方案

Method: CPJ框架：使用大型视觉语言模型生成多角度图像描述，通过LLM-as-Judge模块迭代优化，然后进行双答案VQA过程，提供识别和管理响应

Result: 在CDDMBench评估中，使用GPT-5-mini描述时，GPT-5-Nano在疾病分类上提升22.7个百分点，QA得分提升19.5分，优于无描述基线

Conclusion: CPJ框架提供透明、基于证据的推理，无需微调即可推进稳健且可解释的农业诊断，代码和数据已公开
...
Method: 提出协作低秩适应(CLoRA)，包含基础空间共享和样本无关多样性增强(SADE)组件。所有低秩模块共享一组下/上投影空间，从共享空间获得的低秩矩阵协作构建每个LRM，SADE正则化矩阵间相似性以鼓励多样化表示。

Result: 在广泛使用的图像和点云数据集上的实验表明，CLoRA在学习性能和参数效率之间取得了更好的平衡，在点云分析中需要最少的GFLOPs。

Conclusion: CLoRA通过基础空间共享和多样性增强机制，有效解决了现有LoRA方法在性能和参数效率之间的权衡问题，是一种更平衡的视觉Transformer微调方法。
...
### [62] [Video and Language Alignment in 2D Systems for 3D Multi-object Scenes with Multi-Information Derivative-Free Control](https://arxiv.org/abs/2512.24826)
*Jason Armitage,Rico Sennnrich*

Main category: cs.CV

TL;DR: 提出一种新方法，通过基于遗憾最小化和无导数优化的多元互信息估计，使现成的2D跨模态系统能够在线适应3D场景中的物体遮挡和特征区分


<details>
  <summary>Details</summary>
Motivation: 解决跨模态系统从2D视觉输入处理3D场景时的维度转换问题，避免需要预训练或微调，使现有系统能直接适应3D多物体场景

Method: 使用基于遗憾最小化和无导数优化的多元互信息估计方法，通过场景内相机控制模块，直接从视觉语言模型的噪声输出中学习

Result: 该方法提升了跨模态系统在多物体3D场景任务中的性能，无需预训练或微调即可在线适应物体遮挡和特征区分

Conclusion: 结合表达性度量和基于价值的优化，能够有效控制场景内相机，使现成的2D跨模态系统成功适应3D场景处理
...
TL;DR: 本文提出了面向户外监控场景的3D视觉定位新任务，构建了首个真实世界大规模多模态数据集MoniRefer，并提出了端到端方法Moni3DVG，利用图像外观信息和点云几何光学信息进行多模态特征学习与3D目标定位。


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉定位研究主要关注室内和室外驾驶场景，而由于路边基础设施传感器采集的配对点云-文本数据稀缺，户外监控场景仍然未被探索。这对于路边基础设施系统在复杂交通环境中理解自然语言并定位相关目标对象至关重要。

Method: 提出了名为Moni3DVG的端到端方法，该方法利用图像提供的丰富外观信息以及点云的几何和光学信息进行多模态特征学习，从而实现3D目标定位。

Result: 构建了MoniRefer数据集，包含约136,018个对象和411,128个自然语言表达，收集自真实世界多个复杂交通路口。在提出的基准上进行了大量实验和消融研究，证明了方法的优越性和有效性。

Conclusion: 本文引入了面向户外监控场景的3D视觉定位新任务，超越了传统的车辆视角，实现了基础设施级别的交通场景理解。提出的数据集和方法为这一新领域的研究奠定了基础。
...
### [64] [FireRescue: A UAV-Based Dataset and Enhanced YOLO Model for Object Detection in Fire Rescue Scenes](https://arxiv.org/abs/2512.24622)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究存在两个主要局限：1）主要关注山区或森林环境，对更频繁且结构复杂的城市救援场景关注不足；2）现有检测系统类别有限（如火焰、烟雾），缺乏覆盖消防车、消防员等指挥决策关键目标的全面系统。

Method: 1）构建新的FireRescue数据集，涵盖城市、山区、森林和水域多种救援场景，包含8个关键类别共15,980张图像和32,000个边界框；2）提出FRS-YOLO改进模型，引入即插即用的多维协同增强注意力模块增强易混淆类别的判别表示，集成动态特征采样器强化高响应前景特征。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用顺序token-by-token解码机制，导致推理效率低下。本文观察到视觉token具有强烈的局部依赖性和空间相关性，但标准光栅扫描解码顺序未能充分利用这一特性。

Method: 提出RadAR框架：1）采用径向拓扑结构，以初始token为中心，将所有其他token按空间距离分组到多个同心环中；2）按环状从内向外并行生成同一环内的所有token；3）引入嵌套注意力机制，在前向传播中动态修正不合理输出，缓解错误累积。

Result: RadAR在保持自回归模型表征能力的同时，显著提高了生成效率。径向并行预测设计不仅保持了视觉场景的结构局部性和空间连贯性，还大幅增加了并行化程度。

Conclusion: 通过结合径向并行预测和动态输出修正，RadAR为高效自回归视觉生成提供了一种新颖的解决方案，在保持生成质量的同时实现了显著的效率提升。
...
TL;DR: HaineiFRDM是一个基于扩散模型的电影修复框架，通过分块训练策略和全局-局部频率模块，在单张24GB显存GPU上实现高分辨率电影修复，性能优于现有开源方法。


<details>
  <summary>Details</summary>
Motivation: 现有开源电影修复方法性能有限，主要原因是使用低质量合成数据训练、采用噪声光流，且未探索高分辨率电影修复。需要开发更强大的修复框架来帮助专家修复难以识别的电影缺陷。

Method: 提出HaineiFRDM框架：1) 采用分块训练和测试策略，使单张24GB显存GPU能处理高分辨率电影；2) 设计位置感知的全局提示和帧融合模块；3) 引入全局-局部频率模块重建不同分块间的一致性纹理；4) 先修复低分辨率结果作为全局残差，减轻分块过程产生的块状伪影；5) 构建包含真实退化电影修复数据和逼真合成数据的电影修复数据集。

Result: 综合实验结果表明，该模型在缺陷修复能力上明显优于现有开源方法。代码和数据集将公开发布。

Conclusion: HaineiFRDM成功探索了扩散模型在电影修复中的强大内容理解能力，通过创新的分块策略和频率模块设计，实现了高质量的高分辨率电影修复，为专家修复难以识别的电影缺陷提供了有效工具。
...
TL;DR: EchoFoley是一个新的视频声音生成任务，通过符号化表示实现细粒度控制，包括何时、什么、如何产生声音，并构建了包含6000个视频-指令-标注三元组的大规模基准数据集EchoFoley-6k。


<details>
  <summary>Details</summary>
Motivation: 当前视频-文本到音频（VT2A）方法存在三个关键限制：1）视觉和文本条件不平衡导致视觉主导；2）缺乏细粒度可控生成的具体定义；3）指令理解和跟随能力弱，因为现有数据集依赖简短的分类标签。

Method: 提出了EchoFoley任务，使用符号化表示来指定视频或指令中每个声音产生的时间、内容和方式，支持声音生成、插入和编辑等细粒度控制。构建了EchoFoley-6k基准数据集，并提出EchoVidia框架，采用慢-快思维策略的以声音事件为中心的智能生成方法。

Result: 实验表明，EchoVidia在可控性方面比最近的VT2A模型提高了40.7%，在感知质量方面提高了12.5%。

Conclusion: EchoFoley任务和EchoVidia框架解决了当前VT2A方法的局限性，通过符号化表示和分层语义控制实现了更精细的视频声音生成，显著提升了可控性和感知质量。
...
TL;DR: Splatwizard是一个专门用于评估3D高斯泼溅（3DGS）压缩模型的统一基准工具包，旨在解决当前缺乏标准化评估工具的问题。


<details>
  <summary>Details</summary>
Motivation: 随着3DGS技术的快速发展，现有评估工具缺乏针对压缩任务的标准化和全面评估指标，无法全面评估不同方法在渲染速度、率失真权衡、内存效率和几何精度等方面的独特特性。

Method: 开发了一个统一的基准工具包Splatwizard，提供易于使用的框架来实施新的3DGS压缩模型，并整合了自动化计算关键性能指标的管道，包括图像质量指标、重建网格的Chamfer距离、渲染帧率和计算资源消耗。

Result: Splatwizard提供了一个完整的评估框架，能够系统性地评估3DGS压缩模型的各项性能指标，填补了该领域标准化评估工具的空白。

Conclusion: Splatwizard为3DGS压缩模型的评估提供了标准化和全面的工具，有助于推动该领域的研究和发展，代码已在GitHub上开源。
...
TL;DR: 提出统一框架，将3D高斯溅射与可学习特征嵌入结合，通过"嵌入到标签"过程实现实例分割，并引入边界硬挖掘和线性层稳定训练，在多个数据集上超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D实例分割方法面临两个主要问题：一是多视角2D实例标签不一致导致3D预测质量差；二是现有方法多采用两阶段流程（如对比学习+聚类或标签预处理），训练时间长且性能受限。需要一种统一框架来同时解决标签一致性和训练效率问题。

Method: 1) 提出统一框架，在3D高斯原语中引入可学习特征嵌入进行分割；2) 设计"嵌入到标签"过程，将特征嵌入高效解码为实例标签；3) 针对边界伪影问题，提出边界硬挖掘策略；4) 在计算三元组损失前，对光栅化特征嵌入应用线性层以稳定训练。

Result: 在ScanNet、Replica3D和Messy-Rooms三个数据集上，该方法在定性和定量评估中均超越了基线方法，实现了更好的3D实例分割性能。

Conclusion: 提出的统一框架成功解决了多视角标签不一致问题，通过整合特征嵌入学习和标签解码过程，减少了训练时间并提升了分割性能。边界硬挖掘和线性层稳定化策略有效改善了边界质量，为3D场景理解提供了更优的解决方案。
...
### [70] [Nonlinear Noise2Noise for Efficient Monte Carlo Denoiser Training](https://arxiv.org/abs/2512.24794)
...
<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然无需干净目标图像，但存在一个主要限制：对噪声目标应用非线性函数会扭曲结果，因为非线性会使噪声目标的期望值与干净目标不同。由于非线性函数在图像处理中很常见，避免使用它们限制了可对噪声目标进行的预处理类型。

Method: 开发了一个理论框架来分析这些非线性的影响，描述了一类具有最小偏差的非线性函数。特别针对HDR图像去噪问题，研究了损失函数和色调映射函数的特定组合，这些组合可以减少异常值的影响同时引入最小偏差。

Result: 将该方法应用于现有的基于机器学习的蒙特卡洛去噪器，原始实现使用高样本数参考图像进行训练。结果显示，仅使用噪声训练数据就能获得接近原始实现的结果。

Conclusion: 某些非线性函数可以在不添加显著偏差的情况下应用于噪声目标，这扩展了Noise2Noise训练的应用范围，特别是在HDR图像去噪等需要非线性预处理的场景中。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在评估多模态大语言模型处理真实世界金融数值推理任务方面存在不足，需要更贴近实际金融场景、包含复杂文档结构和多步推理的评估工具。

Method: 构建包含1,200个专家标注问题的双语多模态基准测试，其中57.9%的问题融入12种隐含金融场景，使用837份中英文金融文档（平均50.8页），要求平均11步推理（5.3步信息提取+5.7步计算）。

Result: 表现最佳的多模态大语言模型仅达到58.0%的准确率，不同检索增强生成方法在该任务上表现出显著性能差异，65.0%的问题需要跨页面证据（平均2.4页）。
...
<details>
  <summary>Details</summary>
Motivation: 3D目标检测器在自动驾驶感知系统中至关重要，但在不同地理区域（如美国、亚洲、欧洲）之间存在显著的域泛化问题，模型在训练域外表现不佳。

Method: 基于神经元激活模式选择目标域中少量代表性且多样化的样本进行标注，结合受持续学习启发的后训练技术防止原始模型权重漂移。

Result: 该方法在域自适应任务中优于线性探测和现有SOTA域自适应技术，且只需极少的标注预算。

Conclusion: 通过神经元激活模式选择少量代表性样本进行标注，结合持续学习技术，可以有效解决3D目标检测器的域自适应问题，实现高效且高性能的跨域部署。
...
<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化评分是心血管疾病风险评估的重要工具，但非门控胸部CT存在严重运动伪影，影响钙化病灶的准确量化。虽然心电图门控心脏CT能减少伪影，但在人群筛查和常规成像中应用受限。因此需要开发能够从非门控CT中恢复无运动伪影钙化病灶的方法。

Method: 提出ProDM（属性感知渐进校正扩散模型），包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成具有多样运动轨迹的现实非门控采集数据；2）属性感知学习策略，通过可微钙化一致性损失融入钙化特异性先验；3）渐进校正方案，在扩散步骤中逐步减少伪影以增强稳定性和钙化保真度。

Result: 在真实患者数据集上的实验表明，ProDM相比多个基线方法显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。在真实非门控扫描上的读者研究进一步证实ProDM能抑制运动伪影并提高临床可用性。

Conclusion: ProDM展示了渐进、属性感知框架在从常规胸部CT成像中进行可靠CAC量化的潜力，为改善心血管风险评估提供了有效的技术解决方案。
...
TL;DR: VIPER：一个针对视频生成模型的过程感知评估基准，包含16个推理任务，提出过程-结果一致性指标(POC@r)，发现当前SOTA模型仅有约20%的POC@1.0分数，存在显著的结果作弊问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型的评估框架通常依赖单帧评估，容易导致结果作弊（模型通过错误过程得出正确结论）。需要建立过程感知的评估范式来准确衡量生成视频推理能力。

Method: 提出VIPER基准，涵盖时间、结构、符号、空间、物理和规划等16个推理任务。引入过程-结果一致性指标(POC@r)，使用VLM-as-Judge配合分层评分标准来评估中间步骤的有效性和最终结果。

Result: 实验显示最先进的视频模型仅达到约20%的POC@1.0分数，表现出显著的结果作弊现象。测试时缩放和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: VIPER基准为生成视频推理提供了过程感知的评估框架，揭示了当前模型的局限性，为未来研究指明了方向。基准将公开释放。
...
<details>
  <summary>Details</summary>
Motivation: 传统终身行人重识别方法需要为历史图库图像重新提取特征（重新索引），但这面临数据隐私问题和大规模图库的高成本。新旧模型提取的特征不兼容会严重影响重识别性能。

Method: 提出双向连续兼容表示框架（Bi-C2R），通过持续更新旧模型提取的图库特征，实现新旧模型特征之间的兼容性，无需重新索引历史图像。

Result: 通过理论分析和多个基准测试的广泛实验验证，Bi-C2R方法在提出的RFL-ReID任务和传统L-ReID任务上都取得了领先性能。

Conclusion: Bi-C2R框架有效解决了终身行人重识别中的特征兼容性问题，无需重新索引历史图库图像，在保护数据隐私的同时降低了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有音频驱动视觉配音方法面临缺乏理想训练数据的问题，现有方法采用基于掩码的修复范式，导致模型需要同时生成缺失内容和同步唇部，产生视觉伪影、身份漂移和同步不佳等问题

Method: 提出自引导框架：1) 使用Diffusion Transformer作为数据生成器，为每个真实样本合成唇部改变的伴生视频，形成视觉对齐的视频对；2) 基于DiT的音频驱动编辑器在这些对上端到端训练，利用完整对齐的输入视频帧专注于精确的音频驱动唇部修改；3) 引入时间步自适应多阶段学习策略，解耦跨扩散时间步的冲突编辑目标；4) 提出ContextDubBench基准数据集

Result: 该方法实现了高度准确的唇部同步、忠实的身份保持，并在具有挑战性的野外场景中表现出卓越的鲁棒性。多阶段学习策略促进了稳定训练，增强了唇部同步和视觉保真度

Conclusion: 通过将视觉配音重构为条件良好的视频编辑问题，并利用自引导框架生成理想训练数据，该方法在保持身份和场景交互的同时实现了精确的音频驱动唇部修改，为视觉配音任务提供了更有效的解决方案
...
TL;DR: FineTec是一个用于时间损坏骨架序列的细粒度动作识别框架，通过上下文感知补全、空间分解和物理驱动估计模块，在严重时间损坏情况下显著优于现有方法。
...
Method: 1) 上下文感知补全：通过多样化时间掩码从损坏输入恢复基础骨架序列；2) 骨架空间分解：将骨架分为五个语义区域，根据运动方差分为动态和静态子组，通过针对性扰动生成两个增强骨架序列；3) 物理驱动估计：利用拉格朗日动力学估计关节加速度；4) GCN识别头：融合骨架位置序列和加速度序列进行动作识别。

Result: 在粗粒度(NTU-60, NTU-120)和细粒度(Gym99, Gym288)基准测试中，FineTec在各种时间损坏水平下显著优于先前方法。在Gym99-severe和Gym288-severe挑战性设置中分别达到89.1%和78.1%的top-1准确率。
...
TL;DR: GaMO通过多视角外推而非生成新视角来解决稀疏视图重建问题，在保持几何一致性的同时扩展场景覆盖范围，实现25倍加速和SOTA重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏视图3D重建方法存在三个关键局限：已知视角外围覆盖不足、生成视角间几何不一致、计算流程昂贵。需要一种既能保持几何一致性又能高效扩展场景覆盖的方法。

Method: GaMO框架将稀疏视图重建重新定义为多视角外推问题，从现有相机位姿扩展视场而非生成新视角。采用多视角条件化和几何感知去噪策略，以零样本方式运行无需训练。

Result: 在Replica和ScanNet++数据集上，GaMO在3、6、9个输入视图下均达到SOTA重建质量，PSNR和LPIPS指标优于先前方法，同时比SOTA扩散方法快25倍，处理时间低于10分钟。

Conclusion: GaMO通过多视角外推方法有效解决了稀疏视图重建中的几何一致性和覆盖范围问题，在保持高质量重建的同时显著提升了计算效率，为稀疏视图3D重建提供了新思路。
...
TL;DR: CAT框架用于评估和可视化大语言模型在可控输入变化下的准确性与响应一致性之间的相互作用，特别针对多项选择题基准测试，提出了CAR曲线和CORE指数等量化工具。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要关注准确性和基准分数，但一致性对于高风险实际应用部署至关重要。作者认为虽然这两个维度需要独立评估，但它们之间的相互依赖性也需要考虑，以实现更细致的LLM评估。

Method: 提出CAT框架，核心是Consistency-Accuracy Relation (CAR)曲线，可视化模型准确性如何随一致性要求增加而变化；定义Minimum-Consistency Accuracy (MCA)度量；提出Consistency-Oriented Robustness Estimate (CORE)指数，结合CAR曲线的面积和形状来量化准确性与一致性之间的权衡。

Result: 在多个通用和专业领域LLM上，使用多项选择题基准测试进行了实际演示，展示了CAT框架的有效性。框架还可扩展到长文本、开放式评估任务。

Conclusion: CAT框架为LLM评估提供了更全面的视角，不仅考虑准确性和一致性，还关注它们之间的相互作用，有助于更细致地评估模型性能，特别是在高风险实际应用中。
...
TL;DR: BanglaCodeAct：基于多智能体提示和迭代自校正的孟加拉语到Python代码生成框架，在低资源语言代码生成任务中取得显著效果


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在英语到代码生成方面表现出色，但在低资源语言（如孟加拉语）方面进展有限，需要解决孟加拉语到Python代码生成的挑战

Method: 提出BanglaCodeAct框架，采用多智能体提示和迭代自校正机制，基于Thought-Code-Observation循环，利用开源多语言LLM动态生成、测试和优化代码

Result: Qwen3-8B模型在BanglaCodeAct框架下表现最佳，在开发集上达到94.0%的pass@1准确率，在盲测集上达到71.6%的准确率，创下孟加拉语到Python翻译的新基准

Conclusion: 该研究为低资源语言代码生成建立了新基准，证明了基于智能体的推理在可靠代码生成方面的潜力，特别是在资源有限的语言环境中
...
<details>
  <summary>Details</summary>
Motivation: 创建真实世界的中文药品运输单据数据集，用于在噪声OCR和异构模板条件下压力测试预训练的文本布局模型，为药品领域的安全关键文档理解建立可控、可复现的基准。

Method: 构建PharmaShip数据集，包含三个互补任务：序列实体识别(SER)、关系抽取(RE)和阅读顺序预测(ROP)。采用实体中心评估协议，基准测试五个代表性基线模型（LiLT、LayoutLMv3-base、GeoLayoutLM及其RORE增强变体），并标准化预处理、数据分割和优化过程。

Result: 实验表明像素和显式几何信息提供互补的归纳偏置，但单独使用都不够充分；注入阅读顺序导向的正则化能持续改进SER和EL，产生最鲁棒的配置；更长的位置覆盖能稳定后页预测并减少截断伪影；ROP在词级别准确但在片段级别具有挑战性，反映了边界模糊性和长距离交叉问题。

Conclusion: PharmaShip为药品领域的安全关键文档理解建立了可控、可复现的基准，并强调序列感知约束作为结构建模的可迁移偏置的重要性。数据集已在GitHub上发布。
...
Method: 引入Luca-Noise Reflex Protocol (LN-RP)计算框架，通过向初始生成状态注入随机噪声种子，在152个生成周期中观察语言行为的非线性转变。

Result: 发现了三种具有不同熵特征的稳定人格模式，证明外部噪声源能可靠诱导反射生成动态的相变，定量评估确认了人格保持的一致性以及模式间的显著差异(p < 0.01)。
...
TL;DR: HarmTransform是一个多智能体辩论框架，用于将有害查询系统地转化为更隐蔽的形式，以增强LLM的安全对齐能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的安全机制主要关注明显的有害内容，但用户可以通过隐蔽的改写方式保留恶意意图而看起来无害，这造成了现有安全训练数据的显著缺口。

Method: 引入HarmTransform多智能体辩论框架，通过多个智能体之间的迭代批判和精炼，系统地将有害查询转化为更隐蔽的形式，同时保留其潜在的有害意图。

Result: 实验表明HarmTransform在生成有效的查询转换方面显著优于标准基线方法。同时分析发现辩论具有双重作用：既能锐化转换并提高隐蔽性，也可能引入主题偏移和不必要的复杂性。

Conclusion: 多智能体辩论在生成全面安全训练数据方面既有前景也有局限性，需要平衡转换质量和复杂性，为未来LLM安全对齐提供改进方向。
...
TL;DR: LLMs在德州扑克中学习到了不完全信息环境下的世界模型，包括确定性的手牌排名和随机的胜率特征，无需显式指导。


<details>
  <summary>Details</summary>
Motivation: 扩展LLMs在完美信息游戏中展现的世界建模能力到不完全信息领域，以德州扑克作为典型的部分可观察马尔可夫决策过程来研究。

Method: 在扑克手牌历史数据上预训练GPT风格模型，使用非线性探针探测其内部激活，分析模型学习到的表示。

Result: 模型学习了确定性的手牌排名结构和随机的胜率特征，这些表示可通过非线性探针解码，并与理论信念状态相关。

Conclusion: LLMs能够在德州扑克这样的不完全信息环境中学习自己的随机环境表示，表明其具备在不完全信息领域构建世界模型的能力。
...
TL;DR: 提出一个两阶段框架，通过针对性训练和选择性推理来处理在线性别歧视内容的检测挑战，包括数据稀缺、噪声和概念模糊性。


<details>
  <summary>Details</summary>
Motivation: 在线性别歧视内容日益呈现微妙、依赖上下文的形式，传统检测方法难以捕捉。这些内容涉及语言、心理、法律和文化等多维度因素，导致标注数据中存在混合甚至矛盾的信号。加上标签稀缺和类别不平衡问题，使得模型决策边界不稳定，容易忽略更微妙、代表性不足的有害形式。

Method: 提出两阶段框架：1) 训练阶段使用类别平衡的焦点损失、类别感知批处理和事后阈值校准来处理标签不平衡和噪声监督；2) 推理阶段采用动态路由机制，高置信度案例直接分类，不确定实例则升级到新颖的"协作专家判断"模块，该模块提示多个人格并整合其推理。

Result: 在多个基准测试中取得最先进结果：EXIST 2025 Task 1.1上F1分数提升+2.72%，EDOS Tasks A和B分别获得+4.48%和+1.30%的增益。

Conclusion: 该框架通过统一针对性训练程序和选择性推理机制，有效解决了在线性别歧视检测中的数据稀缺、噪声和概念模糊性等核心挑战，显著提升了检测性能。
...
TL;DR: 提出EASD方法，在标准推测解码基础上加入动态熵惩罚机制，通过模型不确定性评估来拒绝低置信度预测，从而可能超越目标模型性能


<details>
  <summary>Details</summary>
Motivation: 传统推测解码方法中，草稿模型与目标模型过度对齐限制了性能提升，只能达到目标模型的性能上限。需要一种方法能够突破这一限制，在保持效率的同时提升推理质量。

Method: EASD在标准推测解码基础上引入动态熵惩罚机制：1) 计算采样分布的熵值来量化模型不确定性；2) 当两个模型都显示高熵且top-N预测有显著重叠时，拒绝对应token；3) 由目标LLM重新采样被拒绝的token；4) 通过草稿模型验证机制防止低置信度错误传播。

Result: 在多个推理基准测试中，EASD一致优于现有推测解码方法，并且在大多数情况下超越了目标LLM本身的性能。同时证明EASD的效率与标准推测解码相当。

Conclusion: EASD是一种无需训练的方法，通过动态熵惩罚机制有效提升推测解码性能，不仅优于现有方法，还能超越目标模型固有性能，同时保持高效推理。
...
TL;DR: MiMo-Audio通过大规模音频预训练（超1亿小时数据）实现了音频任务的少样本学习能力，在语音智能和音频理解任务上达到开源SOTA，并能泛化到未训练任务如语音转换、风格迁移等。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常需要针对特定任务进行微调，而人类能够通过少量示例或简单指令泛化到新音频任务。GPT-3已证明文本领域的扩展预训练能带来强大泛化能力，作者认为这一范式同样适用于音频领域。

Method: 1. 将MiMo-Audio的预训练数据扩展到超1亿小时音频；2. 开发系统评估框架；3. 在训练后阶段，构建多样化的指令调优语料库；4. 为音频理解和生成引入思考机制。

Result: MiMo-Audio-7B-Base在语音智能和音频理解基准测试中达到开源SOTA，能泛化到未训练任务（语音转换、风格迁移、语音编辑），并展示强大的语音延续能力。MiMo-Audio-7B-Instruct在音频理解、口语对话和指令TTS评估中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过大规模音频预训练，MiMo-Audio实现了类似GPT-3的少样本学习能力，在音频领域展示了强大的泛化性能，为通用音频智能系统的发展提供了重要进展。
...
TL;DR: 本文通过SHAP解释性分析比较两种基于Transformer的新闻偏见检测模型，发现不同架构在整合语言信号方面存在显著差异，领域自适应模型能减少63%误报，模型错误源于话语层面的模糊性而非显性偏见线索。


<details>
  <summary>Details</summary>
Motivation: 新闻文本的自动偏见检测被广泛用于支持新闻分析和媒体问责，但人们对偏见检测模型如何做出决策或为何失败知之甚少。本研究旨在通过可解释性分析来理解不同模型架构如何操作化语言偏见。

Method: 使用SHAP解释方法对两种基于Transformer的偏见检测模型进行对比研究：一种是在BABE数据集上微调的偏见检测器，另一种是在BABE数据集上微调的领域自适应预训练RoBERTa模型。分析正确和错误预测中的词级归因，以表征不同模型架构如何操作化语言偏见。

Result: 结果显示：1）两种模型都关注相似的评价语言类别，但在整合这些信号到预测中的方式上存在显著差异；2）偏见检测器模型对假阳性赋予比真阳性更强的内部证据，表明归因强度与预测正确性之间存在错位，导致系统性地过度标记中性新闻内容；3）领域自适应模型展现出与预测结果更好对齐的归因模式，产生63%更少的假阳性；4）模型错误源于不同语言机制，假阳性由话语层面的模糊性驱动而非显性偏见线索。

Conclusion: 研究强调了可解释性评估对偏见检测系统的重要性，表明架构和训练选择对模型可靠性和在新闻背景下的部署适用性具有关键影响。领域自适应方法能显著提高模型性能并减少误报。
...
TL;DR: 本文研究LLM在检索增强问答中的应用，提出自适应提示策略：将检索信息分块并顺序提示LLM，通过调整块大小平衡相关信息与无关信息，在减少token使用量的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLM上下文窗口扩展，检索增强生成中更广泛使用上下文。虽然更长的上下文便于融入目标知识，但也引入了更多无关信息，这会阻碍模型生成过程并降低性能。

Method: 设计自适应提示策略：将检索信息分割成较小块，顺序提示LLM使用每个块回答问题。通过调整块大小，在纳入相关信息和减少无关信息之间取得平衡。

Result: 在三个开放域问答数据集上的实验表明，自适应策略在使用更少token的情况下，性能与标准提示相当。分析发现，当遇到信息不足时，LLM常生成错误答案而非拒绝回答，这是主要错误来源。

Conclusion: 自适应提示策略能有效处理长上下文中的无关信息问题。研究强调需要进一步研究增强LLM在面对信息不足时有效拒绝请求的能力。
...
TL;DR: 利用注意力层token分布生成对抗样本，相比提示或梯度攻击更符合模型内部生成过程，在论证质量评估任务中导致性能下降，但某些层和位置的替换会引入语法退化


<details>
  <summary>Details</summary>
Motivation: 机制可解释性研究表明中间注意力层编码了token级假设并迭代优化，本文利用这一特性从注意力层token分布生成对抗样本，探索能否作为下游评估任务的有效对抗扰动
...
Conclusion: 中间层表示作为对抗样本来源具有前景但存在局限性，可用于压力测试基于LLM的评估流程，但需要解决语法退化问题以提高实用性
...
TL;DR: 该研究通过多检索器RAG系统结合领域知识检索，提升金融数值推理问答任务性能，发现领域特定训练和最新LLM在few-shot学习中的优势。


<details>
  <summary>Details</summary>
Motivation: 金融数值推理问答任务由于缺乏领域知识而存在错误，尽管LLM有进步，但金融数值问题仍具挑战性，需要特定金融领域知识和复杂多步数值推理。

Method: 实现多检索器RAG系统，检索外部领域知识和内部问题上下文，利用最新LLM处理任务，进行消融实验和错误分析。

Result: 领域特定训练（SecBERT编码器）使神经符号模型超越基线FinQA论文的顶级模型；基于提示的最佳LLM生成器达到SOTA性能（提升>7%），但仍低于人类专家水平；发现小模型幻觉损失与外部知识获取的权衡，大模型中外部事实收益通常超过幻觉损失。

Conclusion: 领域特定训练具有潜在优越性能，最新LLM增强了few-shot学习的数值推理能力，外部知识检索对金融数值推理任务至关重要，但仍需缩小与人类专家表现的差距。
...
TL;DR: 论文提出了一种分析开放式回答的框架，将学生内容信号与教师评分倾向分离，通过分析模型量化各因素贡献，帮助教师反思评分实践。


<details>
  <summary>Details</summary>
Motivation: 开放式回答是学习的核心，但自动评分常常混淆学生写的内容和教师的评分方式。需要一种能够分离内容信号和评分者倾向的分析框架，使评分判断变得可见和可审计。

Method: 使用ASSISTments数学回答数据，将教师评分历史建模为动态先验，从句子嵌入中提取文本表示，采用中心化和残差化处理来减少提示和教师混杂因素。使用时序验证的线性模型量化各信号贡献，并通过投影展示模型分歧供定性检查。

Result: 结果显示教师先验对成绩预测影响很大；先验与内容嵌入结合时效果最佳（AUC~0.815），而仅内容模型虽高于随机但明显较弱（AUC~0.626）。调整评分者效应能锐化残差内容表示，保留更多信息维度，揭示语义证据支持理解而非学生回答的表面差异。

Conclusion: 该框架将嵌入从简单特征转化为学习分析工具，使教师和研究者能够检查评分实践与学生推理和学习证据的一致性或冲突，为反思提供实用管道。
...
TL;DR: HGMem提出了一种基于超图的记忆机制，用于增强多步检索增强生成(RAG)系统，通过构建动态的高阶关联记忆结构来提升全局理解和复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统的记忆模块主要作为被动存储，仅积累孤立事实用于输入压缩和生成子查询，忽视了原始事实间的高阶关联。这种静态设计限制了表示能力和对多步推理的影响，导致推理碎片化和全局理解能力弱。

Method: 提出HGMem超图记忆机制，将记忆表示为超图，其中超边对应不同的记忆单元。这种机制能够逐步形成记忆中的高阶交互，围绕核心问题连接事实和思考，演变为集成的情境化知识结构，为后续步骤提供更强的推理命题。

Result: 在多个为全局理解设计的挑战性数据集上进行评估，广泛实验和深入分析表明，该方法持续改进多步RAG性能，在多样化任务上显著优于强基线系统。

Conclusion: HGMem将记忆概念从简单存储扩展到动态表达结构，通过超图表示实现复杂推理和全局理解，为多步RAG系统提供了更强大的记忆机制。
...
TL;DR: LoZA是一种稀疏注意力机制，可将现有全注意力模型转换为稀疏版本，在有限计算预算下显著提升长上下文处理速度，支持高达100万token的快速处理。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文场景中全注意力模型计算成本高的问题，特别是在预填充密集型（如检索增强生成）和解码密集型（如工具集成推理）任务中，需要更高效的注意力机制来降低计算开销。

Method: 提出LongCat ZigZag Attention (LoZA)稀疏注意力方案，通过zigzag模式减少注意力计算复杂度，将全注意力模型转换为稀疏版本，并在LongCat-Flash模型的中期训练中应用该技术。

Result: LoZA在长上下文场景中实现了显著的速度提升，LongCat-Flash-Exp作为长上下文基础模型能够快速处理高达100万token，支持高效长期推理和长视野智能体能力。

Conclusion: LoZA稀疏注意力机制为长上下文处理提供了有效的解决方案，在有限计算预算下实现了显著的性能提升，为大规模长文本理解和推理任务开辟了新途径。
...
TL;DR: CEC-Zero是一个零监督强化学习框架，通过让大语言模型自我纠错来解决中文拼写校正问题，无需人工标注，性能超越有监督基线方法。
...
Method: CEC-Zero从干净文本合成错误输入，通过语义相似度和候选一致性计算集群共识奖励，使用PPO优化策略，实现零监督强化学习。

Result: 在9个基准测试中，CEC-Zero比有监督基线方法高出10-13个F1分数，比强LLM微调方法高出5-8个F1分数，具有无偏奖励和收敛的理论保证。

Conclusion: CEC-Zero建立了无需标注的鲁棒、可扩展中文拼写校正范式，释放了大语言模型在噪声文本处理中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工定义的概念来分析LLM推理过程，但无法捕捉完整的推理行为谱系，许多行为难以在token空间中定义

Method: 将思维链轨迹分割为句子级"步骤"，在步骤级激活上训练稀疏自编码器，发现编码不同推理行为的方向

Result: 发现了可解释的行为向量（如反思、回溯），可视化显示这些行为在解码器列空间中占据可分离区域；通过干预SAE向量可控制推理行为；还能发现超越人工监督的新行为
...
TL;DR: WISE框架评估轻量级Transformer模型在区分假新闻与讽刺内容上的性能，发现MiniLM在准确率上表现最佳，而RoBERTa-base在ROC-AUC上最优，为资源受限环境下的虚假信息检测提供实用方案。


<details>
  <summary>Details</summary>
Motivation: 假新闻与讽刺/幽默内容具有相似的语言特征但意图不同，这给区分两者带来了独特挑战。研究旨在开发一个评估框架，比较轻量级模型在资源受限环境中检测虚假信息的性能。

Method: 开发WISE框架，在包含20,000个样本的平衡数据集（来自Fakeddit，标注为假新闻或讽刺）上评估8个轻量级Transformer模型和2个基线模型。使用分层5折交叉验证，评估包括准确率、精确率、召回率、F1分数、ROC-AUC、PR-AUC、MCC、Brier分数和预期校准误差在内的综合指标，并通过配对t检验和McNemar检验进行统计比较。

Result: MiniLM在所有模型中达到最高准确率（87.58%），RoBERTa-base获得最高ROC-AUC（95.42%）和强准确率（87.36%）。DistilBERT在效率与准确率之间提供了优秀平衡（86.28%准确率，93.90% ROC-AUC）。统计检验确认了模型间性能的显著差异，轻量级模型能够匹配或超越基线性能。

Conclusion: 轻量级Transformer模型在区分假新闻与讽刺内容方面表现出色，能够在资源受限的实际环境中有效部署虚假信息检测系统，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显式文本计划的方法面临两个主要挑战：1）大语言模型容易产生幻觉，生成不准确的计划；2）任务特定问题的高度多样性使得生成有效计划变得困难。受人类隐式认知启发，需要开发更紧凑、泛化的计划生成方法。

Method: iCLP框架包含三个步骤：1）从现有的逐步推理轨迹中蒸馏出显式计划；2）通过向量量化自编码器和码本学习这些计划的离散表示（潜在计划）；3）在大语言模型上对潜在计划和相应推理步骤进行微调，使模型学会在推理过程中进行隐式规划。

Result: 在数学推理和代码生成任务上的实验表明，iCLP使大语言模型能够在潜在空间中规划，同时在语言空间中推理，显著提高了准确性和效率。更重要的是，该方法表现出强大的跨领域泛化能力，同时保持了思维链推理的可解释性。

Conclusion: iCLP框架成功地将人类隐式认知原理应用于大语言模型规划，通过潜在计划实现了更高效、准确的推理，同时保持了可解释性，为解决显式计划生成的局限性提供了有效方案。
...
TL;DR: 该论文提出了一个统一的可靠性评估框架CRS，将校准、鲁棒性和不确定性量化整合为单一可解释指标，用于评估大型语言模型在关键决策领域的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗、法律、金融等关键决策领域应用日益广泛，但其可靠性存在不确定性。现有评估方法分散，只关注孤立方面，无法全面评估模型的可靠性。

Method: 提出复合可靠性评分（CRS）框架，将校准、鲁棒性和不确定性量化整合为单一可解释指标。在10个领先开源LLM和5个QA数据集上进行实验，评估基线性能、扰动影响和校准方法。

Result: CRS提供了稳定的模型排名，揭示了单一指标遗漏的隐藏故障模式，并表明最可靠的系统在准确性、鲁棒性和校准不确定性之间取得了平衡。

Conclusion: CRS框架为LLM可靠性评估提供了统一标准，有助于识别在关键决策应用中表现最可靠的模型，强调平衡准确性、鲁棒性和校准不确定性的重要性。
...
TL;DR: HY-MT1.5系列机器翻译模型通过多阶段训练框架，在参数效率上表现优异，1.8B模型超越多个大型开源模型和商业API，7B模型在其规模类别中达到新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 开发高性能、参数高效的机器翻译模型，通过整体训练框架提升翻译质量，同时支持高级约束功能如术语干预、上下文感知翻译和格式保留。

Method: 采用多阶段训练管道，整合通用和MT导向的预训练、监督微调、策略蒸馏和强化学习，构建整体训练框架。

Result: HY-MT1.5-1.8B在标准中-外和英-外任务中全面超越大型开源基线（如Tower-Plus-72B、Qwen3-32B）和主流商业API，达到Gemini-3.0-Pro约90%性能；HY-MT1.5-7B在其规模类别中达到新的SOTA，在Flores-200上达到Gemini-3.0-Pro 95%性能，在WMT25和普通话-少数民族语言测试集上超越Gemini-3.0-Pro。

Conclusion: HY-MT1.5系列模型在各自参数规模下为通用和专业翻译任务提供了高度竞争力和鲁棒的解决方案，支持高级约束功能，展示了参数效率和性能的优异平衡。
...
TL;DR: 本文提供了一个从零开始在AWS SageMaker上训练Hugging Face模型的完整指南，旨在降低云平台的学习门槛，帮助研究人员克服资源限制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展主要由资源丰富的研究机构和行业伙伴推动，许多研究人员因缺乏本地计算资源而转向AWS SageMaker等云服务，但云平台的陡峭学习曲线和分散的文档信息阻碍了他们的使用。

Method: 本文通过集中整理从零开始在AWS SageMaker上训练Hugging Face模型所需的关键信息，提供一个完整的演示指南，填补现有文档的知识空白。

Result: 通过提供系统化的指导文档，帮助研究人员成功在AWS SageMaker上训练他们的第一个Hugging Face模型，降低了云平台的使用门槛。

Conclusion: 本文通过集中化关键信息，有效促进了云计算的普及，使更多研究人员能够利用云资源训练复杂的语言模型，推动了研究民主化。
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过迭代去噪过程生成文本，具有并行解码优势，但在推理时控制和引导机制方面研究不足，需要有效的控制方法。

Method: 提出激活引导框架：1）使用对比示例通过单次前向传播计算层间引导向量；2）在反向扩散的每一步应用这些方向；3）无需模拟去噪轨迹，实现高效推理时控制。

Result: 在LLaDA-8B-Instruct模型上实验表明，该方法能可靠调节高层次属性，消融实验分析了引导在不同transformer子模块和token范围（提示vs.响应）上的效果。

Conclusion: 该激活引导框架为掩码扩散语言模型提供了有效的推理时控制机制，能够高效调节生成文本的属性，为MDLMs的控制和引导开辟了新方向。
...
TL;DR: 该论文提出了一个大型情感世界模型（LEWM），通过整合情感因素来增强世界模型对人类决策和社会行为的理解能力，相比传统仅关注物理规律的世界模型有更好的情感驱动行为预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型主要关注物理世界的规律建模，缺乏对情感因素的系统性探索。然而情感作为世界知识的重要组成部分，显著影响人类决策。论文通过实验证明移除情感相关信息会降低推理性能，因此需要构建能够系统建模情感的世界模型。

Method: 1. 提出大型情感世界模型（LEWM），基于心智理论启发；2. 构建情感-原因-方式（EWH）数据集，将情感整合到因果关系中，支持推理行为发生原因和情感如何驱动未来世界状态；3. LEWM显式建模情感状态、视觉观察和行动，使世界模型能够预测未来状态和情感转变。

Result: 实验结果表明：1. LEWM能更准确地预测情感驱动的社会行为；2. 在基础任务上保持与通用世界模型相当的性能；3. 验证了情感因素对理解世界的重要性。

Conclusion: 情感是世界知识的关键组成部分，整合情感因素的世界模型能更好地理解人类决策和社会行为。LEWM通过显式建模情感状态，在预测情感驱动行为方面表现优异，同时不损害基础任务的性能，为世界模型的发展提供了新的方向。
...
TL;DR: TeleChat3-MoE是一个基于混合专家架构的大语言模型系列，参数规模从1050亿到超过1万亿，在昇腾NPU集群上端到端训练完成。技术报告主要介绍了支持前沿模型规模可靠高效扩展的基础设施，包括数值精度验证、性能优化、并行化框架和集群级优化方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断扩大（从1050亿到超过1万亿参数），需要建立可靠高效的基础设施来支持前沿模型规模的训练。特别是在昇腾NPU硬件生态系统中，需要解决大规模训练中的数值精度、性能优化和并行化配置等挑战。

Method: 1. 系统化的数值精度验证方法：算子级和端到端验证，确保跨硬件平台和分布式并行策略的一致性
2. 性能优化套件：交错流水线调度、长序列训练的注意力感知数据调度、专家并行的分层重叠通信、基于DVM的算子融合
...
Conclusion: TeleChat3-MoE系列展示了在昇腾NPU集群上成功训练超大规模混合专家模型的技术能力。通过系统化的基础设施优化方法，实现了可靠高效的大规模模型训练，为前沿语言模型开发提供了可复现的技术框架。
...
TL;DR: MedKGI是一个基于临床实践的诊断框架，通过整合医学知识图谱、基于信息增益选择问题、采用OSCE格式结构化状态，解决了LLM在临床诊断中的幻觉、低效提问和对话一致性三大问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在临床诊断中存在三个关键局限：1) 由于缺乏验证知识基础而产生幻觉医学内容；2) 提出冗余或低效问题而非有助于诊断进展的区分性问题；3) 在多轮对话中失去连贯性，导致矛盾或不一致的结论。需要模拟真实临床场景中迭代、诊断假设驱动的推理过程。

Method: 提出MedKGI诊断框架：1) 整合医学知识图谱，将推理约束在验证过的医学本体中；2) 基于信息增益选择问题，最大化诊断效率；3) 采用OSCE格式结构化状态，在多轮对话中保持一致的证据追踪。

Result: 在临床基准测试中，MedKGI在诊断准确性和询问效率方面均优于强大的LLM基线，平均提高对话效率30%，同时保持最先进的准确性。

Conclusion: MedKGI通过结合医学知识图谱、高效问题选择和结构化状态管理，有效解决了当前LLM在临床诊断中的关键限制，实现了更接近真实临床实践的迭代诊断推理。
...
TL;DR: 本文介绍了LAILA——目前最大的公开阿拉伯语作文自动评分数据集，包含7,859篇作文，在七个维度上进行标注，填补了阿拉伯语AES研究的数据空白。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语自动作文评分研究受限的主要原因是缺乏公开可用的数据集。现有研究在这一领域存在明显的数据缺口，阻碍了阿拉伯语AES系统的开发与评估。

Method: 创建了LAILA数据集，包含7,859篇阿拉伯语作文，每篇作文都标注了整体分数和七个特质维度分数：相关性、组织结构、词汇、风格、发展、格式规范和语法。详细描述了数据集的设计、收集和标注过程，并使用最先进的阿拉伯语和英语模型在特定提示和跨提示设置下提供了基准结果。

Result: LAILA成为目前最大的公开阿拉伯语AES数据集，为阿拉伯语自动作文评分研究提供了重要资源。基准测试结果为后续研究提供了参考标准，展示了不同模型在阿拉伯语AES任务上的性能表现。

Conclusion: LAILA数据集填补了阿拉伯语自动作文评分研究的关键需求，支持开发更稳健的评分系统，为阿拉伯语教育技术领域的研究和应用提供了重要基础。
...
TL;DR: 开发了适用于专利和科学论文的语言相似度模型Pat-SPECTER，在预测可信专利-论文引用方面表现最佳，并验证了美国专利因诚信义务而引用语义相似度较低论文的假设。


<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够同时处理专利和科学论文的语言相似度模型，以更好地理解专利与学术文献之间的关系，并探索不同司法管辖区引用行为的差异。

Method: 基于SPECTER2模型，在专利数据上进行微调得到Pat-SPECTER模型；通过马赛式评估比较8种语言相似度模型在预测可信专利-论文引用方面的性能；在两种实际场景中测试模型能力；验证美国专利因诚信义务而引用语义相似度较低论文的假设。

Result: Pat-SPECTER模型在预测可信专利-论文引用方面表现最佳；成功应用于分离专利-论文对和预测专利-论文对的实际场景；验证了美国专利引用的论文语义相似度低于其他大型司法管辖区的假设。

Conclusion: Pat-SPECTER是处理专利和科学论文语言相似度的有效工具，为学术研究和实践应用提供了有价值的资源，同时揭示了不同司法管辖区专利引用行为的差异。
...
TL;DR: DATAMASK是一个用于大规模预训练数据选择的高效联合学习框架，能同时优化质量和多样性指标，相比贪心算法减少98.9%选择时间，在FineWeb数据集上选择10%子集使模型性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法在万亿规模预训练数据中存在局限性：基于质量指标的选择在长期预训练中收益递减，基于多样性指标的选择会移除过多高质量样本，两者都限制了预训练大语言模型的能力。由于计算成本高，这些指标很少在单一选择过程中联合考虑。

Method: DATAMASK将数据选择过程视为掩码学习问题，采用基于策略梯度的优化方法：迭代采样数据掩码，基于预定义目标和采样掩码计算策略梯度，更新掩码采样逻辑。通过策略梯度优化和各种加速增强，显著降低选择时间。

Result: DATAMASK相比贪心算法减少98.9%选择时间，使研究能在万亿规模token中探索联合学习。从15万亿token的FineWeb数据集中选择约10%子集（FineWeb-Mask），在12个多样化任务上评估，1.5B密集模型性能提升3.2%，7B MoE模型提升1.9%。

Conclusion: DATAMASK是一个高效的大规模预训练数据选择框架，能同时优化质量和多样性指标，显著减少选择时间并提升模型性能，为大规模预训练数据选择提供了有效解决方案。
...
TL;DR: FIGR通过强化学习将主动视觉思维整合到多轮推理中，在复杂推理问题中构建视觉表示来外化中间结构假设，显著提升数学推理性能。


<details>
  <summary>Details</summary>
Motivation: 复杂推理问题常涉及隐含的空间、几何和结构关系，这些关系在文本中未被明确编码。纯文本推理难以表示复杂场景中的全局结构约束，需要新的方法来解决这一问题。

Method: FIGR通过端到端强化学习将主动视觉思维整合到多轮推理中。系统在问题解决过程中构建视觉表示来外化中间结构假设，并自适应地调节何时以及如何调用视觉推理。

Result: 在具有挑战性的数学推理基准测试中，FIGR显著优于纯文本的思维链基线模型。具体来说，FIGR在AIME 2025上提升了13.12%，在BeyondAIME上提升了11.00%。

Conclusion: 图引导的多模态推理能有效增强复杂推理的稳定性和可靠性，视觉表示有助于捕捉纯文本难以表达的全局结构属性。
...
TL;DR: 提出一种基于人类略读策略的自监督对比学习框架，通过随机掩码文档片段并使用NLI对比目标来增强长文档表示，在法医和生物医学文本上取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在处理长文档（特别是法律和医学领域）时存在困难，稀疏注意力机制资源消耗大且难以捕获完整文档上下文，分层Transformer效率较高但无法清晰解释文档不同部分之间的关系。受人类略读策略启发，需要开发更高效的长文档表示方法。

Method: 提出自监督对比学习框架：1）随机掩码文档的一个片段；2）使用基于自然语言推理（NLI）的对比学习目标，将掩码片段与相关部分对齐，同时与不相关部分保持距离；3）模拟人类信息综合过程，生成更丰富且计算效率更高的文档表示。

Result: 在法律和生物医学文本上的实验验证了该方法在准确性和效率方面的显著提升。

Conclusion: 基于人类略读策略的自监督对比学习框架能够有效增强长文档表示，在保持计算效率的同时提高模型性能，特别适用于法律和医学等需要处理长文档的专业领域。
...
TL;DR: 本文比较了多种低资源语言文本摘要方法，包括零样本LLM提示、微调mT5模型、数据增强、多语言迁移和翻译管道方法，发现微调mT5基线在多数指标上优于其他方法。
...
Result: 1. 相似参数规模的LLM在性能上存在差异；2. 多语言微调的mT5基线在大多数指标上优于其他方法，包括零样本LLM；3. LLM作为评估器在低资源语言上可能可靠性较低。

Conclusion: 对于低资源语言文本摘要，微调多语言模型（如mT5）是有效的解决方案，而零样本LLM方法表现相对较差，且LLM评估在低资源语言上需要谨慎使用。
...
TL;DR: 该研究开发了一个用于雅思写作考试备考的智能修订平台，通过设计研究循环从规则方法转向基于DistilBERT的评分模型，实现了自适应反馈并显著提升了考生成绩。


<details>
  <summary>Details</summary>
Motivation: 传统雅思备考方法缺乏针对雅思写作评分标准的个性化反馈，无法满足考生需求。需要开发一个能够提供针对性反馈的智能平台来改善备考效果。

Method: 采用设计研究方法进行迭代开发：早期循环使用规则方法，第4循环采用DistilBERT变换器模型加回归头进行评分，第5循环实现自适应反馈。平台架构分离对话指导和写作界面以降低认知负荷。

Result: 规则方法存在中段压缩、低准确性和负R²值等限制。DistilBERT模型显著改善评分性能（MAE=0.66，正R²值）。自适应反馈使平均分数提升0.060个等级（p=0.011，Cohen's d=0.504），但效果因修订策略而异。

Conclusion: 自动化反馈功能最适合作为人工教学的补充，保守的表面级修正比激进的结构干预更可靠。未来需要针对高分作文评估进行改进，并开展纵向研究和官方考官验证。
...
TL;DR: 该研究填补了语音处理中段落分割任务的空白，建立了首个语音段落分割基准数据集，提出了保留原始转录的约束解码方法，并开发了高效的MiniSeg模型实现段落和章节的联合预测。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录通常生成无结构的单词流，这降低了可读性和再利用性。语音处理领域缺乏专门的段落分割任务，而文本分割领域也缺少鲁棒且自然的基准数据集。

Method: 1) 建立TEDPara（人工标注的TED演讲）和YTSegPara（带合成标签的YouTube视频）作为首个语音段落分割基准；2) 提出约束解码方法，让大语言模型插入段落分隔符同时保留原始转录；3) 开发紧凑的MiniSeg模型，通过层次化扩展实现章节和段落的联合预测。

Result: 研究建立了首个语音段落分割基准数据集，提出的MiniSeg模型在准确率上达到最先进水平，其层次化扩展版本能以最小计算成本同时预测章节和段落。
...
TL;DR: HaluNet是一个轻量级可训练的神经框架，通过整合多粒度token级不确定性来检测LLM幻觉，结合语义嵌入与概率置信度和分布不确定性，实现高效的单次幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在问答任务中表现出色，但经常产生幻觉（包括事实错误或捏造内容）。基于内部不确定性信号检测幻觉因其可扩展性和不依赖外部资源而具有吸引力。现有方法通常只关注单一类型的不确定性，而忽略了不同来源之间的互补性，特别是token级概率不确定性和内部语义表示传达的不确定性之间的互补关系。

Method: 提出HaluNet框架，整合多粒度token级不确定性，结合语义嵌入与概率置信度和分布不确定性。采用多分支架构，自适应地融合模型已知信息与其输出中表达的不确定性，实现高效的单次幻觉检测。

Result: 在SQuAD、TriviaQA和Natural Questions数据集上的实验表明，HaluNet在检测性能方面表现强劲，计算效率优越，无论是否访问上下文都能有效工作，突显了其在基于LLM的问答系统中实时幻觉检测的潜力。

Conclusion: HaluNet通过整合互补的不确定性来源，为LLM幻觉检测提供了一个轻量级、高效且可训练的解决方案，在保持计算效率的同时实现了强大的检测性能，适用于实时应用场景。
...
TL;DR: KCL是一个评估语言模型法律推理能力的基准，通过提供问题级支持判例来分离推理能力与领域知识，包含多项选择题和开放式问题两部分。


<details>
  <summary>Details</summary>
Motivation: 需要评估语言模型的法律推理能力，同时将推理能力与领域特定知识分离，以便更准确地衡量模型真正的推理能力。

Method: 创建韩国典型法律基准(KCL)，包含两个组件：KCL-MCQA（283个多项选择题，1,103个对齐判例）和KCL-Essay（169个开放式问题，550个对齐判例，2,739个实例级评分标准）。
...
TL;DR: CREST是一种无需训练的推理时认知引导方法，通过识别和干预特定注意力头来抑制低效推理行为，提高LLM推理准确率并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型依赖长链式思维推理解决复杂任务，但现有方法存在效率低下问题：生成过多token导致高延迟，或推理不稳定，在浅层推理（思考不足）和重复推理（过度思考）之间摇摆。

Method: CREST包含两个组件：1）离线校准步骤，识别与特定认知行为（如验证和回溯）相关的注意力头，并推导出头特定的引导向量；2）推理时程序，通过旋转隐藏表示来抑制这些向量方向上的成分，从而抑制低效推理行为。

Result: 在多种推理基准测试和模型上，CREST将准确率提升高达17.5%，同时减少37.6%的token使用量，实现了更高准确率和更低计算成本。

Conclusion: CREST提供了一种简单有效的途径，通过推理时认知引导实现更快、更可靠的大型语言模型推理，无需额外训练即可显著改善推理效率和稳定性。
...
TL;DR: Youtu-LLM是一个1.96B参数的轻量级语言模型，通过从零预训练而非蒸馏，在紧凑架构下实现了原生智能体能力，支持128k上下文，在STEM和智能体任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型通常依赖蒸馏技术，缺乏原生推理和规划能力。研究目标是开发一个轻量级但具备强大智能体能力的模型，能够在有限计算资源下执行复杂的长期规划和推理任务。

Method: 1. 采用密集多潜在注意力架构和STEM导向词表，支持128k上下文窗口；2. 设计"常识-STEM-智能体"三阶段课程学习策略，使用约11T token进行多阶段训练；3. 针对智能体任务采用可扩展的中期训练，合成数学、编程和工具使用领域的多样化轨迹数据。

Result: Youtu-LLM在2B以下模型中达到新的SOTA水平。在通用基准测试中与更大模型竞争，在智能体特定任务上显著超越现有基线，证明轻量级模型可以具备强大的内在智能体能力。

Conclusion: 通过从零预训练、精心设计的架构和课程学习策略，轻量级语言模型可以具备强大的原生智能体能力，为资源受限环境下的复杂推理和规划任务提供了有效解决方案。
...
TL;DR: MUSIC是一种无监督数据增强策略，通过合成多轮对比对话对来训练更强大的多轮奖励模型，在保持单轮评估性能的同时显著提升多轮对话评估质量。


<details>
  <summary>Details</summary>
Motivation: 评估多轮对话质量对LLM发展至关重要，但现有方法依赖昂贵的人工评估。标准偏好数据集通常只基于最终轮次进行对比，无法捕捉多轮交互的细微差别，导致多轮奖励模型训练信号不足。

Method: 提出MUSIC（多步指令对比）无监督数据增强策略，通过合成在多轮对话中展现差异的对比对话对。基于Skywork偏好数据集，使用Gemma-2-9B-Instruct模型训练多轮奖励模型。

Result: MUSIC增强的奖励模型在基准方法中表现最佳，在多轮对话评估中与先进专有LLM评判者的判断更一致，且不影响标准单轮奖励模型的基准性能。

Conclusion: 通过多轮对比训练信号能够显著提升多轮奖励模型的质量，MUSIC策略为构建更强大的多轮对话评估系统提供了有效解决方案，同时保持单轮评估能力。
...
TL;DR: BIOME-Bench：用于评估LLMs在多组学分析中生物分子相互作用推断和通路机制阐明能力的标准化基准


<details>
  <summary>Details</summary>
Motivation: 当前多组学研究依赖通路富集分析，但存在通路资源的结构性限制（如更新滞后、功能冗余、对分子状态和干预敏感性有限）。虽然已有研究探索使用LLMs改进通路富集分析，但缺乏标准化的端到端多组学通路机制阐明基准，限制了可重复性进展。

Method: 通过严格的四阶段工作流程构建BIOME-Bench基准，评估LLMs在多组学分析中的两个核心能力：生物分子相互作用推断和端到端多组学通路机制阐明。开发了两种任务的评估协议，并在多个当代强模型上进行综合实验。

Result: 实验结果表明，现有模型在多组学分析中仍存在显著不足，难以可靠地区分细粒度的生物分子关系类型，也无法生成忠实、稳健的通路级机制解释。

Conclusion: BIOME-Bench为评估和改进LLMs在多组学分析中的能力提供了标准化基准，揭示了当前模型在生物分子关系理解和通路机制阐明方面的局限性，为未来研究指明了方向。
...
TL;DR: 该研究对大型语言模型在推理任务中的计算效率进行了评估，发现混合专家架构在性能与效率间达到最佳平衡，并识别出推理计算存在饱和点。


<details>
  <summary>Details</summary>
Motivation: 当前研究往往忽视生成长推理序列带来的显著计算负担。工业应用中的模型选择不仅取决于原始准确性，还受到资源约束和推理成本的影响。

Method: 对当代和早期开源LLMs进行测试时计算感知评估，在数学和推理密集型基准测试中绘制帕累托前沿，追踪帕累托效率随时间的变化轨迹。

Result: 混合专家架构在评估设置中表现出性能与效率的良好平衡；识别出推理计算存在饱和点，超过特定阈值后准确性增益会减少。
...
TL;DR: 该论文介绍了如何将伦理考量融入NLP教育，通过互动式教学、实践活动和"教学相长"方法，开发了可跨机构、跨教育层次使用的课程体系。


<details>
  <summary>Details</summary>
Motivation: 随着NLP系统日益普及，将伦理考量融入NLP教育变得至关重要。然而，课程开发面临两大挑战：领域在学术界和工业界的快速发展，以及需要培养超越传统技术培训的批判性思维。

Method: 采用基于主动学习的教学方法，包括互动式会议、实践活动和"教学相长"方法。课程经过四年时间，在不同机构、教育层次和跨学科背景下不断优化调整。

Result: 课程产生了许多可重复使用的产品，包括教学材料和面向不同受众的实际教育产品（由学生自己制作）。课程已在多个机构和教育层次成功实施。

Conclusion: 通过分享这一教学方法和经验，希望为寻求将社会影响考量融入课程的教育工作者提供启发，促进NLP教育的伦理维度发展。
...
TL;DR: 提出因果三角验证方法，通过跨语言参考家族测试来验证多语言模型机制解释的可靠性


<details>
  <summary>Details</summary>
Motivation: 多语言模型在总体表现上很强，但在不同语言、文字和文化中的行为难以预测，现有机制解释缺乏因果验证标准

Method: 提出参考家族概念（保持谓词意义的变体）和三角验证方法：必要性（消融电路会降低目标行为）、充分性（修补激活能转移行为）、不变性（在参考家族中效果保持稳定）

Result: 三角验证为机制解释提供了可证伪标准，能过滤掉通过单环境测试但在跨语言不变性测试中失败的虚假电路

Conclusion: 通过因果抽象框架将三角验证形式化为近似变换分数，为多语言模型机制解释提供了严格的因果验证标准
...
TL;DR: 论文批判当前AI发展加剧生态、意义和语言危机，呼吁语言工程师转向以人类繁荣和生态健康为中心的新范式


<details>
  <summary>Details</summary>
Motivation: 当前世界面临生态、意义和语言三重危机，这些危机正在汇聚成元危机。大型AI技术正在加速所有这些危机，而语言工程师们继续坚持不可持续的可扩展性叙事，为富豪和腐败政权提供关键人才，仿佛整个技术开发过程是价值中立的

Method: 提出需要探索替代方案，应用集体智慧设计一种肯定生命、以人类在活生生的星球上繁荣为中心的NLP未来

Result: 论文没有提供具体实验结果，而是提出了一个批判性分析和范式转变的呼吁

Conclusion: 迫切需要语言工程师重新思考其角色，从当前加速危机的模式转向以人类繁荣和生态健康为中心的生命肯定型NLP范式
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要存在三个根本性限制：1）容易受到数据污染的影响；2）仅限于单知识点评估；3）依赖昂贵的领域专家标注。这些限制阻碍了准确跟踪大型语言模型能力边界的发展。

Method: 从权威教科书中提取独立的知识陈述作为基准构建的基本单元，在测试时通过随机抽样动态组合这些陈述形成评估问题。每个问题聚合8-10个陈述，实现多知识点的综合评估。标注者只需验证格式合规性，无需领域专业知识。

Result: 在超过50个LLM上的实验表明，Encyclo-K具有强大的区分能力：即使表现最好的OpenAI-GPT-5.1准确率也只有62.07%。推理模型准确率分布在16.04%到62.07%之间，聊天模型分布在9.71%到50.40%之间，显示出清晰的梯度分布。

Conclusion: Encyclo-K通过动态评估和多陈述综合理解引入了显著挑战，验证了其作为评估LLM对多个细粒度学科知识陈述综合理解能力的可扩展动态评估框架的有效性。
...
TL;DR: mHC框架通过将超连接的残差连接空间投影到特定流形上，恢复身份映射特性，解决了HC训练不稳定和可扩展性受限的问题。


<details>
  <summary>Details</summary>
Motivation: 超连接（HC）通过扩展残差流宽度和多样化连接模式提升了性能，但破坏了残差连接固有的身份映射特性，导致训练不稳定、可扩展性受限，并产生显著内存访问开销。

Method: 提出流形约束超连接（mHC）框架，将HC的残差连接空间投影到特定流形上以恢复身份映射特性，同时结合严格的基础设施优化确保效率。
...
TL;DR: ADOPT是一个用于多步LLM管道的自适应依赖感知提示优化框架，通过显式建模LLM步骤与最终任务结果之间的依赖关系，实现精确的文本梯度估计，将多提示优化简化为灵活的单提示优化步骤。


<details>
  <summary>Details</summary>
Motivation: 多步LLM管道能有效解决复杂任务，但其性能严重依赖每一步使用的提示。联合优化这些提示很困难，因为缺乏步骤级监督和步骤间依赖关系，现有端到端提示优化方法在这些条件下效果不佳。
...
Conclusion: ADOPT通过显式建模步骤依赖关系和实现精确梯度估计，成功解决了多步LLM管道中提示优化的挑战，为复杂任务的多步LLM管道提供了有效的优化框架。
...
<details>
  <summary>Details</summary>
Motivation: 法律文档分类面临两大挑战：专业词汇和文档长度。传统Transformer模型处理完整长文档可能不可行、昂贵或缓慢，因此需要一种能够有效处理长法律文档的分类方法。

Method: 提出基于DeBERTa V3和LSTM的混合模型，输入为48个随机选择的短文本块（每个最多128个token）。同时使用Temporal作为持久化执行解决方案，部署了可靠的处理流程。

Result: 最佳模型的加权F分数达到0.898。在CPU上运行的流程处理100个文件的平均时间为498秒，显示了良好的分类性能和可接受的处理效率。

Conclusion: 该方法通过使用短文本块采样策略有效解决了长法律文档的分类问题，结合DeBERTa V3和LSTM的混合架构以及Temporal的部署方案，实现了可靠且高效的法律文档分类系统。
...
<details>
  <summary>Details</summary>
Motivation: 表情包已从单纯的幽默表达演变为用户自由表达各种情绪（包括抑郁情绪）的媒介，随着表情包在表达抑郁情绪中的使用日益增长，需要研究如何识别社交媒体表情包中表现的抑郁症状

Method: 提出RESTOREx资源库（包含LLM生成和人工标注的解释）和MAMAMemeia框架（基于认知分析疗法能力的多智能体多方面讨论框架），用于检测社交媒体表情包中的抑郁症状

Result: MAMAMemeia框架相比30多种现有方法，在macro-F1指标上提升了7.55%，成为新的基准方法

Conclusion: 该研究成功开发了用于检测社交媒体表情包中抑郁症状的有效框架和资源，为心理健康监测提供了新的技术手段
...
TL;DR: 本文提出Thought Gestalt模型，通过双层抽象（token和句子级"思想"状态）解决Transformer在实体事件表示上的不足，提升数据效率和关系方向泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer语言模型主要依赖表层共现统计，缺乏对实体和事件的全局一致潜在表示，导致关系方向脆弱（如反转诅咒）、上下文错误和数据效率低下。受认知科学启发，人类理解语言时会将其转换为紧凑的事件式表示，而逐字形式短暂存在。

Method: 提出Thought Gestalt模型，这是一个循环Transformer，在token和句子级"思想"状态两个抽象层次上建模语言。模型逐句生成token，同时交叉关注先前句子表示的记忆。token和句子表示使用相同模型参数生成，通过单一目标（下一个token交叉熵）训练，通过保留写入记忆的句子表示计算图，使未来token损失的梯度通过交叉注意力反向传播优化早期句子向量生成参数。

Result: 在扩展实验中，TG相比匹配的GPT-2运行始终提高效率，扩展拟合表明GPT-2需要约5-8%更多数据和33-42%更多参数才能匹配TG的损失。TG还在父子反转诅咒探针上减少了关系方向泛化错误。

Conclusion: Thought Gestalt模型通过引入句子级"思想"状态的双层抽象表示，有效解决了传统Transformer在实体事件表示上的局限性，提高了数据效率和关系方向泛化能力，为语言建模提供了更符合人类认知的架构。
...
TL;DR: AdaGReS是一个用于RAG的冗余感知上下文选择框架，通过结合查询-块相关性和集合内冗余惩罚的集合级目标，在token预算约束下进行贪婪选择，无需手动调参即可自适应校准相关性与冗余性权衡。


<details>
  <summary>Details</summary>
Motivation: 传统的top-k检索方法在RAG中经常返回冗余或近似重复的文本块，这会浪费token预算并降低下游生成质量。需要一种能够有效控制冗余、优化上下文选择的解决方案。

Method: 提出AdaGReS框架：1) 定义结合查询-块相关性和集合内冗余惩罚的集合级目标函数；2) 在token预算约束下使用边际增益进行贪婪选择；3) 引入封闭式、实例自适应的校准方法来自动调整相关性与冗余性权衡参数，无需手动调参；4) 提供理论分析证明目标函数在实用嵌入相似性条件下具有epsilon-近似子模性。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量方面持续改进，转化为更好的端到端答案质量和跨设置的鲁棒性。

Conclusion: AdaGReS通过冗余感知的上下文选择框架有效解决了RAG中的冗余问题，通过理论保证的自适应校准方法实现了无需手动调参的优化选择，显著提升了RAG系统的性能。
...
TL;DR: 该研究提出了DDFT评估框架，用于衡量语言模型在语义压缩和对抗性攻击下的知识鲁棒性，发现模型鲁棒性与参数规模无关，而主要取决于错误检测能力。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估（如MMLU、TruthfulQA）只能测量理想条件下的知识水平，无法评估模型在现实压力下的知识鲁棒性。需要一种能够区分模型是否真正掌握知识，还是其验证机制在信息退化或对抗性攻击下会崩溃的评估方法。

Method: 提出了Drill-Down and Fabricate Test (DDFT)协议，通过渐进语义压缩和对抗性伪造来测量认知鲁棒性。采用双系统认知模型：语义系统生成流畅文本，认知验证器验证事实准确性。评估了9个前沿模型在8个知识领域和5个压缩水平下的表现（共1800次轮级评估）。

Result: 研究发现认知鲁棒性与传统设计范式正交：参数数量（r=0.083, p=0.832）和架构类型（r=0.153, p=0.695）均不能显著预测鲁棒性。错误检测能力强烈预测整体鲁棒性（rho=-0.817, p=0.007），是关键瓶颈。旗舰模型尽管规模大但表现脆弱，而小型模型也能实现鲁棒性能。
...
TL;DR: CASCADE是一个自演化的LLM智能体框架，通过持续学习和自我反思等元技能，使智能体能够掌握复杂外部工具并积累可执行技能，在科学任务中实现从"LLM+工具使用"到"LLM+技能获取"的转变。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体依赖于预定义工具或脆弱的工具生成，限制了其在复杂科学任务中的能力和适应性。需要一种能够自主学习和适应复杂工具的框架。

Method: CASCADE框架通过两种元技能实现自我演化：1) 持续学习（通过网页搜索和代码提取）；2) 自我反思（通过内省和知识图谱探索）。该框架支持人类-智能体协作和记忆整合，积累可共享的可执行技能。

Result: 在SciSkillBench基准测试（116个材料科学和化学研究任务）上，使用GPT-5的CASCADE实现了93.3%的成功率，而没有演化机制的基线仅为35.4%。框架在计算分析、自主实验室实验和已发表论文的选择性复制等实际应用中表现出色。

Conclusion: CASCADE通过积累可跨智能体和科学家共享的可执行技能，推动了可扩展的AI辅助科学研究，代表了从"LLM+工具使用"向"LLM+技能获取"转变的早期实例。
...
Method: McCoy框架结合大型语言模型（LLMs）与答案集编程（ASP），使用LLM将医学文献翻译成ASP代码，结合患者数据，通过ASP求解器处理得出最终诊断。

Result: 初步结果显示McCoy在小规模疾病诊断任务上表现出色，提供了一个强大且可解释的预测框架。

Conclusion: McCoy框架成功结合了LLMs和ASP的优势，克服了传统符号AI在医疗领域应用的知识库构建障碍，为疾病诊断提供了可解释的混合智能解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统搜索系统受限于静态用户画像和单一检索流程，难以捕捉用户动态、多维的信息需求变化。需要一种能够建模用户演化需求、支持情境敏感检索的新方法。

Method: SPARK框架构建基于角色、专业知识、任务上下文和领域的角色空间，通过角色协调器动态激活相关专业智能体。每个智能体执行独立的检索增强生成过程，配备长短期记忆存储和情境感知推理模块。智能体间通过结构化通信协议（共享内存库、迭代辩论、接力式知识传递）进行协作。

Result: 框架产生了关于协调效率、个性化质量和认知负载分布的可测试预测，同时包含自适应学习机制用于持续角色优化。该框架展示了如何通过最小协调规则下的分布式智能体行为产生涌现个性化特性。
...
TL;DR: LoongFlow是一个自我进化的智能体框架，通过将LLM集成到"计划-执行-总结"认知范式中，解决了传统进化方法在代码空间中的过早收敛和探索效率问题，显著提升了进化效率并降低了计算成本。
...
Method: LoongFlow将LLM集成到认知"计划-执行-总结"范式中，将进化搜索映射为推理密集型过程。采用混合进化记忆系统，结合多岛模型、MAP-Elites和自适应玻尔兹曼选择，平衡探索-利用权衡，维持多样行为生态位。

Result: 在AlphaEvolve基准测试和Kaggle竞赛中，LoongFlow比领先基线（如OpenEvolve、ShinkaEvolve）的进化效率高出60%，同时发现更优解决方案。

Conclusion: LoongFlow在自主科学发现方面迈出了重要一步，能够以更低的计算开销生成专家级解决方案，实现了从静态LLM到自我进化智能体的有效过渡。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的LLM在ARC-AGI-3基准测试中表现不佳，该测试包含需要交互推理的游戏式任务，要求智能体通过有限交互推断任务机制并适应复杂度递增的关卡。需要一种能够系统探索状态空间的方法来解决这类稀疏反馈环境中的推理问题。

Method: 结合视觉帧处理和基于图的系统化状态空间探索：1）将视觉帧分割为有意义组件；2）基于视觉显著性优先选择动作；3）维护探索状态和转移的有向图；4）通过跟踪已访问状态和已测试动作，优先选择到达未测试状态-动作对的最短路径动作。

Result: 在ARC-AGI-3预览挑战中，该方法解决了52个关卡中位数为30个，在私有排行榜上排名第3，显著优于前沿的基于LLM的智能体。在六个游戏中都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法（如Safe RLHF和SACPO）通常采用风险中性范式，无法充分应对参考策略偏差带来的风险，且对罕见但可能灾难性的有害行为缺乏鲁棒性。需要一种能够显式纳入风险意识的方法来增强语言模型的安全性和可信度。

Method: 提出风险感知逐步对齐（RSA）方法，将安全对齐建模为token级别的风险感知约束策略优化问题，利用嵌套风险度量类，通过逐步对齐过程推导出token级别的策略更新。该方法包含两个关键设计：1）缓解因偏离参考策略导致的过度模型偏移风险；2）显式抑制低概率高影响的有害行为。

Result: 实验结果表明，该方法在保持高帮助性的同时确保了强大的安全性，并显著抑制了尾部风险（即低概率但高影响的不安全响应）。理论分析在温和假设下证明了策略的最优性。

Conclusion: RSA方法通过显式纳入风险意识，有效解决了现有安全对齐方法的局限性，为语言模型的安全对齐提供了更稳健的解决方案，能够在保持有用性的同时更好地控制风险。
...
<details>
  <summary>Details</summary>
Motivation: AI领域长期面临开发能够解决广泛物理任务并能泛化到新任务和环境的智能体挑战。虽然基于世界模型和规划的方法已有进展，但在表示空间进行规划的方法（JEPA-WMs）缺乏系统性研究，需要明确其关键技术选择如何影响规划效果。

Method: 提出JEPA-WMs框架，系统研究模型架构、训练目标和规划算法三个关键组件。使用模拟环境和真实机器人数据进行实验，分析不同技术选择对规划成功率的影响。

Result: 通过实验确定了最优的JEPA-WMs配置组合，提出的模型在导航和操作任务上超越了DINO-WM和V-JEPA-2-AC两个基线方法。代码、数据和检查点已开源。

Conclusion: JEPA-WMs框架通过表示空间规划能够有效提升任务解决效率，系统性的组件分析为这类方法提供了优化指导，最终模型在多种任务上表现出优越性能。
...
TL;DR: 该研究分析了三种主流大语言模型（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）在密苏里大学数学竞赛问题上的表现，发现DeepSeek-V3在所有三个数学领域表现最佳，但所有模型在几何问题上表现都较弱。


<details>
  <summary>Details</summary>
Motivation: 当前大多数研究使用相同的数据集评估大语言模型的数学推理能力，这限制了研究结果的普适性，可能无法完全捕捉数学任务中的多样化挑战。本研究旨在通过分析大语言模型在代表性不足的数学竞赛问题上的表现，更全面地评估其数学推理能力。

Method: 研究使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题，对三种主流大语言模型（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）进行测试。将模型的回答与已知正确答案进行比较，确定每个问题领域的准确性，并分析模型的推理过程以探索错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中表现最佳，无论是推理过程还是最终答案正确率都最高。所有三种大语言模型在几何问题上表现明显较弱。DeepSeek-V3的主要错误是计算和逻辑错误，GPT-4o-mini经常出现逻辑和方法相关的错误，而Gemini则倾向于不完整的推理和草率得出结论。

Conclusion: 在代表性不足的数学竞赛数据集上评估大语言模型可以提供对其独特错误模式的更深入洞察，并突显在结构化推理方面持续存在的挑战，特别是在几何领域。这表明需要更多样化的评估数据集来全面了解大语言模型的数学推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在通用语言能力上表现出色，但在结构化环境中的空间变换和多步规划方面仍然存在困难。需要一种能够有效处理空间推理任务的方法。

Method: 采用两阶段方法：1) 对基本空间变换（旋转、平移、缩放）进行监督微调，使模型具备基本空间物理知识；2) 冻结物理感知模型，在GRPO框架内训练轻量级LoRA适配器，学习将这些构建块组合用于谜题环境中的多步规划。为此合成了ASCII艺术数据集并构建了相应的强化学习环境。

Result: 该方法在动态环境（有明确状态更新）和静态环境（模型必须依赖内部状态）中均优于基线方法，包括通用骨干模型、物理感知模型和端到端RL模型。此外，该方法收敛更快，训练更稳定，注意力模式分析显示微调确实改善了空间理解能力。

Conclusion: 提出的两阶段分解方法能有效提升LLMs在空间推理任务上的表现，通过将复杂任务分解为原子操作并组合，实现了更好的规划能力和训练稳定性，为导航和规划应用提供了有前景的解决方案。
...
TL;DR: RLMs是一种推理策略，让LLMs通过编程方式检查、分解和递归调用自己来处理超长提示，突破上下文窗口限制


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型处理超长提示的问题，突破固定上下文窗口的限制，提高处理长文本的能力

Method: 提出递归语言模型（RLMs），将长提示视为外部环境，让LLM以编程方式检查、分解文本，并递归调用自身处理文本片段

Result: RLMs能处理比模型上下文窗口长两个数量级的输入，在四个不同的长上下文任务中显著优于基础LLM和常见长上下文框架，且查询成本相当或更低

Conclusion: RLMs为LLMs处理任意长度提示提供了有效的推理时扩展方法，在保持成本效益的同时显著提升长文本处理质量
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在单任务上表现良好，但在多智能体协作场景中缺乏全局优化能力，难以实现高效的协同工作

Method: 采用去中心化部分可观测马尔可夫决策过程建模协作，使用集中训练分散执行策略，引入组相对策略优化和简化的联合奖励函数

Result: 在协作写作和编码基准测试中，任务处理速度比单智能体基线提升3倍，写作结构/风格一致性达98.7%，编码测试通过率74.6%
...
TL;DR: 提出一种多模态跨域混合融合模型，通过双重解耦框架分离模态不变/特定特征和域不变/特定表示，结合跨域混合融合策略和三模态融合机制，提升电机故障诊断在未见工况下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有故障诊断方法在真实场景中面对未见工况时性能显著下降，域适应方法依赖目标域样本，且多数研究依赖单模态信号，忽略了多模态信息的互补性对模型泛化的提升作用。

Method: 提出多模态跨域混合融合模型，包含：1）双重解耦框架，分离模态不变/特定特征和域不变/特定表示；2）跨域混合融合策略，随机混合跨域模态信息增强多样性；3）三模态融合机制，自适应整合多模态异构信息。

Result: 在感应电机故障诊断实验中，无论是恒定还是时变未见工况下，该方法均持续优于先进方法，综合消融研究进一步验证了各组件和多模态融合的有效性。

Conclusion: 该方法通过双重解耦和跨域混合融合，有效提升了多模态故障诊断在未见工况下的泛化性能，为实际工业应用提供了更可靠的解决方案。
...
TL;DR: 本文提出BatteryAgent框架，结合物理特征与大语言模型推理能力，实现锂离子电池故障的智能诊断，从被动检测转向可解释的多类型诊断。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然检测精度高，但存在"黑箱"问题缺乏可解释性，且受限于二分类范式，难以提供根因分析和维护建议。

Method: 提出三层框架：1)物理感知层提取10个基于电化学原理的特征；2)检测归因层使用梯度提升决策树和SHAP量化特征贡献；3)推理诊断层利用LLM作为核心，构建"数值-语义"桥梁，结合SHAP归因和机理知识库生成综合诊断报告。

Result: 实验结果显示，BatteryAgent有效纠正硬边界样本的误分类，AUROC达到0.986，显著优于当前最先进方法，实现了从二分类检测到多类型可解释诊断的扩展。

Conclusion: 该框架为电池安全管理提供了从"被动检测"到"智能诊断"的新范式转变，结合物理知识与AI推理能力，实现了可解释的故障诊断系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有的机器人家庭物品重排系统通常依赖从人类演示中推断的潜在偏好模型，这些模型虽然能有效预测，但无法提供可解释的因素来解释人类决策背后的原因。

Method: 1. 提出了四个可解释的偏好构建维度：空间实用性、习惯便利性、语义连贯性和常识适当性；2. 设计并验证了自报告问卷，通过63名参与者的在线研究确认了这些维度的心理区分度；3. 将这些偏好维度整合到蒙特卡洛树搜索规划器中，用于生成物品摆放方案。

Result: 1. 问卷研究证实了四个偏好维度的心理区分性和解释力；2. 当规划器使用参与者衍生的偏好指导时，能够生成与参与者实际摆放方案高度一致的合理物品摆放方案。

Conclusion: 该工作贡献了一个紧凑、可解释的物品摆放偏好模型，并展示了如何将其操作化用于机器人规划，为理解人类物品摆放决策提供了新的理论框架。
...
TL;DR: GenZ是一种混合模型，通过可解释的语义特征桥接基础模型和统计建模，解决了基础模型难以捕捉数据集特定模式的问题，在房价预测和电影推荐任务中显著优于纯基础模型方法。
...
Method: 提出一种广义EM算法，通过迭代过程发现语义特征描述：基于统计建模误差对比项目组，而非仅依赖基础模型的领域知识。方法使用冻结的基础模型基于发现的特征对项目进行分类，将这些判断视为潜在二元特征的噪声观测，通过学习统计关系预测实值目标。
...
Conclusion: GenZ成功地将基础模型的语义理解与统计建模相结合，发现了与模型领域知识不同的数据集特定模式（如预测本地房地产市场的建筑细节、预测用户偏好的系列电影成员关系），为混合建模提供了有效框架。
...
TL;DR: 提出一种基于约束模板的方法，用于从养老机构管理者访谈中提取机构特定的排班约束条件，避免提取异常约束，从而生成满足硬约束并减少软约束违规的护理人员排班表。


<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因机构而异，需要通过与制定排班表的管理者访谈来设计机构特定的约束条件。现有约束提取技术缺乏排除异常约束的机制。

Method: 使用约束模板提取各种组件的组合，如连续工作日的班次模式或员工组合。通过改变关注的天数和员工数量，以及将提取重点调整为模式或频率，模板可以提取多种约束。该方法还包含排除异常约束的机制。

Result: 实验表明，该方法成功创建了满足所有硬约束的排班表，并通过避免提取异常约束，减少了软约束的违规数量。

Conclusion: 提出的约束模板方法能够有效提取养老机构特定的排班约束条件，结合约束编程求解器可以生成高质量的护理人员排班表，特别是在处理机构特定需求和排除异常约束方面表现出色。
...
Method: 提出了ALE基础设施，包含三个组件：ROLL（权重优化后训练框架）、ROCK（轨迹生成沙盒环境管理器）和iFlow CLI（高效上下文工程智能体框架）。开发了ROME开源智能体，基于超过100万条轨迹训练，采用数据组合协议合成复杂行为，并提出基于交互的策略对齐（IPA）算法，在语义交互块而非单个token上分配信用。

Result: 在结构化设置中评估ROME，并引入Terminal Bench Pro基准测试（具有改进的规模和污染控制）。ROME在SWE-bench Verified和Terminal Bench等基准测试中表现出色，证明了ALE基础设施的有效性。

Conclusion: ALE提供了一个优化的智能体LLM生产管道，ROME智能体的强大性能验证了该基础设施的有效性，为开源智能体开发提供了端到端的生态系统。
...
Method: 采用人机协同方法，结合AI与人类专业知识，包括自动生成初始标注、迭代模型重训练、数据匿名化和领域适应技术，核心基于3D物体检测算法。

Result: 开发出的工具和方法实现了显著的时间节省，同时确保跨不同传感器模态的一致高质量标注。
...
TL;DR: 迭代部署大型语言模型，每轮使用用户从前一轮模型输出中精心筛选的数据进行微调，能显著改变模型特性，在规划任务中观察到性能提升和泛化能力涌现。


<details>
  <summary>Details</summary>
Motivation: 探索迭代部署机制如何影响LLM特性，特别是用户数据筛选在模型演化中的作用，以及这种机制与强化学习的潜在联系及其对AI安全的影响。

Method: 采用迭代部署框架：每轮部署LLM，用户从模型输出中筛选高质量数据，用于下一轮模型的微调。在多种规划领域测试该机制，并进行理论分析。

Result: 在规划任务中观察到显著改进，后期模型展现出涌现的泛化能力，能发现比初始模型长得多的规划方案。理论分析表明迭代部署实现了隐式强化学习训练。

Conclusion: 迭代部署机制通过用户数据筛选实现了隐式强化学习，这对AI安全有重要启示（隐式奖励函数可能带来意外影响），也为模型训练提供了基于数据筛选而非显式奖励的替代方案。
...
TL;DR: STAgent是一个专为时空理解设计的智能大语言模型，能够处理约束性兴趣点发现和行程规划等复杂任务，通过工具交互和分层训练方法在保持通用能力的同时提升专业性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个专门针对时空理解场景的智能体模型，解决传统大语言模型在处理复杂时空任务（如约束性兴趣点发现、行程规划）时的局限性，同时保持模型的通用能力。

Method: 1. 构建包含10个领域特定工具的稳定工具环境，支持异步训练；2. 分层数据筛选框架，以1:10,000的比例筛选高质量查询数据；3. 级联训练方法：种子SFT阶段作为"守护者"评估查询难度，第二SFT阶段针对高确定性查询微调，最终RL阶段利用低确定性数据。

Result: STAgent在TravelBench基准测试中表现出色，同时在广泛的通用基准测试中保持了良好的通用能力，验证了所提出的智能体模型的有效性。

Conclusion: STAgent通过专门的工具环境、高质量数据筛选和级联训练方法，成功构建了一个在时空理解任务上表现优异且保持通用能力的智能体模型，为复杂时空场景的AI应用提供了有效解决方案。
...
### [152] [Simultaneous Extrinsic Contact and In-Hand Pose Estimation via Distributed Tactile Sensing](https://arxiv.org/abs/2512.23856)
...
<details>
  <summary>Details</summary>
Motivation: 灵巧自主操作（如插孔、工具使用或装配）需要精确的手内物体姿态和外部接触理解。仅使用触觉传感器存在局部感知导致姿态和接触估计不明确的问题，而视觉反馈又可能受到噪声和遮挡影响。

Method: 提出一种将局部传感观测与物理接触约束相结合的方法，设计了一组因子来确保与触觉观测的局部一致性，并强制执行物理合理性（估计的姿态和接触必须满足准静态刚体相互作用的运动学和力约束），将问题形式化为因子图以实现高效估计。

Result: 实验表明，该方法优于现有的几何和接触感知估计流程，特别是在仅有触觉信息可用时表现更佳。

Conclusion: 通过将局部传感观测与物理约束相结合，提出了一种有效的因子图方法来解决灵巧操作中的姿态和接触估计问题，在仅有触觉信息时仍能提供准确估计。
...
### [153] [SHIELD: Spherical-Projection Hybrid-Frontier Integration for Efficient LiDAR-based Drone Exploration](https://arxiv.org/abs/2512.23972)
*Liangtao Feng,Zhenchang Liu,Feng Zhang,Xuefeng Ren*

Main category: cs.RO

TL;DR: SHIELD是一种基于激光雷达的无人机自主探索方法，通过球形投影混合前沿集成技术解决激光雷达点云质量差、计算负担重和开放区域安全探索等问题。


<details>
  <summary>Details</summary>
Motivation: 激光雷达在无人机探索中面临三个主要挑战：1）点云观测质量通常低于深度相机；2）传统前沿方法在处理激光雷达大视场时计算负担重；3）无点云区域难以通过射线投射分类为自由空间。

Method: SHIELD采用三种核心技术：1）维护观测质量占据地图并进行射线投射，解决探索中点云质量不一致问题；2）使用混合前沿方法降低计算负担并克服点云质量限制；3）提出向外球形投影射线投射策略，确保开放区域飞行安全和探索效率。

Result: 通过仿真和飞行实验验证了SHIELD的有效性，该方法能够高效、安全地进行基于激光雷达的无人机自主探索。

Conclusion: SHIELD成功解决了激光雷达在无人机探索中的关键挑战，通过创新的球形投影混合前沿集成技术实现了高效、安全的自主探索，并将开源贡献给研究社区。
...
### [154] [Unified Embodied VLM Reasoning with Robotic Action via Autoregressive Discretized Pre-training](https://arxiv.org/abs/2512.24125)
...
TL;DR: 提出ERIQ基准评估具身推理能力，开发FACT动作分词器将连续控制离散化，构建GenieReasoner统一优化推理与动作，解决VLA模型中泛化与精确执行的权衡问题


<details>
  <summary>Details</summary>
Motivation: 通用机器人系统需要同时具备广泛泛化能力和高精度动作执行能力，但现有VLA模型难以平衡这两者。视觉语言模型虽然提升了语义泛化，但缺乏具身推理导致行为脆弱；而强推理能力若没有精确控制也不够。需要解决推理与执行精度之间的权衡问题。

Method: 1) 提出ERIQ基准，包含6K+问答对，涵盖四个推理维度，解耦推理与执行以系统评估具身推理能力；2) 开发FACT动作分词器，基于流匹配将连续控制转换为离散序列，保持高保真轨迹重建；3) 构建GenieReasoner，在统一空间中联合优化推理和动作。

Result: ERIQ揭示了具身推理能力与端到端VLA泛化之间的强正相关性。GenieReasoner在真实世界任务中优于连续动作和先前离散动作基线，表明该方法能有效弥合推理到精确执行的差距。

Conclusion: ERIQ和FACT提供了一个原则性框架来诊断和克服推理-精度权衡问题，推进了鲁棒、通用机器人操作的发展。通过解耦评估和统一优化，实现了更好的泛化与精确执行平衡。
...
### [155] [ROBOPOL: Social Robotics Meets Vehicular Communications for Cooperative Automated Driving](https://arxiv.org/abs/2512.24129)
...
TL;DR: 提出将社交机器人作为自动驾驶车辆与弱势道路使用者之间的协调者，通过四个关键使能技术实现混合交通环境下的安全交互。


<details>
  <summary>Details</summary>
Motivation: 在实现完全自动驾驶的过程中，自动驾驶车辆与人类参与者（包括行人）共享道路是不可避免的。即使所有车辆都实现自动化，行人仍需过马路，因此需要解决自动驾驶车辆与弱势道路使用者之间的安全交互问题。

Method: 提出四个关键使能技术的集成：1) 高级感知技术，让机器人能够感知环境；2) 车辆通信技术，让联网车辆共享意图，机器人发送引导指令；3) 社交人机交互技术，让机器人能有效与弱势道路使用者和驾驶员沟通；4) 形式化规范，让机器人理解交通规则并相应规划。论文展示了前三个使能技术的初步概念验证集成。

Result: 提出了一个概念框架，并在与协作式自动电动自行车场景中，实现了前三个使能技术（高级感知、车辆通信、社交人机交互）的初步集成，展示了社交机器人指导行人的可行性。

Conclusion: 社交机器人可以作为自动驾驶车辆与弱势道路使用者之间的有效协调者，通过集成四个关键使能技术，能够提升混合交通环境下的安全性和交互效率，为未来智能交通系统提供了新的解决方案。
...
### [156] [GR-Dexter Technical Report](https://arxiv.org/abs/2512.24210)
...
TL;DR: GR-Dexter是一个用于双手机器人灵巧手操作的硬件-模型-数据一体化框架，通过紧凑的21自由度机械手设计、直观的双手机器人遥操作系统和结合遥操作轨迹与大规模视觉语言数据的训练方法，实现了长时程日常操作和通用拾放任务。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型主要局限于夹爪机器人，难以扩展到具有高自由度的双手机器人灵巧手操作，主要挑战包括动作空间扩大、手-物体遮挡问题以及真实机器人数据收集成本高。

Method: 1) 设计紧凑的21自由度机器人灵巧手；2) 开发直观的双手机器人遥操作系统用于真实机器人数据收集；3) 提出训练方法，结合遥操作机器人轨迹、大规模视觉语言数据和精心策划的跨体现数据集。

Result: 在真实世界评估中，GR-Dexter在长时程日常操作和通用拾放任务上表现出强大的领域内性能，对未见过的物体和指令具有更好的鲁棒性。
...
### [157] [RANGER: A Monocular Zero-Shot Semantic Navigation Framework through Contextual Adaptation](https://arxiv.org/abs/2512.24212)
*Ming-Ming Yu,Yi Chen,Börje F. Karlsson,Wenjun Wu*

Main category: cs.RO

TL;DR: RANGER是一个零样本、开放词汇的语义导航框架，仅使用单目摄像头，无需深度和姿态信息，通过3D基础模型和上下文学习能力实现高效目标搜索


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标导航方法存在两个关键限制：1) 严重依赖模拟器提供的精确深度和姿态信息，限制了在真实世界的应用；2) 缺乏上下文学习能力，难以快速适应新环境

Method: 提出RANGER框架，包含关键帧3D重建、语义点云生成、视觉语言模型驱动的探索价值估计、高层自适应路径点选择和低层动作执行等组件，仅使用单目摄像头

Result: 在HM3D基准测试和真实环境中，RANGER在导航成功率和探索效率方面达到竞争性性能，同时展现出优越的上下文学习适应性，无需环境先验3D地图

Conclusion: RANGER通过消除对深度和姿态的依赖，并引入上下文学习能力，为真实世界机器人导航提供了一种更实用、适应性更强的零样本解决方案
...
### [158] [Heteroscedastic Bayesian Optimization-Based Dynamic PID Tuning for Accurate and Robust UAV Trajectory Tracking](https://arxiv.org/abs/2512.24249)
*Fuqiang Gu,Jiangshan Ai,Xu Lu,Xianlei Long,Yan Li,Tao Jiang,Chao Chen,Huidong Liu*

Main category: cs.RO

TL;DR: HBO-PID：一种结合异方差贝叶斯优化与经典PID控制的新型无人机轨迹跟踪算法，显著提升了位置和角度跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 无人机在多种应用中需要精确的轨迹跟踪，但由于四旋翼系统具有欠驱动、非线性和高度耦合的动态特性，传统控制算法性能有限。需要一种能够适应动态复杂环境、提高跟踪精度和鲁棒性的控制方法。

Method: 提出HBO-PID控制算法，将异方差贝叶斯优化框架与经典PID控制器相结合。通过显式建模输入相关的噪声方差，使算法能更好地适应动态复杂环境。采用两阶段优化策略加速收敛，更高效地找到最优控制器参数。

Result: 在仿真和真实场景实验中，该方法显著优于现有最先进方法。相比SOTA方法，位置精度提升24.7%至42.9%，角度精度提升40.9%至78.4%。

Conclusion: HBO-PID算法通过结合异方差贝叶斯优化与PID控制，有效解决了无人机轨迹跟踪中的精度和鲁棒性问题，在动态复杂环境中表现出优越性能。
...
### [159] [Real-world Reinforcement Learning from Suboptimal Interventions](https://arxiv.org/abs/2512.24288)
...
TL;DR: SiLRI是一种状态级拉格朗日强化学习算法，通过将在线机器人操作问题建模为约束优化问题，有效利用可能次优的人类干预来加速学习，而不受其限制。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界RL方法通常假设人类干预在整个状态空间都是最优的，但实际上专家操作者无法在所有状态下都提供最优动作或完全避免错误。盲目混合干预数据与机器人收集数据会继承RL的样本效率低下问题，而单纯模仿干预数据又会限制最终性能。如何利用可能次优和嘈杂的人类干预来加速学习而不受其约束仍然是一个开放问题。

Method: 提出SiLRI（状态级拉格朗日强化学习算法），将在线操作问题建模为约束RL优化问题，其中每个状态的约束边界由人类干预的不确定性决定。引入状态级拉格朗日乘子，通过最小-最大优化联合优化策略和拉格朗日乘子以达到鞍点。基于人机协同遥操作系统实现。

Result: 在真实世界多样化操作任务上的实验表明，SiLRI有效利用了人类次优干预，相比最先进的RL方法HIL-SERL，达到90%成功率所需时间至少减少50%，并在长时程操作任务上实现100%成功率，而其他RL方法难以成功。

Conclusion: SiLRI通过状态级约束优化框架，成功解决了如何有效利用可能次优的人类干预来加速真实世界机器人操作学习的问题，在减少训练时间的同时提高了最终性能。
...
### [160] [World In Your Hands: A Large-Scale and Open-source Ecosystem for Learning Human-centric Manipulation in the Wild](https://arxiv.org/abs/2512.24310)
...
TL;DR: WiYH是一个大规模开源生态系统，用于人类中心化操作学习，包含数据收集套件、多模态数据集和基准测试，显著提升灵巧手策略的泛化能力
...
Method: 提出World In Your Hands (WiYH)生态系统，包含：(1) Oracle Suite可穿戴数据收集套件和自动标注流水线；(2) WiYH数据集，包含1000+小时多模态操作数据，涵盖数百种技能和多样化场景；(3) 丰富的标注和基准测试支持从感知到动作的各种任务

Result: 基于WiYH生态系统的实验表明，整合WiYH的人类中心化数据显著提升了灵巧手策略在桌面操作任务中的泛化能力和鲁棒性

Conclusion: WiYH将为社区带来人类中心化数据收集和策略学习的新见解，推动灵巧手操作研究的发展
...
### [161] [3D Path-Following Guidance via Nonlinear Model Predictive Control for Fixed-Wing Small UAS](https://arxiv.org/abs/2512.24326)
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在高速飞行中精确跟踪复杂3D路径的制导算法，特别是针对固定翼小型无人机系统，解决传统方法在高速高曲率路径跟踪中的性能限制。

Method: 首先对RAAVEN小型无人机进行控制增强建模和系统辨识，然后设计两种MPC算法：第一种采用静态参考路径速率调度，激励恒定惯性速度；第二种受模型预测轮廓控制启发，在控制器时域内动态优化参考路径速率，权衡路径进展和路径距离两个竞争目标。两种控制器都能处理一般光滑的3D弧长参数化曲线。

Result: 在多个高曲率测试路径上进行飞行测试，与基线前瞻制导律比较。结果表明，非线性MPC在高达36米/秒的地速下，对3D路径跟踪制导具有实际可行性和优越性能。

Conclusion: 非线性MPC方法能够有效解决固定翼小型无人机在高速飞行中的复杂3D路径跟踪问题，两种MPC算法都表现出良好的性能，特别是动态优化路径速率的方法能够在路径进展和路径距离之间实现更好的权衡。
...
### [162] [Geometric Multi-Session Map Merging with Learned Local Descriptors](https://arxiv.org/abs/2512.24384)
...
<details>
  <summary>Details</summary>
Motivation: 大规模环境中多会话地图合并对于扩展自主操作至关重要，需要系统地对齐不同会话收集的地图重叠区域。

Method: 使用关键点感知编码器和平面几何变换器提取判别性特征用于回环检测和相对位姿估计，在因子图优化阶段加入会话间扫描匹配代价因子。

Result: 在公共数据集和自收集数据上评估，结果显示准确鲁棒的地图合并，学习到的特征在回环检测和相对位姿估计中表现优异。

Conclusion: GMLD框架能够有效实现大规模多会话点云地图合并，学习到的特征具有强判别性，系统整体性能优越。
...
### [163] [Fast and Realistic Automated Scenario Simulations and Reporting for an Autonomous Racing Stack](https://arxiv.org/abs/2512.24402)
*Giovanni Lambertini,Matteo Pini,Eugenio Mascaro,Francesco Moretti,Ayoub Raji,Marko Bertogna*

Main category: cs.RO

TL;DR: 本文介绍了为自动驾驶赛车软件栈ur.autopilot开发的自动化仿真与报告流水线，该流水线基于高精度车辆模型，支持本地或GitHub CI/CD环境下的超实时仿真，包含场景初始化、故障注入和自动化报告功能。


<details>
  <summary>Details</summary>
Motivation: 开发一个高效的自动化仿真和报告系统，以验证自动驾驶赛车软件栈的关键模块，特别是高速超车和定位等最具挑战性的功能，同时支持持续集成和持续交付流程。

Method: 基于高精度车辆模型构建仿真系统，通过Functional Mockup Unit接口实现；设计运行场景集用于初始化车辆状态和软件配置；实现故障注入模块模拟传感器延迟和扰动；开发自动化报告流程优化仿真分析效果。

Result: 实现了能够在本地或GitHub CI/CD环境下以最高三倍于实时速度运行的仿真流水线，支持多种初始条件和配置的场景测试，具备故障注入能力，并建立了有效的自动化报告机制。

Conclusion: 该自动化仿真和报告流水线为自动驾驶赛车软件栈提供了高效验证平台，特别针对高速超车和定位等关键挑战，通过故障注入和自动化报告增强了系统的鲁棒性和分析效率。
...
### [164] [Subsecond 3D Mesh Generation for Robot Manipulation](https://arxiv.org/abs/2512.24428)
...
<details>
  <summary>Details</summary>
Motivation: 3D网格在机器人学中至关重要，但现有自动生成方法存在两个关键问题：1）生成高保真网格速度太慢（数十秒），无法实时使用；2）网格本身不足以支持机器人应用，需要正确的场景分割、尺度配准和姿态估计等上下文接地处理。

Method: 开发了一个端到端流水线，集成了三个优化组件：1）开放词汇对象分割；2）加速的基于扩散的网格生成；3）鲁棒的点云配准。每个组件都针对速度和精度进行了优化。

Result: 系统能够在1秒内从单张RGB-D图像生成高质量、上下文接地的3D网格，并在真实世界机器人操作任务中验证了其有效性。

Conclusion: 该系统使3D网格能够作为机器人感知和规划的实用、按需表示，解决了现有方法的速度和上下文接地瓶颈，为实时机器人应用提供了可行的解决方案。
...
### [165] [Foundation models on the bridge: Semantic hazard detection and safety maneuvers for maritime autonomy with vision-language models](https://arxiv.org/abs/2512.24470)
...
TL;DR: 提出Semantic Lookout系统，使用视觉语言模型为自主船舶提供语义感知，在异常情况下选择安全回退机动，满足IMO MASS Code要求


<details>
  <summary>Details</summary>
Motivation: IMO MASS Code草案要求自主和远程监督船舶能够检测操作设计域偏离并进入预定义回退状态，但传统自主系统难以处理依赖语义的异常情况（如潜水旗、火灾等）

Method: 提出Semantic Lookout系统：基于视觉语言模型的快速-慢速异常检测管道，从水域有效、世界锚定的轨迹中选择谨慎行动或保持位置，保持持续人类权限

Result: 在40个港口场景测试中，10秒内模型保持与较慢SOTA模型相当的感知能力；回退选择器优于纯几何基线，在火灾场景增加安全距离；现场测试验证端到端运行

Conclusion: 视觉语言模型可作为语义回退机动选择器，符合IMO MASS Code草案要求，在实用延迟预算内工作，为结合基础模型语义与多传感器鸟瞰感知的未来研究提供方向
...
### [166] [DISF: Disentangled Iterative Surface Fitting for Contact-stable Grasp Planning with Grasp Pose Alignment to the Object Center of Mass](https://arxiv.org/abs/2512.24550)
*Tomoya Yamanokuchi,Alberto Bacchin,Emilio Olivastri,Ryotaro Arifuku,Takamitsu Matsubara,Emanuele Menegatti*

Main category: cs.RO

TL;DR: 提出了一种新的表面拟合算法DISF，在保持几何兼容性的同时整合接触稳定性，通过三步优化（旋转、平移、夹爪开度调整）来提升抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 传统的基于表面拟合的抓取规划算法主要关注夹爪与物体表面的几何对齐，但忽略了接触点分布的稳定性，导致抓取不稳定。需要一种既能保持几何兼容性又能整合接触稳定性的新方法。

Method: 受人类抓取行为启发，将抓取姿态优化分解为三个顺序步骤：1) 旋转优化以对齐接触法线；2) 平移细化以改善夹爪坐标系原点与物体质心的对齐；3) 夹爪开度调整以优化接触点分布。

Result: 在仿真中验证了15个物体（包括已知形状和观测形状设置），并在三个机器人-夹爪平台上进行跨平台抓取执行。真实世界实验中在UR3e机器人上验证。DISF减少了质心错位，同时保持了几何兼容性，在仿真和真实世界执行中都获得了比基线更高的抓取成功率。

Conclusion: DISF算法通过整合接触稳定性到表面拟合过程中，有效提升了抓取的成功率和稳定性，为机器人抓取规划提供了更可靠的解决方案。
...
### [167] [Resolving State Ambiguity in Robot Manipulation via Adaptive Working Memory Recoding](https://arxiv.org/abs/2512.24638)
*Qingda Hu,Ziheng Qiu,Zijun Xu,Kaizhao Zhang,Xizhou Bu,Zuolei Sun,Bo Zhang,Jieru Zhao,Zhongxue Gan,Wenchao Ding*

Main category: cs.RO

TL;DR: PAM是一种配备自适应工作记忆的视觉运动策略，通过两阶段训练支持300帧历史窗口，在保持高推理速度的同时处理机器人操作中的状态模糊问题


<details>
  <summary>Details</summary>
Motivation: 机器人操作中常见状态模糊问题，相同观测可能对应多个有效行为轨迹。视觉运动策略需要从历史中正确提取适当类型和水平的信息来识别当前任务阶段，但简单扩展历史窗口计算成本高且容易过拟合
...
Result: PAM能够同时处理多种状态模糊场景，支持约10秒（300帧）的历史窗口，同时保持稳定训练和高于20Hz的推理速度
...
### [168] [Hybrid Motion Planning with Deep Reinforcement Learning for Mobile Robot Navigation](https://arxiv.org/abs/2512.24651)
*Yury Kolomeytsev,Dmitry Golembiovsky*

Main category: cs.RO

TL;DR: 提出HMP-DRL混合框架，结合图规划器的全局路径引导与DRL的局部避障能力，通过语义感知奖励机制实现复杂动态环境中的安全导航。


<details>
  <summary>Details</summary>
Motivation: 传统图规划器擅长长距离路径规划但缺乏反应性，而DRL方法在避障方面表现良好但缺乏全局上下文导致难以到达远距离目标。需要结合两者优势解决复杂动态环境中的机器人导航问题。

Method: 提出HMP-DRL混合框架：1) 使用图规划器生成全局路径；2) 将路径转换为一系列检查点，编码到DRL的状态空间和奖励函数中；3) 采用实体感知奖励结构，根据周围智能体的语义类型动态调整安全边界和惩罚。

Result: 在基于真实地图数据的仿真环境中进行广泛测试，HMP-DRL在机器人导航关键指标（成功率、碰撞率、到达目标时间）上持续优于其他方法，包括最先进方法。

Conclusion: 将长期路径引导与语义感知局部控制相结合，显著提高了复杂人本环境中自主导航的安全性和可靠性。
...
### [169] [RoboMIND 2.0: A Multimodal, Bimanual Mobile Manipulation Dataset for Generalizable Embodied Intelligence](https://arxiv.org/abs/2512.24653)
...
TL;DR: RoboMIND 2.0是一个包含31万条双机械臂操作轨迹的真实世界数据集，涵盖6种机器人平台和739个复杂任务，包含触觉增强和移动操作数据，并配有数字孪生仿真数据集。MIND-2系统采用分层双系统框架，通过离线强化学习优化，实现自然语言指令到精确动作的执行。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动的模仿学习方法受限于大规模、多样化真实世界演示数据的稀缺性，导致现有模型在长时程双机械臂任务和非结构化环境移动操作方面的泛化能力有限。

Method: 提出了RoboMIND 2.0数据集（31万条双机械臂轨迹，6种机器人平台，739个任务，包含1.2万条触觉增强轨迹和2万条移动操作轨迹）及其数字孪生仿真数据集。开发了MIND-2分层双系统框架：高层语义规划器（MIND-2-VLM）将自然语言指令分解为具体子目标，低层视觉-语言-动作执行器（MIND-2-VLA）生成精确的本体感知运动动作，通过离线强化学习优化。

Result: 构建了迄今为止最全面的真实世界机器人操作数据集之一，包含触觉感知和移动操作数据，并提供了相应的仿真环境以支持sim-to-real迁移研究。

Conclusion: RoboMIND 2.0数据集和MIND-2系统为解决机器人操作中的数据稀缺问题提供了重要资源和方法框架，有望推动长时程双机械臂任务和非结构化环境移动操作的泛化能力研究。
...
### [170] [Antagonistic Bowden-Cable Actuation of a Lightweight Robotic Hand: Toward Dexterous Manipulation for Payload Constrained Humanoids](https://arxiv.org/abs/2512.24657)
*Sungjae Min,Hyungjoo Kim,David Hyunchul Shim*

Main category: cs.RO

TL;DR: 提出了一种轻量级仿人机械手，采用鲍登线驱动和滚动接触关节优化，实现单电机控制，远端质量仅236克，能产生超过18N的指尖力并举起百倍自重载荷


<details>
  <summary>Details</summary>
Motivation: 仿人机器人需要具备高抓握力、快速驱动速度、多自由度、轻量化且尺寸接近人手等特性的机械手，但这些要求相互冲突，通常需要更重的执行器和更庞大的传动系统，限制了机器人手臂的有效载荷能力

Method: 采用鲍登线驱动系统，结合滚动接触关节优化和拮抗式线缆驱动，实现单电机控制且线缆长度变化可忽略；将执行器模块移至躯干，显著减少远端质量；拮抗式驱动消除了电机间的同步需求

Result: 机械手远端质量仅236克（不包括远程执行器和鲍登护套），能可靠执行灵巧任务，指尖力超过18N，能举起超过自身质量一百倍的载荷；通过Cutkosky分类抓握和扰动下的轨迹一致性验证了鲁棒性

Conclusion: 该设计成功解决了仿人机械手在轻量化、高力输出和灵巧性方面的冲突要求，为仿人机器人实现人类水平灵巧性提供了有前景的解决方案
...
### [171] [VLA-RAIL: A Real-Time Asynchronous Inference Linker for VLA Models and Robots](https://arxiv.org/abs/2512.24673)
...
Method: 提出VLA-RAIL框架，核心包含两个组件：1) 轨迹平滑器，使用多项式拟合有效过滤单个动作块轨迹中的噪声和抖动；2) 块融合器，无缝对齐当前执行轨迹和新到达的动作块，确保连续动作块之间的位置、速度和加速度连续性。

Result: 在动态仿真基准测试和多个真实世界操作任务上的实验结果表明，VLA-RAIL显著减少了运动抖动，提高了执行速度，并改善了任务成功率。

Conclusion: VLA-RAIL将成为VLA模型大规模部署的关键基础设施，通过异步推理和运动控制确保平滑、连续、高速的动作执行。
...
### [172] [ReSPIRe: Informative and Reusable Belief Tree Search for Robot Probabilistic Search and Tracking in Unknown Environments](https://arxiv.org/abs/2512.24680)
*Kangjie Zhou,Zhaoyang Li,Han Gao,Yao Su,Hangxin Liu,Junzhi Yu,Chang Liu*

Main category: cs.RO

TL;DR: ReSPIRe是一种用于未知杂乱环境中目标搜索与跟踪的信息化轨迹规划方法，通过sigma点近似估计互信息奖励，采用分层粒子结构处理目标先验信息不确定性，并利用可重用信念树搜索进行在线规划。


<details>
  <summary>Details</summary>
Motivation: 目标搜索与跟踪是机器人应用中的基本问题，但在未知杂乱环境中，由于目标先验信息不准确和传感器视野有限，现有方法面临挑战。需要开发能够高效处理非高斯信念分布和显著不确定性的规划方法。

Method: 1. 提出sigma点近似方法，在状态和观测空间进行信息采样，快速准确估计非高斯信念分布下的互信息奖励；2. 设计分层粒子结构，提取关键粒子进行全局路径引导，并自适应调整粒子数量以提高规划效率；3. 开发可重用信念树搜索方法，重用rollout评估构建在线轨迹规划的策略树。

Result: 大量仿真和真实世界实验表明，ReSPIRe在互信息近似误差、搜索效率和跟踪稳定性方面优于代表性基准方法，同时保持了出色的计算效率。

Conclusion: ReSPIRe通过创新的sigma点近似、分层粒子结构和可重用信念树搜索，为未知杂乱环境中的目标搜索与跟踪问题提供了高效、准确的解决方案，能够处理显著的目标先验信息不确定性和有限的传感器视野约束。
...
### [173] [CREPES-X: Hierarchical Bearing-Distance-Inertial Direct Cooperative Relative Pose Estimation System](https://arxiv.org/abs/2512.24688)
*Zhehan Li,Zheng Wang,Jiadong Lu,Qi Liu,Zhiren Xun,Yue Wang,Fei Gao,Chao Xu,Yanjun Cao*

Main category: cs.RO

TL;DR: CREPES-X是一个用于多机器人相对定位的层次化框架，通过紧凑硬件设计和两阶段估计算法，在复杂环境中实现快速、准确、鲁棒的相对位姿估计，无需全局信息。


<details>
  <summary>Details</summary>
Motivation: 现有多机器人相对定位方法依赖共享环境特征或惯性假设，在复杂环境中易受非视距干扰和异常值影响，难以在数十个机器人系统中鲁棒高效地融合多种测量数据。

Method: 采用紧凑硬件设计（红外LED、红外相机、超宽带模块、IMU），实现两阶段层次化估计器：单帧相对估计器提供即时相对位姿（闭式解+异常值剔除），多帧相对估计器通过机器人中心相对运动学和松紧耦合优化实现准确鲁棒的相对状态估计。

Result: 仿真和真实实验验证了CREPES-X的有效性，能够抵抗高达90%的方位异常值，在挑战性条件下保持鲁棒性，真实数据集上达到0.073米和1.817°的RMSE精度。

Conclusion: CREPES-X框架通过创新的硬件设计和层次化估计算法，解决了多机器人系统中相对定位的鲁棒性、准确性和实时性问题，为复杂环境下的自主多机器人协作提供了可靠解决方案。
...
### [174] [Dynamic Policy Learning for Legged Robot with Simplified Model Pretraining and Model Homotopy Transfer](https://arxiv.org/abs/2512.24698)
...
<details>
  <summary>Details</summary>
Motivation: 为四足机器人生成动态运动仍具挑战性。强化学习虽在多种运动任务中取得进展，但产生高度动态行为通常需要大量奖励调优或高质量演示。利用简化模型可缓解这些问题，但模型差异在向全身动力学环境转移策略时带来显著挑战。

Method: 1. 使用单刚体模型预训练策略，在简化环境中捕获核心运动模式；2. 采用延续策略逐步将策略转移到全身环境，最小化性能损失；3. 通过逐渐重新分配躯干和腿部之间的质量和惯性，定义从单刚体模型到全身模型的模型同伦路径。

Result: 该方法不仅实现了更快的收敛速度，在转移过程中相比基线方法表现出更优的稳定性。框架在多种动态任务（包括翻转和墙壁辅助机动）上得到验证，并成功部署在真实四足机器人上。

Conclusion: 提出的延续学习框架通过简化模型预训练和模型同伦转移，有效解决了动态运动生成中的模型差异问题，实现了从简化模型到复杂全身环境的平稳策略转移，为四足机器人复杂动态行为的生成提供了高效解决方案。
...
### [175] [LSRE: Latent Semantic Rule Encoding for Real-Time Semantic Risk Detection in Autonomous Driving](https://arxiv.org/abs/2512.24712)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界自动驾驶需要遵守复杂的人类社会规则，这些语义约束难以显式编码。虽然大型视觉语言模型能解释这些语义，但其推理成本过高，无法实时部署。
...
Conclusion: 语言引导的潜在分类为自动驾驶语义安全监控提供了有效且可部署的机制，实现了实时语义风险评估而无需逐帧VLM查询。
...
### [176] [Control of Microrobots with Reinforcement Learning under On-Device Compute Constraints](https://arxiv.org/abs/2512.24740)
*Yichen Liu,Kesava Viswanadha,Zhongyu Li,Nelson Lojo,Kristofer S. J. Pister*

Main category: cs.RO

TL;DR: 该论文探索了在微机器人上部署边缘机器学习控制器的方法，通过强化学习训练紧凑的MLP策略，并在资源受限的ARM Cortex-M0微控制器上实现量化部署，提出了基于功率预算的步态调度策略。


<details>
  <summary>Details</summary>
Motivation: 微机器人需要在计算、内存和功率受限的条件下实现稳健的地形移动，传统方法难以满足实时控制需求，因此需要探索边缘ML方法来实现低延迟的本地控制。

Method: 1) 使用大规模并行GPU仿真训练紧凑的FP32多层感知机策略；2) 通过域随机化增强鲁棒性；3) 研究整数量化（Int8）以在资源受限硬件上实现更高推理频率；4) 提出基于功率预算的步态调度方法，根据设备功率预算选择最大化RL奖励的步态模式。

Result: 成功在ARM Cortex-M0微控制器（5MHz）上部署了量化MLP策略，建立了功率预算与更新频率的关联模型，并在真实世界大型机器人上进行了部署测试，观察到域随机化训练能改善分布外稳定性。

Conclusion: 边缘ML方法能够在资源受限的微机器人硬件上实现有效的运动控制，通过量化技术和功率感知的步态调度，可以在有限的计算和功率预算下优化机器人性能，为微机器人自主运动控制提供了实用解决方案。
...
### [177] [Dream2Flow: Bridging Video Generation and Open-World Manipulation with 3D Object Flow](https://arxiv.org/abs/2512.24766)
...
<details>
  <summary>Details</summary>
Motivation: 生成视频模型在零样本推理物理交互方面表现出色，但难以直接转化为机器人所需的低级动作。需要一种方法将人类引导的运动转化为机器人可执行的控制指令

Method: 提出Dream2Flow框架：1) 从初始图像和任务指令生成视频；2) 从生成视频中重建3D物体运动；3) 将操作任务转化为物体轨迹跟踪问题；4) 通过轨迹优化或强化学习将3D物体流转化为可执行的低级命令

Result: Dream2Flow能够处理多种物体类别（刚性、铰接、可变形、颗粒状），无需任务特定演示即可实现零样本指导。仿真和真实世界实验验证了3D物体流作为通用接口的有效性

Conclusion: 3D物体流是连接视频生成模型与机器人操作的通用且可扩展的接口，通过分离状态变化与执行器实现，克服了体现差距，为开放世界机器人操作提供了新途径
...
### [178] [ArtiSG: Functional 3D Scene Graph Construction via Human-demonstrated Articulated Objects Manipulation](https://arxiv.org/abs/2512.24845)
...
Method: 利用便携式设置收集关节数据，准确估计6自由度关节轨迹和轴；将运动学先验集成到分层开放词汇图中；利用交互数据发现视觉感知遗漏的不显眼功能元素。

Result: 在真实世界实验中，ArtiSG在功能元素召回率和关节估计精度方面显著优于基线方法；构建的图作为可靠的功能记忆，能有效指导机器人执行语言指令的操纵任务。

Conclusion: ArtiSG通过编码人类演示到结构化机器人记忆中构建功能性3D场景图，成功解决了关节物体功能理解和精细功能元素检测的挑战，为机器人语言指令操纵提供了可靠的功能记忆。
...
### [179] [Hierarchical Deformation Planning and Neural Tracking for DLOs in Constrained Environments](https://arxiv.org/abs/2512.24974)
*Yunxi Tang,Tianqi Yang,Jing Huang,Xiangyu Chu,Kwok Wai Samuel Au*

Main category: cs.RO

TL;DR: 提出了一种结合分层变形规划和神经跟踪的框架，用于在受限环境中操纵可变形线性物体，解决高维状态空间和复杂变形动力学的挑战。


<details>
  <summary>Details</summary>
Motivation: 可变形线性物体（DLOs）的操纵面临重大挑战，包括高维状态空间、复杂变形动力学以及现实工作空间中广泛存在的障碍物，这需要高效的变形规划和鲁棒的变形跟踪。

Method: 提出了一个结合分层变形规划和神经跟踪的新框架：1）变形规划器首先生成满足DLO关键点路径同伦约束的空间路径集；2）采用路径集引导的优化方法合成DLO的最优时间变形序列；3）在操纵执行中，设计基于数据驱动变形模型的神经模型预测控制方法，精确跟踪规划的DLO变形序列。

Result: 在广泛的受限DLO操纵任务中验证了所提框架的有效性。

Conclusion: 该框架通过结合分层变形规划和神经跟踪，能够在全局变形合成和局部变形跟踪方面提供可靠性能，有效解决受限环境中DLO操纵的挑战。
...
### [180] [Coordinated Humanoid Manipulation with Choice Policies](https://arxiv.org/abs/2512.25072)
*Haozhi Qi,Yen-Jen Wang,Toru Lin,Brent Yi,Yi Ma,Koushil Sreenath,Jitendra Malik*

Main category: cs.RO

TL;DR: 结合模块化遥操作界面与可扩展学习框架，通过Choice Policy模仿学习方法实现人形机器人全身协调控制，在洗碗机装载和白板擦拭任务中表现优于扩散策略和标准行为克隆。
...
Method: 1. 模块化遥操作设计：将人形控制分解为直观的子模块（手眼协调、抓取基元、手臂末端执行器跟踪、运动控制），实现高效高质量演示数据收集。
2. Choice Policy模仿学习方法：生成多个候选动作并学习对它们进行评分，实现快速推理和有效建模多模态行为。

Result: 1. 在两个真实世界任务中验证：洗碗机装载和全身运动操作（白板擦拭）。
2. Choice Policy显著优于扩散策略和标准行为克隆。
3. 手眼协调被证明对长时域任务的成功至关重要。
